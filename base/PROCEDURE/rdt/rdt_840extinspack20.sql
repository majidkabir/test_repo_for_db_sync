...
/************************************************************************/
/* Stored Procedure: rdt_840ExtInsPack20                                */
/*                                                                      */
...
/* 2023-04-12   James     1.0   WMS-22264. Created                      */
/************************************************************************/

CREATE   PROC [RDT].[rdt_840ExtInsPack20] (
   @nMobile                   INT, 
   @nFunc                     INT, 
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
      	...
      	BEGIN
      		DECLARE @nCurMonth   INT
      		DECLARE @nNcounterMonth INT
      		
      		SELECT @nNcounterMonth = MONTH( EditDate) 
      		FROM NCOUNTER WITH (NOLOCK)
      		WHERE keyname = 'SC840LabelSeqNo'
      		
      		SET @nCurMonth = MONTH( GETDATE())
      		
      		-- Reset running no every month
      		IF @nNcounterMonth <> @nCurMonth
      		   DELETE FROM NCOUNTER WHERE keyname = 'SC840LabelSeqNo'
      		   
            -- Get running no here
            EXECUTE dbo.nspg_GetKey
               'SC840LabelSeqNo',
               5 ,
               @cNewLabelSeqNo    OUTPUT,
               @b_success         OUTPUT,
               @n_err             OUTPUT,
               @c_errmsg          OUTPUT
      		...
      		WHERE StorerKey = @cC_contact2
      		
      		SET @cTrackNo = RTRIM( @cB_contact1) + 
      		                RTRIM( @cB_Company) + 
      		                '0' + 
   ...
   RollBackTran:  
         ROLLBACK TRAN rdt_840ExtInsPack20  
   Quit:  
