...
/* 2024-10-08 1.0  PXL009   FCR-778 Violet Pack Changes                 */
/************************************************************************/

CREATE   PROCEDURE [RDT].[rdt_838DIDchkVLT_02] (
   @nMobile         INT,
   @nFunc           INT,
...
BEGIN
   ...
   IF @nFunc = 838
   BEGIN
      SELECT TOP 1 @cSerialNoCapture = SerialNoCapture FROM dbo.SKU (NOLOCK) WHERE sku = @cSKU
      SELECT TOP 1 @cOrderKey = OrderKey FROM dbo.PICKDETAIL (NOLOCK) WHERE DropID = @cFromDropID

      -- get ConsigneeKey for Client Print Matrix
      SELECT TOP 1 @cOrderConsigneeKey = ConsigneeKey,@cOrderC_Zip=[C_Zip] FROM dbo.ORDERS (NOLOCK) WHERE storerkey = @cStorerKey AND OrderKey = @cOrderKey
      ...
      SELECT @nSkuCheck01 = ISNULL(COUNT(DISTINCT(SKU)),0) FROM dbo.PackDetail (NOLOCK) WHERE CartonNo = @nCartonNo AND DropID = @cPackDtlDropID AND PickSlipNo = @cPickSlipNo
      -- WS_20062024 END
      ...
      BEGIN
         ...
         ELSE IF EXISTS (SELECT 1 FROM dbo.PickDetail (NOLOCK) WHERE Dropid = @cPackDtlDropID AND OrderKey <> @cOrderKey AND StorerKey = @cStorerKey)
         BEGIN
            SET @nErrNo = 223603
         ...
         ELSE IF EXISTS (SELECT 1 FROM dbo.PackDetail (NOLOCK) WHERE Dropid = @cPackDtlDropID AND PickSlipNo <> @cPickSlipNo AND StorerKey = @cStorerKey)
         BEGIN
            SET @nErrNo = 223604
         ...
         ELSE IF (1 IN (SELECT short FROM dbo.CODELKUP (NOLOCK) WHERE LISTNAME = 'HUSQPICCHK' AND storerkey = @cStorerKey)
            AND 0 NOT IN (SELECT short FROM dbo.CODELKUP (NOLOCK) WHERE LISTNAME = 'HUSQPICCHK' AND storerkey = @cStorerKey)) AND
            EXISTS
            (SELECT Loc FROM dbo.PICKDETAIL PD (NOLOCK)
            WHERE orderkey = @cOrderKey
            AND (loc NOT IN
            (SELECT OtherReference FROM dbo.MBOL (NOLOCK)
            WHERE mbolkey =
      ...
      IF @nStep = 2 AND @cOption IN (2,3) AND NOT EXISTS (SELECT 1 FROM dbo.PackDetail (NOLOCK) WHERE CartonNo = @nCartonNo AND DropID = @cPackDtlDropID AND PickSlipNo = @cPickSlipNo)
      BEGIN
         SET @nErrNo = 223609
      ...
      IF @nStep = 3 AND @cLabelCode = 'SINGLE'
         AND @cSKU not in (select distinct(isnull(sku,'')) FROM dbo.PackDetail (NOLOCK) WHERE CartonNo = @nCartonNo AND DropID = @cPackDtlDropID AND PickSlipNo = @cPickSlipNo)
      ...
      END
      -- WS_03072024  END
      IF @nStep = 2 AND @cOption = 1 AND EXISTS (SELECT 1 FROM dbo.PackDetail (NOLOCK) WHERE CartonNo = @nCartonNo AND DropID = @cPackDtlDropID AND PickSlipNo = @cPickSlipNo)
      BEGIN
         SET @nErrNo = 223612
         ...
         BEGIN
            ...
            IF @cCustomerPalletMixBrands = N'N'
               AND EXISTS( SELECT 1
                           ...
                           INNER JOIN [dbo].[SKU] ON [PackDetail].[Sku] = [SKU].[Sku] AND [PackDetail].[StorerKey] = [SKU].[StorerKey]
