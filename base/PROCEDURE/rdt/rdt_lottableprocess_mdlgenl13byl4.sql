...
/**************************************************************************/
/* Store procedure: rdt_LottableProcess_MDLGenL13ByL4                     */
/* Copyright      : LF                                                    */
/*                                                                        */
/* Purpose: Key-in value, generate production date (L13) using            */
/*          expriry date (L4)                                             */
/*                                                                        */
...
CREATE PROCEDURE [RDT].[rdt_LottableProcess_MDLGenL13ByL4]
    @nMobile          INT
   ,@nFunc            INT
...
BEGIN
   ...
   IF DATEDIFF( D, GETDATE(), @dLottable04Value) <= 0
   BEGIN
      SET @nErrNo = 149152
      SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') -- Expiry < Today
      GOTO Quit
   END
