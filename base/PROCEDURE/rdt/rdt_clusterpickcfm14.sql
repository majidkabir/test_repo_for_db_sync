...
GO
/************************************************************************/
/* Store procedure: rdt_ClusterPickCfm14                                */
/* Copyright      : IDS                                                 */
/*                                                                      */
/* Purpose: FJ pick and pack, insert packinfo. DropID = LabelNo         */
/*                                                                      */
...
/* 14-May-2019 1.0  James       WMS-9071. Created                       */
/************************************************************************/

CREATE PROC [RDT].[rdt_ClusterPickCfm14] (
   @nMobile                   INT,
   @nFunc                     INT,
...
BEGIN
   ...
   BEGIN TRAN
   SAVE TRAN rdt_ClusterPickCfm14
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               BEGIN
                  ...
                  (Route, OrderKey, OrderRefNo, Loadkey, Consigneekey, StorerKey, PickSlipNo, AddWho) -- SOS356989
                  VALUES
         ...
         IF @@ROWCOUNT > 0 AND @nPackQty > 0
         BEGIN
            IF NOT EXISTS (SELECT 1 FROM dbo.PackInfo WITH (NOLOCK)
                           WHERE PickSlipNo = @cPickSlipNo
                           AND   CartonNo = @nCartonNo)
            BEGIN
               INSERT INTO dbo.PackInfo ( PickSlipNo, CartonNo, Qty) VALUES 
               (@cPickSlipNo, @nCartonNo, @nPD_Qty)
   ...
   RollBackTran:
      ROLLBACK TRAN rdt_ClusterPickCfm14

   Quit:
      WHILE @@TRANCOUNT > @nTranCount -- Commit until the level we started
         COMMIT TRAN rdt_ClusterPickCfm14
