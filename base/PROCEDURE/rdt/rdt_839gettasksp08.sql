...
CREATE   PROC [RDT].[rdt_839GetTaskSP08] (    
  @nMobile          INT,              
  @nFunc            INT,              
      ...
      BEGIN    
         ...
         BEGIN
            ...
            WHERE RKL.PickSlipNo = @cPickSlipNo    
               ...
               OR  (LOC.LogicalLocation = @cCurrLogicalLOC AND LOC.LOC > @cCurrLOC))    
            GROUP BY LOC.LogicalLocation, LOC.LOC, PD.ID, PD.StorerKey, PD.SKU   
            ORDER BY PD.ID, PD.StorerKey, PD.SKU, LOC.LOC  
         END
         ELSE 
         BEGIN
            SELECT TOP 1    
            ...
            WHERE RKL.PickSlipNo = @cPickSlipNo    
               ...
               AND (LOC.LogicalLocation > @cCurrLogicalLOC    
               OR  (LOC.LogicalLocation = @cCurrLogicalLOC AND LOC.LOC > @cCurrLOC))    
            GROUP BY LOC.LogicalLocation, LOC.LOC, PD.ID, PD.StorerKey, PD.SKU,pd.id    
      ...
      BEGIN      
         ...
         BEGIN    
            ...
            WHERE PD.OrderKey = @cOrderKey      
               ...
               AND (LOC.LogicalLocation > @cCurrLogicalLOC      
               OR  (LOC.LogicalLocation = @cCurrLogicalLOC AND LOC.LOC > @cCurrLOC))      
            GROUP BY LOC.LogicalLocation, LOC.LOC, PD.StorerKey, PD.SKU,pd.id      
            ORDER BY LOC.LogicalLocation, LOC.LOC, PD.StorerKey, PD.SKU      
         END    
         ELSE IF EXISTS (  SELECT 1 from codelkup (NOLOCK) 
                            WHERE LISTNAME='CVZONE' 
      ...
      BEGIN      
         IF @cPickZone = ''      
            ...
            WHERE PD.OrderKey = @cOrderKey      
               ...
               AND LOC.LOC = @cCurrLOC      
               AND PD.SKU > @cCurrSKU  
            GROUP BY PD.StorerKey, PD.SKU,pd.id        
         ...
         BEGIN
            ...
            WHERE PD.OrderKey = @cOrderKey      
               ...
               AND LOC.LOC = @cCurrLOC      
               AND PD.SKU > @cCurrSKU     
             GROUP BY PD.ID, PD.StorerKey, PD.SKU, LOC.LOC 
             ORDER BY PD.ID, PD.StorerKey, PD.SKU, LOC.LOC      
         END
         ELSE      
            SELECT TOP 1      
            ...
            WHERE PD.OrderKey = @cOrderKey      
               ...
               AND PD.SKU > @cCurrSKU  
            GROUP BY PD.StorerKey, PD.SKU,pd.id 
            ORDER BY PD.StorerKey, PD.SKU,pd.id      
      END      
      ...
      BEGIN      
         ...
         BEGIN
            ...
            WHERE PD.PickSlipNo = @cPickSlipNo      
              ...
              -- AND LOC.LOC = @cCurrLOC      
               --AND PD.SKU > @cCurrSKU                
             GROUP BY PD.ID, PD.StorerKey, PD.SKU, PD.LOC 
             ORDER BY PD.ID, PD.StorerKey, PD.SKU, PD.LOC    
         END   
         ELSE
         BEGIN
            SELECT TOP 1      
 ...
 -- Assign to temp    
   ...
   '       AND PD.SKU = @cSKU ' + --INC0720911          
      ...
      BEGIN            
         ...
         CASE WHEN @cWhere1 = '' THEN '' ELSE ' AND ' + @cWhere1 END +             
         CASE WHEN @cWhere2 = '' THEN '' ELSE ' > '   + @cWhere2 END +            
         CASE WHEN @cGroupBy = '' THEN 'GROUP BY PD.ID' ELSE ' GROUP BY PD.ID' + @cGroupBy END +            
         CASE WHEN @cOrderBy = '' THEN '' ELSE ' ORDER BY ' + @cOrderBy END             
      END            
