...
GO
/******************************************************************************/
/* Store procedure: rdt_1641ExtValidSP10                                      */
...
CREATE PROC [RDT].[rdt_1641ExtValidSP10] (
   @nMobile      INT,
   @nFunc        INT,
   ...
   BEGIN
      ...
      BEGIN
         ...
         IF EXISTS (SELECT 1
                    ...
                    AND   PalletKey = @cDropID
                    AND   [Status] < '9'
                    AND   Userdefine01 NOT IN (SELECT o.route FROM packdetail pd(NOLOCK) join packheader ph (NOLOCK) 
                                                on pd.pickslipno=ph.pickslipno join  orders o (NOLOCK) ON o.orderkey=ph.orderkey
                                                WHERE pd.storerkey=@cStorerKey AND pd.Labelno=@cUCCNo))
         BEGIN
            SET @nErrNo = 148204
