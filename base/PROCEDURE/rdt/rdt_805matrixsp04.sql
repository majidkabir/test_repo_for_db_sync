...
*/
   ...
   SET @curPTLTran = CURSOR FOR
      ...
      FROM PTL.PTLTran T WITH (NOLOCK)
         LEFT JOIN @tPos P ON (T.IPAddress = P.IPAddress AND T.DevicePosition = P.Position)
   ...
   DECLARE @ccurUpdPtl CURSOR
   IF @nInputKey = 0
   BEGIN
      SET @ccurUpdPtl = CURSOR LOCAL READ_ONLY FAST_FORWARD FOR
      SELECT PTLKey
      ...
      AND   Status = '1'
      OPEN @ccurUpdPtl
      FETCH NEXT FROM @ccurUpdPtl INTO @nPTLKey
      WHILE @@FETCH_STATUS = 0
      BEGIN
         ...
         BEGIN
            SET @nErrNo = 166553
            SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --Upd PTLTran Er
            GOTO Quit
         END
   ...
   ELSE
   BEGIN
      -- Cater for same position multiple sku but ptl status only update 1 line
      SET @ccurUpdPtl = CURSOR LOCAL READ_ONLY FAST_FORWARD FOR
      SELECT PTLKey
      FROM PTL.PTLTRAN WITH (NOLOCK)
      WHERE IPAddress = @cIPAddress
