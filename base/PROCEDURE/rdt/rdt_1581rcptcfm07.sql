...
/******************************************************************************/
/* Store procedure: rdt_1581RcptCfm07                                         */
...
/*          set new line QtyExpected = 0 and oriLine + skuQty                 */
/*                                                                            */
/* Date       Rev  Author     Purposes                                        */
/* 2021-02-24 1.0  Chermaine  WMS-16334 create (base on rdt_1581RcptCfm05)    */
/******************************************************************************/

CREATE   PROCEDURE [RDT].[rdt_1581RcptCfm07] (
   @nFunc            INT,  
   @nMobile          INT,  
   ...
   BEGIN   	
      ...
      SAVE TRAN rdt_1581RcptCfm07 -- For rollback or commit only our own transaction

      SELECT @cExternReceiptKey = ExternReceiptKey, @cRecType = recType FROM Receipt WITH (NOLOCK) WHERE ReceiptKey = @cReceiptKey
      SELECT @nMaxASNLine = MAX(receiptLineNumber) FROM ReceiptDetail WITH (NOLOCK) WHERE ReceiptKey = @cReceiptKey
      
      IF ISNULL( @cLottable01, '') = ''
      BEGIN
         SET @cLottable01 = @cExternReceiptKey
      --   SET @nErrNo = 163751
      --   SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --No RSO
      --   GOTO RollBackTran
      END

      --SKU not exist in ori receipt line
      ...
      BEGIN
         ...
         UPDATE dbo.RECEIPTDETAIL WITH (ROWLOCK) SET 
            ...
            Lottable12 = (CASE WHEN @cRecType IN ('RSO-F','RSO-N') then 'INACCESSIBLE' ELSE Lottable12 END),
            Lottable09 = @cExternReceiptKey,
            Lottable01 = @cExternReceiptKey,
            QtyExpected = 0,
            userdefine10 = '',
            TrafficCop = NULL
         WHERE ReceiptKey = @cReceiptKey
      ...
      --new line created
      IF @cReceiptLineNumber > @nMaxASNLine
      BEGIN
         
         --receive by fn1581 need to set Qtyexpected = 0, so need to update back the qtyExpected for the original line
         UPDATE dbo.RECEIPTDETAIL WITH (ROWLOCK) SET
            QtyExpected = QtyExpected + @nSKUQTY
            --QtyAdjusted = (CASE WHEN QtyAdjusted <> 0 THEN QtyAdjusted + @nSKUQTY ELSE QtyAdjusted END)
         WHERE ReceiptKey = @cReceiptKey
         AND   SKU = @cSKUCode            -- Same SKU
         AND   ReceiptLineNumber = @cDuplicatedFrom  -- Original receiptdetail line
      ...
      RollBackTran:  
         ROLLBACK TRAN rdt_1581RcptCfm07 
      Fail:  
      Quit:  
