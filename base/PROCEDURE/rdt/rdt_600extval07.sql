...
/************************************************************************/  
/* Store procedure: rdt_600ExtVal07                                     */  
...
/* 2019-10-07 1.0  YeeKung    WMS-10111 Created                         */  
/************************************************************************/  

CREATE PROC [RDT].[rdt_600ExtVal07] (  
   @nMobile      INT,             
   @nFunc        INT,             
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         BEGIN  
            ...
            IF EXISTS (SELECT 1 FROM SKU WITH (NOLOCK) WHERE  StorerKey = @cStorerKey   
               AND SKU = @cSKU  AND TemperatureFlag='YES')
            BEGIN
               IF @cLOC NOT IN ('THFRRSTG','THRTSTG','THHLDSTG','THDMGSTG ')
               BEGIN
                  SET @nErrNo = 144810  
                  SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') -- SKUNotColdSto  
                  GOTO Fail
               END
            END
            ELSE
            BEGIN
               ...
               BEGIN
                  SET @nErrNo = 144811  
                  SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') -- Wrong Stage 
                  GOTO Fail
               END
            END 
         END  
