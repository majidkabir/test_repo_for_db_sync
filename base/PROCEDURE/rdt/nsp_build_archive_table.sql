...
GO
/************************************************************************/  
/* Store Procedure: rdt.nsp_BUILD_ARCHIVE_TABLE                         */  
/* Creation Date:                                                       */  
/* Copyright: IDS                                                       */  
/* Written by:                                                          */  
/*                                                                      */  
...
/* 29-Aug-2005  YokeBeen   - SQL2K Upgrading Project-V6.0.              */  
/*                           Changed double quote to single quote.      */  
/*                           Added dbo. for all the EXECUTE statement.  */  
/*                           - (YokeBeen01).                            */  
/* 15-Nov-2010  LimKH      - change dbo to RDT.nsp_BUILD_ARCHIVE_TABLE] */
/*  3-Oct-2012  KHLim      increase storage size for length             */
/*                                                                      */  
/************************************************************************/  
  
CREATE PROCEDURE [RDT].[nsp_BUILD_ARCHIVE_TABLE]  
...
--   
   ...
   SELECT @c_buildstring = RTRIM(@c_copyfrom_db) + '.rdt.' + RTRIM(@c_tablename)  
   ...
   BEGIN   
      ...
      SELECT @c_errmsg =  'NSQL' + CONVERT(CHAR(5),@n_err) + ':' + @c_buildstring +   
            ' does  not exist (rdt.nsp_BUILD_ARCHIVE_TABLE)'  
   END     
  
   SELECT @c_buildstring = RTRIM(@c_copyto_db) + '.rdt.' + RTRIM(@c_tablename)  
   ...
   BEGIN  
      ...
      SELECT @c_msg = 'Create Table ' + RTRIM(@c_copyto_db) + '.rdt.' + RTRIM(@c_tablename) + '('  
   END  
...
--                SELECT @n_inclen_flag = 1  
               ...
               IF ((@c_typename = 'char') OR (@c_typename = 'varchar'))  
               BEGIN  
                  SELECT @c_inclen = ' (' + CONVERT(CHAR(10),@n_length)  + ') DEFAULT ' + '(N''' + ''')'  --KH01
               END  
               ...
               IF (DATALENGTH(@c_testmsg) > 200)  
               BEGIN    
                  SELECT @is_the_table_small = 0  
                  SELECT @n_strlen =  DATALENGTH(RTRIM(@c_msg))  
                  ...
                  SELECT @c_testmsg = '' -- jeff, this did not initialise, therefore causing the line not to have alter table, and therefore failed.  
  
                  IF (RTRIM(@c_field) <> 'TimeStamp')  
                  BEGIN  
                     SELECT @c_msg = 'Alter Table ' + RTRIM(@c_copyto_db) + '.rdt.' + RTRIM(@c_tablename) + ' ADD '  
                  END  
               END    
               ...
               BEGIN  
                  ...
                  SELECT @n_strlen =  DATALENGTH(@c_msg)  
                  SELECT @c_msg = SUBSTRING(RTRIM(@c_msg),1,(@n_strlen - 1)) + RTRIM(@c_paran)  
            ...
            IF (@b_debug = 1)  
            BEGIN  
               SELECT 'len =',datalength(@c_msg),'str =',@c_msg  
            END  
...
END     
      ...
      BEGIN   
         ...
         BEGIN  
            SELECT @c_msg = 'Alter Table ' + @c_copyto_db + '.rdt.' +  @c_tablename +   ' ADD ' + @c_msg  
         ...
         BEGIN    
            ...
            IF (@b_debug = 1)  
            BEGIN  
               SELECT 'in rdt.nsp_BUILD_ARCHIVE_TABLE', @c_tablename  
               SELECT '@c_msg ', @c_msg  
               SELECT '  '  
            END  
  
            SELECT @c_errmsg = CONVERT(CHAR(250),@n_err) + ':  fetch failed. (rdt.nsp_BUILD_ARCHIVE_TABLE) ' + ' ( ' +  
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         END  
      END  
      EXECUTE dbo.nsp_logerror @n_err, @c_errmsg, 'rdt.nsp_BUILD_ARCHIVE_TABLE'  -- (YokeBeen01)  
