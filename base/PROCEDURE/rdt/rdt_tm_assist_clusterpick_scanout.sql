...
/************************************************************************/    
/* Store procedure: rdt_TM_Assist_ClusterPick_ScanOut                   */    
...
CREATE PROC [RDT].[rdt_TM_Assist_ClusterPick_ScanOut] (    
    @nMobile         INT    
   ,@nFunc           INT    
...
BEGIN    
   ...
   BEGIN    
      ...
      BEGIN    
         ...
         BEGIN TRAN    
         SAVE TRAN ScanOut -- For rollback or commit only our own transaction    
...
RollBackTran:    
   ROLLBACK TRAN ScanOut -- Only rollback change made here    
Quit:    
   WHILE @@TRANCOUNT > @nTranCount -- Commit until the level we started    
      COMMIT TRAN    
END    
