...
GO
/************************************************************************/
/* Stored Procedure: rdt_840ExtInsPack11                                */
/*                                                                      */
...
/* 2020-08-09   James     1.0   WMS-14322 Created                       */
/* 2021-04-01   YeeKung   1.1   WMS-16717 Add serialno and serialqty    */
/*                              Params (yeekung01)                      */
/* 2021-04-16   James     1.2   WMS-16024 Standarized use of TrackingNo */
/*                              (james01)                               */
/* 2021-09-07   James     1.3   Bug fix (james02)                       */
/* 2021-11-20   Leong     1.4   JSM-34330 - Bug Fix.                    */
/************************************************************************/

CREATE PROC [RDT].[rdt_840ExtInsPack11] (
   @nMobile     INT,
   @nFunc       INT,
...
BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            IF NOT EXISTS (SELECT 1 FROM dbo.PackDetail WITH (NOLOCK)
                        ...
                        AND CartonNo = @nCartonNo)
            BEGIN
               -- Prepaid orders only allow multi carton per orders
               ...
               BEGIN
                  SET @nErrNo = 156606
                  SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --'NotPrepaid'
                  GOTO RollBackTran
               END
               ...
               UPDATE dbo.CartonTrack WITH (ROWLOCK) SET
                  LabelNo = @cOrderKey,
                  Carrierref2 = 'GET'
               WHERE CarrierName = @cShipperKey    --@cCarrierName -- JSM-34330
               AND   Keyname = @cStorerkey         --@cKeyName     -- JSM-34330
               AND   ISNULL( CarrierRef2, '') = ''                 -- JSM-34330
         ...
         VALUES
            ...
            @cTrackNo, 'rdt.' + sUser_sName(), GETDATE(), 'rdt.' + sUser_sName(), GETDATE(), @cDropID)
   ...
   OPEN @cur_UpdPickDtl
   FETCH NEXT FROM @cur_UpdPickDtl INTO @cPickDetailKey, @nPD_QTY
   ...
   BEGIN
      ...
      BEGIN
         ...
         UPDATE dbo.PickDetail WITH (ROWLOCK) SET
            CaseID = CASE WHEN @nNewCarton > 0 THEN @cLabelNo ELSE @cCurLabelNo END,
            TrafficCop = NULL
   ...
   GOTO Commit_rdt_840ExtInsPack11

   RollBackTran:
         ROLLBACK TRAN rdt_840ExtInsPack11
   Commit_rdt_840ExtInsPack11:
