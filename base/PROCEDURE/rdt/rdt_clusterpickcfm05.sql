...
/************************************************************************/
/* Store procedure: rdt_ClusterPickCfm05                                */
/* Copyright      : IDS                                                 */
/*                                                                      */
/* Purpose: Comfirm Pick & Insert packdetail                            */
/*          Nike enhancement. Support blank dropid and create labelno   */
/*                                                                      */
...
/* 01-Mar-2016 1.0  James       SOS364904 - Created                     */
/* 07-Mar-2016 1.1  James       Bug fix (james01)                       */
/* 23-Feb-2017 1.2  James       Perfomance tuning (james02)             */
/************************************************************************/

CREATE PROC [RDT].[rdt_ClusterPickCfm05] (
   @nMobile                   INT,           
   @nFunc                     INT,           
...
BEGIN
   ...
   DECLARE 
      ...
      @cDropID_New            NVARCHAR( 20), 
	   @cLabelno_New         	NVARCHAR( 20),    
      @cExtendedDropID_SP     NVARCHAR( 20),
   ...
   SET @cExtendedDropID_SP = rdt.RDTGetConfig( @nFunc, 'ExtendedDropID_SP', @cStorerKey)
   IF @cExtendedDropID_SP = '0'
      SET @cExtendedDropID_SP = ''

   IF @cExtendedDropID_SP <> '' AND ISNULL( @cDropID, '') = ''
   BEGIN
      -- Remember old dropid value
      SET @cDropID_New = @cDropID

      SET @cSQLStatement = N'EXEC rdt.' + RTRIM(@cExtendedDropID_SP) +
                            ' @nMobile, @nFunc, @cLangCode, @nStep, @nInputKey, @cFacility, @cStorerkey, ' + 
                            ' @cWaveKey, @cLoadKey, @cOrderKey, @cLOC, @cSKU, @cCartonType, ' + 
      ...
      ELSE
      BEGIN
         -- If old value <> new value and new value not blank
         IF @cDropID_New <> @cDropID AND ISNULL( @cDropID, '') <> ''
         BEGIN
   ...
   BEGIN
      SET @nErrNo = 96651
      SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --'DropID Is Req'
      GOTO RollBackTran
   END
