...
/******************************************************************************/
/* Store procedure: rdt_PTLCart_Assign_OrderPosTote                           */
/* Copyright      : LFLogistics                                               */
/*                                                                            */
/* Date       Rev  Author   Purposes                                          */
/* 06-05-2015 1.0  Ung      SOS333663 Created                                 */
/* 26-01-2018 1.1  Ung      Change to PTL.Schema                              */
/* 04-03-2021 1.2  YeeKung  WMS16448 Add storerconfig check toteid in used    */
/*                          (yeekung01)                                       */
/******************************************************************************/

CREATE PROC [RDT].[rdt_PTLCart_Assign_OrderPosTote] (
   @nMobile          INT, 
   @nFunc            INT, 
...
BEGIN
   ...
   DECLARE @cChkToteIDinPick NVARCHAR(1)

   SET @cChkToteIDinPick=  rdt.RDTGetConfig( @nFunc, 'ToteIDInPick', @cStorerkey)
   ...
   ***********************************************************************************************/
   ...
   BEGIN
      ...
      -- Check finish assign
      IF @nTotalTote > 0 AND @cOrderKey = '' AND @cPosition = '' AND @cToteID = ''
      BEGIN
         GOTO Quit
      END
      ...
      BEGIN TRAN
      SAVE TRAN rdt_PTLCart_Assign_OrderPosTote
      ...
      COMMIT TRAN rdt_PTLCart_Assign_OrderPosTote
...
RollBackTran:
   ROLLBACK TRAN rdt_PTLCart_Assign_OrderPosTote
Quit:
