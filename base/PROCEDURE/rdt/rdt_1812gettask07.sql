...
/************************************************************************/    
/* Store procedure: rdt_1812GetTask07                                   */    
/* Copyright      : maersk                                              */    
/*                                                                      */    
...
/* 2024-10-03  1.0  James     WMS-26122. Created                        */    
/* 2024-11-11  1.1  PXL009    FCR-1125 Merged 1.0 from v0 branch        */
/************************************************************************/    
    
CREATE   PROC [rdt].[rdt_1812GetTask07] (    
   @nMobile          INT,    
   @nFunc            INT,    
...
BEGIN    
   ...
   IF @cAreaKey = '' OR @cAreaKey = 'ALL'    
      SET @curRPTask = CURSOR LOCAL READ_ONLY FAST_FORWARD FOR    
         SELECT TOP 1    
            TD.TaskDetailKey, TD.TaskType, TD.FromLOC, TD.FromID, TD.SKU, TD.LOT, TD.QTY, TD.ToLOC, TD.ToID    
         ...
         ORDER BY TD.Priority, LOC.LogicalLocation, LOC.LOC, TD.TaskDetailKey    
   ELSE    
      SET @curRPTask = CURSOR LOCAL READ_ONLY FAST_FORWARD FOR    
         SELECT TOP 1    
            TD.TaskDetailKey, TD.TaskType, TD.FromLOC, TD.FromID, TD.SKU, TD.LOT, TD.QTY, TD.ToLOC, TD.ToID    
