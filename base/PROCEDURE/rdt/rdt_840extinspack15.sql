...
GO
/************************************************************************/
/* Stored Procedure: rdt_840ExtInsPack15                                */
/*                                                                      */
...
CREATE PROC [RDT].[rdt_840ExtInsPack15] (
   @nMobile                   INT, 
   @nFunc                     INT, 
...
BEGIN
   ...
   IF @nStep=8
   BEGIN
      IF NOT EXISTS(SELECT 1 FROM serialno  (NOLOCK) WHERE sku=@cSKU AND serialno=@cSerialNo AND storerkey=@cStorerkey AND status=1)
      BEGIN
         SET @nErrNo = 165769
      ...
      IF EXISTS(SELECT 1 FROM packserialno  (NOLOCK) WHERE sku=@cSKU AND serialno=@cSerialNo AND storerkey=@cStorerkey AND pickslipno=@cPickSlipNo)
      BEGIN
         SET @nErrNo = 165770
         SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --'UpdLog Failed'
         GOTO RollBackTran
   ...
   BEGIN
      ...
      SELECT @clabelline=LabelLine,@cLabelNo=LabelNo,@nCartonNo=CartonNo
      ...
      IF ISNULL(@cSerialNO,'')<>''
      BEGIN
         INSERT INTO Packserialno(PickSlipNo,LabelNo,CartonNo,LabelLine,StorerKey,sku,qty,SerialNo)
         VALUES(@cPickSlipNo,@cLabelNo,@nCartonNo,@clabelline,@cStorerkey,@cSku,@nQty,@cSerialNO)

         IF @@ERROR <> 0
         BEGIN
            SET @nErrNo = 165766
            SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --'UPDPKSN Failed'
            GOTO RollBackTran
         END
      END
   ...
   BEGIN
      ...
      BEGIN
         ...
         END
         ELSE
            SELECT @nNewCarton = CartonNo ,@clabelline=LabelLine
            FROM dbo.PackDetail WITH (NOLOCK) 
            WHERE PickSlipNo = @cPickSlipNo
      ...
      BEGIN
         ...
         BEGIN
            INSERT INTO Packserialno(PickSlipNo,LabelNo,CartonNo,LabelLine,StorerKey,sku,qty,SerialNo)
            VALUES(@cPickSlipNo,@cCurLabelNo,@nCartonNo,@cCurLabelLine,@cStorerkey,@cSku,@nQty,@cSerialNO)
   ...
   IF @nNewCarton > 0
   BEGIN
      SET @cShipLabel = rdt.RDTGetConfig( @nFunc, 'ShipperLabel', @cStorerKey)
      IF @cShipLabel = '0'
         SET @cShipLabel = ''
      ...
      BEGIN
         ...
         INSERT INTO @tSHIPPLABEL (Variable, Value) VALUES ( '@nFromCartonNo',   @nCartonNo)    -- ZG01
         INSERT INTO @tSHIPPLABEL (Variable, Value) VALUES ( '@nToCartonNo',     @nCartonNo)    -- ZG01
   ...
   RollBackTran:  
         ROLLBACK TRAN rdt_840ExtInsPack15  
   Quit:  
