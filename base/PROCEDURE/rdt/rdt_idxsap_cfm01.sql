...
/************************************************************************/  
/* Store procedure: rdt_IDXSAP_Cfm01                                    */  
/* Copyright: IDS                                                       */  
...
/* 2014-04-20 1.1  Chee     Add @cUCCNo Parameter (Chee01)              */ 
/* 2014-05-08 1.2  James    SOS307345 Cater for 16 & 20 digits label no */
/*                          (james01)                                   */ 
/* 2014-10-29 1.3  James    SOS324404 Bug fix (james02)                 */ 
/************************************************************************/  
  
CREATE PROCEDURE [RDT].[rdt_IDXSAP_Cfm01]  
   @nMobile       INT,  
   @nFunc         INT,  
...
BEGIN  
   ...
   IF LEN( RTRIM( @cLabelNo)) = 16  -- (james01)
   BEGIN
      -- Get Distribution Centre
      SET @cDistCtr = RIGHT( '0000' + SUBSTRING( @cLabelNo, 1, 4), 4) 
/*
      -- Get ConsigneeKey
      SET @cConsigneeKey = ''
      SET @cConsigneeKey = 'ITX' + RIGHT( '0000' + SUBSTRING( @cLabelNo, 5, 4), 4)
*/
      -- Get ConsigneeKey
      SET @cConsigneeKey = ''
      SET @cConsigneeKey = RIGHT( '0000' + SUBSTRING( @cLabelNo, 5, 4), 4)
      ...
      -- Get Separate
      SET @cSeparate = ''
      SET @cSeparate = SUBSTRING( @cLabelNo, 10, 1)
   END
   ELSE
   BEGIN
/*
      SET @cDistCtr = RIGHT( SUBSTRING( RTRIM(LTRIM(@cLabelNo)), 2, 6), 4)
