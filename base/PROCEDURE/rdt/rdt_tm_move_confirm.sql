...
/************************************************************************/
/* Store procedure: rdt_TM_Move_Confirm                                 */
/* Copyright      : IDS                                                 */
/*                                                                      */
...
CREATE PROC [RDT].[rdt_TM_Move_Confirm] (
   @nMobile        INT,
   @nFunc          INT,
...
BEGIN
   ...
   SAVE TRAN rdt_TM_Move_Confirm -- For rollback or commit only our own transaction
   ...
   BEGIN
      ...
      SELECT
         @cNewTaskDetailKey, @cTaskDetailKey, '', '0', '', '', '', (@nSystemQTY - @nQTY), (@nSystemQTY - @nQTY), 
   ...
   COMMIT TRAN rdt_TM_Move_Confirm -- Only commit change made here
   GOTO Quit

RollBackTran:
   ROLLBACK TRAN rdt_TM_Move_Confirm -- Only rollback change made here
Fail:
Quit:
