...
/***************************************************************************/
/* Store procedure: rdt_1764ClosePlt01                                     */
/* Copyright      : IDS                                                    */
/*                                                                         */
...
BEGIN
   ...
   SAVE TRAN rdt_1764ClosePlt01 -- For rollback or commit only our own transaction
   ...
   BEGIN
      ...
      BEGIN
         ...
         EXECUTE rdt.rdt_Move
            ...
            @cSourceType = 'rdt_1764ClosePlt01',
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               DECLARE @ccurPD   CURSOR, @nPickDtl_Qty INT, @nChkQty INT
               SET @nChkQty = @nQTY
               ...
               GROUP BY Lot
               OPEN @ccurPD
               FETCH NEXT FROM @ccurPD INTO @cLOT, @nPickDtl_Qty
               WHILE @@FETCH_STATUS = 0
               BEGIN
                  IF @cMoveQTYAlloc = '1'
                  BEGIN
                     IF @nPickDtl_Qty < @nSystemQTY -- Short replen
                  ...
                  BEGIN
                     IF @cMoveQTYAlloc = '1'
                        SET @nQTYReplen = @nPickDtl_Qty - @nQTYAlloc
                     ELSE
                        SET @nQTYReplen = @nPickDtl_Qty
                  END
                  ELSE
                     SET @nQTYReplen = 0
...
END
   ...
   COMMIT TRAN rdt_1764ClosePlt01 -- Only commit change made here
   GOTO Quit

RollBackTran:
   ROLLBACK TRAN rdt_1764ClosePlt01 -- Only rollback change made here
Fail:
Quit:
