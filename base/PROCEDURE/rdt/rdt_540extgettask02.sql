...
/************************************************************************/
/* Store procedure: rdt_540ExtGetTask02                                 */
/* Copyright      : IDS                                                 */
/*                                                                      */
...
/* 2018-Mar-28 1.0  James       WMS4203. Created                        */
/************************************************************************/

CREATE PROC [RDT].[rdt_540ExtGetTask02] (
   @nMobile                   INT,
   @nFunc                     INT, 
...
BEGIN
   ...
   SELECT @cUserName = UserName, @cLoadKey = V_CaseID, @nStep = Step
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            SET @cLoadKey = ''
            SET @cSKU = ''
            SELECT TOP 1 @cLoadKey = LPD.LoadKey, @cSKU = PD.Sku
            FROM dbo.PickDetail PD WITH (NOLOCK) 
                     JOIN dbo.LoadPlanDetail LPD WITH (NOLOCK) ON PD.OrderKey = LPD.OrderKey
