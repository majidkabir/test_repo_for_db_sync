...
/************************************************************************/
/* Store procedure: rdt_PTLStation_Confirm_ToteID                       */
...
CREATE PROC [RDT].[rdt_PTLStation_Confirm_ToteID] (
    @nMobile      INT
   ,@nFunc        INT
      ...
      BEGIN
            ...
            BEGIN
               ...
               BEGIN -- Have balance, need to split
                  ...
                  SELECT 
                     ...
                     IPAddress, DeviceID, DevicePosition, Status, LightUp, LightMode, LightSequence, PTLType, SourceKey, DropID, CaseID, RefPTLKey, 
                     Storerkey, OrderKey, ConsigneeKey, SKU, LOC, LOT, UOM, Remarks, Func, GroupKey, 'rdt_PTLStation_Confirm_ToteID', ArchiveCop
                  FROM PTL.PTLTran WITH (NOLOCK) 
         			WHERE PTLKey = @nPTLKey			            
                  IF @@ERROR <> 0
                  BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               VALUES
                  (@cPickSlipNo, @nCartonNo, @cCartonID, @cLabelLine, @cStorerKey, @cSKU, @nCartonQTY, 'rdt.' + SUSER_SNAME(), GETDATE(), 'rdt.' + SUSER_SNAME(), GETDATE(), @cCartonID)
               IF @@ERROR <> 0
               BEGIN
                  SET @nErrNo = 101822
                  SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --InsPackDtlFail
