...
GO
/************************************************************************/
/* Store procedure: rdt_858ExtUpdSP01                                   */
/* Copyright      : LF                                                  */
/*                                                                      */
...
CREATE PROC [RDT].[rdt_858ExtUpdSP01] (
   @nMobile        INT,
   @nFunc          INT,
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               BEGIN
                  ...
                  --VALUES ( '857' , Getdate() , @cFieldDescr , @cValue )
               END
               ELSE IF @nCount = 3
               BEGIN
         ...
         IF @nStep = 2  -- Display Information
         BEGIN
...
--            END
         ...
         BEGIN
            ...
            BEGIN
               ...
               BEGIN
                  SET @nErrNo = 93402
                  SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --'CheckInNotDone'
                  GOTO RollBackTran
               END
            END
            ELSE
            BEGIN
               ...
               BEGIN
                  SET @nErrNo = 93403
                  SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --'LoadingNotStart'
                  GOTO RollBackTran
               END
            END

            UPDATE dbo.Booking_Out WITH (ROWLOCK)
            SET   Status     = CASE WHEN @cActionType = '10' THEN '2' ELSE '3' END
                 ,SignInTime = CASE WHEN @cActionType = '10' THEN GetDate() ELSE SignInTime END
                 ,UnloadTime = CASE WHEN @cActionType = '10' THEN UnloadTime ELSE GetDate() END
            WHERE BookingNo = @cAppointmentNo
...
--            INSERT INTO TraceINFO ( TraceName , TimeIN , Col1 , Col2 )
--            VALUES ( '857' , Getdate() , @cAppointmentNo , @cActionType )
   ...
   RollBackTran:
   ROLLBACK TRAN CheckIn85801

   Quit:
   WHILE @@TRANCOUNT>@nTranCount -- Commit until the level we started
          COMMIT TRAN CheckIn85801
Fail:
END
