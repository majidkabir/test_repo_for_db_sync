...
/************************************************************************/
/* Store procedure: rdt_PFLStation_Matrix                               */
/* Copyright      : LF Logistics                                        */
/*                                                                      */
/* Date       Rev  Author   Purposes                                    */
/* 20-06-2019 1.0  Ung      WMS-9372 Created                            */
/************************************************************************/

CREATE PROC [RDT].[rdt_PFLStation_Matrix] (
    @nMobile    INT
   ,@nFunc      INT
...
BEGIN
   ...
   ***********************************************************************************************/
   ...
   SELECT @cStationMatrixSP = ISNULL( UDF02, '')
   ...
   IF @cStationMatrixSP <> ''
   BEGIN
      IF EXISTS( SELECT 1 FROM sys.objects WHERE name = @cStationMatrixSP AND type = 'P')
      BEGIN
         SET @cSQL = 'EXEC rdt.' + RTRIM( @cStationMatrixSP) +
            ' @nMobile, @nFunc, @cLangCode, @nStep, @nInputKey, @cFacility, @cStorerKey, ' +
            ' @cLight, @cStation1, @cStation2, @cStation3, @cStation4, @cStation5, @cMethod, @nErrNo OUTPUT, @cErrMsg OUTPUT, ' +
