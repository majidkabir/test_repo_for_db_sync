...
/******************************************************************************/
/* Store procedure: rdt_PTLStation_Confirm_ToteIDSKU05                        */
/* Copyright      : LF Logistics                                              */
/*                                                                            */
/* Purpose: Get Pickdetail.ID = Scanned ID to offset                          */
/*                                                                            */
/* Date       Rev Author      Purposes                                        */
/* 2019-12-03 1.0 James       WMS-11214 Created                               */
/******************************************************************************/

CREATE PROC [RDT].[rdt_PTLStation_Confirm_ToteIDSKU05] (
    @nMobile      INT
   ,@nFunc        INT
...
BEGIN
   ...
   ***********************************************************************************************/
   ...
   BEGIN
      ...
      BEGIN
        ...
        -- Transaction at order level
         ...
         IF NOT EXISTS ( SELECT 1 FROM dbo.Dropid WITH (NOLOCK) 
                         WHERE Dropid = @cActCartonID
                         AND   [Status] = '5') AND @cUpdatePickDetail = '1'
         BEGIN
            INSERT INTO dbo.Dropid ( Dropid, Droploc, AdditionalLoc, DropIDType, LabelPrinted, 
            ManifestPrinted, [Status], AddDate, AddWho, EditDate, EditWho)
            VALUES
            ( @cActCartonID, '', '', '', '0', '0', '5', GETDATE(), 'rdt.' + SUSER_SNAME(), GETDATE(), 'rdt.' + SUSER_SNAME()) 
      ...
      -- For all orders assigned to the cart, all pickdetail.status >=4
      IF NOT EXISTS ( SELECT 1 FROM dbo.PICKDETAIL PD WITH (NOLOCK)
                  WHERE PD.Storerkey = @cStorerKey
                  AND   PD.[Status] < '4'
                  AND   EXISTS ( SELECT 1 FROM rdt.rdtPTLStationLog PTL WITH (NOLOCK)
                                 WHERE PD.OrderKey = PTL.OrderKey
                                 AND   Station IN (@cStation1, @cStation2, @cStation3, @cStation4, @cStation5)))
      BEGIN
         SET @cErrMsg1 = rdt.rdtgetmessage( 146838, @cLangCode, 'DSP') --ALL TASK COMPLETED
   ...
   BEGIN
            ...
            BEGIN
               ...
               BEGIN -- Have balance, need to split
                  ...
                  SELECT 
                     ...
                     IPAddress, DeviceID, DevicePosition, Status, LightUp, LightMode, LightSequence, PTLType, SourceKey, DropID, CaseID, RefPTLKey, 
                     Storerkey, OrderKey, ConsigneeKey, SKU, LOC, LOT, UOM, Remarks, Func, GroupKey, 'rdt_PTLStation_Confirm_ToteIDSKU05', ArchiveCop
                  FROM PTL.PTLTran WITH (NOLOCK) 
         			WHERE PTLKey = @nPTLKey			            
                  IF @@ERROR <> 0
                  BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            ManifestPrinted, [Status], AddDate, AddWho, EditDate, EditWho)
            VALUES
            ( @cCartonID, '', '', '', '0', '0', '5', GETDATE(), 'rdt.' + SUSER_SNAME(), GETDATE(), 'rdt.' + SUSER_SNAME()) 
