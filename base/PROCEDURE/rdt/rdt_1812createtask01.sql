...
/************************************************************************/
/* Store procedure: rdt_1812CreateTask01                                */
/* Copyright      : LFLogistics                                         */
/*                                                                      */
/* Purpose: Generate next task, if current task is transit              */
/*                                                                      */
/* Date       Rev  Author     Purposes                                  */
/* 24-04-2018 1.0  Ung        WMS-3273 Created                          */
/************************************************************************/

CREATE PROC [RDT].[rdt_1812CreateTask01] (
   @nMobile        INT,
   @nFunc          INT,
...
BEGIN
   ...
   SELECT TOP 1 
      ...
      @cSourceType     = 'rdt_1812CreateTask01'
   ...
   SAVE TRAN rdt_1812CreateTask01 -- For rollback or commit only our own transaction
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            VALUES (
               @cNewTaskDetailKey, 'ASTFPK', '0', '', @cToLOC, @cToID, @cFinalLOC, @cFinalID, @nQTY, @cToLOCAreaKey, 
      ...
      */
      BEGIN
         ...
         INSERT INTO TaskDetail (
            ...
            PickMethod, StorerKey, SKU, LOT, ListKey, TransitCount, SourceType, WaveKey, LoadKey, Priority, SourcePriority, TrafficCop)
         VALUES (
            @cNewTaskDetailKey, 'ASTFPK', '0', '', @cToLOC, @cToID, @cFinalLOC, @cFinalID, 0, @cToLOCAreaKey, 
   ...
   BEGIN 
      ...
      INSERT INTO TaskDetail (
         ...
         PickMethod, Storerkey, SKU, LOT, ListKey, TransitCount, SourceType, WaveKey, LoadKey, Priority, SourcePriority, TrafficCop)
      VALUES (
         @cNewTaskDetailKey, 'ASTFPK', '0', '', @cToLOC, @cToID, @cTransitLOC, @cToID, 0, @cToLOCAreaKey, 
         'FP', @cStorerkey, '', '', @cListKey, @nTransitCount, @cSourceType, @cWaveKey, @cLoadKey, @cPriority, @cSourcePriority, NULL)
      IF @@ERROR <> 0
      BEGIN
         SET @nErrNo = 123355
   ...
   COMMIT TRAN rdt_1812CreateTask01 -- Only commit change made here
   GOTO Quit

RollBackTran:
   ROLLBACK TRAN rdt_1812CreateTask01 -- Only rollback change made here
Fail:
Quit:
