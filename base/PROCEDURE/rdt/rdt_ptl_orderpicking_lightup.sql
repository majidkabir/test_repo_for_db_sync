...
GO
/************************************************************************/
/* Store procedure: rdt_PTL_OrderPicking_LightUp                        */
/* Copyright      : IDS                                                 */
/*                                                                      */
...
/* 23-06-2014 1.2  James    SOS303322 - Support 20 positions (james01)  */
/************************************************************************/

CREATE PROC [RDT].[rdt_PTL_OrderPicking_LightUp] (
     @nMobile          INT
    ,@nFunc            INT
    ...
    ,@cLottable02      NVARCHAR(18) -- (ChewKP01)
    ,@cLottable03      NVARCHAR(18) -- (ChewKP01)
    ,@dLottable04      DATETIME     -- (ChewKP01)
    ,@dLottable05      DATETIME     -- (ChewKP01)
...
BEGIN
   ...
   DEALLOCATE CursorPTLTranLightPosition
    ...
    BEGIN
      ...
      IF NOT EXISTS ( SELECT 1 FROM @t_PositionQty WHERE DevicePosition = CAST(@nCounter AS NVARCHAR(5)) )
      BEGIN
             INSERT INTO @t_PositionQty ( DevicePosition, Qty)
             VALUES ( CAST(@nCounter AS NVARCHAR(5)) , 'XXXXX' )
      END
    ...
    BEGIN
        ...
        IF Len(@cQty) < 5
        BEGIN
         SET @cQty = @cQty + SPACE(5 - LEN(@cQty))
        END

        IF @nCounter = 1
        BEGIN
            SET @cResult = CAST(@cQty AS NVARCHAR(5))
        END
        ELSE
        BEGIN

            IF (SELECT (@nCounter-1) % 3) = 0
            BEGIN
               SET @cResult = @cResult + ',' + CAST(@cQty AS NVARCHAR(5))
            END
            ELSE
            BEGIN
               SET @cResult = @cResult + ' ' + CAST(@cQty AS NVARCHAR(5))
