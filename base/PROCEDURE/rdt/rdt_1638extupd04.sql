...
/************************************************************************/
/* Store procedure: rdt_1638ExtUpd04                                    */
...
CREATE PROC [RDT].[rdt_1638ExtUpd04] (
   @nMobile      INT,           
   @nFunc        INT,           
   @nStep        INT,
   @nAfterStep   INT,        
...
BEGIN
   ...
   BEGIN
      IF @nStep IN ( 3, 6)    -- Case ID/Pack info (after)
      BEGIN
...
RollBackTran:
   ROLLBACK TRAN rdt_1638ExtUpd04 -- Only rollback change made here
Quit:
