...
/***************************************************************************/
/* Store procedure: rdt_ScanToContainer_CaptureInfo                        */
...
CREATE PROC [RDT].[rdt_ScanToContainer_CaptureInfo](
   @nMobile       INT,           
   @nFunc         INT,           
...
BEGIN
   ...
   DECLARE @cCaptureContainerInfoSP NVARCHAR(20)
   ...
   ***********************************************************************************************/
   ...
   BEGIN
      IF EXISTS( SELECT 1 FROM dbo.sysobjects WHERE name = @cCaptureContainerInfoSP AND type = 'P')
      BEGIN
         SET @cSQL = 'EXEC rdt.' + RTRIM( @cCaptureContainerInfoSP) +
            ' @nMobile, @nFunc, @cLangCode, @nStep, @nInputKey, @cFacility, @cStorerKey, @cType,  ' +
            ' @cContainerKey, @cContainerNo, @cMBOLKey, @cSSCCNo, @cPalletKey, @cTrackNo, ' + 
