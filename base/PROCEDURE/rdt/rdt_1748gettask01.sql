...
/************************************************************************/  
/* Store procedure: rdt_1748GetTask01                                   */  
/* Copyright      : IDS                                                 */  
/*                                                                      */  
/* Purpose: Get next move task (only for partial pallet)                */  
/*                                                                      */  
/* Modifications log:                                                   */  
/*                                                                      */  
/* Date        Rev  Author    Purposes                                  */  
/* 21-Oct-2011 1.0  Ung       SOS340175 Created                         */  
/* 05-Jul-2021 1.1  Ung       WMS-15424 Fix ListKey                     */
/************************************************************************/  
  
CREATE PROC [RDT].[rdt_1748GetTask01] (  
   @nMobile          INT,  
   @nFunc            INT,  
...
BEGIN  
   ...
   IF @cAreaKey = ''  
      SET @curRPTask = CURSOR LOCAL READ_ONLY FAST_FORWARD FOR  
         ...
         WHERE TaskDetail.TaskType IN ('MVF')  
   ...
   ELSE  
      SET @curRPTask = CURSOR LOCAL READ_ONLY FAST_FORWARD FOR  
         ...
         WHERE AreaDetail.AreaKey = @cAreaKey  
            AND TaskDetail.TaskType IN ('MVF')  
            AND TaskDetail.PickMethod = 'PP' -- Partial pallet  
            AND TaskDetail.Status = '0'  
            AND TaskDetail.ToLOC = CASE WHEN @cPalletFinalLOC <> '' THEN @cPalletFinalLOC ELSE TaskDetail.ToLOC END  
            AND PAZone2.PutawayZone = CASE WHEN @cPalletFinalZone <> '' THEN @cPalletFinalZone ELSE PAZone2.PutawayZone END  
