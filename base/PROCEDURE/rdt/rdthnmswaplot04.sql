...
GO
/************************************************************************/
/* Store procedure: rdtHnMSwapLot04                                     */
/* Copyright      : IDS                                                 */
/*                                                                      */
/* Purpose: H&M IN swap lot logic                                       */
/*                                                                      */
...
/* 07-Dec-2016 1.0  James       WMS3212. Created                        */
/*                              WMS3252-Add PackByTrackNoGenLabelNo_SP  */
/*                              Add isp_AssignPackLabelToOrderByLoad    */
/* 16-Apr-2021 1.1  James       WMS-16024 Standarized use of TrackingNo */
/*                              (james01)                               */
/************************************************************************/

CREATE PROC [RDT].[rdtHnMSwapLot04] (
   @n_Mobile         INT, 
...
BEGIN
   ...
   DECLARE 
           ...
           @c_Key2                  NVARCHAR( 5), 
           @c_Counter               NVARCHAR( 7), 
           @c_SSCCLabelNo           NVARCHAR( 20), 
           @c_FixedString           NVARCHAR( 10), 
   ...
   BEGIN
      -- If it is sales orders and is COD/NORMAL type then not allow to have multiple carton
      IF EXISTS ( SELECT 1 FROM dbo.Orders WITH (NOLOCK) 
                  WHERE OrderKey = @c_Orderkey
                  AND   StorerKey = @c_StorerKey
                  AND   [Type] IN ( 'NORMAL', 'COD'))
      BEGIN
         SET @n_ErrNo = 117906    
   ...
   SAVE TRAN rdtHnMSwapLot04 -- For rollback or commit only our own transaction  
      ...
      BEGIN    
         ...
         BEGIN
            ...
            BEGIN
               ...
               SELECT @c_TrackNo = TrackingNo
               FROM dbo.ORDERS WITH (NOLOCK)
               WHERE StorerKey = @c_Storerkey
               AND   OrderKey = @c_OrderKey
            END

            -- If waiting tracking no update from interface, use temp labelno value
            IF ISNULL( @c_TrackNo, '') = ''
            BEGIN
               /** get new available pre-paid tracking number **/
               SET @c_CarrierName = 'HMTEMP'
               SET @c_KeyName = 'HM'
               ...
               BEGIN    
                  SET @n_ErrNo = 117927    
                  SET @c_ErrMsg = rdt.rdtgetmessage( @n_ErrNo, @c_LangCode, 'DSP') --'Assign Lbl Err'    
                  EXEC rdt.rdtSetFocusField @n_Mobile, 6    
                  GOTO RollBackTran    
               END 
               /*
               -- No need to generate for 1st carton as it already 
               -- generated during allocation. Tablename: WSCRSOREQMP
            ...
            */
            ...
            BEGIN
               ...
               BEGIN    
                  SET @n_ErrNo = 117928    
                  SET @c_ErrMsg = rdt.rdtgetmessage( @n_ErrNo, @c_LangCode, 'DSP') --'Upd CaseID Err'    
                  EXEC rdt.rdtSetFocusField @n_Mobile, 6    
                  GOTO RollBackTran    
               END 
            END
         ...
         -- If this is first sku in pickslip then need trigger P-status interface
         --IF @n_FirstSKU = 1
         --BEGIN
         --   -- Insert transmitlog2 here
         --   SET @b_Success = 1
   ...
   COMMIT TRAN rdtHnMSwapLot04
   GOTO Quit
   
   RollBackTran:  
      ROLLBACK TRAN rdtHnMSwapLot04  
