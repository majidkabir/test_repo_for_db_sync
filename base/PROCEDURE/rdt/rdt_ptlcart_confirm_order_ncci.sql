...
/************************************************************************/
/* Store procedure: rdt_PTLCart_Confirm_Order_NCCI                      */
/* Copyright      : LF Logistics                                        */
/*                                                                      */
/* Purpose: Close working batch                                         */
/*                                                                      */
/* Date       Rev  Author      Purposes                                 */
/* 24-09-2020 1.0  YeeKung     WMS-15164 Created                        */
/************************************************************************/

CREATE PROC [RDT].[rdt_PTLCart_Confirm_Order_NCCI] (
    @nMobile    INT
   ,@nFunc      INT
...
BEGIN
   ...
   ***********************************************************************************************/
   ...
   BEGIN
      ...
      BEGIN
         ...
         SAVE TRAN updatepick_NCCI -- For rollback or commit only our own transaction
         ...
         BEGIN
             ...
             IF EXISTS (SELECT 1 FROM ORDERS O (NOLOCK) JOIN
                        CODELKUP CL (NOLOCK) ON O.type=CL.code
                        WHERE CL.listname='ORDERTYPE'
                        AND o.orderkey=@cOrderkey
                        AND CL.code IN('B2B','B2C','STO')
         ...
         -- Commit order level
         COMMIT TRAN rdt_PTLCart_Confirm_Order_NCCI
   ...
   ***********************************************************************************************/
   ...
   BEGIN
      ...
      SET @nTranCount = @@TRANCOUNT
      BEGIN TRAN  -- Begin our own transaction
      SAVE TRAN rdt_PTLCart_Confirm_Order_NCCI -- For rollback or commit only our own transaction
            ...
            BEGIN
               ...
               BEGIN -- Have balance, need to split
                  ...
                  INSERT INTO PTL.PTLTran (
                     ...
                     IPAddress, DeviceID, DevicePosition, Status, PTLType, DropID, OrderKey, Storerkey, SKU, LOC, LOT, Remarks, MessageNum, 
                  ...
                  SELECT 
                     ...
                     IPAddress, DeviceID, DevicePosition, Status, PTLType, '', OrderKey, Storerkey, SKU, LOC, LOT, Remarks, MessageNum, 
                     DeviceProfileLogKey, ArchiveCop, SourceKey, ConsigneeKey, CaseID, LightUp, LightMode, LightSequence, UOM, RefPTLKey
                  FROM PTL.PTLTran WITH (NOLOCK) 
         			WHERE PTLKey = @nPTLKey			            
                  IF @@ERROR <> 0
...
RollBackTran:
   ROLLBACK TRAN rdt_PTLCart_Confirm_Order_NCCI -- Only rollback change made here
Quit:
   WHILE @@TRANCOUNT > @nTranCount -- Commit until the level we started
      COMMIT TRAN
END
