...
/************************************************************************/
/* Store procedure: rdt_UCCPostPickAudit_Confirm                        */
/*                                                                      */
/* Modifications log:                                                   */
/*                                                                      */
/* Date        Rev  Author      Purposes                                */
/* 25-04-2013 1.0  Ung         SOS262114 Created                        */
/* 02-09-2013 1.1  Shong       Performance Tuning                       */  
/************************************************************************/

CREATE PROC [RDT].[rdt_UCCPostPickAudit_Confirm] (
   @nMobile        INT,
   @nFunc          INT, 
...
BEGIN
   ...
   SAVE TRAN rdt_UCCPostPickAudit_Confirm -- For rollback or commit only our own transaction
...
-- Not required as PickingInfo update also set Orders.Status = 3
   -- Update Orders  
...
--   END  
   ...
   -- Update PickDetail
   IF EXISTS( SELECT 1 FROM dbo.PickDetail WITH (NOLOCK) WHERE PickDetailKey = @cPickDetailKey AND Status < '3')
   BEGIN
...
*/
   ...
   COMMIT TRAN rdt_UCCPostPickAudit_Confirm
...
RollBackTran:
   ROLLBACK TRAN rdt_UCCPostPickAudit_Confirm
Quit:
