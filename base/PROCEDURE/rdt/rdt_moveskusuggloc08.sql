...
GO
/************************************************************************/
/* Store procedure: rdt_MoveSKUSuggLoc08                                */
/* Copyright      : LFLogistics                                         */
/*                                                                      */
/* Purpose: Find a friend, then find empty LOC                          */
/*          to filter first character of Lotxlocxid.ID not in           */
/*          (æ0Æ, æ1Æ, æ2Æ, æ3Æ, æ4Æ, æ5Æ, æ6Æ, æ7Æ, æ8Æ, æ9Æ)          */
/*                                                                      */
/* Date        Rev  Author      Purposes                                */
/* 11-07-2019  1.0  James       WMS-9712 Created                        */
/************************************************************************/

CREATE PROC [RDT].[rdt_MoveSKUSuggLoc08] (
   @nMobile       INT,
   @nFunc         INT,
   ...
   IF @cLOCCat1 = '' AND @cLOCCat2 = '' AND @cLOCCat3 = '' AND @cLOCCat4 = '' AND @cLOCCat5 = ''
      SET @curLOC = CURSOR FOR
         ...
         WHERE LOC.Facility = @cFacility
            ...
            AND LLI.ID NOT LIKE '[0-9]%'
         GROUP BY LOC.PALogicalLOC, LOC.LOC
         HAVING SUM( LLI.QTY - LLI.QTYPicked) > 0 
         ORDER BY SUM( LLI.QTY - LLI.QTYPicked), LOC.PALogicalLOC, LOC.LOC
   ELSE   
      SET @curLOC = CURSOR FOR
         SELECT TOP 5
             LOC.LOC, SUM( LLI.QTY - LLI.QTYPicked)
