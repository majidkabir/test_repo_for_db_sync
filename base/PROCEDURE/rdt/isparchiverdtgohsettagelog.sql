...
GO
/************************************************************************/  
/* Stor Proc: ispArchiveRDTGOHSettageLog                                */  
/* Copyright: IDS                                                       */  
/*                                                                      */  
/* Purpose: Archive RDTGOHSettageLog table                              */  
/*                                                                      */  
/* Updates:                                                             */  
...
CREATE PROC [RDT].[ispArchiveRDTGOHSettageLog]  
...
AS  
   ...
   BEGIN  
      SET @n_Err = 60751  
      SET @c_ErrMsg = 'ArchiveKey does not exist (ispArchiveRDTGOHSettageLog)'  
      GOTO Fail  
   END  
   ...
   BEGIN  
      SET @n_Err = 60752  
      SET @c_ErrMsg = 'Target database does not exist (ispArchiveRDTGOHSettageLog)'  
      GOTO Fail  
   END  
  
   -- Log archive starts  
   SET @cAlertMessage = 'Archiving RDTGOHSettageLog started with parameters' +   
   ...
   SET @b_success = 1  
   EXECUTE nspLogAlert  
      @c_ModuleName   = 'ispArchiveRDTGOHSettageLog',  
   ...
   -- Candidate rdtGOHSettageLog.RowRef for archive  
...
*/  
   ...
   -- Archivable record in RDTGOHSettageLog, base on RowRef  
   OPEN RowRef_CUR    
   ...
   BEGIN  
      BEGIN TRAN  
      UPDATE RDT.RDTGOHSettageLog WITH (ROWLOCK) SET  
         ArchiveCop = '9'  
      FROM RDT.RDTGOHSettageLog  
      WHERE RowRef = @nRowRef  
      IF @@ERROR <> 0  
      BEGIN  
         SET @n_Err = 60755  
         SET @c_ErrMsg = 'Stamp ArchiveCop fail (ispArchiveRDTGOHSettageLog)'  
         GOTO RollBackTran  
      END  
      COMMIT TRAN  
   ...
   IF @cCopyRowsToArchiveDatabase = 'Y'  
      BEGIN  
         -- Move RDTGOHSettageLog  
         SET @b_success = 1  
         EXECUTE RDT.nsp_Build_Insert  
            @cCopyToDB,   
            'RDTGOHSettageLog', -- @c_tablename  
            1,  
            @b_success OUTPUT,   
            @n_Err     OUTPUT,   
            @c_ErrMsg  OUTPUT  
   ...
   EXECUTE nspLogAlert  
      @c_ModuleName   = 'ispArchiveRDTGOHSettageLog',  
      @c_AlertMessage = 'Archiving RDTGOHSettageLog ended successfully',  
      @n_Severity     = 0,  
      @b_success      = @b_success OUTPUT,  
      @n_Err          = @n_Err     OUTPUT,  
      @c_ErrMsg       = @c_ErrMsg  OUTPUT  
...
Fail:  
   ...
   -- Log the error  
   SET @cAlertMessage = 'Archiving RDTGOHSettageLog failed. ErrNo=' + CAST( @n_Err AS NVARCHAR( 10)) + ' ErrMsg=' + @c_ErrMsg  
   EXECUTE nspLogAlert  
      @c_ModuleName   = 'ispArchiveRDTGOHSettageLog',  
      @c_AlertMessage = @cAlertMessage,   
      @n_Severity     = 0,  
