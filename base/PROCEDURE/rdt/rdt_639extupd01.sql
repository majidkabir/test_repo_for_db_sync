...
GO
/************************************************************************/  
/* Store procedure: rdt_639ExtUpd01                                     */  
...
/************************************************************************/  
CREATE PROCEDURE [RDT].[rdt_639ExtUpd01]  
   @nMobile         INT, 
   @nFunc           INT, 
...
BEGIN  
   ...
   SAVE TRAN rdt_639ExtUpd01 -- For rollback or commit only our own transaction
   ...
   BEGIN  
      ...
      BEGIN
         ...
         SELECT @cToLottable08 = 
               CASE WHEN CountryOfOrigin IN ('','0','99') THEN 'ZZ' 
               ELSE CountryOfOrigin END,
               @cPackkey = PACKKey
         FROM dbo.SKU WITH (NOLOCK)
         WHERE StorerKey = @cStorerKey
         AND   SKU = @cSKU
         
         SET @cToLottable09 = @cToLottable01
         ...
         BEGIN
            ...
            BEGIN
               ...
               BEGIN
                  ...
                  VALUES 
                     (@cTransferKey, @cStorerkey, @cStorerkey, 'MV2UCC', 'RDTMv2UCC', 'rdt_639ExtUpd01', @cFacility, @cFacility)
   ...
   RollBackTran:
      ROLLBACK TRAN rdt_639ExtUpd01 -- Only rollback change made here
   Quit:
