...
BEGIN
   ...
   SELECT @cLastLOCAisle = LOCAisle FROM LOC WITH (NOLOCK) WHERE LOC = @cLastLoc  
         ...
         SET @curRPTask = CURSOR LOCAL READ_ONLY FAST_FORWARD FOR
            ...
            WHERE TaskDetail.TaskType IN ('RPF')
               ...
               AND NOT EXISTS ( SELECT 1 FROM dbo.TaskManagerSkipTasks ST WITH (NOLOCK) 
                                WHERE ST.TaskDetailKey = TaskDetail.TaskDetailKey)
               ...
               --AND PAZone2.InLoc = CASE WHEN @cFinalPAZoneInLOC <> '' THEN @cFinalPAZoneInLOC ELSE PAZone2.InLoc END
            ...
            ORDER BY TaskDetail.Priority, 
                     TaskDetail.Message02,
                     LOC1.LogicalLocation, 
                     LOC1.LOC,
                     TaskDetail.Message01
