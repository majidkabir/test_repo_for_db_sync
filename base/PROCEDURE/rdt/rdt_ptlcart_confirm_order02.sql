...
/************************************************************************/  
/* Store procedure: rdt_PTLCart_Confirm_Order02                         */  
...
CREATE    PROC [RDT].[rdt_PTLCart_Confirm_Order02] (  
    @nMobile    INT  
   ,@nFunc      INT  
   ...
   ***********************************************************************************************/  
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         SAVE TRAN rdt_PTLCart_Confirm_Order02 -- For rollback or commit only our own transaction  
         ...
         BEGIN  
                        ...
                        BEGIN  
                         ...
                         BEGIN  
                          ...
                          EXECUTE rdt.rdt_Move  
                                 ...
                                 @cSourceType  = 'rdt_PTLCart_Confirm_Order02',  
                                 @cStorerKey   = @cStorerKey,  
                                 @cFacility    = @cFacility,  
                                 @cFromLOC     = @cLoc,  
                                 @cToLOC       = @cToLOC,  
            ...
            BEGIN  
               ...
               BEGIN  
                  ...
                  DELETE RDT.rdtPTLCartLog 
                  WHERE OrderKey = @cOrderKey 

                  IF @@ERROR <> 0  
                  BEGIN  
                     SET @nErrNo = 201581  
                     SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') -- 1DelPTlCartlog  
                     GOTO RollBackTran  
                  END  
               END  
         ...
         -- Commit order level  
         COMMIT TRAN rdt_PTLCart_Confirm_Order02  
         ...
         BEGIN  
            ...
            BEGIN  
               ...
               BEGIN  
                  ...
                  BEGIN  
                     SET @nErrNo = 201582  
                     SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') -- DelPTlCartlog  
                     GOTO RollBackTran  
                  END  
               END  
...
RollBackTran:  
   ROLLBACK TRAN rdt_PTLCart_Confirm_Order02 -- Only rollback change made here  
Quit:  
   WHILE @@TRANCOUNT > @nTranCount -- Commit until the level we started  
      COMMIT TRAN  
END 
