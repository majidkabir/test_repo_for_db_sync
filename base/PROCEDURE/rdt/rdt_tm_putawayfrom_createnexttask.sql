...
/************************************************************************/
/* Store procedure: rdt_TM_PutawayFrom_CreateNextTask                   */
/*                                                                      */
...
CREATE PROC [RDT].[rdt_TM_PutawayFrom_CreateNextTask] (
   @nMobile        INT,
   @nFunc          INT,
...
BEGIN
   ...
   SAVE TRAN rdt_TM_PutawayFrom_CreateTask -- For rollback or commit only our own transaction
   ...
   BEGIN
      ...
      BEGIN
         ...
         VALUES (
            @cNewTaskDetailKey, 'PAT', '0', '', @cToLOC, @cToID, @cFinalLOC, @cFinalID, @cUCC, 
   ...
   BEGIN
      ...
      INSERT INTO TaskDetail (
         ...
         PickMethod, StorerKey, ListKey, TransitCount, SourceType, Priority, SourcePriority, TrafficCop)
      VALUES (
         @cNewTaskDetailKey, 'PA1', '0', '', @cToLOC, @cToID, @cFinalLOC, @cToID, 
         @cPickMethod, @cStorerKey, @cListKey, @nTransitCount, @cSourceType, @cPriority, @cSourcePriority, NULL)
      IF @@ERROR <> 0
      BEGIN
         SET @nErrNo = 80154
   ...
   COMMIT TRAN rdt_TM_PutawayFrom_CreateTask -- Only commit change made here
   GOTO Quit

RollBackTran:
   ROLLBACK TRAN rdt_TM_PutawayFrom_CreateTask -- Only rollback change made here
Fail:
Quit:
