...
/************************************************************************/
/* Store procedure: rdt_1764ExtUpd                                      */
/* Purpose: TM Replen From, Extended Update for HK ANF                  */
/*                                                                      */
/* Modifications log:                                                   */
/*                                                                      */
/* Date         Author    Ver.  Purposes                                */
/* 2016-01-11   Ung       1.0   SOS359988 Created                       */
/* 2016-07-21   Ung       1.1   Add SKIP/CANCEL for task with transit   */
/* 2016-08-26   Ung       1.2   Fix last task SKIP/CANCEL               */
/* 2017-05-02   James     1.3   Bug fix (james01)                       */
/* 2019-09-23   LZG       1.4   INC0854719 - Disallowed bypass of       */
/*                              PickDetail trigger (ZG01)               */
/* 2020-04-15   James     1.5   WMS-12480 Add update pickdetail for     */
/*                              pre-cartonize (james02)                 */
/************************************************************************/

CREATE PROCEDURE [RDT].[rdt_1764ExtUpd]
    @nMobile         INT
   ,@nFunc           INT
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            COMMIT TRAN rdt_1764ExtUpd -- Only commit change made here
         END
      END
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               IF @cUOM = '2' AND @cTaskType = 'RPF' AND @cDispatchPiecePickMethod = 'I'
               BEGIN
                  -- Get LabelNo
                  SET @cLabelNo = ''  
                  EXEC isp_GLBL08   
                  ...
                  BEGIN
                     ...
                     VALUES  (@cPickSlipNo, 0, @cLabelNo, '00000', @cStorerKey, @cSKU, @nQTY, @cCaseID)
                     IF @@ERROR <> 0
                     BEGIN
                        SET @nErrNo = 59558
                  ...
                  -- Create PackInfo
                  IF NOT EXISTS( SELECT 1 FROM PackInfo WITH (NOLOCK) WHERE PickSlipNo = @cPickSlipNo AND CartonNo = @nCartonNo)
                  BEGIN
                     SELECT
                     ...
                     VALUES( @cPickSlipNo, @nCartonNo, @fWeight, @fCube, 'CTN')
                     IF @@ERROR <> 0
                     BEGIN
                        SET @nErrNo = 59562
               ...
               -- (james02)
               -- Update pickdetail.Status = '5' for pre-cartonize (full case ucc only)
               IF @cUOM = '2'
               BEGIN 
                  -- Loop PickDetail
                  SET @curPD = CURSOR LOCAL READ_ONLY FAST_FORWARD FOR
                     SELECT PickDetailKey, CaseID, PickSlipNo
                     FROM dbo.PickDetail WITH (NOLOCK)
                     WHERE TaskdetailKey = @cTaskKey
                  OPEN @curPD
                  FETCH NEXT FROM @curPD INTO @cPickDetailKey, @cPD_CaseID, @cPkSlipNo
                  WHILE @@FETCH_STATUS = 0
                  BEGIN
                     IF EXISTS ( SELECT 1 FROM dbo.PackDetail WITH (NOLOCK) 
                                 WHERE PickSlipNo = @cPkSlipNo
                                 AND   LabelNo = @cPD_CaseID)
                     BEGIN
                        -- Reset Status
                        UPDATE dbo.PickDetail SET
                            Status = '5'
...
RollBackTran:
   ROLLBACK TRAN rdt_1764ExtUpd -- Only rollback change made here
Fail:
Quit:
