...
GO
/************************************************************************/  
/* Store procedure: rdt_732ExtGetLoc04                                  */  
/* Copyright      : IDS                                                 */  
/*                                                                      */  
/* Purpose: Get next location from CCDetail. For count 2 & 3, skip loc  */  
/*          with no variance.                                           */  
/*          The CC will be complete by two ways below:                  */  
/*          1.	The Counted_Cnt N <> 0 for all LOCs.                     */  
/*          2.	User could pressÆESCÆ to St1 and finish CC               */  
/*                                                                      */  
...
/* 17-10-2018  1.0  James    WMS6702 Created                            */
/************************************************************************/  
  
CREATE PROC [RDT].[rdt_732ExtGetLoc04] (  
   @nMobile                   INT, 
   @nFunc                     INT, 
...
BEGIN  
   ...
   BEGIN
      ...
      BEGIN
         ...
         -- Restart again from 1st uncounted loc
         IF ISNULL( @cSuggestLoc, '') = ''
         BEGIN
            SELECT TOP 1
                     @cSuggestLogiLOC = Loc.CCLogicalLoc
