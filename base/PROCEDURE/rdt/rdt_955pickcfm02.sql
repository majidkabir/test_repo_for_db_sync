...
/************************************************************************/  
/* Store procedure: rdt_955PickCfm02                                    */  
/* Copyright      : IDS                                                 */  
/*                                                                      */  
...
/* 27-10-2021 1.0  Chermain WMS-18009 Created dup rdt_Pick_ConfirmTask) */
/************************************************************************/  
  
CREATE PROCEDURE [RDT].[rdt_955PickCfm02] (  
   @nMobile       INT,   
   @nFunc         INT, 
...
SET CONCAT_NULL_YIELDS_NULL OFF  
   ...
   BEGIN    
      ...
      BEGIN    
         SET @nErrNo = 178052    
         SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --'BadConfirmQTY'    
         GOTO Fail    
      END    
   END    
   ...
   BEGIN    
      SET @nErrNo = 178055    
      SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --'GetPKDtl fail'    
      GOTO Fail    
   END    
...
--  goto fail  
   ...
   */    
   ...
   --Remy not Ucc Storer but when receive will update Ucc.status = 1
   --so outbound need to update status = '6'
