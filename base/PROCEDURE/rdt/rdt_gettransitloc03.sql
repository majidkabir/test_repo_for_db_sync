...
GO
/************************************************************************/
/* Store procedure: rdt_GetTransitLoc03                                 */
/* Copyright      : IDS                                                 */
/*                                                                      */
...
/* 05-09-2018  1.0  ChewKP    WMS-6178 Created                          */
/************************************************************************/

CREATE PROC [RDT].[rdt_GetTransitLoc03] (
...
BEGIN
   ...
   SELECT @cLangCode = DefaultLangCode
   ...
   IF ISNULL(@cFromPAOutLOC,'')  = '' 
   BEGIN
      -- No Setup No Transit Loc -- 
   ...
   BEGIN
      IF @cFromLOCCat = 'PND'
      BEGIN
         -- FromLoc is PnD No TransitLoc needed
         SET @cTransitLOC = @cToLOC 
      END
      ELSE
      BEGIN
         SET @cTransitLOC = @cFromPAOutLOC
      END   
   END
...
--         BEGIN
--            SET @nErrNo = 128901
--            SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --No PDN LOC
--            GOTO Fail
--         END
--      END
--
