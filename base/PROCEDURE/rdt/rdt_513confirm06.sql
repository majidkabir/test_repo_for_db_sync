...
GO
/******************************************************************************/
/* Store procedure: rdt_513Confirm06                                          */
...
/* 2023-07-26   yeekung   1.0   WMS-23132 Created                             */
/******************************************************************************/

CREATE   PROCEDURE [RDT].[rdt_513Confirm06]
    @nMobile         INT 
   ,@nFunc           INT 
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            DECLARE @cFromLottable03 NVARCHAR( 18)
            ...
            SELECT @cFromLottable03 =LOT.Lottable03 
            ...
            IF EXISTS( SELECT 1 FROM LOC WITH (NOLOCK) WHERE LOC = @cFromLOC AND HostWhcode LIKE 'RET%') AND
               EXISTS( SELECT 1 FROM LOC WITH (NOLOCK) WHERE LOC = @cToLOC AND HostWhcode = 'STD') AND 
               @cFromLottable03 ='RET'
            BEGIN
...
RollBackTran:
   ROLLBACK TRAN rdt_513Confirm06 -- Only rollback change made here
Quit:
   WHILE @@TRANCOUNT > @nTranCount -- Commit until the level we started
      COMMIT TRAN
END
