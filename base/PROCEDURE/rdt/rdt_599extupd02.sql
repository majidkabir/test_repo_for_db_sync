...
/************************************************************************/    
/* Store procedure: rdt_599ExtUpd02                                     */    
...
/* 2022-08-15   yeekung     1.0   WMS-20515 Created                     */    
/************************************************************************/    
    
CREATE PROCEDURE [RDT].[rdt_599ExtUpd02]    
   @nMobile        INT,  
   @nFunc          INT,  
...
BEGIN    
   ...
   SAVE TRAN rdt_599ExtUpd02 -- For rollback or commit only our own transaction              
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         BEGIN
            ...
            ELSE IF EXISTS (SELECT 1 FROM Receipt (nolock)
                WHERE receiptkey=@cReceiptkey
                AND storerkey=@cStorerkey
                AND rectype='RTN')
            BEGIN
   ...
   COMMIT TRAN rdt_599ExtUpd02  
  
   GOTO Commit_Tran  
  
   RollBackTran:  
      ROLLBACK TRAN rdt_599ExtUpd02 -- Only rollback change made here  
   Commit_Tran:  
