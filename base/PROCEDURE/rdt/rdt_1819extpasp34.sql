...
/************************************************************************/
/* Store procedure: rdt_1819ExtPASP34                                   */
/*                                                                      */
/* Modifications log:                                                   */
/*                                                                      */
/* Date         Rev  Author      Purposes                               */
/* 05-Feb-2021  1.0  Chermaine   WMS-16169. Created                     */
/************************************************************************/

CREATE PROC [RDT].[rdt_1819ExtPASP34] (
   @nMobile          INT,
   @nFunc            INT,
...
BEGIN  
   ...
   ---- Check if pallet has mix sku in 1 UCC
   --IF EXISTS ( SELECT 1 FROM dbo.LOTxLOCxID WITH (NOLOCK)
   --              LEFT JOIN UCC U WITH (NOLOCK) ON (LLI.StorerKey = U.Storerkey AND LLI.SKU = U.SKU)
   --            WHERE LLI.StorerKey = @cStorerKey
   --            AND   LOC = @cFromLoc 
   --            AND   ID = @cID
   --            GROUP BY U.SKU
   --            HAVING COUNT( DISTINCT U.uccNo) > 1)
   --BEGIN
   --   SET @nErrNo = 163251
   --   SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') -- MIXSKUUCC
   --   GOTO RollBackTran
   --END
   ...
   SAVE TRAN rdt_1819ExtPASP34 -- For rollback or commit only our own transaction  
...
RollBackTran:  
   ROLLBACK TRAN rdt_1819ExtPASP34 -- Only rollback change made here  
Quit:  
