...
/************************************************************************/
/* Copyright: IDS                                                       */
/* Purpose: Inquiry which UCC not yet ship                              */
/*                                                                      */
/* Modifications log:                                                   */
/*                                                                      */
...
CREATE PROC [RDT].[rdtfnc_UCCOutboundInquiry] (
   @nMobile    int,
   @nErrNo     int  OUTPUT,
...
SELECT 
   ...
   @cTotalUCC   = V_String2, 
   @cScanUCC    = V_String3, 
   @nRemainUCC  = CASE WHEN rdt.rdtIsValidQTY( V_String4,  0) = 1 THEN V_String4 ELSE 0 END,
...
IF @nFunc = 572  -- UCC Outbound inquiry
BEGIN
   -- Redirect to respective screen
   IF @nStep = 0 GOTO Step_0   -- Func = UCC Outbound inquiry
   IF @nStep = 1 GOTO Step_1   -- Scn = 686. LoadKey
   IF @nStep = 2 GOTO Step_2   -- Scn = 687. LoadKey, UCC, QTY, Counter
END
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         SET @nErrNo = 62202
         SET @cErrMsg = rdt.rdtgetmessage( 62202, @cLangCode,'DSP') -- Invalid Load
         GOTO Step_1_Fail
      END
      ...
      BEGIN
         ...
         SELECT DISTINCT TOP 1 
            @cUCC = (IsNull( RTRIM(OD.UserDefine01), '') + IsNull( RTRIM(OD.UserDefine02), '')), -- (Vicky01)
            @cLOC = PD.LOC
         FROM dbo.OrderDetail OD (NOLOCK)
            INNER JOIN dbo.PickDetail PD (NOLOCK) ON (OD.OrderKey = PD.OrderKey AND OD.OrderLineNumber = PD.OrderLineNumber)
            LEFT OUTER JOIN rdt.rdtPPA PPA (NOLOCK) ON (OD.LoadKey = PPA.LoadKey 
               AND (IsNull( RTRIM(OD.UserDefine01), '') + IsNull( RTRIM(OD.UserDefine02), '')) = PPA.UCC) -- (Vicky01)
         WHERE OD.LoadKey = @cLoadKey
            AND PPA.UCC IS NULL
         ORDER BY PD.LOC
         
         -- Get QTY
         SELECT @cQTY = IsNULL( SUM( OD.QTYAllocated + OD.QTYPicked), 0)
         FROM dbo.OrderDetail OD (NOLOCK)
         WHERE OD.LoadKey = @cLoadKey
...
BEGIN
   ...
   BEGIN
      ...
      SET @curUCC = CURSOR LOCAL READ_ONLY FAST_FORWARD FOR
         SELECT DISTINCT
            IsNull( RTRIM(OD.UserDefine01), '') + IsNull( RTRIM(OD.UserDefine02), ''),  -- (Vicky01)
            PD.LOC
         FROM dbo.OrderDetail OD (NOLOCK)
            INNER JOIN dbo.PickDetail PD (NOLOCK) ON (OD.OrderKey = PD.OrderKey AND OD.OrderLineNumber = PD.OrderLineNumber)
            LEFT OUTER JOIN rdt.rdtPPA PPA (NOLOCK) ON OD.LoadKey = PPA.LoadKey 
