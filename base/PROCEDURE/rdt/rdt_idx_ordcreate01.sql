...
GO
/************************************************************************/
/* Store procedure: rdt_IDX_OrdCreate01                                 */
/* Copyright      : IDS                                                 */
/*                                                                      */
...
/* 02-Apr-2014 1.0  James       Created                                 */
/************************************************************************/

CREATE PROC [RDT].[rdt_IDX_OrdCreate01] (
   @nMobile                   INT,
   @nFunc                     INT, 
...
--            @cSectionKey         NVARCHAR( 10), 
   ...
   SET @cSectionKey = SUBSTRING( @cLabelNo, 14, 1)
   SET @cUserDefine02 = SUBSTRING( @cLabelNo, 15, 1)
...
--      SET @cUserDefine02 = SUBSTRING( @cLabelNo, 15, 1)
      ...
      SET @cExternOrderKey = ''
      SET @cExternOrderKey = RTRIM( @cDOID) + @cConsigneeKey + SUBSTRING( @cLabelNo, 14,2)
      ...
      IF ISNULL( @cLoadKey, '') = ''
         ...
         AND   SectionKey = SUBSTRING( @cLabelNo, 14, 1) 
         AND   Status < '5'
         AND   StorerKey = @cStorerKey
