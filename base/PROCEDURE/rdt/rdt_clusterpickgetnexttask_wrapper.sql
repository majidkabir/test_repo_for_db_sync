...
/************************************************************************/
/* Store procedure: RDT.RDT_ClusterPickGetNextTask_Wrapper              */
/* Copyright      : IDS                                                 */
/*                                                                      */
...
CREATE PROC [RDT].[RDT_ClusterPickGetNextTask_Wrapper] (
	@n_Mobile				INT, 
   @n_Func					INT,
...
BEGIN
   ...
   IF @c_UserName = '' OR @c_UserName IS NULL
   BEGIN
      SET @b_Success = 0
      SET @n_ErrNo = 72993    
      SET @c_ErrMsg = rdt.rdtgetmessage( @n_ErrNo, @c_LangCode, 'DSP') --'USER NAME Req'
      GOTO QUIT
   END
      
   IF @b_debug = 1
   BEGIN
     SELECT '@c_UserName', @c_UserName
   END
   ...
   BEGIN
      SET @b_Success = 0
      SET @n_ErrNo = 72994    
      SET @c_ErrMsg = rdt.rdtgetmessage( @n_ErrNo, @c_LangCode, 'DSP') --'FACILITY Req'
      GOTO QUIT
   END
   ...
   BEGIN
	   ...
	   SET @cSQLStatement = N'EXEC RDT.' + RTrim(@c_SPName) + 
			 ' @n_Mobile, 				@n_Func, 				@c_Storerkey,' 			+ 
	       ' @c_UserName, 			@c_Facility, 			@c_PutAwayZone,' 			+ 
	       ' @c_PickZone, 			@c_LangCode,' 											+
