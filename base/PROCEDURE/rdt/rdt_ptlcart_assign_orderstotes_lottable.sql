...
/******************************************************************************/
/* Store procedure: rdt_PTLCart_Assign_OrdersTotes_Lottable                   */
/* Copyright      : LFLogistics                                               */
/*                                                                            */
/* Date       Rev  Author   Purposes                                          */
/* 05-01-2018 1.0  Ung      WMS-3549 Created                                  */
/* 26-01-2018 1.1  Ung      Change to PTL.Schema                              */
/* 26-07-2021 1.2  James    WMS-17607 Add PickConfirmStatus                   */  
/*                          Add sourcekey (orderkey) to ptltran (james01)     */  
/******************************************************************************/

CREATE PROC [RDT].[rdt_PTLCart_Assign_OrdersTotes_Lottable] (
   @nMobile          INT,
   @nFunc            INT,
...
BEGIN
   ...
   IF @cPickConfirmStatus NOT IN ('3', '5')  -- 3=Pick in progress    
            ...
            BEGIN
               SET @cSQL = 
                  ' INSERT INTO PTL.PTLTran ( ' + 
                     ...
                     ' DeviceProfileLogKey, DropID, OrderKey, Storerkey, SKU, LOC, ExpectedQTY, QTY, SourceKey, ' + @cGroupBy + ') ' +   
                  ' SELECT ' + 
                     ' @cIPAddress, @cCartID, @cPosition, ''0'', ''CART'', ' + 
                     ' @cDPLKey, '''', @cOrderKey, @cStorerKey, @cSKU, @cLOC, ISNULL( SUM( PD.QTY), 0), 0, @cOrderKey, ' + @cGroupBy +   
                  ' FROM Orders O WITH (NOLOCK) ' + 
                     ' JOIN PickDetail PD WITH (NOLOCK) ON (PD.OrderKey = O.OrderKey) ' + 
               ...
               EXEC sp_ExecuteSQL @cSQL, @cSQLParam, 
                  @cIPAddress, @cCartID, @cPosition, @cDPLKey, @cOrderKey, @cLOC, @cStorerKey, @cSKU, @cPickConfirmStatus, @cGroupBy  
