...
/************************************************************************/
/* Store procedure: rdt_1638ExtVal01                                    */
/* Copyright: LFLogistics                                               */
/*                                                                      */
/* Date        Rev  Author   Purposes                                   */
/* 06-01-2016  1.0  Ung      IN00132118 Created                         */
/************************************************************************/

CREATE PROC [RDT].[rdt_1638ExtVal01] (
   @nMobile      INT,           
   @nFunc        INT,           
   @nStep        INT,
   @nInputKey    INT,           
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               SELECT @cSuggOrderKey = OrderKey FROM PackHeader WITH (NOLOCK) WHERE PickSlipNo = @cSuggPickSlipNo
               SELECT @cSuggConsigneeKey = ConsigneeKey FROM Orders WITH (NOLOCK) WHERE OrderKey = @cSuggOrderKey
               ...
               IF @cSuggConsigneeKey <> @cConsigneeKey
               BEGIN
                  SET @nErrNo = 103251
