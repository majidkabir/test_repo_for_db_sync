...
GO
/************************************************************************/
/* Store procedure: rdt_ClusterPickCfm21                                */
/* Copyright      : IDS                                                 */
/*                                                                      */
/* Purpose:                                                             */
/*                                                                      */
...
CREATE PROC [RDT].[rdt_ClusterPickCfm21] (
   @nMobile                   INT,           
   @nFunc                     INT,           
...
BEGIN
   ...
   SELECT RowRef, DropID, PickQty, ID,PickSlipNo,orderkey
   ...
   Order By RowRef
   OPEN curRPL
   FETCH NEXT FROM curRPL INTO @nRowRef, @cDropID, @nPickQty, @nCartonNo,@cpickslipno,@cOrderkey
         ...
         BEGIN
            ...
            UPDATE dbo.PickDetail WITH (ROWLOCK) SET
               ...
               DropID = CASE WHEN ISNULL(@cLoadDefaultPickMethod, '') = 'C' AND @cStatus = '4' THEN '' 
                            WHEN @nStep = 15 THEN ''
                        ELSE @cDropID END,
               Status = CASE WHEN @nStep = 15  THEN [Status] ELSE '4' END   -- short pick
         ...
         BEGIN
            ...
            BEGIN
               ...
               BEGIN
                  ...
                  SELECT
                     CaseID, PickHeaderKey, OrderKey, OrderLineNumber, Lot, StorerKey, SKU, AltSku, UOM, UOMQTY, QTYMoved,
                     CASE WHEN @cOption='1' and @nStep=9 then '4' ELSE 0 END,
                  ...
                  SELECT WaveKey, LoadKey, Orderkey, OrderLineNumber, StorerKey, PutAwayZone, PickZone, PickDetailKey  
                  , LOT, LOC, Status, AddWho, getdate(), PickSlipNo, Mobile  
                  FROM RDT.RDTPickLock   WITH (NOLOCK)  
                  WHERE RowRef = @nRowRef 
