...
/************************************************************************/
/* Store procedure: rdt_803MatrixSP06                                   */
/* Copyright      : LF Logistics                                        */
/*                                                                      */
/* Date       Rev  Author     Purposes                                  */
/* 17-09-2021 1.0  Chermaine  WMS-17331 Created (dup rdt_803MatrixSP02) */
/************************************************************************/

CREATE PROC [RDT].[rdt_803MatrixSP06] (
    @nMobile    INT
   ,@nFunc      INT
...
BEGIN
   ...
   BEGIN
   	SET @cResult10 = space(20-LEN('STATION END')-1)+('STATION END')
   END
   ELSE
   BEGIN
      ...
      IF NOT EXISTS( SELECT TOP 1 1
         FROM Orders O WITH (NOLOCK) 
            --JOIN rdt.rdtPTLPieceLog L WITH (NOLOCK) ON (O.OrderKey = L.OrderKey)
            JOIN PickDetail PD WITH (NOLOCK) ON (O.OrderKey = PD.OrderKey)
         WHERE PD.Status <= '5'
            --AND L.Station = @cStation
            AND PD.CaseID <> 'Sorted'
            ...
            AND O.SOStatus <> 'CANC')
      BEGIN
         SET @cResult10 = space(20-LEN('POS END')-1)+('POS END')
      END
   END
