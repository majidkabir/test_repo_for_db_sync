...
BEGIN
   ...
   SAVE TRAN rdt_638RcvCfm09 -- For rollback or commit only our own transaction

   SELECT @ctype=userdefine02
   ...
   BEGIN
      ...
      ELSE
      BEGIN
         SELECT @cNewReceiptLineNumber =  --(yeekung01)
         ...
         SELECT
            @cReceiptKey, @cNewReceiptLineNumber,OD.OrderLineNumber,Od.externorderkey,'OK',
         ...
         WHERE OD.OrderKey = @cOrderKey
            AND OD.ShippedQty<>0
            AND OD.sku=@cSKUCode
         GROUP BY OD.OrderLineNumber,OD.OrderLineNumber,Od.externorderkey,
   ...
   COMMIT TRAN rdt_638RcvCfm09
   GOTO Quit

RollBackTran:
   ROLLBACK TRAN rdt_638RcvCfm09
Fail:
Quit:
