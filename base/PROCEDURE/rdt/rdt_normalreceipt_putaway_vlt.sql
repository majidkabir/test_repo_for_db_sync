...
/******************************************************************************/
/* Store procedure: rdt_NormalReceipt_Putaway_VLT                              */
/* Copyright      : Maersk                                                    */
/*                                                                            */
/* Purpose: Putaway for receiving For client Huqsvarna                        */
/*                                                                            */
/* Date         Author    Ver.  Purposes                                      */
/* 2024-09-26   Dennis    1.0   FCR-632 Created                               */
/******************************************************************************/

CREATE   PROCEDURE rdt.rdt_NormalReceipt_Putaway_VLT
   @nMobile       INT,           
   @nFunc         INT,           
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            SET @cSQL = 'EXEC rdt.' + RTRIM( @cExtendedPutawaySP) +
               ' @nMobile, @nFunc, @cLangCode, @nStep, @nInputKey, @cFacility, @cStorerKey, @cType, @cReceiptKey, @cPOKey, @cLOC, @cID, @cSKU, @nQTY, ' +
      ...
      BEGIN
         ...
         SAVE TRAN rdt_NormalReceipt_Putaway_VLT -- For rollback or commit only our own transaction
         ...
         COMMIT TRAN rdt_NormalReceipt_Putaway_VLT -- Only commit change made here
      END
   END
   ...
   BEGIN
      ...
      COMMIT TRAN rdt_NormalReceipt_Putaway_VLT
   END
...
RollBackTran:
   ROLLBACK TRAN rdt_NormalReceipt_Putaway_VLT -- Only rollback change made here
Fail:
Quit:
