...
/************************************************************************/
/* Store procedure: rdt_805MatrixSP08                                   */
/* Copyright      : Maersk                                              */
/*                                                                      */
...
CREATE   PROC [RDT].[rdt_805MatrixSP08] (
    @nMobile    INT
   ,@nFunc      INT
...
BEGIN
   ...
   SET @curPTLTran = CURSOR FOR
      ...
      ORDER BY CAST(D.LogicalPOS AS INT), D.IPAddress, D.DevicePosition
   ...
   BEGIN
      ...
      BEGIN
         ...
         VALUES ( @nPTLKey, @cStation1 , @cIPAddress , @cPosition, @cLoc , @nQTY)
   ...
   SET @curLightPos = CURSOR FOR
      SELECT PTL.PTLKey, PTL.Station, PTL.IPAddress, PTL.Position, PTL.Loc, PTL.QTY
      FROM @tPos PTL
      WHERE Qty > 0 
      ORDER BY Seq
   OPEN @curLightPos
   ...
   SELECT TOP 1 @cIPAddress = PTL.IPAddress, 
