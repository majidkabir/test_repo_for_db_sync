...
/************************************************************************/
/* Store procedure: rdt_TM_Move_CreateNextTask                          */
/* Copyright      : IDS                                                 */
/*                                                                      */
...
CREATE PROC [RDT].[rdt_TM_Move_CreateNextTask] (
   @nMobile        INT,
   @nFunc          INT,
...
BEGIN
   ...
   SELECT TOP 1 
      ...
      @cSourceType     = 'rdt_TM_Move_CreateNextTask'
   ...
   SAVE TRAN rdt_TM_Move_CreateNextTask -- For rollback or commit only our own transaction
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            INSERT INTO TaskDetail (
               TaskDetailKey, TaskType, Status, UserKey, FromLOC, FromID, ToLOC, ToID, QTY, 
               PickMethod, StorerKey, SKU, LOT, ListKey, TransitCount, SourceType, WaveKey, Priority, SourcePriority, TrafficCop)
            VALUES (
               @cNewTaskDetailKey, 'MVT', '0', '', @cToLOC, @cToID, @cFinalLOC, @cFinalID, @nQTY, 
               'PP', @cStorerKey, @cSKU, @cLOT, @cListKey, @nTransitCount, @cSourceType, @cWaveKey, @cPriority, @cSourcePriority, NULL)
            IF @@ERROR <> 0
            BEGIN
               SET @nErrNo = 87553
      ...
      BEGIN
         ...
         INSERT INTO TaskDetail (
            ...
            PickMethod, StorerKey, SKU, LOT, ListKey, TransitCount, SourceType, WaveKey, Priority, SourcePriority, TrafficCop)
         VALUES (
            @cNewTaskDetailKey, 'MV1', '0', '', @cToLOC, @cToID, @cFinalLOC, @cFinalID, 0, 
   ...
   BEGIN 
      ...
      INSERT INTO TaskDetail (
         ...
         PickMethod, Storerkey, SKU, LOT, ListKey, TransitCount, SourceType, WaveKey, Priority, SourcePriority, TrafficCop)
      VALUES (
         @cNewTaskDetailKey, 'MV1', '0', '', @cToLOC, @cToID, @cTransitLOC, @cToID, 0, 
         'FP', @cStorerkey, '', '', @cListKey, @nTransitCount, @cSourceType, @cWaveKey, @cPriority, @cSourcePriority, NULL)
      IF @@ERROR <> 0
      BEGIN
         SET @nErrNo = 87555
   ...
   COMMIT TRAN rdt_TM_Move_CreateNextTask -- Only commit change made here
   GOTO Quit

RollBackTran:
   ROLLBACK TRAN rdt_TM_Move_CreateNextTask -- Only rollback change made here
Fail:
Quit:
