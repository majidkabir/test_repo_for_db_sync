...
/************************************************************************/
/* Store procedure: rdt_PTLPiece_Confirm_Order03                        */
/* Copyright      : LF Logistics                                        */
/*                                                                      */
/* Purpose: Confirm by order                                            */
/*                                                                      */
/* Date       Rev  Author      Purposes                                 */
/* 12-10-2020 1.0  YeeKung     WMS-15181 Created                        */
/************************************************************************/

CREATE PROC [RDT].[rdt_PTLPiece_Confirm_Order03] (
    @nMobile      INT
   ,@nFunc        INT
...
BEGIN
   ...
   -- Get assign info
   SET @cLoadkey = ''
   SELECT @cLoadkey = loadkey FROM rdt.rdtPTLPieceLog WITH (NOLOCK) WHERE Station = @cStation 
   IF @cLoadkey = ''
      SELECT @cLoadkey = V_LoadKey FROM rdt.rdtMobRec WITH (NOLOCK) WHERE Mobile = @nMobile
   ...
   FROM LoadPlandetail LP WITH (NOLOCK)
      JOIN Orders O WITH (NOLOCK) ON (O.OrderKey = LP.OrderKey)
      JOIN PickDetail PD WITH (NOLOCK) ON (O.OrderKey = PD.OrderKey)
      LEFT JOIN rdt.rdtPTLPieceLog L WITH (NOLOCK) ON (L.OrderKey = O.OrderKey AND LP.loadkey=L.loadkey AND L.Station = @cStation)
   ...
   BEGIN
      ...
      BEGIN
         ...
         -- Save assign
         INSERT rdt.rdtPTLPieceLog (Station, IPAddress, Position, loadkey, OrderKey, CartonID)
