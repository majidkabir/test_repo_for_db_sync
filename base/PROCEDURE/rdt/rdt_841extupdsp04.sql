...
/************************************************************************/    
/* Store procedure: rdt_841ExtUpdSP04                                   */    
/* Copyright      : LF                                                  */    
/*                                                                      */    
...
/* 2016-11-30  1.7  ChewKP   WMS-731 - Update TrackingNo to             */  
/*                           Orders.TrackingNo, create new SP (ChewKP05)*/  
/* 2017-06-30  1.8  ChewKP   WMS-2186 Print by codelkup Setup (CheWKP06)*/  
/* 2018-07-27  1.9  ChewKP   Performance Tuning (ChewKP07)              */  
/* 2020-05-19  2.0  YeeKung  WMS-13131 Add Cartontype param(yeekung01)  */   
/* 2020-09-05  2.1  James    WMS-15010 Add AutoMBOLPack (james02)       */
/* 2021-04-01  2.2  YeeKung  WMS-16718 Add serialno and serialqty       */
/*                           Params (yeekung02)                         */
/* 2021-07-27  2.3  Chermain WMS-17410 Add VariableTable Param (cc01)   */
/************************************************************************/    
    
CREATE PROC [RDT].[rdt_841ExtUpdSP04] (    
   @nMobile       INT,    
   @nFunc         INT,    
...
--         WHERE PH.Orderkey = @cOrderkey    
         ...
         BEGIN    
            -- (ChewKP07)  
            DECLARE C_PICK_DETAIL CURSOR LOCAL FAST_FORWARD READ_ONLY FOR    
            ...
            OPEN C_PICK_DETAIL    
            FETCH NEXT FROM C_PICK_DETAIL INTO  @cPickDetailKey  
            WHILE (@@FETCH_STATUS <> -1)    
            BEGIN    
...
--         END    
         ...
         ****************************/    
         ...
         BEGIN    
            ...
            SET Status = '9'    
               ,EditDate = GetDate() -- (ChewKP07)   
               ,EditWho = @cUserName -- (ChewKP07)   
            WHERE PickSlipNo = @cPickSlipNo    
            AND StorerKey = @cStorerKey    
            ...
            BEGIN  
               SET @nErrNo = 108180    
               SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --'RPTCfgNotSet'    
               GOTO RollBackTran   
            END  
            ...
            --VALUES ( 'rdt_841ExtUpdSP04', GetDate() , '1' , @cOrderKey, @cOrderType )    
            ...
            BEGIN    
               ...
               --INSERT INTO TRACEINFO (TraceName, TimeIn, Step1 , Step2, Step3 )    
               --VALUES ( 'rdt_841ExtUpdSP04', GetDate() , '1.1' , @cOrderKey, @cSOStatus )    
...
--                           AND SOStatus <> '0' )    
--               BEGIN    
                  --INSERT INTO TRACEINFO (TraceName, TimeIn, Step1 , Step2, Step3, Step4 )                    --VALUES ( 'rdt_841ExtUpdSP04', GetDate() , '1.2' , @cOrderKey, @cSOStatus, @cStorerKey )    
...
--                  VALUES ( 'rdt_841ExtUpdSP04', GetDate() , '1.3' , @cOrderKey, @cSOStatus, @cStorerKey )    
--               END    
            END    
...
RollBackTran:        
   ROLLBACK TRAN rdt_841ExtUpdSP04 -- Only rollback change made here        
        
Quit:        
   WHILE @@TRANCOUNT > @nTranCount -- Commit until the level we started        
      COMMIT TRAN rdt_841ExtUpdSP04        
