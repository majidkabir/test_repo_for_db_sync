...
/************************************************************************/    
/* Store procedure: rdt_842ExtUpdSP09                                   */    
/* Copyright      : LF                                                  */    
/*                                                                      */    
/* Purpose: Nike ecom packing (capture packinfo)                        */    
/*                                                                      */    
/* Modifications log:                                                   */    
/* Date        Rev  Author   Purposes                                   */    
/* 2021-01-07  1.0  ChewKP   WMS-15880. Created                         */   
/* 2021-03-18  1.1  YeeKung  Remove double loop (yeekung01)             */
/* 2021-04-16  1.2  James    WMS-16024 Standarized use of TrackingNo    */  
/*                           (james01)                                  */ 
/* 2021-03-20  1.3  YeeKung  WMS-16589 change print from step 3         */
/*                           to step 2 (yeekung02)                      */
/************************************************************************/    
    
CREATE PROC [RDT].[rdt_842ExtUpdSP09] (    
   @nMobile        INT,                
   @nFunc          INT,                
...
BEGIN    
   ...
   DECLARE @tDatawindow AS VariableTable  
...
--      AND PH.PickHeaderKey = @cPickSlipNo   
   ...
   IF @nStep = 2 OR @nStep = 7   -- Serial no      
   BEGIN    
...
--      AND    AddWho = @cUserName    
      ...
      --                 AND Trackingno = @cTrackingNo  
                        ...
                        BEGIN  
                        SET @nErrNo = 161885       
                        SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --'InsCtnTrackFail'    
                        GOTO RollBackTran   
                        END  
...
--                  END  
      ...
      BEGIN  
         ...
         BEGIN  
            ...
            VALUES (@cPickSlipNo, @nCartonNo, @cLabelNo, @cLabelLine, @cStorerKey, @cSKU, @cSerialNo, 1)  
            IF @@ERROR <> 0  
            BEGIN  
               SET @nErrNo = 161893  
               ...
               BEGIN    
                  ...
                  BEGIN    
                     ...
                     BEGIN  
                        ...
                        SET @cWinPrinterName = @cPrinterName         -- ZG01  
                     END  
                     ELSE  
                     BEGIN    
                  ...
                  IF @cProcessType = 'QCOMMANDER' OR @cProcessType = 'TCPSPOOLER'  
                  BEGIN    
                     IF ISNULL( @cFilePath, '') <> ''  -- For pdf printing      
                     BEGIN        
                     ...
                     BEGIN    
                        ...
                        --('842', getdate(), @cStorerKey, @cOrderKey4Printing, @cTrackingNo, @cLabelNo, @nCartonNo, @cReportType)    
                        DELETE FROM @tDatawindow    
                        INSERT INTO @tDatawindow (Variable, Value) VALUES ( '@cStorerKey', @cStorerKey)    
                        INSERT INTO @tDatawindow (Variable, Value) VALUES ( '@cOrderKey', @cOrderKey)     
                        INSERT INTO @tDatawindow (Variable, Value) VALUES ( '@cTrackingNo', @cTrackingNo)     
                        INSERT INTO @tDatawindow (Variable, Value) VALUES ( '@cLabelNo', @cLabelNo)    
                        INSERT INTO @tDatawindow (Variable, Value) VALUES ( '@nCartonNo', @nCartonNo)    
   ...
   BEGIN  
      ...
      ****************************/    
      ...
      BEGIN  
         ...
         --VALUES ( 'rdt_842ExtUpdSP09' , Getdate() , @cDropID , @nTotalPickedQty , @nTotalScannedQty , @cUserName ,@cDropIDType )   
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         BEGIN    
            ...
            IF @cProcessType NOT IN ('QCOMMANDER', 'TCPSPOOLER')  
            BEGIN  
               DELETE FROM @tSHIPLabel    
               INSERT INTO @tSHIPLabel (Variable, Value) VALUES ( '@cStorerKey', @cStorerKey)    
               INSERT INTO @tSHIPLabel (Variable, Value) VALUES ( '@cOrderKey', @cOrderKey)     
         ...
         ('12345', GETDATE(), @cPickSlipNo, @nCartonNo + 1, @cLabelNo)  
         */  
  
         DECLARE @cTablename        NVARCHAR( 10)  
         DECLARE @cCommand          NVARCHAR(1000)=''    
         DECLARE @cTransmitlogKey   NVARCHAR(10)  =''    
         DECLARE @cIP               VARCHAR(20)   =''    
         DECLARE @cPort             VARCHAR(10)   =''    
         DECLARE @nThreadPerAcct    INT           =0    
         DECLARE @nMilisecondDelay  INT           =0    
         DECLARE @cAPP_DB_Name      VARCHAR(10)   =''    
         DECLARE @nThreadPerStream  INT           =0    
         DECLARE @cIniFilePath      NVARCHAR(200) =''    
         DECLARE @cDataStream       VARCHAR(10)   ='4939'    
         DECLARE @cCartonNo         NVARCHAR( 10)  
           
         -- Send ITF to get Tracking # for Next CartonNo    
         SET @cTablename = 'WSTRACKLOG'    
         SET @cCartonNo = CONVERT(NVARCHAR( 5), @nCartonNo + 1)    
       
         SET @cTransmitlogKey = ''    
         EXECUTE nspg_getkey      
            @KeyName = 'TransmitlogKey2'      
           ,@fieldlength = 10      
           ,@keystring = @cTransmitlogKey    OUTPUT      
         ...
         VALUES (@cTransmitlogKey, @cTableName, @cOrderKey, @cCartonNo, @cStorerKey, '0', '')     
       
         IF @@ERROR <> 0              
         BEGIN              
            SET @nErrNo = 161890    
            SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --'InsTL2Fail'    
            GOTO RollBackTran    
         END     
         ...
         BEGIN CATCH      
            SET @nErrNo = 161891    
            SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --'SubmitQTaskFail'    
            GOTO RollBackTran    
         END CATCH     
       
         IF @nErrNo <> 0 AND ISNULL(@cErrMsg,'') <> ''        
         BEGIN    
            SET @nErrNo = 161892    
            SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --'GetNewTrackNoEr'    
            GOTO RollBackTran    
         END       
         -- End of get new tracking now  
...
RollBackTran:        
   ROLLBACK TRAN rdt_842ExtUpdSP09 -- Only rollback change made here        
        
Quit:        
   WHILE @@TRANCOUNT > @nTranCount -- Commit until the level we started        
      COMMIT TRAN rdt_842ExtUpdSP09        
