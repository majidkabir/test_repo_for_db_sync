...
/************************************************************************/  
/* Store procedure: rdt_PTLPiece_Confirm_Order14                        */  
/* Copyright      : LF Logistics                                        */  
/*                                                                      */  
/* Purpose: Confirm by order                                            */  
/*                                                                      */  
/* Date       Rev  Author      Purposes                                 */  
/* 2022-07-01 1.0  James       WMS-20016. Created                       */  
/************************************************************************/  
  
CREATE PROC [RDT].[rdt_PTLPiece_Confirm_Order14] (  
    @nMobile      INT  
   ,@nFunc        INT  
...
BEGIN  
   ...
   FROM WaveDetail WD WITH (NOLOCK)  
      ...
      JOIN rdt.rdtPTLPieceLog L WITH (NOLOCK) ON 
         (L.OrderKey = O.OrderKey AND L.WaveKey = @cWaveKey AND L.Station = @cStation AND PD.DropID = L.SourceKey)  
   ...
   IF NOT EXISTS ( SELECT 1 
                   ...
                   AND   Sku = @cSKU)
   BEGIN  
   	INSERT INTO TRACEINFO(TraceName, TimeIn, Col1, Col2, Col3) VALUES ('803', GETDATE(), @cOrderKey, @cDropID, @cSKU)
      SET @nErrNo = 188065  
