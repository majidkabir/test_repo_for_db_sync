...
CREATE   PROC [RDT].[rdtSetMobColRetActionHttp] (
   @nMobile    INT,
...
-- Get SoundLevel
SET @cSoundLevel = ''
SET @iStart = CHARINDEX( 'soundlevel="', @cInMessage)
IF @iStart > 0
BEGIN
   SET @iStart = @iStart + LEN( 'soundlevel="')
   SET @iLength = CHARINDEX( '"', SUBSTRING( @cInMessage, @iStart, LEN( @cInMessage)))
   SET @cSoundLevel = SUBSTRING( @cInMessage, @iStart, ABS( @iLength - 1))
END

-- Get VibrationLevel
SET @cVibrationLevel = ''
SET @iStart = CHARINDEX( 'vibrationlevel="', @cInMessage)
IF @iStart > 0
BEGIN
   SET @iStart = @iStart + LEN( 'vibrationlevel="')
   SET @iLength = CHARINDEX( '"', SUBSTRING( @cInMessage, @iStart, LEN( @cInMessage)))
   SET @cVibrationLevel = SUBSTRING( @cInMessage, @iStart, ABS( @iLength - 1))
END

-- New sound or vibration setting
IF @cSoundLevel <> '' OR @cVibrationLevel <> ''
BEGIN
   DECLARE @cUserName NVARCHAR( 128) = ''
   SELECT @cUserName = UserName FROM rdt.rdtMobRec WITH (NOLOCK) WHERE Mobile = @nMobile

   -- Login screen might not have user
   IF @cUserName <> ''
	   -- Update the user record
	   UPDATE rdt.RDTUser SET
		   SoundLevel     = CASE WHEN @cSoundLevel     = '' THEN SoundLevel     ELSE @cSoundLevel     END,
		   VibrationLevel = CASE WHEN @cVibrationLevel = '' THEN VibrationLevel ELSE @cVibrationLevel END
