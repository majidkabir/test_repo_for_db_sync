...
/******************************************************************************/
/* Store procedure: rdt_1580ExtUpd08                                          */
/* Copyright      : LF logistics                                              */
/*                                                                            */
/* Purpose: Update ReceiptDetail.UserDefine01 as serial no                    */
/*                                                                            */
/* Date        Rev  Author      Purposes                                      */
/* 03-05-2018  1.0  ChewKP      WMS-7780 Created                              */
/******************************************************************************/

CREATE PROCEDURE [RDT].[rdt_1580ExtUpd08]
    @nMobile      INT
   ,@nFunc        INT
   ,@nStep        INT
   ,@nInputKey    INT
...
BEGIN
  ...
  DECLARE @tCartonList TABLE (RowRef      INT IDENTITY(1,1)
   ...
   SAVE TRAN rdt_1580ExtUpd08 -- For rollback or commit only our own transaction
...
--                     SET @nErrNo = @@ERROR
   ...
   --                
            ...
            --COMMIT TRAN rdt_1580ExtUpd08 -- Only commit change made here
         END
      END
   END
   GOTO Quit
...
RollBackTran:
   ROLLBACK TRAN rdt_1580ExtUpd08 -- Only rollback change made here
Quit:
   WHILE @@TRANCOUNT > @nTranCount -- Commit until the level we started
      COMMIT TRAN rdt_1580ExtUpd08
