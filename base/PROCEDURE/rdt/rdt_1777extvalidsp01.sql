...
/************************************************************************/    
/* Store procedure: rdt_1777ExtValidSP01                                */    
...
CREATE PROC [RDT].[rdt_1777ExtValidSP01] (    
   @nMobile     INT,    
   @nFunc       INT,     
...
BEGIN    
    ...
    -- Update WCSRouting and WCSRouting Detail Status   
      
--    UPDATE dbo.WCSRouting   
--    SET Status = '9'  
--       , Final_Zone = 'QC01' -- Close by Tote Conveyor Move  
...
--    BEGIN  
--         SET @nErrNo = 87853  
--         SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --'UpdWCSRoutingFail'  
--         GOTO QUIT  
--    END  
--      
--    UPDATE WD  
--      SET Status = '9'  
--    FROM dbo.WCSRoutingDetail WD  
--    INNER JOIN dbo.WCSRouting W WITH (NOLOCK) ON W.WCSKey = WD.WCSKey  
--    WHERE WD.ToteNo = @cDropID  
--    AND WD.Status = '0'  
...
--    BEGIN  
--         SET @nErrNo = 87854  
--         SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --'UpdWCSRoutingDetFail'  
--         GOTO QUIT  
--    END  
