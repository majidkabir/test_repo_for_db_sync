...
/************************************************************************/
/* Store procedure: rdt_PTLStation_Confirm_WaveCriteria                 */
/* Copyright      : LF Logistics                                        */
/*                                                                      */
/* Purpose: Close working batch                                         */
/*                                                                      */
/* Date       Rev  Author      Purposes                                 */
/* 25-05-2015 1.0  Ung         SOS361967 Created                        */
/* 24-09-2018 1.1  James       Remove orderdetail.loadkey (james01)     */  
/* 24-06-2020 1.2  James       WMS-13639 Add config to decide carton id */  
/*                             carton id update to which column(james02)*/
/************************************************************************/

CREATE PROC [RDT].[rdt_PTLStation_Confirm_WaveCriteria] (
    @nMobile      INT
   ,@nFunc        INT
            ...
            BEGIN
               ...
               BEGIN -- Have balance, need to split
                  ...
                  SELECT 
                     ...
                     IPAddress, DeviceID, DevicePosition, Status, LightUp, LightMode, LightSequence, PTLType, SourceKey, DropID, CaseID, RefPTLKey, 
                     Storerkey, OrderKey, ConsigneeKey, SKU, LOC, LOT, UOM, Remarks, Func, GroupKey, 'rdt_PTLStation_Confirm_WaveCriteria', ArchiveCop
                  FROM PTL.PTLTran WITH (NOLOCK) 
         			WHERE PTLKey = @nPTLKey			            
                  IF @@ERROR <> 0
                  BEGIN
