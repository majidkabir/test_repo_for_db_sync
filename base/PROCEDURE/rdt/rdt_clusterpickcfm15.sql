...
/************************************************************************/    
/* Store procedure: rdt_ClusterPickCfm15                                */    
/* Copyright      : IDS                                                 */    
/*                                                                      */    
...
/* 2020-05-10  1.0  James       WMS-12223. Created                      */   
/* 2020-11-16  1.1  LZG         Retrieve by WaveKey (ZG01)              */   
/* 2020-12-04  1.2  James       Update PickDetail.QtyMoved = 1 when     */  
/*                              pick confirm (james02)                  */  
/* 2021-01-14  1.3  LZG         Temp trace for No Lot swap issue (ZG01) */
...
BEGIN    
   ...
   -- (james04)
   SET @cProcessShortPickReAllocate = rdt.RDTGetConfig( @nFunc, 'ProcessShortPickReAllocate', @cStorerKey)
   ...
   BEGIN TRAN    
   SAVE TRAN rdt_ClusterPickCfm15    
   ...
   -- If exists dropid then check if allow reuse dropid. If allow then go on.    
      ...
      BEGIN    
         ...
         -- (james04)
         IF @cProcessShortPickReAllocate = '1'
         BEGIN
            -- Get spooler info      
            SELECT      
               @cIPAddress = [IP],      
   ...
   RollBackTran:    
      ROLLBACK TRAN rdt_ClusterPickCfm15    
    
   Quit:    
      WHILE @@TRANCOUNT > @nTranCount -- Commit until the level we started    
         COMMIT TRAN rdt_ClusterPickCfm15    
END    
