...
/************************************************************************/
CREATE   PROCEDURE [RDT].[rdt_600Lot2VLT]  
   @nMobile      INT,
   @nFunc        INT,
...
BEGIN  
   ...
   BEGIN  
      SET @cLottable02 = (SELECT TOP 1 Lottable02 FROM dbo.RECEIPTDETAIL rd WITH(NOLOCK)  
         ...
         AND DateReceived = (SELECT max(DateReceived) FROM dbo.RECEIPTDETAIL rd2 WITH(NOLOCK) WHERE rd.ReceiptKey = rd2.ReceiptKey AND StorerKey = @cStorerKey))  
   END  
END -- End Procedure  
