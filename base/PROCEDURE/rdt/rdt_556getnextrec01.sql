...
/************************************************************************/
/* Store procedure: rdt_556GetNextRec01                                 */
/*                                                                      */
/* Purpose: Get next SKU and/or LOC                                     */
/*                                                                      */
/* Called from: rdt_SKUInquiry_GetNext                                  */
/*                                                                      */
/* Modifications log:                                                   */
/*                                                                      */
/* Date       Rev  Author     Purposes                                  */
/* 2022-01-05 1.0  James      WMS-18570. Created                        */
/************************************************************************/

CREATE PROC [RDT].[rdt_556GetNextRec01] (
   @nMobile       INT,
   @nFunc         INT, 
...
AS
   ...
   BEGIN
      SET @nErrNo = 181251  -- (ChewKP01)
      SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --No pick LOC -- (ChewKP01)
   END
   ...
   BEGIN
      ...
      BEGIN
         SET @nErrNo = 181252 -- (ChewKP01)
         SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --Empty, NoPKLOC -- (ChewKP01)
      END
      ELSE
      BEGIN
         SET @nErrNo = 181253 -- (CheWKP01)
         SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --No pick LOC -- (ChewKP01)
      END
   END
