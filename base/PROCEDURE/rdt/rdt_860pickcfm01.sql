...
/************************************************************************/  
/* Store procedure: rdt_860PickCfm01                                    */  
/* Copyright      : IDS                                                 */  
/*                                                                      */  
/* Purpose: Pick task confirm (Modified from rdt_Pick_Cfm_UNI01)        */  
/*          After pick confirm then insert record into DROPID table     */  
/*                                                                      */  
...
/* 2016-04-12 1.0  James    SOS367362 - Created                         */
/************************************************************************/  
  
CREATE PROCEDURE [RDT].[rdt_860PickCfm01] (  
   @nMobile       INT,   
   @nFunc         INT, 
...
SET CONCAT_NULL_YIELDS_NULL OFF  
   ...
   (  
      ...
      [Status]         NVARCHAR( 10) NOT NULL, -- (james02)  
   ...
   BEGIN  
      IF ISNULL(@cPH_OrderKey, '') <> ''  -- (james03)
      BEGIN
         -- Get PickDetail in the task  
         INSERT INTO @tPD (PickDetailKey, PD_QTY, OrderKey, [Status])  
         SELECT PD.PickDetailKey, PD.QTY, PD.OrderKey, '0'  
...
--  goto fail
   ...
   */  
   ...
   IF rdt.RDTGetConfig( @nFunc, 'ShortPickWithStatus4', @cStorerKey) = '1'
   BEGIN
   ...
   IF ISNULL( @cPH_LoadKey, '') = ''
      SELECT @cPH_LoadKey = LoadKey
      FROM dbo.Orders WITH (NOLOCK)
      WHERE StorerKey = @cStorerKey
      AND   OrderKey = @cPH_OrderKey

   IF NOT EXISTS ( SELECT 1 FROM dbo.DropID WITH (NOLOCK) 
                   WHERE DropID = @cDropID
                   AND   [Status] = '0') 
   BEGIN
      INSERT INTO DropID ( DropID, DropLOC, Status, LoadKey, PickSlipNo) VALUES 
      (@cDropID, @cLOC, '0', @cPH_LoadKey, @cPickSlipNo)
