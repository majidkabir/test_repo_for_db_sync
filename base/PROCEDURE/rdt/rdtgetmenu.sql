...
/*                             (Vicky01)                                   */
/* 04-Dec-2009  1.9  ChewKP     Changes for RDT2 Column Attribute          */
/* 12-Nov-2011  2.0  ChewKP    Changes to Insert into RDT.RDTXML_Elm in    */
/*                             RDT.rdtScr2XML (ChewKP01)                   */
/* 02-Oct-2015  2.1  Ung       Performance tuning for CN Nov 11            */
/* 25-Nov-2016  2.2  James     Add menu 7-9 (james01)                      */
/* 23-Jan-2017  2.3  James     Output blank line if menu not setup(james02)*/
/***************************************************************************/

CREATE PROC [RDT].[rdtGetMenu] (
   @nMobile    INT,
...
AS
   ...
   -- Menu header
   SET @cMsgText = rdt.rdtgetmessage( @nMenu ,@cLangCode, 'MNU')
   SET @cMsgText = rdt.rdtReplaceSpecialCharInXMLData( @cMsgText)
   EXEC RDT.rdtScr2XML @nMobile, @cLangCode, NULL, '1', @cMsgText, @cXML OUTPUT
   ...
   BEGIN
      ...
      BEGIN
         ...
         SET @cMsgText = ''
         EXEC RDT.rdtScr2XML @nMobile, @cLangCode, NULL, @cLine, @cMsgText, @cXML OUTPUT
         SET @nLine = @nLine + 1
      END
   ...
   BEGIN
      ...
      EXEC RDT.rdtScr2XML @nMobile, @cLangCode, NULL, @cLine, @cErrMsg, @cXML OUTPUT
   END
