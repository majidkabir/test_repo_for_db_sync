...
/************************************************************************/
/* Stored Procedure: rdt_840ExtInsPack21                                */
/*                                                                      */
...
/* 2023-07-04   James     1.0   WMS-22925. Created                      */
/************************************************************************/

CREATE   PROC [RDT].[rdt_840ExtInsPack21] (
   @nMobile                   INT,
   @nFunc                     INT,
...
BEGIN
   ...
   DECLARE @nTranCount        INT,
           ...
           @nPackQty          INT,
           @nNewCarton        INT,
           @cDelNoteCV        NVARCHAR( 10),
           @cShipLblCV        NVARCHAR( 10),
   ...
   BEGIN
      ...
      BEGIN
         ...
         AND   SKU = @cPack_SKU
         AND   Status < '9'
         AND   ( (ISNULL( CaseID, '') = '') OR ( CaseID = 'SORTED'))
   ...
   BEGIN
      ...
      IF @nPickQty = @nPackQty
      BEGIN
         SET @cDelNoteCV = rdt.RDTGetConfig( @nFunc, 'DELNOTESCV', @cStorerKey)
         IF @cDelNoteCV = '0'
            SET @cDelNoteCV = ''

         IF @cDelNoteCV <> ''
         BEGIN
            DECLARE @tDelNoteCV AS VariableTable
            INSERT INTO @tDelNoteCV (Variable, Value) VALUES ( '@cStorerKey',  @cStorerKey)
            INSERT INTO @tDelNoteCV (Variable, Value) VALUES ( '@cOrderKey',   @cOrderKey)
            INSERT INTO @tDelNoteCV (Variable, Value) VALUES ( '@cPickSlipNo', @cPickSlipNo)

            -- Print label
            EXEC RDT.rdt_Print @nMobile, @nFunc, @cLangCode, 2, 1, @cFacility, @cStorerkey, '', @cPaperPrinter, 
               @cDelNoteCV, -- Report type
               @tDelNoteCV, -- Report params
               'rdt_840ExtInsPack21', 
               @nErrNo  OUTPUT,
               @cErrMsg OUTPUT
      ...
      SET @cShipLblCV = rdt.RDTGetConfig( @nFunc, 'SHIPLBLCV', @cStorerKey)
      IF @cShipLblCV = '0'
         SET @cShipLblCV = ''

      IF @cShipLblCV <> ''
      BEGIN
         DECLARE @tShipLblCV AS VariableTable
         INSERT INTO @tShipLblCV (Variable, Value) VALUES ( '@cStorerKey',    @cStorerKey)
         INSERT INTO @tShipLblCV (Variable, Value) VALUES ( '@cOrderKey',     @cOrderKey)
         INSERT INTO @tShipLblCV (Variable, Value) VALUES ( '@nFromCartonNo', @nNewCarton)
         INSERT INTO @tShipLblCV (Variable, Value) VALUES ( '@nToCartonNo',   @nNewCarton)

         -- Print label
         EXEC RDT.rdt_Print @nMobile, @nFunc, @cLangCode, 2, 1, @cFacility, @cStorerkey, @cLabelPrinter, '', 
            @cShipLblCV, -- Report type
            @tShipLblCV, -- Report params
            'rdt_840ExtInsPack21', 
            @nErrNo  OUTPUT,
            @cErrMsg OUTPUT
   ...
   RollBackTran:
         ROLLBACK TRAN rdt_840ExtInsPack21
   Quit:
