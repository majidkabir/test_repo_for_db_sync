...
/************************************************************************/
/* Store procedure: rdt_TM_Move_SwapTask                                */
/*                                                                      */
/* Modifications log:                                                   */
/*                                                                      */
...
CREATE PROC [RDT].[rdt_TM_Move_SwapTask] (
   @nMobile           INT,
   @nFunc             INT,
...
BEGIN
   ...
   SAVE TRAN rdt_TM_Move_SwapTask -- For rollback or commit only our own transaction
   ...
   BEGIN
      INSERT INTO rdt.rdtMVFSwapTaskLog (FromTaskKey, FromLOC, FromID, NewTaskKey, NewFromLOC, NewFromID)
      VALUES (@cTaskDetailKey, @cSuggFromLOC, @cSuggID, @cNewTaskDetailKey, @cNewFromLOC, @cNewID)
   END

   COMMIT TRAN rdt_TM_Move_SwapTask -- Only commit change made here
   GOTO Quit

RollBackTran:
   ROLLBACK TRAN rdt_TM_Move_SwapTask -- Only rollback change made here
Fail:
Quit:
