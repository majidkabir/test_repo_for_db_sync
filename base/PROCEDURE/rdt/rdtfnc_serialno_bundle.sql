...
/* 2017-09-15 1.0  ChewKP     Created. WMS-2882                               */  
/* 2018-10-16 1.1  TungGH     Performance                                     */    
/******************************************************************************/        
        
CREATE   PROC [RDT].[rdtfnc_SerialNo_Bundle] (        
   @nMobile    int,        
   @nErrNo     int  OUTPUT,        
...
IF @nFunc = 1017  -- Serial No Bundle  
BEGIN        
           
   -- Redirect to respective screen        
   IF @nStep = 0 GOTO Step_0   -- Serial No   
   IF @nStep = 1 GOTO Step_1   -- Scn = 5020. WorkOrderNo  
   IF @nStep = 2 GOTO Step_2   -- Scn = 5021. Master SerialNo  
   IF @nStep = 3 GOTO Step_3   -- Scn = 5022. BOM SerialNo  
   IF @nStep = 4 GOTO Step_4   -- Scn = 5023. Child SerialNo  
   IF @nStep = 5 GOTO Step_5   -- Scn = 5024. Options  
      ...
      BEGIN  
         ...
         BEGIN  
            SET @cSQL = 'EXEC rdt.' + RTRIM( @cExtendedValidateSP) +  
               ' @nMobile, @nFunc, @cLangCode, @nStep, @nInputKey, @cStorerkey, @cWorkOrderNo OUTPUT, @cSKU, @cMasterSerialNo, @cBOMSerialNo, @cChildSerialNo, @nErrNo OUTPUT, @cErrMsg OUTPUT'  
            SET @cSQLParam =  
               '@nMobile         INT,                 ' +  
               '@nFunc           INT,                 ' +  
               '@cLangCode       NVARCHAR( 3),        ' +  
      ...
      BEGIN  
            ...
            BEGIN  
               SET @cSQL = 'EXEC rdt.' + RTRIM( @cExtendedInfoSP) +  
                  ' @nMobile, @nFunc, @cLangCode, @nStep, @nInputKey, @cStorerkey, @cWorkOrderNo, @cSKU OUTPUT , @cMasterSerialNo, @cBOMSerialNo, @cChildSerialNo, @cOutText1 OUTPUT, @cOutText2 OUTPUT, @cOutText3 OUTPUT, @nErrNo OUTPUT, @cErrMsg OUTPUT'  
               SET @cSQLParam =  
                  '@nMobile         INT,                     ' +    
                  '@nFunc           INT,                     ' +   
                  '@cLangCode       NVARCHAR( 3),            ' +   
               ...
               SET @cOutField01 = @cMasterSerialNo   
               SET @cOutField02 = @cSKU  
               SET @cOutField03 = @cOutText1   
               SET @cOutField04 = @cOutText2   
               SET @cOutField05 = ''   
               SET @cOutField06 = @cOutText3  
...
Step 3. Scn = 5022.         
   ...
   MasterSerialNo    (field01      
   SKU               (field02)        
   SKU Descr         (field03)  
   SKU Descr         (field04)  
   BOM SerialNo      (field05, input)        
...
BEGIN        
   ...
   BEGIN        
      ...
      SET @cBOMSerialNo   = ISNULL(RTRIM(@cInField05),'')     
      ...
      BEGIN  
         SET @nErrNo = 114953     
         SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --BOMSerialReq  
         GOTO Step_3_Fail    
      END  
...
Step 4. Scn = 5023.         
   ...
   MasterSerialNo    (field01)  
   SKU               (field02)        
   BOM SerialNo      (field03)        
   Child SerialNo    (field04, input)  
   Scan Count        (field05)        
********************************************************************************/        
Step_4:        
BEGIN        
   ...
   BEGIN        
      ...
      BEGIN  
         ...
         BEGIN  
            ...
            SET @cSQL = 'EXEC rdt.' + RTRIM( @cExtendedUpdateSP) +  
               ' @nMobile, @nFunc, @cLangCode, @nStep, @nInputKey, @cStorerkey, @cWorkOrderNo, @cSKU, @cMasterSerialNo, @cBOMSerialNo, @cChildSerialNo, @cOption, @nCompleteFlag OUTPUT, @nErrNo OUTPUT, @cErrMsg OUTPUT'  
            SET @cSQLParam =  
               '@nMobile         INT,       '     +  
               '@nFunc           INT,       '     +  
               '@cLangCode       NVARCHAR( 3),  ' +  
      ...
      IF @nCompleteFlag = 1 -- ChildSerial Complete  
      BEGIN  
        IF @cExtendedInfoSP <> ''   
        BEGIN  
               IF EXISTS( SELECT 1 FROM dbo.sysobjects WHERE name = @cExtendedInfoSP AND type = 'P')  
...
'  
                  ...
                  EXEC sp_ExecuteSQL @cSQL, @cSQLParam,  
                      @nMobile, @nFunc, @cLangCode, 2, @nInputKey, @cStorerkey, @cWorkOrderNo, @cSKU OUTPUT , @cMasterSerialNo, @cBOMSerialNo, @cChildSerialNo, @cOutText1 OUTPUT, @cOutText2 OUTPUT, @cOutText3 OUTPUT, @nErrNo OUTPUT, @cErrMsg OUTPUT  
