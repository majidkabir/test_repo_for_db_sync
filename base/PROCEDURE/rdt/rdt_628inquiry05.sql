...
/************************************************************************/    
/* Store procedure: rdt_628Inquiry05                                    */    
/* Copyright      : IDS                                                 */    
/*                                                                      */    
...
CREATE PROCEDURE [RDT].[rdt_628Inquiry05] (    
   @nMobile         INT,
   @nFunc           INT,
   ...
   IF @cLOC = '' OR @cLocationType = 'DYNPPICK'
   BEGIN 
      IF @cLOC = ''
         SET @cTempLOT  = ''
      SET @cSQL = ''
      ...
      '    JOIN dbo.LOC WITH (NOLOCK) ON (LOC.LOC = LLI.LOC) ' + 
      '    JOIN LotAttribute LA WITH (NOLOCK) ON (LA.LOT = LLI.LOT) ' + 
      '    WHERE LOC.Facility = @cFacility  AND LOC.LocationType = ''DYNPPICK'' ' +
      '    AND  (LLI.QTY <> 0 OR LLI.QtyAllocated <> 0 OR LLI.QtyPicked <> 0 OR LLI.QtyExpected <> 0) ' +
      '    AND   LLI.StorerKey = @cStorerKey ' +
      '    AND ( (ISNULL( @cInquiry_LOC, '''') = '''') OR ( LLI.LOC = @cInquiry_LOC)) ' +
      '    AND ( (ISNULL( @cInquiry_ID, '''') = '''') OR ( LLI.ID = @cInquiry_ID)) ' +
      ...
      BEGIN
         ...
         CASE WHEN @cOrderBy = '' THEN ' ORDER BY LLI.Loc,LLI.Lot,LLI.id ' ELSE ' ORDER BY LLI.Loc,LLI.Lot,LLI.id' END       --hcb01
      END
