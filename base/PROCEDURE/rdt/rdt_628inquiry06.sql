...
/************************************************************************/    
/* Store procedure: rdt_628Inquiry06                                    */    
/* Copyright      : IDS                                                 */    
/*                                                                      */    
...
CREATE PROCEDURE [RDT].[rdt_628Inquiry06] (    
   ...
   SET @cTempLocationType = @cLocationType
   ...
   BEGIN 
      ...
      SELECT TOP 1 @cLocationType = ISNULL(LOC.LocationType ,'')
               , @cLogicalLocation = ISNULL(LOC.LogicalLocation ,'')
               , @cLot = ISNULL(LLI.Lot ,'')
      FROM dbo.LOC LOC(NOLOCK)
      JOIN LOTxLOCxID LLI (NOLOCK)
      ON LOC.Loc = LLI.Loc
      WHERE  LOC.Loc = @cLOC
      ORDER BY LLI.LOC, LLI.ID, LLI.SKU, LLI.LOT
   ...
   DECLARE @cPreLocationType NVARCHAR(20)
   SELECT @cPreLocationType = ISNULL(code2 ,'') 
   FROM dbo.CODELKUP(NOLOCK) 
   WHERE Storerkey = @cStorerkey 
   AND LISTNAME = 'RDTINQUIRY' 
   AND Long = 'LocationType' 
   AND Code = @cFacility
   ...
   IF @cLOC = '' OR @cLocationType = @cPreLocationType
   BEGIN 
      IF @cLOC = ''
      BEGIN 
         SET @cTempID = '0'
      ...
      '    JOIN dbo.LOC WITH (NOLOCK) ON (LOC.LOC = LLI.LOC) ' + 
      '    JOIN LotAttribute LA WITH (NOLOCK) ON (LA.LOT = LLI.LOT) ' + 
      '    WHERE LOC.Facility = @cFacility  AND LOC.LocationType = @cPreLocationType ' +
      '    AND  (LLI.QTY <> 0 OR LLI.QtyAllocated <> 0 OR LLI.QtyPicked <> 0 OR LLI.QtyExpected <> 0) ' +
      '    AND   LLI.StorerKey = @cStorerKey ' +
      '    AND ( (ISNULL( @cInquiry_LOC, '''') = '''') OR ( LLI.LOC = @cInquiry_LOC)) ' +
      '    AND ( (ISNULL( @cInquiry_ID, '''') = '''') OR ( LLI.ID = @cInquiry_ID)) ' +
      ...
      BEGIN
         SET @cSQL = @cSQL + 
         --'    AND  (CASE WHEN LOC.LogicalLocation = '''' THEN ''z'' ELSE LOC.LogicalLocation END + LLI.LOC + CASE WHEN LLI.ID = '''' THEN ''Z'' ELSE LLI.ID END  + LLI.SKU' + 
         '    AND  (CASE WHEN LOC.LogicalLocation = '''' THEN ''z'' ELSE LOC.LogicalLocation END + LLI.LOC + LLI.SKU + LLI.Lot' + 
         CASE WHEN @cWhere1 = '' THEN '' ELSE ' + ' + @cWhere1 END  + ') > ' +
         --'         (CASE WHEN @cLogicalLocation = '''' THEN ''z'' ELSE @cLogicalLocation END  + @cLOC +  CASE WHEN @cID = '''' THEN ''Z'' ELSE @cID END   + @cSKU' + CASE WHEN @cWhere2 = '' THEN '' ELSE + ' + ' + @cWhere2 END + ') ' +
         '         (CASE WHEN @cLogicalLocation = '''' THEN ''z'' ELSE @cLogicalLocation END  + @cLOC + @cSKU + @cLot ' + CASE WHEN @cWhere2 = '' THEN '' ELSE + ' + ' + @cWhere2 END + ') ' +
         CASE WHEN @cGroupBy = '' THEN ' GROUP BY CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END ,LLI.LOC, LLI.ID, LLI.SKU, LLI.LOT ' ELSE ' GROUP BY CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END ,' + 'LLI.LOC, LLI.ID, LLI.SKU, ' + @cGroupBy + ', LLI.LOT' END +
         CASE WHEN @cOrderBy = '' THEN ' ORDER BY CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END ,LLI.LOC, LLI.SKU, LLI.LOT ' ELSE ' ORDER BY CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END ,' + 'LLI.LOC, LLI.SKU, LLI.LOT, ' + @cOrderBy END 
      END
      ELSE IF ISNULL( @cInquiry_ID, '') <> ''
      BEGIN
         SET @cSQL = @cSQL + 
         '    AND  (CASE WHEN LOC.LogicalLocation = '''' THEN ''z'' ELSE LOC.LogicalLocation END + CASE WHEN LLI.ID = '''' THEN ''Z'' ELSE LLI.ID END + LLI.LOC + LLI.SKU) > ' +
         '         (CASE WHEN @cLogicalLocation = '''' THEN ''z'' ELSE @cLogicalLocation END  + CASE WHEN @cID = '''' THEN ''Z'' ELSE @cID END + @cLOC + @cSKU) ' +
         CASE WHEN @cGroupBy = '' THEN ' GROUP BY  CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END ,LLI.ID, LLI.LOC, LLI.SKU, LLI.LOT ' ELSE ' GROUP BY CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END ,' + 'LLI.ID, LLI.LOC, LLI.SKU, LLI.LOT,' + @cGroupBy  END +
         CASE WHEN @cOrderBy = '' THEN ' ORDER BY  CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END ,LLI.ID, LLI.LOC, LLI.SKU, LLI.LOT ' ELSE ' ORDER BY CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END ,' + 'LLI.ID, LLI.LOC, LLI.SKU, LLI.LOT, ' + @cOrderBy END 
      END
      ELSE  -- IF ISNULL( @cInquiry_SKU, '') <> ''
      BEGIN
         SET @cSQL = @cSQL + 
            '    AND  (CASE WHEN LOC.LogicalLocation = '''' THEN ''z'' ELSE LOC.LogicalLocation END + LLI.Loc+LLI.Lot + CASE WHEN LLI.ID = '''' THEN ''Z'' ELSE LLI.ID END ) > ' +
            '         (CASE WHEN @cLogicalLocation = '''' THEN ''z'' ELSE @cLogicalLocation END  + @cLOC + @cLot + CASE WHEN @cID = '''' THEN ''Z'' ELSE @cID END) ' +
         CASE WHEN @cGroupBy = '' THEN ' GROUP BY CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END ,LLI.SKU, LLI.LOC, LLI.ID, LLI.LOT ' ELSE ' GROUP BY CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END ,' + 'LLI.SKU, LLI.LOC, LLI.ID, LLI.LOT, ' + @cGroupBy  END +
         CASE WHEN @cOrderBy = '' THEN ' ORDER BY CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END , LLI.Loc,LLI.Lot, CASE WHEN LLI.ID = '''' THEN ''Z'' ELSE LLI.ID END ' ELSE ' ORDER BY CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END ,LLI.Loc,LLI.Lot, CASE WHEN LLI.ID = '''' THEN ''Z'' ELSE LLI.ID END ,' + @cOrderBy END       --hcb01
      END
      ...
      SET @cSQLParam = 
         ...
         '@cInquiry_ID  NVARCHAR( 18) , ' +  
         '@cInquiry_SKU NVARCHAR( 20) , ' +  
         '@cPreLocationType NVARCHAR( 20) , ' +  
         '@cLogicalLocation NVARCHAR( 20) , ' +  
      ...
      EXEC sp_ExecuteSQL @cSQL, @cSQLParam, 
         ...
         @cInquiry_ID = @cInquiry_ID, 
         @cInquiry_SKU= @cInquiry_SKU, 
         @cPreLocationType = @cPreLocationType, 
         @cLogicalLocation = @cLogicalLocation, 
      ...
      BEGIN
         ...
         IF @cLocationType = @cPreLocationType
         BEGIN 
            SET @cTempLOC = ''
            SET @cTempSKU = ''
            SET @cTempID  = ''
            SET @cTempLOT  = ''
            SET @cLogicalLocation  = '0'
            --SET @cLot  = ''
        END
         ...
         BEGIN
            ...
            '         (CASE WHEN @cLogicalLocation = '''' THEN ''z'' ELSE @cLogicalLocation END  + @cLOC + @cSKU + @cLot ' + CASE WHEN @cWhere2 = '' THEN '' ELSE + ' + ' + @cWhere2 END + ') ' +
            CASE WHEN @cGroupBy = '' THEN ' GROUP BY CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END ,LLI.LOC, LLI.ID, LLI.SKU, LLI.LOT ' ELSE ' GROUP BY CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END ,' + 'LLI.LOC, LLI.ID, LLI.SKU, ' + @cGroupBy + ', LLI.LOT' END +
            --CASE WHEN @cOrderBy = '' THEN ' ORDER BY CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END ,LLI.LOC, CASE WHEN LLI.ID = '''' THEN ''Z'' ELSE LLI.ID END, LLI.SKU, LLI.LOT ' ELSE ' ORDER BY CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END ,' + 'LLI.LOC, CASE WHEN LLI.ID = '''' THEN ''Z'' ELSE LLI.ID END, LLI.SKU, ' + @cOrderBy + ', LLI.LOT' END 
            CASE WHEN @cOrderBy = '' THEN ' ORDER BY CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END ,LLI.LOC, LLI.SKU, LLI.LOT ' ELSE ' ORDER BY CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END ,' + 'LLI.LOC, LLI.SKU, LLI.LOT, ' + @cOrderBy END 
         END
         ELSE IF ISNULL( @cInquiry_ID, '') <> ''
         BEGIN
            SET @cSQL = @cSQL + 
            '    AND  (CASE WHEN LOC.LogicalLocation = '''' THEN ''z'' ELSE LOC.LogicalLocation END + LLI.ID + LLI.LOC + LLI.SKU' + 
            CASE WHEN @cWhere1 = '' THEN '' ELSE ' + ' + @cWhere1 END  + ') > ' +
            '         (CASE WHEN @cLogicalLocation = '''' THEN ''z'' ELSE @cLogicalLocation END  + @cID + @cLOC + @cSKU' + CASE WHEN @cWhere2 = '' THEN '' ELSE + ' + ' + @cWhere2 END + ') ' +
            CASE WHEN @cGroupBy = '' THEN ' GROUP BY  CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END ,LLI.ID, LLI.LOC, LLI.SKU, LLI.LOT ' ELSE ' GROUP BY CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END ,' + 'LLI.ID, LLI.LOC, LLI.SKU, ' + @cGroupBy + ', LLI.LOT' END +
            CASE WHEN @cOrderBy = '' THEN ' ORDER BY  CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END ,LLI.ID, LLI.LOC, LLI.SKU, LLI.LOT ' ELSE ' ORDER BY CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END ,' + 'LLI.ID, LLI.LOC, LLI.SKU, ' + @cOrderBy + ', LLI.LOT' END 
         END
         ELSE  -- IF ISNULL( @cInquiry_SKU, '') <> ''
         BEGIN
            SET @cSQL = @cSQL + 
               '    AND  (CASE WHEN LOC.LogicalLocation = '''' THEN ''z'' ELSE LOC.LogicalLocation END + LLI.Loc+LLI.Lot+CASE WHEN LLI.ID = '''' THEN ''Z'' ELSE LLI.ID END  ' +
                ') > ' +
               ' (CASE WHEN @cLogicalLocation = '''' THEN ''z'' ELSE @cLogicalLocation END  + @cLOC + @cLot + @cID) ' +
            
            CASE WHEN @cGroupBy = '' THEN ' GROUP BY CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END ,LLI.SKU, LLI.LOC, LLI.ID, LLI.LOT ' ELSE ' GROUP BY CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END ,' + 'LLI.SKU, LLI.LOC, LLI.ID, ' + @cGroupBy + ', LLI.LOT' END +
            CASE WHEN @cOrderBy = '' THEN ' ORDER BY CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END , LLI.Loc,LLI.Lot, CASE WHEN LLI.ID = '''' THEN ''Z'' ELSE LLI.ID END ' ELSE ' ORDER BY CASE WHEN LOC.LogicalLocation = '''' THEN ''Z'' ELSE LOC.LogicalLocation END ,LLI.Loc,LLI.Lot, CASE WHEN LLI.ID = '''' THEN ''Z'' ELSE LLI.ID END' END       --hcb01
         END
   ...
   BEGIN 
      ...
      BEGIN
         ...
         --   '    AND  (CASE WHEN LOC.LogicalLocation = '''' THEN ''z'' ELSE LOC.LogicalLocation END + LLI.Loc+LLI.Lot + CASE WHEN LLI.ID = '''' THEN ''Z'' ELSE LLI.ID END  ' +
         --   CASE WHEN @cWhere1 = '' THEN '' ELSE ' + ' + @cWhere1 END  + ') > ' +
         --   '         (CASE WHEN @cLogicalLocation = '''' THEN ''z'' ELSE @cLogicalLocation END  + @cLOC + @cLot + CASE WHEN @cID = '''' THEN ''Z'' ELSE @cID END ' + CASE WHEN @cWhere2 = '' THEN '' ELSE + ' + ' + @cWhere2 END + ') ' +
   ...
   END 
      ...
      SET @cLocationType = @cTempLocationType
      ...
      DECLARE @cSUSR3            NVARCHAR( 20),  
         ...
         BEGIN  
            ...
            FROM dbo.LOTxLOCxID LLI WITH (NOLOCK)      
               ...
               INNER JOIN dbo.PickDetail PD WITH (NOLOCK) ON (SKU.SKU = PD.SKU)      
            WHERE PD.storerkey = @cStorerKey  
               AND PD.LOC = @cInquiry_LOC    
               AND PD.SKU = @cSKU   
               AND LLI.qtyallocated<>0  
  
            SELECT @cPickQty = SUM(QTY)    
            FROM dbo.PickDetail  WITH (NOLOCK)    
            WHERE storerkey = @cStorerKey  
               AND LOC = @cInquiry_LOC    
               AND SKU = @cSKU   
               AND status in ('3')
        ...
        BEGIN  
            ...
            SELECT @cPickQty = ISNULL(SUM(QTY),0)    
            ...
            --SELECT @cPickQty = ISNULL(SUM(PD.QTY),0)    
            --FROM dbo.LOTxLOCxID LLI WITH (NOLOCK)      
            --   INNER JOIN dbo.LotAttribute LA WITH (NOLOCK) ON (LLI.LOT = LA.LOT)      
            --   INNER JOIN dbo.LOC LOC WITH (NOLOCK) ON (LOC.LOC = LLI.LOC)      
            --   INNER JOIN dbo.SKU SKU WITH (NOLOCK) ON (LLI.StorerKey = SKU.StorerKey AND LLI.SKU = SKU.SKU)      
         ...
         SET @cTempLottable10 = @cSUSR3+' '+@cExtendedInfo2  
