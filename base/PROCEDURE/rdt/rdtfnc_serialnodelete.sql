...
/************************************************************************/
/* Store procedure: rdtfnc_SerialNoDelete                              */
/* Copyright      : IDS                                                 */
/*                                                                      */
...
/* 02-Sep-2008 1.4  Vicky    Modify to cater for SQL2005 (Vicky01)      */ 
/* 08-Apr-2010 1.5  James    SOS#153915 - Change Serial No length from  */
/*                           18 to 20 digits (james01)                  */ 
/* 23-Feb-2012 1.6  Ung      SOS236331 Reorganize RDT message           */
/* 14-Jul-2014 1.7  James    SOS315487 - Extend length of serial no     */
/*                           from 20 to 30 chars (james02)              */
/* 30-Sep-2016 1.8  Ung      Performance tuning                         */
/* 13-Sep-2017 1.9  James    WMS2954 - Add ExtendedUpdateSP (james03)   */
/*                           ExtendedValidateSP & DeleteBOMSerialNo     */
/* 18-Oct-2018 2.0  TungGH   Performance                                */
/************************************************************************/

CREATE  PROC [RDT].[rdtfnc_SerialNoDelete] (
   @nMobile    int,
   @nErrNo     int  OUTPUT,
...
BEGIN    
   ...
   IF @nStep = 1 GOTO Step_1   -- Scn = 876   enter serial no to delete
   IF @nStep = 2 GOTO Step_2   -- Scn = 877   dispaly serial no info
   IF @nStep = 3 GOTO Step_3   -- Scn = 878   confirm delete serial no
END
...
BEGIN
   ...
   SET @cDeleteBOMSerialNo = rdt.RDTGetConfig( @nFunc, 'DeleteBOMSerialNo', @cStorerKey)
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         -- (james01) - Start
         IF rdt.RDTGetConfig( @nFunc, 'CheckSSCC', @cStorerKey) = '1'
         BEGIN
      ...
      -- Extended validate
      -- Put here to cater for BOM serial no deletion as it probably not exists in serialno table
      IF @cExtendedValidateSP <> ''
      BEGIN
         IF EXISTS( SELECT 1 FROM dbo.sysobjects WHERE name = @cExtendedValidateSP AND type = 'P')
         BEGIN
            SET @cSQL = 'EXEC rdt.' + RTRIM(@cExtendedValidateSP) +
               ' @nMobile, @nFunc, @cLangCode, @nStep, @nInputKey, @cStorerKey, @cSerialNo, @cOption, ' +
      ...
      BEGIN
         ...
         BEGIN
            ...
            SET @cSQL = 'EXEC rdt.' + RTRIM(@cExtendedInfoSP) +
               ' @nMobile, @nFunc, @cLangCode, @nStep, @nInputKey, @cStorerKey, @cSerialNo, @cOption, @cExtendedInfo1 OUTPUT '
            SET @cSQLParam =
               '@nMobile       INT,             ' +
               '@nFunc         INT,             ' +
               '@cLangCode     NVARCHAR( 3),    ' +
      ...
      IF @cDeleteBOMSerialNo = '1'
      BEGIN
         SET @cOutField01 = @cSerialNo
         SET @cOutField02 = ''
         SET @cOutField03 = @cExtendedInfo1
         SET @cOption = ''
...
BEGIN
   ...
   BEGIN
      SET @cSerialNo = @cOutField01 -- (james01)
      SET @cOption = ''
      SET @cOption = @cInField02

      IF ISNULL(LTRIM(RTRIM(@cOption)), '') <> '1' AND ISNULL(LTRIM(RTRIM(@cOption)), '') <> '2' -- (Vicky01)
      BEGIN
      ...
      IF ISNULL(LTRIM(RTRIM(@cOption)), '') = '1'   --Yes & go back to scan serialno screen -- (Vicky01)
      BEGIN
         IF @cDeleteBOMSerialNo <> '1'
         BEGIN
            BEGIN TRAN
            ...
            BEGIN
               ROLLBACK TRAN
               SET @nErrNo = 63203
               SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --Delete SN# Err
               GOTO Step_3_Fail
            END
            ELSE
            BEGIN
      ...
      IF ISNULL(LTRIM(RTRIM(@cOption)), '') = '2'   --No & go back to scan upc screen -- (Vicky01)
      BEGIN
         SET @nScn  = 876
         SET @nStep = 1
         SET @cOutField01 = @cSerialNo
   ...
   BEGIN
      SET @nScn  = 876
      SET @nStep = 1
      SET @cOutField01 = ''--@cSerialNo --Edit by james on 29/01/2008 to clear field when user pressed esc
      GOTO Quit
   END
