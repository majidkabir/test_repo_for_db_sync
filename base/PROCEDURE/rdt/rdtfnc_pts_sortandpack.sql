...
/* 2016-02-23 1.0  ChewKP     Created                                         */
/* 2016-09-30 1.1  Ung        Performance tuning                              */
/* 2017-11-01 1.2  James      Bug fix (james01)                               */
/* 2018-04-11 1.3  Ung        WMS-4300 Add PickStatus                         */
/* 2018-06-14 1.4  Ung        WMS-5408 Add DisableQTYField                    */
/* 2018-08-13 1.5  Ung        WMS-5913 Fix full short                         */
/* 2018-11-07 1.6  Gan        Performance tuning                              */
/******************************************************************************/

CREATE PROC [RDT].[rdtfnc_PTS_SortAndPack] (
   @nMobile    int,
   @nErrNo     int  OUTPUT,
...
SELECT
  ...
  -- @nExpectedQTY             = CASE WHEN rdt.rdtIsValidQTY( LEFT( V_String14, 5), 0) = 1 THEN LEFT( V_String14, 5) ELSE 0 END,
   @cSKUValidated            = V_String15, -- (james01)
  ...
  -- @nPUOM_Div                = CASE WHEN rdt.rdtIsValidQTY( LEFT( V_String17,  5), 0) = 1 THEN LEFT( V_String17,  5) ELSE 0 END,
  -- @nMQTY                    = CASE WHEN rdt.rdtIsValidQTY( LEFT( V_String18,  5), 0) = 1 THEN LEFT( V_String18,  5) ELSE 0 END,
  -- @nPQTY                    = CASE WHEN rdt.rdtIsValidQTY( LEFT( V_String19,  5), 0) = 1 THEN LEFT( V_String19,  5) ELSE 0 END,
  -- @nActMQTY                 = CASE WHEN rdt.rdtIsValidQTY( LEFT( V_String20,  5), 0) = 1 THEN LEFT( V_String20,  5) ELSE 0 END,
  -- @nActPQTY                 = CASE WHEN rdt.rdtIsValidQTY( LEFT( V_String21, 5), 0) = 1 THEN LEFT( V_String21, 5) ELSE 0 END,
  -- @nActQty                  = CASE WHEN rdt.rdtIsValidQTY( LEFT( V_String22, 5), 0) = 1 THEN LEFT( V_String22, 5) ELSE 0 END,
...
IF @nFunc = 760  -- PTS Sort And Pack
BEGIN

   -- Redirect to respective screen
   IF @nStep = 0 GOTO Step_0   -- PTS Sort And Pack
   IF @nStep = 1 GOTO Step_1   -- Scn = 4500. DropID
   IF @nStep = 2 GOTO Step_2   -- Scn = 4501. Position
   IF @nStep = 3 GOTO Step_3   -- Scn = 4502. SKU , Qty
   IF @nStep = 4 GOTO Step_4   -- Scn = 4503. To LabelNo
   --IF @nStep = 5 GOTO Step_5   -- Scn = 4504. New LabelNo
   IF @nStep = 5 GOTO Step_5   -- Scn = 4505. Short Pack
   IF @nStep = 6 GOTO Step_6   -- Scn = 4506. Message
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            SET @cOutField01 = @cSuggPTSPosition
      ...
      BEGIN
         ...
         SELECT STL.Loc, '0', @cDropID, '' ,PD.StorerKey, O.ConsigneeKey, O.OrderKey, PD.SKU, PD.Loc, PD.Lot, PD.UOM
               ,PD.Qty, 0, '', @nFunc, GetDate(), @cUserName
...
--      IF ISNULL(@cSuggLabelNo,'')  <> ''
--      BEGIN
--
--         IF @cSuggLabelNo <> @cToLabelNo
--         BEGIN
--
...
--      END
      ...
      BEGIN
         ...
         AND PTSLOG.AddWho = @cUserName
         AND Status = '0'
         AND PTSLog.PTSPosition = @cPTSPosition
         ORDER BY PTSLOG.PTSPosition, PTSLOG.SKU, PTSLOG.PTSLOGKEY
...
--      VALUES ( 'rdtfnc_PTS_SortAndPack' , Getdate() , @cDisableSKUField , '' , @cSKU, '3' )
--
--      IF @cPUOM_Desc = ''
--      BEGIN
--
