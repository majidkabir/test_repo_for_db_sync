...
GO
/***************************************************************************/      
/* Store procedure: rdt_593ShipLabel14                                     */      
/*                                                                         */      
/* Modifications log:                                                      */      
/*                                                                         */      
...
CREATE PROC [RDT].[rdt_593ShipLabel14] (      
   @nMobile    INT,      
   @nFunc      INT,      
   @nStep      INT,      
...
AS          
   ...
   SELECT 
      ...
      @nCartonNo   = A.cartonno,
		@cTMODE      = C.userdefine03,
		@cConsigneeKey = C.Consigneekey,
		@cLabelNo    = A.labelno
   FROM dbo.PackDetail A WITH (NOLOCK) 
   JOIN dbo.PACKHEADER B WITH (NOLOCK) ON A.PICKSLIPNO = B.PICKSLIPNO
   JOIN dbo.ORDERS     C WITH (NOLOCK) ON B.ORDERKEY = C.ORDERKEY
...
--RollBackTran:            
--   ROLLBACK TRAN rdt_593ShipLabel14 -- Only rollback change made here            
--   EXEC rdt.rdtSetFocusField @nMobile, @nFocusParam      

Quit: 
   --WHILE @@TRANCOUNT > @nTranCount -- Commit until the level we started            
   --   COMMIT TRAN rdt_593ShipLabel14          
   --EXEC rdt.rdtSetFocusField @nMobile, @nFocusParam 
