...
/***************************************************************************/      
/* Store procedure: rdt.rdt_1841PrePltSort04                               */      
/*                                                                         */      
...
/* 2021-10-27  1.0  Chermaine  WMS-18096.Created (dup rdt_1841PrePltSort03)*/      
/***************************************************************************/      
      
CREATE PROC [RDT].[rdt_1841PrePltSort04] (      
   @nMobile         INT,      
   @nFunc           INT,      
...
AS      
   ...
   BEGIN TRAN      
   SAVE TRAN rdt_1841PrePltSort04      
      
   IF @cType = 'GET.POS'      
   BEGIN      
   --St7 Add New SKU already insert data to rdtPreReceiveSort
   IF EXISTS ( SELECT 1 FROM RDT.rdtPreReceiveSort WITH (NOLOCK)      
                  WHERE StorerKey = @cStorerkey      
                  AND   UCCNo = @cUCC)  AND    @nStep <> '7'   
                  --AND   [Status] = '1')      
      BEGIN      
         SET @nErrNo = 177701      
      ...
      --Get newUCC hav pre fix loc--step 7 = create new ucc
      IF EXISTS (SELECT 1 FROM RDT.rdtPreReceiveSort WITH (NOLOCK)      
                  WHERE StorerKey = @cStorerkey      
                  AND   UCCNo = @cUCC)  AND    @nStep = '7' 
      BEGIN
      	SELECT @cCode = Position FROM RDT.rdtPreReceiveSort WITH (NOLOCK)  WHERE StorerKey = @cStorerkey AND   UCCNo = @cUCC AND SKU = @cSKU
      END
      ...
      BEGIN      
         ...
         -- First 3 position reserved as at above      
       SELECT @nMaxPallet = MaxPallet      
         FROM dbo.LOC WITH (NOLOCK)       
         WHERE LOC = @cLane      
         AND   Facility = @cFacility      
               
         -- Max SKUGroup in 1 Pallet      
         SELECT @nMaxSKUGroup = Short      
         FROM codelkup WITH (NOLOCK)       
         WHERE ListName = 'PLTMaxSku'      
         AND   StorerKey = @cStorerkey      
         AND   code = @cSKUGroup      
      
         IF @nMaxPallet > 0      
            SELECT @nMaxAllowedPos = @nMaxPallet - 3      
         ELSE      
            SELECT @nMaxAllowedPos = COUNT( DISTINCT c.Code) - 3      
            FROM dbo.CODELKUP AS c WITH (NOLOCK)      
            WHERE c.LISTNAME = 'PreRcvLane'      
            AND   c.Storerkey = @cStorerKey      
      
--         SET @nNonImmediateNeedsPos = 7      
         SELECT @nNonImmediateNeedsPos = CAST(MIN(Code) AS INT)  
         FROM CODELKUP  
         WHERE LISTNAME = 'PreRcvLane'      
         AND   Storerkey = @cStorerKey      
         AND   Short <> 'R'      
         ...
         BEGIN      
            ...
            BEGIN      
               ...
               BEGIN      
                  ...
                  -- and pallet cannot hit maxSkuGroup Count      
                  IF @nPltAllowMixSKU > 0 AND (( @nSKUCount + @nNewSKU) > @nMaxSKUGroup)      
--                  IF @nSKUCount > 1 AND (( @nSKUCount + @nNewSKU) > @nPltAllowMixSKU) OR (@nSKUCount >= @nMaxSKUGroup)      
                     SET @cToID = ''      
      
                  --Disallow Pallet ID with different putawayzone as in FBR  
                  IF EXISTS (SELECT 1      
                             FROM RDT.rdtPreReceiveSort PRS WITH (NOLOCK)      
                             JOIN SKU WITH (NOLOCK) ON SKU.StorerKey = PRS.StorerKey AND PRS.Sku = SKU.Sku      
                             WHERE PRS.ReceiptKey = @cReceiptKey      
                             AND   PRS.LOC = @cLane      
                             AND   PRS.ID = @cToID      
                             AND   PRS.[Status] = '1'  
                             AND   SKU.Putawayzone <> @cPutawayZone) 
                             AND @nPltAllowMixSKU > 0 
                             AND (( @nSKUCount + @nNewSKU) <= @nMaxSKUGroup)   
                  BEGIN      
                     SET @cToID = ''      
                  END      
...
END            
   ...
   RollBackTran:      
         ROLLBACK TRAN rdt_1841PrePltSort04      
   Quit:      
