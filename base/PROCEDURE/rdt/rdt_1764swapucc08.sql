...
/******************************************************************************/
/* Store procedure: rdt_1764SwapUCC08                                         */
/* Copyright      : LF Logistics                                              */
/*                                                                            */
/* Date       Rev  Author    Purposes                                         */
/* 03-02-2023 1.0  Ung       WMS-20659 Created (based on rdt_1764SwapUCC02)   */
/******************************************************************************/

CREATE   PROCEDURE [RDT].[rdt_1764SwapUCC08]
   @nMobile          INT,
   @nFunc            INT,
...
BEGIN
   ...
   DECLARE @cSwapPackDetail    NVARCHAR(1)
   ...
   SELECT
      ...
      @cTaskSKU = SKU,
      @nTaskQTY = QTY,
      @nTaskSystemQTY = ISNULL( SystemQTY, 0),
      @nTaskReplenQTY = ISNULL( QTYReplen, 0),
   ...
   SET @cSwapPackDetail = rdt.rdtGetConfig( @nFunc, 'SwapPackDetail', @cStorerKey)
   ...
   -- Check L11 and L12 that blocked from picking
   IF EXISTS( SELECT 1 
      FROM dbo.LOTAttribute WITH (NOLOCK)
      WHERE LOT = @cActUCCLOT
         AND (Lottable11 = 'H'
          OR  Lottable12 = 'inaccessible'))
   BEGIN
      SET @nErrNo = 196165
      SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --UCC on HOLD
      GOTO Fail
   END
   ...
   BEGIN
      -- Allow in full pallet, half pallet locations only
      IF EXISTS( SELECT 1 FROM dbo.LOC WHERE LOC = @cTaskLOC AND LocationCategory NOT IN ('FP', 'HP'))
      BEGIN
         SET @nErrNo = 196113
...
*/
   ...
   SELECT
      ...
      @cActUOM = UOM,
      @nActReplenQTY = ISNULL( QTYReplen, 0)
   ...
   BEGIN
      ...
      -- Booking (at FROM)
      IF @nTaskReplenQTY <> @nActReplenQTY AND @cTaskLOT <> @cActUCCLOT
      BEGIN
         -- Task
         UPDATE dbo.LOTxLOCxID SET
            QTYReplen = QTYReplen - @nTaskReplenQTY + @nActReplenQTY, 
            EditDate = GETDATE(),  
            EditWho = SUSER_SNAME(), 
            TrafficCop = NULL 
         WHERE LOC = @cTaskLOC
         ...
         -- Actual
         UPDATE dbo.LOTxLOCxID SET
            QTYReplen = QTYReplen - @nActReplenQTY + @nTaskReplenQTY, 
            EditDate = GETDATE(),  
            EditWho = SUSER_SNAME(), 
            TrafficCop = NULL 
         WHERE LOC = @cActUCCLOC
            AND LOT = @cActUCCLOT
            AND ID = @cActUCCID
         IF @@ERROR <> 0
         BEGIN
...
CommitTran:
   ...
   COMMIT TRAN rdt_1764SwapUCC08
   GOTO Quit

RollBackTran:
   ROLLBACK TRAN rdt_1764SwapUCC08
Fail:
   SET @nUCCQTY = 0
Quit:
