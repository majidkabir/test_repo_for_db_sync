...
/******************************************************************************/
/* Store procedure: rdt_TM_NMVFrom_SwapTask                                   */
/*                                                                            */
/* Modifications log:                                                         */
/*                                                                            */
...
CREATE PROC [RDT].[rdt_TM_NMVFrom_SwapTask] (
   @nMobile           INT,
   @nFunc             INT,
...
BEGIN
   ...
   SAVE TRAN rdt_TM_NMVFrom_SwapTask -- For rollback or commit only our own transaction
   ...
   BEGIN
      -- Get storer config
      DECLARE @cGetSuggestedLOCSP NVARCHAR( 20)
      ...
      BEGIN
         ...
         BEGIN
            ...
            SELECT @cFacility = Facility FROM LOC WITH (NOLOCK) WHERE LOC = @cSuggFromLOC
...
/*
   ...
   END
*/
   COMMIT TRAN rdt_TM_NMVFrom_SwapTask -- Only commit change made here
   GOTO Quit

RollBackTran:
   ROLLBACK TRAN rdt_TM_NMVFrom_SwapTask -- Only rollback change made here
Fail:
Quit:
