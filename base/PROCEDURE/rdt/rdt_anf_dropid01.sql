...
/************************************************************************/  
/* Store procedure: rdt_ANF_DropID01                                    */  
/* Copyright      : IDS                                                 */  
/*                                                                      */  
...
/* 19-Jun-2014 1.0  James      SOS313608 - Created                      */  
/* 10-Dec-2014 1.1  James      Add TrafficCop when upd SOSTATUS(james01)*/  
/* 08-Jan-2015 1.2  James      SOS329888 - Update SOSTATUS PENDPACK for */  
/*                             conso pick by order level (james02)      */  
/* 07-Jul-2015 1.3  James      Perf tuning (james03)                    */
/* 11-Nov-2015 1.4  James      Comment PENDPACK (james04)               */
/* 12-Nov-2016 1.5  James      Replace batch UPDATE (james05)           */
/* 14-Nov-2017 1.6  James      Perf tuning. Filter blank dropid(james06)*/
/************************************************************************/  
  
CREATE PROC [RDT].[rdt_ANF_DropID01] (  
   @nMobile                   INT,             
   @nFunc                     INT,             
...
BEGIN  
   ...
   BEGIN TRAN  
   SAVE TRAN rdt_ANF_DropID01  

   SET @c_TraceName = 'rdt_ANF_DropID01'  
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         BEGIN  
            ...
            AND   [Status] <> '5'  
            OPEN CUR_UPD 
            FETCH NEXT FROM CUR_UPD INTO @cDropID2UPD
            WHILE @@FETCH_STATUS <> -1
            BEGIN
...
--             ,@c_Col1,@c_Col2,@c_Col3,@c_Col4,@c_Col5)  
   ...
   RollBackTran:  
      ROLLBACK TRAN rdt_ANF_DropID01  
  
   Quit:  
      WHILE @@TRANCOUNT > @nTranCount -- Commit until the level we started  
         COMMIT TRAN rdt_ANF_DropID01  
