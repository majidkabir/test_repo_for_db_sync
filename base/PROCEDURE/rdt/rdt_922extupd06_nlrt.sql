...
CREATE       PROC [RDT].[rdt_922ExtUpd06_NLRT] (
   @nMobile     INT,
   @nFunc       INT,
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               SAVE TRAN rdt_922ExtUpd06_NLRT -- For rollback or commit only our own transaction
               ...
               ***********************************************************************************************/
               ...
               SET @curPD = CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
                  ...
                  WHERE O.StorerKey = @cStorerKey
                     ...
                     AND PD.Status = '5'
                     AND PD.DROPID = @cLabelNo      --only check dropid for ACT
              ...
              WHERE OrderKey = @cParentOrderKey  
               ...
               WHERE OrderKey = @cParentOrderKey  
                  ...
                  WHERE OrderKey = @cParentOrderKey  
                     ...
                     --AND CaseID = @cLabelNo
                     AND Status = '5'
                     AND DROPID = @cLabelNo     --only check dropid for ACT
                  IF @@ERROR <> 0  
                  BEGIN
                     SET @nErrNo = 212011
                     SET @cErrMsg = rdt.rdtgetmessage( @nErrNo ,@cLangCode, 'DSP') --UPD OrdDtlFail
               ...
               ***********************************************************************************************/
               ...
               COMMIT TRAN rdt_922ExtUpd06_NLRT -- Only commit change made here
            END
         END
      END
...
RollBackTran:
   ROLLBACK TRAN rdt_922ExtUpd06_NLRT -- Only rollback change made here
Quit:
