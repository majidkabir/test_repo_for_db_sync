...
CREATE   PROC [RDT].[rdtfnc_SerialNo_Update] (
   @nMobile    INT,
   @nErrNo     INT  OUTPUT,
...
BEGIN
   ...
   IF @nStep = 1  GOTO Step_SerialNo    -- Scn = 6170. SerialNo
END

RETURN -- Do nothing if incorrect step

/********************************************************************************
Step_Start. Func = 1019
********************************************************************************/
Step_Start:
BEGIN
   ...
   IF NOT EXISTS (SELECT 1
                  ...
                  WHERE ListName = 'SNoUpd'
                     AND Storerkey = @cStorerKey
                     AND Code2 = @nFunc)
   BEGIN
      SET @nErrNo = 185351        
      SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --SNoLblNotExists      
      GOTO QUIT        
   END
...
BEGIN        
   ...
   BEGIN        
      ...
      BEGIN    
         SET @nErrNo = 196702        
         SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --NeedSNo     
         EXEC rdt.rdtSetFocusField @nMobile, 1
         GOTO Step1_SNO_Fail        
      END  
      

      IF NOT EXISTS (SELECT 1 FROM SERIALNO (NOLOCK)
                     WHERE storerkey=@cStorerkey
                        AND serialno=@cSerialNo
                        AND status='1')
      BEGIN    
         SET @nErrNo = 196703        
         SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --InvalidSNo       
         GOTO Step1_SNO_Fail        
      END 
      ...
      BEGIN TRAN
      SAVE TRAN rdtfnc_SerialNo_Update
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN 
               ...
               ROLLBACK TRAN rdtfnc_SerialNo_Update
               GOTO Quit 
            END
         END
      ...
      COMMIT TRAN rdtfnc_SerialNo_Update
