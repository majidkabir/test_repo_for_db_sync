...
GO
/************************************************************************/
/* Store procedure: rdt_Capture_SKUInfo_Confirm                         */
/* Copyright      : IDS                                                 */
/*                                                                      */
/* Purpose: Update SKU setting                                          */
/*                                                                      */
/* Called from: rdtfnc_Capture_SKUInfo                                  */
/*                                                                      */
/* Modifications log:                                                   */
/*                                                                      */
...
CREATE PROCEDURE [RDT].[rdt_Capture_SKUInfo_Confirm]
   @nMobile          INT,
   @nFunc            INT,
...
BEGIN
   ...
   DECLARE @cExtendedSKUInfoCfmSP   NVARCHAR( 20)
   
   -- Get storer config
   SET @cExtendedSKUInfoCfmSP = rdt.RDTGetConfig( @nFunc, 'ExtendedSKUInfoCfmSP', @cStorerKey)
   IF @cExtendedSKUInfoCfmSP = '0'  
      SET @cExtendedSKUInfoCfmSP = ''

   -- Extended cfm SP
   IF @cExtendedSKUInfoCfmSP <> ''
   BEGIN
      SET @cSQL = 'EXEC rdt.' + RTRIM( @cExtendedSKUInfoCfmSP) +
         ' @nMobile, @nFunc, @cLangCode, @cStorerKey, @cUCCNo, @cSKU, @nQty, ' +
         ' @cParam1Label, @cParam2Label, @cParam3Label, @cParam4Label, @cParam5Label, ' + 
         ' @cParam1Value, @cParam2Value, @cParam3Value, @cParam4Value, @cParam5Value, ' + 
