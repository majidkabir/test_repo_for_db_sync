...
/************************************************************************/
/* Store procedure: rdt_PTLStation_CreateTask_ToteID02                  */
/* Copyright      : LFLogistics                                         */
/*                                                                      */
/* Date        Rev  Author      Purposes                                */
/* 2021-04-12  1.0  James       WMS-15658. Created                      */
/* 2021-11-26  1.1  CheeMun     JSM-35556 Filter Pickdetail <> 4        */
/************************************************************************/

CREATE PROC [RDT].[rdt_PTLStation_CreateTask_ToteID02] (
    @nMobile      INT
   ,@nFunc        INT
...
BEGIN
   ...
   ***********************************************************************************************/
   ...
   SELECT DISTINCT OrderKey, CreatedPTLTran 
   FROM rdt.rdtPTLStationLog WITH (NOLOCK) 
   WHERE Station IN (@cStation1, @cStation2, @cStation3, @cStation4, @cStation5)
      AND OrderKey <> ''
      ...
      BEGIN
         ...
         BEGIN
            ...
            VALUES (
               ...
               @cOrderKey, @cStorerKey, @cSKU, @nPDQTY, 0,  @cScanID, @nFunc, @nRowRef, 'rdt_PTLStation_CreateTask_ToteID02')
            IF @@ERROR <> 0
            BEGIN
               SET @nErrNo = 101703
            ...
            SET @nTTLPDQTY = @nTTLPDQTY + @nPDQTY
         END
      END
      FETCH NEXT FROM @curPD INTO @cOrderKey, @cSKU, @nPDQTY
   END
   ...
   BEGIN
      SET @nPTLQty = 0
      SELECT @nPTLQty = ISNULL( SUM( Qty), 0)
      FROM PTL.PTLTran WITH (NOLOCK)
      WHERE DropID = @cScanID
      AND   Storerkey = @cStorerKey
      AND   Func = 805

      -- Picked/Packed qty + current ptl zone expected qty cannot exceed ucc qty
      IF (@nPTLQty + @nTTLPDQTY) > @nUCCQty
      BEGIN
         SET @nErrNo = 171653
         SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --PTLQTY>UCCQTY
         GOTO RollbackTran
      END
   END
