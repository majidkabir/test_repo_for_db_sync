...
GO
/************************************************************************/                
/* Store procedure: rdt_628Inquiry03                                    */                
/* Copyright      : IDS                                                 */                
/*                                                                      */                
/* Purpose: Inquiry customised rule stored proc filter lottable02       */                
/*                                                                      */                
/* Exceed version: 5.4                                                  */                
/*                                                                      */                
/* Modifications log:                                                   */                
...
/* 2019-03-06 1.0  ChewKP   WMS-8115. Created                           */            
/* 2019-06-25 1.1  KuanYee  INC0748412 Remove Lottable02 (KY01)         */          
/* 2019-06-25 1.2  Walt     INC0748412 Remove By Lot (WW01)             */            
/* 2019-08-30 1.3  James    WMS-10415 Remove Qty hold and replace       */
/*                          with Pendingmovein (james01)                */
/************************************************************************/                
                
CREATE PROCEDURE [RDT].[rdt_628Inquiry03] (                
   @nMobile         INT,            
   @nFunc           INT,            
   ...
   SELECT @cExecStatements = @cExecStatements +             
      ...
      --' AND   LA.Lottable02 = ''' + @cLottable02 + ''' ' +   --(KY01)         
   ...
   -- SET @cExecStatements = N' SELECT TOP 1 @cLOT = LLI.LOT, '   (WW01)         
   ...
   ' FROM dbo.LOTxLOCxID LLI WITH (NOLOCK) ' +             
   ' JOIN dbo.LOC LOC WITH (NOLOCK) ON (LOC.LOC = LLI.LOC) ' +             
   -- ' JOIN dbo.LOTAttribute LA WITH (NOLOCK) ON (LLI.LOT = LA.LOT) ' +   (WW01)          
   ' WHERE LLI.StorerKey = ''' + RTRIM(@cStorerKey)  + ''' ' +            
   ' AND   LOC.Facility = ''' + RTRIM(@cFacility)  + ''' ' +            
   ' AND ( LLI.QTY <> 0 OR LLI.QtyAllocated <> 0 OR LLI.QtyPicked <> 0 OR LLI.QtyExpected <> 0) ' +             
   --' AND   LA.Lottable02 = ''' + RTRIM(@cLottable02)  + ''' ' +   --(KY01)        
   ' AND   LOC.LocationType = ''PICK'' '          
   ...
   CASE WHEN @cInquiry_LOC <> '' THEN ' GROUP BY LLI.ID, LLI.SKU ORDER BY LLI.ID + LLI.SKU '            
   WHEN @cInquiry_ID <> '' THEN ' GROUP BY LLI.LOC, LLI.SKU ORDER BY LLI.LOC + LLI.SKU '            
   WHEN @cInquiry_SKU <> '' THEN ' GROUP BY  LLI.LOC, LLI.ID ORDER BY LLI.LOC + LLI.ID ' END      
/*    
   CASE WHEN @cInquiry_LOC <> '' THEN ' GROUP BY LLI.LOT, LLI.ID, LLI.SKU ORDER BY LLI.ID + LLI.SKU '            
   WHEN @cInquiry_ID <> '' THEN ' GROUP BY LLI.LOT, LLI.LOC, LLI.SKU ORDER BY LLI.LOC + LLI.SKU '            
   WHEN @cInquiry_SKU <> '' THEN ' GROUP BY LLI.LOT, LLI.LOC, LLI.ID ORDER BY LLI.LOC + LLI.ID ' END             
*/  --(WW01)    
    
   -- SET @cExecArguments = N'@cLOT          NVARCHAR( 10)  OUTPUT, ' +    (WW01)         
   SET @cExecArguments = N'@cLOC          NVARCHAR( 10)  OUTPUT, ' +             
                          '@cID           NVARCHAR( 18)  OUTPUT, ' +             
                          '@cSKU          NVARCHAR( 20)  OUTPUT, ' +             
                          '@nMQTY_Alloc   INT            OUTPUT, ' +            
                          '@nMQTY_Pick    INT            OUTPUT, ' +            
