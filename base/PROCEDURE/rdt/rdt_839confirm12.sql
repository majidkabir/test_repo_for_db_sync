...
CREATE   PROC [RDT].[rdt_839Confirm12] (  
   @nMobile       INT,             
   @nFunc         INT,             
...
BEGIN  
   ...
   SAVE TRAN rdt_839Confirm12 -- For rollback or commit only our own transaction    

   -- Short pick    
   IF @cType = 'SHORT' -- Don't need to split    
   BEGIN    
      SET @nErrNo = 189610    
      SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') -- NoAllowShort   
      GOTO RollBackTran    
   END   
...
RollBackTran:    
   ROLLBACK TRAN rdt_839Confirm12 -- Only rollback change made here    
Quit:    
