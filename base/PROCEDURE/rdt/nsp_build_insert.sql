...
GO
/************************************************************************/  
/* Stored Proc : RDT.nsp_Build_Insert                                   */  
/* Creation Date:                                                       */  
/* Copyright: IDS                                                       */  
/* Written by:                                                          */  
/*                                                                      */  
...
/************************************************************************/  
CREATE PROCEDURE [RDT].[nsp_Build_Insert]  
...
BEGIN   
   ...
   SELECT @c_sixteen           =  ' '  
   SELECT @c_sixteena          =  ' '  
   SELECT @c_sixteen = rtrim(@c_copyto_db) + '.rdt.' + rtrim(@c_tablename)  
   ...
   BEGIN   
      ...
      SELECT @c_errmsg = 'NSQL ' + CONVERT(char(5),@n_err)+':Table does not exist in Target Database ' +  
         @c_tablename + '(RDT.nsp_Build_Insert)' -- (Vicky01)  
   END     
   ...
   BEGIN  
      ...
      BEGIN    
         ...
         SELECT @c_errmsg = 'NSQL ' + CONVERT(char(5),@n_err)+':No rows or columns found for ' +  
            rtrim(@c_tablename) + '(RDT.nsp_Build_Insert)' -- (Vicky01)  
      END      
   END  
   ...
   BEGIN    
      ...
      BEGIN    
         ...
         SELECT @c_errmsg = CONVERT(char(250),@n_err)  
            + ':  Open of cursor failed. (RDT.nsp_Build_Insert) ' + ' ( ' +  
      ...
      BEGIN      
         ...
         BEGIN    
            ...
            SELECT @n_err = 73503  
            SELECT @c_errmsg = CONVERT(char(250),@n_err)  
               + ':  fetch failed. (RDT.nsp_Build_Insert) ' + ' ( ' +  
               ' SQLSvr MESSAGE = ' + ISNULL(LTRIM(RTRIM(@c_errmsg)), '') + ')' -- (Vicky01)  
         END     
         IF @n_continue = 1 or @n_continue = 2  
         BEGIN  
            ...
            BEGIN   
               ...
               BEGIN   
                  ...
                  IF ( @c_firsttime = 'Y')  
                  BEGIN   
                     SELECT @c_msg  = 'INSERT ' + rtrim(@c_copyto_db) + '.rdt.' + rtrim(@c_tablename) + '(' + @c_buildfieldstring  
                     SELECT @c_comma = ','  
                  END    
   ...
   BEGIN  
      ...
      BEGIN    
         ...
         IF ( @n_messageno = 1)  
         BEGIN    
            SELECT @c_one   = 'INSERT ' + rtrim(@c_copyto_db) + '.rdt.' + rtrim(@c_tablename) + '('+  
               @c_buildfieldstring + ')'  
            SELECT @c_onea =  'SELECT ' +@c_buildfieldstring  
            SELECT @c_buildfieldstring = ''  
         END      
   ...
   BEGIN  
      IF (@b_archive = 1)  
      BEGIN  
         Select @c_whereclause = ' Where archivecop = ''9'' ' -- (Vicky01)  
      END  
      ELSE  
      BEGIN  
   ...
   BEGIN  
      ...
      DECLARE @c_rdttablename NVARCHAR(50)  
      SET @c_rdttablename = 'rdt.' + @c_tablename  
      INSERT INTO #PrimaryKey (ColName, SeqNo)  
      EXEC ispPrimaryKeyColumns @c_rdttablename  
      ...
      BEGIN  
         ...
         SELECT @cSQL1 = @cSQL1 + ' FROM rdt.' + RTRIM(@c_tablename) + ' (NOLOCK) ' + @c_whereclause  + master.dbo.fnc_GetCharASCII(13)  
         ...
         SELECT @cSQL1 = @cSQL1 + '    IF NOT EXISTS(SELECT 1 FROM ' + RTRIM(@c_copyto_db) + '.rdt.'   
                + RTRIM(@c_tablename)   
...
----  
         SELECT @cSQL2 = @cSQL2 + '    IF EXISTS(SELECT 1 FROM ' + RTRIM(@c_copyto_db) + '.rdt.' + RTRIM(@c_tablename)   
         ...
         SELECT @cSQL2 = @cSQL2 + '       DELETE FROM rdt.' + RTRIM(@c_tablename) + @cWhereSQL + master.dbo.fnc_GetCharASCII(13)     
...
---   
         ...
         EXEC( @cSQL1 +   
               ...
               ' FROM rdt.' + @c_tablename + ' (NOLOCK) ' + @cWhereSQL +   
               @cSQL2 )  
         ...
         BEGIN  
            print @cSQL1 + master.dbo.fnc_GetCharASCII(13) +   
                  ...
                  @c_eighta+@c_ninea +   @c_tena +  @c_elevena +  @c_twelvea + @c_thirteena +  
                  @c_fourteena +  @c_fifteena + @c_sixteena +  
                  ' FROM rdt.' + RTRIM(@c_tablename) + ' (NOLOCK) ' + @cWhereSQL + master.dbo.fnc_GetCharASCII(13) +  
                  @cSQL2   
         END  
      ...
      BEGIN  
         ...
         @c_eighta+@c_ninea +   @c_tena +  @c_elevena +  @c_twelvea + @c_thirteena +  
         @c_fourteena +  @c_fifteena + @c_sixteena +  
         ' From rdt.' + @c_tablename + ' (NOLOCK) '  + @c_whereclause)    
      ...
      BEGIN    
         ...
         SELECT @n_err = 73504  
         SELECT @c_errmsg = CONVERT(char(250),@n_err)  
            + ':  dynamic execute failed. (RDT.nsp_Build_Insert) ' + ' ( ' +  
            ' SQLSvr MESSAGE = ' + ISNULL(LTRIM(RTRIM(@c_errmsg)), '') + ')' -- (Vicky01)  
      END    
  
  
  
--       IF (@c_firsttime = 'N' and @b_archive = 1)  
--       BEGIN   
--          exec (@c_one+@c_two+@c_three+@c_four+@c_five+@c_six+@c_seven+@c_eight+@c_nine+  
--          @c_ten + @c_eleven + @c_twelve + @c_thirteen + @c_fourteen + @c_fifteen +  
--          @c_sixteen +  @c_onea+@c_twoa+@c_threea+@c_foura+@c_fivea+@c_sixa+@c_sevena+  
--          @c_eighta+@c_ninea +   @c_tena +  @c_elevena +  @c_twelvea + @c_thirteena +  
--          @c_fourteena +  @c_fifteena + @c_sixteena +  
--          ' From ' + @c_tablename + ' (NOLOCK) ' + @c_whereclause)  
--       END      
--       IF (@c_firsttime = 'N' and @b_archive <> 1)  
--       BEGIN   
--          exec (@c_one+@c_two+@c_three+@c_four+@c_five+@c_six+@c_seven+@c_eight+@c_nine+  
...
--       END  
--   
--       IF (@c_firsttime = 'Y' and @b_archive <> 1)  
--       BEGIN     
--          exec (@c_one+@c_two+@c_three+@c_four+@c_five+@c_six+@c_seven+@c_eight+@c_nine+  
--          @c_ten + @c_eleven + @c_twelve + @c_thirteen + @c_fourteen + @c_fifteen +  
--          @c_sixteen +  @c_onea+@c_twoa+@c_threea+@c_foura+@c_fivea+@c_sixa+@c_sevena+  
...
--       END       
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         END  
      END  
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'RDT.nsp_Build_Insert'  
