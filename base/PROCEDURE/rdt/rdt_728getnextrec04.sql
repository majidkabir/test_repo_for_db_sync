...
/************************************************************************/
/* Store procedure: rdt_728GetNextRec04                                 */
/*                                                                      */
...
/* 2022-07-28  1.0  James      WMS-20270. Created                       */
/************************************************************************/

CREATE PROC [RDT].[rdt_728GetNextRec04] (
   @nMobile          INT,
   @nFunc            INT, 
...
BEGIN
   ...
   SELECT LLI.SKU, SL.LOCATIONTYPE, ISNULL( SUM( LLI.QTY - LLI.QTYPICKED - (CASE WHEN LLI.QtyReplen < 0 THEN 0 ELSE LLI.QtyReplen END)), 0)
   FROM dbo.LOTxLOCxID LLI WITH (NOLOCK) 
   JOIN dbo.LOC WITH (NOLOCK) ON (LLI.LOC = LOC.LOC)
   ...
   BEGIN
      ...
      BEGIN
         ...
         SET @cDisplay02 = SPACE( 6) + LEFT( CAST( @nLLI_Qty - @nPD_Qty AS NVARCHAR( 5)) + SPACE( 5), 5) + ' ' + @cLLI_LocType
      END

      IF @nLoop = 2
      BEGIN
         SET @cDisplay03 = @cLLI_SKU
         SET @cDisplay04 = SPACE( 6) + LEFT( CAST( @nLLI_Qty - @nPD_Qty AS NVARCHAR( 5)) + SPACE( 5), 5) + ' ' + @cLLI_LocType
      END

      IF @nLoop = 3
      BEGIN
         SET @cDisplay05 = @cLLI_SKU
         SET @cDisplay06 = SPACE( 6) + LEFT( CAST( @nLLI_Qty - @nPD_Qty AS NVARCHAR( 5)) + SPACE( 5), 5) + ' ' + @cLLI_LocType
      END

      IF @nLoop = 4
      BEGIN
         SET @cDisplay07 = @cLLI_SKU
         SET @cDisplay08 = SPACE( 6) + LEFT( CAST( @nLLI_Qty - @nPD_Qty AS NVARCHAR( 5)) + SPACE( 5), 5) + ' ' + @cLLI_LocType
      END
...
BEGIN
   ...
   SET @nLoop = 1
   DECLARE CUR_LOOP CURSOR LOCAL READ_ONLY FAST_FORWARD FOR 
   SELECT LLI.LOC, ISNULL( SUM( LLI.QTY - LLI.QTYPICKED - (CASE WHEN LLI.QtyReplen < 0 THEN 0 ELSE LLI.QtyReplen END)), 0)
   FROM dbo.LOTxLOCxID LLI WITH (NOLOCK) 
   JOIN dbo.LOC WITH (NOLOCK) ON (LLI.LOC = LOC.LOC)
   JOIN dbo.SKUxLOC SL WITH (NOLOCK) ON (LLI.LOC = SL.LOC AND LLI.SKU = SL.SKU)
   WHERE LLI.StorerKey = @cStorerKey
   ...
   BEGIN
      ...
      IF @nLoop = 1
         SET @cDisplay01 = LEFT( CAST( @cLLI_LOC AS NVARCHAR( 10)) + SPACE( 10), 10) + SPACE( 5) + CAST( @nLLI_Qty - @nPD_Qty AS NVARCHAR( 5))

      IF @nLoop = 2
         SET @cDisplay02 = LEFT( CAST( @cLLI_LOC AS NVARCHAR( 10)) + SPACE( 10), 10) + SPACE( 5) + CAST( @nLLI_Qty - @nPD_Qty AS NVARCHAR( 5))

      IF @nLoop = 3
         SET @cDisplay03 = LEFT( CAST( @cLLI_LOC AS NVARCHAR( 10)) + SPACE( 10), 10) + SPACE( 5) + CAST( @nLLI_Qty - @nPD_Qty AS NVARCHAR( 5))

      IF @nLoop = 4
         SET @cDisplay04 = LEFT( CAST( @cLLI_LOC AS NVARCHAR( 10)) + SPACE( 10), 10) + SPACE( 5) + CAST( @nLLI_Qty - @nPD_Qty AS NVARCHAR( 5))
