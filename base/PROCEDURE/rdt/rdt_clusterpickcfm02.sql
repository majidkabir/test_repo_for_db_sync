...
/************************************************************************/  
/* Store procedure: rdt_ClusterPickCfm02                                */  
/* Copyright      : IDS                                                 */  
/*                                                                      */  
...
/* 02-Jul-2015 1.0  James       SOS342407 Created                       */  
/* 29-Feb-2016 1.1  James       Fix weight calc issue (james01)         */   
/* 18-Sep-2018 1.2  LZG         INC0387458 - Added RDT StorerConfig to  */    
/*                              copy DropID to LabelNo (ZG01)           */    
/* 30-Jan-2020 1.3  James       WMS-11856 Don't update CartonType if    */  
/*                              ctntype = N (james02)                   */  
/* 28-Jul-2021 1.4  James       WMS-17474 Add config to group qty by    */
/*                              Sku + Loc (james03)                     */
/************************************************************************/  
  
CREATE PROC [RDT].[rdt_ClusterPickCfm02] (  
   @nMobile                   INT,   
   @nFunc                     INT,   
...
BEGIN  
   ...
   -- (james03)
   SET @cGroupBySKULOC = RDT.RDTGetConfig( @nFunc, 'GroupBySKULOC', @cStorerKey)
   ...
   WHERE StorerKey = @cStorerKey  
      AND OrderKey = @cOrderKey  
      AND SKU = @cSKU  
      AND (( @cGroupBySKULOC = '1' AND LOT = LOT) OR ( LOT = @cLOT))  
      AND LOC = @cLOC  
      AND Status = '1'  
