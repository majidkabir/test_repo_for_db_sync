...
/************************************************************************/          
/* Store procedure: rdt_841ExtUpdSP25                                   */          
/* Copyright      : LF                                                  */          
/*                                                                      */          
...
/* 2021-01-28  1.0 yeekung   WMS-18833 Created (dup rdt_841ExtUpdSP25)  */  
/* 2022-03-09  1.1 James     Fix last sku not pack confirm (james01)    */  
/*                           Add over pack checking                     */  
/* 2022-08-15  1.2 ChewKP    WMS-20582 To print PDF from MarketPlace    */
/*                            (ChewKP01)                                */
/************************************************************************/          
        
CREATE    PROC [RDT].[rdt_841ExtUpdSP25] (          
   @nMobile       INT,          
   @nFunc         INT,          
     ...
     SELECT @cWeight = Value FROM @tExtUpd WHERE Variable = '@cWeight'  
      ...
      BEGIN          
         ...
         BEGIN 
            ...
            SET @cFileName = @cFilePrefix + RTRIM(@cOrderKey) + '_' + RTRIM(@cExternOrderKey)  + '_.pdf'     
            SET @cPrintCommand = '"' + @cPrintFilePath + '" "' + @cFilePath + '\' + @cFileName + '" "50" "2" "' + @cWinPrinterName + '"'                              
         ...
         BEGIN
            ...
            IF @cShippLabel <> ''    
            BEGIN    
             SET @cCheckPaperType = rdt.RDTGetConfig( @nFunc, 'CheckPaperType', @cStorerkey)  
             IF @cCheckPaperType = '1'  
             BEGIN  
              SELECT   
        ...
        GOTO QUIT  
         ...
         BEGIN  
            ...
            BEGIN  
               ...
               ----EXEC [dbo].[nsp_QCmd_Backend_ValidateMBOL]   @c_MBOLKey = '0003930444' , @c_ValidatedFlag = 'N' , @b_Debug = 0   
               --SET @cCommand = N'EXEC [dbo].[nsp_QCmd_Backend_ValidateMBOL]' +        
         --                  N'  @c_MBOLKey = ''' + @cMBOLKey + ''' ' +         
               --                  N', @c_ValidatedFlag = ''N'' ' +              
               --                  N', @b_Debug = 0 '                                 
...
RollBackTran:                
   ROLLBACK TRAN rdt_841ExtUpdSP25 -- Only rollback change made here                
                
Quit:                
   WHILE @@TRANCOUNT > @nTranCount -- Commit until the level we started                
      COMMIT TRAN rdt_841ExtUpdSP25              
