...
/************************************************************************/
/* Store procedure: rdt_1841PrePltSort01                                */
/*                                                                      */
...
/* 2020-07-22  1.1  CheeMun    INC1213854 - Batch Delete UCC            */
/************************************************************************/

CREATE PROC [RDT].[rdt_1841PrePltSort01] (
   @nMobile         INT,
   @nFunc           INT,
...
AS
   ...
   BEGIN TRAN
   SAVE TRAN rdt_1841PrePltSort01
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            @cLane, '', @cCode, 'rdt_PrePltSortGetPos', @cItemClass, @cBUSR7, '1',
   ...
   BEGIN
      ...
      BEGIN
         ...
         GOTO RollBackTran
      END
      --IF @cUCCCounted >= @cBUSR7 AND rdt.rdtIsValidQty( @cBUSR7, 1) = 1
      --   SET @cClosePallet = '1'  
   ...
   RollBackTran:
         ROLLBACK TRAN rdt_1841PrePltSort01
   Quit:
