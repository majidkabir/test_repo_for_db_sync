...
GO
/************************************************************************/
/* Store procedure: rdtfnc_Container_Unload                             */
/* Copyright      : IDS                                                 */
/*                                                                      */
/* Purpose: To Track the time the Container Starts / Complete Unloading */
/*                                                                      */
/* Modifications log:                                                   */
/*                                                                      */
/* Date         Rev  Author     Purposes                                */
/* 2009-10-14   1.0  Vicky      Created                                 */
/* 2011-11-02   1.1  James      Prompt msg if container is finish       */
/*                              uploaded (james01)                      */
/* 2011-11-22   1.2  James      SOS230916 - Validate container no must  */
/*                              be scanned b4 using container arrive    */
/*                              module (james02)                        */
/* 2014-06-12   1.3  James      Change function id due to conflict with */
/*                              other module (james03)                  */
/* 2014-12-02   1.4  ChewKP     SOS#326760 Add Extended Update(ChewKP01)*/
/* 2016-09-30   1.5  Ung        Performance tuning                      */
/* 2018-10-24   1.6  TungGH     Performance                             */
/* 2019-01-02   1.7  Leong      INC0527496 - Bug Fix.                   */
/************************************************************************/

CREATE PROC [RDT].[rdtfnc_Container_Unload] (
   @nMobile    INT,
   @nErrNo     INT  OUTPUT,
...
DECLARE
   ...
   @cAppointmentNo      NVARCHAR(20),   -- (ChewKP01) -- INC0527496
   @cInput01            NVARCHAR(20),   -- (ChewKP01)
   @cInput02            NVARCHAR(20),   -- (ChewKP01)
   @cInput03            NVARCHAR(20),   -- (ChewKP01)
   @cInput04            NVARCHAR(20),   -- (ChewKP01)
   @cActionType         NVARCHAR(10),   -- (ChewKP01)
   @cRefNo1             NVARCHAR(20),   -- (ChewKP01)
   @cReadOnlyField      NVARCHAR(4),    -- (ChewKP01)
...
--   @cInput04          = V_String7,
   ...
   @cRefNo1           = V_String9,
   @cReadOnlyField    = V_String10,
   @cDefaultOption    = V_String11,
   @cDefaultCursor    = V_String12,
...
SELECT
   @nStep_1          = 1,  @nScn_1          = 2137,
   @nStep_2          = 2,  @nScn_2          = 2138


IF @nFunc = 858  -- (james03)
BEGIN
   -- Redirect to respective screen
   IF @nStep = 0  GOTO Step_Start       -- Menu. Func = 858
   IF @nStep = 1  GOTO Step_1           -- Scn = 2137. Container #
   IF @nStep = 2  GOTO Step_2           -- Scn = 2138. Confirm Msg
   IF @nStep = 3  GOTO Step_3           -- Scn = 2139. Confirm Msg
END
...
BEGIN
   ...
   SET @cReadOnlyField = ''
   SET @cReadOnlyField = rdt.RDTGetConfig( @nFunc, 'ReadOnlyField', @cStorerKey)
   ...
   EXEC RDT.rdt_STD_EventLog
     @cActionType   = '1', -- Sign In fucntion
     @cUserID       = @cUserName,
     @nMobileNo     = @nMobile,
     @nFunctionID   = @nFunc,
     @cFacility     = @cFacility,
...
--   BEGIN
--      SET @nErrNo = 67966
--      SET @cErrMsg = rdt.rdtgetmessage( 67966, @cLangCode, 'DSP') -- Container Req
--      GOTO Step_1_Fail
--   END
...
--                  WHERE EventType = 9 AND ActionType = 10
--                  AND RefNo1 = CASE WHEN RTRIM(@cContainerNo) = '' THEN RTRIM(@cAppointmentNo) ELSE RTRIM(@cContainerNo) END)
--   BEGIN
--      SET @cErrMsg = rdt.rdtgetmessage( 67973, @cLangCode, 'DSP') -- INV Container
--      GOTO Step_1_Fail
--   END
...
--     IF NOT EXISTS (SELECT 1 FROM RDT.rdtSTDEventLog WITH (NOLOCK)
--                WHERE EventType = 9 AND ActionType = 10
--                AND RefNo1 = RTRIM(@cContainerNo))
--     BEGIN
--         EXEC RDT.rdt_STD_EventLog
--             @cActionType   = '10', -- UNLOAD START
--             @cUserID       = @cUserName,
--             @nMobileNo     = @nMobile,
--             @nFunctionID   = @nFunc,
--             @cFacility     = @cFacility,
--             @cStorerKey    = '',
--             @cRefNo1       = @cContainerNo,
--             @cRefNo4       = @cMenuType,
--             @nStep         = @nStep
--
--             SET @nErrNo = 67967
--             SET @cErrMsg = rdt.rdtgetmessage( 67967, @cLangCode, 'DSP') -- Unload Starts
--      END
      ...
      BEGIN
         ...
         BEGIN
              ...
              BEGIN
                 ...
                 EXEC RDT.rdt_STD_EventLog
                      @cActionType   = @cActionType, -- UnLoad / Load Start
                      @cUserID       = @cUserName,
                      @nMobileNo     = @nMobile,
                      @nFunctionID   = @nFunc,
                      @cFacility     = @cFacility,
         ...
         BEGIN
            SET @nErrNo = 67974
            SET @cErrMsg = rdt.rdtgetmessage( 67974, @cLangCode, 'DSP') -- Load Done B4
            GOTO Step_1_Fail
         END
            -- Go to Confirm Complete Screen
--            SET @nScn = @nScn + 1
--            SET @nStep = @nStep + 1
--
--            SET @cOutField01 = '1'
...
--            GOTO QUIT
      ...
      BEGIN
            IF NOT EXISTS (SELECT 1 FROM RDT.rdtSTDEventLog WITH (NOLOCK)
                     WHERE EventType = 9 AND ActionType = '11' -- Load / UnLoad Done
                     AND ContainerNo = CASE WHEN RTRIM(@cContainerNo) = '' THEN RTRIM(@cAppointmentNo) ELSE RTRIM(@cContainerNo) END) -- INC0527496
            BEGIN
              IF @cExtendedUpdateSP <> ''
              BEGIN
            ...
            BEGIN
               SET @nErrNo = 67968
               SET @cErrMsg = rdt.rdtgetmessage( 67968, @cLangCode, 'DSP') -- Unload Done B4
               GOTO Step_1_Fail
            END
      END
      -- Screen mapping
   ...
   BEGIN
      ...
      EXEC RDT.rdt_STD_EventLog
         @cActionType   = '9', -- Sign Out fucntion
         @cUserID       = @cUserName,
         @nMobileNo     = @nMobile,
         @nFunctionID   = @nFunc,
         @cFacility     = @cFacility,
...
--   BEGIN
--       SET @nErrNo = 67969
--     SET @cErrMsg = rdt.rdtgetmessage( 67969, @cLangCode, 'DSP') --'Option Req
--     GOTO Step_2_Fail
--     END
--
...
--   BEGIN
--       SET @nErrNo = 67970
--       SET @cErrMsg = rdt.rdtgetmessage( 67970, @cLangCode, 'DSP') --'Invalid Option'
--       GOTO Step_2_Fail
--   END
--
--     IF @cOption = '1'
...
--
--             SET @nErrNo = 67971
--             SET @cErrMsg = rdt.rdtgetmessage( 67971, @cLangCode, 'DSP') -- Unload Complete
--
--       SET @cOutField01 = ''
--       SET @cContainerNo = ''
--
...
--
--        SET @nErrNo = 67972
--        SET @cErrMsg = rdt.rdtgetmessage( 67972, @cLangCode, 'DSP') -- Unload Not End
--
--         -- Go back to Prev screen
--         SET @nScn = @nScn - 1
--         SET @nStep = @nStep - 1
   ...
   BEGIN
      ...
      BEGIN
              ...
              BEGIN
                  ...
                  ELSE
                  BEGIN
                     IF CHARINDEX( '1', @cReadOnlyField ) > 0
                     BEGIN
                        SET @cFieldAttr04 = 'O'
                     END
                  END
                  ...
                  END
                  BEGIN
                     IF CHARINDEX( '2', @cReadOnlyField ) > 0
                     BEGIN
                        SET @cFieldAttr06 = 'O'
                     END
                  END
                  ...
                  END
                  BEGIN
                     IF CHARINDEX( '3', @cReadOnlyField ) > 0
                     BEGIN
                        SET @cFieldAttr08 = 'O'
                     END
                  END
                  ...
                  END
                  BEGIN
                     IF CHARINDEX( '4', @cReadOnlyField ) > 0
                     BEGIN
                        SET @cFieldAttr10 = 'O'
                     END
                  END
...
--      V_String7      = @cInput04,
      ...
      V_String9      = @cRefNo1,
      V_String10     = @cReadOnlyField,
      V_String11     = @cDefaultOption,
      V_String12     = @cDefaultCursor,
