...
/************************************************************************/    
/* Store procedure: rdt_862GetTask02                                    */    
/* Copyright      : LFLogistics                                         */    
/*                                                                      */    
/* Purpose: Get PickDetail regardless of UOM                            */    
/*                                                                      */    
/* Date       Rev  Author   Purposes                                    */    
/* 2016-05-19 1.9  Ung      SOS363736 Created. Not group by UOM         */
/************************************************************************/    
    
CREATE PROCEDURE [RDT].[rdt_862GetTask02] (    
   @n_Mobile        INT,
   @n_Func          INT, 
...
DECLARE 
   ...
   @nMstQTY        INT              -- Remaining QTY in master UOM    
...
IF (@c_ID + @c_SKU + @c_UOM + @c_Lottable1+ @c_Lottable2 + @c_Lottable3 + CONVERT( NVARCHAR( 20), @d_Lottable4, 112)) = '19000101' 
   SET @cCondition = ''
ELSE
   SET @cCondition = (@c_ID + @c_SKU + @c_UOM + @c_Lottable1 + @c_Lottable2 + @c_Lottable3 + CONVERT( NVARCHAR( 20), @d_Lottable4, 112)) 
...
BEGIN    
   ...
   WHERE RPL.PickslipNo = @c_PickSlipNo    
      ...
      AND LTRIM(PD.ID + PD.SKU + PD.UOM + LA.Lottable01 + LA.Lottable02 + LA.Lottable03 + CONVERT( NVARCHAR( 10), LA.Lottable04, 112)) >  
          @cCondition
   GROUP BY PD.ID, PD.SKU, LA.Lottable01, LA.Lottable02, LA.Lottable03, LA.Lottable04    
   ORDER BY PD.ID, PD.SKU, LA.Lottable01, LA.Lottable02, LA.Lottable03, LA.Lottable04    
END    
ELSE    
BEGIN    
...
-- Reassign back the value
SET @c_oFieled01 = CAST(@nTaskQTY AS INT)
SET @c_oFieled02 = CAST(@nTask AS INT)
SET @c_oFieled03 = @cUOMDesc 
SET @c_oFieled04 = @cPPK 
SET @c_oFieled05 = CAST(@nCaseCnt AS INT)
SET @c_oFieled06 = @cPrefUOM_Desc 
SET @c_oFieled07 = CAST(@nPrefQTY AS INT)
SET @c_oFieled08 = @cMstUOM_Desc 
SET @c_oFieled09 = CAST(@nMstQTY AS INT)
