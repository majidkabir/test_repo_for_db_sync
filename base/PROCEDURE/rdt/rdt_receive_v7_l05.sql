...
/******************************************************************************/
/* Store procedure: rdt_Receive_V7_L05                                        */
...
/* 2018-06-28 3.9  Ung         WMS-5564 Capture L05                           */
/* 2018-03-06 4.0  ChewKP      WMS-3872 Add Channel colum (cheWKP04)          */
/* 2019-03-21 4.1  Ung         INC0560080 Add RowVer for concurrent receive   */
/*                             Missing (NOLOCK)                               */
/* 2019-05-28 4.2  James       WMS8158 Add func id to config                  */
/*                             FinalizeReceiptDetail (james04)                */
/* 2019-05-27 4.3  James       WMS-9128 Add ASNStatus 1 (james05)             */
/* 2020-04-09 4.4  Ung         LWP-96 Performance tuning                      */
/******************************************************************************/

CREATE PROCEDURE [RDT].[rdt_Receive_V7_L05] (
   @nFunc          INT,
   @nMobile        INT,
...
SAVE TRAN rdt_Receive_V7_L05 -- For rollback or commit only our own transaction
...
BEGIN
   ...
   BEGIN
      ...
      SELECT
         ...
         ISNULL(@cPoLineNo, ''), @cSubreasonCode , @cReceiptLineNo_Borrowed , @cChannel
      FROM @tRD
   ...
   BEGIN
      ...
      */
      ...
      UPDATE dbo.ReceiptDetail WITH (ROWLOCK) SET
         ...
         SubreasonCode = CASE WHEN @nBeforeReceivedQTY <> @nOrg_BeforeReceivedQTY THEN @cSubreasonCode ELSE SubreasonCode END,
         EditDate = GETDATE(),
         EditWho = SUSER_SNAME()
...
ELSE
BEGIN
   COMMIT TRAN rdt_Receive_V7_L05 -- Only commit change made in here
   GOTO Quit
END

RollBackTran:
   ROLLBACK TRAN rdt_Receive_V7_L05
Fail:
Quit:
