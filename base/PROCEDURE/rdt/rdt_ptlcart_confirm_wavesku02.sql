...
/************************************************************************/
/* Store procedure: rdt_PTLCart_Confirm_WaveSKU02                       */
/* Copyright      : LF Logistics                                        */
/*                                                                      */
/* Purpose: Close working batch                                         */
/*                                                                      */
/* Date       Rev  Author      Purposes                                 */
/* 21-11-2021 1.0  yeekung     WMS-18793 Created                        */
/* 08-07-2022 1.1  Ung         WMS-19982 Add event log                  */
/************************************************************************/

CREATE PROC [RDT].[rdt_PTLCart_Confirm_WaveSKU02] (
    @nMobile    INT
   ,@nFunc      INT
...
BEGIN
   ...
   ***********************************************************************************************/
   ...
   BEGIN
      ...
      BEGIN
         ...
         SAVE TRAN rdt_PTLCart_Confirm_WaveSKU02 -- For rollback or commit only our own transaction
         ...
         -- Commit order level
         COMMIT TRAN rdt_PTLCart_Confirm_WaveSKU02
      ...
      BEGIN
        ...
        IF @cWSPTS='1'
         ...
         BEGIN
            ...
            IF ISNULL(@cShort,'') <> ''
            BEGIN
               DECLARE PickDropID_Cur CURSOR FAST_FORWARD READ_ONLY FOR
               SELECT DISTINCT PD.DropID FROM RDT.rdtPTLCartLog PTL WITH (NOLOCK)
               JOIN PICKDETAIL PD WITH (NOLOCK) ON PD.WaveKey = PTL.WaveKey AND PD.Sku = PTL.SKU AND PD.Storerkey = PTL.StorerKey
               JOIN ORDERS O WITH (NOLOCK) ON O.OrderKey = PD.OrderKey
               WHERE PTL.StorerKey = @cStorerKey
               ...
               FETCH NEXT FROM PickDropID_Cur INTO  @cToteID
               ...
               CLOSE PickDropID_Cur
               DEALLOCATE PickDropID_Cur
            END
         END
      END
   END
   ...
   ***********************************************************************************************/
   ...
   BEGIN
      ...
      IF @cType  in( 'CLOSETOTE','SHORTTOTE') and @nQTY>0
      BEGIN
         -- Get storer config
         EXEC nspGetRight
            @c_Facility   = @cFacility
...
RollBackTran:
   ROLLBACK TRAN rdt_PTLCart_Confirm_WaveSKU02 -- Only rollback change made here
Quit:
   WHILE @@TRANCOUNT > @nTranCount -- Commit until the level we started
      COMMIT TRAN
END
