...
/************************************************************************/    
/* Store procedure: rdt_PTLCart_Confirm_PickSlip                        */    
...
/* 19-09-2018 1.2  Grick       INC0387109 - Invalid column taken from   */
/*                                           PTLtran table (G01)        */ 
/************************************************************************/    
    
CREATE PROC [RDT].[rdt_PTLCart_Confirm_PickSlip] (    
    @nMobile    INT    
   ,@nFunc      INT    
   ...
   ***********************************************************************************************/    
   ...
   BEGIN    
      ...
      SET @curPTL = CURSOR LOCAL FAST_FORWARD READ_ONLY FOR    
         SELECT PTLKey, DevicePosition, ExpectedQTY, SourceKey  							--G01  
      ...
      BEGIN    
         ...
         SAVE TRAN rdt_PTLCart_Confirm_PickSlip -- For rollback or commit only our own transaction    
     ...
     IF @cPSType = 'CONSO'    
         ...
         -- Commit order level    
         COMMIT TRAN rdt_PTLCart_Confirm_PickSlip    
 ...
 AND PD.Status < '4'    
                  ...
                  BEGIN    
                     ...
                     UPDATE PickDetail SET    
                        ...
                        DropID = @cActToteID,     
                        EditWho = SUSER_SNAME(),     
                        EditDate = GETDATE()                         WHERE PickDetailKey = @cPickDetailKey    
                     IF @@ERROR <> 0    
                     BEGIN    
                        SET @nErrNo = 54921    
                        SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --UPD PKDtl Fail    
...
RollBackTran:    
   ROLLBACK TRAN rdt_PTLCart_Confirm_PickSlip -- Only rollback change made here    
Quit:    
   WHILE @@TRANCOUNT > @nTranCount -- Commit until the level we started    
      COMMIT TRAN    
END 
