...
/******************************************************************************/  
/* Store procedure: rdt_608ExtUpd02                                           */  
        ...
        BEGIN 
            ...
            BEGIN 
               ...
               BEGIN
                  ...
                  VALUES (@cStorerKey, @cSKU, @cLOT, @cLOC, @cID, @cSuggToLOC, '', SUSER_SNAME(), @nQTY, '')  
            ...
            BEGIN 
                  ...
                  SELECT  TOP 1  
                     @cSuggToLOC = SL.LOC  
                  FROM dbo.SKUxLoc SL WITH (NOLOCK) 
                  INNER JOIN dbo.SKU SKU WITH (NOLOCK) ON SKU.SKU = SL.SKU AND SKU.StorerKey = SL.StorerKey
...
--                  SAVE TRAN rdt_608ExtUpd02 -- For rollback or commit only our own transaction  
--                    
...
--                  ELSE IF NOT EXISTS( SELECT 1 FROM LOT (NOLOCK) WHERE LOT = @cLOT)  
--                  BEGIN  
...
RollBackTran:  
   ROLLBACK TRAN rdt_608ExtUpd02 -- Only rollback change made here  
Quit:  
