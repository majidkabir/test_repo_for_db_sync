...
/******************************************************************************/
/* Store procedure: rdt_1580ExtUpd06                                          */
/* Copyright      : LF logistics                                              */
/*                                                                            */
/* Purpose: Update ReceiptDetail.UserDefine01 as serial no                    */
/*                                                                            */
/* Date        Rev  Author      Purposes                                      */
/* 28-08-2018  1.0  ChewKP      WMS-6089 Created                              */
/******************************************************************************/

CREATE PROCEDURE [RDT].[rdt_1580ExtUpd06]
    @nMobile      INT
   ,@nFunc        INT
   ,@nStep        INT
   ,@nInputKey    INT
...
BEGIN
   ...
   SAVE TRAN rdt_1580ExtUpd06 -- For rollback or commit only our own transaction
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
              ...
              BEGIN
                 SET @nErrNo = 128401
                 SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') -- IDNotFullyReceived
                 GOTO RollBackTran
              END
...
RollBackTran:
   ROLLBACK TRAN rdt_1580ExtUpd06 -- Only rollback change made here
Quit:
   WHILE @@TRANCOUNT > @nTranCount -- Commit until the level we started
      COMMIT TRAN rdt_1580ExtUpd06
