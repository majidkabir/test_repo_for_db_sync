...
GO
/************************************************************************/
/* Store procedure: 1768ExtendedCfmSP01                   */
/* Copyright      : IDS                                                 */
/*                                                                      */
...
CREATE   PROC [RDT].[1768ExtendedCfmSP01] (
    @nMobile         INT,           
    @nFunc           INT,           
...
--GOH01 End
   ...
   IF ISNULL(@cEnableAllLottables,'') = '1'
      SET @dNewLottable05 = @dLottable05
   BEGIN TRAN
   ...
   BEGIN
      IF EXISTS ( SELECT 1 FROM dbo.CCDetail WITH (NOLOCK)
                  ...
                  AND IsNULL( Lottable14, 0) = IsNULL( @dLottable14, 0)
                  AND IsNULL( Lottable15, 0) = IsNULL( @dLottable15, 0)
   ...
   BEGIN
      IF EXISTS ( SELECT 1 FROM dbo.CCDetail WITH (NOLOCK)
                  ...
                  AND Lottable11 = @cLottable11
                  AND Lottable12 = @cLottable12
                  AND IsNULL( CONVERT( DATETIME, CONVERT( NVARCHAR( 10), Lottable13, 120), 120), 0) = IsNULL( @dLottable13, 0)  --(JH01)
				  AND IsNULL( CONVERT( DATETIME, CONVERT( NVARCHAR( 10), Lottable14, 120), 120), 0) = IsNULL( @dLottable14, 0)  --(JH01)
                  AND IsNULL( CONVERT( DATETIME, CONVERT( NVARCHAR( 10), Lottable15, 120), 120), 0) = IsNULL( @dLottable15, 0)  --(JH01)
                  AND CCSheetNo  = @cTaskDetailKey  )
      BEGIN
         SELECT @nTotalRecord = COUNT(1) FROM dbo.CCDetail WITH (NOLOCK)
         WHERE CCKey    = @cCCKey
   ...
   BEGIN
      ...
      INSERT INTO dbo.CCDetail (
               ...
               Lottable10, Lottable11, Lottable12, Lottable13,Lottable14, Lottable15, SystemQty, RefNo, Status        )
      VALUES ( @cCCKey, @cNewCCDetailKey, @cStorerKey, @cSKU, '', @cLoc, @cID, @nQty, @cTaskDetailKey, @cLottable01,
             @cLottable02, @cLottable03, @dLottable04, @dNewLottable05,@cLottable06, @cLottable07, @cLottable08, @cLottable09, @cLottable10,    
               @cLottable11, @cLottable12, @dLottable13, @dLottable14, @dLottable15, 0, '', '4' )
