...
/******************************************************************************/
/* Store procedure: rdtfnc_BuildToUCC                                         */
/* Copyright      : IDS                                                       */
/*                                                                            */
/* Purpose: Build To UCC                                                      */
/*                                                                            */
/* Modifications log:                                                         */
/*                                                                            */
...
DECLARE
 @cCustomFetchTask_SP        	 NVARCHAR( 20),
...
SELECT
   ...
   @nFromScn            = V_String35,
   @cLottableCode      	= V_String36,
   @cLOCLookupSP        = V_String37,
...
IF @nFunc = 1832
BEGIN
   IF @nStep = 0 GOTO Step_0   -- Menu. Func = 1832
   IF @nStep = 1 GOTO Step_1   -- Scn = 5330  Scan FROM LOC screen
   IF @nStep = 2 GOTO Step_2   -- Scn = 5331  Scan FROM ID screen
   IF @nStep = 3 GOTO Step_3   -- Scn = 5332  Scan TO LOC screen
   IF @nStep = 4 GOTO Step_4   -- Scn = 5333  Scan TO ID screen
   IF @nStep = 5 GOTO Step_5   -- Scn = 5334  Scan SKU screen
   IF @nStep = 6 GOTO Step_6   -- Scn = 5335  lotabble02  screen (optional)
   IF @nStep = 7 GOTO Step_7   -- Scn = 5336  Enter QTY MOVE screen
   IF @nStep = 8 GOTO Step_8   -- Scn = 5337  Scan TO UCC screen
   IF @nStep = 9 GOTO Step_9   -- Scn = 5337  Close Pallet screen
...
/********************************************************************************
Step 0. Called from menu (func = 1832)
********************************************************************************/
...
BEGIN
   ...
   IF EXISTS (SELECT 1 FROM dbo.StorerGroup WITH (NOLOCK) WHERE StorerGroup = @cStorerKey)
      SET @nMultiStorer = 1
   IF EXISTS (SELECT 1 FROM rdt.rdtlottablecode WITH (NOLOCK) WHERE Storerkey= @cStorerKey AND function_id=@nFunc )
      SELECT @cLottableCode = lottablecode FROM rdt.rdtlottablecode WITH (NOLOCK) WHERE Storerkey= @cStorerKey AND function_id=@nFunc;
   ...
   SET @cOutField01 = @cDefaultFromLOC
   SET @cOutField02 = ''
   SET @cOutField03 = ''
   SET @cOutField04 = ''
   SET @cOutField05 = ''
...
BEGIN
	...
	BEGIN
	   ...
	   IF ISNULL(@cDecodeLabelNo,'') NOT IN ('','0')
		...
		IF EXISTS (SELECT 1 FROM UCC WITH (NOLOCK) WHERE Storerkey=@cStorerKey AND Loc=@cToLoc AND id=@c_ID)
		BEGIN
			IF NOT EXISTS (SELECT 1 FROM UCC WITH (NOLOCK) WHERE Storerkey=@cStorerKey AND Loc=@cToLoc AND id=@c_ID AND sku=@cSKU)
			BEGIN
				SET @nErrNo = 134414
				SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --'INV ID/SKU'
				GOTO Step_5_Fail
			END
		END

		-- Dynamic lottable
		EXEC rdt.rdt_Lottable @nMobile, @nFunc, @cLangCode, @nScn, @nInputKey, @cStorerKey, @cSKU, @cLottableCode, 'CAPTURE', 'POPULATE', 1, 1,
		...
		BEGIN
			...
			SET @nCountLot = 1
			SET @cOutField08 = CAST ( @nCountLot AS NVARCHAR(2) ) + '/' + CAST ( @nCountTotalLot AS NVARCHAR(4) )
			...
			ELSE
			BEGIN
				SET @cOutField12 = CAST( @nPQTY_Avail AS NVARCHAR( 7))
			END

			SET @cOutField09 = CAST(@nPUOM_DIV AS NCHAR(6))  + ' ' + @cPUOM_Desc + ' ' + @cMUOM_Desc
			SET @cOutField13 = CASE WHEN ISNULL(@cAvlQTY, '') <> '' THEN CAST(@cAvlQTY AS NVARCHAR(7)) ELSE CAST( @nMQTY_Avail AS NVARCHAR( 7)) END
...
/********************************************************************************
Step 6. Scn = 3990. Dynamic lottables
...
BEGIN
   ...
   BEGIN
		-- Dynamic lottable
		EXEC rdt.rdt_Lottable @nMobile, @nFunc, @cLangCode, @nScn, @nInputKey, @cStorerKey, @cSKU, @cLottableCode, 'CAPTURE', 'CHECK', 1, 1,
		@cInField01  OUTPUT,  @cOutField01 OUTPUT,  @cFieldAttr01 OUTPUT,  @cLottable01 OUTPUT,
		@cInField02  OUTPUT,  @cOutField02 OUTPUT,  @cFieldAttr02 OUTPUT,  @cLottable02 OUTPUT,
		@cInField03  OUTPUT,  @cOutField03 OUTPUT,  @cFieldAttr03 OUTPUT,  @cLottable03 OUTPUT,
		@cInField04  OUTPUT,  @cOutField04 OUTPUT,  @cFieldAttr04 OUTPUT,  @dLottable04 OUTPUT,
		...
		BEGIN
			SET @nErrNo = 134416
			SET @cErrMsg = rdt.rdtgetmessage(@nErrNo, @cLangCode, 'DSP') --INV LOTT02
			GOTO Step_6_Fail
		END
		...
		SET @cFieldAttr02 = '' -- Dynamic lottable
		SET @nScn = @nFromScn+1
		SET @nStep = @nStep + 1
	END
...
--      END
   ...
   BEGIN
      ...
      SET @cOutField13 = ''
		...
		--If dyanmic lottable is available
		IF @cLottableCode <> ''
		BEGIN
			SET @nScn = 3990
			SET @nStep = @nStep - 1
      ...
      BEGIN
         ...
         BEGIN
            ...
            SET @cSQLParam =
               ...
               ' @cErrMsg        NVARCHAR( 20) OUTPUT  '
            EXEC sp_ExecuteSQL @cSQL, @cSQLParam,
               @nMobile, @nFunc, @cLangCode, @nStep, 7, @nInputKey, @cFacility, @cStorerKey, @tVar,
               @cExtendedInfo OUTPUT, @nErrNo OUTPUT, @cErrMsg OUTPUT
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         -- Prep next screen var
         SET @cOutField01 = @cDefaultFromLOC  -- need change
...
BEGIN
   UPDATE RDTMOBREC WITH (ROWLOCK) SET
      ...
      V_String34    = @cConfirmSP, -- (ChewKP07)
		V_String35    = @nFromScn,
		V_String36	  = @cLottableCode,
		V_String37	  = @cLOCLookupSP,
