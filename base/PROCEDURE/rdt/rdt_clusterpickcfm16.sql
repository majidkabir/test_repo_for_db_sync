...
GO
/************************************************************************/
/* Store procedure: rdt_ClusterPickCfm16                                */
/* Copyright      : IDS                                                 */
/*                                                                      */
/* Purpose: Levis cluster pick (1628).                                  */
/* Doctype = N (ins packdetail); Doctype = E (don't ins packdetail)     */
...
CREATE PROC [RDT].[rdt_ClusterPickCfm16] (
   @nMobile                   INT,           
   @nFunc                     INT,           
...
BEGIN
   ...
   DECLARE @cClusterPickInsPackDt            NVARCHAR( 1)
   ...
   -- If doctype = 'E' then no need insert packdetail
   IF EXISTS ( SELECT 1 FROM dbo.ORDERS WITH (NOLOCK) 
               WHERE OrderKey = @cOrderKey
               AND   DocType = 'E')
      SET @cClusterPickInsPackDt = '0'
      ...
      BEGIN
         ...
         BEGIN
            ...
            UPDATE dbo.PickDetail WITH (ROWLOCK) SET
               EditWho = SUSER_SNAME(),
               EditDate = GETDATE(),
               DropID = CASE WHEN ISNULL(@cLoadDefaultPickMethod, '') = 'C' AND @cStatus = '4' THEN '' 
                        ELSE @cDropID END,
               Status = '4'   -- short pick
         ...
         IF @cClusterPickInsPackDt = '0'
         BEGIN
            IF NOT EXISTS ( SELECT 1 FROM dbo.Dropid WITH (NOLOCK)
                            WHERE Dropid = @cDropID)
            BEGIN
               INSERT INTO dbo.DropID (DropID, DropIDType, Loadkey, PickSlipNo) VALUES
               (@cDropID, 'SINGLES', @cLoadKey, @cPickSlipNo)
            ...
            BEGIN
               INSERT INTO dbo.DropIDDetail (DropID, ChildId) VALUES
               (@cDropID, @cOrderKey)
            ...
            IF NOT EXISTS ( SELECT 1 FROM dbo.PICKDETAIL PD WITH (NOLOCK)
                            ...
                            AND   ( PD.[Status] < '5' OR PD.[Status] = '4')
                            AND   PD.QTY > 0)
            BEGIN
