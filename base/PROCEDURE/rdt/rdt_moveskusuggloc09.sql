...
/************************************************************************/
/* Store procedure: rdt_MoveSKUSuggLoc09                                */
/* Copyright      : IDS                                                 */
/*                                                                      */
...
/* 05-03-2021  1.0  Chermaine   WMS-16503 dup: rdt_MoveSKUSuggLoc02     */
/************************************************************************/

CREATE PROC [RDT].[rdt_MoveSKUSuggLoc09] (
   @nMobile       INT,
   @nFunc         INT,
...
BEGIN
   ...
   BEGIN
      ...
      If the scanned sku have stock in normal location (Loc.hostwhcode = ‘ITX-AVA’),
      show the five location IDs of least stock on screen by acsending .If no stock,
      show the  alert message ‘New location ’ on  screen
      */
	  ...
	  FROM DBO.LOTXLOCXID AS LLI(NOLOCK)
	  JOIN DBO.LOTATTRIBUTE AS LA(NOLOCK) ON LLI.LOT = LA.LOT
	  WHERE LLI.SKU = @cSKU
