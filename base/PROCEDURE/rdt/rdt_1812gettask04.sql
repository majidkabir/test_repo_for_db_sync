...
/************************************************************************/
/* Store procedure: rdt_1812GetTask04                                   */
...
/* 28-05-2018  1.0  Ung       WMS-4221 Created                          */
/************************************************************************/

CREATE PROC [RDT].[rdt_1812GetTask04] (
   @nMobile          INT,
   @nFunc            INT,
...
BEGIN
   ...
   IF @cNewTaskKey = ''
   BEGIN
      -- Release skipped task
      IF @cGroupKey <> ''
      BEGIN
         IF EXISTS( SELECT 1 
            FROM TaskDetail TD WITH (NOLOCK) 
               JOIN TaskManagerSkipTasks S WITH (NOLOCK) ON (TD.TaskDetailKey = S.TaskDetailKey AND S.UserID <> '')
            WHERE TD.TaskType = @cTaskType
               AND TD.StorerKey = @cStorerKey
               AND TD.GroupKey = @cGroupKey)
         BEGIN
            DELETE TaskManagerSkipTasks 
            WHERE TaskDetailKey IN 
               (SELECT TaskDetailKey FROM TaskDetail TD WITH (NOLOCK) WHERE TD.GroupKey = @cGroupKey)
            SET @nErrNo = 124304
            SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --RetrySkipTask
            GOTO Fail         
         END
      END
