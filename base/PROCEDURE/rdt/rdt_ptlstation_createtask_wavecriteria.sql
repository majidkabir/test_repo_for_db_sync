...
/************************************************************************/
/* Store procedure: rdt_PTLStation_CreateTask_WaveCriteria              */
/* Copyright      : LFLogistics                                         */
/*                                                                      */
/* Date        Rev  Author      Purposes                                */
/* 16-03-2016  1.0  Ung         SOS360565 Created                       */
/* 12-07-2017  1.1  Ung         WMS-2410 Clear scan ID if not task      */
/* 18-07-2018  1.2  ChewKP      WMS-5759 Change Criteria (ChewKP01)     */
/************************************************************************/

CREATE PROC [RDT].[rdt_PTLStation_CreateTask_WaveCriteria] (
    @nMobile      INT
   ,@nFunc        INT
...
BEGIN
   ...
   ***********************************************************************************************/
   ...
   SET @curPD = CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
      SELECT WD.WaveKey, O.M_Address4, O.UserDefine03, O.OrderKey, PD.DropID, PD.SKU, SUM( PD.QTY)
      FROM WaveDetail WD WITH (NOLOCK) 
         JOIN Orders O WITH (NOLOCK) ON (WD.OrderKey = O.OrderKey)
         JOIN PickDetail PD WITH (NOLOCK) ON (PD.OrderKey = O.OrderKey)
         JOIN @tWaveCriteria t ON (t.WaveKey = WD.WaveKey AND t.Criteria1 = O.M_Address4 AND t.Criteria2 = O.UserDefine03)
      WHERE PD.StorerKey = @cStorerKey
         AND PD.DropID = @cScanID
         AND PD.Status <= '5'
         AND PD.CaseID = ''
         ...
         AND O.Status <> 'CANC' 
         AND O.SOStatus <> 'CANC'
      GROUP BY WD.WaveKey, O.M_Address4, O.UserDefine03, O.OrderKey, PD.SKU, PD.DropID
   OPEN @curPD
      ...
      BEGIN
         ...
         BEGIN
            ...
            VALUES (
               ...
               @cOrderKey, @cStorerKey, @cSKU, @nPDQTY, 0, @cDropID, @nFunc, @nRowRef, 'rdt_PTLStation_CreateTask_WaveCriteria')
            IF @@ERROR <> 0
            BEGIN
               SET @nErrNo = 98052
