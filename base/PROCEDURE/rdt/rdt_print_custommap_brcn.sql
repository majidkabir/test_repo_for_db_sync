...
/******************************************************************************/
/* Store procedure: rdt_Print_CustomMap_BRCN                                  */
...
CREATE PROC [RDT].[rdt_Print_CustomMap_BRCN] (
   @nMobile       INT,           
   @nFunc         INT,           
...
BEGIN
   ...
   BEGIN
      ...
      SELECT @cUDF10 = UserDefine10 FROM Orders WITH (NOLOCK) WHERE OrderKey = @cOrderKey 
      SELECT @cShort = ISNULL( Short, '') FROM CodeLKUP WITH (NOLOCK) WHERE ListName = 'BRRegion' AND Code = @cUDF10
      ...
      IF @cValue02 = 'AU' SET @cDataWindow = 'r_dw_packing_list_by_ctn18_rdt' ELSE            
      IF @cValue02 = 'US' SET @cDataWindow = 'TBC'                            ELSE SET @cDataWindow = ''
