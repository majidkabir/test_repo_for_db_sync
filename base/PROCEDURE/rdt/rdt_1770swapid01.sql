...
/************************************************************************/
/* Store procedure: rdt_1770SwapID01                                    */
/* Copyright      : LFLogistics                                         */
/*                                                                      */
/* Purpose: Swap ID base on same LOC, SKU, L04, QTY                     */
/*                                                                      */
/* Date        Rev  Author      Purposes                                */
/* 07-07-2014  1.0  Ung         Created SOS311415                       */
/* 15-06-2015  1.1  Ung         SOS343362 Remove check L4               */
/************************************************************************/

CREATE PROCEDURE [RDT].[rdt_1770SwapID01]
   @nMobile           INT,
   @nFunc             INT,
...
/*      
      ...
      END
*/
      -- Check LOC commingle LOT
      IF NOT EXISTS( SELECT 1 FROM LOC WITH (NOLOCK) WHERE LOC = @cTaskLOC AND CommingleLOT = '1')
      BEGIN
         SET @nErrNo = 90858
         SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --MixLOTNotAllow
         RETURN
      END
   END
...
CommitTran:
   COMMIT TRAN rdt_1770SwapID01
   GOTO Quit

RollBackTran:
      ROLLBACK TRAN rdt_1770SwapID01
Quit:
