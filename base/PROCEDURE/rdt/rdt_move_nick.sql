...
CREATE    PROCEDURE [RDT].[rdt_Move_Nick] (    
   @nMobile     INT,    
...
BEGIN    
   ...
   SET @cChkStatus = '1' + @cUCCAllocStatus    
   ...
   ELSE    
      ...
      WHERE StorerKey = @cStorerKey    
            AND UCCNo = @cUCC    
            AND Status IN ('1', @cUCCAllocStatus) -- Received, , Allocated    
...
SAVE TRAN rdt_Move_Nick -- For rollback or commit only our own transaction    
      ...
      BEGIN    
         ...
         IF @cFromID IS NULL    
            ...
            WHERE UCC.StorerKey = @cStorerKey    
               ...
               AND UCC.Status IN ('1', @cUCCAllocStatus) -- Received, , Allocated    
               AND UCC.Status <> ''    
         ELSE    
            SELECT @nFromLOC_UCC = IsNULL( SUM( UCC.QTY), 0)    
            FROM dbo.UCC UCC (NOLOCK)    
...
COMMIT TRAN rdt_Move_Nick -- Only commit change made in rdt_Move_Nick    
GOTO Quit    
    
RollBackTran:    
   ROLLBACK TRAN rdt_Move_Nick -- Only rollback change made in rdt_Move_Nick    
Quit:    
