...
/******************************************************************************/              
/* Store procedure: rdt_1580RcptCfm03SNo                                      */              
/* Copyright      : IDS                                                       */              
/*                                                                            */              
/* Purpose: Lookup serial no (UserDefine01) on ReceiptDetail lines to receive */              
/*                                                                            */              
/* Date       Rev  Author      Purposes                                       */              
/* 2017-05-04 3.2  Ung         Copy from rdt_receive                          */              
/*                             WMS-1817 Add serial no                         */              
/*                             Split line by serial no (UserDefine01)         */              
/* 2017-07-13 3.3  Ung         Performance tuning                             */              
/* 2019-05-27 3.4  James       WMS-9128 Add ASNStatus 1 (james01)             */              
/* 2020-03-03 3.5  WinSern     INC1062089 Fix lineno reset to 00001 (ws01)    */   
/******************************************************************************/              
              
CREATE PROCEDURE [RDT].[rdt_1580RcptCfm03SNo] (              
   @nFunc          INT,              
   @nMobile        INT,              
...
IF @cDocType = 'A'              
   ...
   '    AND (UserDefine01 = @cSerialNo OR UserDefine01 = '''') '               
...
INSERT INTO @tRD (ReceiptLineNumber, POLineNumber, QTYExpected, BeforeReceivedQTY,              
   ...
   UserDefine01, UserDefine02, UserDefine03, UserDefine04, UserDefine05,              
   UserDefine06, UserDefine07, UserDefine08, UserDefine09, UserDefine10,              
   ExternPoKey, POKey, UOM, EditDate)              
...
IF @cSkipLottable04 = '1' UPDATE @tRD SET Lottable04 = NULL              
...
BEGIN              
   ...
   WHERE FinalizeFlag <> 'Y'              
      ...
      AND (UserDefine01 = '' OR UserDefine01 = @cSerialNo)               
      AND              
      (  -- Blank lottable              
         (Lottable01 = '' AND              
          Lottable02 = '' AND              
...
SAVE TRAN rdt_1580RcptCfm03SNo -- For rollback or commit only our own transaction              
...
SET @curRD = CURSOR FOR              
   SELECT RowRef,               
      ...
      ReceiptLine_Borrowed -- Added By Vicky              
   FROM @tRD              
...
FETCH NEXT FROM @curRD INTO @nRowRef,               
      ...
      @cReceiptLineNo_Borrowed -- Added By Vicky              
...
BEGIN        
   ...
   SET @cUserDefine08 = ''              
         SET @cUserDefine09 = ''             SET @cUserDefine10 = ''              
         SET @cPoLineNo = ''              
      END              
...
@cAltSku              = AltSku             ,              
    ...
    @cUserDefine09        = UserDefine09       ,              
      ...
      INSERT INTO dbo.ReceiptDetail              
         ...
         UserDefine01, UserDefine02, UserDefine03, UserDefine04, UserDefine05,              
         UserDefine06, UserDefine07, UserDefine08, UserDefine09, UserDefine10, POLineNumber, SubReasonCode, DuplicateFrom) -- (ChewKP01)              
      SELECT              
         ...
         @cSerialNo, ISNULL(@cUserDefine02, ''), ISNULL(@cUserDefine03, ''), ISNULL(@cUserDefine04, ''), ISNULL(@cUserDefine05, ''),              
         @dtUserDefine06, @dtUserDefine07, ISNULL(@cUserDefine08, ''), ISNULL(@cUserDefine09, ''), ISNULL(@cUserDefine10, ''),              
         ISNULL(@cPoLineNo, ''), @cSubreasonCode , @cReceiptLineNo_Borrowed -- (ChewKP01)              
      FROM @tRD              
      WHERE ReceiptLineNumber = @cReceiptLineNumber              
   ...
   BEGIN              
      ...
      DECLARE @cReceiptDetailTrafficCop NVARCHAR(1)              
      SET @cReceiptDetailTrafficCop = 'N'              
      IF @cUCC <> '' AND @cUCCWithMultiSKU = '1'              
      BEGIN              
         IF EXISTS( SELECT 1 FROM UCC WITH (NOLOCK) WHERE StorerKey = @cStorerKey AND UCCNo = @cUCC AND SKU <> @cSKU AND Status = '0')              
            SET @cReceiptDetailTrafficCop = 'Y'              
      END              
  ...
  */              
      ...
      UPDATE dbo.ReceiptDetail WITH (ROWLOCK) SET              
         ...
         UserDefine01 = CASE WHEN @cSerialNoCapture = '1' AND UserDefine01 = '' THEN @cSerialNo ELSE UserDefine01 END, -- SerialNo              
         EditDate = GETDATE(),                
         EditWho = SUSER_SNAME()                  
...
BEGIN              
   ...
   BEGIN                        
      ...
      VALUES (@cReceiptKey, @cReceiptLineNumberOutput, @cStorerKey, @cSKU, @cSerialNo, @nSerialQTY, @nSerialQTY)              
      IF @@ERROR <> 0              
      BEGIN              
         SET @nErrNo = 60351              
...
ELSE                
BEGIN                
   COMMIT TRAN rdt_1580RcptCfm03SNo -- Only commit change made in here                
   GOTO Quit                
END                
                
RollBackTran:                
   ROLLBACK TRAN rdt_1580RcptCfm03SNo                
Fail:                
Quit:                
