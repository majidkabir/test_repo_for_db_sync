...
/***************************************************************************/
/* Store procedure: rdt_638Finalize04                                      */
...
/***************************************************************************/
CREATE   PROC [RDT].[rdt_638Finalize04](
   @nMobile       INT,
   @nFunc         INT,
...
BEGIN
   ...
   SELECT ReceiptLineNumber,toloc
   FROM dbo.RECEIPTDETAIL WITH (NOLOCK)
   WHERE ReceiptKey = @cReceiptKey
   AND   BeforeReceivedQTY > 0
   AND   FinalizeFlag <> 'Y'
   OPEN CUR_UPD
   FETCH NEXT FROM CUR_UPD INTO @cReceiptLineNumber,@cToLoc
   ...
   RollBackTran:
      ROLLBACK TRAN rdt_638Finalize04 -- Only rollback change made here

   Quit:
      WHILE @@TRANCOUNT > @nTranCount -- Commit until the level we started
         COMMIT TRAN rdt_638Finalize04
