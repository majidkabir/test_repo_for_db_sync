...
/************************************************************************/
/* Store procedure: rdt_VAPUnCasingConfirm                              */
...
CREATE PROC [RDT].[rdt_VAPUnCasingConfirm] (
   @nMobile          INT, 
   @nFunc            INT, 
   @nStep            INT, 
   @nInputKey        INT, 
...
AS
   ...
   BEGIN TRAN  
   SAVE TRAN rdt_VAPUnCasingConfirm  
   ...
   RollbackTran:
      ROLLBACK TRAN rdt_VAPUnCasingConfirm  
  
   Quit:
   WHILE @@TRANCOUNT > @nStartTCnt -- Commit until the level we started  
      COMMIT TRAN rdt_VAPUnCasingConfirm  
