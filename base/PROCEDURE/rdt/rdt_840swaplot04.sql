...
/************************************************************************/
/* Store procedure: rdt_840SwapLot04                                    */
/* Copyright      : IDS                                                 */
/*                                                                      */
/* Purpose: H&M swap lot (modified from rdtHnMSwapLot03)                */
...
/* 2021-06-25  1.0  James       INC1515452. Created                     */
/* 2021-09-02  1.1  LZG         JSM-18446 - Align PD.Status before      */
/*                              swapping Lot (ZG01)                     */
...
BEGIN
   ...
   DECLARE
           ...
           @c_TargetStatus          NVARCHAR( 10),
           @n_IsMoveOrder           INT         -- ZG02

   SET @n_IsMoveOrder = 0     -- ZG02
   SET @n_ErrNo = 0
   SET @n_SwapLot = 1
   SET @nPack_QTY = 1
   ...
   SAVE TRAN rdt_840SwapLot04 -- For rollback or commit only our own transaction
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               ELSE
               BEGIN
                  -- Align target PD.Status with source PD first,
                  -- then swap Lot without conflicting with inventory tables
                  UPDATE PickDetail WITH (ROWLOCK) SET      -- ZG01
                     Status = @c_Status
                  WHERE PickDetailKey = @c_TargetPickDetailKey
                  ...
                  -- Revert target PD.Status after swapping Lot, to align with inventory tables
                  UPDATE PickDetail WITH (ROWLOCK) SET      -- ZG01
                     Status = @c_TargetStatus
                  WHERE PickDetailKey = @c_TargetPickDetailKey
   ...
   BEGIN
      ...
      BEGIN
         ...
         UPDATE PickDetail SET
            QTYMoved = QTYMoved + 1,
            --DropID = CASE WHEN DropID = '' THEN @c_LabelNo ELSE DropID END,    -- ZG02
            EditWho = SUSER_SNAME(),
            EditDate = GETDATE(),
            TrafficCop = NULL
   ...
   IF EXISTS ( SELECT 1 FROM dbo.CODELKUP C WITH (NOLOCK)   -- ZG02
                  JOIN dbo.Orders O WITH (NOLOCK) ON (C.Code = O.Type AND C.StorerKey = O.StorerKey)
                  WHERE C.ListName = 'HMORDTYPE'
                  AND   C.UDF01 = 'M'
                  AND   O.OrderKey = @c_OrderKey
   ...
   AND   DropID = CASE WHEN @n_IsMoveOrder = 1 THEN '' ELSE DropID END      -- ZG02
   ...
   COMMIT TRAN rdt_840SwapLot04
   GOTO Quit

   RollBackTran:
      ROLLBACK TRAN rdt_840SwapLot04
