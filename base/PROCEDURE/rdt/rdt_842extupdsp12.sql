...
/************************************************************************/
/* Store procedure: rdt_842ExtUpdSP12                                   */
/* Copyright      : LF                                                  */
/*                                                                      */
...
/* 2023-10-11  1.0  yeekung   WMS-23835 Created                         */
/************************************************************************/

CREATE   PROC [RDT].[rdt_842ExtUpdSP12] (
   @nMobile        INT,
   @nFunc          INT,
...
BEGIN
   ...
   DECLARE  @fCartonWeight       FLOAT
           ...
           ,@cCartonLabelReportType   NVARCHAR(10)
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               SELECT @cTrackingNo = TrackingNo  -- (james03)
               FROM dbo.Orders WITH (NOLOCK)
               WHERE StorerKey = @cStorerKey
               AND OrderKey = @cOrderKey
            END
            ...
            BEGIN
              ...
              BEGIN
                    ...
                    SELECT @cTrackingNo '@cTrackingNo'
...
--                  END
   ...
   BEGIN
      ...
      ****************************/
      ...
      DECLARE @nPackSKUQTY INT

      DECLARE Cursor_Weight CURSOR FOR
      SELECT SUM(Qty),SKU
      FROM dbo.PackDetail WITH (NOLOCK)
      WHERE StorerKey = @cStorerKey
      AND PickSlipNo = @cPickSlipNo
      ...
      OPEN Cursor_Weight
      FETCH NEXT FROM Cursor_Weight INTO @nPackSKUQTY, @cSKU
      ...
      BEGIN
         ...
         SET @fCartonWeight = @fCartonWeight + ( @fSKU_Weight * @nPackSKUQTY)   
      ...
      BEGIN
         ...
         VALUES ( @cPickSlipNo , @nCartonNo, @cCartonType, '', @fCartonWeight, @fCartonCube, @nTotalPackedQty, sUser_sName(), GetDate(), sUser_sName(), GetDate())
...
RollBackTran:
   ROLLBACK TRAN rdt_842ExtUpdSP12 -- Only rollback change made here

Quit:
   WHILE @@TRANCOUNT > @nTranCount -- Commit until the level we started
      COMMIT TRAN rdt_842ExtUpdSP12
