...
BEGIN  
   ...
   SAVE TRAN rdt_839Confirm07 -- For rollback or commit only our own transaction  
   ...
   -- After pack confirm then print label for all dropid
   IF @nPackCfm = 1
   BEGIN
      ...
      BEGIN
         ...
         FROM dbo.PickDetail Wtih (NOLOCK)
         WHERE StorerKey = @cStorerKey
         AND   OrderKey = @cOrderKey
         AND   QTY > 0
         AND   [STATUS] <> '4' 
         ...
         ORDER BY 1
         OPEN @curPrint
         FETCH NEXT FROM @curPrint INTO @cDropID
         WHILE @@FETCH_STATUS = 0
         BEGIN
...
RollBackTran:  
   ROLLBACK TRAN rdt_839Confirm07 -- Only rollback change made here  
Quit:  
