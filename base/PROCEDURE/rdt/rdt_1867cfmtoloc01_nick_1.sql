...
GO
/******************************************************************************/  
/* Store procedure: rdt_1867CfmToLoc01_NICK_1                                        */  
/* Copyright      : Maersk                                                    */  
/*                                                                            */  
...
CREATE   PROCEDURE [RDT].[rdt_1867CfmToLoc01_NICK_1] (  
    @nMobile         INT,  
    @nFunc           INT,  
...
BEGIN  
            ...
            BEGIN
               ...
               BEGIN
                  ...
                  EXECUTE rdt.rdt_Move
                     ...
                     @cSourceType = 'rdt_1867CfmToLoc01_NICK_1',
   ...
   print 'N-0-0 ' + @cGroupKey + ' - ' + @cCartID + ' - ' + @cUserName
   ...
   SELECT TD.TaskDetailKey, PKD.PickDetailKey, PKD.Qty
	   ...
	   INNER JOIN dbo.PickDetail PKD WITH (NOLOCK)  
		...
		INNER JOIN dbo.PickDetail PKD1 WITH (NOLOCK)  
		  ON TD.Storerkey = PKD1.Storerkey AND PKD.TaskDetailKey = PKD1.TaskDetailKey
	...
	BEGIN
		...
		print 'While loop ' + @cPickDetailKey + ' - ' + @cTaskDetailKey
   ...
   AND   TD.[Status] = '3' AND PKD.Status = '5'
	...
	--select 'TaskDetail', * from TaskDetail where OrderKey = '0000004519'
	--select 'PickDetail',TaskDetailKey, * from PickDetail where OrderKey = '0000004519'
  
   SET @cur = CURSOR LOCAL READ_ONLY FAST_FORWARD FOR
   SELECT TD.TaskdetailKey, TD.OrderKey, TD.DropID, TD.SKU, TD.QTY
   FROM dbo.TASKDETAIL TD WITH (NOLOCK)   
   WHERE TD.Groupkey = @cGroupKey 
   AND   TD.DeviceID = @cCartID 
   AND   TD.UserKey = @cUserName
   ...
   BEGIN
      ...
      BEGIN
		...
		PRINT 'N-1 ' + CAST(@nRowCount AS NVARCHAR(5))
         ...
         BEGIN
            ...
            BEGIN
               ...
               BEGIN
                  SET @nErrNo = 227273
                  SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --NSQL94213: GetKey fail. (rdt_1867CfmToLoc01_NICK_1)
                  GOTO RollBackTran
               END
               ...
               SELECT 
				...
				PRINT 'N-2 Insert Serial No ' + @cSerialNo
            END
...
Quit:  
   ...
   SET @cOrderKey = '0000004519' --050117590017560168 050117590017560169
	select 'PickSerialNo', * from  PickSerialNo with(nolock) where PickDetailKey IN (select PickDetailKey from PickDetail with(nolock) where OrderKey = @cOrderKey)
	--select 'SerialNo-0', * from SerialNo where SerialNo in ('NNFG001250212016')
	select 'SerialNo', * from SerialNo where SerialNo in (select SerialNo from  PickSerialNo with(nolock) where PickDetailKey IN (select PickDetailKey from PickDetail with(nolock) where OrderKey = @cOrderKey))
	select 'PackDetail',  PH.Status AS PackHeaderStatus,PD.*  FROM PackHeader PH WITH(NOLOCK) INNER JOIN PackDetail PD WITH(NOLOCK)  ON PH.PickslipNo = PD.Pickslipno  AND PH.storerkey = PD.storerkey WHERE PH.storerkey =  'NKST' AND PH.OrderKey = @cOrderKey
	select 'PackSerialNo', * from PackSerialNo WITH(NOLOCK) where PickSlipNo in (SELECT PickHeaderKey from PICKHEADER WHERE OrderKey = @cOrderKey)
	select 'PickDetail', TaskDetailKey,* from PickDetail with(nolock) where OrderKey = @cOrderKey --0000004029  0000004030
	select 'TaskDetail', WaveKey, GroupKey, DropID, OrderKey, AreaKey, UserKey, DeviceID, PickMethod, SystemQty, Qty, RefTaskKey, PickMethod, * from TaskDetail with(nolock) where OrderKey = @cOrderKey --0000004026  0000004027) --NKARSTOCK1
	ROLLBACK   -- Only rollback change made here  
