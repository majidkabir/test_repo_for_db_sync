...
/************************************************************************/
/* Store procedure: rdtfnc_ReprintStagingLabel                          */
/* Copyright      : IDS                                                 */
/*                                                                      */
...
/* 2012-07-10 1.0  Ung      Created                                     */
/* 2016-09-30 1.1  Ung      Performance tuning                          */ 
/* 2018-11-12 1.2  Gan      Performance tuning                          */
/************************************************************************/

CREATE PROCEDURE [RDT].[rdtfnc_ReprintStagingLabel] (
   @nMobile    INT,
   @nErrNo     INT  OUTPUT,
...
IF @nFunc = 915
BEGIN
   IF @nStep = 0 GOTO Step_0   -- Func = 915. Menu
   IF @nStep = 1 GOTO Step_1   -- Scn  = 3160. ID, LOC
END
RETURN -- Do nothing if incorrect step


/********************************************************************************
Step 0. func = 915. Menu
   @nStep = 0
********************************************************************************/
Step_0:
BEGIN
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         SET @nErrNo = 73753
         SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') -- ID NOT STAGED
         GOTO Step_1_Fail
      END

      -- Check ID location
      IF NOT EXISTS (SELECT 1 FROM dbo.LOC WITH (NOLOCK) WHERE LOC = @cDropLOC AND LocationCategory = 'STAGING')
      BEGIN
         SET @nErrNo = 73754
         SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') -- LOCnotSTAGEING
         GOTO Step_1_Fail
      END
      ...
      SET @cPrintLabelSP = rdt.RDTGetConfig( @nFunc, 'PrintLabel', @cStorer)
      IF NOT EXISTS (SELECT 1 FROM dbo.sysobjects WHERE name = @cPrintLabelSP AND type = 'P')
      BEGIN
         SET @nErrNo = 73755
      ...
      -- Show message screen
      SET @nErrNo = 73756
      SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') -- LABEL SENT TO PRINT
