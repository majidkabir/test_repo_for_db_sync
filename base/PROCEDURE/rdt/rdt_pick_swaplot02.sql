...
/************************************************************************/  
/* Store procedure: rdt_Pick_SwapLot02                                  */  
/* Copyright      : LF Logistics                                        */  
/*                                                                      */  
/* Purpose: Swap LOT with same LOC, SKU, L01, different ID              */  
/*                                                                      */  
/* Date        Rev  Author      Purposes                                */  
/* 06-Jun-2019 1.0  Ung         WMS-9211 Created                        */
/************************************************************************/  

CREATE PROC [RDT].[rdt_Pick_SwapLot02] (  
   @nMobile       INT,
   @nFunc         INT,
...
*/
   ...
   SAVE TRAN rdt_Pick_SwapLot02 -- For rollback or commit only our own transaction  
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            SET @nErrNo = 140501
            SET @cErrMsg = rdt.rdtgetmessage( 140501, @cLangCode, 'DSP') --'UpdPickDtlFail'
            GOTO RollBackTran
         END
      ...
      BEGIN 
         ...
         BEGIN
            SET @nErrNo = 140502
            SET @cErrMsg = rdt.rdtgetmessage( 140502, @cLangCode, 'DSP') -- 'GetDetKey Fail'
            GOTO RollBackTran
         END
         ...
         BEGIN
			   SET @nErrNo = 140503
            SET @cErrMsg = rdt.rdtgetmessage( 140503, @cLangCode, 'DSP') --'Ins PDtl Fail'
            GOTO RollBackTran
         END
         ...
         BEGIN 
            ...
            BEGIN
              ...
              FROM dbo.PickDetail WITH (NOLOCK) 
               ...
               BEGIN
		            SET @nErrNo = 140504
                  SET @cErrMsg = rdt.rdtgetmessage( 140504, @cLangCode, 'DSP') --'InsRefKLupFail'
                  GOTO RollBackTran
               END
            END
         END
         ...
         BEGIN
            SET @nErrNo = 140505
            SET @cErrMsg = rdt.rdtgetmessage( 140505, @cLangCode, 'DSP') --'UpdPickDtlFail'
            GOTO RollBackTran
         END     
   ...
   -- Old stock (aging > 90 days) need to pick as exact match 
   DECLARE @dToday DATETIME
   SELECT @dToday = CONVERT( DATE, GETDATE())
   IF DATEDIFF( dd, @dLottable05, @dToday) > 90
   BEGIN
      SET @nErrNo = 140522
      SET @cErrMsg = rdt.rdtgetmessage( 140506, @cLangCode, 'DSP') -- 'OldStockNoSwap'
      GOTO RollBackTran
   END
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN 
               ...
               BEGIN
                  SET @nErrNo = 140506
                  SET @cErrMsg = rdt.rdtgetmessage( 140506, @cLangCode, 'DSP') -- 'GetDetKey Fail'
                  GOTO RollBackTran
               END
               ...
               BEGIN
				      SET @nErrNo = 140507
                  SET @cErrMsg = rdt.rdtgetmessage( 140507, @cLangCode, 'DSP') --'Ins PDtl Fail'
                  GOTO RollBackTran
               END
               ...
               BEGIN
                  ...
                  BEGIN
                     ...
                     BEGIN
   				         SET @nErrNo = 140508
                        SET @cErrMsg = rdt.rdtgetmessage( 140508, @cLangCode, 'DSP') --'InsRefKLupFail'
                        GOTO RollBackTran
                     END
                  END
               END
               ...
               BEGIN
                  SET @nErrNo = 140509
                  SET @cErrMsg = rdt.rdtgetmessage( 140509, @cLangCode, 'DSP') --'UpdPickDtlFail'
                  GOTO RollBackTran
               END
            END
            ...
            BEGIN
               SET @nErrNo = 140510
               SET @cErrMsg = rdt.rdtgetmessage( 140510, @cLangCode, 'DSP') --'UpdPickDtlFail'
               GOTO RollBackTran
            END
   ...
   -- If still got balance left to swap, look for other pickdetail with different pickslip#
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               SET @nErrNo = 140511
               SET @cErrMsg = rdt.rdtgetmessage( 140511, @cLangCode, 'DSP') --'No Rec Found'
               GOTO Quit
            END
         END 
         ...
         BEGIN
            ...
            BEGIN
               SET @nErrNo = 140512
               SET @cErrMsg = rdt.rdtgetmessage( 140512, @cLangCode, 'DSP') --'Swap Fail'
               GOTO Quit
            END
         END 
         ...
         BEGIN
            ...
            BEGIN
               SET @nErrNo = 140513
               SET @cErrMsg = rdt.rdtgetmessage( 140513, @cLangCode, 'DSP') --'GetDetKey Fail'
               GOTO RollBackTran
            END            
            ...
            BEGIN
			      SET @nErrNo = 140514
               SET @cErrMsg = rdt.rdtgetmessage( 140514, @cLangCode, 'DSP') --'Ins PDtl Fail'
               GOTO RollBackTran
            END
            ...
            BEGIN
               ...
               BEGIN
                  ...
                  BEGIN
				         SET @nErrNo = 140515
                     SET @cErrMsg = rdt.rdtgetmessage( 140515, @cLangCode, 'DSP') --'InsRefKLupFail'
                     GOTO RollBackTran
                  END
               END
            END
            ...
            BEGIN
               SET @nErrNo = 140516
               SET @cErrMsg = rdt.rdtgetmessage( 140516, @cLangCode, 'DSP') --'UpdPickDtlFail'
               GOTO RollBackTran
            END
...
--         ELSE --(james02)
         ...
         BEGIN
            ...
            BEGIN
               SET @nErrNo = 140517
               SET @cErrMsg = rdt.rdtgetmessage( 140517, @cLangCode, 'DSP') -- 'GetDetKey Fail'
               GOTO RollBackTran
            END
            ...
            BEGIN
			      SET @nErrNo = 140518
               SET @cErrMsg = rdt.rdtgetmessage( 140518, @cLangCode, 'DSP') --'Ins PDtl Fail'
               GOTO RollBackTran
            END
            ...
            BEGIN
               ...
               BEGIN
                 ...
                 FROM dbo.PickHeader WITH (NOLOCK) 
                  ...
                  BEGIN
				         SET @nErrNo = 140519
                     SET @cErrMsg = rdt.rdtgetmessage( 140519, @cLangCode, 'DSP') --'InsRefKLupFail'
                     GOTO RollBackTran
                  END
               END
            END
            ...
            BEGIN
               SET @nErrNo = 140520
               SET @cErrMsg = rdt.rdtgetmessage( 140520, @cLangCode, 'DSP') --'UpdPickDtlFail'
               GOTO RollBackTran
            END            
         ...
         BEGIN
            SET @nErrNo = 140521
            SET @cErrMsg = rdt.rdtgetmessage( 140521, @cLangCode, 'DSP') --'UpdPickDtlFail'
            GOTO RollBackTran
         END
         ...
         BEGIN
            SET @nErrNo = 140523
            SET @cErrMsg = rdt.rdtgetmessage( 140523, @cLangCode, 'DSP') --'UpdPickDtlFail'
            GOTO RollBackTran
         END
...
RollBackTran:  
   ...
   ROLLBACK TRAN rdt_Pick_SwapLot02  
Fail:  
Quit:  
   ...
   VALUES
      ('rdt_Pick_SwapLot02 PickSlip No = ' + @cPickSlipNo 
