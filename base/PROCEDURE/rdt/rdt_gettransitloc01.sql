...
/************************************************************************/
/* Store procedure: rdt_GetTransitLOC01                                 */
/* Copyright      : IDS                                                 */
/*                                                                      */
...
/* 16-04-2018  1.0  Ung       WMS-3273 Separate PND for pick            */
/************************************************************************/

CREATE PROC [RDT].[rdt_GetTransitLOC01] (
...
BEGIN
   ...
   DECLARE @cFromPKZone     NVARCHAR( 10)
   DECLARE @cFromPKOutLOC   NVARCHAR( 10)
   ...
   DECLARE @cToPKInLOC      NVARCHAR( 10)
   ...
   -- Get FromPAZone info
   SELECT @cFromPKOutLOC = OutLOC FROM PickZone WITH (NOLOCK) WHERE PickZone = @cFromPKZone
   /*
   SELECT TOP 1 
      @cFromPKOutLOC = LOC 
   FROM LOC WITH (NOLOCK) 
   WHERE Facility = @cFacility
      AND PickZone = @cFromPKZone 
      AND LocationCategory = 'PND_OUT_PK'
   ORDER BY LOC
   */
   
   -- Get ToPAZone info
   SELECT @cToPKInLOC = InLOC FROM PickZone WITH (NOLOCK) WHERE PickZone = @cToPKZone
   /*
   SELECT TOP 1 
      @cToPKInLOC = LOC 
   FROM LOC WITH (NOLOCK) 
   WHERE Facility = @cFacility
      AND PickZone = @cToPKZone 
      AND LocationCategory = 'PND_IN_PK'
   ORDER BY LOC
   */
...
--         ORDER BY LOC.LogicalLocation, LOC.LOC
   ...
   -- 2. PickZone level
   IF @cFromTransitLOC = '' AND @cFromPKOutLOC <> ''
      SET @cFromTransitLOC = @cFromPKOutLOC
...
--         ORDER BY LOC.LogicalLocation, LOC.LOC
   ...
   -- FromLOC is ToLOC's in transit LOC
   IF @cFromLOC IN (@cToPKInLOC, @cGITLOC)
      GOTO Quit

   -- 2. PickZone level
   IF @cToTransitLOC = '' AND @cToPKInLOC <> ''
      SET @cToTransitLOC = @cToPKInLOC
