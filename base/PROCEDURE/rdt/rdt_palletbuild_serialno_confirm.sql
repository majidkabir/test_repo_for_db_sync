...
GO
/************************************************************************/
/* Store procedure: rdt_PalletBuild_SerialNo_Confirm                    */
/* Copyright      : IDS                                                 */
/*                                                                      */
/* Purpose: Pallet build Comfirm Pick SP.                               */
/*                                                                      */
/* Called from: rdtfnc_PalletBuild_SerialNo                             */
/*                                                                      */
/* Date        Rev  Author      Purposes                                */
/* 14-Jan-2019 1.0  James       WMS7505.Created                         */
/* 17-May-2019 1.1  James       Bug fix                                 */
/************************************************************************/

CREATE PROC [RDT].[rdt_PalletBuild_SerialNo_Confirm] (
   @nMobile       INT,
   @nFunc         INT,
...
BEGIN
   ...
   DECLARE @cExtendedPltBuildCfmSP NVARCHAR(20)
   SET @cExtendedPltBuildCfmSP = rdt.rdtGetConfig( @nFunc, 'ExtendedPltBuildCfmSP', @cStorerKey)
   IF @cExtendedPltBuildCfmSP = '0'
      SET @cExtendedPltBuildCfmSP = ''  

   -- Extended putaway
   IF @cExtendedPltBuildCfmSP <> ''
   BEGIN
      IF EXISTS( SELECT 1 FROM dbo.sysobjects WHERE name = @cExtendedPltBuildCfmSP AND type = 'P')
      BEGIN
         SET @cSQL = 'EXEC rdt.' + RTRIM( @cExtendedPltBuildCfmSP) +
            ' @nMobile, @nFunc, @cLangCode, @cStorerKey, @cFacility, @cPickSlipNo, @cDropID, ' + 
   ...
   BEGIN
      ...
      BEGIN TRAN
      SAVE TRAN rdt_PalletBuild_SerialNo_Confirm
      ...
      RollBackTran:
         ROLLBACK TRAN rdt_PalletBuild_SerialNo_Confirm

      Quit:
         WHILE @@TRANCOUNT > @nTranCount -- Commit until the level we started
            COMMIT TRAN rdt_PalletBuild_SerialNo_Confirm
   END
