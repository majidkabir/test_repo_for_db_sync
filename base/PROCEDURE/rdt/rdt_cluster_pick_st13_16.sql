...
/************************************************************************/
/* Store procedure: rdt_Cluster_Pick_St13_16                            */
/* Copyright      : IDS                                                 */
/*                                                                      */
...
CREATE PROC [RDT].[rdt_Cluster_Pick_St13_16] (
   @nMobile                   INT,
   @nFunc                     INT            OUTPUT, 
...
--         SET @cDefaultToAllocatedQty = rdt.RDTGetConfig( @nFunc, 'DefaultToAllocatedQty', @cStorerKey)
   ...
   BEGIN
      IF @nCurStep = 9
      BEGIN
         SET @nOutScn = @nCurScn - 5
         SET @nOutStep = @nCurStep - 5

         GOTO Quit
      END
      ELSE
      BEGIN
         SET @nOutScn = @nCurScn - 2
         SET @nOutStep = @nCurStep - 2
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         AND PickQty > 0
         OPEN CUR_CFMPICKS
         FETCH NEXT FROM CUR_CFMPICKS INTO @cConso_Orders
         WHILE @@FETCH_STATUS <> -1
         BEGIN
            SET @nErrNo = 0
            SET @cPickConfirm_SP = rdt.RDTGetConfig( @nFunc, 'ClusterPickConfirm_SP', @cStorerKey)
