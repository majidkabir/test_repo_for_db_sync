...
GO
/************************************************************************/        
/* Store procedure: rdt_841ExtUpdSP18                                   */        
/* Copyright      : LF                                                  */        
/*                                                                      */        
...
/* 2021-04-01  1.0  YeeKung  WMS-16718 CREATED                          */
...
BEGIN        
   ...
   BEGIN        
      ...
      BEGIN
         ...
         BEGIN
            SET @nErrNo = 168751
            SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --'SerialnotValid'
            GOTO RollBackTran
         END
         ...
         BEGIN
            SET @nErrNo = 168752
            SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --'Duplicate serialno'
            GOTO RollBackTran
         END
      END
            ...
            BEGIN        
               ...
               BEGIN        
                  ...
                  BEGIN
                     ...
                     SELECT @nCartonNo=CartonNo,@cLabelNo=LabelNo,@clabelline=LabelLine
                     FROM packdetail (NOLOCK)
                     WHERE pickslipno=@cPickslipNo
                     AND SKU = @cPackSku 
        
                    INSERT INTO Packserialno(PickSlipNo,LabelNo,CartonNo,LabelLine,StorerKey,sku,qty,SerialNo)
                    VALUES(@cPickSlipNo,@cLabelNo,@nCartonNo,@clabelline,@cStorerkey,@cSku,@nPackQty,@cSerialNO)

                     IF @@ERROR <> 0        
                     BEGIN        
                        SET @nErrNo = 168768      
                        SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --'InsPackSerFail'        
                        GOTO RollBackTran        
                     END   
                  END
...
RollBackTran:              
   ROLLBACK TRAN rdt_841ExtUpdSP18 -- Only rollback change made here              
              
Quit:              
   WHILE @@TRANCOUNT > @nTranCount -- Commit until the level we started              
      COMMIT TRAN rdt_841ExtUpdSP18              
