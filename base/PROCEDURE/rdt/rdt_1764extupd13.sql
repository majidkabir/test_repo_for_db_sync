...
/************************************************************************/
/* Store procedure: rdt_1764ExtUpd13                                    */
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            COMMIT TRAN rdt_1764ExtUpd13 -- Only commit change made here
         END
      END
      ...
      BEGIN
         ...
         DECLARE @cRouteTaskDetailKey NVARCHAR(10)
         
         DECLARE @cRouteWaveKey  NVARCHAR( 10)
...
--(ML01)                  IF @cOrdType <> 'LULUECOM' AND @cUserDefine01 IN ('170146','170149')
                  ...
                  BEGIN 
                     ...
                     BEGIN
                        ...
                        VALUES (
                           ...
                           @nUCCQTY, @cCaseID, 
                           'PP', @cStorerKey, @cUCCSKU, @cUCCLOT, 'rdt_1764ExtUpd13', @cTaskKey, @cWaveKey, @cPriority, @cSourcePriority, NULL)
                        IF @@ERROR <> 0
                        BEGIN
                           SET @nErrNo = 84815
                           SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') -- InsTaskDetFail
               ...
               --IF @nNoNeedRouting = 0
               --   INSERT INTO #Task (TaskDetailKey, WaveKey, CaseID) VALUES (@cNewTaskDetailKey, @cWaveKey, @cCaseID)
            END
...
RollBackTran:
   ROLLBACK TRAN rdt_1764ExtUpd13 -- Only rollback change made here
Fail:
Quit:
