...
/************************************************************************/
/* Store procedure: rdtfnc_LotToOrder                                   */
...
CREATE   PROC [RDT].[rdtfnc_LotToOrder] (
   @nMobile    INT,
   @nErrNo     INT  OUTPUT,
...
IF @nFunc = 655	 -- Serial no capture by order SKU
BEGIN
   -- Redirect to respective screen
   IF @nStep = 0 GOTO Step_0   -- Func = 655	
...
BEGIN
   ...
   BEGIN
         ...
         BEGIN
            ...
            BEGIN
              SET @cSQL = 'EXEC rdt.' + RTRIM( @cDecodeSP) +
                  ' @nMobile, @nFunc, @cLangCode, @nStep, @nInputKey, @cStorerKey, @cFacility,@cOrderKey,@cBarcode OUTPUT,@cSKU  OUTPUT, ' +
      ...
      BEGIN
         ...
         BEGIN
            SET @cChkSKU = ''
            EXEC rdt.rdt_MultiSKUExtLOT @nMobile, @nFunc, @cLangCode,
      ...
      IF ISNULL(@dLottable04,'') = NULL   
         SET @dLottable04 = NULL
      ELSE
      BEGIN
         IF (@dLottable04 <= GETDATE()+@nShelfLife)
         BEGIN
            SET @nErrNo = 198231
            SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @cLangCode, 'DSP') --LotExpired
            GOTO Quit
         END
      END

      -- Confirm
      EXEC rdt.rdt_LotOrder_Confirm @nMobile, @nFunc, @cLangCode, @nStep, @nInputKey, @cStorerKey, @cFacility,
         @cOrderKey,
         @cSKU,
         @nInnerPack,
         @cBarcode    = @cBarcode,
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         -- INSERT INTO rdt.rdtLotOrderLog for Label Item which SKU.SerialNoCapture = '2'
         ...
         BEGIN
            ...
            BEGIN
               ...
               IF NOT EXISTS ( SELECT 1 FROM rdt.rdtLotOrderLog WITH (NOLOCK)
                               WHERE StorerKey = @cStorerKey
               ...
               BEGIN
                  ...
                  VALUES
                     (@nMobile, @cOrderKey, @cStorerKey, @cNonSerializeSKU, @nNonSerializeQty, @cLottable07, '', '')
         ...
         (
            ...
            FROM rdt.rdtLotOrderLog L WITH (NOLOCK)
            WHERE OrderKey = @cOrderKey
            GROUP BY StorerKey, SKU
         ...
         BEGIN
            ...
            SET @cMsg1 = rdt.rdtgetmessage( 198223, @cLangCode, 'DSP') --Variance found:
            ...
            SET @cMsg9 = (SELECT VarSKU + CONVERT(NVARCHAR(3),VarQty) FROM @tVarianList WHERE Num = 7) --RTRIM( rdt.rdtgetmessage( 198224, @cLangCode, 'DSP')) + ' ' + --ORDER QTY:
                         --CAST( @nOrderQTY AS NVARCHAR(10))
            SET @cMsg10 = (SELECT VarSKU + CONVERT(NVARCHAR(3),VarQty) FROM @tVarianList WHERE Num = 8) --RTRIM( rdt.rdtgetmessage( 198225, @cLangCode, 'DSP')) + ' ' + --SCAN  QTY:
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         -- Reset
         EXEC rdt.rdt_LotOrder_Reset @nMobile, @nFunc, @cLangCode, @nStep, @nInputKey, @cStorerKey, @cFacility,
            @cOrderKey, @cOption,
            @nErrNo     OUTPUT,
            @cErrMsg    OUTPUT
         IF @nErrNo <> 0
...
BEGIN
   ...
   BEGIN
      ...
      SET @dExpiryDate = @dLottable04
      IF @nShelfLife > 0
         SET @dExpiryDate = DATEADD( dd, -@nShelfLife, @dLottable04)
