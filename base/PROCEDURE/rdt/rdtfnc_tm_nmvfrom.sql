...
/*******************************************************************************/
/* Store procedure: rdtfnc_TM_NMVFrom                                          */
...
/* 10-10-2014  1.1   Ung      Fix cancel task not free up booking              */
/* 30-09-2016  1.2   Ung      Performance tuning                               */
/* 08-10-2018  1.3   TungGH   Performance                                      */   
/*******************************************************************************/
CREATE  PROC [RDT].[rdtfnc_TM_NMVFrom](
   @nMobile    INT,
   @nErrNo     INT  OUTPUT,
...
BEGIN
   ...
   IF @nStep = 1 GOTO Step_1   -- Scn = 3840. FromLOC      
   IF @nStep = 2 GOTO Step_2   -- Scn = 3841. ID      
   IF @nStep = 3 GOTO Step_3   -- Scn = 3842. ToLOC    
   IF @nStep = 4 GOTO Step_4   -- Scn = 3843. Sucess Msg
...
BEGIN
   ...
   IF @cDefaultFromLOC <> ''
   BEGIN
      IF EXISTS( SELECT 1 FROM dbo.sysobjects WHERE name = @cDefaultFromLOC AND type = 'P')
      BEGIN
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         EXEC rdt.rdt_TM_NMVFrom_SwapTask @nMobile, @nFunc, @cLangCode, @cUserName
            ,@cTaskDetailKey
            ,@cFromID
...
/********************************************************************************
Step 3. screen = 3842 TO LOC screen
   ID          (Field01)
   SUGG TO LOC (Field02)
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         IF EXISTS( SELECT 1 FROM dbo.sysobjects WHERE name = @cOverwriteToLOC AND type = 'P')
         BEGIN
      ...
      SAVE TRAN rdt_TM_NMVFrom -- For rollback or commit only our own transaction

      -- Confirm task
      EXEC rdt.rdt_TM_NMVFrom_Confirm @nMobile, @nFunc, @cLangCode, @cUserName
         ,@cTaskDetailKey
         ,@nErrNo  OUTPUT
         ,@cErrMsg OUTPUT
      IF @nErrNo <> 0
      BEGIN
         ROLLBACK TRAN rdt_TM_NMVFrom -- Only rollback change made here
...
/********************************************************************************
Step 4. screen = 3843. Message screen
   SUCCESSFUL PUTAWAY
   ENTER = Next Task
   ESC   = Exit TM
********************************************************************************/
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               EXEC sp_ExecuteSQL @cSQL, @cSQLParam,
                  @nMobile, @nFunc, @cLangCode, 4, @cTaskdetailKey, @nErrNo OUTPUT, @cErrMsg OUTPUT
...
BEGIN
   ...
   IF (@nFunc <> 1746 AND @nStep = 0) AND -- Other module that begin with step 0
