...
/******************************************************************************/  
/* Store procedure: rdt_607ExtUpd05                                           */  
/* Copyright      : LF Logistics                                              */  
/*                                                                            */  
/* Purpose: print label                                                       */  
/*                                                                            */  
/* Date         Author    Ver.  Purposes                                      */  
/* 03-11-2021   YeeKung   1.0    WMS-18173 Created                            */
/******************************************************************************/  
  
CREATE PROCEDURE [RDT].[rdt_607ExtUpd05]  
   @nMobile       INT,             
   @nFunc         INT,             
   ...
   BEGIN    
      ...
      BEGIN  
         ...
         BEGIN 
            SELECT TOP 1 @cNewSuggToLOC = LLI.LOC       
            FROM LOTxLOCxID LLI WITH (NOLOCK)     
            LEFT JOIN LOC WITH (NOLOCK) ON (LLI.LOC = LOC.LOC)   
            WHERE LLI.StorerKey = @cstorerkey   
            AND LOC.PutawayZone =(SELECT ISNULL(PutawayZone,'') FROM LOC WHERE 
            LOC=@cSuggLoc)
            AND LOC.Facility=@cFacility
            AND LOC.HostWHCode =(SELECT ISNULL(HOSTWHCODE,'') FROM LOC WHERE 
            LOC=@cSuggLoc)
            AND LLI.QTY - LLI.QTYPicked = 0    
            AND LOC.LocationCategory <> 'STAGE'
            GROUP BY PALogicalLoc,LLI.LOC
            ORDER BY PALogicalLoc asc, LLI.LOC ASC
...
RollBackTran:  
   ROLLBACK TRAN rdt_607ExtUpd05 -- Only rollback change made here  
Quit:  
