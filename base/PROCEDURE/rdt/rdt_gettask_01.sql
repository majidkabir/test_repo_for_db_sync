...
GO
/************************************************************************/  
/* Store procedure: rdt_GETTASK_01                                      */  
/* Copyright      : IDS                                                 */  
/*                                                                      */  
/* Purpose: Get next SKU to Pick. Use together when task is not locked  */  
/*          by pickzone                                                 */
/*                                                                      */  
...
/* 01-Aug-2011 1.0  James       Created                                 */  
/************************************************************************/  

CREATE PROC [RDT].[rdt_GETTASK_01] (  
   @n_Mobile                  INT, 
   @n_Func                    INT, 
...
SET CONCAT_NULL_YIELDS_NULL OFF  
   ...
   WHERE RPL.StorerKey = @c_StorerKey  
      ...
      -- Lock by picker + pickzone + orderkey
      AND NOT EXISTS (  SELECT 1 FROM RDT.RDTPickLock RPL2 WITH (NOLOCK)
                        WHERE RPL.OrderKey = RPL2.OrderKey
                        AND L.PickZone = RPL2.PickZone
                        AND RPL2.AddWho <> @c_UserName
                        AND RPL2.StorerKey = @c_StorerKey
                        AND RPL2.Status = '1' )
   ...
   BEGIN
      SET @nErrNo = 73601
      SET @cErrMsg = rdt.rdtgetmessage( @nErrNo, @c_LangCode, 'DSP') --'No More Task' 
		GOTO Quit
   END
