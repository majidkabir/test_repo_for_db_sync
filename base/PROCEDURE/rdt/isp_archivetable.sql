...
CREATE PROC [RDT].[isp_archiveTable]              
...
AS            BEGIN            
   ...
   DECLARE @c_KeyValue NVARCHAR(30), -- KHLim02          
           ...
           @c_SchemaName NVARCHAR(30)
                     
             
   SET @c_SchemaName = 'RDT'            
   ...
   BEGIN          
      ...
      SET @cExecStatements = N'DECLARE C_ITEM CURSOR FAST_FORWARD READ_ONLY FOR '            
                              + 'SELECT DISTINCT ' + @c_SchemaName+ '.' + LTRIM(RTRIM(@cTableName)) + '.' + LTRIM(RTRIM(@cKeyName))              
                              + ' FROM ' + @c_SchemaName+ '.'  + LTRIM(RTRIM(@cTableName))             
                              + ' WITH (NOLOCK) '             
                              + ' JOIN ' + @c_SchemaName+ '.'  + LTRIM(RTRIM(@cTableName1)) + ' WITH (NOLOCK) on '           
                              + @c_SchemaName+ '.' + LTRIM(RTRIM(@cTableName)) + '.' + LTRIM(RTRIM(@cKeyName)) + ' = '          
                              + @c_SchemaName+ '.' + LTRIM(RTRIM(@cTableName1)) + '.' + LTRIM(RTRIM(@cKeyName1))          
                              + ' WHERE ' + @cCondition          
   END          
   ELSE          
   BEGIN          
   ...
   BEGIN            
    ...
    BEGIN          
       ...
       select @b_success = 1          
         ...
         SET @cExecStatements = N'UPDATE ' + @c_SchemaName+ '.'  + LTRIM(RTRIM(@cTableName1)) + ' with (ROWLOCK) '              
                              + ' SET ArchiveCop = ''9'' '           
                              + ' WHERE ' + @c_SchemaName+ '.'  +  LTRIM(RTRIM(@cTableName1)) + '.' + LTRIM(RTRIM(@cKeyName1))           
         ...
         BEGIN             
          ...
          SELECT @local_c_errmsg =            
             ': Update of Archivecop failed - ' + @c_SchemaName+ '.'  + @cTableName1 + '. (isp_archiveTable) ( ' +            
    ...
    BEGIN          
       ...
       select @b_success = 1          
         ...
         SET @cExecStatements = N'UPDATE ' + @c_SchemaName+ '.'  + LTRIM(RTRIM(@cTableName2)) + ' with (ROWLOCK) '              
                              + ' SET ArchiveCop = ''9'' '           
                              + ' WHERE ' + @c_SchemaName+ '.'  +  LTRIM(RTRIM(@cTableName2)) + '.' + LTRIM(RTRIM(@cKeyName2))           
                              + ' =  RTRIM(@c_KeyValue)  '     -- (jay01)        
         BEGIN TRAN   
    ...
    BEGIN          
       ...
       select @b_success = 1          
         ...
         SET @cExecStatements = N'UPDATE ' + @c_SchemaName+ '.'  + LTRIM(RTRIM(@cTableName3)) + ' with (ROWLOCK) '              
                              + ' SET ArchiveCop = ''9'' '           
                              + ' WHERE ' + @c_SchemaName+ '.'  +  LTRIM(RTRIM(@cTableName3)) + '.' + LTRIM(RTRIM(@cKeyName3))           
                              + ' =  RTRIM(@c_KeyValue) ' -- (jay01)          
         BEGIN TRAN                                   
         SET @cExecStmtArg = N'@c_KeyValue NVARCHAR(30) '  -- (jay01)
    ...
    BEGIN          
       ...
       select @b_success = 1          
         ...
         SET @cExecStatements = N'UPDATE ' + @c_SchemaName+ '.'  + LTRIM(RTRIM(@cTableName)) + ' with (ROWLOCK) '              
                              + ' SET ArchiveCop = ''9'' '           
                              + ' WHERE ' + @c_SchemaName+ '.'  +  LTRIM(RTRIM(@cTableName)) + '.' + LTRIM(RTRIM(@cKeyName))           
             + ' =  RTRIM(@c_KeyValue)  '               
         BEGIN TRAN          
 ...
 BEGIN          
      ...
      begin          
         print @c_SQLWhere          
         print 'building insert for ' + @c_SchemaName+ '.'  + @cTableName1          
      end          
      select @b_success = 1          
 ...
 BEGIN          
  select @c_temp = 'attempting to archive ' + convert(varchar(9), @n_archive_tbl2_records) +          
                       ' ' + @c_SchemaName+ '.'  + @cTableName2 + ' records'          
  execute nsplogalert          
   @c_modulename   = 'isp_archiveTable',          
   @c_alertmessage = @c_temp ,          
   @n_severity     = 0,          
