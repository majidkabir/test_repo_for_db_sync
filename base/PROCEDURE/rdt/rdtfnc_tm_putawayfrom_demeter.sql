...
/*******************************************************************************/
/* Store procedure: rdtfnc_TM_PutawayFrom_Demeter                                      */
/* Copyright      : IDS                                                        */
/*                                                                             */
...
/* 18-03-2024  1.8   Shinto Changing logic to putaway by pallet type and ABC   */
/*******************************************************************************/
CREATE                      PROC [RDT].[rdtfnc_TM_PutawayFrom_Demeter](
   @nMobile    INT,
   @nErrNo     INT  OUTPUT,
...
BEGIN
   ...
   /*Demeter putaway by pallet type START*/
   ...
   JOIN LOTxLOCxID WITH (NOLOCK) ON LOTxLOCxID.Id = TaskDetail.FromID AND LOTxLOCxID.StorerKey = TaskDetail.Storerkey
   JOIN LotAttribute WITH (NOLOCK) ON (LOTxLOCxID.Lot = LotAttribute.lot)
   JOIN PutawayZone WITH (NOLOCK) ON PutawayZone.Pallet_type = LotAttribute.Lottable01
   JOIN LOC WITH (NOLOCK) ON PutawayZone.PutawayZone = LOC.PutawayZone AND PutawayZone.facility = LOC.Facility
   JOIN SKU WITH (NOLOCK) ON SKU.Sku = LOTxLOCxID.Sku AND SKU.StorerKey = LOTxLOCxID.StorerKey AND SKU.ABC = LOC.ABC
   JOIN PALLET WITH (NOLOCK) ON PALLET.PalletKey = LOTxLOCxID.Id AND PALLET.GrossWgt < LOC.WeightCapacity
   AND NOT EXISTS (
	SELECT 1 
   ...
   SET @cErrMsg = ''
	...
	,StartTime,EndTime,SourceType,SourceKey,AddDate,AddWho,EditDate,EditWho,SystemQty,AreaKey,TransitCount,PendingMoveIn,QtyReplen)
	...
	,StartTime,EndTime,SourceType,SourceKey,AddDate,AddWho,EditDate,EditWho,SystemQty,AreaKey,'2',PendingMoveIn,QtyReplen
	FROM TaskDetail
  ...
  END
  END
   /*Demeter putaway by pallet type END*/
