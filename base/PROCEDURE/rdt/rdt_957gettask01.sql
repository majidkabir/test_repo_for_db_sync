...
/******************************************************************************/
/* Store procedure: rdt_957GetTask01                                          */
...
CREATE PROC [RDT].[rdt_957GetTask01] (
    @nMobile          INT
   ,@nFunc            INT
...
BEGIN
   ...
   ***********************************************************************************************/
   ...
   BEGIN
      ...
      --SELECT @cStorerKey '@cStorerKey' , @cOrderKey '@cOrderKey' , @cSuggLoc '@cSuggLoc' 
      
      SELECT @nCount = Count (DISTINCT LA.Lottable03)
      FROM dbo.PickDetail PD WITH (NOLOCK) 
      INNER JOIN dbo.LotAttribute LA WITH (NOLOCK) ON LA.Lot = PD.Lot AND LA.StorerKey = PD.StorerKey 
      ...
      SELECT @nTotalQty = COUNT(DISTINCT ID) 
