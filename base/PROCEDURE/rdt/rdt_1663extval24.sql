...
/******************************************************************************/
/* Store procedure: rdt_1663ExtVal24                                          */
/* Copyright      : Maersk                                                    */
/*                                                                            */
...
CREATE   PROC [RDT].[rdt_1663ExtVal24](  
   @nMobile       INT,  
   @nFunc         INT,  
...
BEGIN  
   ...
   DECLARE @cOtherPresaleFlag    NVARCHAR( 2)
      ...
      BEGIN  
         ...
         BEGIN  
            ...
            BEGIN
               ...
               SET @cOtherPresaleFlag = ''
               SELECT
                  @cOtherShipperkey = ISNULL( RTRIM( shipperkey), ''), 
                  @cOtherPresaleFlag = ISNULL( ECOM_PRESALE_FLAG, '')
               ...
               -- Get order info (ECOM_PRESALE_FLAG only have PH, PR and blank value)
               SELECT @cPresaleFlag = ISNULL( ECOM_PRESALE_FLAG, '')
               FROM dbo.Orders WITH (NOLOCK)
               WHERE OrderKey = @cOrderKey

               -- Check same presale flag
               IF @cOtherPresaleFlag <> @cPresaleFlag  
               BEGIN
                  -- PH need to be on its own pallet
                  IF @cOtherPresaleFlag = 'PH' OR @cPresaleFlag = 'PH'
                  BEGIN
                     SET @nErrNo = 200403
