...
/******************************************************************************/    
/* Store procedure: rdt_PFLStation_Assign_OrderKey                            */    
...
CREATE PROC [RDT].[rdt_PFLStation_Assign_OrderKey] (    
   @nMobile          INT,     
   @nFunc            INT,     
   ...
   ***********************************************************************************************/    
   ...
   BEGIN    
      ...
      BEGIN    
         ...
         SAVE TRAN rdt_PFLStation_Assign_OrderKey -- For rollback or commit only our own transaction    
         ...
         BEGIN    
            ...
            BEGIN    
               ...
               BEGIN    
                  ...
                  IF @@ERROR <> 0    
                  BEGIN    
                     ROLLBACK TRAN rdt_PFLStation_Assign_OrderKey -- Only rollback change made here    
         ...
         COMMIT TRAN rdt_PFLStation_Assign_OrderKey -- Only rollback change made here    
         WHILE @@TRANCOUNT > @nTranCount -- Commit until the level we started    
            COMMIT TRAN    
      END    
   ...
   BEGIN    
      ...
      BEGIN    
         ...
         BEGIN    
            ...
            IF @cStation <> ''    
            BEGIN    
               IF NOT EXISTS( SELECT 1 FROM rdt.rdtPFLStationLog WITH (NOLOCK) WHERE Station = @cStation AND dropid = @cOrderKey)    
               BEGIN    
                  INSERT INTO rdt.rdtPFLStationLog (StorerKey, Facility, Station, Method, orderkey)    
                  VALUES (@cStorerKey, @cFacility, @cStation, @cMethod, @cOrderKey)    
                  IF @@ERROR <> 0    
                  BEGIN    
                     SET @nErrNo = 160707    
         ...
         BEGIN    
            ...
            BEGIN    
               ...
               VALUES (    
                  ...
                  @cOrderKey, @cStorerKey, @cSKU, @cLOC, @nPDQTY, 0, @nFunc, 'rdt_PFLStation_Assign_OrderKey')    
               IF @@ERROR <> 0    
               BEGIN    
                  SET @nErrNo = 160708    
...
RollBackTran:    
   ROLLBACK TRAN rdt_PFLStation_Assign_OrderKey    
