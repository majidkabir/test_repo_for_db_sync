...
/***************************************************************************/
/* Store procedure: rdt_838ExtUpd20                                        */
/* Copyright      : Maersk                                                 */
/*                                                                         */
...
/*                               base on rdt_838ExtUpdVLT                  */
/***************************************************************************/

CREATE   PROC [RDT].[rdt_838ExtUpd20] (
   @nMobile         INT,
   @nFunc           INT,
...
BEGIN
   ...
   BEGIN
     ...
     --Inserting DropID into the DropID table at SKU QTY step. Required in scenarios when label will NOT be printed.
      ...
      --    AND @cOption = 1
      --    AND @cAddPackValidtn = N'1'
      --    AND EXISTS (SELECT 1 FROM [PALLET] (NOLOCK) WHERE [PalletKey] = @cPackDtlDropID)
      -- BEGIN
