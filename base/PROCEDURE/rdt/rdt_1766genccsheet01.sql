...
/************************************************************************/
/* Stored Procedure: rdt_1766GenCCSheet01                             	*/
/*                                                                      */
/* Purpose: Generate StockTake Count Sheet for ecom. QtyPicked =        */
/*          PickDetail.Qty for status = 3. Cannot use PickDetail.Status */
/*          5 as for ecom it is consider packed                         */
/*                                                                      */
/* Called By: ispRDTGenCountSheet                                       */
/*                                                                      */
/* Modifications log:                                                   */            
/*                                                                      */            
/* Date         Rev  Author  Purposes                                   */      
/* 2021-04-21   1.0  James   WMS-16634. Created                         */
/************************************************************************/

CREATE PROC [RDT].[rdt_1766GenCCSheet01] (
   @nFunc          INT,  
...
DECLARE @c_AisleSQL	   NVARCHAR(800),
	...
	@cSKUSQL            NVARCHAR(800),
	@cSKUSQL2           NVARCHAR(800),
	@b_success           int,
	@c_AgencySQL         NVARCHAR(800),
	@c_AgencySQL2        NVARCHAR(800),
	@c_ABCSQL            NVARCHAR(800),
	@c_ABCSQL2           NVARCHAR(800), 
	@cSKUGroupSQL       NVARCHAR(800),
	@cSKUGroupSQL2      NVARCHAR(800)

DECLARE @n_QtyAllocated    INT = 0
DECLARE @n_QtyPicked       INT = 0
...
BEGIN
	-- Start : SOS66279
	IF NOT EXISTS (SELECT 1 FROM STOCKTAKEPARM2 WITH (NOLOCK) WHERE Stocktakekey = @cStockTakeKey)
	BEGIN
	-- End : SOS66279
	   ...
	   IF ISNULL(RTRIM(@cSKU),'') <> ''
		BEGIN
		   SET @c_SQL = @c_SQL + ' AND   LOTxLOCxID.SKU = "' + ISNULL(RTRIM(@cSKU),'') + '" '
		END
	   ...
	   SELECT @c_SQL =  @c_SQL +  CASE WHEN @c_ExcludeQtyPicked = 'Y' THEN 'AND   LOTxLOCxID.Qty-LOTxLOCxID.QtyPicked > 0 ' ELSE 'AND   LOTxLOCxID.Qty > 0 ' END
		...
		+ ISNULL(dbo.fnc_RTrim(@cSKUSQL), '') + ' ' + ISNULL(dbo.fnc_RTrim(@cSKUSQL2), '') + ' '
		...
		+ ISNULL(dbo.fnc_RTrim(@cSKUGroupSQL), '') + ' ' + ISNULL(dbo.fnc_RTrim(@cSKUGroupSQL2), '') + ' '
...
-- End - SOS23776
  ...
  SELECT @c_CCSheetNoKeyName = 'CSHEET'+LTRIM(RTRIM(@cStockTakeKey)) --NJOW03
	
	FETCH NEXT FROM cur_1 INTO @c_lot, @cLoc, @c_id, @c_StorerKey, @cSKU, @c_Lottable01, @c_Lottable02, @c_Lottable03,
	@d_Lottable04, @d_Lottable05, @n_qty, @c_Facility, @c_Aisle, @n_LocLevel, @c_CCLogicalLoc, @n_SystemQty, @c_PutawayZone 
	
	WHILE @@FETCH_STATUS <> -1
	BEGIN
      
      IF @c_ExcludeQtyPicked = 'Y'
      BEGIN
         SELECT @n_QtyAllocated = ISNULL( SUM( QtyAllocated), 0), @n_QtyPicked = ISNULL( SUM( QtyPicked), 0) FROM (
         SELECT SUM( CASE WHEN [Status] = '0' then Qty ELSE 0 END) AS QtyAllocated, SUM( CASE WHEN [Status] IN ( '3', '5') THEN Qty ELSE 0 END) AS QtyPicked
         FROM dbo.PICKDETAIL WITH (NOLOCK) 
         WHERE Lot = @c_lot AND Loc = @cLoc AND ID = @c_id 
         GROUP BY [Status]) A
         
         --SELECT @n_SystemQty '@n_SystemQty', @n_QtyAllocated '@n_QtyAllocated', @n_QtyPicked '@n_QtyPicked', @c_lot '@c_lot', @cLoc '@cLoc', @c_id '@c_id', @cStockTakeKey '@cStockTakeKey', @cTaskDetailKey '@cTaskDetailKey'
         --SET @n_SystemQty = @n_SystemQty - @n_QtyAllocated - @n_QtyPicked
         SET @n_SystemQty = @n_SystemQty - @n_QtyPicked
      END
      
      SET @c_CCSheetNo = @cTaskDetailKey
	   ...
	   BEGIN	
	      ...
	      VALUES (@cStockTakeKey, @c_CCDetailKey, @c_StorerKey, @cSKU, @c_lot, @cLoc, @c_id, 0, @c_CCSheetNo,
	   ...
	   FETCH NEXT FROM cur_1 INTO @c_lot, @cLoc, @c_id, @c_StorerKey, @cSKU, @c_Lottable01, @c_Lottable02,
