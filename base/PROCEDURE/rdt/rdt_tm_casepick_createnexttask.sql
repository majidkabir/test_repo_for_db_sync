...
/************************************************************************/
/* Store procedure: rdt_TM_CasePick_CreateNextTask                      */
...
/* 26-Aug-2018 1.1  Ung       WMS-3273 Add CreateNextTaskSP             */
/*                            Change FCP1 to FPK1                       */
/************************************************************************/

CREATE PROC [RDT].[rdt_TM_CasePick_CreateNextTask] (
   @nMobile        INT,
   @nFunc          INT,
...
BEGIN
   ...
   SELECT TOP 1 
      ...
      @cSourceType     = 'rdt_TM_CasePick_CreateNextTask'
   ...
   IF @cCreateNextTaskSPSP = '0'
      SET @cCreateNextTaskSPSP = ''

   /***********************************************************************************************
                                              Custom confirm
   ***********************************************************************************************/
   -- Custom logic
   IF @cCreateNextTaskSPSP <> ''
   BEGIN
      IF EXISTS( SELECT 1 FROM dbo.sysobjects WHERE name = @cCreateNextTaskSPSP AND type = 'P')
      BEGIN
         SET @cSQL = 'EXEC rdt.' + RTRIM( @cCreateNextTaskSPSP) +
   ...
   ***********************************************************************************************/
   ...
   SAVE TRAN rdt_TM_CasePick_CreateNextTask -- For rollback or commit only our own transaction
   ...
   COMMIT TRAN rdt_TM_CasePick_CreateNextTask -- Only commit change made here
   GOTO Quit

RollBackTran:
   ROLLBACK TRAN rdt_TM_CasePick_CreateNextTask -- Only rollback change made here
Fail:
Quit:
