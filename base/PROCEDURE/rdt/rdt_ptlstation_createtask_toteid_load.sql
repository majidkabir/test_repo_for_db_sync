...
/************************************************************************/
/* Store procedure: rdt_PTLStation_CreateTask_ToteID_Load               */
...
CREATE PROC [RDT].[rdt_PTLStation_CreateTask_ToteID_Load] (
    @nMobile      INT
   ,@nFunc        INT
...
BEGIN
   ...
   ***********************************************************************************************/
   ...
   SET @curPD = CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
      SELECT LPD.LoadKey, PD.OrderKey, PD.SKU, SUM( PD.QTY)
      ...
      WHERE PD.StorerKey = @cStorerKey 
         ...
         AND O.Status <> 'CANC' 
         AND O.SOStatus <> 'CANC'
      GROUP BY LPD.LoadKey, PD.OrderKey, PD.SKU
   OPEN @curPD
   FETCH NEXT FROM @curPD INTO @cLoadKey, @cOrderKey, @cSKU, @nPDQTY
      ...
      BEGIN
         ...
         BEGIN            
            ...
            VALUES (
               ...
               @cOrderKey, @cStorerKey, @cSKU, @nPDQTY, 0, @cScanID, @nFunc, @nRowRef, 'rdt_PTLStation_CreateTask_ToteID_Load')
            IF @@ERROR <> 0
            BEGIN
               SET @nErrNo = 114753
