...
/************************************************************************/  
/* Store procedure: rdt_523PABySKUCfm04                                 */  
/*                                                                      */  
...
/*  2023-06-28  1.0  yeekung  WMS-21229 Created (yeekung01)             */
/************************************************************************/  
  
CREATE   PROC [RDT].[rdt_523PABySKUCfm04] (  
   @nMobile         INT,  
   @nFunc           INT,  
...
BEGIN  
   ...
   SAVE TRAN rdt_523PABySKUCfm04 -- For rollback or commit only our own transaction  
 ...
 IF @cLOT = ''  
  ...
  SET @curLLI = CURSOR LOCAL READ_ONLY FAST_FORWARD FOR  
   ...
   BEGIN  
      ...
      -- Trigger transfer if return stock  
      IF EXISTS( SELECT 1 FROM LOC WITH (NOLOCK) WHERE LOC = @cLOC AND HostWhcode LIKE 'RET%') AND  
         EXISTS( SELECT 1 FROM LOC WITH (NOLOCK) WHERE LOC = @cFinalLOC AND HostWhcode = 'STD') AND  
         EXISTS( SELECT 1 FROM LOTAttribute WITH (NOLOCK) WHERE LOT = @cLLI_LOT AND Lottable03 = 'RET')  
      BEGIN  
         -- Get lottables  
         SELECT  
         ...
         BEGIN  
            ...
            SELECT @cLLI_ID = CASE WHEN LoseID = '1' THEN '' ELSE @cID END   
         ...
         VALUES (  
            ...
            @cStorerKey, @cSKU, @cFinalLOC, @cLLI_LOT, @cLLI_ID, @nPA_QTY, @cPackkey, @cUOM,  
            ...
            @cStorerKey, @cSKU, @cFinalLOC, '', @cLLI_ID, @nPA_QTY, @cPackkey, @cUOM, '0', GETDATE(),  
   ...
   COMMIT TRAN rdt_523PABySKUCfm04 -- Only commit change made here  
   GOTO Quit  
  
RollBackTran:  
   ROLLBACK TRAN rdt_523PABySKUCfm04 -- Only rollback change made here  
Quit:  
