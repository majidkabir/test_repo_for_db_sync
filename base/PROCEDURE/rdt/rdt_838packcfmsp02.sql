...
/************************************************************************/
/* Store procedure: rdt_838PackCfmSP02                                  */
/* Copyright      : LF Logistics                                        */
/*                                                                      */
/* Date       Rev  Author      Purposes                                 */
/* 16-06-2019 1.0  Ung         WMS-9050 Created                         */
/************************************************************************/

CREATE PROC [RDT].[rdt_838PackCfmSP02] (
    @nMobile      INT
   ,@nFunc        INT
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         -- Get order info
         DECLARE @cOrderUDF04 NVARCHAR(60)
         DECLARE @cExternOrderKey NVARCHAR(50)
         DECLARE @cShipperKey NVARCHAR(15)
         SELECT
            @cOrderUDF04 = LEFT( ISNULL( UserDefine04, ''), 30), 
            @cExternOrderKey = LEFT( ExternOrderKey, 40), 
            @cShipperKey = ISNULL( ShipperKey, '')
         FROM Orders WTIH (NOLOCK)
         ...
         BEGIN
            ...
            OPEN @curPackDtl
            FETCH NEXT FROM @curPackDtl INTO @cLabelNo
            WHILE @@FETCH_STATUS = 0
            BEGIN
               -- Carton track
               INSERT INTO CartonTrack (TrackingNo, CarrierName, KeyName, Labelno, Carrierref1, Carrierref2)
               VALUES (@cLabelNo, @cOrderUDF04, @cStorerKey, @cLabelNo, @cOrderKey, @cExternOrderKey)
               IF @nErrNo <> 0
               BEGIN
                  SET @nErrNo = 140451
      ...
      BEGIN
         UPDATE Orders SET
            SOStatus = CASE WHEN @cUDF04 = 'ITF' THEN 'PENDGET' ELSE '5' END, 
            EditDate = GETDATE(), 
            EditWho = SUSER_SNAME(), 
            TrafficCop = NULL
         WHERE OrderKey = @cOrderKey
