...
/****************************************************************************/  
/* Trigger: isp_ECOMP_GetPackTaskOrders_M                                   */  
...
/****************************************************************************/  
CREATE   PROC [API].[isp_ECOMP_GetPackTaskOrders_M]  
            @b_Debug          INT            = 0
...
BEGIN  
   ...
   DECLARE @n_Index        INT  
         ...
         , @c_ECOMPShowDevicePosition     NVARCHAR(1)    = ''
         , @c_PT_DevicePosition           NVARCHAR(10)   = ''
   ...
   DECLARE @TMP_PACKTASKDETAIL TABLE  
      ...
      ,  IsTaskPending  BIT            NOT NULL DEFAULT (0)
      )  
   ...
   SET @c_ECOMPShowDevicePosition = dbo.fnc_GetRight(@c_Facility, @c_Storerkey, '', 'ECOMPShowDevicePosition')  
   --(Alex01) - END
   ...
   IF @b_Debug = 1  
   BEGIN  
      select @c_ECOMPShowDevicePosition '@c_ECOMPShowDevicePosition', @c_Facility '@c_Facility', @c_Storerkey '@c_Storerkey'
   END  
   ...
   SELECT DISTINCT  
            CASE 
               WHEN @c_ECOMPShowDevicePosition = '1' AND PT.LogicalName <> '' THEN PT.LogicalName 
               ELSE PTD.LogicalName 
            END --GetLogicalName From PackTask
         ,  PTD.Orderkey  
         ,  PTD.PickSlipNo  
         ,  PTD.[Status]  
         ,  PT.DevicePosition
   FROM PACKTASK PT WITH (NOLOCK)
   JOIN PACKTASKDETAIL PTD WITH (NOLOCK)  
   ON (PTD.TaskBatchNo = @c_TaskBatchNo AND PT.TaskBatchNo = PTD.TaskBatchNo AND PT.Orderkey = PTD.Orderkey )
   ...
   BEGIN  
      IF @b_Debug = 1  
      BEGIN  
         select @c_PTD_LogicalName '@c_PTD_LogicalName', @c_PT_DevicePosition '@c_PT_DevicePosition'  
      END  
      ...
      VALUES  
            ...
            ,  @n_DvcRefKey  
            ,  @c_PT_DevicePosition
            ,  CASE WHEN @c_PTD_OrigStatus = '3' THEN 1 ELSE 0 END
            )  
      ...
      IF @b_Debug = 1  
      BEGIN  
         select @c_PTD_LogicalName '@c_PTD_LogicalName', @n_DvcCol '@n_MaxCol'  
      END  
   ...
   DISPLAY_ORDERS:  
   SET @c_PTOJson = ISNULL((
                        SELECT  
                              ISNULL(PTD.TaskBatchNo,'')          As 'TaskBatchNo'
                           ,  ISNULL(PTD.Orderkey,'')             As 'OrderKey'
                           ,  CASE WHEN ISNULL(PTD.Orderkey,'') = '' THEN '' 
                              --Alex01 Begin
                              --ELSE LEFT(ISNULL(RTRIM(PTD.LogicalName),''),3) + '-' END  
                              ELSE 
                                 CASE WHEN @c_ECOMPShowDevicePosition = '1' AND ISNULL(RTRIM(PTD.DevicePosition),'') <> ''
                                    THEN LEFT(ISNULL(RTRIM(PTD.DevicePosition),''),3)  
                                 ELSE  LEFT(ISNULL(RTRIM(PTD.LogicalName),''),3) 
                                 END
                              END + '-' 
                              --Alex01 End
                              + ISNULL(RTRIM(PTD.Orderkey),'')    As 'DeviceOrderkey' 
                           ,  ISNULL(PTD.Status,'')               As 'Status'
                           ,  @c_InProgOrderkey                   As 'InProgOrderkey'
                           ,  CASE  ISNULL(PTD.Status,'')  
                                       WHEN ''  THEN '#FFFFFF' --16777215     -- WHITE  
                                       WHEN 'X' THEN '#AAAAAA' --8421504      -- GREY (CANC, HOLD)  
                                       WHEN '9' THEN '#1BB52D' --32768        -- GREEN(0,128,0)    (PACKCONFIRM)  
                                       WHEN '3' THEN '#387CF0' --16711680     -- BLUE(0,0,255)     (Assigned Orderkey)  
                                       ...
                                       WHEN '0' THEN '#FA381C' --255          -- RED (255,0,0)     (Open)  
                              END                                 As 'ColorCode'
                           ,  ISNULL(PTD.IsTaskPending, 0)        As 'IsTaskPending'
                        FROM @TMP_DVCPOS DVC  
...
QUIT_SP:  
SET @c_PTOJson = CASE WHEN ISNULL(RTRIM(@c_PTOJson), '') = '' THEN '[]' ELSE @c_PTOJson END
