...
/************************************************************************/
/* Stored Proc : isp_GetHourOrderPromoByStorer                          */
...
/************************************************************************/
CREATE PROC [API].[isp_GetHourOrderPromoByStorer] 
    @c_UserName      NVARCHAR(60)  = ''
  , @c_StorerKey     NVARCHAR(25)  = ''
...
BEGIN     
  ...
  SET ANSI_PADDING ON;
    ...
    ( UserName, Module) VALUES    ( @c_UserName, 'isp_GetHourOrderPromoByStorer')
   ...
   IF CHARINDEX('-', @c_StorerKey) > 0 
   BEGIN
    SET @c_Facility  = RIGHT(@c_StorerKey, LEN(@c_StorerKey) - CHARINDEX('-', @c_StorerKey))
      SET @c_StorerKey = SUBSTRING(@c_StorerKey, 1, CHARINDEX('-', @c_StorerKey) - 1)
   ...
   DECLARE
         ...
         , @nRandom9  INT, @nRandom10   INT, @ct  INT, @nct INT
         , @dtf INT,@dtb VARCHAR(2), @fDate VARCHAR(5), @nRtt INT
         , @nRtt1 INT, @nRtt2 INT, @nRtt3 INT, @nRtt4 INT , @nRtt5 INT
  ...
  BEGIN
    ...
    BEGIN
      ...
      SET @nRtt = @nRtt + @nRandom
      SET @nRtt1 = @nRtt1 + @nRandom1
      SET @nRtt2 = @nRtt2 + @nRandom2
      SET @nRtt3 = @nRtt3 + @nRandom3
      SET @nRtt4 = @nRtt4 + @nRandom4
      SET @nRtt5 = @nRtt5 + @nRandom5
    ...
    BEGIN
      ...
      VALUES (
        'Pct',    0,          
        ((@nRtt1  / (@nRtt * 1.00)) * 100.00),
        ((@nRtt2  / (@nRtt * 1.00)) * 100.00),          
        ((@nRtt3  / (@nRtt * 1.00)) * 100.00),          
        ((@nRtt4  / (@nRtt * 1.00)) * 100.00), 
        ((@nRtt5  / (@nRtt * 1.00)) * 100.00),          
        0,          0, 
        0,          0,          0,  
        0,          0,          1)
  ...
  END
   ...
   BEGIN
     ...
     AND osss.StorerKey = CASE WHEN @c_StorerKey = '' THEN osss.StorerKey ELSE @c_StorerKey END 
     AND osss.Facility =  CASE WHEN @c_Facility = '' THEN osss.Facility ELSE @c_Facility END
     ORDER BY osss.Rowref DESC
