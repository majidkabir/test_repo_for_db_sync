...
/* 2024-12-19   1.4  YeeKung     INC7321216 Change cube (yeekung04)              */
/* 2025-01-28   1.5  YeeKung     UWP-29489 Change API Username (yeekung05)       */
/* 2025-01-21   1.6  YeeKung    TPS-970 Add New Param on Extendedupdate(yeekung21)*/
/*********************************************************************************/  
  
CREATE   PROC [API].[isp_UCCPackConfirm] (  
   @json       NVARCHAR( MAX),    
...
@cScanNo = ScanNo, @nCartonNo = CartonNo, @cCartonType = CartonType, @cType = ctype, @cWorkstation = Workstation,
@cLabelNo = LabelNo,@cBarcode = Barcode,    
...
BEGIN  
   ...
   BEGIN  
      ...
      BEGIN  
         SET @n_Err = 1000055  
         SET @c_ErrMsg = API.TouchPadGetMessage( @n_Err, @cLangCode, 'DSP')--'Incorrect dynamic E-Comm Carton Weight column setup. Function : isp_UCCPackConfirm'  
         SET @jResult = (select '' AS OrderKey, '' as LabelJobID, '' as PackingJobID ,@nProceedPrintFlag AS nProceedPrintFlag, '' AS VasConfig, '' AS VasCol1Name, '' AS VasCol1Value, '' AS WorkInstruction FOR JSON PATH )  
         GOTO EXIT_SP  
      END  
      ELSE  
   ...
   BEGIN  
      ...
      BEGIN  
         SET @n_Err = 1000056  
         SET @c_ErrMsg = API.TouchPadGetMessage( @n_Err, @cLangCode, 'DSP')--'Incorrect dynamic E-Comm Cube column setup. Function : isp_UCCPackConfirm'  
         SET @jResult = (select '' AS OrderKey, '' as LabelJobID, '' as PackingJobID ,@nProceedPrintFlag AS nProceedPrintFlag, '' AS VasConfig, '' AS VasCol1Name, '' AS VasCol1Value, '' AS WorkInstruction FOR JSON PATH )  
         GOTO EXIT_SP  
      END  
      ELSE  
...
BEGIN  
   ...
   BEGIN     
      SET @b_Success = 0    
      SET @n_Err = 1000057    
      SET @c_ErrMsg = API.TouchPadGetMessage( @n_Err, @cLangCode, 'DSP')--'No SKU entered. Please enter or scan valid SKU. Function : isp_UCCPackConfirm'  
      GOTO RollBackTran  
   END  
   ...
   BEGIN       
      SET @b_Success = 0    
      SET @n_Err = 1000058    
      SET @c_ErrMsg = API.TouchPadGetMessage( @n_Err, @cLangCode, 'DSP')--'No Quantity entered. Please enter valid Quantity. Function : isp_UCCPackConfirm'           
      GOTO RollBackTran  
   END   
   ...
   BEGIN  
      ...
      BEGIN           
         SET @b_Success = 0    
         SET @n_Err = 1000061    
         SET @c_ErrMsg = API.TouchPadGetMessage( @n_Err, @cLangCode, 'DSP')--'Fail to update into PackDetail. Function : isp_UCCPackConfirm'   
         GOTO RollBackTran  
      END  
   END  
...
Quit:  
   WHILE @@TRANCOUNT > @nTranCount -- Commit until the level we started  
      COMMIT TRAN --isp_UCCPackConfirm  
