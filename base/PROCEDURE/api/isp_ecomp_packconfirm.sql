...
GO
/************************************************************************/  
/* Trigger: [API].[isp_ECOMP_PackConfirm]                               */  
...
/************************************************************************/  
CREATE   PROC [API].[isp_ECOMP_PackConfirm] 
...
BEGIN  
   ...
   DECLARE    
         ...
         , @n_PackDetailInfoKey     BIGINT   --(Alex02)
   ...
   BEGIN  
      SET @n_continue = 3  
      SET @n_err = 60005    
      SET @c_errmsg='There is Sku qtyallocated <> qtypacked. (isp_ECOMP_PackConfirm)'   
      GOTO QUIT  
   END  
   ...
   BEGIN  
      ...
      --PickHeader.PickHeaderKey = Packheader.PickSlipNo, e.g. T*********
      IF @c_PickSlipNo = '' OR @c_PickSlipNo = @c_TmpPickSlipNo
      BEGIN  
         /* (Wan01) - START  
         EXECUTE nspg_GetKey   
                 @KeyName     = 'PICKSLIP'  
      ...
      , @b_resultset = 0  
         ...
         BEGIN  
            SET @n_continue = 3                                                                                                
            SET @n_err = 60000                                                                                                 
            SET @c_errmsg='NSQL'+ CONVERT(CHAR(5),@n_err)+': Error Executing nspg_GetKey. (isp_ECOMP_PackConfirm)'   
         ...
         --   SET @n_continue = 3  
         --   SET @n_err = 60010    
         --   SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Insert into PICKHEADER Table. (isp_ECOMP_PackConfirm)'   
         --                + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '   
         --   GOTO QUIT  
         --END  
         ...
         BEGIN  
            SET @n_continue = 3  
            SET @n_err = 60020    
            SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Insert into PICKINGINFO Table. (isp_ECOMP_PackConfirm)'   
                         + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '   
            GOTO QUIT  
         END  
      END  
      ...
      BEGIN  
         SET @n_continue = 3  
         SET @n_err = 60030    
         SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Insert into PACKHEADER Table. (isp_ECOMP_PackConfirm)'   
                      + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '   
         GOTO QUIT  
      END  
      ...
      BEGIN  
         ...
         BEGIN  
            SET @n_continue = 3  
            SET @n_err = 60040    
            SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Update PACKINFO Table. (isp_ECOMP_PackConfirm)'   
                         + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '   
            GOTO QUIT  
         END  
      END
   ...
   END  
   ELSE
   BEGIN IF LEFT(@c_PickSlipNo,1) = 'P'  
      SET @c_TmpPickSlipNo = @c_PickSlipNo
   END
   --Alex01 Begin
   --INSERT PickHeader & Generate UCC Label
   EXEC [API].[isp_ECOMP_GenLabelNo] 
        @b_Debug                    = 0
      , @c_PickSlipNo               = @c_TmpPickSlipNo
      , @c_NewPickSlipNo            = @c_PickSlipNo
      , @b_PackConfirm              = 1
      , @b_Success                  = @b_success  OUTPUT
   ...
   BEGIN
      SET @n_Continue = 3 
      SET @n_err = 60041
      SET @c_errmsg = CONVERT(CHAR(5),@n_err) + ': Failed to insert pickheader/generate label no. (isp_ECOMP_PackConfirm)'
                    + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '   
   ...
   --Count Max Carton
   SELECT @n_maxctn  = ISNULL(MAX(CartonNo), 1)
   FROM   PACKDETAIL WITH (NOLOCK)  
   WHERE PickSlipNo = @c_PickSlipNo  
   ...
   --         SET @n_continue = 3  
   --         SET @n_err = 60050   
   --         SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Executing isp_GenUCCLabelNo_Std. (isp_ECOMP_PackConfirm)'   
   --                      + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '   
   --         GOTO QUIT  
   --      END  
   --   END  
   ...
   --         SET @n_continue = 3  
   --         SET @n_err = 60050   
   --         SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Update PACKDETAIL Table. (isp_ECOMP_PackConfirm)'   
   --                      + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '   
   --         GOTO QUIT  
   --      END  
   --      FETCH NEXT FROM CUR_PACKL INTO @c_LabelLine  
   ...
   --         SET @n_continue = 3  
   --         SET @n_err = 60055   
   --         SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Update PACKSERIALNO Table. (isp_ECOMP_PackConfirm)'   
   --                      + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '   
   --         GOTO QUIT  
   --      END  
   --      FETCH NEXT FROM CUR_PACKSN INTO @n_PackSerialNoKey  
   ...
   BEGIN      
      ...
      BEGIN      
         ...
         BEGIN            
            ...
            SET @n_err = 60056      
            SET @c_errmsg='NSQL'+CONVERT(char(6), @n_err)+': Delete Failed On Table PACKQRF. (isp_ECOMP_PackConfirm)'       
                           + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg), '') + ' ) '            
            BREAK      
         END        
         FETCH NEXT FROM @cur_PQRF INTO @n_PackQRFKey      
   ...
   BEGIN  
      SET @n_continue = 3  
      SET @n_err = 60055  
      SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Update ORDERS Table. (isp_ECOMP_PackConfirm)'   
                     + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '   
      GOTO QUIT  
   END  
  
   IF @c_TmpPickSlipNo <> '' AND @c_TmpPickSlipNo <> @c_PickSlipNo
   BEGIN  
      ...
      BEGIN  
         SET @n_continue = 3  
         SET @n_err = 60060    
         SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Delete PACKHEADER Table. (isp_ECOMP_PackConfirm)'   
                        + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '   
         GOTO QUIT  
      END  
   END  
   ...
   BEGIN  
      SET @n_continue = 3  
      SET @n_err = 60065   
      SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Executing nspGetRight. (isp_ECOMP_PackConfirm)'   
                     + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '   
      GOTO QUIT  
   END   
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         SET @c_ErrMsg = 'NSQL' +  CONVERT(CHAR(5),@n_Err)  + ':'    
                        + 'Error Executing isp_AssignPackLabelToOrderByLoad.(isp_ECOMP_PackConfirm)'  
         GOTO QUIT  
      END  
   END  
   ...
   BEGIN  
      SET @n_continue = 3  
      SET @n_err = 60075  
      SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Update PACKHEADER Table. (isp_ECOMP_PackConfirm)'   
                     + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '   
      GOTO QUIT  
   END  
   ...
   BEGIN     
      ...
      SET @c_errmsg = 'NSQL'+CONVERT(char(6), @n_err)+ ': Execute ispPackConfirmSerialNo Failed. (isp_ECOMP_PackConfirm) '     
                     + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg), '') + ' ) '    
      GOTO QUIT                          
   END   
   ...
   BEGIN         
      ...
      SET @n_err = 60085        
      SET @c_errmsg = CONVERT(char(5),@n_err)        
      SET @c_errmsg = 'NSQL'+CONVERT(char(6), @n_err)+ ': Execute isp_PackConfirmQRF Failed. (isp_ECOMP_PackConfirm) '         
                     + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg), '') + ' ) '        
      GOTO QUIT                              
   END       
   --(Wan05) - END      
   ...
   BEGIN     
      ...
      SET @n_err = 60090    
      SET @c_errmsg = CONVERT(char(5),@n_err)    
      SET @c_errmsg = 'NSQL'+CONVERT(char(6), @n_err)+ ': Execute ispPostPackConfirmWrapper Failed. (isp_ECOMP_PackConfirm) '     
                     + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg), '') + ' ) '    
      GOTO QUIT                          
   END     
   --(Wan03) - END  
   ...
   BEGIN  
      ...
      BEGIN  
         SET @n_continue = 3  
         SET @n_err = 60080   
         SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Update PACKTASKDETAIL Table. (isp_ECOMP_PackConfirm)'   
                        + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '   
         GOTO QUIT  
      END  
...
QUIT:  
   ...
   BEGIN  
      ...
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'isp_ECOMP_PackConfirm'  
