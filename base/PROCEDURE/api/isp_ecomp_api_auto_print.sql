...
GO
/************************************************************************/              
/* Store procedure: [API].[isp_ECOMP_API_Auto_Print]                    */              
...
BEGIN
   ...
   DECLARE @n_CartonNo                    INT            = 0   --(AL01)
         , @n_Exist                       INT            = 0   --(AL01)
         , @c_EPACKCloseCartonPrint       NVARCHAR(1)    = ''
         , @c_EPACKNewCartonSkipPrint     NVARCHAR(1)    = ''
         , @b_RecordExists                INT            = 0

   DECLARE @c_DefaultPrinterID            NVARCHAR(30)   = ''
   ...
   IF CURSOR_STATUS('LOCAL' , 'C_Pack_AutoPrint') in (0 , 1)
   BEGIN
      CLOSE C_Pack_AutoPrint 
      DEALLOCATE C_Pack_AutoPrint 
   END
   ...
   SELECT @c_StorerKey        = ISNULL(RTRIM(StorerKey   ), '')
         ...
         ,@n_CartonNo         = ISNULL(CartonNo, 0)   --(AL01)
         ,@b_ReprintCtnLabel  = ISNULL(ReprintCtnLabel, 0)   --(AL01)
   ...
   WITH ( 
      ...
      CartonNo         INT                '$.CartonNo',   --(AL01)
      ReprintCtnLabel  BIT                '$.ReprintCtnLabel'
   )
   ...
   SELECT TOP 1 @c_OrderMode = Left(UPPER(OrderMode),1)
   ...
   SET @c_EPACKCloseCartonPrint = dbo.fnc_GetRight(@c_Facility, @c_Storerkey, '', 'EPACKCloseCartonPrint')
   SET @c_EPACKNewCartonSkipPrint = dbo.fnc_GetRight(@c_Facility, @c_Storerkey, '', 'EPACKNewCartonSkipPrint')

   IF @c_PickSlipNo <> ''
   BEGIN
      DECLARE C_LOOP_RPT CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
      ...
      OPEN C_LOOP_RPT
      FETCH NEXT FROM C_LOOP_RPT INTO @n_TotalWMRDetail, @c_ReportID
      ...
      BEGIN
         ...
         IF @c_OrderMode = 'M' AND (@c_EPACKCloseCartonPrint = '1' AND [API].[fnc_IsCloseCartonPrintReport](@c_ReportID) = 1) AND @c_EPACKNewCartonSkipPrint = '0'
            AND @b_ReprintCtnLabel = 0
         BEGIN
            GOTO NEXT_RECORD
         END
