...
GO
/************************************************************************/    
/* Stored Procedure: isp_ECOMP_EPackVas_Activity                        */    
...
/* 30-MAR-2023 Wan03    1.3   PAC-4 NextGen Ecom Packing - Single       */  
/************************************************************************/     
CREATE   PROCEDURE [API].[isp_ECOMP_EPackVas_Activity]  
     @c_Orderkey    NVARCHAR(15)         
   , @c_Storerkey   NVARCHAR(15)    
   , @c_Sku         NVARCHAR(20) = ''  
   , @c_SourceApp   NVARCHAR(10) = 'WMS' 
AS    
BEGIN    
   SET NOCOUNT ON  
   SET ANSI_NULLS OFF  
   ...
   SET @c_SQL  = N'SELECT Orderkey           = MAX(Orderkey) '
               + '       ,OrderLineNumber    = MAX(OrderLineNumber)'
               + '       ,Storerkey                                '
               + '       ,Sku                                      '
               ...
               + ' ,OD.Storerkey As ''Storerkey'' '  
               + ' ,OD.Sku As ''Sku'' '  
               + ' ,Qty = CASE WHEN @c_Sku = '''' THEN OD.QtyAllocated + OD.QtyPicked + OD.ShippedQty ELSE 1 END '  
               --+ ' ,Qty = (OD.QtyAllocated + OD.QtyPicked + OD.ShippedQty) - ODR.PackCnt '  
               + ' ,Activity = ISNULL(RTRIM(ODR.Note1),'''') '  
               + ' ,Checked = ''N'' '
               + ' FROM ORDERDETAIL    OD  WITH (NOLOCK)'  
               + ' JOIN ORDERDETAILREF ODR WITH (NOLOCK) ON (OD.Orderkey = ODR.Orderkey)'  
               +                                       ' AND(OD.OrderLineNumber = ODR.OrderLineNumber)'  
