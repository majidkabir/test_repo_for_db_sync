...
/* Written by: Tyrion                                   															 */               
/* Copy from [BI].[nsp_IKEA_PreAllocate_Qty_Report_Promo                                                      		 */             
/* Purpose:  CN IKEA Pre-Allocate Report_PROMO																		 */  
...
BEGIN  
	...
	if isnull(@storename,'') = 'TM'
		set @storefilter = N' = ''618'' '
	if isnull(@storename,'') = 'GW'
		set @storefilter = N' <> ''618'' '
	else if isnull(@storename,'') = 'ALL'
  ...
  IF OBJECT_ID('tempdb..#Temp_Result ','u') IS NOT NULL  DROP TABLE #Temp_Result; 
	...
	set @parafilter = N' where o.DocType=''E'' and o.storerkey =' + ''''+@n_StorerKey+'''' + N'and o.facility =' + ''''+ @Facility + '''' +  N'and o.[Status] in (' + @n_Status + ')'
		print @parafilter;
...
--    SELECT * FROM #Open_Qty  
    ...
    --aSELECT * FROM #Open_Qty WHERE QtyReplen > 0  
    ...
    FROM #Open_Qty #O WITH (NOLOCK)  
        JOIN BI.V_LOTxLOCxID lli WITH (NOLOCK)  
            ON #O.SKU = lli.Sku  
               AND lli.StorerKey = @n_StorerKey  
        JOIN BI.V_LOC l WITH (NOLOCK)  
            ON lli.Loc = l.Loc  
               AND l.Facility = @Facility  
        JOIN BI.V_LOTATTRIBUTE lot WITH (NOLOCK)  
            ON lot.Lot = lli.Lot  
        JOIN BI.V_SKU sku (NOLOCK)  
            ON sku.StorerKey = @n_StorerKey  
               AND sku.Sku = lli.Sku  
    WHERE #O.QtyReplen > 0  
    ...
    BEGIN  
        ...
        BEGIN  
            ...
            WHERE l.Facility = @Facility  
                  ...
                  AND lli.Sku = @c_SKU  
            GROUP BY lli.Loc  
            ORDER BY ISNULL(SUM(lli.Qty), 0) DESC; ----recommend pick loc with biggest qty   
