...
/************************************************************************/  
/* Stored Proc: isp_GetPickSlipWave14                                   */  
...
/*        :                                                             */  
/* Called By: R_dw_print_wave_pickslip_14                               */  
/*          :                                                           */  
/* PVCS Version: 1.4                                                    */  
/*                                                                      */  
/* Version: 7.0                                                         */  
...
/* 05-Feb-2018 Wan      1.1   WMS-3980 - KR_Nike_Picking Slip Report_Data*/  
/*                            Window_CR                                 */  
/* 21-Nov-2018 WLCHOOI	1.2	WMS-6779 - Add new logic to find             */
/*                            orderselectionkey and colorcode (WL01)    */
/* 12-MAR-2019 WLCHOOI  1.3   WMS-8053 - Add table Orderdetail and      */
/*                            Orderinfo (WL02)                          */
/* 02-AUG-2019 LZG      1.4   INC0794519 - Minor bug fix (ZG01)         */
/************************************************************************/  
CREATE PROC [dbo].[isp_GetPickSlipWave14]  
...
BEGIN  
   ...
   DECLARE    
		 ...
		 , @c_OrderSelectionKey    NVARCHAR(20) = ''	--(WL01)
		 , @c_ColorCode			   NVARCHAR(30) = ''	--(WL01)
   ...
   SET @c_Type    = SUBSTRING(@c_wavekey_type, 11,2)    
	...
	SELECT DISTINCT OrderSelection.OrderSelectionKey
      ...
      LEFT JOIN ORDERINFO WITH (NOLOCK) ON (ORDERS.Orderkey = ORDERINFO.Orderkey)       --WL02  
	...
	-- ORDER BY NoOfCond DESC
	 ...
	 DECLARE CUR_BUILD_WAVE_COND CURSOR LOCAL FAST_FORWARD READ_ONLY FOR  
		...
		'JOIN ORDERS WITH (NOLOCK) ON wavedetail.ORDERKEY = ORDERS.ORDERKEY '  + --WL02
      'JOIN ORDERDETAIL WITH (NOLOCK) ON (ORDERS.Orderkey = ORDERDETAIL.Orderkey) '+ --WL02
      'LEFT JOIN ORDERINFO WITH (NOLOCK) ON (ORDERS.Orderkey = ORDERINFO.Orderkey) '+ --WL02
		'where wavedetail.wavekey = ' + @c_wavekey
	...
	BEGIN 
            ...
            SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Wavetype and OrderSelectionKey Mismatch (isp_GetPickSlipWave14)'     
   ...
   SELECT PD.Storerkey  
		 ...
		 ,Orderselectionkey = CASE WHEN @c_orderSelectionKey = '0' THEN '' ELSE @c_orderSelectionKey END --WL01
   ...
   --JOIN SKU        SKU  WITH (NOLOCK) ON (PD.Storerkey = SKU.Storerkey)  --(Wan01)  
   --                                   AND(PD.Sku =  SKU.Sku)             --(Wan01)  
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         BEGIN      
            SET @n_continue = 3    
            SET @n_err = 81010  
            SET @c_errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Get PickSlip # Failed. (isp_GetPickSlipWave14)'    
            BREAK     
         END               
         ...
         BEGIN        
            ...
            SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert PICKHEADER Failed (isp_GetPickSlipWave14)'     
                           + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '     
            GOTO QUIT_SP       
         END    
      ...
      JOIN PICKDETAIL PD  WITH (NOLOCK) ON (WD.Orderkey = PD.Orderkey)  
      JOIN LOC        LOC WITH (NOLOCK) ON (PD.Loc = LOC.Loc)                 --(Wan01)  
      --JOIN SKU            WITH (NOLOCK) ON (PD.Storerkey = SKU.Storerkey)   --(Wan01)  
--                                  AND(PD.Sku = SKU.Sku)               --(Wan01)  
      WHERE WD.Wavekey = @c_Wavekey  
      ...
      BEGIN  
         ...
         BEGIN  
            ...
            BEGIN    
               SET @n_continue = 3  
               SET @n_err = 81030  
               SET @c_errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert RefKeyLookup Failed. (isp_GetPickSlipWave14)'      
               GOTO QUIT_SP  
            END                  
         END  
         ...
         BEGIN  
            ...
            BEGIN        
               ...
               SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': UPDATE Pickdetail Failed (isp_GetPickSlipWave14)'     
                              + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '     
               GOTO QUIT_SP       
            END   
         END  
...
QUIT_SP:  
   ...
   BEGIN  
      ...
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'isp_GetPickSlipWave14'  
