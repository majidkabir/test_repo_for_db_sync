...
CREATE PROC [dbo].[isp_Print_SSCC_CartonLabel] ( 
...
BEGIN
   ...
   DECLARE @tTempResult TABLE (
         ...
         BillTo_Barcode          NVARCHAR( 16) NULL,  -- exclude brackets
         SSCC_Label              NVARCHAR( 22) NULL,  -- include brackets
         SSCC_Barcode            NVARCHAR( 20) NULL,  -- exclude brackets
   ...
   DECLARE 
      ...
      @cBillTo_Barcode          NVARCHAR( 16),
      @cPartial_SSCC            NVARCHAR( 17),
      @cSSCC_Label              NVARCHAR( 22),
      @cSSCC_Barcode            NVARCHAR( 20),
      @nFromCartonNo            int,
      @nToCartonNo              int,
      @nCartonNo                int,
   ...
   BEGIN
      ...
      IF LEN(dbo.fnc_LTrim(dbo.fnc_RTrim(@cOrdUserDefine03))) <= 4
         SET @cOrdUserDefine03  = RIGHT(REPLICATE('0', 4) + dbo.fnc_LTrim(dbo.fnc_RTrim(@cOrdUserDefine03)), 4)

      IF LEN(dbo.fnc_LTrim(dbo.fnc_RTrim(@cOrdUserDefine04))) <= 4
         SET @cOrdUserDefine04  = RIGHT(REPLICATE('0', 4) + dbo.fnc_LTrim(dbo.fnc_RTrim(@cOrdUserDefine04)), 4)  

      -- Separate AdvertisementCode into 2 parts
      -- Eg. AD 1103 => part1 = AD, part2 = 1103
      IF LEN(dbo.fnc_LTrim(dbo.fnc_RTrim(@cAdvertisementCode))) > 0 
      BEGIN
