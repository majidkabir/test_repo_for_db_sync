...
/***************************************************************************/
/* Stored Procedure: isp_RPT_REPL_REPLEN_009                               */
/* Creation Date: 18-Aug-2022                                              */
/* Copyright: LFL                                                          */
/* Written by: WLChooi                                                     */
/*                                                                         */
/* Purpose: WMS-20526 - [CN] CN_Yonex_Replenishment_Report                 */
/*                                                                         */
/* Called By: Replenishment Report - RPT_REPL_REPLEN_009                   */
/*                                                                         */
/* PVCS Version: 1.0                                                       */
/*                                                                         */
/* Version: 5.4                                                            */
...
/* 18-Aug-2022  WLChooi 1.0   DevOps Combine Script                        */
/***************************************************************************/
CREATE PROC [dbo].[isp_RPT_REPL_REPLEN_009]
...
BEGIN
   ...
   BEGIN
      ...
      DECLARE Cur_ReplenPickLoc CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
         ...
         --HAVING (SKUxLOC.Qty - SKUxLOC.QtyPicked - SKUxLOC.QtyAllocated <= SKUxLOC.QtyLocationMinimum) 
      ...
      BEGIN      
         ...
         SET @n_ReplenQty = (@n_QtyLocationMinimum + ISNULL(@n_NetQtyExpected,0)) - @n_PendingMoveIn
         ...
         BEGIN
            ...
            JOIN   UCC U WITH (NOLOCK) ON U.Lot = LOTxLOCxID.Lot AND U.LOC = LOTxLOCxID.LOC
                                      AND U.ID = LOTxLOCxID.ID AND U.[Status] < '3'
         ...
         BEGIN
            ...
            JOIN UCC U WITH (NOLOCK) ON U.Lot = LLI.Lot AND U.LOC = LLI.LOC
                                    AND U.ID = LLI.ID AND U.[Status] < '3'
         ...
         BEGIN
            ...
            --END
             ...
             --BEGIN
             --     SET @n_QtyToReplen = FLOOR(@n_LLIQty / (@n_casecnt * 1.00)) * @n_Casecnt --take all available with full case
             --END
             --ELSE
             --BEGIN
             --     SET @n_QtyToReplen = FLOOR(@n_RemainingQty / (@n_casecnt * 1.00)) * @n_Casecnt --take all remaining with full case
   ...
   AND  (RP.Replenishmentgroup = @c_ReplGrp OR @c_ReplGrp = 'ALL') 
   AND   LOC.facility = @c_zone01
   AND  (RP.Storerkey = @c_Storerkey OR @c_Storerkey = 'ALL')
   AND  (LOC.PutawayZone IN (@c_zone02, @c_zone03, @c_zone04, @c_zone05, @c_zone06, @c_zone07, @c_zone08, @c_zone09, @c_zone10, @c_zone11, @c_zone12)
   OR  @c_zone02 = 'ALL')
   ...
   JOIN INV ON (RPT.Storerkey = INV.Storerkey)
            ...
            AND(RPT.ID        = INV.ID)
   LEFT JOIN ALLOC   ON (RPT.Storerkey = ALLOC.Storerkey)
                     AND(RPT.Sku       = ALLOC.Sku)
                     AND(RPT.FromLoc   = ALLOC.Loc)
                     AND(RPT.ID        = ALLOC.ID)

   SELECT Facility      = @c_Zone01
      ,  PutawayZone = Loc.PutawayZone
      ,  BatchNoList    = ''
      ,  ReplenGrpList  = RPT.ReplenGrp
      ,  RPT.Storerkey  
      ,  RPT.Sku
      ,  RPT.Replenishmentkey
      ,  RPT.FromLoc
      ,  ToLoc = RPT.ToLoc
      ,  ID    = MIN(RPT.ID)
      ,  RPT.Packkey
      ...
      ,  QtyReplenInCS= (SELECT COUNT(DISTINCT R.RefNo) 
                         FROM REPLENISHMENT R (NOLOCK)
                         WHERE R.ReplenishmentKey = RPT.Replenishmentkey
                         AND R.Storerkey = RPT.Storerkey)
      ,  QtyReplenInEA= 0
      ,  QtyBalInCS   = (SELECT COUNT(DISTINCT UCC.UCCNo) 
                         FROM UCC (NOLOCK) 
                         WHERE UCC.Loc = RPT.FromLoc AND UCC.Storerkey = RPT.Storerkey
                         AND UCC.[Status] = '1')
      ,  QtyBacInEA   = 0
   ...
   SELECT 
      ...
      ,  RPT.QtyBacInEA   
      ,  RPT.RowID
      ,  TotalQtyReplenInCS = (SELECT ISNULL(SUM(TR.QtyReplenInCS),0) FROM #TMP_RESULT TR WHERE TR.PutawayZone = RPT.PutawayZone)
      ,  TotalQtyReplenInEA = (SELECT ISNULL(SUM(TR.QtyReplenInEA),0) FROM #TMP_RESULT TR WHERE TR.PutawayZone = RPT.PutawayZone)
      ,  TotalFullCasePICK  = (SELECT ISNULL(SUM(TR.FullCasePICK),0)  FROM #TMP_RESULT TR WHERE TR.PutawayZone = RPT.PutawayZone)
      ,  TotalLoc           = (SELECT COUNT(DISTINCT TR.FromLoc) FROM #TMP_RESULT TR WHERE TR.PutawayZone = RPT.PutawayZone)
   FROM #TMP_RESULT RPT WITH (NOLOCK)
   ORDER BY RPT.RowID

   IF OBJECT_ID('tempdb..#TMP_REPLENRPT') IS NOT NULL
