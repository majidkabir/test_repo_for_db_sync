...
/*************************************************************************/     
CREATE   PROCEDURE [dbo].[mspRVWAV01]        
...
BEGIN    
   ...
   DECLARE @n_continue       int = 1      
         ...
         , @c_SourceType     NVARCHAR(30) = 'mspRLWAV01'
   ...
   BEGIN  
      ...
      BEGIN                                            
         SET @n_continue = 3    
         SET @n_err = 81010    
         SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': This Wave has not been released. (mspRVWAV01)'           
      END                   
   END  
   ...
   IF @n_continue = 1 OR @n_continue = 2  
   BEGIN  
      SELECT TOP 1 @n_AllowToRev = CASE WHEN TD.TaskType IN ('FPK','FCP','FPP') AND 
                                             TD.[Status] NOT IN ('H', '0') 
                                        THEN 0 
                                        WHEN TD.TaskType IN ('RPF') AND TD.[Status] NOT IN ('0') 
                                        THEN 0
                                        ELSE 1
                                        END
      ...
      BEGIN  
          SET @n_continue = 3    
          SET @n_err = 81020    
          SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Some Tasks have been started. Not allow to Reverse Wave Released (mspRVWAV01)'         
      END                   
   END  
   ...
   BEGIN 
      ...
      SELECT TaskDetailKey =CASE WHEN TD.TaskType IN ('FPK','FCP','FPP') AND 
                                      TD.[Status] IN ('H', '0') 
                                 THEN TD.TaskDetailkey
                                 WHEN TD.TaskType IN ('RPF') AND TD.[Status] IN ('0') 
                                 THEN TD.TaskDetailkey
                                 ELSE ''
                                 END
      FROM TASKDETAIL TD (NOLOCK)   
      ...
      BEGIN
         ...
         AND TASKDETAIL.TaskType IN ('FPK','FCP','FPP','RPF')  
         ...
         BEGIN  
            ...
            SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Delete Taskdetail Table Failed. (mspRVWAV01)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
         END 
         FETCH NEXT FROM @CUR_DELTASK INTO @c_TaskDetailKey
      END
      CLOSE @CUR_DELTASK
   ...
   BEGIN 
      ...
      BEGIN
         ...
         BEGIN  
            ...
            SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Pickdetail Table Failed. (mspRVWAV01)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
         END 
         FETCH NEXT FROM @CUR_DELPICK INTO @c_PickDetailKey
      END
      CLOSE @CUR_DELPICK
   ...
   BEGIN    
      ...
      BEGIN    
         ...
         SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update on wave Failed (mspRVWAV01)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
      END    
   END    
...
RETURN_SP:  
   ...
   BEGIN    
      ...
      BEGIN    
         ...
         END    
      END    
      execute nsp_logerror @n_err, @c_errmsg, "mspRVWAV01"    
