...
/*************************************************************************/  
/* Stored Procedure: isp_EXG_FilterCondition3                            */  
/* Creation Date: 16 Jun 2020                                            */  
/* Copyright: LFL                                                        */  
/* Written by: GuanHao Chan                                              */  
/*                                                                       */  
/* Purpose: Excel Generator Sub StoredProcedure FilterConfition 3.       */  
/*                                                                       */  
...
CREATE PROCEDURE [dbo].[isp_EXG_FilterCondition3]  
...
BEGIN  
 ...
 IF @b_Debug = 1  
   BEGIN  
      PRINT '[dbo].[isp_EXG_FilterCondition3]: Start...'  
      PRINT '[dbo].[isp_EXG_FilterCondition3]: '  
   ...
   BEGIN  
         SET @n_Err = 230001  
         SET @c_ErrMsg = 'Invalid EXG Hdr ID. (isp_EXG_FilterCondition3)'  
         SET @n_Continue = 3  
         GOTO QUIT  
   END  
   ...
   BEGIN TRY  
      ...
      BEGIN  
         SET @n_Err = 230001  
         SET @c_ErrMsg = ' StorerKey cannot be null. - (isp_EXG_FilterCondition3)'  
     
         IF @b_Debug = 1  
         BEGIN  
            PRINT '[isp_EXG_FilterCondition3]: Execute SP Name Failed...' + ' @c_ErrMsg=' + @c_ErrMsg  
         END  
         GOTO QUIT  
      END  
      ...
      BEGIN  
         SET @n_Err = 230009  
         SET @c_ErrMsg = ' Facility cannot be null. - (isp_EXG_FilterCondition3)'  
      ...
      BEGIN  
         SET @n_Err = 230010  
         SET @c_ErrMsg = ' MbolKey cannot be null. - (isp_EXG_FilterCondition3)'  
      ...
      BEGIN  
         INSERT INTO @Temp_BatchPVal ([filename]  
                                       ...
                                       , [PVal3]  
                                       , [PVal4])  
         SELECT CONCAT('Porsche_Goods_Out_notice_',ISNULL(RTRIM(OrderKey),''), '_', ISNULL(RTRIM(ExternOrderKey),''), @c_FileExt)  
               , StorerKey  
               , Facility  
               , MBOLKey  
               , OrderKey   
         ...
         AND Facility = @c_ParamVal2  
         AND MBOLKey = @c_ParamVal3  
         GROUP BY StorerKey, Facility, MBOLKey, OrderKey, ExternOrderKey  
...
STARTCURSOR:    
   ...
   BEGIN CATCH   
      ...
      SET @n_Err = @n_ITFErr  
      SET @c_ErrMsg = LTRIM(RTRIM(@c_ITFErrMsg)) + ' (isp_EXG_FilterCondition3)'  
                 
      IF @b_Debug = 1  
      BEGIN  
         PRINT '[isp_EXG_FilterCondition3]: Execute SP Name Failed...'   
               + ' @c_ErrMsg=' + @c_ErrMsg  
      END  
      GOTO QUIT  
   END CATCH  
...
QUIT:  
   ...
   BEGIN        
      ...
      IF @b_Debug = 1  
      BEGIN  
         PRINT '[dbo].[isp_EXG_FilterCondition3]: @c_ErrMsg=' + RTRIM(@c_ErrMsg)  
         PRINT '[dbo].[isp_EXG_FilterCondition3]: @b_Success=' + RTRIM(CAST(@b_Success AS NVARCHAR))  
      END  
