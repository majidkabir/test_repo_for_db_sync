...
/************************************************************************/
/* Stored Proc: ispManningsLoadFC2                                      */
...
/************************************************************************/
CREATE PROC [dbo].[ispManningsLoadFC2]
...
BEGIN
   ...
   BEGIN
      PRINT '---------------------------------------'+ CHAR(13) +
            'Sub PICKCOde: ispManningsLoadFC2'+ CHAR(13) +
  ...
  ;WITH ORD AS
   ...
   +' AND   UCC.Qty <= @n_QtyLeftToFullfill'
  ...
  ;WITH UCC AS
   ...
   BEGIN
      ...
      BEGIN CATCH  
         ...
         SELECT @n_continue = 3  
         SET @c_ErrMsg = RTRIM(@c_ErrMsg) + '. (ispManningsLoadFC2)'   
      END CATCH
      ...
      BEGIN
         ...
         --IF @n_Channel_Qty_Available < @n_QtyLeftToFullFill
         --BEGIN
         --   GOTO NEXT_UCC
         --END 

         IF @n_Channel_Qty_Available < @n_UCCQty 
         BEGIN   
            GOTO NEXT_UCC
         END  
      END  
         ...
         BEGIN
            ...
            BEGIN
               ...
               SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0)) 
                              + ': Get PickDetailKey Failed. (ispManningsLoadFC2)'
               GOTO QUIT_SP
            END
            ...
            BEGIN
               ...
               SET @n_Err = 81220
               SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0)) 
                              + ': Insert PickDetail Failed. (ispManningsLoadFC2)'
               GOTO QUIT_SP
            END
...
QUIT_SP:
   ...
   BEGIN
      ...
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'ispManningsLoadFC2'
