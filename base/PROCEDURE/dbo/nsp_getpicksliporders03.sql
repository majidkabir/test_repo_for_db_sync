...
GO
/*********************************************************************************/    
/* Stored Proc : nsp_GetPickSlipOrders03                                         */    
/* Creation Date:                                                                */    
/* Copyright: IDS                                                                */    
/* Written by:                                                                   */    
/*                                                                               */    
...
/* 2019-11-20  WLChooi     2.3   WMS-11179 - New ReportCFG (WL02)                */    
/* 2020-01-22  WLChooi     2.4   WMS-11864 - New ReportCFG to show RetailSKU     */    
/*                               and ManufacturerSKU (WL03)                      */    
...
DECLARE  @c_PrevOrderKey NVARCHAR(10),    
         ...
         @c_InvoiceNo    NVARCHAR(20)        -- (YokeBeen01) (CLVN01)
...
BEGIN TRAN    
   CREATE TABLE #temp_pick    
         (PickSlipNo  NVARCHAR(10) NULL,    
          ...
          InvoiceNo   NVARCHAR(20) NULL,      -- (YokeBeen01) (CLVN01)
  ...
  ,   ShowLottable10    NVARCHAR(10) NULL      --ML01    
   ...
   SELECT Storerkey    
         ...
         ,ShowExtraInfo = ISNULL(MAX(CASE WHEN Code = 'ShowExtraInfo'  THEN 1 ELSE 0 END),0)   --(WL02)    
         ,ShowExtraSKUInfo = ISNULL(MAX(CASE WHEN Code = 'ShowExtraSKUInfo'  THEN 1 ELSE 0 END),0)   --(WL03)    
         ,SORTBYLOC = ISNULL(MAX(CASE WHEN Code = 'SORTBYLOC'  THEN 1 ELSE 0 END),0)   --(CS06)    
   INTO #TMP_RPTCFG    
   FROM CODELKUP WITH (NOLOCK)    
   WHERE ListName = 'REPORTCFG'    
   AND Long       = 'r_dw_print_pickorder03'    
  ...
  ,  ISNULL(CLR2.Short,'') AS ShowLottable10 --ML01    
   ...
   ON (Orders.Storerkey = CLR2.Storerkey AND CLR2.Code = 'ShowLottable10'    
   AND CLR2.Listname = 'REPORTCFG' AND CLR2.Long = 'r_dw_print_pickorder03')    
   --ML01 END    
...
SUCCESS:    
   SELECT    
      ...
      ,  ShowSKUBusr10  --(CS05)    
      ,  SKUBusr10      --(CS05)    
      ,  CASE WHEN ISNULL(ShowExtraInfo,0) = 1 THEN @c_ExtraInfo ELSE '' END AS ExtraInfo  --(WL02)    
   ...
   ORDER BY Company    
         ...
         ,  PickZone                                              --(Wan03)    
         ,  SUSR3    
         ,  CASE WHEN SortBySkuLoc = 1 AND Sortbyloc = 0 THEN Sku ELSE '' END               --(CS06)    
         ,  CASE WHEN SortBySkuLoc = 1 OR Sortbyloc = 1 THEN '' ELSE LogicalLoc END        --(CS06)    
         ,  Loc    
