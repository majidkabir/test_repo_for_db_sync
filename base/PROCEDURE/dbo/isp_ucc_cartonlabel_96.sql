...
/************************************************************************/
/* Store Procedure: isp_UCC_CartonLabel_96                              */
/* Creation Date: 03-SEP-2020                                           */
/* Copyright: IDS                                                       */
/* Written by: CSCHONG                                                  */
/*                                                                      */
/* Purpose: WMS-15027 - [CN] GBMAX Carton label                         */
/*                                                                      */
/* Called By:r_dw_UCC_Carton_Label_96                                   */
/*                                                                      */
/* PVCS Version: 1.0                                                    */
/*                                                                      */
/* Version: 5.4                                                         */
...
/************************************************************************/
CREATE PROC [dbo].[isp_UCC_CartonLabel_96] (
...
BEGIN
   ...
   @c_ExecStatements7    nvarchar(4000),    
   ...
   --  'LEFT OUTER JOIN STORER STORERContact (NOLOCK) ON ( STORERContact.Type = ''2'' AND STORERContact.StorerKey = ORDERS.ConsigneeKey)  '+
     'LEFT OUTER JOIN PACKINFO PIF (NOLOCK) ON (PIF.Pickslipno = PACKDETAIL.Pickslipno AND PIF.CartonNo = PACKDETAIL.CartonNo)'
   ...
   SET @c_ExecStatements5 = N',CASE WHEN ISNULL(CL.SHORT,''N'') = ''Y'' AND CAST(CL.LONG AS INT) <> 0 THEN
   ...
   SET @c_ExecStatements6 = N'OUTER APPLY (SELECT TOP 1 CL.SHORT, CL.LONG, CL.UDF01, CL.UDF02, CL.UDF03, CL.CODE2 FROM
   ...
   SET @c_ExecStatements7 = N',CL.SHORT, CL.LONG, CL.UDF01, CL.UDF02, CL.UDF03'
