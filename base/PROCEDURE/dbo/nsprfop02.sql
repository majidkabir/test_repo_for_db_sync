...
BEGIN
   ...
   IF @n_continue=1 OR @n_continue=2
   BEGIN
      IF NOT EXISTS (SELECT * FROM LOC WHERE Loc = @c_toloc)
      BEGIN
         SELECT @n_continue = 3
         SELECT @n_err = 65702
         SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Bad ToLoc. (nspRFOP02)"
      END
   END
   IF @n_continue=1 OR @n_continue=2
   BEGIN
      ...
      BEGIN
         SELECT @n_continue = 3
         SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err = 65703   -- Should Be Set To The SQL Errmessage but I don't know how to do so.
         SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Update PICKDETAIL Failed. (nspRFOP02)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
      END
   ELSE IF @n_cnt = 0
      BEGIN
         SELECT @n_continue = 3
         SELECT @n_err = 65704
         SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Bad ShipLabel. (nspRFOP02)"
      END
   END
   IF @n_continue=3
   BEGIN
...
ELSE
   ...
   ELSE
      BEGIN
         ...
         END
      END
      execute nsp_logerror @n_err, @c_errmsg, "nspRFOP02"
