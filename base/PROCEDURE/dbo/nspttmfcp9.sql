...
BEGIN
   ...
   DECLARE @cCursor_FCPTaskCandidates Cursor
   ...
   SAVE TRAN nspTTMFCP9 -- For rollback or commit only our own transaction
   ...
   SET @cSQL = ' SET @cCursor_FCPTaskCandidates = CURSOR FOR    
      SELECT TaskDetailkey,
         (SELECT COUNT(1) FROM TaskDetail T (NOLOCK)
         WHERE T.AreaKey = @c_AreaKey01
            AND T.TaskType IN (''FCP'', ''FCP1'')
            AND T.Status = ''0''
   ...
   SET @cSQL = @cSQL +
            'Join dbo.booking_out with (nolock) on (tms_shipment.bookingno = booking_out.bookingno)
               WHERE TaskDetail.TaskType IN (''FCP'', ''FCP1'')' +
               ...
               ORDER BY
                  ...
                  '
   -- Get a task
   SET @cSQL = @cSQL + '    OPEN @cCursor_FCPTaskCandidates
   FETCH NEXT FROM @cCursor_FCPTaskCandidates INTO @c_TaskDetailKey, @cUserKeyOverRide'

   SET @cSQLParam =  
   ' @c_AreaKey01      NVARCHAR(20),   '+
   ' @c_userid         NVARCHAR(18),   '   + 
   ' @cCursor_FCPTaskCandidates CURSOR OUTPUT,' +
    '@c_TaskDetailKey  NVARCHAR(20) OUTPUT,   '     + 
    '@cUserKeyOverRide  NVARCHAR(20)  OUTPUT '     
  
   EXEC sp_ExecuteSQL @cSQL, @cSQLParam,  @c_AreaKey01, @c_userid,@cCursor_FCPTaskCandidates OUTPUT,@c_TaskDetailKey OUTPUT,@cUserKeyOverRide OUTPUT
   ...
   COMMIT TRAN nspTTMFCP9 -- Only commit change made here
   GOTO Quit

RollBackTran:
   ROLLBACK TRAN nspTTMFCP9 -- Only rollback change made here
Fail:
Quit:
