...
GO
/***************************************************************************/
/* Stored Procedure: ispGNPOD02                                            */
/* Creation Date: 24-Apr-2018                                              */
/* Copyright: LFL                                                          */
/* Written by:                                                             */
/*                                                                         */
/* Purpose: WMS-4685 Mast Global Hk POD, stamp tracking number on          */
/*          POD.TrackCol02                                                 */ 
/*        :                                                                */
/*                                                                         */
/* Called By:                                                              */
...
BEGIN
   ...
   IF @b_Success <> 1
   BEGIN
      SELECT @c_ErrMsg = 'ispGNPOD02 ' + dbo.fnc_RTrim(@c_ErrMsg)
   END
   ELSE
   BEGIN
      ...
      BEGIN
         ...
         SELECT
            ...
            MD.UserDefine02,   ISNULL(O.TrackingNo,'')
         ...
         BEGIN
            ...
            SELECT @c_ErrMsg = 'NSQL' + CONVERT(char(5),ISNULL(@n_Err,0))
                             + ': Insert Failed On Table POD. (ispGNPOD02)'
...
QUIT_SP:
   ...
   BEGIN  
      ...
      BEGIN
         ROLLBACK TRAN
      END
      EXECUTE nsp_logerror @n_Err, @c_ErrMsg, 'ispGNPOD02'
