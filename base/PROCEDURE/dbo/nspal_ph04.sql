...
/************************************************************************/  
/* Stored Procedure: nspAL_PH04                                         */  
/* Creation Date: 23-Jan-2017                                           */  
/* Copyright: LFL                                                       */  
/* Written by:                                                          */  
/*                                                                      */  
/* Purpose: WMS-8575 PH Unilever Allocation Strategy                    */  
/*   1. full pallet from bulk then case/pick   1                        */  
/*   2. full case from case,pick then bulk     2                        */  
/*   3. loose from pick,case then bulk         6                        */                     
/*                                                                      */                                                    
/* Called By:                                                           */  
/*                                                                      */  
/* PVCS Version: 1.0                                                    */  
...
/* 01-Sep-2019  SHONG   1.1  Customize for UNILEVER                     */  
/* 03-Feb-2020  LZG     1.2  INC1024793 - Allocate by BULK loc first    */
/*                                        (ZG01)                        */
/************************************************************************/  
CREATE PROC [dbo].[nspAL_PH04]   
@c_lot char(10) ,  
@c_uom char(10) ,  
...
BEGIN  
  ...
  DECLARE @c_OrderKey        VARCHAR(10),  
   ...
   BEGIN       
    ...
    --IF LEN(@c_OtherParms) >= 10   
    --BEGIN  
    --   IF LTRIM(RTRIM(SUBSTRING(@c_OtherParms, 11,6))) = 'W'  
    --   BEGIN  
    --    SET @c_WaveKey = LEFT(@c_OtherParms,10)   
    --    IF EXISTS(SELECT 1   
    --              FROM WAVEDETAIL AS w WITH(NOLOCK)  
    ...
    --   END         
    --END  
    --PRINT '     AllowMonkeyPick: ' +  @c_AllowMonkeyPick  
   END     
   ...
   BEGIN  
      ...
      --ORDER BY CASE WHEN LOC.LocationType NOT IN('PICK') THEN 1 ELSE 2 END, LOC.LogicalLocation, LOC.LOC   	-- ZG01
      ORDER BY CASE WHEN LOC.LocationType NOT IN('CASE','PICK') THEN 1 WHEN LOC.LocationType = 'CASE' THEN 2 ELSE 3 END, LOC.LogicalLocation, LOC.LOC   	-- ZG01
   END  
   ELSE IF @c_UOM = '2'  
   BEGIN   
      DECLARE  CURSOR_CANDIDATES CURSOR FAST_FORWARD READ_ONLY  
