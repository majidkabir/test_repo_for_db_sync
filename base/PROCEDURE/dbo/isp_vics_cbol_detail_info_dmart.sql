...
/************************************************************************/  
/* Store Procedure: isp_Vics_CBOL_Detail_Info_DMART                     */  
...
CREATE PROC [dbo].[isp_Vics_CBOL_Detail_Info_DMART]   
(  
@ai_cbolkey BIGINT  
)  
AS  
BEGIN  
   ...
   IF EXISTS(SELECT 1 FROM DATAMART.ODS.ORDERS O (NOLOCK)   
             ...
             JOIN DATAMART.ODS.MBOL MB (NOLOCK) ON O.Mbolkey = MB.Mbolkey  
   ...
   BEGIN  
      ...
      JOIN DATAMART.ODS.MBOLDETAIL MBOLDETAIL WITH (NOLOCK) ON ( MBOL.Mbolkey = MBOLDETAIL.Mbolkey )         
      ...
      INTO #SUMM  
      FROM [dbo].[fnc_GetVicsCBOL_CartonInfo_DMART](@ai_cbolkey)  
