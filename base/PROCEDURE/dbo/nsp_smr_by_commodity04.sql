...
/************************************************************************/
/* Stored Proc : nsp_smr_by_Commodity04                                 */
/* Creation Date:                                                       */
/* Copyright: IDS                                                       */
/* Written by:                                                          */
/*                                                                      */
...
/* 01.Jun.2005  June         SOS36334 - Include trantype WD-TF & DP-TF  */
/*                           when calculating running total.            */
/* 15.Aug.2005  June         SOS39343 - Display 'Adjustment' in Trantype*/
/*                           shown Adjustment.CustomerRefNo in BuyerPO  */
/*                                                                      */
/************************************************************************/

CREATE PROC [dbo].[nsp_smr_by_Commodity04] (
                     ...
                     , BuyerPO = isnull(CASE
                            WHEN a.SourceType = 'ntrPickDetailUpdate'
                                 THEN ( SELECT ORDERS.BuyerPO
                                          FROM ORDERS(NOLOCK) WHERE orderkey = (select orderkey from pickdetail (NOLOCK) where pickdetailkey = SUBSTRING(a.SourceKey,1,10)))
                                          -- Start : SOS39343
                            WHEN a.SourceType like 'ntrAdjustmentDetail%'
                                 THEN ( SELECT ADJUSTMENT.CustomerRefNo
                                          FROM ADJUSTMENT (NOLOCK) WHERE Adjustmentkey = SUBSTRING(a.SourceKey,1,10))
                                          -- End : SOS39343
                               ELSE " "
                               END, "-")
       ...
       DECLARE @record_number int
          ...
          BEGIN
               ...
               IF (dbo.fnc_RTrim(@TranType) = 'DP' or dbo.fnc_RTrim(@TranType) = 'WD' or 
                   dbo.fnc_RTrim(@TranType) = 'DP-TF' or dbo.fnc_RTrim(@TranType) = 'WD-TF' or  -- SOS36334
     ...
     BEGIN
        ...
        , TranType =
               CASE
                 ...
                 WHEN #BF2.TranType = "AJ" THEN "Adjustment" -- SOS39343
