...
/***************************************************************************/ 
/* Object Name:            isp_GenEOrder_Replenishment                     */
...
/* 16-May-2017  Shong     1.0   Initial Version                            */
/* 31-Jul-2017  Shong     1.1   Fixing ReplenQty Calcualtion issues        */
/* 26-Oct-2017  TLTING01  1.2   Fix Swap PD status filter                  */
/* 03-Nov-2017  Shong     1.3   Fixing Issues                              */
/* 03-Nov-2017  Shong     1.4   Force Lottables for swap lot               */
/* 06-Nov-2017  Shong     1.5   Take loose qty from bulk if no full case   */
/* 07-Nov-2017  Shong     1.6   Add Debug Indicator to ToLoc and           */
/*                              ReplenishZone                              */
/* 09-Nov-2017  Shong     1.7   Bug Fixing (SWT01) (SWT02)                 */
/* 12-Nov-2017  TLTING    1.8   Performance tune - Commmit by stage        */
/* 14-Nov-2017  NJOW01    1.9   Fix pickdetail.qty not tally with order    */
/*                              detail                                     */
/* 22-Jun-2018  Shong     2.0   Change the Debug Message Column in Pick DT */
/* 08-Nov-2018  TLTING    2.1   Performance tune, #tmp to @table           */
/* 12-Dec-2018  TLTING    2.2   Performance tune                           */
/* 12-Jun-2018  Wan01     2.3   WMS-5218 - [CN] UA Relocation Phase II -   */
/*                              Exceed Generate and Confirm Replenishment(B2C)*/
/* 08-Apr-2019  NJOW02    2.4   WMS-8705 - Stll update replengroup to      */
/*                              taskbatch if no replen record created. It  */
...
/*                              Table (WL01)                               */
/* 16-Jun-2021  TLTING03  3.0   Performance tune - add @tTaskOrders        */
/***************************************************************************/    
...
BEGIN  
   ...
   BEGIN
      --IF OBJECT_ID('tempdb..#loadkey') IS NOT NULL
      --BEGIN
      --   DROP TABLE #loadkey
      --END
      --tlting01
      ...
      BEGIN
         ...
         BEGIN
            SELECT @n_continue = 3
            SELECT @n_err = 78301
            SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Insert into Temp Table @tloadkey Failed! (isp_GenEOrder_Replenishment)'        
            GOTO EXIT_SP
         END       
      END
      ELSE
   ...
   --IF OBJECT_ID('tempdb..#MoveLot') IS NOT NULL
   --BEGIN
   --   DROP TABLE #MoveLot
   --END
   --tlting01
   ...
   --IF OBJECT_ID('tempdb..#TaskBatchNo') IS NOT NULL
   --BEGIN
   --   DROP TABLE #TaskBatchNo
   --END
   --tlting01
   ...
   BEGIN
      SET @n_continue = 3
      SET @n_err = 78329
      SET @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': nspGetRight Failed! (isp_GenEOrder_Replenishment)'
      GOTO EXIT_SP      
   END
   --(Wan01) - END  
   ...
   BEGIN    
      ...
      BEGIN
         ...
         BEGIN
            SELECT @n_continue = 3
            SELECT @n_err = 78303
            SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Insert into Temp Table @tTaskBatchNo Failed! (isp_GenEOrder_Replenishment)'
            GOTO EXIT_SP         
         END
      END
      ELSE 
      ...
      BEGIN
         ...
         BEGIN
            ...
            SET @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Replenishment are generated for the selected Task.'
                         +' Generate Replenishment Abort.(isp_GenEOrder_Replenishment)'
            GOTO EXIT_SP  
         END
      END
      --(Wan01) - END   
   ...
   BEGIN
      SELECT @n_continue = 3
      SELECT @n_err = 78307
      SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': nspGetRight Failed! (isp_GenEOrder_Replenishment)'
      GOTO EXIT_SP      
   END
   ...
   BEGIN
      ...
      DECLARE CUR_Bulk_UCCReplen CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
         ...
         JOIN @tTaskOrders AS PT   ON PT.OrderKey = P.OrderKey    --TLTING03
            ...
            BEGIN            	  
            	 ...
            	 BEGIN
                  ...
                  BEGIN
                     SELECT @n_continue = 3
                     SELECT @n_err = 78313
                     SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': INSERT PickDetail Failed! (isp_GenEOrder_Replenishment)'
                  END                   
   ...
   FROM PICKDETAIL AS p WITH (NOLOCK) 
   JOIN @tTaskOrders AS PT   ON PT.OrderKey = P.OrderKey       --tlting01
   -- JOIN PackTask AS PT WITH (NOLOCK) ON PT.OrderKey = P.OrderKey 
   JOIN ORDERS AS o WITH(NOLOCK) ON o.Orderkey = PT.Orderkey 
   --JOIN @tTaskBatchNo AS tbn  ON tbn.TaskBatchNo = PT.TaskBatchNo 
   --JOIN LoadPlanDetail AS lpd WITH (NOLOCK) ON lpd.Orderkey = PT.Orderkey 
   ...
   WHERE (PK.CaseCnt > 0 OR @c_EOrderReplenByUCC = '1') --NJOW03
   AND   P.[Status] = '0'   
   AND   P.UOM IN ('6','7') 
   ...
   BEGIN    
      ...
      IF @c_EOrderReplenByUCC = '1'  --NJOW03
         SET @n_CaseCnt = 0                  
      ...
      BEGIN        
         ...
         BEGIN   
            ...
            BEGIN                            
               ...
               BEGIN
                  SELECT @n_continue = 3
                  SELECT @n_err = 78309
                  SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Updating PickDetail Failed! (isp_GenEOrder_Replenishment)'
                  GOTO EXIT_SP         
               END
                     ...
                     BEGIN
                        ...
                        BEGIN
                           ...
                           BEGIN
                              SELECT @n_continue = 3
                              SELECT @n_err = 78310
                              SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Updating LOTxLOCxID Failed! (isp_GenEOrder_Replenishment)'
                              GOTO EXIT_SP         
                           END
                        END                                                                           
                     END
         ...
         --BEGIN
            ...
            --   SELECT @n_continue = 3
            --   SELECT @n_err = 78300
            --   SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Update Replenishment Failed! (isp_GenEOrder_Replenishment)'
            --   GOTO EXIT_SP       
            --END     
   ...
   --DECLARE CUR_SWAP_PICKDETAIL CURSOR FAST_FORWARD READ_ONLY FOR
   
   --OPEN CUR_SWAP_PICKDETAIL
   ...
   BEGIN
      ...
      AND EXISTS ( SELECT 1 FROM @tTaskOrders AS PT WHERE PT.OrderKey = OD.OrderKey  ) --TLTING03
      ...
      BEGIN
         ...
         BEGIN -- IF @n_QtyAvaliable = 0 
            ...
            WHERE NOT EXISTS ( SELECT 1 FROM @tTaskOrders AS PT WHERE PT.OrderKey = P.OrderKey  ) --TLTING03
            AND   P.DoReplenish = 'N'
            AND   P.LOC = @c_LOC
            AND   P.UOM = '6'
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            SELECT @n_continue = 3
            SELECT @n_err = 78328
            SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Updating PackTask Failed! (isp_GenEOrder_Replenishment)'
            GOTO EXIT_SP         
         END
