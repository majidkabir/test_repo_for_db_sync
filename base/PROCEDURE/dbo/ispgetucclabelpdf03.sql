...
/************************************************************************/
/* Trigger: ispGetUCCLABELPDF03                                         */
/* Creation Date: 06-May-2020                                           */
/* Copyright: LF Logistics                                              */
/* Written by: WLChooi                                                  */
/*                                                                      */
/* Purpose: WMS-13124 - MYS_ADIDAS_Quantium_Exceed_print_Shiplabel      */
/*                                                                      */
...
BEGIN  
   ...
   IF LTRIM(RTRIM(ISNULL(@c_DocType,''))) <> 'E'
   BEGIN
      SET @n_PrintAction = 0
      GOTO QUIT_SP
   END
   ...
   BEGIN
      ...
      BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err) 
               + ': PDF Image Server Not Yet Setup/Enable In Storerconfig for Configkey :' + @c_Configkey + ' (ispGetUCCLABELPDF03) '
               + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
         SET @n_PrintAction = 0
         GOTO QUIT_SP
       END
      ...
      SET @c_PDFFilePath = @c_PdfFolder  + 'ADIDAS_' + @c_Shipperkey + '_SHPLBL_' + RTRIM(@c_TrackingNo) + '.PDF'
      SET @c_PdfFile = @c_PDFFilePath
      EXEC dbo.xp_fileexist @c_PDFFilePath, @n_IsExists OUTPUT

      IF @n_IsExists = 0
      BEGIN
         SET @c_PDFFilePath = @c_ArchiveFolder + 'ADIDAS_' + @c_Shipperkey + '_SHPLBL_' + RTRIM(@c_TrackingNo) + '.PDF'
         SET @c_PdfFile = @c_PDFFilePath
         SET @c_ArchivePath = '' 
         SET @c_ActionType = '2'
         EXEC dbo.xp_fileexist @c_PDFFilePath, @n_IsExists OUTPUT 
      ...
      BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': PDF filename with TrackingNo: ' + @c_TrackingNo + ' not found. '
                         +'(ispGetUCCLABELPDF03)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
         SET @n_PrintAction = 0
         GOTO QUIT_SP  
      END
   END
  ...
  BEGIN
      ...
      BEGIN
         ...
         END          
      END
      EXECUTE nsp_logerror @n_err, @c_errmsg, "ispGetUCCLABELPDF03"
