...
/************************************************************************/
/* Store Procedure:  isp_proforma_01                                    */
...
/* Return Status:  None                                                 */
/*                                                                      */
/* Usage:  Used for report dw = r_dw_proforma_01                			*/
/*                                                                      */
/* Local Variables:                                                     */
/*                                                                      */
...
CREATE PROC [dbo].[isp_proforma_01] (@c_mbolkey NVARCHAR(10)) 
AS
BEGIN
   SET NOCOUNT ON			
   SET QUOTED_IDENTIFIER OFF	
   ...
   BEGIN
        ...
        SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': CODELKUP LISTNAME BOL_NCOUNT Retrieving Failed For Facility '+RTRIM(@c_facility)+' (isp_proforma_01)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
   	 END 
...
--						BookingReference = ISNULL(@cBookingReference,BookingReference),
           ...
           BEGIN
              ...
              SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Update MBOL Failed. (isp_proforma_01)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
   	       END
   	    END
   	  END
   END
   ...
   BEGIN
		  SELECT MB.MBOLKey, MB.ExternMBOLKey,
               ...
               CN.Address2 AS DelAddress2, 
					...
					ISNULL(SK.BUSR5,'') AS GroupSKU,
					GroupDSC =	CASE WHEN ISNULL(sk.BUSR5,'') = 'BR' THEN  'BARS   '  
									WHEN ISNULL(sk.BUSR5,'') <> 'BR' THEN 'OTHERS' END,
					CaseKG = ROUND( sum((PD.QTY/PK.CASECNT) * round(SK.STDGROSSWGT*PK.CASECNT,3) ),3),
					CaseMeasur = ROUND( sum((PD.QTY/PK.CASECNT) * round(SK.STDCUBE*PK.CASECNT,3) ),3),
					CaseAmount = ROUND( sum((PD.QTY/PK.CASECNT) * SK.cost),3),  
					Casecnt = ROUND( sum(PD.QTY/PK.CASECNT ),3), 
					MB.EditWho AS PreparedBy, 
					Convert(NVARCHAR(200),CLK.long) AS OthReference1, 
--					MB.BookingReference AS OthReference2,
					@cBookingReference as OthReference2,
					Convert(NVARCHAR(200),MB.Remarks) AS OthReference3,
				...
				GROUP BY MB.MBOLKey, MB.ExternMBOLKey, CN.company,
				CN.Address1, CN.Address2, CN.Address3, FC.Descr,
				MB.USERDEFINE05, MB.USERDEFINE09, ST.Company,
				FW.Company, FW.Address1, FW.Address2,
				MB.VesselQualifier, MB.USERDEFINE10, ISNULL(sk.BUSR5,''),
				Convert(NVARCHAR(200),MB.Remarks), MB.editdate, MB.EditWho, 
				CLK.long, MB.BookingReference

   END
            
   IF @n_continue = 3
   BEGIN
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'isp_proforma_01'
