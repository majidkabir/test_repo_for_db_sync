...
GO
/************************************************************************/    
/* Store Procedure:  nsp_ReplenishmentRpt_BatchRefill_12                */    
/* Creation Date: 01-Apr-2011                                           */    
/* Copyright: IDS                                                       */    
/* Written by: AQSKC                                                    */    
/*                                                                      */    
/* Purpose: SOS#210476 Replenishment Report for XTEP                    */    
...
/* Usage:                                                               */    
/*                                                                      */    
/* Called By: r_replenishment_report12                                  */    
/*                                                                      */    
/* PVCS Version: 1.3                                                    */    
/*                                                                      */    
/* Version: 5.4                                                         */    
...
/* 23-Feb-2012 KHLim01  1.1   Reduce blocking                           */  
/* 05-MAR-2018 Wan01    1.2   WM - Add Functype                         */  
/* 05-OCT-2018 CZTENG01 1.3   WM - Add StorerKey,ReplGrp                */  
/************************************************************************/    
    
CREATE PROC  [dbo].[nsp_ReplenishmentRpt_BatchRefill_12]    
...
--      SELECT @b_debug = CAST( @c_zone12 AS int)    
 ...
 -- Start : #89979    
     ...
     -- KHLim01 start  
      ...
      BEGIN   
         ...
         BEGIN  
            ...
            BEGIN  
               ...
               SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Fail to get REPLENISHKEY. (nsp_ReplenishmentRpt_BatchRefill_12)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + ' ) '  
            END      
            ELSE  
            BEGIN   
               COMMIT TRAN  
         ...
         BEGIN  
           ...
           SELECT  
            ...
            BEGIN    
               ...
               SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Insert into Replenishment table failed. (nsp_ReplenishmentRpt_BatchRefill_12)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + ' ) '    
            END    
      ...
      BEGIN    
         ...
         BEGIN    
            ...
            END    
         END    
         EXECUTE nsp_logerror @n_err, @c_errmsg, 'nsp_ReplenishmentRpt_BatchRefill_12'    
   ...
   BEGIN    
      ...
      JOIN SKU WITH (NOLOCK) ON SKU.Sku = R.Sku AND SKU.StorerKey = R.StorerKey          JOIN LOC L1 WITH (NOLOCK) ON L1.Loc = R.ToLoc    
      ...
      ORDER BY L1.PutawayZone, R.FromLoc, R.Priority, SKU.SKU    
