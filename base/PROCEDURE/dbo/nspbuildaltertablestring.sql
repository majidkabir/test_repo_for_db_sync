...
GO
/************************************************************************/      
/* Stored Proc : nspBuildAlterTableString                               */      
/* Creation Date:                                                       */      
/* Copyright: IDS                                                       */      
/* Written by:                                                          */      
/*                                                                      */      
...
CREATE PROCEDURE [dbo].[nspBuildAlterTableString]    
...
BEGIN     
 ...
 SELECT @c_exist1 = RTRIM(@c_copyto_db) + '..' + RTRIM(@c_tablename)    
 IF (@b_debug = 1)    
 BEGIN    
 ...
 BEGIN     
  ...
  SELECT @c_errmsg = "NSQL " + CONVERT(char(5),@n_err)+":Table does not exist in Target Database " +    
   @c_tablename + "(nspBuildAlterTableString)"    
 END       
 ...
 BEGIN      
  ...
  BEGIN      
   ...
   SELECT @c_errmsg = CONVERT(char(250),@n_err)    
    + ":  Open of cursor failed. (nspBuildAlterTableString) " + " ( " +    
  ...
  BEGIN        
   ...
   BEGIN      
    ...
    SELECT @n_err = 73403    
    SELECT @c_errmsg = CONVERT(char(250),@n_err)    
     + ":  fetch failed. (nspBuildAlterTableString) " + " ( " +    
     " SQLSvr MESSAGE = " + LTRIM(RTRIM(@c_errmsg)) + ")"    
   END       
    ...
    BEGIN     
     ...
     BEGIN     
      ...
      BEGIN    
         ...
         END   
         SELECT @c_inclen = ' (' + @c_Stringlength  + ') DEFAULT ' + "(''" + "'')"  -- KHLim01    
      END    
      ...
      " EXEC NSPALTERTABLE   '" +  RTRIM(@c_field) + "'," +    
      convert(char(10),@n_length) + ",'" + RTRIM(@c_typename) +    
      ...
      BEGIN      
       ...
       SELECT @n_err = 73404    
       SELECT @c_errmsg = CONVERT(char(250),@n_err)    
        + ":  Alter Table Failed. (nspBuildAlterTableString) " + " ( " +    
        " SQLSvr MESSAGE = " + LTRIM(RTRIM(@c_errmsg)) + ")"    
       BREAK    
      END       
     END     
 ...
 BEGIN    
  ...
  BEGIN    
   ...
   END    
  END    
  EXECUTE nsp_logerror @n_err, @c_errmsg, "nspBuildAlterTableString"    
