...
/************************************************************************/
/* Trigger: nspStockMovementSku_NEW 												*/
/* Creation Date:                                                       */
/* Copyright: IDS                                                       */
/* Written by:                                                          */
/*                                                                      */
/* Purpose:  Stock Movement                                             */
/*                                                                      */
/* Input Parameters:  @StorerKeyMin,   - From StorerKey                 */
/*							 @StorerKeyMax,   - To StorerKey                   */
/*							 @SkuMin,         - From Sku                       */
/*                    @SkuMax,         - To Sku                         */
/*                    @DateMin,        - From Date                      */
/*                    @DateMax         - To Date                        */
...
/* 18-Oct-2004	 Mohit	   Changed cursor type   	                     */
/* 08-Apr-2005  MaryVong   Added Drop object and Grant Execution        */
/*                                                                      */
/************************************************************************/

CREATE PROC [dbo].[nspStockMovementSku_NEW] (
 ...
 SELECT @BFdate = DATEADD(day, -1, convert(datetime,convert(char(10),@datemin,103)))
 SELECT @d_end_date =  DATEADD(day, 1, convert(datetime,convert(char(10),@datemax,103)))
 ...
 SELECT
 		...
 		, ItrnKey
 		, SourceKey
 		, AddDate = convert(datetime,convert(char(10),AddDate,103))
 ...
 SELECT
 		...
 		, SourceKey = "Archived"
 		, ItrnKey = "Archived"
 		, convert(datetime,convert(char(10),ArchiveDate,103))
 FROM LOT A (NOLOCK)
 WHERE EXISTS
 ...
 FROM #ITRN_CUT_BY_SKU
 WHERE
 (AddDate > @d_begin_date and AddDate <= @d_end_date)
 GROUP BY StorerKey, Sku
 SELECT @n_num_recs = @@rowcount
 SELECT @n_err = @@ERROR
 IF NOT @n_err = 0
 ...
 BEGIN
 	...
 	ELSE IF @trantype = 'DP'
 	BEGIN
 		IF (SELECT sourcetype FROM ITRN (NOLOCK) WHERE itrnkey = @itrnkey) LIKE '%RECEIPT%'
 		BEGIN
 			UPDATE #BF2
