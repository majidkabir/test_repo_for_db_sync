...
/************************************************************************/          
/* Stored Procedure: ispPRNIK11                                         */          
/* Creation Date: 24-Feb-2017                                           */          
/* Copyright: LFL                                                       */          
/* Written by:                                                          */          
/*                                                                      */          
/* Purpose: WMS-5342 - CN-Nike SDC WMS Allocation Strategy              */      
/*          Full Pallet from Pallet Location conso by load              */      
/*          Copy from ispPRNIK03                                        */      
/*                                                                      */          
/* Called By:                                                           */          
/*                                                                      */          
/* PVCS Version: 1.1                                                    */          
...
BEGIN          
   ...
   BEGIN         
      ...
      --              + ': Invalid Wave Piece Pick Task Dispatch Method. Must Be INLINE/ECOM (ispPRNIK11)'      
      GOTO QUIT_SP      
   END                           
  ...
  TRUNCATE TABLE #IDxLOCxChannel       --CS01a      
   ...
   --CS01a END      
    ...
    --BEGIN      
      ...
      + CHAR(13) + CASE WHEN ISNULL(RTRIM(@c_Lottable01),'') = '' THEN ' AND LA.Lottable01 = @c_LocationTypeOverride ' ELSE ' AND LA.Lottable01 = @c_Lottable01 ' END   --(Wan02)      
      ...
      --ELSE      
      --BEGIN      
      --SET @c_SQL = N'INSERT INTO #IDxLOCxChannel (Loc, ID, LogicalLocation, QtyAvailable,channel_id, C_IDQtyAvailable) '         --CS01a      
      --   + CHAR(13) +  'SELECT LOC.Loc, LOTxLOCxID.ID, LOC.LogicalLocation '      
      --   + CHAR(13) +  ',SUM(LOTXLOCXID.Qty - LOTXLOCXID.QtyAllocated - LOTXLOCXID.QtyPicked - LOTXLOCXID.QtyReplen) AS QtyAvailable '      
      --   + CHAR(13) +  ',MIN(c.channel_id) as channel_id '      
      --+ CHAR(13) + ',(c.qty - C.QtyAllocated- C.QtyOnHold) '      
      --   + CHAR(13) +  'FROM LOTxLOCxID WITH (NOLOCK) '           
      --   + CHAR(13) +  'JOIN LOC WITH (NOLOCK) ON (LOTxLOCxID.Loc = LOC.LOC AND LOC.Status <> ''HOLD'') '           
      --   + CHAR(13) +  'JOIN ID  WITH (NOLOCK) ON (LOTxLOCxID.Id = ID.ID AND ID.STATUS <> ''HOLD'') '            
      --   + CHAR(13) +  'JOIN LOT WITH (NOLOCK) ON (LOTXLOCXID.LOT = LOT.LOT AND LOT.STATUS <> ''HOLD'') '               
      --   + CHAR(13) +  'JOIN LOTATTRIBUTE LA WITH (NOLOCK) ON (LOT.LOT = LA.LOT) '             
      --   + CHAR(13) +  'CROSS APPLY dbo.fnc_SelectChannelInv (loc.facility, LOTxLOCxID.storerkey, LOTxLOCxID.sku, @c_channel, LOTxLOCxID.Lot) c '           
      --   --+ CHAR(13) +  'WHERE LOC.LocationFlag <> ''HOLD'' '         --CS01      
      --   --+ CHAR(13) +  'AND LOC.LocationFlag <> ''DAMAGE'' '         --CS01      
      --   + CHAR(13) +  ' WHERE LOC.LocationFlag = ''NONE'' '           --CS01      
      --   + CHAR(13) +  'AND LOC.Facility = @c_Facility '         
      ...
      --   + CHAR(13) +      'AND UCC.ID = LOTxLOCxID.ID AND UCC.Status > ''2'' AND  UCC.Status < ''9'' AND UCC.storerkey = @c_StorerKey) '--= 0 ' '                                 
      --   --+ CHAR(13) + 'AND (SELECT COUNT(DISTINCT SKU) FROM LotxLocxID ida1 (NOLOCK) WHERE ida1.ID = LOTxLOCxID.ID) = 1 '         
      --   --+ CHAR(13) + 'AND (SELECT SUM(ida2.QtyAllocated + ida2.QtyPicked + ida2.QtyReplen) FROM LOTxLOCxID ida2 (NOLOCK) '        
      --   --+ CHAR(13) +      'WHERE ida2.ID = LOTxLOCxID.ID) = 0 '        
      --   --+ CHAR(13) + 'AND C.Channel_ID > 0      '       
      --   + CHAR(13) + 'GROUP BY LOC.Loc, LOTxLOCxID.ID, LOC.LogicalLocation,c.qty , C.QtyAllocated, C.QtyOnHold '      
      --+ CHAR(13) + ' HAVING COUNT(DISTINCT C.Channel_ID) = 1 '      
      --   + CHAR(13) + ' AND    MIN(C.Channel_ID) > 0'      
      --   + CHAR(13) + ' AND SUM(LOTxLOCxID.QTY - LOTXLOCXID.QtyAllocated - LOTXLOCXID.QtyPicked - LOTXLOCXID.QtyReplen) <= MIN(c.qty- c.qtyallocated- c.QtyOnHold)'      
      --   + CHAR(13) + ' AND MIN(c.qty- c.qtyallocated- c.QtyOnHold) > 0'      
      --   + CHAR(13) + ' AND COUNT(DISTINCT LOTxLOCxID.SKU)  = 1'      
      --   + CHAR(13) + ' AND SUM(LOTxLOCxID.QtyAllocated + LOTxLOCxID.QtyPicked + LOTxLOCxID.QtyReplen) = 0'      
      --   + CHAR(13) + ' AND SUM(LOTxLOCxID.Qty) > 0'       
      --   + CHAR(13) + 'ORDER BY LOC.LogicalLocation, LOC.Loc, LOTxLOCxID.ID'        
      ...
      --END                                   
         ...
         EXEC sp_ExecuteSQL @c_SQL, @c_SQLParm, @c_Facility, @c_StorerKey, @c_SKU      
          ...
          --   BEGIN      
          --      INSERT INTO #IDxLOC (Loc, ID, LogicalLocation, QtyAvailable, channel_id)      
          --      SELECT ASUM.Loc, ASUM.ID, ASUM.LogicalLocation, ASUM.QtyAvailable, ASUM.channel_id      
          --      FROM (      
          --            SELECT Loc, ID, LogicalLocation, QtyAvailable, channel_id, C_IDQtyAvailable      
          --                  ,AccSumQtyAvail = SUM(QtyAvailable) OVER (PARTITION BY Channel_ID  ORDER BY Channel_ID, SeqNo)      
          --            FROM #IDxLOCxChannel      
          --           ) ASUM      
          --      WHERE ASUM.AccSumQtyAvail <= ASUM.C_IDQtyAvailable      
          --   END      
          /*CS01a END*/      
            ...
            BEGIN       
             ...
             END --CS01a END      
                  ...
                  BEGIN      
                        ...
                        BEGIN      
                         SELECT @c_SQL      
                         SELECT @c_Orderkey '@c_Orderkey', @n_OrderLineQty 'ordqty',@c_channel '@c_channel'      
                        END      
                       --CS01a      
                       IF  @c_ChannelInvMgmt = '1'      
                       BEGIN      
                         ...
                         BEGIN      
                            ...
                            FROM #CHANNELINFO                                  WHERE Orderkey = @c_Orderkey      
                            AND Channel = @c_channel      
                     ...
                     BEGIN      
                        ...
                        SET @n_Err = 61010      
                        SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0))        
                                       + ': Get PickDetailKey Failed. (ispPRNIK11)'      
                        GOTO QUIT_SP      
                     END      
                     ELSE      
                     BEGIN      
              ...
              'OrderKey: ' + @c_OrderKey + CHAR(13) +      
                        ...
                        BEGIN      
                           ...
                           SET @n_Err = 61020      
                           SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0))        
                                          + ': Insert PickDetail Failed. (ispPRNIK11)'      
                           GOTO QUIT_SP      
                        END      
...
QUIT_SP:      
   ...
   BEGIN        
      ...
      BEGIN        
         ...
         END        
      END        
      EXECUTE nsp_logerror @n_Err, @c_ErrMsg, 'ispPRNIK11'        
