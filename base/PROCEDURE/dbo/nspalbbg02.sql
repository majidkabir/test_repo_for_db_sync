...
BEGIN      
   ...
   SET @c_ExtraCondition = ' AND LOC.LocationFlag <> ''HOLD'' AND LOC.Status = ''OK'' ' + CHAR(13) +
                           ' AND ID.Status = ''OK'' AND LOT.Status = ''OK'' '

   SET @c_SortingCondition = ' ORDER BY CASE WHEN SL.LocationType IN (''CASE'', ''PICK'') THEN 1 ELSE 2 END,
