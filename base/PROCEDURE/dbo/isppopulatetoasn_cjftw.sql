...
/************************************************************************/
/* SP: ispPopulateTOASN_CJFTW                                           */
/* Creation Date: 03-Oct-2013                                           */
/* Copyright: LF                                                        */
/* Written by:               						                                */
/*                                                                      */
/* Purpose: Auto Create ASN for CJF-TW Orders SOS#314651                */
/*                                                                      */
...
CREATE PROCEDURE [dbo].[ispPopulateTOASN_CJFTW] 
   ...
   BEGIN  
     ...
     IF NOT EXISTS ( SELECT 1 FROM FACILITY WITH (NOLOCK) WHERE Facility = @c_ToFacility )
	    ...
	    SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Invalid To Facility: ' + RTRIM(ISNULL(@c_ToFacility,'')) +
                        ' (ispPopulateTOASN_CJFTW)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
		 END
     ...
     IF NOT EXISTS ( SELECT 1 FROM LOC WITH (NOLOCK) WHERE LOC = @c_ToLoc) 
	   ...
	   BEGIN 				
	      	 ...
	      	 BEGIN
	            ...
	            BEGIN
                 ...
                 BEGIN
	                   ...
	                   SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err)
				             SET @n_err = 63498   
	   		             SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert Error on Table Receipt (ispPopulateTOASN_CJFTW)' + ' ( ' + 
	            ...
	            BEGIN
	               ...
	               SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err) 
					       SET @n_err = 63499   
	   			       SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Generate Receipt Key Failed! (ispPopulateTOASN_CJFTW)' 
	   			                 + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
	            END
	         END
	      END    
	      ...
	      BEGIN
	         ...
	         SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err)
				   SET @n_err = 63500   
	   		   SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Storer Key is BLANK! (ispPopulateTOASN_CJFTW)' + ' ( ' + 
                          ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
	      END
	   ...
	   BEGIN         
	      ...
	      BEGIN
	         ...
	         SET ROWCOUNT 0 
	            ...
	            SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err)
				      SET @n_err = 63501   
	   		      SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Invalid Sku: ' + RTRIM(@c_Sku) + 'Of Storer: '+ RTRIM(@c_ToStorerkey) + ' (ispPopulateTOASN_CJFTW)' + ' ( ' + 
                         ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
				   END
		          ...
		          BEGIN
		               DECLARE PICK_CUR CURSOR FAST_FORWARD READ_ONLY FOR 
			                ...
			                GROUP BY PICKDETAIL.StorerKey, PICKDETAIL.SKU, LOTATTRIBUTE.Lottable02, 
			                         LOTATTRIBUTE.Lottable03, LOTATTRIBUTE.Lottable04, LOTATTRIBUTE.Lottable05, PICKDETAIL.ID
			       
			             OPEN PICK_CUR
			             
			             FETCH NEXT FROM PICK_CUR
			                INTO @n_QtyReceived, @c_Lottable02, @c_Lottable03, @d_Lottable04, @d_Lottable05, @c_ID 
			             ...
			             BEGIN
			                ...
			                INSERT INTO RECEIPTDETAIL (ReceiptKey,   			  ReceiptLineNumber,   ExternReceiptkey,    
			                                           ...
			                                           Lottable02,          Lottable03,   			 Lottable04,	  
				   		 										               Lottable05,					BeforeReceivedQty,   FinalizeFlag, ToID)			                                          
     ...
     BEGIN
        ...
        BEGIN
	          ...
	          SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err)
				    SET @n_err = 63492   
	   		    SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Error on Table Receipt (ispPopulateTOASN_CJFTW)' + ' ( ' + 
                       ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
        END              
     END*/
	   ...
	   BEGIN
	      ...
	      --BEGIN
	         --ROLLBACK TRAN
	      --END
	      EXECUTE nsp_logerror @n_err, @c_errmsg, 'ispPopulateTOASN_CJFTW'
