...
CREATE PROC [dbo].[nspImportilsSKU_N]
 AS
 ...
 -- delete sku that already exist in SKU table
    ...
    BEGIN
       INSERT INTO SKU ( StorerKey, SKU,  Descr, SUSR1, SUSR2, SUSR3, SUSR4, SUSR5, 
                ...
                StdNetwgt, StdCube, SKUGroup, Lottable01Label, Lottable02Label, 
                Lottable03label, Lottable04label, Lottable05Label, PickCode, Strategykey , PutCode, PutawayLoc, 
                PutawayZone, GrossWgt, NetWgt, Length, Width, Height, Weight, Shelflife, Facility, Notes1, 
                Notes2, Class, Itemclass, BUSR1, BUSR2, BUSR3, BUSR4, BUSR5,
                BUSR6,
                BUSR7,
                BUSR8,
                BUSR9,
       ...
       SELECT Storerkey, SKU, Descr, ISNULL(SUSR1, ' ' ), ISNULL (SUSR2, ' ' ), ISNULL(SUSR3,' '), ISNULL (SUSR4, ' '), ISNULL(SUSR5,' '),
              ...
              ISNULL(StdNetwgt,0), ISNULL(StdCube,0), ISNULL(SKUGROUP, 'STD'), ISNULL(Lottable01Label,' '), ISNULL(Lottable02Label,' '),
              ISNULL(Lottable03Label,' '), ISNULL(Lottable04Label,' '), ISNULL(Lottable05Label,' '), Pickcode, 'TWNSTD', PutCode, 'UNKNOWN',
              ...
              NOTES1, NOTES2, Class, itemclass, ISNULL(BUSR1, ' ' ), ISNULL (BUSR2, ' ' ), ISNULL(BUSR3,' '), ISNULL (BUSR4, ' '), ISNULL(BUSR5,' '), 
              ISNULL(BUSR6, ' ' ),
              ISNULL(BUSR7, ' ' ),
              ISNULL(BUSR8, ' ' ),
              ISNULL(BUSR9, ' ' ),
				  -- Modified by YokeBeen on 23-Sept-2004 - Values needed for Trigantic - Start
				  CONVERT(NCHAR(6), dbo.fnc_LTrim(dbo.fnc_RTrim(SUBSTRING(SKU,1,6))) ) + '-' + CONVERT(NCHAR(3), dbo.fnc_LTrim(dbo.fnc_RTrim(SUBSTRING(SKU,7,3))) )
					+ '-' + CONVERT(NCHAR(2), dbo.fnc_LTrim(dbo.fnc_RTrim('00')) ) + '-' + CONVERT(NCHAR(2), dbo.fnc_LTrim(dbo.fnc_RTrim('01')) ) 
					+ '-' + CONVERT(NCHAR(2), dbo.fnc_LTrim(dbo.fnc_RTrim('01')) ) + '-' + CONVERT(NCHAR(5), dbo.fnc_LTrim(dbo.fnc_RTrim(SUBSTRING(SKU,10,5))) ),
--               ISNULL(BUSR10, ' ' ),
				  -- Modified by YokeBeen on 23-Sept-2004 - Values needed for Trigantic - End
              ISNULL(ReceiptLoc, ' ' ),
              ISNULL(ReturnLoc, ' ' ),
              ISNULL(XDockReceiptLoc, ' ' )
       FROM WMSSKU
       SELECT @n_err = @@ERROR
       IF @n_err <> 0
       BEGIN
          ...
          SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Insert Failed On SKU (nspImportSKU)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
       END
       ...
       BEGIN
          ...
          BEGIN
             ...
             SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Update Failed On WMSsku (nspImportSKU)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
          END
       END 
    END
    IF @n_continue = 1 OR @n_continue = 2
    BEGIN
         INSERT INTO HIERROR (HiErrorGroup, ErrorText, ErrorType , sourcekey)
         VALUES ( @c_hikey, ' -> nspImportSKU . Process completed for ' + @c_hikey + '. Process ended at ' + convert (char(20), getdate()), 'GENERAL', ' ')
         SELECT @n_err = @@ERROR
         IF @n_err <> 0
         BEGIN
            SELECT @n_continue = 3
