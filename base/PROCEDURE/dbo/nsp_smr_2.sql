...
select itemclass, descr, skugroup, count(*)
from sku(nolock)
where storerkey = 'toshiba'
group by itemclass, descr, skugroup
order by count(*)
---------------------------------------------------------------------

list out a list of itemclass, descr, skugroup for toshiba

select itemclass, descr, skugroup
from sku
where skugroup = 'A'
and storerkey = 'toshiba'
order by itemclass, descr, skugroup
---------------------------------------------------------------------
...
BEGIN
   ...
   BEGIN
      ...
      */
      ...
      BEGIN
         ...
         SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Insert Failed On #ITRN_CUT_BY_SKU (BSM) (nsp_smr_2)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
   ...
   GROUP BY storerkey, itemclass, code_descr, skugroup  /**/
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Summary Insert Failed On #BF_TEMP (nsp_smr_2)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
            /* End Trap SQL Server Error */
         END
      END /* if @n_num_recs > 0 */
   END /* continue and stuff */
   ...
   */
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            /* Trap SQL Server Error */
            SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err=74802   -- Should Be Set To The SQL Errmessage but I don't know how to do so.
            SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Summary Insert Failed On #BF_TEMP3a (nsp_smr_2)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
            /* End Trap SQL Server Error */
         END
   ...
   BEGIN
      ...
      , RunningTotal
      FROM #BF
      ORDER BY StorerKey, itemclass, code_descr, skugroup
   END
   ...
   , TranType
   FROM #BF
   ORDER BY StorerKey, itemclass, code_descr, skugroup, EffectiveDate')
   ...
   WHERE a.storerkey = b.storerkey
   group by b.company, a.storerkey, a.itemclass, a.code_descr, a.skugroup
   ...
   from #bb2
   order by company, storerkey, itemclass, code_descr, skugroup
