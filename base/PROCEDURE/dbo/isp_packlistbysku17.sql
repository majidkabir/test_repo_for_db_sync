...
GO
/************************************************************************/  
/* Stored Proc: isp_PackListBySku17                                     */  
/* Creation Date: 29-Sep-2020                                           */  
/* Copyright: LF Logistics                                              */  
/* Written by: WLChooi                                                  */  
/*                                                                      */  
/* Purpose:WMS-15206 - [TW-VFEC] EC PickSlip RCMReport_CR               */  
/*        :Copy from isp_GetPickSlipOrders88                            */  
/*        :                                                             */  
/* Called By: r_dw_packing_list_by_sku17                                */  
/*          :                                                           */  
/* GitLab Version: 1.0                                                  */  
/*                                                                      */  
/* Version: 7.0                                                         */  
...
CREATE PROC [dbo].[isp_PackListBySku17]  
...
BEGIN  
  ...
  , RNotes  
   ...
   LEFT JOIN PICKHEADER WITH (NOLOCK) ON (LOADPLANDETAIL.Orderkey = PICKHEADER.Orderkey)  
...
--select * from #TMP_PCK88 order by loc  
  ...
  /*Sorting by LOC first then group by orderkey*/  
  ...
  END                 
   ...
   BEGIN                   
      ...
      SELECT TOP 1  
         ...
         , ''  
              ...
              , @n_LastPage  
              , ''  
      FROM #TMP_PCK88_Final    
   WHERE #TMP_PCK88_Final.PickSlipNo= @c_PSNo  
      AND #TMP_PCK88_Final.Pageno = @n_LastPage  
   ...
   SELECT * FROM #TMP_PCK88_Final AS tp  
   ORDER BY Pickslipno, CASE WHEN sku = '' THEN 2 ELSE 1 END,tp.Pageno  
  
  
   IF OBJECT_ID('tempdb..#TMP_PCK') IS NOT NULL  
      DROP TABLE #TMP_PCK  
  
   IF OBJECT_ID('tempdb..#TMP_PCK88') IS NOT NULL  
      DROP TABLE #TMP_PCK88  
  
   IF OBJECT_ID('tempdb..#TMP_PCK88_Final') IS NOT NULL  
      DROP TABLE #TMP_PCK88_Final  
  
   IF OBJECT_ID('tempdb..#UniquePSNO') IS NOT NULL  
      DROP TABLE #UniquePSNO  
