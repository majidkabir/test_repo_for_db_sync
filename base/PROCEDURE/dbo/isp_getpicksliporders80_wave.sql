...
/************************************************************************/
/* Store Procedure:  isp_GetPickSlipOrders80_wave                       */
...
/* Usage:                                                               */
/*                                                                      */
/* Called By: r_dw_print_pickorder80_wave                               */
/*                                                                      */
/* PVCS Version: 1.1                                                    */
/*                                                                      */
/* Version: 5.4                                                         */
...
CREATE   PROC [dbo].[isp_GetPickSlipOrders80_wave]
...
BEGIN
   ...
   --WHERE PH.loadkey=@c_waveKey  

   --SELECT LPD.LOADKEY FROM WAVEDETAIL WD WITH (NOLOCK)   
   --JOIN LOADPLANDETAIL LPD WITH (NOLOCK) ON (LPD.ORDERKEY = WD.ORDERKEY)  
   ...
   INSERT INTO #TMP_PickOrd80 ( --wavekey,  
      orderkey, IntVehicle, OHType, ExtOrderkey, loadkey, Pickslipno, DeliveryDate, Userdefine01, consigneekey, [State]
    , City, Zip, Address1, Address2, Address3, Address4, Company, ExternPOKey, Userdefine05, OHSTOP, OrderLineNumber
    , sku, SColor, SKU_SIze, Altsku, PDQty, VAS)
   SELECT --WD.WAVEKEY,   
      PD.OrderKey AS Orderkey
    , O.IntermodalVehicle AS IntermodalVehicle
    , O.[Type] AS [type]
    , O.ExternOrderKey AS externorderkey
    , O.LoadKey AS loadkey
    , ISNULL(PH.PickHeaderKey, '') AS Pickslipno
    , CASE WHEN ISNULL(O.UserDefine10, '') = '' THEN CONVERT(NVARCHAR(10), O.DeliveryDate, 121)
           ELSE O.UserDefine10 END AS DeliveryDate
    , O.UserDefine01
    , O.ConsigneeKey AS Consigneekey
    , ISNULL(O.C_State, '') AS C_STATE
    , ISNULL(O.C_City, '') AS C_CITY
    , ISNULL(O.C_Zip, '') AS C_ZIP
    , ISNULL(O.C_Address1, '') AS C_Address1
    , ISNULL(O.C_Address2, '') AS C_Address2
    , ISNULL(O.C_Address3, '') AS C_Address3
    , ISNULL(O.C_Address4, '') AS C_Address4
    , ISNULL(O.C_Company, '') AS C_Company
    , O.ExternPOKey AS ExternPOKey
    , O.UserDefine05 AS UserDefine05
    , O.[Stop] AS [STOP]
    , PD.OrderLineNumber AS OrderlineNumber
    , PD.Sku AS SKU
    , '' AS color
    , '' AS [SIZE]
    , s.ALTSKU AS ALTSKU
    , SUM(PD.Qty) AS Qty
    , ISNULL(ODF.Note1, N'') AS VAS
   FROM PICKDETAIL AS PD WITH (NOLOCK)
   INNER JOIN ORDERS AS O WITH (NOLOCK) ON PD.OrderKey = O.OrderKey
   LEFT JOIN PICKHEADER AS PH WITH (NOLOCK) ON O.LoadKey = PH.ExternOrderKey
   INNER JOIN SKU AS s WITH (NOLOCK) ON s.Sku = PD.Sku AND s.StorerKey = PD.Storerkey
   INNER JOIN LOC AS L WITH (NOLOCK) ON PD.Loc = L.Loc
   ...
   LEFT OUTER JOIN (  SELECT DISTINCT ODF2.StorerKey
                                    , ODF2.ParentSKU
                                    , ODF2.Orderkey
                                    , ODF2.OrderLineNumber
                                    , SUBSTRING(
                                      (  SELECT ', ' + ODF1.Note1
                                         FROM OrderDetailRef ODF1 (NOLOCK)
                                         WHERE ODF1.StorerKey = ODF2.StorerKey
                                         AND   ODF1.ParentSKU = ODF2.ParentSKU
                                         AND   ODF1.Orderkey = ODF2.Orderkey
                                         AND   ODF1.OrderLineNumber = ODF2.OrderLineNumber
                                         ORDER BY ODF1.RowRef
                                         FOR XML PATH(''))
                                    , 2
                                    , 1000) AS Note1
                      FROM OrderDetailRef ODF2 (NOLOCK)) AS ODF ON  PD.Storerkey = ODF.StorerKey
                                                                AND PD.OrderKey = ODF.Orderkey
                                                                AND PD.OrderLineNumber = ODF.OrderLineNumber
                                                                AND PD.Sku = ODF.ParentSKU
   WHERE WD.WaveKey = @c_waveKey AND L.PickZone = 'BZ'
   GROUP BY --WD.WAVEKEY,   
      PD.OrderKey
    , O.IntermodalVehicle
    , O.Type
   ...
   SELECT DISTINCT Pickslipno    
         ...
         ,sku    
         ,OrderLineNumber    
   FROM #TMP_PickOrd80 WITH (NOLOCK)    
