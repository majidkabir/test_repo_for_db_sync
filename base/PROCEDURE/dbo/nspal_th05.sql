...
GO
/************************************************************************/
/* Stored Procedure: nspAL_TH05                                         */
/* Creation Date: 04-MAY-2021                                           */
/* Copyright: LFL                                                       */
/* Written by:                                                          */
/*                                                                      */
/* Purpose: WMS-16972 - TH Triumph Allocation                           */
/*          UOM6 - allocate all lot of the loc with min lottable05      */
/*                                                                      */
/* Called By:                                                           */
/*                                                                      */
/* PVCS Version: 1.0                                                    */
...
BEGIN    
   ...
   SET @c_SQL = N'      
      ...
      OUTER APPLY (SELECT MIN(LA.Lottable05) AS Lottable05
                   FROM LOTXLOCXID LLI (NOLOCK)
                   JOIN LOTATTRIBUTE LA (NOLOCK) ON LLI.Lot = LA.Lot
                   WHERE LLI.Storerkey = LOTxLOCxID.Storerkey
                   AND LLI.Sku = LOTxLOCxID.Sku
                   AND LLI.Loc = LOTxLOCxID.Loc
                   AND LLI.Qty > 0) AS FIFO
      ...
      'ORDER BY FIFO.Lottable05, LOC.LogicalLocation, LOC.Loc'
