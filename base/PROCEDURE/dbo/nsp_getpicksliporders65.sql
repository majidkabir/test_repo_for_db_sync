...
GO
/************************************************************************/
/* Stored Procedure: nsp_GetPickSlipOrders65                            */
/* Creation Date: 02.May.2008                                           */
/* Copyright: IDS                                                       */
/* Written by: June                                                     */
/*                                                                      */
/* Remarks: WMS-337 -GCI_New Consolidated Picklist format               */
/*                                                                      */
/*                                                                      */
/* Called By: r_dw_print_pickorder65 copy from r_dw_print_pickorder26   */
/*                                                                      */
/* PVCS Version: 1.9                                                    */
/*                                                                      */
/* Version: 5.4                                                         */
...
/* 21-Sep-2016  CSCHONG  2.4  WMS-337 (CS02)                            */
/************************************************************************/

CREATE PROC [dbo].[nsp_GetPickSlipOrders65] (@c_loadkey NVARCHAR(10))
 AS
BEGIN
 ...
 BEGIN
   ...
   SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Failed On Table Pickheader Table. (nsp_GetPickSlipOrders64)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '
 END
 ...
 BEGIN
  DECLARE pickslip_cur CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
   ...
   LEFT OUTER JOIN Codelkup CLR (NOLOCK) ON (ORDERS.Storerkey = CLR.Storerkey AND CLR.Code = 'SHOWORDERGRP'                                      --CS01
                                 AND CLR.Listname = 'REPORTCFG' AND CLR.Long = 'r_dw_print_pickorder64' AND ISNULL(CLR.Short,'') <> 'N')         --CS01
    ...
    (@c_pickheaderkey, '',      @c_LoadKey,     '0',      'LB',  '', @c_WaveKey, @c_Consigneekey )   -- @c_PickZone)
      ...
      BEGIN
       ...
       SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err=73001
        SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert Failed On Table PICKHEADER. (nsp_GetPickSlipOrders64)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '
      END
   END
   ...
   BEGIN
    ...
    BEGIN
     DECLARE PickDet_cur CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
      ...
      --AND    (LOC.LocationType = 'CASE' OR LOC.LocationType = 'PICK' OR LOC.LocationType = 'PALLET' OR ISNULL(LOC.PickZone,'')='')  --(NJOW02)  --(CS02)
     ...
     BEGIN
      ...
      VALUES (@c_PickDetailkey, @c_pickheaderkey, @c_OrderKey, @c_OrderLineNumber, @c_loadkey)
        ...
        BEGIN
          SELECT @n_continue = 3
         SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err=73001
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert Failed On Table RefkeyLookup. (nsp_GetPickSlipOrders64)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '
        END
      ...
      BEGIN
       ...
       WHERE  PickDetailkey = @c_PickDetailkey
         ...
         BEGIN
           SELECT @n_continue = 3
          SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err=73001
           SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Failed On Table PICKDETAIL. (nsp_GetPickSlipOrders64)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '
         END
         
      IF @b_debug=1
		BEGIN
			SELECT @c_Pickdetailkey AS '@c_Pickdetailkey',@c_pickheaderkey AS '@c_pickheaderkey'
		END
