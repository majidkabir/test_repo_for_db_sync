...
CREATE PROC [dbo].[isp_TPB_Receipt_Skechers_Archive]
...
BEGIN
   ...
   SET @c_SQLStatement = N'SELECT R.ReceiptKey, RD.ReceiptLineNumber, RD.ExternReceiptKey, RD.ExternLineNo ' +
                         ' FROM ' + @c_ArchiveDB + '.dbo.V_Receipt R (NOLOCK) ' +
                         ' JOIN ' + @c_ArchiveDB + '.dbo.V_Receiptdetail RD (NOLOCK) ON RD.receiptkey = R.ReceiptKey ' +
   ...
   SET @c_SQLParm =  ' @c_Storerkey Nvarchar(15) , @d_Fromdate datetime '

 
   INSERT INTO #ExternReceipt (Receiptkey, ReceiptLineNumber, ExternReceiptKey, ExternLineNo)
   EXEC sp_ExecuteSQL @c_SQLStatement, @c_SQLParm, @c_storerkey, @d_Fromdate
   ...
   SET @c_SQLStatement = N'SELECT ReceiptKey, ReceiptDate, Signatory, UserDefine09, ' +
                           '    DateReceived = ( SELECT MIN(DC.AddDate) FROM RDT.rdtDataCapture DC (NOLOCK) ' +
                           '                     Where  receipt.VehicleNumber=DC.SerialNo ' +
                           '                     AND DC.storerkey = receipt.storerkey ) ' +
                           ' FROM ' + @c_ArchiveDB + '.dbo.V_receipt receipt WITH (NOLOCK) ' +
                           ' WHERE receipt.doctype=''R'' AND receipt.ReceiptGroup=''ECOM'' ' +
                           ' AND receipt.storerkey = @c_storerkey AND receipt.ASNStatus = ''9'' ' +
                           ' AND CONVERT(VARCHAR, receipt.Editdate, 112) >= CONVERT(VARCHAR, @d_Fromdate, 112) '
   ...
   SET @c_SQLStatement = @c_SQLStatement +
           ...
           'LEFT JOIN Storer PS WITH (NOLOCK) ON PS.storerkey = ''SK''+PO.SellerName ' +
