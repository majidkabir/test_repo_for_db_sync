...
GO
/************************************************************************/
/* Stored Procedure: nspConsolidatedPickList05                          */
/* Creation Date:                                                       */
/* Copyright: IDS                                                       */
/* Written by:                                                          */
/*                                                                      */
...
CREATE PROC [dbo].[nspConsolidatedPickList05] (
...
ELSE
   ...
   ELSE
      ...
      ELSE
         ...
         LabelFlag='Y',
         LoadPlan.TrfRoom TrfRoom--CCLAW20001110
         INTO #CONSOLIDATED
         ...
         DECLARE @c_TrfRoom NVARCHAR(5) --CCLAW20001110
         SELECT  LOC=space(10),
         SKU.SKU SKU,
         ...
         @c_PreLabelPrc NVARCHAR(5)
         DECLARE CUR_1 SCROLL CURSOR FOR
         SELECT 	DISTINCT ORDERDETAIL.OrderKey,
         ORDERDETAIL.SKU,
         PICKDETAIL.LOC,
         Label = CASE WHEN ORDERS.LabelPrice = 'Y' THEN 'Y'
      ELSE 'N'
      END
      FROM LoadplanDetail (NOLOCK), ORDERDETAIL (NOLOCK), PICKDETAIL (NOLOCK), LOC (nolock), ORDERS (nolock)
      ...
      ORDER BY Label, PICKDETAIL.LOC, ORDERDETAIL.SKU, ORDERDETAIL.OrderKey
      OPEN CUR_1
      SELECT @n_GroupNo = 1
      SELECT @n_GroupSeq = 0
      SELECT @c_PreLabelPrc=SPACE(5)
      FETCH NEXT FROM CUR_1 INTO @c_OrderKey, @c_SKU, @c_Loc, @c_LabelPrice
      WHILE @@FETCH_STATUS <> -1
      BEGIN
         IF @c_PreLabelPrc=SPACE(5)
         SELECT @c_PreLabelPrc=@c_LabelPrice
      ...
      --if @a_s_LoadKey = '0000000248'
      --   select * from #SKUGroup order by GroupNo, GroupSeq
      FETCH FIRST FROM CUR_1 INTO @c_OrderKey, @c_SKU, @c_Loc, @c_LabelPrice
      WHILE @@FETCH_STATUS <> -1
      BEGIN
      ...
      SELECT DISTINCT GroupNo, LOC, SKU
      FROM   #SKUGroup
      ORDER BY GroupNo, LOC, SKU
      OPEN CUR_2
      FETCH NEXT FROM CUR_2 INTO @n_GroupNo, @c_LOC, @c_SKU
      WHILE @@FETCH_STATUS <> -1
      BEGIN
      ...
      BEGIN TRAN
         ...
         Trafficcop = NULL
         FROM   PickDetail ,  LoadPlanDetail (NOLOCK), PickHeader (NOLOCK)
      ...
      ELSE
         ...
         BEGIN
            SELECT @n_continue = 3
            SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err = 62241   -- Should Be Set To The SQL Errmessage but I don't know how to do so.
            SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Update Failed On PickDetail Table. (nspConsolidatedPickList05)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
         END
      ELSE
         BEGIN
            SELECT * FROM #CONSOLIDATED ORDER BY ConsoGroupNo, LOC, SKU
         END
         IF @n_continue=3  -- Error Occured - Process And Return
         BEGIN
            SELECT @b_success = 0
            execute nsp_logerror @n_err, @c_errmsg, "nspConsolidatedPickList05"
