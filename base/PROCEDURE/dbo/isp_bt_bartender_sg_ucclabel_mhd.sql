...
/********************************************************************************/
/* Copyright: MAERSK                                                            */
/* Purpose: isp_BT_Bartender_SG_UCCLABEL_MHD                                    */
/*                                                                              */
/* Modifications log:                                                           */
/*                                                                              */
/* Date        Rev  Author    Purposes                                          */
/* 10-May-2023 1.0  WLChooi   Created (WMS-22503)                               */
/* 10-May-2023 1.0  WLChooi   DevOps Combine Script                             */
/********************************************************************************/

CREATE   PROC [dbo].[isp_BT_Bartender_SG_UCCLABEL_MHD]
(
...
BEGIN
   ...
   SET @c_SQLJOIN = N' SELECT DISTINCT ' + CHAR(13)
                  + N'        OH.ExternOrderkey, TRIM(ISNULL(OH.C_Company,'''')), TRIM(ISNULL(OH.C_Address1,'''')) '
                  + N'      , TRIM(ISNULL(OH.C_Address2,'''')), TRIM(ISNULL(OH.C_Address3,'''')),' + CHAR(13) --5
                  + N' TRIM(ISNULL(OH.C_Address4,'''')), ' + CHAR(13) --6
                  + N' TRIM(ISNULL(OH.C_Zip,'''')) + '' '' + TRIM(ISNULL(OH.C_City,'''')) + '' '' + TRIM(ISNULL(OH.C_Country,'''')), ' + CHAR(13) --7
                  + N' PD.CartonNo, '''', '''', ' + CHAR(13) --10   
                  + N' PD.Pickslipno, '''', '''', '''', '''', '''', '''', '''', '''', '''', ' + CHAR(13) --20          
                  + N' '''', '''', '''', '''', '''', '''', '''', '''', '''', '''', ' + CHAR(13) --30     
                  + N' '''', '''', '''', '''', '''', '''', '''', '''', '''', '''', ' + CHAR(13) --40  
                  + N' '''', '''', '''', '''', '''', '''', '''', '''', '''', '''', ' + CHAR(13) --50        
                  + N' '''', '''', '''', '''', '''', '''', '''', '''', PD.LabelNo, ''SG'' ' + CHAR(13) --60
   ...
   UPDATE #Result
   SET Col09 = CASE WHEN @c_Confirmed = 'Y' THEN CAST(@n_MaxCtn AS NVARCHAR) ELSE 'XX' END
     , Col10 = @n_SumQty
   WHERE Col11 = @c_Sparm01
   AND Col59 = @c_Sparm02
