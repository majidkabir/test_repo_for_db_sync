...
/********************************************************************************/                   
/* Copyright: LFL                                                               */                   
/* Purpose: isp_BT_Bartender_CN_SHIPUCCLBL_SKE                                  */                   
/*                                                                              */                   
/* Modifications log:                                                           */                   
/*                                                                              */                   
/* Date       Rev  Author     Purposes                                          */                   
/* 2019-09-03 1.0  WLChooi    Created (WMS-10365)                               */  
/* 2020-01-02 1.1  WLChooi    Fix wrong column retrieved (WL01)                 */ 
/* 2020-01-02 1.2  WLChooi    Add Col31 and Bug Fix (WL02)                      */ 
/* 2020-01-14 1.3  WLChooi    Limit C_State and C_zip to only 4 chars (WL03)    */ 
/* 2020-03-31 1.4  WLChooi    Fix - LEFT(C_Address1 + C_Address2,80) (WL04)     */ 
/* 2021-03-04 1.5  WLChooi    WMS-16435 - Update Col04, Remove Col30 (WL05)     */
/********************************************************************************/                  
                    
CREATE PROC [dbo].[isp_BT_Bartender_CN_SHIPUCCLBL_SKE]                        
...
BEGIN                        
   ...
   DECLARE                    
      ...
      @c_AllExtOrderkey     NVARCHAR(80) = '',   --WL05 
      @c_Col26              NVARCHAR(80) = '',   --WL05   
      @c_Col02              NVARCHAR(80) = '',   --WL05   
      @c_Col03              NVARCHAR(80) = '',   --WL05   
      @c_Col05              NVARCHAR(80) = '',   --WL05   
      @c_Col06              NVARCHAR(80) = '',   --WL05                  
      @c_Storerkey          NVARCHAR(15) = ''    --WL05 
...
-- SET RowNo = 0               
   ...
   SELECT TOP 1 @c_GetOrderkey = ORDERS.Orderkey,
                @c_Col26       = ISNULL(ORDERS.Notes,''),   --WL05
                @c_Col02       = SUBSTRING(LTRIM(RTRIM(ISNULL(ORDERS.C_State,''))),1,4),   --WL05
                @c_Col03       = '(' + SUBSTRING(LTRIM(RTRIM(ISNULL(ORDERS.C_City,''))),1,4) + ')',   --WL05
                @c_Col05       = LTRIM(RTRIM(ISNULL(ORDERS.C_Company,''))),   --WL05
                @c_Col06       = LEFT(LTRIM(RTRIM(ISNULL(ORDERS.C_Address1,''))) + ' ' + LTRIM(RTRIM(ISNULL(ORDERS.C_Address2,''))),80),   --WL05
                @c_Storerkey   = ORDERS.Storerkey   --WL05
   ...
   CREATE TABLE [#Result] (               
      ...
      END   
      /*BEGIN  
         SET @c_SQLJOIN = + ' SELECT DISTINCT OH.Loadkey, SUBSTRING(LTRIM(RTRIM(ISNULL(OH.C_State,''''))),1,4), '  --WL03
                          + ' ''('' + SUBSTRING(LTRIM(RTRIM(ISNULL(OH.C_City,''''))),1,4) + '')'', ' + CHAR(13) --3  --WL01  --WL03
                          ...
                          + ' LEFT(LTRIM(RTRIM(ISNULL(OH.C_Address1,''''))) + '' '' + LTRIM(RTRIM(ISNULL(OH.C_Address2,''''))),80), ' + CHAR(13) --6  --WL04
                          + ' '''' ,'''' ,'''' ,'''' , ' + CHAR(13) --10   
                          + ' '''' ,'''' ,'''' ,'''' ,'''' ,'''' ,'''' ,'''' ,'''' ,'''' ,'  + CHAR(13) --20         
                          + ' '''' ,PD.LabelNo ,PD.CartonNo ,CONVERT(NVARCHAR(80),MAX(PD.EditDate),120), ' + CHAR(13) --24  
                          + ' '''' ,MAX(SUBSTRING(OH.Notes, 1, 80)) ,'''' ,'''' ,ISNULL(ST.B_Address4,''''), ' + CHAR(13) -- 29    --WL02
                          + ' CASE WHEN MAX(OH.ExternOrderKey) LIKE ''SA%'' THEN RIGHT(LTRIM(RTRIM(MAX(OH.ExternOrderKey))),4) ELSE '''' END,'  + CHAR(13) --30     
                          + ' MAX(OH.ExternOrderKey) ,'''' ,'''' ,'''' ,'''' ,'''' ,'''' ,'''' ,'''' ,'''' ,'  + CHAR(13) --40         --WL02
                          + ' '''' ,'''' ,'''' ,'''' ,'''' ,'''' ,'''' ,'''' ,'''' ,'''' , ' + CHAR(13) --50         
                          + ' '''' ,'''' ,'''' ,'''' ,'''' ,'''' ,'''' ,'''' ,PD.Pickslipno ,''CN'' '  --60            
                          + CHAR(13) +              
                          + ' FROM PACKHEADER PH WITH (NOLOCK)'        + CHAR(13)  
                          + ' JOIN PACKDETAIL PD WITH (NOLOCK) ON PH.Pickslipno = PD.Pickslipno'   + CHAR(13)  
                          +   @c_JoinStatement  
                          + ' LEFT JOIN STORER ST WITH (NOLOCK) ON ST.STORERKEY = OH.Consigneekey ' + CHAR(13)   --WL02
                          + ' WHERE PD.Pickslipno = @c_Sparm01 '   + CHAR(13)    
                          + ' AND PD.CartonNo BETWEEN CAST(@c_Sparm02 AS INT) AND CAST(@c_Sparm03 AS INT) ' + CHAR(13)  
                          + ' GROUP BY OH.Loadkey, SUBSTRING(LTRIM(RTRIM(ISNULL(OH.C_State,''''))),1,4), '   --WL03
                          + ' ''('' + SUBSTRING(LTRIM(RTRIM(ISNULL(OH.C_City,''''))),1,4) + '')'', ' + CHAR(13)  --WL01  --WL03
...
EXIT_SP:      
   ...
   --   @c_TraceName = 'isp_BT_Bartender_CN_SHIPUCCLBL_SKE',    
