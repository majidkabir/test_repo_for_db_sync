...
/************************************************************************/
/* Store procedure: isp_808PTLCart_Confirm01                            */
...
CREATE PROC [dbo].[isp_808PTLCart_Confirm01] (
   @cMessageNum   NVARCHAR(10),
   @cIPAddress    NVARCHAR(40),
   @cLightLOC     NVARCHAR(20),
   @cCondition    NVARCHAR(20),
   @cQTYReturn    NVARCHAR(5),
...
BEGIN
   ...
   SET @nCaseCnt = 0
   SET @nCase = 0
   SET @cQTYReturn = RTRIM( LTRIM( @cQTYReturn))
   ...
   SET @nPOS = CHARINDEX( '-', @cQTYReturn)
   IF @nPOS > 0
   BEGIN
      SET @nCase = LEFT( @cQTYReturn, @nPOS-1)
      SET @nPiece = SUBSTRING( @cQTYReturn, @nPOS+1, LEN( @cQTYReturn))
   END
   ELSE
      SET @nPiece = @cQTYReturn
   ...
   SAVE TRAN isp_808PTLCart_Confirm01 -- For rollback or commit only our own transaction
      ...
      BEGIN
         ...
         BEGIN -- Have balance, need to split
            ...
            INSERT INTO dbo.PTLTran (
               ExpectedQty, QTY, DropID, MessageNum, Status, LightUp, TrafficCop, 
               IPAddress, DeviceID, DevicePosition, PTL_Type, OrderKey, Storerkey, SKU, LOC, LOT, Remarks,
...
--      SELECT @cDPLKey '@cDPLKey', @cLightLOC '@cLightLOC', @cSKU '@cSKU', @cLOC '@cLOC', @nExpectedQTY '@nExpectedQTY'
   ...
   COMMIT TRAN isp_808PTLCart_Confirm01
   GOTO Quit

RollBackTran:
   ROLLBACK TRAN isp_808PTLCart_Confirm01 -- Only rollback change made here
Quit:
