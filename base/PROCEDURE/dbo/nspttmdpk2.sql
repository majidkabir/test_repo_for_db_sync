...
/************************************************************************/
/* Stored Procedure: nspTTMDPK2                                         */
/* Creation Date:                                                       */
/* Copyright: IDS                                                       */
/* Written by:                                                          */
/*                                                                      */
/* Purpose: Strategy for Republic CASE Picking for BULK                 */
/*          Only allow 1 SKU per tote                                   */
/*                                                                      */
/* Called By:                                                           */
/*                                                                      */
/* PVCS Version: 1.0                                                    */
...
BEGIN
   ...
   DECLARE @c_LastDropID                  NVARCHAR( 20), 
           @c_LastTaskDetailKey           NVARCHAR( 10), 
           @c_LastTaskSKU                 NVARCHAR( 20), 
           @c_UpdTaskDetailKey            NVARCHAR( 10)     -- (james01)
   ...
   BEGIN
      SET @c_LastLoadKey = ''
      SET @c_LastDropID = ''
      SET @c_LastTaskDetailKey = ''

      SELECT TOP 1 @c_LastLoadKey = TA.LoadKey, 

                   @c_LastTaskDetailKey = TA.TaskDetailKey 
      ...
      BEGIN
         ...
         SELECT 'LastTaskDetailKey' = @c_LastTaskDetailKey
      END
   END

   SET @c_LastTaskSKU = ''

   IF ISNULL( @c_LastTaskDetailKey, '') <> ''
   BEGIN
      SELECT TOP 1 @c_LastDropID = DropID 
      FROM dbo.PickDetail WITH (NOLOCK) 
      WHERE TaskDetailKey = @c_LastTaskDetailKey

      IF EXISTS ( SELECT 1 FROM dbo.DropID WITH (NOLOCK) 
                  WHERE DropID = @c_LastDropID
                  AND   [Status] = '0')   -- 0 = not finish from DPK; 5 = finish picking from DPK
      -- If tote is open then can only have same sku inside the tote.
      -- Get the last SKU picked inside this tote
      BEGIN
         SELECT TOP 1 @c_LastTaskSKU = SKU 
         FROM dbo.PickDetail WITH (NOLOCK) 
         WHERE TaskDetailKey = @c_LastTaskDetailKey
      END
      ...
      IF @b_debug=1
      BEGIN
         SELECT '@c_LastDropID', @c_LastDropID, '@c_LastTaskSKU', @c_LastTaskSKU
      END
   END
   ...
   BEGIN
      ...
      SELECT TaskDetailKey FROM dbo.TaskDetail WITH (NOLOCK)
      WHERE USERKEY = @c_UserID
      AND   [STATUS] = '3' 
      OPEN CUR_UPD
      FETCH NEXT FROM CUR_UPD INTO @c_UpdTaskDetailKey
      ...
      BEGIN
         ...
         BEGIN
            ...
            SELECT @c_errmsg = 'NSQL'+CONVERT(CHAR(5) ,@n_err)+
                   ': Update to TaskDetail table failed. (nspTTMDPK2)'+' ( '+
            ...
            BEGIN
               ...
               BEGIN
                  IF NOT EXISTS (
                     ...
                     WHERE  AreaDetail.AreaKey = CASE WHEN ISNULL( @c_AreaKey01, '') <> '' THEN @c_AreaKey01 
                                                 ELSE AreaDetail.AreaKey END
                            ...
                            AND EXISTS(SELECT 1 FROM TaskManagerUserDetail tmu WITH (NOLOCK)
                                       WHERE PermissionType = TaskDetail.TASKTYPE
                                         ...
                                         AND tmu.AreaKey = CASE WHEN ISNULL( @c_AreaKey01, '') <> '' THEN @c_AreaKey01 
                                                           ELSE tmu.AreaKey END
                                         AND tmu.Permission = '1') 
                           AND TaskDetail.SKU = @c_LastTaskSKU)
               ...
               END
                        ...
                        WHERE  AreaDetail.AreaKey = @c_AreaKey01
                               ...
                               AND EXISTS(SELECT 1 FROM TaskManagerUserDetail tmu WITH (NOLOCK)
                                          WHERE PermissionType = TaskDetail.TASKTYPE
                                            ...
                                            AND tmu.Permission = '1') 
                              AND TaskDetail.SKU = CASE WHEN ISNULL( @c_LastTaskSKU, '') <> '' THEN @c_LastTaskSKU 
                                                   ELSE TaskDetail.SKU END
                ...
                BEGIN
                    ...
                    SELECT @c_errmsg = 'NSQL'+CONVERT(CHAR(5) ,@n_err)+
                           ': Could not Open Cursor_Eval02. (nspTTMDPK2)'+' ( ' 
                          +' SQLSvr MESSAGE='+RTRIM(@c_errmsg) 
                          +' ) '
                END
                ELSE
...
--                     @c_UserID)  -- Col5
     ...
     BEGIN
         ...
         BEGIN
             ...
             END
         END
         EXECUTE nsp_logerror @n_err, @c_errmsg, 'nspTTMDPK2'
