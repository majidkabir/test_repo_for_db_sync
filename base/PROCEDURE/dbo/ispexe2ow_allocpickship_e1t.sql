...
/************************************************************************/  
/* Stored Procedure: ispExe2OW_allocpickship_E1T                        */  
/* Creation Date:                                                       */  
/* Copyright: IDS                                                       */  
/* Written by: June                                                     */  
/*                                                                      */
...
/* 23-Jul-2013  SWYep   Added ROWLOCK to avoid deadlock (SW02)          */
/* 14-Apr-2014  TLTING  SQL2012 Fixing Bugs                             */
/* 28-Jan-2019  TLTING_ext 1.1  enlarge externorderkey field length     */
/************************************************************************/  
  
CREATE PROCEDURE [dbo].[ispExe2OW_allocpickship_E1T]  
AS  
   SET NOCOUNT ON 
   SET ANSI_NULLS OFF 
   SET QUOTED_IDENTIFIER OFF 
  ...
  -- Specific Lottable02, Action Code = 'C' & NewLineNo = ''  
   ...
   BEGIN  
    ...
    BEGIN  
     ...
     SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Insert Failed On Exe2OW_allocpickship. (ispExe2OW_allocpickship_E1T)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "  
    END  
...
*/  
 ...
 BEGIN  
    ...
    SET   Transmitflag = '5'  
    FROM  Transmitlog, Transmitlog TL (nolock)  
    WHERE Transmitlog.Key1 = TL.Key1   
    AND   Transmitlog.Tablename = 'OWORDPICK'  
    AND TL.Tablename IN ('OWORDALLOC', 'OWDPREPICK')  
      AND   Transmitlog.Transmitflag < '5'    
 ...
 *************************************************************************************************/  
 ...
 /*     
   ...
   -- June01 - End : Fixed problem caused by SOS27033  
      ...
      BEGIN  
       SELECT @c_errmsg = CONVERT(CHAR(250),@n_err), @n_err=62301   -- Should Be Set To The SQL Errmessage but I don't know how to do so.  
       SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Update Failed On Exe2OW_allocpickship. (ispExe2OW_allocpickship_E1T)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "  
      END  
   ...
   BEGIN  
      ROLLBACK TRAN  
      EXECUTE nsp_logerror @n_err, @c_errmsg, "ispExe2OW_allocpickship_E1T"  
