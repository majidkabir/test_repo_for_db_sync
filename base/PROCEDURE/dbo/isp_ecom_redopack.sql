...
/************************************************************************/
CREATE PROC [dbo].[isp_Ecom_RedoPack] 
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN  
            SET @n_continue = 3
            SET @n_err = 60005 
            SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Executing ' + @c_CTNTrackNoReverse_SP + '. (isp_Ecom_RedoPack)' 
                         + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
            GOTO QUIT
         END                                                               
      END
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            SET @n_continue = 3
            SET @n_err = 60008 
            SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Update ORDERDETAILREF Table. (isp_Ecom_RedoPack)' 
                         + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
            GOTO QUIT
         END
   ...
   BEGIN
      SET @n_continue = 3
      SET @n_err = 60010  
      SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Delete PACKDETAIL Table. (isp_Ecom_RedoPack)' 
                   + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
      GOTO QUIT
   END
   ...
   BEGIN
      SET @n_continue = 3
      SET @n_err = 60020  
      SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Delete PACKHEADER Table. (isp_Ecom_RedoPack)' 
                     + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
      GOTO QUIT
   END  
   ...
   BEGIN
      ...
      BEGIN
         SET @n_continue = 3
         SET @n_err = 60030
         SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Update PACKTASKDETAIL Table. (isp_Ecom_RedoPack)' 
                        + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
         GOTO QUIT
      END
...
QUIT:
   ...
   BEGIN
      ...
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'isp_Ecom_RedoPack'
