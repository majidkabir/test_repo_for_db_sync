...
/************************************************************************/
/* Stored Procedure: isp_warrant_receipt_03                             */
/* Creation Date: 30/04/2021                                            */
/* Copyright: IDS                                                       */
/* Written by: MINGLE                                                   */
/*                                                                      */
/* Purpose: WMS-16900 MY -iDSMED - Warrant Receipt Report CR            */
/*                                                                      */
/* Called By: r_dw_warrant_receipt_03(copy from  r_dw_warrant_receipt   */
/*                                                                      */
/* PVCS Version: 1.1                                                    */
/*                                                                      */
/* Version: 5.4                                                         */
...
/* 30/04/2021   Mingle  1.0   WMS-16900 Warrant Receipt Report CR       */
/************************************************************************/

CREATE PROC [dbo].[isp_warrant_receipt_03] (
 ...
 BEGIN
	...
	LEFT OUTER JOIN Codelkup CLR (NOLOCK) ON (RECEIPT.Storerkey = CLR.Storerkey AND CLR.Code = 'HIDEQTY'
                                              AND CLR.Listname = 'REPORTCFG' AND CLR.Long = 'r_dw_warrant_receipt_03' AND ISNULL(CLR.Short,'') <> 'N')
	LEFT OUTER JOIN Codelkup CLR1 (NOLOCK) ON (RECEIPT.Storerkey = CLR1.Storerkey AND CLR1.Code = 'SHOWDATERECEIVED'
                                              AND CLR1.Listname = 'REPORTCFG' AND CLR1.Long = 'r_dw_warrant_receipt_03' AND ISNULL(CLR1.Short,'') <> 'N')
