...
GO
/************************************************************************/
/* Store Procedure:  isp_SCE_DL_GENERIC_EO_RULES_100002_10              */
/* Creation Date: 15-Dec-2021                                           */
/* Copyright: LFL                                                       */
/* Written by: GHChan                                                   */
/*                                                                      */
/* Purpose:  Perform ExternorderKey checking                            */
/*                                                                      */
/*                                                                      */
...
CREATE   PROCEDURE [dbo].[isp_SCE_DL_GENERIC_EO_RULES_100002_10] (
   @b_Debug       INT            = 0
 , @n_BatchNo     INT            = 0
 , @n_Flag        INT            = 0
...
BEGIN
   ...
   BEGIN
      ...
      WHERE EO.ExternOrderKey = STG.ExternOrderKey
      )
      )
      BEGIN
         BEGIN TRANSACTION;
         ...
         SET STG.STG_Status = '5'
           , STG.STG_ErrMsg = 'Externorderkey already existed. Not allow to insert'
         FROM dbo.SCE_DL_EO_STG STG
         WHERE STG.STG_BatchNo = @n_BatchNo
         AND   STG.STG_Status    = '1'
         AND   EXISTS (
         SELECT 1
         ...
         BEGIN
            ...
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5), ISNULL(@n_ErrNo, 0))
                            + ': Update record to SCE_DL_EO_STG fail. (isp_SCE_DL_GENERIC_EO_RULES_100002_10)';
            ROLLBACK;
   ...
   IF @b_Debug = 1
   BEGIN
      SELECT '<<SUB-SP-RULES>> - [isp_SCE_DL_GENERIC_EO_RULES_100002_10] EXIT... ErrMsg : ' + ISNULL(RTRIM(@c_ErrMsg), '');
   END;
