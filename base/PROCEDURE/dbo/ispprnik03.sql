...
/* 02-DEC-2019 CSCHONG  1.6   WMS-10204 - Fix uom issue (CS01c)         */      
/************************************************************************/          
CREATE PROC [dbo].[ispPRNIK03]              
...
BEGIN          
   ...
   BEGIN         
      ...
      --              + ': Invalid Wave Piece Pick Task Dispatch Method. Must Be INLINE/ECOM (ispPRNIK03)'      
      GOTO QUIT_SP      
   END                           
 ...
 ,  IDQty             INT       
   ...
   BEGIN      
      SELECT 'idpPRNIK03'      
      SELECT * FROM #ORDERLINES WITH (NOLOCK) order by sku     
     ...
     -- + CHAR(13) + ' ORDER BY CASE WHEN OD.OpenQty - ( OD.QtyAllocated + OD.QtyPreAllocated + OD.QtyPicked ) <= @n_CTNQty THEN 1 ELSE 0 END'      
      ...
      END  --CS01a END      
               ...
               BEGIN       
                ...
                if @n_OrderLineQty > @n_CTNQty --OR @n_OrderLineQty > @n_CHANNELAvaiQty   --CS01c   --CS0ae    
                BEGIN    
                     BREAK--SET @n_OrderLineQty = @n_CTNQty     
                 END    
                ...
                BEGIN      
                  SELECT @c_SQL      
                  SELECT @c_Orderkey '@c_Orderkey',@c_channel 'channel', @n_cntOrder 'cntOrder'  , @c_SKU '@c_SKU',@n_ttlordqty '@n_ttlordqty', @n_CTNQty '@n_CTNQty'     
                END       
              ...
              BEGIN      
                ...
                SET @c_PrevChannel = @c_channel    --CS01c    
              ...
              END --CS01a      
                ...
                BEGIN    
                  ...
                  BEGIN      
                     ...
                     SET @n_Err = 61010      
                     SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0))        
                                    + ': Get PickDetailKey Failed. (ispPRNIK03)'      
                     GOTO QUIT_SP      
                  END      
ELSE      
                  BEGIN      
                     ...
                     BEGIN      
                        ...
                        SET @n_Err = 61020      
                        SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0))        
                                       + ': Insert PickDetail Failed. (ispPRNIK03)'      
                        GOTO QUIT_SP      
                     END      
...
QUIT_SP:      
   ...
   BEGIN        
      ...
      BEGIN        
         ...
         END        
      END        
      EXECUTE nsp_logerror @n_Err, @c_ErrMsg, 'ispPRNIK03'        
