...
/************************************************************************/
/* Store procedure: ispOddSize1SKUPallet                                */
/* Copyright: IDS                                                       */
...
/* 2012-11-28   Ung       1.0   SOS257227 Fit by pallet type            */
/************************************************************************/

CREATE PROCEDURE [dbo].[ispOddSize1SKUPallet]
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         IF @b_debug = 1
         BEGIN
            SELECT @c_Reason = 'FAILED PutCode: OddSize1SKUPallet  from LOC = IND/PTL1001'
      ...
      BEGIN
         IF @b_debug = 1
         BEGIN
            SELECT @c_Reason = 'FAILED PutCode: OddSize1SKUPallet  SKU.Notes1 = ' + RTRIM( @c_Notes1)
            EXEC nspPTD 'nspRDTPASTD', @n_pTraceHeadKey, @c_PutawayStrategyKey, @c_PutawayStrategyLineNumber, @n_PtraceDetailKey, @c_ToLoc, @c_Reason
         END
         SET @b_RestrictionsPassed = 0 --False
      END
      ...
      BEGIN
         IF @b_debug = 1
         BEGIN
            SELECT @c_Reason = 'PASSED PutCode: OddSize1SKUPallet  SKU count = ' + CAST( @nSKUCount AS NVARCHAR( 5)) + ' QTY count = ' + CAST( @nQTYCount AS NVARCHAR( 5))
            EXEC nspPTD 'nspRDTPASTD', @n_pTraceHeadKey, @c_PutawayStrategyKey, @c_PutawayStrategyLineNumber, @n_PtraceDetailKey, @c_ToLoc, @c_Reason
         END
      END
      ELSE
      BEGIN
         IF @b_debug = 1
         BEGIN
            SELECT @c_Reason = 'FAILED PutCode: OddSize1SKUPallet  SKU count = ' + CAST( @nSKUCount AS NVARCHAR( 5))
            EXEC nspPTD 'nspRDTPASTD', @n_pTraceHeadKey, @c_PutawayStrategyKey, @c_PutawayStrategyLineNumber, @n_PtraceDetailKey, @c_ToLoc, @c_Reason
         END
         SET @b_RestrictionsPassed = 0 --False
      END
