...
GO
/************************************************************************/    
/* Stored Procedure: ispPRNKP06                                         */    
/* Creation Date: 03-SEP-2019                                           */    
/* Copyright: LFL                                                       */    
/* Written by: Wan                                                      */    
/*                                                                      */    
/* Purpose: WMS-10156 NIKE - PH Allocation Strategy Enhancement         */
/*          Full Case from Case Loc then Pallet by consolidate order    */
/*                                                                      */    
/* Called By:                                                           */    
/*                                                                      */    
/* PVCS Version: 1.1                                                    */   
...
BEGIN    
   ...
   BEGIN   
      ...
      --              + ': Invalid Wave Piece Pick Task Dispatch Method. Must Be INLINE/ECOM (ispPRNKP06)'
      GOTO QUIT_SP
   END                     
            ...
            BEGIN 
               ...
               BEGIN 
                  ...
                  BEGIN
                     ...
                     SET @n_Err = 61010
                     SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0))   
                                   + ': Get PickDetailKey Failed. (ispPRNKP06)'
                     GOTO QUIT_SP
                  END
                  ELSE
                  BEGIN
                     ...
                     BEGIN
                        ...
                        SET @n_Err = 61020
                        SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0)) 
                                       + ': Insert PickDetail Failed. (ispPRNKP06)'
                        GOTO QUIT_SP
                     END
...
QUIT_SP:
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         END  
      END  
      EXECUTE nsp_logerror @n_Err, @c_ErrMsg, 'ispPRNKP06'  
