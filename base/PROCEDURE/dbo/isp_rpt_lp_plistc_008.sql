...
/************************************************************************/
/* Stored Procedure: isp_RPT_LP_PLISTC_008                              */
/* Creation Date: 2022-02-25                                            */
/* Copyright: LFL                                                       */
/* Written by: CSCHONG                                                  */
/*                                                                      */
/* Purpose: WMS-20255 - PH_IDSMED-Picklist per Order                    */
/*          Copy and modify from nsp_GetPickSlipOrders128               */
/*                                                                      */
/* Called By: RPT_LP_PLISTC_008                                         */
/*                                                                      */
/* GitLab Version: 1.0                                                  */
/*                                                                      */
/* Version: 5.4                                                         */
...
/* 28-FEB-2022  CSCHONG 1.0   Devops Scripts Combine                    */
/************************************************************************/

CREATE   PROC [dbo].[isp_RPT_LP_PLISTC_008] (@c_loadkey NVARCHAR(10))
 AS
BEGIN
   ...
   BEGIN
      ...
      SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Failed On Table Pickheader Table. (isp_RPT_LP_PLISTC_008)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '
   END
   ...
   BEGIN
      ...
      LEFT OUTER JOIN Codelkup CLR (NOLOCK) ON (ORDERS.Storerkey = CLR.Storerkey AND CLR.Code = 'SHOWORDERGRP'
                                    AND CLR.Listname = 'REPORTCFG' AND CLR.Long = 'RPT_LP_PLISTC_008' AND ISNULL(CLR.Short,'') <> 'N')
      ...
      FETCH NEXT FROM pickslip_cur INTO @c_Loadkey, @c_LocType, @d_LoadDate, @c_PickZone, @c_C_Company, @c_Loc, @c_LogicalLoc,
                                        ...
                                        @c_SKUGroup, @c_SKUItemClass, @c_PackUOM3, @c_ExternOrderKey
      ...
      BEGIN
         ...
         SET @n_ttleaches = (@n_Palletcnt * @n_Pallet) + (@n_Cartoncnt * @n_CaseCnt) +  @n_EA
         ...
         INSERT INTO @t_Result (Loadkey, Pickslipno, PickType, LoadingDate, PickZone, Loc, LogicalLoc, SKU, Descr,
                  ...
                  ReprintFlag, PageNo, TotalPage, C_Company, EA, Qty, Route, Storerkey, Consigneekey,OrderGrpFlag, TrfRoom,
                  LEXTLoadKey,LPriority,LPuserdefDate01,showfield,InnerPack,
                  Lottable08, Lottable10, Lottable12, SKUGROUP, itemclass,TTLEACHES,PackUOM3,ExternOrderkey)
         VALUES (@c_Loadkey, '', @c_PickType, @d_LoadDate, @c_PickZone, @c_LOC, @c_LogicalLoc, @c_SKU, @c_SkuDescr,
                 ...
                 @c_C_Company, @n_EA, @n_Qty, @c_Route, @c_Storerkey, @c_Consigneekey,@c_OrdGrpFlag, @c_TrfRoom,
                 @c_LEXTLoadKey,@c_LPriority,@c_LPuserdefDate01,@n_showField,@n_innercnt,
                 @c_Lottable08, @c_Lottable10, @c_Lottable12, @c_SKUGroup, @c_SKUItemClass,@n_ttleaches,@c_PackUOM3,@c_ExternOrderkey)
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert Failed On Table PICKHEADER. (isp_RPT_LP_PLISTC_008)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '
            END
         END
            ...
            BEGIN
               ...
               BEGIN
                  ...
                  BEGIN
                     SELECT @n_continue = 3
                     SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err=73001
                     SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert Failed On Table RefkeyLookup. (isp_RPT_LP_PLISTC_008)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '
                  END
                  ...
                  BEGIN
                     ...
                     BEGIN
                        SELECT @n_continue = 3
                        SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err=73001
                        SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Failed On Table PICKDETAIL. (isp_RPT_LP_PLISTC_008)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '
                     END
                  END
               END
