...
GO
/************************************************************************/  
/* Stored Procedure: isp_LoadPlanDetailTrigger_Wrapper                  */  
/* Creation Date: 16-May-2017                                           */  
/* Copyright: LF                                                        */  
/* Written by:                                                          */  
/*                                                                      */  
/* Purpose: WMS-1907 Call Load Plan Detail custom stored proc           */
/*                                                                      */  
/* Called By: Loadplandetail insert/update/delete trigger               */    
/*            Stored proc naming: ispLPDxx                              */
/*                                                                      */  
/* PVCS Version: 1.0                                                    */  
/*                                                                      */  
/* Version: 5.4                                                         */  
...
/*18/10/2019    mingle01 1.1  Support configure multiple SP and change  */
/*                            to declare local cursor instead           */
/************************************************************************/  

CREATE PROCEDURE [dbo].[isp_LoadPlanDetailTrigger_Wrapper]  
...
BEGIN  
   ...
   BEGIN   	  
      IF EXISTS (SELECT 1 FROM DELETED d   ----->Put INSERTED if INSERT action
                 ...
                 WHERE  s.configkey = 'PODTrigger_SP')   -----> Current table trigger storerconfig
      BEGIN        	  
   ...
   */
   ...
   DECLARE @n_Continue      INT
         ...
         , @c_option5_splist  NVARCHAR(2000) --mingle01
   ...
   SET @c_SQL        = ''
   SET @c_Storerkey  = ''
   SET @c_configkey = 'LoadPlanDetailTrigger_SP'
   ...
   BEGIN   
      DECLARE Cur_SPCode CURSOR LOCAL FAST_FORWARD READ_ONLY FOR  --mingle01
          SELECT DISTINCT O.Storerkey, S.Svalue
                        , S.Option5 --mingle01
          FROM #DELETED D
   ...
   BEGIN
      ...
      BEGIN
         ...
         END
      END
      execute nsp_logerror @n_err, @c_errmsg, 'isp_LoadPlanDetailTrigger_Wrapper'
