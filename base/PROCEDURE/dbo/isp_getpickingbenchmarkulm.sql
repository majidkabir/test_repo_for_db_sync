...
CREATE PROC [dbo].[isp_GetPickingBenchmarkULM] (    
...
BEGIN    
   ...
   SET @nPickedQty = 0    
     ...
     ([Status],Storerkey,Facility,Adddate,Qty,Qty_PCS,Qty_CaseCnt,Qty_Pallet)  
      SELECT PK.STATUS, PK.STORERKEY, S.FACILITY, PK.ADDDATE, PK.QTY, SUM(PK.QTY/nullif(P.QTY,0)) ,  
      ...
      GROUP BY PK.STATUS, PK.STORERKEY, S.FACILITY, PK.ADDDATE, PK.QTY  
      ORDER BY PK.ADDDATE  
 ...
 /*  IF @cStorerKey <> 'Unilever'  
   ...
   BEGIN  
    ...
    WHERE v.StorerKey = @cStorerKey    
     ...
     GROUP BY DATEPART(HOUR, EventDateTime) */  
      ...
      SELECT DATEPART(HOUR, PD.AddDate) As PickHour, (CASE WHEN @cUOM = 1 THEN SUM(Qty_Pallet)  
  ...
  -- END  
   ...
   BEGIN    
       ...
       If @b_debug = '1'  
       BEGIN  
         PRINT 'hour pick ' + convert(varchar(8),@nHour) + 'Pick qty : ' +  convert(varchar(8),@nPickedQty)  
       END  
   ...
   BEGIN    
       ...
       If @b_debug = '1'  
       BEGIN  
         PRINT 'Plan qty : ' +  convert(varchar(8),@nPlannedQty)   
         PRINT 'Pick qty : ' +  convert(varchar(8),@nPickedQty)  
       END  
       ...
       If @b_debug = '1'  
       BEGIN  
         PRINT 'Sum Plan qty : ' +  convert(varchar(8),@nTotPlanQty)   
         PRINT 'Sum Pick qty : ' +  convert(varchar(8),@nTotPickQty)  
       END  
