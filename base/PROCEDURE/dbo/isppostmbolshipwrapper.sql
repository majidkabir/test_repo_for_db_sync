...
GO
/************************************************************************/  
/* Stored Procedure: ispPostMBOLShipWrapper                             */  
...
/************************************************************************/  
CREATE PROC [dbo].[ispPostMBOLShipWrapper]    
...
BEGIN    
   ...
   BEGIN 
      SET @c_PostMBOLShipSP = ''
                                  
      EXEC nspGetRight  
           @c_Facility  = NULL,  
           @c_StorerKey = @c_StorerKey,  
           @c_sku       = NULL,  
           @c_ConfigKey = 'PostMBOLShipSP',   
           @b_Success   = @b_Success                  OUTPUT,  
           @c_authority = @c_PostMBOLShipSP           OUTPUT,   
           @n_err       = @n_err                      OUTPUT,   
           @c_errmsg    = @c_errmsg                   OUTPUT  


      IF EXISTS (SELECT 1 FROM sys.objects o WHERE NAME = @c_PostMBOLShipSP AND TYPE = 'P')
      BEGIN
         BEGIN TRAN

         SET @c_SQL = N'  
            EXECUTE ' + @c_PostMBOLShipSP + CHAR(13) +  
         ...
         BEGIN  
            SET @n_Continue= 3    
            SET @n_Err     = 63502    
            SET @c_ErrMsg  =  'NSQL'+CONVERT(NVARCHAR(5),@n_Err)+': Failed to EXEC ' + @c_PostMBOLShipSP +   
                              CASE WHEN ISNULL(@c_ErrMsg, '') <> '' THEN ' - ' + @c_ErrMsg ELSE '' END + ' (ispPostMBOLShipWrapper)'
            GOTO EXIT_SP                          
         END 
...
EXIT_SP:
   ...
   BEGIN    
      ...
      EXECUTE nsp_logerror @n_Err, @c_ErrMsg, 'ispPostMBOLShipWrapper'    
