...
/************************************************************************/  
/* Stored Procedure: isp_GetKIOSKASRSTRFNewTask_c                       */  
...
/*                                                                      */  
/* Called By: d_dw_kiosk_asrstrf_new_c_form                             */  
/*                                                                      */  
/* PVCS Version: 1.0                                                    */  
/*                                                                      */  
/* Version: 5.4                                                         */  
...
/************************************************************************/  
CREATE PROC [dbo].[isp_GetKIOSKASRSTRFNewTask_c]  
...
BEGIN  
   ...
   INSERT INTO #Result (TaskDetailKey,FROMID,TOID,PalletID,TransferKey,Transferlinenumber,FromStorerkey,FromSKU,Storerkey,SKU,SKUDescr,TransferReason
                     ...
                     ,PackUOM1,PackUOM3,CASECNT,QtyToTrfInCS,QtyToTrfInEA
   ...
   ,CASE WHEN PACK.CaseCnt > 0 AND @n_ShowUOMEA = 0 THEN CONVERT(INT,FLOOR(TRANSFERDETAIL.FromQty / PACK.CaseCnt)) ELSE 0 END  
   ,CASE WHEN PACK.CaseCnt > 0 AND @n_ShowUOMEA = 0 THEN CONVERT(INT,(TRANSFERDETAIL.FromQty % CONVERT(INT,PACK.CaseCnt))) ELSE TRANSFERDETAIL.FromQty END  
   ,TASKDetail.Status
   ,@c_JobStatus
   FROM TASKDETAIL WITH (NOLOCK) 
   ...
   BEGIN
      UPDATE #RESULT
      SET QtyInCS = QtyToTrfInCS
        ,QtyInEA = QtyToTrfInEA
   END

   SELECT @c_JobKey 
         ,Taskdetailkey,FROMID,TOID,PalletID,TransferKey,Transferlinenumber,Storerkey,SKU,SKUDescr,TransferReason,Lottable01label,Lottable02label,Lottable03label,Lottable04label,Lottable05label,
          ...
          PackUOM1,PackUOM3,CASECNT,QtyToTrfInCS,QtyToTrfInEA,QtyInCS,QtyInEA
         ,TaskStatus,@c_JobStatus
   FROM #Result 
END   
