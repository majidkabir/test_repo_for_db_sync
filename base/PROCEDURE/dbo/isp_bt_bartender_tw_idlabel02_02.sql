...
/******************************************************************************/                   
/* Copyright: IDS                                                             */                   
/* Purpose: isp_BT_Bartender_TW_IDLABEL02_01                                  */                   
/*                                                                            */                   
/* Modifications log:                                                         */                   
/*                                                                            */                   
...
CREATE PROC [dbo].[isp_BT_Bartender_TW_IDLABEL02_02]                        
...
BEGIN                        
  ...
  SET @c_SQLJOIN = +' SELECT DISTINCT RECDET.ToID,CONVERT(NVARCHAR(10),RECDET.DateReceived,111),RECDET.Receiptkey,RECDET.Toloc,'''','+ CHAR(13)      --5        
             + ' '''','''','''','''','''','     --10    
             + ' '''','''','''','''','''','     --15    
             + ' '''','''','''','''','''','     --20         
             + CHAR(13) +        
          ...
          --   + ' JOIN RECEIPTDETAIL RECDET WITH (NOLOCK) ON RECDET.Toid = LLI.id '    
       --      + ' JOIN SKU S WITH (NOLOCK) ON S.Sku=RECDET.SKU AND S.Storerkey = RECDET.Storerkey'     
             + ' WHERE RECDET.ToID = @c_Sparm02 AND'      
             + ' RECDET.receiptkey= @c_Sparm01 '  
            -- + ' AND RECDET.Toloc = @c_Sparm03'     
  ...
  SELECT DISTINCT col01,col03       
   FROM #Result                 
   WHERE Col60 = 'O'           
            
   OPEN CUR_RowNoLoop                    
               
   FETCH NEXT FROM CUR_RowNoLoop INTO @c_ToId,@c_RDRECkey      
   ...
   BEGIN                   
      ...
      INSERT INTO [#TEMPSKU] (Receiptkey, ToID, SKU, SSTYLE, SSIZE, SCOLOR, Qty,  
                  Retrieve)  
      SELECT DISTINCT RD.receiptkey,RD.ToId,RD.sku,S.Style,S.Size,S.color,  
             SUM(RD.BeforeReceivedQty),'N'  
      ...
      AND RD.ReceiptKey = @c_RDRECkey    
      GROUP BY RD.receiptkey,RD.ToId,RD.sku,S.Style,S.Size,S.color  
      ORDER BY RD.receiptkey,RD.ToId,RD.sku  
     ...
     BEGIN     
      ...
      FROM #TEMPSKU   
      WHERE ID = @n_intFlag  
      GROUP BY SKU,SSTYLE, SSIZE, SCOLOR  
...
EXIT_SP:      
   ...
   EXEC isp_InsertTraceInfo     
      ...
      @c_TraceName = 'isp_BT_Bartender_TW_IDLABEL02_02',    
