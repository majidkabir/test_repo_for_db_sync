...
/************************************************************************/  
/* Store Procedure:  isp_GetPickSlipOrders91                            */  
/* Creation Date: 12-Jun-2019                                           */  
/* Copyright: IDS                                                       */  
/* Written by: Modify from isp_GetPickSlipOrders29                      */  
/*                                                                      */  
/* Purpose:  WMS-9386 Picking List                                      */  
/*                                                                      */  
...
/* Return Status:  None                                                 */  
/*                                                                      */  
/* Usage:  Used for report dw = r_dw_print_pickorder91                  */  
/*                                                                      */  
/* Local Variables:                                                     */  
/*                                                                      */  
/* Called By:                                                           */  
...
CREATE PROC [dbo].[isp_GetPickSlipOrders91] (@c_loadkey NVARCHAR(10))   
 AS  
 BEGIN  
   SET NOCOUNT ON   
   SET ANSI_NULLS OFF     
  ...
  @n_starttrancnt  int   
    ...
    BEGIN  
       SELECT @n_continue = 3    
       SELECT @n_err = 63500  
     SELECT @c_errmsg = 'NSQL'+CONVERT(char(5),@n_err)+': Pickslip already printed using Consolidated option. (isp_GetPickSlipOrders91)'  
    END  
    --(Kc01) - end  
     ...
     Lottable06       NVARCHAR(30) NULL) 
       ...
       INSERT INTO #TEMP_PICK  
            (PickSlipNo,          LoadKey,         OrderKey,  ConsigneeKey,  
             ...
             LRoute,LEXTLoadKey,LPriority,LPuserdefDate01,Lottable06)    --(CS01)      
        --(Kc01) - start  
        /*   
        ...
        */  
        ...
        --(Kc01) - end  
          ...
          -- ISNULL((SELECT Distinct 'Y' FROM PICKHEADER WITH (NOLOCK) WHERE ExternOrderKey = @c_Loadkey AND  Zone = '3'), 'N') AS PrintedFlag,   
           ...
           LotAttribute.Lottable06 AS Lottable06                                        
...
*/  
      ...
      LEFT JOIN CODELKUP CLR (NOLOCK) ON (O.Storerkey = CLR.Storerkey AND CLR.Code = 'NOSPLITBYLINECNTZONE'   
                                          AND CLR.Listname = 'REPORTCFG' AND CLR.Long = 'r_dw_print_pickorder91' AND ISNULL(CLR.Short,'') <> 'N') --NJOW04  
...
--            BEGIN TRAN  
            ...
            BEGIN    
               ...
               BEGIN    
                  SELECT @n_continue = 3    
                  SELECT @n_err = 63501  
              SELECT @c_errmsg = 'NSQL'+CONVERT(char(5),@n_err)+': Insert into PICKHEADER Failed. (isp_GetPickSlipOrders91)'  
                  GOTO FAILURE  
               END               
            END -- @b_success = 1      
            ELSE     
            BEGIN    
               SELECT @n_continue = 3    
               SELECT @n_err = 63502  
              SELECT @c_errmsg = 'NSQL'+CONVERT(char(5),@n_err)+': Get PSNO Failed. (isp_GetPickSlipOrders91)'    
               BREAK     
            END     
         ...
         BEGIN    
            ...
            SET @c_ExecStatement = N'DECLARE C_PickDetailKey CURSOR FAST_FORWARD READ_ONLY FOR ' +  
                                    ...
                                    ' AND Lotattribute.Lottable06 = N''' + RTRIM(@c_Lottable06) + ''' ' +
                                    @c_LocTypeCriteria +  
            ...
            BEGIN    
               ...
               BEGIN     
                  ...
                  BEGIN    
                     SELECT @n_continue = 3  
                     SELECT @n_err = 63503  
                   SELECT @c_errmsg = 'NSQL'+CONVERT(char(5),@n_err)+': Insert RefKeyLookup Failed. (isp_GetPickSlipOrders91)'      
                     GOTO FAILURE  
                  END                            
               END     
         ...
         BEGIN    
            SELECT @n_continue = 3    
            SELECT @n_err = 63504  
            SELECT @c_errmsg = 'NSQL'+CONVERT(char(5),@n_err)+': Update #Temp_Pick Failed. (isp_GetPickSlipOrders91)'      
            GOTO FAILURE  
         END    
--         ELSE  
--         BEGIN  
...
--(Kc01) - end  
 ...
 BEGIN  
  ...
  BEGIN  
   ...
   END  
  END  
  EXECUTE nsp_logerror @n_err, @c_errmsg, 'isp_GetPickSlipOrders91'  
