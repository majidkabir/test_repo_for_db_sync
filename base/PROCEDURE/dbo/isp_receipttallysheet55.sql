...
/************************************************************************/
/* Stored Procedure: isp_ReceiptTallySheet55    						      */
/* Creation Date: 26/10/2017                                            */
/* Copyright: IDS                                                       */
/* Written by:CSCHONG                                                   */
/*                                                                      */
/* Purpose: WMS-2968 - VF Tally Sheet Enhancement                       */
/*                                                                      */
/* Called By: r_receipt_tallysheet55                                    */
/*                                                                      */
/* PVCS Version: 1.1                                                    */
/*                                                                      */
/* Version: 5.4                                                         */
...
/* 13-Aug-2018  SPChin  1.2   INC0463786 - Bug Fixed                    */
/************************************************************************/

CREATE PROC [dbo].[isp_ReceiptTallySheet55] (
 ...
 BEGIN
    ...
    DECLARE @c_OtherReferenceALL NVARCHAR(250),
            ...
            @n_CTNLott02 INT  --INC0344763
            
    SELECT RECEIPT.Storerkey,
           RECEIPT.Facility,
           RECEIPT.Receiptkey,
		     MAX(RECEIPTDETAIL.Lottable02) as LOTT02,
		     COUNT(DISTINCT RECEIPTDETAIL.Lottable02) as CNTLOTT02,
		     CASE WHEN RECEIPT.Doctype = 'A' THEN 'NORMAL'
               WHEN RECEIPT.Doctype = 'X' THEN 'XD'
               ELSE  RECEIPT.Rectype END AS RecType,
		     RECEIPT.Doctype,
           RECEIPT.Carrierkey,
           ...
           ISNULL(PO.ExternPokey,'') AS ExternPokey,  --INC0463786
           UCC.UCCNo,
    ...
    GROUP BY RECEIPT.Storerkey,
             ...
             CASE WHEN RECEIPT.Doctype = 'A' THEN 'NORMAL'
                WHEN RECEIPT.Doctype = 'X' THEN 'XD'
                ELSE  RECEIPT.Rectype END,
             RECEIPT.Carrierkey,
             Receipt.editdate,
             ISNULL(PO.ExternPokey,''),               --INC0463786
             UCC.UCCNo
    
    SET @c_PrevReceiptkey  = ''
    DECLARE REC_CUR CURSOR FAST_FORWARD READ_ONLY FOR
       SELECT DISTINCT RECEIPT.RECEIPTKEY, PO.OtherReference, PO.SellerName, PO.SellerAddress1, COUNT(DISTINCT RECEIPTDETAIL.Lottable02)  --INC0463786 
       FROM RECEIPT (NOLOCK)
       JOIN RECEIPTDETAIL (NOLOCK) ON RECEIPT.Receiptkey = RECEIPTDETAIL.Receiptkey
       JOIN PO (NOLOCK) ON RECEIPTDETAIL.ExternReceiptkey = PO.ExternPokey AND RECEIPTDETAIL.Storerkey = PO.Storerkey                     --INC0463786
       WHERE RECEIPT.Storerkey BETWEEN @c_storerkeystart AND @c_storerkeyend        
       AND RECEIPT.Receiptkey BETWEEN @c_receiptkeystart AND @c_receiptkeyend
       GROUP BY RECEIPT.Receiptkey, PO.OtherReference, PO.SellerName, PO.SellerAddress1                                                   --INC0463786
       ORDER BY RECEIPT.Receiptkey, PO.OtherReference, PO.SellerName, PO.SellerAddress1
           
    OPEN REC_CUR  
                      
    FETCH NEXT FROM REC_CUR INTO @c_Receiptkey, @c_OtherReference, @c_SellerName, @c_SellerAddress1, @n_CTNLott02                         --INC0463786 
	  ...
	  BEGIN
       ...
       BEGIN
       	  ...
       	  SET OtherReference = @c_OtherReferenceALL,
       	      ...
       	      CNTLOTT02 = @n_CTNLott02   --INC0463786
    ...
    FROM (SELECT DISTINCT #TMP_REC.Storerkey,      
                 ...
                 #TMP_REC.RecDate,
				     #TMP_REC.doctype,
				     #TMP_REC.ExternPokey --INC0463786     
          FROM #TMP_REC) AS TR
    JOIN RECEIPTDETAIL (NOLOCK) ON TR.Receiptkey = RECEIPTDETAIL.Receiptkey AND TR.ExternPokey = RECEIPTDETAIL.ExternReceiptkey  --INC0463786
