...
BEGIN
   ...
   BEGIN
      ...
      SET @c_errmsg = 'NSQL' + CONVERT( NVARCHAR(5), @n_err) + ': ' + 
                      'Update to TaskDetail table failed. (nspTTMNMF1)' + 
   ...
   IF @c_AreaKey01 <> ''
      DECLARE Cursor_NMFTaskCandidates CURSOR FAST_FORWARD READ_ONLY FOR --Note: global cursor
         ...
         WHERE AreaDetail.AreaKey = @c_AreaKey01
            AND TaskDetail.TaskType IN ('NMF', 'NM1')
   ...
   ELSE
      DECLARE Cursor_NMFTaskCandidates CURSOR FAST_FORWARD READ_ONLY FOR --Note: global cursor 
         ...
         FROM dbo.TaskDetail WITH (NOLOCK)
            JOIN dbo.LOC WITH (NOLOCK) ON (TaskDetail.FromLOC = LOC.LOC)
            JOIN dbo.AreaDetail WITH (NOLOCK) ON (AreaDetail.PutawayZone = LOC.PutAwayZone)
         WHERE dbo.TaskDetail.TaskType IN ('NMF', 'NM1')
            AND TaskDetail.Status = '0'
            AND EXISTS( SELECT 1 
               FROM TaskManagerUserDetail tmu WITH (NOLOCK)
               WHERE PermissionType = TaskDetail.TASKTYPE
   ...
   BEGIN
      ...
      BEGIN
        ...
        BEGIN
          ...
          END
        END
        execute nsp_logerror @n_err, @c_errmsg, 'nspTTMNMF1'
