...
/************************************************************************/
/* Stored Procedure: nspPR_TH08                                         */
/* Creation Date: 28-Jun-2019                                           */
/* Copyright: LF                                                        */
/* Written by: Chooi                                                    */
/*                                                                      */
/* Purpose: TH-DSG RM customize Allocate Logic follow JDE WH-Location   */
/*          Preallocation Strategy                                      */
/*          Copy and modify from nspPR_FIFO                             */
/*                                                                      */
...
BEGIN
   ...
   DECLARE @c_Orderkey               NVARCHAR(10),
           ...
           @n_continue               INT = 1,
           @c_SQL                    NVARCHAR(MAX) = '',
           @c_SortingColumn          NVARCHAR(MAX) = 'Lotattribute.Lottable03',
           @c_Lottable03Condition    NVARCHAR(MAX) = ''
                  
   CREATE TABLE #SortingSeq(
   Code          NVARCHAR(30),
   Short         NVARCHAR(10)
   )     
   
   INSERT INTO #SortingSeq (Code, Short)
   SELECT DISTINCT CAST(CL.Code AS INT)
                  ,CAST(RTRIM(LTRIM(REPLACE(REPLACE(REPLACE(CL.Short,CHAR(9),' '),CHAR(10),' '),CHAR(13),' '))) AS NVARCHAR(30))
   FROM CODELKUP CL (NOLOCK)
   WHERE CL.LISTNAME = 'DSGRMAL' AND CL.STORERKEY = 'DSGTH'
   ORDER BY CAST(CL.Code AS INT)
   ...
   BEGIN              
      ...
      FETCH NEXT FROM cur_sort INTO @c_Code,@c_Short

      WHILE(@@FETCH_STATUS <> -1)    
      BEGIN
         IF @n_Runningno = 1
         BEGIN
            SET @c_SQL = @c_SQL + ' CASE ' + @c_SortingColumn + ' WHEN '''+ @c_Short +''' THEN '+ CAST (@n_Runningno AS NVARCHAR)
            SET @c_Lottable03Condition = @c_Lottable03Condition + '''' + @c_Short + ''','
            SET @n_Runningno = @n_Runningno + 1 
         END
         ELSE
         BEGIN
            SET @c_SQL = @c_SQL + ' WHEN '''+ @c_Short +''' THEN '+ CAST (@n_Runningno AS NVARCHAR)
            SET @c_Lottable03Condition = @c_Lottable03Condition + '''' + @c_Short + ''','    
            SET @n_Runningno = @n_Runningno + 1 
         END
         FETCH NEXT FROM cur_sort INTO @c_Code,@c_Short
   ...
   IF @c_SQL <> ''
      SET @c_SQL = @c_SQL + ' ELSE ' + CAST  (@n_Runningno AS NVARCHAR) + ' END, '

   IF @c_Lottable03Condition <> ''
      SET @c_Lottable03Condition = SUBSTRING(@c_Lottable03Condition,1,LEN(@c_Lottable03Condition)-1)
   ...
   BEGIN
      ...
      SELECT @c_SQLStatement = " DECLARE  PREALLOCATE_CURSOR_CANDIDATES CURSOR FAST_FORWARD READ_ONLY FOR " +
             ...
             " AND Lotattribute.Lottable03 IN ( " + @c_Lottable03Condition + " ) " +  
             " ORDER BY " + @c_SQL + " Lotattribute.Lottable05, Lot.Lot " 
   ...
   --          @c_Condition  )  
      SELECT @c_SQLStatement =  " DECLARE  PREALLOCATE_CURSOR_CANDIDATES CURSOR FAST_FORWARD READ_ONLY FOR " +
            ...
            " GROUP By LOT.STORERKEY, LOT.SKU, LOT.LOT, Lotattribute.Lottable05 " + CASE WHEN ISNULL(@c_SortingColumn,'') = '' THEN '' ELSE "," + @c_SortingColumn END +
