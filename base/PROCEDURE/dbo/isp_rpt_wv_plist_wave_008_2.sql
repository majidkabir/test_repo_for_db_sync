...
/***************************************************************************/
CREATE   PROC [dbo].[isp_RPT_WV_PLIST_WAVE_008_2]  
...
BEGIN
   ...
   SELECT WaveSeqOfDay  = @c_WaveSeq   --ROW_NUMBER() OVER (PARTITION BY  WH.MaxOrderQty ORDER BY WH.Wavekey)   
   ...
   JOIN SKU        SKU  WITH (NOLOCK) ON (PD.Storerkey = SKU.Storerkey)
                                      AND(PD.Sku = SKU.Sku)
   JOIN (SELECT OD.Orderkey, Openqty = SUM(OD.QtyAllocated) FROM ORDERS OH WITH (NOLOCK)   	
