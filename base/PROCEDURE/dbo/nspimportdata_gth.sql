...
GO
/************************************************************************/
/* Stored Procedure: nspImportData_gth                                  */
/* Creation Date:                                                       */
/* Copyright: IDS                                                       */
/* Written by:                                                          */
/*                                                                      */
...
CREATE PROC    [dbo].[nspImportData_gth]
...
BEGIN
      ...
      BEGIN
         ...
         ELSE
            BEGIN
               ...
               SELECT @c_pokey, @c_externpokey, Storerkey, POType, SellerName,Loadingdate FROM UPLOADPOHeader (nolock)
         ...
         BEGIN
            ...
            ELSE
               BEGIN -- no error
                  ...
                  FROM UPLOADPODETAIL (NOLOCK)
                  WHERE ExternPOKey = @c_externpokey
                  AND MODE = @c_mode AND POGROUP = @c_POGROUP                    AND STATUS = '0' and POKey = @c_pokey
                  ORDER BY POGROUP, ExternPOkey, Externlinenumber
         ...
         BEGIN
            ...
            BEGIN
               ...
               ELSE
                  ...
                  WHERE Externpokey = @c_detexternpokey                      AND POGROUP = @c_pogroup and pokey = @c_pokey
                  and MODE = @c_mode
                  AND Status = '0'
                  END -- count1 > 0
                  ELSE
   ...
   BEGIN
      ...
      AND uploadorderdetail.storerkey = 'GTH'
      WHERE sku.sku = NULL
      ...
      BEGIN
         ...
         ELSE
            BEGIN -- does not exists
               INSERT INTO OrderS (Orderkey, ExternOrderkey,UPLOADOrderHeader.Storerkey,Orderdate, Deliverydate,
               Priority, c_contact1, c_contact2, c_company, c_address1, c_address2, c_address3, c_address4,
               c_city, c_state, c_zip, buyerpo, notes,invoiceno,  notes2, pmtterm, invoiceamount , ROUTE,Facility,Consigneekey)
               SELECT @c_Orderkey, @c_externOrderkey, UPLOADOrderHeader.Storerkey, Orderdate, Deliverydate,
               Priority, c_contact1, c_contact2, c_company, c_address1, c_address2, c_address3, c_address4,
               c_city, c_state, c_zip, buyerpo, notes,invoiceno,UPLOADOrderHeader.notes2, pmtterm, invoiceamount , ISNULL (ROUTE , '99'),STORER.Facility,UPLOADOrderHeader.Consigneekey
      ...
      ELSE
         ...
         BEGIN
            /*
            Orderheader
            Orderkey, externorderkey, Storerkey, OrderGroup, Orderdate, Deliverydate,
            Priority, c_contact1, c_contact2, c_company, c_address1, c_address2, c_address3, c_address4,
            c_city, c_state, c_zip, buyerpo, notes,invoiceno,  notes2, pmtterm, invoiceamount, Mode, status, remarks
            Orderdetail
            Orderkey, Orderlinenumber, ExternOrderkey, OrderGroup, SKU, Storerkey, Openqty, Packkey, UOM,
            ExternLineno, ExtendedPrice, UnitPrice, Facility, Mode, status, remarks
            */
            IF EXISTS (SELECT 1 FROM OrderS (NOLOCK) WHERE ExternOrderKey = @c_externOrderKey)
            BEGIN
               UPDATE OrderS
               ...
               ORDERS.c_state = ISNULL(OH.c_state,' '),                    ORDERS.c_zip = ISNULL (OH.c_zip,' '),
               ...
               ORDERS.Facility = (STORER.Facility)                 FROM ORDERS (nolock), UPLOADORDERHEADER OH (nolock),STORER(nolock)
