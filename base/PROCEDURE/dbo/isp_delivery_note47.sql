...
GO
/************************************************************************/
/* Stored Proc: isp_delivery_note47                                     */
/* Creation Date: 22-OCT-2020                                           */
/* Copyright: LF Logistics                                              */
/* Written by:                                                          */
/*                                                                      */
/* Purpose: WMS-15129 - SG - MHD - Delivery Note                        */
/*        :                                                             */
/* Called By: r_dw_delivery_note47                                      */
/*          :                                                           */
/* PVCS Version: 1.0                                                    */
/*                                                                      */
/* Version: 7.0                                                         */
...
/************************************************************************/
CREATE PROC [dbo].[isp_delivery_note47]
...
BEGIN
   ...
   LEFT OUTER JOIN Codelkup CLR (NOLOCK) ON (Orders.Storerkey = CLR.Storerkey AND CLR.Code = 'SHOWFIELD'   
                                       AND CLR.Listname = 'REPORTCFG' AND CLR.Long = 'r_dw_delivery_note47' AND ISNULL(CLR.Short,'') <> 'N')  
   LEFT OUTER JOIN Codelkup CLR1 (NOLOCK) ON (Orders.Storerkey = CLR1.Storerkey AND CLR1.Code = 'SHOWLOGO'   
                                       AND CLR1.Listname = 'REPORTCFG' AND CLR1.Long = 'r_dw_delivery_note47' AND ISNULL(CLR1.Short,'') <> 'N')  
   LEFT OUTER JOIN Codelkup CLR3 (NOLOCK) ON (Orders.Storerkey = CLR3.Storerkey AND CLR3.Code = 'SHOWMBOLKEY'   
                                       AND CLR3.Listname = 'REPORTCFG' AND CLR3.Long = 'r_dw_delivery_note47' AND ISNULL(CLR3.Short,'') <> 'N')  
   LEFT OUTER JOIN Codelkup CLR4 (NOLOCK) ON (Orders.Storerkey = CLR4.Storerkey AND CLR4.Code = 'ShowBarcode'   
                                       AND CLR4.Listname = 'REPORTCFG' AND CLR4.Long = 'r_dw_delivery_note47' AND ISNULL(CLR4.Short,'') <> 'N') 
   LEFT OUTER JOIN Codelkup CLR5  (NOLOCK) ON (Orders.Storerkey = CLR5.Storerkey AND CLR5.Code = 'ShowAvailQty'   
                                       AND CLR5.Listname = 'REPORTCFG' AND CLR5.Long = 'r_dw_delivery_note47' AND ISNULL(CLR5.Short,'') <> 'N') 
   LEFT OUTER JOIN Codelkup CLR6  (NOLOCK) ON (Orders.Storerkey = CLR6.Storerkey AND CLR6.Code = 'ShowLast4CharsOfNRIC'   
                                       AND CLR6.Listname = 'REPORTCFG' AND CLR6.Long = 'r_dw_delivery_note47' AND ISNULL(CLR6.Short,'') <> 'N') 
LEFT JOIN CODELKUP CLR7 WITH (NOLOCK) ON CLR7.LISTNAME='REPORTCFG' AND CLR7.Long='r_dw_delivery_note47' 
                                       AND CLR7.code = 'SHOWQRCODE' AND CLR7.Storerkey=ORDERS.storerkey
                                        AND ISNULL(CLR7.Short,'') <> 'N'
LEFT JOIN CODELKUP CLR8 WITH (NOLOCK) ON CLR8.LISTNAME='REPORTCFG' AND CLR8.Long='r_dw_delivery_note47' --//WMS-13853
