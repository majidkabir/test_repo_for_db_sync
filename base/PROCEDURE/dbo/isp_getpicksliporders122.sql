...
/************************************************************************/  
/* Stored Procedure: isp_GetPickSlipOrders122                           */  
/* Creation Date: 03-MAY-2021                                           */  
/* Copyright:                                                           */  
/* Written by: CSCHONG                                                  */  
/*                                                                      */  
/* Purpose: WMS-16897 TH-IDSMED CR-Picking Slip Report                  */  
/*                                                                      */  
/* Called By: r_dw_print_pickorder122                                   */  
/*            duplicate from r_dw_print_pickorder10                     */
/*                                                                      */  
/* PVCS Version: 1.0                                                    */  
/*                                                                      */  
/* Version: 5.4                                                         */  
...
CREATE PROC [dbo].[isp_GetPickSlipOrders122] (@c_loadkey NVARCHAR(10)) 
AS  
BEGIN  
   SET NOCOUNT ON   
   SET ANSI_NULLS OFF   
   ...
   AND   Long = 'r_dw_print_pickorder122'      
   AND   ISNULL(Short,'') <> 'N'      
   ...
   BEGIN  
      ...
      ELSE  
         ...
         LEFT JOIN CODELKUP CL WITH (NOLOCK) ON (CL.ListName = 'RPTCFGPICK' AND CL.Long = 'r_dw_print_pickorder122'    
                                        AND CL.Storerkey = s.Storerkey and CL.Code=s.susr3)   
         LEFT JOIN CODELKUP CL1 WITH (NOLOCK) ON CL1.ListName = 'REPORTCFG' AND CL1.Long = 'r_dw_print_pickorder122'  
                                              AND CL1.Code = 'SHOWSUSR5'   
                                              AND CL1.Storerkey = s.StorerKey  
         LEFT JOIN CODELKUP CL2 WITH (NOLOCK) ON CL2.ListName = 'REPORTCFG' AND CL2.Long = 'r_dw_print_pickorder122'  
                                              AND CL2.Code = 'ShowFullLoc' AND CL2.Storerkey = s.StorerKey    
         LEFT JOIN CODELKUP CL3 WITH (NOLOCK) ON CL3.ListName = 'REPORTCFG' AND CL3.Long = 'r_dw_print_pickorder122'  
                                              AND CL3.Code = 'ShowOrdType' AND CL3.Storerkey = s.StorerKey   
         LEFT JOIN CODELKUP CL4 WITH (NOLOCK) ON CL4.ListName = 'REPORTCFG' AND CL4.Long = 'r_dw_print_pickorder122'  
                                              AND CL4.Code = 'ShowCityState' AND CL4.Storerkey = s.StorerKey                                             
         LEFT JOIN CODELKUP CL5 WITH (NOLOCK) ON CL5.ListName = 'REPORTCFG' AND CL5.Long = 'r_dw_print_pickorder122'  
      ...
      ShowEachInInnerCol,Lottable08,Lottable09,Lottable10,Lottable11,Lottable12,ORDGRP)   
      VALUES  
      ...
      @c_ShowPageNoByOrderkey, @c_ShowEachInInnerCol,@c_Lottable08,@c_Lottable09,@c_Lottable10,@c_Lottable11,@c_Lottable12,@c_OrdGrp) 
   ...
   SELECT DISTINCT OrderKey FROM #temp_pickord122  
   WHERE ORDERKEY <> ''  
   ...
   BEGIN  
      ...
      SELECT DISTINCT t.putawayzone, t.loadkey, t.orderkey, logicallocation, loc, sku, lottable01, lottable02, lottable03, lottable04, id,
                      t.Lottable08,t.Lottable09,t.Lottable10,t.Lottable11,t.Lottable12  
      FROM #temp_pickord122 t  
      ORDER BY loadkey, orderkey, Putawayzone, LogicalLocation, loc, sku, lottable01, lottable02, lottable03, lottable04, id,
               t.Lottable08,t.Lottable09,t.Lottable10,t.Lottable11,t.Lottable12  
   ...
   SET @c_SQL = N'SELECT #temp_pickord122.*, pickheader.adddate ' + CHAR(13) +   
                 'FROM #temp_pickord122, pickheader (nolock) ' + CHAR(13) +   
                 'WHERE #temp_pickord122.pickslipno = pickheader.pickheaderkey '  + CHAR(13)   
  
   IF @c_BreakByPAZone = 'Y'  
   BEGIN  
      SET @c_OrderBy = N'ORDER BY #temp_pickord122.loadkey, #temp_pickord122.orderkey, #temp_pickord122.Putawayzone, #temp_pickord122.LogicalLocation, #temp_pickord122.LOC, ' +  
                       N'#temp_pickord122.sku, #temp_pickord122.lottable01, #temp_pickord122.lottable02, #temp_pickord122.lottable03, #temp_pickord122.lottable04, #temp_pickord122.id ' +
                       N'#temp_pickord122.lottable08, #temp_pickord122.lottable09' 
   END  
   ELSE  
   BEGIN  
      SET @c_OrderBy = N'ORDER BY #temp_pickord122.loadkey, #temp_pickord122.orderkey, #temp_pickord122.logicallocation, #temp_pickord122.loc, ' + CHAR(13)  
                     +  '#temp_pickord122.sku, #temp_pickord122.lottable01, #temp_pickord122.lottable02, #temp_pickord122.lottable03, #temp_pickord122.lottable04 '  
   END  
