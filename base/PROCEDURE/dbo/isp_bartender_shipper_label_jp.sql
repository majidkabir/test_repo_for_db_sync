...
/********************************************************************************/                 
/* Copyright: IDS                                                               */                 
/* Purpose: isp_Bartender_Shipper_Label_JP                                      */                 
/*                                                                              */                 
/* Modifications log:                                                           */                 
/*                                                                              */                 
/* Date       Rev  Author     Purposes                                          */                 
/* 2015-10-16 1.0  CSCHONG    Created (SOS353874)                               */      
...
/* 2019-09-03 1.4  WLChooi    WMS-10391 - Revised VAT calculation formula (WL01)*/  
/* 2019-09-04 1.5  WLChooi    Add missing Col41 (WL02)                          */  
/* 2020-04-23 1.6  WLChooi    WMS-12988 - Modify column logic by Codelkup (WL03)*/
/* 2020-12-28 1.7  WLChooi    WMS-15863 - Add Logic for C_Company = 451 (WL04)  */
/* 2021-01-04 1.8  WLChooi    WMS-15863 - Modify Col21 for C_Company = 451(WL05)*/
/* 2021-01-19 1.9  CheeMun    INC1404857 - Join Codelkup.Storerkey              */
/********************************************************************************/                
                  
CREATE PROC [dbo].[isp_Bartender_Shipper_Label_JP]                      
...
BEGIN                      
   ...
   DECLARE                  
      ...
      @n_intFlag         INT,     
      @n_CntRec          INT,    
      @c_SQL             NVARCHAR(MAX),   --WL04  
      @c_SQLParm         NVARCHAR(MAX),   --tlting01   --WL04 
      @c_SQLSORT         NVARCHAR(MAX),   --WL04      
      @c_SQLJOIN         NVARCHAR(MAX),   --WL04 
  ...
  DECLARE @d_Trace_StartTime   DATETIME,   
      ...
      [Col60] [NVARCHAR] (80) NULL             
        ...
        IF @b_debug=1        
         BEGIN        
            PRINT 'start'          
         END        
   SET @c_SQLJOIN = + N' SELECT DISTINCT (N''Ï€Ã‡Ã†'' + Substring(ORD.C_Zip,1,3) + ''-'' + Substring(ORD.C_Zip,4,4)),ORD.c_phone1,' 
                    ...
                    + ' CASE WHEN ISNULL(ORD.Dischargeplace,'''') <> '''' THEN CONCAT(ORD.Dischargeplace , '
                    ...
                    + ' PICKDET.CaseId,(Substring(PICKDET.CaseID,1,4) + ''-'' + Substring(PICKDET.CaseID,5,4)  +''-'' +  Substring(PICKDET.CaseID,9,4)),'            --10
                    + ' CASE WHEN ISNULL(C4.Short,'''') <> '''' AND ORD.ExternOrderkey LIKE C4.Short THEN ISNULL(C4.UDF01,'''') '--WL03   --WL04
                    + '      WHEN ISNULL(C5.Short,'''') <> '''' AND ORD.C_Company = C5.Short THEN ISNULL(C5.UDF01,'''') '   --WL04
                    + ' ELSE STO.company END, '   --WL04
                    + ' CASE WHEN ISNULL(C4.Short,'''') <> '''' AND ORD.ExternOrderkey LIKE C4.Short THEN ISNULL(C4.UDF03,'''') '   --WL03   --WL04
                    + '      WHEN ISNULL(C5.Short,'''') <> '''' AND ORD.C_Company = C5.Short THEN ISNULL(C5.UDF03,'''') '   --WL04
                    + ' ELSE STO.Phone1 END, '   --WL04
                    + ' (STO.Address1 + STO.Address2),STO.Zip,STO.B_Company, '                       --15   
                    + CHAR(13) +      
                    + ' STO.B_Phone1,STO.Susr2,STO.Susr3,STO.Susr5, '
                    + ' CASE WHEN ISNULL(C4.Short,'''') <> '''' AND ORD.ExternOrderkey LIKE C4.Short THEN ISNULL(C4.UDF02,'''') '   --WL03   --WL04         --20      
                    + '      WHEN ISNULL(C5.Short,'''') <> '''' AND ORD.C_Company = C5.Short THEN ISNULL(C5.UDF02,'''') '   --WL04
                    + ' ELSE STO.Notes1 END, '   --WL04
                    + ' CASE WHEN ISNULL(C5.Short,'''') <> '''' AND ORD.C_Company = C5.Short THEN ISNULL(C5.UDF04,'''') ELSE STO.Notes2 END, '   --WL05
                    + ' '''',C1.Description,C2.long,CONVERT(VARCHAR(10), getdate(),112),'
                    + N' Substring(CONVERT(VARCHAR(10),dateadd(day,30,getdate()),112),3,2)+ N''Ïƒâ•£â”¤'' + Substring(CONVERT(VARCHAR(10),dateadd(day,30,getdate()),112),5,2) '
                    + N' + N''ÂµÂ£Ãª'' +  Substring(CONVERT(VARCHAR(10), dateadd(day, 30 , getdate()),112) , 7,2) + N''ÂµÃ¹Ã‘Î¦â”Ã¤'','
                    ...
                    +'  ISNULL(ORD.C_address1,''''),ISNULL(ORD.C_address3,''''),ISNULL(ORD.C_address4,''''),ISNULL(ORD.C_Phone2,''''),'''','   --40    --CS01       
                    + ' (ORD.C_State + Space(1) + ORD.C_City + Space(1) + ORD.C_Address2 ) AS Add2, '''','''','''','''','''','''','''','''','''', '  --50  --WL02
                    + ' '''','''','''','''','''','''','''','''','''', '''' '   --60          
                    + CHAR(13) +            
                    ...
                    + ' LEFT JOIN CODELKUP C4 WITH (NOLOCK) ON C4.Listname = ''YamatoLbl'' AND C4.Storerkey = ORD.Storerkey AND ISNULL(C4.Short,'''') = ''NBA%'' '   --WL03
                    + ' LEFT JOIN CODELKUP C5 WITH (NOLOCK) ON C5.Listname = ''YamatoLbl'' AND C5.Storerkey = ORD.Storerkey AND ISNULL(C5.Short,'''') = ORD.C_Company '   --WL04
   ...
   BEGIN
      ...
      ELSE
      BEGIN
         --SELECT @c_VATAmt = CONVERT(INT,ROUND((ORD.UserDefine05/(1+STO.percenta)) * NULLIF(STO.percenta,0),0))    --WL01                   
         SELECT @c_VATAmt = CASE WHEN ISNUMERIC(STO.PercentA) = 1 AND ISNUMERIC(ORD.UserDefine05) = 1               --WL01
                            THEN CONVERT(INT,( ORD.UserDefine05 / ((STO.PercentA / 100) + 1) ) )                    --WL01
                            ELSE 0 END                                                                              --WL01                                                                        
         FROM STORER STO WITH (NOLOCK)
         JOIN PACKDETAIL PADET WITH (NOLOCK) ON PADET.Storerkey = STO.Storerkey 
         JOIN PACKHEADER PAH WITH (NOLOCK) ON PAH.Pickslipno = PADET.Pickslipno
...
EXIT_SP:    
   ...
   EXEC isp_InsertTraceInfo   
      ...
      @c_TraceName = 'isp_Bartender_Shipper_Label_JP',  
