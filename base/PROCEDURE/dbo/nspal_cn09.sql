...
GO
/************************************************************************/    
/* Stored Procedure: nspAL_CN09                                         */    
/* Creation Date: 09-SEP-2021                                           */    
/* Copyright: LFL                                                       */    
/* Written by:                                                          */    
/*                                                                      */    
/* Purpose: WMS-17858 CN Diageo Allocation for kitting                  */
/*                                                                      */
   ...
   IF @n_StorerMinShelfLife IS NULL
      ...
      SET @c_SQL = N'   
         ...
         SELECT LOTxLOCxID.LOT,
                LOTxLOCxID.LOC,
                LOTxLOCxID.ID,
                QTYAVAILABLE = (LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED - LOTxLOCxID.QtyReplen - ISNULL(KITLLI.Qty,0))
         ...
         LEFT JOIN (SELECT KD.Lot, KD.Loc, KD.ID, SUM(KD.ExpectedQty) AS Qty
                    FROM KIT K (NOLOCK)
         ...
         AND (LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED - LOTxLOCxID.QtyReplen - ISNULL(KITLLI.Qty,0)) >= @n_UOMBase
