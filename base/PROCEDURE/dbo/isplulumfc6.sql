...
--(ML02)     AND OH.UserDefine10 IN ('170146','170149') --Mexico Order & middle east
   ...
   ;WITH MIXUCC (UCCNo, Storerkey, Orderkey) AS
   (  SELECT u.UCCNo, u.Storerkey, Orderkey = ''
      ...
      AND COUNT(DISTINCT ISNUMERIC(t.orderkey)) = 1 
   ...
   SELECT UCC_RowRef = u.UCC_RowRef, u.UCCNo, u.Storerkey, u.Sku, u.Lot, u.Loc, u.ID, u.Qty, ''
   FROM MIXUCC mu 
   JOIN UCC AS u WITH (NOLOCK) ON  u.Storerkey = mu.Storerkey AND u.UCCNo = mu.UCCNo 
   
   IF @b_Debug = 2
   BEGIN
      SELECT * FROM #TMPOD AS t 
      ...
      BEGIN 
         ...
         BEGIN
            ...
            SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0)) 
                           + ': Get PickDetailKey Failed. (ispLuLuMFC6)'
            GOTO QUIT_SP
         END
         ...
         BEGIN 
            SET @n_Continue = 3
            SET @n_Err = 88020
            SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5), @n_Err) + ': Insert Into Pickdetail table Fail. (ispLuLuMFC6)'
            GOTO QUIT_SP
         END
         ...
         BEGIN 
            SET @n_Continue = 3
            SET @n_Err = 88030
            SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5), @n_Err) + ': Update UCC table Fail.  (ispLuLuMFC6)'
            GOTO QUIT_SP
         END
...
QUIT_SP:
   ...
   BEGIN
      ...
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'ispLuLuMFC6'
