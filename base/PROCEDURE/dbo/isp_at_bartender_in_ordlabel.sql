...
/* 15-Feb-2024    1.0  SK        Created                                       */              
/******************************************************************************/              
                
CREATE PROC [dbo].[isp_AT_Bartender_IN_ORDLabel]                     
...
BEGIN                    
 ...
 -- Get the Label No.
   SELECT 
       @c_OrderNumber = ExternOrderKey
      ,@c_labelno = LabelNo 
   FROM 
      PACKHEADER (NOLOCK) PH
   JOIN PACKDETAIL (NOLOCK) PD   ON PH.PickSlipNo = PD.PickSlipNo
   JOIN ORDERS (NOLOCK) ORD ON PH.Orderkey = ORD.Orderkey  
   WHERE PH.Orderkey = @c_Sparm1 -- '0000004616'
...
EXIT_SP:  
   ...
   EXEC isp_InsertTraceInfo 
      ...
      @c_TraceName = 'isp_AT_Bartender_IN_ORDLabel',
