...
/* Written by: CHONG                                                    */
/*                                                                      */
/* Purpose:WMS-17059 [TW] PEC/QKS PreAllocateStrategy CR                */
/*                                                                      */
/* Input Parameters:                                                    */
/*                                                                      */
/* Output Parameters:                                                   */
...
BEGIN
   ...
   DECLARE @n_StorerMinShelfLife int,
           ...
           @c_nofilteremptylot2 NVARCHAR(10), 
    ...
    BEGIN
         ...
         QTYAVAILABLE = (LOT.QTY - LOT.QTYALLOCATED - LOT.QTYPICKED - LOT.QTYPREALLOCATED) - LOTXLOCXID.QtyReplen  
    ...
    BEGIN                         
         ...
         BEGIN
                   ...
                   SELECT @c_LocType = CL.Short, 
                          @c_LoadPickMethod = ISNULL(L.LoadPickMethod,''),  
                   ...
                   BEGIN
                     ...
                     SET @c_condition = dbo.fnc_RTrim(@c_Condition) + ',' + @c_Casecond 
                      ...
                      SELECT @c_condition = dbo.fnc_RTrim(@c_Condition) + @c_Orderby + ' CASE WHEN LOTATTRIBUTE.Lottable06 = ''' + RTRIM(ISNULL(@c_lottable06,'')) +''' THEN 1 ELSE 2 END, LOTATTRIBUTE.Lottable04, LOT.Lot' 
                  END
                  ...
                  BEGIN
                     ...
                     SET @c_condition = dbo.fnc_RTrim(@c_Condition) + ',' + @c_Casecond 
                      ...
                      SELECT @c_condition = dbo.fnc_RTrim(@c_Condition) + @c_Orderby +' CASE WHEN LOTATTRIBUTE.Lottable06 = ''' + RTRIM(ISNULL(@c_lottable06,'')) +''' THEN 1 ELSE 2 END, ' +
                                                                     'CASE WHEN MIN(LOC.LocationType) = ''' +RTRIM(@c_LocType)+ ''' THEN 0 ELSE 1 END,LOT.Lot'                                
               END
               ELSE
               BEGIN
                      IF EXISTS (SELECT 1 FROM CODELKUP CL (NOLOCK)
                     ...
                     SELECT @c_condition = dbo.fnc_RTrim(@c_Condition) + ' GROUP BY LOT.StorerKey, LOT.Sku, LOT.Lot, LOTATTRIBUTE.Lottable04, ISNULL(LOC.Floor,''''), LOTATTRIBUTE.Lottable06 '
                        ...
                        SELECT @c_condition = dbo.fnc_RTrim(@c_Condition) + ' HAVING SUM(LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED) - MIN(LOT.QTYPREALLOCATED) > 0  '
                        SELECT @c_condition = dbo.fnc_RTrim(@c_Condition) + @c_Orderby + ' CASE WHEN LOTATTRIBUTE.Lottable06 = ''' + RTRIM(ISNULL(@c_lottable06,'')) +''' THEN 1 ELSE 2 END, ISNULL(LOC.Floor,'''') DESC, LOT.Lot'                
                    END
                    ELSE 
                    BEGIN                              
                     SELECT @c_condition = dbo.fnc_RTrim(@c_Condition) + ' GROUP BY LOT.StorerKey, LOT.Sku, LOT.Lot, LOTATTRIBUTE.Lottable04, LOTATTRIBUTE.Lottable06 '
                        ...
                        SELECT @c_condition = dbo.fnc_RTrim(@c_Condition) + ' HAVING SUM(LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED) - MIN(LOT.QTYPREALLOCATED) > 0  '
                        SELECT @c_condition = dbo.fnc_RTrim(@c_Condition) + @c_Orderby + ' CASE WHEN LOTATTRIBUTE.Lottable06 = ''' + RTRIM(ISNULL(@c_lottable06,'')) +''' THEN 1 ELSE 2 END, LOT.Lot'            
                    END
               END
               END
               ELSE
               BEGIN
                   SELECT @c_condition = dbo.fnc_RTrim(@c_Condition) + ' GROUP BY LOT.StorerKey, LOT.Sku, LOT.Lot, LOTATTRIBUTE.Lottable04, LOTATTRIBUTE.Lottable06 '
                   SELECT @c_condition = dbo.fnc_RTrim(@c_Condition) + ' HAVING SUM(LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED) - MIN(LOT.QTYPREALLOCATED) > 0  '
                   SELECT @c_condition = dbo.fnc_RTrim(@c_Condition) + ' ORDER BY CASE WHEN LOTATTRIBUTE.Lottable06 = ''' + RTRIM(ISNULL(@c_lottable06,'')) +''' THEN 1 ELSE 2 END, LOTATTRIBUTE.Lottable04, LOT.Lot'            
               END

            EXEC (' DECLARE  PREALLOCATE_CURSOR_CANDIDATES CURSOR FAST_FORWARD READ_ONLY FOR ' +
            ' SELECT LOT.STORERKEY, LOT.SKU, LOT.LOT, ' +
            ' QTYAVAILABLE = SUM(LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED) - MIN(LOT.QTYPREALLOCATED) -MIN(LOTXLOCXID.QtyReplen) ' +    
