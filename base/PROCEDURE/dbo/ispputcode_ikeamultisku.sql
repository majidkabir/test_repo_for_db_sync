...
/************************************************************************/
/* Store procedure: ispPutCode_IKEAMultiSKU                             */
...
CREATE PROCEDURE [dbo].[ispPutCode_IKEAMultiSKU]
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         IF @b_debug = 1
         BEGIN
            SELECT @c_Reason = 'FAILED PutCode: ispPutCode_IKEAMultiSKU  SKU count = ' + CAST( @nSKUCount AS NVARCHAR(5))
      ...
      BEGIN
         IF @b_debug = 1
         BEGIN
            SELECT @c_Reason = 'PASSED PutCode: ispPutCode_IKEAMultiSKU  SKU count = ' + CAST( @nSKUCount AS NVARCHAR( 5))
            EXEC nspPTD 'nspRDTPASTD', @n_pTraceHeadKey, @c_PutawayStrategyKey, @c_PutawayStrategyLineNumber, @n_PtraceDetailKey, @c_ToLoc, @c_Reason
         END
      END
      ELSE
