...
GO
/************************************************************************/
/* Store Procedure:  isp_SCE_DL_GENERIC_IQC_RULES_100005_10             */
/* Creation Date: 25-May-2022                                           */
/* Copyright: LFL                                                       */
/* Written by: GHChan                                                   */
/*                                                                      */
/* Purpose:  Perform Lot Checking                                       */
/*                                                                      */
/*                                                                      */
...
CREATE   PROCEDURE [dbo].[isp_SCE_DL_GENERIC_IQC_RULES_100005_10] (
   @b_Debug       INT            = 0
 , @n_BatchNo     INT            = 0
 , @n_Flag        INT            = 0
...
BEGIN
   ...
   BEGIN
      ...
      AND   STG_Status                 = '1'
      AND   (FromLot IS NULL OR TRIM(FromLot) = '')
      )
      BEGIN
         BEGIN TRANSACTION;
         ...
         SET STG_Status = '3'
           , STG_ErrMsg = LTRIM(TRIM(ISNULL(STG_ErrMsg, ''))) + '/FromLot is Null'
         WHERE STG_BatchNo              = @n_BatchNo
         AND   STG_Status                 = '1'
         AND   (FromLot IS NULL OR TRIM(FromLot) = '');
         ...
         BEGIN
            ...
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5), ISNULL(@n_ErrNo, 0))
                            + ': Update record fail. (isp_SCE_DL_GENERIC_IQC_RULES_100005_10)';
            ROLLBACK;
   ...
   BEGIN
      ...
      SELECT DISTINCT TRIM(FromLot)
                    , TRIM(StorerKey)
                    , TRIM(Sku)
      FROM dbo.SCE_DL_IQC_STG WITH (NOLOCK)
      WHERE STG_BatchNo = @n_BatchNo
      AND   STG_Status    = '1'
      AND   FromLot IS NOT NULL
      AND   FromLot <> '';
   ...
   IF @b_Debug = 1
   BEGIN
      SELECT '<<SUB-SP-RULES>> - [isp_SCE_DL_GENERIC_IQC_RULES_100005_10] EXIT... ErrMsg : ' + ISNULL(TRIM(@c_ErrMsg), '');
   END;
