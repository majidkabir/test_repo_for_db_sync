...
/*          SkipPreallocation = '1'                                     */
/*          ChannelInventoryMgmt = '1'                                  */
/*          Full UCC Control                                            */
/*          Cater for if remaining qty > (AGV+DPP) Qty                  */
...
BEGIN    
   ...
   SET @c_Wavekey = @c_key1
   
   SELECT @c_WaveType = W.DispatchPiecePickMethod
   FROM WAVE W (NOLOCK)
   ...
   --Check RemainingQty > (AGV+DPP), if not, skip to next pickcode
   SET @c_SQL = N'   
      SELECT @n_ContinueAllocation = COUNT(1)
      ...
      AND @n_QtyLeftToFulfill > (LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED - LOTxLOCxID.QtyReplen + LOTxLOCxID.PendingMoveIN)
      AND LOTxLOCxID.STORERKEY = @c_StorerKey
      AND LOTxLOCxID.SKU = @c_SKU 
      AND ((LOC.LocationCategory = ''AGV'' AND LOC.LocationType = ''PICK'' )
           OR (LOC.LocationCategory = ''DPP'' AND LOC.LocationType = ''DYNPPICK'' )) ' + 
      @c_ExtraCond +
      ...
      --' ORDER BY CASE WHEN LOC.LocationCategory = ''AGV'' THEN 1 ELSE 2 END,  LA.Lottable05, LOC.LogicalLocation, LOC.LOC '
   ...
   SET @c_SQLParm =  N'@c_Facility   NVARCHAR(5),  @c_StorerKey  NVARCHAR(15), @c_SKU NVARCHAR(20), @n_QtyLeftToFulfill INT, @n_UOMBase INT, ' +
                      ...
                      '@n_ContinueAllocation INT OUTPUT'
   ...
   EXEC sp_ExecuteSQL @c_SQL, @c_SQLParm, @c_Facility, @c_StorerKey, @c_SKU, @n_QtyLeftToFulfill, @n_UOMBase, @c_Lottable01, @c_Lottable02, @c_Lottable03,
                      ...
                      @d_Lottable13, @d_Lottable14, @d_Lottable15, @n_ContinueAllocation OUTPUT

   IF @n_ContinueAllocation = 0
   BEGIN
      --Check if any AGV or DPP inventory
      SET @c_SQL = N'   
         SELECT @n_ContinueAllocation = COUNT(1)
         FROM LOTxLOCxID (NOLOCK)
         JOIN LOC (NOLOCK) ON (LOTxLOCxID.Loc = LOC.LOC)
         ...
         CASE WHEN CONVERT(NVARCHAR(8) ,@d_Lottable14 ,112) <> '19000101' AND @d_Lottable14 IS NOT NULL THEN ' AND LA.Lottable14 = RTRIM(CONVERT( NVARCHAR(20), @d_Lottable14, 106)) ' ELSE ' ' END +
         CASE WHEN CONVERT(NVARCHAR(8) ,@d_Lottable15 ,112) <> '19000101' AND @d_Lottable15 IS NOT NULL THEN ' AND LA.Lottable15 = RTRIM(CONVERT( NVARCHAR(20), @d_Lottable15, 106)) ' ELSE ' ' END +
         ' HAVING ISNULL(SUM(LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED - LOTxLOCxID.QtyReplen + LOTxLOCxID.PendingMoveIN),0) > 0'
      ...
      --Since no more AGV and DPP loc have available Qty, allocate on Bulk
      IF @n_ContinueAllocation = 0
         SET @n_ContinueAllocation = 1
   END
   ...
   SET @c_SQL = N'   
      ...
      CASE WHEN CONVERT(NVARCHAR(8) ,@d_Lottable14 ,112) <> '19000101' AND @d_Lottable14 IS NOT NULL THEN ' AND LA.Lottable14 = RTRIM(CONVERT( NVARCHAR(20), @d_Lottable14, 106)) ' ELSE ' ' END +
      CASE WHEN CONVERT(NVARCHAR(8) ,@d_Lottable15 ,112) <> '19000101' AND @d_Lottable15 IS NOT NULL THEN ' AND LA.Lottable15 = RTRIM(CONVERT( NVARCHAR(20), @d_Lottable15, 106)) ' ELSE ' ' END +
      ' ORDER BY CASE WHEN LOC.LocationHandling = ''2'' THEN 1 WHEN LOC.LocationHandling = ''1'' THEN 2 ELSE 3 END, LA.Lottable05, LOC.LogicalLocation, LOC.LOC '
