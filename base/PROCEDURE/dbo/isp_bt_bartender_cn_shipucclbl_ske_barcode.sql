...
/********************************************************************************/                   
/* Copyright: LFL                                                               */                   
/* Purpose: isp_BT_Bartender_CN_SHIPUCCLBL_SKE_Barcode                          */                   
/*                                                                              */                   
/* Modifications log:                                                           */                   
/*                                                                              */                   
...
CREATE PROC [dbo].[isp_BT_Bartender_CN_SHIPUCCLBL_SKE_Barcode]                        
...
BEGIN                        
  ...
  DECLARE  @d_Trace_StartTime   DATETIME,     
   ...
   SET @n_intFlag = 1    
   SET @c_ExtraSQL = ''  
   SET @c_JoinStatement = ' JOIN ORDERS OH (NOLOCK) ON OH.ORDERKEY = PH.ORDERKEY '  
   SET @c_IsConso = 'N'  
...
-- SET RowNo = 0               
   ...
   CREATE TABLE [#Result] (               
     ...
     BEGIN  
        SELECT @n_Count = COUNT(1)  
        FROM PACKHEADER PH (NOLOCK)  
        JOIN Loadplan LP (NOLOCK) ON LP.Loadkey = PH.Loadkey  
        WHERE PH.Pickslipno = @c_Sparm01  
  
        IF @n_Count > 0  
        BEGIN  
           SET @c_IsConso = 'Y'  
           SET @c_JoinStatement = ' JOIN LOADPLANDETAIL LPD (NOLOCK) ON LPD.Loadkey = PH.Loadkey ' + CHAR(13) +   
                                  ' JOIN ORDERS OH (NOLOCK) ON OH.ORDERKEY = LPD.ORDERKEY '  
        END  
     END  
  
     IF @c_IsConso NOT IN ('Y','N')  
     BEGIN  
       GOTO EXIT_SP  
     END  
     ...
     SET @c_SQLJOIN = ' SELECT DISTINCT ' + CHAR(13)  
                    ...
                    + @c_JoinStatement  
                    + ' JOIN CODELKUP CL (NOLOCK) ON CL.LISTNAME = ''skeprintTC'' AND CL.SHORT = OH.CONSIGNEEKEY '  
...
EXIT_SP:      
   ...
   --   @c_TraceName = 'isp_BT_Bartender_CN_SHIPUCCLBL_SKE_Barcode',    
