...
/************************************************************************/
/* Trigger: isp_Ecom_GetPackTaskOrderStatus                             */
...
/* 23-AUG-2016 Wan01    1.1   Fixed. Include Not packed sku for SO      */  
/* 08-Sep-2016 TLTING   1.2   Performance tune                          */ 
/* 21-SEP-2016 Wan02    1.2   Performance Tune                          */ 
/* 25-OCT-2016 Wan03    1.3   Update PACKTASKDETAIl.QtyAllocated        */
/* 13-OCT-2017 Wan04    1.4   Performance tune                          */  
/* 13-OCT-2017 Wan05    1.5   Fix Issue - Check Max OD editdate against */ 
/*                            PTD adddate for 2 same OD sku             */
/* 30-AUG-2018 Wan06    1.6   Performance tune                          */ 
/* 24-JUN-2019 Wan07    1.7   Performance Tune                          */ 
/* 16-AUG-2019 Wan08    1.8   Performance Tune                          */
/* 30-JUL-2020 NJOW01   1.9   Fix status update X condition             */
/* 10-AUG-2020 Wan09    2.0   WMS-14554 - [CN]Doterra Ecompacking CR    */
/* 04-DEC-2020 Wan10    2.1   WMS-15700 - MY ADIDAS EXCEED_ECOM PACKING */
/*                            TO HANDLE SHORT PACKPICK                  */
/* 04-MAR-2021 Wan11    2.2   WMS-16390 - [CN] NIKE_O2_Ecompacking_Check*/
/*                            _Pickdetail_status_CR                     */
/* 20-JUL-2023 Wan12    2.3   WMS-23156 - [CN] GBMAX_Ecompack_Show Cancel*/
/*                            qty_CR                                    */
/* 10-AUG-2023 Wan13    2.4   Fixed NikeCN Issue that Pending Pick Order*/
/*                            not Created into PACKTASKDETAIL           */
/************************************************************************/
CREATE   PROC [dbo].[isp_Ecom_GetPackTaskOrderStatus] 
      ...
      BEGIN TRAN
         BEGIN TRY
            ...
            BEGIN
               ...
               SELECT  O2P.TaskBatchNo                                                                --(Wan12) - START   
                     , O2P.LogicalName
                     , O2P.Orderkey
                     , O2P.Storerkey
                     , O2P.Sku
               ...
               FROM (         
                  SELECT 
                       RowID = ROW_NUMBER() OVER (ORDER BY CASE WHEN PS.Status = 'X' THEN 9                 --(Wan12)
                                                                WHEN PD.[Status]= 4 THEN 8 
                                                                ELSE 0 END
                                                           , PD.Orderkey)  
                     , SinglePackOrd =  CASE WHEN PS.Status IN ('0','P' ) AND OD.QtyAllocated > 0 THEN 1    --(Wan13)(Wan12)
                                             WHEN PS.Status = 'X' THEN 1   
                                             ELSE 0
                                             END                                                                             
                     , PS.TaskBatchNo                 --(Wan11)
                     , PS.LogicalName
                     , OD.Orderkey
                     , OD.Storerkey
                     , OD.Sku
                  ...
                  FROM @TMP_PTSTATUS PS   
                  JOIN ORDERDETAIL   OD WITH (NOLOCK) ON (PS.Orderkey = OD.Orderkey)
                  LEFT OUTER JOIN PICKDETAIL PD WITH (NOLOCK) ON OD.Orderkey = PD.Orderkey               --(Wan12)
                                                              AND OD.OrderLineNumber = PD.OrderLineNumber--(Wan11) 2021-05-06
                  WHERE PS.TaskBatchNo = @c_TaskBatchNo
                  AND NOT EXISTS ( SELECT 1 FROM PACKTASKDETAIL PTD WITH (NOLOCK)  
                                   WHERE PS.TaskBatchNo = PTD.TaskBatchNo )
                  AND PS.RowRef > 0  
                  --AND OD.QtyAllocated > 0             --(Wan09)                                        --(Wan12)
                  --ORDER BY CASE WHEN PD.[Status]= 4 THEN 9 ELSE 0 END   --(Wan11) 2021-05-06           --(Wan12) 
                         --, PD.Orderkey                                  --(Wan11) 2021-05-06           --(Wan12)
                       ) O2P  
                  WHERE O2P.SinglePackOrd = 1
                  ORDER BY O2P.RowID               
            END
