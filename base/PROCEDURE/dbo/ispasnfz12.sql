...
BEGIN  
     ...
     -- tlting01
		IF EXISTS (SELECT 1 FROM RECEIPTDETAIL (NOLOCK) WHERE RECEIPTKEY = @cReceiptkey AND ReceiptLineNumber <> @cReceiptLineNumber AND ISNULL(EXTERNRECEIPTKEY,'') <> '' AND SKU = @cSKU)
 		...
 		ELSE
			BEGIN
               -- tlting01
				SET @nExternReceiptkey = (SELECT TOP 1 ExternReceiptkey FROM RECEIPTDETAIL (NOLOCK) WHERE RECEIPTKEY = @cReceiptkey AND ISNULL(EXTERNRECEIPTKEY,'') <> '' )
				UPDATE RECEIPTDETAIL SET EXTERNRECEIPTKEY = @nExternReceiptkey WHERE RECEIPTKEY = @cReceiptkey AND ReceiptLineNumber = @cReceiptLineNumber
			END
			...
			SELECT @n_err = @@ERROR
			   ...
			   SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Update ExtenReceiptKey Failed! (ispASNFZ12)' + ' ( '
   ...
   BEGIN
	  ...
	  SELECT @nLottable05 = OPTION1 FROM STORERCONFIG (NOLOCK) WHERE STORERKEY = @nStorerKey AND CONFIGKEY = 'PreFinalizeReceiptSP' AND SVALUE = 'ispASNFZ12'

   	  Declare RT_CUR Cursor LOCAL Fast_forward Read_only For
	  ...
	  BEGIN
		...
		SELECT @n_err = @@ERROR
			   ...
			   SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Update Lottable05 Failed! (ispASNFZ12)' + ' ( '
                                  + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
			   	 GOTO QUIT_SP
			   END
   ...
   BEGIN
      ...
      BEGIN
         ROLLBACK TRAN
      END
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'ispASNFZ12'
