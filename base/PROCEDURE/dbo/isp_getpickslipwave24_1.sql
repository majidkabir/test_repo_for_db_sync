...
/************************************************************************/
/* Stored Proc: isp_GetPickSlipWave24_1                                 */
...
/*                                                                      */
/* Called By: r_dw_print_wave_pickslip_24_1                             */  
/*          :                                                           */  
/* GitLab Version: 1.0                                                  */  
/*                                                                      */  
/* Version: 7.0                                                         */  
...
/************************************************************************/   
CREATE PROC [dbo].[isp_GetPickSlipWave24_1]    
...
--(Wan03) - START      
  IF OBJECT_ID('tempdb..#TMP_WAVORD','u') IS NOT NULL      
   ...
   --NJOW01 S       
   IF OBJECT_ID('tempdb..#TMP_ALLWAVE','u') IS NOT NULL      
   BEGIN      
      DROP TABLE #TMP_ALLWAVE;      
   END           
       
   CREATE TABLE #TMP_ALLWAVE        
      (  Wavekey  NVARCHAR(10) NOT NULL   DEFAULT ('')        
      ,  AddDate  DATETIME     NULL               
      ,  EditDate DATETIME     NULL               
      )          
          
   CREATE INDEX IDX_wkey ON #TMP_ALLWAVE (Wavekey)    
   CREATE INDEX IDX_wadddate ON #TMP_ALLWAVE (AddDate)    
             
   INSERT INTO #TMP_ALLWAVE (Wavekey, AddDate, EditDate)    
   SELECT TOP 500 W.Wavekey, W.AddDate, W.EditDate    
   FROM WAVE W (NOLOCK)    
   JOIN WAVEDETAIL WD (NOLOCK) ON W.Wavekey = WD.Wavekey    
   JOIN ORDERS O (NOLOCK) ON WD.Orderkey = O.Orderkey    
   WHERE O.Storerkey = @c_Storerkey    
   AND O.Status <> '9'    
   --AND W.Status = '1'          --Wan04   
   AND   W.TMReleaseFlag = 'Y'   --Wan04
   GROUP BY W.Wavekey, W.AddDate, W.EditDate    
   ORDER BY W.AddDate DESC, W.Wavekey DESC       
...
--LEFT JOIN Taskdetail TD WITH (NOLOCK) ON TD.wavekey=WH.wavekey             --(CS01)          
  ...
  --select * from #TMP_WaveSeq                                                   
   ...
   SELECT @n_SumQtyPerWave = SUM(PD.Qty)
   ...
   JOIN REFKEYLOOKUP RL WITH (NOLOCK) ON (PD.PickDetailKey = RL.PickDetailkey)     
   JOIN PICKHEADER PH WITH (NOLOCK) ON (RL.PickSlipNo = PH.PickHeaderkey) 
   WHERE PH.Wavekey = @c_Wavekey AND PH.Pickheaderkey = @c_PickSlipNo
   ...
   SELECT PH.Wavekey    
         ...
         ,SumQtyPerWave = @n_SumQtyPerWave
         ,SumQtyPerZone = @n_SumQtyPerZone
         ,Shipperkey = (SELECT MAX(ISNULL(Shipperkey,'''')) FROM ORDERS (NOLOCK) WHERE UserDefine09 = @c_Wavekey)    
   FROM PICKDETAIL PD   WITH (NOLOCK)     
