...
/************************************************************************/  
/* Stored Procedure: ispGenEOrderReplen02                               */  
/* Creation Date: 29-MAR-2022                                           */  
/* Copyright: LFL                                                       */  
/* Written by:                                                          */  
/*                                                                      */  
/* Purpose: WMS-19284 CN Loreal release Ecom replenishment              */  
/*                                                                      */  
...
/* 27-Oct-2022  NJOW01   1.1  Fix locationtype,insert notes to pickdetail*/
/* 11-Nov-2022  NJOW02   1.2  Performance tuning                        */
/************************************************************************/   

CREATE PROCEDURE [dbo].[ispGenEOrderReplen02]  
...
BEGIN  
   ...
   --NJOW02
   DECLARE @n_QtyAvaliable_tmp      INT = 0,
           @c_FromLOT_tmp           NVARCHAR(10) = '',
           @c_FromLOC_tmp           NVARCHAR(10) = '',
           @c_FromID_tmp            NVARCHAR(20) = '',
           @c_DuplicateRunning      NVARCHAR(1) = 'N'
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            SELECT @n_continue = 3
            SELECT @n_err = 78301
            SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Insert into Temp Table @tloadkey Failed! (ispGenEOrderReplen02)'        
            GOTO EXIT_SP
         END       
      END
      ELSE
   ...
   BEGIN
      SET @n_continue = 3
      SET @n_err = 78329
      SET @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': nspGetRight Failed! (ispGenEOrderReplen02)'
      GOTO EXIT_SP      
   END
   ...
   BEGIN    
      ...
      BEGIN
         ...
         BEGIN
            SELECT @n_continue = 3
            SELECT @n_err = 78303
            SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Insert into Temp Table @tTaskBatchNo Failed! (ispGenEOrderReplen02)'
            GOTO EXIT_SP         
         END
      END
      ELSE 
      ...
      --BEGIN
         ...
         BEGIN
            ...
            SET @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Replenishment are generated for the selected Task.'
                         +' Generate Replenishment Abort.(ispGenEOrderReplen02)'
            GOTO EXIT_SP  
         END
      --END
   ...
   JOIN @tloadkey LS ON LP.Loadkey = LS.Loadkey
   WHERE LP.ProcessFlag = 'R'
   AND DATEDIFF(minute, LP.EditDate ,GETDATE()) < 90
   ORDER BY LP.Loadkey
   
   IF ISNULL(@c_Loadkey,'') <> ''
   BEGIN
   	  SET @c_DuplicateRunning = 'Y'
      SELECT @n_continue = 3
      SELECT @n_err = 78308
      SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Generate replenishment already in process. Loadkey: ' + @c_Loadkey + '. LoadPlan.PROCESSFLAG=R (ispGenEOrderReplen02)'
      GOTO EXIT_SP      
   END
   ...
   BEGIN
      SELECT @n_continue = 3
      SELECT @n_err = 78307
      SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': nspGetRight Failed! (ispGenEOrderReplen02)'
      GOTO EXIT_SP      
   END
   ...
   BEGIN
      SELECT @n_continue = 3
      SELECT @n_err = 78308
      SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': nspGetRight ForceAllocLottable Failed! (ispGenEOrderReplen02)'
      GOTO EXIT_SP      
   END
         ...
         BEGIN         	
            ...
            BEGIN            	  
            	 ...
            	 BEGIN
                  ...
                  BEGIN
                     SELECT @n_continue = 3
                     SELECT @n_err = 78315
                     SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Update PickDetail Failed! (ispGenEOrderReplen02)'
                  END 
            	 ...
            	 BEGIN
                  ...
                  BEGIN
                     SELECT @n_continue = 3
                     SELECT @n_err = 78313
                     SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': INSERT PickDetail Failed! (ispGenEOrderReplen02)'
                  END                   
            ...
            BEGIN
               ...
               BEGIN
                  SELECT @n_continue = 3
                  SELECT @n_err = 78315
                  SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Update UCC Failed! (ispGenEOrderReplen02)'
               END                               
            END
         ...
         BEGIN   
            ...
            BEGIN                            
               ...
               BEGIN
                  SELECT @n_continue = 3
                  SELECT @n_err = 78309
                  SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Updating PickDetail Failed! (ispGenEOrderReplen02)'
                  GOTO EXIT_SP         
               END
                     ...
                     BEGIN
                        ...
                        BEGIN
                           ...
                           BEGIN
                              SELECT @n_continue = 3
                              SELECT @n_err = 78310
                              SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Updating LOTxLOCxID Failed! (ispGenEOrderReplen02)'
                              GOTO EXIT_SP         
                           END
                        END                                                                           
                     END
   ...
   --NJOW02 S
   SET @c_PickDetailKey = ''
   DECLARE CUR_PICKDOREPLEN CURSOR LOCAL FAST_FORWARD READ_ONLY FOR  
      SELECT p.PickDetailKey
      FROM PICKDETAIL AS p WITH (NOLOCK)  
      JOIN SKUxLOC AS  SL WITH (NOLOCK) 
               ON SL.Storerkey = P.Storerkey 
      ...
      AND   P.UOM = '6' 
      AND   P.DoReplenish = 'N'
      AND EXISTS ( SELECT 1 FROM @tTaskOrders AS PT WHERE PT.OrderKey = P.OrderKey  ) 
      ORDER BY P.PickDetailKey
      
   OPEN CUR_PICKDOREPLEN  
  
   FETCH NEXT FROM CUR_PICKDOREPLEN INTO @c_Pickdetailkey   
   ...
   BEGIN     
      ...
      BEGIN
         ...
         SELECT @n_err = 78311
         SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Update PickDetail Failed! (ispGenEOrderReplen02)'
         CLOSE CUR_PICKDOREPLEN                 
         DEALLOCATE CUR_PICKDOREPLEN            
         GOTO EXIT_SP         
      END      
   ...
   BEGIN
      ...
      FROM PICKDETAIL AS p WITH (NOLOCK)        
      --JOIN Orderdetail as OD (NOLOCK) on OD.orderkey = p.orderkey and od.orderlinenumber = p.orderlinenumber      --NJOW02 removed       
      JOIN SKUxLOC AS  SL WITH (NOLOCK) 
               ON SL.Storerkey = P.Storerkey 
              AND SL.Sku = P.Sku   
              AND SL.Loc = P.Loc 
      ...
      AND EXISTS ( SELECT 1 FROM @tTaskOrders AS PT WHERE PT.OrderKey = P.OrderKey  )  --NJOW02
      ...
      BEGIN
         ...
         --NJOW02
         SELECT @n_QtyAvaliable_tmp = 0, @c_FromLOT_tmp = '', @c_FromLOC_tmp = '', @c_FromID_tmp = ''
         SELECT TOP 1 @n_QtyAvaliable_tmp =  lli.Qty - lli.QtyAllocated - lli.QtyPicked, 
                      @c_FromLOT_tmp = lli.Lot,
                      @c_FromLOC_tmp = lli.Loc, 
                      @c_FromID_tmp  = lli.ID   
         FROM LOTxLOCxID AS lli WITH(NOLOCK) 
         ...
         IF ISNULL(@n_QtyAvaliable_tmp,0) = 0                       
         BEGIN               
         	  GOTO NOT_AVAILABLE
         END                                     
         
         --find same lot,loc,id
         IF @c_FromLOT_tmp = @c_LOT AND @c_FromLOC_tmp = @c_LOC AND @c_FromID_tmp = @c_ID --NJOW02
         BEGIN
         	  SELECT @n_QtyAvaliable = @n_QtyAvaliable_tmp, @c_FromLOT = @c_FromLOT_tmp, @c_FromLOC = @c_FromLOC_tmp, @c_FromID = @c_FromID_tmp
         END
         ELSE
         BEGIN
            SELECT TOP 1 
         ...
         BEGIN
            ...
            IF @c_FromLOT_tmp = @c_LOT AND @c_FromLOC_tmp = @c_LOC --NJOW02
            BEGIN
            	  SELECT @n_QtyAvaliable = @n_QtyAvaliable_tmp, @c_FromLOT = @c_FromLOT_tmp, @c_FromLOC = @c_FromLOC_tmp, @c_FromID = @c_FromID_tmp
            END
            ELSE
         ...
         BEGIN
            ...
            BEGIN -- Split PickDetail 
               ...
               --- change optimizecop to NULL. select from @c_Lot,@c_Loc, @c_DoReplenish, @c_UOM, @n_splitQty
               INSERT INTO dbo.PICKDETAIL
                  (
                   PickDetailKey    ,CaseID           ,PickHeaderKey
                  ,OrderKey         ,OrderLineNumber  ,Lot
         ...
         BEGIN -- IF @n_QtyAvaliable = 0 
            ...
            BEGIN
               ...
               ELSE IF @n_QtyAllocPick = @n_SwapQty 
               BEGIN
      ...
      --FETCH NEXT FROM CUR_SWAP_PICKDETAIL INTO @c_PickDetailKey, @n_QtyAllocPick, @c_LOT, @c_LOC, @c_ID, @c_UOM, @c_Facility, @c_StorerKey, @c_SKU
   END -- WHILE LOOP
   --CLOSE CUR_SWAP_PICKDETAIL
   --DEALLOCATE CUR_SWAP_PICKDETAIL
   ...
   AND   P.UOM = '7'  
   AND   P.DoReplenish = 'N'
   GROUP BY L.Facility, P.Storerkey, P.Sku, P.Loc, SL.QtyPicked, SL.Qty, SL.QtyLocationLimit, SKU.Busr9
                         
   OPEN CUR_OVERALLOCATE_LOC
   
   FETCH NEXT FROM CUR_OVERALLOCATE_LOC INTO @c_Facility, @c_StorerKey ,@c_SKU, @c_LOC, @n_ReplenQty, @n_LocCapacity, @c_Busr9
      ...
      BEGIN          
            ...
            BEGIN
               ...
               BEGIN
                  ...
                  BEGIN
                     SELECT @n_continue = 3
                     SELECT @n_err = 78325
                     SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': nspg_GetKey Failed! (ispGenEOrderReplen02)'
                     GOTO EXIT_SP         
                  END
                  ...
                  BEGIN
                     SELECT @n_continue = 3
                     SELECT @n_err = 78326
                     SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Insert into Replenishment Failed! (ispGenEOrderReplen02)'
                     GOTO EXIT_SP         
                  END                         
         ...
         BEGIN
            ...
            BEGIN
               ...
               --AND (SL.LocationType NOT IN ('PICK','CASE') ) 
               AND L.Facility = @c_Facility
               AND LLI.Qty - (LLI.QtyPicked + LLI.QtyAllocated + ISNULL(LLI.QtyReplen, 0)) > 0 --= @n_CaseCnt              
               AND LLI.Qty - (LLI.QtyPicked + LLI.QtyAllocated + ISNULL(LLI.QtyReplen, 0)) >= @n_ReplenQty              
               ORDER BY LLI.Qty - (LLI.QtyPicked + LLI.QtyAllocated + ISNULL(LLI.QtyReplen, 0)), L.LogicalLocation, L.Loc, L.LocationHandling, LLI.Qty
            ...
            BEGIN
               ...
               BEGIN               
                  ...
                  --AND L.LocationType NOT IN ('PICK') 
                  --AND (SL.LocationType NOT IN ('PICK','CASE') ) 
                  AND L.LocationType = 'OTHER' --NJOW01
                  AND L.Facility = @c_Facility
                  AND LLI.StorerKey = @c_StorerKey 
                  AND LLI.Sku = @c_SKU
                  AND LLI.Qty - (LLI.QtyPicked + LLI.QtyAllocated + ISNULL(LLI.QtyReplen, 0)) > 0 --= @n_CaseCnt                
            ...
            BEGIN
               ...
               IF @cFastPickLoc = 'N' AND @n_QtyAvailable >= @n_CaseCnt 
               BEGIN
               ...
               IF @n_QtyAvailable >= @n_FullCasePickQty AND @c_Busr9 <> 'P'                     
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            SELECT @n_continue = 3
            SELECT @n_err = 78328
            SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Updating PackTask Failed! (ispGenEOrderReplen02)'
            GOTO EXIT_SP         
         END
   ...
   BEGIN
       ...
       BEGIN
           ...
           END
       END
       EXECUTE nsp_logerror @n_Err, @c_ErrMsg, 'ispGenEOrderReplen02'  --NJOW02
   END
   ELSE
   BEGIN
      WHILE @@TRANCOUNT > @n_StartTCnt
