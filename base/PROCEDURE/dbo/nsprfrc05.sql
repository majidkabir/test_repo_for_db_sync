...
/****** Object:  Stored Procedure dbo.nspRFRC05    Script Date: 18/10/2000 11:56:51 ******/
CREATE PROC    [dbo].[nspRFRC05]
...
BEGIN
  IF NOT EXISTS(SELECT 1 FROM RECEIPT(nolock)
		WHERE RECEIPTKEY = @c_asn and STORERKEY = @c_storer)
  BEGIN
    SELECT @n_continue = 3
    SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err=95001   -- Should Be Set To The SQL Errmessage but I don't know how to do so.
    SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": ASN# is invalid. (nspRFRC05)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
  END  
END

IF @n_continue=1 OR @n_continue=2
BEGIN
  SELECT @c_asnstatus = status from RECEIPT (NOLOCK) WHERE RECEIPTKEY = @c_asn
			                             AND STORERKEY = @c_storer
  IF dbo.fnc_LTrim(dbo.fnc_RTrim(@c_asnstatus)) <> '0' 
  BEGIN        
    SELECT @n_continue = 3
    SELECT @n_err=95002
    SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": ASN Already RECEIVED (nspRFRC05)"
  END      
END 
...
BEGIN
  ...
  BEGIN
    ...
    END
  END
  execute nsp_logerror @n_err, @c_errmsg, "nspRFRC05"
