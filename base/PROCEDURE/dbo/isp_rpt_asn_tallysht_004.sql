...
/***************************************************************************/
/* Stored Procedure: isp_RPT_ASN_TALLYSHT_004                              */
...
/* Purpose: WMS-18712                                                      */
/*                                                                         */
/* Called By: RPT_ASN_TALLYSHT_004                                         */
/*                                                                         */
/* GitLab Version: 1.0                                                     */
/*                                                                         */
/* Version: 1.0                                                            */
...
/***************************************************************************/
CREATE PROC [dbo].[isp_RPT_ASN_TALLYSHT_004]
...
BEGIN
     ...
     ,ReceivedEA = CASE PACK.InnerPack WHEN 0
            ...
            CASE PACK.Casecnt WHEN 0 THEN (cast(RECEIPTDETAIL.BeforeReceivedQty as int) % cast(PACK.InnerPack as int))
                              ELSE (cast(RECEIPTDETAIL.BeforeReceivedQty as int) % cast(PACK.InnerPack as int)) END
                                        END
     ,ISNULL(RC.ShowSkuBarcode,0) AS ShowSkuBarcode
     ,RECEIPTDETAIL.ToID
   ...
   --LEFT OUTER JOIN CODELKUP WITH(NOLOCK) ON CODELKUP.LISTNAME = 'CONTAINERT' AND CODELKUP.CODE = Receipt.ContainerType
