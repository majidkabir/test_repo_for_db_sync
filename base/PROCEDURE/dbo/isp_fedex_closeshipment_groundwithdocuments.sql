...
GO
/************************************************************************/
/* SP: isp_FedEx_CloseShipment_GroundWithDocuments                      */
/* Creation Date: 01 Nov 2011                                           */
/* Copyright: IDS                                                       */
/* Written by: Chee Jun Yan                                             */
/*                                                                      */
/* Purpose: Send Ground Close Shipment Web Service Request to FedEx     */
/*          (SOS#227408)                                                */ 
/*                                                                      */
/* Usage:                                                               */
/*                                                                      */
...
/*                                isp_CloseShipment_GroundWithDocuments */
/*                                (Chee01)                              */
/************************************************************************/

CREATE PROC [dbo].[isp_FedEx_CloseShipment_GroundWithDocuments]
(
...
BEGIN
   ...
   SET @n_CloseDocumentType = 1 -- COD_REPORT = 0, MANIFEST = 1, MULTIWEIGHT_REPORT = 2, OP_950 = 3

   EXEC [master].[dbo].[isp_CloseShipment_GroundWithDocuments]
   ...
   BEGIN
      ...
      ELSE
      BEGIN
         SET @c_errmsg = 'Error: '+ CAST(@n_err AS NVARCHAR(11)) + ' occurred while executing [master].[dbo].[isp_CloseShipment_GroundWithDocuments].' 
      END   
   ...
   FROM @X_Message.nodes('/GroundCloseDocumentsReply') AS R(nref)
   ...
   BEGIN
      ...
      FROM @x_Message.nodes('/GroundCloseDocumentsReply/ns:Notifications') AS R(nref)
      WHERE nref.value('ns:Severity[1]', 'VARCHAR(10)') = 'ERROR' 
      OR nref.value('ns:Severity[1]', 'VARCHAR(10)') ='FAILURE'
      OR nref.value('ns:Severity[1]', 'VARCHAR(10)') ='WARNING'
...
--    FROM @x_Message.nodes('/GroundCloseDocumentsReply/ns:CloseDocuments/ns:Parts') AS R(nref)
-- END
