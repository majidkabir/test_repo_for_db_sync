...
GO
/************************************************************************/  
/* Stored Proc: ispPreAllocateWaveProcessing                            */  
/* Creation Date: 04-Oct-2012                                           */  
/* Copyright: IDS                                                       */  
/* Written by:                                                          */  
/*                                                                      */  
/* Purpose: Pre-Allocate Wave Processing                                */  
/*                                                                      */  
/* Called By: nspWaveProcessing                                         */  
/*                                                                      */  
/* PVCS Version: 2.8                                                    */  
/*                                                                      */  
/* Version: 1.0                                                         */  
...
/* 31-Oct-2014  Shong   1.1   Extend UOM Variable to 10 chars           */  
/* 24-Apr-2014  Shong   1.2   Added 10 Lottables                        */  
/* 20-05-2014   NJOW01  1.3   256920-Sort order by Priority, orderkey   */      
/* 13-11-2014   NJOW02  1.4   distribute preallocated qty to order sort */  
/*                            by uom pallet,case,innter & each.         */  
/* 21-AUG-2015  Wan01   1.5   Fixed - @c_aLottable05 = @d_aLottable05   */  
/*                            Fixed - @c_aLottable13 - @c_aLottable15   */  
/*                            TO      @d_aLottable13 - @d_aLottable15   */  
/* 20-Sep-2016  TLTING  1.6   Remove SetROWCOUNT                        */  
/* 23-Nov-2016  NJOW03  1.7   WMS-657 able to group and send custom     */  
/*                            custom field to pickcode for filtering    */  
/* 01-NOV-2016  Wan02   1.7   WMS-669 - PMS Allocation Logic            */  
/* 24-Feb-2016  Wan03   1.8   WMS-1106 - CN-Nike SDC WMS Allocation     */  
/*                            Strategy                                  */  
/* 17-Apr-2017  TLTING  1.8   Bug fix (NOLOCK)                          */  
/* 03-May-2017  NJOW03  1.9   Fix to prevent incorrect lot qty rtn from */  
/*                            pickcode                                  */  
/* 01-Nov-2017  NJOW04  2.0   WMS-3290 Get casecnt from lottable        */  
/* 24-Nov-2017  NJOW05  2.1   Check Facility Lot Qty Available          */  
/* 01-Dec-2017  JHTAN01 2.2   INC0060098 Cannot allocate, @c_OrdUOM     */  
/*                            should be NVARCHAR(10)                    */  
/* 20-Apr-2018  SWT01   2.3   Channel Management Check Qty Available    */  
/* 15-Dec-2018 TLTING01 2.4   MIssing NOLOCK& dynamic SQL cache         */  
/* 16-Dec-2018 CHEEMUN  2.5   INC0509111 - Enhance Dynamic SQL          */  
/* 10-Sep-2019  NJOW06  2.6   WMS-10479 Allocation based on orderdetail */
/*                            packkey by config.                        */ 
/* 23-JUL-2019  Wan04   2.7   ChannelInventoryMgmt use nspGetRight2     */ 
/* 08-OCT-2019  Wan05   2.8   WMS - 9914 [MY] JDSPORTSMY - Channel      */
/*                            Inventory Ignore QtyOnHold - CR           */ 
/* 08-OCT-2019  Wan06   2.8   Fixed to Get Channel If there is candidate*/
/*                            in Cursor                                 */ 
/* 16-JAN-202   Wan07   2.9   Performance tune.Use Index to search instead*/
/*                            of ORDERS.Userdefine09 = @c_Wavekey       */
/* 08-Jan-2020  NJOW07  2.9 WMS-10420 add strategykey parameter         */ 
/* 12-Feb-2020  Wan08   2.9   SQLBindParm. Create Temp table to Store   */
...
DECLARE   
   @c_OtherParms                NVARCHAR(200),  -- For CDC  
  ...
  ,@c_AllocateByOrderPackkey NVARCHAR(30) --NJOW06
  ,@n_ChannelHoldQty         INT          --NJOW08
  
--NJOW03  
DECLARE     
   @c_Storerkey                 NVARCHAR(15),   
   @c_Facility                  NVARCHAR(10),   
   @c_WaveConsoAllocationOParms NVARCHAR(10),   
...
BEGIN  
   IF (LTRIM(RTRIM(@c_WaveKey)) IS NULL OR LTRIM(RTRIM(@c_WaveKey))='' )   
   BEGIN  
      SELECT @n_continue = 3  
      SELECT @n_err = 78300  
      SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Invalid Parameters Passed (ispPreAllocateWaveProcessing)'  
   END  
END -- @n_continue =1 or @n_continue = 2  
  
IF @n_continue = 1 OR @n_continue =2  
BEGIN  
   CREATE TABLE #OPWAVES (  
   ...
   BEGIN  
      SELECT @n_continue = 3  
      SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err = 78301   -- Should Be Set To The SQL Errmessage but I don't know how to do so.  
      SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Creation Of Temp Table Failed (ispPreAllocateWaveProcessing)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '  
   END  
END  
...
IF @n_continue=1 OR @n_continue=2   
BEGIN  
    SELECT TOP 1 @c_Storerkey = ORDERS.Storerkey,                    --Wan07  
                 @c_Facility = ORDERS.Facility                       --Wan07  
    FROM ORDERS(NOLOCK)   
    JOIN WAVEDETAIL (NOLOCK) ON ORDERS.Orderkey = WAVEDETAIL.Orderkey--Wan07     
    WHERE WAVEDETAIL.Wavekey = @c_Wavekey                            --Wan07 Use Index to search instead of ORDERS.Userdefine09 = @c_Wavekey 
   ...
   NULL,            -- Sku  
   'WaveConsoAllocationOParms',         -- Configkey  
   @b_Success    OUTPUT,  
   @c_WaveConsoAllocationOParms OUTPUT,  
   @n_err        OUTPUT,  
   @c_errmsg     OUTPUT,   
   ...
   IF @b_Success <> 1  
   BEGIN  
      SELECT @n_continue = 3, @c_errmsg = 'ispPreAllocateWaveProcessing' + RTRIM(@c_errmsg)  
   END     
   ...
   Begin  
      ...
      If @b_success <> 1  
      Begin  
         Select @n_continue = 3, @c_ErrMsg = 'ispPreAllocateWaveProcessing:' + ISNULL(RTRIM(@c_ErrMsg),'')  
      END  
   END                      
...
BEGIN  
   ...
   BEGIN  
      SELECT @n_continue = 3  
      SELECT @c_errmsg = 'ispPreAllocateWaveProcessing : ' + RTRIM(@c_errmsg)  
   END  
   ELSE  
   BEGIN  
...
IF @n_continue=1 OR @n_continue=2  
BEGIN  
   IF LTRIM(RTRIM(@c_WaveKey)) IS NOT NULL  
   BEGIN  
      SELECT @b_Success = 0  
      EXECUTE nspGetRight NULL,  -- Facility  
      NULL,                      -- Storer  
      ...
      SELECT @n_Cnt = COUNT(1) FROM #OPWAVES  
      SELECT @n_err = @@ERROR, @n_cnt = @@ROWCOUNT  
      IF @n_err <> 0  
      BEGIN  
         SELECT @n_continue = 3  
         SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err = 78303   -- Should Be Set To The SQL Errmessage but I don't know how to do so.  
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Creation Of Temp Table Failed (ispPreAllocateWaveProcessing)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '  
      END  
      ELSE IF @n_cnt = 0      
      BEGIN    
         --tlting01   
         IF NOT EXISTS (SELECT 1 FROM ORDERS (NOLOCK) WHERE TYPE IN ('M', 'I') AND OrderKey = @c_WaveKey)  
         BEGIN  
             SELECT @n_continue = 3       
             SELECT @n_err = 78304      
             SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Order Does Not Exist. (ispPreAllocateWaveProcessing)'      
         END  
      END      
...
BEGIN  
   PRINT 'Pre-Allocation: Started at ' + CONVERT(NVARCHAR(20), GETDATE())   
   PRINT ''   
   PRINT 'Wave Key: ' + RTRIM(@c_WaveKey)   
   PRINT ''  
END  
...
--(Wan02) - START  
   ...
   SELECT DISTINCT   
         ...
         ,ORDERS.Storerkey  
   FROM #OPWAVES  
   JOIN ORDERS WITH (NOLOCK) ON (#OPWAVES.Orderkey = ORDERS.Orderkey)  
...
--(Wan02) - END  
   ...
   BEGIN  
      ...
      JOIN   dbo.WaveDetail WaveDetail WITH (NOLOCK) ON PreAllocatePickDetail.OrderKey = WaveDetail.OrderKey  
      JOIN   #OPWAVES ON WaveDetail.OrderKey = #OPWAVES.OrderKey  
      JOIN   ORDERS WITH (NOLOCK) ON (#OPWAVES.Orderkey = ORDERS.Orderkey)    --(Wan02)  
      ...
      BEGIN  
         ...
         BEGIN  
            SELECT @n_continue = 3  
            SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err = 78329   -- Should Be Set To The SQL Errmessage but I don't know how to do so.  
            SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Delete From PreallocatePickDetail Failed (ispPreAllocateWaveProcessing)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '  
         END  
         ELSE  
         BEGIN        
...
BEGIN  
   IF EXISTS(SELECT 1   
             FROM dbo.WaveDetail WaveDetail (NOLOCK)  
             JOIN dbo.PICKDETAIL PickDetail WITH (NOLOCK) ON WaveDetail.OrderKey = PickDetail.OrderKey  
             JOIN   #OPWAVES ON PickDetail.OrderKey = #OPWAVES.OrderKey  
             WHERE WaveDetail.WaveKey = @c_WaveKey   
             AND   PICKDETAIL.CaseID LIKE 'C%')  
   BEGIN  
      DELETE PICKDETAIL   
      FROM dbo.PICKDETAIL  
      JOIN dbo.WaveDetail WITH (NOLOCK) ON dbo.PICKDETAIL.OrderKey = dbo.WaveDetail.OrderKey  
      JOIN   #OPWAVES ON PickDetail.OrderKey = #OPWAVES.OrderKey   
      WHERE dbo.WaveDetail.WaveKey = @c_WaveKey   
        AND dbo.PICKDETAIL.CaseID LIKE 'C%'  
     
      IF @n_err <> 0  
      BEGIN  
         SELECT @n_continue = 3  
         SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err = 78330   -- Should Be Set To The SQL Errmessage but I don't know how to do so.  
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Delete From PickDetail Failed (ispPreAllocateWaveProcessing)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '  
      END  
   END   
END  
...
BEGIN  
   ...
   SELECT @c_SQL =  
          N'DECLARE CURSOR_ORDERS CURSOR FAST_FORWARD READ_ONLY FOR   
             SELECT   ORDERDETAIL.StorerKey,  
                      ...
                      ORDERS.Facility,  ' +
                       CASE WHEN @c_AllocateByOrderPackkey = '1' THEN 
                         ' ORDERDETAIL.Packkey, ' ELSE ' SKU.Packkey, '  END +  --NJOW06                       
                    ' ORDERDETAIL.UOM,  
           ...
           ' FROM     WaveDetail WD WITH ( NOLOCK )   
             JOIN     ORDERDETAIL  WITH ( NOLOCK ) ON WD.OrderKey = ORDERDETAIL.OrderKey   
             JOIN     ORDERS WITH ( NOLOCK ) ON ORDERDETAIL.OrderKey = ORDERS.OrderKey   
             JOIN     #OPWAVES OL WITH (NOLOCK) ON OL.OrderKey = ORDERS.OrderKey   
   ...
   --tlting01           
    EXEC sp_executesql @c_SQL, N'@c_WaveKey nvarchar(10)', @c_WaveKey  --INC0509111
   ...
   BEGIN  
      SELECT @n_continue = 3  
      SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err = 78318   -- Should Be Set To The SQL Errmessage but I don't know how to do so.  
      SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Could not Open CURSOR_ORDERS (ispPreAllocateWaveProcessing)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '  
   END  
   ELSE    
   BEGIN  
...
BEGIN  
   DECLARE @c_aStorerKey NVARCHAR(15),            
      ...
      @c_OrdUOM NVARCHAR(10),                    @n_QtyLeftToFulfill INT,           
      ...
      @c_OrderType NVARCHAR(20)--,                 @c_WaveConsoAllocationOParms NVARCHAR(1)--  tlitng01  
   DECLARE   
   ...
   DECLARE @c_StorerDefaultAllocStrategy        NVARCHAR(10)         --(Wan03)  
   SET @c_StorerDefaultAllocStrategy = ''                            --(Wan03)  
   ...
   SELECT   @c_aStorerKey = SPACE(15),  
            ...
            @c_aLottable04 = SPACE(18),  
            @c_aLottable05 = SPACE(18),  
            @d_aLottable04 = NULL,  
            @d_aLottable05 = NULL,  
            @c_aLottable06 = '',  
            @c_aLottable07 = '',  
   ...
   BEGIN  
      ...
      BEGIN  
         SELECT @n_continue = 3  
         SELECT @n_err = 78317  
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Cannot Fetch Next Group. (ispPreAllocateWaveProcessing)'  
         BREAK  
      END  
      ...
      BEGIN  
         ...
         BEGIN  
            ...
            IF ISNULL(@c_StrategykeyParm,'') <> ''  --NJOW07    
            BEGIN    
              SET @c_StorerDefaultAllocStrategy = ''    
                  
              SELECT @c_aPreAllocateStrategyKey = PreAllocateStrategyKey,    
                     @c_StorerDefaultAllocStrategy = Strategykey    
              FROM STRATEGY (NOLOCK)    
              WHERE Strategykey = @c_StrategykeyParm                  
            END    
            ELSE    
            BEGIN   
               --(Wan03) - START  
               SET @c_StorerDefaultAllocStrategy = ''  
               SELECT @c_StorerDefaultAllocStrategy = ISNULL(RTRIM(SCF.SValue),'')  
                     ,@c_aPreAllocateStrategyKey = S.PreAllocateStrategyKey  
               FROM  dbo.STORERCONFIG SCF WITH ( NOLOCK )   
               JOIN  dbo.STRATEGY S       WITH ( NOLOCK ) ON (SCF.SValue = S.StrategyKey)    
               WHERE SCF.Facility = @c_Facility  
               AND   SCF.StorerKey= @c_aStorerKey              AND   SCF.Configkey = 'StorerDefaultAllocStrategy'  
               --(Wan03) - END                  
            END
      ...
      BEGIN  
         ...
         PRINT ''  
         PRINT '-----------------------------------------------------'   
         PRINT '-- SKU: ' + RTRIM(@c_aSKU) + ' Open Qty:' + CAST(@n_QtyLeftToFulfill AS NVARCHAR(10))  
         ...
         PRINT '-- Exp Date: ' +   CASE WHEN @d_aLottable04 IS NOT NULL AND @d_aLottable04 <> '19000101' THEN   
                                        CONVERT(NVARCHAR(20), @d_aLottable04, 112) ELSE '' END   
               + ' Receipt Dt :' + CASE WHEN @d_aLottable05 IS NOT NULL AND @d_aLottable05 <> '19000101' THEN --(JHTAN01)  
                          CONVERT(NVARCHAR(20), @d_aLottable05, 112) ELSE '' END    
      ...
      BEGIN  
         ...
         SELECT @c_EndString = LTRIM(CONVERT(NVARCHAR(10),@n_PackQty)) + ',' + LTRIM(CONVERT(NVARCHAR(10),@n_QtyLeftToFulfill))  
        
         IF  @n_MinShelfLife <> 0   
         BEGIN  
            SELECT @c_aLOT = '*' + RTRIM(CONVERT(NVARCHAR(5), @n_MinShelfLife))  
         END  
         ...
         BEGIN  
            SET @cExecSQL = RTRIM(@cExecSQL) + CASE WHEN @n_OrdinalPosition = 1 THEN ' ' ELSE ' ,' END +   
               CASE @c_ParameterName   
                  ...
                  WHEN '@d_Lottable13' THEN '@d_Lottable13 = N''' + RTRIM(@c_aLottable13) + ''''   
                  WHEN '@d_Lottable14' THEN '@d_Lottable14 = N''' + RTRIM(@c_aLottable14) + ''''    
                  WHEN '@d_Lottable15' THEN '@d_Lottable15 = N''' + RTRIM(@c_aLottable15) + ''''    
                  WHEN '@c_UOM'        THEN '@c_UOM = N''' + RTRIM(@c_sUOM) + ''''   
                  WHEN '@c_OtherParms' THEN '@c_OtherParms= N''' +   LTRIM(RTRIM(@c_WaveKey)) + '     W' + LTRIM(RTRIM(ISNULL(@c_Oparms,''))) + ''' ' --NJOW01 NJOW03 NJOW07    
                                            /*CASE       
                                              WHEN @c_WaveConsoAllocationOParms = '1'      
                                                  THEN LTRIM(RTRIM(@c_WaveKey)) + '     W' + LTRIM(RTRIM(ISNULL(@c_Oparms,''))) + ''' ' --NJOW01 NJOW03       
                                              ELSE      
         ...
         BEGIN  
            ...
            SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Creation/Opening of Candidate Cursor Failed! (ispPreAllocateWaveProcessing)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '  
         END  
         ELSE   
         BEGIN  
            ...
            BEGIN  
               ...
               BEGIN  
                  ...
                  BEGIN  
                     ...
                     SELECT @n_Channel_Qty_Available = ci.Qty - ( ci.QtyAllocated - @n_AllocatedHoldQty ) - ci.QtyOnHold - @n_ChannelHoldQty --NJOW08
               ...
               BEGIN  
                  ...
                  BEGIN  
                     ...
                     SELECT LOC.Facility, LOT.SKU, LOT.LOT,  
                            ...
                            Lottable01, Lottable02, Lottable03, CONVERT(NVARCHAR(10), Lottable04, 112) AS Lottable04,   
                            CONVERT(NVARCHAR(10), Lottable05, 112) AS Lottable05   
...
BEGIN  
   ...
   END  
END  
EXECUTE nsp_logerror @n_err, @c_errmsg, 'ispPreAllocateWaveProcessing'  
...
BEGIN  
   ...
   BEGIN   
      ...
      JOIN #OPWAVES PL ON PL.OrderKey = O.OrderKey  
   ...
   N'DECLARE CUR_PREALLOCATE CURSOR FAST_FORWARD READ_ONLY FOR  
     ...
     JOIN #OPWAVES PL ON PL.OrderKey = ORDERS.OrderKey  
     JOIN SKU (NOLOCK) ON OD.Storerkey = SKU.Storerkey AND OD.Sku = SKU.Sku --NJOW02  
     ...
     OD.MinShelfLife = @n_MinShelfLife AND  
     (OD.OpenQty-OD.QtyPreAllocated-OD.QtyAllocated-OD.QtyPicked) > 0 ' +  
     CASE WHEN ISNULL(@c_WaveConsoAllocationOParms,'') = '1' AND ISNULL(@c_OparmsOption5,'') <> '' THEN  
          ' AND ' + RTRIM(REPLACE(@c_OparmsOption5,'ORDERDETAIL','OD')) + ' = ''' + RTRIM(@c_Oparms) + ''' '   
          ELSE ' ' END +              
    ' ORDER BY ORDERS.Priority, --NJOW02  
      CASE WHEN PACK.Pallet > 0 THEN FLOOR((OD.OpenQty - OD.QtyPreAllocated - OD.QtyAllocated - OD.QtyPicked) / PACK.Pallet) ELSE 0 END DESC, --NJOW02  
      ...
      CASE WHEN PACK.InnerPack > 0 THEN (OD.OpenQty - OD.QtyPreAllocated - OD.QtyAllocated - OD.QtyPicked) % CAST(PACK.InnerPack AS INT)  
           ...
           WHEN PACK.Pallet > 0 THEN (OD.OpenQty - OD.QtyPreAllocated - OD.QtyAllocated - OD.QtyPicked) % CAST(PACK.Pallet AS INT)   
           ELSE (OD.OpenQty - OD.QtyPreAllocated - OD.QtyAllocated - OD.QtyPicked) END DESC --NJOW02'     
   ...
   EXEC sp_executesql @c_SQL,  
      N'@c_sStorerkey NVARCHAR(15), @c_sSKU NVARCHAR(20), @c_aLottable01 NVARCHAR(18), @c_aLottable02 NVARCHAR(18), @c_aLottable03 NVARCHAR(18), @d_aLottable04 DATETIME, @d_aLottable05 DATETIME,  
        ...
        @d_aLottable13 DATETIME, @d_aLottable14 DATETIME, @d_aLottable15 DATETIME, @c_aLot NVARCHAR(10), @c_Facility NVARCHAR(10), @c_OrdUOM NVARCHAR(10), @n_MinShelfLife INT',  --(JHTAN01)  
        @c_sStorerkey,   
        @c_sSKU,   
        @c_aLottable01,   
        @c_aLottable02,   
   ...
   ORDER BY O.Priority, --NJOW02  
            ...
            CASE WHEN PACK.InnerPack > 0 THEN (OD.OpenQty - OD.QtyPreAllocated - OD.QtyAllocated - OD.QtyPicked) % CAST(PACK.InnerPack AS INT)  
                 ...
                 ELSE (OD.OpenQty - OD.QtyPreAllocated - OD.QtyAllocated - OD.QtyPicked) END DESC --NJOW02                      
   */                                    
   --ORDER BY O.Priority, O.Orderkey --NJOW01           
