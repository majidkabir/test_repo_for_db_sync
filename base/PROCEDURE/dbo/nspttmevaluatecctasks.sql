...
GO
/************************************************************************/    
/* Stored Procedure: nspTTMEvaluateCCTasks                              */    
/* Creation Date:                                                       */    
/* Copyright: IDS                                                       */    
/* Written by:                                                          */    
/*                                                                      */    
...
/* 09-11-2011   1.0   ChewKP    Add Parameters to SP (ChewKP01)         */    
/* 23-10-2015   1.1   James     Fix for skip task (james01)             */    
/* 13-06-2019   1.2   Leong     Revise close cursor (L01).              */    
/************************************************************************/    
    
CREATE PROC [dbo].[nspTTMEvaluateCCTasks]    
...
,              @c_ptcid            NVARCHAR(5)               -- (ChewKP01)    
,              @c_FromLoc          NVARCHAR(10)   OUTPUT     -- (ChewKP01)    
,              @c_TaskDetailKey    NVARCHAR(10)   OUTPUT     -- (ChewKP01)    
AS    
BEGIN    
   SET NOCOUNT ON    
   SET QUOTED_IDENTIFIER OFF    
   ...
   BEGIN    
      ...
      BEGIN    
         ...
         SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Execute Of Move Tasks Pick Code Failed. (nspTTMEvaluateCCTasks)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "    
      END    
      -- IF @n_err = 16915 -- L01    
      -- BEGIN    
      ...
      -- END    
      IF @n_err <> 0 AND CURSOR_STATUS('GLOBAL' , 'CURSOR_CCTASKCANDIDATES') IN (0 , 1)    
      BEGIN    
         CLOSE CURSOR_CCTASKCANDIDATES    
         DEALLOCATE CURSOR_CCTASKCANDIDATES    
         GOTO DECLAREcursor_CCTASKCANDIDATES    
   ...
   BEGIN    
      ...
      BEGIN    
         -- Intitialise as blank first (james01)    
         -- Cursor already opened in pick code    
         SET @c_TaskDetailKey = ''    
...
--               SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Error Occurred While Attempting To Update TaskDetail. (nspTTMEvaluateCCTasks)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "    
--            END    
...
ELSE    
   ...
   ELSE    
      BEGIN    
         ...
         END    
      END    
      execute nsp_logerror @n_err, @c_errmsg, "nspTTMEvaluateCCTasks"    
