...
GO
/************************************************************************/
/* Store Procedure: isp_TPP_TPPRN_06                                    */
/* Creation Date: 30-MAY-2022                                           */
/* Copyright: LF                                                        */
/* Written by:CHONGCS                                                   */
/*                                                                      */
/* Purpose:  WMS-19515 [CN] NIKE- BaoZun Platform Cloud Print Cmd SP    */
/*                                                                      */
/*                                                                      */
/* Input Parameters:                                                    */
/*                                                                      */
...
/* 15-Jun-2022  CSCHONG 1.1  WMS-19515 revised field mapping (CS01)     */
/* 22-JUL-2023  CSCHONG 1.2  WMS-23026 add filter on print job task(CS02)*/
/************************************************************************/ 

CREATE    PROC [dbo].[isp_TPP_TPPRN_06] (
   @n_JobNo             BIGINT
   )   
AS
BEGIN
   ...
   DECLARE @n_FromCartonNo   INT = 0,
           ...
           @c_ECOM_OAID      NVARCHAR(256) = '',
           @c_ccontact1      NVARCHAR(45)  = '',
           @c_ccphone1       NVARCHAR(45) = '',
           @c_Address        NVARCHAR(200) = '',
           @c_MCompany       NVARCHAR(30)='',
           @c_JobStatus      NVARCHAR(10)     --CS02     
   ...
   DECLARE @c_Printername   NVARCHAR(128) 
          ...
          ,@c_piiuse        NVARCHAR(4000) = ''
          ,@c_Customarea    NVARCHAR(4000) = ''      --TPPRINTCONFIG.UDF05
          ,@c_Extrainfo     NVARCHAR(4000) = ''      --TPPRINTCONFIG.UDF03
   ...
   BEGIN
     SELECT @c_Module=Module, @c_ReportType=ReportType, @c_Storerkey=Storerkey,@c_PrinterID=PrinterID, @c_Printer=Printer, 
               ...
               @c_Platform=Platform ,@c_JobStatus = Status   --CS02
      ...
      SELECT  --@c_ExternOrdkey = OH.ExternOrderKey
            @c_ExternOrdkey = OH.M_Company --CS01
            ,@c_ECOM_OAID = ISNULL(OH.ECOM_OAID,'')
            ,@c_ccontact1 = ISNULL(OH.C_contact1,'')
            ,@c_ccphone1 = ISNULL(OH.C_Phone1,'')
            ,@c_Address = (ISNULL(Oh.C_State,'') + ISNULL(oh.C_City,'')+ISNULL(oh.C_Address1,'')+ISNULL(oh.C_Address2,''))
       ...
       IF ISNULL(@c_PrinterID,'') = ''
        BEGIN
         ...
         SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_err,0))   
                        + ': TPP Printer Group not setup for printer : ' + @c_printer + ' with platform' + @c_Platform + ' (isp_TPP_TPPRN_06)'
         SET @c_Status = '5'
        ...
        BEGIN
         ...
         SET @n_err = 68010     
         SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_err,0))   
                        + ': TPP Printer Group not setup for printer : ' + @c_printerid + ' with platform' + @c_Platform + ' (isp_TPP_TPPRN_06)'
         SET @c_Status = '5'
         SET @c_Message = RTRIM(ISNULL(@c_Message,'')) + @c_ErrMsg + ' '                                
        ...
        ELSE
        BEGIN
              SET @c_WebSocketURL = 'ws://' + @c_PrnServerIP +':'+ @c_PrnServerPort+'/bz-pii-printer/ws'  -- CS01
        END
   ...
   BEGIN
      ...
      BEGIN
         ...
         SET @n_err = 68010     
         SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_err,0))   
                        + ': Unable to find print record for Job No: ' +RTRIM(CAST(@n_JobNo AS NVARCHAR)) + ' (isp_TPP_TPPRN_06)'
         SET @c_Status = '5'
         SET @c_Message = RTRIM(ISNULL(@c_Message,'')) + @c_ErrMsg + ' '                                
      END
      ...
      BEGIN       
          ...
          BEGIN
            ...
            SET @n_err = 68010     
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_err,0))   
                           + ': Get RequestID fail for Order# ' + RTRIM(@c_CurrOrderkey) + ' Carton# ' + RTRIM(CAST(@n_CartonNo AS NVARCHAR)) + ' (isp_TPP_TPPRN_06)'
            SET @c_Status = '5'
            SET @c_Message = RTRIM(ISNULL(@c_Message,'')) + @c_ErrMsg + ' '                                
          END
          ELSE
          ...
          BEGIN
            ...
            SET @n_err = 68010     
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_err,0))   
                           + ': Unable to find tracking# for Order# ' + RTRIM(@c_CurrOrderkey) + ' Carton# ' + RTRIM(CAST(@n_CartonNo AS NVARCHAR)) + ' (isp_TPP_TPPRN_06)'
            SET @c_Status = '5'
            SET @c_Message = RTRIM(ISNULL(@c_Message,'')) + @c_ErrMsg + ' '                                
          END
         ...
         BEGIN
                   ...
                   SET @c_piiuse = ' "tid":'  + '"' +  @c_ExternOrdkey + '"' + ', "exchangeOrderCode": '+ '"' +  @c_ExternOrdkey  + '", ' + 
                                   ' "receiverOaid": ' + '"' +  @c_ECOM_OAID  + '"' +  ', '  +
                                   ' "plaintextReceiverName" : '  + '"' +  @c_ccontact1  + '"' + ' , "plaintextReceiverMobile" : ' + '"' +  @c_ccphone1  + '"' + ', '  +
                                   ' "plaintextReceiverPhone" : ' + '"' +  @c_ccphone1  + '"' + ', "plaintextReceiverAddressDetail" : '   + '"' +  @c_Address  

                   IF ISNULL(@c_TemplathURL,'') <> '' 
                   BEGIN
                          --SET  @c_CloseContent  = ',{"data": { "value":'  + '"' +  @c_contentdata + '"' + ',}, "templateURL": '+ '"' +  @c_TemplathURL + '"' + '}'    --CS02
                         ...
                         BEGIN
                            -- SET @c_GetPrintData = REPLACE(@c_PrintData,'}','') + ','  --remove replace logic zoe

                              SET @c_Extrainfo =@c_SqlOutput --'"extrainfo": ' + @c_SqlOutput 
                         END

                         SET @c_contentdata = @c_SqlOutput05

                         SET  @c_Customarea = ',"customizeData": { "templateURL": ' +'"' + @c_TemplathURL + '"' +',"CustomArea": [ ' + @c_contentdata + '] }'   
                   END 

                SET @c_RequestString = N'{"cmd": ' + '"' + @c_cmdstart + '"' + ', "requestID":' + '"' + @c_RequestID + '"' + ', "version":' + '"' + @c_version + '"' + ', "printer": '  + '"' + @c_printer + '"' + 
                                        ', "document": {  "content": {  "piiUse": {'  + @c_piiuse + '"' + '},"useData": { "printData" :' + @c_GetPrintData + @c_Customarea + '}, "extrainfo" :' + 
                                         @c_Extrainfo  + ' }}}'   --remove   '"'  zoe
   ...
   BEGIN
      ...
      BEGIN
         ...
         END          
      END
      EXECUTE nsp_logerror @n_err, @c_errmsg, "isp_TPP_TPPRN_06"
