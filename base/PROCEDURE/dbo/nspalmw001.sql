...
GO
/*************************************************************************/            
/* Stored Procedure: nspALMW001                                          */            
/* Creation Date: 12-DEC-2023                                            */            
/* Copyright: MAERSK                                                     */            
/* Written by:                                                           */            
/*                                                                       */            
/* Purpose: WMS-24328 - Puma Chile UCC Allocation Strategy For UOM       */       
/*          2 (UCC) and 6 (piece)                                        */
/*          Modified from nspAL_AU01 to change Location type from CASE   */
/*          to OTHERS for UOM 2.                                         */
/*                                                                       */
/*                                                                       */            
/* Called By:                                                            */            
...
/* 04-DEC-2023  NJOW    1.0   DevOps Script Combine                      */
/*************************************************************************/            
CREATE   PROC [dbo].[nspALMW001]
   @c_DocNo      NVARCHAR(10),  
...
BEGIN   
   ...
   BEGIN
      ...
      SELECT @c_SQL = ' DECLARE CURSOR_AVAILABLE CURSOR FAST_FORWARD READ_ONLY FOR ' +
                      ...
                      ' GROUP BY LOTxLOCxID.STORERKEY, LOTxLOCxID.SKU, LOT.LOT, LOTxLOCxID.LOC, LOTxLOCxID.ID, LOC.Loc, LOC.LogicalLocation, LA.Lottable05, UCC.UCCNo, UCC.Qty ' +    
   ...
   BEGIN
      ...
      SELECT @c_SQL = ' DECLARE CURSOR_AVAILABLE CURSOR FAST_FORWARD READ_ONLY FOR ' +
                      ' SELECT LOTxLOCxID.STORERKEY, LOTxLOCxID.SKU, LOT.LOT, LOTxLOCxID.LOC, LOTxLOCxID.ID, ' +
                      '        QTYAVAILABLE = SUM(LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED - LOTxLOCxID.QtyReplen), ''1'' ' +
                      ' FROM LOTATTRIBUTE LA  (NOLOCK) ' +
                      ' JOIN LOT (NOLOCK) ON LOT.LOT = LA.LOT ' +
                      ' JOIN LOTxLOCxID (NOLOCK) ON LOTXLOCXID.Lot = LOT.LOT AND LOTXLOCXID.LOT = LA.LOT ' + 
                      ' JOIN LOC (NOLOCK) ON LOTXLOCXID.LOC = LOC.LOC ' +
                      ...
                      CASE WHEN CONVERT(NVARCHAR(8) ,@d_Lottable14 ,112) <> '19000101' AND @d_Lottable14 IS NOT NULL THEN ' AND LA.Lottable14 = RTRIM(CONVERT( NVARCHAR(20), @d_Lottable14, 106)) ' ELSE ' ' END +
                      CASE WHEN CONVERT(NVARCHAR(8) ,@d_Lottable15 ,112) <> '19000101' AND @d_Lottable15 IS NOT NULL THEN ' AND LA.Lottable15 = RTRIM(CONVERT( NVARCHAR(20), @d_Lottable15, 106)) ' ELSE ' ' END +
                      ' GROUP BY LOTxLOCxID.STORERKEY, LOTxLOCxID.SKU, LOT.LOT, LOTxLOCxID.LOC, LOTxLOCxID.ID, LOC.Loc, LOC.LogicalLocation, LA.Lottable05 ' +  
                      ' HAVING SUM(LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED - LOTxLOCxID.QtyReplen) > 0 ' + 
                      RTRIM(ISNULL(@c_OrderBy,''))
   END
   --PRINT @c_SQL
