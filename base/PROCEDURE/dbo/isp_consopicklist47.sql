...
GO
/************************************************************************/  
/* Store Procedure:  isp_ConsoPickList47                                */  
/* Creation Date:  28-Dec-2020                                          */  
/* Copyright: IDS                                                       */  
/* Written by:  CSCHONG                                                 */  
/*                                                                      */  
/* Purpose: WMS-15912 [JP] Desigual - B2C -Consolidate Picking Slip     */  
/*                                                                      */  
...
/* Local Variables:                                                     */  
/*                                                                      */  
/* Called By:  r_dw_consolidated_pick47                                 */  
/*                                                                      */  
/* PVCS Version: 1.0                                                    */  
/*                                                                      */  
/* Version: 5.4                                                         */  
...
CREATE PROC [dbo].[isp_ConsoPickList47]
...
BEGIN
   ...
   SELECT @n_CtnOrder  = COUNT(Orderkey)
   ...
   BEGIN
      ...
      BEGIN
         ...
         SET @c_errmsg = 'NSQL'+CONVERT(CHAR(5) ,@n_err)+
                       + ': Insert Into PICKHEADER Failed. (isp_ConsoPickList47)'
         GOTO QUIT
      END
   END
   ELSE
   ...
   BEGIN
      ...
      SET @n_err = 63502  
      SET @c_errmsg = 'NSQL'+CONVERT(CHAR(5) ,@n_err) 
                   + ': Get LoadKey Failed. (isp_ConsoPickList47)'
      GOTO QUIT
   END  
   ...
   BEGIN
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'isp_ConsoPickList47' 
