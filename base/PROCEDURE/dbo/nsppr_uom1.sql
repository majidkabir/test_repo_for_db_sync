...
/* 01-Feb-2010  Shong   1.0  SOS162789 Strategy special design for NIVEA  */      
/*                           China.                                       */      
/* 19-Apr-2010  NJOW01  1.1  162789 - Fix preallocation on pick loc type  */    
/*                           where the qty available can be more than 1   */    
/*                           carton.                                      */    
...
/* 26-Oct-2016  NJOW04  1.5  WMS-564 Configure to skip sku outgoing       */  
/*                           shelflife checking                           */  
/* 17-Jul-2018  TLTING01 1.6  Dynamic SQL - cache issue                   */  
/**************************************************************************/      
      
CREATE PROCEDURE [dbo].[nspPR_UOM1]     
...
BEGIN    
    ...
    DECLARE @c_Limitstring1                NVARCHAR(255)    
           ...
           ,@c_DisableSkuOutShelfLifeFlag  NCHAR(1) --NJOW04  
           ,@c_PreAllocateStrategyKey      NVARCHAR(10) --NJOW04  
    ...
    WHERE CODELKUP.Listname = 'nspPR_UOM1'  
    AND ISNULL(CODELKUP.Storerkey,'') = CASE WHEN ISNULL(CODELKUP.Short,'') = @c_PreallocateStrategykey AND STORER.Storerkey IS NULL THEN ISNULL(CODELKUP.Storerkey,'') ELSE @c_Storerkey END 
    --if setup short and no setup storer ignore storer otherwise by storer.   
    AND ISNULL(CODELKUP.Short,'') = CASE WHEN ISNULL(CODELKUP.Short,'') <> '' THEN @c_PreallocateStrategykey ELSE ISNULL(CODELKUP.Short,'') END 
    --if short setup must match preallocate strategykey  
      
    SELECT TOP 1 @c_DisableSkuOutShelfLifeFlag = ISNULL(UDF01,'')  
    FROM #TMP_CODELKUP  
    WHERE Code = 'DISABLESKUOUTSHELFLIFE' --disable sku outgoing shelflife  
    ...
    IF @b_debug=1    
    BEGIN     
        SELECT 'nspPR_UOM1 : Before Lot Lookup .....'        
        SELECT '@c_lot' = @c_lot    
    ...
    IF @b_debug=1    
    BEGIN    
        SELECT 'nspPR_UOM1 : After Lot Lookup .....'        
        SELECT '@c_lot' = @c_lot    
              ,'@c_Lottable01' = @c_Lottable01    
              ,'@c_Lottable02' = @c_Lottable02    
              ,'@c_Lottable03' = @c_Lottable03    
  ...
  SELECT LOT.StorerKey    
    ...
    BEGIN   
      ...
      IF ISNULL(@c_DisableSkuOutShelfLifeFlag,'') = 'Y' --NJOW04  
         SET @n_ShelfLife = 0  
      ...
      BEGIN    
          ...
          BEGIN    
              SELECT @c_Limitstring = RTrim(@c_LimitString)+    
                     ' AND Lottable04  > Convert(Datetime, CONVERT(VARCHAR(15) ,DATEADD(MONTH ,@n_ShelfLife ,GETDATE()) ,106)) '    
          END    
          ELSE    
          BEGIN    
              SELECT @c_Limitstring = RTrim(@c_LimitString)+    
                     ' AND Lottable04  > Convert(Datetime, CONVERT(VARCHAR(15) ,DATEADD(DAY ,@n_ShelfLife ,GETDATE()) ,106)) '
          END    
      END    
      ELSE     
      BEGIN  --NJOW02    
      ...
      SELECT @c_SQL =     
              ...
              +' CASE WHEN      
           SUM(LOTxLOCxID.QTY)- SUM(LOTxLOCxID.QTYALLOCATED)- SUM(LOTxLOCxID.QTYPICKED) - SUM(LOTxLOCxID.QTYREPLEN) - MIN(ISNULL(P.QtyPreallocated,0)) <     
           MIN(LOT.QTY)- MIN(LOT.QTYALLOCATED)- MIN(LOT.QTYPICKED) - MIN(LOT.QtyPreallocated) - MIN(LOT.QtyOnHold)      
           THEN SUM(LOTxLOCxID.QTY)- SUM(LOTxLOCxID.QTYALLOCATED)- SUM(LOTxLOCxID.QTYPICKED) - SUM(LOTxLOCxID.QTYREPLEN) - MIN(ISNULL(P.QtyPreallocated,0))     
           ...
           END '     
              ...
              +                 ' AND    P.PreAllocatePickCode = ''nspPR_UOM1'''    
              ...
              --+ CASE WHEN @c_UOM = '6' THEN  ' HAVING (SUM(LOTxLOCxID.QTY) - SUM(LOTxLOCxID.QtyAllocated) - SUM(LOTxLOCxID.QTYPicked)- MIN(ISNULL(P.QtyPreallocated,0)) ) BETWEEN 1 AND '     
              --+ CAST(@n_CaseCnt AS NVARCHAR(10)) ELSE '' END      
              + CASE WHEN @c_UOM = '6' THEN  ' HAVING (SUM(LOTxLOCxID.QTY) - SUM(LOTxLOCxID.QtyAllocated) - SUM(LOTxLOCxID.QTYPicked)- SUM(LOTxLOCxID.QTYREPLEN) - MIN(ISNULL(P.QtyPreallocated,0)) ) > 0 ' ELSE '' END  --NJOW01    
              + CASE WHEN @c_UOM = '2' THEN  ' HAVING (SUM(LOTxLOCxID.QTY) - SUM(LOTxLOCxID.QtyAllocated) - SUM(LOTxLOCxID.QTYPicked)- SUM(LOTxLOCxID.QTYREPLEN) - MIN(ISNULL(P.QtyPreallocated,0)) ) >= '     
              + CAST(@n_CaseCnt AS NVARCHAR(10)) ELSE '' END                  
              + CASE WHEN @c_UOM = '7' THEN  ' HAVING (SUM(LOTxLOCxID.QTY) - SUM(LOTxLOCxID.QtyAllocated) - SUM(LOTxLOCxID.QTYPicked)- SUM(LOTxLOCxID.QTYREPLEN) - MIN(ISNULL(P.QtyPreallocated,0)) ) >  0 ' ELSE '' END       
