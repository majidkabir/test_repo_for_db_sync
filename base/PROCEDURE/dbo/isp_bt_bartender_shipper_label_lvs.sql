...
CREATE PROC [dbo].[isp_BT_Bartender_Shipper_Label_LVS]                       
...
BEGIN                      
  ...
  SET @c_SQLJOIN = +'SELECT DISTINCT F.DESCR,F.Address1,F.Address2,F.Address3,F.Address4,'  
                   ...
                   +' '''','''',PD.DropID,ORD.loadkey,PT.DevicePosition,'''','''','''','''','''', '  --50    --(CS03)   
                   +' RPS.POSITION,'''','''','''','''','''','''','''','''','''' '   --60        
                   ...
                   +' LEFT JOIN PACKTASK PT WITH (NOLOCK) ON PT.Orderkey = ORD.Orderkey and PT.Taskbatchno = ORD.Userdefine09 '   
                   +' LEFT JOIN PACKINFO PIF WITH (NOLOCK) ON PIF.Pickslipno = PD.Pickslipno'  
                   +' AND PIF.CartonNo = PD.CartonNo '      
                   +' LEFT JOIN RDT.RDTPTLSTATIONLOG RPS WITH (NOLOCK) ON RPS.ORDERKEY = ORD.ORDERKEY'  
...
--                   +' AND PD.Dropid = CASE WHEN ISNULL(RTRIM(''' + @c_Sparm3+ '''),'''') <> '''' THEN ''' + @c_Sparm3+ ''' ELSE PD.Dropid END'         --(CS02)   
  ...
  SET @c_SQL='INSERT INTO #Result (Col01,Col02,Col03,Col04,Col05, Col06,Col07,Col08,Col09'  + CHAR(13) +           
   ...
   SELECT DISTINCT col29,col31,Col33 FROM #Result          
       
   OPEN CUR_RowNoLoop            
       
   FETCH NEXT FROM CUR_RowNoLoop INTO @c_orderkey,@c_CntNo,@c_labelno     
...
EXIT_SP:    
   ...
   EXEC isp_InsertTraceInfo   
      ...
      @c_TraceName = 'isp_BT_Bartender_Shipper_Label_LVS',  
