...
/***************************************************************************/
/* Stored Procedure: ispVoicePick01                                        */
/* Creation Date: 12-Jul-2021                                              */
/* Copyright: LFL                                                          */
/* Written by:  WLChooi                                                    */
/*                                                                         */
/* Purpose: WMS-17427 - [CN] Voice Picking General Trigger Logic - B2C     */
/*                                                                         */
...
/***************************************************************************/  
CREATE PROC [dbo].[ispVoicePick01]  
...
BEGIN  
   ...
   BEGIN   
      SET @c_SQL = N' DECLARE cur_Loadkey CURSOR FAST_FORWARD READ_ONLY FOR
                         ...
                         JOIN PACKTASK (NOLOCK) ON PACKTASK.Orderkey = ORDERS.Orderkey
                         WHERE ORDERS.StorerKey = @c_Storerkey AND LOADPLAN.[Status] < ''5''
               ...
               EXEC sp_ExecuteSQL @c_SQL
      	      ...
      	      BEGIN
                  ...
                  BEGIN      
                     ...
                     SET @c_errmsg = 'NSQL'+CONVERT(char(5),@n_err)+': Unable to Obtain TransmitLogKey2. (ispVoicePick01)' + 
                  ...
                  BEGIN
                     ...
                     SELECT @c_errmsg = 'NSQL' + CONVERT(char(5),ISNULL(@n_err,0))   
                                      + ': Insert Failed On Table TRANSMITLOG2. (ispVoicePick01)'   
...
QUIT_SP:
   ...
   BEGIN
      ...
      BEGIN
         ROLLBACK TRAN
      END
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'ispVoicePick01'
