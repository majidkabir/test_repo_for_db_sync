...
/************************************************************************/
/* Stored Proc : nsp_GetPickSlipXD02                                    */
/* Creation Date:                                                       */
/* Copyright: IDS                                                       */
/* Written by:                                                          */
/*                                                                      */
...
/* SOS30931		June				- Remark the BEGIN TRAN, COMMIT & ROLLBACK*/
/*                            - Use one BEGIN TRAN, like standard SP		*/
/* 2007-04-05	ONG01				SOS#71903 - Add Column POTYPE					*/
/*                                                                      */
/************************************************************************/

CREATE PROC [dbo].[nsp_GetPickSlipXD02] (@c_refkey NVARCHAR(20), @c_type NVARCHAR(2)) 
AS
BEGIN
...
--            ROLLBACK TRAN 
            ...
            SELECT @c_errmsg = CONVERT(CHAR(250),@n_err), @n_err = 63314   
      		SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Insert Into PickHeader Failed. (nsp_GetPickSlipXD02)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + ' ) '
            BREAK 
         END
...
--               ROLLBACK TRAN 
               SELECT @n_continue = 3
               SELECT @c_errmsg = CONVERT(CHAR(250),@n_err), @n_err = 63314   
         		SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Update PickDetail Failed. (nsp_GetPickSlipXD02)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + ' ) '
					Break
            END
...
--            ROLLBACK TRAN 
            SELECT @n_continue = 3
            SELECT @c_errmsg = CONVERT(CHAR(250),@n_err), @n_err = 63314   
      		SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Insert Into RefKeyLookup Failed. (nsp_GetPickSlipXD02)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + ' ) '
         END
		END
...
--             COMMIT TRAN 
			...
			SELECT #TEMPPICKDETAIL.PickDetailKey, #TEMPPICKDETAIL.OrderKey, 
					 #TEMPPICKDETAIL.OrderLineNumber, #TEMPPICKDETAIL.StorerKey, #TEMPPICKDETAIL.Sku, 
                #TEMPPICKDETAIL.Qty, #TEMPPICKDETAIL.Lot, #TEMPPICKDETAIL.Loc, 
					 #TEMPPICKDETAIL.ID, #TEMPPICKDETAIL.Packkey, #TEMPPICKDETAIL.PickslipNo, 
...
--    BEGIN 
-- --      ROLLBACK TRAN 
--       EXECUTE nsp_logerror @n_err, @c_errmsg, 'nsp_GetPickSlipXD02'
