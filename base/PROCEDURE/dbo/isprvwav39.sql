...
GO
/*************************************************************************/  
/* Stored Procedure: ispRVWAV39                                          */  
/* Creation Date: 16-SEP-2020                                            */  
/* Copyright: LFL                                                        */  
/* Written by:                                                           */  
/*                                                                       */  
/* Purpose: WMS-14638 - CN Converse reverse task                         */
/*                                                                       */  
/* Called By: wave                                                       */  
/*                                                                       */  
/* PVCS Version: 1.1                                                     */  
...
/*************************************************************************/   
CREATE PROCEDURE [dbo].[ispRVWAV39]        
 ...
 BEGIN    
    ...
    BEGIN  
        IF NOT EXISTS (SELECT 1 FROM REPLENISHMENT RP (NOLOCK)   
                       WHERE RP.Wavekey = @c_Wavekey AND RP.OriginalFromLoc = 'ispRLWAV39')  
        ...
        BEGIN                                            
          SELECT @n_continue = 3    
          SELECT @n_err = 81010    
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Nothing to reverse. (ispRVWAV39)'           
        END                   
    END  
    ...
    BEGIN  
        ...
        BEGIN  
          SELECT @n_continue = 3    
          SELECT @n_err = 81020    
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Some Tasks have been started. Not allow to Reverse Wave Released (ispRVWAV39)'         
        END                   
    END  
    ...
    BEGIN  
       DECLARE CUR_REPL CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
         ...
         AND OriginalFromLoc = 'ispRLWAV39'
         AND Confirmed = 'N'
       ...
       BEGIN                                     	            	  
          ...
          BEGIN  
            ...
            SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Delete Replenishment Table Failed. (ispRVWAV39)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
          END   
          ...
          BEGIN  
            ...
            SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Pickdetail Table Failed. (ispRVWAV39)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
          END   
    ...
    BEGIN
       ...
       BEGIN
          ...
          BEGIN  
            ...
            SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Loadplan Table Failed. (ispRVWAV39)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
          END   
    ...
    BEGIN              
       ...
       BEGIN    
          ...
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update on wave Failed (ispRVWAV39)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
       END    
    END    
...
RETURN_SP:  
    ...
    BEGIN    
       ...
       BEGIN    
          ...
          END    
       END    
       execute nsp_logerror @n_err, @c_errmsg, "ispRVWAV39"    
