...
/************************************************************************/
/* Trigger: ispSplitLane_Adidas                                         */
/* Creation Date: 21-Nov-2014                                           */
/* Copyright: LF Logistics                                              */
/* Written by: LZG                                                      */
/*                                                                      */
/* Purpose: Split orders to new MBOL                                    */
/* PVCS Version: 1.0                                                    */
/*                                                                      */
/* Version: 6.0                                                         */
/*                                                                      */
...
/************************************************************************/
CREATE   PROC [dbo].[ispSplitLane_Adidas] (@cLane NVARCHAR(30), @bDebug INT = 0)
AS
BEGIN
   SET NOCOUNT ON
   SET QUOTED_IDENTIFIER OFF
   ...
   DECLARE 
       ...
       @nQty_Packed        INT,
       @nCount             INT = 0,
       --@nCheckCtnFullScan  INT = 1,        -- Toggle carton full scan check
       @nFullScanCnt       INT = 0,
       @nMBOLCnt           INT = 0,
       @nTranCount         INT,
       @nPltRowRef         INT
   ...
   BEGIN
       ...
       IF EXISTS ( SELECT 1   
               ...
               AND ISNULL( CL.Code, '') <> '')
       BEGIN 
           PRINT 'Palletized customer: ' + @cPalletKey
           GOTO NEXT_REC
       END 
       ...
       --BEGIN 
         ...
         BEGIN 
             ...
             IF @bDebug = 1
                 PRINT '@cNewPalletKey: ' + @cNewPalletKey 
