...
GO
/************************************************************************/          
/* Stored Procedure: isp_TPB_Orders_Levis                               */          
/* Creation Date:                                                       */          
/* Copyright: IDS                                                       */          
/* Written by: WeiLi                                                    */          
/*                                                                      */          
/* Purpose: TPB billing for Levis Orders Transaction                    */          
/*                                                                      */          
...
/* 05-09 -19     kocy      1.2  bug fix                                 */  
/************************************************************************/      
    
    
CREATE PROC [dbo].[isp_TPB_Orders_Levis]    
...
BEGIN    
   ...
   ',(Select TOP 1 CD.Notes from DBO.CODELKUP CD (NOLOCK)     
      JOIN DBO.ORDERS ORDERS1 (NOLOCK) ON ORDERS1.Facility=CD.Code     
      where CD.Listname = ''TPBFAC'' AND ORDERS1.Facility = ORDERS.Facility) ' +        
   ...
   ',(Select TOP 1 CD.Code from DBO.CODELKUP CD (NOLOCK)     
      JOIN DBO.ORDERS ORDERS1 (NOLOCK) ON ORDERS1.Storerkey=CD.Storerkey     
      where CD.Listname = ''TPBCLIENT'' AND ORDERS1.Storerkey = ORDERS.Storerkey) ' +      
   ',UPPER(RTRIM(ORDERS.StorerKey)) ' +         
   ',UPPER(RTRIM(Orders.Facility)) ' +        
   ',RTRIM(ORDERS.ConsigneeKey) ' +          
   ',RTRIM(ORDERS.C_Company) ' +          
   ',RTRIM(ORDERS.C_ISOCntryCode) ' +         
   ',RTRIM(OD.LoadKey) ' +        
   ',RTRIM(OD.MBOLKey) ' +        
   ...
   ',PD.Qty ' +                    
   ',RTRIM(PD.UOM) ' +           
  -- ',(Select count(distinct PCD.cartonno) from DBO.packheader PH (NOLOCK)    
  --JOIN DBO.packdetail PCD (NOLOCK) ON PCD.pickslipno=PH.pickslipno    
  --WHERE PH.orderkey=ORDERS.orderkey and PH.Orderkey <> '''') ' +    -- kocy command length not enough ',(selecT count(distinct PCD.cartonno) from 
  --packheader PH (NOLOCK)JOIN packdetail PCD (NOLOCK) ON PCD.pickslipno=PH.pickslipno where PH.orderkey=OD.OrderKey ) ' +      --WL       
   ',(Select  count(distinct PD1.labelno) from DBO.LOADPLAN LP1 (NOLOCK) join DBO.orders ORDERS1 (NOLOCK) on LP1.loadkey=ORDERS1.loadkey  
      join DBO.packheader PH1 (NOLOCK) on LP1.loadkey=PH1.loadkey  
      join DBO.packdetail PD1 (NOLOCK) on PH1.pickslipno=PD1.pickslipno  
      where ORDERS1.storerkey=ORDERS.storerkey and LP1.loadkey=LOADPLAN.LoadKey ) ' +  
   ',S.GrossWgt ' +                                     
 ...
 ',RTRIM(C.SUSR4) ' +        
   ...
   SET @c_SQLStatement = @c_SQLStatement +  N'FROM DBO.ORDERS (NOLOCK) ' +        
          ...
          'JOIN DBO.LOADPLAN (NOLOCK) on LOADPLAN.LoadKey = ORDERS.LoadKey ' +  
...
INSERT INTO [DBO].[WMS_TPB_BASE](        
  ...
  ,DOC_TYPE        
  ,DOC_SUB_TYPE        
 -- ,DOC_GROUPING_1   -- kocy extra column --(WL)      
  ,DOC_GROUPING_2   -- kocy extra column      
  ,DOCUMENT_ID        
  ,DOCUMENT_LINE_NO        
  ,CLIENT_REF        
  ,CLIENT_REF_LINE_NO        
