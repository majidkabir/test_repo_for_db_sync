...
/******************************************************************************/                       
/* Copyright: LFL                                                             */                       
/* Purpose: isp_BT_Bartender_CN_SHIPUCCLB3_STUSSY_XDO                         */                       
/*                                                                            */                       
/* Modifications log:                                                         */                       
/*                                                                            */                       
/* Date        Rev  Author     Purposes                                       */      
/* 10-NOV-2021 1.0  CHONGCS    Created - DEVOPS Script combine (WMS-18254)    */   
/******************************************************************************/                      
                        
CREATE PROC [dbo].[isp_BT_Bartender_CN_SHIPUCCLB3_STUSSY_XDO]                            
...
BEGIN                            
   ...
   SET @n_noofcopy = CAST(@c_Sparm03 AS INT)  
   SET @c_col12 = @c_Sparm02           
   ...
   SELECT TOP 1 @c_Col01     = LEFT(
              ...
              , @c_Col05     = ISNULL(ORDERS.ConsigneeKey,'')
              , @c_Col06     = ISNULL(ORDERS.PmtTerm,'')
              , @c_Col07     = ISNULL(ORDERS.BuyerPO,'')
             -- , @c_Col08     = ISNULL(ORDERS.BuyerPO,'')
              , @c_Col08     = ISNULL(ORDERS.DischargePlace,'')
              , @c_Col09     = ISNULL(ORDERS.MBOLKey,'')
              , @c_Col10     = ISNULL(ORDERS.ExternOrderKey,'')
              , @c_Storerkey = ORDERS.Storerkey
   FROM ORDERS (NOLOCK) 
   JOIN FACILITY (NOLOCK) ON FACILITY.Facility = ORDERS.Facility
   WHERE ORDERS.Orderkey = @c_Sparm01 
   
   SET @c_Col14 = ''

   SELECT TOP 1 @c_Col14 = LOTT.Lottable09
   FROM dbo.PICKDETAIL PIDET WITH (NOLOCK)
   JOIN dbo.LOTATTRIBUTE LOTT WITH (NOLOCK) ON LOTT.lot = PIDET.Lot
   WHERE PIDET.OrderKey = @c_Sparm01 
  
  

   SET @c_SQLJOIN = + ' SELECT TOP 1 @c_Col01, @c_Col02, @c_Col03, @c_Col04, @c_Col05, ' + CHAR(13)   --5
                    + ' @c_Col06, @c_Col07, @c_Col08, @c_Col09, @c_Col10, '   + CHAR(13)   --10 
                    + ' PO.ExternPOkey,@c_Col12, '   --12
                    + '  @c_Sparm01, @c_Col14, '''', '''', '''', '''', '''', '''', ' + CHAR(13)  --20     
                    + ' '''', '''', '''', '''', '''', '''', '''', '''', '''', '''', ' + CHAR(13)  --30     
                    + ' '''', '''', '''', '''', '''', '''', '''', '''', '''', '''', ' + CHAR(13)  --40        
                    + ' '''', '''', '''', '''', '''', '''', '''', '''', '''', '''', ' + CHAR(13)  --50                           
                    + ' '''', '''', '''', '''', '''', '''', '''', '''', '''', @c_Sparm01 ' + CHAR(13)  --60                
                    + ' FROM PO PO (NOLOCK) ' + CHAR(13)
                    + ' WHERE PO.sellersreference =LEFT(@c_Col10,7) ' + CHAR(13)  
