...
GO
/***************************************************************************/
/* Stored Procedure: nspTTMCC10                                            */
/* Copyright: Maersk                                                       */
/*                                                                         */
/* Purpose: Consider GroupKey when get CCSUP task for PAGE                 */
/*                                                                         */
/*                                                                         */
/* Data Modifications:                                                     */
/*                                                                         */
/* Date         Author  Ver   Purposes                                     */
/* 2025-02-11   JCH507  1.0   FCR-1917 Get CCSUP task in assigned group    */
/*                             or which group not assigned.(from TTMCC04)  */
/***************************************************************************/

CREATE   PROC    nspTTMCC10
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         AND TaskDetail.TaskType = 'CCSUP'
         AND TaskDetail.UserKey = ''
         AND TaskManagerUserDetail.UserKey = @c_userid
         AND TaskManagerUserDetail.PermissionType = 'CCSUP'
   ...
   ELSE
      ...
      BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Execute Of CrossDock Tasks Pick Code Failed. (nspTTMCC10)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + ' ) '
      END
   END
   ...
   BEGIN
      ...
      BEGIN
         ...
         END
      END
      execute nsp_logerror @n_err, @c_errmsg, 'nspTTMCC10'
