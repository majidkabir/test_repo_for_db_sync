...
/* 2019-12-13 1.0  CSCHONG    Created (WMS-11454)                               */     
/* 2020-01-31 1.1  CSCHONG    WMS-11454 revised address field mapping (CS01)    */  
/* 2020-02-06 1.2  Grick      Fixed the codelkup listname (G01)                 */  
/********************************************************************************/                  
                    
CREATE PROC [dbo].[isp_Bartender_Shipper_Label_JP_HMCOS]                        
      ...
      [Col60] [NVARCHAR] (80) NULL               
        ...
        IF @b_debug=1          
         BEGIN          
            PRINT 'start'            
         END          
   SET @c_SQLJOIN = + N' SELECT DISTINCT (N''Ã¼Âº'' + Substring(ORD.C_Zip,1,3) + ''-'' + Substring(ORD.C_Zip,4,4)),ORD.c_phone1,'   
                   -- + '(ORD.C_Address1 + Space(1) + ORD.C_City + Space(1) + ORD.C_State ) AS Add1,'+ CHAR(13) +     --3       --(CS01)   
                    + '(ORD.C_State + Space(1) + ORD.C_City + Space(1) + ORD.C_Address1 ) AS Add1,'+ CHAR(13) +     --3         --(CS01)  
                    + ' ORD.C_company,'  
               ...
               --+ 'round(OI.PayableAmount,0),'  
                    ...
                    + '  STO.company,STO.Phone1,(STO.Address1 + STO.Address2),STO.Zip,ISNULL(C4.UDF02,''''), '                       --15     
                    + CHAR(13) +        
                    + ' ISNULL(C4.UDF01,''''),STO.Susr2,STO.Susr3,STO.Susr5,STO.Notes1,'         --20        
                    + ' STO.Notes2,'''',C1.Description,C2.long,CONVERT(VARCHAR(10), getdate(),112),'  
                    + N' Substring(CONVERT(VARCHAR(10),dateadd(day,30,getdate()),112),3,2)+ N''Ã¶N'' + Substring(CONVERT(VARCHAR(10),dateadd(day,30,getdate()),112),5,2) '  
                    + N' + N''Ã®Ã„'' +  Substring(CONVERT(VARCHAR(10), dateadd(day, 30 , getdate()),112) , 7,2) + N''Ã´Â·Ã»Ã¿'','  
                    + ' ORD.c_VAT,PACKH.Orderkey,PACKDET.Cartonno,ISNULL(STO.B_Phone2,''''),'  --30    
                    ...
                    + ' LEFT JOIN CODELKUP C2 WITH (NOLOCK) ON C2.Listname = ''COSCARTON'' AND C2.UDF01 = CT.CarrierName AND C2.short=PI.Cartontype'       --G01  
                    + ' LEFT JOIN CODELKUP C3 WITH (NOLOCK) ON C3.Listname = ''Timeslot'' AND C3.Code = ''BLANK'''  
                    + ' LEFT JOIN CODELKUP C4 WITH (NOLOCK) ON C4.Listname = ''COSCS'' AND C4.Storerkey = ORD.Storerkey AND C4.long = ORD.ordergroup'  
...
EXIT_SP:      
   ...
   EXEC isp_InsertTraceInfo     
      ...
      @c_TraceName = 'isp_Bartender_Shipper_Label_JP_HMCOS',    
