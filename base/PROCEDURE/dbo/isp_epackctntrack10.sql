...
/************************************************************************/
/* Trigger: isp_EPackCtnTrack10                                         */
/* Creation Date: 30-May-2022                                           */
/* Copyright: LF Logistics                                              */
/* Written by: WLChooi                                                  */
/*                                                                      */
/* Purpose: WMS-19766 - DOTERRA CN ECOM Packing Get Tracking No         */
/*        :                                                             */
...
/************************************************************************/
CREATE PROC [dbo].[isp_EPackCtnTrack10]
...
BEGIN
   ...
   JOIN ORDERS OH (NOLOCK) ON OH.OrderKey = PACKHEADER.OrderKey
   ...
   BEGIN
      SET @n_continue = 3
      SET @n_err = 60010
      SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Get Empty Tracking #. (isp_EPackCtnTrack10)'
      GOTO QUIT_SP
   END
   ...
   BEGIN
      ...
      BEGIN
         SET @n_continue = 3
         SET @n_err = 60020
         SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Update PACKINFO Table. (isp_EPackCtnTrack10)'
   ...
   BEGIN
      ...
      BEGIN
         SET @n_continue = 3
         SET @n_err = 60030
         SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Insert PACKINFO Table. (isp_EPackCtnTrack10)'
                        + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
         GOTO QUIT_SP
      END
   END
   ...
   BEGIN
      ...
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'isp_EPackCtnTrack10'
