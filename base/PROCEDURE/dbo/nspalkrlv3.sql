...
/************************************************************************/      
/* Stored Procedure: nspALKRLV3                                         */      
/* Creation Date: 06-SEP-2020                                           */      
/* Copyright: LFL                                                       */      
/* Written by:                                                          */      
/*                                                                      */      
/* Purpose: WMS-14732 KR Levis allocation                               */  
/*          piece allocation from bulk (last carton)                    */
/*                                                                      */  
/* Called By: Wave                                                      */      
/*                                                                      */      
/* PVCS Version: 1.0                                                    */      
...
BEGIN      
   ...
   SET @c_ExtraCondition = ' AND LOC.LocationType = ''OTHER'' '
   ...
   BEGIN  
      ...
      BEGIN
         GOTO EXIT_SP  --skip if discrete allocation. only for wave conso
      END  
   END
   ...
   BEGIN      
      ...
      IF @n_QtyToTake > 0  
      BEGIN        	
      	SELECT TOP 1 @c_OtherValue = UCC.UCCNo
      	FROM UCC (NOLOCK)
      	LEFT JOIN PICKDETAIL PD (NOLOCK) ON UCC.UCCNo = PD.DropID AND UCC.Lot = PD.Lot AND UCC.Loc = PD.Loc AND UCC.Id = PD.Id AND PD.Status <> '9'
      	WHERE UCC.Lot = @c_Lot
      	AND UCC.Loc = @c_Loc
      	AND UCC.Id = @c_Id
      	AND UCC.Status <= '3'
      	GROUP BY UCC.UCCNo, UCC.Qty
      	HAVING UCC.Qty - SUM(ISNULL(PD.Qty,0)) >= @n_QtyToTake 
      	ORDER BY UCC.Qty - SUM(ISNULL(PD.Qty,0))
