...
/******************************************************************************/  
/* Store Procedure: nsp_PackListBySku03_chs                                   */  
/* Creation Date: 27-Jul-2011                                                 */  
/* Copyright: IDS                                                             */  
/* Written by: YTWAN (Copy from nsp_PackListBySku03)                          */  
/*                                                                            */  
/* Purpose:  SOS#221709 -Generate PickPack List - Chinese version             */  
/*                                                                            */  
...
/* 07-Dec-2011  YTWan     1.1   SOS#23201 - Add buyerPo to report. (Wan03)    */
...
BEGIN
	...
	(
		...
		BuyerPO            NVARCHAR(20) NULL --(Wan01)
	)                                               
	...
	BEGIN
	    ...
	    BEGIN
	       DECLARE SIZE_CUR CURSOR FAST_FORWARD READ_ONLY 
	       FOR
              SELECT CASE WHEN ISNULL(C.short,'')='Y' THEN                                                    --(CS01)
                     CASE WHEN S.measurement IN ('','U') THEN SUBSTRING(ISNULL(RTRIM(S.Size), ''), 1, 4)    --(CS01)
                                                           ELSE ISNULL(S.measurement,'') END                --(CS01)
                     ELSE S.Size END [Size] ,                                                            --(CS01)   
	           --SELECT SUBSTRING(ISNULL(RTRIM(S.Size), ''), 1, 4) SIZE,
	                  ISNULL(PD.Qty, 0)
	           FROM   PackDetail PD WITH (NOLOCK)
	                  JOIN SKU S WITH (NOLOCK)
	                       ON  (S.Storerkey = PD.Storerkey AND S.SKU = PD.SKU)
              LEFT JOIN CODELKUP C WITH (nolock) ON C.storerkey= PD.Storerkey               --(CS01)
               AND listname = 'REPORTCFG' and code ='GetSkuMeasurement'                             --(CS01)
	            AND long='r_dw_packing_list_by_sku03_chs'                                            --(CS01)
	           WHERE  PD.PickSlipNo = @c_PickSlipNo
	                  AND PD.CartonNo = @n_CartonNo
	                  AND S.Style = @c_Style
	                  AND S.Color = @c_Color
