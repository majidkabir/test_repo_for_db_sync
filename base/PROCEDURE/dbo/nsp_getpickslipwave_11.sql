...
GO
/************************************************************************/
/* Stored Procedure: nsp_GetPickSlipWave_11                             */
/* Creation Date: 10-Jul-2015                                           */
/* Copyright: IDS                                                       */
/* Written by: CHONG                                                    */
/*                                                                      */
/* Purpose: SOS#347168 - Under Armour (UA) - Pick Slip                  */
/*          - copy from nsp_GetPickSlipWave_11                          */
/*                                                                      */
...
CREATE PROC [dbo].[nsp_GetPickSlipWave_11] (
...
IF EXISTS(SELECT 1 FROM PickHeader (NOLOCK) WHERE Wavekey = @c_wavekey AND Zone = '3')
BEGIN
...
AND    (PICKDETAIL.Pickmethod = '3' OR PICKDETAIL.Pickmethod = ' ' OR
        PICKDETAIL.Pickmethod = 'L' OR PICKDETAIL.Pickmethod = 'F')-- user wants it to be on lists
-- Ignore this portion..
...
@c_orderkey,  @c_UOM, @c_logicalloc, @c_lot, @c_toloc, @c_moveid, @c_Id 

WHILE (@@FETCH_STATUS <> -1)
BEGIN --While
	IF @c_OrderKey <> @c_PrevOrderKey
	BEGIN
	   IF NOT EXISTS( SELECT 1 FROM PICKHEADER (NOLOCK) WHERE WaveKey = @c_wavekey AND OrderKey = @c_OrderKey AND ZONE = '3')
	   BEGIN  --Not Exist in PickHeader
         ...
         @c_errmsg      	OUTPUT
      	...
      	BEGIN TRAN
         --select '1' ,@c_pickheaderkey,@c_wavekey,@c_OrderKey

      	INSERT INTO PICKHEADER
      	(PickHeaderKey,    OrderKey,    WaveKey, PickType, Zone, TrafficCop)
      	VALUES
      	(@c_pickheaderkey, @c_OrderKey, @c_wavekey,     '0',      '3',  '')
   ...
   ELSE
   BEGIN --if @c_orderkey <> ''
      SELECT @c_ConsigneeKey = CASE WHEN ISNULL(SC.Svalue,'') =  '1' THEN   
                                 Orders.Consigneekey               
                               ELSE 
                                 Orders.BillToKey
                               END,
      ...
      FROM   ORDERS (NOLOCK)
      LEFT JOIN   STORERCONFIG SC (NOLOCK) ON (ORDERS.Storerkey = SC.Storerkey AND SC.Configkey='WAVEPS11_SHOWCONSIGNEE')  
   ...
   */
   ...
   IF @c_Lottable06    IS NULL SELECT @c_Lottable06 = ''
   IF @c_Lottable07    IS NULL SELECT @c_Lottable07 = ''
   IF @c_Lottable08    IS NULL SELECT @c_Lottable08 = ''
   IF @c_Lottable09    IS NULL SELECT @c_Lottable09 = ''
   IF @c_Lottable10    IS NULL SELECT @c_Lottable10 = ''
   IF @c_Lottable11    IS NULL SELECT @c_Lottable11 = ''
   ...
   PackUOM1,            PackUOM2,         PackUOM3,         ID, 
   BuyerPO,Lottable06,Lottable07,Lottable08,Lottable09,Lottable10,Lottable11)  
   VALUES
   ...
   CAST(@c_UOM as int), @n_UOMQty,	   	@c_PrintedFlag,   '3',
   @c_Lot,		   		ISNULL(@c_Carrierkey,''),    ISNULL(@c_VehicleNo,''),     @c_Lottable02,
   ...
   @n_innerpack,			@c_busr8,         @c_Lottable03,    ISNULL(@c_AltSKU,''),
	ISNULL(@c_Susr2,''),            @c_BUSR10,        @c_toloc,         @c_moveid,
   ISNULL(@c_ToLocPutawayzone, ''),       @c_Lottable01,    @n_StdCube,   @n_StdGrossWgt, 
   @c_PackUOM1,         @c_PackUOM2,      @c_PackUOM3 ,     @c_Id,   
   ISNULL(@c_BuyerPO,''),@c_Lottable06,@c_Lottable07,@c_Lottable08,@c_Lottable09,
               ...
               BEGIN
                  ...
                  BEGIN
                     ...
                     BEGIN
                        ...
                        Select @c_errmsg= 'NSQL'+CONVERT(char(5), @n_err)+':Insert failed on TransmitLog2. (nsp_GetPickSlipWave_11)'+'('+'SQLSvr MESSAGE='+LTrim(RTrim(@c_errmsg))+')'
                        ROLLBACK TRAN 
                        GOTO QUIT 
                     END
                     ELSE
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN  
            ...
            SELECT @n_continue=3
            SELECT @n_err = @@ERROR
            SELECT @c_errMsg = 'Insert into TransmitLog Failed (nsp_GetPickSlipWave_11)'                     
            GOTO QUIT 
         END  
         ELSE 
         BEGIN
...
-- End : SOS49892
   ...
   ,Lottable06,Lottable07,Lottable08,Lottable09,Lottable10,Lottable11
   FROM #TEMP_PICK
...
QUIT:
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         END  
      END  
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'nsp_GetPickSlipWave_11'  
