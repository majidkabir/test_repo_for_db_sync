...
/***************************************************************************/
/* Stored Procedure: ispTransferAllocation05                               */
/* Creation Date: 06-Jul-2023                                              */
/* Copyright: MAERSK                                                       */
/* Written by: WLChooi                                                     */
/*                                                                         */
/* Purpose: WMS-22933 - [CN] LULULEMON WMS Transfer Allocation NEW         */
/*                                                                         */
...
CREATE   PROC [dbo].[ispTransferAllocation05](    
...
BEGIN    
   ...
   DECLARE @c_Transmitlogkey     NVARCHAR(10)   = ''  
         ...
         , @c_Status_Current        NVARCHAR(10) = ''    
         , @n_Open_Current          INT = 0   
  
   DECLARE @CUR_MANTRAN             CURSOR  
         , @CUR_TFRDET              CURSOR    
         , @CUR_LLI                 CURSOR    
         , @CUR_PM                  CURSOR  
   ...
   BEGIN  
      ...
      INSERT INTO @tTRF (TransferKey, Facility, FromStorerkey, ToFacility, TRFType)  
   ...
   BEGIN    
      ...
      BEGIN    
         ...
         SET @c_errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Transfer(s) is not allowed to re-allocate. (ispTransferAllocation05)'    
         GOTO QUIT_SP               
      END    
   END    
   ...
   SET @CUR_MANTRAN = CURSOR LOCAL FORWARD_ONLY STATIC READ_ONLY FOR    
   ...
   FETCH NEXT FROM @CUR_MANTRAN INTO   @c_Transmitlogkey    
                                    ,  @c_Transferkey    
                                    ,  @c_TransferType    
                                    ,  @c_FromFacility  
                                    ,  @c_FromStorerkey  
   ...
   BEGIN    
      ...
      BEGIN  
         ...
         BEGIN      
            ...
            SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': UPDATE TRANSMITLOG3 Failed. (ispTransferAllocation05)'     
      ...
      BEGIN   
         ...
         BEGIN    
            ...
            BEGIN 
               ...
               --Always Finalize Line no matter fully/partially/not allocated
               ...
               BEGIN    
                  ...
                  BEGIN    
                     ...
                     SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': UPDATE TRANSFERDETAIL Failed. (ispTransferAllocation05)'    
                                    + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
                     GOTO NEXT_TRF    
                  END    
               ...
               BEGIN    
                  ...
                  BEGIN    
                     ...
                     SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': INSERT TRANSFERDETAIL Failed. (ispTransferAllocation05)'    
                                    + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
                     GOTO NEXT_TRF    
                  END    
               END    
               ...
               BEGIN  
                  ...
                  BEGIN    
                     SET @n_Continue= 3    
                     SET @n_err  = 81100    
                     SET @c_errmsg = 'Execute ispFinalizeTransfer Failed. (ispTransferAllocation05)'    
                                    + '(' + @c_errmsg + ')'    
                     GOTO NEXT_TRF    
                  END           
               END
               ...
               BEGIN  
                  ...
                  BEGIN    
                     SET @n_continue = 3    
                     SET @n_err = 81110    
                     SET @c_errmsg  = CONVERT(char(5),@n_err)+': Error Executing isp_ChannelInvHoldWrapper. (ispTransferAllocation05)'    
                  END    
               END  
               ---------------------------------------  
         ...
         BEGIN   
            ...
            IF @n_FromQty = @n_QtyRemaining   --Not Allocated - FromQty = QtyRemaining
            BEGIN
               SET @c_TransferStatus_D = '0'
            END
      ...
      --Calculate Transfer Header Status    
      SELECT @c_TransferStatus_H = '9'
      ...
      BEGIN   
         ...
         BEGIN    
            ...
            BEGIN    
               ...
               SET @b_Success = 0    
               SET @n_err  = 81190    
               SET @c_errmsg = 'Execute ispPostFinalizeTransferWrapper Failed. (ispTransferAllocation05)'    
                              + '(' + @c_errmsg + ')'    
               GOTO NEXT_TRF    
            END    
         END   
      ...
      END   
      /*    
      SET @c_SQL = N' UPDATE [TRANSFER] ' 
                 + N' SET [Status] = ''9'' ' 
                 + N'    ,OpenQty  = @n_OpenQty'
                 + N'    ,EditWho  = @c_UserID '
                 + N'    ,EditDate = GETDATE() '
                 + N'    ,Trafficcop = NULL' 
                 + N' WHERE Transferkey = @c_Transferkey'

      SET @c_SQLParms = N' @c_TransferStatus_H  NVARCHAR(10)' 
                      + N',@n_OpenQty     INT' 
                      + N',@c_UserID      NVARCHAR(128)'
      ...
      */
      ...
      BEGIN    
         ...
         BEGIN
            ...
            SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5), @n_Err) + ': UPDATE TRANSFER Failed. (ispTransferAllocation05)'
                            + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_ErrMsg) + ' ) '
            GOTO NEXT_TRF
         END
         ...
         BEGIN
            SET @n_Continue = 3
            SET @c_ErrMsg = CONVERT(NVARCHAR(250), @n_Err)
            SET @n_Err = 81201 -- Should Be Set To The SQL Errmessage but I don't know how to do so.    
            SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5), @n_Err) + ': EXEC isp_ITF_ntrTransfer Failed. (ispTransferAllocation05)'
                            + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_ErrMsg) + ' ) '
            GOTO NEXT_TRF
         END
      END
      ...
      BEGIN    
         ...
         EXEC nspLogAlert    
               @c_modulename       = 'ispTransferAllocation05'    
   ...
   BEGIN    
      ...
      AND   Code = 'ispTransferAllocation05'    
      AND StorerKey = @c_FromStorerkey    
    
      IF ISNULL(@cRecipients, '') <> ''    
      BEGIN    
         SET @cSubject = 'LULULEMON Transfer Allocation Alert - ' + REPLACE(CONVERT(NVARCHAR(19), GETDATE(), 126),'T',' ')    
        ...
        EXEC msdb.dbo.sp_send_dbmail    
         ...
         BEGIN    
            SET @n_err = 81220    
            SET @c_Errmsg = 'Error executing sp_send_dbmail. (ispTransferAllocation05)'    
