...
GO
/*************************************************************************/  
/* Stored Procedure: ispRVWAV33                                          */  
/* Creation Date: 31-Mar-2020                                            */  
/* Copyright: LFL                                                        */  
/* Written by: WLChooi                                                   */  
/*                                                                       */  
/* Purpose: WMS-12529 - CN DYSON Reverse Wave Released                   */
/*                                                                       */  
/* Called By: Wave                                                       */  
/*                                                                       */  
/* PVCS Version: 1.0                                                     */  
...
CREATE PROCEDURE [dbo].[ispRVWAV33]      
 ...
 BEGIN  
   ...
   BEGIN
       IF NOT EXISTS (SELECT 1 FROM TASKDETAIL TD (NOLOCK) 
                      WHERE TD.Wavekey = @c_Wavekey AND TD.SourceType = 'ispRLWAV33'
                      AND TD.TaskType IN ('RPF','FPK','FCP'))
       BEGIN                                          
         SELECT @n_continue = 3  
         SELECT @n_err = 81010  
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': This Wave has not been released. (ispRVWAV33)'         
       END                 
   END
   ...
   BEGIN
       IF EXISTS (SELECT 1 FROM TASKDETAIL TD (NOLOCK) 
                  ...
                  AND TD.Sourcetype = 'ispRLWAV33'
                  AND  TD.TaskType IN ('RPF','FPK','FCP')
                  AND TD.Status <> '0')
       BEGIN
         SELECT @n_continue = 3  
         SELECT @n_err = 81020  
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Some Tasks have been started. Not allow to Reverse Wave Released (ispRVWAV33)'       
       END                 
   END
      ...
      BEGIN      
       ...
       EXEC rdt.rdt_Putaway_PendingMoveIn 
        ...
        BEGIN
            SELECT @n_continue = 3  
            SELECT @n_err = 81025  
            SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+':  Execute rdt.rdt_Putaway_PendingMoveIn Failed. (ispRVWAV33)'       
        END                            
   ...
   */
   ...
   BEGIN
        ...
        AND TASKDETAIL.Sourcetype = 'ispRLWAV33'
        AND TASKDETAIL.TaskType IN ('RPF','FPK','FCP')
        ...
        BEGIN
          ...
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Delete Taskdetail Table Failed. (ispRVWAV33)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
        END 
   END   
   ...
   BEGIN
      DELETE FROM REPLENISHMENT
      WHERE OriginalFromLoc = 'ispRLWAV33' AND Wavekey = @c_WaveKey
      ...
      BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Delete from Replenishment Table Failed. (ispRVWAV33)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
      END          
   END  
   ...
   BEGIN
        ...
        BEGIN
          ...
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Pickdetail Table Failed. (ispRVWAV33)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
        END          
   END        
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update on wave Failed (ispRVWAV33)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
      END  
   END  
...
RETURN_SP:
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         END  
      END  
      execute nsp_logerror @n_err, @c_errmsg, "ispRVWAV33"  
