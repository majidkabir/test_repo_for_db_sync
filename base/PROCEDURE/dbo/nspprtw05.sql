      ...
      BEGIN
         ...
         BEGIN
            IF NOT EXISTS (SELECT 1 FROM CODELKUP CL (NOLOCK)
                             ...
                             AND CL.Long = 'nspPRTW05'
                             AND CL.Code2 = 'nspPRTW05'
                             AND ISNULL(CL.Short,'') <> 'N')
            BEGIN
               SELECT @c_Condition = dbo.fnc_RTrim(@c_Condition) + ' AND LOTTABLE10 = '''' '                                                       --(Wan01)
            END
         END
         --WL01 End
