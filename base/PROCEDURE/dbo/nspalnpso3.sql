...
/************************************************************************/        
/* Stored Procedure: nspALNPSO3                                         */        
/* Creation Date: 06-APR-2022                                           */        
/* Copyright: LFL                                                       */        
/* Written by:                                                          */        
/*                                                                      */        
/* Purpose:  WMS-19401 TH Nespresso allocation                          */  
/*          copy from nspALNPSO2 (WMS-17792)                            */
/*                                                                      */    
/* Called By: Wave                                                      */        
/*                                                                      */        
/* PVCS Version: 1.0                                                    */        
...
BEGIN        
   ...
   IF @c_OrderType = 'OOH'
   BEGIN
      SELECT @n_B2CCnt = SUM(CASE WHEN LOC.Putawayzone IN('PTLB2CZONE','PTLB2CZON2','PTLB2CZON3') THEN  1 ELSE 0 END),
             @n_B2BCnt = SUM(CASE WHEN LOC.Putawayzone IN('PTLB2BZONE','PTLB2BZON2') THEN  1 ELSE 0 END)      
      FROM ORDERDETAIL (NOLOCK)
      JOIN SKUXLOC (NOLOCK) ON ORDERDETAIL.Storerkey = SKUXLOC.Storerkey AND ORDERDETAIL.Sku = SKUXLOC.Sku
      JOIN LOC (NOLOCK) ON SKUXLOC.Loc = LOC.Loc  
      WHERE SKUXLOC.Qty - SKUXLOC.QtyAllocated - SKUXLOC.QtyPicked > 0   
      AND Loc.LocationType = 'PICK'
      AND ORDERDETAIL.Orderkey = @c_Orderkey
      
      IF ISNULL(@n_B2CCnt,0) > 0 AND ISNULL(@n_B2CCnt,0) > 0
         SET @c_PAZones = 'PTLB2BZONE'',''PTLB2CZON3'
   END

   IF @c_OrderType IN('HOME','OOH') AND @c_SkuGroup = 'NESCOFEE' AND ISNULL(@c_PAZones,'') = ''
   BEGIN       	
      ...
      BEGIN  
         ...
         WHERE LOC.Putawayzone IN ('PTLB2CZONE','PTLB2CZON2','PTLB2CZON3')  
         AND SL.Qty - SL.QtyAllocated - SL.QtyPicked > 0   
         AND Loc.LocationType = 'PICK'  
         AND OD.Orderkey = @c_Orderkey  
         --AND SKU.SkuGroup = 'NESCOFEE'
         GROUP BY OD.Sku  
      ...
      IF ISNULL(@c_OrderPutwayzone,'') <> '' AND  --check if the sku available in the same PAZone
         NOT EXISTS(SELECT 1    
                    ...
                    AND LOC.Putawayzone = @c_OrderPutwayzone
      ...
      */                
      ...
      BEGIN
         ...
         ELSE IF EXISTS (SELECT 1    
                    ...
                    AND SKUXLOC.Qty - SKUXLOC.QtyAllocated - SKUXLOC.QtyPicked > 0   
                    AND Loc.LocationType = 'PICK'  
                    HAVING COUNT(DISTINCT LOC.Putawayzone) = 3)   
         BEGIN                                                
            SELECT @c_PAZones = CASE WHEN (S.rowno % 3) = 1 THEN 'PTLB2CZONE' 
                                     WHEN (S.rowno % 3) = 2 THEN 'PTLB2CZON2' 
                                     ELSE 'PTLB2CZON3' END             
            FROM (SELECT WD.orderkey, ROW_NUMBER() OVER (ORDER BY WD.Orderkey) AS rowno  
                  FROM WAVEDETAIL WD (NOLOCK)  
   ...
   BEGIN  
      ...
      BEGIN
      	 IF EXISTS(SELECT 1    
                   ...
                   WHERE SKUXLOC.Storerkey = @c_Storerkey  
                   AND SKUXLOC.Sku = @c_Sku  
                   AND LOC.Putawayzone = @c_OrderPAzone
                   AND SKUXLOC.Qty - SKUXLOC.QtyAllocated - SKUXLOC.QtyPicked > 0   
                   AND Loc.LocationType = 'PICK')        
         BEGIN                              
   ...
   ELSE IF @C_OrderType IN('HOME','OOH')   
      SET @c_PAZones = ' AND Loc.Putawayzone IN ( ''PTLB2CZON2'', ''PTLB2BZONE'',''PTLB2BZON2'',''PTLB2CZONE'',''PTLB2CZON3'') '  
   ELSE  
      SET @c_PAZones = ' AND Loc.Putawayzone IN ( ''AIRCON'', ''PTLB2BZONE'',''PTLB2BZON2'', ''PTLB2CZONE'',''PICK2PLTZ1'') '   

   SET @n_ShelfLife = 0     
   IF @c_SkuGroup IN('NESCOFEEB','NESCOFEE')
   BEGIN
   ...
   SELECT --@n_StorerMinShelfLife = ((Sku.Shelflife * Storer.MinShelflife/100) * -1),  
          @n_SkuMinShelfLife = CASE WHEN ISNUMERIC(Sku.Susr2) = 1 THEN CAST(Sku.Susr2 AS INT) ELSE 0 END,  
   ...
   SET @c_SQL = N'       
     ...
     WHERE LOC.Status <> ''HOLD''    
      ...
      CASE WHEN @C_OrderType = 'BQ' THEN ' AND Loc.Putawayzone IN(''AMZONE'',''AIRCON'',''PICK2PLTZ1'') ' --NJOW05  
           WHEN @C_OrderType = 'HOME' AND @c_UserDefine02 = 'H3' THEN ' AND Loc.Putawayzone IN(''AMZONE'',''AIRCON'',''PICK2PLTZ1'') '  
