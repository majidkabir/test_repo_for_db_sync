...
GO
/************************************************************************/
/* Stored Procedure: nspALCONV1                                         */
/* Creation Date: 10-OCT-2018                                           */
/* Copyright: LFL                                                       */
/* Written by:                                                          */
/*                                                                      */
/* Purpose: WMS-6435 - CN Converse BTB PTS                              */
/*          UOM2 - order/conso full pallet from pallet loc              */
/*          Only take unallocated pallet with full carton               */
/*          Order qty >= pallet id qty                                  */
/*                                                                      */
/* Called By:                                                           */
/*                                                                      */
/* PVCS Version: 1.0                                                    */
...
BEGIN    
   ...
   SET @c_SQL = N'      
      DECLARE CURSOR_ID CURSOR FAST_FORWARD READ_ONLY FOR
      ...
      AND LOTxLOCxID.QTY % IIF(PACK.Casecnt=0, 1, CAST(PACK.Casecnt AS INT)) = 0
      AND LOTxLOCxID.QTY <= @n_QtyLeftToFulfill
      ...
      'GROUP BY LOTxLOCxID.LOT, LOTxLOCxID.LOC, LOTxLOCxID.ID, LOC.LogicalLocation, LOC.LOC, LOC.LocationHandling, LOC.LocationType, LA.Lottable05 ' +    
      @c_SortBy
   ...
   FETCH NEXT FROM CURSOR_ID INTO @c_LOT, @c_LOC, @c_ID, @n_QtyAvailable   
   ...
   BEGIN                         	                  
      ...
      FETCH NEXT FROM CURSOR_ID INTO @c_LOT, @c_LOC, @c_ID, @n_QtyAvailable  
   END -- END WHILE FOR CURSOR_ID          

   IF CURSOR_STATUS('GLOBAL' , 'CURSOR_ID') in (0 , 1)          
   BEGIN          
      CLOSE CURSOR_ID          
