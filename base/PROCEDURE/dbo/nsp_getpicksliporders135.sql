...
GO
/************************************************************************/  
/* Store Procedure:  nsp_GetPickSlipOrders135                           */  
...
/* Return Status:  None                                                 */  
/*                                                                      */  
/* Usage:  Used for report dw = r_dw_print_pickorder135                 */  
/*                                                                      */  
/* Local Variables:                                                     */  
/*                                                                      */  
/* Called By:                                                           */  
...
/* 2022-11-03  Mingle    1.0   DevOps Script Combine(WMS-21123)         */  
/************************************************************************/  
  
CREATE PROC [dbo].[nsp_GetPickSlipOrders135] (@c_loadkey NVARCHAR(10))  
 AS  
 BEGIN  
   SET NOCOUNT ON  
   SET ANSI_NULLS OFF  
 ...
 SET @c_ColName = @c_udf03  
   ...
   ,PFLoc      NVARCHAR(10)    
       ...
       INSERT INTO #TEMP_PICK  
            ...
            ,Storerkey,showbarcode,showcontactphone,c_contact1,c_phone1, ShowLottable02,PFLoc )        
        SELECT  
       ...
       ,ORDERS.InvoiceNo                                
       ,IsNull(ORDERS.C_Address4,'') AS Addr4l  
   ...
   ,isnull(orders.c_phone1,'')  as c_phone1  
       ,ISNULL(CLR2.Short,'N') AS ShowLottable02     
   ,MAX(UPPER(sl.Loc)) AS PFLoc   
  ...
  LEFT OUTER JOIN SKUXLOC sl (NOLOCK) ON pickdetail.SKU = sl.SKU AND sl.LocationType = 'PICK' AND sl.Storerkey = PickDetail.Storerkey    
