...
/************************************************************************/  
/* Stored Proc : ispPKLBLToOrd04                                        */  
/* Creation Date: 12/11/2021                                            */  
/* Copyright: LFL                                                       */  
/* Written by:                                                          */  
/*                                                                      */  
/* Purpose: WMS-18240 Assign pack label# to picketail include UPC       */ 
/*          to ALTSKU                                                   */            
/*                                                                      */  
...
BEGIN  
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         SELECT @c_errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Total PackQty ('+ CAST(@n_TotPackQty AS VARCHAR) +  
                            ') vs PickQty ('+ CAST(@n_TotPickQty AS VARCHAR) +') Not Tally. (ispPKLBLToOrd04)'  
      END  
   END  
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         BEGIN  
            ...
            SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Pickdetail Table Failed. (ispPKLBLToOrd04)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '  
         END  
         ...
         BEGIN  
            ...
            BEGIN  -- pickqty > packqty  
               ...
               BEGIN  
                  ...
                  SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert Pickdetail Table Failed. (ispPKLBLToOrd04)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '  
                  BREAK  
               END  
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         END  
      END  
      EXECUTE nsp_logerror @n_err, @c_errmsg, "ispPKLBLToOrd04"  
