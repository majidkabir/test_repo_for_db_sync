...
/************************************************************************/
/* Store Procedure:  isp_Transfer_Ticket_04                             */
...
/* Return Status:  None                                                 */
/*                                                                      */
/* Usage:  Used for report dw = r_dw_transfer_ticket_04                 */
/*                                                                      */
/* Local Variables:                                                     */
/*                                                                      */
/* Called By:                                                           */
...
/************************************************************************/
CREATE PROC [dbo].[isp_Transfer_Ticket_04] (@c_TransferKey NVARCHAR(10))
 AS
BEGIN
    SET NOCOUNT ON
    SET QUOTED_IDENTIFIER OFF
    ...
    DECLARE @n_continue       INT
         ...
         ,  @c_ClkStorerKey   NVARCHAR(15)	
         ,  @b_ClkStorerKey	NVARCHAR(15)	
   ...
   CREATE TABLE #TMP_TRFTICKET04
	  ...
	  ,  FromSKUBUSR10     NVARCHAR(30)   NULL
	  ,  ToSKUBUSR10       NVARCHAR(30)   NULL
      )

   INSERT INTO #TMP_TRFTICKET04
      (  Transferkey
      ,  CustomerRefNo
      ,  ReasonCode
      ,  AddDate
   ...
   JOIN CODELKUP CODELKUP_b  WITH (NOLOCK) ON ( CODELKUP_b.Listname = 'TRNReason' )
                                         ...
                                         AND( CODELKUP_b.StorerKey = @b_ClkStorerKey)	
   ...
   JOIN SKU FromSKU WITH (NOLOCK) ON FromSKU.StorerKey = TRANSFERDETAIL.FromStorerKey AND FromSKU.Sku = TRANSFERDETAIL.FromSku
   JOIN SKU ToSKU WITH (NOLOCK) ON ToSKU.StorerKey = TRANSFERDETAIL.ToStorerKey AND ToSKU.Sku = TRANSFERDETAIL.ToSku
   WHERE ( TRANSFERDETAIL.TransferKey = @c_Transferkey )
   GROUP BY TRANSFER.TransferKey, 
   TRANSFER.CustomerRefNo,
   TRANSFER.ReasonCode,
   TRANSFER.AddDate,
   TRANSFER.FromStorerKey,
   ...
   AND   Long = 'r_dw_transfer_ticket_04'
   ...
   BEGIN
      ...
      SET @c_SQL = ''
      SET @c_SQL = N' UPDATE #TMP_TRFTICKET04'
                 +  ' SET FromSKU = ' + @c_UDF01 + ' + ' + @c_UDF02 + ' + ' + @c_UDF03
                 +  ' FROM  #TMP_TRFTICKET04 TMP '
   ...
   BEGIN
      ...
      BEGIN
         ...
         END
      END
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'isp_Transfer_Ticket_04'
