...
GO
/************************************************************************/
/* Stored Procedure: nspTTMEvaluateMVTasks                              */
/* Creation Date:                                                       */
/* Copyright: IDS                                                       */
/* Written by:                                                          */
/*                                                                      */
...
CREATE PROC    [dbo].[nspTTMEvaluateMVTasks]
...
BEGIN
   ...
   BEGIN
      DECLAREcursor_MVTASKCANDIDATES:
      ...
      BEGIN
         SELECT @n_continue = 3
         SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err=78801   -- Should Be Set To The SQL Errmessage but I don't know how to do so.
         SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Execute Of Move Tasks Pick Code Failed. (nspTTMEvaluateMVTasks)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
      END
      IF @n_err = 16915
      BEGIN
         CLOSE cursor_MVTASKCANDIDATES
         DEALLOCATE cursor_MVTASKCANDIDATES
         GOTO DECLAREcursor_MVTASKCANDIDATES
      END
      OPEN CURSOR_MVTASKCANDIDATES
   ...
   BEGIN
      ...
      BEGIN
         FETCH NEXT FROM cursor_MVTASKCANDIDATES INTO @c_taskdetailkey
         IF @@FETCH_STATUS = -1
         BEGIN
            BREAK
         END
         ...
         BEGIN
            ...
            @c_toloc =  LogicalToLoc, -- FBR028c, to handle the case where toloc <> logicalToloc, toloc = final destination --taskdetail.toloc ,
            ...
            BEGIN
               ...
               BEGIN
                  BEGIN TRANSACTION
                     ...
                     BEGIN
                        SELECT @n_continue = 3
                        SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err=78802   -- Should Be Set To The SQL Errmessage but I don't know how to do so.
                        SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Error Occurred While Attempting To Update TaskDetail. (nspTTMEvaluateMVTasks)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
                     END
                     IF @n_continue = 1 or @n_continue = 2
                     BEGIN
                        UPDATE TASKDETAIL
                        ...
                        BEGIN
                           SELECT @n_continue = 3
                           SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err=78803   -- Should Be Set To The SQL Errmessage but I don't know how to do so.
                           SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Error Occurred While Attempting To Update TaskDetail. (nspTTMEvaluateMVTasks)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
                        END
                        IF @n_cnt = 1
                        BEGIN
            ...
            ELSE
               BEGIN
                  ...
                  SELECT @b_success = 1
                  EXECUTE nspLogAlert
                  @c_ModuleName   = "nspTTMEvaluateMVTasks",
   ...
   ELSE
      ...
      ELSE
         BEGIN
            ...
            END
         END
         execute nsp_logerror @n_err, @c_errmsg, "nspTTMEvaluateMVTasks"
