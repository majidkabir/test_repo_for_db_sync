...
/************************************************************************/  
/* Stored Procedure: isp_GetKIOSKASRSPKRevTask_b                        */  
...
/*                                                                      */  
/* Called By: d_dw_kiosk_asrspk_rev_b_form                              */  
/*                                                                      */  
/* PVCS Version: 1.2                                                    */  
/*                                                                      */  
/* Version: 7.0                                                         */  
...
/* 12-JAN-2016 Wan01    1.1   Fixed. Pallet Qty = Qty - QtyPicked       */  
/* 03-JAN-2019 Wan02    1.2   WMS-7286 - PRHK-GTM Picking For COPACK Sku*/   
/************************************************************************/  
CREATE PROC [dbo].[isp_GetKIOSKASRSPKRevTask_b]   
...
,    Remarks             NVARCHAR(80)  NOT NULL DEFAULT ('')      --(Wan02)
   ...
   INSERT INTO #Result (TaskDetailKey,FROMID,TOID,PalletID,OrderKey,OrderStatus,OrderLineNumber
                     ,Storerkey,SKU,SKUDescr,SUSR4,Lot,LottableCode
                     ...
                     ,CaseCnt, PackUOM1,PackUOM3,PalletQtyInCS,PalletQtyInEA,OrderQtyInCS,OrderQtyInEA
                     ,QtyToPickInCS,QtyToPickInEA,TaskStatus,JobStatus)
   SELECT TOP 1 
   ...
   ,PalletQtyInCS   = 0--CASE WHEN PACK.CaseCnt > 0 AND @n_ShowUOMEA = 0 THEN FLOOR(@n_Qty / CaseCnt) ELSE 0 END
   ,PalletQtyInEA   = 0--CASE WHEN PACK.CaseCnt > 0 AND @n_ShowUOMEA = 0 THEN @n_Qty % CONVERT(INT,CaseCnt) ELSE @n_Qty END
   ,OrderQtyInCS    = 0--CASE WHEN PACK.CaseCnt > 0 AND @n_ShowUOMEA = 0 THEN FLOOR(@n_OrderQty / PACK.CaseCnt) ELSE 0 END 
   ,OrderQtyInEA    = 0--CASE WHEN PACK.CaseCnt > 0 AND @n_ShowUOMEA = 0 THEN @n_OrderQty % CONVERT(INT, PACK.CaseCnt) ELSE @n_OrderQty END
   ,QtyToPickInCS   = 0--CASE WHEN PACK.CaseCnt > 0 AND @n_ShowUOMEA = 0 THEN FLOOR(@n_QtyToPick / PACK.CaseCnt) ELSE 0 END 
   ,QtyToPickInEA   = 0--CASE WHEN PACK.CaseCnt > 0 AND @n_ShowUOMEA = 0 THEN @n_QtyToPick % CONVERT(INT, PACK.CaseCnt) ELSE @n_QtyToPick END
   ,TaskStatus= @c_TaskStatus  
   ,JobStatus = @c_JobStatus
   FROM @t_Pickdetail t                                                                            --(Wan02)
   JOIN TASKDETAIL   WITH (NOLOCK) ON (t.TaskDetailKey = TASKDETAIL.TaskDetailKey)                 --(Wan02)
   LEFT JOIN PICKDETAIL   WITH (NOLOCK) ON (t.PickDetailKey = PICKDETAIL.PickDetailKey)            --(Wan02)
   ...
   ORDER BY CASE WHEN PICKDETAIL.Status = '5' AND @c_JobStatus < '5' THEN 5 ELSE 1 END
   ...
   SET PalletQtyInCS   = CASE WHEN CaseCnt > 0 AND @n_ShowUOMEA = 0 THEN FLOOR(@n_Qty / CaseCnt) ELSE 0 END 
      ...
      ,OrderQtyInCS    = CASE WHEN CaseCnt > 0 AND @n_ShowUOMEA = 0 THEN FLOOR(@n_OrderQty / CaseCnt) ELSE 0 END 
      ,OrderQtyInEA    = CASE WHEN CaseCnt > 0 AND @n_ShowUOMEA = 0 THEN @n_OrderQty % CONVERT(INT, CaseCnt) ELSE @n_OrderQty END
   ...
   SELECT @c_JobKey,TaskDetailKey,FROMID,TOID,PalletID,OrderKey,OrderStatus,OrderLineNumber
         ,Storerkey,SKU,SKUDescr,Susr4,Lot,LottableCode 
         ,Lottable01label,Lottable02label,Lottable03label,Lottable04label,Lottable05label 
         ,Lottable06label,Lottable07label,Lottable08label,Lottable09label,Lottable10label 
         ,Lottable11label,Lottable12label,Lottable13label,Lottable14label,Lottable15label
