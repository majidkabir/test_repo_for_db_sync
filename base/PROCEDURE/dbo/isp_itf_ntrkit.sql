...
GO
/************************************************************************/  
/* Store Procedure:  isp_ITF_ntrKIT                                     */  
/* Creation Date: 08-Nov-2016                                           */  
/* Copyright: LF                                                        */  
/* Written by: MCTang                                                   */  
/*                                                                      */  
/* Purpose:  Handling trigger points for KIT's module.                  */  
/*           Including KIT Header trigger points for Add&Update         */  
/*                                                                      */  
...
CREATE PROC [dbo].[isp_ITF_ntrKIT]  
...
BEGIN  
   ...
   IF (ISNULL(RTRIM(@c_TriggerName),'') = '') OR   
      ...
      (ISNULL(RTRIM(@c_KitKey),'') = '')  
   BEGIN  
      RETURN  
   END  
  
   IF (ISNULL(RTRIM(@c_TriggerName),'') <> 'ntrKitHeaderUpdate')  
   BEGIN  
      IF (ISNULL(RTRIM(@c_TriggerName),'') <> 'ntrKitHeaderAdd')  
      BEGIN  
         RETURN  
      END  
   END  
  
   IF (ISNULL(RTRIM(@c_SourceTable),'') <> 'KIT')  
   BEGIN  
      RETURN  
   END  
   /*************************************************************************************/  
   ...
   BEGIN  
      SELECT @c_StorerKey     = ISNULL(RTRIM(KIT.StorerKey),'')   
           , @c_Facility      = ISNULL(RTRIM(KIT.Facility),'')   
      ...
      BEGIN   
         ...
         BEGIN 
            ...
            BEGIN
               ...
               EXEC sys.sp_executesql @c_StoredProc, N'@c_KitKey NVARCHAR(10), @b_Success INT OUTPUT, @c_ErrNo INT OUTPUT, @c_ErrMsg NVARCHAR(215) OUTPUT',
                           @c_KitKey, 
                           @b_Success OUTPUT, 
                           @n_Err     OUTPUT, 
                           @c_ErrMsg  OUTPUT 
            ...
            BEGIN
               ...
               BEGIN  
                  EXEC ispGenTransmitLog3 @c_Tablename, @c_KitKey, '', @c_StorerKey, ''  
                                          , @b_success OUTPUT  
                                          , @n_Err OUTPUT  
                                          , @c_ErrMsg OUTPUT  
                  ...
                  BEGIN  
                     ...
                     SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_Err,0)) +   
                                     ': Insert into TRANSMITLOG3 Failed. (isp_ITF_ntrKIT) ( SQLSvr MESSAGE = ' +   
               ...
               BEGIN  
                  EXEC ispGenTransmitLog2 @c_Tablename, @c_KitKey, '', @c_StorerKey, '' 
                                          , @b_success OUTPUT  
                                          , @n_Err OUTPUT  
                                          , @c_ErrMsg OUTPUT  
                  ...
                  BEGIN  
                     ...
                     SET @n_Err = 68001  
                     SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_Err,0)) +   
                                     ': Insert into TRANSMITLOG2 Failed. (isp_ITF_ntrKIT) ( SQLSvr MESSAGE = ' +   
                                     ISNULL(LTRIM(RTRIM(@c_ErrMsg)),'') + ' ) '  
                     GOTO QUIT  
                  END   
...
QUIT:  
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         END  
      END  
      EXECUTE dbo.nsp_logerror @n_Err, @c_ErrMsg, 'isp_ITF_ntrKIT'  
