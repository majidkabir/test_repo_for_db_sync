...
BEGIN
   ...
   DECLARE @t_PPA_CTN            TABLE 
         ...
         , RefNo                 NVARCHAR(10) NOT NULL   DEFAULT('')   
         )            
   ...
   IF @c_PPA100Export = 'Y'
   BEGIN
      INSERT INTO @t_PPA_CTN ( PickSlipNo, CartonNo )
      SELECT PI.PickSlipNo, PI.CartonNo
      FROM @t_PPA_WIP AS tpw
      JOIN dbo.PackInfo AS PI ON PI.PickSlipNo = tpw.PickSlipNo
   ...
   INSERT INTO @t_PPA_CTN ( PickSlipNo, CartonNo )
   SELECT TOP (@n_NofPPA) 
         PI.PickSlipNo, PI.CartonNo
   FROM @t_PPA_WIP AS tpw
   JOIN dbo.PackInfo AS PI ON PI.PickSlipNo = tpw.PickSlipNo
   WHERE NOT EXISTS (   SELECT 1
                        FROM @t_PPA_CTN AS tpc
                        WHERE tpc.PickSlipNo = PI.PickSlipNo
                        AND tpc.CartonNo = PI.CartonNo
                     )
   ORDER BY NEWID()
   ...
   BEGIN  
      SET @n_continue = 3  
      SET @n_err = 68010    
      SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': UPDATE PACKINFO Table Failed. (ispRLWAV43_PPA)'   
      GOTO QUIT_SP  
   END                                                                                                                
...
QUIT_SP:
   ...
   BEGIN
      ...
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'ispRLWAV43_PPA'
