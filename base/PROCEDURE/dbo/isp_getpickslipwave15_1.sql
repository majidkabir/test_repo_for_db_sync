...
/************************************************************************/    
/* Store Procedure: isp_GetPickSlipWave04_4                             */    
/* Creation Date: 21-MAR-2018                                           */    
/* Copyright: IDS                                                       */    
/* Written by: CSCHONG                                                  */    
/*                                                                      */    
/* Purpose: Pickslip WMS-4178                                           */    
/*                                                                      */    
...
/* 04-OCT-2018     Joseph Yu   1.0   Pickslip WMS-6220					*/
/************************************************************************/    
	 
CREATE PROC [dbo].[isp_GetPickSlipWave15_1] (@c_wavekey NVARCHAR(10))     
 AS    
 BEGIN    
	SET NOCOUNT ON     
	SET ANSI_NULLS OFF
	...
	END
		...
		SELECT ST.Company AS SCompany,
			  ...
			  ,COUNT(DISTINCT OD.Sku) AS SKUCount
			FROM ORDERS (NOLOCK)  
	...
	BEGIN
		...
		STR(CAST(@c_pickheaderkey AS INT) + (SELECT COUNT(DISTINCT OrderKey)
														 FROM #TEMP_PICK15 AS Rank
														 WHERE Rank.OrderKey < #TEMP_PICK15.OrderKey
		...
		WHERE PICKHEADER.WaveKey = #TEMP_PICK15.Wavekey
		AND   PICKHEADER.OrderKey = #TEMP_PICK15.OrderKey
		AND   PICKHEADER.Zone = '8'
		AND   ISNULL(RTRIM(#TEMP_PICK15.Pickheaderkey),'') = '' -- SOS# 283436
	END
