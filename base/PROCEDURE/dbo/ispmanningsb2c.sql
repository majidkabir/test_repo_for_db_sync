...
/************************************************************************/
/* Stored Proc: ispManningsB2C                                          */
/* Creation Date: 2021-03-29                                            */
/* Copyright: LF Logistics                                              */
/* Written by: WLChooi                                                  */
/*                                                                      */
/* Purpose: WMS-16621 - [CN] Mannings_WMS_AllocationStrategy_CR         */
/*        :                                                             */
/* Called By:                                                           */
/*          :                                                           */
/* GitLab Version: 1.1                                                  */
...
/************************************************************************/
CREATE   PROC [dbo].[ispManningsB2C]
...
BEGIN
   ...
   --This Main Pickcode only proceed MANB2C
   IF @c_WaveType NOT IN ('MANB2C')
   BEGIN
      GOTO QUIT_SP
   END
   ...
   BEGIN
      ...
      SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0))  
                    ...
                    + '. Allocation is not allowed (ispManningsB2C)'
      GOTO QUIT_SP
   END
   ...
   BEGIN
      IF @b_debug IN ( 1, 9 )
      BEGIN
         PRINT 'Main PICKCOde: ispManningsB2C'+ CHAR(13) +
               'SKU: ' + @c_SKU + CHAR(13) +
   ...
   SET @c_SQLParms  = N' @c_Facility           NVARCHAR(5)'  
    ...
    EXEC sp_ExecuteSQL  @c_SQL
      ...
      BEGIN
         ...
         BEGIN
            PRINT '-----------------------------------'+ CHAR(13) +
                  'Main PICKCOde: ispManningsB2C'+ CHAR(13) +
                  'SKU: ' + @c_SKU + CHAR(13) +
                  '@n_QtyLeftToFullFill: ' + CAST(@n_QtyLeftToFullFill AS VARCHAR) + CHAR(13) +
                  '@dt_Lottable13: ' + CONVERT(NVARCHAR(25), @dt_Lottable13, 121) + CHAR(13)
         END 

         EXEC ispManningsLoadFP2      --//Pallet @Pallet LOC - Single
           @c_WaveKey          = @c_WaveKey         
         ...
         BEGIN
            ...
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5), @n_Err) + ': Error Executing ispManningsLoadFP2.'
                          + '(ispManningsB2C)' + ' (' + @c_ErrMsg + ')'
            GOTO QUIT_SP
         END
         ...
         EXEC ispManningsLoadFC2      --//CASE @Case & @Pallet Loc, UOM = '2' - Single
           @c_WaveKey          = @c_WaveKey         
         , @c_WaveType         = @c_WaveType  
         , @c_Facility         = @c_Facility         
         , @c_Storerkey        = @c_Storerkey        
         ...
         BEGIN
            ...
            SET @n_Err = 82030
            SET @c_ErrMsg = ISNULL(ERROR_MESSAGE(),'')
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5), @n_Err) + ': Error Executing ispManningsLoadFC2.'
                          + '(ispManningsB2C)' + ' (' + @c_ErrMsg + ')'
            GOTO QUIT_SP
         END
         ...
         EXEC ispManningsLoadFP6      --//Pallet @Pallet LOC - Multi
           @c_WaveKey          = @c_WaveKey         
         , @c_WaveType         = @c_WaveType  
         , @c_Facility         = @c_Facility         
         , @c_Storerkey        = @c_Storerkey        
         ...
         BEGIN
            ...
            SET @n_Err = 82040
            SET @c_ErrMsg = ISNULL(ERROR_MESSAGE(),'')
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5), @n_Err) + ': Error Executing ispManningsLoadFP6.'
                          + '(ispManningsB2C)' + ' (' + @c_ErrMsg + ')'
            GOTO QUIT_SP
         END
         ...
         EXEC ispManningsLoadFC6      --//CASE @Case & @Pallet Loc, UOM = '2' - Multi
           @c_WaveKey          = @c_WaveKey         
         , @c_WaveType         = @c_WaveType  
         , @c_Facility         = @c_Facility         
         , @c_Storerkey        = @c_Storerkey        
         ...
         BEGIN
            ...
            SET @n_Err = 82050
            SET @c_ErrMsg = ISNULL(ERROR_MESSAGE(),'')
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5), @n_Err) + ': Error Executing ispManningsLoadFC6.'
                          + '(ispManningsB2C)' + ' (' + @c_ErrMsg + ')'
            GOTO QUIT_SP
         END
         ...
         EXEC ispManningsConsoFP6   --//Pallet @Pallet LOC - Conso Single & Multi
           @c_WaveKey          = @c_WaveKey         
         , @c_WaveType         = @c_WaveType   
         , @c_Facility         = @c_Facility         
         , @c_Storerkey        = @c_Storerkey        
         ...
         BEGIN
            ...
            SET @n_Err = 82060
            SET @c_ErrMsg = ISNULL(ERROR_MESSAGE(),'')
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5), @n_Err) + ': Error Executing ispManningsConsoFP6.'
                          + '(ispManningsB2C)' + ' (' + @c_ErrMsg + ')'
            GOTO QUIT_SP
         END
         ...
         EXEC ispManningsConsoFC6  --//CASE @Case & @Pallet Loc, UOM = '6' - Conso Single & Multi
           @c_WaveKey          = @c_WaveKey         
         , @c_WaveType         = @c_WaveType   
         , @c_Facility         = @c_Facility         
         , @c_Storerkey        = @c_Storerkey        
         ...
         BEGIN
            ...
            SET @n_Err = 82070
            SET @c_ErrMsg = ISNULL(ERROR_MESSAGE(),'')
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5), @n_Err) + ': Error Executing ispManningsConsoFC6.'
                          + '(ispManningsB2C)' + ' (' + @c_ErrMsg + ')'
            GOTO QUIT_SP
         END
         ...
         EXEC ispManningsConsoFQ7  --//Loose Qty @DPP Loc, UOM = '7' - Conso single + Multi, Get DPP <= @n_QtyLeftToFullfill   --(Wan01)
           @c_WaveKey          = @c_WaveKey 
         , @c_WaveType         = @c_WaveType                      
         , @c_Facility         = @c_Facility         
         , @c_Storerkey        = @c_Storerkey        
         ...
         BEGIN
            ...
            SET @n_Err = 82080
            SET @c_ErrMsg = ISNULL(ERROR_MESSAGE(),'')
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5), @n_Err) + ': Error Executing ispManningsConsoLQ7.'
                          + '(ispManningsB2C)' + ' (' + @c_ErrMsg + ')'
            GOTO QUIT_SP
         END
         ...
         EXEC ispManningsConsoLC7  --//Loose Case Qty @Case & @Pallet Loc, UOM = '7' - Conso single + Multi
           @c_WaveKey          = @c_WaveKey 
         , @c_WaveType         = @c_WaveType                    
         , @c_Facility         = @c_Facility         
         , @c_Storerkey        = @c_Storerkey        
         ...
         BEGIN
            ...
            SET @n_Err = 82090
            SET @c_ErrMsg = ISNULL(ERROR_MESSAGE(),'')
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5), @n_Err) + ': Error Executing ispManningsConsoLC7.'
                          + '(ispManningsB2C)' + ' (' + @c_ErrMsg + ')'
            GOTO QUIT_SP
         END
         ...
         EXEC ispManningsConsoLQ7  --//Loose Qty @DPP Loc, UOM = '7' - Conso single + Multi, Get DPP >= 0   
           @c_WaveKey          = @c_WaveKey 
         , @c_WaveType         = @c_WaveType                      
         , @c_Facility         = @c_Facility         
         , @c_Storerkey        = @c_Storerkey        
...
QUIT_SP:
   ...
   BEGIN
      ...
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'ispManningsB2C'
