...
/************************************************************************/
CREATE PROC  [dbo].[nsp_TransferDetailInterface_CCOPACK]
...
BEGIN
   ...
   IF NOT EXISTS( SELECT 1 FROM TRANSMITLOG (NOLOCK) WHERE TableName = 'CCOPACK' AND KEY1 = @c_transferkey
   ...
   BEGIN
      ...
      BEGIN
         ...
         VALUES (@c_trmlogkey, 'CCOPACK', @c_transferkey , @c_transferlinenumber, '0')
         SELECT @n_err = @@ERROR
         IF @n_err <> 0
         BEGIN
            SELECT @n_continue = 3
   ...
   BEGIN
      ...
      BEGIN
         ...
         END
      END
      execute nsp_logerror @n_err, @c_errmsg, "nsp_TransferDetailInterface_CCOPACK"
