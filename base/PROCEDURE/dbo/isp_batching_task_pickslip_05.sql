...
GO
/************************************************************************/
/* Store Procedure:  isp_batching_task_pickslip_05                      */
/* Creation Date:  22-DEC-2020                                          */
/* Copyright: LF                                                        */
/* Written by:                                                          */
/*                                                                      */
/* Purpose: WMS15855 -PH ECOM02 PICKLIST MODIFICATION CR                */
/*                                                                      */
/* Input Parameters:                                                    */
/*                                                                      */
...
CREATE PROC [dbo].[isp_batching_task_pickslip_05] (
 ...
 BEGIN
    ...
    BEGIN 
       SELECT @n_Continue = 3  
       SELECT @n_Err = 63200  
       SELECT @c_ErrMsg='NSQL'+CONVERT(NVARCHAR(5),@n_Err)+': Invalid Mode. The value must be 1,4,5,9 (isp_batching_task_pickslip_05)' 
       GOTO Quit
    END
    ...
    LEFT JOIN Codelkup CL2 (NOLOCK) ON (PD.Storerkey = CL2.Storerkey AND CL2.Code = 'SHOWSTYLESIZE' 
                                              AND CL2.Listname = 'REPORTCFG' AND CL2.Long = 'r_dw_batching_task_pickslip_05' AND ISNULL(CL2.Short,'') <> 'N') 
    LEFT JOIN Codelkup CL3 (NOLOCK) ON (PD.Storerkey = CL3.Storerkey AND CL3.Code = 'SHOWLOADKEYBARCODE' 
                                              AND CL3.Listname = 'REPORTCFG' AND CL3.Long = 'r_dw_batching_task_pickslip_05' AND ISNULL(CL3.Short,'') <> 'N')  
    LEFT JOIN Codelkup CL4 (NOLOCK) ON (PD.Storerkey = CL4.Storerkey AND CL4.Code = 'ShowCurrentDateTime' 
                                              AND CL4.Listname = 'REPORTCFG' AND CL4.Long = 'r_dw_batching_task_pickslip_05' AND ISNULL(CL4.Short,'') <> 'N')  
    LEFT JOIN Codelkup CL5 (NOLOCK) ON (PD.Storerkey = CL5.Storerkey AND CL5.Code = 'ShowSalesman' 
                                              AND CL5.Listname = 'REPORTCFG' AND CL5.Long = 'r_dw_batching_task_pickslip_05' AND ISNULL(CL5.Short,'') <> 'N')  
    LEFT JOIN Codelkup CL6 (NOLOCK) ON (PD.Storerkey = CL6.Storerkey AND CL6.Code = 'ShowVAS' 
                                              AND CL6.Listname = 'REPORTCFG' AND CL6.Long = 'r_dw_batching_task_pickslip_05' AND ISNULL(CL6.Short,'') <> 'N') 
         ...
         BEGIN
             ...
             BEGIN
                 INSERT INTO #TMP_RESULT (TaskBatchNo, Loc, LogicalLocation, Sku, SkuBarcode, Descr, Orderkey, OrderQty, LogicalName, Qty,altsku,Showloadkeybarcode)
                              VALUES (@c_TaskBatchNo, @c_Loc, @c_LogicalLocation, @c_Sku, @c_SkuBarcode, @c_Descr, @c_Orderkey, @n_OrderQty, @c_LogicalName, 1, @c_altsku, @c_Showloadkeybarcode) 
                 /*
