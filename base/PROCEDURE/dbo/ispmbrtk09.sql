...
/* Written by:                                                          */
/*                                                                      */
/* Purpose: WMS-17721 VN PACSUN - MBOL release Pick to split container  */
/*           orders and populate to MBOL.    (Copy SOS#360341)          */
/*                                                                      */
...
BEGIN
   ...
   DECLARE @c_Storerkey NVARCHAR(15),
           ...
           @n_PackTotalCube FLOAT,
           @n_PackTotalGrossWgt FLOAT,
   ...
   BEGIN
      ...
      SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Move Task has been released for the MBOL. Not allow to release pick again. (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
      GOTO QUIT_SP    	  
   END
   ...
   BEGIN
      ...
      BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': CaseId ''' + RTRIM(@c_CaseId) + ''' of Container '''+ RTRIM(@c_Containerkey) +''' is not found at Packdetail or Pickdetail. (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
         GOTO QUIT_SP 
      END         	            
      ...
      BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Staging lane not yet assigned to the MBOL (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
         GOTO QUIT_SP 
      END
      */         	
      ...
      BEGIN
         SELECT @n_continue = 3  
         SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err = 36020   -- Should Be Set To The SQL Errmessage but I don't know how to do so.  
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Container Detail not found for the MBOL (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
         GOTO QUIT_SP 
      END         	            
   END  
   ...
   BEGIN
      ...
      BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Container Orders Have Been Populated to Other MBOL (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
         GOTO QUIT_SP 
      END
      ...
      BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Container Orders Have Been Populated to MBOL (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
         GOTO QUIT_SP 
      END
   END
   ...
   BEGIN           
      ...
      BEGIN
      	 ...
      	 BEGIN  
 	          ...
 	          BEGIN
	   		       SELECT @n_continue = 3
				       ...
				       SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Split Order Failed. Order is in MBOL. (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
	          END
               	...
               	WHERE Orderkey = @c_orderkey
	   	          ...
	   	          BEGIN
	   		          SELECT @n_continue = 3
				          ...
				          SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Insert Orders Table. (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
			          END
	   	          ...
	   	          BEGIN
	   		          SELECT @n_continue = 3
				          ...
				          SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Insert OrderInfo Table. (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
			          END
			      END            	  	
               ...
               ORDER BY #TMP_ORDERLINE.Orderlinenumber
   	           ...
   	           BEGIN
   		           SELECT @n_continue = 3
			           ...
			           SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Insert Orderdetail Table. (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
		           END		    		    
               ...
               WHERE Orderkey = @c_neworderkey
   	           ...
   	           BEGIN
   		           SELECT @n_continue = 3
			           ...
			           SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Update Orders Table. (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
		           END
		           */		           
            END
               ...
               BEGIN
                  ...
                  AND PICKDETAIL.OrderLineNumber = @c_OrderLineNumber  
   	              ...
   	              BEGIN
   		              SELECT @n_continue = 3
			              SELECT @c_errmsg = CONVERT(CHAR(250),@n_err), @n_err = 36060
			              SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Update Pickdetail Table. (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 			              
		              END		    		    
                  ...
                  AND   ORDERDETAIL.OrderLineNumber = @c_NewOrderLineNumber  
   	              ...
   	              BEGIN
   		              SELECT @n_continue = 3
			              ...
			              SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Update ORDERDETAIL Table. (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 			              
		              END		    		    
               ...
               WHERE #TMP_ORDER.Orderkey = @c_orderkey     
   	           ...
   	           BEGIN
   		           SELECT @n_continue = 3
			           ...
			           SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Delete Preallocatepickdetail Table. (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
		           END		    		    
		           */
                     ...
                     WHERE Loadkey = @c_Loadkey
   	                 ...
   	                 BEGIN
   		                 SELECT @n_continue = 3
			                 ...
			                 SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Insert LoadPlan Table. (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
		                 END   
		              END         	                  
               END
            END
               ...
               BEGIN
                  ...
                  SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert LOADPLANDETAIL Error. (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
                  GOTO QUIT_SP 
               END                               
       	 	  END
                     ...
                     INSERT INTO WAVEDETAIL ( Wavedetailkey, Wavekey, Orderkey )
   	                 ...
   	                 BEGIN
   		                 SELECT @n_continue = 3
			                 ...
			                 SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Insert WaveDetail Table. (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
		                 END   
		              END         	                  
               END       	 	  
            END
            ...
            BEGIN
               ...
               BEGIN  
                  ...
                  BEGIN
                     ...
                     SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Error Getkey(PICKSLIP) (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
                     GOTO QUIT_SP
                  END
                    
                  SELECT @c_Pickslipno = 'P'+@c_Pickslipno      
                             
                  INSERT INTO PICKHEADER (PickHeaderKey, ExternOrderKey, Orderkey, PickType, Zone, TrafficCop, Loadkey)  
                                  VALUES (@c_Pickslipno , '', @c_neworderkey, '0', 3, '', @c_NewLoadkey)              
                  ...
                  BEGIN
                     ...
                     SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Error Insert Pickheader Table (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
                     GOTO QUIT_SP
                  END
               END
               ...
               BEGIN
                  ...
                  SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Error Update Pickdetail Table (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
                  GOTO QUIT_SP
               END                                    
               ...
               BEGIN
                  ...
                  BEGIN  
                     ...
                     SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Error Insert RefkeyLookUp Table (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
                     GOTO QUIT_SP
                  END   
               END
               */
            ...
            BEGIN
               ...
               BEGIN
                  ...
                  BEGIN
                     ...
                     SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Error Insert PickingInfo Table (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
                     GOTO QUIT_SP
                  END
               END            	
            END
            ...
            BEGIN
               IF NOT EXISTS (SELECT 1 FROM PACKHEADER (NOLOCK) WHERE Pickslipno = @c_Pickslipno)
	             BEGIN
                  INSERT INTO PACKHEADER (Route, OrderKey, OrderRefNo, Loadkey, Consigneekey, StorerKey, PickSlipNo, Status, CartonGroup)      
                         SELECT TOP 1 O.Route, O.Orderkey, O.ExternOrderkey, O.LoadKey, O.Consigneekey, O.Storerkey, @c_PickSlipNo, '9', STORER.CartonGroup       
                  ...
                  BEGIN
                     ...
                     SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Error Insert Packheader Table (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
                     GOTO QUIT_SP
                  END
	             END
            END
            
      	 	  --Move packdetail and packinfo to new pickslip(Order)
            IF @n_Continue IN(1,2)
            BEGIN
               ...
               BEGIN               	
               	  ...
               	  BEGIN
                      ...
                      BEGIN
                         ...
                         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Error Update PackInfo Table (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
                      END
               	  END
                  ...
                  BEGIN
                     ...
                     SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Error Update PackDetail Table (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
                  END
            ...
            --Update Carton Count & Type to packheder of old and new
            IF @n_Continue IN(1,2)
            BEGIN
            	 SET @c_MoveFromPickslipno = ''
               SELECT @c_MoveFromPickslipno = Pickslipno
               FROM PACKHEADER(NOLOCK)
               WHERE Orderkey = @c_Orderkey
               
               --Update original packheader
               IF ISNULL(@c_MoveFromPickslipno,'') <> ''
               BEGIN 
                   ...
                   BEGIN
                      ...
                      SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Error Update PackHeader Table (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
                   END
               	   
               	   DECLARE CUR_PACKSUMM CURSOR LOCAL FAST_FORWARD READ_ONLY FOR  
               	      SELECT CartonType, COUNT(DISTINCT CartonNo) AS CartonCnt,
               	             SUM(Weight) AS CartonWeight, SUM(Cube) AS CartonCube
                  ...
                  FETCH NEXT FROM CUR_PACKSUMM INTO @c_CartonType, @n_CartonCnt, @n_CartonWeight, @n_CartonCube
                  ...
                  BEGIN         
                     ...
                     UPDATE PACKHEADER WITH (ROWLOCK)
                     SET CtnTyp1 = CASE WHEN @n_cnt = 1 THEN @c_CartonType ELSE CtnTyp1 END,
                         CtnTyp2 = CASE WHEN @n_cnt = 2 THEN @c_CartonType ELSE CtnTyp2 END,                  	 
                         CtnTyp3 = CASE WHEN @n_cnt = 3 THEN @c_CartonType ELSE CtnTyp3 END,                  	 
                         CtnTyp4 = CASE WHEN @n_cnt = 4 THEN @c_CartonType ELSE CtnTyp4 END,                  	 
                         CtnTyp5 = CASE WHEN @n_cnt = 5 THEN @c_CartonType ELSE CtnTyp5 END,         
                         CtnCnt1 = CASE WHEN @n_cnt = 1 THEN @n_CartonCnt ELSE CtnCnt1 END,
                         CtnCnt2 = CASE WHEN @n_cnt = 2 THEN @n_CartonCnt ELSE CtnCnt2 END,
                         CtnCnt3 = CASE WHEN @n_cnt = 3 THEN @n_CartonCnt ELSE CtnCnt3 END,
                         CtnCnt4 = CASE WHEN @n_cnt = 4 THEN @n_CartonCnt ELSE CtnCnt4 END,
                         CtnCnt5 = CASE WHEN @n_cnt = 5 THEN @n_CartonCnt ELSE CtnCnt5 END,
                         TTLCNTS = TTLCNTS + @n_CartonCnt,
                         TotCtnWeight = TotCtnWeight + @n_CartonWeight,
                         TotCtnCube = TotCtnCube + @n_CartonCube, 
                         ArchiveCop = NULL
                 	   WHERE PickslipNo = @c_MoveFromPickslipno
               	  ...
               	  END 
               	  CLOSE CUR_PACKSUMM
               	  DEALLOCATE CUR_PACKSUMM                  	                  	                     	     
               END

               --Update new packheader
               IF ISNULL(@c_Pickslipno,'') <> ''
               BEGIN 
               	  UPDATE PACKHEADER WITH (ROWLOCK)
               	  SET TTLCNTS = 0,
                   ...
                   BEGIN
                      SELECT @n_continue = 3  
                      SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err = 36210   -- Should Be Set To The SQL Errmessage but I don't know how to do so.  
                      SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Error Update PackHeader Table (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
                   END
               	   
               	   DECLARE CUR_PACKSUMM2 CURSOR LOCAL FAST_FORWARD READ_ONLY FOR  
               	      SELECT CartonType, COUNT(DISTINCT CartonNo) AS CartonCnt,               	      
               	             SUM(Weight) AS CartonWeight, SUM(Cube) AS CartonCube
               	      FROM PACKINFO (NOLOCK)
               	      WHERE Pickslipno = @c_Pickslipno
                  ...
                  FETCH NEXT FROM CUR_PACKSUMM2 INTO @c_CartonType, @n_CartonCnt, @n_CartonWeight, @n_CartonCube
                  ...
                  BEGIN         
                     ...
                     BEGIN
                        SELECT @n_continue = 3  
                        SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err = 36220   -- Should Be Set To The SQL Errmessage but I don't know how to do so.  
                        SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Error Update PackHeader Table (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
                     END
      	 ...
      	 BEGIN
      	 	  /*
      	 	  SET @n_PackTotalGrossWgt = 0
      	 	  SET @n_PackTotalCube = 0
      	 	  SELECT @n_PackTotalGrossWgt = TotCtnWeight,
      	 	         @n_PackTotalCube = TotCtnCube      	 	          
      	 	  FROM PACKHEADER (NOLOCK)
            ...
            FROM (SELECT PACKDETAIL.CartonNo,
                   ...
                   JOIN PACKINFO (NOLOCK) ON (PACKHEADER.Pickslipno = PACKINFO.Pickslipno AND PACKDETAIL.CartonNo = PACKINFO.CartonNo)  
                   LEFT JOIN CARTONIZATION CZ (NOLOCK) ON (STORER.CartonGroup = CZ.CartonizationGroup AND CZ.CartonType = PACKINFO.CartonType)  
                   WHERE PACKHEADER.Orderkey = @c_MBOLOrderkey  
                   GROUP BY PACKDETAIL.CartonNo, ISNULL(CZ.[Cube],0), ISNULL(CZ.CartonWeight,0)) AS PKSUM
            ...
            BEGIN
               SELECT @n_continue = 3  
               SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err = 36190   -- Should Be Set To The SQL Errmessage but I don't know how to do so.  
               SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert MBOLDETAL Error. (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
               GOTO QUIT_SP 
            END
         END
         ...
         BEGIN      
           INSERT TASKDETAIL  
            ...
            BEGIN
                SELECT @n_continue = 3  
                SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err = 36200   -- Should Be Set To The SQL Errmessage but I don't know how to do so.  
                SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert Taskdetail Failed. (ispMBRTK09)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
            END            
         END
