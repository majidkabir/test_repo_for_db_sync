...
/***************************************************************************/      
/* Stored Proc : isp_GetPickSlipWave_25_c                                  */      
...
/* Local Variables:                                                        */      
/*                                                                         */      
/* Called By: r_dw_print_wave_pickslip_25_c                                */
/*            copy from r_dw_print_wave_pickslip_25                        */      
/*                                                                         */      
/* PVCS Version: 1.1                                                       */      
/*                                                                         */      
/* Version: 5.4                                                            */      
...
CREATE PROC [dbo].[isp_GetPickSlipWave_25_c] (@c_wavekey NVARCHAR(10), 
...
CREATE INDEX IDX_WAVEPICK25c ON #TEMP_WAVEPICK25c(caseid)  
  
     CREATE TABLE #TEMP_WAVEPICK25c_LOC(  
...
CREATE INDEX IDX_WAVEPICK25c_LOC ON #TEMP_WAVEPICK25c_LOC(caseid,Loc)  
  
  CREATE TABLE #TEMP_CWAVETTLORD     
    (  WaveKey       NVARCHAR(20) NULL,    
       caseid        NVARCHAR(40) NULL,      
       StorerKey     NVARCHAR(20),   
...
CREATE INDEX IDX_CWAVETTLPICKFACE ON #TEMP_CWAVETTLPICKFACE(Pickdetailkey)  
   ...
   FETCH NEXT FROM CUR_LOAD INTO @c_Gloadkey,@c_orderkey     
                              ,  @c_PZone  
                                ,@c_GetPickDetailKey  
                                ,@c_GetStorerkey  
   ...
   BEGIN      
     ...
     -- BEGIN   
      ...
      IF NOT EXISTS (SELECT 1 FROM  PICKHEADER (NOLOCK) WHERE wavekey = @c_Wavekey  AND Orderkey = @c_orderkey AND loadkey = @c_Gloadkey)  
      BEGIN  
         EXECUTE nspg_GetKey         
                  'PICKSLIP'      
               ,  9      
     ...
     END   
         ...
         BEGIN          
            SET @c_ExecStatement = N'DECLARE C_PickDetailKey CURSOR FAST_FORWARD READ_ONLY FOR ' +  
                                    ...
                                    ' AND    OrderDetail.LoadKey  =  @c_Gloadkey ' +  
            ...
            BEGIN    
               ...
               BEGIN     
                  ...
                  VALUES (@c_GetPickDetailKey, @c_RPickSlipNo, @c_OrderKey, @c_OrdLineNo, @c_Gloadkey)  
   ...
   INSERT INTO #TEMP_CWAVETTLORD(WaveKey,caseid,StorerKey,TTLORD,SUSR5)   
   ...
   INSERT INTO #TEMP_CWAVETTLPICKFACE  (Pickdetailkey, PICKZONE, LOGICALLOCATION,LOC)  
  ...
  LOC =   
   ...
   AND PID.CaseID = @c_caseid
   AND PID.Loc = L.Loc  
   AND PID.OrderKey IN (SELECT orderkey FROM ORDERS OH WITH (NOLOCK) WHERE OH.LoadKey=@c_loadkey)
  
  INSERT INTO #TEMP_WAVEPICK25c_LOC (WaveKey,caseid, Storerkey,LocPickZone,Logicallocation,B_Fax2,Qty,sku,Altsku,--LineNum,RecGrp,PageLine,  
   ...
   JOIN PICKDETAIL PID WITH (NOLOCK) ON PID.Orderkey = OH.Orderkey  
   JOIN SKU S WITH (NOLOCK) ON S.storerkey = PID.Storerkey AND S.sku = PID.sku  
   JOIN #TEMP_CWAVETTLPICKFACE TMP ON PID.PickDetailKey = TMP.Pickdetailkey  
   WHERE wd.wavekey = @c_wavekey  
   AND OH.LoadKey = @c_loadkey
   AND PID.CaseID = @c_caseid
  ...
  SELECT @n_Tot_Cnt = COUNT(1)  
   ...
   BEGIN   
      ...
      IF @c_Insert = 'Y'  
      BEGIN  
         INSERT INTO #TEMP_WAVEPICK25c (WaveKey,caseid, Storerkey,LogicalLoc ,LOC,SKU,    
         ...
         BEGIN  
            IF @n_PKPage > 1  
            BEGIN  
               UPDATE #TEMP_WAVEPICK25c SET TTLPage = @n_PKPage  
...
--   FETCH NEXT FROM CUR_LineLoop INTO @n_DETLineID,@c_getwavekey,@c_getcaseid, @c_getlocpickzone,@c_storerkey,@c_sku,@c_altsku,@n_lineNum,@n_RecGrp,@n_pageline  
   ...
   DEALLOCATE CUR_LineLoop       
      ...
      FROM #TEMP_WAVEPICK25c TWP25  
      JOIN #TEMP_CWAVETTLORD TORD ON TORD.WaveKey = TWP25.WaveKey AND TORD.caseid=TWP25.caseid   
      AND TORD.StorerKey= TWP25.StorerKey    
      ORDER BY Wavekey,Caseid,LocPickZone,TWP25.logicalloc,loc,sku,altsku,recrowno,pageno   
         
   DROP Table #TEMP_WAVEPICK25c    
   DROP Table #TEMP_WAVEPICK25c_LOC  
   DROP Table #TEMP_CWAVETTLORD   
   DROP Table #TEMP_CWAVETTLPICKFACE  
QUIT_SP:    
END    
