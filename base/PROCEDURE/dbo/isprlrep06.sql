...
/***************************************************************************/      
/* Stored Procedure: ispRLREP06                                            */      
/* Creation Date: 06-Sep-2022                                              */
/* Copyright: LFL                                                          */
/* Written by: WLChooi                                                     */      
/*                                                                         */      
/* Purpose: WMS-20668 - AdidasVN Release replenishment task(RPF) from      */
   ...
   BEGIN
      ...
      DECLARE CUR_REPLENISH CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
         SELECT R.StorerKey, R.Sku, R.FromLoc, R.Id, R.ToLoc, R.ToID, R.UOM, R.Qty, R.Lot,
                R.ReplenishmentKey, ISNULL(R.ReplenishmentGroup,''), R.Priority, R.DropID
         FROM  REPLENISHMENT R (NOLOCK)   
         JOIN  LOC (NOLOCK) ON (LOC.Loc = R.ToLoc)  
         WHERE (LOC.putawayZone IN (@c_Zone02, @c_Zone03, @c_Zone04, @c_Zone05, @c_Zone06, @c_Zone07, @c_Zone08, @c_Zone09, @c_Zone10, @c_Zone11, @c_Zone12)
                OR @c_Zone02 = 'ALL'
         ...
         AND (LOC.Facility = @c_Facility OR ISNULL(@c_Facility,'') = '') 
         AND R.Confirmed = 'N'  
         AND R.StorerKey = CASE WHEN @c_StorerKey = 'ALL' OR @c_StorerKey = '' 
                                THEN R.StorerKey ELSE @c_StorerKey END              
         ORDER BY R.ReplenishmentGroup, R.Replenishmentkey

      OPEN CUR_REPLENISH
       
      FETCH FROM CUR_REPLENISH INTO @c_Storerkey, @c_Sku, @c_FromLoc, @c_ID, @c_ToLoc, @c_ToID, @c_UOM
                                  , @n_Qty, @c_Lot, @c_Replenishmentkey, @c_ReplenishmentGroup, @c_Priority, @c_DropID
...
RETURN_SP:
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         END  
      END  
      execute nsp_logerror @n_err, @c_errmsg, "ispRLREP06"  
