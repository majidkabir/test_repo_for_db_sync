...
GO
/*************************************************************************/
/* Stored Procedure: isp_r_hk_picking_control_list_08b                   */
...
/*                                                                       */
/* Called By: Datawidnow r_hk_picking_control_list_08b                   */
/*                                                                       */
/* PVCS Version: 1.0                                                     */
/*                                                                       */
/* Version: 7.0                                                          */
...
*/
   ...
   SELECT PickslipNo        = X.PickslipNo
        ...
        , Courier           = MAX( X.Courier )
        , AddDate           = MAX( X.AddDate )
        , Replen            = MAX( X.Replen )
        , MultiPAZone       = MAX( X.MultiPAZone )
        , PutawayZone       = RTRIM( LOC.PutawayZone )
        , LogicalLocation   = RTRIM( ISNULL( LOC.LogicalLocation, '') )
        , Loc               = RTRIM( PD.Loc )
        , LocDescr          = RTRIM( MAX( IIF(ISNULL(LOC.Descr,'')='', LOC.Loc, LOC.Descr) ) )
        , ID                = RTRIM( PD.ID )
        , Sku               = RTRIM( PD.Sku )
        , Qty               = SUM( PD.Qty )
   ...
   ORDER BY Replen, MultiPAZone, FirstPAZone, Orderkey, PutawayZone, LogicalLocation, Loc, ID, Sku
END
