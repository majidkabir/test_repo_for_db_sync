...
/*************************************************************************/
/* Store procedure: isp_WSITF_GeekPlusRobot_Generic_RELContainer_Outbound*/
...
/*************************************************************************/
CREATE PROC [dbo].[isp_WSITF_GeekPlusRobot_Generic_RELContainer_Outbound](         
...
BEGIN
   ...
   SET @c_Application                     = 'GEEK+_RELCONTAINER_OUT'
   ...
   IF @b_Debug = 1
   BEGIN
      PRINT '>>>>>>>> isp_WSITF_GeekPlusRobot_Generic_RELContainer_Outbound - INITAL BEGIN'
      PRINT '@c_TransmitlogKey: ' + @c_TransmitlogKey
      PRINT '>>>>>>>> isp_WSITF_GeekPlusRobot_Generic_RELContainer_Outbound - INITAL END'
   END 
   ...
   BEGIN
      IF @b_Debug = 1
      BEGIN
         PRINT '>>>>>>>> isp_WSITF_GeekPlusRobot_Generic_RELContainer_Outbound - BEGIN'
      ...
      WHERE OrderKey = @c_TLKey1

      IF @n_IsExists = 0
      BEGIN
         SET @c_ErrMsg = 'ORDERS Record Not found! OrderKey: ' + @c_TLKey1
      ...
      WHERE OrderKey = @c_TLKey1
      AND ISNULL(RTRIM(DropID), '') <> ''

      IF @n_IsExists = 0
      BEGIN
         SET @c_ErrMsg = 'PICKDETAIL Record Not found! OrderKey: ' + @c_TLKey1
      ...
      WHERE InterfaceName = 'RELCNTNR_OUTBOUND' AND StorerKey = @c_StorerKey
      
      IF @n_IsExists = 0 OR @c_ReservedSQLQuery2 =''
      BEGIN
         SET @c_ErrMsg = '[dbo].[GEEKPBOT_INTEG_CONFIG](InterfaceName=RELCNTNR_OUTBOUND, StorerKey='
                       + @c_StorerKey + ') is not setup.(isp_WSITF_GeekPlusRobot_Generic_RELContainer_Outbound)'
      ...
      --Validation for ReservedSQLQuery1 (BEGIN)
      IF @c_ReservedSQLQuery1 <> ''
      BEGIN
         BEGIN TRY
            ...
            IF @n_Exists = 0
            BEGIN
               --SET @c_ErrMsg = 'Failed Validation in ReservedSQLQuery1'
         ...
         END CATCH
      END 
      --Validation for ReservedSQLQuery1 (END)

      --Execute Dynamic ReservedSQLQuery2 (BEGIN)
      BEGIN TRY
      ...
      END TRY
      BEGIN CATCH
         SET @c_ErrMsg = 'Execute ReservedSQLQuery2 Error - ' + ERROR_MESSAGE()
         ...
         GOTO QUIT
      END CATCH
      --Execute Dynamic ReservedSQLQuery2 (END)
...
QUIT:
   ...
   BEGIN
      ...
      PRINT '>>>>>>>> isp_WSITF_GeekPlusRobot_Generic_RELContainer_Outbound - END'
   END 
