...
/************************************************************************/  
/* Store Procedure: isp_TPP_TPPRN_02                                    */  
/* Creation Date: 20-Aug-2021                                           */  
/* Copyright: LF                                                        */  
/* Written by:CSCHONG                                                   */  
/*                                                                      */  
/* Purpose:WMS-17761 CN - JD Platform Cloud Print Cmd SP                */  
/*                                                                      */  
/*                                                                      */  
/* Input Parameters:                                                    */  
/*                                                                      */  
...
CREATE PROC [dbo].[isp_TPP_TPPRN_02] (  
   @n_JobNo             BIGINT  
...
BEGIN  
   ...
   BEGIN  
       ...
       --CS01 START  
        ...
        BEGIN  
         ...
         SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_err,0))     
                        + ': TPP Printer Group not setup for printeris : ' + @c_printerid + ' with platform' + @c_Platform + ' (isp_TPP_TPPRN_02)'  
         SET @c_Status = '5'  
     ...
     WHERE RP.Printerid= @c_printerid  
        ...
        BEGIN  
         ...
         SET @n_err = 68010       
         SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_err,0))     
                        + ': TPP Printer Group not setup for printer : ' + @c_printerid + ' with platform' + @c_Platform + ' (isp_TPP_TPPRN_02)'  
         SET @c_Status = '5'  
         SET @c_Message = RTRIM(ISNULL(@c_Message,'')) + @c_ErrMsg + ' '                                  
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         SET @n_err = 68010       
         SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_err,0))     
                        + ': Unable to find print record for Job No: ' +RTRIM(CAST(@n_JobNo AS NVARCHAR)) + ' (isp_TPP_TPPRN_02)'  
         SET @c_Status = '5'  
         SET @c_Message = RTRIM(ISNULL(@c_Message,'')) + @c_ErrMsg + ' '                                  
      END  
      ...
      BEGIN         
          ...
          BEGIN  
            ...
            SET @n_err = 68010       
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_err,0))     
                           + ': Get key value fail for Order# ' + RTRIM(@c_CurrOrderkey) + ' Carton# ' + RTRIM(CAST(@n_CartonNo AS NVARCHAR)) + ' (isp_TPP_TPPRN_02)'  
            SET @c_Status = '5'  
            SET @c_Message = RTRIM(ISNULL(@c_Message,'')) + @c_ErrMsg + ' '                                  
          END  
          ELSE  
          ...
          BEGIN  
            ...
            SET @n_err = 68010       
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_err,0))     
                           + ': Unable to find tracking# for Order# ' + RTRIM(@c_CurrOrderkey) + ' Carton# ' + RTRIM(CAST(@n_CartonNo AS NVARCHAR)) + ' (isp_TPP_TPPRN_02)'  
            SET @c_Status = '5'  
            SET @c_Message = RTRIM(ISNULL(@c_Message,'')) + @c_ErrMsg + ' '                                  
          END  
    ...
    SET @c_CustomData = @c_SqlOutput  
         ...
         BEGIN  
                ...
                SET @c_RequestString = N'{"orderType": ' + '"' + @c_cmdstart + '"' + ', "key":' + '"' + @c_RequestID + '"' + ', "parameters": {"printName":'  + '"' + @c_printer + '"' +   
                                      ',"tempUrl": '+ '"' + @c_contentdata + '"' + ',"customTempUrl": '+ '"' + @c_TemplathURL + '"' + ', "customData": [' + @c_CustomData + ']' +  
                                      ',"printData":["' + @c_PrintData +'"]} }'  
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         END            
      END  
      EXECUTE nsp_logerror @n_err, @c_errmsg, "isp_TPP_TPPRN_02"  
