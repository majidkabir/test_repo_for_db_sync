...
IF @b_debug = 1  
BEGIN  
    SELECT "nspPRFIFO1 : Before Lot Lookup ....."  
...
IF @b_debug = 1  
BEGIN  
    SELECT "nspPRFIFO1 : After Lot Lookup ....."  
    SELECT '@c_lot'=@c_lot,'@c_lottable01'=@c_lottable01, '@c_lottable02'=@c_lottable02, '@c_lottable03'=@c_lottable03
...
BEGIN            
   ...
   BEGIN  
      ...
      SELECT @c_SortOrder = " ORDER BY LOTATTRIBUTE.Lottable05, LOT.Lot"      
      ...
      SELECT @c_Sku = dbo.fnc_RTrim(@c_SKU)  		
		...
		SELECT @c_sql = " DECLARE PREALLOCATE_CURSOR_CANDIDATES CURSOR FAST_FORWARD READ_ONLY FOR " +  
			...
			" QTYAVAILABLE = CASE WHEN ( SUM(LOTXLOCXID.QTY) - SUM(LOTXLOCXID.QTYALLOCATED) - " +
			               " SUM(LOTXLOCXID.QTYPICKED) - MIN(ISNULL(P.QtyPreallocated, 0))) < " + dbo.fnc_RTrim(@c_UOMBase) +
			         ...
			         " WHEN ( SUM(LOTXLOCXID.QTY) - SUM(LOTXLOCXID.QTYALLOCATED) - " +
			               " SUM(LOTXLOCXID.QTYPICKED) - MIN(ISNULL(P.QtyPreallocated, 0))) % " + dbo.fnc_RTrim(@c_UOMBase) + " = 0 " +
			         " THEN ( SUM(LOTXLOCXID.QTY) - SUM(LOTXLOCXID.QTYALLOCATED) " +
			               " - SUM(LOTXLOCXID.QTYPICKED) - MIN(ISNULL(P.QtyPreallocated, 0))) " +
			         " ELSE " +
			         " ( SUM(LOTXLOCXID.QTY) - SUM(LOTXLOCXID.QTYALLOCATED) - SUM(LOTXLOCXID.QTYPICKED) - MIN(ISNULL(P.QtyPreallocated, 0))) " +
			         " -  ( SUM(LOTXLOCXID.QTY) - SUM(LOTXLOCXID.QTYALLOCATED) - SUM(LOTXLOCXID.QTYPICKED) - MIN(ISNULL(P.QtyPreallocated,0))) % " + dbo.fnc_RTrim(@c_UOMBase) + " " + 
			         " END " +
			" FROM LOT (NOLOCK) " +  
			" INNER JOIN LOTXLOCXID (NOLOCK) ON LOT.LOT = LOTXLOCXID.LOT " + 
			" INNER JOIN LOC (NOLOCK) ON LOTXLOCXID.LOC = LOC.LOC " + 
         ...
         dbo.fnc_RTrim(@c_LimitString) + " " +   
			...
			" HAVING (SUM(LOTXLOCXID.QTY) - SUM(LOTXLOCXID.QtyAllocated) - SUM(LOTXLOCXID.QTYPicked)- MIN(ISNULL(P.QtyPreAllocated, 0))) >= " + dbo.fnc_RTrim(@c_UOMBase) + " " +
			dbo.fnc_RTrim(@c_SortOrder) 
