...
BEGIN  
   ...
   BEGIN
      ...
      SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Found invalid ordergroup. (ispWVM01)' 
      GOTO QUIT_SP
   END
   ...
   BEGIN
      SELECT @n_continue = 3  
      SELECT @n_err = 36100   -- Should Be Set To The SQL Errmessage but I don't know how to do so.  
      SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Found more than 1 ordergroup in a wave is not allowed. (ispWVM01)' 
      GOTO QUIT_SP
   END
   ...
   IF @c_OrdType = '2'  --Retail new launch
   ...
   QUIT_SP:
	 ...
	 BEGIN
	    ...
	    BEGIN
	    	...
	    	END
	    END
	    EXECUTE dbo.nsp_LogError @n_Err, @c_Errmsg, 'ispWVM01'		
