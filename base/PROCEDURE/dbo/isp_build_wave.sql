...
GO
/************************************************************************/
/* Store Procedure: isp_Build_Wave                                      */
...
/* 28-Feb-2019  WLCHOOI   1.4   WMS-8053 LEFT JOIN Orderinfo (WL01)     */ 
/* 01-Jun-2019  NJOW03    1.5   Fix 618. Support insert wavedetail from */
/*                              backend                                 */ 
...
CREATE PROC [dbo].[isp_Build_Wave]
   ...
   ,@c_Option               NVARCHAR(10) = '' --Country get from client option (user.ini)
   ,@nSuccess               INT = 0 OUTPUT
   ,@cErrorMsg              NVARCHAR(255) = '' OUTPUT
   ,@cSQLPreview            NVARCHAR(4000) = '' OUTPUT
   ,@bDebug                 INT = 0
   ,@c_Wavekey              NVARCHAR(10) = '' --NJOW03   
AS
   SET NOCOUNT ON
   SET ANSI_NULLS OFF
   SET QUOTED_IDENTIFIER OFF
...
DECLARE @bInValid          BIT,
        ...
        @cSQL              NVARCHAR(MAX),
        @cSQL2             NVARCHAR(MAX),
        @cSQL3             NVARCHAR(MAX), --NJOW04
        @cSQL4             NVARCHAR(MAX), --NJOW04
...
BEGIN
   ...
   PRINT 'SP-isp_Build_Wave DEBUG-START...'
...
DECLARE CUR_BUILD_WAVE_SORT CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
   ...
   AND    [Type] = 'SORT'
   AND    FieldName <> 'Sku_Total_OpenQty'  --NJOW04
...
--NJOW04   	
SELECT @cColumnName = '', @cOperator = ''

SELECT TOP 1 @cColumnName = FieldName,
             @cOperator = Operator
FROM   OrderSelectionCondition WITH (NOLOCK)
WHERE  OrderSelectionKey = @c_OrderSelectionKey
AND FieldName = 'Sku_Total_OpenQty'

IF @cColumnName = 'Sku_Total_OpenQty'
BEGIN   
   IF ISNULL(@cSortBy,'') = ''
      IF @cOperator = 'DESC'
         SET @cSortBy ='MAX(ISNULL(SKUEXT.OpenQty,0)) DESC, MAX(ISNULL(SKUEXT.Seq,0)) DESC, ORDERS.Orderkey'   --Sort the order with high total order qty. second sorting try put the same sku into a wave if consist of more than one sku with same order qty
      ELSE
         SET @cSortBy ='MIN(ISNULL(SKUEXT.OpenQty,0)), MIN(ISNULL(SKUEXT.Seq,0)), ORDERS.Orderkey'  --Sort the order with low total order qty
   ELSE
      IF @cOperator = 'DESC'
         SET @cSortBy ='MAX(ISNULL(SKUEXT.OpenQty,0)) DESC, MAX(ISNULL(SKUEXT.Seq,0)) DESC' + ', ' + @cSortBy 
      ELSE
         SET @cSortBy ='MIN(ISNULL(SKUEXT.OpenQty,0)), MIN(ISNULL(SKUEXT.Seq,0))' + ', ' + @cSortBy          
   
   SET @cSQL3 = N'
   INSERT INTO #SKUEXT (Storerkey, Sku, OpenQty)
...
SET @cSQL = CASE WHEN @bDebug  = 1 THEN N'' ELSE
   ...
   N'SELECT ' + @c_TopCond + ' ROW_NUMBER() OVER (ORDER BY ' + RTRIM(@cSortBy) + ') AS Number, ORDERS.OrderKey, ORDERS.ExternOrderkey, ORDERS.OpenQty
     ...
     JOIN OrderSelection WITH (NOLOCK) ON  (ORDERS.Storerkey >= OrderSelection.storerkeystart)
         ...
         AND (ORDERS.OrderDate <= OrderSelection.orderdateend)
     LEFT JOIN V_StorerConfig2 SC WITH (NOLOCK) ON ORDERS.Storerkey = SC.Storerkey AND SC.Configkey = ''WaveSkipUserdefine08Chk''  
     LEFT JOIN ORDERINFO WITH (NOLOCK) ON (ORDERS.Orderkey = ORDERINFO.Orderkey)  --WL01                                   
     LEFT JOIN #SKUEXT SKUEXT ON (ORDERDETAIL.Storerkey = SKUEXT.Storerkey AND ORDERDETAIL.Sku = SKUEXT.Sku) ' +  --NJOW04
    'WHERE (OrderSelection.OrderSelectionKey = N''' + @c_OrderSelectionKey + '''' + ') ' + ' 
     AND (ORDERS.UserDefine08 = ''Y'' OR ISNULL(SC.Svalue,'''')=''1'')     
     AND (NOT ORDERS.Status IN (''8'', ''9'') )
     AND (ORDERS.ConsigneeKey >= OrderSelection.consigneekeystart)
...
SET @cSQL4 = RTRIM(@cSQL2) + CHAR(13) + ' GROUP BY ORDERDETAIL.Storerkey, ORDERDETAIL.Sku ' + CHAR(13) +  
                                        ' ORDER BY SUM(ORDERDETAIL.OpenQty), ORDERDETAIL.Storerkey, ORDERDETAIL.Sku ' --NJOW04
...
BEGIN
   SET @bInValid = 1
   SET @cErrorMsg = 'No Orders Found(isp_Build_Wave)'
   GOTO QUIT
END
...
BEGIN
   ...
   BEGIN       	      
      ...
      BEGIN         
         ...
         BEGIN
           SET @bInValid = 1
           SET @cErrorMsg = 'Insert Wavedetail Failed(isp_Build_Wave)'
           CLOSE cur_WaveOrd
...
ELSE
BEGIN
   SELECT T.Orderkey, T.ExternOrderkey
   FROM #tOrderData T
   WHERE T.RNUM <= @n_sNum
   ORDER BY T.RNUM
END       
...
BEGIN
   SET @bInValid = 1
   SET @cErrorMsg = 'Insert Into TranInfo Failed. (isp_Build_Wave)'
   GOTO QUIT
END
...
BEGIN
   ...
   BEGIN
       ...
       END
   END
   execute nsp_logerror 36000, @cErrorMsg, 'isp_Build_Wave'
...
IF @bDebug = 2
BEGIN
   PRINT 'SP-isp_Build_Wave DEBUG-STOP...'
