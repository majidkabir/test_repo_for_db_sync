...
/************************************************************************/
/* Stored Procedure: isp_SplitWaveNotFullAllocOrder                     */
/* Creation Date: 13-Jan-2016                                           */
/* Copyright: LF                                                        */
/* Written by:                                                          */
/*                                                                      */
/* Purpose: SOS#358719 - Splitting Partial and UnAllocated Order        */
/*                       in a Wave to New Order                         */
/*                                                                      */
/* Called By:                                                           */ 
/*                                                                      */
/* Parameters:                                                          */
...
CREATE  PROCEDURE [dbo].[isp_SplitWaveNotFullAllocOrder]
         	...
         	WHERE Orderkey = @c_orderkey
	   	    ...
	   	    BEGIN
	   		    SELECT @n_continue = 3
				    ...
				    SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Insert Orders Table. (isp_SplitWaveNotFullAllocOrder)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
				    BREAK
			    END
   ...
   BEGIN
  	  ...
  	  BEGIN
  	    SELECT @n_continue = 3
			  ...
			  SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Insert #TMP_NEWORDERS Table. (isp_SplitWaveNotFullAllocOrder)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
			  GOTO ERR
		  END
      */
   ...
   BEGIN
  	  ...
  	  BEGIN
  	    SELECT @n_continue = 3
			  ...
			  SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Update Loadplan Table. (isp_SplitWaveNotFullAllocOrder)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
		  END
   ...
   BEGIN        
      ...
      WHERE LOADPLANDETAIL.Loadkey = @c_loadkey
  	  ...
  	  BEGIN
  	    SELECT @n_continue = 3
			  ...
			  SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Delete Loadplandetail Table. (isp_SplitWaveNotFullAllocOrder)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
			  GOTO ERR
		  END
      ...
      INSERT INTO LoadPlan
       ...
       WHERE Loadkey = @c_loadkey
  	   ...
  	   BEGIN
  	     SELECT @n_continue = 3
			   SELECT @c_errmsg = CONVERT(CHAR(250),@n_err), @n_err = 30108
			   SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Insert Loadplan Table. (isp_SplitWaveNotFullAllocOrder)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
 			   GOTO ERR
		   END
      ...
      ORDER BY #TMP_NEWORDERS.Orderkey
  	  ...
  	  BEGIN
  	    SELECT @n_continue = 3
			  SELECT @c_errmsg = CONVERT(CHAR(250),@n_err), @n_err = 30109
			  SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Insert Loadplandetail Table. (isp_SplitWaveNotFullAllocOrder)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
			  GOTO ERR
		  END
   ...
   IF @n_continue=3  -- Error Occured - Process And Return
	 BEGIN
	    ...
	    BEGIN
	       ...
	       END
	    END
  	  execute nsp_logerror @n_err, @c_errmsg, 'isp_SplitWaveNotFullAllocOrder'
