...
/************************************************************************/  
/* Stored Proc: isp_GetPickSlipOrders85_1                               */  
...
/*        :                                                             */  
/* Called By:r_dw_print_pickorder85_1    (ECOM)                         */  
/*          :                                                           */  
/* PVCS Version: 1.0                                                    */  
/*                                                                      */  
/* Version: 7.0                                                         */  
...
QUIT_SP:  
  ...
  BEGIN
      ...
      SELECT PickSlipNo, Contact1, ODUDF03, Loadkey, Orderkey, DelDate, SHIPPERKEY, ExternOrderkey, Notes, Loc  
       ...
       ,'Y'
      FROM #TMP_PCK_185 WHERE PickSlipNo = @c_Pickslipno AND ORDERKEY =  @c_Orderkey

  select @n_MaxRec = COUNT(rowid) from #TMP_PCK_185 WHERE PickSlipNo = @c_Pickslipno AND ORDERKEY =  @c_Orderkey
  ...
  BEGIN
      ...
      SELECT TOP 1 PickSlipNo, Contact1, Loadkey, Orderkey, DelDate, SHIPPERKEY, ExternOrderkey 
       ...
       FROM #TMP_PCK_185_1 WHERE PickSlipNo = @c_Pickslipno AND ORDERKEY =  @c_Orderkey
       ORDER BY ROWID DESC
  ...
  select PickSlipNo, Contact1, ODUDF03, Loadkey, Orderkey, DelDate, SHIPPERKEY, ExternOrderkey, Notes, Loc  
       , Storerkey, SKU, CUDF01, CUDF02, Qty, ODNotes2, Pageno, OIPlatform, CarrierCharges,ShowNo from #TMP_PCK_185_1
