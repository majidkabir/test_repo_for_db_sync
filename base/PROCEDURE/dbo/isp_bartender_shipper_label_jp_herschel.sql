...
/* Purpose: isp_Bartender_Shipper_Label_JP_Herschel                           */   
/*          Yamato Courier label CR for Herschel                              */
/*                                                                            */                 
/* Modifications log:                                                         */                 
/*                                                                            */                 
/* Date       Rev  Author     Purposes                                        */                 
/* 2018-08-09 1.2  CSCHONG    WMS-6280                                        */    
/******************************************************************************/                
                  
CREATE PROC [dbo].[isp_Bartender_Shipper_Label_JP_Herschel]                      
...
BEGIN                      
  ...
  SET @c_SQLJOIN = +N' SELECT DISTINCT (N''Â¿Ã´'' + Substring( Replace(ORD.C_Zip,''-'',''''),1,3) + ''-'' + Substring( Replace(ORD.C_Zip,''-'',''''),4,4)), ' --1
                   + 'ORD.c_phone1,' -- Col2  
                   + '( ORD.C_State + '' '' + ORD.C_City + '' '' + ORD.C_Address1  + ORD.C_Address2 + ORD.C_Address3 + ORD.C_Address4 ) AS Add1,'+   --3     
                   + ' ORD.C_company,' -- 4
                   + ' ISNULL(C3.Description,''''),  ' --5
                   + ' '''', ' -- 6
                   ...
                   + ' PICKDET.CaseId, ' -- 9 
                   + ' (Substring(PICKDET.CaseID,1,4) + ''-'' + Substring(PICKDET.CaseID,5,4)  +''-'' +  Substring(PICKDET.CaseID,9,4)),'            --10
                   + ' STO.company, ' --11         
                   + ' STO.Phone1,'   --12         
                   + ' (STO.Address1 + STO.Address2),' --13
                   + ' STO.Zip,' --14         
                   + ' STO.B_Company, ' --15
                   + ' STO.B_Phone1,'   --16         
                   + ' STO.Susr2,'      --17         
                   ...
                   + ' '''','  --23
                   + ' C2.long,'  --24 
                   + ' CONVERT(VARCHAR(10), getdate(),112),' --25
                   + N' Substring(CONVERT(VARCHAR(10), dateadd(day,30,getdate()),112),3,2)+ N''â”€Î©'' + Substring(CONVERT(VARCHAR(10),dateadd(day,30,getdate()),112),5,2) '
                   + N' + N''â•˜â”¬'' +  Substring(CONVERT(VARCHAR(10), dateadd(day, 30 , getdate()),112) , 7,2) + N''â•šâ•’â•žâˆ™'',' --26
                   + ' '''','  --27
                   + ' PACKH.Orderkey,'  --28  
                   + ' PACKDET.Cartonno,'  --29  
                   + ' ISNULL(STO.B_Phone2,''''),'  --30  
                   + ' ISNULL(ORD.C_address2,''''),'  --31  
                   + ' ORD.c_contact1,'  --32  
                   + ' ORD.c_contact2,'  --33  
                   + ' ISNULL(LEFT(ORD.notes,80),''''),'  --34  
                   + ' ISNULL(LEFT(ORD.notes2,80),''''),' --35
                   ...
                   + ' LEFT JOIN CODELKUP C2 WITH (NOLOCK) ON C2.Listname = ''HSCARTON'' AND C2.UDF01 = CT.CarrierName AND C2.short=PI.Cartontype'
                   + ' LEFT JOIN CODELKUP C3 WITH (NOLOCK) ON C3.Listname = ''HSTimeslot'' AND C3.Code = ''BLANK'' AND C3.StorerKey = ORD.StorerKey ' 
  ...
  IF @b_debug=1        
   ...
   EXIT_SP:    
      ...
      EXEC isp_InsertTraceInfo   
         ...
         @c_TraceName = 'isp_Bartender_Shipper_Label_JP_Herschel',  
