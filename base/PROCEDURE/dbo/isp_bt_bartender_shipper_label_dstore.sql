...
/* 2014-05-15 2.3  CSCHONG    For Tote Consolidation (CS03)                   */  
/* 2014-05-19 2.4  CSCHONG    Add new Field & modify on Col13 (CS04)          */   
...
BEGIN                              
 ...
 Declare          
      ...
      @c_skuGrp          NCHAR(5),        
      @c_SkuStyle        NCHAR(5),  
      @c_colContent11    NVARCHAR(80),     --(CS04)        
      @c_colContent12    NVARCHAR(80),     --(CS04)       
      @c_colContent13    NVARCHAR(80),     --(CS04)     
      @c_colContent14    NVARCHAR(80),     --(CS04)     
      @c_colContent15    NVARCHAR(80),     --(CS04)          
...
--      DROP TABLE #SKU;        
 ...
 /* SET @c_SQLJOIN = +' SELECT DISTINCT s1.company as ShipTo_Company,(s1.address1+s1.address2+s1.address3+s1.address4) as shipTo_Address,s1.state as ShipTo_State,s1.city as ShipTo_City,s1.zip as ShipTo_Zip ,'    --5                
             + CHAR(13) +                   
             +'s1.country as ShipTo_Country,s2.company as ShipFrom_Company,ISNULL((s2.address1+s2.address2+s2.address3+s2.address4),'''') as shipFrom_address,s2.state as ShipFrom_State,ISNULL(s2.city,'''') as ShipFrom_City,'  --5              
             + CHAR(13) +                  
             +'ISNULL(s2.zip,'''') as ShipFrom_Zip,ISNULL(s2.country,'''') as ShipFrom_Country,ISNULL(ORD.userdefine04,'''') HUdef04,'''',WD.wavekey,'+ CHAR(13) +     --5             
             +'pd.caseid,MAX(ISNULL(od.userdefine01,'''')) as DUDef01,replace(max(od.userdefine02),''ANF'','''') as DUdef02,substring(pd.caseID,7,5),CP.Short, '          
             + ' '''','''','''','''','''','    --25               
             +' '''','''','''',ORD.orderkey,'''','     --30                
             + CHAR(13) +                  
             +' '''','''','''','''','''','''','''','''','''','''','   --40               
             ...
             +' '''','''','''','''','''','''','''','''','''','''' '   --60                  
             + CHAR(13) +                    
             + ' FROM ORDERS ORD INNER JOIN ORDERDETAIL od WITH (NOLOCK) ON od.orderkey=ORD.orderkey  '               
             + ' FULL JOIN STORER s1 WITH (NOLOCK) ON s1.storerkey=od.userdefine02 '                
             + ' FULL JOIN STORER s2 WITH (NOLOCK) ON s2.storerkey = ORD.facility'                
             + ' FULL JOIN Wavedetail WD WITH (NOLOCK) ON WD.Orderkey = ORD.Orderkey '                 
             + ' FULL JOIN SKU s WITH (NOLOCK) ON s.sku=od.sku '           
             ...
             + ' AND ORD.Storerkey =  ''' + @c_Sparm5 + ''' '            
             + 'group by s1.company ,(s1.address1+s1.address2+s1.address3+s1.address4),s1.state,s1.city,s1.zip ,s1.country, '           
             + 's2.company,(s2.address1+s2.address2+s2.address3+s2.address4),s2.state,s2.city ,s2.zip ,s2.country,ORD.userdefine04,WD.wavekey,pd.caseid,substring(pd.caseID,7,5),CP.Short,ORD.orderkey '                 
...
*/          
    ...
    JOIN CODELKUP CP WITH (NOLOCK) ON CP.CODE=ORD.Facility          
    --WHERE ORD.LoadKey =  CASE WHEN ISNULL(RTRIM(@c_Sparm1),'') <> '' THEN @c_Sparm1 ELSE ORD.LoadKey END --@c_Sparm1        --(CS03)    
   WHERE (ISNULL(RTRIM(@c_Sparm1),'') = '' OR ORD.LoadKey = RTRIM(@c_Sparm1))                                               --(CS04)  
    AND CP.listname='ANFFAC'           
    --AND ORD.OrderKey = CASE WHEN ISNULL(RTRIM(@c_Sparm2),'') <> '' THEN @c_Sparm2 ELSE ORD.OrderKey END      
 ...
 --SELECT @c_ORDUdef10 = ISNULL(o.userdefine10,'') ,          
 --       @c_ORDUdef03 = ISNULL(o.userdefine03,'') ,          
 --       @c_ItemClass = s.itemclass,          
 --       @c_SKUGRP = s.skugroup,          
 --       @c_Style = s.style,          
...
--       s.itemclass,s.skugroup,ISNULL(s.style,''),SUM(pd.qty)          
-- FROM orders o --JOIN ORDERDETAIL od WITH (NOLOCK)          
-- --on od.orderkey=o.orderkey          
-- JOIN pickdetail pd WITH (NOLOCK) ON pd.orderkey=o.orderkey          
-- join sku s WITH (NOLOCK) on s.sku=pd.sku AND s.StorerKey = pd.Storerkey            
-- WHERE pd.caseid=@c_caseid1 and pd.orderkey=case when @c_Sparm2 <> '' then @c_Sparm2 else pd.orderkey end          
-- GROUP BY pd.orderkey,          
--          ISNULL(o.userdefine10,'')  ,          
--          ISNULL(o.userdefine03,'') ,           
--          s.itemclass,s.skugroup,s.style                 
   ...
   GROUP BY      
     ...
     SET @c_colContent09 = ''          
     SET @c_colContent10 = ''   
     SET @c_colContent11 = ''  --(CS04)         
     SET @c_colContent12 = ''  --(CS04)        
     SET @c_colContent13 = ''  --(CS04)        
     SET @c_colContent14 = ''  --(CS04)        
     SET @c_colContent15 = ''  --(CS04)          
    ...
    END          
     ...
     BEGIN          
      ...
      /* IF @n_intFlag > 1 and (@n_intFlag%@n_MaxLine) = 1  
       BEGIN          
       --SET @n_intFlag = 1          
...
--       IF @b_debug='1'          
--       BEGIN          
--       SELECT convert(nvarchar(5),@n_CurrentPage) as CurrentPage,* from #Result          
--       END          
...
--        SET @c_colContent15 = ''  --(CS04)  
     ...
     END  */        
       ...
       --SELECT @c_Colcontent = RIGHT(space(20) + DUdef10, 20) + space(2) + RIGHT(space(20) + DUdef03, 20) + space(2) + itemclass + space(2) + skugroup + space(2) + style + space(2) + convert(nvarchar(5),TTLPICKQTY)          
--       SELECT @c_Colcontent = cast(DUdef10 as nchar(2)) + space(2)         
--                             + cast(DUdef03 as nchar(2)) + space(2)         
...
--      SELECT @c_OrdUdef10 = case when DUdef10 = '' then space(2) else DUdef10 END,        
--             @c_OrdUdef03 = case when DUdef03 = '' then space(2) else DUdef03 END,        
--             @c_itemclass = case when itemclass = '' then space(4) else itemclass END,        
--             @c_skuGrp    = case when skugroup = '' then space(5) else skugroup END,        
--             @c_SkuStyle  = case when style = '' then space(5) else style END,        
--             @n_TTLPickQTY = TTLPICKQTY        
--       FROM  #CartonContent c WITH (NOLOCK)                      
--       WHERE c.ID = @n_intFlag          
        
            
    -- SET @c_Colcontent = @c_OrdUdef10 + space(2) + @c_OrdUdef03 + space(2) + @c_itemclass+ space(2) +@c_skuGrp+ space(2) +@c_SkuStyle+ space(2) + convert(nchar(5),@n_TTLPickQTY)        
  ...
  FETCH NEXT FROM CUR_RowNoLoop INTO @c_ToCompany1,@C_ToAddress1,@C_ToState1,@C_ToCity1,@C_ToZip1,@C_ToCountry1,@c_FromCompany1,          
   ...
   EXEC isp_InsertTraceInfo           
      ...
      @c_TraceName = 'isp_BT_Bartender_Shipper_Label_DSTORE',          
