...
/* 19-May-2020  TLTING01   Update editdate, editwho                     */
/* 26-Apr-2021  James      WMS-16634 Add custom gen count sheet(james01)*/
/* 23-Nov-2021  Ung        WMS-17017 Add empty LOC                      */
/************************************************************************/

CREATE PROC [dbo].[ispRDTGenCountSheet] (
...
BEGIN
   ...
   ***********************************************************************************************/
   ...
   DECLARE @cExtendedGenCountSheetSP NVARCHAR(20)
   SET @cExtendedGenCountSheetSP = rdt.RDTGetConfig( @n_Func, 'ExtendedGenCountSheetSP', @c_StorerKey)
   IF @cExtendedGenCountSheetSP = '0'
      SET @cExtendedGenCountSheetSP = ''

   -- Custom cart matrix
   IF @cExtendedGenCountSheetSP <> ''
   BEGIN
      IF EXISTS( SELECT 1 FROM dbo.sysobjects WHERE name = @cExtendedGenCountSheetSP AND type = 'P')
      BEGIN
         SET @cSQL = 'EXEC rdt.' + RTRIM( @cExtendedGenCountSheetSP) +
            ' @nFunc, @cStorerKey, @cStockTakeKey, @cLoc, @cSKU, @cTaskDetailKey' 
         SET @cSQLParam =
            ' @nFunc          INT,  ' +
         ...
         EXEC sp_ExecuteSQL @cSQL, @cSQLParam,
            @n_Func, @c_StorerKey, @c_StockTakeKey, @c_Loc, @c_SKU, @c_TaskDetailKey
...
-- End - SOS23776
  ...
  SELECT @c_CCSheetNoKeyName = 'CSHEET'+LTRIM(RTRIM(@c_StockTakeKey)) --NJOW03
   ...
   BEGIN
      ...
      SET @cPickStatus = rdt.RDTGetConfig( @n_Func, 'CCExcludePICK', @c_StorerKey)
      IF @cPickStatus <> '0'
      BEGIN
         SELECT @nSumPick = SUM(Qty)
         FROM PICKDETAIL WITH (NOLOCK)
         WHERE Storerkey = @c_StorerKey AND ID = ISNULL(@c_id,'')
         AND Loc = @c_loc AND Sku = @c_sku AND Status = @cPickStatus
         GROUP BY Loc,Storerkey

         IF @n_SystemQty > @nSumPick
         BEGIN
            SET @n_SystemQty = @n_SystemQty - @nSumPick
         END
      END
