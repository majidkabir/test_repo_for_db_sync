...
GO
/************************************************************************/
/* Store Procedure: nspConsoPickList25                                  */
/* Creation Date: 22-Dec-2009                                           */
/* Copyright: IDS                                                       */
/* Written by: NJOW                                                     */
/*                                                                      */
/* Purpose: Remy Picking Slip (print from LoadPlan)  SOS#155659         */
/*                                                                      */
/* Input Parameters: @as_LoadKey - (LoadKey)                            */
/*                                                                      */
/* Called By: r_dw_consolidated_pick25_2                                */
/*                                                                      */
/* PVCS Version: 1.0                                                    */
/*                                                                      */
/* Version: 5.4                                                         */
...
CREATE PROC [dbo].[nspConsoPickList25] (@as_LoadKey NVARCHAR(10) )
AS
BEGIN
   SET NOCOUNT ON 
   SET ANSI_NULLS OFF  
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				SELECT @n_continue = 3
				SELECT @n_err = 63501
				SELECT @c_errmsg = 'NSQL'+CONVERT(char(5),@n_err)+': Insert Into PICKHEADER Failed. (nspConsoPickList25)'
			END
   ...
   BEGIN
		SELECT @n_continue = 3
		SELECT @n_err = 63502
		SELECT @c_errmsg = 'NSQL'+CONVERT(char(5),@n_err)+': Get LoadKey Failed. (nspConsoPickList25)'
   END
   ...
   END
	...
	BEGIN
		...
		BEGIN
			...
			END
		END
		EXECUTE nsp_logerror @n_err, @c_errmsg, 'nspConsoPickList25'
