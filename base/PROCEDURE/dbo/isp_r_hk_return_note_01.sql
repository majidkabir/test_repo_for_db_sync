...
GO
/*************************************************************************/  
/* Stored Procedure: isp_r_hk_return_note_01                             */  
...
/* Purpose: Return note                                                  */  
/*                                                                       */  
/* Called By: Report Module. Datawidnow r_hk_return_note_01              */  
/*                                                                       */  
/* PVCS Version: 1.0                                                     */  
/*                                                                       */  
/* Version: 7.0                                                          */  
...
/* CODELKUP.REPORTCFG  
   [MAPFIELD]  
      BrandLogoCode, ReportTitle, SplitPrintKey, DocNumber, ExternReceiptkey, LFLRefNo, ReferenceNo, ReferenceNo2, ReferenceNo3, ReferenceNo4, ReferenceNo5  
      BilltoKey, B_Company, B_Address, ConsigneeKey, C_Company, C_Address  
      ...
      SubTotalRef1, SubTotalRef2, SubTotalRef3  
      SizeScale_FieldName, SizeScale_ListName, ShowField  
      ConsigneePrefix  
   [MAPVALUE]  
      T_CopyDescr, T_ReportTitle, T_BillTo, T_ShipTo  
      T_ReceiptDate, T_DocNumber, T_LFLRefNo, T_ReferenceNo, T_ReferenceNo2, T_ReferenceNo3, T_ReferenceNo4, T_ReferenceNo5, T_Remark  
      ...
      T_SubTotal, T_TotalQty, T_TotalAmount, T_ApprovedBy, T_TermsNCond1, T_TermsNCond2  
      ...
      N_Xpos_ApprovedBy, N_Xpos_TotalQty, N_Xpos_TotalAmount, N_Xpos_SubTotalRef1, N_Xpos_SubTotalRef2, N_Xpos_SubTotalRef3, N_Ypos_ApprovedBy  
      ...
      N_Width_TotalQty, N_Width_TotalAmount, N_Width_SubTotalRef1, N_Width_SubTotalRef2, N_Width_SubTotalRef3, N_Width_ApprovedBy  
      N_NoOfSizeCol  
   [SHOWFIELD]  
      UseLFLogo, Barcode, UseCode39, FullSizeScale, UsePackDetail  
      Storer_B_ComAddr, AddressDirectConcate, OrderType  
      ReferenceNo, ReferenceNo2, ReferenceNo3, ReferenceNo4, ReferenceNo5  
      HideLFLRefNo, HideStorerCompany, HideStorerAddress, HideBillToKey, HideBillToCompany, HideBillToAddress  
      HideConsigneeKey, HideShipToCompany, HideShipToAddress, HideRemark  
      HideStyle, HideColor, HideMeas, HideDescr, HideTotalQty, HideTotalCarton, HideApprovedBy, HideDataWindowName  
      ...
      LineGrouping_Separateline, SubTotal  
      BoldReceiptDate, BoldDocNumber, BoldLFLRefNo, BoldReferenceNo, BoldReferenceNo2, BoldReferenceNo3, BoldReferenceNo4, BoldReferenceNo5  
      WaterMark  
   [SQLJOIN]  
   [SQLWHERE]  
*/  
   ...
   IF OBJECT_ID('tempdb..#TEMP_EXTERNRECEIPTKEY') IS NOT NULL  
      DROP TABLE #TEMP_EXTERNRECEIPTKEY  
   IF OBJECT_ID('tempdb..#TEMP_FINALRECEIPTKEY') IS NOT NULL  
      DROP TABLE #TEMP_FINALRECEIPTKEY  
   IF OBJECT_ID('tempdb..#TEMP_RECDT') IS NOT NULL  
      DROP TABLE #TEMP_RECDT  
   ...
   DECLARE @c_DataWidnow         NVARCHAR(40)  
         ...
         , @c_ExtReceiptkeyExp   NVARCHAR(4000)  
         ...
         , @c_SubTotalRef1Exp    NVARCHAR(4000)  
         , @c_SubTotalRef2Exp    NVARCHAR(4000)  
         , @c_SubTotalRef3Exp    NVARCHAR(4000)  
         ...
         , @c_ExternReceiptkey   NVARCHAR(500)  
         ...
         , @c_BilltoKey          NVARCHAR(500)  
         , @c_B_Company          NVARCHAR(500)  
         , @c_B_Address          NVARCHAR(500)  
         , @c_ConsigneeKey       NVARCHAR(500)  
         , @c_C_Company          NVARCHAR(500)  
         , @c_C_Address          NVARCHAR(500)  
         ...
         , @n_Amount             FLOAT  
         , @n_GrossAmount        FLOAT  
         , @c_SubTotalRef1       NVARCHAR(500)  
         , @c_SubTotalRef2       NVARCHAR(500)  
         , @c_SubTotalRef3       NVARCHAR(500)  
         , @c_ShowField          NVARCHAR(4000)  
         , @c_UOM                NVARCHAR(10)  
         , @c_BRAND_Logo_Code    NVARCHAR(500)  
         , @c_DocKey             NVARCHAR(10)  
         , @c_FirstReceiptkey    NVARCHAR(10)  
   ...
   SELECT @c_DataWidnow = 'r_hk_return_note_01'  
   ...
   SELECT SeqNo    = MIN(SeqNo)  
     ...
     FROM dbo.fnc_DelimSplit(',',replace(@as_receiptkey,char(13)+char(10),','))  
    WHERE ColValue<>''  
    GROUP BY LTRIM(RTRIM(ColValue))  
  
   SET @n_ReceiptkeyCnt = @@ROWCOUNT  
  
   -- ExternReceiptkey List  
   SELECT SeqNo    = MIN(SeqNo)  
        , ColValue = LTRIM(RTRIM(ColValue))  
     INTO #TEMP_EXTERNRECEIPTKEY  
     FROM dbo.fnc_DelimSplit(',',replace(@as_externReceiptkey,char(13)+char(10),','))  
    WHERE ColValue<>''  
    GROUP BY LTRIM(RTRIM(ColValue))  
  
   SET @n_ExternReceiptkeyCnt = @@ROWCOUNT  
  
  
   -- Final Receiptkey List  
   SELECT Receiptkey     = RH.Receiptkey  
        , DocKey         = MAX( RH.Receiptkey )  
        , Storerkey      = MAX( RH.Storerkey )  
     INTO #TEMP_FINALRECEIPTKEY  
     FROM dbo.RECEIPT       RH (NOLOCK)  
    WHERE ( RH.Storerkey = @as_storerkey )  
      AND ( @n_ExternReceiptkeyCnt>0 OR @n_ReceiptkeyCnt>0 )  
      AND (@n_ExternReceiptkeyCnt=0 OR RH.ExternReceiptkey IN (SELECT ColValue FROM #TEMP_EXTERNRECEIPTKEY ) )  
      AND (@n_ReceiptkeyCnt=0 OR RH.Receiptkey IN (SELECT ColValue FROM #TEMP_RECEIPTKEY ) )  
   ...
   BEGIN  
      ...
      SELECT TOP 1  
       ...
       SELECT TOP 1  
             @c_WhereClause  = Notes  
        FROM dbo.CodeLkup (NOLOCK)  
       WHERE Listname='REPORTCFG' AND Code='SQLWHERE' AND Long=@c_DataWidnow AND Short='Y'  
         AND Storerkey = @c_Storerkey  
       ORDER BY Code2  
      ...
      SELECT TOP 1  
           ...
           , @c_ExtReceiptkeyExp   = ISNULL(RTRIM((select top 1 b.ColValue  
                                     from dbo.fnc_DelimSplit(LTRIM(RTRIM(UDF01)),RTRIM(Notes)) a, dbo.fnc_DelimSplit(LTRIM(RTRIM(UDF01)),RTRIM(Notes2)) b  
                                     where a.SeqNo=b.SeqNo and a.ColValue='ExternReceiptkey')), '' )  
           ...
           , @c_SubTotalRef1Exp    = ISNULL(RTRIM((select top 1 b.ColValue  
                                     from dbo.fnc_DelimSplit(LTRIM(RTRIM(UDF01)),RTRIM(Notes)) a, dbo.fnc_DelimSplit(LTRIM(RTRIM(UDF01)),RTRIM(Notes2)) b  
                                     where a.SeqNo=b.SeqNo and a.ColValue='SubTotalRef1')), '' )  
           , @c_SubTotalRef2Exp    = ISNULL(RTRIM((select top 1 b.ColValue  
                                     from dbo.fnc_DelimSplit(LTRIM(RTRIM(UDF01)),RTRIM(Notes)) a, dbo.fnc_DelimSplit(LTRIM(RTRIM(UDF01)),RTRIM(Notes2)) b  
                                     where a.SeqNo=b.SeqNo and a.ColValue='SubTotalRef2')), '' )  
           , @c_SubTotalRef3Exp    = ISNULL(RTRIM((select top 1 b.ColValue  
                                     from dbo.fnc_DelimSplit(LTRIM(RTRIM(UDF01)),RTRIM(Notes)) a, dbo.fnc_DelimSplit(LTRIM(RTRIM(UDF01)),RTRIM(Notes2)) b  
                                     where a.SeqNo=b.SeqNo and a.ColValue='SubTotalRef3')), '' )  
      ...
      ----------  
      SET @c_ExecStatements = N'INSERT INTO #TEMP_RECDT'  
          +' (Receiptkey, Storerkey, ReportTitle, SplitPrintKey, DocNumber, ExternReceiptkey, LFLRefNo, ReferenceNo, ReferenceNo2'  
          + ', ReferenceNo3, ReferenceNo4, ReferenceNo5, BilltoKey, B_Company, B_Address, ConsigneeKey, C_Company, C_Address, Remark'  
          + ', LineGrouping, ReceiptLineNumber, Sku, Style, Color, Measurement, Size'  
          + ', SizeScaleSeq, Descr, LineRemark, LineRef1, LineRef2, LineRef3'  
          + ', Qty, UOM, Unitprice, Discount, Amount, GrossAmount, SubTotalRef1, SubTotalRef2, SubTotalRef3, ShowField'  
          + ', ConsigneePrefix, BRAND_Logo_Code'  
          + ', DocKey, FirstReceiptkey)'  
      ...
      SET @c_ExecStatements +=  
                 ', ISNULL(RTRIM(' + CASE WHEN ISNULL(@c_DocNumberExp      ,'')<>'' THEN @c_DocNumberExp       ELSE 'RH.ExternReceiptkey' END + '),'''')'  
      SET @c_ExecStatements +=  
                 ', ISNULL(RTRIM(' + CASE WHEN ISNULL(@c_ExtReceiptkeyExp  ,'')<>'' THEN @c_ExtReceiptkeyExp   ELSE 'RH.ExternReceiptkey' END + '),'''')'  
      SET @c_ExecStatements +=  
                 ', ISNULL(RTRIM(' + CASE WHEN ISNULL(@c_LFLRefNoExp       ,'')<>'' THEN @c_LFLRefNoExp        ELSE 'RH.Receiptkey' END + '),'''')'  
      SET @c_ExecStatements +=  
      ...
      SET @c_ExecStatements +=  
                 ', ISNULL(RTRIM(' + CASE WHEN ISNULL(@c_BilltoKeyExp      ,'')<>'' THEN @c_BilltoKeyExp       ELSE '''''' END + '),'''')'  
      SET @c_ExecStatements +=  
                 ', ISNULL(RTRIM(' + CASE WHEN ISNULL(@c_B_CompanyExp      ,'')<>'' THEN @c_B_CompanyExp       ELSE '''''' END + '),'''')'  
      SET @c_ExecStatements +=  
                 ', ISNULL(RTRIM(' + CASE WHEN ISNULL(@c_B_AddressExp      ,'')<>'' THEN @c_B_AddressExp       ELSE '''''' END + '),'''')'  
      SET @c_ExecStatements +=  
                 ', ISNULL(RTRIM(' + CASE WHEN ISNULL(@c_ConsigneeKeyExp   ,'')<>'' THEN @c_ConsigneeKeyExp    ELSE '''''' END + '),'''')'  
      SET @c_ExecStatements +=  
                 ', ISNULL(RTRIM(' + CASE WHEN ISNULL(@c_C_CompanyExp      ,'')<>'' THEN @c_C_CompanyExp       ELSE '''''' END + '),'''')'  
      SET @c_ExecStatements +=  
                 ', ISNULL(RTRIM(' + CASE WHEN ISNULL(@c_C_AddressExp      ,'')<>'' THEN @c_C_AddressExp       ELSE '''''' END + '),'''')'  
      SET @c_ExecStatements +=  
                 ', ISNULL(RTRIM(' + CASE WHEN ISNULL(@c_RemarkExp         ,'')<>'' THEN @c_RemarkExp          ELSE 'RH.Notes' END + '),'''')'  
      SET @c_ExecStatements +=  
      ...
      SET @c_ExecStatements +=  
                 ', ISNULL('       + CASE WHEN ISNULL(@c_QtyExp            ,'')<>'' THEN @c_QtyExp             ELSE 'RD.QtyExpected'    END + ',0)'  
      SET @c_ExecStatements +=  
                 ', ISNULL(RTRIM(' + CASE WHEN ISNULL(@c_UOMExp            ,'')<>'' THEN @c_UOMExp             ELSE 'PACK.PackUOM3'     END + '),'''')'  
      SET @c_ExecStatements +=  
                 ', '              + CASE WHEN ISNULL(@c_UnitpriceExp      ,'')<>'' THEN @c_UnitpriceExp       ELSE 'NULL'              END  
      SET @c_ExecStatements +=  
      ...
      SET @c_ExecStatements +=  
                 ', ISNULL(RTRIM(' + CASE WHEN ISNULL(@c_SubTotalRef1Exp   ,'')<>'' THEN @c_SubTotalRef1Exp    ELSE ''''''              END + '),'''')'  
      SET @c_ExecStatements +=  
                 ', ISNULL(RTRIM(' + CASE WHEN ISNULL(@c_SubTotalRef2Exp   ,'')<>'' THEN @c_SubTotalRef2Exp    ELSE ''''''              END + '),'''')'  
      SET @c_ExecStatements +=  
                 ', ISNULL(RTRIM(' + CASE WHEN ISNULL(@c_SubTotalRef3Exp   ,'')<>'' THEN @c_SubTotalRef3Exp    ELSE ''''''              END + '),'''')'  
      SET @c_ExecStatements +=  
      ...
      SET @c_ExecStatements +=  
               ...
               + ', FRK.DocKey'  
               + ', FIRST_VALUE(FRK.Receiptkey) OVER(PARTITION BY FRK.DocKey ORDER BY FRK.Receiptkey)'  
          +' FROM dbo.RECEIPT RH (NOLOCK)'  
      SET @c_ExecStatements +=  
           ' JOIN #TEMP_FINALRECEIPTKEY FRK ON RH.Receiptkey=FRK.Receiptkey'  
          +' JOIN dbo.RECEIPTDETAIL RD (NOLOCK) ON RH.Receiptkey=RD.Receiptkey'  
          +' LEFT JOIN dbo.SKU     SKU (NOLOCK) ON RD.StorerKey=SKU.StorerKey AND RD.Sku=SKU.Sku'  
      ...
      BEGIN  
         UPDATE #TEMP_RECDT SET LineGrouping = Style +'|'+ Color +'|'+ Measurement  
      END  
      ...
      BEGIN  
         SET @c_ExecStatements = N'DECLARE C_SIZESCALE CURSOR FAST_FORWARD READ_ONLY FOR'  
                               ...
                               +       ', RECDT.LineGrouping'  
                               +       ', SS.Notes2'  
                               +   ' FROM #TEMP_RECDT  RECDT'  
                               +   ' JOIN dbo.SKU        SKU (NOLOCK) ON RECDT.Storerkey=SKU.Storerkey AND RECDT.Sku=SKU.Sku'  
                               +   ' JOIN dbo.RECEIPTDETAIL RD (NOLOCK) ON RECDT.Receiptkey=RD.Receiptkey AND RECDT.ReceiptLineNumber=RD.ReceiptLineNumber'  
                               +   ' JOIN dbo.RECEIPT       RH (NOLOCK) ON RECDT.Receiptkey=RH.Receiptkey'  
                               +   ' JOIN dbo.CodeLkup    SS (NOLOCK)'  
                               +     ' ON SS.Listname=@c_SS_Listname AND RECDT.Storerkey=SS.Storerkey AND SS.Short=' + ISNULL(@c_SS_FieldNameExp,'')  
                               +   ' WHERE RECDT.Storerkey = @c_Storerkey'  
         ...
         BEGIN  
            ...
            BEGIN  
               ...
               SELECT TOP 1  
                    ...
                    , @c_SubTotalRef1      = SubTotalRef1  
                    , @c_SubTotalRef2      = SubTotalRef2  
                    , @c_SubTotalRef3      = SubTotalRef3  
                    , @c_ShowField         = ShowField  
                    , @c_UOM               = UOM  
                    , @c_ConsigneePrefix   = ConsigneePrefix  
                    , @c_BRAND_Logo_Code   = BRAND_Logo_Code  
                    , @c_FirstReceiptkey   = FirstReceiptkey  
                 FROM #TEMP_RECDT  
                WHERE DocKey=@c_DocKey AND Storerkey=@c_Storerkey AND LineGrouping=@c_LineGrouping  
                ORDER BY ReceiptLineNumber, Style, Color, Sku  
  
               INSERT INTO #TEMP_RECDT  
                     (Receiptkey, Storerkey, ReportTitle, SplitPrintKey, DocNumber, ExternReceiptkey, LFLRefNo, ReferenceNo, ReferenceNo2  
                    , ReferenceNo3, ReferenceNo4, ReferenceNo5, BilltoKey, B_Company, B_Address, ConsigneeKey, C_Company, C_Address  
                    , Remark, ReceiptLineNumber, Sku, LineGrouping, Style, Color, Measurement, Size, SizeScaleSeq  
                    , Descr, LineRemark, LineRef1, LineRef2, LineRef3  
                    , Qty, Unitprice, Discount, Amount, GrossAmount, SubTotalRef1, SubTotalRef2, SubTotalRef3, ShowField, UOM  
                    , ConsigneePrefix, BRAND_Logo_Code  
                    , DocKey, FirstReceiptkey)  
               SELECT @c_Receiptkey, @c_Storerkey, @c_ReportTitle, @c_SplitPrintKey, @c_DocNumber, @c_ExternReceiptkey, @c_LFLRefNo, @c_ReferenceNo, @c_ReferenceNo2  
                    , @c_ReferenceNo3, @c_ReferenceNo4, @c_ReferenceNo5, @c_BilltoKey, @c_B_Company, @c_B_Address, @c_ConsigneeKey, @c_C_Company, @c_C_Address  
                    , @c_Remark, @c_ReceiptLineNumber, '', @c_LineGrouping, @c_Style, @c_Color, @c_Measurement, LTRIM(RTRIM(a.ColValue)), 0  
                    , @c_Descr, @c_LineRemark, @c_LineRef1, @c_LineRef2, @c_LineRef3  
                    , 0, @n_Unitprice, @n_Discount, @n_Amount, @n_GrossAmount, @c_SubTotalRef1, @c_SubTotalRef2, @c_SubTotalRef3, @c_ShowField, @c_UOM  
                    , @c_ConsigneePrefix, @c_BRAND_Logo_Code  
                    , @c_DocKey, @c_FirstReceiptkey  
                 FROM dbo.fnc_DelimSplit(',', @c_FullSizeScale) a  
                 LEFT JOIN #TEMP_RECDT b ON LTRIM(RTRIM(a.ColValue)) = b.Size  
             ...
             UPDATE b  
               ...
               FROM dbo.fnc_DelimSplit(',', @c_FullSizeScale) a  
               JOIN #TEMP_RECDT b ON LTRIM(RTRIM(a.ColValue)) = b.Size  
                     AND b.DocKey=@c_DocKey AND b.Storerkey=@c_Storerkey AND b.LineGrouping=@c_LineGrouping  
         END  
   ...
   UPDATE a  
      SET LFLRefNo        = b.LFLRefNo  
        ...
        , Remark          = b.Remark  
        , SubTotalRef1    = b.SubTotalRef1  
        , SubTotalRef2    = b.SubTotalRef2  
        , SubTotalRef3    = b.SubTotalRef3  
     FROM #TEMP_RECDT a  
     JOIN (  
        SELECT *, SeqNo = ROW_NUMBER() OVER(PARTITION BY DocKey ORDER BY Receiptkey)  
          FROM #TEMP_RECDT  
     ) b ON a.DocKey = b.DocKey AND b.SeqNo = 1  
  
  
   ------------------------  
   SELECT Storerkey         = UPPER( RTRIM ( RECDT.Storerkey ) )  
        , DocKey            = ISNULL( RTRIM( RECDT.DocKey ), '' )  
        , Receiptkey        = MAX ( RTRIM ( RECDT.Receiptkey ) )  
        , DocNumber         = MAX ( ISNULL( RTRIM ( RECDT.DocNumber ), '') )  
        , ExternReceiptkey  = MAX ( ISNULL( RTRIM ( RECDT.ExternReceiptkey ), '') )  
        , Loadkey           = MAX ( ISNULL( RTRIM ( RH.Loadkey ), '') )  
        , ReceiptDate       = MAX ( RH.ReceiptDate )  
        , ReferenceNo       = MAX ( ISNULL( RTRIM ( RECDT.ReferenceNo ), '') )  
        ...
        , BilltoKey         = RTRIM ( ISNULL( MAX( CASE WHEN LEFT(RECDT.BilltoKey, LEN(RECDT.ConsigneePrefix))=RECDT.ConsigneePrefix  
                                           THEN SUBSTRING(RECDT.BilltoKey, LEN(RECDT.ConsigneePrefix)+1, LEN(RECDT.BillToKey))  
                                           ELSE RECDT.BilltoKey END ), '' ) )  
        , B_Company         = MAX ( ISNULL( RTRIM ( RECDT.B_Company ), '' ) )  
        , B_Address         = MAX ( ISNULL( RECDT.B_Address , '' ) )  
  
        , ConsigneeKey      = RTRIM ( ISNULL( MAX( CASE WHEN LEFT(RECDT.ConsigneeKey, LEN(RECDT.ConsigneePrefix))=RECDT.ConsigneePrefix  
                                           THEN SUBSTRING(RECDT.ConsigneeKey, LEN(RECDT.ConsigneePrefix)+1, LEN(RECDT.ConsigneeKey))  
                                           ELSE RECDT.ConsigneeKey END ), '' ) )  
        , C_Company         = MAX ( ISNULL( RTRIM ( RECDT.C_Company ), '' ) )  
        , C_Address         = MAX ( ISNULL( RECDT.C_Address, '' ) )  
  
        , Notes             = MAX ( ISNULL( RTRIM ( RH.Notes ), '' ) )  
  
        , LineGrouping      = RTRIM ( RECDT.LineGrouping )  
        , Line_No           = ROW_NUMBER() OVER(PARTITION BY RECDT.DocKey, COPY.Copies  
                              ORDER BY RECDT.LineGrouping, FLOOR((SSEQ.SizeSeq-1)/@n_Col), RECDT.Style, RECDT.Color, RECDT.Measurement, RECDT.Unitprice)  
        , Style             = RTRIM ( RECDT.Style )  
        , Color             = RTRIM ( RECDT.Color )  
        , Measurement       = RTRIM ( RECDT.Measurement )  
        , Descr             = MAX ( RTRIM ( RECDT.Descr ) )  
        , Unitprice         = RECDT.Unitprice  
        , UOM               = MAX ( RTRIM ( RECDT.UOM ) )  
        , Discount          = RECDT.Discount  
        , Amount            = RECDT.Amount  
        , SizeLine          = FLOOR((SSEQ.SizeSeq-1)/@n_Col)  
  
        , Size01            = MAX ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col= 0 THEN LTRIM(RTRIM(RECDT.Size)) ELSE '' END )  
        , Size02            = MAX ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col= 1 THEN LTRIM(RTRIM(RECDT.Size)) ELSE '' END )  
        , Size03            = MAX ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col= 2 THEN LTRIM(RTRIM(RECDT.Size)) ELSE '' END )  
        , Size04            = MAX ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col= 3 THEN LTRIM(RTRIM(RECDT.Size)) ELSE '' END )  
        , Size05            = MAX ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col= 4 THEN LTRIM(RTRIM(RECDT.Size)) ELSE '' END )  
        , Size06            = MAX ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col= 5 THEN LTRIM(RTRIM(RECDT.Size)) ELSE '' END )  
        , Size07            = MAX ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col= 6 THEN LTRIM(RTRIM(RECDT.Size)) ELSE '' END )  
        , Size08            = MAX ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col= 7 THEN LTRIM(RTRIM(RECDT.Size)) ELSE '' END )  
        , Size09            = MAX ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col= 8 THEN LTRIM(RTRIM(RECDT.Size)) ELSE '' END )  
        , Size10            = MAX ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col= 9 THEN LTRIM(RTRIM(RECDT.Size)) ELSE '' END )  
        , Size11            = MAX ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=10 THEN LTRIM(RTRIM(RECDT.Size)) ELSE '' END )  
        , Size12            = MAX ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=11 THEN LTRIM(RTRIM(RECDT.Size)) ELSE '' END )  
        , Size13            = MAX ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=12 THEN LTRIM(RTRIM(RECDT.Size)) ELSE '' END )  
        , Size14            = MAX ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=13 THEN LTRIM(RTRIM(RECDT.Size)) ELSE '' END )  
        , Size15            = MAX ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=14 THEN LTRIM(RTRIM(RECDT.Size)) ELSE '' END )  
        , Size16            = MAX ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=15 THEN LTRIM(RTRIM(RECDT.Size)) ELSE '' END )  
        , Size17            = MAX ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=16 THEN LTRIM(RTRIM(RECDT.Size)) ELSE '' END )  
        , Size18            = MAX ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=17 THEN LTRIM(RTRIM(RECDT.Size)) ELSE '' END )  
        , Size19            = MAX ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=18 THEN LTRIM(RTRIM(RECDT.Size)) ELSE '' END )  
        , Size20            = MAX ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=19 THEN LTRIM(RTRIM(RECDT.Size)) ELSE '' END )  
        , Size21            = MAX ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=20 THEN LTRIM(RTRIM(RECDT.Size)) ELSE '' END )  
        , Size22            = MAX ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=21 THEN LTRIM(RTRIM(RECDT.Size)) ELSE '' END )  
        , Size23            = MAX ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=22 THEN LTRIM(RTRIM(RECDT.Size)) ELSE '' END )  
        , Size24            = MAX ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=23 THEN LTRIM(RTRIM(RECDT.Size)) ELSE '' END )  
  
        , Qty01             = SUM ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col= 0 THEN RECDT.Qty ELSE 0 END)  
        , Qty02             = SUM ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col= 1 THEN RECDT.Qty ELSE 0 END)  
        , Qty03             = SUM ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col= 2 THEN RECDT.Qty ELSE 0 END)  
        , Qty04             = SUM ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col= 3 THEN RECDT.Qty ELSE 0 END)  
        , Qty05             = SUM ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col= 4 THEN RECDT.Qty ELSE 0 END)  
        , Qty06             = SUM ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col= 5 THEN RECDT.Qty ELSE 0 END)  
        , Qty07             = SUM ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col= 6 THEN RECDT.Qty ELSE 0 END)  
        , Qty08             = SUM ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col= 7 THEN RECDT.Qty ELSE 0 END)  
        , Qty09             = SUM ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col= 8 THEN RECDT.Qty ELSE 0 END)  
        , Qty10             = SUM ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col= 9 THEN RECDT.Qty ELSE 0 END)  
        , Qty11             = SUM ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=10 THEN RECDT.Qty ELSE 0 END)  
        , Qty12             = SUM ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=11 THEN RECDT.Qty ELSE 0 END)  
        , Qty13             = SUM ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=12 THEN RECDT.Qty ELSE 0 END)  
        , Qty14             = SUM ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=13 THEN RECDT.Qty ELSE 0 END)  
        , Qty15             = SUM ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=14 THEN RECDT.Qty ELSE 0 END)  
      , Qty16             = SUM ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=15 THEN RECDT.Qty ELSE 0 END)  
        , Qty17             = SUM ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=16 THEN RECDT.Qty ELSE 0 END)  
        , Qty18             = SUM ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=17 THEN RECDT.Qty ELSE 0 END)  
        , Qty19             = SUM ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=18 THEN RECDT.Qty ELSE 0 END)  
        , Qty20             = SUM ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=19 THEN RECDT.Qty ELSE 0 END)  
        , Qty21             = SUM ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=20 THEN RECDT.Qty ELSE 0 END)  
        , Qty22             = SUM ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=21 THEN RECDT.Qty ELSE 0 END)  
        , Qty23             = SUM ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=22 THEN RECDT.Qty ELSE 0 END)  
        , Qty24             = SUM ( CASE WHEN (SSEQ.SizeSeq-1)%@n_Col=23 THEN RECDT.Qty ELSE 0 END)  
        ...
        , ShowFields        = CASE WHEN ISNULL(MAX(RptCfg.ShowFields),'') = ''  
                                   THEN ',' + MAX(LOWER(RECDT.ShowField)) + ','  
                                   ELSE MAX(RptCfg.ShowFields) + ISNULL( MAX(LOWER(RECDT.ShowField)) + LEFT(MAX(RptCfg.ShowFields),1), '')  
                              END  
        , SortReceiptkey      = MAX ( CASE WHEN @as_sortbyinputseq='Y' THEN '' ELSE RTRIM(RECDT.Storerkey)+'|'+RTRIM(RECDT.DocKey) END )  
        , SeqERK            = MAX ( ISNULL( SelERK.SeqNo, 0 ) )  
        , SeqRK             = MAX ( ISNULL( SelRK.SeqNo, 0 ) )  
        , datawindow        = @c_DataWidnow  
  
        , RecType           = MAX ( ISNULL( RTRIM ( RH.RECType ), '' ) )  
        , ReferenceNo2      = MAX ( ISNULL( RTRIM ( RECDT.ReferenceNo2 ), '') )  
        , ReferenceNo3      = MAX ( ISNULL( RTRIM ( RECDT.ReferenceNo3 ), '') )  
        , ReferenceNo4      = MAX ( ISNULL( RTRIM ( RECDT.ReferenceNo4 ), '') )  
        , ReferenceNo5      = MAX ( ISNULL( RTRIM ( RECDT.ReferenceNo5 ), '') )  
        , Remark            = MAX ( ISNULL( RTRIM ( RECDT.Remark ), '') )  
        , LineRemark        = MAX ( ISNULL( RTRIM ( RECDT.LineRemark ), '') )  
        , LineRef1          = RTRIM ( RECDT.LineRef1 )  
        , LineRef2          = RTRIM ( RECDT.LineRef2 )  
        , LineRef3          = RTRIM ( RECDT.LineRef3 )  
        ...
        , Lbl_ReceiptDate  = CAST( RTRIM( (select top 1 b.ColValue  
                                   from dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes)) a, dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes2)) b  
                                   where a.SeqNo=b.SeqNo and a.ColValue='T_ReceiptDate') ) AS NVARCHAR(500))  
        ...
        , Lbl_ApprovedBy    = CAST( RTRIM( (select top 1 replace(b.ColValue, '\n', char(10))  
                                   from dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes)) a, dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes2)) b  
                                   where a.SeqNo=b.SeqNo and a.ColValue='T_ApprovedBy') ) AS NVARCHAR(500))  
        ...
        , N_Xpos_ApprovedBy = CAST( RTRIM( (select top 1 b.ColValue  
                                   from dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes)) a, dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes2)) b  
                                   where a.SeqNo=b.SeqNo and a.ColValue='N_Xpos_ApprovedBy') ) AS NVARCHAR(50))  
        , N_Xpos_TotalQty   = CAST( RTRIM( (select top 1 b.ColValue  
                                   from dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes)) a, dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes2)) b  
                                   where a.SeqNo=b.SeqNo and a.ColValue='N_Xpos_TotalQty') ) AS NVARCHAR(50))  
        , N_Ypos_ApprovedBy = CAST( RTRIM( (select top 1 b.ColValue  
                                   from dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes)) a, dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes2)) b  
                                   where a.SeqNo=b.SeqNo and a.ColValue='N_Ypos_ApprovedBy') ) AS NVARCHAR(50))  
    ...
    from dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes)) a, dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes2)) b  
        ...
        , N_Width_ApprovedBy= CAST( RTRIM( (select top 1 b.ColValue  
                                   from dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes)) a, dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes2)) b  
                                   where a.SeqNo=b.SeqNo and a.ColValue='N_Width_ApprovedBy') ) AS NVARCHAR(50))  
        ...
        , GrossAmount       = RECDT.GrossAmount  
        , SubTotalRef1      = MAX( RTRIM ( RECDT.SubTotalRef1 ) )  
        , SubTotalRef2      = MAX( RTRIM ( RECDT.SubTotalRef2 ) )  
        , SubTotalRef3      = MAX( RTRIM ( RECDT.SubTotalRef3 ) )  
  
        , S_Fax2            = MAX ( ISNULL( LTRIM(RTRIM(STORER.Fax2)), '' ) )  
        , S_B_Fax2          = MAX ( ISNULL( LTRIM(RTRIM(STORER.B_Fax2)),'' ) )  
        , ReportTitle       = MAX ( RTRIM ( RECDT.ReportTitle ) )  
        , LFLRefNo          = MAX ( ISNULL( RTRIM ( RECDT.LFLRefNo ), '') )  
        ...
        , Lbl_SubTotal      = CAST( RTRIM( (select top 1 b.ColValue  
                                   from dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes)) a, dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes2)) b  
                                   where a.SeqNo=b.SeqNo and a.ColValue='T_SubTotal') ) AS NVARCHAR(500))  
        ...
        , N_Xpos_SubTotalRef1= CAST( RTRIM( (select top 1 b.ColValue  
                                   from dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes)) a, dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes2)) b  
                                   where a.SeqNo=b.SeqNo and a.ColValue='N_Xpos_SubTotalRef1') ) AS NVARCHAR(50))  
        , N_Xpos_SubTotalRef2= CAST( RTRIM( (select top 1 b.ColValue  
                                   from dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes)) a, dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes2)) b  
                                   where a.SeqNo=b.SeqNo and a.ColValue='N_Xpos_SubTotalRef2') ) AS NVARCHAR(50))  
        , N_Xpos_SubTotalRef3= CAST( RTRIM( (select top 1 b.ColValue  
                                   from dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes)) a, dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes2)) b  
                                   where a.SeqNo=b.SeqNo and a.ColValue='N_Xpos_SubTotalRef3') ) AS NVARCHAR(50))  
        ...
        , N_Width_SubTotalRef1= CAST( RTRIM( (select top 1 b.ColValue  
                                   from dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes)) a, dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes2)) b  
                                   where a.SeqNo=b.SeqNo and a.ColValue='N_Width_SubTotalRef1') ) AS NVARCHAR(50))  
        , N_Width_SubTotalRef2= CAST( RTRIM( (select top 1 b.ColValue  
                                   from dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes)) a, dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes2)) b  
                                   where a.SeqNo=b.SeqNo and a.ColValue='N_Width_SubTotalRef2') ) AS NVARCHAR(50))  
        , N_Width_SubTotalRef3= CAST( RTRIM( (select top 1 b.ColValue  
                                   from dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes)) a, dbo.fnc_DelimSplit(MAX(RptCfg3.Delim),MAX(RptCfg3.Notes2)) b  
                                   where a.SeqNo=b.SeqNo and a.ColValue='N_Width_SubTotalRef3') ) AS NVARCHAR(50))  
        , SplitPrintKey     = MAX ( ISNULL( RTRIM ( RECDT.SplitPrintKey ), '') )  
  
   FROM #TEMP_RECDT RECDT  
   JOIN dbo.RECEIPT    RH (NOLOCK) ON (RECDT.FirstReceiptkey = RH.Receiptkey)  
   JOIN dbo.STORER STORER (NOLOCK) ON (RH.StorerKey = STORER.StorerKey)  
   LEFT OUTER JOIN #TEMP_SSEQ SSEQ ON (RECDT.Storerkey = SSEQ.Storerkey AND RECDT.DocKey = SSEQ.DocKey  
                                   AND RECDT.LineGrouping = SSEQ.LineGrouping AND RECDT.Size = SSEQ.Size)  
   LEFT OUTER JOIN dbo.CODELKUP BL (NOLOCK) ON (BL.Listname = 'BRAND_LOGO' AND BL.Storerkey = RECDT.Storerkey AND BL.Code = RECDT.BRAND_Logo_Code)  
   LEFT OUTER JOIN dbo.CODELKUP RL (NOLOCK) ON (RL.Listname = 'RPTLOGO' AND RL.Code='LOGO' AND RL.Storerkey = RECDT.Storerkey AND RL.Long = @c_DataWidnow)  
   ...
   ON RptCfg.Storerkey=RECDT.Storerkey AND RptCfg.SeqNo=1  
   ...
   ON RptCfg3.Storerkey=RECDT.Storerkey AND RptCfg3.SeqNo=1  
  
   LEFT JOIN #TEMP_EXTERNRECEIPTKEY SelERK ON RECDT.ExternReceiptkey = SelERK.ColValue  
   LEFT JOIN #TEMP_RECEIPTKEY       SelRK  ON RECDT.Receiptkey       = SelRK.ColValue  
   LEFT JOIN #TEMP_COPYDESCR        COPY   ON RECDT.Storerkey        = COPY.Storerkey  
  
   GROUP BY RECDT.Storerkey  
          , RECDT.DocKey  
          , COPY.Copies  
          , RECDT.LineGrouping  
          , RECDT.Style  
   ...
   ORDER BY SortReceiptkey, SeqERK, SeqRK, DocKey, Copies, Line_No  
