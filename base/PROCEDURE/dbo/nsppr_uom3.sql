...
/**************************************************************************/    
/* Store Procedure:  nspPR_UOM3                                           */    
/* Creation Date:                                                         */    
/* Copyright: IDS                                                         */    
/* Written by:                                                            */    
/*                                                                        */    
/* Purpose:  Pre-Allocation Strategy of IDSCN - ECOM                      */    
/*                                                                        */    
...
/* 18-Mar-2015  Shong     SOS336160 Strategy special design for ECOM      */    
/*                        China, Single Pcs Order for Conso Case Pick.    */    
/* 11-May-2015  NJOW01    Fix single item order filtering                 */
/* 18-Sep-2015  NJOW02    353061 - add lottable06-15                      */
/* 17-Jul-2018  TLTING01 1.6  Dynamic SQL - cache issue                   */  
/**************************************************************************/    
    
CREATE PROCEDURE [dbo].[nspPR_UOM3]   
...
BEGIN  
    ...
    SET @n_CaseCnt = 0 
    SELECT @n_CaseCnt  = ISNULL(p.CaseCnt,0)   
   ...
   BEGIN
      IF EXISTS(SELECT 1 
                ...
                AND   sc.ConfigKey = 'LoadConsoAllocationOParms'
                AND   sc.Facility = CASE WHEN ISNULL(RTRIM(sc.Facility),'') = '' THEN sc.Facility ELSE @c_Facility END  
                AND   sc.SValue = '1')
      BEGIN
         SELECT @c_LoadKey = LEFT(ISNULL(LTRIM(@c_OtherParms), ''), 10) 

         SET @n_MultiOpenQty = 0     
   ...
   END
    ...
    IF @b_debug=1  
    BEGIN   
        SELECT 'nspPR_UOM3 : Before Lot Lookup .....'      
        SELECT '@c_lot' = @c_lot  
    ...
    IF @b_debug=1  
    BEGIN  
        SELECT 'nspPR_UOM3 : After Lot Lookup .....'      
        SELECT '@c_lot' = @c_lot  
              ,'@c_Lottable01' = @c_Lottable01  
              ,'@c_Lottable02' = @c_Lottable02  
              ,'@c_Lottable03' = @c_Lottable03  
    ...
    BEGIN  
     ...
     SELECT @c_SQL =   
           ...
           END '   
              ...
              +                 ' AND    P.PreAllocatePickCode = ''nspPR_UOM3'''  
