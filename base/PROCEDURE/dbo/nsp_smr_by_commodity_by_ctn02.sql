...
/************************************************************************/
/* Stored Procedure: nsp_smr_by_Commodity_by_Ctn02                      */
/* Creation Date:                                                       */
/* Copyright: IDS                                                       */
/* Written by:                                                          */
/*                                                                      */
...
/*                                                                      */
/* Called By: IDSRPVU2.PBL - r_smr_by_commodity_by_ctn02	               */
/*                                                                      */
/* PVCS Version: 1.1                                                    */
/*                                                                      */
/* Version: 5.4                                                         */
...
/* 29.JUN.06	 June  			Include ArchiveDB pass-in parameter &	   */
...
/*****************************************************************/
CREATE PROC [dbo].[nsp_smr_by_Commodity_by_Ctn02] (
     ...
     @DateStringMax  NVARCHAR(10),
	  @cArchiveDB	 NVARCHAR(30)  -- Added by June 29.June.06
) AS
BEGIN
   SET NOCOUNT ON 
   SET QUOTED_IDENTIFIER OFF 
...
-- Start : SOS35104, Remarked by June 06.May.2005     
/*
...
-- Start : SOS35104, Changed by June 06.May.2005     
/*
...
-- 	AND a.TranType IN ('DP', 'WD', 'AJ')
	...
	SELECT @c_sql = 'INSERT INTO #ITRN_CUT_BY_SKU_ITT '
					   ...
					   + ', 0 as picked '
						+ 'FROM itrn a(nolock), sku b(nolock) '
						+ 'WHERE  a.StorerKey = N''' + dbo.fnc_RTrim(@IN_StorerKey) + ''' '
						+ 'AND a.storerkey = b.storerkey  ' --by steo, to eliminate 2 storers having 2 same skus, 12:20, 13-OCT-2000
						+ 'AND a.sku = b.sku '
						+ 'AND b.itemclass BETWEEN N''' + dbo.fnc_RTrim(@ItemClassMin) + ''' AND N''' + dbo.fnc_RTrim(@ItemClassMax) + ''' '
						+ 'AND a.Sku BETWEEN N''' + dbo.fnc_RTrim(@SkuMin) + ''' AND N''' + dbo.fnc_RTrim(@SkuMax) + ''' '
						+ 'AND a.Lot BETWEEN N''' + dbo.fnc_RTrim(@LotMin) + ''' AND N''' + dbo.fnc_RTrim(@LotMax) + ''' '
						+ 'AND a.AddDate < N''' + dbo.fnc_RTrim(@d_end_date) + ''' '
						+ 'AND a.TranType IN (''DP'', ''WD'', ''AJ'') '
						+ 'UNION ' 
						+ 'SELECT Storerkey = UPPER(a.StorerKey) '
					   + ', b.itemclass '
					   ...
					   + '       WHEN a.SourceType IN (''ntrReceiptDetailUpdate'',''ntrReceiptDetailAdd'') '
					   + '            THEN (SELECT RECEIPT.ExternReceiptKey '
					   + '                    FROM ' + dbo.fnc_RTrim(@cArchiveDB) + '..RECEIPT RECEIPT (NOLOCK) WHERE RECEIPT.ReceiptKey = SUBSTRING(a.SourceKey,1,10)) '
						+ '/* Orders */ '
					   + '       WHEN a.SourceType = ''ntrPickDetailUpdate'' ' 
					   + '            THEN (select ORDERS.ExternOrderKey '
					   + '                  from ' + dbo.fnc_RTrim(@cArchiveDB) + '..ORDERS ORDERS (NOLOCK) WHERE orderkey = (select orderkey from ' + dbo.fnc_RTrim(@cArchiveDB) + '..pickdetail PICKDETAIL (NOLOCK) where pickdetailkey = SUBSTRING(a.SourceKey,1,10))) '
						+ '/* Transfer */ '
					   + '       WHEN a.SourceType = ''ntrTransferDetailUpdate'' '
					   + '            THEN (SELECT CustomerRefNo '
					   + '                    FROM ' + dbo.fnc_RTrim(@cArchiveDB) + '..TRANSFER TRANSFER (NOLOCK) WHERE TRANSFER.TransferKey = SUBSTRING(a.SourceKey,1,10)) '
						+ '/* Adjustment */ '
					   + '		 WHEN a.SourceType = ''ntrAdjustmentDetailAdd'' ' 
					   + '            THEN (SELECT CustomerRefNo '
					   + '                    FROM ' + dbo.fnc_RTrim(@cArchiveDB) + '..ADJUSTMENT ADJUSTMENT (NOLOCK) WHERE ADJUSTMENT.AdjustmentKey = SUBSTRING(a.SourceKey,1,10)) '
					   + '       ELSE '' '' '    
					   + '       END, ''-'') '				
					   + ', BuyerPO = isnull(CASE '
					   + '       WHEN a.SourceType = ''ntrPickDetailUpdate'' '
					   ...
					   + ', 0 as distinct_sku '
					   + ', 0 as picked '
						+ 'FROM ' + dbo.fnc_RTrim(@cArchiveDB) + '..itrn a(nolock), sku b(nolock) '
						+ 'WHERE  a.StorerKey = N''' + dbo.fnc_RTrim(@IN_StorerKey) + ''' '
						+ 'AND a.storerkey = b.storerkey  ' --by steo, to eliminate 2 storers having 2 same skus, 12:20, 13-OCT-2000
						+ 'AND a.sku = b.sku '
						+ 'AND b.itemclass BETWEEN N''' + dbo.fnc_RTrim(@ItemClassMin) + ''' AND N''' + dbo.fnc_RTrim(@ItemClassMax) + ''' '
	...
	/*   
			AND 1 in ( (SELECT 1 FROM RECEIPT (NOLOCK) 
							WHERE RECEIPT.ReceiptKey = SUBSTRING(a.SourceKey,1,10) 
									AND (a.sourcetype = 'ntrReceiptDetailUpdate' or a.sourcetype = 'ntrReceiptDetailAdd')),
...
--      AND a.status = '5'  -- all the picked records
	...
	SELECT @c_sql = 'INSERT INTO #ITRN_CUT_BY_SKU_ITT '
   					...
   					+ 'SELECT Storerkey = a.storerkey, '
						...
						+ '0, '
						+ '1 ' -- 1 means picked record
						+ 'FROM ' + dbo.fnc_RTrim(@cArchiveDB) + '..pickdetail a (nolock), '
						+ '     sku b(nolock), '
						--orderdetail c(nolock), 
						+ ' ' + dbo.fnc_RTrim(@cArchiveDB) + '..orders d(nolock) '
						+ 'WHERE a.StorerKey = N''' + dbo.fnc_RTrim(@IN_StorerKey) + ''' '
						+ 'AND a.storerkey = b.storerkey '
						+ 'AND a.sku = b.sku '
						--AND a.sku = c.sku      -- steo, admended as of 26/sep/2000 due to -250 qty appear in the report, reported by kenny
...
BEGIN
    ...
    BEGIN
         ...
         SELECT  Storerkey = UPPER(a.StorerKey)
               ...
               , ExternNum = '                              '
               , BuyerPO = '                    '
               , convert(datetime,convert(char(11), a.ArchiveDate,106))  
               , picked = 0
...
*/
   ...
   /*
	...
	BEGIN
		...
		SELECT @c_sql = 'INSERT INTO #ITRN_CUT_BY_SKU '
							...
							+ ', ExternNum = SPACE(30) '
							+ ', BuyerPO = SPACE(20) '
							+ ', adddate = MAX(N''' + dbo.fnc_RTrim(@BFDate) + ''') '
							+ ', 0 as picked '
							+ 'FROM Archive..itrn a(nolock), sku b(nolock) '
							+ 'WHERE a.StorerKey = N''' + dbo.fnc_RTrim(@IN_StorerKey) + ''' '
							+ 'AND a.storerkey = b.storerkey  '
							+ 'AND a.sku = b.sku '
							+ 'AND b.itemclass BETWEEN N''' + dbo.fnc_RTrim(@ItemClassMin) + ''' AND N''' + dbo.fnc_RTrim(@ItemClassMax) + ''' '
							+ 'AND a.Sku BETWEEN N''' + dbo.fnc_RTrim(@SkuMin) + ''' AND N''' + dbo.fnc_RTrim(@SkuMax) + ''' '
							+ 'AND a.Lot BETWEEN N''' + dbo.fnc_RTrim(@LotMin) + ''' AND N''' + dbo.fnc_RTrim(@LotMax) + ''' '
							+ 'AND a.AddDate <= N''' + dbo.fnc_RTrim(@BFdate) + ''' '
							+ 'AND a.TranType IN (''DP'', ''WD'', ''AJ'') '
    ...
    /* BB means those unique lot records which fall in between begin_date and end_date
       ...
       BEGIN
           select StorerKey = UPPER(a.storerkey),
		      ...
		      into #BF_TEMP3a
            from #bf_temp3 a
            WHERE not exists	(SELECT * from #BF b
  ...
  BEGIN
    SELECT
        ...
        , AddDate = 
               CASE 
                 WHEN #BF2.AddDate < @d_begin_date THEN null
                 ELSE #BF2.AddDate
               END
        , #BF2.Flag
        , TranType =
  ...
  BEGIN
    SELECT
        ...
        , #BF2.Sku
        , #BF2.Qty
        , EffDate = convert(char(10),#BF2.AddDate,106)
        , #BF2.Flag
        , #BF2.TranType
        , #BF2.ExceedNum
        , #BF2.ExternNum
