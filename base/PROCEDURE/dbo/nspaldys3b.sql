...
BEGIN    
   ...
   SET @c_SQL = N'   
      ...
      AND LOC.LocationCategory IN (''VNA'',''Shuttle'') ' + CHAR(13) + CASE WHEN @c_IsValid = 'Y' THEN @c_Conditions ELSE '' END + CHAR(13) +
      CASE WHEN ISNULL(@n_PackPallet,0) > 0 THEN 'AND (LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED - LOTxLOCxID.QtyReplen) = @n_PackPallet ' ELSE '' END +
