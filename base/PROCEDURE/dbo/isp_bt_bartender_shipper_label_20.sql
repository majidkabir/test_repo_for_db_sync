...
/*            isp_BT_Bartender_Shipper_Label_11                               */                             
/* Modifications log:                                                         */                 
/*                                                                            */                 
/* Date       Rev  Author     Purposes                                        */ 
/* 2021-06-02 1.0  WLChooi    Created (WMS-17187)                             */   
/******************************************************************************/                
                  
CREATE PROC [dbo].[isp_BT_Bartender_Shipper_Label_20]                       
...
BEGIN                      
  ...
  DECLARE @d_Trace_StartTime   DATETIME,   
   ...
   SET @c_GrpBy = N'GROUP BY ORD.loadkey,ORD.orderkey,ORD.externorderkey,ORD.type,buyerpo,salesman,ORD.facility,STO.Secondary, ' +CHAR(13)
                ...
                +' ORD.Userdefine02,ORD.trackingno,ORD.Userdefine05,' +CHAR(13) 
                +' ORD.InvoiceAmount,ORD.ShipperKey,' +CHAR(13)
                +' STO.B_Company,(STO.B_Address1+STO.B_Address2+STO.B_Address3),STO.B_Contact1,ORD.DeliveryPlace,' +CHAR(13)
                +' ORD.M_Address1,ORD.M_Address2,ORD.M_City,Substring(ORD.Notes,1,80),ORD.Userdefine10,PD.CartonNo,' +CHAR(13)
                +' PD.DropID, PH.TTLCNTS'
   ...
   SET @c_SQLJOIN = +'SELECT ORD.loadkey,ORD.orderkey,ORD.externorderkey,ORD.type,buyerpo,salesman,ORD.facility,STO.Secondary,'    --8        
                  ...
                  +'PD.DropID,ORD.trackingno,ORD.Userdefine05,'        
                  + CHAR(13) +   
                  +'PH.TTLCNTS,'     
                  +'ORD.InvoiceAmount,'''','''','           
                  +'ORD.ShipperKey,STO.B_Company,(STO.B_Address1+STO.B_Address2+STO.B_Address3),STO.B_Contact1,'''',ORD.DeliveryPlace,'''', '  --50       
  ...
  SET @c_SQL='INSERT INTO #Result (Col01,Col02,Col03,Col04,Col05, Col06,Col07,Col08,Col09'  + CHAR(13) +           
   ...
   BEGIN       
      ...
      SELECT @c_OINotes = ISNULL(OI.Notes,'')
      FROM ORDERINFO OI (NOLOCK)
      WHERE Orderkey = @c_OrderKey

      IF ISNULL(@c_OINotes,'') <> ''
      BEGIN
         SELECT SeqNo, ISNULL(ColValue,'') AS ColValue
         INTO #TMP_DSplit
         FROM dbo.fnc_DelimSplit(',', @c_OINotes)
         ORDER BY SeqNo ASC

         SELECT @c_Col48 = MAX(CASE WHEN SeqNo = 1 THEN ColValue ELSE '' END)
              , @c_Col49 = MAX(CASE WHEN SeqNo = 2 THEN ColValue ELSE '' END)
              , @c_Col50 = MAX(CASE WHEN SeqNo = 3 THEN ColValue ELSE '' END)
              , @c_Col51 = MAX(CASE WHEN SeqNo = 4 THEN ColValue ELSE '' END)
              , @c_Col55 = MAX(CASE WHEN SeqNo = 5 THEN ColValue ELSE '' END)
              , @c_Col56 = MAX(CASE WHEN SeqNo = 6 THEN ColValue ELSE '' END)
              , @c_Col35 = MAX(CASE WHEN SeqNo = 7 THEN ColValue ELSE '' END)
         FROM #TMP_DSplit

         IF @b_debug = '2'
         BEGIN
            SELECT @c_Col48 AS Col48
                 , @c_Col49 AS Col49
                 , @c_Col50 AS Col50
                 , @c_Col51 AS Col51
                 , @c_Col55 AS Col55
         ...
         IF OBJECT_ID('tempdb..#TMP_DSplit') IS NOT NULL
            DROP TABLE #TMP_DSplit
      END
...
EXIT_SP:    
   ...
   EXEC isp_InsertTraceInfo   
      ...
      @c_TraceName = 'isp_BT_Bartender_Shipper_Label_20',  
