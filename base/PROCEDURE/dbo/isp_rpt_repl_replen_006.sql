...
/***************************************************************************/
/* Stored Procedure: isp_RPT_REPL_REPLEN_006                               */
/* Creation Date: 09-Dec-2021                                              */
/* Copyright: LFL                                                          */
/* Written by: Harshitha                                                   */
/*                                                                         */
/* Purpose: WMS- 18553 - Migrate WMS report to Logi Report.                */
/*         : modify from isp_ReplenishmentRpt_PC25                         */
/*                                                                         */
/* Called By: RPT_REPL_REPLEN_006                                          */
/*                                                                         */
/* GitLab Version: 1.0                                                     */
/*                                                                         */
/* Version: 1.0                                                            */
...
/***************************************************************************/
CREATE PROC [dbo].[isp_RPT_REPL_REPLEN_006]
...
BEGIN
   ...
   BEGIN
      ...
      OverAllocation = CASE WHEN SUM(ISNULL(LOTxLOCxID.QtyExpected - LOTXLOCXID.PendingMoveIn, 0)) > 0
                              OR SKUxLOC.QtyLocationMinimum < (SKUxLOC.Qty - (SKUxLOC.QtyPicked + SKUxLOC.QtyAllocated)) + SUM(ISNULL(LOTXLOCXID.PendingMoveIn,0)) THEN
                                 'Y'
                            ELSE 'N' END,
      SUM(ISNULL(LOTxLOCxID.QtyExpected - LOTXLOCXID.PendingMoveIn, 0))
      From SKUxLOC (NOLOCK)
      ...
      BEGIN
       ...
       /*' ORDER BY ' +  CASE WHEN ISNULL(@c_LottableName,'') = '' THEN
         ...
         */
         ...
         ORDER BY Priority, LOTTABLE04, LOTTABLE05
         */
