...
BEGIN   
   ...
   DECLARE @c_faciflag NVARCHAR(1),
           @c_looseflag NVARCHAR(1),
           @c_sku NVARCHAR(20),
           @c_AllowULMSKULoose NVARCHAR(10)
    
   SELECT @c_AllowULMSKULoose = ISNULL(SCF.svalue,''),
          @c_sku = LOT.Sku
   FROM LOT (NOLOCK)
   LEFT JOIN Storerconfig SCF (NOLOCK) ON (LOT.Storerkey = SCF.Storerkey AND SCF.Configkey='AllowULMSKULoose')
   WHERE LOT.Lot = @c_lot
   
   SET @c_looseflag = '0'
   IF @c_AllowULMSKULoose = '1'
   BEGIN
