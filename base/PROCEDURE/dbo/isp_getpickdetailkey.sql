...
/************************************************************************/  
/* Stored Procedure:  isp_GetPICKDETAILKey                              */  
/* Creation Date:                                                       */  
/* Copyright: IDS                                                       */  
/* Written by:                                                          */  
/*                                                                      */  
...
/*                                                                      */  
/* 27-May-2013  TLTING     1.1  Replace GetKey with isp_GetPICKDETAILKey*/
/*                              to reduce blocking                      */
/* 13-May-2015  TLTING     1.2  Change variable BigINT                  */
/************************************************************************/ 
CREATE PROC [dbo].[isp_GetPICKDETAILKey]     
(   @n_FieldLength  INT     
...
BEGIN    
   ...
   DECLARE @n_NewPICKDETAILKey BigINT     
   ...
   INSERT INTO PICKDETAILKey (AddDate) VALUES (GETDATE())    
   SELECT @n_err = @@ERROR, @n_cnt = @@ROWCOUNT, @n_NewPICKDETAILKey = @@IDENTITY    
   IF @n_err <> 0    
   BEGIN    
     SELECT @n_continue = 3     
   END    
   SELECT @n_NewPICKDETAILKey = @@IDENTITY    
  
   IF @n_continue = 1 OR @n_continue = 2    
   BEGIN    
 
      SELECT @c_PICKDETAILKey = RIGHT(Replicate('0',@n_FieldLength) + CAST(@n_NewPICKDETAILKey As VARCHAR(10)), @n_FieldLength)     
   END    
   ELSE    
   ...
   BEGIN    
      ...
      BEGIN    
          ...
          END              
      END    
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'isp_GetPICKDETAILKey'    
