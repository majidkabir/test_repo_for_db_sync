...
/* 2021-01-20  Wan01    1.1   WMS-16026 - PB-Standardize TrackingNo     */
/************************************************************************/
CREATE PROC [dbo].[isp_EPackCtnTrack01]
...
BEGIN
   ...
   --(Wan01) - START  
   SELECT @c_TrackingNo = CASE WHEN ISNULL(PI.TrackingNo,'') <> '' THEN RTRIM(PI.TrackingNo) ELSE ISNULL(RTRIM(PI.RefNo),'') END  --(Wan01)
   ...
   --)
   --(Wan01) - END
   IF @c_CTNTrackNo = @c_TrackingNo                      --(Wan01)
   BEGIN
      GOTO QUIT_SP
   END
   ...
   BEGIN
      SET @n_continue = 3
      SET @n_err = 60010  
      SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Get Empty Tracking #. (isp_EPackCtnTrack01)' 
      GOTO QUIT_SP
   END
   ...
   BEGIN
      ...
      BEGIN
         SET @n_continue = 3
         SET @n_err = 60020  
         SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Update PACKINFO Table. (isp_EPackCtnTrack01)' 
   ...
   BEGIN
      ...
      BEGIN
         SET @n_continue = 3
         SET @n_err = 60030  
         SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Insert PACKINFO Table. (isp_EPackCtnTrack01)' 
                        + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
         GOTO QUIT_SP
      END
   END
   ...
   BEGIN
      ...
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'isp_EPackCtnTrack01'
