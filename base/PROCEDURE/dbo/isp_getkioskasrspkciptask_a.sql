...
/************************************************************************/  
/* Stored Procedure: isp_GetKIOSKASRSPKCIPTask_a                        */  
...
/*                                                                      */  
/* Called By: r_dw_kiosk_asrspk_cip_a_form                              */  
/*                                                                      */  
/* PVCS Version: 1.1                                                    */  
/*                                                                      */  
/* Version: 7.0                                                         */  
...
CREATE PROC [dbo].[isp_GetKIOSKASRSPKCIPTask_a]  
...
,    Remarks             NVARCHAR(80)  NOT NULL DEFAULT ('')      --(Wan01)
   ...
   INSERT INTO #Result (TaskdetailKey,RefTaskKey,FROMID,TOID,PalletID,OrderKey,OrderStatus,OrderLineNumber
                     ...
                     ,PACK.CaseCnt,PackUOM1,PackUOM3,PalletQtyInCS,PalletQtyInEA,RemainingQtyInCS,RemainingQtyInEA
   ...
   ,PalletQtyInCS   = 0--CASE WHEN PACK.CaseCnt > 0 AND @n_ShowUOMEA = 0 THEN FLOOR(@n_Qty / PACK.CaseCnt) ELSE 0 END 
   ,PalletQtyInEA   = 0--CASE WHEN PACK.CaseCnt > 0 AND @n_ShowUOMEA = 0 THEN (@n_Qty % CONVERT(INT,PACK.CaseCnt)) ELSE @n_Qty END
   ,RemainingQtyInCS= 0--CASE WHEN PACK.CaseCnt > 0 AND @n_ShowUOMEA = 0 THEN FLOOR((@n_Qty - @n_QtyPicked) / PACK.CaseCnt) ELSE 0 END 
   ,RemainingQtyInEA= 0--CASE WHEN PACK.CaseCnt > 0 AND @n_ShowUOMEA = 0 THEN (@n_Qty - @n_QtyPicked) % CONVERT(INT, PACK.CaseCnt) ELSE (@n_Qty - @n_QtyPicked) END
   ,TaskStatus      = @c_TaskStatus --TASKDETAIL.Status--ISNULL(PICKDETAIL.Status,'')
   ...
   SELECT @c_JobKey,TaskdetailKey,FROMID,TOID,PalletID,OrderKey,OrderStatus,OrderLineNumber
         ...
         ,Lottable06Code,Lottable07Code,Lottable08Code,Lottable09Code,Lottable10Code
         ,Lottable11Code,Lottable12Code,Lottable13Code,Lottable14Code,Lottable15Code
         ,CaseCnt,PackUOM1,PackUOM3,PalletQtyInCS,PalletQtyInEA,RemainingQtyInCS,RemainingQtyInEA
