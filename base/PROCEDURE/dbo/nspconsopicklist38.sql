...
GO
/************************************************************************/
/* Store Procedure:  nspConsoPickList38                        		    */
/* Creation Date:                                     					*/
/* Copyright: IDS                                                       */
/* Written by:                                           				*/
/*                                                                      */
...
/* Return Status:  None                                                 */
/*                                                                      */
/* Usage:  Used for report dw = r_dw_consolidated_pick38_4       		*/
/*                                                                      */
/* Local Variables:                                                     */
/*                                                                      */
/* Called By:                                       					*/
...
CREATE PROC [dbo].[nspConsoPickList38] (
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			--SELECT @c_PickHeaderKey AS '@c_PickHeaderKey'
			--SELECT TOP 10 * FROM PICKHEADER (NOLOCK) 
	...
	BEGIN
	  SELECT LoadPlanDetail.LoadKey,   
			...
			PICKDETAIL.Loc,   
			PICKDETAIL.Sku,   
			UOMqty = PICKDETAIL.UOMQty,   
			SKU.DESCR,   
			--ODUOM = ORDERDETAIL.UOM,              --(CS01)
			ODUOM = CASE WHEN PICKDETAIL.UOM = '6' THEN pack.PackUOM3 ELSE 
						CASE WHEN  PICKDETAIL.UOM = '3' THEN pack.PackUOM2 ELSE
						CASE WHEN PICKDETAIL.UOM = '2' THEN pack.PackUOM1 ELSE '' END END END  ,                 --(CS01)
	  ...
	  ORDER BY CASE WHEN ISNULL(loc.LogicalLocation,'') = '' THEN 0 ELSE 1 END,PICKDETAIL.Loc,PICKDETAIL.Sku
	END
