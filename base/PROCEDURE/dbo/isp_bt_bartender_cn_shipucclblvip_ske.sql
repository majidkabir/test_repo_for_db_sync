...
/********************************************************************************/                     
/* Copyright: LFL                                                               */                     
/* Purpose: isp_BT_Bartender_CN_SHIPUCCLBLVIP_SKE                               */                     
/*                                                                              */                     
/* Modifications log:                                                           */                     
/*                                                                              */                     
...
CREATE PROC [dbo].[isp_BT_Bartender_CN_SHIPUCCLBLVIP_SKE]                          
...
--PRINT @c_SQLJOIN    
   ...
   BEGIN  
      ...
      BEGIN    
         ...
         /*  
         SELECT @n_FirstPos = CHARINDEX('*',@c_SKU)  
         SELECT @n_SecondPos = CHARINDEX('*',@c_SKU, @n_FirstPos + 1)  
         SELECT @n_TotalLength = LEN(LTRIM(RTRIM(@c_SKU)))  
  
         IF @n_FirstPos > 0 AND @n_SecondPos > 0  
         BEGIN  
            SELECT @c_SKU = SUBSTRING(@c_SKU, @n_FirstPos + 1, @n_SecondPos - @n_FirstPos - 1)  
          END  
  
         IF @b_debug = 1    
            SELECT @n_FirstPos, @n_SecondPos, @c_Col15, @c_Col16, @n_TotalLength, @c_SKU*/  
