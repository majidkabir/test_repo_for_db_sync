...
/*************************************************************************/
/* Stored Procedure: isp_Packing_List_60                                 */
/* Creation Date: 26-Feb-2019                                            */
/* Copyright: LFL                                                        */
/* Written by:                                                           */
/*                                                                       */
/* Purpose: Packing List for Levi's                                      */    
/*          WMS-8102 - [RG] Levis B2B Phase 2 - Packing List RCM Report  */
/*          DW copied from r_hk_packing_list_01                          */
/*          SP copied from isp_r_hk_packing_list_01                      */
/*                                                                       */
/* Called By: RCM. Datawindow r_dw_packing_list_60                       */
/*                                                                       */
/*                                                                       */
/* PVCS Version: 1.0                                                     */
/*                                                                       */
...
/* 2020-02-26   WLChooi  1.1  WMS-12277 - Enable print from MBOL (WL01)  */
/*************************************************************************/

CREATE PROCEDURE [dbo].[isp_Packing_List_60] (
     --  @as_storerkey       NVARCHAR(15)
     --, @as_wavekey         NVARCHAR(10)
     --, @as_loadkey         NVARCHAR(10)
     --, 
     @as_pickslipno      NVARCHAR(4000), @as_Type   NVARCHAR(10) = ''   --WL01
...
BEGIN
   ...
   IF EXISTS (SELECT 1 FROM MBOL (NOLOCK) WHERE MBOLKEY = @as_pickslipno)
   BEGIN
      SELECT @as_pickslipno = CAST(STUFF((SELECT ',' + RTRIM(PH.Pickslipno) 
                                   FROM MBOL M (NOLOCK)
                                   JOIN ORDERS OH (NOLOCK) ON M.MBOLKEY = OH.MBOLKEY
                                   JOIN PACKHEADER PH (NOLOCK) ON PH.ORDERKEY = OH.ORDERKEY
                                   WHERE M.MBOLKEY = @as_pickslipno
                                   ORDER BY PH.Pickslipno FOR XML PATH('')),1,1,'' ) AS NVARCHAR(4000))
   ...
   DECLARE @n_PickslipNoCnt      INT

   SET @n_PickslipNoCnt = @@ROWCOUNT

   IF @as_Type = 'MAIN'
   BEGIN
      SELECT DISTINCT PACKHEADER.PICKSLIPNO AS Pickslipno, 
                      ISNULL(CODELKUP.SHORT,'N') AS Short
      FROM PACKHEADER WITH (NOLOCK)
      LEFT JOIN CODELKUP (NOLOCK) ON CODELKUP.LISTNAME = 'REPORTCFG' AND CODELKUP.CODE = 'ShowPrice' 
                                 AND CODELKUP.SHORT = 'Y' AND CODELKUP.LONG = 'r_dw_packing_list_60' 
                                 AND CODELKUP.STORERKEY = PACKHEADER.STORERKEY
      JOIN #TEMP_PICKSLIPNO t ON (t.ColValue = PACKHEADER.Pickslipno)
      --WHERE PACKHEADER.PICKSLIPNO = :as_Pickslipno
   ...
   BEGIN
      SELECT  PH.Pickslipno
            , OD.SKU as SKU
            , SUM( (OD.QtyAllocated + OD.QtyPicked + OD.ShippedQTY) * OD.UNITPRICE) AS MSRP_Price
            , MIN(OD.USERDEFINE01) AS Curr
            , SUM( (OD.QtyAllocated + OD.QtyPicked + OD.ShippedQTY) * OD.ExtendedPrice) AS WSP_Price
      FROM PACKHEADER PH WITH (NOLOCK) 
      JOIN ORDERDETAIL OD WITH (NOLOCK) ON (OD.STORERKEY = PH.STORERKEY) AND  (OD.ORDERKEY = PH.ORDERKEY)
      JOIN #TEMP_PICKSLIPNO t ON (t.ColValue = PH.Pickslipno)
      --WHERE PH.PICKSLIPNO = :as_pickslipno
      GROUP BY PH.Pickslipno, OD.SKU
      ORDER BY PH.Pickslipno, OD.SKU ASC
...
*/
   ...
   SELECT @c_DataWidnow = 'r_dw_Packing_List_60'
   ...
   LEFT JOIN CODELKUP CLK (NOLOCK) ON CLK.LISTNAME = 'REPORTCFG' AND CLK.STORERKEY = PAKDT.Storerkey AND CLK.LONG = 'r_dw_packing_list_60' AND CLK.CODE = 'ShowPrice'
