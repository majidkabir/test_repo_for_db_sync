...
/******************************************************************************/                       
/* Copyright: IDS                                                             */                       
/* Purpose: isp_BT_Bartender_KR_shipLabel_CJKE8                               */  
/*          duplicate from isp_BT_Bartender_KR_shipLabel_CJKE                 */                     
/*                                                                            */                       
/* Modifications log:                                                         */                       
/*                                                                            */                       
...
BEGIN                                    
  ...
  DECLARE          
   ...
   IF @c_StorerKey IN ('IIC')  
   BEGIN    
     
   SELECT @n_TTLSKUQTY = SUM(QTY) FROM PACKDETAIL (NOLOCK) WHERE PICKSLIPNO = @c_Sparm1 AND LABELNO = @c_Sparm2  
   UPDATE #Result SET Col58 = @n_TTLSKUQTY  
  
    
   IF @c_StorerKey NOT IN ('NIKEKR', 'AOS', 'COS','IIC')  
   BEGIN    
   ...
   BEGIN      
      ...
      SELECT @c_Pickslipno, @c_LabelNo, PD.LabelLine, PD.SKU, S.DESCR, SUM(PD.Qty), 'N'      
   ...
   EXEC isp_InsertTraceInfo                 
      ...
      @c_TraceName = 'isp_BT_Bartender_KR_shipLabel_CJKE8',                
