...
/* Written by:                                                          */
/*                                                                      */
/* Purpose: WMS-6035 - [JP] CR DW Pick Slip Data Window_PB              */
/*          Notes: Duplicate from nsp_GetPickSlipOrders04a and modified */
/*                                                                      */
...
CREATE PROC [dbo].[nsp_GetPickSlipOrders04e] (@c_loadkey NVARCHAR(10))
AS
BEGIN
   SET NOCOUNT ON 
   SET QUOTED_IDENTIFIER OFF 
  ...
  DECLARE @c_getorderkey     NVARCHAR(10),
   ...
   INNER JOIN FACILITY (nolock) 
	...
	GROUP BY 
      ...
      LOC.LogicalLocation -- SOS52808. All pick list must sort by LogicalLocation
      -- process PICKSLIPNO
		,ISNULL(ORDERDETAIL.notes,'')
		,CASE WHEN ISNULL(PST.PackInst,'') <> ''THEN PST.PackInst ELSE ORDERDETAIL.Userdefine03 END                 --CS01
