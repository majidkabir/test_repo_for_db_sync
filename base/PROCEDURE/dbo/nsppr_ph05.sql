...
GO
/************************************************************************/
/* Stored Procedure: nspPR_PH05                                         */
/* Creation Date: 23-Jan-2017                                           */
/* Copyright: LFL                                                       */
/* Written by:                                                          */
/*                                                                      */
/* Purpose: WMS-978 PH GCI PreAllocation Strategy - PICK(LOOSE)         */
/*                                                                      */
...
BEGIN 
   ...
   BEGIN    
      ...
      SELECT @c_LimitString =  dbo.fnc_RTrim(@c_LimitString) + ' AND Loc.LocationType IN (''PICK'') '
      ...
      '               AND    P.SKU = @c_Sku ' +
      '               AND    P.Qty > 0 ' +
      '               AND    P.PreAllocatePickCode = ''nspPR_PH05'' ' +
      '               GROUP BY P.LOT, ORDERS.Facility) P ON LOTXLOCXID.LOT = P.LOT AND P.Facility = LOC.Facility ' +
