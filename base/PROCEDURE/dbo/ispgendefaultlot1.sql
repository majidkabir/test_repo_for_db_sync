...
CREATE PROCEDURE [dbo].[ispGenDefaultLot1]  
...
--   AND Short = @c_Storerkey  
   ...
   BEGIN  
      -- (ChewKP01)  
      --IF ISNULL(@c_Lottable01Value,'') = '' --SOS241329  
      BEGIN  
         SELECT @c_Lottable01 = Long From dbo.CodeLKUP (NOLOCK)  
         WHERE LISTNAME = 'DLOTTABLE1'  
         AND CODE = @c_Storerkey  
         AND Short = 'Lottable01'  
      END  
   END  
   ELSE  
   BEGIN    
