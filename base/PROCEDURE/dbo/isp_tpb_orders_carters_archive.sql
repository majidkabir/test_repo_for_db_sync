...
CREATE PROC [dbo].[isp_TPB_Orders_Carters_Archive]
...
BEGIN
   ...
   SET @c_SQLStatement =   @c_SQLStatement +
    ...
    'JOIN dbo.SKU S WITH (NOLOCK) ON S.StorerKey = OD.StorerKey AND  S.SKU = OD.SKU ' +
    'LEFT JOIN dbo.FACILITY F WITH (NOLOCK) ON F.Facility = ORDERS.Facility ' +
    'LEFT JOIN dbo.PACK P WITH (NOLOCK) ON P.PACKKey = OD.PACKKey ' +
...
INSERT INTO [DBO].[WMS_TPB_BASE](
 ...
 EXEC sp_ExecuteSQL @c_SQLStatement, @c_SQLParm, @n_WMS_BatchNo, @n_TPB_Key, @c_COUNTRYISO, @d_Fromdate, @d_Todate
   ...
   SET @c_SQLStatement = @c_SQLStatement +
            N'FROM ' + @c_ArchiveDB + '.dbo.V_ORDERS ORDERS WITH (NOLOCK) ' +
          'JOIN ' + @c_ArchiveDB + '.dbo.V_ORDERDETAIL OD WITH (NOLOCK) ON ORDERS.Orderkey = OD.Orderkey ' +
          'JOIN ' + @c_ArchiveDB + '.dbo.V_PICKDETAIL PD WITH (NOLOCK) ON PD.Orderkey = OD.Orderkey AND PD.OrderlineNumber = OD.OrderlineNumber ' +
          'JOIN ' + @c_ArchiveDB + '.dbo.V_MBOLDETAIL MD WITH (NOLOCK) ON MD.OrderKey = ORDERS.OrderKey ' +
          'JOIN ' + @c_ArchiveDB + '.dbo.V_MBOL M WITH (NOLOCK) ON  M.MbolKey = MD.MbolKey ' +
          'JOIN dbo.SKU S WITH (NOLOCK) ON S.StorerKey = OD.StorerKey AND  S.SKU = OD.SKU ' +
          'LEFT JOIN dbo.FACILITY F WITH (NOLOCK) ON F.Facility = ORDERS.Facility ' +
          'JOIN dbo.PACK P WITH (NOLOCK) ON P.PACKKey = PD.PACKKey ' +
          'JOIN dbo.LOTATTRIBUTE L WITH (NOLOCK) ON L.Lot = PD.Lot ' +
          'JOIN dbo.LOC WITH (NOLOCK) ON LOC.LOC = PD.Loc ' +
          'WHERE ORDERS.Status = ''9''  ' +
          'AND CONVERT(VARCHAR, M.EditDate, 112) = CONVERT(VARCHAR, @d_Fromdate, 112) ' +
