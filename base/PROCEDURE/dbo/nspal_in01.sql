...
GO
/************************************************************************/
/* Stored Procedure: nspAL_IN01                                         */
/* Creation Date: 21-MAY-2021                                           */
/* Copyright: LFL                                                       */
/* Written by:                                                          */
/*                                                                      */
/* Purpose: WMS-17085 - IN THGUNI Allocation                            */
/*                                                                      */
/* Called By:                                                           */
/*                                                                      */
/* PVCS Version: 1.0                                                    */
...
BEGIN    
   ...
   BEGIN   	    
      ...
      SELECT @c_OrderGroup = ISNULL(OrderGroup,'') 
      FROM ORDERS (NOLOCK)
      WHERE Orderkey = @c_Key1
   END
   
   SELECT TOP 1 @c_FilterHWCode = ISNULL(Short,'')
   FROM CODELKUP (NOLOCK)
   WHERE Storerkey = @c_Storerkey
   AND Long = 'nspAL_IN01'
   AND Listname = 'PKCODECFG'
   AND Code = 'FILTERHWCODE'
   AND UDF01 = 'OrderGroup'
      
   IF @c_UOM  = '1' 
   	  SET @c_SortBy = ' ORDER BY LA.Lottable04, LA.Lot, CASE WHEN LOC.LocationType = ''PICK'' THEN 2 ELSE 1 END, LOC.LogicalLocation, LOC.LOC '
   ELSE 
   	  SET @c_SortBy = ' ORDER BY LA.Lottable04, LA.Lot, CASE WHEN LOC.LocationType = ''PICK'' THEN 1 ELSE 2 END, LOC.LogicalLocation, LOC.LOC '   
   ...
   SET @c_SQL = N'      
      ...
      CASE WHEN @c_FilterHWCode = 'Y' THEN ' AND LOC.HostWHCode = @c_OrderGroup ' ELSE ' ' END +
   ...
   SET @c_SQLParm =  N'@c_Facility   NVARCHAR(5),  @c_StorerKey  NVARCHAR(15), @c_SKU NVARCHAR(20), @n_QtyLeftToFulfill INT, ' +        
                      ...
                      '@c_Lottable12 NVARCHAR(30), @n_UOMBase INT, @c_OrderGroup NVARCHAR(20) '  
   ...
   EXEC sp_ExecuteSQL @c_SQL, @c_SQLParm, @c_Facility, @c_StorerKey, @c_SKU, @n_QtyLeftToFulfill, @c_Lottable01, @c_Lottable02, @c_Lottable03,  
                      @c_Lottable06, @c_Lottable07, @c_Lottable08,@c_Lottable09, @c_Lottable10, @c_Lottable11, @c_Lottable12, @n_UOMBase, @c_OrderGroup
