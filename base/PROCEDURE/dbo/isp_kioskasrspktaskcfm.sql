...
/************************************************************************/
/* Stored Procedure: isp_KioskASRSPKTaskCfm                             */
...
/*          : u_kiosk_asrspk_c.cb_confirmpick.click event               */
/* PVCS Version: 1.1                                                    */
/*                                                                      */
/* Version: 7.0                                                         */
/*                                                                      */
...
/************************************************************************/
CREATE PROC [dbo].[isp_KioskASRSPKTaskCfm] 
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         SET @c_Status = CASE WHEN @c_FullPallet = 'Y' THEN '0' ELSE '4' END 

         --Unallocated and Reallocated
         ...
         BEGIN  
            ...
            SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Failed on Table PICKDETIAL. (isp_KioskASRSPKTaskCfm)' 
            GOTO QUIT_SP
         END 
         ...
         BEGIN    
            ...
            SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Get PickDetailKey Failed. (isp_KioskASRSPKTaskCfm)' 
            GOTO QUIT_SP  
         END  
         ...
         SELECT @c_NewPickdetailKey
            ...
            , CASE WHEN @c_FullPallet = 'Y' THEN 'FULLPALLET' ELSE 'SHORT' END 
         FROM PICKDETAIL WITH (NOLOCK)
         WHERE PickDetailkey = @c_PickDetailKey
         ...
         BEGIN  
            ...
            SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert Failed into Table PCIKDETAIL. (isp_KioskASRSPKTaskCfm)' 
            GOTO QUIT_SP
         END 

         -- Confirm Pick New Pickdetailkey
         SET @c_Status= '5'
      ...
      BEGIN    
         ...
         SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Get MoveRefKey Failed. (isp_KioskASRSPKTaskCfm)' 
         GOTO QUIT_SP  
      END 
      ...
      EXEC dbo.nspItrnAddMove
         ...
         ,  @f_otherunit2     = 0.00  
         ,  @c_SourceKey      = ''
         ,  @c_SourceType     = 'isp_KioskASRSPKTaskCfm'
         ,  @c_PackKey        = @c_Packkey
         ,  @c_UOM            = @c_UOM
         ,  @b_UOMCalc        = 0
      ...
      BEGIN
         ...
         SET @n_Err = 61030    
         SET @c_ErrMsg='NSQL'+CONVERT(NVARCHAR(5),@n_Err)+': Error Executing Move To ToID - nspItrnAddMove (isp_KioskASRSPKTaskCfm)'
   ...
   BEGIN
      ...
      BEGIN  
         ...
         SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Failed on Table ID. (isp_KioskASRSPKTaskCfm)' 
         GOTO QUIT_SP
      END 
   END
   ...
   IF NOT EXISTS( SELECT 1
                  ...
                  AND   Qty > 0      -- Unallocate without delete pickdetail
                  )  
   BEGIN
      SET @c_TaskStatus = '9'
      ...
      BEGIN  
         ...
         SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update TASKDETAIL Failed. (isp_KioskASRSPKTaskCfm)' 
         GOTO QUIT_SP
      END 
...
IF @n_Continue=3  -- Error Occured - Process And Return
   BEGIN
      ...
      BEGIN
         ...
         END
      END
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'isp_KioskASRSPKTaskCfm'
