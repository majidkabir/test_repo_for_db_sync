...
/************************************************************************/
/* SP: ispPopulateTOASN_TWQKS_Vas                                       */
/* Creation Date: 05-Jul-2018                                           */
/* Copyright: LF                                                        */
/* Written by:                                                          */
/*                                                                      */
/* Purpose: WMS-5609 - Auto Create ASN for TW Quicksilver VAS Orders    */
/*                                                                      */
...
CREATE PROCEDURE [dbo].[ispPopulateTOASN_TWQKS_Vas] 
   ...
   BEGIN  
     ...
     IF ISNULL(@c_Type,'') = '' 
      ...
      BEGIN             
         ...
         BEGIN
            ...
            BEGIN
               ...
               BEGIN
                  ...
                  VALUES (@c_NewReceiptKey, @c_ExternReceiptKey, @c_WarehouseReference, @c_ToStorerKey, @c_Type, @c_ToFacility, 'A', 'N')
                  ...
                  BEGIN
                     ...
                     SET @n_err = 63498   
                     SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert Error on Table Receipt (ispPopulateTOASN_TWQKS_Vas)' + ' ( ' + 
               ...
               BEGIN
                  ...
                  SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err) 
                  SET @n_err = 63499   
                  SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Generate Receipt Key Failed! (ispPopulateTOASN_TWQKS_Vas)' 
                                + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
               END
            END
         END    
         ...
         BEGIN
            ...
            SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err)
            SET @n_err = 63500   
            SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Storer Key is BLANK! (ispPopulateTOASN_TWQKS_Vas)' + ' ( ' + 
                          ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
         END
      ...
      BEGIN         
         ...
         BEGIN
            ...
            SELECT @c_SKU        = SKU.SKU,   
                   ...
                   @c_ExternPOKey = ISNULL(ORDERDETAIL.ExternPOKey,'')
               ...
               BEGIN
                  ...
                  BEGIN
                     ...
                     INSERT INTO RECEIPTDETAIL (ReceiptKey,          ReceiptLineNumber,   ExternReceiptkey,    
                                                ...
                                                BeforeReceivedQty,   FinalizeFlag,        ToID, 				ExternPOKey)                                                   
     ...
     BEGIN
        ...
        BEGIN
             ...
             SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err)
                SET @n_err = 63492   
                SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Error on Table Receipt (ispPopulateTOASN_TWQKS_Vas)' + ' ( ' + 
                       ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
        END              
     END*/
      ...
      BEGIN
         ...
         --BEGIN
            --ROLLBACK TRAN
         --END
         EXECUTE nsp_logerror @n_err, @c_errmsg, 'ispPopulateTOASN_TWQKS_Vas'
