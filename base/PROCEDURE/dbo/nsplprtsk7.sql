...
/* 03-Nov-2017  Barnett  1.5  WMS - 2127 SerialNo Scanning at GTM Station*/
/*                            for Full Pallet                            */
/* 18-Dec-2018  Wan01    1.6  WMS-7285 - PRHK - Load Plan Release Task   */
/*************************************************************************/

CREATE PROC [dbo].[nspLPRTSK7]
...
BEGIN
  ...
  DECLARE  @c_ExecStatements              NVARCHAR(4000)
   ...
   DECLARE @c_GTMLoop                     NVARCHAR(10)
         ...
         , @c_COPACKSku                   NVARCHAR(20)   -- Wan01
         , @n_COPackQty                   INT            -- Wan01 
         , @b_COPickPack                  BIT            --(Wan01)
   ...
   BEGIN
      ...
      BEGIN
         ...
         --SELECT @c_Sku = Sku                           --(Wan01)               
         --FROM  LOTxLOCxID WITH (NOLOCK)                --(Wan01)
         --WHERE Loc = @c_FromLoc and ID =@c_Id          --(Wan01)


         -- IN00049940 (Start)
         SELECT @c_SUSR4 = ''
         SET @c_COPACKSku = ''                           --(Wan01)
         SELECT @c_SUSR4 = ISNULL(RTRIM(SUSR4),'')
            ,   @c_COPACKSku = ISNULL(RTRIM(Style),'')   --(Wan01)
         ...
         BEGIN
            ...
            SET @n_COPackQty  = 0
            SELECT @b_COPickPack= 1
            FROM  PICKDETAIL PD WITH (NOLOCK)  
            WHERE PD.Orderkey   = @c_OrderKey
            AND   PD.Storerkey = @c_Storerkey
            AND   PD.Sku = @c_COPACKSku
         ...
         BEGIN
            ...
            SELECT  @c_ToId = '', @c_Status = 'Q', @c_PnDLocation = '', @c_FinalLoc = '', @c_WhereClause = ''
            ...
            BEGIN
               ...
               BEGIN
                  ...
                  SELECT  @c_ToId = '', @c_Status = 0, @c_PnDLocation = '', @c_FinalLoc = '', @c_Sku = ''
         ...
         SELECT  @c_TaskDetailKey  , @c_PickTaskType, @c_Storerkey, @c_Sku, @c_Lot
               ...
               , '' , '' , '' , '' , '' , '' , ''
               , '' , GETDATE() , GETDATE() , '' , '' , ''
               , '' , @c_COPACKSku , '' , GETDATE() , SYSTEM_USER , GETDATE() , SYSTEM_USER  --(TKLIM)   --(Wan01)  
               , '' , '' , '' , '' , '' , @c_FinalLoc
      ...
      BEGIN  
         ...
         BEGIN  
            ...
            BEGIN  
               ...
               SELECT @c_ErrMsg = 'NSQL'+CONVERT(CHAR(5) ,@n_err)+ ': Insert Into PickHeader Failed (nspLPRTSK7)'+' ( ' +  
                                    ' SQLSvr MESSAGE='+@c_ErrMsg + ' ) '  
               GOTO Quit_SP  
            END  
   ...
   IF @n_continue = 3
   BEGIN
      IF @@TRANCOUNT = 1 AND @@TRANCOUNT > @n_StartTranCnt     --(Wan01) - Fixed
         ROLLBACK TRAN
