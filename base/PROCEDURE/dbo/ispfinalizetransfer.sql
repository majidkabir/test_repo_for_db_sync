...
CREATE   PROC [dbo].[ispFinalizeTransfer]
...
/*CS01 End*/
   ...
   DECLARE @c_LoseUCC NVARCHAR(1) -- (ChewKP02)
         , @c_LoseUCC_from             NVARCHAR(1)             --(Wan09)
...
--                       AND   LLI.Qty - LLI.QtyPicked > 0
         ...
         BEGIN
            ...
            BEGIN
               ...
               IF @cFromUCC = '' OR @cFromUCC IS NULL
               BEGIN
                  IF @c_LoseUCC_from <> '1'
                  BEGIN
                     SET @nContinue = 3
                     SET @n_err = 80021
                     SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)
                                 +': From UCC # is required. (ispFinalizeTransfer)'
                     GOTO Quit_Proc
                  END
               END

               IF @cToUCC = '' OR @cToUCC IS NULL
               BEGIN
                  IF @c_LoseUCC <> '1'
                  BEGIN
                     SET @nContinue = 3
                     SET @n_err = 80022
                     SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)
                                 +': To UCC # is required. (ispFinalizeTransfer)'
                     GOTO Quit_Proc
                  END
               END
               --Wan09 - END
