...
/************************************************************************/
/* Stored Procedure: nsp_ReplenishmentRpt_BatchRefill_09                */
/* Creation Date: 08-April-2009                                         */
/* Copyright: IDS                                                       */
/* Written by: KC                                                       */
/*                                                                      */
/* Purpose: Replenishment Report for IDSMY Unilever                     */
/*                                                                      */
/* Called By:                                                           */
/*                                                                      */
/* PVCS Version: 1.7                                                    */
...
/* 10-Jan-2011 NJOW01  1.0   200977-Wave Replen Report to add Page      */
/*                            Breaks. Zone group & sort by to loc.      */
/*                            Change putawayzone to pickzone            */ 
/* 05-Jul-2011 NJOW02  1.1   219787 - Add pack.pallet                   */
/* 05-Jul-2011 NJOW03  1.2   219789 - Add storerconfig to enable page   */
/*                                     break by zone                    */
/*                                    (REPLENRPT09_PAGEBRKBYZONE)       */
/* 11-Oct-2011 NJOW04  1.3   227620 - Exclude Near Expiry stocks by     */
/*                            storerconfig (REPLENRPT09_ExcludeNEP)     */
...
BEGIN
   ...
   --   WHERE 1 = 2
      ...
      BEGIN
         ...
         @c_InvalidInvType01     NVARCHAR(2),
         @c_InvalidInvType02     NVARCHAR(2)
         ...
         SELECT   @c_CurrentSKU = SPACE(20), 
                  ...
                  @n_numberofrecs = 0,
                  @n_limitrecs = 5,
                  @c_InvalidInvType01 = 'QI',
                  @c_InvalidInvType02 = 'BL'
         ...
         BEGIN
            ...
            BEGIN
               ...
               BEGIN
                  ...
                  WHERE Storerkey = @c_CurrentStorer
                  AND Configkey = 'REPLENRPT09_ExcludeNEP'
               
                  IF ISNULL(@c_svalue,'') = '' OR ISNUMERIC(@c_svalue) = 0               
                     SELECT @n_ExcludeNEPMth = 0
                  ELSE
                  ...
                  BEGIN
                     ...
                     IF @b_debug = 1
                     BEGIN
                        Print 'Working on @c_CurrentSKU:' + ISNULL(RTRIM(@c_CurrentSKU),'') + ' @c_CurrentLOC:' + ISNULL(RTRIM(@c_CurrentLOC),'')
                     END 
                     ...
                     BEGIN
                        ...
                        BEGIN
                           ...
                           BEGIN
                              ...
                              IF @b_debug = 1
                              BEGIN
                                 Print 'No Lot Found! SKU: ' + ISNULL(RTRIM(@c_CurrentSKU),'') + ' @c_CurrentLOC: ' + ISNULL(RTRIM(@c_CurrentLOC),'') 
                                       + ' @c_fromlot2: ' + ISNULL(RTrim(@c_fromlot2), '')
                              END
                        ...
                        BEGIN
                           ...
                           BEGIN
                              ...
                              EXEC(@c_PickCode + " N'" + @c_CurrentStorer + "',"
                                               ...
                                               + " N'" + @c_zone01 + "',"
                                               + " N'" + @c_fromlot2 + "'," 
                                               + " N'" + @c_InvalidInvType01 + "',"
                                               + " N'" + @c_InvalidInvType02 + "'" )

                              IF @b_debug = 1
                              BEGIN
                                 print "EXEC " + @c_PickCode + " N'" + ISNULL(RTRIM(@c_CurrentStorer),'') + "',"
                                               + " N'" + ISNULL(RTRIM(@c_CurrentSKU),'') + "',"
                                               + " N'" + ISNULL(RTRIM(@c_CurrentLOC),'') + "',"
                                               + " N'" + ISNULL(RTRIM(@c_zone01),'') + "',"
                                               + " N'" + ISNULL(RTRIM(@c_fromlot2),'') + "',"
                                               + " N'" + @c_InvalidInvType01 + "',"
                                               + " N'" + @c_InvalidInvType02 + "'"
                              END 
                           END
                           ...
                           BEGIN
                              IF @b_debug = 1
                                 print 'No Lot Available! SKU: ' + ISNULL(RTRIM(@c_CurrentSKU),'') + ' LOC:' + ISNULL(RTRIM(@c_CurrentLOC),'')
                              BREAK
                           END
                           ELSE
                           BEGIN
                              IF @b_debug = 1
                              BEGIN
                                 print '*** Lot picked from LOTxLOCxID : ' + ISNULL(RTRIM(@c_fromlot),'')
                              END
                           END 
                           IF @b_debug = 1
                           BEGIN
                              print '*** Selected Lot: ' + ISNULL(RTRIM(@c_fromlot),'')
                           END
                        ...
                        BEGIN
                           ...
                           --NJOW04
                           SELECT @c_FromLot = L.LOT,
                                  @n_SerialNo = L.SerialNo 
                           FROM   #LOT L
                           JOIN   LOTATTRIBUTE LA (NOLOCK) ON L.Lot = LA.Lot
                           WHERE  L.SerialNo > @n_SerialNo 
                           AND    (@n_ExcludeNEPMth = 0 OR LA.Lottable04 >= CONVERT(DATETIME,CONVERT(VARCHAR(10),DATEADD(mm, @n_ExcludeNEPMth, GETDATE()),112))
                                   OR LA.Lottable04 IS NULL)
                           ORDER BY L.SerialNo
...
--                               END 
      ...
      BEGIN 
         ...
         BEGIN
            ...
            BEGIN
               ...
               SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Fail to get REPLENISHKEY. (nsp_ReplenishmentRpt_BatchRefill_09)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + ' ) '
            END    
            ELSE
            BEGIN 
               COMMIT TRAN
      ...
      BEGIN   
         ...
         BEGIN
            ...
            BEGIN
               ...
               SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Insert into Replenishment table failed. (nsp_ReplenishmentRpt_BatchRefill_09)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + ' ) '
            END
      ...
      BEGIN
         ...
         BEGIN
            ...
            END
         END
         EXECUTE nsp_logerror @n_err, @c_errmsg, 'nsp_ReplenishmentRpt_BatchRefill_09'
   ...
   BEGIN
      ...
      SKU.Descr, R.Priority, CASE WHEN SC.Svalue='1' THEN L1.PickZone ELSE 'ALL ZONES' END, PACK.CASECNT, PACK.PACKUOM1, PACK.PACKUOM3, R.ReplenishmentKey, (LT.Qty - LT.QtyAllocated - LT.QtyPicked) As QtyAvailable, LA.Lottable02, LA.Lottable04,
      PACK.Pallet
      FROM  REPLENISHMENT R
      ...
      JOIN LOTATTRIBUTE LA (NOLOCK) ON LT.LOT = LA.LOT AND LT.SKU = LA.SKU AND LT.STORERKEY = LA.STORERKEY -- Pack table added by Jacob Date Jan 03, 2001
      LEFT JOIN V_STORERCONFIG2 SC (NOLOCK) ON R.Storerkey = SC.Storerkey AND SC.Configkey = 'REPLENRPT09_PAGEBRKBYZONE'
      WHERE R.confirmed = 'N'
      ...
      ORDER BY CASE WHEN SC.Svalue='1' THEN L1.PickZone ELSE 'ALL ZONES' END, R.ToLoc, R.SKU 
   END
   ELSE
   BEGIN
      SELECT R.FromLoc, R.Id, R.ToLoc, R.Sku, R.Qty, R.StorerKey, R.Lot, R.PackKey,
      ...
      AND   L1.PickZone in (@c_zone02, @c_zone03, @c_zone04, @c_zone05, @c_zone06, @c_zone07)
