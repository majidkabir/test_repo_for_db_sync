...
/* 2017-06-06 1.0  CSCHONG    Created(WMS-2267)                               */       
/******************************************************************************/                      
                        
CREATE PROC [dbo].[isp_BT_Bartender_SerialNo_RDTLabel_LOGI]                             
...
BEGIN                            
   ...
   DECLARE          
      ...
      @c_toteid          NVARCHAR(30),                          
      @c_vsku            NVARCHAR(30),
      @c_rdttoteid       NVARCHAR(30),
      @c_gettoteid       NVARCHAR(20), 
      @c_getvSKU         NVARCHAR(20)
...
--      DROP TABLE #PICK;      
    ...
    SELECT DISTINCT rdc.v_string1,rdc.v_sku,rdc.v_string2
    FROM RDT.rdtdatacapture RDC WITH (NOLOCK)
     WHERE RDC.v_string1 = @c_Sparm1 
     AND	RDC.v_sku =@c_Sparm2 
     AND RDC.v_string2 = @c_Sparm3
     GROUP BY  rdc.v_string1,rdc.v_sku,rdc.v_string2 
     ORDER BY RDC.v_string2
        
   OPEN CUR_StartRecLoop                  
             
   FETCH NEXT FROM CUR_StartRecLoop INTO @c_toteid,@c_vsku,@c_getSN         
   ...
   FETCH NEXT FROM CUR_RowNoLoop INTO @c_rdttoteid 
   ...
   BEGIN                 
	...
	(
		...
		SELECT DISTINCT rdc.v_string3,rdc.v_string2,rdc.v_string1,rdc.v_sku,
		(Row_Number() OVER (PARTITION BY RDC.v_string3 ORDER BY RDC.v_string2 Asc)-1)/2 AS RECGRP
	   FROM RDT.rdtdatacapture RDC WITH (NOLOCK)
      WHERE RDC.v_string1 = @c_Sparm1 
      AND	RDC.v_sku =@c_Sparm2 
      AND RDC.v_string2 = @c_Sparm3 
      ORDER BY RDC.v_string3 


   DECLARE CUR_SRESULT CURSOR LOCAL FAST_FORWARD READ_ONLY FOR   
   SELECT DISTINCT SerialNo,Mserialno,toteid,vsku   
   FROM   #TEMPMasterSN   
   WHERE toteid = @c_rdttoteid
  
   OPEN CUR_SRESULT   
     
   FETCH NEXT FROM CUR_SRESULT INTO @c_getserialno,@c_getmserialno,@c_gettoteid,@c_getvsku   
   ...
   BEGIN   
      ...
      BEGIN 
		...
		SELECT DISTINCT rdc.v_string3,rdc.v_string2 AS Mserialno
	    FROM RDT.rdtdatacapture RDC WITH (NOLOCK)
		WHERE rdc.v_string3 = @c_getserialno
      ORDER BY rdc.v_string3
    ...
    FROM #TEMPSNExtration
     ...
     --IF @b_debug = '1'            
     --  BEGIN            
     --    SELECT 'masterSN',* FROM #TEMPMasterSN 
     --    SELECT 'SNEXT',* FROM #TEMPSNExtration 
     --    --break      
     --  END 
    
    IF @n_ctnRec>0
    BEGIN
    	 INSERT INTO #TEMPSerialNo (Pickslipno, SerialNo, MSerialNo,vsku)
    	 SELECT TMSN.toteid,TMEX.SerialNo,TMSN.SerialNo,TMSN.vsku
    	 FROM #TEMPMasterSN TMSN
    	 JOIN #TEMPSNExtration TMEX ON TMEX.MSerialNo = TMSN.MSerialNo AND TMEX.SerialNo = TMSN.SerialNo
    END
    ELSE
    BEGIN
    	INSERT INTO #TEMPSerialNo (Pickslipno, SerialNo, MSerialNo,vsku)
    	SELECT TMSN.toteid,TMSN.MSerialNo,TMSN.SerialNo,TMSN.vsku
    	 FROM #TEMPMasterSN TMSN
    END	     
  ...
  FETCH NEXT FROM CUR_RowNoLoop INTO @c_rdttoteid                
   ...
   EXEC isp_InsertTraceInfo         
      ...
      @c_TraceName = 'isp_BT_Bartender_SerialNo_RDTLabel_LOGI',        
