...
/************************************************************************/
/* Stored Proc : ispArchiveNSCLog                                       */
/* Creation Date: 21-Mar-2008                                           */
/* Copyright: IDS                                                       */
/* Written by: Shong                                                    */
/*                                                                      */
/* Purpose: Housekeeping NSCLog table                                   */
/*				Note: Duplicate from ispArchiveTMSLog     						*/
/*                                                                      */
...
/* 30-Jun-08    Leong      SOS# 110238 - Duplicate from ispArchiveTMSLog*/
/*                                       to archive NSCLog              */
/* 13-Feb-18    TLTING     add CANC                                     */
/************************************************************************/

CREATE PROC  [dbo].[ispArchiveNSCLog]
...
BEGIN  
   ...
   SET CONCAT_NULL_YIELDS_NULL OFF  
	...
	DECLARE @n_retain_days 		int, -- days to hold data
	   ...
	   @n_Archive_TL_records	int -- No. of NSCLog records to be Archived
   ...
   WHERE Archivekey = @c_Archivekey
	...
	BEGIN
	   ...
	   ' SQLSvr MESSAGE = ' + LTRIM(RTRIM(@local_c_errmsg)) + ')' +' (ispArchiveNSCLog) '
	END
	...
	BEGIN
	   ...
	   SELECT @d_today = CONVERT(datetime,CONVERT(NVARCHAR(11),GETDATE(),106))
	...
	BEGIN
	   SELECT @b_success = 1
	   SELECT @c_temp = 'Archive Of IDS NSCLog Started with Parms; Datetype = ' + RTRIM(@c_datetype) +
	   ' ; TransmitFlag = '+ RTRIM(@c_TransmitFlag)+ ' ; NSCLogKey = '+RTRIM(@c_TLStart)+'-'+RTRIM(@c_TLEnd)+
	   ' ; Copy Rows to Archive = '+RTRIM(@c_CopyRowsToArchiveDatabase)+ ' ; Retain Days = '+ CONVERT(NVARCHAR(6),@n_retain_days)
	   EXECUTE nspLogAlert
	   @c_ModuleName   = 'ispArchiveNSCLog',
...
-- 	   BEGIN
-- 	      SELECT @c_temp =  ' AND NSCLog.NSCLogKey BETWEEN '+ ''' + RTRIM(@c_TLStart) + ''' +' AND '+
-- 	            '''+RTRIM(@c_TLEnd)+'''
-- 	   END
	   ...
	   BEGIN 
	      SELECT @b_success = 1
	      EXEC nsp_BUILD_Archive_TABLE @c_Copyfrom_db, @c_CopyTo_DB, 'NSCLog',@b_success OUTPUT, @n_err OUTPUT, @c_errmsg OUTPUT
	      IF NOT @b_success = 1
      	BEGIN
      		SELECT @dummy 
      		SELECT @n_continue = 3
      ...
      BEGIN  
   	...
   	END
		...
		BEGIN
		   ...
		   EXECUTE nspBuildAlterTableString @c_CopyTo_DB,'NSCLog',@b_success OUTPUT,@n_err OUTPUT, @c_errmsg OUTPUT
		   IF NOT @b_success = 1
	   	BEGIN
	   		SELECT @n_continue = 3
	   	END
	   ...
	   BEGIN
	      IF (@b_debug = 1)
	      BEGIN
	         print 'building alter table string for NSCLog...'
	      END
	      EXECUTE nspBuildAlterTableString @c_CopyTo_DB,'NSCLog',@b_success OUTPUT,@n_err OUTPUT, @c_errmsg OUTPUT
	      IF NOT @b_success = 1
	      BEGIN
	   ...
	   BEGIN
	      ...
	      BEGIN
		      ...
		      BEGIN
		         SELECT @c_whereclause = 'WHERE NSCLog.EditDate  <= ' + ''''+ CONVERT(char(11),@d_result,106)+''''
                                       + ' AND (NSCLog.TRANSMITFLAG in ( ''9'' , ''5'', ''IGNOR'' ) ) ' 
		      END
		      IF @c_datetype = '2' -- AddDate
		      BEGIN
		         SELECT @c_whereclause = 'WHERE NSCLog.AddDate  <= ' + ''''+ CONVERT(char(11),@d_result,106) +''''
		                               + ' AND (NSCLog.TRANSMITFLAG in ( ''9'' , ''5'', ''IGNOR'' ) ) ' 
		      END
	         ...
	         EXEC (
	         ' DECLARE CUR_NSCLogKey CURSOR FAST_FORWARD READ_ONLY FOR ' + 
	         ' SELECT NSCLogKey FROM NSCLog (NOLOCK) ' + @c_whereclause + 
	         ' ORDER BY NSCLogKey ' ) 
	         
	         OPEN CUR_NSCLogKey 
	         
	         FETCH NEXT FROM CUR_NSCLogKey INTO @n_NSCLogKey
	         ...
	         BEGIN
					...
					IF (@b_debug = 2)
					BEGIN                          
						SELECT @n_NSCLogKey '@n_NSCLogKey to Update ArchiveCop 9'
					END

	            UPDATE NSCLog WITH (ROWLOCK)
	               SET ArchiveCop = '9' 
	            WHERE NSCLogKey = @n_NSCLogKey  
	            ...
	            SELECT @n_Archive_TL_records = @n_Archive_TL_records + 1     
					...
					IF (@b_debug = 1)
					BEGIN                          
						SELECT @n_NSCLogKey '@n_NSCLogKey',  @n_Archive_TL_records '@n_Archive_TL_records'
					END
	            ...
	            IF @local_n_err <> 0
	         	BEGIN 
	               ...
	               SELECT @local_c_errmsg = CONVERT(NVARCHAR(5),@local_n_err)
	               SELECT @local_c_errmsg =
	               ': Update of Archivecop failed - CC Table. (ispArchiveNSCLog) ' + ' ( ' +
      	...
      	END 
		   ...
		   END
			...
			BEGIN 
			   SELECT @b_success = 1
			   EXEC nsp_BUILD_INSERT   @c_CopyTo_DB, 'NSCLog',1 ,@b_success OUTPUT, @n_err OUTPUT, @c_errmsg OUTPUT
			   IF NOT @b_success = 1
			   BEGIN
			      SELECT @n_continue = 3
			   END
		   ...
		   IF (@b_debug = 1)
		   BEGIN
		      SELECT Count(1) FROM NSCLog (NOLOCK)
		      WHERE ArchiveCop = '9'
		   END
		END
	END
	...
	BEGIN
		...
		EXECUTE nspLogAlert
		@c_ModuleName   = 'ispArchiveNSCLog',
		@c_AlertMessage = 'Archive Of NSCLog Ended Normally.',
		@n_Severity     = 0,
		@b_success      = @b_success OUTPUT,
		@n_err          = @n_err OUTPUT,
		@c_errmsg       = @c_errmsg OUTPUT
	...
	BEGIN
		...
		BEGIN
			...
			EXECUTE nspLogAlert
			@c_ModuleName   = 'ispArchiveNSCLog',
			@c_AlertMessage = 'Archive Of NSCLog Ended Abnormally - Check This Log For Additional Messages.',
			@n_Severity     = 0,
			@b_success      = @b_success OUTPUT,
			@n_err          = @n_err OUTPUT,
			@c_errmsg       = @c_errmsg OUTPUT
	...
	BEGIN
		...
		END
		EXECUTE nsp_logerror @n_err, @c_errmsg, 'ispArchiveNSCLog'
