...
/*************************************************************************/
/* Stored Procedure: isp_EXG_FilterCondition_Coach                       */
/* Creation Date: 24-Mar-2022                                            */
/* Copyright: LFL                                                        */
/* Written by: GuanHao Chan                                              */
/*                                                                       */
/* Purpose: Excel Generator SP FilterConfition for Coach.                */
/*                                                                       */
...
CREATE PROCEDURE [dbo].[isp_EXG_FilterCondition_Coach] (
...
BEGIN
   ...
   IF @b_Debug = 1
   BEGIN
      PRINT '[dbo].[isp_EXG_FilterCondition_Coach]: Start...';
      PRINT '[dbo].[isp_EXG_FilterCondition_Coach]: ' + ' @c_Username=' + ISNULL(RTRIM(@c_Username), '') + ' @n_EXG_Hdr_ID='
   ...
   BEGIN
      SET @n_Err = 210001;
      SET @c_ErrMsg = 'Invalid EXG Hdr ID. (isp_EXG_FilterCondition_Coach)';
      SET @n_Continue = 3;
      GOTO QUIT;
   END;
   ...
   BEGIN TRY
      ...
      BEGIN
         SET @n_Err = 210002;
         SET @c_ErrMsg = ' StorerKey cannot be null. - (isp_EXG_FilterCondition_Coach)';

         IF @b_Debug = 1
         BEGIN
            PRINT '[isp_EXG_FilterCondition_Coach]: Execute SP Name Failed...' + ' @c_ErrMsg=' + @c_ErrMsg;
         END;
         GOTO QUIT;
      END;
      ...
      BEGIN
         SET @n_Err = 210003;
         SET @c_ErrMsg = ' ParamVal10(TransmitlogKey) cannot be null. - (isp_EXG_FilterCondition_Coach)';
      ...
      BEGIN
         ...
         BEGIN
            ...
            SELECT CONCAT('Coach_Carton_Detail_Report_', O.ConsigneeKey, '_', CONVERT(VARCHAR(8), GETDATE(), 112), @c_FileExt)
                 , @c_ParamVal1
                 , @c_ParamVal2
                 , O.ConsigneeKey
                 , @c_ParamVal10
   ...
   BEGIN CATCH
      ...
      SET @n_Err = @n_ITFErr;
      SET @c_ErrMsg = LTRIM(RTRIM(@c_ITFErrMsg)) + ' (isp_EXG_FilterCondition_Coach)';
   ...
   BEGIN
      ...
      IF @b_Debug = 1
      BEGIN
         PRINT '[dbo].[isp_EXG_FilterCondition_Coach]: @c_ErrMsg=' + RTRIM(@c_ErrMsg);
         PRINT '[dbo].[isp_EXG_FilterCondition_Coach]: @b_Success=' + RTRIM(CAST(@b_Success AS NVARCHAR));
      END;
