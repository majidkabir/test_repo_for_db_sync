...
/************************************************************************/  
/* Stored Procedure:  isp_GetITRNKey                                    */  
/* Creation Date:                                                       */  
/* Copyright: IDS                                                       */  
/* Written by:                                                          */  
/*                                                                      */  
...
/*                                                                      */  
/* 09-Apr-2013  Shong      1.1  Replace GetKey with isp_GetITRNKey      */
/*                              to reduce blocking                      */
/* 13-May-2015  TLTING     1.2  Change variable BigINT                  */
/************************************************************************/ 
CREATE PROC [dbo].[isp_GetITRNKey]     
(   @n_FieldLength  INT     
...
BEGIN    
   ...
   INSERT INTO ITRNKey (AddDate) VALUES (GETDATE())    
   SELECT @n_err = @@ERROR, @n_cnt = @@ROWCOUNT, @n_NewITRNKey = @@IDENTITY    
   IF @n_err <> 0    
   BEGIN    
     SELECT @n_continue = 3     
   END    
   SELECT @n_NewITRNKey = @@IDENTITY    
  
   IF @n_continue = 1 OR @n_continue = 2    
   BEGIN    
 
      SELECT @c_ITRNKey = RIGHT(Replicate('0',@n_FieldLength) + CAST(@n_NewITRNKey As VARCHAR(10)), @n_FieldLength)     
   END    
   ELSE    
      SELECT @c_ITRNKey = ''    
   ...
   BEGIN    
      ...
      BEGIN    
          ...
          END              
      END    
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'isp_GetITRNKey'    
