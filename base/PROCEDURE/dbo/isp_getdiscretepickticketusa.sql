...
GO
/************************************************************************/
/* Stored Procedure: isp_GetDiscretePickTicketUSA                       */
/* Creation Date: 28-Sept-2007                                          */
/* Copyright: IDS                                                       */
/* Written by: June	                                                   */
/*                                                                      */
/* Purpose:  SOS#83917 - Discrete Pick Ticket for IDSUS.            		*/
/*                                                                      */
/* Called By:  PB - r_dw_consolidated_pick16_discrete                   */
/*                                                                      */
/* PVCS Version: 1.0                                                    */
/*                                                                      */
/* Version: 5.4                                                         */
...
/* 04-Sep-2008	 KC	     SOS#111163 - Update temp pickslip             */
/* 30 Apr 2009  Rick Liew SOS#107784 - Added Storer.Notes1 field        */
/* 09-Jul-2009  Leong     SOS#141857 - Include spacing                  */
/* 17-Dec-2013  NJOW01    Fix commit issue                              */
/************************************************************************/

CREATE PROC [dbo].[isp_GetDiscretePickTicketUSA] ( 
  ...
  DECLARE @t_DiscretePS TABLE 
	...
	BEGIN		
		...
		FROM ( 
            SELECT BOMQty.Storerkey, BOMQty.Orderkey, BOMQty.AltSKU, BOMQty.Loc, BOMQty.NoBOMQty, MINPPK.PPKQty
				FROM (SELECT #TempDiscPickSlip.Storerkey, #TempDiscPickSlip.Orderkey, #TempDiscPickSlip.AltSKU, #TempDiscPickSlip.Loc, NoBOMQty = #TempDiscPickSlip.Qty/BOM.Qty
						...
						GROUP BY #TempDiscPickSlip.Storerkey, #TempDiscPickSlip.Orderkey, #TempDiscPickSlip.AltSKU, #TempDiscPickSlip.Loc, (#TempDiscPickSlip.Qty/BOM.Qty)
						) BOMQty
			   JOIN (	
						SELECT #TempDiscPickSlip.Storerkey, #TempDiscPickSlip.Orderkey, AltSku, Loc, PPKQty = MIN(#TempDiscPickSlip.Qty / BOM.Qty)
						FROM  #TempDiscPickSlip (nolock) 
						LEFT OUTER JOIN BillOfMaterial BOM (nolock) ON BOM.ComponentSku = #TempDiscPickSlip.Sku 
																			    AND BOM.Sku = #TempDiscPickSlip.AltSku
																				 AND BOM.Storerkey = #TempDiscPickSlip.Storerkey 
			
			GROUP BY #TempDiscPickSlip.Storerkey, #TempDiscPickSlip.Orderkey, AltSku, Loc) MINPPK 
	...
	BEGIN
	   ...
	   SELECT   PickSlipNo, Loadkey, Orderkey, StorerKey, PrintedFlag, 
		...
		FROM   #TempDiscPickSlip2
		WHERE  LoadKey = @c_LoadKey
		ORDER BY Loadkey, Storerkey, Orderkey, PickSlipno, Putawayzone, LogicalLoc, Loc, Style, Lottable03	
					...
					BEGIN
                        ...
                        -- Modified By Vicky on 09-Oct-2007 (Start)
								...
								VALUES (@c_Pickslipno, @c_Loadkey, @c_Orderkey, @c_Printedflag, @c_Storerkey, 
                                 ...
                                 CONVERT(CHAR(3), ISNULL(dbo.fnc_RTrim(@c_UOM), '')) +
										 	CONVERT(CHAR(17), ISNULL(dbo.fnc_RTrim(@c_PPK_per), '')) + CONVERT(CHAR(5), ISNULL(@n_NoOfUnits, '')) + 
                                 CONVERT(CHAR(60), ISNULL(dbo.fnc_RTrim(@c_Busr1), '') ) +
							...
							BEGIN
								...
								VALUES (@c_Pickslipno, @c_Loadkey, @c_Orderkey, @c_Printedflag, @c_Storerkey, 
                                 ...
                                 CONVERT(CHAR(20), ISNULL(dbo.fnc_RTrim(@c_Lottable03), '')) + 
											--LEFT(dbo.fnc_RTrim(CONVERT(CHAR(10), @n_UOMQty)) + REPLICATE('', 10), 10) + 
                                 LEFT(dbo.fnc_RTrim(CONVERT(CHAR(10), @n_UOMQty)) + REPLICATE('', 10), 10) + '        '+  -- SOS141857
											LEFT(dbo.fnc_RTrim(CONVERT(CHAR(10), @n_EAQty)) + REPLICATE('', 10), 10), '')
							END							 
						END
						ELSE -- Print Loose
