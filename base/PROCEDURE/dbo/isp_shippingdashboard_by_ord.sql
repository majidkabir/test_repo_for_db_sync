...
/*************************************************************************/  
/* Stored Procedure: isp_ShippingDashBoard_By_ORD                        */  
...
/*                      datawindow - d_dw_shiping_dashboard_by_ORD       */  
/*                                                                       */  
/* PVCS Version: 1.0                                                     */  
/*                                                                       */  
/* Version: 5.4                                                          */  
...
CREATE PROC [dbo].[isp_ShippingDashBoard_By_ORD]
...
BEGIN
   ...
   INSERT INTO #TMPMBOL (CBOLKey, MBOLKey, Door, CID, Orderkey, PickSlipNo, CartonLBL, DropID, LocCategory)
   SELECT CBOLKey = CASE WHEN MH.CBOLKey = 0 OR MH.CBOLKey IS NULL THEN NULL ELSE MH.CBOLKey END
         ,MH.MBOLKey
   ...
   JOIN PICKDETAIL   PK WITH (NOLOCK) ON (OD.Orderkey= PK.Orderkey) AND (OD.OrderlineNumber = PK.OrderLineNumber)
   ...
   LEFT JOIN PACKDETAIL   PD WITH (NOLOCK) ON (PH.PickSlipNo = PD.PickSlipNo)   
                                           AND((ISNULL(RTRIM(PK.DropID),'') = CASE WHEN ISNULL(RTRIM(PK.DropID),'') = '' THEN ISNULL(RTRIM(PK.DropID),'') ELSE PD.DropID END) 
                                           OR  (ISNULL(RTRIM(PK.CaseID),'') = CASE WHEN ISNULL(RTRIM(PK.CaseID),'') = '' THEN ISNULL(RTRIM(PK.CaseID),'') ELSE PD.LabelNo END))
