...
GO
/************************************************************************/
/* Store Procedure: isp_TPP_TPPRN_01                                    */
...
/* 09-AUG-2021  CSCHONG 1.0  Add in print command (CS01)                */
...
BEGIN
   ...
   DECLARE @n_FromCartonNo   INT = 0,
           ...
           @c_UDF03          NVARCHAR(4000)='',       --(CS03)       
           @c_UDF04          NVARCHAR(4000)='',       --(CS03)       
           @c_UDF05          NVARCHAR(4000)='',          
           @c_SourceType     NVARCHAR(30) = '', --print from which function    
           @c_Platform       NVARCHAR(30)='',       
           @c_UserName       NVARCHAR(128)= '',
           @c_SqlOutput      NVARCHAR(max) = '',      --(CS02)
           @c_sqlparm        NVARCHAR(max),           --(CS02)
           @c_SqlOutput03    NVARCHAR(max) = '',      --(CS03)
           @c_SqlOutput05    NVARCHAR(max) = '',      --(CS03) 
           @c_GetPrintData   NVARCHAR(MAX) = '',      --(CS09)
           @c_PrnContent     NVARCHAR(4000)=''        --(CS03) 
...
--CS01 END
   ...
   BEGIN
     SELECT @c_Module=Module, @c_ReportType=ReportType, @c_Storerkey=Storerkey,@c_PrinterID=PrinterID, @c_Printer=Printer, 
      ...
      ELSE
       ...
       --  JOIN dbo.TPPRINTERGROUP TPGRP ON TPGRP.TPPrinterGroup=P.TPPrinterGroup
       --  WHERE TPGRP.PrinterPlatform = @c_Platform
       ----HERE U.UserName = @c_UserName
       -- AND P.WinPrinter  = @c_printer 
       ...
       IF ISNULL(@c_PrinterID,'') = ''
        BEGIN
         ...
         SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_err,0))   
                        + ': TPP Printer Group not setup for printer : ' + @c_printer + ' with platform' + @c_Platform + ' (isp_TPP_TPPRN_01)'
         SET @c_Status = '5'
        ...
        BEGIN
         ...
         SET @n_err = 68010     
         SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_err,0))   
                        + ': TPP Printer Group not setup for printer : ' + @c_printerid + ' with platform' + @c_Platform + ' (isp_TPP_TPPRN_01)'
         SET @c_Status = '5'
         SET @c_Message = RTRIM(ISNULL(@c_Message,'')) + @c_ErrMsg + ' '                                
        ...
        SET @c_version = '1.0'
        --SET @c_contentdata = @c_UDF05 -- @c_UDF01    --(CS02)
   ...
   BEGIN
      ...
      BEGIN
         ...
         SET @n_err = 68010     
         SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_err,0))   
                        + ': Unable to find print record for Job No: ' +RTRIM(CAST(@n_JobNo AS NVARCHAR)) + ' (isp_TPP_TPPRN_01)'
         SET @c_Status = '5'
         SET @c_Message = RTRIM(ISNULL(@c_Message,'')) + @c_ErrMsg + ' '                                
      END
      ...
      BEGIN       
          ...
          BEGIN
            ...
            SET @n_err = 68010     
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_err,0))   
                           + ': Get RequestID fail for Order# ' + RTRIM(@c_CurrOrderkey) + ' Carton# ' + RTRIM(CAST(@n_CartonNo AS NVARCHAR)) + ' (isp_TPP_TPPRN_01)'
            SET @c_Status = '5'
            SET @c_Message = RTRIM(ISNULL(@c_Message,'')) + @c_ErrMsg + ' '                                
          END
          ELSE
          ...
          BEGIN
            ...
            SET @n_err = 68010     
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_err,0))   
                           + ': Unable to find tracking# for Order# ' + RTRIM(@c_CurrOrderkey) + ' Carton# ' + RTRIM(CAST(@n_CartonNo AS NVARCHAR)) + ' (isp_TPP_TPPRN_01)'
            SET @c_Status = '5'
            SET @c_Message = RTRIM(ISNULL(@c_Message,'')) + @c_ErrMsg + ' '                                
          END

           SET @c_GetPrintData =@c_PrintData       --CS09     
         ...
         BEGIN
                   SET @c_CloseContent = ''
                   SET @c_contentdata = ''
                   SET @c_SqlOutput = ''      --CS02
                   SET @c_SQL = @c_UDF05      --CS02
                   SET @c_SqlOutput03 = ''    --CS03
                   SET @c_SQL03 = @c_UDF03    --CS03
                   SET @c_SqlOutput05 = ''    --CS03  

                   IF ISNULL(@c_TemplathURL,'') <> '' --ISNULL(@c_contentdata,'') <> ''    --CS02
                   BEGIN
                ...
                SET @c_RequestString = N'{"cmd": ' + '"' + @c_cmdstart + '"' + ', "requestID":' + '"' + @c_RequestID + '"' + ', "version":' + '"' + @c_version + '"' + ', "task": {"taskID":'  + '"' + @c_nCounter + '"' + 
                                     ...
                                     + '"' + @c_trackingno + '"' + ',"contents": [ ' + @c_GetPrintData + @c_SqlOutput03  + @c_CloseContent + '] }] } }'    
   ...
   BEGIN
      ...
      BEGIN
         ...
         END          
      END
      EXECUTE nsp_logerror @n_err, @c_errmsg, "isp_TPP_TPPRN_01"
