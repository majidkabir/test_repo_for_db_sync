...
DECLARE @c_PrevOrderKey       NVARCHAR(10),
        ...
        @n_Eaches             int,
        @n_UOMQty             int,
        @c_ServiceMode        NVARCHAR(30),                                                         --(Wan01)
        @c_IntermodalVehicle  NVARCHAR(30)                                                          --(Wan01)

SET @c_ServiceMode       = ''                                                                      --(Wan01)
SET @c_IntermodalVehicle = ''                                                                      --(Wan01)
...
CREATE TABLE #temp_pick (
   ...
   InterModalVehicle NVARCHAR(30) NULL,                                                                   --(Wan01)
   ServiceMode       NVARCHAR(30) NULL)                                                                   --(Wan01)
SELECT @n_continue = 1
SELECT @n_RowNo = 0
...
BEGIN --While
   ...
   BEGIN --if @c_orderkey <> ''
      SELECT @c_ConsigneeKey = Orders.Consigneekey,
            ...
            @c_rdd           = ORDERS.RDD
           ,@c_IntermodalVehicle = ISNULL(RTRIM(ORDERS.IntermodalVehicle),'')                      --(Wan01)
               ...
               BEGIN
                  ...
                  BEGIN
                     ...
                     BEGIN
                        ...
                        Select @c_errmsg= 'NSQL'+CONVERT(char(5), @n_err)+':Insert failed on TransmitLog2. (isp_GetPickSlipWave06)'+'('+'SQLSvr MESSAGE='+dbo.fnc_LTRIM(dbo.fnc_RTRIM(@c_errmsg))+')'
                        ROLLBACK TRAN 
                        GOTO QUIT 
                     END
                     ELSE
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN  
            ...
            SELECT @n_continue=3
            SELECT @n_err = @@ERROR
            SELECT @c_errMsg = 'Insert into TransmitLog Failed (isp_GetPickSlipWave06)'                     
            GOTO QUIT 
         END  
         ELSE 
         BEGIN
...
QUIT:
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         END  
      END  
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'isp_GetPickSlipWave06'  
