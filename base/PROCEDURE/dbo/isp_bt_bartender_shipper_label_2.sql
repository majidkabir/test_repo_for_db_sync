...
/*            isp_BT_Bartender_Shipper_Label_1 to only remove the             */ 
/*            filter by userdefine04 is null                                  */              
/*                                                                            */               
/* Modifications log:                                                         */               
/*                                                                            */               
...
BEGIN                    
  ...
  IF @c_Sparm4='1'          
   BEGIN        
     SET @c_SQLJOIN = +' SELECT ORD.loadkey,ORD.orderkey,ORD.externorderkey,ORD.type,buyerpo,salesman,ORD.facility,STO.Secondary,'    --8      
               ...
               -- + ' AND ORD.OrderKey = CASE WHEN ISNULL(RTRIM(''' + @c_Sparm2+ '''),'''') <> '''' THEN ''' + @c_Sparm2+ ''' ELSE ORD.OrderKey END'   --(CS02)      --CS01
 ...
 END      
  ...
  SET @c_SQL='INSERT INTO #Result (Col01,Col02,Col03,Col04,Col05, Col06,Col07,Col08,Col09'  + CHAR(13) +         
   ...
   SET @c_SQL = @c_SQL + @c_SQLJOIN +  CHAR(13) + @c_condition1  + CHAR(13) + @c_condition2  --CS01     
   --CS01 Start
...
BEGIN     
   ...
   IF  @c_consigneeFor = 'A' 
    BEGIN  
        ...
        -- LEFT OUTER JOIN ORDERS ORD WITH (NOLOCK) ON ORD.ShipperKey = C.Short    
         JOIN ORDERS ORD WITH (NOLOCK) ON ORD.ShipperKey = C.Short         
         WHERE ORD.Orderkey =  @c_OrderKey    
   ...
   SELECT TOP 1 
          @c_notes2 = C.Notes2
   FROM   Codelkup C WITH (NOLOCK) 
  -- LEFT OUTER JOIN ORDERS ORD WITH (NOLOCK)       
   JOIN ORDERS ORD WITH (NOLOCK)                     
   ...
   BEGIN  
       ...
       SELECT R.*   
           ...
           INNER JOIN #PICK P WITH (NOLOCK) 
                      ON  P.Orderkey = R.Col02  
           --WHERE  ISNULL(Col38 ,'') <> ''   
           WHERE    P.TTLPICKQTY > 1 AND P.PickZone=1 
           ORDER BY P.PickZone,        
                    P.picknotes,        
                    col02,
                    col60  
...
EXIT_SP:  
   ...
   EXEC isp_InsertTraceInfo 
      ...
      @c_TraceName = 'isp_BT_Bartender_Shipper_Label_2',
