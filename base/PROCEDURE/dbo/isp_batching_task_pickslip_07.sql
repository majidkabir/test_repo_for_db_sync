...
/************************************************************************/
/* Store Procedure:  isp_batching_task_pickslip_07                      */
/* Creation Date:  27-FEB-2023                                          */
/* Copyright: LF                                                        */
/* Written by:                                                          */
/*                                                                      */
/* Purpose: WMS-21849-MYÎ“Ã‡Ã´SKECHERSÎ“Ã‡Ã´Enlarge Font in Task Pickslip         */
...
CREATE   PROC [dbo].[isp_batching_task_pickslip_07] (
 ...
 BEGIN
    ...
    BEGIN
       SELECT @n_Continue = 3
       SELECT @n_Err = 63200
       SELECT @c_ErrMsg='NSQL'+CONVERT(NVARCHAR(5),@n_Err)+': Invalid Mode. The value must be 1,4,5,9 (isp_batching_task_pickslip_07)'
       GOTO Quit
    END
   ...
   --            AND 1 = CASE WHEN (@c_ReportType = '3' AND @c_MultiConsoTaskPick = '1') OR (@c_ReportType = '5' AND @c_MultiConsoTaskPick <> '1') THEN 2 ELSE 1 END)
    ...
    LEFT JOIN Codelkup CL2 (NOLOCK) ON (PD.Storerkey = CL2.Storerkey AND CL2.Code = 'SHOWSTYLESIZE'
                                              AND CL2.Listname = 'REPORTCFG' AND CL2.Long = 'isp_batching_task_pickslip_07' AND ISNULL(CL2.Short,'') <> 'N')
    LEFT JOIN Codelkup CL3 (NOLOCK) ON (PD.Storerkey = CL3.Storerkey AND CL3.Code = 'SHOWLOADKEYBARCODE'
                                              AND CL3.Listname = 'REPORTCFG' AND CL3.Long = 'isp_batching_task_pickslip_07' AND ISNULL(CL3.Short,'') <> 'N')
...
Quit:
    ...
    BEGIN
       ...
       BEGIN
          ...
          END
       END
       EXECUTE nsp_logerror @n_Err, @c_ErrMsg, 'isp_batching_task_pickslip_07'
