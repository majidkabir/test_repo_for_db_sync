...
/* 2022-08-29     1.0  CHONGCS     DevOps Combine Script &WMS-20622           */ 
/******************************************************************************/              
                
CREATE PROC [dbo].[isp_BT_Bartender_Shipper_Label_WAYBILL_PVHSZ]                     
...
BEGIN                    
  ...
  SET @c_SQLJOIN = +'SELECT DISTINCT PIF.trackingno,orders.c_contact1,'
                   +' ISNULL(orders.c_company,''''), '
                   +' ISNULL(orders.c_phone1,''''),'''','           --5
                   +' '''','''','''',ISNULL(orders.c_contact2,''''),'   --9
                  ...
                  -- + ' FROM ORDERS orders WITH (NOLOCK) '
                   ...
                   +' LEFT JOIN CODELKUP C3 WITH (NOLOCK) ON C3.LISTNAME=''SFCTRYCODE'' ' 
                   +'                                     AND C3.Storerkey=orders.StorerKey '
                   +'                                     AND C3.code=orders.c_city'
                   + ' WHERE orders.loadkey =  @c_Sparm1 '                                            
                   + ' AND orders.orderkey = @c_Sparm2 '    
                   + ' AND PD.cartonno = @c_Sparm4 ' 
...
EXIT_SP:  
   ...
   --   @c_TraceName = 'isp_BT_Bartender_Shipper_Label_WAYBILL_PVHSZ',
