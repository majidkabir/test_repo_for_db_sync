...
/************************************************************************/  
/* Store Procedure:  isp_ITF_ntrIQC                                     */  
/* Creation Date: 13-May-2015                                           */  
/* Copyright: LF                                                        */  
/* Written by: MCTang                                                   */  
/*                                                                      */  
/* Purpose:  Handling trigger points for IQC's module.                  */  
/*           Including IQC Header trigger points for Add & Update       */  
/*                                                                      */  
...
/*             - ntrInventoryQCHeaderAdd                                */  
/*             - ntrInvenotryQCHeaderUpdate                             */  
/*                                                                      */  
/* PVCS Version: 1.0                                                    */  
/*                                                                      */  
/* Version: 5.4                                                         */  
...
CREATE PROC [dbo].[isp_ITF_ntrIQC]  
...
BEGIN  
   ...
   IF (ISNULL(RTRIM(@c_TriggerName),'') = '') OR   
      ...
      (ISNULL(RTRIM(@c_IQCKey),'') = '')  
   BEGIN  
      RETURN  
   END  
  
   IF (ISNULL(RTRIM(@c_TriggerName),'') <> 'ntrInventoryQCHeaderUpdate')  
   BEGIN  
      IF (ISNULL(RTRIM(@c_TriggerName),'') <> 'ntrInventoryQCHeaderAdd')  
      BEGIN  
         RETURN  
      END  
   END  
  
   IF (ISNULL(RTRIM(@c_SourceTable),'') <> 'InventoryQC')  
   BEGIN  
      RETURN  
   END  
   /*************************************************************************************/  
   ...
   BEGIN  
      SELECT @c_StorerKey     = ISNULL(RTRIM(InventoryQC.StorerKey),'')   
           , @c_Facility      = ISNULL(RTRIM(InventoryQC.From_Facility),'')   
           , @c_FinalizeFlag  = ISNULL(RTRIM(InventoryQC.FinalizeFlag),'')
      FROM   InventoryQC WITH (NOLOCK)   
      WHERE  InventoryQC.QC_Key = @c_IQCKey  
   END   
   /*************************************************************************************/  
      ...
      BEGIN
         ...
         BEGIN 
            ...
            BEGIN
               ...
               EXEC sys.sp_executesql @c_StoredProc, N'@c_IQCKey NVARCHAR(10), @b_Success INT OUTPUT, @c_ErrNo INT OUTPUT, @c_ErrMsg NVARCHAR(215) OUTPUT',
                           @c_IQCKey, 
                           @b_Success OUTPUT, 
                           @n_Err     OUTPUT, 
                           @c_ErrMsg  OUTPUT 
            ...
            ELSE
            BEGIN
               IF (@c_RecordStatus <> '' AND @c_RecordStatus = @c_FinalizeFlag AND UPPER(@c_UpdatedColumns) = 'FINALIZEFLAG')
               BEGIN 
                  SET @b_Success = 1
               END 
            END
            ...
            BEGIN
               ...
               BEGIN  
                  EXEC ispGenTransmitLog3 @c_Tablename, @c_IQCKey, '', @c_StorerKey, ''  
                                          , @b_success OUTPUT  
                                          , @n_Err OUTPUT  
                                          , @c_ErrMsg OUTPUT  
                  ...
                  BEGIN  
                     ...
                     SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_Err,0)) +   
                                     ': Insert into TRANSMITLOG3 Failed. (isp_ITF_ntrIQC) ( SQLSvr MESSAGE = ' +   
               ...
               BEGIN  
                  EXEC ispGenTransmitLog2 @c_Tablename, @c_IQCKey, '', @c_StorerKey, ''  
                                          , @b_success OUTPUT  
                                          , @n_Err OUTPUT  
                                          , @c_ErrMsg OUTPUT  
                  ...
                  BEGIN  
                     ...
                     SET @n_Err = 68001  
                     SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_Err,0)) +   
                                     ': Insert into TRANSMITLOG2 Failed. (isp_ITF_ntrIQC) ( SQLSvr MESSAGE = ' +   
                                     ISNULL(LTRIM(RTRIM(@c_ErrMsg)),'') + ' ) '  
                     GOTO QUIT  
                  END   
...
QUIT:  
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         END  
      END  
      EXECUTE dbo.nsp_logerror @n_Err, @c_ErrMsg, 'isp_ITF_ntrIQC'  
