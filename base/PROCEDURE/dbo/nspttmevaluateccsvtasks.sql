...
GO
/************************************************************************/
/* Stored Procedure: nspTTMEvaluateCCSVTasks                            */
...
CREATE PROC    [dbo].[nspTTMEvaluateCCSVTasks]
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Execute Of Move Tasks Pick Code Failed. (nspTTMEvaluateCCSVTasks)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
      END
      IF @n_err = 16915
      BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Error Occurred While Attempting To Update TaskDetail. (nspTTMEvaluateCCSVTasks)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
            END
            IF @n_continue = 1 or @n_continue = 2
            BEGIN
               UPDATE TASKDETAIL
...
ELSE
   ...
   ELSE
      BEGIN
         ...
         END
      END
      execute nsp_logerror @n_err, @c_errmsg, "nspTTMEvaluateCCSVTasks"
