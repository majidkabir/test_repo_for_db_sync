...
/* 05-Jun-2018  Shong   1.1   Bug Fix Cursor error                      */
...
BEGIN
   ...
   @n_Priority             INT = 9, 
   @n_FetchStatus          INT = 0,
   @c_ODLottable01         NVARCHAR(18) = '',  --NJOW01
   ...
   BEGIN
   	...
   	BEGIN
         ...
         BEGIN         	          	
            ...
            SELECT [Priority] = 
                  CASE 
                WHEN SO.OrderKey = @c_OrderKey THEN 1
                  ...
                  PD.Qty, 
                  PD.ShipFlag,
                  OD.Lottable01, --NJOW01
                  OD.Lottable02, --NJOW01
                  OD.Lottable03, --NJOW01
                  OD.Lottable06, --NJOW01
                  OD.Lottable07, --NJOW01
                  OD.Lottable08, --NJOW01
                  OD.Lottable09, --NJOW01
                  OD.Lottable10, --NJOW01
                  OD.Lottable11, --NJOW01
                  OD.Lottable12  --NJOW01
            FROM PICKDETAIL PD WITH (NOLOCK) 
            JOIN ORDERS AS SO WITH(NOLOCK) ON PD.OrderKey = SO.OrderKey 
            JOIN ORDERDETAIL OD WITH(NOLOCK) ON SO.Orderkey = OD.Orderkey AND PD.OrderLineNumber = OD.OrderLineNumber  --NJOW01
            WHERE PD.SKU       = @c_SKU
               AND PD.StorerKey = @c_StorerKey
               AND PD.LOC       = @c_LOC
               AND PD.LOT       = @c_ReplenLOT
               AND PD.ID        = @c_ReplenID
               AND PD.ShipFlag  <> 'Y' 
               AND (( @c_ForcePicked = 'Y' AND (PD.Status BETWEEN '5' AND '8' OR PD.ShipFlag = 'P')) OR 
                     ( @c_ForcePicked = 'N' )) 
            ORDER BY [Priority], PD.PickDetailKey 
              
            OPEN PICK_CUR

            FETCH NEXT FROM PICK_CUR INTO @n_Priority, @c_PickDetailKey, @n_Qty, @c_ShipFlag,
                 @c_ODLottable01, @c_ODLottable02, @c_ODLottable03, @c_ODLottable06, @c_ODLottable07, --NJOW01
                 @c_ODLottable08, @c_ODLottable09, @c_ODLottable10, @c_ODLottable11, @c_ODLottable12  --NJOW02
               ...
               BEGIN
                  ...
                  /*               	
               	  IF NOT EXISTS (SELECT 1
               	                 ...
               	                 AND OD.Lottable01 = CASE WHEN ISNULL(OD.Lottable01,'') <> '' AND CHARINDEX('LOTTABLE01', @c_ForceLottableList) > 0 THEN @c_Lottable01 ELSE OD.Lottable01 END 
               	                 AND OD.Lottable02 = CASE WHEN ISNULL(OD.Lottable02,'') <> '' AND CHARINDEX('LOTTABLE02', @c_ForceLottableList) > 0 THEN @c_Lottable02 ELSE OD.Lottable02 END 
               	                 AND OD.Lottable03 = CASE WHEN ISNULL(OD.Lottable03,'') <> '' AND CHARINDEX('LOTTABLE03', @c_ForceLottableList) > 0 THEN @c_Lottable03 ELSE OD.Lottable03 END 
               	                 AND OD.Lottable06 = CASE WHEN ISNULL(OD.Lottable06,'') <> '' AND CHARINDEX('LOTTABLE06', @c_ForceLottableList) > 0 THEN @c_Lottable06 ELSE OD.Lottable06 END 
               	                 AND OD.Lottable07 = CASE WHEN ISNULL(OD.Lottable07,'') <> '' AND CHARINDEX('LOTTABLE07', @c_ForceLottableList) > 0 THEN @c_Lottable07 ELSE OD.Lottable07 END 
               	                 AND OD.Lottable08 = CASE WHEN ISNULL(OD.Lottable08,'') <> '' AND CHARINDEX('LOTTABLE08', @c_ForceLottableList) > 0 THEN @c_Lottable08 ELSE OD.Lottable08 END 
               	                 AND OD.Lottable09 = CASE WHEN ISNULL(OD.Lottable09,'') <> '' AND CHARINDEX('LOTTABLE09', @c_ForceLottableList) > 0 THEN @c_Lottable09 ELSE OD.Lottable09 END 
               	                 AND OD.Lottable10 = CASE WHEN ISNULL(OD.Lottable10,'') <> '' AND CHARINDEX('LOTTABLE10', @c_ForceLottableList) > 0 THEN @c_Lottable10 ELSE OD.Lottable10 END 
               	                 AND OD.Lottable11 = CASE WHEN ISNULL(OD.Lottable11,'') <> '' AND CHARINDEX('LOTTABLE11', @c_ForceLottableList) > 0 THEN @c_Lottable11 ELSE OD.Lottable11 END 
               	                 AND OD.Lottable12 = CASE WHEN ISNULL(OD.Lottable12,'') <> '' AND CHARINDEX('LOTTABLE12', @c_ForceLottableList) > 0 THEN @c_Lottable12 ELSE OD.Lottable12 END
               	                 )
               	  BEGIN
               	     GOTO SKIP_NEXT
               	  END           
