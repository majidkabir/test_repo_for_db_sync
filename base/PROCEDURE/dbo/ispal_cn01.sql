...
GO
/************************************************************************/    
/* Stored Procedure: ispAL_CN01                                         */    
/* Creation Date: 28-mar-2018                                           */    
/* Copyright: LF                                                        */    
/* Written by:                                                          */    
/*                                                                      */    
/* Purpose: WMS-4424 - CN Costco allocation by FIFO                     */
 ...
 END    
   ...
   SELECT @c_SQL =   
            ...
            + CASE WHEN @c_UOM = '2' THEN  ' AND (LOTxLOCxID.QTY - LOTxLOCxID.QtyAllocated - LOTxLOCxID.QTYPicked - LOTxLOCxID.QtyReplen) >= @n_CaseCnt '   
              ELSE '' END                
            +' ORDER BY '
            + CASE WHEN @c_UOM = '7' THEN  ' (LOTxLOCxID.QTY - LOTxLOCxID.QtyAllocated - LOTxLOCxID.QTYPicked - LOTxLOCxID.QtyReplen) % CASE WHEN @n_CaseCnt = 0 THEN 1 ELSE @n_CaseCnt END , ' ELSE '' END  
            +        ' LOTATTRIBUTE.Lottable05, QtyAvailable, LOC.LogicalLocation, LOC.LOC '
