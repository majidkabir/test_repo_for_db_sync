...
/************************************************************************/
/* Stored Procedure: isp_Print_Putaway13    				                  */
...
CREATE PROC [dbo].[isp_Print_Putaway13] (
 ...
 BEGIN
    ...
    CREATE TABLE #TEMP_SKU (Storerkey NVARCHAR(15) NULL, SKU NVARCHAR(20) NULL, DESCR NVARCHAR(60) NULL, 
                            ...
                            COMBINESKU NVARCHAR(100) NULL, SKUGROUP NVARCHAR(10)) 
    ...
    BEGIN
       ...
       BEGIN
          ...
          SET @c_InsertSelect = ' INSERT INTO #TEMP_SKU SELECT DISTINCT SKU.Storerkey, SKU.Sku, SKU.Descr, SKU.Packkey, SKU.Busr6, SKU.IVAS, SKU.RetailSku, SKU.ManufacturerSku, SKU.ShelfLife ,SKU.SKUGROUP'
       ...
       BEGIN
          ...
          SELECT DISTINCT SKU.Storerkey, SKU.Sku, SKU.Descr, SKU.Packkey, SKU.Busr6, SKU.IVAS, SKU.RetailSku, SKU.ManufacturerSku, SKU.ShelfLife, SKU.Sku, SKU.SKUGROUP  
          FROM SKU (NOLOCK)
          JOIN RECEIPTDETAIL RD (NOLOCK) ON SKU.Storerkey = RD.Storerkey AND SKU.Sku = RD.Sku
          JOIN RECEIPT R (NOLOCK) ON RD.Receiptkey = R.Receiptkey
    ...
    SELECT DISTINCT Storerkey, SKU, DESCR, Packkey, Busr6, IVAS, RetailSku, ManufacturerSku, ShelfLife, COMBINESKU, SKUGROUP   
    INTO #TEMP_SKU2
    FROM #TEMP_SKU
