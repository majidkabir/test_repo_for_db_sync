...
GO
/*  TLTING 2008/7/12  cahnge Storerkey value '66001' */

CREATE PROC [dbo].[nspImportData_tri]
 ...
 IF @c_modulename = 'Order'
 	BEGIN
 		...
 		ON uploadorderdetail.storerkey = sku.storerkey
 			...
 			AND uploadorderdetail.storerkey = '66001'
 		WHERE sku.sku = NULL
...
/*
                	...
                	c_city, c_state,c_zip,buyerpo,notes,invoiceno,notes2,pmtterm,ROUTE,Facility,ConsigneeKey)
         			SELECT @c_Orderkey,@c_externOrderkey,uploadorderheader.Storerkey, Orderdate, Deliverydate, 
           			Priority,STORER.contact1, STORER.contact2, STORER.company, STORER.address1, 
           			STORER.address2, STORER.address3, STORER.address4, STORER.city, STORER.state, STORER.zip,
           			buyerpo, notes,invoiceno,Uploadorderheader.notes2,pmtterm,Route=isnull(route,'77'),STORER.Facility,Consigneekey
...
*/
                ...
                WHERE ExternOrderKey = @c_externOrderkey and OrderGROUP = @c_Ordergroup
                AND MODE = @c_mode  AND OrderKey = @c_Orderkey AND UPLOADOrderHeader.STATUS = '0' 
                AND STORER.STORERKEY=UPLOADOrderHeader.ConsigneeKey 
 					...
 					BEGIN --insert successful, update the status = '9'
 						...
 						begin
 							...
 							INSERT INTO OrderDETAIL ( Orderkey, Orderlinenumber, Storerkey, ExternOrderkey, ExternLineno, 
 								...
 								UD.UnitPrice,
 								ISNULL (UD.Facility, ' ')
 							FROM UPLOADOrderDETAIL UD (nolock) JOIN SKU (nolock)
 							ON UD.SKU = SKU.SKU
 								AND UD.Storerkey = SKU.Storerkey
 							JOIN pack (nolock)
