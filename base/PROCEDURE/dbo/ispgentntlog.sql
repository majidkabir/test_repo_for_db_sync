...
BEGIN
   ...
   BEGIN
      IF NOT EXISTS ( SELECT 1 FROM TNTLog (NOLOCK) 
                      WHERE TableName = @c_TableName
                      AND Key1 = @c_Key1 AND Key2 = @c_Key2 AND Key3 = @c_Key3)
      BEGIN
         INSERT INTO TNTLog (tablename, key1, key2, key3, transmitflag, TransmitBatch)
         ...
         BEGIN
            ...
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_Err,0)) 
                          + ': Insert into TNTLog Failed (ispGenTNTLog)' 
            GOTO QUIT 
         END
      END
   END
   ...
   BEGIN
      ...
      BEGIN
         ...
         END
      END
      execute nsp_logerror @n_err, @c_errmsg, "ispGenTNTLog"
