...
/************************************************************************/
/* Store Procedure:  nsp_PrintCaseLabel_WTC                           	*/
...
/*                                                                      */
/*                                                                      */
/* Called By:  dw = r_dw_caselabel_wtc             					      */
/*                                                                      */
/* PVCS Version: 1.2                                                    */
/*                                                                      */
/* Version: 5.4                                                         */
...
CREATE PROC [dbo].[nsp_PrintCaseLabel_WTC] (@c_refkey NVARCHAR(20), @c_type NVARCHAR(2)) 
AS
BEGIN
   ...
   CREATE TABLE #TEMPRESULT (
	...
	BEGIN
            ...
            BEGIN
               ...
               SELECT @c_errmsg = CONVERT(CHAR(250),@n_err), @n_err = 63100
            	SELECT @c_errmsg = 'NSQL'+CONVERT(char(5),@n_err)+': CaseCnt for Packkey ' + @c_PackKey + ' is zero. ' + 
                                  ' (nsp_PrintCaseLabel_WTC)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + ' ) '
               GOTO EXIT_SP
            END 
            ...
            BEGIN
               ...
               BEGIN
                  ...
                  BEGIN
                     ...
                     SELECT @c_NewCaseID = 'C'+ RIGHT (REPLICATE('0',10) + dbo.fnc_RTrim(CONVERT (char(10), @n_CaseID)),9)
      ...
      IF dbo.fnc_RTrim(@c_type) = 'R' 
   	BEGIN 
	      DECLARE RESULT_CUR CURSOR LOCAL FAST_FORWARD READ_ONLY FOR         
            ...
            AND   PD.UOM = '2'
            AND   PD.CaseID <> ''
            ORDER BY PD.PickSlipNo, StorerSODefault.XDockLane, PD.CaseID
      END
      ELSE -- 'L'
      BEGIN
	      DECLARE RESULT_CUR CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
      ...
      SELECT @c_ResPrevPickSlipNo = @c_ResPickSlipNo
   	...
   	BEGIN
         ...
         BEGIN
            ...
            INSERT INTO #TEMPRESULT (XDockLane, SellerName, ExternPOKey, Sku, SkuDescr, Qty, PickSlipNo, CaseID)
            VALUES ('*', '', '', '', '',0, '', '')            
         END
      ...
      BEGIN
         WHILE @@TRANCOUNT > @n_starttcnt
         ROLLBACK TRAN
         EXECUTE nsp_logerror @n_err, @c_errmsg, 'nsp_PrintCaseLabel_WTC'
