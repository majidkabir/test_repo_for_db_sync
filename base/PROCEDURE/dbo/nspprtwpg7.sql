...
/* 07-05-2014   Audrey  1.4   SOS#310562 - Get next available Lot.      */
/* 22-09-2021   WLChooi 1.5   DEVOPS Combine Script                     */
/* 22-09-2021   WLChooi 1.6   WMS-18018 - Filter LocationCategory based */
/*                            on Codelkup (WL01)                        */
/************************************************************************/
CREATE PROC [dbo].[nspPRTWPG7] (   
...
BEGIN
   ...
   BEGIN
      ...
      SELECT @n_ConsigneeMinShelfLife = ((ISNULL(Sku.Shelflife,0) * @n_ConsigneeMinShelfLife /100) * -1),
             @n_PalletCnt = PACK.Pallet,
             @c_Pallet = CAST(PACK.Pallet AS VARCHAR(10))
      FROM  Sku (NOLOCK)
   ...
   BEGIN
      ...
      BEGIN
         ...
         IF @n_QtyAvailable < @n_PalletCnt
            BREAK
         -- SOS#310562 (End)

         SELECT @c_LOC = LLI.Loc,
                @c_ID  = LLI.Id,
                @n_Qty = LLI.Qty - LLI.QtyAllocated - LLI.QtyPicked
         ...
         JOIN LOT WITH (NOLOCK) ON LLI.Lot = LOT.Lot -- SOS#299047
         ...
         AND ((LLI.Qty - LLI.QtyAllocated - LLI.QtyPicked ) = @n_QtyLeftToFulfill
          OR ((LLI.Qty - LLI.QtyAllocated - LLI.QtyPicked ) < @n_QtyLeftToFulfill) ) -- SOS#299047
         ...
         IF @n_LotQtyAvail < @n_QtyLeftToFulfill
            SET @n_Qty = 0

         IF @n_Qty > 0
         BEGIN
            --PRINT 'Qty = QtyLeftToFulfill OR Qty < QtyLeftToFulfill, Qty=' + CAST(@n_Qty AS VARCHAR) + ' QtyLeftToFulfill:' + CAST(@n_QtyLeftToFulfill AS VARCHAR)
         ...
         ELSE
         BEGIN
            IF @n_QtyLeftToFulfill < @n_PalletCnt
               BREAK
         END
         ...
         BEGIN
            ...
            WHERE LLI.Lot = @c_SelectedLOT
            AND NOT EXISTS(SELECT 1 FROM @t_LOTXLOCXID TB WHERE LLI.Lot = TB.LOT AND LLI.Loc = TB.LOC AND LLI.Id = TB.ID)
            AND (LLI.Qty - LLI.QtyAllocated - LLI.QtyPicked ) = @n_PalletCnt
            ORDER BY LLI.Qty, L.LogicalLocation, L.LOC
     ...
     END
         ...
         --select @c_SelectedLOT'@c_SelectedLOT', @d_SelectedLot4'@d_SelectedLot4', @n_QtyAvailable'@n_QtyAvailable',@n_QtyLeftToFulfill'@n_QtyLeftToFulfill'--ang01
