...
/* 20-NOV-2019 CSCHONG  1.4   WMS-10204 - fix channel issue (CS01b)     */   
/************************************************************************/        
CREATE PROC [dbo].[ispPRNIK02]            
...
BEGIN        
   ...
   BEGIN       
      ...
      --              + ': Invalid Wave Piece Pick Task Dispatch Method. Must Be INLINE (ispPRNIK02)'    
      GOTO QUIT_SP    
   END                         
   ...
   BEGIN    
      SELECT 'ispPRNIK02'    
      SELECT * FROM #ORDERLINES WITH (NOLOCK) --where sku in('928484010M')   
      SELECT 'Get #CHANNELINFO table '    
      SELECT * FROM #CHANNELINFO WITH (NOLOCK)  --where sku in('928484010M')  --CS01a    
   END    
   ...
   FETCH NEXT FROM CUR_ORDERLINES INTO @c_StorerKey, @c_SKU, @c_Facility, @c_Packkey       
                                    ...
                                    ,  @c_Lottable11, @c_Lottable12,@c_Channel,@c_ChannelInvMgmt                  --CS01a    
              
   WHILE (@@FETCH_STATUS <> -1)              
   BEGIN     
    
      IF @b_Debug = 1 --and @c_sku = '928484010M'  
      BEGIN    
         PRINT '--------------------------------------------' + CHAR(13) +    
               '  @c_SKU: ' +@c_SKU + CHAR(13) +     
  ...
  --   SELECT 'UCC',*  
  --from UCC (nolock)  
  ----where uccno ='00000000002003306905'  
  -- END  
...
+ CHAR(13) +  'AND UCC.CTNCount - CEILING(LOTxLOCxID.QTYALLOCATED/(UCC.Qty * 1.0)) > 0 '    
   ...
   IF @b_Debug = 1    
   BEGIN   
      SELECT @c_Facility '@c_Facility',@c_SKU '@c_SKU',@c_Lottable01 '@c_Lottable01',@c_Lottable02 '@c_Lottable02',@c_Lottable07 '@c_Lottable07',  
  @c_LocationType '@c_LocationType',@c_LocationCategory '@c_LocationCategory', @c_LocationHandling '@c_LocationHandling'   
     PRINT @c_SQL    
  SELECT 'GET #UCCxLOTxLOCxID'  
 -- SELECT '#UCCxLOTxLOCxID',* from #UCCxLOTxLOCxID --where sku = @c_SKU  
   END          
   ...
   IF @b_Debug='1'  
   BEGIN  
     SELECT '#NumPool', @c_SKU '@c_sku'  
  select * from #NumPool  
   END  
    
      --Get Lower Bound to reduce loop size    
      SET @n_LowerBound = 0    
      SELECT @n_LowerBound = ISNULL(MIN(UCCQty),0)     --CS01e     
      FROM #NumPool    
      WHERE AvailCTNCount > 0    
     
   IF @b_Debug='1'  
   BEGIN  
     SELECT @c_SKU '@c_sku',@n_LowerBound '@n_LowerBound'  
   END   
    
      IF @n_LowerBound = 0          BEGIN    
         GOTO NEXT_ORDERLINES    
      END    
      /*****************************************************************************/    
      /***  START PRE-ALLOC UCC (Get Combination of pool numbers for orderQty)   ***/    
      /*****************************************************************************/    
      SET @c_SQL = N'DECLARE CUR_ORDERLINE_SKU CURSOR FAST_FORWARD READ_ONLY FOR '      
      + CHAR(13) + 'SELECT Orderkey, OrderQty '           
      + CHAR(13) + 'FROM #ORDERLINES WITH (NOLOCK) '       
      + CHAR(13) + 'WHERE OrderQty >= @n_LowerBound '      
      + CHAR(13) + 'AND StorerKey = @c_StorerKey '     
      + CHAR(13) + 'AND SKU = @c_SKU '                     
      + CHAR(13) + 'AND Facility  = @c_Facility '       
      + CHAR(13) + 'AND Packkey   = @c_Packkey '       
      + CHAR(13) + 'AND Lottable01 = @c_Lottable01 '            
      + CHAR(13) + 'AND Lottable02 = @c_Lottable02 '            
      + CHAR(13) + 'AND Lottable03 = @c_Lottable03 '        
      + CHAR(13) + 'AND Lottable06 = @c_Lottable06 '        
      + CHAR(13) + 'AND Lottable07 = @c_Lottable07 '        
      + CHAR(13) + 'AND Lottable08 = @c_Lottable08 '        
      + CHAR(13) + 'AND Lottable09 = @c_Lottable09 '       
      + CHAR(13) + 'AND Lottable10 = @c_Lottable10 '        
      + CHAR(13) + 'AND Lottable11 = @c_Lottable11 '        
      + CHAR(13) + 'AND Lottable12 = @c_Lottable12 '           
      + CHAR(13) + 'ORDER BY Orderkey'            
     
      SET @c_SQLParm =  N'@n_LowerBound INT,@c_StorerKey NVARCHAR(15), @c_SKU NVARCHAR(20), @c_Facility NVARCHAR(5), @c_Packkey NVARCHAR(10) '          
                     +  ',@c_Lottable01 NVARCHAR(18), @c_Lottable02 NVARCHAR(18), @c_Lottable03 NVARCHAR(18) '       
                     +  ',@c_Lottable06 NVARCHAR(30), @c_Lottable07 NVARCHAR(30), @c_Lottable08 NVARCHAR(30) '       
                     +  ',@c_Lottable09 NVARCHAR(30), @c_Lottable10 NVARCHAR(30), @c_Lottable11 NVARCHAR(30), @c_Lottable12 NVARCHAR(30) '     
             
      EXEC sp_ExecuteSQL @c_SQL, @c_SQLParm, @n_LowerBound, @c_StorerKey, @c_SKU, @c_Facility, @c_Packkey    
                        ,@c_Lottable01, @c_Lottable02, @c_Lottable03    
                        ,@c_Lottable06, @c_Lottable07, @c_Lottable08    
                        ,@c_Lottable09, @c_Lottable10, @c_Lottable11, @c_Lottable12    
      ...
      BEGIN    
         ...
         IF @b_Debug = 1 --and @c_SKU = '928484010M'  
         BEGIN  
      SELECT 'CHK #UCCxLOTxLOCxID'    
            SELECT * from #UCCxLOTxLOCxID --where lot in('0108769128')  and Loc='NPC133631'   
            PRINT 'Orderkey: ' + @c_Orderkey + ', OrderQty: ' + CAST(@n_OrderQty AS NVARCHAR) + ',SKU : ' +  @c_SKU  
   ...
   CLOSE CUR_COMBINATION    
      ...
      SELECT @n_UCCQty '@n_UCCQty',UCC_RowRef    
                     ,UCCNo    
   ...
   IF @b_Debug ='1'    
      BEGIN  
     SELECT @n_CTNQty '@n_CTNQty'  
     SELECT 'GGG',* from #CHANNELINFO where channel ='SYHH' and sku = '928484010M'  
  select * from orderdetail (nolock) where orderkey = '1501689139' and sku = '928484010M'  
   END    
      SET @c_SQL =N'DECLARE CUR_ORDLINE CURSOR FAST_FORWARD READ_ONLY FOR '     
                  + CHAR(13) + 'SELECT OD.Orderkey, OD.OrderLineNumber, '--OD.OpenQty - ( OD.QtyAllocated + OD.QtyPreAllocated + OD.QtyPicked ) AS Qty '    
                  ...
                  + CHAR(13) + 'AND OD.OpenQty - ( OD.QtyAllocated + OD.QtyPreAllocated + OD.QtyPicked ) >= 0 '                   --CS01c  
     ...
     -- + CHAR(13) + ' ORDER BY CASE WHEN OD.OpenQty - ( OD.QtyAllocated + OD.QtyPreAllocated + OD.QtyPicked ) <= @n_CTNQty THEN 1 ELSE 0 END'    
      ...
      END  --CS01a END    
                  ...
                  SET @c_SQLParm =  N'@c_Orderkey NVARCHAR(10),@c_StorerKey NVARCHAR(15), @c_SKU NVARCHAR(20), @c_Packkey NVARCHAR(10)'         
                                 + ', @c_Lottable01 NVARCHAR(18), @c_Lottable02 NVARCHAR(18), @c_Lottable03 NVARCHAR(18)'      
                                 + ', @c_Lottable06 NVARCHAR(30), @c_Lottable07 NVARCHAR(30), @c_Lottable08 NVARCHAR(30)'      
                                 + ', @c_Lottable09 NVARCHAR(30), @c_Lottable10 NVARCHAR(30), @c_Lottable11 NVARCHAR(30)'     
                                 + ', @c_Lottable12 NVARCHAR(30) , @n_CTNQty INT '                   --CS01a    
                      
                  EXEC sp_ExecuteSQL @c_SQL, @c_SQLParm, @c_Orderkey, @c_StorerKey ,@c_SKU, @c_Packkey    
                                    , @c_Lottable01, @c_Lottable02, @c_Lottable03    
  , @c_Lottable06, @c_Lottable07, @c_Lottable08    
                                    , @c_Lottable09, @c_Lottable10, @c_Lottable11, @c_Lottable12,@n_CTNQty        --CS01a    
       
                  OPEN CUR_ORDLINE                   
                  FETCH NEXT FROM CUR_ORDLINE INTO @c_Orderkey, @c_OrderLineNumber, @n_OrderLineQty,@c_channel,@n_Channel_id     
    ...
    END        
     ...
     BEGIN    
       --CS01b START  
    --if @n_OrderLineQty <> @n_CTNQty --OR @n_OrderLineQty > @n_CHANNELAvaiQty   --CS01e  
    --BEGIN  
    ...
    IF @b_Debug = 1        
          BEGIN  
      SELECT 'B4',@c_Orderkey '@c_Orderkey', @n_InsertQty '@n_InsertQty',@n_CHANNELAvaiQty '@n_ChannelqtyAvail',@c_SKU '@c_SKU'  
    END   
    ...
    IF @b_Debug = '1'  
    BEGIN  
      SELECT 'before insert pickdetail ', @n_CHANNELAvaiQty '@n_CHANNELAvaiQty'  
    END    
    ...
    BEGIN   
          ...
          BEGIN    
             ...
             SET @n_Err = 61010    
             SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0))       
                            + ': Get PickDetailKey Failed. (ispPRNIK02)'    
             GOTO QUIT_SP    
          END    
          ELSE    
          BEGIN    
       ...
       'channel: ' + @c_channel + CHAR(13) +   
       'Channel_id : ' + CAST(@n_channel_id as nvarchar(10)) + CHAR(13) +  
       'CHANNELAvaiQty : ' + CAST(@n_CHANNELAvaiQty as nvarchar(10)) + CHAR(13)  
         
       SELECT 'ChannelInv',* FROM ChannelInv AS ci WITH(NOLOCK)    
                      WHERE ci.Channel_ID = @n_channel_id and ci.sku = @c_SKU  
  
       select channel,openqty,qtyallocated,qtypicked,* from orderdetail (nolock)  
       where orderkey = @c_OrderKey  
       and sku = @c_SKU  
             END    
             ...
             BEGIN    
                ...
                SET @n_Err = 61020    
                SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0))     
                               + ': Insert PickDetail Failed. (ispPRNIK02)'    
                GOTO QUIT_SP    
             END    
...
QUIT_SP:    
   ...
   BEGIN      
      ...
      BEGIN      
         ...
         END      
      END      
      EXECUTE nsp_logerror @n_Err, @c_ErrMsg, 'ispPRNIK02'      
