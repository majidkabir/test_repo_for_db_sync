...
/************************************************************************/
/* Store Procedure:  nsp_GetPickSlipOrders71                            */
...
/* Return Status:  None                                                 */
/*                                                                      */
/* Usage:  Used for report dw = r_dw_print_pickorder71                  */
/*         copy from r_dw_print_pickorder06                             */
/*                                                                      */
/* Local Variables:                                                     */
/*                                                                      */
/* Called By:                                                           */
...
/* 18-JUL-2018  CSCHONG  1.0  WMS-5541 add new field (CS01)             */
...
BEGIN
   ...
   FETCH NEXT FROM pick_cur INTO @c_sku, @c_loc, @n_Qty, @n_uom3, @c_storerkey,
      @c_orderkey,  @c_UOM, @c_logicalloc, @c_lot,@c_PDETID
      ...
      BEGIN
         ...
         SELECT @c_SkuDesc = IsNULL(Descr,'')
               ...
               ,@c_Measurement = ISNULL(RTRIM(Measurement),'')
					,@n_PackQtyIndicator = PackQtyIndicator                   --CS01    
         FROM   SKU  (NOLOCK)
