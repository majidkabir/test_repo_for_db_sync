...
GO
/************************************************************************/
/* Stored Procedure: nspPR_HK08                                         */
/* Creation Date: 06-Mar-2007                                           */
/* Copyright: IDS                                                       */
/* Written by: MaryVong                                                 */
/*                                                                      */
/* Purpose: Modify from nspPR_HK05                                      */
/*          Sort by Lottable04 Asc, Lottable02 Desc, Lottable05 Asc     */
/*                                                                      */
/* Called By:                                                           */
/*                                                                      */
/* PVCS Version: 1.1                                                    */
...
BEGIN
   ...
   IF @b_debug = 1
   BEGIN
      SELECT "nspPR_HK08 : Before Lot Lookup ....."
   ...
   BEGIN
      ...
      ORDER BY LOTATTRIBUTE.Lottable04, LOTATTRIBUTE.Lottable02 DESC, LOTATTRIBUTE.LOTTABLE05, LOC.LogicalLocation
   END
   ELSE
   BEGIN
      ...
      SELECT @c_SortOrder = ''
      IF (@c_Lottable04Label IS NOT NULL AND @c_Lottable04Label <> '')
      BEGIN
         SELECT @c_SortOrder = " ORDER BY lotATTRIBUTE.LOTTABLE04, LOTATTRIBUTE.Lottable02 DESC, LOTATTRIBUTE.LOTTABLE05, MIN(LOC.LogicalLocation) "
      END
      ELSE
      BEGIN
         SELECT @c_SortOrder = " ORDER BY LOTATTRIBUTE.Lottable02 DESC, LOTATTRIBUTE.LOTTABLE05, MIN(LOC.LogicalLocation) "
      END
