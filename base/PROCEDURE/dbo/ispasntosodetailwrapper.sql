...
GO
/***************************************************************************/
/* Store Procedure: ispASNToSODetailWrapper                                */
/* Creation Date: 27-AUG-2014                                              */
/* Copyright: LF                                                           */
/* Written by: YTWan                                                       */
/*                                                                         */
/* Purpose: Populate to Orders Mapping                                     */
/*        : SOS#314650 - [TW] 20784 - Populate Multiple Return to Order    */
/*                                                                         */
/* Called By: n_cst_orderdetail.of_colmapping                              */
/*                                                                         */
/* PVCS Version: 1.0                                                       */
/*                                                                         */
/* Version: 5.4                                                            */
...
/***************************************************************************/
CREATE PROC [dbo].[ispASNToSODetailWrapper]
...
BEGIN 
   ...
   BEGIN
      ...
      SET @c_ToTable        = 'ORDERS'
      IF @c_SourceTable = 'RECEIPT'
      BEGIN
         SET @c_Where = ' WHERE RECEIPT.ReceiptKey = N''' + @c_SourceKey + ''' '
      END
      ELSE IF @c_SourceTable = 'ORDERS'
      BEGIN
         SET @c_Where = ' WHERE ORDERS.Orderkey = N''' + @c_SourceKey + ''' '
      END
   END
   ELSE
   BEGIN
      SET @c_TableAttribute = 'D'
      SET @c_ToTable        = 'ORDERDETAIL'
