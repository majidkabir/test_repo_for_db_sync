...
/************************************************************************/
/* Stored Procedure: nspALLOT3C                                         */
/* Creation Date: 02-Aug-2008                                           */
/* Copyright: IDS                                                       */
/* Written by: YokeBeen                                                 */
/*                                                                      */
/* Purpose: Duplicated from mspALIDS02.											*/
...
BEGIN 
   ...
   BEGIN
      SELECT @c_OrderKey = LEFT(ISNULL(dbo.fnc_RTrim(dbo.fnc_LTrim(@c_OtherParms)),''), 10)
      SELECT @c_OrderLineNumber = SUBSTRING(ISNULL(dbo.fnc_RTrim(dbo.fnc_LTrim(@c_OtherParms)),''), 11, 5)
   END
   ...
   END
	...
	SELECT @c_SQLStatement = 
          ...
          ' AND LOC.Status <> ''HOLD'' ' + 
          ' AND LOC.HostWhCode = N''' + ISNULL(dbo.fnc_RTrim(@c_HostWHCode),'') + '''' +  
