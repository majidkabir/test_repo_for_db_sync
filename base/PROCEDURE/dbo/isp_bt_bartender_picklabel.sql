...
/* 2018-10-28 1.0  FayLiuHY   Change to map RetailSKU (Fy01)                  */           
/******************************************************************************/                      
                        
CREATE PROC [dbo].[isp_BT_Bartender_PickLabel]                             
...
-- SET RowNo = 0                   
  ...
  SET @c_style = ''      
   ...
   END            
      ...
      SELECT @n_TTLPickQTY = SUM(pd.qty)--,s.altsku      
  FROM PiCKDETAIL pd (nolock)      
  join sku s with (nolock) on s.storerkey = pd.storerkey and s.sku = pd.sku      
  join orders ord (nolock) on ord.orderkey = pd.orderkey      
  where ord.loadkey = @c_Sparm5      
      
  SELECT @n_ORIQTY = SUM(od.originalqty)      
  FROM orderdetail od WITH (NOLOCK)      
  WHERE od.orderkey = @c_Sparm2      
      
  SELECT @c_altsku = S.Retailsku         -- Fy01    
        ,@c_style = S.Style      
  FROM SKU S WITH (NOLOCK)      
  WHERE S.storerkey = @c_Sparm7      
  AND S.sku = @c_Sparm3      
       ...
       VALUES (@c_Sparm4,@c_altsku,@c_style,@c_Sparm6,@c_Sparm1,@c_Sparm5,@c_Sparm2,CAST(@n_TTLPickQTY as nvarchar(50)),      
          CAST(@n_ORIQTY as NVARCHAR(50)),'',      
      '','','','','','','','','','',      
      '','','','','','','','','','',      
      '','','','','','','','','','',      
      '','','','','','','','','','',      
...
EXIT_SP:          
   ...
   EXEC isp_InsertTraceInfo         
      ...
      @c_TraceName = 'isp_BT_Bartender_PickLabel',        
