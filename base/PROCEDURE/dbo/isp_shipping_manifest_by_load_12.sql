...
/*************************************************************************/
/* Stored Procedure: isp_shipping_manifest_by_load_12                    */
/* Creation Date: 2018-07-09                                             */
/* Copyright: IDS                                                        */
/* Written by:                                                           */
/*                                                                       */
/* Purpose:WMS-4973 CN - CN_NIVEA_DeliveryNote_New                       */
/*                                                                       */
...
/* 08-Aug-2019  WLChooi 1.1   WMS-10175 - New input param (optional),    */
/*                            QR Code and sorting sequence (WL01)        */
/* 15-Oct-2019  WLChooi 1.2   WMS-9975 - Fixed bug (WL02)                */
/* 16-Oct-2019  WLChooi 1.3   Group same SKU together, sort by           */ 
/*                            Externorderkey, Fix no result if different */
/*                            storerkey (WL03)                           */
/* 11-Nov-2019  mingle01 1.4  add mapping facility(mingle01)             */
/* 23-Jul-2019  WLChooi  1.5  WMS-14396 - Add new logic (WL04)           */
/* 10-SEP-2020  CSCHONG 1.2   WMS-14938 add print new report (CS01)      */ 
/*************************************************************************/
CREATE PROC [dbo].[isp_shipping_manifest_by_load_12]
...
BEGIN
  ...
  CREATE TABLE #TMP_LoadOH12(
   ...
   CREATE TABLE #TMP_SMBLOAD12 (
          ...
          OrdLineNo       NVARCHAR(5)  NULL,    --WL01
          facility        NVARCHAR(50) NULL,    --mingle01
          SSUSR1          NVARCHAR(20) NULL,    --CS01
          SSUSR2          NVARCHAR(20) NULL,    --CS01
          SSUSR3          NVARCHAR(20) NULL,    --CS01
          SSUSR4          NVARCHAR(20) NULL     --CS01  
          )   

   IF @c_Orderkey = NULL SET @c_Orderkey = ''   --WL01
   ...
   BEGIN
      ...
      BEGIN
         INSERT INTO #TMP_LoadOH12 (storerkey, loadkey, Orderkey,ExtOrdKey)
     ...
     (
      ...
      SELECT orders.orderkey,orders.loadkey,orders.c_company,
             ...
             CASE WHEN L.locationtype<>'PICK' THEN SUM(pickdetail.qty) ELSE 0 END % CAST(pack.casecnt as int) +
                                                   ...
                                                   cast(Sum(pickdetail.qty)*sku.StdGrossWgt as decimal(20,4)) as STDNETWGT,    --WL02 Fix conversion issue
             ...
             CASE WHEN Orders.facility ='BTS01' THEN N'上海配送中心' 
                  ...
                  WHEN Orders.facility ='647'   THEN N'北京配送中心' 
                  WHEN Orders.facility ='CD01'  THEN N'成都配送中心' 
                  WHEN Orders.facility ='WH01'  THEN N'武汉配送中心' 
                  ELSE '' END  + ' (' + ISNULL(CL.Short,'') + ')' AS facility    --mingle01   --WL04
             ,ISNULL(sku.susr1,''), ISNULL(sku.SUSR2,''),ISNULL(sku.SUSR3,''),ISNULL(sku.SUSR4,'') --CS01
      ...
      OUTER APPLY (SELECT TOP 1 CL.Long FROM CODELKUP CL WITH (NOLOCK)                        --WL01
                   WHERE orders.C_State = CL.Code AND CL.listname='NVREGION'                  --WL01
                   ORDER BY CASE WHEN CL.Storerkey = ORDERS.Storerkey THEN 1 ELSE 2 END) AS C --WL01            
      LEFT JOIN CODELKUP CL WITH (NOLOCK) ON CL.LISTNAME = 'NIVPODRPT' AND CL.Storerkey = ORDERS.StorerKey AND CL.Code = ORDERS.StorerKey   --WL04                               
      ...
      /*Group by orders.orderkey,orders.loadkey,orders.c_company,
               ...
               CASE WHEN Orders.facility ='BTS01' THEN N'上海配送中心' 
               WHEN Orders.facility ='PY06' THEN  N'广州配送中心' WHEN Orders.facility ='647' THEN N'北京配送中心'
               WHEN Orders.facility ='CD01' THEN N'成都配送中心' WHEN Orders.facility ='WH01' THEN N'武汉配送中心' ELSE '' END   --mingle01 */
      GROUP BY orders.orderkey,orders.loadkey,orders.c_company,
               CASE WHEN LEN(Orderdetail.SKU)=12 THEN LEFT(LTRIM(RTRIM(Orderdetail.Sku)),5)+'-'+SUBSTRING(LTRIM(RTRIM(orderdetail.SKU)),6,5)
                         +'-'+RIGHT(LTRIM(RTRIM(Orderdetail.SKU)),2) ELSE Orderdetail.Sku END,         
               RTRIM(sku.DESCR),
               ...
               CASE WHEN Orders.facility ='BTS01' THEN N'上海配送中心' 
                    ...
                    WHEN Orders.facility ='CD01'  THEN N'成都配送中心' 
                    WHEN Orders.facility ='WH01'  THEN N'武汉配送中心' 
                    ELSE '' END  + ' (' + ISNULL(CL.Short,'') + ')'   --mingle01   --WL04
               ,ISNULL(sku.susr1,''), ISNULL(sku.SUSR2,''),ISNULL(sku.SUSR3,''),ISNULL(sku.SUSR4,'')     --CS01  
   ...
   --       ST_Notes2,
   --       c_contact1
   --FROM   #TMP_SMBLOAD12 AS ts   --WL01
   ----WHERE ts.loadkey = @c_loadkey
   ----ORDER BY ts.loadkey,ts.Orderkey,ts.ExtOrdKey,ts.SKU         --WL01
   --ORDER BY ts.loadkey,ts.Orderkey,ts.ExtOrdKey,ts.OrdLineNo     --WL01
   --CS01 START
IF @c_RPTTYPE = '1'
BEGIN
   --WL03 Start
