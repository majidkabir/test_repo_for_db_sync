...
GO
/************************************************************************/
/* Store Procedure:  nspConsoPickList11                                 */
...
CREATE PROC [dbo].[nspConsoPickList11] (@a_s_LoadKey NVARCHAR(10) )
 AS
 BEGIN
   SET NOCOUNT ON 
   SET ANSI_NULLS OFF
...
--	IF dbo.fnc_RTrim(@c_PickHeaderKey) IS NULL OR dbo.fnc_RTrim(@c_PickHeaderKey) = '' 
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				SELECT @n_continue = 3
				SELECT @n_err = 63501
				SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Insert Into PICKHEADER Failed. (nspConsoPickList11)"
			END
   ...
   BEGIN
		SELECT @n_continue = 3
		SELECT @n_err = 63501
		SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Get LoadKey Failed. (nspConsoPickList11)"
   END
...
--         ORDERDETAIL.OpenQty AS OrderQty,   /*BEGIN added by Ong sos35102 050505 */
--          ( SELECT OpenQty
--            FROM ORDERDETAIL (NOLOCK)
--            WHERE ORDERDETAIL.Loadkey = LOADPLANDETAIL.LoadKey ) AS OrderQty, 
...
-- 			END
-- 		END
		execute nsp_logerror @n_err, @c_errmsg, "nspConsoPickList11"
