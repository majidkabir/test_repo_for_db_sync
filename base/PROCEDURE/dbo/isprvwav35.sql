...
GO
/*************************************************************************/  
/* Stored Procedure: ispRVWAV35                                          */  
/* Creation Date: 10-SEP-2020                                            */  
/* Copyright: LFL                                                        */  
/* Written by:                                                           */  
/*                                                                       */  
/* Purpose: WMS-14747 - KR Levis reverse release task                    */
/*                                                                       */  
/* Called By: wave                                                       */  
/*                                                                       */  
/* PVCS Version: 1.1                                                     */  
...
/*************************************************************************/   
CREATE PROCEDURE [dbo].[ispRVWAV35]        
 ...
 BEGIN    
    ...
    BEGIN  
        IF NOT EXISTS (SELECT 1 FROM REPLENISHMENT RP (NOLOCK)   
                       WHERE RP.Wavekey = @c_Wavekey AND RP.OriginalFromLoc = 'ispRLWAV35')  
        BEGIN                                            
          SELECT @n_continue = 3    
          SELECT @n_err = 81010    
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Nothing to reverse. (ispRVWAV35)'           
        END                   
    END  
    ...
    BEGIN  
        ...
        BEGIN  
          SELECT @n_continue = 3    
          SELECT @n_err = 81020    
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Some Tasks have been started. Not allow to Reverse Wave Released (ispRVWAV35)'         
        END                   
    END  
    ...
    BEGIN  
       DECLARE CUR_REPL CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
         SELECT Replenishmentkey, RefNo --DropID
         FROM REPLENISHMENT (NOLOCK)
         WHERE Wavekey = @c_Wavekey
         AND OriginalFromLoc = 'ispRLWAV35'
         AND Confirmed = 'N'

       OPEN CUR_REPL  
         
       FETCH NEXT FROM CUR_REPL INTO @c_Replenishmentkey, @c_DropID
       ...
       BEGIN
          ...
          BEGIN          	
             ...
             BEGIN  
               ...
               SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Pickdetail Table Failed. Pickdetailkey:' + RTRIM(@c_Pickdetailkey)+ ' (ispRVWAV35)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
             END   
          ...
          BEGIN  
            ...
            SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Delete Replenishment Table Failed. (ispRVWAV35)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
          END   
    ...
    BEGIN
    	 ...
    	 AND Status <> '0'
    	 AND DeviceID IN (SELECT Station FROM RDT.rdtPTLStationLog (NOLOCK) WHERE Wavekey = @c_Wavekey AND Storerkey = @c_Storerkey)   
       ...
       BEGIN  
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update DeviceProfile Table Failed. (ispRVWAV35)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
       END   
    	 
    	 DELETE FROM RDT.rdtPTLStationLog
    	 WHERE  Wavekey = @c_Wavekey AND Storerkey = @c_Storerkey    	 
       ...
       BEGIN  
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Delete rdtPTLStationLog Table Failed. (ispRVWAV35)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
       END   
    END    	           
    ...
    BEGIN
       ...
       BEGIN  
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Delete PACKTASK Table Failed. (ispRVWAV35)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
       END   
    END         
    ...
    BEGIN              
       ...
       BEGIN    
          ...
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update on wave Failed (ispRVWAV35)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
       END    
    END    
...
RETURN_SP:  
    ...
    BEGIN    
       ...
       BEGIN    
          ...
          END    
       END    
       execute nsp_logerror @n_err, @c_errmsg, "ispRVWAV35"    
