...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         SELECT Pickdetail.Pickdetailkey, PickDetail.sku, PickDetail.loc, SUM(PickDetail.qty), PACK.Qty,
...
--         AND    PACK.PackUom3 = 'EA'
         ...
         GROUP BY Pickdetail.Pickdetailkey, PickDetail.sku, PickDetail.loc, PACK.Qty,
    ...
    DECLARE pick_cur CURSOR  FAST_FORWARD READ_ONLY FOR
         SELECT Pickdetail.Pickdetailkey, PickDetail.sku, PickDetail.loc, SUM(PickDetail.qty), PACK.Qty,
                PickDetail.storerkey, "A", PickDetail.UOM, LOC.Logicallocation
         FROM   PickDetail (NOLOCK), LoadPlanDetail (NOLOCK), SKUxLOC (NOLOCK), PACK (NOLOCK),
                LOC (NOLOCK)
         WHERE  PickDetail.OrderKey = LoadPlanDetail.OrderKey
         AND    PickDetail.Status < '5'
...
--         AND    PACK.PackUom3 = 'EA'
         ...
         AND    LoadPlanDetail.LoadKey = @c_loadkey
         GROUP BY Pickdetail.Pickdetailkey, PickDetail.sku, PickDetail.loc, PACK.Qty,
                  PickDetail.storerkey, PICKDETAIL.UOM, LOC.Logicallocation
         ORDER BY LOC.Logicallocation, PickDetail.Loc
      END

      OPEN pick_cur
      FETCH NEXT FROM pick_cur INTO @c_Pickdetailkey, @c_sku, @c_loc, @n_Qty, @n_uom3, @c_storerkey,
            @c_orderkey, @c_UOM, @c_logicalloc

      WHILE (@@FETCH_STATUS <> -1)
      BEGIN
         IF @c_firstorderkey = 'N' 
         BEGIN
            EXECUTE nspg_GetKey
                     "PICKSLIP",
                      9,   
       ...
       BEGIN TRAN
           ...
           (PickHeaderKey,    OrderKey,    ExternOrderKey, PickType, Zone, TrafficCop)
            VALUES
                (@c_pickheaderkey, @c_OrderKey, @c_LoadKey,     "0",      "6",  "")
    ...
    IF @c_superorderflag = "Y" SELECT @c_orderkey = ""
         ...
         VALUES 
              (@c_pickheaderkey,   @c_LoadKey,       @c_OrderKey,     @c_ConsigneeKey,
               ...
               @c_SKUDesc,         @n_Qty,        @n_uom3,
               @n_uom3,       @c_PrintedFlag,   "6",
              @c_Lot,         @c_Transporter,   @c_VehicleNo)
 ...
 --        AND PACK.PackUom1 = 'EA'
   ...
   BEGIN
      ...
      SELECT PickSlipNo,
        ...
        Vehicle
      FROM TempPickSlip
      WHERE PickSlipType = '6'
      AND LoadKey = @c_loadkey
   END

      SELECT #temp_pick.*, PACK.Packkey, PACK.Packuom3, ORDERS.Deliverydate, LOTATTRIBUTE.Lottable02, LOTATTRIBUTE.Lottable04, AREADETAIL.Areakey 
