...
GO
/************************************************************************/
/* Store Procedure:  isp_batching_task_pickslip_04                      */
/* Creation Date:  11-Sep-2020                                          */
/* Copyright: LFL                                                       */
/* Written by: WLChooi                                                  */
/*                                                                      */
/* Purpose: WMS-14999 - CN CNA Pick Slip Report CR                      */
/*                                                                      */
/* Input Parameters:                                                    */
/*                                                                      */
...
CREATE PROC [dbo].[isp_batching_task_pickslip_04] (
 ...
 BEGIN
    ...
    DECLARE @b_Success             INT           
           ...
           ,@c_LimitShowPickzone   NVARCHAR(10)  
           ,@n_ShowNoOfPickzone    INT           
    ...
    BEGIN 
       SELECT @n_Continue = 3  
       SELECT @n_Err = 63200  
       SELECT @c_ErrMsg='NSQL'+CONVERT(NVARCHAR(5),@n_Err)+': Invalid Mode. The value must be 1,4,5,9 (isp_batching_task_pickslip_04)' 
       GOTO Quit
    END
    ...
    SELECT PT.TaskBatchNo, 
         ...
         , ISNULL(CL7.Short,'N') AS LimitShowPickzone  
         , ISNULL(CL7.UDF01,'N') AS ShowNoOfPickzone   
    INTO #TMP_TASK
    ...
    LEFT JOIN CODELKUP CL ON RIGHT(RTRIM(ISNULL(PD.Notes,'')),1) = CL.Code AND CL.Listname = 'REPORTCFG' AND CL.code2 = 'ShowCustomBatchMode' AND CL.Storerkey = PD.Storerkey
    LEFT JOIN Codelkup CL2 (NOLOCK) ON (PD.Storerkey = CL2.Storerkey AND CL2.Code = 'SHOWSTYLESIZE' 
                                              AND CL2.Listname = 'REPORTCFG' AND CL2.Long = 'r_dw_batching_task_pickslip_04' AND ISNULL(CL2.Short,'') <> 'N')  --NJOW03
    LEFT JOIN Codelkup CL3 (NOLOCK) ON (PD.Storerkey = CL3.Storerkey AND CL3.Code = 'SHOWLOADKEYBARCODE' 
                                              AND CL3.Listname = 'REPORTCFG' AND CL3.Long = 'r_dw_batching_task_pickslip_04' AND ISNULL(CL3.Short,'') <> 'N')  --CS02
    LEFT JOIN Codelkup CL4 (NOLOCK) ON (PD.Storerkey = CL4.Storerkey AND CL4.Code = 'ShowCurrentDateTime' 
                                              AND CL4.Listname = 'REPORTCFG' AND CL4.Long = 'r_dw_batching_task_pickslip_04' AND ISNULL(CL4.Short,'') <> 'N')  --WL01
    LEFT JOIN Codelkup CL5 (NOLOCK) ON (PD.Storerkey = CL5.Storerkey AND CL5.Code = 'ShowSalesman' 
                                              AND CL5.Listname = 'REPORTCFG' AND CL5.Long = 'r_dw_batching_task_pickslip_04' AND ISNULL(CL5.Short,'') <> 'N')  --WL03
    LEFT JOIN Codelkup CL6 (NOLOCK) ON (PD.Storerkey = CL6.Storerkey AND CL6.Code = 'ShowVAS' 
                                              AND CL6.Listname = 'REPORTCFG' AND CL6.Long = 'r_dw_batching_task_pickslip_04' AND ISNULL(CL6.Short,'') <> 'N')  --WL02
    LEFT JOIN Codelkup CL7 (NOLOCK) ON (PD.Storerkey = CL7.Storerkey AND CL7.Code = 'LimitShowPickzone' 
                                              AND CL7.Listname = 'REPORTCFG' AND CL7.Long = 'r_dw_batching_task_pickslip_04' AND ISNULL(CL7.Short,'') <> 'N')  --WL04
    ...
    BEGIN
      ...
      SELECT PD.Pickslipno AS Pickslipno, 
             CASE WHEN COUNT(DISTINCT LOC.[Floor]) > 1 THEN N'多楼层'
                  WHEN COUNT(DISTINCT LOC.[Floor]) = 1 THEN N'单楼层'
                  ELSE N'' END AS [Floor]
      FROM #TMP_TASK t
      JOIN PICKDETAIL PD (NOLOCK) ON PD.PickSlipNo = t.TaskBatchNo AND PD.OrderKey = t.Orderkey
      JOIN LOC (NOLOCK) ON LOC.LOC = PD.Loc
      WHERE ISNULL(LOC.[Floor],'') <> ''
      ...
      SELECT TaskBatchNo, 
         ...
         , ShowSalesman  
         , Salesman      
         , ISNULL(#TMP_FLOOR.[Floor],'') AS FloorRemark
      FROM #TMP_TASK     
      LEFT JOIN #TMP_FLOOR ON #TMP_FLOOR.Pickslipno = #TMP_TASK.TaskBatchNo  
      GROUP BY TaskBatchNo,
               Notes, 
               Loadkey, 
               PickZone,
    ...
    IF @c_ReportType = '4'
    BEGIN
        SELECT  @c_LimitShowPickzone = LimitShowPickzone
               ,@n_ShowNoOfPickzone  = CASE WHEN ISNUMERIC(ShowNoOfPickzone) = 1 THEN CAST(ShowNoOfPickzone AS INT) ELSE NULL END
        FROM #TMP_TASK T  
        
        IF ISNULL(@c_LimitShowPickzone,'') = 'Y' AND ISNULL(@n_ShowNoOfPickzone,0) > 0
        BEGIN
            SELECT TOP (@n_ShowNoOfPickzone) 
                        T.Pickzone,
                        SUM(T.Qty) AS TotalQty,
                        COUNT(DISTINCT T.Sku) AS TotalSku,
                        (SELECT SUM(T.Qty) FROM #TMP_TASK T ) AS SUMTotalQty,
                        (SELECT COUNT(DISTINCT T.Sku) FROM #TMP_TASK T) AS SUMTotalSku
            FROM #TMP_TASK T      
            GROUP BY T.Pickzone
            ORDER BY T.PickZone
        END
...
Quit:
    IF OBJECT_ID('tempdb..#TMP_FLOOR') IS NOT NULL   --WL04
      DROP TABLE #TMP_FLOOR
