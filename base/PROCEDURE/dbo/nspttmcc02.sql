...
/* 07-11-2019   James   1.1   WMS-11041 Add UserKeyOverRide (james01)   */
/************************************************************************/

CREATE PROC    [dbo].[nspTTMCC02]
...
BEGIN
   ...
   ELSE
      ...
      BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Execute Of CrossDock Tasks Pick Code Failed. (nspTTMCC02)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + ' ) '
      END
   END
   ...
   BEGIN
      ...
      BEGIN
         ...
         END
      END
      execute nsp_logerror @n_err, @c_errmsg, 'nspTTMCC02'
