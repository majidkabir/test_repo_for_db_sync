...
/************************************************************************/
/* Store Procedure: nsp_ReplenishLetdown_rpt2                           */
...
/* 15-Mar-2008  Shong      SOS94831 Split NIKE15 Report into 4 diff     */
/*                         reports base on Replen/Pick Type.            */
/************************************************************************/
-- EXEC nsp_ReplenishLetdown_rpt2 'NSH01', '0000340877', '0000340901'
CREATE PROC [dbo].[nsp_ReplenishLetdown_rpt2] (
...
-- End : Changed by June 19.Oct.04 SOS28521
      ...
      -- 7 DEC 2004 YTWan FBR012 - Replenishment to BBA - START
		...
		IF NOT EXISTS (SELECT 1 FROM REPLENISHMENT (NOLOCK), #RESULT2
							...
							AND   REPLENISHMENT.SKU = #RESULT2.SKU
		...
		BEGIN
			DECLARE Cursor_BBA CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
				SELECT #RESULT2.Storerkey, #RESULT2.LOC
				FROM #RESULT2
						...
						INNER JOIN LOC L (NOLOCK) ON (L.Loc = #RESULT2.Loc) AND
															  (L.LocationType <> 'BBA')
				GROUP BY #RESULT2.Storerkey, SC.sValue, #RESULT2.LOC
            ...
            DECLARE Cursor_BBA_SKU CURSOR LOCAL FAST_FORWARD READ_ONLY FOR 
               SELECT DISTINCT SKU FROM #RESULT2 
               WHERE StorerKey = @c_StorerKey 
               AND   LOC = @c_LOC 

            OPEN Cursor_BBA_SKU

            FETCH NEXT FROM Cursor_BBA_SKU INTO @c_SKU 
            WHILE @@FETCH_STATUS <> -1 
            BEGIN 
               -- Search for the BBA location already have same SKU 
               ...
               -- If this SKU assign to BBA location previously
               IF ISNULL(dbo.fnc_RTrim(@c_toloc), '') = '' 
               BEGIN 
                  SELECT @c_toloc = MoveToloc FROM #RESULT2 
                  WHERE StorerKey = @c_storerkey 
                  AND   SKU = @c_SKU 
                  AND   MoveToloc > ''
               END 

               -- If this SKU have no BBA location, assign one 
               IF ISNULL(dbo.fnc_RTrim(@c_toloc), '') = '' 
               BEGIN 
      				SELECT @c_toloc = LOC.Loc
      				FROM LOC (NOLOCK)
      				LEFT OUTER JOIN SKUxLOC SL (NOLOCK) ON (SL.Loc = LOC.Loc) And
      																	(SL.Storerkey = @c_storerkey)
                  LEFT OUTER JOIN #RESULT2 R2 ON R2.MoveToloc = LOC.LOC
      				WHERE LOC.LocationType = 'BBA'
      				AND   ISNULL(SL.Qty,0) - ISNULL(SL.Qtypicked,0) = 0
      				AND   R2.MoveToLoc IS NULL
               ...
               BEGIN 
      				...
      				DECLARE Cursor_Repl CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
               ...
               END -- IF ISNULL(dbo.fnc_RTrim(@c_toloc), '') <> ''
   				IF @n_continue = 1 
   				BEGIN
   					UPDATE #RESULT2
                  ...
                  AND   SKU       = @c_SKU 
   					AND   EXISTS ( SELECT 1 FROM REPLENISHMENT (NOLOCK) 
                                 ...
                                 AND   SKU     = @c_SKU
   										...
   										AND   ReplenNo  = 'Y')
   				END -- @n_continue = 1           
            END -- While Cursor_BBA_SKU
            CLOSE Cursor_BBA_SKU
            DEALLOCATE Cursor_BBA_SKU
			END  -- End While
      ...
      DECLARE @nVNACasePickQty int 

      SET ROWCOUNT 1

      SELECT @nVNACasePickQty = CASE WHEN ISNUMERIC(sValue) = 1 THEN CAST(sValue as int) ELSE 5 END 
      FROM #RESULT2 
      JOIN StorerConfig SC (NOLOCK) ON (SC.Storerkey = #RESULT2.Storerkey) And
												   (ISNULL(SValue, 0) BETWEEN 1 AND 999) And 
								  					(Configkey = 'VNACTNPKQTY' )

      SET ROWCOUNT 0

      SET @nVNACasePickQty = ISNULL(@nVNACasePickQty, 5) 

      UPDATE #RESULT2
         SET ReportType = 'Group1'
      FROM #RESULT2 
      LEFT OUTER JOIN LOC WITH (NOLOCK) ON LOC.LOC = #RESULT2.Loc
      WHERE LOC.LocationType = 'BBA' 
      
      UPDATE #RESULT2 
         SET ReportType = 'Group2'
      FROM #RESULT2 
      JOIN (SELECT LOC FROM #RESULT2 
            GROUP BY LOC 
            HAVING SUM(PickQty + ReplQty) <= @nVNACasePickQty 
               AND SUM(CaseBalRtnToRack)  >  @nVNACasePickQty ) AS LOCGrp ON LOCGrp.LOC = #RESULT2.LOC 
      WHERE #RESULT2.ReportType <> 'Group1' 

      UPDATE #RESULT2
         SET ReportType = 'Group3'
      FROM #RESULT2 
      JOIN (SELECT LOC FROM #RESULT2 
            GROUP BY LOC 
            HAVING SUM( #RESULT2.CaseBalRtnToRack ) = 0 ) AS LOCGrp ON LOCGrp.LOC = #RESULT2.LOC 
      WHERE #RESULT2.ReportType NOT IN ('Group1','Group2') 

      UPDATE #RESULT2
         SET ReportType = 'Group4'
      FROM #RESULT2 
      WHERE #RESULT2.ReportType NOT IN ('Group1','Group2','Group3') 
		...
		SELECT #RESULT2.Storerkey, 
             ...
             #RESULT2.ID, 
             SUM(#RESULT2.Qty) Qty, 
             Sum(#RESULT2.QtyInEA) QtyInEA,
             #RESULT2.Packkey, 
             #RESULT2.CaseCnt, 
             SUM(#RESULT2.PickQty) PickQty, 
             Sum(#RESULT2.ReplQty) ReplQty, 
             Sum(#RESULT2.ReplQtyInEA) ReplQtyInEA, 
             #RESULT2.ToLoc, 
				 Sum(#RESULT2.CaseBalRtnToRack) CaseBalRtnToRack, 
             Sum(#RESULT2.CaseBalRtnToRackInEA) CaseBalRtnToRackInEA,
				 #RESULT2.MoveToLoc, 
             #RESULT2.facility, 
             ...
             ReportType 
		FROM #RESULT2 
      LEFT OUTER JOIN PACK WITH (NOLOCK) ON #RESULT2.Packkey = PACK.packkey 
      LEFT OUTER JOIN LOC WITH (NOLOCK) ON LOC.LOC = #RESULT2.Loc
      -- WHERE MoveToLoc > ''
		GROUP BY #RESULT2.Storerkey, #RESULT2.Sku, #RESULT2.Lottable02, 
		         #RESULT2.Loc, #RESULT2.ID, #RESULT2.Packkey, #RESULT2.CaseCnt, 
		         #RESULT2.ToLoc, #RESULT2.MoveToLoc, #RESULT2.facility, 
               #RESULT2.loadkeystart, #RESULT2.loadkeyend, pack.packuom3, 
               #RESULT2.ReportType       			   
		ORDER BY #RESULT2.ReportType, #RESULT2.Loc, #RESULT2.ID, #RESULT2.Sku, #RESULT2.CaseBalRtnToRack 
   ...
   BEGIN
      ...
      BEGIN
         ...
         END
      END
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'nsp_ReplenishLetdown_rpt2'
