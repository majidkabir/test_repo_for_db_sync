...
/***************************************************************************/  
/* Stored Proc : isp_GetPickSlipOrders92                                   */  
/* Creation Date:24/06/2019                                                */  
/* Copyright: IDS                                                          */  
/* Written by:                                                             */  
/*                                                                         */  
/* Purpose:  WMS-9037 - Modified from nsp_GetPickSlipOrders78              */  
/*                                                                         */  
/*                                                                         */  
/* Usage:                                                                  */  
/*                                                                         */  
/* Local Variables:                                                        */  
/*                                                                         */  
/* Called By: r_dw_print_pickorder92                                       */  
/*                                                                         */  
/* PVCS Version: 1.1                                                       */  
/*                                                                         */  
/* Version: 5.4                                                            */  
...
DECLARE @c_pickheaderkey    NVARCHAR(10),  
    ...
    CREATE TABLE #TEMP_PICK92  
       ( PickSlipNo       NVARCHAR(10) NULL,  
         ...
         LocPickZone      NVARCHAR(10) NULL)       --CS01  
         
       INSERT INTO #TEMP_PICK92  
            (PickSlipNo,          LoadKey,         OrderKey,   ConsigneeKey,  
             ...
             Susr4,               Vat,                OVAS,         SKUGROUP,    ContainerType,RptGrp,LocPickZone) -- SOS144415  --CS01
    ...
    -- Changed by YokeBeen on 08-Aug-2002 (Ticket # 6692) - from BillToKey to ConsigneeKey.    
       ...
       ,LocPickZone = ISNULL(Loc.Pickzone,'')                            --CS01
 ...
 --             FROM #TEMP_PICK92 WHERE PickSlipNo IS NULL  
 --             GROUP By LoadKey, OrderKey  
         ...
         WHERE PICKHEADER.ExternOrderKey = #TEMP_PICK92.LoadKey  
         AND   PICKHEADER.OrderKey = #TEMP_PICK92.OrderKey  
         AND   PICKHEADER.Zone = '3'  
   AND   #TEMP_PICK92.PickSlipNo IS NULL  
     END  
     GOTO SUCCESS  
 FAILURE:  
     DELETE FROM #TEMP_PICK92  
 SUCCESS:  
     SELECT * FROM #TEMP_PICK92 
    order by pickslipno,loadkey,orderkey,consigneekey,locpickzone 
      
   DROP Table #TEMP_PICK92    
 END
