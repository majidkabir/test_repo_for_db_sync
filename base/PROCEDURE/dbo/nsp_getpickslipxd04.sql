...
/************************************************************************/
/* Store Procedure:  nsp_GetPickSlipXD04                           	   */
/* Creation Date: 08-Oct-2004                           						*/
/* Copyright: IDS                                                       */
/* Written by: MaryVong                                    					*/
/*                                                                      */
/* Purpose:  Cross Dock PickSlip (04)for IDSHK-WTC	(SOS28137)           */
/*           Note: Duplicated from nsp_GetPickSlipXD01 which is         */
/*                 currently used by IDSHK and IDSTH					      */
/*                                                                      */
...
CREATE PROC [dbo].[nsp_GetPickSlipXD04] (@c_refkey NVARCHAR(20), @c_type NVARCHAR(2)) 
AS
BEGIN
-- Type = P for ExternPOKey, L for LoadKey
   ...
   BEGIN
      ...
      BEGIN 
         ...
         SELECT @c_errmsg = CONVERT(CHAR(250),@n_err), @n_err = 63314   
   		SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Update PickHeader Failed. (nsp_GetPickSlipXD04)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + ' ) '
      END 
	   ...
	   BEGIN 
	      ...
	      BEGIN
            ...
            BEGIN
               ...
               SELECT @n_continue = 3
               SELECT @c_errmsg = CONVERT(CHAR(250),@n_err), @n_err = 63314   
         		SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Insert Into PickHeader Failed. (nsp_GetPickSlipXD04)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + ' ) '
               BREAK 
            END
            ...
            BEGIN
	            ...
	            BEGIN
	               ...
	               SELECT @n_continue = 3
	               SELECT @c_errmsg = CONVERT(CHAR(250),@n_err), @n_err = 63314   
	         		SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Update PickDetail Failed. (nsp_GetPickSlipXD04)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + ' ) '
						Break
	            END
            ...
            BEGIN
               ...
               SELECT @n_continue = 3
               SELECT @c_errmsg = CONVERT(CHAR(250),@n_err), @n_err = 63314   
         		SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Insert Into RefKeyLookup Failed. (nsp_GetPickSlipXD04)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + ' ) '
            END
			END
	   END
		...
		BEGIN 
         ...
         WHILE @@TRANCOUNT > 0 
			...
			SELECT #TEMPPICKDETAIL.*, ISNULL(SKU.DESCR,'') SKUDESCR, OD.EXTERNPOKEY, OH.CONSIGNEEKEY, OH.C_COMPANY, 
					 ...
					 ISNULL(PACK.Casecnt, 0) CASECNT, ISNULL(PACK.Innerpack, 0) Innerpack, @c_recvby, SUSER_SNAME() UserName 
   ...
   IF @n_continue=3
   BEGIN
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'nsp_GetPickSlipXD04'
