...
GO
/************************************************************************/    
/* Stored Procedure: isp_Delivery_Receipt_MDL02                         */    
/* Creation Date: 10/03/2021                                            */    
/* Copyright: IDS                                                       */    
/* Written by: CSCHONG                                                  */    
/*                                                                      */    
/* Purpose:    WMS-16515 PH_Huhtamaki_Delivery_Receipt_CR               */   
/*          Copy from isp_Delivery_Receipt_MDL01                        */    
/*                                                                      */    
/* Called By:                                                           */    
/*                                                                      */    
/* PVCS Version: 1.0                                                    */    
...
CREATE PROC [dbo].[isp_Delivery_Receipt_MDL02] (@cMBOLkey NVARCHAR(10) )    
AS    
BEGIN    
   SET NOCOUNT ON   
   SET QUOTED_IDENTIFIER OFF   
 ...
 CREATE TABLE #TempFlag (        
         ...
         ConverttoKG       [INT]             NULL DEFAULT (0)   
         )    
  ...
  DECLARE @TempData TABLE (    
         ...
         Lot13             [datetime] NULL,
         Lot04Title        [nvarchar] (50)  NULL,    
         Lot13Title        [nvarchar] (50)  NULL ,
         PUOM1             [nvarchar] (10)  NULL                     
         )    
 ...
 -- Assign DR Number (at externorderkey level) to all orders under this MBOLKey!     
      ...
      BEGIN    
         ...
         BEGIN       
            ...
            BEGIN    
               ...
               SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": No Setup for CodeLkUp.ListName = DR_NCOUNT. (isp_Delivery_Receipt_MDL02)"    
            END    
            ...
            BEGIN    
               ...
               BEGIN    
                  SELECT @n_continue = 3    
                  SELECT @n_err = 63500  -- should assign new error code    
                  SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Fail to Generate Userdefine10 . (isp_Delivery_Receipt_MDL02)"    
               END    
               ELSE    
               BEGIN    
                  UPDATE ORDERS     
                  ...
                  BEGIN    
                     ...
                     SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": UPDATE ORDERS Failed. (isp_Delivery_Receipt_MDL02)"    
                  END    
               END    
         ...
         INSERT INTO #TempFlag(PrintFlag, ExternOrderKey, Storerkey, ConvertToKg)    
         VALUES(@cPrintFlag, @cExternOrderKey, @cStorerkey, @n_ConvertToKg)                         
           
         SET  @c_Storerkey_Prev =  @cStorerkey                                                       
         FETCH NEXT FROM CurExternOrder INTO @cStorerkey, @cExternOrderKey, @cUserDefine10  
      END    
   ...
   BEGIN     
      ...
      SELECT     
         ...
         CONVERT(DECIMAL(12,2),SUM(Pickdetail.Qty) / (CASE WHEN ORDERDETAIL.UOM = PACK.PackUOM1 THEN PACK.CaseCnt     
                               WHEN ORDERDETAIL.UOM = PACK.PackUOM2 THEN PACK.InnerPack    
                               ELSE 1 END)/ PACK.CaseCnt) As QtyCtn,      
         ORDERDETAIL.UOM    
        ,KG = CONVERT(DECIMAL(12,3), ISNULL(SUM(PICKDETAIL.QTY),0)/1000.00)         
        ,T.ConvertToKg                                                             
        ,ISNULL(ST2.Notes1,'')   
        ,ISNULL(ORDERS.NOTES,'')  
      ...
      LOTATTRIBUTE.Lottable13 AS Lot13,
      ISNULL(CL1.description,'') AS Lot04Title,    
      ISNULL(CL2.description,'') AS Lot13Title,
      PACK.PackUOM1 as PUOM1                                                                            
      FROM ORDERS (NOLOCK)    
      ...
      LEFT OUTER JOIN #TempFlag T ON T.ExternOrderkey = ORDERS.ExternOrderkey
     ...
     LEFT OUTER JOIN CODELKUP CL1 (NOLOCK) ON (ORDERS.Storerkey = CL1.Storerkey AND  CL1.Code = 'Lottable04' AND CL1.Listname = 'RPTCOLHDR' 
                                               AND   CL1.Long = 'r_dw_delivery_receipt_mdl02'  ) 
     LEFT OUTER JOIN CODELKUP CL2 (NOLOCK) ON (ORDERS.Storerkey = CL2.Storerkey AND  CL2.Code = 'Lottable13' AND CL2.Listname = 'RPTCOLHDR'
                                               AND   CL2.Long = 'r_dw_delivery_receipt_mdl02' ) 
   ...
   BEGIN    
      EXECUTE nsp_logerror @n_err, @c_errmsg, "isp_Delivery_Receipt_MDL02"    
