...
/* Written by: Wan                                                      */
/*                                                                      */
/* Purpose: WMS-4294 - HK PVH Exceed Generate Pack by Picked            */
/*        : Copy & Modified from ispWAVPK02                             */
/* Usage:                                                               */
/*                                                                      */
/* Called By:                                                           */
/*                                                                      */
...
/* 06-JUN-2018 Wan02    1.1      1) remove create new carton logic for  */
/*                                  different value for SKU             */
/* 06-JUN-2018 Wan03    1.1      2) total qty of same SKU must be packed*/
/*                                  in the same carton                  */
/* 20-JUN-2018 Wan04    1.1      3) Fixed- Cartonization Sequence = 1   */
/* 09-Oct-2018 NJOW01   1.2      WMS-6568 Handle full case by ID        */
...
BEGIN
   ...
   DECLARE    @c_Orderkey              VARCHAR(15)
            ...
            , @n_LastCartonNo          INT --NJOW03
            , @c_sts                   NVARCHAR(1) --NJOW03
            , @c_LocationType          NVARCHAR(10) --NJOW03
            , @n_LLIQty                INT  --NJOW03
            , @n_PDQty                 INT  --NJOW03
            , @c_UOM                   NVARCHAR(10) --NJOW03
   ...
   DECLARE @n_TotalSkuQty     INT            --(Wan03)
         , @n_TotalSkuCube    FLOAT          --(Wan03)
         , @n_TotalSkuCapacityLimit FLOAT    --(Wan03)
         , @n_SkuCapacityLimit      FLOAT    --(Wan03)
   ...
   SELECT @c_MinPickSlipNo = ISNULL(MIN(PD.PickSlipNo), '')
         ...
         ,@n_MinStdCube    = ISNULL(Min(SKU.StdCube), 0.00) --NJOW03
   ...
   LEFT JOIN CARTONIZATION CT WITH (NOLOCK) ON CT.CartonizationGroup = ST.cartongroup  and CT.usesequence = '1'
 ...
 -- Store PickDetail by OrderKey
   ...
   JOIN Storer WITH (NOLOCK) ON Storer.StorerKey = CodelKup.StorerKey AND Storer.Type ='1'
   JOIN Cartonization WITH (NOLOCK) ON Cartonization.Cartonizationgroup = Storer.CartonGroup
                                    AND Cartonization.UseSequence = 1         -- (Wan04) 20-Jun-2018 Fixed
   ...
   DECLARE CUR_OrderList CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
      -- BL02 Start
      SELECT DropID,  SKU, Storerkey, SUM(Qty), PickSlipNo, ID --PickDetailKey  NJOW01
            ,BillToKey 
      FROM #PickDetail 
      WHERE UOM = 2 and Sts = '0' -- Full UCC only      
      GROUP BY DropID,  SKU, Storerkey, PickSlipNo, ID, BillToKey --NJOW01
      Order by Pickslipno, SKU
                    
   OPEN CUR_OrderList  
   FETCH NEXT FROM CUR_OrderList INTO @c_DropID, @SKU, @c_StorerKey, @Qty, @c_PickSlipno, @c_ID --@c_PickDetailKey NJOW01
                                    , @c_BillToKey                             
   ...
   BEGIN   
      ...
      BEGIN 
         ...
         WHERE #PickDetail.ID = @c_ID --NJOW01
         AND #PickDetail.PickSlipNo = @c_PickSlipno --NJOW01
         AND #PickDetail.Storerkey = @c_Storerkey --NJOW01
         AND #PickDetail.Sku = @Sku --NJOW01
      ...
      BEGIN      
         ...
         BEGIN
            INSERT INTO dbo.PackHeader
               ...
               WHERE a.ID = @c_ID --NJOW01
               AND a.PickSlipNo = @c_PickSlipno --NJOW01
               AND a.Storerkey = @c_Storerkey --NJOW01
               AND a.Sku = @Sku --NJOW01
               --WHERE PickDetailKey = @c_PickDetailKey
            END
   ...
   --BEGIN
      ...
      BEGIN
         ...
         SELECT @CartonNo, PickDetailKey, '', SKU, Qty, 
                  @SKUCUBE,@SKUWeight 
                  --, @SKULength, @SKUHeight, @SKUWidth
         FROM #PickDetail 
         WHERE ID = @c_ID  --NJOW01
...
NEXT_CartonType:
      ...
      SET   --LabelNo = @cLabelNo,
       ...
       Sts = '9'
      WHERE ID = @c_ID AND UOM= '2'  --NJOW01
      AND PickSlipNo = @c_PickSlipno --NJOW01
      AND Storerkey = @c_Storerkey --NJOW01
      AND Sku = @Sku --NJOW01
      --WHERE PickDetailKey = @c_PickDetailKey AND UOM= '2'
      
      DELETE CartonList 
      WHERE CartonKey IN ( SELECT CartonListDetail.CartonKey 
                           FROM CartonListDetail WITH (NOLOCK) 
                           JOIN #PickDetail ON CartonListDetail.Pickdetailkey = #PickDetail.Pickdetailkey  --NJOW01
                           WHERE #PickDetail.ID = @c_ID  --NJOW01
                           AND #PickDetail.PickSlipNo = @c_PickSlipno --NJOW01
                           AND #PickDetail.Storerkey = @c_Storerkey --NJOW01
                           AND #PickDetail.Sku = @Sku --NJOW01
      ...
      BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update CartonList Table Failed. (ispWAVPK03)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
         GOTO QUIT_SP
      END

      DELETE CartonListDetail 
      WHERE Pickdetailkey IN (SELECT Pickdetailkey FROM #PickDetail WHERE ID = @c_ID) --NJOW01
      --WHERE PickDetailKey = @c_PickDetailKey
      SET @n_err = @@ERROR
      IF @n_err <> 0
      BEGIN
   ...
   LEFT JOIN CODELKUP CLAI008 WITH (NOLOCK) ON a.BillToKey = CLAI008.Code AND CLAI008.ListName = 'PVHCONSO' AND CLAI008.Long = '1'  --NJOW03                                   
   WHERE a.UOM in ('6','7') and a.Sts = '0' -- Full UCC only

   SET @c_PickDetailKey = '' 
   SET @CartonNo = ''

   SET @CurrCube = 0                             --(Wan03)
         ...
         BEGIN
            ...
            BEGIN 
               ...
               SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': New Carton Does not fit pack item:' + RTRIM(@sku)
                     ...
                     +', Carton Cube: ' + RTRIM(CONVERT(NCHAR(15), @Cube)) + '. (ispWAVPK03)'
               GOTO QUIT_SP
            END
   ...
   */
   ...
   BEGIN      
      ...
      BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Delete PACKDETAIL Table Failed. (ispWAVPK03)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
         GOTO QUIT_SP
      END
      ...
      SELECT  --+ Convert( NVARCHAR(10), ROW_NUMBER() OVER(ORDER BY a.PickSlipNo)),
             RIGHT('00000' + Convert( NVARCHAR(10), ROW_NUMBER() OVER(PARTITION BY a.pickslipno,b.cartonno ORDER BY a.PickSlipNo, b.cartonno, )),5), --NJOW03
      ...
      WHERE a.PickSlipNo = @c_PickSlipno
      GROUP by a.PickSlipNo, b.CartonNo, b.LabelNo, a.SKU, a.StorerKey, c.CartonSeq
      ...
      */            
      ...
      INSERT INTO PACKDETAIL ( LabelLine, PickSlipNo,CartonNo,LabelNo,StorerKey,SKU, Qty,RefNo,ArchiveCop
                              ,ExpQty,UPC,DropID,RefNo2)        
      SELECT  --+ Convert( NVARCHAR(10), ROW_NUMBER() OVER(ORDER BY a.PickSlipNo)),
             RIGHT('00000' + Convert( NVARCHAR(10), ROW_NUMBER() OVER(PARTITION BY a.pickslipno,b.cartonno ORDER BY a.PickSlipNo, b.cartonno, PE.ItemStyle, PE.ItemBusr1, PE.ItemColor, PE.ItemSize, PE.Itemmeasure)),5), --NJOW03
             a.PickSlipNo, c.CartonSeq, b.LabelNo, a.StorerKey, a.SKU , sum(b.PackQty), '', null
            , 0, null, CASE WHEN a.uom = '2' THEN a.dropid ELSE '' END,''
      from #PickDetail a
      JOIN #OpenCartonDetail b on b.PickDetailKey = a.PickDetailKey
      ...
      */      
      ...
      SELECT  PD.PickSlipNo, PD.CartonNo
                           ,CASE WHEN c.FullCase = 'Y' THEN 0 ELSE CZ.[Cube]END  --NJOW03
                           ,ISNULL(SUM(PD.Qty),0)
                           ,CASE WHEN c.FullCase = 'Y' THEN '' ELSE CZ.CartonType END  --NJOW03
                           ,CASE WHEN c.FullCase = 'Y' THEN 0 ELSE CZ.CartonLength END  --NJOW03
                           ,CASE WHEN c.FullCase = 'Y' THEN 0 ELSE CZ.CartonWidth END --NJOW03
                           ,CASE WHEN c.FullCase = 'Y' THEN 0 ELSE CZ.CartonHeight END --NJOW03
      FROM #OpenCarton c 
      ...
      BEGIN
         ...
         SET @c_ErrMsg = 'NSQL' +  CONVERT(CHAR(5),@n_Err)  + ':'  
                        + 'Error Executing isp_AssignPackLabelToOrderByLoad.(ispWAVPK03)'
         GOTO QUIT_SP
      END
...
/*
   ...
   FETCH NEXT FROM CUR_SplitPickDetail INTO @c_PickDetailKey, @CntCount
      ...
      BEGIN   
         ...
         WHILE (@@FETCH_STATUS <> -1)     
         BEGIN
            IF (select  Qty - @Qty from PICKDETAIL WHERE PickdetailKey = @c_PickDetailKey) > 0
            BEGIN
               ...
               BEGIN
                  SELECT @n_continue = 3
                  SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err = 60028   -- Should Be Set To The SQL Errmessage but I don't know how to do so.  
                  SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert Pickdetail Table Failed. (ispWAVPK03)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
                  GOTO QUIT_SP
               END
...
QUIT_SP:
   ...
   BEGIN                                 
      ...
      BEGIN
         ...
         SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err)
         SET @n_err = 60027   -- Should Be Set To The SQL Errmessage but I don't know how to do so.  
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert CartonList Table Failed. (ispWAVPK03)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
         GOTO QUIT_SP
      END
      ...
      BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert CartonListdetail Table Failed. (ispWAVPK03)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
         GOTO QUIT_SP
      END
   ...
   BEGIN
      ...
      BEGIN
         ...
         END
      END
      EXECUTE dbo.nsp_LogError @n_Err, @c_Errmsg, 'ispWAVPK03'    
