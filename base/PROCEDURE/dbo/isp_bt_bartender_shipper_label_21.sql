...
BEGIN                      
   ...
   SET @c_SQLJOIN = +' SELECT DISTINCT ORD.loadkey,ORD.orderkey,ORD.externorderkey,ORD.type,buyerpo,salesman,ORD.facility,STO.Secondary,'    --8        
                    + CHAR(13) +           
                    +'STO.Company,STO.SUSR1,STO.SUSR2,'''',ORD.Notes,'''',ORD.Storerkey,STO.State,'  --16         
                    + CHAR(13) +          
                    +'STO.City,STO.Zip,STO.Contact1,STO.Phone1,STO.phone2,ORD.Consigneekey,ORD.c_Company,ORD.c_Address1,'+ CHAR(13) +   --24       
                    +'ISNULL(ORD.c_Address2,''''),ISNULL(ORD.C_Address3,''''),ORD.C_Address4,ORD.C_State,ORD.C_City,ORD.C_Zip,ORD.C_Contact1,ORD.C_Phone1,'   --32          
                    +'ISNULL(ORD.C_Phone2,''''),CASE WHEN STO.VAT=''LEV'' THEN ORD.Notes2 ELSE ORD.M_Company END,ORD.Userdefine01,ORD.Userdefine02,'   --36          
                    +'CONVERT(NVARCHAR(80), GETDATE(), 111),PD.labelno,ORD.Userdefine05,'   --39   
                    + CHAR(13) +   
   ...
   BEGIN           
      ...
      SELECT @n_PackInfoWgt = SUM(PKI.Weight)
           , @c_TrackingNo  = MAX(PKI.TrackingNo)
   ...
   BEGIN       
      ...
      SET    Col50     = @c_CLong,  
             ...
             Col14     = @c_UDF01,   
             Col55     = @c_Col55,    
             Col31     = CASE WHEN @c_EncryptPhoneNum = 'Y' AND LEN(Col31) >= 1
                         THEN LEFT(Col31, 1) + REPLACE(SPACE(LEN(Col31) - 1) , ' ', '*') ELSE Col31 END,
...
EXIT_SP:    
   ...
   EXEC isp_InsertTraceInfo   
      ...
      @c_TraceName = 'isp_BT_Bartender_Shipper_Label_21',  
