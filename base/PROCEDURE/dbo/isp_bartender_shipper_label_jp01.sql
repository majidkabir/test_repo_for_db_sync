...
/******************************************************************************/
/* Copyright: IDS                                                             */
/* Purpose: isp_Bartender_Shipper_Label_JP01                                  */
/******************************************************************************/
/*          - Contain Japanese Character in this stored procedure.            */
/*          - Must save as unicode format.                                    */
/******************************************************************************/
/*                                                                            */
/* Modifications log:                                                         */
/*                                                                            */
...
BEGIN
   ...
   SET @c_SQLJOIN = +N' SELECT DISTINCT (N''〒'' + Substring(ORD.C_Zip,1,3) + ''-'' + Substring(ORD.C_Zip,4,4)),ORD.c_phone1,'
                  ...
                  -- + ' ORD.USERDEFINE04,(Substring(ORD.USERDEFINE04,1,4) + ''-'' + Substring(ORD.USERDEFINE04,5,4)  +''-'' +  Substring(ORD.USERDEFINE04,9,4)),'  --10 --CS02
                   ...
                   + ' '''',ORD.Orderkey,''1'',ISNULL(STO.B_Phone2,''''),'  --30
                   ...
                   + ' LEFT JOIN CODELKUP C2 WITH (NOLOCK) ON C2.Listname = ''HMCARTON'' AND C2.UDF01 = CT.CarrierName AND C2.STORERKEY=ORD.STORERKEY'
   ...
   BEGIN
      ...
      BEGIN
         ...
         --JOIN PACKDETAIL PADET WITH (NOLOCK) ON PADET.Storerkey = STO.Storerkey
         --JOIN PACKHEADER PAH WITH (NOLOCK) ON PAH.Pickslipno = PADET.Pickslipno
         JOIN ORDERS ORD WITH (NOLOCK) ON ORD.storerkey = STO.storerkey
         WHERE
         --PAH.Orderkey = @c_orderkey
         --AND PADET.CartonNo = @c_CartonNo
         ORD.Orderkey = @c_orderkey
      END
...
EXIT_SP:
   ...
   EXEC isp_InsertTraceInfo
      ...
      @c_TraceName = 'isp_Bartender_Shipper_Label_JP_DW',
