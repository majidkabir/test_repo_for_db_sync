...
/*************************************************************************/  
/* Store Procedure:  isp5651P_WOL_AU_Adidas_Addverb_Pack_Import_Short    */  
...
CREATE   PROC [dbo].[isp5651P_WOL_AU_Adidas_Addverb_Pack_Import_Short] (    
...
BEGIN     
   ...
   SELECT DISTINCT ISNULL(RTRIM(SKU),'')
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN  
               ...
               SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0))   
                              ...
                              + '. (isp5651P_WOL_AU_Adidas_Addverb_Pack_Import_Short)'  
               GOTO QUIT  
            END  
         END
         ...
         BEGIN
            ...
            BEGIN  
               ...
               SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0))   
                              ...
                              + ', LabelNo = ' + @c_PackDetail_LabelNo
                              + ', LabelLine = ' + @c_PackDetail_LabelLine_NEW
                              + '. (isp5651P_WOL_AU_Adidas_Addverb_Pack_Import_Short)'  
               GOTO QUIT  
            END 
         ...
         BEGIN
            ...
            BEGIN  
               ...
               SET @n_Err = 68115  
               SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0))   
                              + ': Fail to get PickDetailKey for PickDetailKey. (isp5651P_WOL_AU_Adidas_Addverb_Pack_Import_Short)'  
               GOTO QUIT  
            END  
   ...
   BEGIN  
      ...
      SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0)) +  
                        ...
                        @c_PickSlipNo + '. (isp5651P_WOL_AU_Adidas_Addverb_Pack_Import_Short)'  
      GOTO QUIT  
   END

   UPDATE ORDERS WITH (ROWLOCK)
   SET Notes = Notes + '|SHORT'
   WHERE OrderKey = @c_OrderKey
   AND Storerkey = @c_Storerkey
   ...
   BEGIN  
      ...
      SET @n_Err = 68119  
      SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0)) +  
                        ': Fail to update Orders for OrderKey = ' +  
                        @c_OrderKey + '. (isp5651P_WOL_AU_Adidas_Addverb_Pack_Import_Short)'  
      GOTO QUIT  
   END
