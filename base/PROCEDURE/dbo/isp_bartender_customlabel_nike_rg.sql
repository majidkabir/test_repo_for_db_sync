...
/******************************************************************************/
/* Copyright: IDS                                                             */
/* Purpose: isp_Bartender_CustomLabel_NIKE_RG                                 */
/*                                                                            */
/* Modifications log:                                                         */
/*                                                                            */
...
CREATE   PROC [dbo].[isp_Bartender_CustomLabel_NIKE_RG]
...
BEGIN
  ...
  DECLARE @d_Trace_StartTime   DATETIME,
   ...
   BEGIN
    ...
    SET @c_SQLJOIN = +' SELECT DISTINCT U.Uccno,RECDET.ExternReceiptkey,Substring(RECDET.Sku,1,9),U.Qty,RECDET.SKU,'+ CHAR(13) +  
             ...
             + ' LEFT JOIN CODELKUP C WITH (NOLOCK) ON C.listname = ''SKUGROUP'' and C.code = s.Busr7  AND C.storerkey = REC.Storerkey'
             + ' LEFT JOIN CODELKUP C1 WITH (NOLOCK) ON C1.listname = ''SUGTPAZONE'' and C1.code = s.Busr7 '
             + '                                          AND C1.storerkey = REC.Storerkey AND C1.short=LEFT(@c_sku,1) AND C1.UDF01 = c.UDF01'
             + ' WHERE U.SKU = @c_sku '                  
  ...
  --    EXEC sp_executesql @c_SQL
   ...
   EXIT_SP:
      ...
      --   @c_TraceName = 'isp_Bartender_CustomLabel_NIKE_RG',
