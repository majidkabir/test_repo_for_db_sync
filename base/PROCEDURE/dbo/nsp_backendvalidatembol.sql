...
/* 2019-11-12  TLTING01      Initialize variable                        */
/*                           Start validate before scan to pallet       */
/* 2020-04-28  TLTING03      StorerConfig - NoCont4VldMBOL              */
/************************************************************************/
...
BEGIN
   ...
   BEGIN
      ...
      SELECT DISTINCT Mbol.MbolKey, ISNULL(Mbol.ValidatedFlag, 'N'), Mbol.EditDate, O.StorerKey   --TLTING03
   ...
   OPEN CUR1
   FETCH NEXT FROM CUR1 INTO @c_MBOLKey, @c_ValidatedFlag, @d_EditDate, @c_VStorerKey
   ...
   BEGIN
      ...
      SET @b_ReturnCode = 0  --TLTING01
      SET @b_ReturnErr  = 0   --TLTING01
      SET @c_ReturnErrMsg = '' --TLTING01
      ...
      BEGIN
         ...
         ELSE
         BEGIN
            -- tlting02 with container
            IF  ( SELECT TOP 1  ISNULL(C.[Status],'0')
