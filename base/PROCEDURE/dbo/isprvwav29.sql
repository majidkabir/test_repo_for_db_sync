...
/*************************************************************************/  
/* Stored Procedure: ispRVWAV29                                          */  
/* Creation Date: 24-JUN-2019                                            */  
/* Copyright: LFL                                                        */  
/* Written by: Chooi                                                     */  
/*                                                                       */  
/* Purpose: WMS-9378 - TW EAT Wave reverse pick task                     */
/*                                                                       */  
/* Called By: wave                                                       */  
/*                                                                       */  
/* PVCS Version: 1.1                                                     */  
...
CREATE PROCEDURE [dbo].[ispRVWAV29]      
 ...
 BEGIN  
    ...
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM TASKDETAIL TD (NOLOCK) 
                   ...
                   AND TD.Sourcetype IN ('ispRLWAV29') 
                   AND TD.Tasktype IN ('FCP')) 
        BEGIN
           SELECT @n_continue = 3  
           SELECT @n_err = 81010  
           SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': This Wave has not been released. (ispRVWAV29)'    
        END                 
    END
    ...
    BEGIN
       ...
       BEGIN
          SELECT @n_continue = 3  
          SELECT @n_err = 81020  
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Some Tasks have been started. Not allow to Reverse Wave Released (ispRVWAV29)'       
       END                 
    END
    ...
    BEGIN
       ...
       BEGIN
          ...
          AND TASKDETAIL.Sourcetype = 'ispRLWAV29'  
          AND TASKDETAIL.TaskType = 'FCP'  
          AND TASKDETAIL.Status = '0'  
          AND TASKDETAIL.Orderkey = @c_Orderkey
       END
       ELSE
       BEGIN
          DELETE TASKDETAIL  
       ...
       BEGIN  
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Delete Taskdetail Table Failed. (ispRVWAV29)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
       END   
    END
    ...
    BEGIN
       ...
       BEGIN  
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Pickdetail Table Failed. (ispRVWAV29)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
       END            
    END 
    ...
    BEGIN
       ...
       BEGIN  
          ...
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update on wave Failed (ispRVWAV29)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
       END  
    END  
...
RETURN_SP:
    ...
    BEGIN  
       ...
       BEGIN  
          ...
          END  
       END  
       execute nsp_logerror @n_err, @c_errmsg, "ispRVWAV29"  
