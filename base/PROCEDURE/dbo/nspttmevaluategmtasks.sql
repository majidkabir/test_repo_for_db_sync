...
GO
/************************************************************************/  
/* Stored Procedure: nspTTMEvaluateGMTasks                              */  
/* Creation Date:                                                       */  
/* Copyright: IDS                                                       */  
/* Written by:                                                          */  
/*                                                                      */  
...
/* 20-09-2010   ChewKP        RDT Compatible Error Message              */
/*                            TaskDetail Update (ChewKP01)              */
/************************************************************************/  
  
CREATE PROC    [dbo].[nspTTMEvaluateGMTasks]
...
,              @c_TaskDetailKey    NVARCHAR(10)   OUTPUT -- (KHLim01)  
AS  
BEGIN  
   SET NOCOUNT ON   
   SET QUOTED_IDENTIFIER OFF   
   ...
   DECLARE @b_cursor_open int --, @c_taskdetailkey NVARCHAR(10)   (KHLim01)
   ...
   BEGIN  
      DECLAREcursor_GMTASKCANDIDATES:  
      ...
      BEGIN  
         SELECT @n_continue = 3  
         SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err=78701   -- Should Be Set To The SQL Errmessage but I don't know how to do so.  
         SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Execute Of Move Tasks Pick Code Failed. (nspTTMEvaluateGMTasks)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "  
      END  
      IF @n_err = 16915  
      BEGIN  
         CLOSE cursor_GMTASKCANDIDATES  
         DEALLOCATE cursor_GMTASKCANDIDATES  
         GOTO DECLAREcursor_GMTASKCANDIDATES  
      END  
      OPEN cursor_GMTASKCANDIDATES  
   ...
   BEGIN  
      ...
      BEGIN  
         FETCH NEXT FROM cursor_GMTASKCANDIDATES INTO @c_taskdetailkey  
         IF @@FETCH_STATUS = -1  
         BEGIN  
            BREAK  
         END  
...
--            UPDATE TASKDETAIL  
--            SET status = "0" ,  
--            userKey = ( CASE tasktype  
--                        WHEN "GM" then userkey  
--                        ELSE ""  
--                        END  
--                        )  
--            WHERE status = "3"  
...
--            BEGIN  
--               SELECT @n_continue = 3  
--               SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err=78702   -- Should Be Set To The SQL Errmessage but I don't know how to do so.  
--               SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Error Occurred While Attempting To Update TaskDetail. (nspTTMEvaluateGMTasks)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "  
--            END  
--            IF @n_continue = 1 or @n_continue = 2  
--            BEGIN  
--               UPDATE TASKDETAIL  
--               SET status = "9" , -- In all other transactions, this would be a "3" because a reply was expected.  In this case, no reply is needed to this dispatch.  
--               reasonkey = "" ,  
--               StartTime = CURRENT_TimeStamp  
--               WHERE TaskDetailKey = @c_taskDetailKey  
--               AND STATUS = "0"  
...
--               BEGIN  
--                  SELECT @n_continue = 3  
--                  SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err=78703   -- Should Be Set To The SQL Errmessage but I don't know how to do so.  
--                  SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Error Occurred While Attempting To Update TaskDetail. (nspTTMEvaluateGMTasks)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "  
--               END  
--               IF @n_cnt = 1  
--               BEGIN  
...
--            END  
   ...
   BEGIN  
      ...
      + dbo.fnc_RTrim(@c_msg03)             + @c_senddelimiter  
      + ""                          + @c_senddelimiter  
      + ""                          + @c_senddelimiter  
      + ""  
   END  
   ...
   BEGIN  
      ...
      BEGIN  
       ...
       BEGIN  
           ...
           END  
       END  
       execute nsp_logerror @n_err, @c_errmsg, 'nspTTMEvaluateGMTasks'  
