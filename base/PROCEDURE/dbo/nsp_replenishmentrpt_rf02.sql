...
CREATE PROC    [dbo].[nsp_ReplenishmentRpt_RF02]
 ...
 BEGIN
   ...
   DECLARE @n_qty int   -- SOS 9963 wally 5mar03
 	...
 	IF @c_zone12 <> ''  SELECT @b_debug = CAST( @c_zone12 AS int)
 				...
 				BEGIN
 					...
 					BEGIN
                        ...
                        END     
 								IF @@ROWCOUNT = 0
 								BEGIN
 									IF @b_debug = 1 SELECT 'Not Lot Available! SKU= ' + @c_currentSKU + ' LOC=' + @c_currentLOC
 									SET ROWCOUNT 0
 									BREAK
 								END
 								SET ROWCOUNT 0
 							...
 							BEGIN
 								...
 								BEGIN
 									...
 									WHERE LOT = @c_fromlot
 										...
 										AND LOTxLOCxID.qty - qtypicked - qtyalLOCated > 0
 										AND LOTxLOCxID.qtyexpected = 0 
 										-- make sure we aren't going to try to pull from a LOCation that needs stuff to satisfy existing demAND 
 										AND LOTxLOCxID.LOC <> @c_currentLOC
 									ORDER BY LOTxLOCxID.LOC
 								END
 								IF @@ROWCOUNT = 0
