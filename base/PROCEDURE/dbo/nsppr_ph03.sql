...
GO
/************************************************************************/
/* Stored Procedure: nspPR_PH03                                         */
/* Creation Date: 23-Jan-2017                                           */
/* Copyright: LFL                                                       */
/* Written by:                                                          */
/*                                                                      */
/* Purpose: WMS-978 PH GCI PreAllocation Strategy - BULK(PALLET)        */
/*                                                                      */
...
BEGIN 
   ...
   END
     ...
     BEGIN    
      ...
      SELECT @c_LimitString =  dbo.fnc_RTrim(@c_LimitString) + ' AND Loc.LocationType NOT IN (''PICK'',''CASE'') '
       ...
       '               AND    P.SKU = @c_sku ' +
       '               AND    P.Qty > 0 ' +
       '            AND    P.PreAllocatePickCode = ''nspPR_PH03'' ' +
       '               GROUP BY P.LOT, ORDERS.Facility) P ON LOTXLOCXID.LOT = P.LOT AND P.Facility = LOC.Facility ' +
