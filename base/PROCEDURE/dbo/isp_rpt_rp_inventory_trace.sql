...
/************************************************************************/
/* Stored Procedure: isp_RPT_RP_INVENTORY_TRACE                         */
...
/* Purpose:                                                             */
/*                                                                      */
/* Called By: RPT_RP_INVENTORY_TRACE                                    */
/*                                                                      */
/* Parameters: (Input)                                                  */
/*                                                                      */
/* PVCS Version: 1.0	                                                   */
...
/* 06-Apr-2023  WZPang    1.0   DevOps Combine Script                   */
/************************************************************************/

CREATE   PROCEDURE [dbo].[isp_RPT_RP_INVENTORY_TRACE]
  ...
  DECLARE  @d_Trace_StartTime    DATETIME,
   ...
   SELECT @n_continue = 1
     ...
     BEGIN
	   ...
	   SELECT ITRN.Storerkey, ITRN.adddate, ITRN.SourceType, ITRN.Trantype, ITRN.Sku,
            ...
            ITRN.PackKey, ITRN.UOMQty, ITRN.Status
     FROM ITRN (NOLOCK)    
       ...
       -- + ' AND (ITRN.Trantype = RTRIM(@c_trantype) OR RTRIM(@c_trantype)=N''ALL'') '
        ...
        + ' OPTION(RECOMPILE) '  
         ...
         EXEC sp_executesql @sql, @c_SQLArgument, @c_storerkey_start, @c_storerkey_end
               ...
               , @dt_date_start, @dt_date_end, @d_CutofDate
         SET @c_cnt2 = @@ROWCOUNT
      END
      ELSE
      BEGIN
        ...
        + ' OR (ITRN.ToID BETWEEN RTRIM(@c_id_start)  AND RTRIM(@c_id_end) )) '
		  + ' AND (ITRN.Adddate BETWEEN @dt_date_start AND @dt_date_end ) '
        --+ ' AND (ITRN.Trantype = RTRIM(@c_trantype) OR RTRIM(@c_trantype)=N''ALL'') '
        + ' AND (ITRN.Trantype = RTRIM(@c_trantype) ) '
        + ' ORDER BY ITRN.adddate DESC '
        + ' OPTION(RECOMPILE) '  
   ...
   SET @c_Trace_Col2 = cast(@c_cnt2 as varchar)
       ...
       INSERT INTO  #TMP_ITRN   (    storerkey, Facility, effectivedate, sourcetype --4
            ...
            --sourcetype and sourcekey missing in logi
	   SELECT TOP 1000000 ITRN.Storerkey, LOC.Facility, ITRN.adddate AS Effectivedate, ITRN.SourceType, ITRN.trantype, --5
            ITRN.Sku, ITRN.FromLoc, ITRN.ToLoc, ITRN.FromID, ITRN.ToID, ITRN.Lot, ITRN.Qty, --12
            ...
            ITRN.UOM, LA.Lottable01, LA.Lottable02, LA.Lottable03, LA.Lottable04, LA.Lottable05, --20
				LA.Lottable06, LA.Lottable07, LA.Lottable08, LA.Lottable09, LA.Lottable10,  --25             
				LA.Lottable11, LA.Lottable12, LA.Lottable13, LA.Lottable14, LA.Lottable15,  --30              
            ITRN.AddWho, ITRN.AddDate, ITRN.EditWho, ITRN.EditDate, ITRN.Sourcekey,    --35
            CONVERT(NVARCHAR(30),'') AS SourceTypeDesc, CONVERT(NVARCHAR(30),'') AS ReferenceKey, --37
            CONVERT(NVARCHAR(30),'') AS ExternReferenceKey, CONVERT(NVARCHAR(30),'') AS ExternReferenceType, --39
            CONVERT(NVARCHAR(215),'') AS Remarks, ITRN.Itrnkey, SKU.Descr, SKU.Style, SKU.Color, --44        
            SKU.Size, SKU.Measurement, ITRN.PackKey, ITRN.UOMQty, ITRN.ITRNstatus -- 49                                    
...
--     AND (ITRN.Trantype = @c_trantype OR @c_trantype='ALL')
     ORDER BY ITRN.Adddate desc, ITRN.Itrnkey desc    
     SET @c_cnt3 = @@ROWCOUNT
     ...
     -- CREATE UNIQUE INDEX PKTMP_ITRN ON #TMP_ITRN (rowid)
      ...
      EXEC isp_InsertTraceInfo
         ...
         @c_TraceName = 'isp_RPT_RP_INVENTORY_TRACE',
         ...
         @c_ErrMsg = ''
	 	 ...
	 	 SELECT Storerkey, Facility, Effectivedate, SourceTypeDesc, Trantype,
            ...
            descr, style, color, size, measurement, packkey, uomqty, ITRNstatus
     FROM #TMP_ITRN
