...
/************************************************************************/
/* SP: ispPopulateTOASN_AllBirds                                        */
/* Creation Date: 19th MAR 2019                                         */
/* Copyright: IDS                                                       */
/* Written by:                                                          */
/*                                                                      */
/* Purpose: Populate ASN Detail from ORDERS                             */
/*                                                                      */
/* Usage: Copy from ispPopulateTOASN for Storerkey = 'ALLBIRDS'         */
/*                                                                      */
...
CREATE PROCEDURE [dbo].[ispPopulateTOASN_AllBirds] 
   ...
   BEGIN         
      ...
      BEGIN
         ...
         BEGIN
            ...
            SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Generate Receipt Key Failed! (ispPopulateTOASN_AllBirds)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
         END
      END    
      ELSE
      BEGIN
         SELECT @n_continue = 3
         SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err = 63526   
         SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Storer Key is BLANK! (ispPopulateTOASN_AllBirds)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
      END
