...
/*          Copy from isp_BT_Bartender_Shipper_Label_SLKR                     */
/*                                                                            */
/* Modifications log:                                                         */
/*                                                                            */
/* Date       Rev  Author     Purposes                                        */
/* 2023-07-03 1.0  CSCHONG    Devops Scripts Combine & WMS-22941              */
/******************************************************************************/

CREATE   PROC [dbo].[isp_BT_Bartender_Shipper_Label_CNKKR]
...
BEGIN
  ...
  DECLARE @d_Trace_StartTime   DATETIME,
   ...
   SET @c_SQLJOIN = +' SELECT DISTINCT F.DESCR,F.Address1,F.Address2,F.Address3,F.Address4,'   --5
                    +' F.City,F.State,F.zip,F.Country,S.company,'   --10
                    +' MB.externmbolkey,MB.carrierkey,MB.mbolkey,ORD.consigneekey,ORD.c_company,'   --15
                    +' ORD.c_address1,ORD.c_address2,ORD.c_address3,ORD.c_address4,ORD.c_city,'   --20
                    +' ORD.c_state,ORD.c_zip,ORD.c_country,ORD.c_ISOCntryCode,ORD.Type,'   --25
                    +' ORD.externorderkey,ORD.externpokey,ORD.buyerpo,ORD.Orderkey,PIF.CartonType,'   --30
                    +' PD.CartonNo,'''',PD.Labelno,'''','''', '   --35
                    +' '''','''','''',ORD.Shipperkey,ORD.Userdefine02,'     --40
                    +' '''','''',PD.DropID,ORD.loadkey,'''',ORD.Userdefine03,ISNULL(ST.SUSR5,''''),'''','''',CT.Trackingno, '  --50
                    +' '''','''','''','''','''','''','''','''','''','''' '   --60
                    +' FROM PACKDETAIL (NOLOCK) PD '
                    +' INNER JOIN dbo.PACKHEADER (NOLOCK) PH ON PD.PICKSLIPNO = PH.PICKSLIPNO '
                    +' INNER JOIN dbo.PICKDETAIL (NOLOCK) PICD ON PICD.PICKSLIPNO = PH.PICKSLIPNO '
                    +'                                         AND PICD.SKU = PD.SKU '
                    +'                                         AND PICD.dropid = PD.LABELNO '   
                    +' INNER JOIN dbo.orderdetail (nolock) ORDD ON ORDD.ORDERKEY = PICD.ORDERKEY '
                    + '                                        AND ORDD.orderlinenumber = PICD.orderlinenumber '  
                    +' INNER JOIN dbo.orderS (nolock) ORD ON ORD.ORDERKEY = ORDD.ORDERKEY'
                    ...
                    +' left outer JOIN CartonTrack CT WITH (NOLOCK) ON PD.labelno = CT.labelno and CT.carriername = ''SLKR'' and CT.keyname = ORD.Storerkey'
                    +' LEFT JOIN PACKINFO PIF WITH (NOLOCK) ON PIF.Pickslipno = PD.Pickslipno '
                    +' AND PIF.CartonNo = PD.CartonNo '
                    +' LEFT JOIN STORER ST WITH (NOLOCK) ON ST.Storerkey =  ORD.Consigneekey AND ST.ConsigneeFor = ORD.Storerkey AND ST.Type = ''2'' '
...
EXEC sp_ExecuteSql     @c_SQL
   ...
   SELECT DISTINCT col29,col31,Col33,col43
   FROM #Result

   OPEN CUR_RowNoLoop

   FETCH NEXT FROM CUR_RowNoLoop INTO @c_orderkey, @c_CntNo, @c_labelno, @c_dropid
   ...
   BEGIN
    ...
    SELECT @n_CntLabel = Count(distinct cartonno)
     ...
     -- FROM PACKHEADER PH WITH (NOLOCK)
      FROM PackDetail PD WITH (NOLOCK) --ON PD.Pickslipno =PH.Pickslipno
      ...
      FROM PackDetail PD WITH (NOLOCK) --ON PD.Pickslipno =PH.Pickslipno AND PD.Storerkey=OD.Storerkey AND PD.SKU=OD.SKU
      ...
      IF @n_cntsku = 1
      BEGIN
         SELECT DISTINCT @c_ODSKU   = PD.SKU,
                         @c_SAltSKU = S.altsku,
                         @c_SDESCR  = S.descr,
                         @c_BUSR5   = S.BUSR5
         --FROM ORDERDETAIL OD WITH (NOLOCK)
...
EXIT_SP:
   ...
   --   @c_TraceName = 'isp_BT_Bartender_Shipper_Label_CNKKR',
