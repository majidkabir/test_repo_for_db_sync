...
/*************************************************************************/  
/* Stored Procedure: ispRVWAV59                                          */  
/* Creation Date: 28-APR-2023                                            */  
/* Copyright: MAERSK                                                     */  
/* Written by:                                                           */  
/*                                                                       */  
/* Purpose: WMS-22425 VN Nike AirMi Reverse Released wave                */
/*                                                                       */  
/* Called By: Wave                                                       */  
/*                                                                       */  
/* GitLab Version: 1.0                                                   */  
...
CREATE   PROCEDURE [dbo].[ispRVWAV59]      
...
BEGIN  
   ...
   BEGIN
      IF NOT EXISTS (SELECT 1 FROM TASKDETAIL TD (NOLOCK) 
                     WHERE TD.Wavekey = @c_Wavekey AND TD.SourceType = 'ispRLWAV59'
                     AND TD.TaskType IN ('FPK','FCP') 
                     )
      BEGIN                                          
         SELECT @n_continue = 3  
         SELECT @n_err = 81010  
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': This Wave has not been released. (ispRVWAV59)'         
      END                 
   END
   ...
   BEGIN
      IF EXISTS (SELECT 1 FROM TASKDETAIL TD (NOLOCK) 
                 ...
                 AND TD.Sourcetype = 'ispRLWAV59'
                 AND TD.TaskType IN ('FPK','FCP')
                 AND TD.Status <> '0')
      BEGIN
         SELECT @n_continue = 3  
         SELECT @n_err = 81020  
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Some Tasks have been started. Not allow to Reverse Wave Released (ispRVWAV59)'       
      END                 
   END
   ...
   BEGIN
      DECLARE CUR_TASKDET CURSOR LOCAL FAST_FORWARD READ_ONLY FOR  
         ...
         AND SourceType = 'ispRLWAV59'
         AND Status = '0'
         AND TaskType IN ('FPK','FCP')
      ...
      BEGIN    
         ...
         BEGIN
            ...
            SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Delete Taskdetail Table Failed. (ispRVWAV59)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
         END 
   ...
   BEGIN
      ...
      BEGIN    
         ...
         BEGIN
           ...
           SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Pickdetail Table Failed. (ispRVWAV59)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
         END          
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update on wave Failed (ispRVWAV59)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
      END  
   END  
...
RETURN_SP:
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         END  
      END  
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'ispRVWAV59'  
