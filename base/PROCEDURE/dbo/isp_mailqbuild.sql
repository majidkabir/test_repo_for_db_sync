...
/********************************************************************************************************/    
/* Stored Procedure: isp_MailQBuild                                                                     */    
/* Creation Date: 2018-08-03                                                                            */    
/* Copyright: IDS                                                                                       */    
/* Written by: KHLim  https://jira.lfapps.net/browse/WMS-5656                                           */    
/*                                                                                                      */    
/* Purpose: create an email from MailQ & MailQDet   isp_MailQBuild 1                                    */    
/*                                                                                                      */    
/* Called By:                                                                                           */    
/*                                                                                                      */    
/* PVCS Version:                                                                                        */    
...
/* 2018-08-03   KHLim         1.0  https://jira.lfapps.net/browse/WMS-5656                              */
/* 2018-08-23   KHLim         1.1  https://jira.lfapps.net/browse/WMS-5979                              */    
/* 2019-03-12   kelvinongcy   1.2 Count sum for Qty & carton(s) https://jira.lfapps.net/browse/WMS-7986 */    
/********************************************************************************************************/    
CREATE PROC  [dbo].[isp_MailQBuild]        
   @Qid   int         
  ,@Debug   bit   = 0        
AS        
BEGIN        
...
IF      @C15Name<>''  SET @TotalCol= 15    --kelvinongcy01    
...
ELSE IF @C02Name<>''  SET @TotalCol= 2     
   ...
   -- Start: count total     
    ...
    BEGIN    
      ...
      IF       @C02Agg<> ''  SET @C01AggLabel = 'Total'    
      ELSE IF  @C03Agg<> ''  SET @C02AggLabel = 'Total'    
      ELSE IF  @C04Agg<> ''  SET @C03AggLabel = 'Total'    
      ELSE IF  @C05Agg<> ''  SET @C04AggLabel = 'Total'    
      ELSE IF  @C06Agg<> ''  SET @C05AggLabel = 'Total'    
      ELSE IF  @C07Agg<> ''  SET @C06AggLabel = 'Total'    
      ELSE IF  @C08Agg<> ''  SET @C07AggLabel = 'Total'    
      ELSE IF  @C09Agg<> ''  SET @C08AggLabel = 'Total'    
      ELSE IF  @C10Agg<> ''  SET @C09AggLabel = 'Total'    
      ELSE IF  @C11Agg<> ''  SET @C10AggLabel = 'Total'    
      ELSE IF  @C12Agg<> ''  SET @C11AggLabel = 'Total'    
      ELSE IF  @C13Agg<> ''  SET @C12AggLabel = 'Total'    
      ELSE IF  @C14Agg<> ''  SET @C13AggLabel = 'Total'    
      ELSE IF  @C15Agg<> ''  SET @C14AggLabel = 'Total'    
    
      SET @SQLAgg = N'SELECT @MailQDetAggXML = CAST ( ( SELECT ' +      
      CASE WHEN @C01Agg<> '' THEN ', '''',''td/@class'' = '''+@C01Align+''', td = '+@c01Agg+'(TRY_CAST(C01 AS INT)) ' ELSE ', '''',td = '''+@C01AggLabel+''' ' END +        
      CASE WHEN @C02Agg<> '' THEN ', '''',''td/@class'' = '''+@C02Align+''', td = '+@c02Agg+'(TRY_CAST(C02 AS INT)) ' ELSE ', '''',td = '''+@C02AggLabel+''' ' END +        
