...
/* 13/01/2020  YeeKung   1.0  TTMRP02->TTMRP15 (WMS-11247)              */   
/************************************************************************/    
CREATE   PROC [dbo].[nspTTMRP15]    
...
BEGIN    
   ...
   BEGIN    
      ...
      SET @c_errmsg = 'NSQL' + CONVERT( NVARCHAR(5), @n_err) + ': ' +     
                      'Update to TaskDetail table failed. (nspTTMRP15)' +     
   ...
   SELECT  @c_LocCount = COUNT(DISTINCT Taskdetailkey)
   FROM dbo.TaskDetail WITH (NOLOCK)        
      JOIN dbo.LOC WITH (NOLOCK) ON (TaskDetail.FromLOC = LOC.LOC)        
      JOIN dbo.AreaDetail WITH (NOLOCK) ON (AreaDetail.PutawayZone = LOC.PutAwayZone)        
   WHERE AreaDetail.AreaKey = @c_AreaKey01        
   ...
   DECLARE @c_UCCQTY INT = 0 , @c_LocationHandling NVARCHAR(20) = ''
   ...
   SELECT @c_LocationHandling = LocationHandling FROM LOC WITH (NOLOCK) WHERE LOC = @c_FromLOC
   ...
   ELSE  
   BEGIN
      IF @c_Pickmethod ='PP' and ISNULL(@c_QtyAvaiable,0) <= 0 AND @c_LocationHandling = '1'
      BEGIN  
         UPDATE TaskDetail WITH (Rowlock)      
         SET Pickmethod='FP'  
         WHERE  taskdetailkey=@c_TaskDetailkey 
   ...
   BEGIN    
      ...
      BEGIN    
        ...
        BEGIN    
          ...
          END    
        END    
        execute nsp_logerror @n_err, @c_errmsg, 'nspTTMRP15'    
