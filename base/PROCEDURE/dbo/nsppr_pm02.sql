...
/* 20-Mar-2013 1.0  NJOW01   272654-Shelflife control enhancements      */
/************************************************************************/

CREATE PROC [dbo].[nspPR_PM02] 
   ...
   BEGIN
      ...
      DECLARE @c_SQLStatement NVARCHAR(2048), 
       ...
       BEGIN
          ...
          IF @n_SkuShelfLife > 0
          BEGIN
             SELECT @c_Limitstring = dbo.fnc_RTrim(@c_LimitString) + " AND DATEDIFF(day, Lottable04, GETDATE()) <= " + CAST(@n_SkuShelfLife as NVARCHAR(10))
          END 
      END
