...
--   WHERE Loc = @c_TmpLoc
  ...
  IF RTRIM(@c_Lottable06) <> ''
   ...
   SET @c_OtherStatement =  RTRIM(@c_OtherStatement) + ' AND SKU.BUSR3 = ''DGE-GEN'''
   ...
   SET @c_SQLStatement = N'DECLARE  CURSOR_CANDIDATES CURSOR FAST_FORWARD READ_ONLY FOR'
                       ...
                       + ' AND ID.STATUS = CASE WHEN RID.Jobkey IS NULL THEN ''OK'' ELSE ''HOLD'' END'               				--(Wan01)                       + ' ' + @c_OtherStatement
                       + ' ORDER BY SKU.ShelfLife, ISNULL(LOTATTRIBUTE.Lottable04,''1900-01-01''), LOTATTRIBUTE.Lottable01' 
                       +         ' ,MS.NoOfSku DESC'                                                                 --(Wan01)
--                       + ' ORDER BY CASE WHEN SKU.ShelfLife > 0 THEN 10 ELSE 110 END '
--                       +          ',CASE WHEN LOTATTRIBUTE.Lottable04 > ''1900-01-01'' AND LOTATTRIBUTE.Lottable04 IS NOT NULL '
--                       +               ' THEN 20 ELSE 120 END'
--                       +          ',CASE WHEN LOTATTRIBUTE.Lottable01 > '''' AND LOTATTRIBUTE.Lottable01 IS NOT NULL '
--                       +               ' THEN 30 ELSE 130 END'
