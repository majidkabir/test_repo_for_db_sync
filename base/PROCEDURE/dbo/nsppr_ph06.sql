...
GO
/************************************************************************/
/* Stored Procedure: nspPR_PH06                                         */
/* Creation Date: 05-Apr-2018                                           */
/* Copyright: LFL                                                       */
/* Written by:                                                          */
/*                                                                      */
/* Purpose: WMS-4427 PH Fonterra Allocation Based on ConsigneeSKU       */
...
BEGIN 
   ...
   BEGIN
       ...
       IF EXISTS(SELECT 1 FROM CODELKUP (NOLOCK)
                 ...
                 AND Code = 'NoPartialItemAlloc'
                 AND Long = 'nspPR_PH06'
                 AND Short <> 'N')
       BEGIN                   
          SELECT @n_QtyAvailable = SUM(LOTXLOCXID.Qty-LOTXLOCXID.QtyAllocated-LOTXLOCXID.QtyPicked-LOTXLOCXID.QtyReplen) - MIN(ISNULL(P.QtyPreallocated, 0))
          FROM LOTXLOCXID (NOLOCK) 
             JOIN LOT (NOLOCK) ON LOTXLOCXID.LOT = LOT.LOT
          JOIN LOTATTRIBUTE (NOLOCK) ON LOTXLOCXID.LOT = LOTATTRIBUTE.LOT
          JOIN LOC (NOLOCK) ON LOTXLOCXID.LOC = LOC.LOC
          ...
          IF ISNULL(@n_QtyOrder,0) > ISNULL(@n_QtyAvailable,0)
          BEGIN
            DECLARE PREALLOCATE_CURSOR_CANDIDATES CURSOR FAST_FORWARD READ_ONLY FOR
            SELECT TOP 0 NULL, NULL, NULL, 0
       ...
       --BEGIN
         ...
         WHERE SKU.StorerKey = dbo.fnc_RTrim(@c_storerkey)
          ...
          --  Else
           ...
           --          WHERE Configkey = 'nspPR_PH06_LOT4EXP' 
           --          AND Storerkey = @c_Storerkey
           --          AND Svalue = '1')  
           --BEGIN         
