...
/************************************************************************/        
/* Trigger:  isp_WMS2WCSRoutingUseLinkedServer                          */        
/* Creation Date: 05-Jul-2010                                           */        
/* Copyright: IDS                                                       */        
/* Written by: AQSACM                                                   */        
/*                                                                      */        
/* Purpose:  Need set option on and cannot use transaction block        */        
/*                                                                      */        
/* PVCS Version: 1.1                                                    */        
/*                                                                      */        
/* Version: 5.4                                                         */        
...
/* 2021-05-12   James     1.1   WMS-15656 Copy from isp_WMS2WCSRouting  */      
/************************************************************************/        
        
CREATE PROCEDURE [dbo].[isp_WMS2WCSRoutingUseLinkedServer]         
...
BEGIN        
   ...
   SET ANSI_WARNINGS ON    -- Need set to On here (Important) 
 ...
 DECLARE  @c_ActionFlag    NVARCHAR(6),        
   ...
   -- (james01)
   IF CHARINDEX('LINK', @c_TargetDBName) > 0        
   BEGIN
      SET @nIsLinkedServer = 1
      --SET @c_ExecStatements = N'SET ANSI_NULLS ON SET ANSI_WARNINGS ON '
 ...
 -- Check SourceKey/WCSKEY        
   ...
   BEGIN          
      ...
      BEGIN  
         ...
         --SET @c_ExecStatements = CASE WHEN @nIsLinkedServer = 1 THEN 'SET ANSI_NULLS ON SET ANSI_WARNINGS ON ' ELSE '' END
         SET @c_ExecStatements = @c_ExecStatements +        
                  N'IF NOT EXISTS(SELECT 1 FROM ' + ISNULL(RTRIM(@c_TargetDBName),'') + '.dbo.ORDER_HEADER WITH (NOLOCK) '        
         ...
         BEGIN      
            --SET @c_ExecStatements = CASE WHEN @nIsLinkedServer = 1 THEN 'SET ANSI_NULLS ON SET ANSI_WARNINGS ON ' ELSE '' END
            SET @c_ExecStatements = @c_ExecStatements +        
                     N'IF EXISTS(SELECT 1 FROM ' + ISNULL(RTRIM(@c_TargetDBName),'') + '.dbo.ORDER_HEADER WITH (NOLOCK) '        
                    + ' WHERE SEQNUM_HEADER = @n_SEQNUM_HEADER) '      
         ...
         BEGIN        
            ...
            --SET @c_ExecStatements = CASE WHEN @nIsLinkedServer = 1 THEN 'SET ANSI_NULLS ON SET ANSI_WARNINGS ON ' ELSE '' END
            SET @c_ExecStatements = @c_ExecStatements +        
                   N'IF NOT EXISTS(SELECT 1 FROM ' + ISNULL(RTRIM(@c_TargetDBName),'') + '.dbo.ORDER_DETAIL WITH (NOLOCK) '       
         ...
         SET @c_ExecStatements = @c_ExecStatements +        
         --SET @c_ExecStatements = 
         N'UPDATE ' + ISNULL(RTRIM(@c_TargetDBName),'') + '.dbo.ORDER_HEADER WITH (ROWLOCK) '         
         ...
         BEGIN        
            ...
            SET @c_ExecStatements = @c_ExecStatements +        
            --SET @c_ExecStatements = 
            N'UPDATE ' + ISNULL(RTRIM(@c_TargetDBName),'') + '.dbo.ORDER_DETAIL WITH (ROWLOCK) '         
                    + ' SET STATE_HCOM = ''10'', DATE_INSERTED = Getdate() '         
                    --+ ' SET DATE_INSERTED = Getdate() '         
                    + ' WHERE SEQNUM_HEADER = @n_SEQNUM_HEADER AND STATE_HCOM = ''0'' '                       
