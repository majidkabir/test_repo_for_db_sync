...
--  				and lottable02 = @c_lottable02		
  	...
  	begin
      IF ISNULL(dbo.fnc_RTRIM(@c_lottable03),'') = 'GOLDC'
      BEGIN
        IF @b_debug = 1  
        BEGIN 
        ...
        END
  		   ...
  		   select lot.storerkey, lot.sku, lot.lot, 
			   ...
			   join lot (nolock) on lotxlocxid.lot = lot.lot
  			   join lotattribute (nolock) on lotxlocxid.lot = lotattribute.lot
  			   join loc (nolock) on lotxlocxid.loc = loc.loc
            JOIN SKU (NOLOCK) ON (LOTxLOCxID.SKU = SKU.SKU)
            LEFT OUTER JOIN (SELECT p.lot, ORDERS.Facility, QtyPreallocated = SUM(p.Qty) 
         					     FROM   PreallocatePickdetail p (NOLOCK), ORDERS (NOLOCK) 
         					     WHERE  p.Orderkey = ORDERS.Orderkey 
         					     AND    p.Storerkey = dbo.fnc_RTrim(@c_storerkey)
   ...
   --  				and lottable02 = @c_lottable02
            ...
            having SUM(LOTXLOCXID.QTY - LOTXLOCXID.QTYALLOCATED - LOTXLOCXID.QTYPICKED) - MIN(ISNULL(P.QTYPREALLOCATED, 0)) > 0
  			   order by CASE SKU.BUSR7 WHEN 'GOLDC' THEN 0 WHEN NULL THEN 1 WHEN ' ' THEN 2 ELSE 3 END ASC, CASE LOC.Putawayzone WHEN 'GOLDC' THEN 0 WHEN NULL THEN 1 WHEN ' ' THEN 2 ELSE 3 END ASC, lottable04, lottable02
      END
      ELSE
      BEGIN
