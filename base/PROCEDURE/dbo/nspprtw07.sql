   ...
   IF dbo.fnc_LTrim(dbo.fnc_RTrim(@c_lot)) IS NOT NULL
      ...
      BEGIN
         ...
         BEGIN 
            ...
            ELSE
            BEGIN
               SELECT @n_StorerMinShelfLife = CASE WHEN ISNUMERIC(Sku.SUSR2) > 0 THEN ((ISNULL(Sku.SUSR2,0) ) * -1)
                                              ELSE 0 END
               FROM  Sku (nolock) 
               WHERE Sku.Sku = @c_SKU 
               AND   Sku.Storerkey = @c_Storerkey             
            ...
            IF ISNUMERIC(@n_StorerMinShelfLife) > 0 
            BEGIN
            ...
            ELSE
            BEGIN
               SELECT @c_Condition = dbo.fnc_RTrim(@c_Condition) + " AND ( Lotattribute.Lottable04 > GetDate() OR Lotattribute.Lottable04 IS NULL ) " 
            END 
         END 
         ...
         -- 12-Oct-2004 YTWAN Fixed if overallocated for lot in pick face
			SELECT @c_condition = dbo.fnc_RTrim(@c_Condition) + " HAVING SUM(LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED) - MIN(LOT.QTYPREALLOCATED) "
         SELECT @c_Condition = dbo.fnc_RTrim(@c_Condition) + "        - SUM(CASE WHEN (SKUxLOC.LocationType = 'CASE' OR SKUxLOC.LocationType = 'PICK') AND "
			SELECT @c_Condition = dbo.fnc_RTrim(@c_Condition) + "                        (LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED) > 0 "
			SELECT @c_Condition = dbo.fnc_RTrim(@c_Condition) + "                   THEN LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED "
			SELECT @c_Condition = dbo.fnc_RTrim(@c_Condition) + "                   ELSE 0 "
			SELECT @c_Condition = dbo.fnc_RTrim(@c_Condition) + "                   END)  >= " + dbo.fnc_RTrim(CAST ( @n_uombase AS NVARCHAR(10) ) ) + " "   
