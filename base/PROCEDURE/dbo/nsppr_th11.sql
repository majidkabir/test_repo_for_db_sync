...
/*          condition                                                    */
/*          Pre-Allocation Strategy of RMBL (FEFO)                       */
...
BEGIN
   ...
   IF @b_debug = 1  
   BEGIN  
      SELECT 'nspPR_TH11 : Before Lot Lookup .....'  
   ...
   IF @b_debug = 1  
   BEGIN  
      SELECT 'nspPR_TH11 : After Lot Lookup .....'  
      SELECT '@c_lot'=@c_lot,'@c_lottable01'=@c_lottable01, '@c_lottable02'=@c_lottable02, '@c_lottable03'=@c_lottable03  
      ...
      BEGIN  
         ...
         BEGIN  
            ...
            BEGIN  
               ...
               BEGIN  
                  IF @n_shelflife < 13  
                     SELECT @c_Limitstring = ISNULL(RTRIM(@c_LimitString),'') + ' AND CONVERT(CHAR(8),Lottable04, 112) >= CONVERT(CHAR(8), DATEADD(MONTH, @n_shelflife, GETDATE()), 112) '
                  ELSE  
                     SELECT @c_Limitstring = ISNULL(RTRIM(@c_LimitString),'') + ' AND CONVERT(CHAR(8),Lottable04, 112) >= CONVERT(CHAR(8), DATEADD(DAY, @n_shelflife, GETDATE()), 112) '
               END   
            END  
         ...
         BEGIN
            SELECT @c_Limitstring = ISNULL(RTRIM(@c_Limitstring),'') + ' AND LOTxLOCxID.LOC = ''OVER3183'' '
         END
         ELSE
         BEGIN
            SELECT @c_Limitstring = ISNULL(RTRIM(@c_Limitstring),'') + ' AND LOC.STATUS = ''OK'' '
         END
         ...
         SELECT @c_sql = ' DECLARE PREALLOCATE_CURSOR_CANDIDATES CURSOR FAST_FORWARD READ_ONLY FOR ' +  
            ...
            CASE WHEN @c_FilterHOSTWHCode = 'Y' AND ISNULL(@c_Load_Userdef1,'') <> '' THEN  
            ' LEFT OUTER JOIN (SELECT p.Lot, ORDERS.Facility, QtyPreallocated = SUM(P.Qty) ' +  
            ...
            ' AND LOT.STATUS = ''OK'' ' +
            ' AND ID.STATUS = ''OK'' And LOC.LocationFlag = ''NONE'' ' +  
