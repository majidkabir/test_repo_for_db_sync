...
/******************************************************************************/    
/* Copyright: LFL                                                             */    
/* Purpose: isp_BT_Bartender_US_GNCTN_CL99                                 */    
/*                                                                            */    
/* Modifications log:                                                         */    
/*                                                                            */    
...
CREATE   PROC [dbo].[isp_BT_Bartender_US_GNCTN_CL99]
(
...
+ N'	P.TOTALSQTY,P.PO,' + CHAR(13)
+ N'	P.PALLETTYPE,P.CAR,P.SKID,P.PALLETSTACK,' + CHAR(13)
+ N'	P.CUSTOMER,P.CONTACT,P.DEPT,SUBSTRING(P.CARTONID,LEN(P.CARTONID)-8,9),' + CHAR(13)
+ N'	P.SEQ,P.COUNTS,P.BOL,P.PRO,P.OCN,32 AS S3,P.SKUGROUP AS MASTERGROUP,P.CARTONID AS SSCC,' + CHAR(13)
+ N'        '''', '''', '''', '''', '''', '''', '''', ' + CHAR(13) --50
+ N'        '''', '''', '''', '''', '''', '''', '''', '''', '''', ''''  ' + CHAR(13) --60
+ N'	FROM (' + CHAR(13)
...
+ N'	CAST(SUBSTRING(O.ConsigneeKey,LEN(O.ConsigneeKey)-4,5) AS INT) AS CUSTOMER,' + CHAR(13)
+ N'	CASE WHEN O.M_CONTACT1 IS NOT NULL THEN O.M_CONTACT1 ELSE O.C_CONTACT1 END AS CONTACT,' + CHAR(13)
+ N'	OI.NOTES AS DEPT,PI.UCCNo AS CARTONID,COUNT(DISTINCT PD.CARTONNO)AS SEQ, MAX(PD.CARTONNO) AS COUNTS,' + CHAR(13)
--PH.TTLCNTS AS COUNTS,' + CHAR(13)
+ N'	M.MBOLKEY AS BOL ,M.Carrieragent AS PRO,O.ExternOrderKey AS OCN,WD.WkOrdUdef1 AS SPECIALINS,' + CHAR(13)
+ N'	'''' AS HANGERS,'''' AS PACKINGLIST,O.BillToKey,S.SKUGROUP,SUM(PI.Qty) AS TOTALSQTY,' + CHAR(13)
