...
/************************************************************************/
/* Stored Proc: isp_RPT_LP_PLISTC_004                                   */
/* Creation Date: 29-04-2022                                            */
/* Copyright: LF Logistics                                              */
/* Written by: Amal                                                     */
/*                                                                      */
/* Purpose: WMS-19504 - Migrate WMS report to Logi Report               */
/*        : r_dw_print_pickorder83 (PH)                                 */
/*                                                                      */
/* Called By:RPT_LP_PLISTC_004                                          */
/*          :                                                           */
/* PVCS Version: 1.0                                                    */
/*                                                                      */
/* Version: 7.0                                                         */
...
/************************************************************************/
CREATE PROC [dbo].[isp_RPT_LP_PLISTC_004]
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         IF @c_CSCompanies <> ''
         BEGIN
            SET @c_CSCompanies = @c_CSCompanies + N', '
         END
      ...
      BEGIN
         SET @c_Lot02Label = N'Batch No'
      END

      IF ISNULL(RTRIM(@c_Lot04Label), '') = ''
      BEGIN
         SET @c_Lot04Label = N'Exp Date'
      END
      ...
      SELECT LOC.LogicalLocation
           ...
           , LA.Lottable04
           , CASE WHEN PCK.PackUOM4 = OD.UOM THEN PCK.Pallet
                  ELSE 0 END
           , CASE WHEN PCK.PackUOM1 = OD.UOM THEN PCK.CaseCnt
                  ELSE 0 END
           , CASE WHEN PCK.PackUOM2 = OD.UOM THEN PCK.InnerPack
                  ELSE 0 END
      FROM ORDERS OH WITH (NOLOCK)
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN --@c_SortLocType = 'BULK'
               SET @n_QtyInPLT = CASE WHEN @n_Pallet > 0 THEN @n_Qty / @n_Pallet
                                      ELSE 0 END
               IF @n_QtyInPLT > 0
               BEGIN
                  SET @c_PickType = N'FP'
               ...
               ELSE
               BEGIN --loose PLT
                  SET @n_QtyInCS = CASE WHEN @n_CaseCnt > 0 THEN @n_Qty / @n_CaseCnt
                                        ELSE 0 END
                  SET @n_QtyInIP = CASE WHEN @n_InnerPack > 0 THEN @n_Qty / @n_InnerPack
                                        ELSE 0 END

                  IF @n_QtyInCS > 0
                  BEGIN
                     SET @c_PickType = N'PPFC'
                     SET @n_QtyRemain = (@n_QtyInCS * @n_CaseCnt)
                  END
                  ELSE IF @n_QtyInIP > 0
                  BEGIN --@n_QtyInCS <= 0
                     SET @c_PickType = N'PPLI'
                     SET @n_QtyRemain = (@n_QtyInIP * @n_InnerPack)
                  END
                  ELSE
                  BEGIN
                     SET @c_PickType = N'PPLC'
                     SET @n_QtyInEA = @n_Qty
                     SET @n_QtyRemain = @n_Qty
                  END
               END --loose PLT end
            END --@c_SortLocType = 'BULK'
            ELSE IF @c_SortLocType = 'CASE'
            BEGIN
               SET @n_QtyInCS = CASE WHEN @n_CaseCnt > 0 THEN @n_Qty / @n_CaseCnt
                                     ELSE 0 END
               SET @n_QtyInIP = CASE WHEN @n_InnerPack > 0 THEN @n_Qty / @n_InnerPack
            ...
            INSERT INTO #TMP_PCK (Loadkey, CSCompanies, ConsoOrderkey, OrderGroup, SortLocType, PickType, PickZone
                                , LocationType, LogicalLocation, Storerkey, Sku, SkuDescr, Loc, ID, Pallet, CaseCnt
                                , Qty, QtyInPLT, QtyInCS, QtyInEA, Lot02Label, Lot04Label, Lottable02, Lottable04
                                , INNERPACK, QtyInIP)
            VALUES (@c_Loadkey, @c_CSCompanies, @c_ConsoOrderkey, @c_OrderGroup, @c_SortLocType, @c_PickType
                  , @c_PickZone, @c_LocationType, @c_LogicalLocation, @c_Storerkey, @c_Sku, @c_SkuDescr, @c_Loc, @c_ID
                  , @n_Pallet, @n_CaseCnt, @n_Qty, @n_QtyInPLT, @n_QtyInCS, @n_QtyInEA, @c_Lot02Label, @c_Lot04Label
                  , @c_Lottable02, @dt_Lottable04, @n_InnerPack, @n_QtyInIP)
   ...
   BEGIN
      ...
      BEGIN
         ...
         JOIN LOTATTRIBUTE LA WITH (NOLOCK) ON  (PD.Lot = LA.Lot)
                                            ...
                                            AND (ISNULL(RTRIM(TPK.Lottable04), '1900-01-01') = ISNULL(
                                                                                                  RTRIM(LA.Lottable04)
                                                                                                , '1900-01-01'))
         ...
         BEGIN
            ...
            IF  @c_PickType IN ( 'PPFC', 'FC' )
            AND @n_Qty < @n_CaseCnt
            BEGIN
               GOTO NEXT_PD
            END

            IF  @c_PickType IN ( 'PPLC', 'LC' )
            AND @n_Qty >= @n_CaseCnt
            BEGIN
               GOTO NEXT_PD
            END
      ...
      ELSE
      BEGIN
         SELECT @c_PrintedFlag = CASE WHEN PickType = '1' THEN 'Y' ELSE 'N' END
   ...
   INSERT INTO #TMP_PKPAGE (PageGroup, Loadkey, PickSlipNo, PrintedFlag, CSCompanies, ConsoOrderkey, OrderGroup
                          , SortLocType, PickType, PickTypeDescr, PickZone, LocationType, LogicalLocation, Storerkey
                          , Sku, SkuDescr, Loc, ID, Pallet, CaseCnt, Qty, QtyInPLT, QtyInCS, QtyInEA, Lot02Label
                          , Lot04Label, Lottable02, Lottable04, INNERPACK, QtyInIP)
   SELECT CEILING(((ROW_NUMBER() OVER (PARTITION BY PickSlipNo
                                       ORDER BY TPK.Loadkey
                                              , TPK.OrderGroup
                                              , TPK.PickZone
                                              , TPK.SortLocType
        ...
        , TPK.SkuDescr
        , TPK.Loc
        , ID = CASE WHEN SortLocType = 'BULK' THEN TPK.ID
                    ELSE '' END
        , TPK.Pallet
        , TPK.CaseCnt
        , TPK.Qty
   ...
   INSERT INTO #TMP_PAGING (Loadkey, PickSlipNo, OrderGroup, PickZone, SortLocType, PickType, PageGroup, PageNo)
   SELECT DISTINCT TPKPG.Loadkey
                 , TPKPG.PickSlipNo
                 , TPKPG.OrderGroup
                 , TPKPG.PickZone
                 , TPKPG.SortLocType
                 , TPKPG.PickType
                 , TPKPG.PageGroup
                 , PageNo = DENSE_RANK() OVER (PARTITION BY TPKPG.Loadkey
                                                          , TPKPG.OrderGroup
                                               ORDER BY TPKPG.Loadkey
                                                      , TPKPG.OrderGroup
                                                      , TPKPG.PickZone
                                                      , TPKPG.SortLocType
                                                      , TPKPG.PickType
