...
GO
/************************************************************************/  
/* Stored Procedure: ispPOA04                                           */  
/* Creation Date: 24-Apr-2018                                           */  
/* Copyright: LFL                                                       */  
/* Written by:                                                          */  
/*                                                                      */  
/* Purpose: WMS-4427 PH Remove partial allocation line                  */
/*                                                                      */  
...
BEGIN    
   ...
   BEGIN    
      ...
      BEGIN    
         SELECT @n_Continue = 3    
         SELECT @n_Err = 63500    
         SELECT @c_ErrMsg='NSQL'+CONVERT(NVARCHAR(5),@n_Err)+': Loadkey & Orderkey & Wavekey are Blank (ispPOA04)'
         GOTO EXIT_SP    
      END    
   ...
   BEGIN       
      ...
      BEGIN           	      	 
         ...
         BEGIN
               ...
               SELECT @n_Continue = 3    
               SELECT @n_Err = 63510    
               SELECT @c_ErrMsg='NSQL'+CONVERT(NVARCHAR(5),@n_Err)+': Error delete PICKDETAIL (ispPOA04)'
               GOTO EXIT_SP    
            END
...
EXIT_SP:
   ...
   BEGIN    
      ...
      BEGIN    
         ...
         END    
      END    
      EXECUTE nsp_logerror @n_Err, @c_ErrMsg, 'ispPOA04'    
