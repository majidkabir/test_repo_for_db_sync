...
GO
/************************************************************************/    
/* Store Procedure:  isp_GetPackList11                                  */    
/* Creation Date: 11-Nov-2022                                           */    
/* Copyright: LFL                                                       */    
/* Written by: Mingle                                                   */    
/*                                                                      */    
/* Purpose:  WMS-21091 [TW] MOT B2B Packing List New                    */    
/*                                                                      */    
/* Usage:  Used for report dw = r_dw_print_packlist_11                  */    
/*                                                                      */    
/* Called By: Exceed                                                    */    
/*                                                                      */    
/* PVCS Version: 1.0                                                    */    
...
/* 11-Nov-2022  Mingle  1.0   WMS-21091 DevOps Combine Script(Created)  */    
/************************************************************************/    
    
CREATE   PROC dbo.isp_GetPackList11 (@c_Pickslipno NVARCHAR(10))    
AS    
BEGIN    
   SET NOCOUNT ON    
   SET ANSI_NULLS OFF    
   ...
   SELECT @c_Loadkey = orders.loadkey    
   FROM orders(NOLOCK)    
   ...
   ----JOIN loadplandetail(NOLOCK) ON LoadPlanDetail.OrderKey = ORDERDETAIL.OrderKey    
   --JOIN packheader(NOLOCK) ON PackHeader.OrderKey = orders.OrderKey    
   --WHERE packheader.pickslipno = 'P009138821'    
    
   SELECT DISTINCT PACKHEADER.OrderKey    
   INTO #TempOrder    
   FROM  ORDERS (NOLOCK)    
   JOIN PACKHEADER(NOLOCK) ON PackHeader.OrderKey = ORDERS.OrderKey    
   WHERE packheader.pickslipno = @c_Pickslipno    
   ...
   BEGIN    
      ...
      BEGIN    
       ...
       --Qty = SUM(Packdetail.QTY ),   
            ...
            --JOIN PICKDETAIL PD WITH (NOLOCK) ON PD.Storerkey = SKU.StorerKey AND PD.Sku = SKU.Sku AND PD.OrderLineNumber = OD.OrderLineNumber  
            --JOIN PACKHEADER(NOLOCK) ON PackHeader.OrderKey = OD.OrderKey     
            --JOIN packdetail WITH (NOLOCK) ON PackDetail.PickSlipNo = PackHeader.PickSlipNo AND PackDetail.SKU = OD.Sku    
...
--   GROUP BY OD.Orderkey    
   ...
   SELECT T.PickSlipNo, T.Loadkey, T.OrderKey, T.ExternOrderKey, T.Notes,    
         ...
         ,  T.Division, T.InvoiceNo, T.OrderType, T.TotalCartons,T.OrdDetUserdef02,T.OrdDetUserdef09,dropid                   
   FROM #TempPickSlip T    
