...
/*                            Performance Issues.                                   */
/* 11-Sep-2917  Leong         Duplicate from nspArchiveReceipt.                     */
...
CREATE PROCEDURE [dbo].[nspArchiveReceipt2]  
   ...
   @c_WhereClauseExtra NVARCHAR (4000) = ''  --kocy02
AS  
BEGIN  -- main  
   SET NOCOUNT ON   
   SET ANSI_NULLS OFF  
            ...
            BEGIN  
               ...
               BEGIN   
                  ...
                  SELECT @local_c_errmsg =  
                   ": Update of Archivecop failed - ReceiptDetail. (nspArchiveReceipt2) " + " ( " +  
                   " SQLSvr MESSAGE = " + dbo.fnc_LTrim(dbo.fnc_RTrim(@local_c_errmsg)) + ")"  
                  ROLLBACK TRAN   
               END    
               ELSE  
               ...
               BEGIN   
                  ...
                  SELECT @local_c_errmsg = CONVERT(char(5),@local_n_err)  
                  SELECT @local_c_errmsg = ": Update of Archivecop failed - ReceiptSerialno. (nspArchiveReceipt2) " + " ( " +  
                                           " SQLSvr MESSAGE = " + dbo.fnc_LTrim(dbo.fnc_RTrim(@local_c_errmsg)) + ")"  
                  ROLLBACK TRAN   
               END    
               ELSE  
