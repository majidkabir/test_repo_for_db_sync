...
/* 02-Oct-2015  James    3.9  350677-Add storerconfig to control release*/
/*                            Store Pick Task in PPA either group by SKU*/
/*                            or consignee (james03)                    */
/* 30-Aug-2016  Shong    4.0  Changing Replenishment Calculation logic  */
/* 20-Oct-2016  James    4.1  WMS493-Group SINGLES task based on orders */
/*                            Incoterm (james05)                        */
/* 30-Nov-2016  NJOW07   4.2  Performance Tuning                        */
...
BEGIN  
   ...
   DECLARE @n_SGANo        INT,  
           ...
           @c_SplitPickTaskByStore NVARCHAR(10) --NJOW07

   -- (james03)
   DECLARE @c_StorePPATaskGroupBySKU   NVARCHAR( 10)
   
   SET @b_mydebug = 9  
   SET @c_myTraceName = 'nspLPRTSK3'  
   ...
   SELECT @n_continue = 1  
    ...
    BEGIN  
        ...
        BEGIN  
            ...
            SELECT @c_ErrMsg = 'NSQL'+CONVERT(NVARCHAR(5) ,@n_err)+  
                   ': Update of LoadPlan Failed (nspLPRTSK3)'+' ( '  
                  +' SQLSvr MESSAGE='+@c_ErrMsg  
                  +' ) '  
      ...
      EXECUTE nspGetRight null,     -- facility  
         ...
         null,                      -- Sku  
         'StorePPATaskGroupBySKU',  -- Configkey  
         @b_success                 output,  
         @c_StorePPATaskGroupBySKU  output,  
         @n_err                     output,  
         @c_errmsg                  output  

      IF @c_StorePPATaskGroupBySKU <> '1' OR @b_success <> 1  
      BEGIN  
         SET @c_StorePPATaskGroupBySKU = ''  
      END  
      ...
      IF @c_StorePPATaskGroupBySKU = '1'
       ...
       CREATE TABLE #Incoterm4PPK  (  
          RowRef        BIGINT IDENTITY(1,1)  Primary Key,  
          Incoterm      NVARCHAR(10))  

       IF EXISTS ( SELECT 1 FROM dbo.CODELKUP WITH (NOLOCK)
                   WHERE ListName = 'PPKByIncoT'
                   AND   (@c_Storerkey = '') OR (StorerKey = @c_Storerkey))
       BEGIN
         INSERT INTO #Incoterm4PPK (Incoterm) 
         SELECT Code
     ...
     , P.PackKey      , 'nspLPRTSK3', P.AddDate  
      ...
      -- (james03)
      IF @c_StorePPATaskGroupBySKU = '1'
         INSERT INTO #StorePick   ( PickDetailKey, StorerKey, SKU, LOT, LOC, ID, Qty, ConsigneeKey, StoreLoc, Done )  
         SELECT p.PickDetailKey, p.StorerKey, p.SKU, p.LOT, p.LOC, p.ID, p.Qty, O.ConsigneeKey,  
         ISNULL(StoreLOC.LOC, ''), 'N'  
         FROM  Orders O WITH (NOLOCK)  
         JOIN  LoadPlanDetail lpd WITH (NOLOCK) ON lpd.OrderKey = O.OrderKey  
    ...
    BEGIN  
      ...
      CREATE TABLE  #Orders (  
         ...
         ,Incoterm  NVARCHAR(10) -- (james05)
         ,Loadkey   NVARCHAR(10) --NJOW07
         ,Consigneekey NVARCHAR(15) --NJOW07
         )  
  
       INSERT INTO #Orders  ( OrderKey, OrderType, SKUCount, TotalPick,   
                            Userdefine01, Incoterm, Loadkey, Consigneekey ) --NJOW06/(james05)
         SELECT o.OrderKey  
       ...
       BEGIN  
          ...
          -- (james06)
          IF EXISTS ( SELECT 1 FROM #Incoterm4PPK WHERE IncoTerm = @c_IncoTerm)
             SET @c_TaskType = 'PPK'   -- Priority pick TASK TYPE
          ELSE
            SET @c_TaskType = 'PK'
          ...
          BEGIN  
              ...
              SELECT @c_ErrMsg = 'NSQL'+CONVERT(NVARCHAR(5) ,@n_err)+  
                     ': Unable to Get TaskDetailKey (nspLPRTSK3)'  
                +' ( '+' SQLSvr MESSAGE='+@c_ErrMsg  
                    +' ) '  
          ...
          BEGIN  
              ...
              VALUES  
                (  
                  @c_TaskDetailKey, @c_TaskType, @c_Storerkey, @c_SKU, '' -- Lot,  
                  , '' -- UOM,  
                  , 0  -- UOMQty,  
                  , @n_PickQty  
                  , @c_FromLoc -- FromLoc  
              ...
              BEGIN  
                  ...
                  SELECT @c_ErrMsg = 'NSQL'+CONVERT(NVARCHAR(5) ,@n_err)+  
                         ': Insert Into TaskDetail Failed (nspLPRTSK3)'  
                        +' ( '+' SQLSvr MESSAGE='+@c_ErrMsg  
                        +' ) '  
                  CLOSE C_SinglePickTask  
                  DEALLOCATE C_SinglePickTask  
              ...
              FOR  
                 ...
                 WHERE p.STATUS = '0'  
                   ...
                   AND o.IncoTerm = @c_IncoTerm    --(james05)
              ...
              BEGIN  
                 ...
                 BEGIN  
                     ...
                     SELECT @c_ErrMsg = 'NSQL'+CONVERT(NVARCHAR(5) ,@n_err)+  
                            ': Update of Pickdetail Failed (nspLPRTSK3)'+' ( '  
                           +' SQLSvr MESSAGE='+@c_ErrMsg  
                           +' ) '  
                     GOTO QUIT_SP  
                 END  
                 ...
                 BEGIN  
                    ...
                    BEGIN  
                        ...
                        SELECT @c_ErrMsg = 'NSQL'+CONVERT(NVARCHAR(5) ,@n_err)+  
                               ': Update of TaskDetail.ListKey Failed (nspLPRTSK3)'+' ( '  
                              +' SQLSvr MESSAGE='+@c_ErrMsg  
                              +' ) '  
                        GOTO QUIT_SP  
                    END  
       ...
       BEGIN  
          ...
          -- (james06)
          IF EXISTS ( SELECT 1 FROM #Incoterm4PPK WHERE IncoTerm = @c_IncoTerm)
             SET @c_TaskType = 'PPK'   -- Priority pick
          ELSE
            SET @c_TaskType = 'PK'
          ...
          BEGIN  
              ...
              BEGIN  
                 ...
                 BEGIN  
                     ...
                     SELECT @c_ErrMsg = 'NSQL'+CONVERT(NVARCHAR(5) ,@n_err)+  
                           ': Update Pick Detail Failed (nspLPRTSK3)'  
                           +' ( '+' SQLSvr MESSAGE='+@c_ErrMsg  
 +' ) '  
       ...
       BEGIN  
           ...
           SELECT @c_ErrMsg = 'NSQL'+CONVERT(NVARCHAR(5) ,@n_err)+  
                  ': Update of Orders.Userdefine01 Failed (nspLPRTSK3)'+' ( '  
                 +' SQLSvr MESSAGE='+@c_ErrMsg  
                 +' ) '  
           GOTO QUIT_SP  
       END  
      ...
      DECLARE @c_TableName NVARCHAR(30)  
       ...
       BEGIN  
          ...
          SELECT @c_ErrMsg = 'NSQL'+CONVERT(NVARCHAR(5) ,@n_err)+  
                 ': Source Field Not Setup. (nspLPRTSK3)'  
                +' ( '+' SQLSvr MESSAGE='+@c_ErrMsg  
                +' ) '  
          GOTO QUIT_SP  
       END  
       ...
       BEGIN  
          ...
          SELECT @c_ErrMsg = 'NSQL'+CONVERT(NVARCHAR(5) ,@n_err)+  
                --'Source Field Only Allow Refer To Orders Or Sku Table. Invalid Table: '+RTRIM(@c_TableColumnName)+ '.(nspLPRTSK3)'  
                'Source Field Only Allow Refer To Sku Table. Invalid Table: '+RTRIM(@c_TableColumnName)+ '.(nspLPRTSK3)'  
                +' ( '+' SQLSvr MESSAGE='+@c_ErrMsg  
                +' ) '  
          GOTO QUIT_SP  
       END   
       ...
       BEGIN  
          ...
          SELECT @c_ErrMsg = 'NSQL'+CONVERT(NVARCHAR(5) ,@n_err)+  
                'Invalid Source Field Name: '+RTRIM(@c_TableColumnName)+ '.(nspLPRTSK3)'  
                +' ( '+' SQLSvr MESSAGE='+@c_ErrMsg  
                +' ) '  
          GOTO QUIT_SP  
       END  
      ...
      --NJOW07 move out from loop
      SET @b_success = 0  
      SET @c_SplitPickTaskByStore = ''
      EXECUTE nspGetRight null,     -- facility  
         @c_StorerKey,              -- Storerkey  
         null,                      -- Sku  
         'SplitPickTaskByStore',    -- Configkey  
         @b_success    output,  
         @c_SplitPickTaskByStore  output,  
         @n_err        output,  
         @c_errmsg     output       
       ...
       BEGIN  
          ...
          */  
          ...
          BEGIN  
              ...
              WHERE l.Loc = @c_FromLoc  
               ...
               --(Kc01) - Start           
               IF @c_SplitPickTaskByStore = '1' --AND @b_success = 1  
               BEGIN  
                  SET @c_TaskType = 'SPK'  
               END  
               ELSE  
              ...
              DECLARE CUR_PICKDETAILKEY CURSOR LOCAL FAST_FORWARD READ_ONLY FOR  
                 SELECT p.PickDetailKey  
                 FROM  #Orders O  
                 --JOIN ORDERS OD  WITH (NOLOCK, INDEX(PKOrders)) ON OD.OrderKey = O.OrderKey  --NJOW07 removed
        ...
        CLOSE C_PiecePickTask  
        DEALLOCATE C_PiecePickTask          
      END   -- @c_StorePPATaskGroupBySKU <> '1'
...
SET @n_SGANo = 0  
   ...
   --          @c_ConsigneeKey'@c_ConsigneeKey', @c_SKU '@c_SKU'  
           ...
           BEGIN  
              ...
              BEGIN  
                ...
                SELECT @n_err = @@ERROR  
                 ...
                 BEGIN  
                     ...
                     SELECT @c_ErrMsg = 'NSQL'+CONVERT(NVARCHAR(5) ,@n_err)+  
                            ': Insert Into LOTxLOCxID Failed (nspLPRTSK3)'  
                           +' ( '+' SQLSvr MESSAGE='+@c_ErrMsg  
                           +' ) '  
               CLOSE C_PickDetail  
              ...
              BEGIN  
                 ...
                 BEGIN  
                     ...
                     SELECT @c_ErrMsg = 'NSQL'+CONVERT(NVARCHAR(5) ,@n_err)+  
                            ': Insert Into SKUxLOC Failed (nspLPRTSK3)'  
                          +' ( '+' SQLSvr MESSAGE='+@c_ErrMsg  
                           +' ) '  
                     CLOSE C_PickDetail  
                     DEALLOCATE C_PickDetail  
              ...
              BEGIN  
                  ...
                  SELECT @c_ErrMsg = CONVERT(NVARCHAR(250) ,@n_err)  
                        ,@n_err = 81023 -- Should Be Set To The SQL Errmessage but I don't know how to do so.  
                  SELECT @c_ErrMsg = 'NSQL'+CONVERT(NVARCHAR(5) ,@n_err)+  
                         ': Update LOTxLOCxID Failed (nspLPRTSK3)'  
                        +' ( '+' SQLSvr MESSAGE='+@c_ErrMsg  
                        +' ) '  
                  CLOSE C_PickDetail  
                  DEALLOCATE C_PickDetail  
      ...
      BEGIN
         ...
         BEGIN  
           ...
           -- Step 2: Swap the Bulk Picking with Store Location  
           DECLARE C_PickDetail CURSOR LOCAL FAST_FORWARD READ_ONLY FOR  
           SELECT SP.PickDetailKey, SP.LOT, SP.StoreLoc, SP.Qty  
           FROM #StorePick SP  
           JOIN LOC WITH (NOLOCK) ON LOC.LOC = SP.StoreLoc -- (Shong06)  
           WHERE SP.StoreLoc IS NOT NULL  
           AND   SP.StorerKey = @c_StorerKey  
           AND   SP.SKU = @c_SKU  
           AND   SP.LOC = @c_FromLoc  
           AND   SP.ID  = @c_ID  
           ORDER BY LOC.PutAwayZone, SP.StoreLoc, SP.ConsigneeKey -- (Shong06)  
     ...
     END -- @c_StorePPATaskGroupBySKU = '1'
      ...
      IF OBJECT_ID('tempdb..#StorePick') IS NOT NULL  
         DROP TABLE #StorePick             
       -------------------------------------------------  
       -- Scan Pick Location For Replenishment Task  
       -- Generate Force Replenishment Task  
       -------------------------------------------------  
       
      DECLARE @n_ReplenCaseCnt   INT
                     
      DECLARE @t_ReplenLot TABLE (
      	LOT           NVARCHAR(10),
      	LOT_Priority  INT,
      	CaseCnt       INT)
       ...
       JOIN   (SELECT DISTINCT StorerKey, SKU, FromLoc AS LOC  
               ...
               WHERE  LoadKey = @c_LoadKey  
               AND    STATUS < '9'  
               AND    TaskDetail.TaskType in ('PK', 'SPK', 'PPK') ) AS PickLOC               --(Kc01)  
       ...
       FETCH NEXT FROM Cursor_Replenishment INTO @c_StorerKey, @c_SKU, @c_ToLOC, @n_ReplenQty, 
                                                 @c_Facility, @c_Putawayzone, @c_ReplenPriority  
       WHILE @@FETCH_STATUS <> -1  
       BEGIN         	            	   
          INSERT INTO @t_ReplenLot (LOT, LOT_Priority, CaseCnt)
          SELECT lli.Lot, 
                 LOT_Priority = CASE WHEN lli.Qty < (lli.QtyAllocated + lli.QtyPicked) THEN 1 
                                     WHEN lli.Qty < lli.QtyAllocated THEN 2
                                     WHEN lli.QtyAllocated > 0 THEN 3
                                     ELSE 9 
                                END, 
                 CASE WHEN ISNUMERIC(LA.Lottable06) = 1 THEN CAST(LA.Lottable06 AS INT) ELSE 0 END     
          ...
          AND   LA.Lottable07 = 'General' --NJOW08
         ...
         JOIN   @t_ReplenLot ReplenLot ON ReplenLot.Lot = lli.Lot 
         ...
         ORDER BY ReplenLot.LOT_Priority, 
                  CASE WHEN (lli.Qty - 
                             lli.QtyAllocated - 
                             lli.QtyPicked - 
                             CASE WHEN lli.QtyReplen > 0 THEN lli.QtyReplen ELSE 0 END) = @n_ReplenQty THEN 1 
                        WHEN (lli.Qty - 
                             lli.QtyAllocated - 
                             lli.QtyPicked - 
                             CASE WHEN lli.QtyReplen > 0 THEN lli.QtyReplen ELSE 0 END) > @n_ReplenQty THEN 2
                         WHEN  (lli.Qty - 
                             lli.QtyAllocated - 
                             lli.QtyPicked - 
                             CASE WHEN lli.QtyReplen > 0 THEN lli.QtyReplen ELSE 0 END) < @n_ReplenQty THEN 9
                         ELSE 3
                    END 
  
         OPEN CUR_AvailableQty  
  
         FETCH NEXT FROM CUR_AvailableQty INTO @c_LOT, @c_FromLoc, @c_ID, @n_Qty, @n_ReplenCaseCnt  
         WHILE @@FETCH_STATUS <> -1  
         BEGIN  
       	   -- Round to full carton size 
       	   IF @n_ReplenCaseCnt > 0 AND @n_ReplenQty > @n_ReplenCaseCnt
         	   SET @n_ReplenQty = FLOOR( @n_ReplenQty / @n_ReplenCaseCnt ) * @n_ReplenCaseCnt 
            ...
            BEGIN
               ...
               BEGIN  
                  ...
                  -- Only suggest 1 Task even not fullfill the expected replen qty, 
                  --- operator can override the suggested qty  
         ...
         FETCH NEXT FROM Cursor_Replenishment INTO @c_StorerKey, @c_SKU, @c_ToLOC, @n_ReplenQty, 
                                                   @c_Facility, @c_Putawayzone, @c_ReplenPriority  
      END  -- While Cursor_Replenishment  
   ...
   BEGIN  
      ...
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'nspLPRTSK3'  
