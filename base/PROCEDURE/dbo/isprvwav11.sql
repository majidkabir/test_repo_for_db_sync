...
GO
/*************************************************************************/  
/* Stored Procedure: ispRVWAV11                                          */  
/* Creation Date: 02-Oct-2017                                            */  
/* Copyright: LFL                                                        */  
/* Written by:                                                           */  
/*                                                                       */  
/* Purpose: WMS-2851 - CN PVH Reverse Released Wave                      */  
/*                                                                       */  
/* Called By: wave                                                       */  
/*                                                                       */  
/* PVCS Version: 1.1                                                     */  
...
CREATE PROCEDURE [dbo].[ispRVWAV11]      
 ...
 BEGIN  
    ...
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM TASKDETAIL TD (NOLOCK) 
                   ...
                   AND TD.Sourcetype IN ('ispRLWAV11') 
                   AND TD.Tasktype IN ('RPF')) 
        BEGIN
           SELECT @n_continue = 3  
           SELECT @n_err = 81010  
           SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': This Wave has not been released. (ispRVWAV11)'         
        END                 
    END
    ...
    BEGIN
       ...
       BEGIN
          SELECT @n_continue = 3  
          SELECT @n_err = 81020  
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Some Tasks have been started. Not allow to Reverse Wave Released (ispRVWAV11)'       
       END                 
    END
    ...
    BEGIN
       ...
       AND TASKDETAIL.Sourcetype IN ('ispRLWAV11')
       ...
       BEGIN
          ...
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Delete Taskdetail Table Failed. (ispRVWAV11)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
       END 
    END
    ...
    BEGIN
       ...
       AND RDT.rdtPTLStationLog.SourceType = 'ispRLWAV11'
       ...
       BEGIN
          ...
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Delete rdtPTLStationLog Table Failed. (ispRVWAV11)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
       END 
    END
    ...
    BEGIN
         ...
         BEGIN
           ...
           SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Pickdetail Table Failed. (ispRVWAV11)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
         END          
    END        
    ...
    BEGIN  
       ...
       BEGIN  
          ...
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update on wave Failed (ispRVWAV11)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
       END  
    END  
...
RETURN_SP:
    ...
    BEGIN  
       ...
       BEGIN  
          ...
          END  
       END  
       execute nsp_logerror @n_err, @c_errmsg, "ispRVWAV11"  
