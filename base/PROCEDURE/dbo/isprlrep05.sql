   ...
   -----Create replenishment task
    ...
    BEGIN
       ...
       BEGIN       	      
           ...
           --Check Codelkup for Priority
           IF NOT EXISTS (SELECT 1 FROM #TMP_LOC WHERE #TMP_LOC.LOC = @c_ToLoc AND #TMP_LOC.SKU = @c_SKU)
           BEGIN
              INSERT INTO #TMP_LOC
              SELECT LLI.LOC, LLI.SKU,
                     CASE WHEN ISNULL(MAX(SL.QtyLocationLimit),0) > 0 AND SUM(LLI.Qty - LLI.QtyPicked + LLI.PendingMoveIn) <= MAX(SL.QtyLocationLimit) THEN
                               ROUND(CAST(SUM(LLI.Qty - LLI.QtyPicked + LLI.PendingMoveIn) AS FLOAT) / CAST(MAX(SL.QtyLocationLimit) AS FLOAT) * 100,0)
                     WHEN SUM(LLI.Qty - LLI.QtyPicked + LLI.PendingMoveIn) > MAX(SL.QtyLocationLimit) THEN 100 
                     ELSE 0 END AS [Percentage]
              FROM LOTXLOCXID LLI (NOLOCK)
              JOIN SKUXLOC SL (NOLOCK) ON LLI.LOC = SL.LOC AND LLI.SKU = SL.SKU
              WHERE LLI.LOC = @c_ToLoc AND LLI.SKU = @c_SKU
              GROUP BY LLI.LOC, LLI.SKU
           END
           
           IF NOT EXISTS (SELECT 1 FROM #TMP_LOCxPriority WHERE #TMP_LOCxPriority.LOC = @c_ToLoc AND #TMP_LOCxPriority.SKU = @c_SKU)
           BEGIN
              INSERT INTO #TMP_LOCxPriority
              SELECT t.LOC, t.SKU,
                     ISNULL(CL.Code2,'8')
              FROM #TMP_LOC t
              LEFT JOIN CODELKUP CL (NOLOCK) ON CL.LISTNAME = 'PFUtil' AND (t.[Percentage] >= CAST(CL.Short AS INT) AND t.[Percentage] <= CAST(CL.Long AS INT))
           END

           SELECT @c_Priority = ISNULL(CL.Short,'8')
           FROM CODELKUP CL (NOLOCK) 
           JOIN #TMP_LOCxPriority t ON t.LOC = @c_ToLoc AND t.SKU = @c_SKU
           WHERE CL.Listname = 'ReplenPrio'
           AND CL.Code = @c_Priority AND CL.Code2 = t.[Priority]
       	  ...
       	  IF ISNULL(@c_Priority,'') = ''
       	     SET @c_Priority = '8'
       	 --print @c_Sku + ' ' + @c_ToLoc + ' ' + CAST(@n_Qty AS NVARCHAR(10))
...
RETURN_SP:
    ...
    BEGIN  
       ...
       BEGIN  
          ...
          END  
       END  
       execute nsp_logerror @n_err, @c_errmsg, "ispRLREP05"  
