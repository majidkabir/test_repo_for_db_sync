...
/************************************************************************/  
/* Stored Procedure: nspPR_PH12                                         */  
/* Creation Date: 13-Aug-2021                                           */  
/* Copyright: LFL                                                       */  
/* Written by:                                                          */  
/*                                                                      */  
/* Purpose: WMS-17650 PH P&G                                            */  
...
BEGIN   
   ...
   BEGIN  
      ...
      --IF @c_lottable01 <> ' '--OR CHARINDEX('LOTTABLE01', @c_LottableList) > 0  
         SELECT @c_LimitString =  dbo.fnc_RTrim(@c_LimitString) + ' AND ISNULL(LOC.HostWHCode,'''') = @c_lottable01 '    
      ...
      IF @c_UOM = '1'
         SELECT @c_LimitString =  dbo.fnc_RTrim(@c_LimitString) + ' AND LOC.LocationType NOT IN(''CASE'',''PICK'') '
      ELSE
