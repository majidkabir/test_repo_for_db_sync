...
/* 2019-03-06  Wan01    1.1   Fixed. to allocate available UCC even if  */        
/*                            UCC # from same lot,loc,id allocated before*/        
/* 18-JUL-2019 CSCHONG  1.2   WMS-9822-revised report condition (CS01)  */        
/* 08-AUG-2019 CSCHONG  1.3   WMS-10204 - add channel checking (CS01a)  */        
/* 28-NOV-2019 CSCHONG  1.45   WMS-10204 - Fix uom issue (CS01c)        */        
/************************************************************************/            
CREATE PROC [dbo].[ispPRNIK10]                
...
BEGIN            
   ...
   BEGIN        
    ...
    SELECT * FROM #CHANNELINFO WITH (NOLOCK)        
   END        
   ...
   SELECT   StorerKey,  SKU, Facility, Packkey, OrderQty = SUM(OrderQty)        
   ...
   BEGIN         
    ...
    ', @c_Lottable09: ' + @c_Lottable09 + CHAR(13) +         
      ...
      SET @c_SQL =         
              ...
              + CHAR(13) + 'AND (SELECT COUNT(1) FROM UCC WITH (NOLOCK) WHERE UCC.Lot = LOTxLOCxID.Lot AND UCC.Loc = LOTxLOCxID.Loc '          
              --+ CHAR(13) +      'AND UCC.ID = LOTxLOCxID.ID AND UCC.Status > ''2'' AND  UCC.Status < ''9'') = 0 ' -- (Wan01) -FIXED            
              + CHAR(13) +      'AND UCC.ID = LOTxLOCxID.ID AND UCC.Status = ''1'') > 0 '                           -- (Wan01) -FIXED            
              + CHAR(13) + 'ORDER BY LOC.LocationHandling DESC'         
              + CHAR(13) +     ', CASE WHEN (LOTXLOCXID.Qty - LOTXLOCXID.QtyAllocated - LOTXLOCXID.QtyPicked - LOTXLOCXID.QtyReplen) >= @n_QtyLeftToFullFill THEN 0 ELSE 1 END '        
      ...
      BEGIN          
         ...
         BEGIN            
            ...
            select @c_Facility '@c_Facility',@c_SKU '@c_SKU',@c_LocationType '@c_LocationType',@c_LocationCategory '@c_LocationCategory',        
                @n_QtyAvail '@n_QtyAvail',@n_Channel_ID   '@n_Channel_ID'        
         ...
         END         
             ...
             IF @b_Debug = 1            
             BEGIN         
               SELECT @n_QtyAvail '@n_QtyAvail',@n_QtyToTake '@n_QtyToTake'        
             END        
         ...
         AND   UCC.ID  = @c_ID        
         AND   Status < '3'        
         AND ( SELECT SUM(CTN.Qty)         
               FROM UCC CTN WITH (NOLOCK)         
         ...
         OPEN CUR_UCC                       
         FETCH NEXT FROM CUR_UCC INTO @n_UCC_RowRef, @c_UCCNo, @n_RemainUCCQty         
        
         WHILE (@@FETCH_STATUS <> -1) AND @n_RemainUCCQty > 0        
         BEGIN          
            SET @b_UpdateUCC = 1        
            SET @n_UCCQty = @n_RemainUCCQty        
                   
            --NJOW        
            IF  @c_ChannelInvMgmt <> '1'   --CS01a         
            BEGIN        
                     --SET @cur_ORDERLINES = CURSOR FAST_FORWARD READ_ONLY FOR         
             SET @c_SQL =        
            ...
            BEGIN        
              IF @b_Debug = 1            
              BEGIN      
               select 'B4 #CHANNELINFO',* from #CHANNELINFO where orderkey in ('1501689085','1501689088') and sku ='AQ966100212C'      
             END      
              SET @c_SQL =        
                  N'DECLARE CUR_ORDLINE CURSOR FAST_FORWARD READ_ONLY FOR '         
                ...
                + CHAR(13) + '      , CASE WHEN OD.OpenQty - ( OD.QtyAllocated + OD.QtyPreAllocated + OD.QtyPicked ) >= @n_UCCQty THEN 1 ELSE 0 END'        
             END --CS01a END        
               ...
               SET @c_SQLParm =  N'@c_WaveKey NVARCHAR(10), @c_StorerKey NVARCHAR(15), @c_SKU NVARCHAR(20), @c_Packkey NVARCHAR(10)'             
                              ...
                              + ', @c_Lottable06 NVARCHAR(30), @c_Lottable07 NVARCHAR(30), @c_Lottable08 NVARCHAR(30)'          
                              + ', @c_Lottable09 NVARCHAR(30), @c_Lottable10 NVARCHAR(30), @c_Lottable11 NVARCHAR(30)'         
                              + ', @c_Lottable12 NVARCHAR(30), @n_UCCQty INT '                          
                          
               EXEC sp_ExecuteSQL @c_SQL, @c_SQLParm, @c_WaveKey, @c_StorerKey ,@c_SKU, @c_Packkey         
                                 , @c_Lottable01, @c_Lottable02, @c_Lottable03        
                                 , @c_Lottable06, @c_Lottable07, @c_Lottable08        
                                 , @c_Lottable09, @c_Lottable10, @c_Lottable11, @c_Lottable12,@n_UCCQty        
            ...
            --FETCH NEXT FROM @cur_ORDERLINES INTO @c_Orderkey, @c_OrderLineNumber, @n_OrderQty,@c_Channel      --CS01a        
          ...
          BEGIN        
               ...
               END        
                   ...
                   IF @b_Debug = 1            
                   BEGIN       
                     IF @c_Orderkey in ('1501689085','1501689088') and  @c_SKU = 'AQ966100212C'      
                     BEGIN      
                      SELECT OD.OpenQty - ( OD.QtyAllocated + OD.QtyPreAllocated + OD.QtyPicked ) as ODqty,* FROM ORDERDETAIL OD (NOLOCK)      
                      where OD.orderkey =@c_Orderkey and OD.sku='AQ966100212C'      
                          
                      SELECT @c_Orderkey '@c_Orderkey',@c_OrderLineNumber '@c_OrderLineNumber',@c_SKU '@c_SKU',@n_OrderQty '@n_OrderQty',@n_RemainUCCQty '@n_RemainUCCQty',@n_QtyToInsert '@@n_QtyToInsert'--,@n_QtyToTake '@n_QtyToTake'        
                      select '#CHANNELINFO',* from #CHANNELINFO where Channel = @c_channel and sku = @c_SKU      
                      SELECT * FROM ChannelInv AS ci WITH(NOLOCK) where Channel = @c_channel and sku = @c_SKU      
                     END      
                   END        
               ...
               SET @c_UOM=(SELECT CASE WHEN @n_UCCQty=@n_QtyToInsert THEN 2 WHEN @n_UCCQty<>@n_QtyToInsert THEN 7 END ) --CS01 20191226    
               ...
               BEGIN        
                  ...
                  SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0))         
                                 + ': Get PickDetailKey Failed. (ispPRNIK10)'        
                  GOTO QUIT_SP        
               END        
               ELSE        
               BEGIN        
                  ...
                  BEGIN        
                     ...
                     SET @n_Err = 61020        
                     SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0))         
                         + ': Insert PickDetail Failed. (ispPRNIK10)'        
                     GOTO QUIT_SP        
                  END        
   ...
   --CS01e START    
    ...
    DECLARE @c_Pdkey nvarchar(10)    
    --DECLARE @c_SKU nvarchar(10)    
    DECLARE @c_QtyUcc int    
    
      
     IF EXISTS(            
     SELECT 1    
     FROM PICKDETAIL(NOLOCK) AS PD    
     LEFT JOIN LOC(NOLOCK) AS LOC ON PD.LOC=LOC.LOC    
     WHERE   PD.STORERKEY='NIKESDC'    
             AND PD.WAVEKEY=@c_Wavekey    
             AND PD.UOM=7    
              AND PD.UOMQTY=PD.QTY     
              AND PD.DROPID<>''    
     ...
     SELECT PickDetailKey,DROPID,SKU    
     FROM PICKDETAIL(NOLOCK) AS PD    
     LEFT JOIN LOC(NOLOCK) AS LOC ON PD.LOC=LOC.LOC    
     WHERE   PD.STORERKEY='NIKESDC'    
             AND PD.WAVEKEY=@c_Wavekey    
    ...
    OPEN CUR_UOM                       
    FETCH NEXT FROM CUR_UOM INTO  @c_Pdkey,@c_Dropid,@c_SKU    
    
    WHILE (@@FETCH_STATUS <> -1)    
    BEGIN    
             
         SET @c_QtyUcc=(SELECT SUM(QTY) FROM UCC(NOLOCK) WHERE UCCNO=@c_Dropid AND STORERKEY='NIKESDC' and sku=@c_SKU)    
        ...
        ,EditDate= GETDATE()           
        ,TrafficCop = NULL     
         WHERE STORERKEY='NIKESDC' AND PICKDETAILKEY=@c_Pdkey    
...
QUIT_SP:        
   ...
   BEGIN          
      ...
      BEGIN          
         ...
         END          
      END          
      EXECUTE nsp_logerror @n_Err, @c_ErrMsg, 'ispPRNIK10'          
