...
/************************************************************************/      
/* Stored Procedure: nspAL_SG09                                         */      
/* Creation Date: 15-MAR-2022                                           */      
/* Copyright: LFL                                                       */      
/* Written by:                                                          */      
/*                                                                      */      
/* Purpose: WMS-19173 SG Adidas allocation by UCC                       */  
/*          UCC allocation from bulk for UOM 2                          */
/*          UOM 2 for discrete only                                     */
/*                                                                      */  
/* Called By: Wave                                                      */      
/*                                                                      */      
/* PVCS Version: 1.0                                                    */      
...
/* 15-Mar-2021  NJOW    1.0   DEVOPS Combine script                     */
/************************************************************************/      
CREATE   PROC [dbo].[nspAL_SG09]          
...
BEGIN      
   ...
   BEGIN  
      ...
      IF ISNULL(@c_key1,'')<>'' AND ISNULL(@c_key2,'')<>'' AND @c_UOM <> '2'  --if discrete allocation is not uom 2 skip. For uom 2 only.
         GOTO EXIT_SP
      
      IF ISNULL(@c_key1,'')<>'' AND ISNULL(@c_key2,'')=''  --Skip Conso allocation
         GOTO EXIT_SP
         
      IF NOT EXISTS(SELECT 1 
                    FROM ORDERS O (NOLOCK)
                    JOIN CODELKUP CL (NOLOCK) ON O.Storerkey = CL.Storerkey AND O.DocType = CL.Code
                    WHERE CL.ListName = 'ADUCCALLOC'
                    AND O.Orderkey = @c_Orderkey)
         OR EXISTS (SELECT 1 
                    FROM ORDERS O (NOLOCK)
                    JOIN CODELKUP CL (NOLOCK) ON O.Storerkey = CL.Storerkey AND O.OrderGroup = CL.Code
                    WHERE CL.Listname = 'UCCALLOCGR'
                    AND O.Orderkey = @c_Orderkey)
      BEGIN
         GOTO EXIT_SP
      END              
   ...
   SET @c_ExtraCondition = ' AND UCC.Qty > 0 AND LOC.HostWHCode LIKE ''UU%'' ' + CHAR(13) +
                           ' AND LOC.LocationCategory = ''BULK''  '

   SET @c_SortingCondition = ' ORDER BY LA.Lottable04, LA.Lottable05, LOC.LogicalLocation, LOC.Loc '
