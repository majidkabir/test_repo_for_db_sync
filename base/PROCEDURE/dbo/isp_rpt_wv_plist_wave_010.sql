...
/************************************************************************/
CREATE   PROC [dbo].[isp_RPT_WV_PLIST_WAVE_010] (  
...
BEGIN  
   ...
   JOIN dbo.WAVE wv (NOLOCK) ON wv.WaveKey=wd.WaveKey
   ...
   BEGIN             
      ...
      ELSE
      BEGIN
         IF NOT EXISTS ( SELECT 1 FROM RefKeyLookup RKL (NOLOCK) WHERE RKL.PickDetailkey = @c_GetPickDetailKey) AND @c_PreGenRptData = 'Y'
         BEGIN
            INSERT INTO RefKeyLookUp (PickDetailKey, PickSlipNo, OrderKey, OrderLineNumber)
            SELECT PD.PickdetailKey, @c_RPickSlipNo, PD.OrderKey, PD.OrderLineNumber 
            FROM LOADPLANDETAIL LPD (NOLOCK)
