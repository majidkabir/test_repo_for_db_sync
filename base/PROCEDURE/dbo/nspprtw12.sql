...
/************************************************************************/
/* Stored Procedure: nspPRTW12                                          */
/* Creation Date: 29-Jul-2016                                           */
/* Copyright: LF                                                        */
/* Written by:                                                          */
/*                                                                      */
/* Purpose: WMS-4596-TW-DGE-VIP Pre-Allocation FIFO (SOS#370257)        */
/*                                                                      */
...
BEGIN
   ...
   DECLARE @c_Orderkey            NVARCHAR(10),
           ...
           @n_OrderShelfLife      INT,
           @n_ConsigneeShelfLife  INT,
           @n_ShelfLife           INT,
           @c_SkuGroup            NVARCHAR(10)

   SET @n_StorerMinShelfLife = 0
   SET @n_OrderShelfLife = 0
   SET @n_ConsigneeShelfLife = 0
   SET @n_ShelfLife = 0
   
   IF LEFT(@c_lot,1) = '*'
      SELECT @n_OrderShelfLife = CONVERT(int, SUBSTRING(@c_lot, 2, 9))
   ...
   BEGIN
        ...
        SELECT TOP 1 @c_Secondary = ISNULL(Secondary,'')
        FROM  ORDERS O (NOLOCK) 
        JOIN STORER S(NOLOCK) ON O.Consigneekey = S.Storerkey
        WHERE O.Orderkey = @c_Orderkey 

        SELECT TOP 1 @c_SkuGroup = SkuGroup
        FROM SKU(NOLOCK)
        WHERE Storerkey = @c_Storerkey
        AND Sku = @c_Sku
                
        IF @c_Secondary = 'GT000'
        BEGIN
           SELECT TOP 1 @n_ConsigneeShelfLife = ISNULL(ShelfLife,0)
           FROM DOCLKUP (NOLOCK)
           WHERE ConsigneeGroup = @c_Secondary        
   ...
   BEGIN
      ...
      IF ISNULL(@n_StorerMinShelfLife,0) > 0
         SET @n_ShelfLife = @n_StorerMinShelfLife 

      IF @n_OrderShelfLife > 0 
         SET @n_ShelfLife = @n_OrderShelfLife

      IF @n_ConsigneeShelfLife > 0 
         SET @n_ShelfLife = @n_ConsigneeShelfLife
         
      IF @c_Secondary = 'GT000' AND @n_ConsigneeShelfLife = 0
      BEGIN
         SET @c_Lottable06 = 'A'
         SET @n_ShelfLife = 0
      END
      ...
      IF @n_ShelfLife > 0 
      BEGIN
         SET @c_Condition = dbo.fnc_RTrim(@c_Condition) + " AND DateAdd(Day, " + CAST(@n_ShelfLife AS NVARCHAR(10)) + ", Lotattribute.Lottable04) > GetDate() " 
      END 
      ...
      SET @c_OrderBy = " ORDER BY Lotattribute.Lottable05, LOT.Lot " 
