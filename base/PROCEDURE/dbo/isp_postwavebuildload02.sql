...
GO
/************************************************************************/
/* Trigger: isp_PostWaveBuildLoad02                                     */
...
/************************************************************************/
CREATE   PROC isp_PostWaveBuildLoad02 
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            SET @c_ErrMsg   = 'NSQL' + CONVERT(NVARCHAR(6), @n_Err)   
                            + ': Update Order Fail. (isp_PostWaveBuildLoad02)'            
         END   
      ...
      IF @n_Continue = 1
      BEGIN
         SELECT @c_Load_Userdef1 = ISNULL(o.ConsigneeKey,'')
         ...
         BEGIN
            ...
            SET @n_Err      = 60020       
            SET @c_ErrMsg   = 'NSQL' + CONVERT(NVARCHAR(6), @n_Err)   
                            + ': Update Loadplan Fail. (isp_PostWaveBuildLoad02)'                  
         END
      END
      FETCH @cur_Load INTO @c_Loadkey
   END
...
QUIT_SP:
   ...
   BEGIN
      ...
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'isp_PostWaveBuildLoad02'
