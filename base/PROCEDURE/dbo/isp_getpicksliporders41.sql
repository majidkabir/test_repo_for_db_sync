...
/***************************************************************************/  
/* Stored Proc : isp_GetPickSlipOrders41                                   */  
/* Creation Date:                                                          */  
/* Copyright: IDS                                                          */  
/* Written by:                                                             */  
/*                                                                         */  
/* Purpose: 212633 - Modified from nsp_GetPickSlipOrders08                 */  
/*                                                                         */  
/*                                                                         */  
/* Usage:                                                                  */  
/*                                                                         */  
/* Local Variables:                                                        */  
/*                                                                         */  
/* Called By: r_dw_print_pickorder41                                       */  
/*                                                                         */  
/* PVCS Version: 1.1                                                       */  
/*                                                                         */  
/* Version: 5.4                                                            */  
...
/* 21-Jan-2013 NJOW01  1.0  267262-Nike PickSlipNo by PickZone             */  
/* 28-Jan-2019  TLTING_ext 1.1   enlarge externorderkey field length      */  
/***************************************************************************/  
  
CREATE PROC [dbo].[isp_GetPickSlipOrders41] (@c_loadkey NVARCHAR(10))   
 AS  
 BEGIN  
   SET NOCOUNT ON   
   SET ANSI_NULLS OFF
...
--            BEGIN TRAN  
            ...
            BEGIN    
               ...
               BEGIN    
                  SELECT @n_continue = 3    
                  SELECT @n_err = 63501  
              SELECT @c_errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert into PICKHEADER Failed. (nsp_GetPickSlipOrders41)'  
                  GOTO FAILURE  
               END               
            END -- @b_success = 1      
            ELSE     
            BEGIN    
               SELECT @n_continue = 3    
               SELECT @n_err = 63502  
             SELECT @c_errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Get PSNO Failed. (nsp_GetPickSlipOrders41)'    
               BREAK     
            END     
         END   
  ...
  OPEN C_PickDetailKey    
            ...
            BEGIN    
               ...
               BEGIN     
                  ...
                  BEGIN    
                     SELECT @n_continue = 3  
                     SELECT @n_err = 63503  
                    SELECT @c_errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert RefKeyLookup Failed. (nsp_GetPickSlipOrders41)'      
                     GOTO FAILURE  
                  END                            
               END     
         ...
         BEGIN    
            SELECT @n_continue = 3    
            SELECT @n_err = 63504  
            SELECT @c_errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update #Temp_Pick Failed. (nsp_GetPickSlipOrders41)'      
            GOTO FAILURE  
         END    
--         ELSE  
--         BEGIN  
