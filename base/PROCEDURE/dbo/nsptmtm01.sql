...
/* 22-01-2025   4.6   JCH507     UWP-29488 infinite loop when           */ 
/*                                  continue task config on               */
/************************************************************************/
      ...
      , @c_ttmpickcode=@c_ttmpickcode
            ...
            BEGIN
                ...
                BEGIN
                    ...
                    BEGIN
                        IF @b_debug=1
                            SELECT 'No open task, break'
                        BREAK
                    END -- no open task
                    IF EXISTS(SELECT 1 FROM TaskDetail td WITH (NOLOCK)
                                WHERE td.status = '0'
                                AND td.userkey = ''
                                AND NOT EXISTS (SELECT 1 FROM @t_ProcessTaskType t
                                                WHERE  t.TaskType = td.TaskType
                                                )) 
                                AND @c_ContinueTask = 1
                    BEGIN
                        IF @b_debug=1
                            SELECT 'no specific open task when continue config on, break'
                        BREAK
                    END-- no specific type open task when continue config on
                    --v4.5.0 end
                    IF EXISTS(SELECT 1 FROM TaskDetail td WITH (NOLOCK)
                                WHERE td.status = '0'
                                AND td.userkey = ''
