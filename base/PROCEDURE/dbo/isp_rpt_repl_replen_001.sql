...
/***************************************************************************/
/* Stored Procedure: isp_RPT_REPL_REPLEN_001                               */
...
/***************************************************************************/
CREATE PROC [dbo].[isp_RPT_REPL_REPLEN_001]
...
BEGIN
   ...
   BEGIN
      ...
      AND  (LOC.PutawayZone in (@c_zone02, @c_zone03, @c_zone04, @c_zone05, @c_zone06, @c_zone07, @c_zone08, @c_zone09, @c_zone10, @c_zone11, @c_zone12)
            OR @c_zone02 = 'ALL')
      AND (SKUxLOC.Storerkey = @c_Storerkey OR ISNULL(@c_Storerkey,'') IN('ALL',''))
...
--       AND StorerKey = @c_CurrentStorer
         ...
         N'DECLARE LOT_CUR CURSOR FAST_FORWARD READ_ONLY FOR
          ...
          'AND LOTxLOCxID.LOC <> @c_CurrentLoc
           ...
           CASE WHEN ISNULL(@c_Lottable02,'') <> '' THEN
   ...
   BEGIN
      ...
      AND   R.confirmed = 'N'
      AND (R.Storerkey = @c_Storerkey OR ISNULL(@c_Storerkey,'') IN('ALL',''))
