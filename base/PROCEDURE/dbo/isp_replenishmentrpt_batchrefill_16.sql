...
/****************************************************************************/
/* Stored Procedure: isp_ReplenishmentRpt_BatchRefill_16                    */
/* Creation Date: 24-APR-2018                                               */
/* Copyright: IDS                                                           */
/* Written by: CSCHONG                                                      */
/*                                                                          */
/* Purpose: WMS-4417 - ECOM - New Retail Replenishment Report               */
/*        : modify from isp_ReplenishmentRpt_BatchRefill_15                 */ 
/*                                                                          */  
/* Called By: r_replenishment_report16                                      */  
/*                                                                          */  
/* PVCS Version: 1.2                                                        */  
/*                                                                          */  
/* Version: 7.0                                                            */  
...
/* 09-Jul-2018  NJOW01  1.0   WMS-5638 Add pendingmovein,qtyreplen checking */
/*                            ,replenishmentgroup & refno for working with  */
/*                            release task                                  */
/* 18-JAN-2019  Wan01   1.2   WM - Fitler @c_ReplGrp                        */ 
/****************************************************************************/  
CREATE PROC  [dbo].[isp_ReplenishmentRpt_BatchRefill_16] 
...
BEGIN  
   ...
   DECLARE @c_CurrentStorer      NVARCHAR(15)  
         ...
         , @n_QtyPicked          INT  
           ...
           , @n_TotalQtyPicked     INT
         , @n_QtyExpected        INT  
         , @n_QtyExpNeeded       INT  
         , @n_QtyAvailable       INT  
           , @n_OriQtyToReplen     INT
         , @n_QtyToReplen        INT  
         , @n_LeftToReplen       INT  
  ...
  -- SET @c_Functype = ''
   ...
   --      ,pd.LOC
   --      ,pd.SKU
   --      ,SUM(CASE WHEN PD.[Status] IN ('0','1','2','3','4') AND PD.ShipFlag <> 'P' THEN pd.qty ELSE '0' END)
   --      ,SUM(CASE WHEN PD.[Status] IN ('5','6','7','8') OR PD.ShipFlag = 'P' THEN pd.qty ELSE '0' END)
   ...
   --  AND PD.[Status] < '9' 
   --  AND PD.ShipFlag <> 'Y'
   --GROUP BY pd.Storerkey, pd.loc, pd.sku
   ...
   SET @c_SQLStatement     = N'DECLARE CUR_SKUxLOC CURSOR FAST_FORWARD READ_ONLY FOR'  
                         ...
                         --  + ' HAVING SUM(LOTxLOCxID.QtyExpected) > 0 OR ISNULL(SKUxLOC.QtyLocationMinimum,0) > (ISNULL(SKUxLOC.Qty,0)+sum(ISNULL(LOTxLOCxID.PendingMoveIN,0))- ISNULL(CASE WHEN PKD.Status = ''0'' AND ORD.DocType = ''N'' THEN PKD.Qty ELSE 0 END, 0) + ISNULL(CASE WHEN PKD.Status = ''5'' AND ORD.DocType = ''N'' THEN PKD.Qty ELSE 0 END, 0))'  
   ...
   BEGIN         
      ...
      SET @n_QtyInLoc = @n_Qty - ISNULL(@n_QtyAllocated, 0) - ISNULL(@n_QtyPicked, 0)
       SET @n_TotalQtyInLoc = @n_Qty - @n_TotalQtyAllocated - @n_TotalQtyPicked
       --SELECT @n_QtyAllocated, @n_QtyPicked  
      ...
      IF @n_QtyExpected > 0  
      BEGIN  
         -- Qty Expected should exclude ECOM Expected Qty
          SET @n_ECOM_QtyExpected = 0 

          SELECT @n_ECOM_QtyExpected = SUM(PD.Qty) 
          FROM   PICKDETAIL AS PD (NOLOCK)
         INNER JOIN ORDERS AS O (NOLOCK)  ON  PD.OrderKey = O.OrderKey
         WHERE PD.Storerkey = @c_CurrentStorer 
         AND   PD.SKU = @c_CurrentSku 
         AND   PD.LOC = @c_CurrentLoc
         AND   PD.UOM = '7'
         AND   PD.DoReplenish = 'N'
         AND   O.DocType = 'E'
         AND   PD.Status = '0'
         
           SET @n_ECOM_QtyExpected = ISNULL(@n_ECOM_QtyExpected, 0)
         
          IF @n_ECOM_QtyExpected > 0 
             SET @n_QtyExpected = @n_QtyExpected - @n_ECOM_QtyExpected 
      END 
      
      SET @n_PendingMoveIn = 0 
      SELECT @n_PendingMoveIn = SUM(lli.PendingMoveIN)
      ...
      IF @n_PendingMoveIn > 0 
         SET @n_QtyExpected = @n_QtyExpected - @n_PendingMoveIn 
      ...
      BEGIN         
         ...
         SELECT DISTINCT  
                LLI.LOT  
              , GroupType  = 1  
              , CEILING(((LLI.QtyAllocated + LLI.QtyPicked) - LLI.Qty - ISNULL(ECPKDT.Qty,0)) / (@n_CaseCnt * 1.00)) * @n_CaseCnt  --WWANG01
              , SortColumn = ' '  
         ...
         LEFT JOIN CODELKUP     CL  WITH (NOLOCK) ON (CL.ListName = 'REPLSWAP')  
                                             AND(LLI.Storerkey = CL.Storerkey)  
                                             AND(LA.Lottable02 = CL.Short)  
                                             AND(CL.UDF01 = '0')  
         /* ECOM INV WWANG01*/
          LEFT JOIN (SELECT LOT, ISNULL(SUM(Qty) , 0) AS Qty
                      FROM PICKDETAIL WITH (NOLOCK)
                        JOIN ORDERS     WITH (NOLOCK) ON PICKDETAIL.OrderKey = ORDERS.OrderKey
                        WHERE PICKDETAIL.StorerKey = @c_CurrentStorer
                        AND   PICKDETAIL.SKU = @c_CurrentSku  
                        AND   PICKDETAIL.UOM = '7'
                        AND   PICKDETAIL.Status = '0'
                        AND   PICKDETAIL.DoReplenish = 'N'
                        AND   ORDERS.DocType = 'E'
                        GROUP BY LOT) ECPKDT ON LLI.LOT = ECPKDT.LOT
         /*WWANg01*/
         WHERE LLI.Loc       = @c_CurrentLoc  
         AND   LLI.Storerkey = @c_CurrentStorer  
         AND   LLI.Sku       = @c_CurrentSku  
         AND  (LLI.QtyAllocated + LLI.QtyPicked) - LLI.Qty - ISNULL(ECPKDT.Qty,0) > 0   --WWANG01
         AND   EXISTS ( SELECT 1  
     ...
     IF @n_TotalQtyInLoc < @n_QtyLocationMinimum  
    BEGIN         
        IF @n_QtyExpected > 0 --WWANG01
            SET @n_LeftToReplen = @n_QtyLocationLimit - CEILING(@n_QtyExpected/(@n_CaseCnt * 1.00)) * @n_CaseCnt   --WWANG01
        ELSE IF @n_QtyExpected <= 0 AND (@n_ECOM_QtyExpected - @n_TotalQtyInLoc) > 0 
            SET @n_LeftToReplen = @n_QtyLocationLimit
         ELSE  
             SET @n_LeftToReplen = @n_QtyLocationLimit - @n_TotalQtyInLoc   --WWANG01 --( CASE WHEN  @n_QtyInLoc > 0 THEN @n_QtyInLoc ELSE @n_QtyInLoc * -1 END )  
       
         SET @n_OriQtyToReplen = FLOOR(@n_LeftToReplen/(@n_CaseCnt * 1.00)) * @n_CaseCnt  --@n_LeftToReplen  
       
        IF @n_OriQtyToReplen > 0  
       BEGIN             
          INSERT INTO #LOT_SORT  
           (  LOT  
           ,  GroupType  
          ...
          AND LA.Lottable01 = CASE WHEN CL.Notes2 = '1' AND CHARINDEX('LOTTABLE01', CL.Notes) > 0 THEN ISNULL(@c_Lottable01,'') ELSE LA.Lottable01 END  
    ...
    END
      ...
      BEGIN  
        ...
        /*  
         ...
         END  */
           IF @n_GroupType = '2'
              SET @n_QtyToReplen = @n_OriQtyToReplen
  
         IF @b_debug = 1  
         BEGIN  
            Print 'Working on @n_QtyExpected: ' + Convert(VARCHAR(10),@n_QtyExpected) +  
               ' @c_FromLot: ' +  RTRIM(@c_FromLot) +  
...
--            END    
            ...
            SET @n_QtyToReplen = @n_QtyToReplen - @n_FromQty  
               SET @n_OriQtyToReplen = @n_OriQtyToReplen - @n_FromQty
            ...
            IF @b_debug = 1  
            BEGIN  
                  PRINT '>>>  @c_CurretSKU: ' + @c_CurrentSKU
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         SET @c_errmsg= 'NSQL'+CONVERT(char(5),@n_err)+': Fail to get REPLENISHKEY. (isp_ReplenishmentRpt_BatchRefill_16)'  
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         SET @c_errmsg = 'NSQL'+CONVERT(char(5),@n_err)  
                       + ': Insert into Replenishment table failed. (isp_ReplenishmentRpt_BatchRefill_16)'  
       + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
         GOTO QUIT  
      END  
      COMMIT TRAN  
...
QUIT:  
   ...
   --         ,  QtyAvailableEA       FLOAT          NULL  DEFAULT (0)  
      ...
      BEGIN  
         ...
         SELECT  R.ReplenishmentGroup  
               ...
               , PK.PackUOM3  
               , ReplQty = SUM(R.Qty)                                                                    
               , QtyAvailable =  SUM(LLT.Qty - LLT.QtyAllocated - LLT.QtyPicked - LLT.QtyReplen)      --NJOW01
      ...
      SET Replenishmentkey = 'LFL'  
         ,Lot = ''  
         ,ID  = ''  
         ,LocQtyAvailable = SKUxLOC.Qty - SKUxLOC.QtyAllocated - SKUxLOC.QtyPicked         
         ,GroupByLoc = 1
      FROM #TMP_REPLRPT  
      ...
      AND   CODELKUP.Long = 'r_replenishment_report16'  
      ...
      SELECT   ReplenishmentGroup = ISNULL(ReplenishmentGroup,'')      
            ...
            ,  ExchgSkuNQtyUsedPOS= ISNULL(ExchgSkuNQtyUsedPOS,'')      
            ,  IncreaseFontSize    = ISNULL(IncreaseFontSize,'')        
      FROM #TMP_REPLRPT  
      GROUP BY ISNULL(ReplenishmentGroup,'')  
            ...
            ,  ISNULL(PackUOM3,'')  
            ,  ISNULL(RemoveID,0)  
            ,  ISNULL(LocQtyAvailable,0)                              
            ,  ISNULL(GroupByLoc,0)                                  
            ,  ISNULL(ExchgSkuNQtyUsedPOS,'')                         
            ,  ISNULL(IncreaseFontSize,'')                            
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         END  
      END  
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'isp_ReplenishmentRpt_BatchRefill_16'  
