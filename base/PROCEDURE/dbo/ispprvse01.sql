...
/* 21-05-2018   CheeMun	  1.0   INC0239498 - Pass in PackKey value      */
/* 14-11-2018   CheeMun   1.1   INC0468357 - Performance Tuning         */
/************************************************************************/    
CREATE PROC [dbo].[ispPRVSE01]        
...
BEGIN    
   ...
   --INC0468357 (START)
    CREATE INDEX IDX_SSF ON #ORDERLINES (STORERKEY,SKU,FACILITY)  
    CREATE INDEX IDX_LOT1 ON #ORDERLINES (LOTTABLE01)  
    CREATE INDEX IDX_LOT2 ON #ORDERLINES (LOTTABLE02)  
    CREATE INDEX IDX_LOT3 ON #ORDERLINES (LOTTABLE03)  
    CREATE INDEX IDX_LOT6 ON #ORDERLINES (LOTTABLE06)  
    CREATE INDEX IDX_LOT7 ON #ORDERLINES (LOTTABLE07)  
    CREATE INDEX IDX_LOT8 ON #ORDERLINES (LOTTABLE08)  
    CREATE INDEX IDX_LOT9 ON #ORDERLINES (LOTTABLE09)  
    CREATE INDEX IDX_LOT10 ON #ORDERLINES (LOTTABLE10)  
    CREATE INDEX IDX_LOT11 ON #ORDERLINES (LOTTABLE11)  
    CREATE INDEX IDX_LOT12 ON #ORDERLINES (LOTTABLE12)  
   --INC0468357 (END)   
   
   INSERT INTO #ORDERLINES (Loadkey,
                            OrderQty,
                            SKU, 
                            PackKey, 
                            StorerKey,
   ...
   GROUP BY ISNULL(RTRIM(O.Loadkey),''),
            ISNULL(RTRIM(OD.Sku),''),
            ISNULL(RTRIM(SKU.PackKey),''),
            ISNULL(RTRIM(OD.Storerkey),''),
            ISNULL(RTRIM(O.Facility),''),
   ...
   SELECT DISTINCT SKU, PackKey, StorerKey, Facility, Lottable01, Lottable02, Lottable03, Lottable06,   --INC0239498
                   Lottable07, Lottable08, Lottable09, Lottable10, Lottable11, Lottable12  
   FROM #ORDERLINES  
  
   OPEN CURSOR_SKULOT                 
   FETCH NEXT FROM CURSOR_SKULOT INTO @c_SKU, @c_PackKey, @c_StorerKey, @c_Facility, @c_Lottable01, @c_Lottable02, @c_Lottable03, @c_Lottable06,  --INC0239498
   ...
   BEGIN 
      ...
      SET @c_SQL = N'         
      DECLARE CURSOR_ORDERLINE_LOAD CURSOR FAST_FORWARD READ_ONLY FOR                                                          
         SELECT Loadkey, OrderQty                                                                                   
      ...
      SET @c_SQLParm =  N'@n_UCCQty INT, @c_SKU NVARCHAR(20), @c_StorerKey NVARCHAR(15), @c_Facility NVARCHAR(5), ' +     
      ...
      EXEC sp_ExecuteSQL @c_SQL, @c_SQLParm, @n_UCCQty, @c_SKU, @c_StorerKey, @c_Facility, @c_Lottable01, @c_Lottable02, @c_Lottable03,
                         @c_Lottable06, @c_Lottable07, @c_Lottable08, @c_Lottable09, @c_Lottable10, @c_Lottable11, @c_Lottable12
                                                                                                                              
      OPEN CURSOR_ORDERLINE_LOAD                                                                                               
      FETCH NEXT FROM CURSOR_ORDERLINE_LOAD INTO @c_Loadkey, @n_OrderQty                                       
      ...
      BEGIN 
         ...
         BEGIN
            ...
            DECLARE CUR_OrderLines CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
               ...
               JOIN   WAVEDETAIL WD WITH (NOLOCK) ON O.Orderkey = WD.Orderkey						--INC0468357
               ...
               AND WD.WaveKey = @c_Wavekey          --INC0468357                                         
            ...
            BEGIN
               ...
               BEGIN
                  ...
                  SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0)) + 
                                  ': Get PickDetailKey Failed. (ispPRVSE01)'
               END
               ...
               BEGIN
                  ...
                  SET @n_Err = 13001
                  SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0)) + 
                                  ': Insert PickDetail Failed. (ispPRVSE01)'
               END
         ...
         FETCH NEXT FROM CURSOR_ORDERLINE_LOAD INTO @c_Loadkey, @n_OrderQty                                       
      END
      CLOSE CURSOR_ORDERLINE_LOAD
      DEALLOCATE CURSOR_ORDERLINE_LOAD      
...
QUIT:
   ...
   IF (SELECT CURSOR_STATUS('LOCAL','CURSOR_ORDERLINE_LOAD')) >=0 
   BEGIN
      CLOSE CURSOR_ORDERLINE_LOAD           
      DEALLOCATE CURSOR_ORDERLINE_LOAD      
   END  
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         END  
      END  
      EXECUTE nsp_logerror @n_Err, @c_ErrMsg, 'ispPRVSE01'  
