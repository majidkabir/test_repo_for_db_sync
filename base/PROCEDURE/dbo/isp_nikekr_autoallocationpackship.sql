...
/***************************************************************************/
/* Stored Procedure: isp_NIKEKR_AutoAllocationPackShip                     */
/* Creation Date: 17-Jun-2021                                              */
/* Copyright: LFL                                                          */
/* Written by: WLChooi                                                     */
/*                                                                         */
/* Purpose: WMS-17166 - [KR] NIKE auto allocate_pack_ship (Exceed - NEW)   */
/*                                                                         */
/* Called By: SQL Job                                                      */
/*                                                                         */
/* GitLab Version: 1.0                                                     */
...
/***************************************************************************/  
CREATE PROC [dbo].[isp_NIKEKR_AutoAllocationPackShip]    
(
...
BEGIN  	
   ...
   JOIN CODELKUP CL (NOLOCK) ON CL.LISTNAME = 'NIKEDS' 
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            --SELECT @n_continue = 3
            --SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err = 64000
            --SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Allocation Failed for Order# ' + RTRIM(@c_GetOrderkey) + ' (isp_NIKEKR_AutoAllocationPackShip)' + ' ( '
         ...
         BEGIN
            --SELECT @n_continue = 3
            --SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err = 64005
            --SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Not Allocated for Order# ' + RTRIM(@c_GetOrderkey) + ' (isp_NIKEKR_AutoAllocationPackShip)' + ' ( '
            --                + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
         ...
         BEGIN
            --SELECT @n_continue = 3
            --SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err = 64010
            --SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Partially Allocated Failed for Order# ' + RTRIM(@c_GetOrderkey) + ' (isp_NIKEKR_AutoAllocationPackShip)' + ' ( '
            --                + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
      ...
      BEGIN
         ...
         BEGIN 
            --SELECT @n_continue = 3
            --SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err = 64015
            --SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error executing isp_CreatePickSlip for Order# ' + RTRIM(@c_GetOrderkey) + ' (isp_NIKEKR_AutoAllocationPackShip)' + ' ( '
            --                + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
         ...
         BEGIN 
            --SELECT @n_continue = 3
            --SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err = 64020
            --SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error executing isp_GenUCCLabelNo_Std for Order# ' + RTRIM(@c_GetOrderkey) + ' (isp_NIKEKR_AutoAllocationPackShip)' + ' ( '
            --                + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
         ...
         BEGIN  
            ...
            BEGIN   
               ...
               BEGIN 
                  --SELECT @n_continue = 3
                  --SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err = 64025
                  --SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Generate Packdetail Failed for Order# ' + RTRIM(@c_GetOrderkey) + ' (isp_NIKEKR_AutoAllocationPackShip)' + ' ( '
                  --                + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
      ...
      BEGIN
         ...
         BEGIN 
            --SELECT @n_continue = 3
            --SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err = 64030
            --SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Getting MBOLKey for Order# ' + RTRIM(@c_GetOrderkey) + ' (isp_NIKEKR_AutoAllocationPackShip)' + ' ( '
            --                + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
         ...
         BEGIN
            --SELECT @n_continue = 3
            --SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err = 64035
            --SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Generate MBOL Failed for Order# ' + RTRIM(@c_GetOrderkey) + ' (isp_NIKEKR_AutoAllocationPackShip)' + ' ( '
            --                + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
         ...
         BEGIN 
            ...
            BEGIN
               --SELECT @n_continue = 3
               --SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err = 64040
               --SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Executing isp_InsertMBOLDetail for Order# ' + RTRIM(@c_GetOrderkey) + ' (isp_NIKEKR_AutoAllocationPackShip)' + ' ( '
               --                + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
      ...
      BEGIN
         ...
         BEGIN
            --SELECT @n_continue = 3
            --SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err = 64040
            --SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Executing isp_ValidateMBOL for Order# ' + RTRIM(@c_GetOrderkey) + ' (isp_NIKEKR_AutoAllocationPackShip)' + ' ( '
            --                + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
         ...
         BEGIN
            --SELECT @n_continue = 3
            --SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err = 64040
            --SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Validate MBOL Failed for Order# ' + RTRIM(@c_GetOrderkey) + ' (isp_NIKEKR_AutoAllocationPackShip)' + ' ( '
            --                + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
      ...
      BEGIN
         ...
         BEGIN
            --SELECT @n_continue = 3
            --SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err = 64040
            --SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Executing isp_ShipMBOL ' + RTRIM(@c_GetOrderkey) + ' (isp_NIKEKR_AutoAllocationPackShip)' + ' ( '
            --                + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
...
QUIT_SP:
   ...
   BEGIN   	  
      ...
      SET @c_Subject = TRIM(@c_Storerkey) + ' Auto Allocation Pack Ship Alert - ' + @c_Date  
      ...
      BEGIN           
         ...
         BEGIN           
            ...
            --SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Executing sp_send_dbmail alert Failed! (isp_NIKEKR_AutoAllocationPackShip)' + ' ( '
            --               + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
            ...
            --   SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Update ORDERS for Order# ' + RTRIM(@c_GetOrderkey) + ' Failed! (isp_NIKEKR_AutoAllocationPackShip)' + ' ( '
            --                  + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
            --END                             
         END  
      END
   ...
   BEGIN
      ...
      BEGIN
         ROLLBACK TRAN
      END
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'isp_NIKEKR_AutoAllocationPackShip'
