...
BEGIN    
   ...
   BEGIN  
      ...
      SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5), @n_Err)   
                    ...
                    + ': Channel Attribute Configuration Not Found! (ispCHNLHLD03)'  
      GOTO QUIT_SP  
   END         
   ...
   ELSE   
   BEGIN  
      SET @c_Condition = ' AND LOC.HostWHCode IN'
                       + ' (SELECT Code FROM CODELKUP (NOLOCK)'
                       + ' WHERE Listname = ''ADSTKSTS'' AND Storerkey = @c_Storerkey'
                       + ' AND Long IN(''B'',''I'')) '  
   END     
...
QUIT_SP:  
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         END  
      END  
      EXECUTE dbo.nsp_LogError @n_Err, @c_Errmsg, 'ispCHNLHLD03'    
