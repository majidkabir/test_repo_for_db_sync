...
GO
/************************************************************************/
/* Stored Procedure: ispPreProcessing                                   */
...
/* 25-03-2013   Chee    1.0   Initial Version                           */
/* 19-10-2017   Wan     1.1   WMS-1893 - CN-Nike SDC WMS Allocation     */
/*                            Strategy CR                               */
/* 09-08-2018   NJOW01  1.2   Increase pickcode to char 30              */
/************************************************************************/
CREATE PROC [dbo].[ispPreProcessing]  
     @c_Key         NVARCHAR(10)
...
BEGIN  
   ...
   BEGIN  
      ...
      BEGIN  
         SELECT @n_Continue = 3  
         SELECT @n_Err = 63500  
         SELECT @c_ErrMsg='NSQL'+CONVERT(NVARCHAR(5),@n_Err)+': Invalid Parameters Passed (ispPreProcessing)'  
      END  
   ...
   BEGIN  
      ...
      SET @c_SQLParm =  N'@c_WaveKey NVARCHAR(10), @c_UOM NVARCHAR(10), ' + 
      ...
      EXEC sp_ExecuteSQL @c_SQL, @c_SQLParm, @c_Key, @c_UOM, @c_LocationTypeOverride, @c_LocationTypeOverRideStripe, --(Wan01) - Fixed: @c_LocationTypeOverride instead of LocationTypeOverride
      ...
      BEGIN
         ...
         SELECT @c_ErrMsg='NSQL'+CONVERT(NVARCHAR(5),@n_Err)+': Failed to EXEC ' + @c_PickCode + 
                          CASE WHEN ISNULL(@c_ErrMsg, '') <> '' THEN ' - ' + @c_ErrMsg ELSE '' END + ' (ispPreProcessing)' 
      END
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         END  
      END  
      EXECUTE nsp_logerror @n_Err, @c_ErrMsg, 'ispPreProcessing'  
