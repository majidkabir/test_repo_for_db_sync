...
GO
CREATE PROC [dbo].[isp_RdsGenUPC1] (
   ...
   BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               IF @b_debug = 1
               BEGIN
                  SELECT @n_found '@n_found', @c_size '@c_size', @c_Measurement '@c_Measurement', @c_seqno '@c_seqno', @c_upc 'UPC'
               END
            END

            -- 2008-07-03 Change Request to UPC generation:- Last 6 digit of Storer, 5 digit sequential, Check digit
            -- IF dbo.fnc_RTrim(ISNULL(@c_upc,'')) = '' -- Commented by Ricky to Trim first then Check Null
				IF ISNULL(dbo.fnc_RTrim(@c_upc),'') = ''
            BEGIN
               ---- Get UPC SeqNo
              DELETE FROM #UPCSeq
              ...
              SELECT Storerkey, CAST(SUBSTRING(SKU,7,5) AS int)
               ...
               AND   ISNULL(dbo.fnc_RTrim(UPC), '') <> ''
               --  AND LEFT(UPC,5) = RIGHT(@c_storerkey,5)
            
               IF @b_debug = 1
               BEGIN
                  select count(*) from #UPCSeq
						Select @n_minseq 'minseq', @n_maxseq 'maxseq', @n_upcseq 'upcseq', @b_getmax 'max'
               END
               ...
               BEGIN
                  ...
                  FROM #UPCSeq
                  WHERE SeqNo < @n_maxseq and Seqno > 0
