...
/********************************************************************************/  
/* Store Procedure:  isp_batching_task_pickslip_03                              */  
/* Creation Date:  02-JUL-2020                                                  */  
/* Copyright: LF                                                                */  
/* Written by:                                                                  */  
/*                                                                              */  
/* Purpose: WMS-13884-MYSâ€“ADIDASâ€“Modify Task Pickslip Report                    */  
/*          copy from isp_batching_task_pickslip                                */  
/*                                                                              */  
/* Input Parameters:                                                            */  
/*                                                                              */  
...
/* 20/07/2022  ian		1.0	  INC1862944 - fix unable to take all pickzone issue*/
/* 16/01/2023  Calvin   1.1   JSM-124978 Fix Pickzone variable (CLVN01)         */
/********************************************************************************/  
  
CREATE   PROC [dbo].[isp_batching_task_pickslip_03] (  
 ...
 BEGIN  
    ...
    BEGIN   
       SELECT @n_Continue = 3    
       SELECT @n_Err = 63200    
       SELECT @c_ErrMsg='NSQL'+CONVERT(NVARCHAR(5),@n_Err)+': Invalid Mode. The value must be 1,4,5,9 (isp_batching_task_pickslip_03)'   
       GOTO Quit  
    END  
   ...
   SET  @c_BuildOptDescr  = ''  
    ...
    SELECT top 1  @c_BuildOptDescr = c.listname + ' - ' + c.DESCRIPTION      
    FROM BuildLoadDetailLog a with (nolock)  
    JOIN BuildLoadLog b with (nolock) on  a.BatchNo = b.BatchNo  
    JOIN codelist c with (nolock) on  c.listname = b.BuildParmCode  
   ...
   --            AND 1 = CASE WHEN (@c_ReportType = '3' AND @c_MultiConsoTaskPick = '1') OR (@c_ReportType = '5' AND @c_MultiConsoTaskPick <> '1') THEN 2 ELSE 1 END)  
    ...
    LEFT JOIN Codelkup CL2 (NOLOCK) ON (PD.Storerkey = CL2.Storerkey AND CL2.Code = 'SHOWSTYLESIZE'   
                                              AND CL2.Listname = 'REPORTCFG' AND CL2.Long = 'isp_batching_task_pickslip_03' AND ISNULL(CL2.Short,'') <> 'N')     
    LEFT JOIN Codelkup CL3 (NOLOCK) ON (PD.Storerkey = CL3.Storerkey AND CL3.Code = 'SHOWLOADKEYBARCODE'   
                                              AND CL3.Listname = 'REPORTCFG' AND CL3.Long = 'isp_batching_task_pickslip_03' AND ISNULL(CL3.Short,'') <> 'N')     
    LEFT JOIN Codelkup CL4 (NOLOCK) ON (PD.Storerkey = CL4.Storerkey AND CL4.Code = 'ShowCurrentDateTime'   
                                              AND CL4.Listname = 'REPORTCFG' AND CL4.Long = 'isp_batching_task_pickslip_03' AND ISNULL(CL4.Short,'') <> 'N')     
    WHERE LP.Loadkey = @c_Loadkey  
...
Quit:  
    ...
    BEGIN    
       ...
       BEGIN    
          ...
          END    
       END    
       EXECUTE nsp_logerror @n_Err, @c_ErrMsg, 'isp_batching_task_pickslip_03'    
