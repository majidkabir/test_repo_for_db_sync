...
START:
   ...
   FROM #ORDERLINES
   GROUP BY SKU, StorerKey, Facility, Lottable01, Lottable02, Lottable03, Lottable06, 
   ...
   BEGIN 
      ...
      BEGIN
         ...
         */
         ...
         SELECT OrderQty, PackKey, Result
         FROM #ORDERLINES WITH (NOLOCK)
         WHERE ISNULL(Result,'''') <> ''''
         AND SKU = @c_SKU 
         AND StorerKey = @c_StorerKey
         ...
         FETCH NEXT FROM CURSOR_ORDERLINE_SKU INTO @n_OrderQty, @c_PackKey, @c_Result
               ...
               BEGIN
                  ...
                  BEGIN 
                     ...
                     BEGIN
                        ...
                        SET @n_Err = 13000
                        SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0)) + 
                                        ': Get PickDetailKey Failed. (ispPRCAR04)'
                        GOTO Quit
                     END
                     ELSE
                     BEGIN
...
QUIT:
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         END  
      END  
      EXECUTE nsp_logerror @n_Err, @c_ErrMsg, 'ispPRCAR04'  
