...
GO
/************************************************************************/
/* Store Procedure:  isp_GetPickSlipWave_39                             */
...
/* Return Status:  None                                                 */
/*                                                                      */
/* Usage:  Used for report dw = r_dw_print_wave_pickslip_39             */
/*         duplicate from r_dw_print_wave_pickslip_04                   */
/*                                                                      */
/* Local Variables:                                                     */
/*                                                                      */
/* Called By:                                                           */
...
CREATE PROC [dbo].[isp_GetPickSlipWave_39] (@c_wavekey NVARCHAR(10))
AS
BEGIN
   SET NOCOUNT ON
   SET ANSI_NULLS OFF
   ...
   AND Long = 'r_dw_print_wave_pickslip_39'
   AND ISNULL(Short,'') <> 'N'
   ...
   ,CASE WHEN MAX(ISNULL(CL1.Code,'')) = '' AND MIN(PICKDETAIL.UOM) IN (1,2) AND MAX(ISNULL(CL3.Code,'')) <> '' THEN '*' ELSE '' END AS scancaseidflag  
    ...
    ,''  AS cskuudf04                          
    ,CASE WHEN ISNULL(CL7.Code,'') <> '' THEN 'Y' ELSE 'N' END AS showpltpackage  
   ...
   LEFT OUTER JOIN CODELKUP CL2 (NOLOCK) ON (PICKDETAIL.Storerkey = CL2.Storerkey AND CL2.Code = 'SHOWBARCODE' 
                                          AND CL2.Listname = 'REPORTCFG' AND CL2.Long = 'r_dw_print_wave_pickslip_39' AND ISNULL(CL2.Short,'') <> 'N')  
   LEFT OUTER JOIN CODELKUP CL3 (NOLOCK) ON (PICKDETAIL.Storerkey = CL3.Storerkey AND CL3.Code = 'SHOWSCNFG' 
                                          AND CL3.Listname = 'REPORTCFG' AND CL3.Long = 'r_dw_print_wave_pickslip_39' AND ISNULL(CL3.Short,'') <> 'N')  
   LEFT OUTER JOIN CODELKUP CL4 (NOLOCK) ON (PICKDETAIL.Storerkey = CL4.Storerkey AND CL4.Code = 'PRTSKUDESC' 
                                          AND CL4.Listname = 'REPORTCFG' AND CL4.Long = 'r_dw_print_wave_pickslip_39' AND ISNULL(CL4.Short,'') <> 'N')    
   LEFT OUTER JOIN CODELKUP CL5 (NOLOCK) ON (PICKDETAIL.Storerkey = CL5.Storerkey AND CL5.Code = 'SHOWFIELD' 
                                          AND CL5.Listname = 'REPORTCFG' AND CL5.Long = 'r_dw_print_wave_pickslip_39' AND ISNULL(CL5.Short,'') <> 'N')              
   LEFT OUTER JOIN CODELKUP CL6 (NOLOCK) ON (PICKDETAIL.Storerkey = CL6.Storerkey AND CL6.Code = 'SHOWBUYERPO' 
                                          AND CL6.Listname = 'REPORTCFG' AND CL6.Long = 'r_dw_print_wave_pickslip_39' AND ISNULL(CL6.Short,'') <> 'N')        
   LEFT OUTER JOIN CODELKUP CL7 (NOLOCK) ON (PICKDETAIL.Storerkey = CL7.Storerkey AND CL7.Code = 'showPalletPackage' 
                                          AND CL7.Listname = 'REPORTCFG' AND CL7.Long = 'r_dw_print_wave_pickslip_39' AND ISNULL(CL7.Short,'') <> 'N')                                                                                                                                                                                                                                             
...
-- AND (PICKDETAIL.PickMethod = '8' OR PICKDETAIL.PickMethod = '')       
   ...
   ,CASE WHEN LEFT(ORDERS.Userdefine10,3) = 'YFD' AND SKU.SKUGroup = 'YFG' THEN
    ...
    ,CASE WHEN ISNULL(CL7.Code,'') <> '' THEN 'Y' ELSE 'N' END 
    ,CASE WHEN ISNULL(CS.UDF04,'') <> '' AND ISNULL(CS.UDF05,'') <> '' THEN ISNULL(CS.UDF04,'') + 'x' +  ISNULL(CS.UDF05,'')  ELSE '' END              
 ...
 SUCCESS:
         ...
         BEGIN
            ...
            BEGIN
               ...
               SELECT @c_errmsg = 'NSQL' + CONVERT(char(5),@n_err) +
                                  ': Insert PickingInfo Failed. (isp_GetPickSlipWave_39)' + ' ( ' +
   ...
   SELECT PickSlipNo ,OrderKey ,ExternOrderkey ,WaveKey ,StorerKey  ,InvoiceNo ,Route  ,RouteDescr ,ConsigneeKey
         ...
         ,showfield,ODUDF05,showbuyerpo,CSKUUDF04,showpltpackage,pltpackage                                 
         FROM #TEMP_PICK 
   GROUP BY PickSlipNo ,OrderKey ,ExternOrderkey ,WaveKey ,StorerKey  ,InvoiceNo ,Route  ,RouteDescr ,ConsigneeKey
         ...
         ,showbuyerpo,CSKUUDF04 ,showpltpackage,pltpackage                                             
   ...
   BEGIN
      ...
      EXECUTE dbo.nsp_logerror @n_err, @c_errmsg, 'isp_GetPickSlipWave_39'
