...
BEGIN
   ...
   BEGIN
      ...
      SET @c_errmsg = 'NSQL' + CONVERT( NVARCHAR(5), @n_err) + ': ' + 
                      'Update to TaskDetail table failed. (nspTTMPT01)' + 
   ...
   IF CURSOR_STATUS( 'global', 'Cursor_PATTaskCandidates') IN (-1)   -- -1=cursor is closed
   ...
   IF @c_AreaKey01 <> ''
      DECLARE Cursor_PATTaskCandidates CURSOR FAST_FORWARD READ_ONLY FOR --Note: global cursor
   ...
   ELSE
      DECLARE Cursor_PATTaskCandidates CURSOR FAST_FORWARD READ_ONLY FOR --Note: global cursor 
         ...
         FROM dbo.TaskDetail WITH (NOLOCK)
            JOIN dbo.LOC WITH (NOLOCK) ON (TaskDetail.FromLOC = LOC.LOC)
            JOIN dbo.AreaDetail WITH (NOLOCK) ON (AreaDetail.PutawayZone = LOC.PutAwayZone)
         WHERE dbo.TaskDetail.TaskType = 'PAT'
            AND TaskDetail.Status = '0'
            AND TaskDetail.UserKeyOverRide IN (@c_userid, '')  --(cc01)
            AND EXISTS( SELECT 1 
               FROM TaskManagerUserDetail tmu WITH (NOLOCK)
   ...
   BEGIN
      ...
      BEGIN
        ...
        BEGIN
          ...
          END
        END
        execute nsp_logerror @n_err, @c_errmsg, 'nspTTMPT01'
