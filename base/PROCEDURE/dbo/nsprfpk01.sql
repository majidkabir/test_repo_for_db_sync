...
BEGIN
   ...
   BEGIN
      IF NOT EXISTS (SELECT PICKSLIPNO FROM PICKINGINFO(NOLOCK) WHERE PICKSLIPNO = @C_PSNO )
      BEGIN
         SELECT @n_continue = 3
         SELECT @n_err = 90004
         SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Invalid Pick Slip# (nspRFPK01)'
      END
   END

   IF @n_continue = 1 or @n_continue = 2
   BEGIN
      IF EXISTS (SELECT PICKSLIPNO FROM PICKINGINFO (NOLOCK) WHERE PICKSLIPNO = @C_PSNO AND SCANOUTDATE IS NOT NULL)
      BEGIN
         SELECT @n_continue = 3
         SELECT @n_err = 90003
         SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Pick Slip# already scanned out (nspRFPK01)'
      END
   END

   IF @n_continue = 1 or @n_continue = 2
   BEGIN
      SELECT @c_ordno = ISNULL(ORDERKEY, ''),
             @c_loadno = ISNULL(ExternOrderKey, '')
      FROM  PICKHEADER (NOLOCK)
      WHERE PICKHEADERKEY = @c_psno
      IF NOT @@ERROR = 0
      BEGIN
         SELECT @n_continue = 3 
         SELECT @n_err = 66209
         SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error retriving Pick Header. (nspRFPK01)'
      END
   END
   ...
   + dbo.fnc_RTrim(@c_ordno)            + @c_senddelimiter
   ...
   BEGIN
      ...
      BEGIN
         ...
         END
      END
      execute nsp_logerror @n_err, @c_errmsg, 'nspRFPK01'
