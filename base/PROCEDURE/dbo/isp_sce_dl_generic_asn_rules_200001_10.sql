...
/* 13-Feb-2025  BDI048    1.3   Update LineNumber in STG                */
/************************************************************************/

CREATE PROCEDURE [dbo].[isp_SCE_DL_GENERIC_ASN_RULES_200001_10] (
   @b_Debug       INT            = 0
 , @n_BatchNo     INT            = 0
 , @n_Flag        INT            = 0
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
			   ...
			   set @c_TempLineNum = (select @c_LineNum + CASE WHEN LEN(BOMat.Sequence) = 1 THEN '0' + LTRIM(RTRIM(BOMat.Sequence)) ELSE LTRIM(RTRIM(BOMat.Sequence)) END 
			   ...
			   UPDATE SCE_DL_ASN_STG
				SET ReceiptLineNumber = @c_TempLineNum
				WHERE RowRefNo = @n_RowRefNo;
			   --Update line number--End
            ...
            BEGIN
			   ...
			   --Update line number--Start	
			   UPDATE SCE_DL_ASN_STG
				SET ReceiptLineNumber = CAST(FORMAT(@n_iNo, 'D5') AS NVARCHAR(10))
				WHERE RowRefNo = @n_RowRefNo;
			   --Update line number--End
