...
/************************************************************************/
/* Trigger:  ispPopulateToASN_DIAGEO                                    */
/* Creation Date: 02-May-2006                                           */
/* Copyright: IDS                                                       */
/* Written by: Vicky                                                    */
/*                                                                      */
/* Purpose:  SOS#49959 - Auto-Populate Receipt Header when MBOL Ship.   */
/*                                                                      */
/* Input Parameters:                                                    */ 
/*                                                                      */
...
CREATE PROCEDURE [dbo].[ispPopulateToASN_DIAGEO] 
      ...
      BEGIN
         ...
         BEGIN
            INSERT INTO RECEIPT (ReceiptKey, ExternReceiptKey, WarehouseReference, StorerKey, RecType, Facility, 
                                 ReceiptDate, DOCTYPE)
            VALUES (@c_NewReceiptKey, @c_ExternReceiptKey, @c_warehousereference, @c_StorerKey, 'WOR', 'UBDV',
                    @c_ReceiptDate, 'R')
         END
         ELSE
         BEGIN
            ...
            SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Generate Receipt Key Failed! (ispPopulateToASN_DIAGEO)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
         END
      END    
      ELSE
      BEGIN
         SELECT @n_continue = 3
         SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err = 63526   
         SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Storer Key is BLANK! (ispPopulateToASN_DIAGEO)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
      END
              ...
              DECLARE PICK_CUR CURSOR FAST_FORWARD READ_ONLY FOR 
               ...
               BEGIN
                  ...
                  INSERT INTO RECEIPTDETAIL (ReceiptKey,          ReceiptLineNumber,   ExternReceiptKey, 
                                             ...
                                             BeforeReceivedQty,   DateReceived )
                                             
                              VALUES        (@c_NewReceiptKey,    @c_ReceiptLine,      @c_ExternReceiptKey,
                                             @c_OrderLine,        @c_StorerKey,        @c_SKU,
                                             @c_ID,               ISNULL(@n_QtyReceived,0),              0,
                                             @c_UOM,              @c_Packkey,          'VA_MA',
                                             @c_Lottable01,       @c_Lottable02,       @c_Lottable03,    @d_Lottable04,    @d_Lottable05,         
                                             @c_Lottable06,       @c_Lottable07,       @c_Lottable08,    @c_Lottable09,    @c_Lottable10,
                                             @c_Lottable11,       @c_Lottable12,       @d_Lottable13,    @d_Lottable14,    @d_Lottable15,
                                             @n_QtyReceived,      @c_ReceiptDate )
