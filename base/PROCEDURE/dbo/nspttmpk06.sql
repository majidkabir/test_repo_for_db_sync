...
BEGIN  
   ...
   BEGIN  
      ...
      WHERE  TA.Tasktype IN ('PK','DPK','DRP', 'SPK')  
            AND TA.Status = '9'  
   ...
   BEGIN  
      ...
      AND    STATUS = '3' 
      AND    Tasktype IN ('PK','DPK','DRP', 'SPK')    
      ...
      BEGIN  
         ...
         SELECT @c_errmsg = 'NSQL'+CONVERT(CHAR(5) ,@n_err)+  
                ': Update to TaskDetail table failed. (nspTTMPK06)'+' ( '+  
         ...
         BEGIN  
            ...
            BEGIN  
              ...
              FOR  
                  ...
                  AND TaskDetail.TASKTYPE IN ('PK', 'DPK', 'DRP', 'SPK')  
                        ...
                        -- Cannot mix pickmethod in a tote
                        AND TaskDetail.PickMethod = CASE WHEN ISNULL( @c_LastPickMethod, '') = '' 
                                                         THEN TaskDetail.PickMethod 
                                                         ELSE CASE WHEN ISNULL( @c_LastLoc, '') <> '' 
                                                              THEN @c_LastPickMethod ELSE TaskDetail.PickMethod END 
                                                         END */
                  ORDER BY  
                         TaskDetail.Priority   
            ...
            BEGIN  
               ...
               LEFT OUTER JOIN (SELECT SR.BoxNumber, AD.AreaKey, MAX(SR.READING_TIME) As Reading_Time     
                     ...
                     AND VR.BoxNumber = CASE WHEN ISNUMERIC(ISNULL(TaskDetail.DropID,'X')) = 1     
                                        ...
                                        END     
                     AND VR.Reading_Time >= TaskDetail.AddDate                                                                              
                     WHERE  TaskDetail.TASKTYPE IN ('PK', 'DPK', 'DRP', 'SPK')  
                            AND TaskDetail.USERKEY = ''  
                            AND TaskDetail.STATUS = '0'  
                            AND Loadplan.Status < '9'    
                            AND EXISTS(SELECT 1 FROM TaskManagerUserDetail tmu WITH (NOLOCK)  
            ...
            BEGIN  
               ...
               SELECT @c_errmsg = 'NSQL'+CONVERT(CHAR(5) ,@n_err)+  
                     ': Could not Open Cursor_Eval02. (nspTTMPK06)'+' ( '   
                    +' SQLSvr MESSAGE='+RTRIM(@c_errmsg)   
                    +' ) '  
            END  
            ELSE  
            ...
            BEGIN  
               ...
               BEGIN  
                  ...
                  -- Get Orders.UserDefine03 for SPK task
                  IF @c_TaskType = 'SPK'
                  BEGIN
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         WHERE TD.Storerkey = @c_StorerKey  
          ...
          AND SKU.ItemClass = ISNULL(RTRIM(@c_ItemClass ),'') -- (james01)
          AND O.UserDefine03 = ISNULL(RTRIM(@c_OrderUserDefine03),'') ) 
          BEGIN  
            GOTO EVALUATIONDONE  
          END  
   ...
   BEGIN  
      ...
      BEGIN  
          ...
          BEGIN  
              ...
              SELECT @c_errmsg = CONVERT(CHAR(250) ,@n_err)  
                    ,@n_err = 81204 -- Should Be Set To The SQL Errmessage but I don't know how to do so.  
              SELECT @c_errmsg = 'NSQL'+CONVERT(CHAR(5) ,@n_err)+': UPDATE TaskDetail Failed. (nspTTMPK06)'+' ( '   
                    + ' SQLSvr MESSAGE='+RTRIM(@c_errmsg)+' ) '  
          END  
      END  
         ...
         END  
          ...
          BEGIN  
              ...
              SELECT @c_errmsg = CONVERT(CHAR(250) ,@n_err)  
                    ,@n_err = 81204 -- Should Be Set To The SQL Errmessage but I don't know how to do so.  
              SELECT @c_errmsg = 'NSQL'+CONVERT(CHAR(5) ,@n_err)+  
                     ': UPDATE TaskDetail Failed. (nspTTMPK06)'+' ( '   
                    +' SQLSvr MESSAGE='+RTRIM(@c_errmsg)   
                    +' ) '  
          END  
      END  
              
      -- Lock PickMethod = PIECE Task  
      IF @c_PickMethod LIKE 'PIECE%'   
      BEGIN  
        -- (Vicky01) - Start  
        IF @c_DropID <> '' AND @c_PrevToteFullFlag = 'N'  
        BEGIN  
          ...
          JOIN   ORDERS O WITH (NOLOCK) ON O.OrderKey = TASKDETAIL.OrderKey 
          WHERE  TASKDETAIL.Storerkey = @c_StorerKey  
           AND TASKDETAIL.TaskType = 'SPK'  
           AND TASKDETAIL.PickMethod = @c_PickMethod
           AND TASKDETAIL.UserKey = ''  
           AND TASKDETAIL.[Status] = '0'  
           AND TASKDETAIL.DropID = @c_DropID  
           AND TASKDETAIL.Message01 = @c_Consigneekey 
           AND TASKDETAIL.Loadkey = @c_LoadKey 
      ...
      BEGIN  
         ...
         END  
         ELSE IF @c_PickMethod LIKE 'CASE%'   
         BEGIN  
            SET @nNumUserFound = 1  
         END  
         IF @nNumUserFound <> 1  
         BEGIN  
            ...
            SELECT @c_errmsg = CONVERT(CHAR(250) ,@n_err)  
                  ,@n_err = 81204 -- Should Be Set To The SQL Errmessage but I don't know how to do so.  
            SELECT @c_errmsg = 'NSQL'+CONVERT(CHAR(5) ,@n_err)+  
             ': Lock TaskDetail Failed. (nspTTMPK06)'+' ( '   
                  +' SQLSvr MESSAGE='+RTRIM(@c_errmsg)   
                  +' ) '  
            GOTO NOTHINGTOSEND                    
         END                     
   ...
   BEGIN  
      ...
      BEGIN  
          ...
          END  
      END  
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'nspTTMPK06'  
