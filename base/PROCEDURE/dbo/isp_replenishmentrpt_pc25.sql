...
/***************************************************************************/    
/* Stored Procedure: isp_ReplenishmentRpt_PC25                             */    
/* Creation Date: 28-Aug-2018                                              */    
/* Copyright: LFL                                                          */    
/* Written by:                                                             */    
/*                                                                         */    
/* Purpose: WMS-6069 TW DTDC Replenishment Report.                         */    
/*                                                                         */    
/*                                                                         */    
/* Called By: Replenishment Report r_replenishment_report_PC25             */    
/*                                                                         */    
/* PVCS Version: 1.1                                                       */    
/*                                                                         */    
/* Version: 7.0                                                            */    
...
/***************************************************************************/    
CREATE PROC [dbo].[isp_ReplenishmentRpt_PC25]    
...
BEGIN    
   ...
   DECLARE @c_CurrentSKU NVARCHAR(20)  
          ...
          ,@c_CurrLottableValue NVARCHAR(30)
          ,@c_SQL NVARCHAR(MAX)
          ,@n_MinReplZoneQty INT
          ,@c_ByReplenZone NVARCHAR(5)
          ,@c_RepLottableValue NVARCHAR(30)
          ,@CUR_REPLOT CURSOR
   ...
   CREATE TABLE #TMP_REPLZONELOT (LOT NVARCHAR(10) NULL,
                                  LottableValue NVARCHAR(30) NULL,
                                  QtyOnReplZone INT NULL) 
                                  
   CREATE TABLE #TMP_LOT (Priority NVARCHAR(5) NULL, 
                          LOT NVARCHAR(10) NULL,
                          QtyOverAllocated INT NULL,
...
--                              END,    
      ReplenishmentSeverity = CASE   
                                 WHEN ISNULL(sc.SValue, '0') = '1' THEN  
                                    ...
                                    -- SUM(LOTxLOCxID.QtyExpected)   
                                 WHEN SUM(ISNULL(LOTxLOCxID.QtyExpected - LOTXLOCXID.PendingMoveIn, 0)) > 0 AND   
                                    ((SKUxLOC.Qty - SKUxLOC.QtyPicked - SKUxLOC.QtyAllocated) + SUM(ISNULL(LOTXLOCXID.PendingMoveIn,0)) > SKUxLOC.QtyLocationMinimum) THEN      
                                    SUM(ISNULL(LOTxLOCxID.QtyExpected - LOTXLOCXID.PendingMoveIn, 0))     
                                 ELSE   
                                    SKUxLOC.QtyLocationLimit - ( (SKUxLOC.Qty - (SKUxLOC.QtyPicked + SKUxLOC.QtyAllocated )) + SUM(ISNULL(LOTXLOCXID.PendingMoveIn,0)) )    
                              END,    
      SKUxLOC.StorerKey,    
      SKUxLOC.SKU,    
      SKUxLOC.LOC,    
      ReplenishmentCasecnt,    
      OverAllocation = CASE WHEN SUM(ISNULL(LOTxLOCxID.QtyExpected - LOTXLOCXID.PendingMoveIn, 0)) > 0     
                              OR SKUxLOC.QtyLocationMinimum < (SKUxLOC.Qty - (SKUxLOC.QtyPicked + SKUxLOC.QtyAllocated)) + SUM(ISNULL(LOTXLOCXID.PendingMoveIn,0)) THEN   -- SHONG02  
                                 'Y'    
                            ELSE 'N' END,  
      SUM(ISNULL(LOTxLOCxID.QtyExpected - LOTXLOCXID.PendingMoveIn, 0))   
      From SKUxLOC (NOLOCK)   
      ...
      GROUP BY SKUxLOC.ReplenishmentPriority,    
               ...
               ISNULL(sc.SValue, '0')    
      HAVING SUM(ISNULL(LOTxLOCxID.QtyExpected - LOTXLOCXID.PendingMoveIn, 0)) > 0 OR     
            ( ((SKUxLOC.Qty - SKUxLOC.QtyPicked - SKUxLOC.QtyAllocated) + SUM(ISNULL(LOTXLOCXID.PendingMoveIn,0)) <= SKUxLOC.QtyLocationMinimum ) AND ISNULL(sc.SValue, '0') = '0')    
      ...
      BEGIN    
         ...
         IF @b_debug = 1    
         BEGIN   
            SELECT @c_CurrentStorer '@c_CurrentStorer', @c_CurrentSKU '@c_CurrentSKU',  @c_CurrentLoc '@c_CurrentLoc', @n_CurrentSeverity '@n_CurrentSeverity', @n_QtyExpedted '@n_QtyExpedted'  
         END   
         
         IF EXISTS(SELECT 1 
                   FROM  CODELKUP (NOLOCK) 
                   WHERE Listname = 'REPLZONE'
                   AND Storerkey = @c_CurrentStorer)
            SET @c_ByReplenZone = 'Y'
         ELSE
            SET @c_ByReplenZone = 'N'
                 
         SELECT @c_LottableName = '', @c_Seq = ''
         ...
         SELECT @c_Packkey = PACK.PackKey,    
                ...
                @n_Pallethi = ISNULL(PACK.Pallethi, 0)
         ...
         IF @c_LottableName IN('LOTTABLE01','LOTTABLE02','LOTTABLE03','LOTTABLE04','LOTTABLE05','LOTTABLE06','LOTTABLE07','LOTTABLE08',
                               'LOTTABLE09','LOTTABLE10','LOTTABLE11','LOTTABLE12','LOTTABLE13','LOTTABLE14','LOTTABLE15')
         BEGIN
            SET @c_SQL = N'       
            SELECT TOP 1 @c_LottableValue = ' +
            CASE WHEN @c_LottableName IN ('LOTTABLE04','LOTTABLE05','LOTTABLE13','LOTTABLE14','LOTTABLE15') THEN
                 ' CONVERT(NVARCHAR(10),' + 'LA.' + RTRIM(LTRIM(@c_LottableName)) + ', 112) '
                 ELSE 'LA.' + RTRIM(LTRIM(@c_LottableName)) END +
          ' FROM LOTxLOCxID LLI (NOLOCK)  
            ...
            AND (LLI.Qty > 0 OR (LLI.QtyExpected - LLI.PendingMoveIn > 0))'  
         
            EXEC sp_executesql @c_SQL,
            N'@c_LottableValue NVARCHAR(30) OUTPUT, @c_CurrentStorer NVARCHAR(15), @c_CurrentSKU NVARCHAR(20), @c_CurrentLoc NVARCHAR(10)', 
         ...
         --ORDER BY ' +CASE WHEN ISNULL(@c_LottableName,'') <> '' THEN 'LA.' + RTRIM(LTRIM(@c_LottableName)) + ' ' + RTRIM(@c_Seq) + ', ' END +
         ...
         */   
         ...
         SELECT LOTxLOCxID.LOT, SUM(((LOTxLOCxID.QtyPicked + LOTxLOCxID.QtyAllocated) - LOTxLOCxID.Qty) - LOTxLOCxID.PendingMoveIn) AS [QtyOverAllocated], LA.Lottable04, LA.Lottable05    
         INTO #TMP_OVERALLOC    
         From LOTxLOCxID (NOLOCK)  
         ...
         AND LOTxLOCxID.Qty < (LOTxLOCxID.QtyPicked + LOTxLOCxID.QtyAllocated) - LOTxLOCxID.PendingMoveIn      
         AND LOTxLOCxID.LOC = @c_CurrentLoc    
         AND LOT.Status     = 'OK'         
         GROUP BY LOTxLOCxID.LOT, LA.Lottable04, LA.Lottable05
         
         TRUNCATE TABLE #TMP_REPLZONELOT
         TRUNCATE TABLE #TMP_LOT
         
         SET @c_SQL = N'
         INSERT INTO #TMP_REPLZONELOT (Lot, LottableValue, QtyOnReplZone)
         SELECT LOTxLOCxID.Lot, ' + 
         CASE WHEN ISNULL(@c_LottableName,'') = '' THEN 
                '''''' 
              WHEN @c_LottableName IN ('LOTTABLE04','LOTTABLE05','LOTTABLE13','LOTTABLE14','LOTTABLE15') THEN
                ' CONVERT(NVARCHAR(10),' + 'LOTATTRIBUTE.' + RTRIM(LTRIM(@c_LottableName)) + ', 112) '
              ELSE 'LOTATTRIBUTE.' + RTRIM(LTRIM(@c_LottableName)) END + ' AS LottableValue, ' +
       ' SUM(LOTxLOCxID.Qty - LOTxLOCxID.QtyPicked - LOTxLOCxID.QtyAllocated - LOTxLOCxID.QtyReplen) AS QtyOnReplZone
         FROM LOTxLOCxID (NOLOCK)    
         JOIN LOC (NOLOCK) ON LOTxLOCxID.LOC = LOC.LOC    
         ...
         AND (LOC.Putawayzone IN (SELECT Code FROM CODELKUP (NOLOCK) WHERE Listname = ''REPLZONE'' AND Storerkey = @c_CurrentStorer) OR @c_ByReplenZone = ''N'') 
         GROUP BY LOTxLOCxID.Lot ' + 
         CASE WHEN ISNULL(@c_LottableName,'') = '' THEN
                 ''
              WHEN @c_LottableName IN ('LOTTABLE04','LOTTABLE05','LOTTABLE13','LOTTABLE14','LOTTABLE15') THEN
                 ' ,CONVERT(NVARCHAR(10),' + 'LOTATTRIBUTE.' + RTRIM(LTRIM(@c_LottableName)) + ', 112) '
                 ELSE ',LOTATTRIBUTE.' + RTRIM(LTRIM(@c_LottableName)) END 
       /*' ORDER BY ' +  CASE WHEN ISNULL(@c_LottableName,'') = '' THEN
                 ''
              WHEN @c_LottableName IN ('LOTTABLE04','LOTTABLE05','LOTTABLE13','LOTTABLE14','LOTTABLE15') THEN
                 ' CONVERT(NVARCHAR(10),' + 'LOTATTRIBUTE.' + RTRIM(LTRIM(@c_LottableName)) + ', 112) '
                 ELSE 'LOTATTRIBUTE.' + RTRIM(LTRIM(@c_LottableName)) END +*/
         ...
         EXEC sp_executesql @c_SQL,
               N'@c_CurrentStorer NVARCHAR(15), @c_CurrentSKU NVARCHAR(20), @c_CurrentLoc NVARCHAR(10), @c_Zone01 NVARCHAR(10), @c_HostWHCode NVARCHAR(10), @c_ByReplenZone NVARCHAR(5)', 
               @c_CurrentStorer,
               @c_CurrentSKU,
               @c_CurrentLoc,           
               @c_Zone01,
         ...
         IF @n_Pallethi > 0 AND @n_PalletCnt > 0         
            SET @n_MinReplZoneQty = @n_PalletCnt / @n_Pallethi
         ELSE
            SET @n_MinReplZoneQty = 0

         IF @b_debug = 1    
         BEGIN   
            SELECT @c_LottableValue '@c_LottableValue', @c_HostWHCode '@c_HostWHCode', @n_Pallethi '@n_Pallethi', @n_MinReplZoneQty '@n_MinReplZoneQty' 
            SELECT * FROM #TMP_OVERALLOC
            SELECT * FROM #TMP_REPLZONELOT 
         END   
         
         IF @c_Seq = 'DESC'              
         BEGIN
            SET @CUR_REPLOT = CURSOR LOCAL FAST_FORWARD READ_ONLY FOR 
               SELECT TOP 1 LottableValue
               FROM #TMP_REPLZONELOT 
               WHERE (LottableValue = @c_LottableValue OR @c_LottableValue = '')           
               GROUP BY LottableValue              
               HAVING SUM(QtyOnReplZone) > @n_MinReplZoneQty
               ORDER BY LottableValue DESC
         END  
         ELSE
         BEGIN
            SET @CUR_REPLOT = CURSOR LOCAL FAST_FORWARD READ_ONLY FOR 
               ...
               GROUP BY LottableValue              
               HAVING SUM(QtyOnReplZone) > @n_MinReplZoneQty
               ORDER BY LottableValue
         END      

         OPEN @CUR_REPLOT    
    
         FETCH NEXT FROM @CUR_REPLOT INTO @c_RepLottableValue     
         
         WHILE @@FETCH_STATUS = 0    
         BEGIN
            INSERT INTO #TMP_LOT (Priority, Lot, QtyOverAllocated, Lottable04, Lottable05, LottableValue)
                 SELECT 2 AS Priority, RZ.Lot, 0 as QtyOverAllocated, LA.Lottable04, LA.Lottable05, RZ.LottableValue
                 FROM #TMP_REPLZONELOT RZ
                 JOIN LOTATTRIBUTE LA (NOLOCK) ON RZ.Lot = LA.Lot              
               WHERE RZ.LottableValue = @c_RepLottableValue
               AND RZ.Lot NOT IN (SELECT Lot FROM #TMP_OVERALLOC)                                
            
            FETCH NEXT FROM @CUR_REPLOT INTO @c_RepLottableValue     
         END
         CLOSE @CUR_REPLOT
         DEALLOCATE @CUR_REPLOT
         ...
         BEGIN
              ...
              FROM #TMP_REPLZONELOT RZ
              JOIN LOTATTRIBUTE LA (NOLOCK) ON RZ.Lot = LA.Lot              
            WHERE (RZ.LottableValue = @c_LottableValue OR @c_LottableValue = '')
            AND RZ.Lot NOT IN (SELECT Lot FROM #TMP_OVERALLOC)  
         END
         */                     
         ...
         BEGIN                             
            ...
            SELECT 1 AS Priority, LOT, [QtyOverAllocated], LOTTABLE04, LOTTABLE05, '' AS LottableValue
            FROM   #TMP_OVERALLOC   
            UNION ALL   
            SELECT 2 AS Priority, Lot, QtyOverallocated, Lottable04, Lottable05, LottableValue 
            FROM #TMP_LOT 
            ORDER BY Priority, LottableValue DESC, Lottable04, Lottable05, Lot
         END
         ELSE
         BEGIN
            DECLARE LOT_CUR CURSOR LOCAL FAST_FORWARD READ_ONLY FOR     
            ...
            SELECT 2 AS Priority, Lot, QtyOverallocated, Lottable04, Lottable05, LottableValue 
            FROM #TMP_LOT 
            ORDER BY Priority, LottableValue, Lottable04, Lottable05, Lot
         END 
         
         /*SELECT 2 AS Priority, LOTxLOCxID.LOT, 0 AS [QtyOverAllocated], LOTATTRIBUTE.LOTTABLE04, LOTATTRIBUTE.LOTTABLE05      
         From LOTxLOCxID (NOLOCK)    
         JOIN LOC (NOLOCK) ON LOTxLOCxID.LOC = LOC.LOC    
         JOIN LOTATTRIBUTE (NOLOCK) ON LOTxLOCxID.LOT = LOTATTRIBUTE.LOT    
         JOIN LOT (NOLOCK) ON LOTxLOCxID.LOT = LOT.Lot    
         ...
         ORDER BY Priority, LOTTABLE04, LOTTABLE05 --IN00318574   
         */
    
         OPEN LOT_CUR    
    
         FETCH NEXT FROM LOT_CUR INTO @n_Priority, @c_FromLot, @n_QtyOverAllocated, @d_CurrLottable04, @d_CurrLottable05, @c_CurrLottableValue     
         ...
         BEGIN     
            ...
            BEGIN   
               SELECT @c_Fromlot '@c_Fromlot', CONVERT(VARCHAR(12), @d_CurrLottable04, 101) '@d_CurrLottable04',   
                      ...
                      @c_OverAllocation '@c_OverAllocation'  
            END   
            ...
            DECLARE LOC_CUR CURSOR LOCAL FAST_FORWARD READ_ONLY FOR     
               ...
               ORDER BY SUM(LOTxLOCxID.Qty - LOTxLOCxID.QtyPicked - LOTxLOCxID.QtyAllocated - LOTxLOCxID.QtyReplen),    
            ...
            BEGIN                                               
               ...
               BEGIN    
                  SELECT TOP 1  
                         @c_Fromid = ID,    
                         @n_OnHandQty = LOTxLOCxID.Qty - LOTxLOCxID.QtyPicked - LOTxLOCxID.QtyAllocated - LOTxLOCxID.QtyReplen    
                  ...
                  AND LOC.Status <> 'HOLD'    
                  AND LOTxLOCxID.Qty - LOTxLOCxID.QtyPicked - LOTxLOCxID.QtyAllocated - LOTxLOCxID.QtyReplen > 0    
                  AND LOTxLOCxID.Qty - LOTxLOCxID.QtyPicked - LOTxLOCxID.QtyAllocated - LOTxLOCxID.QtyReplen <= @n_RemainingQty    
                  AND LOTxLOCxID.QtyExpected = 0 -- make sure we aren't going to try to pull From a Location that needs stuff to satisfy existing demAND    
                  AND LOTxLOCxID.LOC <> @c_CurrentLoc    
                  AND LOC.Facility = @c_Zone01    
                  AND LOC.HostWHCode = @c_HostWHCode -- sos 2199    
                  ...
                  SELECT @n_FullPallet = (Qty - QtyAllocated - QtyPicked - QtyReplen)    
                  From LOTxLOCxID (NOLOCK)    
                  WHERE StorerKey = @c_CurrentStorer    
                  AND SKU = @c_CurrentSKU    
                  AND LOC = @c_FromLoc    
...
--                  ELSE    
                  ...
                  BEGIN    
                     ...
                     IF @b_debug = 1    
                     BEGIN    
                        SELECT 'INSERTED : ' as Title, @c_CurrentSKU ' SKU', @c_Fromlot 'LOT',  @c_CurrentLoc 'Curr LOC', @c_Fromid 'ID',     
                               @n_FromQty 'Qty', @c_FromLoc 'FromLoc'    
                     END     
   ...
   QUIT_SP:
      ...
      BEGIN    
         ...
         AND  (LOC.PickZone = @c_ReplGrp OR @c_ReplGrp = 'ALL')   --(Wan01)                                                          
