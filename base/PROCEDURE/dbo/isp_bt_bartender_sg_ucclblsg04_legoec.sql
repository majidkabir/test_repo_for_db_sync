...
/********************************************************************************/                   
/* Copyright: MAERSK                                                            */                   
/* Purpose: isp_BT_Bartender_SG_UCCLBLSG04_LEGOEC                               */                   
/*                                                                              */                   
/* Modifications log:                                                           */                   
/*                                                                              */                   
...
CREATE   PROC [dbo].[isp_BT_Bartender_SG_UCCLBLSG04_LEGOEC]                        
...
BEGIN                        
  ...
  DECLARE  @d_Trace_StartTime   DATETIME,     
   ...
   SET @c_SQLJOIN = + ' SELECT DISTINCT '''', ' + CHAR(13) --1
                    + ' CASE WHEN @c_Sparm04 = ''FALSE'' OR ISNULL(PIF.TrackingNo,'''') = '''' THEN PH.Pickslipno ELSE OH.ExternOrderKey END, ' + CHAR(13) --2
                    + ' PD.LabelNo, ' + CHAR(13) --3
                    + ' CASE WHEN @c_Sparm04 = ''FALSE'' OR ISNULL(PIF.TrackingNo,'''') = '''' THEN '''' ELSE TRIM(ISNULL(OH.C_Company,'''')) END, ' + CHAR(13) --4
                    + ' CASE WHEN @c_Sparm04 = ''FALSE'' OR ISNULL(PIF.TrackingNo,'''') = '''' THEN '''' ELSE TRIM(ISNULL(OH.C_Address1,'''')) END,' + CHAR(13) --5
                    + ' CASE WHEN @c_Sparm04 = ''FALSE'' OR ISNULL(PIF.TrackingNo,'''') = '''' THEN '''' ELSE TRIM(ISNULL(OH.C_Address2,'''')) END,' + CHAR(13) --6
                    + ' CASE WHEN @c_Sparm04 = ''FALSE'' OR ISNULL(PIF.TrackingNo,'''') = '''' THEN '''' ELSE TRIM(ISNULL(OH.C_Address3,'''')) END, ' + CHAR(13) --7
                    + ' CASE WHEN @c_Sparm04 = ''FALSE'' OR ISNULL(PIF.TrackingNo,'''') = '''' '
                    + '      THEN '''' '
                    + '      ELSE TRIM(ISNULL(OH.C_Zip,'''')) + '' '' + TRIM(ISNULL(OH.C_City,'''')) + '' '' + TRIM(ISNULL(OH.C_Country,''''))  END, ' + CHAR(13) --8  
