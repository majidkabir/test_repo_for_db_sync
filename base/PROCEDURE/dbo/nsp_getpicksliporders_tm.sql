...
/************************************************************************/
/* Stored Procedure: nsp_GetPickSlipOrders_TM                           */
/* Creation Date:                                                       */
/* Copyright: IDS                                                       */
/* Written by:                                                          */
/*                                                                      */
...
CREATE PROC [dbo].[nsp_GetPickSlipOrders_TM] (@c_loadkey NVARCHAR(10))
AS
BEGIN
   SET NOCOUNT ON 
   SET ANSI_NULLS OFF
...
ELSE
   ...
   ELSE
      ...
      PACK (NOLOCK),        LOC (NOLOCK), 	SKUXLOC (NOLOCK)
      ...
      BEGIN
         ...
         BEGIN
            --tlting01
            IF NOT EXISTS( SELECT 1 FROM PICKHEADER (NOLOCK) WHERE ExternOrderKey = @c_LoadKey AND OrderKey = @c_OrderKey AND ZONE = '5')
            BEGIN
               EXECUTE nspg_GetKey
               "PICKSLIP",
               9,
               ...
               BEGIN TRAN
                  ...
                  VALUES
                  (@c_pickheaderkey, @c_OrderKey, @c_LoadKey,     "0",      "5",  "")
