...
/*                             Strategy CR                              */      
/* 09-OCT-2017 Wan02    1.2   WMS-1893 - CN-Nike SDC WMS Allocation     */      
/*                            Strategy CR                               */      
/* 18-JUL-2019 CSCHONG  1.3   WMS-9822-revised report condition (CS01)  */      
/* 08-AUG-2019 CSCHONG  1.4   WMS-10204 - add channel checking (CS01a)  */      
/* 26-NOV-2019 CSCHONG  1.5   WMS-10204 - Fix uom issue (CS01c)         */      
/************************************************************************/          
CREATE  PROC [dbo].[ispPRNIK04]              
...
BEGIN          
   ...
   BEGIN         
      ...
      --              + ': Invalid Wave Piece Pick Task Dispatch Method. Must Be INLINE/ECOM (ispPRNIK04)'      
      GOTO QUIT_SP      
   END                           
   ...
   BEGIN      
      ...
      WHERE WD.Wavekey = @c_WaveKey      
         ...
         AND O.Status < '9'         
         AND (OD.OpenQty - ( OD.QtyAllocated + OD.QtyPreAllocated + OD.QtyPicked )) > 0       
         AND ISNULL(RTRIM(OD.Lottable01),'') = ''  --(Wan02)          
      GROUP BY O.Facility      
            ,  O.Orderkey      
            ,  OD.Storerkey               
            ,  OD.Sku               
   ...
   SELECT tp.facility,tp.orderkey as Orderkey,tp.storerkey as storerkey,tp.sku as sku,      
          ...
          ci.Channel_ID,(ci.Qty-ci.QtyAllocated-ci.QtyOnHold) as qtyavail ,      
          CASE WHEN Orderqty > (ci.Qty-ci.QtyAllocated-ci.QtyOnHold) THEN (ci.Qty-ci.QtyAllocated-ci.QtyOnHold) ELSE Orderqty END,'N'      
   ...
   BEGIN      
      SELECT 'ispPRNIK04'      
      SELECT * FROM #ORDERLINES WITH (NOLOCK) where SKU ='AA8854010XL'     
   SELECT 'Get table '      
   SELECT * FROM #CHANNELINFO WITH (NOLOCK)   where SKU ='AA8854010XL' --CS01a      
   END      
   ...
   FETCH NEXT FROM CUR_ORDERLINES INTO @c_StorerKey, @c_SKU, @c_Facility, @c_Packkey         
                                    ...
                                    ,  @n_OrderQty--,@c_Channel      
                                    ,  @c_ChannelInvMgmt                  --CS01a      
                
   WHILE (@@FETCH_STATUS <> -1)                
   BEGIN       
      
   --CS01a START      
   SET @n_TTLCHANNELAvaiQty = 1      
      
   IF @c_ChannelInvMgmt = '1'      
   BEGIN      
      SELECT @n_TTLCHANNELAvaiQty = SUM(Qtyneed)      
      FROM #CHANNELINFO      
   ...
   IF @n_OrderQty > @n_TTLCHANNELAvaiQty      
   BEGIN      
     SET @n_OrderQty = @n_TTLCHANNELAvaiQty      
   END      
   ...
   BEGIN   
         PRINT '--------------------------------------------' + CHAR(13) +      
               ...
               ', @n_TTLCHANNELAvaiQty: ' + CONVERT(NVARCHAR(5),@n_TTLCHANNELAvaiQty) + CHAR(13) +      
    ...
    --BEGIN      
      ...
      SET @c_SQLParm =  N'@c_Facility   NVARCHAR(5),  @c_StorerKey  NVARCHAR(15), @c_SKU NVARCHAR(20) '            
                     ...
                     +  ',@c_Lottable12 NVARCHAR(30) '       
                     +  ',@c_LocationTypeOverride NVARCHAR(10),@c_channel NVARCHAR(30) '  --(Wan02)   --CS01a      
      ...
      EXEC sp_ExecuteSQL @c_SQL, @c_SQLParm, @c_Facility, @c_StorerKey, @c_SKU      
                        ...
                        ,@c_LocationTypeOverride ,@c_channel                 --(Wan02)  --CS01a      
            
      
   IF @b_Debug='1'  --and  @c_SKU ='AA8854010XL'       
   BEGIN      
        SELECT @c_channel '@c_channel', @c_Facility '@c_Facility',@c_SKU '@c_SKU',@c_Lottable01 '@c_Lottable01',@c_Lottable02 '@c_Lottable02',@c_Lottable07 '@c_Lottable07',  
               @c_LocationType '@c_LocationType',@c_LocationCategory '@c_LocationCategory', @c_LocationHandling '@c_LocationHandling'     
        print @c_SQL      
   ...
   IF @b_Debug='1'  --and  @c_SKU ='AA8854010XL'      
   BEGIN      
     SELECT @n_OrderQty,'#NumPool'      
     SELECT * from #NumPool      
   END      
                        
      --Get Lower Bound to reduce loop size      
      SET @n_LowerBound = 0      
      SELECT @n_LowerBound = ISNULL(MIN(UCCQty),0)            
      FROM #NumPool      
   ...
   BEGIN      
      ...
      WHERE FLOOR(@n_OrderQty/UCCQty) <= AvailCTNCount      
      AND AvailCTNCount > 0      
      ORDER BY CASE WHEN @n_OrderQty % UCCQty = 0 THEN 1 ELSE 2 END   
   ...
   END      
      ...
      IF @b_Debug = 1    --and  @c_SKU ='AA8854010XL'    
      BEGIN      
         SELECT 'CHK UCCQTY ', @n_UCCQty '@n_UCCQty'  
      END      
      ...
      BEGIN      
         ...
         IF @b_Debug = 1  --and  @c_SKU ='AA8854010XL'    
         BEGIN      
            SELECT '#CombinationPool',@n_OrderQty '@n_OrderQty', *       
            FROM #CombinationPool WITH (NOLOCK)      
      ...
      BEGIN      
         ...
         FETCH NEXT FROM CUR_PICKCTN INTO @n_SeqNo, @c_Lot, @c_Loc, @c_ID, @n_AvailCTNCount--,@n_channel_id          --CS01a      
         ...
         BEGIN       
            ...
            BEGIN       
               ...
               SET @b_UpdateUCC = 1      
               SET @n_CHANNELAvaiQty = 1       --CS01      
               SET @c_PrevOrderKey  = ''      
               SET @c_PrevChannel  = ''        --CS01a      
    
            IF @b_debug='1'  --and  @c_SKU ='AA8854010XL'    
            BEGIN    
              PRINT 'Check Order Cursor'    
              SELECT @c_sku '@c_sku'    
            END    
     ...
     SET @c_SQL =      
           ...
           --    + CHAR(13) + 'AND OD.OpenQty - ( OD.QtyAllocated + OD.QtyPreAllocated + OD.QtyPicked ) >=1 '      
     -- + CHAR(13) + ' AND CA.Allocated = ''N'' '      
     ...
     END      
               ...
               SET @c_SQLParm =  N'@c_WaveKey NVARCHAR(10), @c_StorerKey NVARCHAR(15), @c_SKU NVARCHAR(20), @c_Packkey NVARCHAR(10)'           
                              ...
                              + ', @c_Lottable06 NVARCHAR(30), @c_Lottable07 NVARCHAR(30), @c_Lottable08 NVARCHAR(30)'        
                              + ', @c_Lottable09 NVARCHAR(30), @c_Lottable10 NVARCHAR(30), @c_Lottable11 NVARCHAR(30)'       
                              + ', @c_Lottable12 NVARCHAR(30), @n_CTNQty INT '                        
                        
               EXEC sp_ExecuteSQL @c_SQL, @c_SQLParm, @c_WaveKey, @c_StorerKey ,@c_SKU, @c_Packkey       
                                 , @c_Lottable01, @c_Lottable02, @c_Lottable03      
                                 , @c_Lottable06, @c_Lottable07, @c_Lottable08      
                                 , @c_Lottable09, @c_Lottable10, @c_Lottable11, @c_Lottable12,@n_CTNQty      
               ...
               BEGIN       
                   ...
                   BEGIN      
                     --SELECT @c_SQL      
                     SELECT @c_Orderkey '@c_Orderkey', @n_OrderLineQty 'ordqty',@n_CTNQty '@n_CTNQty'      
                   END      
                   ...
                   if @n_OrderLineQty > @n_CTNQty --OR @n_OrderLineQty > @n_CHANNELAvaiQty   --CS01c   --CS01e    
                      BEGIN    
                           
                       SET @n_OrderLineQty = @n_CTNQty     
                  ...
                  --if @b_debug='2'    
                  --BEGIN    
                   IF OBJECT_ID('tempdb..#TMPCHKORDQtyNIK4','u') IS NOT NULL    
                    DROP TABLE #TMPCHKORDQtyNIK4;    
    
                    SELECT distinct OD.orderkey    
                                --,@n_ttlordqty = SUM( OD.OpenQty - ( OD.QtyAllocated + OD.QtyPreAllocated + OD.QtyPicked ))      
                                   ,CASE WHEN (OD.OpenQty - ( OD.QtyAllocated + OD.QtyPreAllocated + OD.QtyPicked )) <  (CA.CHANNELAVIQTY) THEN    
                                   (OD.OpenQty - ( OD.QtyAllocated + OD.QtyPreAllocated + OD.QtyPicked )) ELSE (CA.CHANNELAVIQTY) END as ordqty    
                          INTO #TMPCHKORDQtyNIK4    
                          FROM ORDERDETAIL OD (NOLOCK)       
                          JOIN WAVEDETAIL  WD (NOLOCK) ON (OD.Orderkey = WD.Orderkey)      
                          JOIN #CHANNELINFO CA WITH (NOLOCK) ON CA.Orderkey = OD.Orderkey AND CA.Storerkey = OD.Storerkey     
                              AND CA.SKU = OD.SKU AND CA.Channel = OD.Channel    
                          WHERE OD.SKU = @c_sku     
                    ...
                    --END    
                          ...
                          SELECT @n_cntOrder = COUNT(1)      
                                --,@n_ttlordqty = SUM( OD.OpenQty - ( OD.QtyAllocated + OD.QtyPreAllocated + OD.QtyPicked ))      
                                --,@n_ttlordqty =CASE WHEN sum(OD.OpenQty - ( OD.QtyAllocated + OD.QtyPreAllocated + OD.QtyPicked )) <  sum(CA.CHANNELAVIQTY) THEN    
                                --               sum(OD.OpenQty - ( OD.QtyAllocated + OD.QtyPreAllocated + OD.QtyPicked )) ELSE sum(CA.CHANNELAVIQTY) END    
                                 ,@n_ttlordqty = sum(ordqty)    
                    FROM #TMPCHKORDQtyNIK4    
                    --      FROM ORDERDETAIL OD (NOLOCK)       
                    --      JOIN WAVEDETAIL  WD (NOLOCK) ON (OD.Orderkey = WD.Orderkey)      
                    --      JOIN #CHANNELINFO CA WITH (NOLOCK) ON CA.Orderkey = OD.Orderkey AND CA.Storerkey = OD.Storerkey     
                    --          AND CA.SKU = OD.SKU AND CA.Channel = OD.Channel    
                    ...
                    --AND EXISTS (SELECT 1 FROM #ORDERLINES OL (NOLOCK) WHERE OD.Orderkey = OL.Orderkey)    
                      ...
                      BEGIN      
                        --SELECT @c_SQL      
                        SELECT @c_Orderkey '@c_Orderkey',@c_channel 'channel', @n_cntOrder 'cntOrder'     
                       , @c_SKU '@c_SKU',@n_ttlordqty '@n_ttlordqty', @n_OrderLineQty '@n_OrderLineQty',@n_CTNQty '@n_CTNQty',@n_UCCQty '@n_UCCQty'   
                    , @n_CHANNELAvaiQty '@n_CHANNELAvaiQty'   
                      END      
      
                      --     IF @n_cntOrder = 1      
                      --     BEGIN      
                      --      IF @n_ttlordqty = @n_UCCQty    
                      --   BEGIN    
                   ...
                   BEGIN      
                       ...
                       SELECT top 1 @n_CHANNELAvaiQty = CHANNELAVIQTY      
                                    --,@n_Channel_id = Channel_ID      
                              FROM #CHANNELINFO      
                        ...
                        IF @b_debug='1'  --and  @c_SKU ='AA8854010XL'    
                        BEGIN    
                          SELECT 'B4 update channel',@c_channel '@c_channel',@c_sku '@c_sku',@n_CHANNELAvaiQty '@n_CHANNELAvaiQty'    
                        END    
                     ...
                     SET @c_PrevChannel = @c_channel      
                       ...
                       IF @b_Debug ='1'    --and  @c_SKU ='AA8854010XL'    
                       BEGIN      
                          SELECT @c_UCCNo '@c_UCCNo' , @n_PickQty '@n_PickQty' , @n_InsertQty '@n_InsertQty',@n_CHANNELAvaiQty '@n_CHANNELAvaiQty'    
                  ...
                  -- INSERT #PickDetail     
                if @n_InsertQty <= @n_CHANNELAvaiQty --AND @n_InsertQty = @n_UCCQty --CS01b start    
                BEGIN    
      
                   SET @c_UOM=(SELECT CASE WHEN @n_UCCQty=@n_InsertQty THEN 2 WHEN @n_UCCQty<>@n_InsertQty THEN 6 END ) --CS01 20191226  
                  ...
                  BEGIN      
                     ...
                     SET @n_Err = 61010      
                     SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0))         
                                   + ': Get PickDetailKey Failed. (ispPRNIK04)'      
                     GOTO QUIT_SP      
                  END      
                  ELSE      
                  BEGIN      
                     ...
                     BEGIN      
                        PRINT 'PickDetailKey: ' + @c_PickDetailKey + CHAR(13) +      
                              ...
                              '@n_CTNQty: ' + CAST(@n_CTNQty AS NVARCHAR) + CHAR(13)       
                     END      
                     ...
                     BEGIN      
                        ...
                        SET @n_Err = 61020      
                        SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0))       
                                       + ': Insert PickDetail Failed. (ispPRNIK04)'      
                        GOTO QUIT_SP      
                     END      
    
                      --CS01e    
                      UPDATE #CHANNELINFO      
                      SET CHANNELAVIQTY = CHANNELAVIQTY-@n_InsertQty       
                      WHERE Channel = @c_channel      
                      AND channel_id = @n_Channel_id      
                      AND SKU = @c_SKU    
   ...
   --   UPDATE  PICKDETAIL WITH (ROWLOCK) SET UOM=7 WHERE DROPID IN(  
   --      SELECT DROPID FROM PICKDETAIL(NOLOCK) WHERE WAVEKEY=@c_WaveKey AND SKU=@c_SKU AND UOM=6 GROUP BY DROPID,UOMQTY HAVING(SUM(QTY)<>UOMQTY)  
   --      ) AND STORERKEY=@c_StorerKey AND  WAVEKEY=@c_WaveKey  
   ...
   BEGIN      
      SELECT PD.PickDetailKey, PD.OrderKey, PD.OrderLineNumber, PD.Qty, PD.SKU, PD.PackKey, PD.Lot, PD.Loc, PD.ID, PD.UOM      
           , PD.UOMQty, PD.DropID, PD.PickMethod,PD.Channel_id      
...
--CS01 20191227 START  
         ...
         WHERE   STORERKEY=@c_StorerKey  
                 ...
                 AND UOM=6   
         GROUP BY DROPID,UOMQTY,SKU   
         HAVING(SUM(QTY)<>UOMQTY))  
         BEGIN  
  
  
                DECLARE CUR_UOM CURSOR FAST_FORWARD READ_ONLY FOR       
                SELECT DROPID,SKU FROM PICKDETAIL(NOLOCK)   
                WHERE WAVEKEY=@c_WaveKey  
                       AND UOM=6   
                 GROUP BY DROPID,UOMQTY,SKU   
                 HAVING(SUM(QTY)<>UOMQTY)  
                 ...
                 OPEN CUR_UOM                     
                 FETCH NEXT FROM CUR_UOM INTO  @c_Dropid ,@c_SKU   
                 ...
                 BEGIN  
                       ...
                       SET UOM=7   
                        ...
                        ,EditDate= GETDATE()         
                        ,TrafficCop = NULL   
                       WHERE STORERKEY=@c_StorerKey AND WAVEKEY=@c_WaveKey AND SKU=@c_SKU AND DROPID=@c_Dropid  
...
QUIT_SP:      
   ...
   BEGIN        
      ...
      BEGIN        
         ...
         END        
      END        
      EXECUTE nsp_logerror @n_Err, @c_ErrMsg, 'ispPRNIK04'        
