...
/************************************************************************/
/* Stored Procedure: isp_GetConsoPickTicketUSA_v2                       */
/* Creation Date: 28-Jul-2009                                           */
/* Copyright: IDS                                                       */
/* Written by: NJOW  (Modified from isp_GetConsoPickTicketUSA)          */
/*                                                                      */
/* Purpose:  SOS#141342 - Consolidated Pick Ticket By Loc for IDSUS.    */
/*                                                                      */
/* Called By:  PB - r_dw_consolidated_pick16_conso_v2                   */
/*                                                                      */
/* PVCS Version: 1.0                                                    */
/*                                                                      */
/* Version: 5.4                                                         */
...
/* 20-May-2010  GTGOH   1.5   Fix ConsoPT no size if exist in Carton and*/      
/*                            Loose - SOS#170946 (Goh01)                */
/* 28-Apr-2010  NJOW06  1.6   213129 - Add total in uom and sku fields  */
/* 26-Nov-2013  TLTING  1.7   Change user_name() to SUSER_SNAME()       */
/* 17-Dec-2013  NJOW07  1.8   Fix commit issue                          */
/************************************************************************/

CREATE PROC [dbo].[isp_GetConsoPickTicketUSA_v2] ( 
...
-- Start : June01
			  ...
			  ,@n_cnt int
           ,@n_trancount int
			  ,@c_cartonflag NVARCHAR(1)		--GOH01
			  ,@c_nonecartonflag NVARCHAR(1)	--GOH01
			  ,@c_cartonflag_all NVARCHAR(1)	--GOH01
			  ,@c_nonecartonflag_all NVARCHAR(1) --GOH01
			  ,@c_tempUOM NVARCHAR(10) --GOH01
        ,@c_id NVARCHAR(18)
        ,@n_total_sku int --NJOW06
        ,@n_total_inuom int --NJOW06
        ,@c_total_uom NVARCHAR(10) --NJOW06
        
   SELECT @n_total_sku = 0, @n_total_inuom = 0, @c_total_uom = '' --NJOW06
...
--						LOC.Putawayzone, -- June01
	...
	--NJOW06
	SELECT @n_total_sku = COUNT(DISTINCT SKU)
	FROM #TempConsoPickSlip
	...
	BEGIN
      ...
      SET @c_PrevColor = ''
      SET @c_PrevBusr8 = ''
 	   SET @c_cartonflag_all = 'N'		--GOH01
		SET @c_nonecartonflag_all = 'N'	--GOH01
...
--		ORDER BY Storerkey, Putawayzone, LogicalLoc, Loc, Style, Lottable03	
      ...
      #TempConsoPickSlip2.Lottable03, #TempConsoPickSlip2.Style, SKU.Color, SKU.BUSR8, #TempConsoPickSlip2.Id
		...
		BEGIN		
			...
			END 
					...
					BEGIN
							...
							BEGIN
								...
								BEGIN
									...
									INSERT INTO @t_ConsoPS (LoadPickSlipNO, Loadkey, Route, LoadPlanAddDate, LoadPrintedFlag, Storerkey,
                                                   ...
                                                   TotalAllocateQty, TotalOrderQty, LineText, LineFlag, UOM) --GOH01
                                  ...
                                  @n_SumAllocatedQty, @n_SumOriginalQty, 
											 ...
											 LEFT(dbo.fnc_RTrim(@c_DispQty) + REPLICATE('', 70), 70), 'B1', @c_tempuom) --GOH01
								END
						...
						BEGIN
							...
							VALUES (@c_LoadPickHeaderKey, @c_Loadkey, @c_Route, @d_LoadPlanAddDate, @c_LoadPrintedFlag, @c_Storerkey,
									 @c_Company, @c_PrevFacility,-- @c_PrevZone, 
                            @n_SumAllocatedQty, @n_SumOriginalQty, 
									 SPACE(15) + '^' + '  Size  ' + '^' + LEFT(dbo.fnc_RTrim(@c_DispSize) +  REPLICATE('', 70), 70), 'B1', @c_tempuom)	--GOH01
				...
				SET @n_TotalPPKQty = 0						
				SET @b_Repeat = '1'
			   SET @c_cartonflag = 'N'		--GOH01
			   SET @c_nonecartonflag = 'N' --GOH01
			   ...
			   Insert_t_Conso:
					...
					--GOH01 Start
						...
						BEGIN
							 ...
							 --NJOW06
							 SET @c_total_uom = @c_UOM
							 SET @n_total_inuom = @n_total_inuom + @n_UOMqty
						END
						ELSE
						BEGIN
                  ...
                  ORDER BY Style, Color, BUSR8, Measurement
                        ...
                        -- Modified By Vicky on 09-Oct-2007 (Start)
                               ...
                               CONVERT(CHAR(50), ISNULL(LEFT(dbo.fnc_RTrim(@c_Busr1),50), ''))  ,'', @c_tempuom) --GOH01
...
--                     RIGHT(SPACE(8)+dbo.fnc_RTrim(CONVERT(CHAR(8), @n_UOMQty)),8),'')
								...
								INSERT INTO @t_ConsoPS (LoadPickSlipNO, Loadkey, Route, LoadPlanAddDate, LoadPrintedFlag, Storerkey,
																Company, Facility, --Putawayzone, 
                                                TotalAllocateQty, TotalOrderQty, LineText, LineFlag, UOM, Id)	--GOH01
								VALUES (@c_LoadPickHeaderKey, @c_Loadkey, @c_Route, @d_LoadPlanAddDate, @c_LoadPrintedFlag, @c_Storerkey,
										 @c_Company, @c_Facility,-- @c_PutawayZone, 
                               @n_SumAllocatedQty, @n_SumOriginalQty, 
                     ...
                     SPACE(3) + RIGHT(SPACE(8)+dbo.fnc_RTrim(CONVERT(CHAR(8), @n_NoOfUnits)),8) + SPACE(13)+  -- 'PCs' + SPACE(20) + -- Vicky02										 
										 RIGHT(SPACE(8)+dbo.fnc_RTrim(CONVERT(CHAR(8), @n_EAQty)),8) + SPACE(3)+
                     RIGHT(SPACE(8)+dbo.fnc_RTrim(CONVERT(CHAR(8), @n_UOMQty)),8),'', @c_tempuom, @c_id)	--GOH01
                     ...
                     -- Modified By Vicky on 09-Oct-2007 (End)
                            ...
                            @n_SumAllocatedQty, @n_SumOriginalQty, 
									 ...
									 CONVERT(CHAR(25), dbo.fnc_RTrim(@c_Size) + ' ^ ' + dbo.fnc_RTrim(@c_Measurement)),'C1', @c_tempuom)  --GOH01
  ...
  END
	...
	UPDATE @t_ConsoPS SET UOM ='OTHER' WHERE UOM <> 'CARTON' OR UOM IS NULL      --GOH01
    ...
    , LineText, LineFlag, Id, suser_sname()
    , @n_total_sku, @n_total_inuom, @c_total_uom --NJOW06
	FROM  @t_ConsoPS
 	ORDER BY UOM, RowID	--GOH01
	-- End : June01
