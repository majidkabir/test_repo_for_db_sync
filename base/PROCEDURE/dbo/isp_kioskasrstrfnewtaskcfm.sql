...
/************************************************************************/
/* Stored Procedure: isp_KioskASRSTRFNewTaskCfm                         */
...
/*          : u_kiosk_asrstrf_new_c.cb_confirmpick.click event          */
/*                                                                      */
/* PVCS Version: 1.0                                                    */
/*                                                                      */
/* Version: 6.0                                                         */
...
/************************************************************************/
CREATE PROC [dbo].[isp_KioskASRSTRFNewTaskCfm] 
...
BEGIN
   ...
   BEGIN
      ...
      SET @c_errmsg='ID''s Loc not found. (isp_KioskASRSTRFNewTaskCfm)' 
      GOTO QUIT_SP
   END
   ...
   BEGIN  
      ...
      SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update TRANSFERDETAIL Failed. (isp_KioskASRSTRFNewTaskCfm)' 
      GOTO QUIT_SP
   END 
   ...
   BEGIN
      ...
      SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Execute ispFinalizeTransfer Failed. (isp_KioskASRSTRFNewTaskCfm)' 
                   + @c_errmsg
      GOTO QUIT_SP
   END
   ...
   BEGIN
      ...
      BEGIN  
         ...
         SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update TASKDETAIL Failed. (isp_KioskASRSTRFNewTaskCfm)' 
         GOTO QUIT_SP
      END 
...
QUIT_SP:
   ...
   BEGIN
      ...
      BEGIN
         ...
         END
      END
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'isp_KioskASRSTRFNewTaskCfm'
