...
/***************************************************************************/
/* Stored Procedure: isp_RPT_REPL_REPLEN_005                               */
/* Creation Date: 09-Dec-2021                                              */
/* Copyright: LFL                                                          */
/* Written by: Harshitha                                                   */
/*                                                                         */
/* Purpose: WMS- 18552 - Migrate WMS report to Logi Report.                */
/*                       Amend From isp_ReplenishmentRpt_PC20              */
/*                                                                         */
/* Called By: RPT_REPL_REPLEN_005                                          */
/*                                                                         */
/* GitLab Version: 1.0                                                     */
/*                                                                         */
/* Version: 1.0                                                            */
...
/***************************************************************************/
CREATE PROC [dbo].[isp_RPT_REPL_REPLEN_005]
...
BEGIN
   ...
   BEGIN
      ...
      OverAllocation = CASE WHEN SUM(LOTxLOCxID.QtyExpected) > 0
                              OR SKUxLOC.QtyLocationMinimum < (SKUxLOC.Qty - (SKUxLOC.QtyPicked + SKUxLOC.QtyAllocated)) THEN
                                 'Y'
                            ELSE 'N' END,
      ...
      IF @b_debug = 1
         SELECT '#TempSKUxLOC',* FROM #TempSKUxLOC
      ...
      BEGIN
         ...
         BEGIN
            ...
            DECLARE LOC_CUR CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
               ...
               LEFT JOIN CODELKUP CL (NOLOCK) ON LOC.Putawayzone = CL.Code AND LOTxLOCxID.Storerkey = CL.Storerkey AND CL.Listname = 'JTIREPLN'
               ...
               AND LOC.Facility = @c_Zone01
               AND LOC.HostWHCode = @c_HostWHCode
               GROUP BY LOC.LogicAllocation, LOTxLOCxID.LOC, PACK.Pallet, ISNULL(CL.Short,'ZZ')
               ORDER BY CASE WHEN PACK.Pallet > 0 THEN
                           CASE WHEN FLOOR(SUM(LOTxLOCxID.Qty - LOTxLOCxID.QtyPicked - LOTxLOCxID.QtyAllocated) / PACK.Pallet) >= 1 THEN
                                1 ELSE 0 END
                        ELSE 0 END,
                        ISNULL(CL.Short,'ZZ'),
                        SUM(LOTxLOCxID.Qty - LOTxLOCxID.QtyPicked - LOTxLOCxID.QtyAllocated),
                        LOC.LogicAllocation,
                        LOTxLOCxID.LOC
               ...
               BEGIN
                  ...
                  BEGIN
                     ...
                     BEGIN
                        ...
                        IF @c_OverAllocation <> 'Y' OR @n_QtyExpedted <= 0
                           SET @n_RemainingQty = 0
                     END
...
QUIT_SP:
   ...
   --IF ( @c_Zone02 = 'ALL')
   --BEGIN
   --   SELECT REPLENISHMENT.FromLoc, REPLENISHMENT.Id, REPLENISHMENT.ToLoc, REPLENISHMENT.Sku, REPLENISHMENT.Qty, REPLENISHMENT.StorerKey, REPLENISHMENT.Lot, REPLENISHMENT.PackKey
   --        , SKU.Descr, REPLENISHMENT.Priority, LOC.PutawayZone, PACK.CASECNT, PACK.PACKUOM1, PACK.PACKUOM3, REPLENISHMENT.ReplenishmentKey
   --        , LOTATTRIBUTE.Lottable04, LOTATTRIBUTE.Lottable02, LOTATTRIBUTE.Lottable03
   --   FROM  REPLENISHMENT (NOLOCK)
   --   JOIN  SKU (NOLOCK) ON (SKU.Sku = REPLENISHMENT.Sku AND  SKU.StorerKey = REPLENISHMENT.StorerKey)
   --   JOIN  LOC (NOLOCK) ON (LOC.Loc = REPLENISHMENT.FromLoc)
   --   JOIN  PACK (NOLOCK) ON (SKU.PackKey = PACK.PackKey)
   --   JOIN  LOTATTRIBUTE (NOLOCK) ON (REPLENISHMENT.Lot = LOTATTRIBUTE.Lot)
   --   WHERE LOC.Facility = @c_Zone01
   --   AND  REPLENISHMENT.Confirmed = 'N'
   --   AND  REPLENISHMENT.StorerKey = CASE WHEN @c_StorerKey = 'ALL' OR @c_StorerKey = '' THEN
   --                                 REPLENISHMENT.StorerKey ELSE @c_StorerKey END
   --   AND (REPLENISHMENT.Replenishmentgroup = @c_ReplGrp OR @c_ReplGrp = 'ALL')
   --   ORDER BY LOC.PutawayZone, REPLENISHMENT.Priority
   --END
   --ELSE
   --BEGIN
   --   SELECT REPLENISHMENT.FromLoc, REPLENISHMENT.Id, REPLENISHMENT.ToLoc, REPLENISHMENT.Sku, REPLENISHMENT.Qty, REPLENISHMENT.StorerKey, REPLENISHMENT.Lot, REPLENISHMENT.PackKey
   ...
   IF ISNULL(TRIM(@c_Sorting),'') IN ('', 'ORDER BY')
      SET @c_Sorting = 'Priority '

   SET @c_SQL = ' ;WITH CTE AS ('
              + ' SELECT REPLENISHMENT.FromLoc, REPLENISHMENT.Id, REPLENISHMENT.ToLoc, REPLENISHMENT.Sku, REPLENISHMENT.Qty, REPLENISHMENT.StorerKey, REPLENISHMENT.Lot, REPLENISHMENT.PackKey ' + CHAR(13)
              + '      , SKU.Descr, REPLENISHMENT.Priority, LOC.PutawayZone, PACK.CASECNT, PACK.PACKUOM1, PACK.PACKUOM3, REPLENISHMENT.ReplenishmentKey ' + CHAR(13)
              + '      , LOTATTRIBUTE.Lottable04, LOTATTRIBUTE.Lottable02, LOTATTRIBUTE.Lottable03 ' + CHAR(13)
              + ' FROM  REPLENISHMENT (NOLOCK) ' + CHAR(13)
              + ' JOIN  SKU (NOLOCK) ON (SKU.Sku = REPLENISHMENT.Sku AND  SKU.StorerKey = REPLENISHMENT.StorerKey) ' + CHAR(13)
              + ' JOIN  LOC (NOLOCK) ON (LOC.Loc = REPLENISHMENT.FromLoc) ' + CHAR(13)
              + ' JOIN  PACK (NOLOCK) ON (SKU.PackKey = PACK.PackKey) ' + CHAR(13)
              + ' JOIN  LOTATTRIBUTE (NOLOCK) ON (REPLENISHMENT.Lot = LOTATTRIBUTE.Lot) ' + CHAR(13)
              + ' WHERE LOC.Facility = @c_Zone01 ' + CHAR(13)
              + ' AND Confirmed = ''N'' ' + CHAR(13)
              + ' AND REPLENISHMENT.StorerKey = CASE WHEN @c_StorerKey = ''ALL'' OR @c_StorerKey = '''' THEN ' + CHAR(13)
              + '                               REPLENISHMENT.StorerKey ELSE @c_StorerKey END ' + CHAR(13)
              + ' AND (REPLENISHMENT.Replenishmentgroup = @c_ReplGrp OR @c_ReplGrp = ''ALL'') ' + CHAR(13)
              + CASE WHEN @c_Zone02 = 'ALL' THEN ' )' ELSE ' AND LOC.PutawayZone IN (@c_Zone02, @c_Zone03, @c_Zone04, @c_Zone05, @c_Zone06, @c_Zone07, @c_Zone08, @c_Zone09, @c_Zone10, @c_Zone11, @c_Zone12) )' END
              + ' SELECT FromLoc, Id, ToLoc, Sku, Qty, StorerKey, Lot, PackKey ' + CHAR(13)
              + '      , Descr, Priority, PutawayZone, CASECNT, PACKUOM1, PACKUOM3, ReplenishmentKey ' + CHAR(13)
              + '      , Lottable04, Lottable02, Lottable03 ' + CHAR(13)
              + ' FROM CTE ' + CHAR(13)
              + ' ORDER BY Putawayzone ASC, ' + @c_Sorting

   SET @c_ExecArguments = N'   @c_Zone01      NVARCHAR(10) '   
                        + N' , @c_Zone02      NVARCHAR(10) '
                        + N' , @c_Zone03      NVARCHAR(10) '
                        + N' , @c_Zone04      NVARCHAR(10) '
                        + N' , @c_Zone05      NVARCHAR(10) '
                        + N' , @c_Zone06      NVARCHAR(10) '
                        + N' , @c_Zone07      NVARCHAR(10) '
                        + N' , @c_Zone08      NVARCHAR(10) '
                        + N' , @c_Zone09      NVARCHAR(10) '
                        + N' , @c_Zone10      NVARCHAR(10) '
                        + N' , @c_Zone11      NVARCHAR(10) '
                        + N' , @c_Zone12      NVARCHAR(10) '
                        + N' , @c_StorerKey   NVARCHAR(15) '
                        + N' , @c_ReplGrp     NVARCHAR(30) '
