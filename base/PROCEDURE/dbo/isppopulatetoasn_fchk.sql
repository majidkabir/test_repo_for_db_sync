...
/**************************************************************************/
/* SP: ispPopulateTOASN_FCHK                                              */
/* Creation Date: 02-OCT-2015                                             */
/* Copyright: LF                                                          */
/* Written by:                                                            */
/*                                                                        */
/* Purpose: 354123-Auto Create ASN for FrieslandCampina Transfer Orders   */
/*                                                                        */
...
/* 06-May-2022  WLChooi 1.1  DevOps Combine Script                        */
/* 06-May-2022  WLChooi 1.1  WMS-19602 - Add SubReasonCode to ASN (WL01)  */
/**************************************************************************/

CREATE PROCEDURE [dbo].[ispPopulateTOASN_FCHK] 
...
BEGIN
   ...
   DECLARE  @c_NewReceiptKey      NVARCHAR(10),
            ...
            @c_SubReasonCode      NVARCHAR(10) = ''   --WL01
   ...
   SELECT @c_ID = '', @c_TOLOC = '', @c_FinalizeFlag = 'Y'
   ...
   BEGIN                
      ...
      --WL01 S
      SELECT @c_SubReasonCode = ISNULL(CL.Short,'')
      FROM CODELKUP CL (NOLOCK)
      WHERE CL.LISTNAME = 'DFTASNSRC'
      AND CL.Code = 'SRC01'
      AND CL.Storerkey = @c_Storerkey

      IF ISNULL(@c_SubReasonCode,'') = ''
         SET @c_SubReasonCode = ''
      --WL01 E
     ...
     */
     ...
     IF NOT EXISTS ( SELECT 1 FROM FACILITY WITH (NOLOCK) WHERE Facility = @c_ToFacility )
       ...
       SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Invalid To Facility: ' + RTRIM(ISNULL(@c_ToFacility,'')) +
                        ' (ispPopulateTOASN_FCHK)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
       END
     ...
     IF NOT EXISTS ( SELECT 1 FROM LOC WITH (NOLOCK) WHERE LOC = @c_ToLoc) 
      ...
      BEGIN             
         ...
         BEGIN
             ...
             BEGIN
               ...
               BEGIN
                  ...
                  VALUES (@c_NewReceiptKey, @c_ExternReceiptKey, @c_WarehouseReference, @c_ToStorerKey, @c_Type, @c_ToFacility, @c_DocType, 'N')
                 ...
                 BEGIN
                      ...
                      SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err)
                         SET @n_err = 63498   
                         SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert Error on Table Receipt (ispPopulateTOASN_FCHK)' + ' ( ' + 
               ...
               BEGIN
                  ...
                  SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err) 
                      SET @n_err = 63499   
                      SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Generate Receipt Key Failed! (ispPopulateTOASN_FCHK)' 
                                + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
               END
            END
         END    
         ...
         BEGIN
            ...
            SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err)
               SET @n_err = 63500   
               SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Storer Key is BLANK! (ispPopulateTOASN_FCHK)' + ' ( ' + 
                          ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
         END
      ...
      BEGIN         
         ...
         BEGIN
            ...
            /*
               ...
               SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err)
                  SET @n_err = 63501   
                  SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Invalid Sku: ' + RTRIM(@c_Sku) + 'Of Storer: '+ RTRIM(@c_ToStorerkey) + ' (ispPopulateTOASN_FCHK)' + ' ( ' + 
                         ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
               END
               */
                   ...
                   BEGIN
                      ...
                      INSERT INTO RECEIPTDETAIL (ReceiptKey,               ReceiptLineNumber,   ExternReceiptkey,    
                                  ...
                                  VALUES        (@c_NewReceiptKey,         @c_ReceiptLine,      @c_ExternReceiptkey,   
                                                 ...
                                                 @n_QtyExpected,           @n_QtyReceived,               
                                                 @c_UOM,                   @c_Packkey,          @c_Toloc,                                                    
                                                 @c_Lottable01,            @c_Lottable02,       @c_Lottable03New,       
                                                 @d_Lottable04,            @d_Lottable05,       @c_Lottable06,
                                                 @c_Lottable07,            @c_Lottable08,       @c_Lottable09,
                                                 @c_Lottable10,            @c_Lottable11,       @c_Lottable12,
                                                 @d_Lottable13,            @d_Lottable14,       @d_Lottable15,
                                                 @n_BeforeReceivedQty,     @c_FinalizeFlag,     @c_ID,
                                                 @c_SubReasonCode)   --WL01
      ...
      BEGIN
         ...
         BEGIN
            ...
            SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err)
            SET @n_err = 63492   
            SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Error on Table Receipt (ispPopulateTOASN_FCHK)' + ' ( ' + 
                          ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
         END              
      END
...
QUIT_SP:
      ...
      BEGIN
         ...
         --BEGIN
            --ROLLBACK TRAN
         --END
         EXECUTE nsp_logerror @n_err, @c_errmsg, 'ispPopulateTOASN_FCHK'
