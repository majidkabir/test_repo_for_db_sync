...
GO
/************************************************************************/
/* Store Procedure:  isp_GetPickSlipOrders94                            */
/* Creation Date: 2019-08-08                                            */
/* Copyright: LFL                                                       */
/* Written by:                                                          */
/*                                                                      */
/* Purpose:  Pickslip for KFMY                                          */
/*           Copied from nsp_GetPickSlipOrder05                         */
/*                                                                      */
...
/* Return Status:  None                                                 */
/*                                                                      */
/* Usage:  Used for report dw = r_dw_print_pickorder94                  */
/*                                                                      */
/* Local Variables:                                                     */
/*                                                                      */
/* Called By:                                                           */
...
/*25/11/2019   WLChooi   1.1   WMS-11221 - Page break by LocationType   */
/*                             (WL01)                                   */
/*06/02/2020   WLChooi   1.2   WMS-12002 - Add VAS info (WL02)          */
/************************************************************************/

CREATE PROC [dbo].[isp_GetPickSlipOrders94] (@c_loadkey NVARCHAR(10)) 
 AS
 BEGIN
   SET NOCOUNT ON 
   SET ANSI_NULLS OFF
    ...
    CREATE TABLE #TEMP_PICK
       ...
       INSERT INTO #TEMP_PICK
            (PickSlipNo,          LoadKey,         OrderKey,         ConsigneeKey,
             ...
             LocationType,        VAS,             ShowVAS)  --WL01   --WL02
        SELECT
      ...
      LEFT OUTER JOIN Codelkup CLR (NOLOCK) ON (ORDERS.Storerkey = CLR.Storerkey AND CLR.Code = 'SHOWBARCODE'
                                       AND CLR.Listname = 'REPORTCFG' AND CLR.Long = 'r_dw_print_pickorder94' AND ISNULL(CLR.Short,'') <> 'N')
      LEFT OUTER JOIN Codelkup CLR1 (NOLOCK) ON (ORDERS.Storerkey = CLR1.Storerkey AND CLR1.Code = 'SHOWCONTACTPHONE'
                                       AND CLR1.Listname = 'REPORTCFG' AND CLR1.Long = 'r_dw_print_pickorder94' AND ISNULL(CLR1.Short,'') <> 'N')
      LEFT OUTER JOIN Codelkup CLR2 (NOLOCK) ON (ORDERS.Storerkey = CLR2.Storerkey AND CLR2.Code = 'ShowVAS'
                                       AND CLR2.Listname = 'REPORTCFG' AND CLR2.Long = 'r_dw_print_pickorder94' AND ISNULL(CLR2.Short,'') <> 'N')
