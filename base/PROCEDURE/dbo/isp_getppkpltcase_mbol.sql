...
/************************************************************************/
/* Stored Procedure: isp_GetPPKPltCase_MBOL                             */
/* Creation Date:                                                       */
/* Copyright: IDS                                                       */
/* Written by: NJOW                                                     */
/*                                                                      */
/* Purpose:  229328-MBOL Assigns Lane                                   */
/*                                                                      */
/* Called By:                                                           */ 
/*                                                                      */
/* Parameters: (Input)  Mbolkey, externorderkey, consigneekey           */
/*                                                                      */
/* PVCS Version: 1.0	                                                  */
/*                                                                      */
/* Version: 5.4                                                         */
...
/************************************************************************/
CREATE PROCEDURE [dbo].[isp_GetPPKPltCase_MBOL]
    ...
    BEGIN
        ...
        BEGIN
            ...
            SELECT PD.Storerkey
                  ...
                  ,PD.Status
                  ,PD.Id
            FROM   MBOLDETAIL LD(NOLOCK)
                   JOIN PICKDETAIL PD(NOLOCK)
                        ON  (LD.Orderkey=PD.Orderkey)
                   JOIN LOTATTRIBUTE LA(NOLOCK)
                        ON  (PD.Lot=LA.Lot)
                   JOIN ORDERS O (NOLOCK) 
                        ON  (LD.Orderkey=O.Orderkey)
            WHERE  LD.Mbolkey = @c_MbolKey AND
                   (
                       O.Externorderkey=@c_externorderkey OR
                       ISNULL(@c_externorderkey ,'')=''
                   ) AND
                   (
                       O.Consigneekey=@c_consigneekey OR
