...
GO
/************************************************************************/    
/* Stored Procedure: nspALPVH1H                                         */    
/* Creation Date: 28-SEP-2017                                           */    
/* Copyright: LFL                                                       */    
/* Written by:                                                          */    
/*                                                                      */    
/* Purpose: WMS-4293 HK PVH Allocate from bulk                          */
...
BEGIN    
   ...
   IF @c_UOM = 2   
      SET @c_SortBy = ' ORDER BY ' + RTRIM(@c_SortBy) + ', LOTXLOCXID.Qty DESC'
   ELSE
      SET @c_SortBy = ' ORDER BY ' + RTRIM(@c_SortBy) 
   ...
   SET @c_SQL = N'   
      ...
      CASE WHEN @c_UOM = '2' THEN ' AND (LOTxLOCxID.QTY > 0 AND (LOTxLOCxID.QTYALLOCATED + LOTxLOCxID.QTYPICKED + LOTxLOCxID.QtyReplen) = 0 AND @n_QtyLeftToFulfill >= LOTxLOCxID.QTY) ' ELSE ' AND (LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED - LOTxLOCxID.QtyReplen) >= @n_UOMBase ' END  +  --NJOW01
      ISNULL(@c_Cond,'') + ' ' +
      ...
      --' ORDER BY LOTATTRIBUTE.Lottable05, LOTATTRIBUTE.Lot, LOC.LogicalLocation, LOC.LOC '
   ...
   BEGIN    
      ...
      BEGIN
     		 ...
     		 SET @c_OtherValue = 'UOM=' + LTRIM(CAST(@n_QtyAvailable AS NVARCHAR))
      END
      ELSE
      BEGIN             	                  
