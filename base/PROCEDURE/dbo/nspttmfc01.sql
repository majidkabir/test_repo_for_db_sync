...
BEGIN
   ...
   SAVE TRAN nspTTMFC01 -- For rollback or commit only our own transaction
   ...
   BEGIN
      ...
      EXECUTE nspCheckSkipTasks
         ...
         , @n_Err          OUTPUT
         , @c_ErrMsg       OUTPUT
        print concat(@c_TaskDetailKey,',',@b_success,',',@b_SkipTheTask)
      IF @b_success <> 1
         GOTO Fail
      IF @b_SkipTheTask = 1
      BEGIN
   ...
   COMMIT TRAN nspTTMFC01 -- Only commit change made here
   GOTO Quit

RollBackTran:
   ROLLBACK TRAN nspTTMFC01 -- Only rollback change made here
Fail:
Quit:
