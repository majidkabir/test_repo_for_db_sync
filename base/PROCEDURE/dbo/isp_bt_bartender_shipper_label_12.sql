...
/*27/06/2019  1.0  WLChooi    WMS-9421 (Created)                              */     
/*26/09/2019  1.1  WLChooi    WMS-10763 - Fix issue with Codelkup (WL01)      */         
/******************************************************************************/                
                  
CREATE PROC [dbo].[isp_BT_Bartender_Shipper_Label_12]                       
...
BEGIN                      
  ...
  DECLARE @d_Trace_StartTime     DATETIME,   
           ...
           @c_condition1         NVARCHAR(150),    --(CS01)
           @c_condition2         NVARCHAR(150),     --(CS01)     
           @n_SumStdGrossWgtxQty FLOAT,
           @c_SUSR3UDF01         NVARCHAR(80)     --WL01
   ...
   --CS01 END
     ...
     SET @c_SQLJOIN = +' SELECT DISTINCT ORD.loadkey,ORD.orderkey,ORD.externorderkey,ORD.type,buyerpo,salesman,ORD.facility,STO.Secondary,'    --8        
                ...
                + CHAR(13) +          
                +'STO.City,STO.Zip,STO.Contact1,STO.Phone1,STO.phone2,ORD.Consigneekey,ORD.c_Company,ISNULL(ORD.c_Address1,'''')+ '' '' + '+ CHAR(13) +     
                +'ISNULL(ORD.c_Address2,'''')+ '' '' + ISNULL(ORD.c_Address3,'''') + '' '' + ISNULL(ORD.c_Address4,''''), '+ CHAR(13) + 
                +' '''', '--25 --WL01
                +'ISNULL(CL2.UDF01,''''), ISNULL(CL3.UDF01,''''),ORD.C_State,ORD.C_City,ORD.C_Zip,ORD.C_Contact1,ORD.C_Phone1,'          
                +'ISNULL(ORD.C_Phone2,''''),CASE WHEN STO.VAT=''LEV'' THEN ORD.Notes2 ELSE ORD.M_Company END,ORD.Userdefine01,ORD.Userdefine02,'         
                +'CASE WHEN STO.VAT=''ITX'' THEN ORD.door ELSE  ORD.Userdefine03 END,PD.labelno,ORD.Userdefine05,' --ORD.PmtTerm,'      
                + CHAR(13) +   
                ...
                +' '''',ORD.M_Address1,ORD.M_Address2,ORD.M_City,'''','''',ORD.Priority,ORD.Userdefine10,PD.CartonNo,'''' '        
                + CHAR(13) +            
                ...
                + ' INNER JOIN SKU S WITH (NOLOCK) ON S.SKU = PD.SKU AND S.STORERKEY = PH.STORERKEY '   
                + ' LEFT JOIN CODELKUP CL1 WITH (NOLOCK) ON CL1.LISTNAME = ''SKUGROUP'' AND CL1.CODE = S.SUSR3 AND CL1.STORERKEY = ORD.STORERKEY' + CHAR(13) +
                + ' LEFT JOIN CODELKUP CL2 WITH (NOLOCK) ON CL2.LISTNAME = ''SFUA'' AND CL2.CODE = SUBSTRING(LTRIM(RTRIM(ISNULL(ORD.Stop,''''))),1,1) AND CL2.STORERKEY = ORD.STORERKEY'  + CHAR(13) +
                + ' LEFT JOIN CODELKUP CL3 WITH (NOLOCK) ON CL3.LISTNAME = ''SFUA'' AND CL3.CODE = ''S_Country'' AND CL3.STORERKEY = ORD.STORERKEY' + CHAR(13) +
  ...
  SET @c_SQL='INSERT INTO #Result (Col01,Col02,Col03,Col04,Col05, Col06,Col07,Col08,Col09'  + CHAR(13) +           
   ...
   SET @c_SQL = @c_SQL + @c_SQLJOIN  + @c_condition +  CHAR(13) + @c_condition1  + CHAR(13) + @c_condition2    --CS01     
   ...
   --Update SUM(Stdgrosswgt*OriginalQty) to Col60
   SELECT @n_SumStdGrossWgtxQty = SUM(S.STDGROSSWGT*OD.ORIGINALQTY) FROM ORDERDETAIL OD (NOLOCK)
   JOIN ORDERS ORD (NOLOCK) ON ORD.ORDERKEY = OD.ORDERKEY
   JOIN SKU S (NOLOCK) ON OD.SKU = S.SKU AND OD.STORERKEY = S.STORERKEY
   WHERE ORD.LOADKEY = @c_Sparm1

   UPDATE #Result
   SET COL60 = @n_SumStdGrossWgtxQty
   WHERE COL01 = @c_Sparm1
...
BEGIN       
  ...
  END
   ...
   --WL01 Start
   SELECT @c_SUSR3UDF01 = CONVERT(NVARCHAR(80), STUFF((SELECT DISTINCT ' ' + RTRIM(ISNULL(CL1.UDF01,'''')) 
                          ...
                          JOIN CODELKUP CL1 (NOLOCK) ON CL1.LISTNAME = 'SKUGROUP' 
                                                    AND CL1.CODE = SKU.SUSR3 
                                                    AND CL1.STORERKEY = ORD.STORERKEY
                          WHERE ORD.ORDERKEY = @c_OrderKey
                          FOR XML PATH('')),1,1,'' ) )  
   --WL01 End
   ...
   SET    Col50     = @c_CLong,  
          ...
          Col14     = @c_UDF01,   
          Col55     = @c_Col55, 
          Col02     = 'LFUA' + @c_OrderKey,
          Col25     = @c_SUSR3UDF01         --WL01
   WHERE  Col02     = @c_OrderKey        
...
EXIT_SP:    
   ...
   EXEC isp_InsertTraceInfo   
      ...
      @c_TraceName = 'isp_BT_Bartender_Shipper_Label_12',  
