...
/* 22-Feb-2021  Wan   1.1   Created.                                       */
/***************************************************************************/  
CREATE PROC [dbo].[ispPOTFR02]  
...
BEGIN  
   ...
   BEGIN
      ...
      BEGIN  
         SET @n_Continue = 3 
         SET @n_err=82005  
         SET @c_ErrMsg='NSQL'+CONVERT(VARCHAR(5),@n_err)+': Getting KitKey Fail. (ispPOTFR02)' 
         GOTO QUIT_SP  
      END  
      ...
      BEGIN
         SET @n_Continue = 3
         SET @n_err=82010   
         SET @c_ErrMsg='NSQL'+CONVERT(VARCHAR(5),@n_err)+': Insert Into KIT Fail. (ispPOTFR02)'
         GOTO QUIT_SP
      END
   END
   ...
   BEGIN
      ...
      SET @c_KitLoc = ''
      SELECT TOP 1 @c_KitLoc = ISNULL(td.LogicalToLoc,'')
      FROM dbo.TaskDetail AS td WITH (NOLOCK)
      WHERE td.TaskType = 'RPF'
      AND td.SourceType = 'ispTransferAllocation02'
      AND td.SourceKey = @c_Transferkey + @c_TransferLineNumber
      ...
      BEGIN
         SET @n_Continue = 3
         SET @n_err=82015  
         SET @c_ErrMsg='NSQL'+CONVERT(VARCHAR(5),@n_err)+': Insert Into KITDETAIL Fail. (ispPOTFR02)'
         GOTO QUIT_SP
      END
   ...
   BEGIN
      ...
      BEGIN
         ...
         END
      END
      Execute nsp_logerror @n_err, @c_errmsg, 'ispPOTFR02'
