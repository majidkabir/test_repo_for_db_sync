...
/* 28-Apr-2023 WLChooi  1.0   DevOps Combine Script                     */
/************************************************************************/
CREATE   PROC [dbo].[ispManningsLQ]
...
BEGIN
   ...
   IF @c_WaveType NOT IN ('MANB2B','MANB2C','MANLQ')
   BEGIN
      SET @n_Err = 82010
      SET @n_Continue = 3
      SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0))  
                    + ': Invalid Wave Piece Pick Task Dispatch Method'
                    + '. Must Be MANB2B or MANB2C or MANLQ (ispManningsLQ)'
      GOTO QUIT_SP
   END

   --This Main Pickcode only proceed MANLQ
   IF @c_WaveType NOT IN ('MANLQ')
   BEGIN
      GOTO QUIT_SP
   END
   ...
   BEGIN
      ...
      SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0))  
                    ...
                    + '. Allocation is not allowed (ispManningsLQ)'
      GOTO QUIT_SP
   END
   ...
   BEGIN
      ...
      WHERE WH.Wavekey = @c_WaveKey
        ...
        --AND OH.ECOM_SINGLE_Flag IN ('S','M')
        --AND OH.DocType = 'E'  
   ...
   BEGIN
      IF @b_debug IN ( 1, 9 )
      BEGIN
         PRINT 'Main PICKCOde: ispManningsLQ'+ CHAR(13) +
               'SKU: ' + @c_SKU + CHAR(13) +
   ...
   SET @c_SQLParms  = N' @c_Facility           NVARCHAR(5)'  
    ...
    EXEC sp_ExecuteSQL  @c_SQL
      ...
      BEGIN
         ...
         BEGIN
            PRINT '-----------------------------------'+ CHAR(13) +
                  'Main PICKCOde: ispManningsLQ'+ CHAR(13) +
                  'SKU: ' + @c_SKU + CHAR(13) +
                  '@n_QtyLeftToFullFill: ' + CAST(@n_QtyLeftToFullFill AS VARCHAR) + CHAR(13) +
                  '@dt_Lottable13: ' + CONVERT(NVARCHAR(25), @dt_Lottable13, 121) + CHAR(13)
         END 

         EXEC [dbo].[ispManningsLQConsoLQ7]   --DPP
           @c_WaveKey          = @c_WaveKey 
         ...
         BEGIN
            ...
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5), @n_Err) + ': Error Executing ispManningsLQConsoLQ7.'
                          + '(ispManningsLQ)' + ' (' + @c_ErrMsg + ')'
            GOTO QUIT_SP
         END
         ...
         EXEC [dbo].[ispManningsLQConsoLC7]   --Bulk
           @c_WaveKey          = @c_WaveKey 
         , @c_WaveType         = @c_WaveType                      
         , @c_Facility         = @c_Facility         
         , @c_Storerkey        = @c_Storerkey        
         ...
         BEGIN
            ...
            SET @n_Err = 82095
            SET @c_ErrMsg = ISNULL(ERROR_MESSAGE(),'')
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5), @n_Err) + ': Error Executing ispManningsLQConsoLC7.'
                          + '(ispManningsLQ)' + ' (' + @c_ErrMsg + ')'
            GOTO QUIT_SP
         END
...
QUIT_SP:
   ...
   BEGIN
      ...
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'ispManningsLQ'
