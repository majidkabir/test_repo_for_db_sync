...
GO
/************************************************************************/      
/* Store Procedure: ISP_RPT_WV_WAVPICKSUM_007                           */      
/* Creation Date: 14-MAY-2023                                           */      
/* Copyright:                                                           */      
/* Written by: CSCHONG                                                  */      
/*                                                                      */      
/* Purpose: WMS-22357 SG-AESOP-B2C Picking Slip Summary                 */      
/*                                                                      */      
/* Called By: RPT_WV_WAVPICKSUM_007                                     */      
/*                                                                      */      
/* PVCS Version: 1.1                                                    */      
/*                                                                      */      
/* Version: 5.4                                                         */      
...
/*  14-MAY-2022  CHONGCS   1.0   Devops Scripts Combine                 */  
/************************************************************************/      

CREATE   PROC [dbo].[ISP_RPT_WV_WAVPICKSUM_007]
     (  
...
BEGIN
   ...
   DECLARE @c_PickHeaderkey      NVARCHAR(10)   
         ...
         , @c_DataWindow         NVARCHAR(60) = 'RPT_WV_WAVPICKSUM_007'
 ...
 CREATE TABLE #TEMP_WAVPICKSUM007 
  ...
  EcomSingleFlag   NVARCHAR(1)
   ...
   IF EXISTS (SELECT 1
              ...
              WHERE WD.WaveKey = @c_Wavekey
              AND OH.DocType = 'E'  AND OH.STATUS >2  AND OH.ECOM_SINGLE_Flag<>'M')
   BEGIN
      GOTO QUIT
   END 
      
   INSERT INTO #TEMP_WAVPICKSUM007
   (
       OrderKey,
       WaveKey,
       Pickheaderkey,
  ...
  SELECT DISTINCT orders.OrderKey,WD.WaveKey,
                   RefKeyLookup.PickSlipNo,        ORDERS.Storerkey, PICKDETAIL.caseid,'',ORDERS.ECOM_SINGLE_Flag
   FROM WAVEDETAIL      WD  WITH (NOLOCK) 
   ...
   SELECT DISTINCT   
           orderkey 
         , Pickheaderkey
   FROM #TEMP_WAVPICKSUM007  
   WHERE ISNULL(Pickheaderkey,'') = ''
   ORDER BY Orderkey      
   ...
   BEGIN              
     ...
     END
      ...
      BEGIN               
         ...
         BEGIN                 
            ...
            BEGIN      
               ...
               SET @n_err = 81008         
               SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert PICKHEADER Failed (ISP_RPT_WV_WAVPICKSUM_007)'   
         ...
         BEGIN  
            SELECT @n_continue = 3  
            SELECT @n_err = 63502
            SELECT @c_errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Get PSNO Failed. (ISP_RPT_WV_WAVPICKSUM_007)'  
            BREAK   
         END            
      END            
         ...
         BEGIN  
            ...
            BEGIN   
               ...
               BEGIN  
                  SELECT @n_continue = 3
                  SELECT @n_err = 63503
                   SELECT @c_errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert RefKeyLookup Failed. (ISP_RPT_WV_WAVPICKSUM_007)'    
                  GOTO QUIT
               END                          
            END   


      UPDATE #TEMP_WAVPICKSUM007  
      SET Pickheaderkey = @c_RPickSlipNo  
      WHERE OrderKey = @c_OrderKey  
      AND   ISNULL(Pickheaderkey,'') = '' 
      ...
      BEGIN  
         SELECT @n_continue = 3  
         SELECT @n_err = 63504
         SELECT @c_errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update #TEMP_WAVPICKSUM007 Failed. (ISP_RPT_WV_WAVPICKSUM_007)'    
         GOTO QUIT
      END
      ...
      BEGIN      
         ...
         SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err)    
         SET @n_err = 81009       
         SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': UPDATE Pickdetail Failed (ISP_RPT_WV_WAVPICKSUM_007)'   
                      + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '   
         GOTO QUIT     
      END  
...
QUIT:  
   ...
   BEGIN   
      ...
      BEGIN    
         ROLLBACK TRAN    
      END   
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'ISP_RPT_WV_WAVPICKSUM_007'    
   END  
   ...
   BEGIN
      ...
      ,  @c_RetVal     = @c_RetVal           OUTPUT
          ...
          SELECT DISTINCT tp.WaveKey AS wavekey, CASE WHEN tp.EcomSingleFlag='S' THEN tp.Pickheaderkey ELSE '' END AS Pickheaderkey,  
                          tp.Storerkey, tp.CaseID, CASE WHEN tp.EcomSingleFlag='S' THEN 'SINGLE' ELSE 'MULTI' END AS ORDTYPE,
          CASE WHEN tp.EcomSingleFlag='M' THEN tp.WaveKey+tp.CaseID ELSE '' END AS wavecaseid,
          (SELECT COUNT(tp1.OrderKey) FROM #TEMP_WAVPICKSUM007 tp1 WHERE tp1.WaveKey=tp.WaveKey AND tp1.CaseID=tp.CaseID) CtnOrd
          FROM #TEMP_WAVPICKSUM007 AS tp  
          group by tp.WaveKey,tp.Storerkey, tp.EcomSingleFlag,tp.CaseID,tp.Pickheaderkey
          ORDER BY tp.WaveKey,tp.CaseID
       
   END
     
   IF OBJECT_ID('tempdb..#TEMP_WAVPICKSUM007') IS NOT NULL
      DROP TABLE   #TEMP_WAVPICKSUM007
