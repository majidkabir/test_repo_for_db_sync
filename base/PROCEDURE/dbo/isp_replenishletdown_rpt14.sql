...
/************************************************************************/  
/* Store Procedure: isp_ReplenishLetdown_rpt14                          */  
/* Creation Date: 12-Apr-2021                                           */  
/* Copyright: IDS                                                       */  
/* Written by: CSCHONG                                                  */  
/*                                                                      */  
/* Purpose: WMS-16743 - [CN] Converse Letdown View Report CR            */    
/*                     (Duplicate from isp_ReplenishLetdown_rpt04)      */  
/*                                                                      */  
/* Input Parameters: Facility, loadkeystart, loadkeyend                 */  
/*                                                                      */  
...
/* Local Variables:                                                     */  
/*                                                                      */  
/* Called By: r_dw_replenishletdown_rpt14                               */  
/*                                                                      */  
/* PVCS Version: 1.0                                                    */  
/*                                                                      */  
/* Version: 5.4                                                         */  
...
CREATE PROC [dbo].[isp_ReplenishLetdown_rpt14] (@c_facility     NVARCHAR(5)  
...
BEGIN  
   ...
   SELECT ISNULL(MIN(RTRIM(TSL.Storerkey)),'') AS Storerkey   
         ...
         ,@c_loadkeystart                 AS loadkeystart  
         ,@c_loadkeyend                   AS loadkeyend  
   ...
   LEFT OUTER JOIN #temprepl TRP ON (TRP.Storerkey = TSL.Storerkey)  
                                 ...
                                 AND(TRP.ID        = TSL.ID)  
                                 AND(TRP.Lot       = TSL.Lot)  
   LEFT JOIN LOTATTRIBUTE(NOLOCK) AS LA ON TSL.LOT=LA.LOT        -----Add by Hu Yuan on 2021-04-21  
   ...
   WHERE  CaseBalRtnToRackInEA<=QtyInEA    -----Add by Hu Yuan on 2021-04-21  
   GROUP BY #RESULT2.Storerkey  
          , #RESULT2.Sku  
          , #RESULT2.Lottable02  
          , #RESULT2.Loc  
