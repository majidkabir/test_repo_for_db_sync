...
GO
/*************************************************************************/  
/* Stored Procedure: ispRVWAV09                                          */  
/* Creation Date: 13-Apr-2017                                            */  
/* Copyright: LFL                                                        */  
/* Written by:                                                           */  
/*                                                                       */  
/* Purpose: WMS-1578 - CN Victoria Secret Ecom - Reverse Wave Released   */
/*                     Replenishment Tasks                               */            
/*                                                                       */  
/* Called By: wave                                                       */  
/*                                                                       */  
 ...
 BEGIN  
    ...
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM TASKDETAIL TD (NOLOCK) 
                       WHERE TD.Wavekey = @c_Wavekey AND TD.SourceType = 'ispRLWAV09'
                       AND TD.TaskType = 'RPF')
        BEGIN                                          
          SELECT @n_continue = 3  
          SELECT @n_err = 81010  
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': This Wave has not been released. (ispRVWAV09)'         
        END                 
    END
    ...
    BEGIN
        ...
        BEGIN
          SELECT @n_continue = 3  
          SELECT @n_err = 81020  
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Some Tasks have been started. Not allow to Reverse Wave Released (ispRVWAV09)'       
        END                 
    END
    ...
    BEGIN
         ...
         AND TASKDETAIL.Sourcetype = 'ispRLWAV09'
    ...
    */
    ...
    BEGIN
         ...
         BEGIN
           ...
           SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Pickdetail Table Failed. (ispRVWAV09)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
         END          
    END        
    ...
    BEGIN  
       ...
       BEGIN  
          ...
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update on wave Failed (ispRVWAV09)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
       END  
    END  
...
RETURN_SP:
    ...
    BEGIN  
       ...
       BEGIN  
          ...
          END  
       END  
       execute nsp_logerror @n_err, @c_errmsg, "ispRVWAV09"  
