...
BEGIN
   ...
   BEGIN 
      ...
      IF @c_Lottable03Label = 'EXPDATEYYYY-MM' AND @c_Lottable04Label = 'EXP_DATE'
      BEGIN
         SELECT @n_continue = 1
      END
      ELSE 
...
--         SET @b_Success = 0
         ...
         IF @c_Lottable03Label <> 'EXPDATEYYYY-MM'
         BEGIN
            SET @n_Err = 61326
            SET @c_Errmsg = CONVERT ( NVARCHAR(5), @n_Err) + ' Invalid Lottable03Label Setup.  (ispGenLot4ByLot3ByJulianDate)'
         END           
         ELSE IF @c_Lottable04Label <> 'EXP_DATE'
         BEGIN
            SET @n_Err = 61327
            SET @c_Errmsg = CONVERT ( NVARCHAR(5), @n_Err) + ' Invalid Lottable04Label Setup.  (ispGenLot4ByLot3ByJulianDate)'
         END       
         GOTO QUIT
      END         
   END
   ...
   BEGIN 
      -- Eg. yyyy-mm
      SELECT @c_Year = LEFT(@c_Lottable03Value, 4) -- yyyy
      SELECT @c_Month = SUBSTRING(@c_Lottable03Value, 6, 2) -- mm

      IF ISNUMERIC(@c_Year) = 0
      BEGIN
--         SET @b_Success = 0
         SET @n_Err = 61328
         SET @c_Errmsg = CONVERT ( NVARCHAR(5), @n_Err) + ' Year Not Numeric. (ispGenLot4ByLot3ByJulianDate)'    
         GOTO QUIT
      END
...
--         SET @b_Success = 0
         SET @n_Err = 61328
         SET @c_Errmsg = CONVERT ( NVARCHAR(5), @n_Err) + ' Month Not Numeric. (ispGenLot4ByLot3ByJulianDate)'    
         GOTO QUIT
      END
...
--         SET @b_Success = 0
         SET @n_Err = 61330
         SET @c_Errmsg = CONVERT ( NVARCHAR(5), @n_Err) + ' Year Less Than or Equal to Zero.  (ispGenLot4ByLot3ByJulianDate)'  
         GOTO QUIT
      END
...
--         SET @b_Success = 0
         SET @n_Err = 61330
         SET @c_Errmsg = CONVERT ( NVARCHAR(5), @n_Err) + ' Month Less Than or Equal to Zero.  (ispGenLot4ByLot3ByJulianDate)'  
         GOTO QUIT
      END
   
      -- Get Lottable04 
         SET @dt_Lottable04 = RIGHT(dbo.fnc_RTrim('0' + CONVERT(CHAR, @n_Year)), 4) + RIGHT(dbo.fnc_RTrim('0' + CONVERT(CHAR, @n_Month)), 2) + '01'
      IF @b_debug = 1
      BEGIN
