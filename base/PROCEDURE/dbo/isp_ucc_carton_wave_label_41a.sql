...
/*****************************************************************************/
/* Store Procedure:  isp_UCC_Carton_wave_Label_41a                          */
...
/* Output Parameters:                                                       */
/*                                                                          */
/* Usage:WMS-10700 CARTERSZ - Change sorting sequence content label         */
/*       duplicate from isp_UCC_Carton_wave_Label_41                        */
/*                                                                          */
/* Called By:  r_dw_ucc_carton_wave_label_41a_wave                          */
/*                                                                          */
/* PVCS Version: 1.1                                                        */
/*                                                                          */
/* Version: 5.4                                                             */
...
CREATE PROC [dbo].[isp_UCC_Carton_wave_Label_41a] (
...
BEGIN
 ...
 DECLARE @c_Getprinter     NVARCHAR(10),
  ...
  INSERT INTO #TMP_WAVEPICK (Storerkey,Orderkey,Loadkey,wavekey,Pickslipno,Labelno,SKU,qty,CartonNo,UpdateSource,rowno
                             ,consigneekey,MarkForKey,ExternOrderkey,CartonType)


  SELECT DISTINCT ORD.Storerkey,ORD.ORDERKEY,ORD.LOADKEY,WVDET.WAVEKEY,PIDET.PICKSLIPNO
 ,PADET.Labelno,S.Style,PADET.qty,PADET.CartonNo,ORD.UpdateSource,0, ORD.ConsigneeKey, ORD.MarkforKey
 ,ORD.ExternOrderKey,PIF.CartonType
  ...
  JOIN PACKINFO PIF WITH (NOLOCK) ON PIF.Pickslipno = PADET.Pickslipno AND PIF.CartonNo = PADET.CartonNo
 ...
 --,PADET.Labelno,PADET.SKU,PADET.CartonNo,ORD.UpdateSource
  ORDER BY ORD.ConsigneeKey, ORD.MarkforKey,ORD.ExternOrderKey,PIF.CartonType
 ...
 DECLARE CUR_StartRecLoop CURSOR LOCAL FAST_FORWARD READ_ONLY FOR 
   ...
   SELECT DISTINCT p.storerkey, P.pickslipno,t.labelno ,p.cartonno,p.updatesource,label_content
                  ,P.consigneekey,P.MarkForKey,P.ExternOrderkey,P.CartonType
   FROM #tmp_sortOutput  t 
   JOIN #TMP_WAVEPICK P ON P.labelno=t.labelno
   ORDER BY P.consigneekey,P.MarkForKey,P.ExternOrderkey,P.CartonType
