...
/*************************************************************************/   
CREATE PROCEDURE [dbo].[ispRVWAV36]        
 ...
 BEGIN    
    ...
    BEGIN  
        IF NOT EXISTS (SELECT 1 FROM REPLENISHMENT RP (NOLOCK)   
                       WHERE RP.Wavekey = @c_Wavekey AND RP.OriginalFromLoc = 'ispRLWAV36')  
        BEGIN                                            
          SELECT @n_continue = 3    
          SELECT @n_err = 81010    
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Nothing to reverse. (ispRVWAV36)'           
        END                   
    END  
    ...
    BEGIN  
        ...
        BEGIN  
          SELECT @n_continue = 3    
          SELECT @n_err = 81020    
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Some Tasks have been started. Not allow to Reverse Wave Released (ispRVWAV36)'         
        END                   
    END  
    ...
    BEGIN  
       DECLARE CUR_REPL CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
         ...
         AND OriginalFromLoc = 'ispRLWAV36'
         AND Confirmed = 'N'
       ...
       BEGIN
          ...
          BEGIN            
             ...
             BEGIN  
               ...
               SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Pickdetail Table Failed. Pickdetailkey:' + RTRIM(@c_Pickdetailkey)+ ' (ispRVWAV36)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
             END   
          ...
          BEGIN  
            ...
            SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Delete Replenishment Table Failed. (ispRVWAV36)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
          END   
    ...
    BEGIN
       ...
       BEGIN  
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Delete PACKTASK Table Failed. (ispRVWAV36)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
       END   
    END         
    ...
    BEGIN              
       ...
       BEGIN    
          ...
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update on wave Failed (ispRVWAV36)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
       END    
    END    
...
RETURN_SP:  
    ...
    BEGIN    
       ...
       BEGIN    
          ...
          END    
       END    
       execute nsp_logerror @n_err, @c_errmsg, "ispRVWAV36"    
