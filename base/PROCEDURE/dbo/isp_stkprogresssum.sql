...
/*************************************************************************/  
/* Stored Procedure: isp_STKProgressSum                                  */  
...
CREATE PROC [dbo].[isp_STKProgressSum]  
...
BEGIN
   ...
   SELECT Cnt
         ,NoOfLocCntText = 'Number of Locations counted in Count ' + CONVERT( VARCHAR(1), Cnt)
         ,NoOfLocCntKeys = CONVERT(VARCHAR(5), SUM(Counted)) 
                         + ' of ' + CONVERT(VARCHAR(5), @n_LocCnt) 
         ,NoOfLocCntPctg = CONVERT(DECIMAL(5,2),SUM(Counted) / CASE WHEN @n_LocCnt = 0 THEN 1 ELSE @n_LocCnt END * 100)
         ,SysQty      = SUM(SysQty)
         ,Qty         = SUM(Qty)
         ...
         ,NoOfRefKeysPctg1= CONVERT(DECIMAL(5,2),@n_FinalizeCnt1 / CASE WHEN @n_CCCnt = 0 THEN 1 ELSE @n_CCCnt END * 100)
         ...
         ,NoOfRefKeysPctg2= CONVERT(DECIMAL(5,2),@n_FinalizeCnt2 / CASE WHEN @n_CCCnt = 0 THEN 1 ELSE @n_CCCnt END * 100)
         ...
         ,NoOfRefKeysPctg3= CONVERT(DECIMAL(5,2),@n_FinalizeCnt3 / CASE WHEN @n_CCCnt = 0 THEN 1 ELSE @n_CCCnt END * 100)
   ...
   (
   SELECT  Cnt         = 1
         , Counted     = CASE WHEN Counted_Cnt1 = '1' THEN COUNT(DISTINCT ISNULL(RTRIM(CC.LOC),'')) ELSE 0 END
         , SysQty      = SUM(CC.SystemQty)
         , Qty         = SUM(CC.Qty)
         , SysQtyValue = SUM(CC.SystemQty * ISNULL(SKU.Price,0))
         , QtyValue    = SUM(CC.Qty * ISNULL(SKU.Price,0))
   FROM #TEMP_STOCKTAKE ST   WITH (NOLOCK)
   ...
   GROUP BY Counted_Cnt1
   UNION 
   SELECT  Cnt         = 2
         , Counted     = CASE WHEN Counted_Cnt2 = '1' THEN COUNT(DISTINCT ISNULL(RTRIM(CC.LOC),'')) ELSE 0 END
         , SysQty      = SUM(CC.SystemQty)
         , Qty         = SUM(CC.Qty_Cnt2)
         , SysQtyValue = SUM(CC.SystemQty * ISNULL(SKU.Price,0))
   ...
   JOIN LOC        LOC  WITH (NOLOCK) ON (CC.Loc = LOC.Loc)
   LEFT JOIN SKU   SKU  WITH (NOLOCK) ON (CC.Storerkey = SKU.Storerkey) AND (CC.Sku = SKU.Sku)
   GROUP BY Counted_Cnt2
   UNION
   SELECT  Cnt         = 3
         , Counted     = CASE WHEN Counted_Cnt3 = '1' THEN COUNT(DISTINCT ISNULL(RTRIM(CC.LOC),'')) ELSE 0 END
         , SysQty      = SUM(CC.SystemQty)
         , Qty         = SUM(CC.Qty_Cnt3)
         , SysQtyValue = SUM(CC.SystemQty * ISNULL(SKU.Price,0))
