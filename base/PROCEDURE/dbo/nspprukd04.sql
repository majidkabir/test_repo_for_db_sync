...
/* 19-Jan-2011 Leong   1.4   SOS# 202931 - Do not consider any Lot that      */
/*                                         exists in PICK location           */
/* 2014-11-20  Shong   1.5   Make WS01 Qty as Qty Available                  */        
/* 2015-01-21  Shong   1.6   Comment the QtyReplen. System alraedy included  */
/*                           Over Allocation                                 */
/* 2015-09-29  NJOW01  1.7   345748-allocation from bulk only                */
/* 17-Jun-2016 NJOW02  1.8   Fix bulk lot qty cater for overallocation       */
/* 25-Jul-2016 NJOW03  1.9   373644-Add lottable07 filtering                 */
/*****************************************************************************/

CREATE PROC [dbo].[nspPRUKD04]
   ...
   BEGIN
      ...
      EXEC (' DECLARE PREALLOCATE_CURSOR_CANDIDATES SCROLL CURSOR  FOR ' +
            ...
            ' QTYAVAILABLE = SUM(CASE WHEN LOC.Locationflag="HOLD" AND LOC.LOC <> "WS01" THEN 0 ELSE LOTxLOCxID.QTY END)
                            ...
                            ' - CASE WHEN SUM(ISNULL(LOTxLOCxID.QtyReplen,0)) < 0 THEN 0 ELSE SUM(ISNULL(LOTxLOCxID.QtyReplen,0)) END ' +  --NJOW01
            ...
            --NJOW01 Remark    
              ...
              ----                WHERE LLI.LOT = LOT.LOT AND (LLI.Qty - LLI.QTyAllocated - LLI.QtyPicked - LLI.QtyReplen) > 0
              ----                AND L.LocationType IN ("CASE", "PICK", "DYNPICKP", "DYNPICKR") ) ' +
            -- SOS# 202931 (End)
            -- LOT.QtyOnHold > 0) ' +                    -- Recalculate ON HOLD QTY
            ' AND SKUxLOC.LocationType NOT IN ("PICK" ,"CASE") ' + --NJOW01
            ' AND LOC.LocationType NOT IN ("PICK","CASE","DynPickP", "DYNPICKR") ' + --NJOW01
