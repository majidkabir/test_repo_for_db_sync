...
/************************************************************************/
/* Store Procedure: nsp_UCC_CartonLabel_46                              */
/* Creation Date: 19-Aug-2016                                           */
/* Copyright: IDS                                                       */
/* Written by: CSCHONG                                                  */
/*                                                                      */
/* Purpose: SOS#374828 -Pearson - New 4x4 Label                         */
/*                                                                      */
/*                                                                      */
/* Called By:r_dw_ucc_carton_label46 (copy from r_dw_ucc_carton_label40)*/
/*                                                                      */
/* PVCS Version: 1.0                                                    */
/*                                                                      */
/* Version: 5.4                                                         */
...
/* 2016-Oct-21 CSCHONG   Add new field (CS01)                           */
/* 2016-Nov-23 CSCHONG   WMS-656 Change field logic (CS02)              */
/************************************************************************/
CREATE PROC [dbo].[nsp_UCC_CartonLabel_46](
...
BEGIN
   ...
   SET @c_ExecStatements = N'SELECT PACKHEADER.PickSlipNo,'+
          ...
          'ORDERS.ConsigneeKey, '+  --(HFLiew01)
   		 'PACKHEADER.Route, '+
   		 --'ORDERS.C_Zip, '+       --(CS02)
   		 'Substring(ORDERS.Notes2,1,40) as C_Zip,' + --(CS02)
   ...
   BEGIN
      SET @c_ExecStatements1 = N'(SELECT COUNT(*)'+
      		 ...
      		 'ORDERS.OrderKey,CONVERT(NVARCHAR(8),ORDERS.DeliveryDate,3) AS DeliveryDate,ORDERS.ordergroup,'+
      		 'ORDERS.UserDefine09,ORDERS.C_Country,SUM(PACKDETAIL.qty) as qty , '  +            --(CS01)   
      		 'Case WHEN ISNULL(PACKDETAIL.Refno2,'''') <> '''' THEN PACKDETAIL.Refno2 ELSE PACKDETAIL.refno END AS PDID '              --(CS01)   
   END
   ...
   SET @c_ExecStatements4 = N'GROUP BY PACKHEADER.PickSlipNo, '+
			...
			'PACKHEADER.Route, '+
			--'ORDERS.C_Zip, '+                        --(CS02)
			'Substring(ORDERS.Notes2,1,40),'+         --(CS02)
			'PACKHEADER.OrderKey,'+
			'SUBSTRING(ORDERS.Notes2, 1, 30),'+
			'ORDERS.Type,'+
		   'ORDERS.OrderKey,'+
		   'CONVERT(NVARCHAR(8),ORDERS.DeliveryDate,3),ORDERS.ordergroup,ORDERS.UserDefine09,ORDERS.C_Country,'+   --(CS01)         
		   'Case WHEN ISNULL(PACKDETAIL.Refno2,'''') <> '''' THEN PACKDETAIL.Refno2 ELSE PACKDETAIL.refno END'    --(CS01)
