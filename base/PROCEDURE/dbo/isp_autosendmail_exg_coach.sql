...
GO
/*********************************************************************************/  
/* Stored Procedure: isp_AutoSendMail_EXG_Coach                                  */  
...
/* 28-Mar-2022  GHCHAN   1.0.0      Initial Development                          */  
/*********************************************************************************/  
  
CREATE PROC [dbo].[isp_AutoSendMail_EXG_Coach]  
(  
    @b_Debug            INT               = 0
...
--                        + N'<font face="Microsoft YaHei">如果需要提供签收单扫描件，请发送订单号至LFLPorscheDC@lflogistics.com, 进行查询。</font><br>'  
  ...
  SET @c_FileName = REPLACE(@c_FileName, 'Coach_Carton_Detail_Report_', '')
  ...
  SET @c_Consigneeval = SUBSTRING(@c_Consigneeval, PATINDEX('%[^0]%', @c_Consigneeval+'.'), LEN(@c_Consigneeval)) 
   ...
   SET @SQL = N' SELECT @c_EmailToList=notes1 FROM dbo.Storer WITH (NOLOCK)'
            + N' WHERE StorerKey LIKE ''%' + @c_Consigneeval + '%'''
            + N' AND ConsigneeFor = ''COACH'''
            + N' AND notes1 IS NOT NULL'
            + N' AND notes1 <> '''''
            + N' GROUP BY notes1'
   
   SET @c_ExecArgs = N' @c_EmailToList NVARCHAR(500) OUTPUT'
