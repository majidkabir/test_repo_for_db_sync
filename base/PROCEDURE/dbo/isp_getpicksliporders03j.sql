...
GO
/****************************************************************************/      
/* Stored Proc : isp_GetPickSlipOrders03j                                   */      
/* Creation Date:15-FEB-2022                                                */      
/* Copyright: IDS                                                           */      
/* Written by:CCHONG                                                        */      
/*                                                                          */      
/* Purpose: WMS-18629 and WMS-18847                                         */      
/*                                                                          */      
/*                                                                          */      
/* Usage:                                                                   */      
/*                                                                          */      
/* Local Variables:                                                         */      
/*                                                                          */      
/* Called By: r_dw_print_pickorder03j                                       */      
/*                                                                          */      
/* PVCS Version: 2.0                                                        */      
/*                                                                          */      
/* Version: 5.4                                                             */      
...
/* 15-FEB-2022 CSCHONG     1.0   Devops Scripts Combine                     */   
/****************************************************************************/      
      
CREATE    PROC [dbo].[isp_GetPickSlipOrders03j] (@c_loadkey NVARCHAR(10))      
AS      
BEGIN      
   SET NOCOUNT ON      
   SET ANSI_NULLS OFF      
...
DECLARE  @c_pickheaderkey        NVARCHAR(10),      
       ...
       , @n_SortByLoc            INT         
       , @c_Ordstatusfilter      NVARCHAR(1) = 'N'       
       , @c_doctype              NVARCHAR(1) = ''  
       , @c_ECOM_SINGLE_Flag     NVARCHAR(1) = ''  
       , @c_ApplySortBySkuLOGICALLOC NVARCHAR(1) = 'N'  
       , @c_skipprint                NVARCHAR(1) = 'N'  
...
BEGIN TRAN      
   CREATE TABLE #temp_pick      
      ...
      ,   SortByZipAddress1 INT   
      ,   SortBySkuLOGICALLOC INT       
      ,   ODUDF0708           NVARCHAR(50)      
      ,   ODUDF06             NVARCHAR(18)   
      ,   SPHLdesc            NVARCHAR(255)  
      ,   showsignoff         NVARCHAR(1)  
      )      
  
  
    SELECT  @c_StorerKey = MAX(Ord.storerkey)           
    FROM dbo.ORDERS ord WITH (NOLOCK)  
    WHERE ord.LoadKey= @c_loadkey  
  
   IF EXISTS (SELECT 1 FROM ORDERS OH WITH (NOLOCK) WHERE oH.StorerKey = @c_StorerKey AND OH.LoadKey = @c_loadkey AND OH.DocType = 'E' AND OH.ECOM_SINGLE_Flag = 'S')  
   BEGIN  
      SET @c_ApplySortBySkuLOGICALLOC = 'Y'  
   END  
   
  
    SELECT @c_Ordstatusfilter = ISNULL(CLR.Short,'N')  
    FROM Codelkup CLR (NOLOCK)   
    WHERE  CLR.Storerkey = @c_StorerKey  
    AND  CLR.Listname = 'PLSLOG' AND CLR.Long = 'r_dw_print_pickorder03j'    
  
  
   IF @c_Ordstatusfilter ='Y'  
   BEGIN  
      IF EXISTS (SELECT 1 FROM ORDERS OH WITH (NOLOCK) WHERE oH.StorerKey = @c_StorerKey AND OH.LoadKey = @c_loadkey AND OH.Status<2)  
      BEGIN  
            GOTO FAILURE  
      END  
   END  
   ...
   SELECT Storerkey      
         ...
         ,SortByZipAddress1 = ISNULL(MAX(CASE WHEN Code = 'SortByZipAddress1'  THEN 1 ELSE 0 END),0)    
         ,SortBySkuLOGICALLOC = ISNULL(MAX(CASE WHEN Code = 'SortBySkuLOGICALLOC'  THEN 1 ELSE 0 END),0)   
   INTO #TMP_RPTCFG      
   FROM CODELKUP WITH (NOLOCK)      
   WHERE ListName = 'REPORTCFG'      
   AND Long       = 'r_dw_print_pickorder03j'      
   ...
   SELECT (SELECT PICKHEADERKEY FROM PICKHEADER (NOLOCK)      
      ...
      ,  ISNULL(RC.SortByZipAddress1,0)    
      ,  ISNULL(RC.SortBySkuLOGICALLOC,0)    
      ,  CASE WHEN ORDERS.specialhandling ='P' THEN Substring(Orderdetail.Userdefine07,1,18) + Substring(Orderdetail.UserDefine08,1,14) ELSE '' END  
      ,  CASE WHEN ORDERS.specialhandling ='P' THEN ISNULL(Orderdetail.UserDefine06,'') ELSE '' END  
      ,  CASE WHEN ORDERS.specialhandling ='P' THEN ISNULL(clk.Description,'') ELSE '' END  
      ,  CASE WHEN ORDERS.specialhandling ='P' THEN 'Y' ELSE 'N' END  
   ...
   ON (Orders.Storerkey = CLR.Storerkey AND CLR.Code = 'ShowPickdetailID' AND CLR.Code2 = 'r_dw_print_pickorder03j'                                              
   AND CLR.Listname = 'REPORTCFG' AND CLR.Long = 'r_dw_print_pickorder03j' AND ISNULL(CLR.Short,'') <> 'N')      
   LEFT JOIN dbo.CODELKUP clk (NOLOCK) ON Clk.LISTNAME='SOHANDLING' AND clk.Code=ORDERS.specialhandling  
...
FAILURE:      
   ...
   IF OBJECT_ID('tempdb..#TEMP_PICK','u') IS NOT NULL          
   BEGIN          
...
SUCCESS:      
   SELECT       
      ...
      ,  showsignoff  
   FROM #TEMP_PICK        
   ORDER BY CASE WHEN SortByZipAddress1 = 1 AND SortBySkuLOGICALLOC = 0 THEN PostCode ELSE '' END  
         ,  CASE WHEN SortByZipAddress1 = 1 AND SortBySkuLOGICALLOC = 0 THEN Addr1 ELSE '' END  
         ,  CASE WHEN SortBySkuLOGICALLOC = 1 AND @c_ApplySortBySkuLOGICALLOC = 'Y' THEN sku ELSE '' END  
         ,  CASE WHEN SortBySkuLOGICALLOC = 1 AND @c_ApplySortBySkuLOGICALLOC = 'Y' THEN LogicalLoc ELSE '' END  
         ,  Company      
         ,  Orderkey      
         ,  PickZone                                                   
         ,  SUSR3      
         ,  CASE WHEN SortBySkuLoc = 1 AND Sortbyloc = 0 AND (SortBySkuLOGICALLOC = 0 OR (SortBySkuLOGICALLOC = 1 AND @c_ApplySortBySkuLOGICALLOC = 'N')) THEN Sku ELSE '' END                
         ,  CASE WHEN SortBySkuLoc = 1 OR Sortbyloc = 1 OR (SortBySkuLOGICALLOC = 1 AND @c_ApplySortBySkuLOGICALLOC = 'Y' ) THEN '' ELSE LogicalLoc END         
         ,  Loc      
         ,  CASE WHEN SortBySkuLoc = 1 OR (SortBySkuLOGICALLOC = 1 AND @c_ApplySortBySkuLOGICALLOC = 'Y') THEN '' ELSE Sku END      
         ,  Lottable01         
   --DROP Table #TEMP_PICK      
