...
GO
/************************************************************************/
/* Store Procedure:  isp_SCE_DL_GENERIC_IQC_RULES_100008_10             */
...
CREATE   PROCEDURE [dbo].[isp_SCE_DL_GENERIC_IQC_RULES_100008_10] (
   @b_Debug       INT            = 0
 , @n_BatchNo     INT            = 0
 , @n_Flag        INT            = 0
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         SET STG_Status = '3'
           , STG_ErrMsg = LTRIM(TRIM(ISNULL(STG_ErrMsg, ''))) + '/UOM is Null'
         WHERE STG_BatchNo              = @n_BatchNo
         AND   STG_Status                 = '1'
         AND   (UOM IS NULL OR TRIM(UOM) = '');
         ...
         BEGIN
            ...
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5), ISNULL(@n_ErrNo, 0))
                            + ': Update record fail. (isp_SCE_DL_GENERIC_IQC_RULES_100008_10)';
            ROLLBACK;
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            SET STG.UOM = CASE WHEN ISNULL(P.PackUOM3,'') = '' THEN STG.UOM ELSE P.PackUOM3 END
   ...
   IF @b_Debug = 1
   BEGIN
      SELECT '<<SUB-SP-RULES>> - [isp_SCE_DL_GENERIC_IQC_RULES_100008_10] EXIT... ErrMsg : ' + ISNULL(TRIM(@c_ErrMsg), '');
   END;
