...
/*************************************************************************/  
/* Stored Procedure: isp_EXG_FilterCondition2                            */  
/* Creation Date: 16 Jun 2020                                            */  
/* Copyright: LFL                                                        */  
/* Written by: GuanHao Chan                                              */  
/*                                                                       */  
/* Purpose: Excel Generator Sub StoredProcedure FilterConfition 2.       */  
/*                                                                       */  
...
CREATE PROCEDURE [dbo].[isp_EXG_FilterCondition2]  
...
BEGIN  
 ...
 IF @b_Debug = 1  
   BEGIN  
      PRINT '[dbo].[isp_EXG_FilterCondition2]: Start...'  
      PRINT '[dbo].[isp_EXG_FilterCondition2]: '  
   ...
   BEGIN  
         SET @n_Err = 220001  
         SET @c_ErrMsg = 'Invalid EXG Hdr ID. (isp_EXG_FilterCondition2)'  
         SET @n_Continue = 3  
         GOTO QUIT  
   END  
   ...
   BEGIN TRY  
      ...
      BEGIN  
         SET @n_Err = 220002  
         SET @c_ErrMsg = ' StorerKey cannot be null. - (isp_EXG_FilterCondition2)'  
     
         IF @b_Debug = 1  
         BEGIN  
            PRINT '[isp_EXG_FilterCondition2]: Execute SP Name Failed...' + ' @c_ErrMsg=' + @c_ErrMsg  
         END  
         GOTO QUIT  
      END  
      ...
      BEGIN  
         ...
         BEGIN  
            SET @n_Err = 220003  
            SET @c_ErrMsg = '(MbolKey To) cannot be null. - (isp_EXG_FilterCondition2)'  
      ...
      AND MbolKey BETWEEN @c_ParamVal2  
      AND @c_ParamVal3  
...
STARTCURSOR:    
   ...
   BEGIN CATCH   
      ...
      SET @n_Err = @n_ITFErr  
      SET @c_ErrMsg = LTRIM(RTRIM(@c_ITFErrMsg)) + ' (isp_EXG_FilterCondition2)'  
                 
      IF @b_Debug = 1  
      BEGIN  
         PRINT '[isp_EXG_FilterCondition2]: Execute SP Name Failed...'   
               + ' @c_ErrMsg=' + @c_ErrMsg  
      END  
      GOTO QUIT  
   END CATCH  
...
QUIT:  
   ...
   BEGIN        
      ...
      IF @b_Debug = 1  
      BEGIN  
         PRINT '[dbo].[isp_EXG_FilterCondition2]: @c_ErrMsg=' + RTRIM(@c_ErrMsg)  
         PRINT '[dbo].[isp_EXG_FilterCondition2]: @b_Success=' + RTRIM(CAST(@b_Success AS NVARCHAR))  
      END  
