...
/* 2020-11-25  Wan      1.0   Created                                      */
/***************************************************************************/  
CREATE PROC [dbo].[ispPOTRF04]  
...
--         , @c_TransferLineNumber NVARCHAR(5)  --(Wan01) 
   ...
   --Do not for Retail Transfer Type
   IF EXISTS (SELECT 1 FROM TRANSFER TF WITH (NOLOCK) WHERE TF.Transferkey  = @c_Transferkey AND TF.[Type] = 'Retail')
   BEGIN
      GOTO QUIT_SP
   END
   ...
   BEGIN  
      SET @n_Continue = 3 
      SET @n_err=82005  
      SET @c_ErrMsg='NSQL'+CONVERT(VARCHAR(5),@n_err)+': Getting KitKey Fail. (ispPOTRF04)' 
      GOTO QUIT_SP  
   END  
   ...
   BEGIN
      SET @n_Continue = 3
      SET @n_err=82010   
      SET @c_ErrMsg='NSQL'+CONVERT(VARCHAR(5),@n_err)+': Insert Into KIT Fail. (ispPOTRF04)'
      GOTO QUIT_SP
   END
   ...
   BEGIN
      ...
      BEGIN
         SET @n_Continue = 3
         SET @n_err=82015  
         SET @c_ErrMsg='NSQL'+CONVERT(VARCHAR(5),@n_err)+': Insert Into KITDETAIL Fail. (ispPOTRF04)'
         GOTO QUIT_SP
      END
   ...
   BEGIN
      ...
      BEGIN
         ...
         END
      END
      Execute nsp_logerror @n_err, @c_errmsg, 'ispPOTRF04'
