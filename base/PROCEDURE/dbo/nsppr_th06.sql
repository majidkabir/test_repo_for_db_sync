...
/************************************************************************/
/* Stored Procedure: nspPR_TH06                                         */
/* Creation Date: 27-JUN-2016                                           */
/* Copyright: LF                                                        */
/* Written by:                                                          */
/*                                                                      */
/* Purpose: PreAllocateStrategy : 372360-TH MFG-Preallocate FIFO by     */
/*                                custom fields                         */
/*                                                                      */
...
BEGIN   
   ...
   BEGIN
        ...
        SELECT TOP 1 @c_ID = OD.ID,
                     @c_Lottable01 = O.Loadkey,
                     @c_Lottable02 = O.ExternOrderkey,
        ...
        SELECT TOP 1 @c_Lottable03 = POD.Lottable03
        FROM PO (NOLOCK)
        JOIN PODETAIL POD (NOLOCK) ON PO.POKey = POD.POKey
        WHERE POD.Lottable01 = @c_Lottable01       
        AND POD.Lottable02 = @c_Lottable02
        --AND POD.Userdefine04 = @c_Consigneekey
        AND POD.Userdefine10 = @c_Orderkey
        AND PO.Userdefine03 = @c_Lottable01 --Loadkey
        AND POD.Sku = @c_Sku
   END
