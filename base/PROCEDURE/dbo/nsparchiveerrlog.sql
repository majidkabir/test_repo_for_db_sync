...
begin -- main
   ...
   SET CONCAT_NULL_YIELDS_NULL OFF
	...
	declare @n_retain_days int      , -- days to hold data
		...
		@c_datetype NVARCHAR(10),      -- 1=Logdate
		@n_archive_errlog_records   int, -- # of ID records to be archived
		@n_default_id int,
		@n_strlen int,
		@local_n_err         int,
	...
	begin -- 3
		select  @c_copyfrom_db = livedatabasename,
			...
			@n_retain_days = errlognumberofdaystoretain,
			@c_errlogactive = errlogactive,
			@c_datetype = errlogdatetype,
		...
		select @b_success = 1
		select @c_temp = 'archive of errlog started with parms; active = '+ dbo.fnc_RTrim(@c_errlogactive)+
			' ; copy rows to archive = '+dbo.fnc_RTrim(@copyrowstoarchivedatabase)+
			' ; retain days = '+ convert(char(6),@n_retain_days)
	
		execute nsplogalert
			@c_modulename   = "nsparchiveerrlog",
	...
	begin -- 4
		...
		begin 
			if (@b_debug =1 )
			begin
				print 'starting table existence check for errlog...'
			end
			select @b_success = 1
		...
		begin
			if (@b_debug =1 )
			begin
				print 'building alter table string for errlog...'
			end
		...
		begin -- 5
			...
			begin	
				select @n_archive_errlog_records = count(1)
        	                from errlog 
			...
			if ((@n_continue = 1 or @n_continue = 2)  and @copyrowstoarchivedatabase = 'y')
			begin
				select @c_temp = "attempting to archive " + dbo.fnc_RTrim(convert(char(6),@n_archive_errlog_records )) +
					" errlog records."
				execute nsplogalert
					@c_modulename   = "nsparchiveerrlog",
					@c_alertmessage = @c_temp ,
			...
			begin  
				if (@b_debug =1 )
				begin
					print "delete for errlog..."
				end
				if (@b_debug = 0)
				begin
					select @c_executestatement = 'delete from errlog '
				...
				begin  
					...
					select @local_c_errmsg = convert(char(5),@local_n_err)
					select @local_c_errmsg =
					":  dynamic delete errlog failed. (nsparchiveerrlog) " + " ( " +
	...
	begin
		...
		execute nsplogalert
			@c_modulename   = "nsparchiveerrlog",
			@c_alertmessage = "archive of errlog ended normally.",
			@n_severity     = 0,
			@b_success       = @b_success output,
			@n_err          = @n_err output,
			@c_errmsg       = @c_errmsg output
	...
	begin
		...
		begin
			...
			execute nsplogalert
				@c_modulename   = "nsparchiveerrlog",
				@c_alertmessage = "archive of errlog ended abnormally - check this log for additional messages.",
				@n_severity     = 0,
				@b_success       = @b_success output ,
				@n_err          = @n_err output,
				@c_errmsg       = @c_errmsg output
	...
	begin
		...
		execute nsplogalert
			@c_modulename   = "nsparchiveerrlog",
			@c_alertmessage = "purging errlog tables",
			@n_severity     = 0,
			@b_success       = @b_success output,
			@n_err          = @n_err output,
			@c_errmsg       = @c_errmsg output
	...
	begin
		...
		execute nsp_logerror @n_err, @c_errmsg, "nsparchiveerrlog"
