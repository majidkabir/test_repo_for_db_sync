...
/*                         (SW02)                                       */
/* 30-Jan-2018  TLTING     remove SETROWCOUNT                           */
/* 06-Mar-2018  TLTING01   DISTINCT on Select data                      */  
/* 28-Jan-2019  TLTING_ext 1.1  enlarge externorderkey field length     */
/************************************************************************/
CREATE PROC [dbo].[ispGenOWORDALLOC] (
...
BEGIN
   ...
   BEGIN
      ...
      SET @c_ExecStatements = N'DELETE OWORDALLOC '
                              + 'FROM   OWORDALLOC '
                              + 'INNER JOIN Orders WITH (NOLOCK) '
                              + '   ON Orders.ExternOrderKey = OWORDALLOC.ExternOrderkey '
                              + 'INNER JOIN TransmitLog TL WITH (NOLOCK) '
                              + '   ON TL.Key1 = ORDERS.OrderKey AND TL.TableName '
                              + ' IN (''OWORDALLOC'', ''OWDPREPICK'', ''OWORDPICK'') AND '
                              + '      TL.TransmitFlag = ''1'' '
                              
      IF ISNULL(RTRIM(@c_FilterFlag),'') = 'E1T'
      BEGIN
         SET @c_ExecStatements = @c_ExecStatements 
                                 + ' WHERE EXISTS ( SELECT 1 FROM Codelkup AS CDL WITH (NOLOCK) '       
      END 
      ELSE
      BEGIN
         SET @c_ExecStatements = @c_ExecStatements 
                                 + ' WHERE NOT EXISTS ( SELECT 1 FROM Codelkup AS CDL WITH (NOLOCK) '                             
      END
   ...
   BEGIN
      ...
      SET @c_ExecStatements = ''
      SET @c_ExecArguments  = ''      
      SET @c_ExecStatements = N'INSERT INTO OWORDALLOC (ExternOrderkey,ExternOrderkey2,ExternOrderKey3, '
                              +'ExternLineNo,SKU,UOM,Storerkey,Lottable02, '
                              ...
                              +'INNER JOIN TransmitLog TL WITH (NOLOCK) '
                              +'   ON TL.Key1 = OrderKey AND TL.TableName IN (''OWORDALLOC'', ''OWDPREPICK'', ''OWORDPICK'') '
                              ...
                              +'INNER JOIN Exe2OW_allocpickship Exe2Ow WITH (NOLOCK) '  --(SW02)
                              +'   ON (Exe2Ow.ExternOrderkey = OrderDetail.ExternOrderkey '
      ...
      SET @c_ExecStatements = @c_ExecStatements
                              ...
                              +'   OrderDetail.LoadKey, ISNULL(RTRIM(LOC.HostWHCode),'' ''), TL.TableName, Orders.Userdefine08, '
                              + '  NewLineNo, '    --(KC01)
   ...
   BEGIN
      ...
      SET @c_ExecArguments  = ''        
      SET @c_ExecStatements = N'INSERT INTO OWORDALLOC (ExternOrderkey,ExternOrderkey2,ExternOrderKey3, '
                              +'ExternLineNo,SKU,UOM,Storerkey, '
   ...
   BEGIN
      ...
      IF ISNULL(RTRIM(@c_FilterFlag),'') = 'E1T'
      BEGIN
         DECLARE CUR_OWORDALLOC_E1T CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
         SELECT DISTINCT OWORDALLOC.ExternOrderkey,     -- tlting01  
         ...
         OPEN CUR_OWORDALLOC_E1T
         FETCH NEXT FROM CUR_OWORDALLOC_E1T INTO @c_ExternOrderKey, @c_storerkey
         ...
         BEGIN
            ...
            DECLARE CUR1_E1T CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
            ...
            FETCH NEXT FROM CUR1_E1T INTO @c_ExternLineNo, -- @c_Storerkey, SOS14179
               ...
               BEGIN
                  ...
                  BEGIN
                    -- Wrong Qty, update flag to 5 (Invalid)
                     ...
                     UPDATE OWORDALLOC WITH (ROWLOCK)    --(SW02)
                        SET TransmitFlag = '5'
                     WHERE TransmitFlag = '0'
                     AND   ExternOrderKey = @c_ExternOrderKey
            ...
            BEGIN
               ...
               IF @b_success <> 1
               BEGIN
                  SELECT @n_continue = 3, @c_errmsg = 'ispGenOWOrdAlloc' + dbo.fnc_RTrim(@c_errmsg)
               END
         ...
         CLOSE CUR_OWORDALLOC_E1T
         DEALLOCATE CUR_OWORDALLOC_E1T
      ...
      ELSE
      BEGIN
         DECLARE CUR_OWORDALLOC_E1 CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
         SELECT DISTINCT OWORDALLOC.ExternOrderkey,      --tlting01  
                OWORDALLOC.Storerkey
         FROM   OWORDALLOC WITH (NOLOCK)
         INNER JOIN ORDERDETAIL WITH (NOLOCK)                                             --(SW01)
         ...
         OPEN CUR_OWORDALLOC_E1
         FETCH NEXT FROM CUR_OWORDALLOC_E1 INTO @c_ExternOrderKey, @c_storerkey
         ...
         BEGIN
            ...
            -- Date: 06-06-2002
            -- Should SUM the qty cause if there is duplicate line, it will pass thru if not GROUP BY externline, lottable02, sku
            DECLARE CUR1_E1 CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
               SELECT  ExternLineNo, -- StorerKey, SOS14179
                       SKU, Lottable02, SUM(Qty) AS Qty, TransmitLogKey
               FROM    OWORDALLOC WITH (NOLOCK)
               WHERE   TransmitFlag = '0'
            ...
            FETCH NEXT FROM CUR1_E1 INTO @c_ExternLineNo, -- @c_Storerkey, SOS14179
                                      @c_SKU, @c_Lottable02, @n_Qty, @c_TransmitLogKey
            WHILE @@FETCH_STATUS <> -1
            BEGIN
               IF @n_Qty > 0
            ...
            FETCH NEXT FROM CUR_OWORDALLOC_E1 INTO @c_ExternOrderKey, @c_storerkey
         END -- While @@fetch_status <> -1
         CLOSE CUR_OWORDALLOC_E1
         DEALLOCATE CUR_OWORDALLOC_E1
