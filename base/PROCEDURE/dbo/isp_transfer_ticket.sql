...
/************************************************************************/  
/* Store Procedure:  isp_Transfer_Ticket                                */  
/* Creation Date:                                                       */  
/* Copyright: IDS                                                       */  
/* Written by: YTWan                                                    */  
/*                                                                      */  
...
/* Return Status:  None                                                 */  
/*                                                                      */  
/* Usage:  Used for report dw = r_dw_transfer_ticket                    */  
/*                                                                      */  
/* Local Variables:                                                     */  
/*                                                                      */  
/* Called By:                                                           */  
...
/* 11-Jul-2014  Audrey  1.1   SOS315929 - Revise Codelkup Logic  (ang01)*/  
/* 22-Dec-2014  SPChin  1.2   SOS329051 Add StorerKey Filter            */  
/* 25-MAR-2016  Wan01   1.3   SOS#366941- SG - Transfer Ticket Report   */  
/*                            (lottable06):Add LA 06-15& show by config */  
/* 17-APR-2018 LZG  1.4 Changed to Sum of FromQty & ToQty (ZG01)  */  
/* 09-AUG-2019 WLChooi  1.5   Fixed Qty issue and add condition only    */  
/*                            for UNILEVER                              */  
/* 21-JUL-2021 CheeMun  1.6   JSM-10233 Extend CustomerRefNo Length     */    
/************************************************************************/  
CREATE PROC [dbo].[isp_Transfer_Ticket] (@c_TransferKey NVARCHAR(10))  
 AS  
BEGIN  
    SET NOCOUNT ON  
    SET QUOTED_IDENTIFIER OFF  
 ...
 END  
   ...
   CREATE TABLE #TMP_TRFTICKET  
      ...
      ,  CustomerRefNo     NVARCHAR(20)   NULL      --JSM-10233    
   ...
   SELECT TRANSFER.TransferKey,  
      ...
      SUM(TRANSFERDETAIL.FromQty) AS FromQty,     -- ZG01  
      ...
      SUM(TRANSFERDETAIL.ToQty) AS ToQty,      -- ZG01  
      ...
      --(Wan01) - END       
      MIN(CODELKUP.Description) AS Desr_a, --ang01  
      FromCompany = STORER_a.Company,  
      ToCompany = STORER_b.Company,  
      fromcasecnt = PACK_a.casecnt,  
    tocasecnt = PACK_b.casecnt,  
    fromuom1 = RTRIM(PACK_a.Packuom1),     --(Wan01) - add rtrim  
    touom1 = RTRIM(PACK_b.Packuom1),       --(Wan01) - add rtrim  
    fromuom3 = RTRIM(PACK_a.Packuom3),     --(Wan01) - add rtrim  
    touom3 = RTRIM(PACK_b.Packuom3),       --(Wan01) - add rtrim  
    user_name = user_name(),  
    MIN(CODELKUP_b.Description) AS Desr_b, --ang01  
    TRANSFER.Facility,  
    TRANSFER.ToFacility  
   FROM TRANSFER             WITH (NOLOCK)  
