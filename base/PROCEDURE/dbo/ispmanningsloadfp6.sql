...
/************************************************************************/
/* Stored Proc: ispManningsLoadFP6                                      */
...
/*                            (WL01)                                    */
/************************************************************************/
CREATE PROC [dbo].[ispManningsLoadFP6]
...
BEGIN
   ...
   BEGIN
      PRINT '---------------------------------------'+ CHAR(13) +
            'Sub PICKCOde: ispManningsLoadFP6'+ CHAR(13) +
  ...
  ;WITH ORD AS
   (  SELECT OH.Loadkey
      ...
      AND OH.ECOM_SINGLE_Flag IN ('M') -- Only B2C Multi  
   ...
   SET @c_SQL = N'INSERT INTO #LOCxID ( Loc, ID, Qty )'
   ...
   +' AND   LOC.LocationHandling IN ( @c_LocationHandlingPL ) '
   +' AND   LOC.LocationType = @c_LocationType'
   +' GROUP BY LLI.Loc, LLI.ID'
   +' HAVING SUM(LLI.Qty) <= @n_TotalOrderQty'  
   +' AND    SUM(LLI.QtyAllocated + LLI.QtyPicked + LLI.QtyReplen) = 0'
   +' AND    COUNT(DISTINCT(LLI.Sku)) = 1'
   +' AND    COUNT(DISTINCT LOT.[Status]) = 1 AND MIN(LOT.[Status]) = ''OK'''
   ...
   FETCH NEXT FROM @CUR_LI INTO @c_Lot, @c_Loc, @n_IDQty 

   WHILE @@FETCH_STATUS <> -1 AND @n_QtyLeftToFullFill > 0 AND @n_QtyLeftToFullFill >= @n_IDQty
   BEGIN
      --Check Channel Qty Available for all Lot for that Pallet
      ...
      FETCH NEXT FROM CUR_ID INTO @c_Lot
      ...
      BEGIN 
         ...
         BEGIN CATCH  
            ...
            SELECT @n_continue = 3  
            SET @c_ErrMsg = RTRIM(@c_ErrMsg) + '. (ispManningsLoadFP6)'   
         END CATCH
         ...
         BEGIN
            ...
            SELECT @n_Channel_Qty_Available = ci.Qty - (ci.QtyAllocated) - ci.QtyOnHold
            FROM ChannelInv ci WITH (NOLOCK)  
            WHERE ci.Channel_ID = @n_Channel_ID  
            
            SET @n_TotalChannelQty = @n_TotalChannelQty + @n_Channel_Qty_Available
         END    
      ...
      --If ChannelQty < PalletQty - Reject
      IF @n_TotalChannelQty < @n_IDQty
      BEGIN
         GOTO NEXT_LOCxID
      END

      SET @c_Loadkey = ''
      SELECT TOP 1 @c_Loadkey = OL.Loadkey                    
            ...
            BEGIN
               ...
               BEGIN
                  ...
                  SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0)) 
                                 + ': Get PickDetailKey Failed. (ispManningsLoadFP6)'
                  GOTO QUIT_SP
               END
               ...
               BEGIN
                  PRINT 'PickDetailKey: ' + @c_PickDetailKey + CHAR(13) +
                        ...
                        'Channel_ID: ' +  CAST(@n_Channel_ID AS NVARCHAR)  
               END
               ...
               BEGIN
                  ...
                  SET @n_Err = 81220
                  SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0)) 
                                 + ': Insert PickDetail Failed. (ispManningsLoadFP6)'
                  GOTO QUIT_SP
               END
...
QUIT_SP:
   ...
   BEGIN
      ...
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'ispManningsLoadFP6'
