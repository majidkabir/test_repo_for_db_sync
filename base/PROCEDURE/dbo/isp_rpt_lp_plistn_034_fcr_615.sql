...
/***************************************************************************/      
/* Stored Procedure: isp_RPT_LP_PLISTN_034_FCR_615                         */      
/* Creation Date: 22-Aug-2024                                             */    
/* Copyright: LF Logistics, update by LAM CE                               */    
/* Written by: WZPang, update by ADURAN					                           */    
/*                                                                         */    
/* Purpose: WMS-21913 - [ID] Diversey û LoadSheet, update by FCR-615       */
/* Based: isp_RPT_LP_PLISTN_034                                            */      
/* Called By: RPT_LP_PLISTN_034_FCR_615                                    */      
/*                                                                         */      
/* GitLab Version: 1.0                                                     */      
/*                                                                         */      
/* Version: 1.0                                                            */      
...
/* 30-Jul-2024  ADURAN   1.0  DevOps Combine Script                        */
/* 22-Aug-2024  LAM CE   1.1  update by FCR-615                            */
/***************************************************************************/  
CREATE  	PROC [dbo].[isp_RPT_LP_PLISTN_034_FCR_615] (
 ...
 BEGIN    
   ...
   DECLARE @c_pickheaderkey        NVARCHAR(10),    
          ...
          , @c_Type            NVARCHAR(1) = '1'                      
          , @c_DataWindow      NVARCHAR(60) = 'RPT_LP_PLISTN_034_FCR-615'  
          , @c_RetVal          NVARCHAR(255)  
		  , @c_UserDefine01    NVARCHAR(255) 
   ...
   BEGIN    
      SELECT @n_continue = 3      
      SELECT @n_err = 63500    
      SELECT @c_errmsg = 'NSQL'+CONVERT(char(5),@n_err)+': Pickslip already printed using Consolidated option. (isp_RPT_LP_PLISTN_034_FCR-615)'    
   END    
   -- end   
   ...
   SELECT @c_Storerkey = PH.Storerkey, 
          @c_UserDefine01 = WA.UserDefine01
   FROM PICKHEADER PH (NOLOCK)
   JOIN WAVE WA (NOLOCK) ON WA.WaveKey = PH.WaveKey
   WHERE WA.UserDefine01 =  @c_UserDefine01
   ...
   BEGIN    
      CREATE TABLE #TEMP_PICK (
		 ...
		 UserDefine01     NVARCHAR(255)NULL )                 
      ...
      SELECT RefKeyLookup.PickSlipNo,         
			 ...
			 WA.UserDefine01 AS UserDefine01
	  ...
	  JOIN WAVE WA (NOLOCK) ON WA.WaveKey = LOADPLAN.UserDefine09
      ...
      BEGIN
         ...
         LEFT JOIN CODELKUP CLR (NOLOCK) ON (O.Storerkey = CLR.Storerkey AND CLR.Code = 'NOSPLITBYLINECNTZONE'     
                                             AND CLR.Listname = 'REPORTCFG' AND CLR.Long = 'RPT_LP_PLISTN_034_FCR-615' AND ISNULL(CLR.Short,'') <> 'N')    
         ...
         BEGIN -- while 1      
            ...
            BEGIN            
               ...
               BEGIN      
                  ...
                  BEGIN      
                     SELECT @n_continue = 3      
                     SELECT @n_err = 63501    
                 SELECT @c_errmsg = 'NSQL'+CONVERT(char(5),@n_err)+': Insert into PICKHEADER Failed. (isp_RPT_LP_PLISTN_034_FCR-615)'    
                     GOTO FAILURE    
                  END                 
               END -- @b_success = 1        
               ELSE       
               BEGIN      
                  SELECT @n_continue = 3      
                  SELECT @n_err = 63502    
                 SELECT @c_errmsg = 'NSQL'+CONVERT(char(5),@n_err)+': Get PSNO Failed. (isp_RPT_LP_PLISTN_034_FCR-615)'      
                  BREAK       
               END       
               ...
               BEGIN      
                  ...
                  BEGIN       
                     ...
                     BEGIN      
                        SELECT @n_continue = 3    
                        SELECT @n_err = 63503    
                      SELECT @c_errmsg = 'NSQL'+CONVERT(char(5),@n_err)+': Insert RefKeyLookup Failed. (RPT_LP_PLISTN_034_FCR-615)'        
                        GOTO FAILURE    
                     END                              
                  END       
            ...
            BEGIN      
               SELECT @n_continue = 3      
               SELECT @n_err = 63504    
               SELECT @c_errmsg = 'NSQL'+CONVERT(char(5),@n_err)+': Update #Temp_Pick Failed. (isp_RPT_LP_PLISTN_034_FCR-615)'        
               GOTO FAILURE    
            END         
 ...
 SUCCESS:    
   ...
   BEGIN    
      ...
      BEGIN    
         ...
         END    
       END    
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'isp_RPT_LP_PLISTN_034_FCR-615'    
