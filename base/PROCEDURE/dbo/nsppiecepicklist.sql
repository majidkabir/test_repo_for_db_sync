...
CREATE PROC [dbo].[nspPiecePickList] (@c_facility		 NVARCHAR(5)
...
,										@c_LoadKeyEnd	 NVARCHAR(10)	 )
 ...
 /* Start Modification */
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					SELECT @n_continue = 3
					SELECT @n_err = 63501
					SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Insert Into PICKHEADER Failed. (nspPiecePickList)"
				END
   ...
   END -- @n_continue = 1 or @n_continue = 2
	...
	BEGIN
   	execute nsp_logerror @n_err, @c_errmsg, "nspPiecePickList"
