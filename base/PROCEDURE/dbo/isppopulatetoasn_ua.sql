...
/**************************************************************************/
/* SP: ispPopulateTOASN_UA                                                */
/* Creation Date: 06-Jul-2015                                             */
/* Copyright: LF                                                          */
/* Written by:               						                                  */
/*                                                                        */
/* Purpose: Auto Create ASN for UA Transfer Orders SOS#344855             */
/*                                                                        */
...
/*04/08-2015   NJOW01  1.0  344855 - change map receiptdetail.userdefine01*/
/*                          to orderdetail.lottable02                     */  
/**************************************************************************/

CREATE PROCEDURE [dbo].[ispPopulateTOASN_UA] 
...
BEGIN
   ...
   DECLARE  @c_Lottable01        NVARCHAR(18),
            ...
            @c_Lottable02_new    NVARCHAR(18),
            @c_Lottable03        NVARCHAR(18),
            @d_Lottable04        datetime,
            @d_Lottable05        datetime,
            @c_Lottable06        NVARCHAR(30),
            @c_Lottable06_new    NVARCHAR(30),
   ...
   BEGIN  
     ...
     SELECT TOP 1 
            @c_ExternReceiptKey   = RTRIM(ORDERS.Storerkey) + ORDERS.Mbolkey, 
            @c_WarehouseReference = ORDERS.Orderkey,
            @c_Type               = ISNULL(CL2.Short,'NIF'),
            @c_ToFacility         = ORDERS.Facility,             
            @c_ToStorerkey        = ORDERS.Storerkey,
            @c_Storerkey          = ORDERS.Storerkey,
            @c_Mbolkey            = ORDERS.Mbolkey,
            @c_Wavekey            = ORDERS.Userdefine09,
            @c_Pickslipno         = PICKHEADER.Pickheaderkey,
            @c_UDF04              = ISNULL(CODELKUP.UDF04,''),
            @c_UDF05              = CASE WHEN ISNULL(CODELKUP.UDF05,'') = '' AND ORDERS.Type = 'TRFB2E' THEN
                                          'ECOM'
                                         WHEN ISNULL(CODELKUP.UDF05,'') = '' AND ORDERS.Type = 'TRFE2B' THEN
                                          'RETAIL'
                                         ELSE ISNULL(CODELKUP.UDF05,'')
                                    END
     FROM  ORDERS WITH (NOLOCK)
     JOIN  STORER WITH (NOLOCK) ON ORDERS.Storerkey = STORER.Storerkey
     LEFT JOIN PICKHEADER WITH (NOLOCK) ON ORDERS.Orderkey = PICKHEADER.Orderkey 
     LEFT JOIN CODELKUP WITH (NOLOCK) ON ORDERS.Type = CODELKUP.Code AND CODELKUP.Listname = 'ORDERTYPE'
     LEFT JOIN CODELKUP CL2 WITH (NOLOCK) ON CL2.Listname = 'UATRFCFM' AND CL2.Storerkey = ORDERS.Storerkey AND CL2.Code = ORDERS.Type
     ...
     IF NOT EXISTS ( SELECT 1 FROM FACILITY WITH (NOLOCK) WHERE Facility = @c_ToFacility )
	    ...
	    SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Invalid To Facility: ' + RTRIM(ISNULL(@c_ToFacility,'')) +
                        ' (ispPopulateTOASN_UA)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
		 END
	   ...
	   BEGIN 				
	      ...
	      BEGIN
	      	 ...
	      	 BEGIN
	            ...
	            IF @b_success = 1
	            BEGIN
	               INSERT INTO RECEIPT (ReceiptKey, ExternReceiptkey, Warehousereference, StorerKey, RecType, Facility, DocType, RoutingTool, Mbolkey)
	               VALUES (@c_NewReceiptKey, @c_ExternReceiptKey, @c_WarehouseReference, @c_ToStorerKey, @c_Type, @c_ToFacility, 'A', 'N', @c_Mbolkey)
                 ...
                 BEGIN
	                   ...
	                   SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err)
				             SET @n_err = 63498   
	   		             SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert Error on Table Receipt (ispPopulateTOASN_UA)' + ' ( ' + 
	            ...
	            BEGIN
	               ...
	               SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err) 
					       SET @n_err = 63499   
	   			       SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Generate Receipt Key Failed! (ispPopulateTOASN_UA)' 
	   			                 + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
	            END
	         END
	      END    
	      ...
	      BEGIN
	         ...
	         SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err)
				   SET @n_err = 63500   
	   		   SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Storer Key is BLANK! (ispPopulateTOASN_UA)' + ' ( ' + 
                          ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
	      END
	   ...
	   BEGIN         
	      ...
	      BEGIN
	         ...
	         /*
	            ...
	            SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err)
				      SET @n_err = 63501   
	   		      SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Invalid Sku: ' + RTRIM(@c_Sku) + 'Of Storer: '+ RTRIM(@c_ToStorerkey) + ' (ispPopulateTOASN_UA)' + ' ( ' + 
                         ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
				   END
				   */
		               ...
		               DECLARE PICK_CUR CURSOR FAST_FORWARD READ_ONLY FOR 
			             ...
			             FETCH NEXT FROM PICK_CUR
			                INTO @n_QtyReceived,  @c_Lottable01, @c_Lottable02, @c_Lottable03, @d_Lottable04, @d_Lottable05, 
			                     ...
			                     @c_Lottable12, @d_Lottable13, @d_Lottable14, @d_Lottable15, @c_ID, @c_Pickdetailkey, @c_Toloc, 
			                     @c_Caseid, @c_DropId, @c_CartonGroup
			             ...
			             BEGIN
			                ...
			                SET @c_Lottable06_new = @c_Lottable06 
			                IF ISNULL(@c_UDF04,'') <> ''
			                   SELECT @c_Lottable06_new = @c_UDF04

			                SET @c_Lottable02_new = @c_Lottable02
			                IF ISNULL(@c_UDF05,'') <> ''
			                   SELECT @c_Lottable02_new = @c_UDF05
			                ...
			                INSERT INTO RECEIPTDETAIL (ReceiptKey,   			  ReceiptLineNumber,   ExternReceiptkey,    
			                                           ...
			                                           Lottable01,          Lottable02,   			 Lottable03,	  
				   		 										               ...
				   		 										               Userdefine04,				Userdefine05,				 Userdefine08,
				   		 										               Userdefine09)			                                          
			                            VALUES        (@c_NewReceiptKey, 	  @c_ReceiptLine,      @c_ExternReceiptkey,   
			                                           @c_Pickdetailkey,    @c_ToStorerKey,      @c_SKU,
			                                           @n_QtyReceived,      0,					
			                                           @c_UOM,            	@c_Packkey,          @c_Toloc,			                                           
			                                           @c_Lottable01,       @c_Lottable02_new, 	 @c_Lottable03,       
				   		 																	 @d_Lottable04,				@d_Lottable05,			 @c_Lottable06_new,
				   		 																	 @c_Lottable07,				@c_Lottable08,			 @c_Lottable09,
				   		 																	 @c_Lottable10,				@c_Lottable11,			 @c_Lottable12,
				   		 																	 @d_Lottable13,				@d_Lottable14,			 @d_Lottable15,
				   		 																	 @n_QtyReceived,      'N', 								 @c_ID,
				   		 																	 @c_Orderkey,					@c_Lottable02,  		 @c_Caseid,
				   		 																	 @c_DropId,						@c_CartonGroup,			 @c_Wavekey,
				   		 																	 @c_Pickslipno)	
     ...
     BEGIN
        ...
        BEGIN
	          ...
	          SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err)
				    SET @n_err = 63492   
	   		    SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Error on Table Receipt (ispPopulateTOASN_UA)' + ' ( ' + 
                       ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
        END              
     END*/
	   ...
	   BEGIN
	      ...
	      --BEGIN
	         --ROLLBACK TRAN
	      --END
	      EXECUTE nsp_logerror @n_err, @c_errmsg, 'ispPopulateTOASN_UA'
