...
BEGIN        
    ...
    DECLARE @t_OrderWorkInProgress TABLE (        
        OrderKey    NVARCHAR(10) )        
               
    INSERT INTO @t_OrderWorkInProgress( OrderKey )         
    SELECT DISTINCT td.OrderKey    
    FROM TaskDetail td WITH (NOLOCK)        
    WHERE  td.TASKTYPE = 'VNPK' AND        
    td.STATUS = '3' AND         
    td.Userkey <> '' AND         
    td.UserKey <> @c_userid         
        
    SELECT TOP 1 @c_LastPickMethod = PickMethod     
    ...
    /* Doesn't need to do this. Not for VNA          
  ...
  INNER JOIN TaskManagerUserDetail WITH (NOLOCK)         
         ...
         WHERE  TaskManagerUserDetail.UserKey = @c_userid AND        
                ...
                TaskManagerUserDetail.AreaKey =         
                  CASE WHEN ISNULL(RTRIM(@c_AreaKey01),'') = '' THEN AreaDetail.AreaKey ELSE @c_AreaKey01 END AND         
                TaskDetail.OrderKey NOT IN (SELECT OrderKey FROM @t_OrderWorkInProgress) AND     
                --TaskDetail.PickMethod = CASE WHEN ISNULL(@c_LastPickMethod,'')  = '' THEN TAskDetail.PickMethod ELSE @c_LastPickMethod END    
                TaskDetail.PickMethod = 'PP'     
         ORDER BY         
         ...
         BEGIN        
            ...
            SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Could not Open CUR_PICK_TASK. (nspTTMPK01)' + ' ( ' + ' SQLSvr MESSAGE=' + LTrim(RTrim(@c_errmsg)) + ' ) '        
         END                                             
         ELSE        
         BEGIN        
         ...
         BEGIN        
            WHILE (1=1)        
            BEGIN        
               FETCH NEXT FROM CUR_PICK_TASK INTO @c_taskdetailkey, @c_Area, @c_Aisle, @c_Priority, @c_OrderKey,        
               ...
               SELECT TOP 1        
                    ...
                    ,@n_Qty  = Qty         
                    ,@c_OrderKey = OrderKey     -- (ChewKP01)      
                    ,@c_PickMethod = PickMethod -- (ChewKP01)    
                    ,@c_LastOrderKey = OrderKey -- (ChewKP01)    
              FROM   TASKDETAIL WITH (NOLOCK)        
       ...
       END        
        ...
        -- Lock TaskDetail Picks by PickMethod -- (ChewKP01)      
        IF @n_continue=1 OR @n_continue=2        
        BEGIN      
            SET @c_LocAisle = ''    
            SELECT @c_LocAisle = LocAisle    
   ...
   END -- WHILE 1=1        
    ...
    BEGIN        
        ...
        BEGIN        
            ...
            END        
        END        
        EXECUTE nsp_logerror @n_err, @c_errmsg, 'nspTTMVNPK'        
