...
/************************************************************************/  
/* Stored Procedure: isp_GetPickSlipWave10                              */  
/* Creation Date: 28-APR-2014                                           */  
/* Copyright: IDS                                                       */  
/* Written by: YTWan                                                    */  
/*                                                                      */  
/* Purpose: SOS#309528 - ANF - Exceed Picking Slip                      */  
/*        : Rename from isp_GetWavePickSlip_10 to follow Print wave     */  
/*        : Pickslip SP standard name & sequence No                     */   
/*                                                                      */  
...
/* 09-MAY-2014  YTWan    1.0  Fixed to re-generate RefKeyLookup to sync */  
/*                            with Pickdetail; possible regen after     */  
/*                            reverse picktask(Wan01)                   */    
/* 23-Sep-2014  TLTING   Performance tune and Bug fix                   */
/* 28-Jan-2019  TLTING_ext 1.1  enlarge externorderkey field length      */
/************************************************************************/  
  
CREATE PROC [dbo].[isp_GetPickSlipWave10] (  
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         BEGIN      
            ...
            SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert PICKHEADER Failed (isp_GetPickSlipWave10)'   
      ...
      END                         
         ...
         IF EXISTS ( SELECT 1  
                     ...
                     AND NOT EXISTS ( SELECT 1  
                                      FROM REFKEYLOOKUP RKLK WITH (NOLOCK)   
                                      WHERE RKLK.PickDetailkey = PD.PickDetailKey )  
                   )  
...
QUIT:  
   ...
   BEGIN   
      ...
      BEGIN    
         ROLLBACK TRAN    
      END   
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'isp_GetPickSlipWave10'    
   END  
