...
/************************************************************************/        
/* Stored Proc : ispArchiveTransmitLog                                  */        
/* Creation Date:                                                       */        
/* Copyright: IDS                                                       */        
/* Written by: Wanyt                                                    */        
/*                                                                      */        
/* Purpose: Housekeeping Transmitlog table                              */        
/*                                                                      */        
...
BEGIN        
   ...
   DECLARE @n_retain_days        int, -- days to hold data        
           ...
           @n_archive_TL_records int -- No. of TransmitLog records to be archived        
   ...
   BEGIN        
      ...
      SELECT @local_c_errmsg =        
            ...
            " SQLSvr MESSAGE = " + dbo.fnc_LTrim(dbo.fnc_RTrim(@local_c_errmsg)) + ")" +' (ispArchiveTransmitLog) '        
   END        
   ...
   BEGIN        
      SELECT @b_success = 1        
      SELECT @c_temp = "Archive Of IDS TransmitLog Started with Parms; Datetype = " + dbo.fnc_RTrim(@c_datetype) +        
      ...
      EXECUTE nspLogAlert        
               @c_ModuleName   = "ispArchiveTransmitLog",        
   ...
   BEGIN        
      ...
      BEGIN        
         SELECT @c_temp =  ' AND TransmitLog.TransmitLogKey BETWEEN '+ '''' + dbo.fnc_RTrim(@c_TLStart) + '''' +' AND '+        
      ...
      BEGIN        
         SELECT @b_success = 1        
         EXEC nsp_BUILD_ARCHIVE_TABLE @c_copyfrom_db, @c_copyto_db, 'TransmitLog',@b_success OUTPUT, @n_err OUTPUT, @c_errmsg OUTPUT        
         IF NOT @b_success = 1        
         BEGIN        
            SELECT @dummy        
            SELECT @n_continue = 3        
      ...
      BEGIN        
         ...
         EXECUTE nspBuildAlterTableString @c_copyto_db,'TransmitLog',@b_success OUTPUT,@n_err OUTPUT, @c_errmsg OUTPUT        
         IF NOT @b_success = 1        
         BEGIN        
            SELECT @n_continue = 3        
         END        
      ...
      BEGIN        
         IF (@b_debug = 1)        
         BEGIN        
            PRINT "building alter table string for TransmitLog..."        
         END        
         EXECUTE nspBuildAlterTableString @c_copyto_db,'TransmitLog',@b_success OUTPUT,@n_err OUTPUT, @c_errmsg OUTPUT        
         IF NOT @b_success = 1        
         BEGIN        
      ...
      BEGIN        
         ...
         BEGIN        
           ...
           EXEC (        
            ...
            FETCH NEXT FROM CUR_TransmitLogkey INTO @cTransmitLogKey        
            ...
            BEGIN        
               ...
               BEGIN        
                  ...
                  SELECT @local_c_errmsg = CONVERT(char(5),@local_n_err)        
                  SELECT @local_c_errmsg =        
                  ": Update of Archivecop failed - CC Table. (ispArchiveTransmitLog) " + " ( " +        
         ...
         BEGIN        
            SELECT @c_temp = "Attempting to Archive " + dbo.fnc_RTrim(CONVERT(char(6),@n_archive_TL_records )) +        
                             " TransmitLog records and " + dbo.fnc_RTrim(CONVERT(char(6),@n_archive_TL_records )) + " TransmitLog records"        
            EXECUTE nspLogAlert        
                     @c_ModuleName   = "ispArchiveTransmitLog",        
                     @c_AlertMessage = @c_Temp ,        
                     @n_Severity     = 0,        
         ...
         BEGIN        
            SELECT @b_success = 1        
            EXEC nsp_BUILD_INSERT  @c_copyto_db, 'TransmitLog',1 ,@b_success OUTPUT, @n_err OUTPUT, @c_errmsg OUTPUT        
            IF NOT @b_success = 1        
            BEGIN        
               SELECT @n_continue = 3        
            END        
   ...
   BEGIN        
      ...
      EXECUTE nspLogAlert        
               @c_ModuleName   = "ispArchiveTransmitLog",        
               @c_AlertMessage = "Archive Of TransmitLog Ended Normally.",        
               @n_Severity     = 0,        
               @b_success      = @b_success OUTPUT,        
               @n_err          = @n_err OUTPUT,        
               @c_errmsg       = @c_errmsg OUTPUT        
   ...
   BEGIN        
      ...
      BEGIN        
         ...
         EXECUTE nspLogAlert        
       @c_ModuleName   = "ispArchiveTransmitLog",        
                  @c_AlertMessage = "Archive Of TransmitLog Ended Abnormally - Check This Log For Additional Messages.",        
                  @n_Severity     = 0,        
                  @b_success      = @b_success OUTPUT,        
                  @n_err          = @n_err OUTPUT,        
                  @c_errmsg       = @c_errmsg OUTPUT        
   ...
   BEGIN        
      ...
      END        
      EXECUTE nsp_logerror @n_err, @c_errmsg, "ispArchiveTransmitLog"        
