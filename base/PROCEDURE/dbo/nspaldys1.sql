...
GO
/************************************************************************/    
/* Stored Procedure: nspALDYS1                                          */    
/* Creation Date: 11-MAR-2017                                           */    
/* Copyright: LFL                                                       */    
/* Written by:                                                          */    
/*                                                                      */    
/* Purpose: WMS-1886 CN DYSON alloction by locationflag                 */
...
BEGIN    
   ...
   --NJOW01
   IF @c_ConsigneeStrategy = 'Y' 
   BEGIN   	   
   	   --Not to overallocation proceed to next pickcode
   	   IF (@c_UOM = '6' AND @c_LocationTypeOverride IN('PICK','CASE')) OR @c_UOM = '1'
   	      GOTO EXIT_SP
   END      
   ...
   SET @c_SQL = N'   
      ...
      CASE WHEN @c_UOM = '6' AND @c_LocationTypeOverride IN('PICK','CASE') THEN 
              ...
              --    ' AND LOC.LocationFlag IN (SELECT Long FROM CODELKUP (NOLOCK) WHERE Listname = ''DYSONSL2'' AND ISNULL(Short,'''') <> ''Y'' AND Storerkey = @c_Storerkey AND UDF01 = @c_Consigneekey) '
              --ELSE ' AND LOC.LocationFlag IN (SELECT Long FROM CODELKUP (NOLOCK) WHERE Listname = ''DYSONSL'' AND ISNULL(Short,'''') <> ''Y'') ' END 
              ' AND LOC.LocationFlag IN (SELECT Long FROM CODELKUP (NOLOCK) WHERE Listname = ''DYSONSL'' AND ISNULL(Short,'''') <> ''Y'') '
           ELSE ' ' END +
