...
/************************************************************************/  
/* Stored Procedure: nspPR_PH10                                         */  
/* Creation Date: 05-Apr-2021                                           */  
/* Copyright: LFL                                                       */  
/* Written by:                                                          */  
/*                                                                      */  
/* Purpose: WMS-16539 PH Mondelez allocation by FEFO                    */  
/*          Notes: Turn on configkey 'OrderInfo4Preallocation'          */  
/*          copy from nspPR_PH07                                        */
/*                                                                      */  
...
BEGIN   
   ...
   EXEC isp_Init_Preallocate_Candidates  
   ...
   BEGIN  
      ...
      --Sorting  
      SET @c_OrderBy = ' ORDER BY LOTATTRIBUTE.LOTTABLE04, LOTATTRIBUTE.Lottable05, LOT.Lot '
      ...
      */ 
      ...
      SELECT @c_SQLStmt = 'DECLARE CURSOR_LOTAVAI CURSOR FAST_FORWARD READ_ONLY FOR ' +  
      ' SELECT LOT.StorerKey, LOT.SKU, LOT.LOT, ' +  
      ' QtyAvailable = SUM(LOTXLOCXID.Qty-LOTXLOCXID.QtyAllocated-LOTXLOCXID.QtyPicked) - MIN(ISNULL(P.QtyPreallocated, 0)) ' +  
      ' FROM LOTXLOCXID (NOLOCK) ' +  
      ' JOIN LOT (NOLOCK) ON LOTXLOCXID.LOT = LOT.LOT ' +  
      ...
      FETCH NEXT FROM CURSOR_LOTAVAI INTO @c_Storerkey, @c_Sku, @c_LOT, @n_QtyAvailable   
      ...
      IF CURSOR_STATUS('GLOBAL' , 'CURSOR_LOTAVAI') in (0 , 1)          
      BEGIN          
         CLOSE CURSOR_LOTAVAI          
         DEALLOCATE CURSOR_LOTAVAI          
      END    
