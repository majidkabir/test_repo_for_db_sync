...
GO
/************************************************************************/    
/* Store Procedure:  isp_loadmani_mbol04                                */    
/* Creation Date:                                                       */    
/* Copyright: IDS                                                       */    
/* Written by: YTWan                                                    */    
/*                                                                      */    
...
/* Return Status:  None                                                 */    
/*                                                                      */    
/* Usage:  Used for report dw = r_dw_load_manifest_mbol04               */    
/*                                                                      */    
/* Local Variables:                                                     */    
/*                                                                      */    
/* Called By:                                                           */    
...
/* 04-Oct-2018  TLTING  1.2                                             */    
/* 12-Oct-2018  MuruH   1.3   WMS-6389 - Add Priority field and change  */  
/*                            DeliveryDate format to DD/MM/YYYY (MH01)  */    
/************************************************************************/    
CREATE PROC [dbo].[isp_loadmani_mbol04] (    
...
BEGIN    
    ...
    DECLARE @n_continue       INT     
         ...
         ,  @c_Priority       NVARCHAR(250) -- (MH01)    
         ,  @c_UserDefine10   NVARCHAR(10)  -- (MH01)    
         ,  @c_UserDefine03   NVARCHAR(20)  -- (MH01) 
		 ,  @c_DocType        NVARCHAR(1)   -- (MH01)
...
,  @n_ShowMultiExtSO INT    
   ...
   CREATE TABLE #TMP_LOAD    
    ...
    --,  DeliveryDate         DATETIME         -- (MH01)   
      ,  DeliveryDate         NVARCHAR(11)     -- (MH01)   
      ...
      ,  Cartons              INT    
      ,  LM_SG                NVARCHAR(1)    
      ,  Priority             NVARCHAR(250)    -- (MH01)  
	  ,  UserDefine10         NVARCHAR(11)     -- (MH01)  
      ,  UserDefine03         NVARCHAR(11)     -- (MH01)  
      )    
   ...
   BEGIN    
      ...
      SET @c_Priority     = ''    -- (MH01) 
      SET @c_UserDefine10 = ''    -- (MH01) 
      SET @c_UserDefine03 = ''    -- (MH01) 
      SET @c_DocType      = ''    -- (MH01) 
      ...
      AND   Long     = 'r_dw_load_manifest_mbol04'    
      ...
      SELECT TOP 1    
            ...
            ,@c_Priority     = ISNULL(CODELKUP.Long,'')      -- (MH01)  
            ,@c_UserDefine10 = ISNULL(OH.UserDefine10, '')   -- (MH01) 
            ,@c_UserDefine03 = ISNULL(OH.UserDefine03, '')   -- (MH01)   
			,@c_DocType      = ISNULL(OH.DocType , '')       -- (MH01)         
      FROM ORDERS OH  WITH (NOLOCK)    
   LEFT JOIN CODELKUP WITH (NOLOCK) ON CODELKUP.LISTNAME = 'ORDRPRIOR' AND CODELKUP.code = OH.Priority -- (MH01)    
      ...
      VALUES    
            ...
            ,  @c_MultiExternSO    
            ,  @c_Consigneekey    
        --  ,  @dt_DeliveryDate  -- (MH01)    
			, CASE WHEN @c_DocType = 'E' OR ISNULL(@c_Userdefine10, '') = '' 
                   THEN REPLACE( CONVERT(NVARCHAR(11), @dt_DeliveryDate, 106), ' ', '-')  ELSE '' END   -- (CJY01)    		     
			,  @c_c_Company    
            ,  @c_c_Address1    
            ,  @c_c_Address2    
            ,  @c_c_Address3    
            ...
            ,  @c_Priority -- (MH01)    
			,CASE WHEN @c_DocType = 'N' AND ISNULL(@c_UserDefine10, '') <> '' 
				  THEN REPLACE( CONVERT(NVARCHAR(11), CAST(@c_UserDefine10 AS DATETIME), 106), ' ', '-') ELSE '' END   -- (CJY01)  
			,CASE WHEN ISNULL(@c_UserDefine03, '') <> ''   
				  THEN REPLACE( CONVERT(NVARCHAR(11), CAST(@c_UserDefine03 AS DATETIME), 106), ' ', '-') ELSE '' END   -- (CJY01)   
            )    
   ...
   BEGIN      
      ...
      BEGIN      
         ...
         END      
      END      
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'isp_loadmani_mbol04'      
