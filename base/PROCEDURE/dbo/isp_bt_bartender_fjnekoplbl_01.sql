...
/******************************************************************************/
/* Copyright: LFL                                                             */
/* Purpose: isp_BT_Bartender_FJNekoPLBL_01                                    */
/*                                                                            */
/* Modifications log:                                                         */
/*                                                                            */
/* Date       Rev  Author     Purposes                                        */
/* 2020-07-28 1.0  WLChooi    Created (WMS-14458)                             */
/* 2021-04-02 1.1  CSCHONG    WMS-16024 PB-Standardize TrackingN (CS01)       */  
/******************************************************************************/

CREATE PROC [dbo].[isp_BT_Bartender_FJNekoPLBL_01]
...
BEGIN                        
   ...
   SET @c_SQLJOIN = '  SELECT DISTINCT ORD.Orderkey, ORD.ExternOrderKey, ORD.Consigneekey, ORD.C_contact1, ORD.C_contact2 ' + CHAR(13)   --5   --WL01
                  ...
                  + ', ISNULL(ORD.C_address3,''''), (Substring(ORD.TrackingNo,1,4) + ''-'' + Substring(ORD.TrackingNo,5,4)  +''-'' +  Substring(ORD.TrackingNo,9,4)) ' + CHAR(13)   --10
                  ...
                  + ', ORD.TrackingNo, CONVERT(NVARCHAR(80), ORD.InvoiceAmount), CONVERT(NVARCHAR(10), getdate(),112) ' + CHAR(13)   --35
                  +N', Substring(CONVERT(VARCHAR(10), dateadd(day, 30, getdate()),112),3,2) + N''年'' + '
                  +N'  Substring(CONVERT(VARCHAR(10), dateadd(day, 30, getdate()),112),5,2) + N''月'' + '
                  +N'  Substring(CONVERT(VARCHAR(10), dateadd(day, 30, getdate()),112),7,2) + N''日迄'', PAH.PickSlipNo ' + CHAR(13)   --37
                  + ', ORD.UserDefine09, PAH.TTLCNTS, PAH.CtnTyp1, PAH.CtnTyp2, CONCAT(ORD.Dischargeplace ,(ORD.Dischargeplace - (CONVERT(INT,ORD.Dischargeplace / 7) * 7))) ' + CHAR(13)   --42
                  + ', Substring(ORD.Dischargeplace,2,2) + ''-'' + Substring(ORD.Dischargeplace,4,2) + ''-'' + Substring(ORD.Dischargeplace,6,2) ' + CHAR(13)   --43
                  + ', ISNULL(ST.B_Phone2,''''), PAD.DropID ' + CHAR(13)   --45
                  + ', (SELECT SUM(PD.Qty) FROM PACKDETAIL PD (NOLOCK) WHERE PD.PickSlipNo = PAH.PickSlipNo AND PD.CartonNo = PAD.CartonNo) ' + CHAR(13)   --46
                  ...
                  + ', ISNULL(ST.B_Phone1,''''), ISNULL(ST.Notes1,'''') ' + CHAR(13)   --58
                  + ', ISNULL(ST.Notes2,''''), '''' ' + CHAR(13)   --60
   ...
   SET @c_SQLGroup = ' GROUP BY ORD.Orderkey, ORD.ExternOrderKey, ORD.Consigneekey, ORD.C_contact1, ORD.C_contact2 ' + CHAR(13) 
                  + ', ORD.C_company, ISNULL(ORD.C_address1,''''), ISNULL(ORD.C_address2,'''') ' + CHAR(13)  
                  + ', ISNULL(ORD.C_address3,''''), (Substring(ORD.TrackingNo,1,4) + ''-'' + Substring(ORD.TrackingNo,5,4)  +''-'' +  Substring(ORD.TrackingNo,9,4)) ' + CHAR(13) 
                  ...
                  + ', ISNULL(ORD.DischargePlace,''''), ORD.Loadkey, ORD.Mbolkey, ISNULL(ORD.trackingno,'''') ' + CHAR(13)  --CS01
                  + ', ORD.TrackingNo, CONVERT(NVARCHAR(80), ORD.InvoiceAmount), PAH.PickSlipNo, PAD.CartonNo ' + CHAR(13)  
                  + ', ORD.UserDefine09, PAH.TTLCNTS, PAH.CtnTyp1, PAH.CtnTyp2, CONCAT(ORD.Dischargeplace ,(ORD.Dischargeplace - (CONVERT(INT,ORD.Dischargeplace / 7) * 7))) ' + CHAR(13) 
                  + ', Substring(ORD.Dischargeplace,2,2) + ''-'' + Substring(ORD.Dischargeplace,4,2) + ''-'' + Substring(ORD.Dischargeplace,6,2) ' + CHAR(13)  
                  + ', ISNULL(ST.B_Phone2,''''), PAD.DropID ' + CHAR(13) 
                  ...
                  + ', ISNULL(ST.B_Phone1,''''), ISNULL(ST.Notes1,''''), ISNULL(ST.Notes2,'''') '
 ...
 PRINT @c_SQLJOIN
   ...
   SET @c_SQL = @c_SQL + @c_SQLJOIN + @c_SQLGroup      
