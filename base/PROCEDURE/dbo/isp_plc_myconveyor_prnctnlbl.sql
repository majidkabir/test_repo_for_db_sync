...
GO
/************************************************************************/
/* Store Procedure:  isp_PLC_MYConveyor_PrnCtnLbl                       */
/* Creation Date: 29-MAY-2023                                           */
/* Copyright: MAERSK                                                    */
/* Written by:                                                          */
/*                                                                      */
/* Purpose: WMS-22682 MY PLC Conveyor auto print carton label           */
/*          refer to WMS-17432                                          */
/*													                                            */
/*                                                                      */
...
/* 29-MAY-2023 NJOW    1.0  DEVOPS combine script                       */
/************************************************************************/

CREATE   PROCEDURE [dbo].[isp_PLC_MYConveyor_PrnCtnLbl](
           @n_SerialNo           INT
...
BEGIN
   ...
   BEGIN
      ...
      SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),@n_Err) 
                    + ': DefaultPrinter is not setup for RDT User: ' + RTRIM(ISNULL(@c_RDTUserName,'')) + '. (isp_PLC_MYConveyor_PrnCtnLbl)'
      GOTO QUIT
   END
   ...
   BEGIN
      ...
      SET @n_Err = 88510
      SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),@n_Err) 
                    + ': DefaultStorer is not setup for RDT User: ' + RTRIM(ISNULL(@c_RDTUserName,'')) + '. (isp_PLC_MYConveyor_PrnCtnLbl)'
      GOTO QUIT
   END*/
   ...
   BEGIN
      ...
      SET @n_Err = 88520
      SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),@n_Err) 
                    + ': Invalid CartonID: ' + RTRIM(ISNULL(@c_CartonID,'')) + '. (isp_PLC_MYConveyor_PrnCtnLbl)'
      GOTO QUIT
   END 
