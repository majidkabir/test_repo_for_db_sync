...
/*************************************************************************/    
/* Stored Procedure: ispRVWAV40                                          */    
/* Creation Date: 08-Mar-2020                                            */    
/* Copyright: LFL                                                        */    
/* Written by: WLChooi                                                   */    
/*                                                                       */    
/* Purpose: WMS-16304 - [CN] ANFQHW_WMS_ReverseWaveReleased              */    
/*                                                                       */    
/* Called By: Wave                                                       */    
/*                                                                       */    
/* GitLab Version: 1.0                                                   */    
...
CREATE PROCEDURE [dbo].[ispRVWAV40]      
 ...
 BEGIN  
    ...
    BEGIN
       SELECT @c_TMReleaseFlag = TMReleaseFlag
       FROM WAVE (NOLOCK) 
       WHERE WaveKey = @c_wavekey
       
       IF NOT EXISTS (SELECT 1 FROM TASKDETAIL TD (NOLOCK) 
                   WHERE TD.Wavekey = @c_Wavekey
                   AND TD.Sourcetype IN ('ispRLWAV40-B2B','ispRLWAV40-DC')
                   AND TD.Tasktype IN ('RPF')) 
       BEGIN
         IF @c_TMReleaseFlag = 'Y'
         BEGIN 
            GOTO UPDATE_TMRELEASEFLAG
         END 
         ELSE
         BEGIN
            SELECT @n_continue = 3  
            SELECT @n_err = 81010  
            SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': This Wave has not been released. (ispRVWAV40)'         
         END
       END                 
    END
    ...
    BEGIN
        ...
        BEGIN
          SELECT @n_continue = 3  
          SELECT @n_err = 81020  
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Some Tasks have been started. Not allow to Reverse Wave Released (ispRVWAV40)'       
        END                 
    END
    ...
    BEGIN
         ...
         AND TASKDETAIL.Sourcetype IN ('ispRLWAV40-B2B','ispRLWAV40-DC')
         ...
         BEGIN
           ...
           SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Delete Taskdetail Table Failed. (ispRVWAV40)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
         END 
    END
    ...
    BEGIN
         ...
         BEGIN
           ...
           SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Delete rdtPTLStationLog Table Failed. (ispRVWAV40)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
         END 
    END
    ...
    BEGIN
         ...
         BEGIN
           ...
           SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Pickdetail Table Failed. (ispRVWAV40)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
         END          
    END    

UPDATE_TMRELEASEFLAG:
    ...
    BEGIN  
      ...
      WHERE WAVEKEY = @c_wavekey  
       ...
       BEGIN  
          ...
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update on wave Failed (ispRVWAV40)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
       END  
    END  
...
RETURN_SP:
    ...
    BEGIN  
       ...
       BEGIN  
          ...
          END  
       END  
       execute nsp_logerror @n_err, @c_errmsg, "ispRVWAV40"  
