...
/*************************************************************************/    
/* Stored Procedure: [isp_ExcelLoader_DelPartitionInsert]                */    
...
/*************************************************************************/    
CREATE PROC [dbo].[isp_ExcelLoader_DelPartitionInsert] (    
...
BEGIN    
   ...
   BEGIN  
      ...
      SET @b_Success = 0
      SET @n_ErrNo = 730001    
      SET @c_ErrMsg = N'Invalid Batch No. (isp_ExcelLoader_DelPartitionInsert)'    
      GOTO QUIT  
   END  
   ...
   BEGIN  
      ...
      SET @b_Success = 0
      SET @n_ErrNo = 730002  
      SET @c_ErrMsg = N'Staging or Post TableName Cannot be Null or Empty.(isp_ExcelLoader_DelPartitionInsert)'    
      GOTO QUIT  
   END  
   ...
   BEGIN  
      ...
      SET @b_Success = 0
      SET @n_ErrNo = 730003  
      SET @c_ErrMsg = N'PrimaryKey Cannot be empty!(isp_ExcelLoader_DelPartitionInsert)'    
      GOTO QUIT  
   END  
   ...
   BEGIN TRY  
      ...
      SET @SQL_WITH = N' ;WITH Result AS (' +  
                      ...
                      N' FETCH NEXT @n_Limit ROWS ONLY)' + CHAR(13) + CHAR(10)  
      
      SET @SQL = N''
      SET @SQL = N' IF @n_Offset = 0' + CHAR(13) + CHAR(10) +   
                 N' BEGIN' + CHAR(13) + CHAR(10) + CHAR(9)+   
                 N' IF EXISTS(SELECT 1 FROM ' + @c_POSTTableName + ' WITH (NOLOCK))' + CHAR(13) + CHAR(10) + CHAR(9)+  
                 N' BEGIN' + CHAR(13) + CHAR(10) + CHAR(9)+CHAR(9)+
                 N' IF EXISTS(SELECT 1 FROM ' + @c_STGTableName + ' WITH (NOLOCK)' + CHAR(13) + CHAR(10) + CHAR(9)+ CHAR(9)+
                 N' WHERE STG_BatchNo = @c_BatchNo)' + CHAR(13) + CHAR(10) + CHAR(9)+ CHAR(9)+ --STG_Status IN (''0'', ''1'') AND
                 N' BEGIN' + CHAR(13) + CHAR(10) + CHAR(9)+CHAR(9)+ CHAR(9)+
                 N' DELETE FROM ' + @c_POSTTableName + CHAR(13) + CHAR(10) + CHAR(9)+ CHAR(9)+
                 ...
                 N' ON '+  @c_WhereParams + CHAR(13) + CHAR(10)+ CHAR(9) +
                 N' WHERE TableA.STG_Status IN (''0'', ''1'')' + CHAR(13) + CHAR(10)+ CHAR(13) + CHAR(10)
      ...
      SET @SQL = @SQL_WITH +CHAR(13) + CHAR(10)+  
                 ...
                 N' WHERE r.row_count = 1 AND r.STG_Status NOT IN (''5'',''9'')' + CHAR(13) + CHAR(10) +CHAR(13) + CHAR(10) --
         
      EXEC sp_executesql @SQL, N'@c_BatchNo INT, @n_Offset INT, @n_Limit INT ',
                                 @c_BatchNo, @n_Offset,@n_Limit 
      ...
      BEGIN  
         ...
         SET @SQL = N' IF @n_RowCnt >= 1' + CHAR(13) + CHAR(10)+  
                    ...
                    N' CASE WHEN row_count = 1' + CHAR(13) + CHAR(10)+ CHAR(9)+  
                    N' THEN '''' ELSE ''Duplicated Records Occurred!'' END ' + CHAR(13) + CHAR(10)+ CHAR(9)+  
                    N' WHERE STG_Status IN (''0'', ''1'')'+ CHAR(13) + CHAR(10)+ CHAR(9)+  
                    N' END' +CHAR(13) + CHAR(10)
   ...
   BEGIN CATCH  
      ...
      SET @c_ErrMsg = ERROR_MESSAGE() + ' (isp_ExcelLoader_DelPartitionInsert)'    
      GOTO QUIT  
   END CATCH  
