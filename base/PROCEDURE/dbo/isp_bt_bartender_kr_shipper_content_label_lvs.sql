...
CREATE PROC [dbo].[isp_BT_Bartender_KR_Shipper_Content_Label_LVS]                                 
...
--          ,orddet.sku    
--          ,s.descr    
--         ,SUM(Pdet.qty)        --WinSern  
--   FROM   orderdetail orddet WITH (NOLOCK)   
--    join pickdetail pdet with (nolock) on pdet.orderkey = orddet.orderkey AND pdet.sku=orddet.sku and pdet.orderlinenumber=orddet.orderlinenumber       --WinSern  
--          join packheader ph WITH (NOLOCK) ON PH.Orderkey = orddet.Orderkey            
--          JOIN packdetail pd WITH (NOLOCK) ON PH.Pickslipno = PD.Pickslipno AND pd.sku=orddet.sku and pd.labelno=pdet.caseid     --WinSern  
--          JOIN sku s WITH (NOLOCK)        
--               ON  s.sku = orddet.sku        
--   AND             s.StorerKey = orddet.Storerkey        
--   WHERE  orddet.orderkey = @C_Getorderkey     
--   and pdet.status='5'     --WinSern  
--    AND PD.Cartonno >= CONVERT(INT,@c_Sparm2)    
--           AND PD.Cartonno <= CONVERT(INT,@c_Sparm3)         
...
--             ,OD.sku    
  ...
  FETCH NEXT FROM CUR_RowNoLoop INTO @c_GetExternORDKey,@C_GetExternPOKey,@C_Getbuyerpo,@C_Getorderkey,    
   ...
   EXEC isp_InsertTraceInfo             
      ...
      @c_TraceName = 'isp_BT_Bartender_KR_Shipper_Content_Label_UA',            
