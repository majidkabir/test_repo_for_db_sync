...
/***************************************************************************/      
/* Stored Procedure: isp_RPT_LP_PLISTN_034                                 */      
/* Creation Date: 14-APRIL-2022                                            */    
/* Copyright: LF Logistics                                                 */    
/* Written by: WZPang                                                      */    
/*                                                                         */    
/* Purpose: WMS-21913 - [ID] Diversey û LoadSheet                          */         
/*                                                                         */      
...
/* 22-Mar-2023  WZPang   1.0  DevOps Combine Script                        */
/***************************************************************************/  
CREATE   PROC [dbo].[isp_RPT_LP_PLISTN_034] (
 ...
 BEGIN    
   ...
   DECLARE @c_pickheaderkey        NVARCHAR(10),    
          ...
          , @c_Type            NVARCHAR(1) = '1'                      
          , @c_DataWindow      NVARCHAR(60) = 'RPT_LP_PLISTN_034'  
   ...
   BEGIN    
      SELECT @n_continue = 3      
      SELECT @n_err = 63500    
      SELECT @c_errmsg = 'NSQL'+CONVERT(char(5),@n_err)+': Pickslip already printed using Consolidated option. (isp_RPT_LP_PLISTN_034)'    
   END    
   -- end   
   ...
   BEGIN    
      ...
      BEGIN
         ...
         LEFT JOIN CODELKUP CLR (NOLOCK) ON (O.Storerkey = CLR.Storerkey AND CLR.Code = 'NOSPLITBYLINECNTZONE'     
                                             AND CLR.Listname = 'REPORTCFG' AND CLR.Long = 'RPT_LP_PLISTN_034' AND ISNULL(CLR.Short,'') <> 'N')    
         ...
         BEGIN -- while 1      
            ...
            BEGIN            
               ...
               BEGIN      
                  ...
                  BEGIN      
                     SELECT @n_continue = 3      
                     SELECT @n_err = 63501    
                 SELECT @c_errmsg = 'NSQL'+CONVERT(char(5),@n_err)+': Insert into PICKHEADER Failed. (isp_RPT_LP_PLISTN_034)'    
                     GOTO FAILURE    
                  END                 
               END -- @b_success = 1        
               ELSE       
               BEGIN      
                  SELECT @n_continue = 3      
                  SELECT @n_err = 63502    
                 SELECT @c_errmsg = 'NSQL'+CONVERT(char(5),@n_err)+': Get PSNO Failed. (isp_RPT_LP_PLISTN_034)'      
                  BREAK       
               END       
               ...
               BEGIN      
                  ...
                  BEGIN       
                     ...
                     BEGIN      
                        SELECT @n_continue = 3    
                        SELECT @n_err = 63503    
                      SELECT @c_errmsg = 'NSQL'+CONVERT(char(5),@n_err)+': Insert RefKeyLookup Failed. (isp_RPT_LP_PLISTN_034)'        
                        GOTO FAILURE    
                     END                              
                  END       
            ...
            BEGIN      
               SELECT @n_continue = 3      
               SELECT @n_err = 63504    
               SELECT @c_errmsg = 'NSQL'+CONVERT(char(5),@n_err)+': Update #Temp_Pick Failed. (isp_RPT_LP_PLISTN_034)'        
               GOTO FAILURE    
            END         
 ...
 SUCCESS:    
   ...
   BEGIN    
      ...
      BEGIN    
         ...
         END    
       END    
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'isp_RPT_LP_PLISTN_034'    
