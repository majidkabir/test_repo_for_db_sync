         ...
         WHERE AreaDetail.AreaKey = @c_AreaKey01
            ...
            AND NOT EXISTS( SELECT 1 
               FROM dbo.TaskDetail TD WITH (NOLOCK) 
                  JOIN LOC LOC1 WITH (NOLOCK) ON (LOC1.Loc = TD.FromLOC)
               WHERE LOC1.Facility = LOC.Facility
                  AND LOC1.LocAisle = LOC.LocAisle
                  AND LOC.LocAisle <> ''  
                  AND LOC1.LocAisle <> ''
   ...
   BEGIN
      ...
      BEGIN
        ...
        BEGIN
          ...
          END
        END
        execute nsp_logerror @n_err, @c_errmsg, 'nspTTMRP09'
