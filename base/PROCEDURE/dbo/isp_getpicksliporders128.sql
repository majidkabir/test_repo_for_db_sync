...
/************************************************************************/
/* Stored Procedure: isp_GetPickSlipOrders128                           */
/* Creation Date: 2022-02-25                                            */
/* Copyright: LFL                                                       */
/* Written by: CSCHONG                                                  */
/*                                                                      */
/* Purpose: WMS-18966 - PH_IDSMED Consolidated Picklist_CR              */
/*          Copy and modify from nsp_GetPickSlipOrders123               */
/*                                                                      */
/* Called By: r_dw_print_pickorder128                                   */
/*                                                                      */
/* GitLab Version: 1.0                                                  */
/*                                                                      */
/* Version: 5.4                                                         */
...
CREATE PROC [dbo].[isp_GetPickSlipOrders128] (@c_loadkey NVARCHAR(10))
 AS
BEGIN
   ...
   BEGIN
      ...
      SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Failed On Table Pickheader Table. (isp_GetPickSlipOrders128)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '
   END
   ...
   BEGIN
      ...
      LEFT OUTER JOIN Codelkup CLR (NOLOCK) ON (ORDERS.Storerkey = CLR.Storerkey AND CLR.Code = 'SHOWORDERGRP'
                                    AND CLR.Listname = 'REPORTCFG' AND CLR.Long = 'r_dw_print_pickorder128' AND ISNULL(CLR.Short,'') <> 'N')
      ...
      BEGIN
         ...
         AND   Long = 'r_dw_print_pickorder128'
         AND Code = 'showfield'
         AND   ISNULL(Short,'') <> 'N'
         ...
         INSERT INTO @t_Result (Loadkey, Pickslipno, PickType, LoadingDate, PickZone, Loc, LogicalLoc, SKU, Descr,
                  ...
                  Lottable08, Lottable10, Lottable12, SKUGROUP, itemclass,TTLEACHES)
         VALUES (@c_Loadkey, '', @c_PickType, @d_LoadDate, @c_PickZone, @c_LOC, @c_LogicalLoc, @c_SKU, @c_SkuDescr,
                 ...
                 @c_Lottable08, @c_Lottable10, @c_Lottable12, @c_SKUGroup, @c_SKUItemClass,@n_ttleaches)
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert Failed On Table PICKHEADER. (isp_GetPickSlipOrders128)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '
            END
         END
            ...
            BEGIN
               ...
               BEGIN
                  ...
                  BEGIN
                     SELECT @n_continue = 3
                     SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err=73001
                     SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert Failed On Table RefkeyLookup. (isp_GetPickSlipOrders128)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '
                  END
                  ...
                  BEGIN
                     ...
                     BEGIN
                        SELECT @n_continue = 3
                        SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err=73001
                        SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Failed On Table PICKDETAIL. (isp_GetPickSlipOrders128)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '
                     END
                  END
               END
