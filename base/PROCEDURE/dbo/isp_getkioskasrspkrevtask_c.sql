...
/************************************************************************/  
/* Stored Procedure: isp_GetKIOSKASRSPKRevTask_c                        */  
...
/*                                                                      */  
/* Called By: d_dw_kiosk_asrspk_rev_c_form                              */  
/*                                                                      */  
/* PVCS Version: 1.2                                                    */  
/*                                                                      */  
/* Version: 7.0                                                         */  
...
/************************************************************************/  
CREATE PROC [dbo].[isp_GetKIOSKASRSPKRevTask_c]  
...
,    Remarks             NVARCHAR(80)  NOT NULL DEFAULT ('')      --(Wan02)
   ...
   INSERT INTO #Result (TaskdetailKey,FROMID,TOID,PalletID,OrderKey,OrderStatus,OrderLineNumber,Storerkey,SKU,SKUDescr,SUSR4
                        ,Lot,LottableCode
                        ...
                        ,CaseCnt,PackUOM1,PackUOM3,QtyToPutInCS,QtyToPutInEA,QtyInCS,QtyInEA,PackSize
                        ,TaskStatus,JobStatus,Remarks)                                              --(Wan02)
   SELECT TOP 1 TASKDETAIL.TaskdetailKey
      ...
      ,QtyToPutInCS    = 0--CASE WHEN PACK.CaseCnt > 0 AND PICKDETAIL.Status = '0' AND @n_ShowUOMEA = 0 THEN FLOOR((@n_Qty - @n_QtyToPut) / PACK.CaseCnt) ELSE 0 END  
      ,QtyToPutInEA    = 0--CASE WHEN PACK.CaseCnt > 0 AND PICKDETAIL.Status = '0' AND @n_ShowUOMEA = 0 THEN (@n_Qty - @n_QtyToPut) % CONVERT(INT,PACK.CaseCnt) ELSE (@n_Qty - @n_QtyToPut) END
      ,QtyInCS         = 0
      ,QtyInEA         = 0
      ,PackSize        = '1' + ' ' + RTRIM(PACK.PackUOM1) + ' ' + '=' 
                       + ' ' + convert(nvarchar(5),PACK.CaseCnt) + ' '+  RTRIM(PACK.PackUOM3)
      ,TaskStatus = @c_TaskStatus --ISNULL(PICKDETAIL.Status,'')
      ,JobStatus  = @c_JobStatus
      ,Remarks = CASE WHEN TASKDETAIL.Message02 = '' THEN ''                                       --(Wan02)
                      ELSE 'COPACK ITEM WITH ' +  RTRIM(TASKDETAIL.Message02) END                  --(Wan02)
   ...
   SET @n_QtyToPut = 0
   SELECT @n_QtyToPut = ISNULL(SUM(PICKDETAIL.Qty),0)
   FROM #RESULT 
   ...
   UPDATE #RESULT
   SET QtyToPutInCS    = CASE WHEN CaseCnt > 0 AND @n_ShowUOMEA = 0 THEN FLOOR((@n_Qty - @n_QtyToPut) / CaseCnt) ELSE 0 END  
      ,QtyToPutInEA    = CASE WHEN CaseCnt > 0 AND @n_ShowUOMEA = 0 THEN (@n_Qty - @n_QtyToPut) % CONVERT(INT,CaseCnt) ELSE (@n_Qty - @n_QtyToPut) END
   ...
   SELECT @c_JobKey,TaskdetailKey,FROMID,TOID,PalletID,OrderKey,OrderStatus,OrderLineNumber 
         ...
         ,Lottable06Code,Lottable07Code,Lottable08Code,Lottable09Code,Lottable10Code
         ,Lottable11Code,Lottable12Code,Lottable13Code,Lottable14Code,Lottable15Code
         ,CaseCnt,PackUOM1,PackUOM3,QtyToPutInCS,QtyToPutInEA 
         ,QtyInCS,QtyInEA,Packsize,TaskStatus,JobStatus
         ,Remarks                                                                                  --(Wan02)
   FROM #Result 
END         
