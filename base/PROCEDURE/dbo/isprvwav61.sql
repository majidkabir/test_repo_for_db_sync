...
/*************************************************************************/
/* Stored Procedure: ispRVWAV61                                          */
/* Creation Date: 30-MAY-2023                                            */
/* Copyright: MAERSK                                                     */
/* Written by:                                                           */
/*                                                                       */
/* Purpose: WMS-22724 - CN ANTA Reverse Released wave                    */
/*                                                                       */
/* Called By: wave                                                       */
/*                                                                       */
/* PVCS Version: 1.0                                                     */
...
/*************************************************************************/
CREATE   PROCEDURE [dbo].[ispRVWAV61]
 ...
 BEGIN
    ...
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM REPLENISHMENT RP (NOLOCK)
                       WHERE RP.Wavekey = @c_Wavekey AND RP.OriginalFromLoc = 'ispRLWAV61')
        BEGIN
          SELECT @n_continue = 3
          SELECT @n_err = 81020
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Nothing to reverse. (ispRVWAV61)'
        END
    END
    ...
    BEGIN
        ...
        BEGIN
          SELECT @n_continue = 3
          SELECT @n_err = 81030
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Some Tasks have been started. Not allow to Reverse Wave Released (ispRVWAV61)'
        END
    END
    ...
    BEGIN
       DECLARE CUR_REPL CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
         ...
         AND OriginalFromLoc = 'ispRLWAV61'
         AND Confirmed = 'N'
       ...
       BEGIN
          ...
          BEGIN
            ...
            SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Delete Replenishment Table Failed. (ispRVWAV61)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '
          END
    ...
    BEGIN
       ...
       BEGIN
          ...
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update on wave Failed (ispRVWAV61)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '
       END
    END
...
RETURN_SP:
    ...
    BEGIN
       ...
       BEGIN
          ...
          END
       END
       execute nsp_logerror @n_err, @c_errmsg, "ispRVWAV61"
