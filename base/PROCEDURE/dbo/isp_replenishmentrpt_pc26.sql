...
/***************************************************************************/    
/* Stored Procedure: isp_ReplenishmentRpt_PC26                             */    
/* Creation Date: 28-Aug-2018                                              */    
/* Copyright: LFL                                                          */    
/* Written by:                                                             */    
/*                                                                         */    
/* Purpose: WMS-6777 TW MHT Replenishment Report.                          */    
/*                                                                         */    
/*                                                                         */    
/* Called By: Replenishment Report r_replenishment_report_PC26             */    
/*                                                                         */    
/* PVCS Version: 1.1                                                       */    
/*                                                                         */    
/* Version: 7.0                                                            */    
...
BEGIN    
   ...
   DECLARE CUR_SKUXLOC CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
      SELECT SKUxLOC.Storerkey, SKUxLOC.Sku, SKUxLOC.Loc, LOC.Facility,
             ...
             LOC.NoMixLottable14, 
             LOC.NoMixLottable15,  
             ISNULL(EXT.QtyExpected,0) --NJOW02           
             --SKUXLoc.QtyExpected 
      From SKUxLOC (NOLOCK)   
      ...
      AND ((SKUxLOC.Qty - SKUxLOC.QtyAllocated) + ISNULL(EXT.PendingMoveIn,0) < SKUxLOC.QtyLocationMinimum
           OR ISNULL(EXT.QtyExpected,0) > 0) --NJOW02
         ...
         BEGIN
           ...
           EXEC sp_executesql @c_SQL,
            ...
            BEGIN
                ...
                IF (@c_PrevLoc <> @c_FromLoc OR @c_PrevId <> @c_FromID) AND @n_FromQty > @n_ReplenQty AND @n_OverAllocateQty = 0 --NJOW02
                BEGIN
                   SET @n_ReplenQty = 0
                   BREAK   
                END
