   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            SELECT @c_errmsg = 'NSQL' + Convert(CHAR(5),@n_err) + ': Select Error On LOTxLOCxID.' + ' (nspRFTPA02) (SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ')'
         END
         ELSE IF @n_cnt = 0  -- For some reason there's no match (maybe qty = 0)
         BEGIN
            SELECT @n_continue = 3 
            SELECT @n_err = 67794--84002
            SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': No Match For ID At Loc (nspRFTPA02)'
         END
         ELSE IF @n_cnt > 1  -- ID belongs to a multi-lot pallet
         BEGIN
            SELECT @n_continue = 3 
            SELECT @n_err = 67795--84004
            SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': You Cannot Specify A QTY To Move When Moving A Multi-Lot Pallet (nspRFTPA02)'
         END
         ELSE
         BEGIN
            ...
            BEGIN
               ...
               SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Qty Specified Exceeds Qty On Pallet (nspRFTPA02)'
            END
...
--               SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Cannot PutAway Partial Pallet (nspRFTPA02)'
            END
          ...
          BEGIN
           ...
           END
            ...
            BEGIN
               ...
               SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Update Failed On Table TaskDetail. (nspRFTRP01)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg), '') + ' ) '
            END
  ...
  BEGIN
      ...
      BEGIN
        ...
        BEGIN
          ...
          END
        END
        execute nsp_logerror @n_err, @c_errmsg, 'nspRFTPA02'
