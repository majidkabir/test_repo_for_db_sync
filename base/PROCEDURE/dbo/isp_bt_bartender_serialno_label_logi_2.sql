...
/* 2017-10-25 1.2  SWT01      Performance Tuning                              */
/******************************************************************************/                      
                        
CREATE PROC [dbo].[isp_BT_Bartender_SerialNo_Label_LOGI_2]                             
...
--      DROP TABLE #PICK;      
   ...
   SET @n_StartCartonNo = CONVERT(INT,@c_Sparm2)
   SET @n_EndCartonNo   = CONVERT(INT,@c_Sparm3)  

   INSERT INTO #TEMPMasterSN (SerialNo, MSerialNo, Pickslipno, cartonno, RecGrp)
	...
	JOIN MasterSerialNo MSN WITH (NOLOCK) ON MSN.ParentSerialNo = SN.SerialNo
	JOIN PACKHEADER PH WITH (NOLOCK) ON PH.PickSlipNo=PD.PickSlipNo
	JOIN ORDERS ORD WITH (NOLOCK) ON ord.OrderKey=PH.OrderKey
	JOIN CODELKUP C WITH (NOLOCK) ON c.listname='LOGAUSLBL' AND C.Code=ORD.ConsigneeKey
   WHERE PD.PickSlipNo = @c_Sparm1
   AND	PD.CartonNo BETWEEN @n_StartCartonNo AND @n_EndCartonNo       
   ORDER BY SN.SerialNo --MSN.SerialNo
                     
   DECLARE CUR_StartRecLoop CURSOR LOCAL FAST_FORWARD READ_ONLY FOR                  
   SELECT DISTINCT PDET.Pickslipno as PSlipno, 
         PDET.sku as sku, 
         SN.SerialNo
   ...
   AND PDET.Cartonno >= @n_StartCartonNo                           
   AND PDET.Cartonno <= @n_EndCartonNo                                           
