...
BEGIN                          
  ...
  DECLARE @d_Trace_StartTime   DATETIME,       
   ...
   END     
    ...
    IF @c_Sparm7 = 'S' OR  @c_Sparm7 = ''    
    BEGIN    
       
   SET @c_SQLJOIN = +' SELECT DISTINCT @n_id,ORD.loadkey,ORD.orderkey,ORD.externorderkey,ORD.type,buyerpo,salesman,ORD.facility,STO.Secondary,'    --8            
             + CHAR(13) +               
             +'STO.Company,STO.SUSR1,''1'' ,'    
             +' (STO.Address1+STO.Address2+STO.Address3),SUBSTRING(ORD.Notes, 1, 80),'''',ORD.StorerKey,STO.State,'  --8          
             + CHAR(13) +              
             ...
             +' '''',ORD.M_Address1,ORD.M_Address2,ORD.M_City,'''','''', ''1'' ,'    
             + ' ORD.Userdefine10,'''','''' '      
             + CHAR(13) +                
  ...
  BEGIN    
      SET @c_SQLJOIN = +' SELECT DISTINCT @n_id,ORD.loadkey,ORD.orderkey,ORD.externorderkey,ORD.type,buyerpo,salesman,ORD.facility,STO.Secondary,'    --8            
             + CHAR(13) +               
             +'STO.Company,STO.SUSR1,PD.CartonNo ,' --11    
             +' (STO.Address1+STO.Address2+STO.Address3),SUBSTRING(ORD.Notes, 1, 80),'''',ORD.StorerKey,STO.State,'  --16         
             + CHAR(13) +              
             +'STO.City,STO.Zip,STO.Contact1,STO.Phone1,STO.phone2,ORD.Consigneekey,ORD.c_Company,ORD.c_Address1,'+ CHAR(13) +      --24        
             +'ISNULL(ORD.c_Address2,''''),ISNULL(ORD.C_Address3,''''),ORD.C_Address4,ORD.C_State,ORD.C_City, ' + CHAR(13) +   --29    
             + 'ORD.C_Zip,ORD.C_Contact1,ORD.C_Phone1,'              
             ...
             +'ORD.ShipperKey,STO.B_Company,(STO.B_Address1+STO.B_Address2+STO.B_Address3),STO.B_Contact1,STO.B_Phone1,ORD.DeliveryPlace,'''', '  --50           
             +' '''',ORD.M_Address1,ORD.M_Address2,ORD.M_City,'''','''',PH.EstimatetotalCtn ,'  --57    
             + ' ORD.Userdefine10,'''','''' '      
             + CHAR(13) +                
             + ' FROM ORDERS ORD (NOLOCK) JOIN STORER STO (NOLOCK) ON STO.StorerKey = ORD.StorerKey '      
             + ' LEFT JOIN ORDERINFO ORDIF WITH (NOLOCK) ON ORDIF.orderkey = ORD.Orderkey '       
  ...
  SET @c_SQL='INSERT INTO #t_BartenderResult (ID,Col01,Col02,Col03,Col04,Col05, Col06,Col07,Col08,Col09'  + CHAR(13) +               
   ...
   SET @c_SQL = @c_SQL +  CHAR(13) + @c_SQLJOIN +  CHAR(13) + @c_condition1  + CHAR(13) + @c_condition2  + CHAR(13) + @c_condition     
...
EXIT_SP:        
  ...
  ('isp_BT_Bartender_Shipper_Label_8', @d_Trace_StartTime, @d_Trace_EndTime    
