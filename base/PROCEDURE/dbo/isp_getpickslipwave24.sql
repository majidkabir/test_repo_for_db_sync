...
/************************************************************************/  
/* Stored Proc: isp_GetPickSlipWave24                                   */  
/* Creation Date: 16-JUN-2020                                           */  
/* Copyright: LF Logistics                                              */  
/* Written by: WLChooi                                                  */  
/*                                                                      */  
/* Purpose: WMS-13694 - HM Australia - Outbound Pick Slip Report        */  
/*        : Copy from isp_GetPickSlipWave14                             */  
/*                                                                      */
/* Called By: r_dw_print_wave_pickslip_24                               */  
/*          :                                                           */  
/* GitLab Version: 1.0                                                  */  
/*                                                                      */  
/* Version: 7.0                                                         */  
...
/************************************************************************/  
CREATE PROC [dbo].[isp_GetPickSlipWave24]  
...
BEGIN  
   ...
   /*
	...
	BEGIN
      ...
      SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Wavetype and OrderSelectionKey Mismatch (isp_GetPickSlipWave24)'     
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         BEGIN      
            SET @n_continue = 3    
            SET @n_err = 81010  
            SET @c_errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Get PickSlip # Failed. (isp_GetPickSlipWave24)'    
            BREAK     
         END               
         ...
         BEGIN        
            ...
            SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert PICKHEADER Failed (isp_GetPickSlipWave24)'     
                           + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '     
            GOTO QUIT_SP       
         END    
      ...
      BEGIN  
         ...
         BEGIN  
            ...
            BEGIN    
               SET @n_continue = 3  
               SET @n_err = 81030  
               SET @c_errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert RefKeyLookup Failed. (isp_GetPickSlipWave24)'      
               GOTO QUIT_SP  
            END                  
         END  
         ...
         BEGIN  
            ...
            BEGIN        
               ...
               SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': UPDATE Pickdetail Failed (isp_GetPickSlipWave24)'     
                              + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '     
               GOTO QUIT_SP       
            END   
         END  
...
QUIT_SP:  
   ...
   BEGIN  
      ...
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'isp_GetPickSlipWave24'  
