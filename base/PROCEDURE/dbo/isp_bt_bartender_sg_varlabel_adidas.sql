...
/********************************************************************************/                   
/* Copyright: LFL                                                               */                   
/* Purpose: isp_BT_Bartender_SG_VARLABEL_ADIDAS                                 */                   
/*                                                                              */                   
/* Modifications log:                                                           */                   
/*                                                                              */                   
/* Date       Rev  Author     Purposes                                          */                   
/* 2021-12-14 1.0  WLChooi    Created (WMS-18526)                               */ 
/* 2021-12-14 1.0  WLChooi    DevOps Combine Script                             */
/********************************************************************************/                  

CREATE PROC [dbo].[isp_BT_Bartender_SG_VARLABEL_ADIDAS]                        
...
BEGIN                        
   ...
   BEGIN
      ...
      --SELECT R.DropID, R.SKU, R.Descr, SUM(R.PQty), SUM(R.CQty), 'N'
      --FROM RDT.RDTPPA R (NOLOCK)
      --WHERE R.DropID = @c_Sparm01
      --AND R.Storerkey = @c_Sparm02
      --GROUP BY R.DropID, R.SKU, R.Descr
      --HAVING SUM(R.PQty) <> SUM(R.CQty)
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            --INSERT INTO #TEMPSKU (DataCol, SKU, SKUDESCR, PQty, CQty, Retrieve)     
            --SELECT R.Pickslipno, R.SKU, R.Descr, SUM(R.PQty), SUM(R.CQty), 'N'
            --FROM RDT.RDTPPA R (NOLOCK)
            --WHERE R.Pickslipno = @c_Sparm01
            --GROUP BY R.Pickslipno, R.SKU, R.Descr
            --HAVING SUM(R.PQty) <> SUM(R.CQty)
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            --INSERT INTO #TEMPSKU (DataCol, SKU, SKUDESCR, PQty, CQty, Retrieve)     
            --SELECT R.ID, R.SKU, R.Descr, SUM(R.PQty), SUM(R.CQty), 'N'
            --FROM RDT.RDTPPA R (NOLOCK)
            --WHERE R.ID = @c_Sparm01
            --AND R.Storerkey = @c_Sparm02
            --GROUP BY R.ID, R.SKU, R.Descr
            --HAVING SUM(R.PQty) <> SUM(R.CQty)
      ...
      BEGIN  
         ...
         BEGIN    
            ...
            BEGIN         
              ...
              SET @c_SKU_CQty05 = CONVERT(NVARCHAR(10),@n_SKU_CQty)        
            END  
            ...
            BEGIN         
              ...
              SET @c_SKU_CQty06 = CONVERT(NVARCHAR(10),@n_SKU_CQty)      
            END  
            ...
            BEGIN         
              ...
              SET @c_SKU_CQty07 = CONVERT(NVARCHAR(10),@n_SKU_CQty)      
            END    
