SET ANSI_NULLS OFF;
GO
SET QUOTED_IDENTIFIER OFF;
GO
    
/******************************************************************************/                     
/* Copyright: IDS                                                             */                     
/* Purpose: isp_BT_Bartender_TW_VASLABEL                                      */                     
/*                                                                            */                     
/* Modifications log:                                                         */                     
/*                                                                            */                     
/* Date       Rev  Author     Purposes                                        */                     
/* 08-FEB-2022  1.0  CSCHONG    Devops scripts combine                        */     
/* 08-FEB-2022  1.1  CSCHONG    Created (WMS-18779)                           */    
/* 04-JAN-2023  1.2  Mingle     WMS-21422 - Add Col12 and change sign(ML01)   */
/******************************************************************************/                    
                    
CREATE   PROC [dbo].[isp_BT_Bartender_TW_VASLABEL]                          
(  @c_Sparm01            NVARCHAR(250),                  
   @c_Sparm02            NVARCHAR(250),                  
   @c_Sparm03            NVARCHAR(250),                  
   @c_Sparm04            NVARCHAR(250),                  
   @c_Sparm05            NVARCHAR(250),                  
   @c_Sparm06            NVARCHAR(250),                  
   @c_Sparm07            NVARCHAR(250),                  
   @c_Sparm08            NVARCHAR(250),                  
   @c_Sparm09            NVARCHAR(250),                  
   @c_Sparm10            NVARCHAR(250),            
   @b_debug              INT = 0                                                                                                                                                                                                     
)                          
AS                          
BEGIN                          
   SET NOCOUNT ON                     
   SET ANSI_NULLS OFF                    
   SET QUOTED_IDENTIFIER OFF                     
   SET CONCAT_NULL_YIELDS_NULL OFF                                         
                                  
   DECLARE                      
      @c_OHUDF05          NVARCHAR(40) = '',                        
      @c_col07            NVARCHAR(20) = '', 
      @c_col08            NVARCHAR(20) = '',
      @c_col10            NVARCHAR(20) = '',   
      @c_DelimiterSign    NVARCHAR(5) ,       
      @n_SeqNo            INT, 
      @c_ColValue         NVARCHAR(50) ,                 
      @n_intFlag          INT,         
      @n_NoofCopy         INT,        
      @c_SQL              NVARCHAR(4000),            
      @c_SQLSORT          NVARCHAR(4000),            
      @c_SQLJOIN          NVARCHAR(4000),
      @c_col12            NVARCHAR(60) = ''	--ML01
        
  DECLARE  @d_Trace_StartTime   DATETIME,       
           @d_Trace_EndTime    DATETIME,      
           @c_Trace_ModuleName NVARCHAR(20),       
           @d_Trace_Step1      DATETIME,       
           @c_Trace_Step1      NVARCHAR(20),      
           @c_UserName         NVARCHAR(20),
           @c_ExecStatements   NVARCHAR(4000),          
           @c_ExecArguments    NVARCHAR(4000)          
      
   SET @d_Trace_StartTime = GETDATE()      
   SET @c_Trace_ModuleName = ''      
            
   -- SET RowNo = 0                 
   SET @c_SQL = ''      
   SET @n_intFlag  = 1    

   --SET @c_DelimiterSign = ';'
   SET @c_DelimiterSign = ','	--ML01
 
                  
    CREATE TABLE [#Result] (                 
      [ID]    [INT] IDENTITY(1,1) NOT NULL,                                
      [Col01] [NVARCHAR] (80) NULL,                  
      [Col02] [NVARCHAR] (80) NULL,                  
      [Col03] [NVARCHAR] (80) NULL,                  
      [Col04] [NVARCHAR] (80) NULL,                  
      [Col05] [NVARCHAR] (80) NULL,                  
      [Col06] [NVARCHAR] (80) NULL,                  
      [Col07] [NVARCHAR] (80) NULL,                  
      [Col08] [NVARCHAR] (80) NULL,                  
      [Col09] [NVARCHAR] (80) NULL,            
      [Col10] [NVARCHAR] (80) NULL,                  
      [Col11] [NVARCHAR] (80) NULL,                  
      [Col12] [NVARCHAR] (80) NULL,                  
      [Col13] [NVARCHAR] (80) NULL,                  
      [Col14] [NVARCHAR] (80) NULL,                  
      [Col15] [NVARCHAR] (80) NULL,                  
      [Col16] [NVARCHAR] (80) NULL,                  
      [Col17] [NVARCHAR] (80) NULL,                  
      [Col18] [NVARCHAR] (80) NULL,                  
      [Col19] [NVARCHAR] (80) NULL,                  
      [Col20] [NVARCHAR] (80) NULL,                  
      [Col21] [NVARCHAR] (80) NULL,                  
      [Col22] [NVARCHAR] (80) NULL,                  
      [Col23] [NVARCHAR] (80) NULL,                  
      [Col24] [NVARCHAR] (80) NULL,                  
      [Col25] [NVARCHAR] (80) NULL,                  
      [Col26] [NVARCHAR] (80) NULL,                  
      [Col27] [NVARCHAR] (80) NULL,                  
      [Col28] [NVARCHAR] (80) NULL,                  
      [Col29] [NVARCHAR] (80) NULL,                  
      [Col30] [NVARCHAR] (80) NULL,                  
      [Col31] [NVARCHAR] (80) NULL,                  
      [Col32] [NVARCHAR] (80) NULL,                  
      [Col33] [NVARCHAR] (80) NULL,                  
      [Col34] [NVARCHAR] (80) NULL,                  
      [Col35] [NVARCHAR] (80) NULL,                  
      [Col36] [NVARCHAR] (80) NULL,                  
      [Col37] [NVARCHAR] (80) NULL,                  
      [Col38] [NVARCHAR] (80) NULL,                  
      [Col39] [NVARCHAR] (80) NULL,                  
      [Col40] [NVARCHAR] (80) NULL,                  
      [Col41] [NVARCHAR] (80) NULL,                  
      [Col42] [NVARCHAR] (80) NULL,                  
      [Col43] [NVARCHAR] (80) NULL,                  
      [Col44] [NVARCHAR] (80) NULL,                  
      [Col45] [NVARCHAR] (80) NULL,                  
      [Col46] [NVARCHAR] (80) NULL,                  
      [Col47] [NVARCHAR] (80) NULL,                  
      [Col48] [NVARCHAR] (80) NULL,                  
      [Col49] [NVARCHAR] (80) NULL,                  
      [Col50] [NVARCHAR] (80) NULL,                 
      [Col51] [NVARCHAR] (80) NULL,                  
      [Col52] [NVARCHAR] (80) NULL,                  
      [Col53] [NVARCHAR] (80) NULL,                  
      [Col54] [NVARCHAR] (80) NULL,                  
      [Col55] [NVARCHAR] (80) NULL,                  
      [Col56] [NVARCHAR] (80) NULL,                  
      [Col57] [NVARCHAR] (80) NULL,                  
      [Col58] [NVARCHAR] (80) NULL,                  
      [Col59] [NVARCHAR] (80) NULL,                  
      [Col60] [NVARCHAR] (80) NULL                 
     )                


     SELECT @c_OHUDF05 = ISNULL(OH.userdefine05,'')
     FROM ORDERS OH WITH (NOLOCK)
     WHERE OH.StorerKey = @c_Sparm01 
     AND OH.OrderKey = @c_Sparm02


   IF ISNULL(@c_OHUDF05,'') <> ''
   BEGIN
          DECLARE C_DelimSplit CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
         SELECT SeqNo, ColValue
         FROM dbo.fnc_DelimSplit(@c_DelimiterSign,@c_OHUDF05)

         OPEN C_DelimSplit
         FETCH NEXT FROM C_DelimSplit INTO @n_SeqNo, @c_ColValue

        WHILE (@@FETCH_STATUS=0)
        BEGIN

                IF @n_SeqNo = 1
                BEGIN
                 SET @c_col07 = @c_ColValue

                END
                ELSE IF @n_SeqNo = 2
                BEGIN
                 SET @c_col08 = @c_ColValue

                END
                ELSE  IF @n_SeqNo = 3
                BEGIN
                 SET @c_col10 = @c_ColValue

                END  

       FETCH NEXT FROM C_DelimSplit INTO @n_SeqNo, @c_ColValue
       END -- WHILE (@@FETCH_STATUS <> -1) AND @n_Continue <> 3

       CLOSE C_DelimSplit
       DEALLOCATE C_DelimSplit

  END

    
         
    INSERT INTO #Result
    (
        Col01,
        Col02,
        Col03,
        Col04,
        Col05,
        Col06,
        Col07,
        Col08,
        Col09,
        Col10,
        Col11,
        Col12,
        Col13,
        Col14,
        Col15,
        Col16,
        Col17,
        Col18,
        Col19,
        Col20,
        Col21,
        Col22,
        Col23,
        Col24,
        Col25,
        Col26,
        Col27,
        Col28,
        Col29,
        Col30,
        Col31,
        Col32,
        Col33,
        Col34,
        Col35,
        Col36,
        Col37,
        Col38,
        Col39,
        Col40,
        Col41,
        Col42,
        Col43,
        Col44,
        Col45,
        Col46,
        Col47,
        Col48,
        Col49,
        Col50,
        Col51,
        Col52,
        Col53,
        Col54,
        Col55,
        Col56,
        Col57,
        Col58,
        Col59,
        Col60
    )

   SELECT DISTINCT ISNULL(dpf.deviceid,''),ISNULL(dpf.DevicePosition,''),ISNULL(OIF.EcomOrderID,''),oh.orderkey,oh.trackingno,      --5    
             ISNULL(oh.c_contact1,''),@c_col07,@c_col08 ,SUBSTRING(ISNULL(oh.notes2,''),1,80),@c_col10,  --10  
             '','','','','',  --15                         
             '','','','','',     --20           
         --    + CHAR(13) +          
              '','','','','','','','','','',  --30      
             '','','','','','','','','','',   --40           
             '','','','','','','','','','',   --50           
              '','','','','','','','','','TW'  --60              
           --  + CHAR(13) +                
              FROM ORDERS OH WITH (NOLOCK)   
              JOIN OrderInfo OIF WITH (NOLOCK) ON OIF.orderkey = OH.OrderKey
              LEFT JOIN rdt.rdtPTLCartLog rdtc WITH (NOLOCK) ON rdtc.OrderKey=oh.OrderKey
              LEFT JOIN dbo.DeviceProfile dpf WITH (NOLOCK) ON dpf.DeviceID = rdtc.CartID AND dpf.DevicePosition = rdtc.Position
              WHERE OH.Storerkey =@c_Sparm01       
              AND OH.Orderkey =@c_Sparm02
                     
            
   IF @b_debug=1            
   BEGIN              
      PRINT @c_SQL              
   END            
   
	
   --START ML01
   SELECT TOP 1 @c_Col12 = PACKDETAIL.refno2
   FROM ORDERS(NOLOCK)
   JOIN PACKHEADER(NOLOCK) ON PackHeader.OrderKey = ORDERS.OrderKey
   JOIN PACKDETAIL(NOLOCK) ON PackDetail.PickSlipNo = PackHeader.PickSlipNo
   WHERE ORDERS.ORDERKEY = @c_Sparm02	--ML01

   UPDATE #Result  
   SET Col12 = @c_Col12  
   --END ML01

	IF @b_debug=1
   BEGIN            
      SELECT * FROM #Result (nolock)            
   END                            

                
EXIT_SP:        
      
   SET @d_Trace_EndTime = GETDATE()      
   SET @c_UserName = SUSER_SNAME()      
  
       
   SELECT * FROM #Result (nolock)     
    
                                     
END -- procedure     


GO