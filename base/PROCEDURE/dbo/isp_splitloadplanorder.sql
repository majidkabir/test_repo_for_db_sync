...
/************************************************************************/
/* Stored Procedure: isp_SplitLoadPlanOrder                             */
/* Creation Date: 23-Sep-2009                                           */
/* Copyright: IDS                                                       */
/* Written by: NJOW                                                     */
/*                                                                      */
/* Purpose: SOS#144067 - Splitting Partial and UnAllocated Order        */
/*                       in a Loadplan                                  */
/*                                                                      */
...
/* 26-Aug-2011  NJOW01    1.1   SOS#224280-Split order in loadplan issue*/
/* 29-APR-2014  CSCHONG   1.2   Add Lottable06-15 (CS01)                */
/************************************************************************/

CREATE PROCEDURE [dbo].[isp_SplitLoadPlanOrder]
...
BEGIN 
   ...
   BEGIN
   	  ...
   	  IF @c_status = '9'
   	  BEGIN
   	  	 SELECT @c_errmsg = RTRIM(@c_status) +' - Load Plan is closed. Splitting of Orders are not allowed'
   	  	 SELECT @n_continue = 4
   	  END  
   	  IF @c_status = 'C'
   	  BEGIN
   	  	 SELECT @c_errmsg = RTRIM(@c_status) + ' - Load Plan is cancelled. Splitting of Orders are not allowed'
   	  	 SELECT @n_continue = 4
   	  END  
   END   
   ...
   BEGIN
         ...
         (
          ...
          InvoiceAmount, Salesman, GrossWeight, Capacity, Rdd, Notes2, SequenceNo,
         	...
         	M_Fax1, M_Fax2, M_vat, ShipperKey 
         	...
         	WHERE Orderkey = @c_orderkey
	   	    ...
	   	    BEGIN
	   		    SELECT @n_continue = 3
				    ...
				    SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Insert Orders Table. (isp_SplitLoadPlanOrder)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
				    BREAK
			    END
       ...
       /*CS01 End*/
   	    ...
   	    BEGIN
   		    SELECT @n_continue = 3
			    ...
			    SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Insert Orderdetail Table. (isp_SplitLoadPlanOrder)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
			    BREAK
		    END		    		    
        ...
        WHERE Orderkey = @c_neworderkey
   	    ...
   	    BEGIN
   		    SELECT @n_continue = 3
			    ...
			    SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Update Orders Table. (isp_SplitLoadPlanOrder)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
			    BREAK
		    END
        ...
        WHERE #TMP_ORDER.Orderkey = @c_orderkey
   	    ...
   	    BEGIN
   		    SELECT @n_continue = 3
			    ...
			    SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Delete Preallocatepickdetail Table. (isp_SplitLoadPlanOrder)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
			    BREAK
		    END		    		    
        ...
        WHERE OD.Orderkey = @c_orderkey
   	    ...
   	    BEGIN
   		    SELECT @n_continue = 3
			    ...
			    SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Update Orderdetail Table. (isp_SplitLoadPlanOrder)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
			    BREAK
		    END
        ...
        INSERT #TMP_NEWORDERS (Orderkey, OldOrderkey, Rectype) VALUES (@c_neworderkey, @c_orderkey, 'N')        
   	    ...
   	    BEGIN
   		    SELECT @n_continue = 3
			    ...
			    SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Insert #TMP_NEWORDER Table. (isp_SplitLoadPlanOrder)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
			    BREAK
		    END
   	  END  -- while
   END
   ...
   BEGIN
   	  ...
   	  SELECT LOADPLANDETAIL.Orderkey, LOADPLANDETAIL.Orderkey, 'O'
  	  ...
  	  BEGIN
  	    SELECT @n_continue = 3
			  ...
			  SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Insert #TMP_NEWORDERS Table. (isp_SplitLoadPlanOrder)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
			  GOTO ERR
		  END

   	  IF (SELECT COUNT(1) FROM #TMP_NEWORDERS) = 0
   	  BEGIN
   	  	 SELECT @c_errmsg = 'No Orders to Split & Move'
   	  	 SELECT @n_continue = 4
   	  END
   END
   ...
   BEGIN
  	  ...
  	  BEGIN
  	    SELECT @n_continue = 3
			  SELECT @c_errmsg = CONVERT(CHAR(250),@n_err), @n_err = 30106
			  SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Update Loadplan Table. (isp_SplitLoadPlanOrder)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
		  END
   ...
   BEGIN        
      ...
      WHERE LOADPLANDETAIL.Loadkey = @c_loadkey
  	  ...
  	  BEGIN
  	    SELECT @n_continue = 3
			  SELECT @c_errmsg = CONVERT(CHAR(250),@n_err), @n_err = 30110
			  SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Delete Loadplandetail Table. (isp_SplitLoadPlanOrder)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
			  GOTO ERR
		  END
       ...
       WHERE Loadkey = @c_loadkey
  	   ...
  	   BEGIN
  	     SELECT @n_continue = 3
			   ...
			   SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Insert Loadplan Table. (isp_SplitLoadPlanOrder)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
 			   GOTO ERR
		   END
      ...
      ORDER BY #TMP_NEWORDERS.Orderkey
  	  ...
  	  BEGIN
  	    SELECT @n_continue = 3
			  ...
			  SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Error Insert Loadplandetail Table. (isp_SplitLoadPlanOrder)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) ' 
			  GOTO ERR
		  END
   ...
   IF @n_continue=3  -- Error Occured - Process And Return
	 BEGIN
	    ...
	    BEGIN
	       ...
	       END
	    END
  	  execute nsp_logerror @n_err, @c_errmsg, 'isp_SplitLoadPlanOrder'
