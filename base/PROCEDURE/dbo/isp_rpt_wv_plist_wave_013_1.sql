...
/************************************************************************/        
/* Stored Procedure: isp_RPT_WV_PLIST_WAVE_013_1                        */        
/* Creation Date: 25-AUG-2022                                           */    
/* Copyright: LF Logistics                                              */    
/* Written by: WZPang                                                   */    
/*                                                                      */    
/* Purpose: Convert to Logi Report - r_dw_print_wave_pickslip_21_1  (TW)*/      
/*                                                                      */        
/* Called By: RPT_WV_PLIST_WAVE_013_1										      */        
/*                                                                      */        
/* PVCS Version: 1.0                                                    */        
/*                                                                      */        
/* Version: 7.0                                                         */        
...
/* 25-AUG-2022  WZPang   1.0  DevOps Combine Script                     */     
/************************************************************************/        
CREATE   PROC [dbo].[isp_RPT_WV_PLIST_WAVE_013_1] (
 ...
 BEGIN        
   ...
   CREATE TABLE #TEMP_PICK  
      ...
      ShowPutawayzone   NVARCHAR(10) NULL  )  
 ...
 WHERE Code = 'PICKPGRP'   
   ...
   ,''  AS cskuudf04                                                           
    ,CASE WHEN ISNULL(CL7.Short,'N') = 'Y' THEN LOC.Putawayzone ELSE '' END AS Putawayzone   
    ,ISNULL(CL7.Short,'N') AS ShowPutawayzone                                                         
   ...
   LEFT OUTER JOIN CODELKUP CL2 (NOLOCK) ON (PICKDETAIL.Storerkey = CL2.Storerkey AND CL2.Code = 'SHOWBARCODE'   
                                          AND CL2.Listname = 'REPORTCFG' AND CL2.Long = 'r_dw_print_wave_pickslip_21' AND ISNULL(CL2.Short,'') <> 'N')   
   LEFT OUTER JOIN CODELKUP CL3 (NOLOCK) ON (PICKDETAIL.Storerkey = CL3.Storerkey AND CL3.Code = 'SHOWSCNFG'   
                                          AND CL3.Listname = 'REPORTCFG' AND CL3.Long = 'r_dw_print_wave_pickslip_21' AND ISNULL(CL3.Short,'') <> 'N')   
   LEFT OUTER JOIN CODELKUP CL4 (NOLOCK) ON (PICKDETAIL.Storerkey = CL4.Storerkey AND CL4.Code = 'PRTSKUDESC'   
                                          AND CL4.Listname = 'REPORTCFG' AND CL4.Long = 'r_dw_print_wave_pickslip_21' AND ISNULL(CL4.Short,'') <> 'N')       
   LEFT OUTER JOIN CODELKUP CL5 (NOLOCK) ON (PICKDETAIL.Storerkey = CL5.Storerkey AND CL5.Code = 'SHOWFIELD'   
                                          AND CL5.Listname = 'REPORTCFG' AND CL5.Long = 'r_dw_print_wave_pickslip_21' AND ISNULL(CL5.Short,'') <> 'N')             
   LEFT OUTER JOIN CODELKUP CL6 (NOLOCK) ON (PICKDETAIL.Storerkey = CL6.Storerkey AND CL6.Code = 'SHOWBUYERPO'   
                                          AND CL6.Listname = 'REPORTCFG' AND CL6.Long = 'r_dw_print_wave_pickslip_21' AND ISNULL(CL6.Short,'') <> 'N')         
   LEFT OUTER JOIN CODELKUP CL7 (NOLOCK) ON (PICKDETAIL.Storerkey = CL7.Storerkey AND CL7.Code = 'ShowPutawayzone'   
                                          AND CL7.Listname = 'REPORTCFG' AND CL7.Long = 'r_dw_print_wave_pickslip_21' AND ISNULL(CL7.Short,'') <> 'N')                                                                                                 
...
-- AND (PICKDETAIL.PickMethod = '8' OR PICKDETAIL.PickMethod = '')        
   ...
   ,CASE WHEN ISNULL(CL6.Code,'') <> '' THEN 'Y' ELSE 'N' END 
    ,CASE WHEN ISNULL(CL7.Short,'N') = 'Y' THEN LOC.Putawayzone ELSE '' END   
    ,ISNULL(CL7.Short,'N')     
   ...
   IF EXISTS (SELECT * FROM CODELKUP CLK WITH (NOLOCK)   
              ...
              AND code = 'afroutedes'  
              AND long ='RPT_WV_PLIST_WAVE_013'  
              AND storerkey = @c_StorerKey)  
    BEGIN  
   ...
   BEGIN  
      GOTO FAILURE  
   END  
   ELSE IF @n_pickslips_required > 0  AND @c_PreGenRptData = 'Y'
   BEGIN  
 ...
 SUCCESS:  
         ...
         BEGIN  
            ...
            BEGIN  
               ...
               SELECT @c_errmsg = 'NSQL' + CONVERT(char(5),@n_err) +  
                                  ': Insert PickingInfo Failed. (isp_GetPickSlipWave21)' + ' ( ' +  
   ...
   SELECT PickSlipNo ,OrderKey ,ExternOrderkey ,WaveKey ,StorerKey  ,InvoiceNo ,Route  ,RouteDescr ,ConsigneeKey  
         ...
         ,CASE WHEN ShowPutawayzone = 'Y' THEN Putawayzone ELSE '' END AS Putawayzone, ShowPutawayzone    
   FROM #TEMP_PICK   
   ...
   GROUP BY PickSlipNo ,OrderKey ,ExternOrderkey ,WaveKey ,StorerKey  ,InvoiceNo ,Route  ,RouteDescr ,ConsigneeKey  
         ...
         ,CASE WHEN ShowPutawayzone = 'Y' THEN Putawayzone ELSE '' END, ShowPutawayzone    
   ORDER BY PickSlipNo, CASE WHEN ShowPutawayzone = 'Y' THEN Putawayzone ELSE '' END, LogicalLoc, Loc, Sku, SkuGroup    
   ...
   BEGIN  
      ...
      END  
      IF @c_PreGenRptData = 'Y'
      EXECUTE dbo.nsp_logerror @n_err, @c_errmsg, 'isp_RPT_WV_PLIST_WAVE_013'  
