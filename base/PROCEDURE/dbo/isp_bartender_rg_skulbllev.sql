...
CREATE PROC [dbo].[isp_Bartender_RG_SKULBLLEV]                      
...
BEGIN                      
  ...
  IF NOT EXISTS ( SELECT 1 FROM Orders ORD WITH (NOLOCK) 
                  WHERE Orderkey = @c_Sparm04)
  BEGIN
                       
            
  SET @c_SQLJOIN = +' SELECT TOP 1 S.Style,''/'',S.Size,s.ManufacturerSKU,s.ManufacturerSKU,'+ CHAR(13) +     --5     
             + ' '''','''','''','''','''', '      --10  
             + ' '''','''','''','''','''','     --15       
             + CHAR(13) +      
             + ' '''','''','''','''','''','         --20      
  ...
  BEGIN
   ...
   SET @c_SQLJOIN = +' SELECT TOP 1 S.Style,OD.UnitPrice,OD.Userdefine01,'''','''','+ CHAR(13) +     --5     
             + ' '''','''','''','''','''', '      --10  
             + ' '''','''','''','''','''','     --15       
             + CHAR(13) +      
             + ' '''','''','''','''','''','         --20      
