...
/* Written by: WLChooi                                                   */    
/*                                                                       */    
/* Purpose: WMS-17722 - [CN] Sephora Chengdu Reverse Wave Released       */    
/*                                                                       */    
/* Called By: Wave                                                       */    
/*                                                                       */    
/* GitLab Version: 1.0                                                   */    
...
/*************************************************************************/     
CREATE PROCEDURE [dbo].[ispRVWAV46]      
...
BEGIN  
   ...
   BEGIN
      ...
      BEGIN
         SELECT @n_continue = 3  
         SELECT @n_err = 81010  
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': This Wave has not been released. (ispRVWAV46)'         
       END                 
   END
   ...
   BEGIN
       ...
       BEGIN
         SELECT @n_continue = 3  
         SELECT @n_err = 81020  
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Some Tasks have been started. Not allow to Reverse Wave Released (ispRVWAV46)'       
       END                 
   END
   
   BEGIN TRAN
   
   ----Delete Pickheader (Conso)
   IF @n_continue = 1 OR @n_continue = 2
   BEGIN
   ...
   BEGIN
        ...
        AND TASKDETAIL.Sourcetype IN ('ispRLWAV46-B2B','ispRLWAV46-B2C')
        ...
        BEGIN
          ...
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Delete Taskdetail Table Failed. (ispRVWAV46)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
        END 
   END
   ...
   BEGIN
      ...
      BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Pickdetail Table Failed. (ispRVWAV46)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
      END          
   END    
   ...
   BEGIN  
     ...
     WHERE WAVEKEY = @c_wavekey  
      ...
      BEGIN  
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update on wave Failed (ispRVWAV46)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
      END  
   END  
...
RETURN_SP:
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         END  
      END  
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'ispRVWAV46'
