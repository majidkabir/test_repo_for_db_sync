...
GO
/************************************************************************/  
/* Stored Procedure: isp_MBOL_Bartender_Print                           */  
/* Creation Date: 08-Jul-2020                                           */  
/* Copyright: LFL                                                       */  
/* Written by: WLChooi                                                  */  
/*                                                                      */  
/* Purpose: WMS-13954 - MBOL module print to bartender                  */  
/*          Call custom SP ispMBBTxx                                    */
/*                                                                      */  
/* Called By: MBOL                                                      */  
/*                                                                      */  
/* GitLab Version: 1.0                                                  */  
...
/************************************************************************/   
CREATE PROCEDURE [dbo].[isp_MBOL_Bartender_Print]
   @c_printerid  NVARCHAR(50) = '',  
   @c_labeltype  NVARCHAR(30) = '',  
   @c_userid     NVARCHAR(18) = '',  
   @c_Parm01     NVARCHAR(60) = '', --MBOLKey         
...
BEGIN  
   ...
   AND    ConfigKey = 'MBOL_Bartender_SP'  
   ...
   BEGIN
      ...
      BEGIN
          ...
          SELECT @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5), @n_Err) + 
                 ': Storerconfig MBOL_Bartender_SP - Stored Proc name invalid ('+RTRIM(ISNULL(@c_SPCode,''))+') (isp_MBOL_Bartender_Print)'  
          GOTO QUIT_SP
      END
   ...
   BEGIN
       SELECT @b_success = 0
       EXECUTE nsp_logerror @n_Err, @c_ErrMsg, 'isp_MBOL_Bartender_Print'  
