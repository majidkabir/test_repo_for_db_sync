...
GO
CREATE PROC [dbo].[nspStockAgingByExpiry_Gillete04] (
...
-- SOS 6944
-- properly rename and define expiry columns
-- wally 5.aug.2002
...
-- Modify By Vicky 20 Feb 2002 SOS 3455
     ...
     CREATE TABLE #Result (
      ...
      Expiredqty int NOT NULL DEFAULT (0),
		thirty_mths int NOT NULL DEFAULT (0),
		twentyfour_mths int NOT NULL DEFAULT (0),
		eighteen_mths int NOT NULL DEFAULT (0),
		twelve_mths int NOT NULL DEFAULT (0),
		nine_mths int NOT NULL DEFAULT (0),
		six_mths int NOT NULL DEFAULT (0),
		three_mths int NOT NULL DEFAULT (0),
		lessthree_mths int NOT NULL DEFAULT (0),  
...
--                            AND (PickDetail.Status = '5' OR PickDetail.Status = '9'))
	...
	BEGIN
		...
		WHERE SKU.sku = @c_sku
			AND SKU.StorerKey = @c_storerkey -- SOS 26422 : wally 20.aug.04
...
-- END SOS 3455
		...
		-- expiring in 30 months or later
		IF @n_expiry >= 900
			INSERT #RESULT VALUES(@c_storerkey, @c_company, @c_skugroup, @c_sku, @c_descr, @c_uom, @c_batchno, 
