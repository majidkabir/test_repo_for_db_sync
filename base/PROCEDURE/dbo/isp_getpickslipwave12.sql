...
/************************************************************************/  
/* Stored Procedure: isp_GetPickSlipWave12                              */  
/* Creation Date: 20-JUN-2016                                           */  
/* Copyright: IDS                                                       */  
/* Written by: CSCHONG                                                  */  
/*                                                                      */  
/* Purpose: SOS#369137 - SG NIKE E-Com - Generate Pickslip              */  
/*                                                                      */  
/* Called By: RCM - Generate Pickslip                                   */  
/*          : Datawindow - r_dw_print_wave_pickslip_12                  */  
/*                                                                      */  
/* PVCS Version: 1.0                                                    */  
/*                                                                      */  
/* Version: 5.4                                                         */  
...
BEGIN  
   ...
   SELECT DISTINCT RefKeyLookup.PickSlipNo,  
          ...
          , 'N') AS PrintedFlag,  
          loc.PickZone AS Zone,--Pickheader.consoorderkey as Zone,  
          '' AS PickdetailLot,  
          '' CarrierKey,  
          '' AS                             VehicleNo,  
          CASE WHEN ISNULL(orders.c_country,'') = 'TH' THEN LotAttribute.Lottable01 ELSE LotAttribute.Lottable02 END Lottable02,   --(CS01)  
   ...
   GROUP BY RefKeyLookup.PickSlipNo,orders.loadkey ,
          ...
          ISNULL(Sku.Descr, ''),  
          loc.pickzone,
          CASE WHEN ISNULL(orders.c_country,'') = 'TH' THEN LotAttribute.Lottable01 ELSE LotAttribute.Lottable02 END,--LotAttribute.Lottable02,  --(CS01)
   ...
   SELECT             
      ...
      ,  #TMP_ORDSUM.totalqtyoverallocate,  
          ...
          (#TMP_PICK.grosswgt*#TMP_PICK.Qty) /1000 As [Wgt],
		   ...
		   #TMP_PICK.ALTSKU, --WL01
		   #TMP_PICK.CLKUPSHORT   --WL01           
   FROM   #TMP_PICK  
