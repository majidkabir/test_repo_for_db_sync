...
/*            isp_BT_Bartender_Shipper_Label_1                                */                             
/* Modifications log:                                                         */                 
/*                                                                            */                 
  ...
  SET @c_SQL='INSERT INTO #Result (Col01,Col02,Col03,Col04,Col05, Col06,Col07,Col08,Col09'  + CHAR(13) +               
   ...
   BEGIN      
       SELECT @n_SumPICKDETQty = SUM(PD.QTY)      
             -- @n_SumUnitPrice = SUM(PD.QTY * ORDDET.Unitprice)     ---ellalou  
       FROM PACKHEADER PH (NOLOCK)    
       JOIN PACKDETAIL PD (NOLOCK) ON PH.PickSlipNo=PD.PickSlipNo    
       JOIN ORDERS ORD (NOLOCK) ON ORD.OrderKey = PH.OrderKey    
       JOIN (SELECT DISTINCT SKU FROM ORDERDETAIL OD (NOLOCK) WHERE OD.ORDERKEY = @c_OrderKey) AS ORDDET ON PD.sku = ORDDET.sku  --(JH01)  --ellalou  
      ...
      SELECT --@n_SumPICKDETQty = SUM(PD.QTY),      
              @n_SumUnitPrice = SUM(ORDDET.QtyPicked * ORDDET.Unitprice)       
       FROM PACKHEADER PH (NOLOCK)    
       JOIN PACKDETAIL PD (NOLOCK) ON PH.PickSlipNo=PD.PickSlipNo    
       JOIN ORDERS ORD (NOLOCK) ON ORD.OrderKey = PH.OrderKey    
       JOIN (SELECT DISTINCT SKU, Unitprice,OD.QtyPicked FROM ORDERDETAIL OD (NOLOCK) WHERE OD.ORDERKEY = @c_OrderKey) AS ORDDET ON PD.sku = ORDDET.sku  --(JH01)    
       --JOIN ORDERDETAIL ORDDET(NOLOCK)                   --(JH01)    
       --            ON  ORD.OrderKey = ORDDET.OrderKey    --(JH01)     
       --            AND PD.sku = ORDDET.sku           --(JH01)    
       WHERE  PH.OrderKey = @c_OrderKey    
...
EXIT_SP:        
  ...
  EXEC isp_InsertTraceInfo       
   ...
   @c_TraceName = 'isp_BT_Bartender_Shipper_Label_18',      
