...
/************************************************************************/
/* Stored Procedure: ispPCKH02                                          */
/* Creation Date: 02-APR-2018                                           */
/* Copyright: LFL                                                       */
/* Written by:                                                          */
/*                                                                      */
/* Purpose: WMS-4405 NikeSG check weight no more than 30kg              */
/*                                                                      */
...
/* 18-APR-2022  NJOW01   1.0  WMS-19462 Weight not allow <= 0           */
/* 18-APR-2022  NJOW01   1.0  DEVOPS Combine script                     */
/************************************************************************/

CREATE PROC [dbo].[ispPCKH02]
   ...
   END
	 ...
	 BEGIN
	    IF EXISTS (SELECT 1
	               ...
	               JOIN PACKINFO PIF (NOLOCK) ON I.Pickslipno = PIF.Pickslipno
	 ...
	 BEGIN
	    ...
	    BEGIN
         ...
         SET @c_errmsg='NSQL'+CONVERT(char(5), @n_err)+': Carton weight is not allowed <= 0 kg. (ispPCKH02)'
   ...
   QUIT_SP:
	 ...
	 BEGIN
	    ...
	    BEGIN
	    	...
	    	END
	    END
	    EXECUTE dbo.nsp_LogError @n_Err, @c_Errmsg, 'ispPCKH02'
