...
/* 06-MAR-2017  Wan01     1.5   WMS-1268 - UNITY-Consolidate PTS Qty per*/
/*                              SOline                                  */        
/************************************************************************/    
CREATE PROC [dbo].[ispPOUNI99]    
...
BEGIN    
   ...
   --(Wan01) - START
   DECLARE @n_TotalPickQty       INT
         , @c_DELPickDetailKey   NVARCHAR(10)     
         , @c_ID                 NVARCHAR(10)       
    --(Wan01) - END
   ...
   --(Wan01) - START
   DECLARE CUR_CBNPD CURSOR LOCAL FAST_FORWARD READ_ONLY FOR    
   SELECT DISTINCT
   ...
   OPEN CUR_CBNPD    
   FETCH NEXT FROM CUR_CBNPD INTO  @c_PickDetailKey
                                 , @c_Orderkey
                                 , @c_OrderLineNumber
                                 , @c_Lot
                                 , @c_Loc
   ...
   BEGIN 
      ...
      DECLARE CUR_DELPD CURSOR LOCAL FAST_FORWARD READ_ONLY FOR    
      ...
      OPEN CUR_DELPD    
      FETCH NEXT FROM CUR_DELPD INTO @c_DelPickDetailKey
                                    ,@n_Qty
    
      WHILE (@@FETCH_STATUS <> -1)    
      BEGIN
         IF @c_PickDetailKey <> @c_DelPickDetailKey 
         BEGIN
            DELETE PICKDETAIL 
            WHERE PickdetailKey = @c_DelPickDetailKey
         ...
         SET @n_TotalPickQty = @n_TotalPickQty + @n_Qty
         NEXT_DELPD:
         FETCH NEXT FROM CUR_DELPD INTO @c_DelPickDetailKey
                                       ,@n_Qty
      END
      CLOSE CUR_DELPD
      DEALLOCATE CUR_DELPD
      
      UPDATE PD WITH (ROWLOCK)
      SET UOM = CASE WHEN PACK.Pallet > 0 AND @n_TotalPickQty >= PACK.Pallet
                     THEN '1'
                     WHEN PACK.CaseCnt > 0 AND @n_TotalPickQty >= PACK.CaseCnt
                     THEN '2'
                     WHEN PACK.InnerPack > 0 AND @n_TotalPickQty >= PACK.InnerPack
                     THEN '3'
                     ELSE '6'
                     END
         ,UOMQty = @n_TotalPickQty
         ,Qty    = @n_TotalPickQty
         ,Notes= 'Combine for order line''s with same inventory'
      FROM PICKDETAIL PD
      ...
      FETCH NEXT FROM CUR_CBNPD INTO   @c_PickDetailKey
                                    ...
                                    ,  @c_Loc
                                    ,  @c_ID
   END -- END WHILE FOR CUR_CBNPD        
   CLOSE CUR_CBNPD        
   DEALLOCATE CUR_CBNPD
   SET @c_PickDetailKey = ''
   SET @c_Orderkey = ''
   SET @c_CaseID = ''
   SET @c_Lot = ''
...
QUIT:        
   ...
   END     
   --(Wan01) - START   
   IF (SELECT CURSOR_STATUS('LOCAL','CUR_CBNPD')) >=0        
   BEGIN        
      CLOSE CUR_CBNPD        
      DEALLOCATE CUR_CBNPD        
   END   
   
   IF (SELECT CURSOR_STATUS('LOCAL','CUR_DELPD')) >=0        
   BEGIN        
      CLOSE CUR_DELPD        
      DEALLOCATE CUR_DELPD        
   END         
