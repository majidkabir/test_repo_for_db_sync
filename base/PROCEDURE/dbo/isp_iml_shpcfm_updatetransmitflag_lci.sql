...
/************************************************************************/      
/* Store Procedure:  isp_IML_SHPCFM_UpdateTransmitFlag_LCI              */      
/* Creation Date: 01-Feb-2012                                           */      
/* Copyright: IDS                                                       */      
/* Written by: MCTang                                                   */      
/*                                                                      */      
/* Purpose:  SOS#234497 - USA LCI EDI SO Confirm Outbound Process       */  
/*           - To retrieve records for Shipment confirmation.           */   
/*           - Copy from isp_IML_SHPCFM_UpdateTransmitFlag_LCI          */        
/*                                                                      */      
...
/*                                                                      */      
/* Version: 5.4                                                         */     
/* 01-Feb-2012  MCTang    1.0   SOS#234497 - Export by CBOL (MC05)      */ 
/************************************************************************/      
      
CREATE PROC [dbo].[isp_IML_SHPCFM_UpdateTransmitFlag_LCI]      
(      
...
BEGIN      
   ...
   DECLARE CUR_CBOL CURSOR LOCAL FAST_FORWARD READ_ONLY FOR      
   SELECT MH.CBOLKEY       
   ...
   AND   ISNULL(MH.CBOLKEY, '') <> ''  
   GROUP BY MH.CBOLKEY      
      
   IF @b_debug = '1'      
   BEGIN      
      SELECT 'CBOL', MH.CBOLKEY       
            ,COUNT(T3.Key1) ShippedOrderCnt      
      FROM  dbo.MBOLDetail MD WITH (NOLOCK)      
      JOIN  dbo.MBOL MH WITH (NOLOCK) ON (MD.MBOLKEY = MH.MBOLKEY )      
      JOIN  dbo.TransmitLog3 T3 WITH (NOLOCK) ON (T3.Key1 = MD.OrderKey)      
   ...
   FETCH NEXT FROM CUR_CBOL INTO @n_CBOLKey, @n_ShippedOrderCnt      
      ...
      BEGIN      
         ...
         BEGIN      
            ...
            SELECT @c_errmsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_err,0)) +      
                               ': Update records in TransmitLog3 failed. (isp_IML_SHPCFM_UpdateTransmitFlag_LCI)'      
            BREAK      
         END      
   ...
   IF @b_debug = '1'      
   BEGIN      
      SELECT 'EXTERNMBOLKEY', MH.EXTERNMBOLKEY      
            ,COUNT(T3.Key1) ShippedOrderCnt      
      FROM dbo.MBOLDetail MD WITH (NOLOCK)      
      JOIN dbo.MBOL MH WITH (NOLOCK) ON  (MD.MBOLKEY = MH.MBOLKEY)      
      JOIN dbo.TransmitLog3 T3 WITH (NOLOCK) ON  (T3.Key1 = MD.OrderKey)      
...
QUIT:      
   ...
   BEGIN      
      ...
      BEGIN      
         ...
         END      
      END      
      EXECUTE dbo.nsp_logerror @n_err, @c_errmsg, 'isp_IML_SHPCFM_UpdateTransmitFlag_LCI'      
