...
/************************************************************************/
/* Stored Procedure: nspALLOT3D                                         */
/* Creation Date: 02-Aug-2008                                           */
/* Copyright: IDS                                                       */
/* Written by: YokeBeen                                                 */
/*                                                                      */
/* Purpose: Duplicated from mspALIDS01.											*/
...
BEGIN  
   ...
   BEGIN
      SELECT @c_OrderKey = LEFT(ISNULL(dbo.fnc_LTrim(@c_OtherParms),''), 10)
      SELECT @c_OrderLineNumber = SUBSTRING(ISNULL(dbo.fnc_LTrim(@c_OtherParms),''), 11, 5)
   END
   ...
   BEGIN
      SET @c_ConditionS = ISNULL(dbo.fnc_RTrim(@c_Conditions),'') + ' AND ( LOC.Locationtype <> ''PICK''' 
      SET @c_ConditionS = ISNULL(dbo.fnc_RTrim(@c_Conditions),'') + ' AND LOC.Locationtype <> ''CASE'' )' 
   END
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            SET @c_ConditionS = ISNULL(dbo.fnc_RTrim(@c_Conditions),'') + ' AND ( LOC.Locationtype = ''PICK''' 
            SET @c_ConditionS = ISNULL(dbo.fnc_RTrim(@c_Conditions),'') + ' OR LOC.Locationtype = ''CASE'' )' 
         END
      END -- IF @b_success = 1
   END
   ELSE IF (ISNULL(dbo.fnc_RTrim(@c_Lottable03),'') = '') AND (ISNULL(dbo.fnc_RTrim(@c_uom),'') = '1')
   ...
   END
	...
	SELECT @c_SQLStatement = 
          ...
          ' AND (LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED) >= N''' + 
            ISNULL(dbo.fnc_RTrim(@n_uombase),'') + '''' + 
          ' AND LOC.Status <> ''HOLD'' ' +   
