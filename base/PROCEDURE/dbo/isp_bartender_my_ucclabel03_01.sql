...
/******************************************************************************/                       
/* Copyright: IDS                                                             */                       
/* Purpose: isp_Bartender_MY_UCCLABEL03_01                                    */                       
/*                                                                            */                       
/* Modifications log:                                                         */                       
/*                                                                            */                       
...
BEGIN                            
   ...
   SET @c_SQLJOIN = +' SELECT DISTINCT ST.Company,ISNULL(RTRIM(ORDERS.C_ADDRESS1),''''),ORDERS.Orderkey,'      
                    +' ISNULL(RTRIM(ORDERS.ExternPOkey),''''),ISNULL(RTRIM(ORDERS.DeliveryNote),''''),'+ CHAR(13)      --5            
                    +' ISNULL(RTRIM(ORDERS.ExternOrderkey),''''),CONVERT(NVARCHAR(10),Orders.DeliveryDate,111),PACKDETAIL.PICKSLIPNO,'    --7  
                    +' ISNULL(RTRIM(ORDERS.route),''''),ISNULL(RTRIM(ORDERS.Shipperkey),''''),'+ CHAR(13)      --10        
                    +' '''','''','''', '''', '''', ' + CHAR(13) --15    
                    +' '''','''', '''','''', '''',  ' + CHAR(13)--20      
                    +' '''','''','''','''','''','''','''','''','''','''','''','''','''','''','''',' + CHAR(13)  --35        
                   ...
                   -- +' LEFT JOIN  ROUTEMASTER WITH (NOLOCK)  ON (ORDERS.Route  = ROUTEMASTER.Route)  '+ CHAR(13)              
                    ...
                    +',CONVERT(NVARCHAR(10),Orders.DeliveryDate,111), ISNULL(RTRIM(ORDERS.ExternPOkey),''''),ISNULL(RTRIM(ORDERS.DeliveryNote),'''')'+ CHAR(13)    
   ...
   SELECT DISTINCT col03,col08,col59           
   FROM #Result                     
   WHERE Col60 = 'O'               
   ...
   BEGIN                       
      ...
      INSERT INTO #TEMPSKU (Pickslipno,orderkey,Cartonno,SKU,PQty,Retrieve)              
      SELECT DISTINCT pd.pickslipno,@c_orderkey,pd.cartonno,  
                      PD.SKU , pd.qty,'N'     
      ...
      BEGIN        
         ...
         BEGIN      
            ...
            SELECT TOP 1 Col01,Col02,Col03,Col04,Col05,Col06,Col07,Col08,Col09,Col10,                       
                         '','','','','','','','','','',                    
                         '','','','','', '','','','','',                    
                         '','','','',CAST(@n_sumskuqty as NVARCHAR(10)), col36,col37,col38,col39,col40,                       
                         col41,col42,col43,'','', '','','','','',   
                         '','','','','', '','','','',Col60      
            FROM  #Result       
            WHERE Col60='O'       
         ...
         SET Col11 = @c_Line01,    
             ...
             Col33 = @c_sku08,      
             Col34 = @c_SKUQty08 ,
             Col35 = CAST(@n_sumskuqty as NVARCHAR(10))     
