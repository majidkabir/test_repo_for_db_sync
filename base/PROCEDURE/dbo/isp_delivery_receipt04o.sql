...
/************************************************************************/  
/* Stored Procedure: isp_Delivery_Receipt04o                            */  
/* Creation Date: 12-Aug-2014                                           */  
/* Copyright: LF                                                        */  
/* Written by:                                                          */  
/*                                                                      */  
/* Purpose: IDSPH GSK Delivery Receipt SOS#315905 (by Order)            */  
/*                                                                      */  
/* Called By:                                                           */  
/*                                                                      */  
/* PVCS Version: 1.3                                                    */  
...
CREATE PROC [dbo].[isp_Delivery_Receipt04o] (@cOrderkey NVARCHAR(10) )  
AS  
BEGIN  
   SET NOCOUNT ON 
   SET QUOTED_IDENTIFIER OFF 
 ...
 ELSE  
  IF @b_debug = 1  
   PRINT 'Start Processing...  OrderKey=' + @cOrderkey  
  ...
  BEGIN  
   ...
   BEGIN     
    ...
    BEGIN  
     ...
     SELECT @c_errmsg="NSQL"+CONVERT(nvarchar(5),@n_err)+": No Setup for CodeLkUp.ListName = DR_NCOUNT. (isp_Delivery_Receipt04o)"  
    END  
    ...
    BEGIN  
     ...
     BEGIN  
      SELECT @n_continue = 3  
      SELECT @n_err = 63500  -- should assign new error code  
      SELECT @c_errmsg="NSQL"+CONVERT(nvarchar(5),@n_err)+": Fail to Generate Userdeine10 . (isp_Delivery_Receipt04o)"  
     END  
     ELSE  
      BEGIN  
      UPDATE ORDERS   
      ...
      BEGIN  
       ...
       SELECT @c_errmsg="NSQL"+CONVERT(nvarchar(5),@n_err)+": UPDATE ORDERS Failed. (isp_Delivery_Receipt04o)"  
      END  
     END  
 ...
 BEGIN  
  EXECUTE nsp_logerror @n_err, @c_errmsg, "isp_Delivery_Receipt04o"  
