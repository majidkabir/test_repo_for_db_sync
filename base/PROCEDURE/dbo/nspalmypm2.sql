...
/*************************************************************************/
/* Stored Procedure: nspALMYPM2                                          */
/* Creation Date: 18-MAY-2023                                            */
/* Copyright: MAERSK                                                     */
/* Written by:                                                           */
/*                                                                       */
/* Purpose: WMS-22533 - MY PUMA Allocation                               */
/*          UOM2 - full case from Mezzanine by order and wave(B2C only)  */
/*          UOM6 - loose from Mezzanine by wave                          */
/*                                                                       */
/* Called By:                                                            */
/*                                                                       */
/* PVCS Version: 1.0                                                     */
...
/* 18-MAY-2023  NJOW01  1.0   DEVOPS combine script                      */
/*************************************************************************/
CREATE   PROC [dbo].[nspALMYPM2]
...
BEGIN
   ...
   IF @c_UOM IN('2') AND ISNULL(@c_key2,'')='' AND ISNULL(@c_key3,'')='W'  --Wave conso carton for B2C only and not for B2B
   BEGIN
   ...
   IF @c_UOM = '6'
      SET @c_SortBy = 'ORDER BY LA.Lottable05, LOTxLOCxID.Lot, LOC.LogicalLocation, LOC.Loc, LOTxLOCxID.ID'
   ELSE
      SET @c_SortBy = 'ORDER BY LA.Lottable05, ABS(@n_QtyLeftToFulfill - (LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED - LOTxLOCxID.QtyReplen)),
                       LOTxLOCxID.Lot, LOC.LogicalLocation, LOC.Loc, LOTxLOCxID.ID'
   ...
   SET @c_SQL = N'
      ...
      SELECT LOTxLOCxID.LOT,
             ...
             CASE WHEN @c_UOM = '2' THEN ' ''@c_DYNUOMQTY='' + CAST(LOTxLOCxID.QTY AS NVARCHAR)' ELSE '1' END +
    ' FROM LOTxLOCxID (NOLOCK)
      ...
      CASE WHEN @c_UOM = '2' THEN
           ' AND LOTxLOCxID.QTY > 0 AND (LOTxLOCxID.QTYALLOCATED + LOTxLOCxID.QTYPICKED + LOTxLOCxID.QtyReplen) = 0 '
           ELSE ' AND (LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED - LOTxLOCxID.QtyReplen) >= @n_UOMBase '
      END +
