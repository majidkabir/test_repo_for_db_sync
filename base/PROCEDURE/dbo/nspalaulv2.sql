...
BEGIN      
   ...
   SET @c_ExtraCondition = ' AND LOC.LocationType IN(''PICK'',''DYNPPICK'') '
   ...
   BEGIN  
      ...
      BEGIN
      	 IF EXISTS(SELECT 1 FROM WAVE (NOLOCK) WHERE Wavekey = @c_Key1 AND WaveType = 'LVS-PAL') --Not to proceed conso piece allocation if PAL
      	    GOTO EXIT_SP
      END      
   END
