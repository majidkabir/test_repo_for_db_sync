...
/*********************************************************************** */
/* Stored Procedure: nspALHNB03                                          */
/* Creation Date: 06-Jul-2023                                            */
/* Copyright: LFL                                                        */
/* Written by:                                                           */
/*                                                                       */
/* Purpose: WMS-22944  CN HNB Allocation Strategy                        */
/*          (modified from nspALHNB02)                                   */
/*                                                                       */
/* Called By:                                                            */
/*                                                                       */
/* PVCS Version: 1.0                                                     */
...
/* 06-Jul-2023  NJOW    1.0   DEVOPS Combine Script                      */
/*************************************************************************/

CREATE   PROC [dbo].[nspALHNB03]
...
BEGIN
   ...
   BEGIN
      ...
      SELECT TOP 1 @n_DayRate = CASE WHEN ISNUMERIC(Short) = 1 THEN CAST(Short AS DECIMAL(10,2)) ELSE 0.00 END 
      FROM CODELKUP (NOLOCK)
      ...
      AND Code = @c_UDF03_EXPCode     
   	  ...
   	  SET @c_ExtraCondition = ' AND ((DATEDIFF(Day, GETDATE(), LA.Lottable04) * 1.00) / 
   	                            CASE WHEN ISDATE(LA.Lottable01) = 1 THEN 
   	                                DATEDIFF(day, CONVERT(DATETIME, LA.Lottable01), LA.Lottable04)
   	                            ELSE 1.00 END) >= @n_DayRate '  
      ...
      SET @c_SortingCondition = ' ORDER BY (DATEDIFF(Day, GETDATE(), LA.Lottable04) * 1.00) / 
   	                              CASE WHEN ISDATE(LA.Lottable01) = 1 THEN 
   	                                  DATEDIFF(day, CONVERT(DATETIME, LA.Lottable01), LA.Lottable04)
   	                              ELSE 1.00 END, LA.Lottable09, LOC.LogicalLocation, LOC.Loc '  
   END
   ELSE
   BEGIN
   ...
   EXEC sp_ExecuteSQL @c_SQL, @c_SQLParm, @c_Facility, @c_StorerKey, @c_SKU, @n_QtyLeftToFulfill, @n_UOMBase, @c_Lottable01, @c_Lottable02, @c_Lottable03,
                      ...
                      @d_Lottable13, @d_Lottable14, @d_Lottable15, @n_DayRate  --, @n_FromDay, @n_Today
