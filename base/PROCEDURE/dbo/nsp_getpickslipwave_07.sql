...
/************************************************************************/
/* Stored Procedure: nsp_GetPickSlipWave_07                             */
/* Creation Date: 16-Mar-2009                                           */
/* Copyright: IDS                                                       */
/* Written by: TLTING                                                   */
/*                                                                      */
/* Purpose: SOS#130982 - Replensihment Report for IDSHK REMY principle  */
...
/* 28Apr2010	 GTGOH	  Add OrderGroup and Sku.Susr4(SOS#170582)-GOH01*/
/* 28-Jan-2019  TLTING_ext 1.1 enlarge externorderkey field length      */
/************************************************************************/

CREATE PROC [dbo].[nsp_GetPickSlipWave_07] (
...
@n_StdCube          Float,
@n_StdGrossWgt      Float,
@c_OrderGroup		  NVARCHAR(20),		--GOH01
@c_Susr4				  NVARCHAR(18)		--GOH01	
...
CREATE TABLE #temp_pick (
   ...
   StdGrossWgt      Float NULL, 
	OrderGroup		  NVARCHAR(20) NULL,		--GOH01
	Susr4			     NVARCHAR(18) NULL		--GOH01	
	)	
...
@c_orderkey,  @c_UOM, @c_logicalloc, @c_lot, @c_toloc, @c_moveid
...
BEGIN --While
   ...
   BEGIN --if @c_orderkey <> ''
      SELECT @c_ConsigneeKey = Orders.BillToKey,
            ...
            @c_rdd			 = ORDERS.RDD,
				@c_OrderGroup	 = ORDERS.OrderGroup		--GOH01
      FROM   ORDERS (NOLOCK)
   ...
   */
   ...
   ToLocPutawayzone,    Lottable01,       StdCube,          StdGrossWgt,
	OrderGroup,				Susr4      )	--GOH01
   VALUES
   (@c_pickheaderkey,   @c_wavekey,       @c_OrderKey,		@c_ConsigneeKey,
   dbo.fnc_RTrim(@c_Company),  	dbo.fnc_RTrim(@c_Addr1),  dbo.fnc_RTrim(@c_Addr2),  0,
   dbo.fnc_RTrim(@c_Addr3),    	@c_PostCode,      @c_Route,  			@c_Route_Desc,
   @c_TrfRoom,       	dbo.fnc_RTrim(@c_Notes1),	@n_RowNo,  			dbo.fnc_RTrim(@c_Notes2),
   @c_LOC,           	@c_SKU,  			@c_SKUDesc,       @n_Qty,
   CAST(@c_UOM as int), @n_UOMQty,	   	@c_PrintedFlag,   '8',
   @c_Lot,		   		@c_Carrierkey,    @c_VehicleNo,     @c_Lottable02,
   @d_Lottable04, 	   @c_labelprice,    dbo.fnc_RTrim(@c_externorderkey), @c_ExternPoKey,
   @c_invoiceno,	   	@d_deliverydate,  @c_rdd, 				@c_storerkey,	
   @c_storercompany, 	@n_CaseCnt,			@c_putawayzone, 	dbo.fnc_RTrim(@c_ZoneDesc),	
               ...
               BEGIN
                  ...
                  BEGIN
                     ...
                     BEGIN
                        ...
                        Select @c_errmsg= 'NSQL'+CONVERT(char(5), @n_err)+':Insert failed on TransmitLog2. (nsp_GetPickSlipWave_07)'+'('+'SQLSvr MESSAGE='+dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg))+')'
                        ROLLBACK TRAN 
                        GOTO QUIT 
                     END
                     ELSE
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN  
            ...
            SELECT @n_continue=3
            SELECT @n_err = @@ERROR
            SELECT @c_errMsg = 'Insert into TransmitLog Failed (nsp_GetPickSlipWave_07)'                     
            GOTO QUIT 
         END  
         ELSE 
         BEGIN
...
QUIT:
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         END  
      END  
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'nsp_GetPickSlipWave_07'  
