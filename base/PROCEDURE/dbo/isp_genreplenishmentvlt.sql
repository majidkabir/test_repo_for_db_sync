...
/*                                                                         */
/* Purpose: Creating replenishment FROM storage locations WITH checks for  */ 
/* holds AND flags.                                                        */
/*                                                                         */
...
/* Date           Author      Change                                       */
/* 03/05/2024     PPA374                                                   */
/* 31/05/2024     PPA374      Ensure that replenishment is NOT OVER max    */
/***************************************************************************/

CREATE   PROC [dbo].[isp_GenReplenishmentVLT]
   ...
   @c_ReplenFlag NVARCHAR(10) = 'FP', -- N = Normal
                                    -- W = Wave
                                    -- PP = (Pre-Pick)
                                    -- FP = Full Pallet
                                    -- WHC = Wave Healthcare
   ...
   BEGIN
      ...
      END
        ...
        SELECT @c_SQLStatement = 'DECLARE Cur_ReplenSkuLoc CURSOR FAST_FORWARD READ_ONLY FOR ' +
                        ...
                        'AND LOC.Loc NOT IN (SELECT loc FROM INVENTORYHOLD WITH (NOLOCK) WHERE hold = 1 AND loc <> '''')'+
                        'AND LOC.status = ''OK''' +
                        'AND (skuxloc.QtyLocationLimit - skuxloc.qty - (SELECT isnull(sum(PendingMoveIN),0) FROM lotxlocxid (NOLOCK) WHERE loc = skuxloc.loc AND PendingMoveIN > 0) > 0 AND skuxloc.qty + (SELECT isnull(sum(PendingMoveIN),0) FROM lotxlocxid (NOLOCK) WHERE loc = skuxloc.loc AND PendingMoveIN > 0) <= skuxloc.QtyLocationMinimum)'+
         ...
         BEGIN
            ...
            BEGIN
               INSERT INTO #LOT_SORT (LOT, SortColumn)
                  ...
                  LOC.Loc NOT IN (SELECT loc FROM INVENTORYHOLD WITH (NOLOCK) WHERE hold = 1 AND loc <> '') AND
                   LOC.LocationCategory IN (SELECT code2 FROM CODELKUP WITH (NOLOCK) WHERE listname = 'HUSQREPLOC' AND Storerkey = 'HUSQ') AND
                   LOC.LocationType IN (SELECT Code FROM CODELKUP WITH (NOLOCK) WHERE listname = 'HUSQREPLOC' AND Storerkey = 'HUSQ') AND
                   LOC.PutawayZone IN (SELECT Code FROM CODELKUP WITH (NOLOCK) WHERE listname = 'HUSQREPZON' AND Storerkey = 'HUSQ') AND
         ...
         IF @n_Cnt = 0
            ...
            BEGIN
               ...
               DECLARE CUR_LOTxLOCxID_REPLEN CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
                ...
                ORDER BY lot, OnHandQty DESC, LOC.LogicalLocation
     ...
     ;
      ...
      WITH replenchk1 AS
      (
      SELECT QtyLocationLimit,
      r.* FROM REPLENISHMENT R (NOLOCK)
      LEFT JOIN SKUxLOC SL (NOLOCK)
      ON r.sku = sl.sku
      AND r.ToLoc = sl.Loc
      AND r.FromLoc IN (SELECT loc FROM loc (NOLOCK) WHERE Facility = 'UK001') 
      WHERE QtyLocationLimit > 0
      AND r.Storerkey = 'HUSQ'
      )

      DELETE FROM REPLENISHMENT
      WHERE EXISTS(
      SELECT 1 FROM replenchk1
      WHERE qty > QtyLocationLimit AND REPLENISHMENT.toloc = replenchk1.ToLoc AND REPLENISHMENT.Sku = replenchk1.Sku
      AND REPLENISHMENT.Qty = replenchk1.qty AND REPLENISHMENT.Id = replenchk1.Id AND replenishment.Lot = replenchk1.Lot
      AND REPLENISHMENT.FromLoc = replenchk1.FromLoc AND REPLENISHMENT.ReplenNo = replenchk1.ReplenNo
      AND FromLoc NOT LIKE '__999A____');

      WITH replenchk AS
      (
      SELECT QtyLocationLimit, sum(r.qty)OVER(PARTITION BY toloc, r.sku ORDER BY lot, r.qty DESC ROWS UNBOUNDED PRECEDING)RS,  
      ROW_NUMBER()OVER(PARTITION BY ToLoc, r.Sku ORDER BY CASE WHEN fromloc LIKE '__999A____' THEN 1 ELSE 0 END DESC)RowID,
      r.* FROM REPLENISHMENT R (NOLOCK)
      LEFT JOIN SKUxLOC SL (NOLOCK)
      ON r.sku = sl.sku
      AND r.ToLoc = sl.Loc
      ...
      DELETE FROM REPLENISHMENT
      WHERE EXISTS(
      SELECT 1 FROM replenchk
      WHERE rowid > 1 AND rs > qtylocationlimit AND REPLENISHMENT.toloc = replenchk.ToLoc AND REPLENISHMENT.Sku = replenchk.Sku
      AND REPLENISHMENT.Qty = replenchk.qty AND REPLENISHMENT.Id = replenchk.Id AND replenishment.Lot = replenchk.Lot
      AND REPLENISHMENT.FromLoc = replenchk.FromLoc AND REPLENISHMENT.ReplenNo = replenchk.ReplenNo
      AND FromLoc NOT LIKE '__999A____')
