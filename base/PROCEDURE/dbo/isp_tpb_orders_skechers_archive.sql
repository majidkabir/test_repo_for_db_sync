...
CREATE PROC [dbo].[isp_TPB_Orders_Skechers_Archive]
...
BEGIN
 ...
 -- Cancel Order
   ...
   SET @c_SQLStatement =   @c_SQLStatement +
          N'FROM ' + @c_ArchiveDB + '.dbo.V_ORDERS ORDERS WITH (NOLOCK) ' +
          'JOIN ' + @c_ArchiveDB + '.dbo.V_ORDERDETAIL OD WITH (NOLOCK) ON ORDERS.Orderkey = OD.Orderkey ' +
          'LEFT JOIN DBO.Storer C WITH (NOLOCK) ON C.StorerKey = ORDERS.ConsigneeKey ' +
          'JOIN DBO.SKU WITH (NOLOCK) ON SKU.StorerKey = OD.StorerKey AND  SKU.SKU = OD.SKU ' +
          'LEFT JOIN DBO.FACILITY F WITH (NOLOCK) ON F.Facility = ORDERS.Facility ' +
          'JOIN DBO.PACK WITH (NOLOCK) ON PACK.PACKKey = OD.PACKKey ' +
          'WHERE ORDERS.Status = ''CANC'' AND ORDERS.Doctype = ''E'' ' +
          'AND CONVERT(VARCHAR, ORDERS.EditDate, 112) = CONVERT(VARCHAR, @d_Fromdate, 112) ' +
           @c_SQLCondition
...
INSERT INTO [DBO].[WMS_TPB_BASE](
 ...
 EXEC sp_ExecuteSQL @c_SQLStatement, @c_SQLParm, @n_WMS_BatchNo, @n_TPB_Key, @c_COUNTRYISO, @d_Fromdate, @d_Todate
   ...
   SET @c_SQLStatement =   @c_SQLStatement +
             N'FROM ' + @c_ArchiveDB + '.dbo.V_ORDERS ORDERS WITH (NOLOCK) ' +
             'JOIN ' + @c_ArchiveDB + '.dbo.V_ORDERDETAIL OD WITH (NOLOCK) ON ORDERS.Orderkey = OD.Orderkey ' +
             'JOIN ' + @c_ArchiveDB + '.dbo.V_PICKDETAIL PICKDETAIL WITH (NOLOCK) ON PICKDETAIL.Orderkey = OD.Orderkey AND PICKDETAIL.OrderlineNumber = OD.OrderlineNumber ' +
             'JOIN ' + @c_ArchiveDB + '.dbo.V_MBOLDETAIL MBOLDetail WITH (NOLOCK) ON MBOLDETAIL.OrderKey = ORDERS.OrderKey ' +
             'JOIN ' + @c_ArchiveDB + '.dbo.V_MBOL MBOL WITH (NOLOCK) ON  MBOL.MbolKey = MBOLDETAIL.MbolKey ' +
             'LEFT JOIN DBO.Storer C WITH (NOLOCK) ON C.StorerKey = ORDERS.ConsigneeKey ' +
             'JOIN DBO.SKU WITH (NOLOCK) ON SKU.StorerKey = OD.StorerKey AND  SKU.SKU = OD.SKU ' +
             'LEFT JOIN DBO.FACILITY F WITH (NOLOCK) ON F.Facility = ORDERS.Facility ' +
             'JOIN DBO.PACK WITH (NOLOCK) ON PACK.PACKKey = PICKDETAIL.PACKKey ' +
             'JOIN DBO.LOTATTRIBUTE L WITH (NOLOCK) ON L.Lot = PICKDETAIL.Lot ' +
             'JOIN DBO.LOC WITH (NOLOCK) ON LOC.LOC = PICKDETAIL.Loc ' +
             'WHERE ORDERS.Status =''9'' ' +
             'AND CONVERT(VARCHAR, MBOL.EditDate, 112) = CONVERT(VARCHAR, @d_Fromdate, 112) ' +
              @c_SQLCondition
