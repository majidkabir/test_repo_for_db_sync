...
GO
/************************************************************************/
/* Stored Proc : isp_fullcasepick_bywave                                */
/* Creation Date: 22-Apr-2021                                           */
/* Copyright: IDS                                                       */
/* Written by:CSCHONG                                                   */
/*                                                                      */
/* Purpose: WMS-16731 - [CN] CONVERSE Sorting Sheet Report by wave      */
/*        : Copy & Modified from isp_fullcasepick_sortsheet_bysku03_01  */
/*                                                                      */
/* Input Parameters: facility, wavekeyend, stopstart                    */
/*                 , stopend                                            */
/*                                                                      */
...
/* Local Variables:                                                     */
/*                                                                      */
/* Called By: r_dw_fullcasepick_sortsheet_bywavesku_1                   */
/*                                                                      */
/* PVCS Version: 1.0                                                    */
/*                                                                      */
/* Version: 5.4                                                         */
...
CREATE PROC [dbo].[isp_fullcasepick_bywave]
...
BEGIN     
   ...
   DECLARE @c_Putawayzone     NVARCHAR(10)   
         ...
         , @c_Barcodeline     NVARCHAR(50)           
         , @n_L2cnt           INT                    
         , @c_getline2        NVARCHAR(50)           
         , @c_SortBy          NVARCHAR(100)         
         , @c_GetSortBy       NVARCHAR(100)        
         , @c_query           NVARCHAR(4000)       
         , @c_ExecStatements  NVARCHAR(4000)       
         , @c_SortCondition   NVARCHAR(4000)      
   ...
   BEGIN        
      SELECT ISNULL(RTRIM(L.Putawayzone),'')                            AS Putawayzone
            ...
            ,ISNULL(RTRIM(LP.Loadkey),'')                               AS Loadkey   
      ...
      CREATE NONCLUSTERED INDEX IX_1 on #Result1 (Putawayzone, loadkey, Style, Color, [Size], Lottable02)
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN      
               SET @n_continue = 3      
               SET @n_err = 63501      
               SET @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': INSERT fullcasepick_rpt_result Failed. (isp_fullcasepick_bywave2)'      
               GOTO QUIT    
            END  
            ...
            BEGIN      
               SELECT @n_continue = 3      
               SELECT @n_err = 63505      
               SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': INSERT @ShowResult Failed. (isp_fullcasepick_bywave)'      
               GOTO QUIT    
            END  
         ...
         BEGIN      
            SET @n_continue = 3      
            SET @n_err = 63510      
            SET @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': INSERT @ShowResult Failed. (isp_fullcasepick_bywave)'      
            GOTO QUIT    
         END  
         ...
         VALUES ( @c_Putawayzone   
                , CONVERT(CHAR(20), 'Loadkey')                -- Section 2 Label Line
                + SPACE(10)   
                + RIGHT(REPLICATE(' ',15)+ 'Total Case Qty',15)      -- Section 2 Label Line
                , 'L2' --)     -- Grant total
                , CONVERT(CHAR(20), 'Picking Slip No') )            
         ...
         WHERE c.LISTNAME='COVSORTCB' AND Code='Sort'
       ...
       BEGIN   
          ...
          SET @c_query = '  DECLARE CURSOR_SOGRP CURSOR FAST_FORWARD READ_ONLY FOR ' + 
                         '  SELECT DISTINCT loadkey, ' + @c_SortBy +
                         '  FROM #Result1 ' +
                         '  WHERE PutawayZone = @c_Putawayzone '
       ...
       BEGIN     
           SET @c_query = '  DECLARE CURSOR_SOGRP CURSOR FAST_FORWARD READ_ONLY FOR ' + CHAR(13) +
                         '  SELECT DISTINCT loadkey,'''' ' + CHAR(13) +
                         '  FROM #Result1 ' + CHAR(13) +
                         '  WHERE PutawayZone = @c_Putawayzone ' + CHAR(13) 
                                   
          SET @c_SortCondition = 'ORDER BY loadkey '
       END
       ...
       SET @c_ExecStatements = @c_query + CHAR(13) + @c_SortCondition
        ...
        EXEC sp_ExecuteSql @c_ExecStatements   
         ...
         FETCH NEXT FROM CURSOR_SOGRP INTO @c_loadkey,@c_GetSortBy      
         ...
         BEGIN
            ...
            JOIN ORDERS ORD WITH (NOLOCK) ON ORD.loadkey = PH.externorderkey
   ...
   INSERT INTO  @ShowResult (Putawayzone,  Line,  LineFlag)     
   VALUES (''
          ,CONVERT(CHAR(30), 'Summary By Loadkey' )           -- Section 3 title
          ,'')                                                       -- Grant total
   ...
   INSERT INTO  @ShowResult (Putawayzone,  Line,  LineFlag)     
   VALUES ( ''   
          , CONVERT(CHAR(20), 'Loadkey')                       -- Section 3 Label Line
          + SPACE(10)   
...
--        , CONVERT(CHAR(10), ISNULL(RTRIM(@c_StopEnd ),''))        AS StopEnd   
   ...
   JOIN PACK P      WITH (NOLOCK)         ON  ( S.Packkey    = P.Packkey )
   JOIN ORDERS OH   WITH (NOLOCK)         ON  ( PD.OrderKey  = OH.OrderKey )
   WHERE OH.userdefine09 = ISNULL(RTRIM(@c_wavekey),'')
   ...
   BEGIN        
      execute nsp_logerror @n_err, @c_errmsg, 'isp_fullcasepick_bywave'        
