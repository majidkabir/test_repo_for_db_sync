...
GO
/************************************************************************/    
/* Stored Procedure: nspALPMA3B                                         */    
/* Creation Date: 01-APR-2021                                           */    
/* Copyright: LFL                                                       */    
/* Written by:                                                          */    
/*                                                                      */    
/* Purpose: WMS-16742 CN PUMA Allocation (LIFO)                         */
/*         	Piece by wave from pick if fulfill otherwise bulk  uom 7    */
/*          SkipPreallocation = '1' (Refer to WMS-10420)                */
/*                                                                      */
/* Called By: Wave                                                      */    
/*                                                                      */    
/* PVCS Version: 1.0                                                    */    
...
BEGIN    
   ...
   DECLARE @c_SQL                NVARCHAR(MAX),    
           ...
           @c_LocationGroup      NVARCHAR(20),  --DPP Area
           @c_Busr8              NVARCHAR(30)           
   ...
   BEGIN
      ...
      IF  @c_key3 <> 'W' --Not wave conso 
          GOTO EXIT_SP         
       
      SELECT @c_LocationGroup = Userdefine01
      FROM WAVE (NOLOCK)
      WHERE Wavekey = @c_key1             
   END
   ...
   --Check available qty from pick   
   IF ISNULL(@c_Busr8,'') <> 'Y'  --NJOW01
   BEGIN
      SET @c_SQL = N'        
      ...
      SET @c_SQLParm =  N'@c_Facility   NVARCHAR(5),  @c_StorerKey  NVARCHAR(15), @c_SKU NVARCHAR(20), @n_QtyLeftToFulfill INT, @n_UOMBase INT, ' +
                         ...
                         '@c_Lottable11 NVARCHAR(30), @c_Lottable12 NVARCHAR(30), @d_Lottable13 DATETIME, @d_Lottable14 DATETIME, @d_Lottable15 DATETIME, @n_QtyAvailable INT OUTPUT ' 
      ...
      EXEC sp_ExecuteSQL @c_SQL, @c_SQLParm, @c_Facility, @c_StorerKey, @c_SKU, @n_QtyLeftToFulfill, @n_UOMBase, @c_Lottable01, @c_Lottable02, @c_Lottable03,
                         ...
                         @d_Lottable13, @d_Lottable14, @d_Lottable15, @n_QtyAvailable OUTPUT                   
   END                                                  
   ...
   SET @c_SQL = N'   
      ...
      CASE WHEN ISNULL(@c_Busr8,'') <> 'Y' THEN  --NJOW01
           ...
           ELSE ' AND LOC.LocationType = ''OTHER'' ' END +
      CASE WHEN ISNULL(RTRIM(@c_Lottable01),'') = '' THEN '' ELSE ' AND LA.Lottable01 = @c_Lottable01 ' END +
      CASE WHEN ISNULL(RTRIM(@c_Lottable02),'') = '' THEN '' ELSE ' AND LA.Lottable02 = @c_Lottable02 ' END +
      CASE WHEN ISNULL(RTRIM(@c_Lottable03),'') = '' THEN '' ELSE ' AND LA.Lottable03 = @c_Lottable03 ' END +
      CASE WHEN CONVERT(NVARCHAR(8) ,@d_Lottable04 ,112) <> '19000101' AND @d_Lottable04 IS NOT NULL THEN ' AND LA.Lottable04 = RTRIM(CONVERT( NVARCHAR(20), @d_Lottable04, 106)) ' ELSE ' ' END +
      ...
      CASE WHEN ISNULL(@n_QtyAvailable,0) >= @n_QtyLeftToFulfill THEN
                CASE WHEN ISNULL(@c_Busr8,'') <> 'Y' THEN  --NJOW01
                   ' ORDER BY CASE WHEN LOC.LocationType = ''PICK'' THEN 1 ELSE 2 END, CASE WHEN LA.Lottable07 = '''' THEN 0 ELSE 1 END, LA.LOTTABLE07 DESC, LA.Lottable05, LOC.LocationRoom, LOC.LogicalLocation, LOC.LOC ' 
                ELSE
                   ' ORDER BY CASE WHEN LOC.LocationType = ''PICK'' THEN 1 ELSE 2 END, CASE WHEN LA.Lottable07 = '''' THEN 1 ELSE 0 END, LA.LOTTABLE07, LA.Lottable05 DESC, LOC.LocationRoom, LOC.LogicalLocation, LOC.LOC '            
                END           
           ELSE 
                CASE WHEN ISNULL(@c_Busr8,'') <> 'Y' THEN  --NJOW01
                   ' ORDER BY CASE WHEN LOC.LocationType = ''OTHER'' THEN 1 ELSE 2 END, CASE WHEN LA.Lottable07 = '''' THEN 0 ELSE 1 END, LA.LOTTABLE07 DESC, LA.Lottable05, LOC.LocationRoom, LOC.LogicalLocation, LOC.LOC ' 
                ELSE
                   ' ORDER BY CASE WHEN LOC.LocationType = ''OTHER'' THEN 1 ELSE 2 END, CASE WHEN LA.Lottable07 = '''' THEN 1 ELSE 0 END, LA.LOTTABLE07, LA.Lottable05 DESC, LOC.LocationRoom, LOC.LogicalLocation, LOC.LOC '            
                END           
                --' ORDER BY CASE WHEN LOC.LocationType = ''OTHER'' THEN 1 ELSE 2 END, LOC.LocationRoom, LA.Lottable05 DESC, LOC.LogicalLocation, LOC.LOC '
           END 

   SET @c_SQLParm =  N'@c_Facility   NVARCHAR(5),  @c_StorerKey  NVARCHAR(15), @c_SKU NVARCHAR(20), @n_QtyLeftToFulfill INT, @n_UOMBase INT, ' +
                      '@c_Lottable01 NVARCHAR(18), @c_Lottable02 NVARCHAR(18), @c_Lottable03 NVARCHAR(18), @d_Lottable04 DATETIME, @d_Lottable05 DATETIME, ' +
                      '@c_Lottable06 NVARCHAR(30), @c_Lottable07 NVARCHAR(30), @c_Lottable08 NVARCHAR(30), @c_Lottable09 NVARCHAR(30), @c_Lottable10 NVARCHAR(30), ' +
                      '@c_Lottable11 NVARCHAR(30), @c_Lottable12 NVARCHAR(30), @d_Lottable13 DATETIME, @d_Lottable14 DATETIME, @d_Lottable15 DATETIME, @c_LocationGroup NVARCHAR(20) ' 

   EXEC sp_ExecuteSQL @c_SQL, @c_SQLParm, @c_Facility, @c_StorerKey, @c_SKU, @n_QtyLeftToFulfill, @n_UOMBase, @c_Lottable01, @c_Lottable02, @c_Lottable03,
                      @d_Lottable04, @d_Lottable05, @c_Lottable06, @c_Lottable07, @c_Lottable08, @c_Lottable09, @c_Lottable10, @c_Lottable11, @c_Lottable12,
                      @d_Lottable13, @d_Lottable14, @d_Lottable15, @c_LocationGroup
