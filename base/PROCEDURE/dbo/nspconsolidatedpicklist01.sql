...
GO
/***************************************************************************/  
/* Stored Procedure: nspConsolidatedPickList01                             */  
/* Creation Date:                                                          */  
/* Copyright: IDS                                                          */  
/* Written by:                                                             */  
/*                                                                         */  
/* Purpose:                                                                */  
/*                                                                         */  
/* Called By: r_dw_consolidated_pick01                                     */  
/*                                                                         */  
/* PVCS Version: 1.1                                                       */  
/*                                                                         */  
/* Version: 5.4                                                            */  
...
/* 11-Jan-2013 NJOW  1.0  265852-group by lot                    */  
/* 30-Jan-2013  NJOW    1.1   265852-fix 2nd orderkey not display at 1st pg*/  
/* 29-Apr-2015  CSCHONG 1.2   SOS339808  (CS01)                            */  
/* 09-Jul-2015  CSCHONG 1.3   SOS346307 (CS02)                             */  
/* 26-Aug-2019  WLChooi 1.4   WMS-10328 - Add reportCFG to NOT sort by     */  
/*                            LogicalLoc (WL01)                            */  
/* 04-SEP-2019  CSCHONG 1.5   WMS-10248 reportCFG show/hide sub report (CS03)*/  
/***************************************************************************/  
CREATE PROC [dbo].[nspConsolidatedPickList01] (    
 ...
 DECLARE @d_date_start       datetime,    
   ...
   @c_showsubreport        NVARCHAR(5) = 'N'  --CS03  
 ...
 SELECT @c_NoSortByLogicalLoc = Short  
 FROM CODELKUP WITH (NOLOCK)      
 WHERE ListName = 'REPORTCFG'      
 AND   Storerkey= @c_Storerkey    
 AND   Code = 'NoSortByLogicalLoc'    
 AND   Long = 'r_dw_consolidated_pick01'      
  
 IF (ISNULL(@c_NoSortByLogicalLoc,'') = '' OR @c_NoSortByLogicalLoc NOT IN ('N','Y') )  
    SET @c_NoSortByLogicalLoc = 'N'  
  
 --WL01 End  
  
 --CS01 START  
  
  SELECT @c_showsubreport = Short  
 FROM CODELKUP WITH (NOLOCK)      
 WHERE ListName = 'REPORTCFG'      
 AND   Storerkey= @c_Storerkey    
 AND   Code = 'ShowSubReport'    
 AND   Long = 'r_dw_consolidated_pick01'    
  
 --CS01 END  
 --select @c_Storerkey, @c_NoSortByLogicalLoc  
 BEGIN TRAN    
 ...
 /*Create Temp Result table */    
  ...
  Pack6=Convert(NVarchar(10), ''),  -- Space(10),    
        ...
        LogicalLoc = LOC.LogicalLocation,       --WL01  
  ShowSubReport = @c_showsubreport         --CS03  
   INTO #CONSOLIDATED     
   FROM LOADPLAN (NOLOCK), ORDERS (NOLOCK), PICKDETAIL (NOLOCK), LOTATTRIBUTE(NOLOCK), LOC (NOLOCK) --WL01    
   where 1 = 2    
  ...
  @c_TotPack  NVARCHAR(10),    
   ...
   BEGIN    
      ...
      BEGIN    
        ...
        -- Customize For Thailand for 6 Orders per pick slip    
         IF @n_GroupSeq > 6    
         BEGIN    
         ...
         END  
         --WL01 Start  
         IF @c_NoSortByLogicalLoc = 'Y'  
         BEGIN  
            INSERT INTO #SKUGroup     
              ( LOC, SKU, OrderKey, GroupNo, GroupSeq, Lot, Storerkey, LogicalLoc)    
               VALUES (@c_loc, @c_sku, @c_OrderKey, IsNULL(@n_GroupNo,  1), IsNULL(@n_GroupSeq, 1), @c_Lot, @c_Storerkey, '')    
         END   
         ELSE  
         BEGIN    
            INSERT INTO #SKUGroup     
              ( LOC, SKU, OrderKey, GroupNo, GroupSeq, Lot, Storerkey, LogicalLoc)  --WL01  
               VALUES (@c_loc, @c_sku, @c_OrderKey, IsNULL(@n_GroupNo,  1), IsNULL(@n_GroupSeq, 1), @c_Lot, @c_Storerkey, @c_logicallocation)  --WL01  
         END  
         --WL01 End  
      ...
      BEGIN    
        ...
        BEGIN    
           ...
           ELSE  
           BEGIN    
              INSERT INTO #SKUGroup values (@c_loc, @c_sku, @c_orderkey, @n_groupno1, @n_groupseq1, @c_Lot, @c_Storerkey, @c_logicallocation)  
           END  
           --WL01 End   
        end    
      end    
   ...
   DECLARE CUR_2 CURSOR LOCAL FAST_FORWARD READ_ONLY FOR    
      SELECT DISTINCT GroupNo, LOC, SKU, LOT, Storerkey, LogicalLoc  --WL01  
      FROM   #SKUGroup    
      ORDER BY GroupNo, LOC, SKU, LOT   
   OPEN CUR_2    
     
   SET @n_PrevGroupNo = 0  
   FETCH NEXT FROM CUR_2 INTO @n_GroupNo, @c_LOC, @c_SKU, @c_Lot, @c_Storerkey, @c_logicallocation --WL01    
   WHILE @@FETCH_STATUS <> -1    
   BEGIN     
     ...
     DECLARE CUR_3 CURSOR LOCAL FAST_FORWARD READ_ONLY FOR     
      ...
      INSERT INTO #CONSOLIDATED VALUES (    
            ...
            @c_LPriority,           --(CS01)   
            @c_LUdef01,             --(CS01)  
            @c_logicallocation,      --WL01   
   @c_showsubreport        --CS03  
            )           
