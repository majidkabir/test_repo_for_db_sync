...
/* 06-Sep-2018    1.0  CSCHONG    WMS-5999-6131 Created                       */        
/* 26-Apr-2019    1.1  WLCHOOI    WMS-8777 - Add Orders.C_Company (WL01)      */      
/******************************************************************************/              
                
CREATE PROC [dbo].[isp_BT_Bartender_HK_WAYBILLLBL_label]                     
...
BEGIN                    
  ...
  SET @c_SQLJOIN = +' SELECT DISTINCT orders.trackingno,orders.c_contact1,'
                   ...
                   +' CASE WHEN C1.short=''2'' THEN ISNULL(orders.c_address1,'''') '
                   +' ELSE ISNULL(orders.c_address2,'''') END,'
                   +' CASE WHEN C1.short=''2'' THEN ISNULL(orders.c_address2,'''') '
                   +' ELSE ISNULL(orders.c_address3,'''') END,'
                   +' CASE WHEN C1.short=''2'' THEN (ISNULL(orders.c_address3,'''') + space(2) + ISNULL(orders.c_address4,'''')'
                   +' + space(2) + ISNULL(orders.c_city,'''') + space(2) + ISNULL(orders.c_zip,'''') + space(2) +'
                   +' ISNULL(orders.c_country,'''')) ELSE (ISNULL(orders.c_address4,'''')+ space(2) +ISNULL(orders.c_address1,'''')) END,'     --20
                   +' ISNULL(ST.B_address1,''''),ISNULL(ST.B_address2,''''),ISNULL(ST.B_address3,''''),ISNULL(orders.C_Company,''''),'''','    --25  --WL01                                                                 
                   +' '''','''','''','''','''','''','''','''','''','''', '  --35  
                   +' '''','''','''','''','''','''','''','''','''','''' ,'''','''','''','''','''','   --50
                   +' '''','''','''','''','''','''','''','''','''','''' '                              --60'
                   ...
                   +' LEFT JOIN CODELKUP C1 WITH (NOLOCK) ON C1.LISTNAME=''COUR_PREF'' ' 
                   +'                                     AND C1.Storerkey=orders.StorerKey AND C1.Code2=''YMTHK'' '   
                   +' LEFT JOIN CODELKUP C2 WITH (NOLOCK) ON C2.LISTNAME=''YMT_PREF'' ' 
...
EXIT_SP:  
   ...
   EXEC isp_InsertTraceInfo 
      ...
      @c_TraceName = 'isp_BT_Bartender_HK_WAYBILLLBL_label',
