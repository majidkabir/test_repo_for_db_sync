...
/************************************************************************/
/* Stored Procedure: nsp_GetPickSlipOrders15 SOS# 11482                 */
/* Creation Date:                                                       */
/* Copyright: IDS                                                       */
/* Written by:                                                          */
/*                                                                      */
/* Remarks:                                                             */
/*                                                                      */
/* Purpose: To speed up the process of picking, some large volume orders*/
/*          need to divide into several pick slips and distribute to few*/
/*          picker to do the picking.                                   */
/* Called By:                                                           */
/*                                                                      */
/* PVCS Version: 1.1                                                    */
/*                                                                      */
...
/************************************************************************/
CREATE PROC [dbo].[nsp_GetPickSlipOrders15] (@c_loadkey NVARCHAR(10)) 
 AS
 BEGIN
   SET NOCOUNT ON 
   SET ANSI_NULLS OFF 
   ...
   SET CONCAT_NULL_YIELDS_NULL OFF
    ...
    BEGIN TRAN
       ...
       SELECT PickDetail.OrderKey, pickdetail.Storerkey
       ...
       FETCH NEXT FROM pickslip_cur INTO @c_orderkey, @c_storerkey
       ...
       BEGIN
 			 ...
 			 BEGIN
 				...
 				BEGIN
 					...
 					BEGIN TRAN
					-- Parent P/S, Zone = '3', Child P/S, Zone = '1'
 					...
 					BEGIN
						...
						BEGIN
 					      COMMIT TRAN
							...
							BEGIN TRAN
 							EXEC nsp_SplitChildParallel @c_loadkey, @c_orderkey, @c_pickheaderkey
 		 					SELECT @n_err = @@ERROR
 		 					IF @n_err <> 0 
 		 					BEGIN
 		 						IF @@TRANCOUNT >= 1
          ...
          BEGIN
             SELECT @c_ConsigneeKey = Orders.BillToKey,
                    ...
                    @c_Notes2       = CONVERT(NVARCHAR(60), ORDERS.Notes2),
				 		  @c_labelprice   = ISNULL( UPPER(ORDERS.LabelPrice), 'N' ),
          ...
          IF @c_Route_Desc    IS NULL SELECT @c_Route_Desc = ""
		    ...
		    DECLARE pick_cur CURSOR FAST_FORWARD READ_ONLY FOR 
		       SELECT PickDetail.sku,       PickDetail.loc, 
		              ...
		              Pickdetail.Lot,			Pickheader.PickHeaderkey,
						  LOC.Putawayzone
		       FROM   PickDetail (NOLOCK),  LoadPlanDetail (NOLOCK), 
		       ...
		       AND    LoadPlanDetail.LoadKey = @c_loadkey
				 AND    PickHeader.Consigneekey = @c_pickheaderkey
				 AND    PickDetail.Pickslipno = PickHeader.Pickheaderkey
				 AND    PickHeader.Zone = "1" -- Child P/S
		       GROUP BY PickDetail.sku,       PickDetail.loc,      PACK.Qty,
		                PICKDETAIL.UOM,		  LOC.LogicalLocation,  Pickdetail.Lot, 
							 Pickheader.PickHeaderkey, LOC.Putawayzone
		       ORDER BY PickHeader.Pickheaderkey  
		       OPEN pick_cur

		       FETCH NEXT FROM pick_cur INTO @c_sku, @c_loc, @n_Qty, @n_uom3, 
		 		      @c_UOM, @c_logicalloc, @c_lot, @c_ChildPS, @c_PAZone
		       WHILE (@@FETCH_STATUS <> -1)
		       BEGIN
		          ...
		          INSERT INTO #Temp_Pick
		               (PickSlipNo,          LoadKey,          OrderKey,         ConsigneeKey,
		                ...
		                Lottable04, 			 LabelPrice, 		 ExternOrderKey,	 ChildPS )
		          VALUES 
		               (@c_pickheaderkey,   @c_LoadKey,       @c_OrderKey,     @c_ConsigneeKey,
		                ...
		                @n_UOMQty, 	   	@c_PrintedFlag,   @c_PAZone,
				 	       @c_Lot,		   		@c_Carrierkey,   	@c_VehicleNo,     @c_Lottable02,
		                @d_Lottable04, 		@c_labelprice, 	@c_externorderkey,@c_ChildPS)

		          FETCH NEXT FROM pick_cur INTO @c_sku, @c_loc, @n_Qty, @n_uom3, @c_UOM, @c_logicalloc, @c_LOT, @c_ChildPS, @c_PAZone
		       END
		       CLOSE pick_cur	
		       DEALLOCATE pick_cur   
       ...
       DEALLOCATE pickslip_cur   
		...
		-- ORDER BY Orderkey, ChildPS

		SELECT PickSlipNo, Loadkey, Orderkey, Consigneekey, Company, Addr1, Addr2, Addr3, PostCode, 
			Route, Route_Desc, TrfRoom, Notes1, Notes2, 
			LOC, SKU, SkuDesc, Qty = SUM(Qty), TempQty1	= TempQty1, TempQty2	= SUM(TempQty2),
			PrintedFlag, Zone, PgGroup, Carrierkey, VehicleNo, 
			Lottable02, Lottable04, LabelPrice, ExternOrderKey, ChildPS
		FROM #temp_pick
		GROUP BY PickSlipNo, Loadkey, Orderkey, Consigneekey,	Company, Addr1, Addr2, Addr3, PostCode, Route, Route_Desc, TrfRoom, Notes1, Notes2, 
			LOC, SKU, SkuDesc, PrintedFlag, Zone, PgGroup, Carrierkey, VehicleNo, 
			Lottable02, Lottable04, LabelPrice, ExternOrderKey, ChildPS, TempQty1
		ORDER BY Orderkey, ChildPS, Zone, Loc, SKU
