...
/*************************************************************************/    
/* Stored Procedure: ispRVWAV62                                          */    
/* Creation Date: 31-May-2023                                            */    
/* Copyright: MAERSK                                                     */    
/* Written by: WLChooi                                                   */    
/*                                                                       */    
/* Purpose: WMS-22628 - [CN] Desigual_WMS_ReverseWaveRelease             */    
/*                                                                       */    
/* Called By: Wave                                                       */    
/*                                                                       */    
/* GitLab Version: 1.0                                                   */    
...
/*************************************************************************/  
CREATE   PROCEDURE [dbo].[ispRVWAV62]
 ...
 BEGIN
    ...
    BEGIN
       ...
       BEGIN
         ...
         BEGIN
            SELECT @n_continue = 3
            SELECT @n_err = 81010
            SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': This Wave has not been released. (ispRVWAV62)'
         END
       END
    END
    ...
    BEGIN
        ...
        BEGIN
          SELECT @n_continue = 3
          SELECT @n_err = 81020
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Some Tasks have been started. Not allow to Reverse Wave Released (ispRVWAV62)'
        END
    END
    ...
    BEGIN
         ...
         AND TASKDETAIL.Sourcetype IN ('ispRLWAV62')
         ...
         BEGIN
           ...
           SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Delete Taskdetail Table Failed. (ispRVWAV62)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '
         END
    END
    ...
    BEGIN
         ...
         BEGIN
           ...
           SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Delete rdtPTLStationLog Table Failed. (ispRVWAV62)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '
         END
    END
    ...
    BEGIN
         ...
         BEGIN
           ...
           SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Pickdetail Table Failed. (ispRVWAV62)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '
         END
    END
    ...
    BEGIN
      ...
      WHERE WAVEKEY = @c_wavekey
       ...
       BEGIN
          ...
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update on wave Failed (ispRVWAV62)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '
       END
    END
...
RETURN_SP:
    ...
    BEGIN
       ...
       BEGIN
          ...
          END
       END
       EXECUTE nsp_logerror @n_err, @c_errmsg, 'ispRVWAV62'
