...
/*************************************************************************/
/* Stored Procedure: isp_Delivery_Note23                                  */
...
/*                                                                        */
/*                                                                        */
/* Called By: report dw = r_dw_dlivery_note_23                            */
/*                                                                        */
/* PVCS Version: 1.1                                                      */
/*                                                                        */
/* Version: 5.4                                                           */
...
/* 19-MAY-2021  ENTONG    2.4   WMS-17006 - Add new facility - 'WGQUS'(ET01)*/
/**************************************************************************/

CREATE PROC [dbo].[isp_Delivery_Note23] (
...
DECLARE @c_OrderKey            NVARCHAR(10)
       ...
       ,@c_OrderKey_Inv        NVARCHAR(10)    --CS05
       ,@c_CLKUDF01            NVARCHAR(60) = '' --WL03
       ,@c_CLKUDF02            NVARCHAR(60) = '' --WL03

 CREATE TABLE #TEMP_DelNote23
         (  Rowid            INT IDENTITY(1,1),
            ...
            OrderKey_Inv     NVARCHAR(10) NULL    --(CS05)           
         )
    ...
    CREATE TABLE #TEMP_CTHTYPEDelNote23 (
        ...
        SELECT DISTINCT ORDERS.OrderKey,
               ...
               ,OrderKey_Inv = CASE WHEN @c_ShipType = 'L' THEN ORDERS.Orderkey ELSE '' END  --(CS05)  
      ...
      FETCH FROM CS_ORDERS_INFO INTO @c_OrderKey, @c_MBOLKey, @c_pmtterm, @c_ExtPOKey,
        ...
        BEGIN
         ...
         IF @c_facility IN ('BULIM','WGQAP','WGQBL','WGQUS')   --CS06 --(ET01)
            BEGIN
             ...
             BEGIN
               ...
               /*CS04a End*/
                ...
                -- SELECT @n_TTLPLT= CASE WHEN C.UDF01='P' THEN COUNT(DISTINCT CD.palletkey)  ELSE 0 END   --CS04a
                 ...
                 IF @c_facility in ('WGQAP','YPCN1','WGQBL','WGQUS')     --CS06 --ET(01)
                 BEGIN
    ...
    --WL03 End
      ...
      , InvoiceNo = CASE WHEN OrderKey_Inv = '' THEN MBOLKey ELSE 'A' + RTRIM(OrderKey_Inv) END  --CS05
