...
GO
/************************************************************************/    
/* Stored Procedure: nspALLOG2                                          */    
/* Creation Date: 22-AUG-2019                                           */    
/* Copyright: LFL                                                       */    
/* Written by:                                                          */    
/*                                                                      */    
/* Purpose: WMS-10084 CN/SG Logitech Allocation for kitting             */
/*                                                                      */
...
BEGIN    
   ...
   IF @n_StorerMinShelfLife IS NULL
      ...
      SET @c_SQL = N'   
         ...
         CASE WHEN @c_UOM IN ('1','2') THEN ' AND LOC.LocationType NOT IN (''DYNPPICK'',''PICK'') AND LOC.LocationCategory <> ''DYNPPICK'' ' ELSE '' END +
         ...
         ' ORDER BY LA.Lottable05, LA.Lottable08, ' +
           CASE WHEN @c_UOM = '1' THEN ' CASE WHEN LOC.LocationHandling = ''1'' THEN 1 ELSE 2 END, 4, LOC.LogicalLocation, LOC.LOC ' 
                WHEN @c_UOM = '2' THEN ' CASE WHEN LOC.LocationHandling = ''2'' THEN 1 ELSE 2 END, 4, LOC.LogicalLocation, LOC.LOC ' 
                ELSE ' CASE WHEN SL.LocationType IN(''PICK'',''CASE'') OR LOC.LocationType IN(''PICK'',''CASE'',''DYNPPICK'') OR LOC.LocationCategory = ''DYNPPICK'' ' + 
                                          ' THEN 1 ' +
                          ' WHEN LOC.LocationHandling = ''2'' AND LOC.LocationCategory = ''SHELVING'' THEN 2 ELSE 3 END,3, LOC.LogicalLocation, LOC.LOC ' END  
