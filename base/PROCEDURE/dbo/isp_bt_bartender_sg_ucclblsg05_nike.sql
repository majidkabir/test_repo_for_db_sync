...
/********************************************************************************/
/* Copyright: MAERSK                                                            */
/* Purpose: isp_BT_Bartender_SG_UCCLBLSG05_NIKE                                 */
/*                                                                              */
/* Modifications log:                                                           */
/*                                                                              */
/* Date        Rev  Author    Purposes                                          */
/* 11-JUL-2023 1.0  CSCHONG   Created (WMS-23038)                               */
/* 11-JUL-2023 1.0  CSCHONG   DevOps Combine Script                             */
/********************************************************************************/

CREATE   PROC [dbo].[isp_BT_Bartender_SG_UCCLBLSG05_NIKE]
(
...
BEGIN
   ...
   SET @c_SQLJOIN = N' SELECT DISTINCT OH.ExternOrderkey, OH.Orderkey, PH.Pickslipno + CAST(PD.CartonNo AS NVARCHAR(10)), TRIM(ISNULL(OH.C_Contact1,'''')), '
                  + N' TRIM(ISNULL(OH.C_Phone1,'''')),'  --5
                  + N' SUBSTRING(TRIM(ISNULL(OH.C_Address1,''''))+ '' '' + TRIM(ISNULL(OH.C_Address2,''''))+ '' '' + TRIM(ISNULL(OH.C_Address3,'''')),1,80), ' + CHAR(13) --6
                  + N' TRIM(ISNULL(OH.C_Zip,'''')),UPPER(REPLACE(CONVERT(NVARCHAR(11),GETDATE(),106),'' '' ,''/'')), '  --8
                  + N' CAST(PD.CartonNo AS NVARCHAR(10)) + ''/'' + CAST(@c_Sparm03 AS NVARCHAR(10)), TRIM(ISNULL(OH.Userdefine10,'''')), ' + CHAR(13) --10   
                  + N' TRIM(ISNULL(OH.Route,'''')), ''P''+ PH.Pickslipno + CAST(PD.CartonNo AS NVARCHAR(10)), ' + CHAR(13)  --12
                  + N' '''', '''', '''','''', '''', '''', '''', '''', ' + CHAR(13) --20          
                  + N' '''', '''', '''', '''', ' + CHAR(13) --24
                  + N' '''','''', '''', '''', '''', '''', ' + CHAR(13) --30     
                  + N' '''', '''', '''', '''', '''', '''', '''', '''', '''', '''', ' + CHAR(13) --40  
                  ...
                  + N' AND PD.cartonno = CAST(@c_Sparm02 AS INT) ' 
                  + @c_Sorting
