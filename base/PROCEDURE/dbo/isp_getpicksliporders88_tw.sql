...
GO
/************************************************************************/  
/* Stored Proc: isp_GetPickSlipOrders88_tw                              */  
/* Creation Date: 17-Mar-2022                                           */  
/* Copyright: LF Logistics                                              */  
/* Written by: CSCHONG                                                  */  
/*                                                                      */  
/* Purpose:WMS-19116-[TW] VFEC-EC PickSlip RCMReport_CR                 */  
/*        :                                                             */  
/* Called By:r_dw_print_pickorder88_tw                                  */  
/*           duplicate from r_dw_print_pickorder88                      */  
/*          :                                                           */  
/* PVCS Version: 1.0                                                    */  
/*                                                                      */  
/* Version: 7.0                                                         */  
...
CREATE PROC [dbo].[isp_GetPickSlipOrders88_tw]  
...
BEGIN  
   ...
   IF @c_OrderkeyFrom = NULL SET @c_OrderkeyFrom = ''   
   IF @c_OrderkeyTo   = NULL SET @c_OrderkeyTo   = ''   
   ...
   CREATE TABLE #TMP_PCK88TW  
      ...
      , RowNo           NVARCHAR(40)  
      , FootertxtboxA   NVARCHAR(4000) NULL  -- CS01 S  
      , FootertxtboxB   NVARCHAR(4000) NULL    
      , FootertxtboxC   NVARCHAR(4000) NULL    
      , FootertxtboxD   NVARCHAR(4000) NULL  --CS01 E  
      )  
   ...
   AND LOADPLANDETAIL.OrderKey >= CASE WHEN @c_OrderkeyFrom = '' THEN @c_MinOrderkey ELSE @c_OrderkeyFrom END     
   AND LOADPLANDETAIL.OrderKey <= CASE WHEN @c_OrderkeyTo = '' THEN @c_MaxOrderkey ELSE @c_OrderkeyTo END         
   ...
   BEGIN  
      ...
      LEFT JOIN CODELKUP CL (NOLOCK) ON CL.Listname = 'REPORTCFG' AND CL.Storerkey = OH.Storerkey  
                            AND CL.Code = ISNULL(OI.[Platform],'') AND CL.Short = OH.OrderGroup  
                                    AND CL.Long = 'r_dw_print_pickorder88_tw'  
...
QUIT_SP:  
   ...
   SELECT #TMP_PCK.PickSlipNo  
         ...
         ,ROW_NUMBER() OVER(Partition by #TMP_PCK.PickSlipNo ORDER BY PICKDETAIL.Loc,PICKDETAIL.sku) AS RowNo  
          ...
          ,FooterTextBoxC = ISNULL(C3.UDF02,'')   
                 -- @c_FooterTextBoxD = ISNULL(C3.Notes2,'')--REPLACE(ISNULL(C4.Notes2,''),'$','~n')   
   ...
   LEFT JOIN CODELKUP C3 WITH (NOLOCK) ON C3.LISTNAME='REPORTCFG'   AND C3.Storerkey=ORDERS.StorerKey  
                                      AND C3.Code = OI.Platform AND C3.long='r_dw_print_pickorder88_tw'  
...
--select * from #TMP_PCK88 order by loc  
  ...
  SELECT Pickslipno, LOC, SKU from #TMP_PCK88TW WHERE ROWNO = 1 ORDER BY LOC,PICKSLIPNO,SKU DESC  
  OPEN CUR_sort  
 ...
 --select * from #UniquePSNO order by rowid  
  ...
  BEGIN  
     ...
     INSERT INTO #TMP_PCK88TW_Final (PickSlipNo, Contact1, SYCOLORSZ, Loadkey, Orderkey, OrdDate, EditDate  
      ...
      , FootertxtboxA,FootertxtboxB,FootertxtboxC,FootertxtboxD  )   --CS01  
     SELECT PickSlipNo, Contact1, SYCOLORSZ, Loadkey, Orderkey, OrdDate, EditDate  
      , ExternOrderkey, Notes, Loc, Storerkey, SKU, PFUDF01, EMUDF01 , Qty, JCLONG, Pageno, RptNotes, RowNo  
      , FootertxtboxA,FootertxtboxB,FootertxtboxC,FootertxtboxD     --CS01  
      FROM #TMP_PCK88TW  
     WHERE PickSlipNo NOT IN (SELECT DISTINCT PickSlipNo from #TMP_PCK88TW_Final) and PickSlipNo = @c_psno  
     ORDER BY LOC,SKU  
  ...
  END              
   ...
   --exec isp_GetPickSlipOrders88_tw '0001139617'  
   
   WHILE @n_ReqLine >= 1               
   BEGIN                   
                  
      INSERT INTO #TMP_PCK88TW_Final (  
            PickSlipNo  
              , Contact1  
              , SYCOLORSZ  
              , Loadkey  
   ...
   WHERE #TMP_PCK88TW_Final.PickSlipNo= @c_PSNo  
      AND #TMP_PCK88TW_Final.Pageno = @n_LastPage  
  ...
  SELECT * FROM #TMP_PCK88TW_Final AS tp  
  ORDER BY Pickslipno, CASE WHEN sku = '' THEN 2 ELSE 1 END,tp.Pageno	--ml01
