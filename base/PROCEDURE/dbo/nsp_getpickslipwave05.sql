...
GO
/************************************************************************/
/* Stored Procedure: nsp_GetPickSlipWave05                              */
/* Creation Date: 11-Sept-2006                                          */
/* Copyright: IDS                                                       */
/* Written by: Vicky                                                    */
/*                                                                      */
/* Purpose:                                                             */
...
/* 25-09-2006   ybyong  1.0   Created                                   */
/* 01-12-2006   dhung   1.01  SOS63283 Change grouping from LOT to      */
/*                                     Lottable02, Lottable04,Lottable03*/
/* 01-12-2010   Leong   1.1   Add (NOLOCK) for LotAttribute -- Leong01  */
/*                            Add TraceInfo -- Leong01                  */
/* 23-MAY-2012  YTWan   1.2   SOS#244656:Split by PickZone (Wan01)      */
/* 20-DEC-2012  YTWan   1.3   SOS#262552:PNG Sorting by logicalloc, loc */
/*                            if pickzon='P' (Wan02)                    */
/* 01-NOV-2013  NJOW01  1.4   291449-Add OVAS and summary pick detail   */ 
/* 07-JUL-2014  NJOW02  1.5   315384-add full pallet pick indicator     */
/* 07-OCT-2015  CSCHONG 1.6   SOS#353485 (CS01)                         */
/* 16-JUN-2017  CSCHONG 1.7   WMS-2114 - revise inner qty (CS02)        */
/* 28-Jan-2019  TLTING_ext 1.8 enlarge externorderkey field length      */
/* 29-Jul-2019  WLCHOOI 1.9   WMS-9907 - Add ReportCFG to show or hide  */
/*                                       Pickdetail.ID (WL01)           */
/* 30-Nov-2021  MINGLE  2.0   WMS-18460 add buyerpo(ML01)               */
/* 30-Nov-2021  Mingle  2.0   DevOps Combine Script                     */
/* 06-Dec-2021  MINGLE  2.1   WMS-18460 remove filter(ML02)             */
/************************************************************************/

CREATE PROC [dbo].[nsp_GetPickSlipWave05] (
...
DECLARE
   ...
   @n_StartTCnt        int,
   @c_OVAS             NVARCHAR(30),
   @c_fullpallet       NVARCHAR(5), --NJOW02
   @c_RetailSku        NVARCHAR(20), --CS01
   @c_ID               NVARCHAR(18), --WL01
   @c_ShowID           NVARCHAR(10), --WL01
   @c_Buyerpo          NVARCHAR(20)  --ML01
...
DECLARE
   ...
   @n_TotalCubic       int,
   @n_TraceFlag        Int
   ,  @c_PickZoneCfg NVARCHAR(10)             --(Wan01)
   ,@c_puom02        NVARCHAR(10)            --CS02
...
--   SKUxLOCQty       int NULL,
   ...
   FullPallet       NVARCHAR(5) NULL, --NJOW02
   RetailSKU        NVARCHAR(20) NULL,--CS01
  -- innerqty         INT NULL,       --CS02
   --Ctnqty           INT NULL)       --CS02
   PUOM02           NVARCHAR(10),     --CS02
   ID               NVARCHAR(18),     --WL01
   ShowID           NVARCHAR(10),     --WL01
...
SET @c_PickZoneCfg   = ''                    --(Wan01)
SET @c_Retailsku = ''                        --(CS01)  
...
LEFT JOIN Codelkup CLR (NOLOCK) ON (O.Storerkey = CLR.Storerkey AND CLR.Code = 'SHOWFULLPLT' 
                                    AND CLR.Listname = 'REPORTCFG' AND CLR.Long = 'r_dw_print_wave_pickslip_05' AND ISNULL(CLR.Short,'') <> 'N')
WHERE W.Wavekey = @c_wavekey 
--AND O.Userdefine08 = 'Y'    --ML02
AND P.Status < '5'
...
SELECT PickDetail.sku,       PickDetail.loc,
       ...
       ISNULL(PT.FullPallet,'') AS FullPallet, --NJOW02
       CASE WHEN CS.B_Company = 'YFY' THEN SKU.RetailSKU ELSE '' END AS RetailSku,     --CS01 
       CASE WHEN ISNULL(CL1.Short,'N') = 'Y' THEN PickDetail.ID ELSE '' END AS ID,     --WL01
       ISNULL(CL1.Short,'N') AS ShowID                                                 --WL01
...
JOIN   LotAttribute LA (NOLOCK) ON (PickDetail.LOT = LA.LOT) -- Leong01
...
LEFT JOIN #TMP_PICKTYPE PT (NOLOCK) ON (PickDetail.Storerkey = PT.Storerkey AND PickDetail.Sku = PT.Sku AND LOC.Loc = PT.Loc 
                                    ...
                                    AND LA.Lottable04 = PT.Lottable04 AND Pickdetail.Orderkey = PT.Orderkey) --NOW02
LEFT JOIN CODELKUP CL1 (NOLOCK) ON CL1.LISTNAME = 'REPORTCFG' AND CL1.CODE = 'ShowPickDetailID'  --WL01
                               AND CL1.Long = 'r_dw_print_wave_pickslip_05'                      --WL01
                               AND CL1.Storerkey = ORDERS.StorerKey                              --WL01
WHERE PickDetail.Status < '5'
--ORDERS.Userdefine08 = 'Y' -- only for wave plan orders.    --ML02
...
CASE WHEN ISNULL(CL.Code,'') <> '' THEN
         SKU.OVAS ELSE '' END ,
ISNULL(PT.FullPallet,''), --NJOW02  
CASE WHEN CS.B_Company = 'YFY' THEN SKU.RetailSKU ELSE '' END,            --CS01      
CASE WHEN ISNULL(CL1.Short,'N') = 'Y' THEN PickDetail.ID ELSE '' END,     --WL01
...
FETCH NEXT FROM pick_cur INTO @c_sku, @c_loc, @n_Qty, @n_uom3, @c_storerkey,
                              ...
                              @c_FullPallet,@c_Retailsku, --NJOW02  --CS01
                              @c_ID, @c_ShowID  --WL01
...
BEGIN --While
   ...
   BEGIN
      ...
      BEGIN
         ...
         SELECT 'nsp_GetPickSlipWave05', GetDate(), @c_wavekey, OrderKey, OrderLineNumber, Lot, Loc, Id, Sku, StorerKey, PickDetailKey, Suser_Sname()
   ...
   BEGIN --if @c_orderkey <> ''
      SELECT @c_ConsigneeKey  = Orders.Consigneekey,
            ...
            @c_Addr1          = ISNULL(ORDERS.C_City,'') + ISNULL(ORDERS.C_Address1,''),  --ML02
   ...
   */
   ...
   SELECT @c_putawayzone = CASE @c_PickZoneCfg WHEN '1' THEN ISNULL(RTRIM(LOC.PickZone), '')       --(Wan01)
                                                        ELSE ISNULL(RTRIM(LOC.Putawayzone), '')    --(Wan01)
                                                        END,                                       --(Wan01)
   ...
   (PickSlipNo,         wavekey,          OrderKey,         ConsigneeKey,
    ...
    LogicalLoc,         OVAS,					FullPallet, RetailSku ,PUOM02,      --CS02
    ID,                 ShowID,           Buyerpo          ) --WL01 --ML01
   VALUES
   (@c_pickheaderkey,    @c_wavekey,       @c_OrderKey,      @c_ConsigneeKey,
    ...
    @c_logicalloc,       @c_OVAS,          @c_FullPallet,@c_retailsku,@c_puom02,    --CS02
    @c_ID,               @c_ShowID,        @c_buyerpo) --WL01 --ML01
...
QUIT:
   ...
   BEGIN
      ...
      BEGIN
         ...
         END
      END
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'nsp_GetPickSlipWave05'
