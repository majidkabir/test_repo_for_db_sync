...
/************************************************************************/
/* Stored Procedure: nspPRTW11                                          */
/* Creation Date: 03-May-2016                                           */
/* Copyright: LF                                                        */
/* Written by:                                                          */
/*                                                                      */
/* Purpose: 368678-TW-PHA-VIP Pre-Allocation FIFO                       */
/*                                                                      */
...
/* 24-May-2016 NJOW01   1.0  368678-Map lottable01 to LOC.HostWHCode    */
/************************************************************************/

CREATE PROC [dbo].[nspPRTW11]
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         SELECT @c_Condition = " AND LOC.HostWhCode = N'" + dbo.fnc_RTrim(dbo.fnc_LTrim(@c_Lottable01)) + "' " --NJOW01
      END
      ...
      IF ISNULL(@c_Secondary,'') = 'LUMVIP'
      BEGIN
         SET @c_OrderBy = " ORDER BY CASE WHEN Lottable06 = 'A' THEN 0 ELSE 1 END, Lotattribute.Lottable05, LOT.Lot " 
      END
      ELSE
      BEGIN
         SET @c_OrderBy = " ORDER BY CASE WHEN ISNULL(Lottable06,'') <> 'A' THEN 0 ELSE 1 END, Lotattribute.Lottable05, LOT.Lot " 
      END
