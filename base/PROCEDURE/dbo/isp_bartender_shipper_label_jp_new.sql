...
/********************************************************************************/                 
/* Copyright: IDS                                                               */                 
/* Purpose: isp_Bartender_Shipper_Label_JP_New                                  */   
/*          (copy from isp_Bartender_Shipper_Label_JP)                          */              
/*                                                                              */                 
/* Modifications log:                                                           */                 
/*                                                                              */                 
/* Date        Rev  Author     Purposes                                         */                 
/* 26-Mar-2021 1.0  ML01       to remove table linkage for storer = 'HAJP'      */
/********************************************************************************/                
                  
CREATE PROC [dbo].[isp_Bartender_Shipper_Label_JP_New]                      
...
BEGIN                      
  ...
  DECLARE @d_Trace_StartTime   DATETIME,   
   ...
   SET @c_SQLJOIN = + N' SELECT DISTINCT (N''?'' + Substring(ORD.C_Zip,1,3) + ''-'' + Substring(ORD.C_Zip,4,4)),ORD.c_phone1,' 
                    ...
                    + N' Substring(CONVERT(VARCHAR(10),dateadd(day,30,getdate()),112),3,2)+ N''?'' + Substring(CONVERT(VARCHAR(10),dateadd(day,30,getdate()),112),5,2) '
                    + N' + N''?'' +  Substring(CONVERT(VARCHAR(10), dateadd(day, 30 , getdate()),112) , 7,2) + N''??'','
                    ...
                    --+ ' JOIN PICKDETAIL PICKDET WITH (NOLOCK) ON PICKDET.caseid = PACKDET.labelno'          --ML01
                    ...
                    + ' LEFT JOIN CODELKUP C5 WITH (NOLOCK) ON C5.Listname = ''YamatoLbl'' AND C5.Storerkey = ORD.Storerkey AND ISNULL(C5.Short,'''') = ORD.C_Company '   
...
EXIT_SP:    
   ...
   EXEC isp_InsertTraceInfo   
      ...
      @c_TraceName = 'isp_Bartender_Shipper_Label_JP_New',  
