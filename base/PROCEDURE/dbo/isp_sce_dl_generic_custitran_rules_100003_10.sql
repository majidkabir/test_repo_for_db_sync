...
GO
/************************************************************************/
/* Store Procedure:  isp_SCE_DL_GENERIC_CUSTITRAN_RULES_100003_10       */
...
CREATE   PROCEDURE [dbo].[isp_SCE_DL_GENERIC_CUSTITRAN_RULES_100003_10] (
   @b_Debug       INT            = 0
 , @n_BatchNo     INT            = 0
 , @n_Flag        INT            = 0
...
BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5), ISNULL(@n_ErrNo, 0))
                            + ': Update record fail. (isp_SCE_DL_GENERIC_CUSTITRAN_RULES_100003_10)';
            ROLLBACK;
   ...
   BEGIN
      ...
      SELECT DISTINCT ISNULL(RTRIM(UOM), '')
                    , RTRIM(StorerKey)
                    , RTRIM(SKU)
      FROM dbo.SCE_DL_CUSTITRAN_STG WITH (NOLOCK)
      WHERE STG_BatchNo = @n_BatchNo
      ...
      BEGIN
         ...
         IF @c_UOM <> ''
         BEGIN
            SELECT @c_PackKey =PACKKey FROM WMS.SKU WITH(NOLOCK)
            WHERE StorerKey = @c_StorerKey
            AND Sku = @c_SKU

            IF EXISTS (
            SELECT 1
            FROM WMS.PACK WITH (NOLOCK)
         ...
         --   FROM dbo.SCE_DL_CUSTITRAN_STG STG
   ...
   IF @b_Debug = 1
   BEGIN
      SELECT '<<SUB-SP-RULES>> - [isp_SCE_DL_GENERIC_CUSTITRAN_RULES_100003_10] EXIT... ErrMsg : ' + ISNULL(RTRIM(@c_ErrMsg), '');
   END;
