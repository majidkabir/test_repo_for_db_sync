 ...
 BEGIN    
   ...
   BEGIN    
      ...
      BEGIN    
         SELECT @n_continue = 3    
         SELECT @n_err = 81010    
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Invalid Parameters Passed (ispRLWAV34)'    
      END    
   END      
   ...
   IF @n_continue=1 or @n_continue=2    
   BEGIN            
      IF ISNULL(@c_DispatchPiecePickMethod,'') NOT IN ('SEPB2BPTS','SEPB2CAGV','SEPB2CPTS')
      BEGIN  
         SELECT @n_continue = 3    
         SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err = 81020   -- Should Be Set To The SQL Errmessage but I don't know how to do so.    
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Invalid Wave.DispatchPiecePickMethod (ispRLWAV34)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '             
      END                  
   END  
   
   --Validate Userdefine02 and Userdefine03 (Do not validate for SEPB2CAGV)
   IF @n_continue=1 or @n_continue=2    
   BEGIN            
      IF (ISNULL(@c_Userdefine02,'') = '' OR ISNULL(@c_Userdefine03,'') = '') AND ISNULL(@c_DispatchPiecePickMethod,'') NOT IN ('SEPB2CAGV')
      BEGIN           
         SELECT @n_continue = 3    
         SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err = 81030   -- Should Be Set To The SQL Errmessage but I don't know how to do so.    
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Must key-in location range at userdefine02 & 03 (ispRLWAV34)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '             
      END                  
   END  
   ...
   BEGIN  
      IF EXISTS (SELECT 1 FROM TASKDETAIL TD (NOLOCK)   
                 ...
                 AND TD.Sourcetype IN('ispRLWAV34-B2B','ispRLWAV34-B2C')
                 AND TD.Tasktype IN ('RPF'))   
      BEGIN  
         SELECT @n_continue = 3    
         SELECT @n_err = 81040    
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': This Wave has beed released. (ispRLWAV34)'         
      END                   
   END  
   ...
   BEGIN  
      ...
      BEGIN   
         ...
         BEGIN  
            ...
            SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Pickdetail Table Failed. (ispRLWAV34)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
         END    
   ...
   BEGIN
      ...
      BEGIN
         SELECT @c_SourcekeyCNT = COUNT(DISTINCT S.BUSR4)
         FROM WAVEDETAIL WD (NOLOCK)  
         JOIN ORDERS O (NOLOCK) ON WD.Orderkey = O.Orderkey  
         ...
         BEGIN
            IF @c_Userdefine04 < @c_SourcekeyCNT
            BEGIN
               SELECT @n_continue = 3    
               SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err = 81069   -- Should Be Set To The SQL Errmessage but I don't know how to do so.    
               SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': WAVE.Userdefine04 < COUNT(DISTINCT SKU.BUSR4) . (ispRLWAV34)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
               GOTO RETURN_SP 
            END
         END
      ...
      IF (@n_continue = 1 OR @n_continue = 2) AND @c_DispatchPiecePickMethod IN ('SEPB2BPTS','SEPB2CPTS')
      BEGIN
         DECLARE cur_UCCDetail CURSOR LOCAL FAST_FORWARD READ_ONLY FOR    
         SELECT CASE WHEN @c_DispatchPiecePickMethod = 'SEPB2BPTS' THEN S.BUSR4 ELSE '' END,
                CASE WHEN @c_DispatchPiecePickMethod = 'SEPB2BPTS' THEN O.Loadkey ELSE O.Orderkey END  
         FROM WAVEDETAIL WD (NOLOCK)  
         JOIN ORDERS O (NOLOCK) ON WD.Orderkey = O.Orderkey  
         JOIN PICKDETAIL PD (NOLOCK) ON O.Orderkey = PD.Orderkey  
         JOIN SKU S (NOLOCK) ON PD.SKU = S.SKU AND PD.Storerkey = S.Storerkey
         WHERE WD.Wavekey = @c_Wavekey AND PD.UOM <> '2'  
         GROUP BY CASE WHEN @c_DispatchPiecePickMethod = 'SEPB2BPTS' THEN S.BUSR4 ELSE '' END,
                  CASE WHEN @c_DispatchPiecePickMethod = 'SEPB2BPTS' THEN O.Loadkey ELSE O.Orderkey END  
         ORDER BY CASE WHEN @c_DispatchPiecePickMethod = 'SEPB2BPTS' THEN O.Loadkey ELSE O.Orderkey END,
                  CASE WHEN @c_DispatchPiecePickMethod = 'SEPB2BPTS' THEN S.BUSR4 ELSE '' END
        
         OPEN cur_UCCDetail    
         FETCH NEXT FROM cur_UCCDetail INTO @c_SKUBUSR4, @c_Sourcekey  
         ...
         BEGIN                        
            ...
            IF @b_debug = 1
               SELECT @c_SKUBUSR4 AS SKUBUSR4, @c_Sourcekey AS Sourcekey 
      
            IF ISNUMERIC(@c_Userdefine04) = 1
            BEGIN
               IF @c_PrevSourcekey = ''   --First Loadkey/Orderkey
               BEGIN
                  SET @c_DPCount = ''
               END
               ELSE IF @c_PrevSourcekey <> @c_Sourcekey
               BEGIN
                  SET @c_DPCount = CAST(@c_FirstDP AS INT) + CAST(@c_Userdefine04 AS INT)
               END
            END
            ...
            AND (PTL.RowRef IS NULL   
                 OR ((PTL.Loadkey = @c_Sourcekey AND PTL.Userdefine01 = @c_SKUBUSR4) OR PTL.Orderkey = @c_Sourcekey)
                 )  
            ...
            BEGIN  
               ...
               SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': PTS Location Not Setup / Not enough PTS Location. (ispRLWAV34)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
               GOTO RETURN_SP  
            END   
            ...
            BEGIN  
               IF @c_DispatchPiecePickMethod = 'SEPB2BPTS'  
               BEGIN
                  INSERT INTO RDT.rdtPTLStationLog (Station, IPAddress, Position, Loc, Wavekey, Storerkey, ShipTo, Userdefine01, Loadkey, Sourcekey)  
                  VALUES (@c_DeviceId, @c_IPAddress, @c_DevicePosition, @c_PTSLoc, @c_Wavekey, @c_Storerkey, '', @c_SKUBUSR4, @c_Sourcekey, @c_Wavekey)   
               END
               ELSE --SEPB2CPTS
               BEGIN
                  INSERT INTO RDT.rdtPTLStationLog (Station, IPAddress, Position, Loc, Wavekey, Storerkey, ShipTo, Userdefine01, OrderKey, Sourcekey)  
               ...
               BEGIN  
                  ...
                  SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert RDT.rdtPTLStationLog Failed. (ispRLWAV05)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
                  GOTO RETURN_SP  
               END     
            END  
              
            --Get first DevicePosition
            IF @c_PrevSourcekey <> @c_Sourcekey
            BEGIN
               SET @c_FirstDP = @c_DevicePosition
               --SELECT @c_FirstDP
            END
            ...
            IF NOT EXISTS(SELECT 1 FROM #TMP_PTS WHERE SourceKey = @c_Sourcekey AND BUSR4 = @c_SKUBUSR4)
            BEGIN
               INSERT INTO #TMP_PTS
               SELECT @c_Sourcekey, @c_SKUBUSR4, @c_ToLoc
            END
   ...
   BEGIN 
      ...
      IF @c_DispatchPiecePickMethod = 'SEPB2BPTS'  
         SELECT @c_SourceType = 'ispRLWAV34-B2B'  
      ELSE IF @c_DispatchPiecePickMethod = 'SEPB2CPTS'  
         SELECT @c_SourceType = 'ispRLWAV34-B2C'   
      ...
      BEGIN   
         ...
         IF @c_DispatchPiecePickMethod IN ('SEPB2BPTS','SEPB2CPTS') AND @c_LocType IN ('DPP','AGV')
         BEGIN
            IF @c_DispatchPiecePickMethod = 'SEPB2BPTS' AND @c_LocType = 'AGV' AND @n_TLogGenerated = 0
            BEGIN
               IF @c_DocType = 'N'
               BEGIN
                  IF EXISTS (SELECT TOP 1 1 FROM CODELKUP CL (NOLOCK) 
                             WHERE CL.LISTNAME = 'AGVCAT' 
                             AND CL.Code IN (SELECT LOC.LocationCategory FROM LOC (NOLOCK) WHERE LOC = @c_FromLoc)
                             AND CL.Storerkey = @c_Storerkey )
                  BEGIN
                     IF EXISTS (SELECT TOP 1 1 FROM CODELKUP CL (NOLOCK) 
                             WHERE CL.LISTNAME = 'AGVNOR' 
                             AND CL.Code IN (SELECT LOC.LocationType FROM LOC (NOLOCK) WHERE LOC = @c_FromLoc)
                             AND CL.Storerkey = @c_Storerkey )
                     BEGIN
                        SELECT @b_success = 1   
                        EXECUTE nspg_getkey
                        ...
                        BEGIN
                           SELECT @n_continue = 3
                           SELECT @c_errmsg = CONVERT(CHAR(250),@n_err), @n_err=83810   -- Should Be Set To The SQL Errmessage but I don't know how to do so.
                           SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Unable to Obtain transmitlogkey3. (ispRLWAV34)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + ' ) '
                           GOTO RETURN_SP  
                        END
                        ELSE
                        BEGIN
                           INSERT INTO Transmitlog3 (transmitlogkey, tablename, key1, key2, key3, transmitflag, TransmitBatch)
                           VALUES (@c_trmlogkey, 'RBTWAVEREL', @c_Wavekey, 'SKU', @c_StorerKey, '0', '')
         
                           SET @n_err = @@ERROR
                           IF @n_err <> 0    
                           BEGIN  
                              SELECT @n_continue = 3    
                              SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err = 83811   -- Should Be Set To The SQL Errmessage but I don't know how to do so.    
                              SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert Transmitlog Failed. (ispRLWAV34)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
                              GOTO RETURN_SP  
                           END  
                        END

                        SET @n_TLogGenerated = 1
                     END
                  END
               END
            END
         ...
         BEGIN  
            ...
            BEGIN     
               ...
               FROM #TMP_PTS t
               WHERE t.Sourcekey = @c_Sourcekey AND t.BUSR4 = @c_SKUBUSR4
            ...
            --      ELSE IF @c_PrevSourcekey <> @c_Sourcekey
            --      BEGIN
            --         SET @c_DPCount = CAST(@c_DevicePosition AS INT) + CAST(@c_Userdefine04 AS INT)
            --      END
            --   END
            ...
            --        )  
            --   AND DP.DevicePosition > @c_DPCount
            --   ORDER BY DP.DevicePosition, LOC.LogicalLocation, LOC.Loc  
   ...
   -----Trigger Order Out Interface-------  
   IF (@n_continue = 1 or @n_continue = 2) AND @c_DispatchPiecePickMethod IN ('SEPB2CAGV')
   BEGIN
      DECLARE CUR_LOOP CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
      SELECT OH.Loadkey, OH.Storerkey, MAX(OH.ECOM_SINGLE_FLAG)   --Check ECOM Single or Multi
      FROM ORDERS OH (NOLOCK) 
      JOIN WAVEDETAIL WD (NOLOCK) ON WD.Orderkey = OH.Orderkey
      WHERE WD.WaveKey = @c_wavekey
      AND OH.DocType = 'E'
      GROUP BY OH.Loadkey, OH.Storerkey

      OPEN CUR_LOOP

      FETCH NEXT FROM CUR_LOOP INTO @c_Loadkey, @c_Storerkey, @c_ECOMSingleFlag
      ...
      BEGIN
         ...
         BEGIN
            INSERT INTO Transmitlog3 (transmitlogkey, tablename, key1, key2, key3, transmitflag, TransmitBatch)
            VALUES (@c_trmlogkey, 'RBTLOADRCM', @c_Loadkey, CASE WHEN @c_ECOMSingleFlag = 'M' THEN 'ORDER' ELSE 'LOAD' END, @c_StorerKey, '0', '')
...
RETURN_SP:
   ...
   BEGIN    
      ...
      BEGIN    
         ...
         END    
      END    
      execute nsp_logerror @n_err, @c_errmsg, "ispRLWAV34"    
   ...
   BEGIN        
     INSERT TASKDETAIL    
      ...
      BEGIN  
          ...
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert Taskdetail Failed. (ispRLWAV34)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
          GOTO RETURN_SP  
      END     
   END  
