...
GO
/************************************************************************/  
/* Stored Procedure: isp_Kitting_Bartender_Print                        */  
/* Creation Date: 26-Apr-2019                                           */  
/* Copyright: LF                                                        */  
/* Written by:                                                          */  
/*                                                                      */  
/* Purpose: WMS-8769 - Kitting module print to bartender                */  
/*          Call custom SP ispKTBTxx                                    */
/*                                                                      */  
/* Called By: Packing                                                   */  
/*                                                                      */  
/* PVCS Version: 1.0                                                    */  
...
/************************************************************************/   
CREATE PROCEDURE [dbo].[isp_Kitting_Bartender_Print]
   @c_printerid  NVARCHAR(50) = '',  
   @c_labeltype  NVARCHAR(30) = '',  
   @c_userid     NVARCHAR(18) = '',  
   @c_Parm01     NVARCHAR(60) = '', --Kitkey         
...
BEGIN  
   ...
   SET @c_KitKey = @c_Parm01
   SET @c_Module  = 'KITTING'   
   ...
   AND    ConfigKey = 'Kitting_Bartender_SP'  
   ...
   BEGIN
      ...
      BEGIN
          ...
          SELECT @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5), @n_Err) + 
                 ': Storerconfig Kitting_Bartender_Print - Stored Proc name invalid ('+RTRIM(ISNULL(@c_SPCode,''))+') (isp_Kitting_Bartender_Print)'  
          GOTO QUIT_SP
      END
   ...
   BEGIN
       SELECT @b_success = 0
       EXECUTE nsp_logerror @n_Err, @c_ErrMsg, 'isp_Kitting_Bartender_Print'  
