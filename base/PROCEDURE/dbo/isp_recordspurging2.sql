...
GO
/************************************************************************/        
/* Store Procedure:  isp_RecordsPurging2                                */        
...
/*                    @cCondition    - set extra filtering              */        
/*                                                                      */        
...
/* 02-May-2012  TLTING    Add Filtering                                 */ 
/* 28-Jun-2012  TLTING    Delete by ArchiveCop if column exists         */  
/* 20-Nov-2015  TLTING    change table name length to 100               */
/* 23-Nov-2015  JayLim    Fix @cArchiveCopFlag value reset (Jay01)      */
/* 10-Aug-2017  TLTING01  1.1   Fix Dynamic SQL                         */
/* 05-Dec-2019   kocy     Fix tablename data string truncated issue     */
/************************************************************************/        
        
CREATE PROC [dbo].[isp_RecordsPurging2]        
     @cTableName1 NVARCHAR(100)        
   , @cTableName2 NVARCHAR(100)        
   , @cTableName3 NVARCHAR(100)        
   , @cTableName4 NVARCHAR(100)        
   , @cTableName5 NVARCHAR(100)        
   , @nDays       INT        
   , @cDateCol    NVARCHAR(30)   
...
BEGIN        
   ...
   CREATE TABLE [#TempTables] (        
         ...
         [TableName] [varchar] (100) NULL )        -- kocy extend the data length
...
SET @cMaxAddDate = CONVERT(CHAR(8), DATEADD(DAY, (@nDays * -1), GETDATE()), 112)      
...
IF @b_debug = 1        
BEGIN        
   SELECT '@cTableName : ' , @cTableName, ' @cMaxAddDate : ', @cMaxAddDate        
END        
...
DECLARE TempTables CURSOR READ_ONLY FAST_FORWARD FOR        
   ...
   WHILE @@fetch_status <> -1 AND (@cMaxAddDate <> '') AND (@cMaxAddDate IS NOT NULL)       
   BEGIN        
      IF @b_debug = 1        
      BEGIN        
      ...
      BEGIN  
         ...
         -- filter by condition  
         IF ISNULL(RTRIM(@cCondition), '') <> ''  
         BEGIN  
            SELECT @cSQL1 = @cSQL1 + ' AND ' + RTRIM(@cCondition) + master.dbo.fnc_GetCharASCII(13)   
         END  
...
---   
         ...
         SET @cExecStatements = @cSQL1 + @cSQL2 
         EXEC sp_executesql @cExecStatements, N'@cMaxAddDate Varchar(10) ', @cMaxAddDate
