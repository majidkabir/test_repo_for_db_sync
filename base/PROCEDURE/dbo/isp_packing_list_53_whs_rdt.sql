...
GO
/************************************************************************/
/* Stored Proc: isp_packing_list_53_WHS_rdt                             */
/* Creation Date: 24-FEB-2020                                           */
/* Copyright: LF Logistics                                              */
/* Written by: CSCHONG                                                  */
/*                                                                      */
/* Purpose: WMS-12195 - [JP] DW Delivery Note for Wholesale             */
/*        :                                                             */
...
/*                            (WL02)                                    */
/*2021-12-24   LZG      1.3   JSM-41804 - Used CEILING to fix decimal   */
/*                            to INT conversion error (ZG01)            */
/************************************************************************/
CREATE PROC [dbo].[isp_packing_list_53_WHS_rdt]
...
BEGIN
 ...
 CREATE TABLE #TMP_INV53WHS_Final
   ...
   SELECT @c_C1 = ISNULL(MAX(CASE WHEN CL.Code = 'C1'  THEN ISNULL(RTRIM(CL.Long),'') ELSE '' END),'')
         ...
         ,@c_C47= ISNULL(MAX(CASE WHEN CL.Code = 'C47' THEN ISNULL(RTRIM(CL.Long),'') ELSE '' END),'')
         ,@c_C48= ISNULL(MAX(CASE WHEN CL.Code = 'C48' THEN ISNULL(RTRIM(CL.Long),'') ELSE '' END),'')
         ,@c_C49= ISNULL(MAX(CASE WHEN CL.Code = 'C49' THEN ISNULL(RTRIM(CL.Long),'') ELSE '' END),'')
         ,@c_C50= ISNULL(MAX(CASE WHEN CL.Code = 'C50' THEN ISNULL(RTRIM(CL.Long),'') ELSE '' END),'')
   FROM CODELKUP CL WITH (NOLOCK)
   WHERE CL.ListName = 'DNCONST'

   INSERT INTO #TMP_INV53WHS
         (
            SortBy
         ,  PageGroup
         ,  Storerkey
   ...
   SELECT  SortBy  = ROW_NUMBER() OVER (ORDER BY LP.Loadkey
         ...
         --, UDUDF04 = CASE WHEN ISNUMERIC(OD.userdefine04) = 1 AND CAST(OD.userdefine04 as int) > 0 THEN OD.userdefine04 ELSE '' END  -- ZG01
         --, UDUDF05 = CASE WHEN ISNUMERIC(OD.userdefine05) = 1 AND CAST(OD.userdefine05 as int) > 0 THEN OD.userdefine05 ELSE '' END  -- ZG01
         , UDUDF04 = CASE WHEN ISNUMERIC(OD.userdefine04) = 1 AND CEILING(OD.userdefine04) > 0 THEN OD.userdefine04 ELSE '' END        -- ZG01
         , UDUDF05 = CASE WHEN ISNUMERIC(OD.userdefine05) = 1 AND CEILING(OD.userdefine05) > 0 THEN OD.userdefine05 ELSE '' END        -- ZG01
   ...
   GROUP BY OH.Storerkey
         ...
         --,  CASE WHEN ISNUMERIC(OD.userdefine04) = 1 AND CAST(OD.userdefine04 as int) > 0 THEN OD.userdefine04 ELSE '' END  -- ZG01
         --,  CASE WHEN ISNUMERIC(OD.userdefine05) = 1 AND CAST(OD.userdefine05 as int) > 0 THEN OD.userdefine05 ELSE '' END  -- ZG01
         ,  CASE WHEN ISNUMERIC(OD.userdefine04) = 1 AND CEILING(OD.userdefine04) > 0 THEN OD.userdefine04 ELSE '' END        -- ZG01
         ,  CASE WHEN ISNUMERIC(OD.userdefine05) = 1 AND CEILING(OD.userdefine05) > 0 THEN OD.userdefine05 ELSE '' END        -- ZG01
         ,  OH.buyerpo
   ...
   UPDATE INV
      ...
      ,   TotalQtyShipped = ( SELECT RTRIM(CONVERT(NVARCHAR(8), SUM(QtyShipped))) + ' ' + MAX(TMP.UOM)
                              FROM #TMP_INV53WHS TMP WHERE TMP.orderkey = INV.Orderkey)
      ,  TTLExtendedPrice  = ( SELECT SUM(ExtendedPrice)
                              FROM #TMP_INV53WHS TMP WHERE TMP.orderkey = INV.Orderkey)
   FROM #TMP_INV53WHS INV
   ...
   BEGIN
      INSERT INTO #TMP_INV53WHS_Final
         (
            SortBy
         ,  PageGroup
         ,  Storerkey
