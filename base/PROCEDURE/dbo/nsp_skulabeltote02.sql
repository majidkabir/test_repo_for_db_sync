...
GO
/************************************************************************/
/* Store Procedure:  nsp_SkuLabelTote02                                 */
/* Creation Date: 29-Jun-2010                           						*/
/* Copyright: IDS                                                       */
/* Written by: Vanessa                                          			*/
/*                                                                      */
/* Purpose:  SOS#177741 Project Diana - Kimball Label Euro   		      */
/*                                                                      */
/* Input Parameters:  @c_DropID , - DropID                              */
/*                                                                      */
/* Called By:  dw = r_dw_sku_label_tote02             		            */
/*                                                                      */
/* PVCS Version: 1.2		                                                */
/*                                                                      */
/* Version: 5.4                                                         */
...
CREATE PROC [dbo].[nsp_SkuLabelTote02] (
...
BEGIN
   ...
   INSERT INTO #TEMPLABEL 
                  (DropID, ManufacturerSku, Descr, SkuGroup, Style, Color, Size, Busr9, Price, Cost)
   ...
   BEGIN
      ...
      SELECT @c_errmsg = CONVERT(CHAR(250),@n_err), @n_err = 63104   
   	SELECT @c_errmsg = 'NSQL'+CONVERT(char(5),@n_err)+': Insert #TEMPLABEL Failed. ' + 
                         ' (nsp_SkuLabelTote02)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + ' ) '
      GOTO EXIT_SP
   END
...
--   WHERE PackDetail.DropID = @c_DropID
   ...
   -- Retrieve values
   --(Wan01) - START
   --SELECT DropID, UPPER(SKU), RTRIM(LTRIM(DESCR)), SKUSize, Cost
   --FROM #TEMPLABEL
   SELECT DropID
         ,ManufacturerSku
         ,Descr
   ...
   BEGIN
      WHILE @@TRANCOUNT > @n_starttcnt
      ROLLBACK TRAN
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'nsp_SkuLabelTote02'
