...
/********************************************************************************/                 
/* Copyright: LFL                                                               */                 
/* Purpose: isp_Bartender_Shipper_Label_DSJP                                    */                 
/*                                                                              */                 
/* Modifications log:                                                           */                 
/*                                                                              */                 
/* Date       Rev  Author     Purposes                                          */                 
/* 2020-10-13 1.0  WLChooi    Created (WMS-15426)                               */                   
/********************************************************************************/ 

CREATE PROC [dbo].[isp_Bartender_Shipper_Label_DSJP]                      
...
BEGIN                      
  ...
  DECLARE @d_Trace_StartTime   DATETIME,   
   ...
   SET @c_SQLJOIN = + N' SELECT DISTINCT (N''〒'' + Substring(ORD.C_Zip,1,3) + ''-'' + Substring(ORD.C_Zip,4,4)),ORD.c_phone1,' 
                    ...
                    + ' CASE WHEN ISNULL(C4.Short,'''') <> '''' AND ORD.ExternOrderkey LIKE C4.Short THEN ISNULL(C4.UDF01,'''') ELSE STO.company END, '  
                    + ' CASE WHEN ISNULL(C4.Short,'''') <> '''' AND ORD.ExternOrderkey LIKE C4.Short THEN ISNULL(C4.UDF03,'''') ELSE STO.Phone1 END, '   
                    + ' (STO.Address1 + STO.Address2),STO.Zip,STO.B_Company, '                       --15   
                    + CHAR(13) +      
                    + ' STO.B_Phone1,STO.Susr2,STO.Susr3,STO.Susr5, '
                    + ' CASE WHEN ORD.ExternOrderkey LIKE C4.Short THEN ISNULL(C4.UDF02,'''') ELSE STO.Notes1 END,'            --20      
                    ...
                    + ' JOIN PICKDETAIL PICKDET WITH (NOLOCK) ON PICKDET.caseid = PACKDET.RefNo'   
                    + ' LEFT JOIN CARTONTRACK CT WITH (NOLOCK) ON CT.Trackingno = PACKDET.RefNo'    
                    ...
                    + ' LEFT JOIN CODELKUP C4 WITH (NOLOCK) ON C4.Listname = ''YamatoLbl'' AND C4.Storerkey = ORD.Storerkey AND ISNULL(C4.Short,'''') = ''NBA%'' '  
...
EXIT_SP:    
   ...
   EXEC isp_InsertTraceInfo   
      ...
      @c_TraceName = 'isp_Bartender_Shipper_Label_DSJP',  
