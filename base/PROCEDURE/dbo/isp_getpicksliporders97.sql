...
CREATE PROC [dbo].[isp_GetPickSlipOrders97] (@c_loadkey NVARCHAR(10))
 AS
BEGIN
 ...
 BEGIN
   ...
   SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Failed On Table Pickheader Table. (nsp_GetPickSlipOrders97)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '
 END
 ...
 BEGIN
  DECLARE pickslip_cur CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
   ...
   JOIN  ORDERDETAIL OD WITH (NOLOCK) ON OD.Orderkey = Pickdetail.Orderkey AND OD.sku = Pickdetail.sku --OD.Orderkey = ORDERS.Orderkey       
   ...
   JOIN  PACK WITH (NOLOCK) ON PACK.Packkey = OD.Packkey
   ...
   LEFT OUTER JOIN Codelkup CLR (NOLOCK) ON (ORDERS.Storerkey = CLR.Storerkey AND CLR.Code = 'SHOWORDERGRP'                                       
                                 AND CLR.Listname = 'REPORTCFG' AND CLR.Long = 'r_dw_print_pickorder97' AND ISNULL(CLR.Short,'') <> 'N')          
  ...
  BEGIN
   ...
   AND   Long = 'r_dw_print_pickorder97'
   AND Code = 'showfield'
   AND   ISNULL(Short,'') <> 'N'
   ...
   BEGIN
    ...
    (@c_pickheaderkey, '',      @c_LoadKey,     '0',      'LB',  '', @c_WaveKey, @c_Consigneekey )   -- @c_PickZone)
      ...
      BEGIN
        ...
        SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert Failed On Table PICKHEADER. (nsp_GetPickSlipOrders97)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '
      END
   END
     ...
     BEGIN
      ...
      VALUES (@c_PickDetailkey, @c_pickheaderkey, @c_OrderKey, @c_OrderLineNumber, @c_loadkey)
        ...
        BEGIN
          SELECT @n_continue = 3
          SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err=73001
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert Failed On Table RefkeyLookup. (nsp_GetPickSlipOrders97)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '
        END
      ...
      BEGIN
       ...
       WHERE  PickDetailkey = @c_PickDetailkey
         ...
         BEGIN
           SELECT @n_continue = 3
           SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err=73001
           SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Failed On Table PICKDETAIL. (nsp_GetPickSlipOrders97)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '
         END
      END
     END
