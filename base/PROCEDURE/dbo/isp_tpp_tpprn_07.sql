...
GO
/************************************************************************/
/* Store Procedure: isp_TPP_TPPRN_07                                    */
/* Creation Date: 30-MAY-2022                                           */
/* Copyright: LF                                                        */
/* Written by:CSCHONG                                                   */
/*                                                                      */
/* Purpose:WMS-19720 CN - Kuaishou Platform Cloud Print Cmd SP          */
/*                                                                      */
/*                                                                      */
/* Input Parameters:                                                    */
/*                                                                      */
...
CREATE   PROC [dbo].[isp_TPP_TPPRN_07] (    
   @n_JobNo             BIGINT    
   )       
AS    
BEGIN    
       ...
       --CS01 START    
        ...
        BEGIN    
         ...
         SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_err,0))       
                        + ': TPP Printer Group not setup for printeris : ' + @c_printerid + ' with platform' + @c_Platform + ' (isp_TPP_TPPRN_07)'    
         SET @c_Status = '5'    
        ...
        BEGIN    
         ...
         SET @n_err = 68010         
         SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_err,0))       
                        + ': TPP Printer Group not setup for printer : ' + @c_printerid + ' with platform' + @c_Platform + ' (isp_TPP_TPPRN_07)'    
         SET @c_Status = '5'    
         SET @c_Message = RTRIM(ISNULL(@c_Message,'')) + @c_ErrMsg + ' '                                   
        ...
        ELSE    
        BEGIN    
              SET @c_WebSocketURL = 'ws://' + @c_PrnServerIP +':'+ @c_PrnServerPort + '/ks/printer'  
        END    
   ...
   BEGIN    
      ...
      BEGIN    
         ...
         SET @n_err = 68010         
         SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_err,0))       
                        + ': Unable to find print record for Job No: ' +RTRIM(CAST(@n_JobNo AS NVARCHAR)) + ' (isp_TPP_TPPRN_07)'    
         SET @c_Status = '5'    
         SET @c_Message = RTRIM(ISNULL(@c_Message,'')) + @c_ErrMsg + ' '                                   
      END    
      ...
      BEGIN           
        ...
        BEGIN    
            ...
            SET @n_err = 68010         
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_err,0))       
                           + ': Get key value fail for Order# ' + RTRIM(@c_CurrOrderkey) + ' Carton# ' + RTRIM(CAST(@n_CartonNo AS NVARCHAR)) + ' (isp_TPP_TPPRN_07)'    
            SET @c_Status = '5'    
            SET @c_Message = RTRIM(ISNULL(@c_Message,'')) + @c_ErrMsg + ' '                                   
        END    
          ELSE    
        ...
        BEGIN    
            ...
            SET @n_err = 68010         
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5),ISNULL(@n_err,0))       
                           + ': Unable to find tracking# for Order# ' + RTRIM(@c_CurrOrderkey) + ' Carton# ' + RTRIM(CAST(@n_CartonNo AS NVARCHAR)) + ' (isp_TPP_TPPRN_07)'    
            SET @c_Status = '5'    
            SET @c_Message = RTRIM(ISNULL(@c_Message,'')) + @c_ErrMsg + ' '                                   
        END    
      ...
      EXEC sp_ExecuteSQL @c_SQL    
         ...
         BEGIN    
                 SET @c_RequestString = N'{"cmd":' + '"' + @c_cmdstart + '"' + ',"requestID":' + '"' + @c_RequestID + '"' + ',"version":' + '"' + @c_version + '"' +  
                                       ...
                                       ',"documents":[{"documentID":' + '"' + @c_TrackingNo + '"' + ',"waybillCode":' + '"' + @c_TrackingNo + '"' + ',"ksOrderFlag":' + @c_ksOrderFlag +  
                                       ',"contents":[{' + @c_PrintData + ',"templateURL":"' + @c_TemplathURL +'"}' + @c_CustomData + ']}]}}'    
   ...
   BEGIN    
      ...
      BEGIN    
         ...
         END              
      END    
      EXECUTE nsp_logerror @n_err, @c_errmsg, "isp_TPP_TPPRN_07"    
