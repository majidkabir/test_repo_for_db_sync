...
GO
/*************************************************************************/
/* Stored Procedure: isp_SCE_DL_ChkDuplicate                             */
/* Creation Date: 02 Dec 2020                                            */
/* Copyright: LFL                                                        */
/* Written by: GHChan                                                    */
/*                                                                       */
/* Purpose: Check STG&TGT table whether exists duplicate records         */
/*                                                                       */
...
/*************************************************************************/
CREATE PROC [dbo].[isp_SCE_DL_ChkDuplicate] (
   @b_Debug      INT            = 0
 , @n_BatchNo    INT            = 0
...
BEGIN
   ...
   BEGIN TRY
      ...
      IF ISNULL(RTRIM(@c_UniqKeyCol), '') = ''
      BEGIN
         SET @n_Continue = 3;
         SET @b_Success = 0;
         SET @n_ErrNo = N'12000';
         SET @c_ErrMsg = N'List of unique key columns are not found!' + ERROR_MESSAGE() + '(isp_SCE_DL_ChkDuplicate)';
         GOTO QUIT;
      END;
      ...
      BEGIN
         ...
         BEGIN
            ...
            IF @n_UKCount = 1 -- ISNULL(RTRIM(@c_FParams), '') = '' AND ISNULL(RTRIM(@c_WHRParams), '') = ''  
            BEGIN
      ...
      BEGIN
         ...
         --SET @c_FParams = N'@PK1 '  
         SET @c_WHRParams = @c_UniqKeyCol + N'=@PK1 ';
      ...
      SET @SQL1 = N' DECLARE @C0 INT, ' + @c_DCLParams
                ...
                + N' SELECT ' + @c_SETValues + ' FROM ' + @c_STGTBL +' WITH(NOLOCK) ' 
                + N' WHERE RowRefNo=@C0 '
                + N' IF EXISTS (SELECT 1 FROM ' + @c_STGTBL + ' WITH(NOLOCK) '
                + N' WHERE STG_BatchNo=@n_BatchNo AND STG_Status=''1'' AND ' + @c_WHRParams 
                + N' GROUP BY ' + @c_UniqKeyCol + ' HAVING COUNT(1)>1) '
                + N' BEGIN '
      SET @SQL2 = N' UPDATE ' + @c_STGTBL + ' WITH (ROWLOCK) '
                + N' SET STG_Status =''3'' '
                + N' ,STG_ErrMsg=''Duplicate records in this same BatchNo.'' '
                + N' WHERE RowRefNo = @C0'
                + N' END '
                + N' ELSE IF EXISTS (SELECT 1 FROM ' + @c_POSTTBL + ' WITH (NOLOCK) '
                + N' WHERE ' + @c_WHRParams 
                + N' GROUP BY ' + @c_UniqKeyCol + ' HAVING COUNT(1)>1) '
                + N' BEGIN '
      ...
      --          + N' BEGIN '
      --          + N' IF EXISTS (SELECT 1 FROM ' + @c_STGTBL + ' WITH(NOLOCK) '
      --          + N' WHERE STG_BatchNo=@n_BatchNo AND ' + @c_WHRParams 
   ...
   BEGIN CATCH
      ...
      SET @b_Success = 0;
      SET @n_ErrNo = N'12001';
      SET @c_ErrMsg = N'Failed to Check Duplicate Records in STG&TGT Table!' + ERROR_MESSAGE() + '(isp_SCE_DL_ChkDuplicate)';
   END CATCH;
