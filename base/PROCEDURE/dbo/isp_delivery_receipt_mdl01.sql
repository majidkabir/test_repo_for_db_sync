...
GO
/************************************************************************/    
/* Stored Procedure: isp_Delivery_Receipt_MDL01                         */    
/* Creation Date: 23/03/2020                                            */    
/* Copyright: IDS                                                       */    
/* Written by: CSCHONG                                                  */    
/*                                                                      */    
/* Purpose: WMS-12456 - Mondelez RCM of DR Report_New                   */   
...
CREATE PROC [dbo].[isp_Delivery_Receipt_MDL01] (@cMBOLkey NVARCHAR(10) )    
AS    
BEGIN    
   SET NOCOUNT ON   
   SET QUOTED_IDENTIFIER OFF   
 ...
 -- Assign DR Number (at externorderkey level) to all orders under this MBOLKey!     
      ...
      BEGIN    
         ...
         BEGIN       
            ...
            BEGIN    
               ...
               SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": No Setup for CodeLkUp.ListName = DR_NCOUNT. (isp_Delivery_Receipt_MDL01)"    
            END    
            ...
            BEGIN    
               ...
               BEGIN    
                  SELECT @n_continue = 3    
                  SELECT @n_err = 63500  -- should assign new error code    
                  SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Fail to Generate Userdefine10 . (isp_Delivery_Receipt_MDL01)"    
               END    
               ELSE    
               BEGIN    
                  UPDATE ORDERS     
                  ...
                  BEGIN    
                     ...
                     SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": UPDATE ORDERS Failed. (isp_Delivery_Receipt_MDL01)"    
                  END    
               END    
   ...
   BEGIN    
      EXECUTE nsp_logerror @n_err, @c_errmsg, "isp_Delivery_Receipt_MDL01"    
