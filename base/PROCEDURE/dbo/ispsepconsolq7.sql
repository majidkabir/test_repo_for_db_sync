...
/************************************************************************/
CREATE PROC [dbo].[ispSEPConsoLQ7]
...
BEGIN
   ...
   IF @c_WaveType IN ('SEPB2BPTS', 'SEPB2CPTS') 
   BEGIN
      SET @c_LocationCategory = 'SHELVING'
      SET @c_LocationType     = 'DYNPPICK'      
   END
   ELSE IF @c_WaveType IN ('SEPB2CAGV', 'SEPB2B_AGV')
   BEGIN
      SET @c_LocationCategory = 'AGV'
      SET @c_LocationType     = 'PICK' 
   END

   IF @b_debug = 9 -- Self test
   BEGIN
       SET @c_LocationCategory = 'SHELVING'
       SET @c_LocationType     = 'DYNPPICK' 
   END
   ...
   BEGIN
      PRINT '---------------------------------------'+ CHAR(13) +
            'Sub PICKCOde: ispSEPConsoLQ7'+ CHAR(13) +
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               SET @n_Err = 81220
               SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0)) 
                              + ': Insert PickDetail Failed. (ispSEPConsoLQ7)'
               GOTO QUIT_SP
            END
...
QUIT_SP:
   ...
   BEGIN
      ...
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'ispSEPConsoLQ7'
