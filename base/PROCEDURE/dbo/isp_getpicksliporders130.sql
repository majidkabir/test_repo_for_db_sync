...
GO
/************************************************************************/
/* Stored Procedure: nsp_GetPickSlipOrders130                           */
...
/* Remarks:                                                             */
/*                                                                      */
/* Purpose: WMS-19127 - PH PNG Picking List                             */
/*                                                                      */
/*                                                                      */
/* Called By: r_dw_print_pickorder130                                   */
/*           duplicate from r_dw_print_pickorder26                      */
/*                                                                      */
/* PVCS Version: 1.9                                                    */
/*                                                                      */
/* Version: 5.4                                                         */
...
CREATE PROC [dbo].[isp_GetPickSlipOrders130] (@c_loadkey NVARCHAR(10))
 AS
BEGIN
 ...
 BEGIN
   ...
   SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Failed On Table Pickheader Table. (isp_GetPickSlipOrders130)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '
 END
 ...
 BEGIN
  DECLARE pickslip_cur CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
   ...
   LEFT OUTER JOIN Codelkup CLR (NOLOCK) ON (ORDERS.Storerkey = CLR.Storerkey AND CLR.Code = 'SHOWORDERGRP'                                      
                                 AND CLR.Listname = 'REPORTCFG' AND CLR.Long = 'r_dw_print_pickorder130' AND ISNULL(CLR.Short,'') <> 'N')         
  ...
  FETCH NEXT FROM pickslip_cur INTO @c_Loadkey, @c_LocType, @d_LoadDate, @c_PickZone, @c_C_Company, @c_Loc, @c_LogicalLoc, 
              ...
              @c_TrfRoom, 
              @c_LEXTLoadKey,@c_LPriority,@c_LPuserdefDate01,@n_innerpack ,@c_packuom3 
  ...
  BEGIN
   ...
   SET @n_TotalCarton = 0.00    
     ...
     -- SELECT @c_SKU '@c_SKU',@n_Pallet '@n_Pallet',@n_CaseCnt '@n_CaseCnt',@n_Qty '@n_Qty'

      SET @n_Palletcnt = CASE WHEN @n_Pallet > 0 THEN @n_Qty/@n_Pallet ELSE 0 END
      SET @n_Cartoncnt = CASE WHEN @n_CaseCnt> 0 THEN (@n_Qty - (@n_Palletcnt * @n_Pallet))/@n_CaseCnt ELSE CASE WHEN @c_packuom3='CS' THEN @n_Qty ELSE 0 END END
      SET @n_EA        = CASE WHEN @c_packuom3<>'CS' THEN @n_Qty - (@n_Palletcnt * @n_Pallet) - (@n_Cartoncnt * @n_CaseCnt) ELSE 0 END
   ...
   AND   Long = 'r_dw_print_pickorder130'
   AND Code = 'showfield'
   AND   ISNULL(Short,'') <> 'N'
    ...
    (@c_pickheaderkey, '',      @c_LoadKey,     '0',      'LB',  '', @c_WaveKey, @c_Consigneekey )   -- @c_PickZone)
      ...
      BEGIN
       ...
       SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err=73001
        SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert Failed On Table PICKHEADER. (isp_GetPickSlipOrders130)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '
      END
   END
   ...
   BEGIN
    ...
    BEGIN
     DECLARE PickDet_cur CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
      ...
      AND    (LOC.LocationType = 'CASE' OR LOC.LocationType = 'PICK' OR LOC.LocationType = 'PALLET' OR ISNULL(LOC.PickZone,'')='') 
     ...
     BEGIN
      ...
      VALUES (@c_PickDetailkey, @c_pickheaderkey, @c_OrderKey, @c_OrderLineNumber, @c_loadkey)
        ...
        BEGIN
          SELECT @n_continue = 3
         SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err=73001
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert Failed On Table RefkeyLookup. (isp_GetPickSlipOrders130)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '
        END
      ...
      BEGIN
       ...
       WHERE  PickDetailkey = @c_PickDetailkey
         ...
         BEGIN
           SELECT @n_continue = 3
          SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err=73001
           SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Failed On Table PICKDETAIL. (isp_GetPickSlipOrders130)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '
         END
      END
     END
 ...
 BEGIN
  ...
  IF ISNULL(@c_Lottable04label,'') = ''
   ...
   SELECT Loadkey
      ...
      , OrderGrp   
      , TrfRoom 
      ,LEXTLoadKey,LPriority,LPuserdefDate01 
      ,showfield,InnerPack
   FROM @t_Result
