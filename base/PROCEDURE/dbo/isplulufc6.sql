...
BEGIN
   ...
   ,  UCC_RowRef        BIGINT         NOT NULL DEFAULT('')
...
--(ML02)     AND OH.UserDefine10 NOT IN ('170146','170149') 
   ...
   SELECT UCC_RowRef = MAX(u.UCC_RowRef), u.UCCNo, Storerkey = u.Storerkey, Sku = MAX(t.Sku), Lot = MAX(u.Lot), Loc = MAX(u.Loc), ID = MAX(u.ID), Qty = MAX(u.Qty)
...
QUIT_SP:
   ...
   BEGIN
      ...
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'ispLuLuFC6'
