...
GO
/************************************************************************/
/* Store Procedure:  isp_SCE_DL_GENERIC_ASN_RULES_100012_10             */
...
CREATE PROCEDURE [dbo].[isp_SCE_DL_GENERIC_ASN_RULES_100012_10] (
   @b_Debug       INT            = 0
 , @n_BatchNo     INT            = 0
 , @n_Flag        INT            = 0
...
BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5), ISNULL(@n_ErrNo, 0))
                            + ': Update record fail. (isp_SCE_DL_GENERIC_ASN_RULES_100012_10)';
            ROLLBACK;
   ...
   AND   (
        Packkey IS NOT NULL
      AND ISNULL(RTRIM(Packkey), '') <> ''
   )
   AND   NOT EXISTS (
   SELECT 1
   FROM dbo.V_PACK P WITH (NOLOCK)
   WHERE STG.Packkey = P.PackKey
   )
   )
   BEGIN
      BEGIN TRANSACTION;
   ...
   IF @b_Debug = 1
   BEGIN
      SELECT '<<SUB-SP-RULES>> - [isp_SCE_DL_GENERIC_ASN_RULES_100012_10] EXIT... ErrMsg : ' + ISNULL(RTRIM(@c_ErrMsg), '');
   END;
