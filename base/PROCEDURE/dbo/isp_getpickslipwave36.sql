...
/************************************************************************/  
/* Stored Procedure: isp_GetPickSlipWave36                              */  
/* Creation Date: 14-JAN-2022                                           */  
/* Copyright: IDS                                                       */  
/* Written by: CSCHONG                                                  */  
/*                                                                      */  
/* Purpose: WMS-18736-MYûUAMYûStandardized paper size for Picking Slip  */  
/*                                                                      */  
/* Called By: RCM - Generate Pickslip                                   */  
/*          : Datawindow - r_dw_print_wave_pickslip_36                  */  
/*                                                                      */  
/* PVCS Version: 1.0                                                    */  
/*                                                                      */  
/* Version: 5.4                                                         */  
...
/* 17/JAN/2022	 CSCHONG  Devops Scripts Combine          				  	*/
/************************************************************************/  
  
CREATE PROC [dbo].[isp_GetPickSlipWave36] (  
...
BEGIN  
   ...
   AND Long      = 'r_dw_print_wave_pickslip_36'
   ...
   SELECT DISTINCT RefKeyLookup.PickSlipNo,  
          ...
          '' CarrierKey,  
          '' AS                             VehicleNo,  
          CASE WHEN ISNULL(orders.c_country,'') = 'TH' THEN LotAttribute.Lottable01 ELSE LotAttribute.Lottable02 END Lottable02,  
   ...
   left outer join RefKeyLookup (NOLOCK) ON (RefKeyLookup.PickDetailKey = PICKDETAIL.PickDetailKey) 
	 LEFT JOIN CODELKUP C (NOLOCK) ON (C.Storerkey = Storer.Storerkey) AND C.Long = 'r_dw_print_wave_pickslip_36' 
   ...
   GROUP BY RefKeyLookup.PickSlipNo,orders.loadkey ,
          ...
          ISNULL(Sku.Descr, ''),  
          loc.pickzone,
          CASE WHEN ISNULL(orders.c_country,'') = 'TH' THEN LotAttribute.Lottable01 ELSE LotAttribute.Lottable02 END,
          ...
          ,wd.WaveKey,Loc.PutawayZone ,pickdetail.PickDetailKey
		  ,ISNULL(SKU.ALTSKU,'') 
		  ,ISNULL(C.SHORT,'N') 
   ...
   BEGIN     	   	
     ...
     BEGIN	      		
         ...
         BEGIN                 
            SET @c_RPickSlipNo = 'P' + @c_RPickSlipNo          
					  ...
					  BEGIN      
					  	...
					  	SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert PICKHEADER Failed (isp_GetPickSlipWave36)'   
         ...
         BEGIN  
            SELECT @n_continue = 3  
            SELECT @n_err = 63502
	          SELECT @c_errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Get PSNO Failed. (isp_GetPickSlipWave36)'  
            BREAK   
         END            
     END            
        ...
        BEGIN  
           ...
           BEGIN   
              ...
              BEGIN  
                 SELECT @n_continue = 3
                 SELECT @n_err = 63503
	               SELECT @c_errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert RefKeyLookup Failed. (isp_GetPickSlipWave36)'    
                 GOTO QUIT
              END                          
           END   
     ...
     BEGIN  
        SELECT @n_continue = 3  
        SELECT @n_err = 63504
        SELECT @c_errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update #TMP_PICK Failed. (isp_GetPickSlipWave36)'    
        GOTO QUIT
     END
     ...
     AND Pickdetailkey = @c_GetPickDetailKey
		 ...
		 BEGIN      
		 	  ...
		 	  SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': UPDATE Pickdetail Failed (isp_GetPickSlipWave36)'   
		 	  				 + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '   
		 	  GOTO QUIT     
		 END  
...
QUIT:  
   ...
   BEGIN   
      ...
      BEGIN    
         ROLLBACK TRAN    
      END   
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'isp_GetPickSlipWave36'    
   END  
   ...
   SELECT             
      ...
      ,  #TMP_ORDSUM.totalqtyoverallocate,  
          ...
          (#TMP_PICK.grosswgt*#TMP_PICK.Qty) /1000 As [Wgt],
           #TMP_PICK.OrderGrp, #TMP_PICK.Wavekey,#TMP_Pick.PAZone,
		   #TMP_PICK.ALTSKU, 
		   #TMP_PICK.CLKUPSHORT             
   FROM   #TMP_PICK  
