...
/*************************************************************************/  
/* Stored Procedure: isp_EXG_FilterCondition5                            */  
/* Creation Date: 16 Dec 2020                                            */  
/* Copyright: LFL                                                        */  
/* Written by: GuanHao Chan                                              */  
/*                                                                       */  
/* Purpose: Excel Generator Sub StoredProcedure FilterConfition 5.       */  
/*                                                                       */  
...
/* 16-Dec-2020   GHChan   1.0  Initial Development                       */  
/*************************************************************************/  
  
CREATE PROCEDURE [dbo].[isp_EXG_FilterCondition5]  
...
BEGIN  
 ...
 IF @b_Debug = 1  
   BEGIN  
      PRINT '[dbo].[isp_EXG_FilterCondition5]: Start...'  
      PRINT '[dbo].[isp_EXG_FilterCondition5]: '  
   ...
   BEGIN  
         SET @n_Err = 210001  
         SET @c_ErrMsg = 'Invalid EXG Hdr ID. (isp_EXG_FilterCondition5)'  
         SET @n_Continue = 3  
         GOTO QUIT  
   END  
   ...
   BEGIN TRY  
      ...
      BEGIN  
         SET @n_Err = 210002  
         SET @c_ErrMsg = ' StorerKey cannot be null. - (isp_EXG_FilterCondition5)'  
     
         IF @b_Debug = 1  
         BEGIN  
            PRINT '[isp_EXG_FilterCondition5]: Execute SP Name Failed...' + ' @c_ErrMsg=' + @c_ErrMsg  
         END  
         GOTO QUIT  
      END  
      ...
      BEGIN  
         SET @n_Err = 210003  
         SET @c_ErrMsg = ' MbolKey cannot be null. - (isp_EXG_FilterCondition5)'  
      ...
      IF EXISTS (SELECT 1 FROM dbo.CODELKUP WITH(NOLOCK)  
                 WHERE LISTNAME='UAPOD'  
                 AND StorerKey = @c_ParamVal1  
                 AND Short = '1')  
      BEGIN  
         INSERT INTO @Temp_BatchPVal ([filename]  
                                    ...
                                    , [PVal4]  
                                    , [PVal5])  
            SELECT CONCAT(RTRIM(SUBSTRING(ConsigneeKey, PATINDEX('%[^0]%', ConsigneeKey+'.'), LEN(ConsigneeKey)))  
                        , '_'  
                        , RTRIM(ExternOrderKey)  
                        , CASE WHEN ISNULL(RTRIM(C_Company),'') <> '' THEN '_' + RTRIM(C_Company) END, @c_FileExt)  
                  , @c_ParamVal1  
                  , MbolKey  
                  , ConsigneeKey  
                  , ExternOrderKey  
            ...
            AND ExternOrderKey <> ''  
            AND C_Company <> ''  
            AND ConsigneeKey IS NOT NULL  
            AND ExternOrderKey IS NOT NULL  
            AND C_Company IS NOT NULL  
            GROUP BY MbolKey, ConsigneeKey, ExternOrderKey, C_Company  
      END  
...
STARTCURSOR:    
   ...
   BEGIN CATCH   
      ...
      SET @n_Err = @n_ITFErr  
      SET @c_ErrMsg = LTRIM(RTRIM(@c_ITFErrMsg)) + ' (isp_EXG_FilterCondition5)'  
                 
      IF @b_Debug = 1  
      BEGIN  
         PRINT '[isp_EXG_FilterCondition5]: Execute SP Name Failed...'   
               + ' @c_ErrMsg=' + @c_ErrMsg  
      END  
      GOTO QUIT  
   END CATCH  
...
QUIT:  
   ...
   BEGIN        
      ...
      IF @b_Debug = 1  
      BEGIN  
         PRINT '[dbo].[isp_EXG_FilterCondition5]: @c_ErrMsg=' + RTRIM(@c_ErrMsg)  
         PRINT '[dbo].[isp_EXG_FilterCondition5]: @b_Success=' + RTRIM(CAST(@b_Success AS NVARCHAR))  
      END  
