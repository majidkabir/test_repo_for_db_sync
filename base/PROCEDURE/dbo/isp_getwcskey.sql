...
/************************************************************************/  
/* Stored Procedure:  isp_GetWCSKey                                     */  
/* Creation Date:                                                       */  
/* Copyright: IDS                                                       */  
/* Written by:                                                          */  
/*                                                                      */  
...
/*                                                                      */  
/* 07-Dec-2014  TLTING     1.1  Replace GetKey with isp_GetWCSKey       */
/*                              to reduce blocking                      */
/* 13-May-2015  TLTING     1.2  Change variable BigINT                  */
/************************************************************************/ 
CREATE PROC [dbo].[isp_GetWCSKey]     
(   @n_FieldLength  INT     
  , @c_WCSKey  NVARCHAR(10)  OUTPUT     
...
BEGIN    
   ...
   INSERT INTO WCSKey (AddDate) VALUES (GETDATE())    
   SELECT @n_err = @@ERROR, @n_cnt = @@ROWCOUNT, @n_NewWCSKey = @@IDENTITY    
   IF @n_err <> 0    
   BEGIN    
     SELECT @n_continue = 3     
   END    
   SELECT @n_NewWCSKey = @@IDENTITY    
  
   IF @n_continue = 1 OR @n_continue = 2    
   BEGIN    
 
      SELECT @c_WCSKey = RIGHT(Replicate('0',@n_FieldLength) + CAST(@n_NewWCSKey As VARCHAR(10)), @n_FieldLength)     
   END    
   ELSE    
      SELECT @c_WCSKey = ''    
   ...
   BEGIN    
      ...
      BEGIN    
          ...
          END              
      END    
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'isp_GetWCSKey'    
