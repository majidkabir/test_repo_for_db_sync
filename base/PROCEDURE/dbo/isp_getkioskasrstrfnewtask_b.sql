...
/************************************************************************/  
/* Stored Procedure: isp_GetKIOSKASRSTRFNewTask_b                       */  
...
/*                                                                      */  
/* Called By: d_dw_kiosk_asrstrf_new_b_form                             */  
/*                                                                      */  
/* PVCS Version: 1.0                                                    */  
/*                                                                      */  
/* Version: 5.4                                                         */  
...
/************************************************************************/  
CREATE PROC [dbo].[isp_GetKIOSKASRSTRFNewTask_b]   
...
BEGIN  
   ...
   INSERT INTO #Result (TaskDetailKey,FromID,TOID,PalletID,TransferKey,Transferlinenumber,FromStorerkey,FromSKU,Storerkey,SKU,SKUDescr,TransferReason
                     ...
                     ,PackUOM1,PackUOM3,CASECNT,PalletQtyInCS,PalletQtyInEA
   ...
   SET @n_TTLTransfer = 0
   SELECT @n_TTLTransfer = COUNT(TransferDetail.FromID)
   FROM #RESULT WITH (NOLOCK)
   JOIN TRANSFERDETAIL WITH (NOLOCK) ON (#RESULT.TransferKey = TRANSFERDETAIL.TransferKey)
                                     AND(#RESULT.FromID = TRANSFERDETAIL.FromID)

   UPDATE #RESULT
   SET TTLTransfer    = @n_TTLTransfer
      ,PalletQtyInCS = CASE WHEN CaseCnt > 0 AND @n_ShowUOMEA = 0 THEN CONVERT(INT,FLOOR(@n_Qty / CaseCnt)) ELSE 0 END
   ...
   SELECT @c_JobKey
         ...
         ,Lottable01label,Lottable02label,Lottable03label,Lottable04label,Lottable05label,
          ...
          PackUOM1,PackUOM3,CASECNT,TTLTransfer,PalletQtyInCS,PalletQtyInEA
         ,TaskStatus,JobStatus
   FROM #Result 
END        
