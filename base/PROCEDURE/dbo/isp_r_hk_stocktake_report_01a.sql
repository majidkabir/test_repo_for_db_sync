...
GO
/*************************************************************************/
/* Stored Procedure: isp_r_hk_stocktake_report_01a                       */
/* Creation Date: 03-Sep-2019                                            */
/* Copyright: LFL                                                        */
/* Written by: Michael Lam (HK LIT)                                      */
/*                                                                       */
/* Purpose: L'Oreal Stocktake Count Sheet                                */
/*                                                                       */
/* Called By: Report Module. Datawidnow r_hk_stocktake_report_01a        */
/*                                                                       */
/* PVCS Version: 1.0                                                     */
/*                                                                       */
/* Version: 7.0                                                          */
...
RESULTS:
   SELECT CC.*
        , Facility_bySheet    = CAST( STUFF((SELECT DISTINCT ', ', LTRIM(RTRIM(a.Facility)) FROM #TEMP_CCDETAIL2 a
                                      WHERE a.CCKey=CC.CCKey AND a.PageNo=CC.PageNo AND a.Facility<>'' ORDER BY 2 FOR XML PATH('')), 1, 2, '') AS NVARCHAR(500))
        , PutawayZone_bySheet = CAST( STUFF((SELECT DISTINCT ', ', LTRIM(RTRIM(a.PutawayZone)) FROM #TEMP_CCDETAIL2 a
                                      WHERE a.CCKey=CC.CCKey AND a.PageNo=CC.PageNo AND a.PutawayZone<>'' ORDER BY 2 FOR XML PATH('')), 1, 2, '') AS NVARCHAR(500))
        , Div_byLoc           = CAST( STUFF((SELECT DISTINCT ', ', LTRIM(RTRIM(a.Div)) FROM #TEMP_CCDETAIL2 a
                                      WHERE a.CCKey=CC.CCKey AND a.Loc=CC.Loc AND a.Div<>'' ORDER BY 2 FOR XML PATH('')), 1, 2, '') AS NVARCHAR(500))
        , Brand_byLoc         = CAST( STUFF((SELECT DISTINCT ', ', LTRIM(RTRIM(a.Brand)) FROM #TEMP_CCDETAIL2 a
                                      WHERE a.CCKey=CC.CCKey AND a.Loc=CC.Loc AND a.Brand<>'' ORDER BY 2 FOR XML PATH('')), 1, 2, '') AS NVARCHAR(500))
   FROM #TEMP_CCDETAIL2 CC
   ORDER BY CCKey, PageNo
