...
/************************************************************************/
/* Stored Procedure: isp_ReplenishmentRpt_BatchRefill_25                */
/* Creation Date: 16-Sep-2022                                           */
/* Copyright: LFL                                                       */
/* Written by: Mingle                                                   */
/*                                                                      */
/* Purpose: WMS-20791 TH_RC2_CR_Add lottable06 in replenishment Report  */
/*          Copy from nsp_ReplenishmentRpt_BatchRefill_21 and modify    */
/*                                                                      */
/* Called By: r_replenishment_report23                                  */
/*                                                                      */
/* GitLab Version: 1.0                                                  */
/*                                                                      */
/* Version: 5.4                                                         */
...
/************************************************************************/
CREATE PROC  [dbo].[isp_ReplenishmentRpt_BatchRefill_25]
...
BEGIN
   ...
   BEGIN
      ...
      SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Zone11 must be DATETIME format. (isp_ReplenishmentRpt_BatchRefill_25)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + ' ) '
      GOTO R_ERROR
   END
   ...
   BEGIN
      ...
      SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Zone12 must be DATETIME format. (isp_ReplenishmentRpt_BatchRefill_25)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + ' ) '
      GOTO R_ERROR
   END
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Fail to get REPLENISHKEY. (isp_ReplenishmentRpt_BatchRefill_25)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + ' ) '
            END
            ELSE
            BEGIN
               COMMIT TRAN
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Insert into Replenishment table failed. (isp_ReplenishmentRpt_BatchRefill_25)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + ' ) '
            END
...
R_ERROR:
      ...
      BEGIN
         ...
         BEGIN
            ...
            END
         END
         EXECUTE nsp_logerror @n_err, @c_errmsg, 'isp_ReplenishmentRpt_BatchRefill_25'
   ...
   BEGIN
      ...
      SKU.Descr, R.Priority, L2.PutawayZone, PACK.CASECNT, PACK.PACKUOM1, PACK.PACKUOM3, R.ReplenishmentKey, (LT.Qty - LT.QtyAllocated - LT.QtyPicked) As QtyAvailable, LA.Lottable02, LA.Lottable04, LA.Lottable06	--ML01
