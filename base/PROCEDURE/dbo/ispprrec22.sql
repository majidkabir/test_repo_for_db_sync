...
/* Written by: CSCHONG                                                  */
/*                                                                      */
/* Purpose:WMS-18824-[HK] Friesland_Receiptdetail_Lottable02_Builder_CR */      
/*                                                                      */
...
BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN 
               SET @n_continue= 3 
               SET @n_err  = 72810
               SET @c_errmsg = 'NSQL' + CONVERT(CHAR(5),@n_Err) + ': Execute nspg_GetKey Failed. (ispPRREC22)'
                             + '( ' + RTRIM(@c_errmsg) + ' )'
            END 
         END
       ...
       /*CS01 END*/
         ...
         BEGIN
            SET @n_continue = 3
            SET @n_Err   = 72815
            SET @c_errmsg= 'NSQL'+CONVERT(char(5),@n_err)+': UPDATE RECEIPTDETAIL Fail. (ispPRREC22)'
   ...
   BEGIN
      ...
      INSERT INTO @tAlert VALUES ('FCBATCHNO2 NCounter keycount reaches ''99990''')

      IF @@ERROR <> 0
      BEGIN
         SET @n_Continue=3
         SET @n_err = 72820
         SET @c_Errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_Err)+ 'INSERT INTO @tAlert Fail. (ispPRREC22)'
         GOTO QUIT_SP
      END
      ...
      BEGIN
         SET @n_Continue=3
         SET @n_err = 62030
         SET @c_Errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_Err)+ 'Error executing sp_send_dbmail. (ispPRREC22)'
...
QUIT_SP:
   ...
   BEGIN
      ...
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'ispPRREC22'
