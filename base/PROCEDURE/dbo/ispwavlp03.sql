...
GO
/************************************************************************/
/* Store Procedure:  ispWAVLP03                                         */
/* Creation Date:  29-Nov-2012                                          */
/* Copyright: LF                                                        */
/* Written by:  NJOW                                                    */
/*                                                                      */
/* Purpose:  SOS#274170/256796 - Create load plan by wave               */
/*                                                                      */
...
/* 20-May-2015 NJOW01   1.0  341459-update grouping value to load plan. */
/*                           set LP superorderflag and defaultstrategy  */
/*                           for Ecom order by config.                  */
/* 08-Oct-2015 NJOW02   1.1  Fix join to wrong loadkey belong to other  */
/*                           wave                                       */
/* 04-Oct-2016 SHONG01  1.2  Performance Tuning                         */
/* 05-Jun-2017 SPChin   1.3  IN00363190 - Bug Fixed                     */
/* 12-SEP-2017 Wan01    1.4  Fixed (datatime to datetime)               */
/* 20-OCT-2017 Wan02    1.5  Use Orders.status to insert loadplandetail */
/*                           instead of '0'                             */
/* 15-May-2018 NJOW03   1.6  Fix - include NCHAR                        */
/* 17-Jul-2018 NJOW04   1.6  WMS-5746 Support field with function       */
/* 28-Jan-2019 TLTING_ext 1.7 enlarge externorderkey field length       */
/* 28-May-2021 Wan03    1.8   WMS-17076 - CN - VF ECOM Wave Generate    */
/*                            Loadplan Logic_CR                         */
/* 13-Oct-2022 WLChooi  1.9   WMS-20973 - Restrict MaxOrderPerLoad(WL01)*/
/* 13-Oct-2022 WLChooi  1.9   DevOps Combine Script                     */
/* 19-Oct-2022 WLChooi  2.0   WMS-20973 - Fix facility is missing (WL02)*/
/* 08-Jun-2023 NJOW05   2.1   WMS-22743 - Add config to allow combine   */
/*                            order to existing load of other wave if   */
/*                            same grouping criteria                    */
/* 08-Jun-2023 NJOW05   2.1   DEVOPS Combine Script                     */
/************************************************************************/
CREATE   PROC [dbo].[ispWAVLP03]
...
BEGIN
   ...
   DECLARE @c_ListName           NVARCHAR(10)
         ...
         , @n_OrderCnt           INT --WL01
         , @n_MaxOrderPerLoad    INT --WL01
         , @c_OrderSort          NVARCHAR(2000) --WL01
         , @c_IsEcom             NCHAR(1)='N'  --NJOW05
         , @c_CombineOthWaveLoad NVARCHAR(30)='' --NJOW05                  
   ...
   BEGIN
      ...
      SELECT @c_errmsg = "NSQL" + CONVERT(NVARCHAR(5), @n_err)
                         + ": No Orders being populated into WaveDetail. (ispWAVLP03)"
      GOTO RETURN_SP
   END
   ...
   BEGIN
      SELECT TOP 1 @c_StorerKey = StorerKey
                 , @c_Facility = Facility
                 , @c_IsEcom = CASE WHEN OH.DocType = 'E' THEN 'Y' ELSE 'N' END  --NJOW05
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               IF ISNULL(@c_Userdefine01, '') NOT IN ( 'L', 'N', 'E' ) --(Wan03)
               BEGIN
                  SELECT @n_continue = 3
                  SELECT @c_errmsg = CONVERT(NVARCHAR(250), @n_err)
                       , @n_err = 63501 -- Should Be Set To The SQL Errmessage but I don't know how to do so.  
                  SELECT @c_errmsg = "NSQL" + CONVERT(NVARCHAR(5), @n_err)
                                     + ": Invalid Code to determine Launch or Retail/Wholesale/ECOM Order (ispWAVLP03)" --(Wan03)
                  GOTO RETURN_SP
               END
   ...
   BEGIN
      ...
      BEGIN
         ...
         SELECT @n_err = 63503
         SELECT @c_errmsg = "NSQL" + CONVERT(NVARCHAR(5), @n_err)
                            + ": Empty/Invalid Load Plan Group Is Not Allowed. (LIST GROUP: WAVELPGROUP) (ispWAVLP03)"
         GOTO RETURN_SP
      END
      ...
      AND Code NOT IN ( 'MAXORDER', 'SORTORDER' ) --WL01
      ORDER BY Code
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               SELECT @c_errmsg = "NSQL" + CONVERT(NVARCHAR(5), @n_err)
                                  + ": Grouping Only Allow Refer To Orders Table's Fields. Invalid Table: "
                                  + RTRIM(@c_TableColumnName) + " (ispWAVLP03)"
               GOTO RETURN_SP
            END
         END
   ...
   BEGIN
      ...
      BEGIN
         ...
         SELECT @c_SQLDYN03 = N' SELECT TOP 1 @c_FoundLoadkey = ORDERS.Loadkey ' + N' FROM ORDERS WITH (NOLOCK) '
                              + N' JOIN WaveDetail WD WITH (NOLOCK) ON (ORDERS.OrderKey = WD.OrderKey) '
                              + N' WHERE  ORDERS.StorerKey = @c_StorerKey ' 
                              + CASE WHEN @c_CombineOthWaveLoad = 'Y' THEN ' ' 
                                     ELSE N' AND WD.WaveKey = @c_WaveKey ' END  --NJOW05
         ...
         AND (NOT EXISTS ( SELECT 1
                           FROM ORDERS (NOLOCK)
                           WHERE LoadKey = @c_FoundLoadkey AND ISNULL(LoadKey, '') <> '' AND UserDefine09 <> @c_WaveKey) --NJOW02
              OR @c_CombineOthWaveLoad = 'Y'  --NJOW05
              )             
         BEGIN        
         ...
         BEGIN
            ...
            BEGIN
               SELECT @n_continue = 3
               SELECT @c_errmsg = 'ispWAVLP03:' + RTRIM(ISNULL(@c_errmsg, ''))
            END
            ...
            BEGIN
               SELECT @n_continue = 3
               SELECT @n_err = 63560
               SELECT @c_errmsg = "NSQL" + CONVERT(NVARCHAR(5), @n_err) + ": Insert Into LOADPLAN Failed. (ispWAVLP03)"
            END
         END
         --WL01 E
         ...
         BEGIN
            ...
            BEGIN
               ...
               BEGIN
                  ...
                  SELECT @n_err = 63560
                  SELECT @c_errmsg = "NSQL" + CONVERT(NVARCHAR(5), @n_err)
                                     + ": Insert Into LOADPLAN Failed. (ispWAVLP03)"
               END
            END
            ...
            BEGIN
               ...
               BEGIN
                  ...
                  SELECT @n_err = 63508
                  SELECT @c_errmsg = "NSQL" + CONVERT(NVARCHAR(5), @n_err)
                                     + ": Insert Into LOADPLANDETAIL Failed. (ispWAVLP03)"
                  GOTO RETURN_SP
               END
            END
         ...
         --   SELECT @n_continue = 3  
         --   SELECT @n_err = 63518  
         --   SELECT @c_errmsg="NSQL"+CONVERT(NVARCHAR(5),@n_err)+": Update Into LOADPLAN Failed. (ispWAVLP03)"  
         --   GOTO RETURN_SP  
         --END  
         --NJOW01 End
         --WL01 E
...
BEGIN
    ...
    BEGIN
        ...
        END
    END 
    EXECUTE nsp_logerror @n_err, @c_errmsg, 'ispWAVLP03' 
