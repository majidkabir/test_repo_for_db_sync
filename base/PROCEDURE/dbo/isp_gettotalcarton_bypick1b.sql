...
/************************************************************************/
/* Stored Procedure: isp_GetTotalCarton_ByPick1b                        */
/* Creation Date:                                                       */
/* Copyright: IDS                                                       */
/* Written by:                                                          */
/*                                                                      */
/* Purpose:  296090-modified from isp_GetTotalCarton_ByPick1            */
/*                                                                      */
/* Called By:                                                           */
/*                                                                      */
/* PVCS Version: 1.0                                                    */
...
/* 27-Nov-2013  NJOW01        296090-Count carton not based on loc      */
/************************************************************************/
 
CREATE PROCEDURE [dbo].[isp_GetTotalCarton_ByPick1b] 
...
AS
   ...
   BEGIN
      ...
      --NJOW01      
      SELECT PD.Sku, SUM(PD.Qty) AS Qty, PD.Packkey
      INTO #TEMP_PICKDEAIL
      FROM PICKDETAIL PD(NOLOCK)
      WHERE PD.Orderkey = @cOrderkey
      GROUP BY PD.Sku, PD.Packkey
      
      SELECT @nCtnCnt1 = ISNULL( SUM( FLOOR( PD.QTY / Pack.CaseCnt)), 0)
      FROM #TEMP_PICKDEAIL PD WITH (NOLOCK)
         INNER JOIN dbo.Pack WITH (NOLOCK) ON PD.PackKey = Pack.PackKey
      --WHERE PD.OrderKey = @cOrderKey
      WHERE FLOOR( PD.QTY / CAST( Pack.CaseCnt AS INT)) > 0
   ...
   BEGIN
      ...
      SELECT @nCtnCnt1 = ISNULL( SUM( FLOOR( PD.QTY / Pack.CaseCnt)), 0)
      FROM #TEMP_PICKDEAIL2 PD WITH (NOLOCK) 
      JOIN Pack WITH (NOLOCK) ON Pack.PackKey = PD.PackKey
      WHERE FLOOR( PD.QTY / CAST( Pack.CaseCnt AS INT)) > 0
      ...
      FROM dbo.OrderDetail OD WITH (NOLOCK)
         ...
         INNER JOIN dbo.Pack WITH (NOLOCK) ON Pack.PackKey = OD.PackKey
      WHERE OD.LoadKey = @cLoadKey
   ...
   BEGIN
      ...
      --NJOW01
      SELECT PD.Sku, SUM(PD.Qty) AS Qty, PD.Packkey
      INTO #TEMP_PICKDEAIL3
      FROM PICKDETAIL PD(NOLOCK)
      JOIN RefKeyLookup RKL(NOLOCK) ON RKL.PickDetailKey = PD.PickDetailKey
      WHERE RKL.PickSlipNo = @cPickSlipNo
      GROUP BY PD.Sku, PD.Packkey

      SELECT @nCtnCnt1 = ISNULL( SUM( FLOOR( PD.QTY / Pack.CaseCnt)), 0)
      FROM #TEMP_PICKDEAIL3 PD WITH (NOLOCK)
      JOIN Pack WITH (NOLOCK) ON PD.PackKey = Pack.PackKey
      WHERE FLOOR( PD.QTY / CAST( Pack.CaseCnt AS INT)) > 0
