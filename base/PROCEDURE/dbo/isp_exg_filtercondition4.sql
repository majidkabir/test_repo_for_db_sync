...
/*************************************************************************/  
/* Stored Procedure: isp_EXG_FilterCondition4                            */  
/* Creation Date: 16 Jun 2020                                            */  
/* Copyright: LFL                                                        */  
/* Written by: GuanHao Chan                                              */  
/*                                                                       */  
/* Purpose: Excel Generator Sub StoredProcedure FilterConfition 4.       */  
/*                                                                       */  
...
CREATE PROCEDURE [dbo].[isp_EXG_FilterCondition4]  
...
BEGIN  
 ...
 IF @b_Debug = 1  
   BEGIN  
      PRINT '[dbo].[isp_EXG_FilterCondition4]: Start...'  
      PRINT '[dbo].[isp_EXG_FilterCondition4]: '  
   ...
   BEGIN  
         SET @n_Err = 210001  
         SET @c_ErrMsg = 'Invalid EXG Hdr ID. (isp_EXG_FilterCondition4)'  
         SET @n_Continue = 3  
         GOTO QUIT  
   END  
   ...
   BEGIN TRY  
      ...
      BEGIN  
         SET @n_Err = 210002  
         SET @c_ErrMsg = ' StorerKey cannot be null. - (isp_EXG_FilterCondition4)'  
     
         IF @b_Debug = 1  
         BEGIN  
            PRINT '[isp_EXG_FilterCondition4]: Execute SP Name Failed...' + ' @c_ErrMsg=' + @c_ErrMsg  
         END  
         GOTO QUIT  
      END  
      ...
      BEGIN  
         SET @n_Err = 210003  
         SET @c_ErrMsg = ' MbolKey cannot be null. - (isp_EXG_FilterCondition4)'  
      ...
      BEGIN  
         ...
         BEGIN  
            SET @n_Err = 210004  
            SET @c_ErrMsg = '(ConsigneeKey To) cannot be null. - (isp_EXG_FilterCondition4)'  
         ...
         INSERT INTO @Temp_BatchPVal ([filename]  
                                    ...
                                    , [PVal3]  
                                    , [PVal4])  
         SELECT CONCAT(REPLACE(REPLACE(@c_FileNameFormat, '<MBOLKey>', TRIM(MbolKey)), '<ConsigneeKey>', TRIM(ConsigneeKey)), @c_FileExt)  
               , @c_ParamVal1  
               , MbolKey  
               , ConsigneeKey  
               , ConsigneeKey  
...
STARTCURSOR:    
   ...
   BEGIN CATCH   
      ...
      SET @n_Err = @n_ITFErr  
      SET @c_ErrMsg = LTRIM(RTRIM(@c_ITFErrMsg)) + ' (isp_EXG_FilterCondition4)'  
                 
      IF @b_Debug = 1  
      BEGIN  
         PRINT '[isp_EXG_FilterCondition4]: Execute SP Name Failed...'   
               + ' @c_ErrMsg=' + @c_ErrMsg  
      END  
      GOTO QUIT  
   END CATCH  
...
QUIT:  
   ...
   BEGIN        
      ...
      IF @b_Debug = 1  
      BEGIN  
         PRINT '[dbo].[isp_EXG_FilterCondition4]: @c_ErrMsg=' + RTRIM(@c_ErrMsg)  
         PRINT '[dbo].[isp_EXG_FilterCondition4]: @b_Success=' + RTRIM(CAST(@b_Success AS NVARCHAR))  
      END  
