...
/************************************************************************/
/* Stored Procedure: nspTTMEvaluateRTTasks                              */
/* Creation Date:                                                       */
/* Copyright: IDS                                                       */
/* Written by:                                                          */
/*                                                                      */
...
/* 17-12-2012   1.0   ChewKP    SOS#224380                              */   
/* 16-01-2019   1.1   TLTING01  Wrong table lock hint                   */   
/* 05-09-2018   1.2   James     WMS-14152. Param to Move all Validation */  
/*                              to be done in Pickcode (james01)        */  
/************************************************************************/

CREATE PROC    [dbo].[nspTTMEvaluateRTTasks]
...
BEGIN
    ...
    BEGIN  
        DECLAREcursor_RPTTASKCANDIDATES:  
         -- (james01)   
        ...
        BEGIN  
            ...
            SELECT @c_errmsg = "NSQL"+CONVERT(CHAR(5) ,@n_err)+  
                   ": Execute Of Move Tasks Pick Code Failed. (nspTTMEvaluateSPKTasks)"   
        ...
        BEGIN  
            ...
            GOTO DECLAREcursor_RPTTASKCANDIDATES  
        END  
          
        OPEN cursor_RPTTASKCANDIDATES  
        SELECT @n_err = @@ERROR  
              ,@n_cnt = @@ROWCOUNT  
    ...
    BEGIN  
        ...
        BEGIN  
            ...
            FETCH NEXT FROM cursor_RPTTASKCANDIDATES  
   ...
   BEGIN  
      ...
      BEGIN  
       ...
       BEGIN  
           ...
           END  
       END  
       execute nsp_logerror @n_err, @c_errmsg, 'nspTTMEvaluateSPKTasks'  
