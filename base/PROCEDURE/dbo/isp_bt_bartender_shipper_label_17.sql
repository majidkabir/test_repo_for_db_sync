...
/* 2021-04-02 1.2  CSCHONG    WMS-16024 PB-Standardize TrackingNo (CS01)      */    
/******************************************************************************/                
...
BEGIN                      
   ...
   SET @c_SQLJOIN = +' SELECT DISTINCT ORD.loadkey,ORD.orderkey,ORD.externorderkey, '
                ...
                +' '''',ORD.M_Address1,ORD.M_Address2,ORD.M_City,'''','''',ORD.Priority,ORD.Userdefine10,LEFT(ISNULL(ORD.Notes2,''''),80),'''' '        
                + CHAR(13) +            
   ...
   SELECT DISTINCT col02,col38,'1' from #Result          
   ...
   BEGIN       
      ...
      SELECT TOP 1 @c_UDF01 = C.UDF01  
      FROM Codelkup C WITH (NOLOCK)    
      WHERE C.Short = @c_Type              
      AND C.Storerkey = @c_Storerkey            
      AND C.Listname = 'ordCourier' 
      AND C.Long = @c_doctype
      AND C.Notes = @c_GetShipperkey    
...
EXIT_SP:    
   ...
   EXEC isp_InsertTraceInfo   
      ...
      @c_TraceName = 'isp_BT_Bartender_Shipper_Label_17',  
