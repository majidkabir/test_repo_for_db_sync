...
/* Copyright: IDS                                                       */
/* Written by: NJOW (Modified from isp_Print_SSCC_CartonLabel_vital)		*/
/*                                                                      */
/* Purpose:  MUJI - Print SSCC Carton Label (SOS#136252) 	              */
...
CREATE   PROC [dbo].[isp_Print_SSCC_CartonLabel_02_UF] ( 
   ...
   BEGIN
      ...
      SELECT 
         ...
         */
         ...
         @cOrdUserDefine03   = ISNULL(RTRIM(OH.UserDefine03),''),
         @cOrdUserDefine04   = ISNULL(RTRIM(OH.UserDefine04),''),
         /* SOS117424 
      ...
      SET @curLABEL = CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
   	...
   	BEGIN
         ...
         SELECT @cBillTo_Barcode = master.dbo.fnc_GetCharASCII(202)+'421036' +
                  dbo.fnc_RTrim(dbo.fnc_LTrim(@cBillTo_Zip)) + master.dbo.fnc_GetCharASCII(202) +
                  '90' +
                  dbo.fnc_RTrim(dbo.fnc_LTrim(@cOrdUserDefine03))	--SOS117424
                  /*
                  CASE WHEN @cOrderType = 'DTDC' THEN dbo.fnc_RTrim(dbo.fnc_LTrim(@cOrdUserDefine03))
