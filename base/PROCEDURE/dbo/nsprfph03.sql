...
BEGIN
   ...
   BEGIN
      SELECT Lot FROM LOTATTRIBUTE (NOLOCK)
      WHERE Lot = @c_lot
      ...
      BEGIN
         ...
         SELECT @c_errmsg="NSQL"+CONVERT(char(5), @n_err)+": Invalid Lot. (nspRFPH03)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
         /* End Trap SQL Server Error */
         execute nsp_logerror @n_err, @c_errmsg, "nspRFPH03"
      ...
      BEGIN
         ...
         SELECT @c_errmsg="NSQL"+CONVERT(char(5), @n_err)+": Select Failed On LOTATTRIBUTE. (nspRFPH03)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
         /* End Trap SQL Server Error */
      END
   END
   /* Tomd End */
   ...
   ELSE IF @b_debug = 1
      ...
      END
   END
   --**ZY retrieve sku,loc,id lot,storerkey from lotxlocxid based on input
   IF @n_continue = 1 OR @n_continue = 2
   BEGIN
      ...
      BEGIN
         SELECT @n_continue = 3
         SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err=70406
         SELECT @c_errmsg="NSQL"+CONVERT(char(5), @n_err)+": Select Failed On LOTxLOCxID. (nspRFPH03)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
      END
   ELSE
      BEGIN
         IF @n_cnt > 1
         BEGIN
            SELECT @n_continue = 3
            SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err=70406
            SELECT @c_errmsg="NSQL"+CONVERT(char(5), @n_err)+": Duplicate record found (nspRFPH03)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
         END
      ELSE
         BEGIN
            IF @n_cnt = 0
            BEGIN
               SELECT @n_continue = 3
               SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err=70406
               SELECT @c_errmsg="NSQL"+CONVERT(char(5), @n_err)+": No record found (nspRFPH03)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
            END
         END
      END
   END
   ...
   BEGIN
      ...
      BEGIN
         ...
         SELECT @c_errmsg="NSQL"+CONVERT(char(5), @n_err)+": Update1 Failed On PHYSICAL. (nspRFPH03)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
         /* End Trap SQL Server Error */
      END
   ELSE IF @n_cnt = 0 AND @n_continue <> 3
      BEGIN
         ...
         BEGIN
            ...
            SELECT @c_errmsg="NSQL"+CONVERT(char(5), @n_err)+": Insert Failed On PHYSICAL. (nspRFPH03)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
            /* End Trap SQL Server Error */
         END
      END
   END
ELSE
   BEGIN
      ...
      BEGIN
         ...
         SELECT @c_errmsg="NSQL"+CONVERT(char(5), @n_err)+": Update Failed On PHYSICAL. (nspRFPH03)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
         /* End Trap SQL Server Error */
      END
   ELSE IF @n_cnt = 0 AND @n_continue <> 3
      BEGIN
