...
BEGIN
   ...
   DECLARE @n_cnt int
   DECLARE @c_displaymsg NVARCHAR(60)
   ...
   BEGIN
      ...
      BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Scan out of pick slip failed. (nspRFPK06)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + ' ) '
      END
      ELSE
      BEGIN
         SELECT @c_displaymsg = 'Pick slip scanned out'
      END
   END
   ...
   + dbo.fnc_RTrim(@c_displaymsg)       + @c_senddelimiter
   ...
   BEGIN
      ...
      BEGIN
         ...
         END
      END
      execute nsp_logerror @n_err, @c_errmsg, 'nspRFPK06'
