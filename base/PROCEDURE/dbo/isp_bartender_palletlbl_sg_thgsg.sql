...
/******************************************************************************/                 
/* Copyright: IDS                                                             */                 
/* Purpose: isp_Bartender_PALLETLBL_SG_THGSG                                  */                 
/*                                                                            */                 
/* Modifications log:                                                         */                 
/*                                                                            */                 
/* Date       Rev  Author     Purposes                                        */                 
/* 2019-06-10 1.0  CSCHONG    Created (WMS-9309)                              */            
/******************************************************************************/                
                  
CREATE PROC [dbo].[isp_Bartender_PALLETLBL_SG_THGSG]                      
  ...
  DECLARE @d_Trace_StartTime    DATETIME,   
   ...
   SET @c_Trace_ModuleName = ''  
    ...
    SET @c_GroupBy = ' GROUP BY RECDET.SKU,substring(S.descr,1,80),CONVERT(NVARCHAR(80),RECDET.Lottable04,103),RECDET.Lottable02,'
                    +' RECDET.Lottable01,RECDET.toid,CASE WHEN RECDET.toloc =''THINSTG'' THEN S.Putawayzone ELSE l.putawayzone END,'
               + ' RECDET.Receiptkey'
    ...
    SET @c_SQLJOIN = +' SELECT DISTINCT RECDET.SKU,substring(S.descr,1,80),CONVERT(NVARCHAR(80),RECDET.Lottable04,103),RECDET.Lottable02,'+ CHAR(13)
             ...
             + ' CASE WHEN RECDET.toloc =''THINSTG'' THEN S.Putawayzone ELSE l.putawayzone END ,'''', '      --10   
             + ' '''','''','''','''','''','     --15       
             + CHAR(13) +      
             + ' '''','''','''','''','''','         --20      
             + ' '''','''','''','''','''','
  ...
  SELECT DISTINCT col60,col07,col09     
   ...
   EXIT_SP:    
      ...
      EXEC isp_InsertTraceInfo   
         ...
         @c_TraceName = 'isp_Bartender_PALLETLBL_SG_THGSG',  
