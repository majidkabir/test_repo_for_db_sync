...
/************************************************************************/  
/* Stored Procedure: nspAL10005                                         */  
/* Creation Date: 17-Mar-2014                                           */  
/* Copyright: IDS                                                       */  
/* Written by:                                                          */  
/*                                                                      */  
/* Purpose:   SOS#305113 - Wave Allocation (Wan01)                      */  
/*            Copy And Modify From nspAL10001                           */  
/* Called By:                                                           */  
/*                                                                      */  
/* PVCS Version: 1.3                                                    */  
/*                                                                      */  
...
/* 17-Mar-2014  YTWan         SOS#305113 - Wave Allocation (Wan01)      */
/************************************************************************/  
CREATE PROC    [dbo].[nspAL10005]  
...
BEGIN  
   ...
   DECLARE @c_LocationType     NVARCHAR(10),  
         ...
         , @n_TotatAvailable     INT  
  
   SET @c_LocationType = ''  
   SET @c_LocationCategory = ''  

   SET @c_SQL = N'  
      SELECT @n_TotatAvailable = ISNULL(SUM(LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED - LOTxLOCxID.QtyReplen),0)
   ...
   SET @c_SQLParm =  N'@c_Facility   NVARCHAR(5),  @c_StorerKey  NVARCHAR(15), @c_SKU        NVARCHAR(20), ' +  
                      ...
                      '@d_Lottable04 NVARCHAR(18), @d_Lottable05 NVARCHAR(18), @n_TotatAvailable INT OUTPUT' 
   ...
   SET @c_SQL = N'  
      ...
      AND ISNULL(RTRIM(CL.Short),''R'') = ''S''      
      AND LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED - LOTxLOCxID.QtyReplen > 0' +
    ' AND ' + CAST(@n_TotatAvailable AS VARCHAR(10)) + ' >=' + CAST(@n_QtyLeftToFulfill AS VARCHAR(10)) +  
    ' AND LOTxLOCxID.STORERKEY = @c_StorerKey AND LOTxLOCxID.SKU = @c_SKU ' + master.dbo.fnc_GetCharASCII(13) +  
      ...
      CASE WHEN ISNULL(CONVERT(DATETIME, @d_Lottable05, 112), '1900-01-01 00:00:00.000') = '1900-01-01 00:00:00.000'   
                THEN '' ELSE ' AND LA.Lottable05 = @d_Lottable05 ' + master.dbo.fnc_GetCharASCII(13) END +                    
      'ORDER BY (LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED - LOTxLOCxID.QtyReplen)
      ,CASE LOC.LocationType WHEN ''DYNPPICK'' THEN 1 ELSE 99 END  
