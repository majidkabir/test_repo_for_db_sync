...
/***************************************************************************/
/* Stored Procedure: isp_RPT_REPL_REPLEN_010                               */
/* Creation Date: 06-Sep-2022                                              */
/* Copyright: LFL                                                          */
/* Written by: WLChooi                                                     */
/*                                                                         */
/* Purpose: WMS-20668 - VN-ADIDAS-Generate Replenishment Task              */
/*                                                                         */
...
/***************************************************************************/
CREATE   PROC [dbo].[isp_RPT_REPL_REPLEN_010]
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN      
         ...
         SET @n_MaxQtyInLoc = (FLOOR(@n_QtyLocationLimit * 1.2) - (@n_Qty - @n_QtyPicked)) - @n_PendingMoveIn   --20% Tolerance
         SET @n_QtyinPickLoc = @n_Qty - @n_QtyPicked
                   
         IF @n_ReplenQty < @n_NetQtyExpected --if multi lots, the pendingmovein qty might apply to diffent lot caused the replenqty less, so have make sure at least replen overallocate qty.
         ...
         BEGIN
            ...
            IF @n_Casecnt <= @n_RemainingQty    --50 <= 100
            BEGIN
               SET @n_QtyToReplen = @n_Casecnt
            END
            ELSE --@n_Casecnt > @n_RemainingQty  --110 > 100
            BEGIN
               IF @n_Casecnt <= @n_MaxQtyInLoc   --110 <= 120 (100 * 1.2) - Still allow replenish
               BEGIN
                  SET @n_QtyToReplen = @n_Casecnt
               END
               ELSE
   ...
   IF OBJECT_ID('tempdb..#REPLENISHMENT') IS NOT NULL
