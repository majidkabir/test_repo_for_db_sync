   ...
   BEGIN
      DECLARE Cursor_RPFTaskCandidates CURSOR FAST_FORWARD READ_ONLY FOR --Note: global cursor
         ...
         FROM dbo.TaskDetail WITH (NOLOCK)
            JOIN dbo.LOC LOC1 WITH (NOLOCK) ON (TaskDetail.FromLOC = LOC1.LOC)
            JOIN dbo.LOC LOC2 WITH (NOLOCK) ON (TaskDetail.ToLOC = LOC2.LOC)
            JOIN dbo.AreaDetail WITH (NOLOCK) ON (AreaDetail.PutawayZone = LOC1.PickZone)
   ...
   BEGIN
      DECLARE Cursor_RPFTaskCandidates CURSOR FAST_FORWARD READ_ONLY FOR --Note: global cursor
         ...
         FROM dbo.TaskDetail WITH (NOLOCK)
            ...
            JOIN dbo.LOC LOC2 WITH (NOLOCK) ON (TaskDetail.ToLOC = LOC2.LOC)
            JOIN dbo.AreaDetail WITH (NOLOCK) ON (AreaDetail.PutawayZone = LOC1.PickZone)
         WHERE LOC1.Facility = @c_Facility
            AND TaskDetail.TaskType = 'RPF'
            AND TaskDetail.Status = '0'
            AND TaskDetail.UserKeyOverRide IN (@c_UserID, '')
            AND EXISTS( SELECT 1
   ...
   BEGIN
      ...
      BEGIN
        ...
        BEGIN
          ...
          END
        END
        execute nsp_logerror @n_err, @c_errmsg, 'nspTTMRP19'
