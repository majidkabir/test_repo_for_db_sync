...
/************************************************************************/
/* Trigger:  ispGenerateSOfromKit02                                     */
...
/* 15-Mar-2022 MINGLE   1.0  WMS-19124 Created.                         */
/* 15-Mar-2022 Mingle   1.0  DevOps Combine Script                      */
/************************************************************************/

CREATE   PROCEDURE [dbo].[ispGenerateSOfromKit02] 
...
AS
   ...
   BEGIN 
      SELECT @c_StorerKey = KIT.Storerkey,  
             ...
             @c_Remarks = CASE WHEN KITDETAIL.TYPE = 'T' THEN ISNULL(KITDETAIL.SKU,'') ELSE ISNULL(KIT.REMARKS,'') END
      FROM   KIT (NOLOCK)
      JOIN KITDETAIL(NOLOCK) ON KITDETAIL.KITKey = KIT.KITKey
      WHERE  KIT.KitKey = @c_KitKey
      ...
      JOIN SKU(NOLOCK) ON SKU.Sku = KITDETAIL.Sku AND SKU.StorerKey = KITDETAIL.StorerKey
   ...
   BEGIN         
      ...
      BEGIN
         ...
         BEGIN
            ...
            INSERT INTO ORDERS (OrderKey, ExternOrderKey, BuyerPO, StorerKey, Type, OrderGroup, Facility, C_Company, DeliveryDate, Notes, Userdefine01,Userdefine02)  
            VALUES (@c_NewOrderKey, @c_ExternKitKey, @c_BuyerPO, @c_StorerKey, @c_Ordertype, 'KIT', @c_Facility, @c_UsrDef2, @d_EffectiveDate, @c_Remarks, @c_KitKey,@c_SKUGroup)  
         END
         ELSE
         BEGIN
            SELECT @n_continue = 3
            SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err = 63526   
            SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Generate Order Key Failed! (ispGenerateSOfromKit02)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
         END
      END    
      ELSE
      BEGIN
         SELECT @n_continue = 3
         SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err = 63526   
         SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Storer Key is BLANK! (ispGenerateSOfromKit02)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
      END
      ...
      BEGIN
         SELECT @n_continue = 3
         SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err = 63526   
         SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Codelkup Code2 is BLANK! (ispGenerateSOfromKit02)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
      END
   ...
   BEGIN         
      ...
      BEGIN
         ...
         IF EXISTS (SELECT 1 FROM CODELKUP (NOLOCK) WHERE Storerkey = dbo.fnc_RTrim(@c_StorerKey) 
                  AND CODE = @c_SKUGroup AND LISTNAME = 'EAShelflif' AND LONG IS NOT NULL)
         BEGIN
            SELECT @n_slife = LONG
            FROM CODELKUP (NOLOCK)
            WHERE LISTNAME = 'EAShelflif'
            AND Storerkey = dbo.fnc_RTrim(@c_StorerKey)
            AND CODE = @c_SKUGroup
         END 
