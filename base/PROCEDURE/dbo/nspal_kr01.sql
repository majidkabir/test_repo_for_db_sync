...
GO
/************************************************************************/    
/* Stored Procedure: nspAL_KR01                                         */    
/* Creation Date: 21-AUG-2020                                           */    
/* Copyright: LFL                                                       */    
/* Written by:                                                          */    
/*                                                                      */    
/* Purpose: WMS-14813 KR Allbirds Allocation                            */
/*          SkipPreallocation = '1'                                     */
/*                                                                      */
/* Called By: Wave                                                      */    
/*                                                                      */    
   ...
   */
   ...
   SET @c_SQL = N'   
    ...
    ' FROM LOTxLOCxID (NOLOCK)
      ...
      LEFT JOIN PutawayZone PA (NOLOCK) ON PA.putawayzone = LOC.putawayzone   
      ...
      CASE WHEN @c_UOM = '7' THEN ' AND (LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED) >= @n_UOMBase '
           ELSE ' AND (LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED - LOTxLOCxID.QtyReplen) >= @n_UOMBase ' END +
      CASE WHEN @c_UOM = '6' THEN ' AND LOC.LocationType IN(''PICK'',''CASE'') ' 
           WHEN @c_UOM = '7' THEN ' AND (LOC.LocationType NOT IN (''PICK'',''CASE'') OR SL.LocationType IN(''PICK'',''CASE'')) ' ELSE ' ' END +
      ...
      ' ORDER BY PA.ZoneCategory, LA.Lottable05, LA.Lot, LOC.LogicalLocation, LOC.LOC '
