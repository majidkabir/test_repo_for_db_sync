...
/* Purpose: POD                                                         */
/*                                                                      */
/* Called By: r_dw_pod_06  WMS-827 (copy from r_dw_pod_03)              */ 
/*                                                                      */
...
/* 28-Mar-2017 CSCHONG    1.1  WMS-1428 - Change field mapping (CS01)   */
/* 15-Dec-2018  TLTING01  1.2   Missing nolock                          */
/* 28-Jan-2019  TLTING_ext 1.3  enlarge externorderkey field length      */
/************************************************************************/
CREATE PROCEDURE [dbo].[isp_pod_06]
...
BEGIN
   ...
   (mbolkey           NVARCHAR(10) null,
    ...
    EstleadTime    NVARCHAR(50),
    ORDUDef01      NVARCHAR(20),
    ORDUDEF02      NVARCHAR(20),
    domainLong     NVARCHAR(10))  
   ...
   WHERE ORDERS.Mbolkey = @c_Mbolkey
    ...
    ( mbolkey,        MbolLineNumber ,    ExternOrderKey,        Orderkey,        type,
      ...
      Domain,ConsigneeKey,EstleadTime,ORDUDef01,ORDUDef02,domainLong )   
    SELECT 
      ...
      f.Susr2,    
         ...
         ,ISNULL(e.UDF01,''),c.UserDefine01,c.UserDefine02,ISNULL(g.long,'')
    ...
    BEGIN 
      ...
      BEGIN
         ...
         SELECT @n_casecnt = CASE WHEN @n_CountCntByLabelno = 0 
                                  THEN mb.CtnCnt1--COUNT(DISTINCT f.cartonno)  --(CS01)
                                  ELSE COUNT(DISTINCT f.labelno)    
                                  END,
                @n_qty     = SUM(f.qty)
         FROM PICKHEADER e (nolock), PACKDETAIL f (nolock)
         ,MBOLDETAIL AS mb WITH (NOLOCK)                        --(CS01)
         WHERE e.orderkey = @c_orderkey and e.PickHeaderKey = f.pickslipno 
         AND mb.OrderKey=e.OrderKey                              --(CS01)
         GROUP BY mb.CtnCnt1                                    --(CS01)
      END
