      ...
      IF @b_debug=1            
        ...
        BEGIN            
            ...
            SELECT @c_errmsg = 'NSQL'+CONVERT(CHAR(5) ,@n_err)+            
                   ': Update to TaskDetail table failed. (nspTTMPK09)'+' ( '+            
   ...
   BEGIN            
      ...
      BEGIN             
          ...
          BEGIN            
             ...
             JOIN dbo.PickDetail PD (NOLOCK) ON (TD.TaskDetailKey = PD.TaskDetailKey)    --tlting01        
             JOIN dbo.OrderDetail OD (NOLOCK)  ON (PD.OrderKey = OD.OrderKey AND PD.OrderLineNumber = OD.OrderLineNumber)            
             JOIN dbo.Orders O (NOLOCK)  ON (OD.OrderKey = O.OrderKey AND TD.WaveKey = O.UserDefine09)            
          ...
          BEGIN            
              ...
              SELECT @c_errmsg = 'NSQL'+CONVERT(CHAR(5) ,@n_err)+            
                     ': Could not Open Cursor_Eval02. (nspTTMPK09)'+' ( '             
                    +' SQLSvr MESSAGE='+RTRIM(@c_errmsg)             
                    +' ) '            
          END            
          ELSE            
...
--                  AND TaskDetailKey = @c_TaskDetailKey          
                ...
                BEGIN            
                    ...
                    SELECT @c_errmsg = 'NSQL'+CONVERT(CHAR(5) ,@n_err)+': UPDATE TaskDetail Failed. (nspTTMPK09)'+' ( '             
                          + ' SQLSvr MESSAGE='+RTRIM(@c_errmsg)+' ) '            
              ...
              BEGIN            
               ...
               END            
                ...
                BEGIN            
                    ...
                    SELECT @c_errmsg = 'NSQL'+CONVERT(CHAR(5) ,@n_err)+            
                           ': UPDATE TaskDetail Failed. (nspTTMPK09)'+' ( '             
                          +' SQLSvr MESSAGE='+RTRIM(@c_errmsg)             
                          +' ) '            
                END            
            END            
...
--                    AND TD.LoadKey = @cLoadKey          
               ...
               BEGIN            
                  ...
                  SELECT @c_errmsg = 'NSQL'+CONVERT(CHAR(5) ,@n_err)+            
                   ': Lock TaskDetail Failed. (nspTTMPK09)'+' ( '             
                        +' SQLSvr MESSAGE='+RTRIM(@c_errmsg)             
                        +' ) '            
                  GOTO NOTHINGTOSEND                              
               END                               
 ...
 DISPATCHCHECK:            
     ...
     BEGIN            
         ...
         BEGIN            
             ...
             END            
         END            
         EXECUTE nsp_logerror @n_err, @c_errmsg, 'nspTTMPK09'            
