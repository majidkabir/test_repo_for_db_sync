...
/***************************************************************************/
/* Stored Procedure: ispASNFZ01                                            */
/* Creation Date: 14-FEB-2014                                              */
/* Copyright: IDS                                                          */
/* Written by:                                                             */
/*                                                                         */
/* Purpose: SOS#302461 - ShipGreen Batch Release via ASN                   */
...
BEGIN  
   ...
   DECLARE CUR_RECEIPT CURSOR LOCAL FAST_FORWARD READ_ONLY FOR  
      ...
      LEFT JOIN CODELKUP CL (NOLOCK) ON CL.Short = RD.Lottable01  
      WHERE RD.Receiptkey = @c_Receiptkey
      AND CL.Listname = 'WMSSAPGRMT'

   OPEN CUR_RECEIPT  
  
   FETCH NEXT FROM CUR_RECEIPT INTO @c_UDF01, @c_UDF02, @c_UDF03, @c_UDF04, @c_Notes, @c_Storerkey, @c_ExternReceiptkey, @c_Facility, 
                                    @c_ReceiptLineNumber, @c_Sku, @c_ToLoc, @n_QtyReceived, @c_Packkey, @c_UOM, 
   ...
   BEGIN  
   	  IF ISNULL(@c_UDF01,'') <> ''
   	  BEGIN
   	  	 IF EXISTS(SELECT 1 FROM INVENTORYHOLD (NOLOCK)
                   WHERE Storerkey = @c_Storerkey  
                   AND SKU = @c_Sku
                   AND Status = @c_UDF01
                   AND LEFT(Lottable02,8) = @c_Lottable02) --Found ON HOLD
         BEGIN
     	  	  SET @c_ExportStatus = LEFT(ISNULL(@c_UDF02,''),1)
               ...
               BEGIN
                  INSERT INTO TRANSFER (Transferkey, FromStorerkey, ToStorerkey, Type, ReasonCode, CustomerRefNo, Remarks, Facility, ToFacility)
                                VALUES (@c_TransferKey, @c_Storerkey, @c_Storerkey, @c_UDF01, @c_UDF01, @c_ExternReceiptkey, @c_Notes, @c_Facility, @c_Facility)
                  ...
                  BEGIN
                     ...
                     SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err = 63504
   	                 SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Insert Transfer Failed! (ispASNFZ01)' + ' ( '
               ...
               BEGIN
                  ...
                  SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err = 63505
   	              SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Generate Transfer Key Failed! (ispASNFZ01)' + ' ( '
                                         + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
                  GOTO QUIT_SP
               END
            ...
            VALUES (@c_Transferkey, @c_TransferLineNumber, @c_Storerkey, @c_Sku, @c_Lot, @c_ToLoc, @c_ID, @n_QtyReceived, @c_Packkey, @c_UOM,
                    ...
                    ISNULL(@c_UDF04,''), @c_Lottable02, @c_Lottable03, @dt_Lottable04, @dt_Lottable05)
            ...
            BEGIN
               ...
               SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err = 63506
   	           SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Insert TransferDetail Failed! (ispASNFZ01)' + ' ( '
                                      + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
               GOTO QUIT_SP
            END
         ...
         ELSE
         BEGIN
     	  	  SET @c_ExportStatus = LEFT(ISNULL(@c_UDF03,''),1)
         END         
   	  END
   	  ELSE
   	  BEGIN
      ...
      BEGIN
         ...
         SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err = 63507
   	     SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Update ReceiptDetail Failed! (ispASNFZ01)' + ' ( '
                                + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
         GOTO QUIT_SP
      END
   ...
   BEGIN
      ...
      BEGIN
         ...
         SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err = 63508
   	     SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Finalize Transfer Failed! (ispASNFZ01)' + ' ( '
                                + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
         GOTO QUIT_SP
      END
   END   
   ...
   BEGIN
      ...
      BEGIN
         ROLLBACK TRAN
      END
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'ispASNFZ01'
