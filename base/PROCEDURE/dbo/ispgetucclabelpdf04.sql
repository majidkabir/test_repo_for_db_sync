...
/************************************************************************/
/* Trigger: ispGetUCCLABELPDF04                                         */
/* Creation Date: 10-Sep-2020                                           */
/* Copyright: LF Logistics                                              */
/* Written by: CSCHONG                                                  */
/*                                                                      */
/* Purpose: WMS-14517 - [TW] New MLB_Exceed Loadplan print carton       */
/*                      label to retrieve PDF                           */
/*                                                                      */
...
BEGIN  
   ...
   SET @c_GetDefaultPrinter_Paper = ''
   SET @c_GetDefaultPrinter = ''
   ...
   --IF LTRIM(RTRIM(ISNULL(@c_OrderKey,''))) = ''
   --BEGIN
   --   SET @n_PrintAction = 0
   --   GOTO QUIT_SP
   --END

   IF LTRIM(RTRIM(ISNULL(@c_OHStatus,''))) <> '5'
   BEGIN
      SET @n_PrintAction = 0
      GOTO QUIT_SP
   END
   ...
   BEGIN
      ...
      BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err) 
               + ': PDF Image Server Not Yet Setup/Enable In Storerconfig for Configkey :' + @c_Configkey + ' (ispGetUCCLABELPDF04) '
               + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
         SET @n_PrintAction = 0
         GOTO QUIT_SP
       END
      ...
      SET @c_PDFFilePath = @c_PdfFolder  + 'MOMO_' + @c_Storerkey + '_' + RTRIM(@c_Orderkey) + '.PDF'
      SET @c_PdfFile = @c_PDFFilePath
      EXEC dbo.xp_fileexist @c_PDFFilePath, @n_IsExists OUTPUT

      IF @n_IsExists = 0
      BEGIN
         SET @c_PDFFilePath = @c_ArchiveFolder + 'MOMO_' + @c_Storerkey + '_' + RTRIM(@c_Orderkey) + '.PDF'
         SET @c_PdfFile = @c_PDFFilePath
         SET @c_ArchivePath = '' 
         SET @c_ActionType = '2'
         EXEC dbo.xp_fileexist @c_PDFFilePath, @n_IsExists OUTPUT 
      ...
      BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': PDF filename with Orderkey : ' + @c_Orderkey + ' not found. '
                         +'(ispGetUCCLABELPDF04)' + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '
         SET @n_PrintAction = 0
         GOTO QUIT_SP  
      END
   END
  ...
  BEGIN
      ...
      BEGIN
         ...
         END          
      END
      EXECUTE nsp_logerror @n_err, @c_errmsg, "ispGetUCCLABELPDF04"
