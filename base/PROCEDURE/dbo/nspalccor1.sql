...
GO
/************************************************************************/    
/* Stored Procedure: nspALCCOR1                                         */    
/* Creation Date: 06-MAR-2019                                           */    
/* Copyright: LFL                                                       */    
/* Written by:                                                          */    
/*                                                                      */    
/* Purpose: WMS-11282 CN Coolcore Allocation                            */
/*          SkipPreallocation = '1'                                     */
/*                                                                      */
/* Called By: Wave                                                      */    
/*                                                                      */    
...
BEGIN    
   ...
   SET @c_SQL = N'   
      ...
      CASE WHEN @c_Mode = 'CONSO' THEN ' AND (LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED - LOTxLOCxID.QtyReplen) >= @n_QtyLeftToFulfill ' ELSE  ' ' END +
      ...
      CASE WHEN @c_Mode = 'CONSO' THEN ' ORDER BY  4, LOC.LogicalLocation, LOC.LOC, LA.Lot ' ELSE  ' ' END +      
      CASE WHEN @c_Mode = 'DISCRETE' THEN ' ORDER BY CASE WHEN (LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED - LOTxLOCxID.QtyReplen) >= ' + CAST(@n_QtyLeftToFulfill AS NVARCHAR) + ' THEN 1 ELSE 2 END, 4, LOC.LogicalLocation, LOC.LOC, LA.Lot ' ELSE  ' ' END    
