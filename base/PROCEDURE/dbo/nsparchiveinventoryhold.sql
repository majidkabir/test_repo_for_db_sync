...
GO
CREATE PROC [dbo].[nsparchiveinventoryHold]         
...
begin -- main
   ...
   SET CONCAT_NULL_YIELDS_NULL OFF
	...
	declare @n_retain_days int      , -- days to hold data
		...
		@n_archive_InvHold_records   int, -- # of Inventoryhold records to be archived
		@n_default_id int,
		@n_strlen int,
		@local_n_err         int,
	...
	begin -- 3
		...
		begin
			...
			select @local_c_errmsg =
				...
				" sqlsvr message = " + dbo.fnc_LTrim(dbo.fnc_RTrim(@local_c_errmsg)) + ")" + ' (nsparchiveinventoryhold)'
		end
		...
		select @c_temp = "archive of inventoryhold started with parms; datetype = " + dbo.fnc_RTrim(@c_datetype) +
			...
			' ; inventoryholdkey = '+dbo.fnc_RTrim(@c_invholdstart)+'-'+dbo.fnc_RTrim(@c_invholdend)+
			' ; copy rows to archive = '+dbo.fnc_RTrim(@copyrowstoarchivedatabase) +
			' ; retain days = '+ convert(char(6),@n_retain_days)
	
		execute nsplogalert
			@c_modulename   = "nsparchiveinventoryhold",
	...
	begin -- 4
		...
		select @c_temp = 'and inventoryhold.inventoryholdkey between '+ 'N'''+dbo.fnc_RTrim(@c_invholdstart) + ''''+ ' and '+
			'N'''+dbo.fnc_RTrim(@c_invholdend)+''''
		...
		begin 
			if (@b_debug =1 )
			begin
				print 'starting table existence check for inventoryhold...'
			end
			select @b_success = 1
		...
		begin -- 5
			...
			begin
				select @c_whereclause = "update inventoryhold set archivecop = '9' where ( inventoryhold.dateon <= " + '"'+ convert(char(10),@d_result,101)+'"' + " and hold = '0' ) "
            			execute (@c_whereclause+ @c_temp)
				select @local_n_err = @@error, @n_cnt = @@rowcount
				select @n_archive_invhold_records = @n_cnt
			end
	
			if @c_datetype = '2' -- dateoff
			begin
				select @c_whereclause = "update inventoryhold set archivecop = '9' where ( inventoryhold.dateoff <= " +'"'+ convert(char(10),@d_result,101)+'"' + " and hold = '0' ) "
				execute (@c_whereclause + @c_temp)
				select @local_n_err = @@error, @n_cnt = @@rowcount
				select @n_archive_invhold_records = @n_cnt
			end
			...
			begin 
				...
				select @local_c_errmsg = convert(char(5),@local_n_err)
				select @local_c_errmsg =
				': update of archivecop failed - inventoryhold (nsparchiveinventoryhold) ' + ' ( ' +
			...
			if ((@n_continue = 1 or @n_continue = 2)  and @copyrowstoarchivedatabase = 'y')
			begin
				select @c_temp = "attempting to archive " + dbo.fnc_RTrim(convert(char(6),@n_archive_invhold_records )) +
					" inventoryhold records."
				execute nsplogalert
					@c_modulename   = "nsparchiveinventoryhold",
					@c_alertmessage = @c_temp ,
					@n_severity     = 0,
			...
			begin   
				if (@b_debug =1 )
				begin
					print "building insert for inventoryhold..."
				end
				select @b_success = 1
			...
			begin  
				if (@b_debug =1 )
				begin
					print "delete for inventoryhold..."
				end
				if (@b_debug = 0)
				begin
					delete from inventoryhold
					where archivecop = '9'
					select @local_n_err = @@error, @n_cnt = @@rowcount
				end
				if (@b_debug = 1)
				begin
					select * from inventoryhold(nolock)
					where archivecop = '9'
				end
				if @local_n_err <> 0
				begin  
					...
					select @local_c_errmsg = convert(char(5),@local_n_err)
					select @local_c_errmsg =
					":  inventoryhold delete failed. (nsparchiveinventoryhold) " + " ( " +
					" sqlsvr message = " + dbo.fnc_LTrim(dbo.fnc_RTrim(@local_c_errmsg)) + ")"
				end      
			end    
	...
	begin
		...
		execute nsplogalert
			@c_modulename   = "nsparchiveinventoryhold",
			@c_alertmessage = "purging inventoryhold tables",
			@n_severity     = 0,
			@b_success       = @b_success output,
			@n_err          = @n_err output,
			@c_errmsg       = @c_errmsg output
	...
	begin
		...
		execute nsp_logerror @n_err, @c_errmsg, "nsparchiveinventoryhold"
