...
GO
/************************************************************************/
/* Store Procedure:  isp_CartonManifestLabel13                          */
...
/*                                  End Label#                          */                                         
/*                                                                      */
/* Called By:  dw = r_dw_carton_manifest_label_13                       */
/* PVCS Version: 1.0                                                    */
/*                                                                      */
/* Version: 5.4                                                         */
/*                                                                      */
...
/* 28-Oct-2013  YTWan    1.1  Fixed. (Wan01)                            */
/* 15-MAY-2018  CSCHONG  1.2  WMS-4907 - add new field (CS01)           */
/************************************************************************/
CREATE PROC [dbo].[isp_CartonManifestLabel13] (
...
BEGIN
   ...
   --IF @n_IsRDT = 1
   IF EXISTS (SELECT 1 FROM PACKDETAIL WITH (NOLOCK) WHERE PickSlipNo = @c_PickslipNo AND (LabelNo = @c_CartonNoStart OR LabelNo = @c_CartonNoEnd) )
   BEGIN
      SELECT PACKHEADER.PickSlipNo      
            ...
            ,PrintDate = GETDATE()
            ,Refno = CASE WHEN ISNULL(PACKDETAIL.refno,'') <> '' THEN 'Inbound UCC#:  ' + PACKDETAIL.refno ELSE '' END   --(CS01)
       ...
       GROUP BY PACKHEADER.PickSlipNo
               ...
               ,ISNULL(RTRIM(SKU.Measurement),'') 
                ,CASE WHEN ISNULL(PACKDETAIL.refno,'') <> '' THEN 'Inbound UCC#:  ' + PACKDETAIL.refno ELSE '' END  --(CS01)
