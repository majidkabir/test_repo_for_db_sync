...
GO
/************************************************************************/  
/* Stored Procedure: nsp_GetPickSlipOrders10                            */  
/* Creation Date:                                                       */  
/* Copyright: IDS                                                       */  
/* Written by:                                                          */  
/*                                                                      */  
...
/* 7-OCT-2014   CSCHONG       SOS 321764 (CS01)                         */  
/* 29-Apr-2015  CSCHONG       SOS339808  (CS02)                         */  
/* 08-JUL-2015  CSCHONG       SOS345970 & SOS346307  (CS03)             */  
/* 30-MAY-2016  CSCHONG       SOS#369708 Add report config (CS04)       */  
/* 06-APR-2017  CSCHONG       WMS-1425 - Add new report config (CS05)   */  
/* 28-Jan-2019  TLTING_ext 1.1 enlarge externorderkey field length      */  
/* 30-May-2019  WLCHOOI       WMS-9233 - Add Pickdetail.ID and new      */  
/*                                        reportcfg (WL01)              */  
...
BEGIN  
   ...
   @n_loccnt          int,  
   @c_ID              NVARCHAR(18) = NULL,     --(WL01)  
   @c_PAZone          NVARCHAR(10) = NULL,     --(WL02)  
   @c_RetailSKU       NVARCHAR(20) = NULL,     --(WL02)   
   @n_CurrentPG       INT = 1,                 --(WL03)  
   @c_LastPutawayzone NVARCHAR(10) = '',       --(WL03)  
   @c_LastLoadkey     NVARCHAR(10) = '',       --(WL03)  
   @c_LastOrderkey    NVARCHAR(10) = ''        --(WL03)  
   ...
   DECLARE @n_qtyorder         int,  
            @n_qtyallocated        int,  
            @c_skuindicator           NVARCHAR(1) ,                --(CS01)  
            @c_ShowSusr5              NVARCHAR(5),                 --(CS03)  
            @c_susr5                  NVARCHAR(18),                --(CS03)  
            @c_ShowFullLoc            NVARCHAR(5),                 --(CS04)       
            @c_showordtype            NVARCHAR(5),                 --(CS05)    
            @c_showcitystate          NVARCHAR(5),                 --(CS05)   
            @c_OHTypeDesc             NVARCHAR(50),                --(CS05)  
            @c_NewPostCode            NVARCHAR(120),               --(CS05)  
            @c_ShowPickdetailID       NVARCHAR(5),                 --(WL01)  
            @c_BreakByPAZone          NVARCHAR(5),                 --(WL02)  
            @c_ShowRetailSKU          NVARCHAR(5),                 --(WL02)  
            @c_SQL                    NVARCHAR(4000),              --(WL02)  
            @c_OrderBy                NVARCHAR(4000),              --(WL02)  
            @n_MaxLine                INT = 10,                    --(WL03)  
            @c_GetPutawayzone         NVARCHAR(10),                --(WL03)  
            @c_GetLoadkey             NVARCHAR(10),                --(WL03)  
            @c_GetOrderkey            NVARCHAR(10),                --(WL03)  
   ...
   --WL01 Start  
   --SELECT @c_ShowPickdetailID = ISNULL(CL.Short,'N')  
   --FROM CODELKUP CL (NOLOCK)   
   --WHERE CL.LISTNAME = 'REPORTCFG' AND CL.CODE = 'ShowPickdetailID'  
   --AND CL.LONG = 'r_dw_print_pickorder10' AND CL.STORERKEY IN (SELECT TOP 1 STORERKEY FROM ORDERS (NOLOCK) WHERE LOADKEY = @c_loadkey)  
   ...
   SELECT @c_ShowPickdetailID      = ISNULL(MAX(CASE WHEN Code = 'ShowPickdetailID' THEN 'Y' ELSE 'N' END),'N')      
         ...
         ,@c_ShowPageNoByOrderkey  = ISNULL(MAX(CASE WHEN Code = 'ShowPageNoByOrderkey' THEN 'Y' ELSE 'N' END),'N') --WL03  
   ...
   IF @c_ShowPageNoByOrderkey = NULL SET @c_ShowPageNoByOrderkey = ''  --WL03  
     
   --WL02 End  
   --WL01 End  
   --SET @c_ShowPageNoByOrderkey = 'Y'  
   ...
   pallet               int,  
   innerpack            int,  
   Skuindicator         NVARCHAR(1) NULL,                --(CS01)  
   LRoute               NVARCHAR(10) NULL,   --(CS02)  
   LEXTLoadKey          NVARCHAR(20) NULL,   --(CS02)  
   LPriority            NVARCHAR(10) NULL,   --(CS02)   
   LUDef01              NVARCHAR(20) NULL,    --(CS02)  
   SUSR5                NVARCHAR(18) NULL,    --(CS03)  
   ShowSUSR5            NVARCHAR(5)  NULL,     --(CS03)  
   ShowFullLoc          NVARCHAR(5) NULL,      --(CS04)  
   ShowOrdType          NVARCHAR(5) NULL,      --(CS05)  
   ShowCityState        NVARCHAR(5) NULL,      --(CS05)  
   OHTypeDesc           NVARCHAR(50) NULL,     --(CS05)  
   NewPostcode          NVARCHAR(120) NULL,    --(CS05)  
   ShowPickdetailID     NVARCHAR(5) NULL,      --(WL01)  
   ID                   NVARCHAR(18) NULL,     --(WL01)  
   Putawayzone          NVARCHAR(10) NULL,     --(WL02)  
   BreakByPAZone        NVARCHAR(10) NULL,     --(WL02)  
   ...
   Pickdetail.Lot, Loadplan.Route ,                       --(CS02)  
   Loadplan.Externloadkey,                --(CS02)   
   Loadplan.Priority,                      --(CS02)  
   --Loadplan.UserDefine01                  --(CS02)     --(CS03)  
   convert(nvarchar(10),Loadplan.LPuserdefDate01,103),    --(CS03)  
   CASE WHEN @c_ShowPickdetailID = 'Y' THEN Pickdetail.ID ELSE '' END,  --(WL01)  
   CASE WHEN @c_BreakByPAZone = 'Y' THEN LOC.PutawayZone ELSE '' END,   --(WL02)  
   CASE WHEN @c_ShowRetailSKU = 'Y' THEN SKU.RETAILSKU ELSE '' END      --(WL02)  
   ...
   AND Loadplan.loadkey=loadplandetail.loadkey                    --(CS02)  
   AND PickDetail.SKU = SKU.SKU AND PickDetail.Storerkey = SKU.Storerkey   --(WL02)  
   AND    LoadPlanDetail.LoadKey = @c_loadkey  
   GROUP BY PickDetail.sku,       PickDetail.loc,      PACK.Qty,  
   PickDetail.storerkey, PickDetail.OrderKey, PICKDETAIL.UOM,  
   LOC.LogicalLocation,  Pickdetail.Lot, Loadplan.Route ,                       --(CS02)  
   Loadplan.Externloadkey,                --(CS02)   
   Loadplan.Priority,                      --(CS02)   
   --Loadplan.UserDefine01                   --(CS02)  --CS03  
   convert(nvarchar(10),Loadplan.LPuserdefDate01,103),    --(CS03)  
   ...
   @c_orderkey, @c_UOM, @c_logicalloc, @c_lot,@c_LRoute,@c_LEXTLoadKey,@c_LPriority,@c_LUDef01,  --(CS01)  
   @c_ID,    --(WL01)  
   @c_PAZone, @c_RetailSKU --(WL02)  
   ...
   BEGIN  
      ...
      ELSE  
         BEGIN  
            ...
            @c_NewPostCode = (ISNULL(ORDERS.C_City,'') + ' ' + ISNULL(ORDERS.C_State,'') + ' ' +ISNULL(ORDERS.C_Zip,'')), --(CS05)  
            @c_OHTypeDesc = ISNULL(CODELKUP.short,'')                                                                     --(CS05)  
...
--         END  
      ...
      SET QtyOrder     = @n_qtyorder,  
       ...
       --   SKUIndicator = @c_skuindicator            --(CS01)  
