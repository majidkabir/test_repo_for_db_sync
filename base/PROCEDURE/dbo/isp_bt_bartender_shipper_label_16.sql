...
/* 2023-09-08 1.5  WLChooi    WMS-23596 - Mask C_Address (WL04)               */
/******************************************************************************/  
...
BEGIN  
  ...
  DECLARE @n_RowNo             INT,  
   ...
   BEGIN  
      ...
      --END  
      ----CS02 End  
      --ELSE IF UPPER(@c_Sparm7) = 'M'  
      --BEGIN  
      --   SELECT @c_col57 = PH.EstimateTotalCtn  
      ...
      --SELECT @c_col57 = substring(@c_Str2,CHARINDEX('-',@c_Str2)+1,1)        --CS01  
...
EXIT_SP:  
   ...
   /*INSERT INTO TraceInfo VALUES  
      ('isp_BT_Bartender_Shipper_Label_16', @d_Trace_StartTime, @d_Trace_EndTime  
