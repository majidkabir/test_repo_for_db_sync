...
GO
/************************************************************************/
/* Store Procedure:  isp_SCE_DL_GENERIC_ADJ_RULES_100010_10             */
...
/*         @c_InParm2 = 'Channel'                                       */
/*         @c_InParm3 = 'FinalizeAdjustment'  Storer ConfigKey          */
/*                                                                      */
/*                                                                      */
...
CREATE PROCEDURE [dbo].[isp_SCE_DL_GENERIC_ADJ_RULES_100010_10] (
   @b_Debug       INT            = 0
 , @n_BatchNo     INT            = 0
 , @n_Flag        INT            = 0
...
BEGIN
   ...
   BEGIN
      ...
      SELECT DISTINCT ISNULL(RTRIM(StorerKey), '')
      ...
      BEGIN
         ...
         IF EXISTS(SELECT 1 FROM dbo.V_StorerConfig WITH(NOLOCK)
         WHERE StorerKey = @c_StorerKey
         AND ConfigKey = @c_InParm3
         AND (ISNULL(RTRIM(SValue),'') = ''
         OR SValue <> '1'))
         BEGIN
            SET @c_ttlMsg += N'/storerconfig FinalizeAdjustment not allow setup to 0 ';
         END
         IF EXISTS (
         SELECT 1
         ...
         BEGIN
            ...
            AND   ISNULL(RTRIM(Channel), '')   = @c_Channel;
            ...
            BEGIN
               ...
               SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5), ISNULL(@n_ErrNo, 0))
                               + ': Update record fail. (isp_SCE_DL_GENERIC_ADJ_RULES_100010_10)';
               ROLLBACK;
   ...
   IF @b_Debug = 1
   BEGIN
      SELECT '<<SUB-SP-RULES>> - [isp_SCE_DL_GENERIC_ADJ_RULES_100010_10] EXIT... ErrMsg : ' + ISNULL(RTRIM(@c_ErrMsg), '');
   END;
