...
GO
/************************************************************************/
/* Store Procedure:  isp_GetPackList05                                  */
/* Creation Date: 09-Sep-2013                                           */
/* Copyright: IDS                                                       */
/* Written by:                                                          */
/*                                                                      */
/* Purpose:  288737-Convert - Pack Summary Report (TW)                  */
/*           (modified from isp_GetPackList03                           */
/*                                                                      */
/* Usage:  Used for report dw = r_dw_print_packlist_05                  */
/*                                                                      */
/* Called By: Exceed                                                    */
/*                                                                      */
/* PVCS Version: 1.5                                                    */
...
/* 02-Jan-2014  YTWan   1.2   SOS#299180 - Remove Sku descr & display   */
/*                            Styleonly (Wan01)                         */
/* 21-Jan-2014  Audrey  1.3   SOS#300324 - change to OD.SKU (ang01)     */
/* 19-Feb-2014  YTWan   1.4   SOS#300868 - [Converse] Taiwan Packing    */
/*                            List Modification. (Wan02)                */
/* 27-Feb-2014  YTWan   1.5   SOS#300868. (Wan03) if '10' then 'APP/ACC'*/
/* 11-Feb-2015  CSCHONG 1.6   SOS#332943 (CS01)                         */
/* 30-Nov-2015  YTWan   1.5   SOS#357940 -US Size(Wan04)                */
/* 24-Dec-2015  Leong   1.6   SOS# 359934 - Use DISTINCT for duplicate  */
/*                            sku per orders.                           */
/* 07-Sep-2016 CSCHONG  1.7   WMS-252 - Add new field (CS02)            */
/* 28-Jan-2019  TLTING_ext 1.8 enlarge externorderkey field length      */
/************************************************************************/

CREATE PROC [dbo].[isp_GetPackList05] (@c_LoadKey NVARCHAR(10)) 
AS
BEGIN
   SET NOCOUNT ON 
   SET ANSI_NULLS OFF 
   ...
   DECLARE @c_OrderKey        NVARCHAR(10),
         ...
         , @n_StyleWColor     INT         --(Wan01) 
         , @n_PrevStyleWColor INT         --(Wan01)

         , @c_Division        NVARCHAR(18)--(Wan02)
         , @c_Ordertype       NVARCHAR(20)--(Wan02)
         , @n_TotalCartons    INT         --(Wan02) 
         , @c_SizeSort        NVARCHAR(10)--(Wan02)
   ...
   SET @n_StyleWColor     = 0             --(Wan01) 
   SET @n_PrevStyleWColor = 0             --(Wan01) 

   SET @c_SizeSort        = ''            --(Wan02)
   
   CREATE TABLE #TempPickSlip
          (SeqNo              INT   IDENTITY(1,1), --(Wan02)
           ...
           Qty31              int            NULL,
           Qty32              int            NULL,
           Division           NVARCHAR(18)   NULL,       --(Wan02)
           OrderType          NVARCHAR(10)   NULL,       --(Wan02)
           TotalCartons       INT            NULL,        --(Wan02)
           OrdDetUserdef02    NVARCHAR(18)   NULL,         --(CS01)
           OrdDetUserdef09    NVARCHAR(18)   NULL         --(CS02)
         )
...
--(Wan02) - START
   ...
   BEGIN
      ...
      LEFT JOIN CODELKUP  RCFG WITH (NOLOCK) ON (RCFG.ListName = 'REPORTCFG')
                                             ...
                                             AND(RCFG.Long = 'r_dw_print_packlist_05')
      ...
      SELECT DISTINCT    
            ...
            ,CASE WHEN ISNULL(RTRIM(OD.Lottable01),'') LIKE '10%'
                  ...
                  ELSE 0 END                                                   
            --(Wan01) - END
            --,RIGHT(ISNULL(RTRIM(SKU.SKU),''),3)(ang01)
	    ,OD.SKU  --ang01
       --(Wan04) - START
       --(Wan03) - START
       --(Wan02) - START
       --, CASE WHEN (@c_Division LIKE '10%' OR @c_Division LIKE '30%') AND CL.Code IS NOT NULL 
      ...
      BEGIN
         ...
         SET @n_PrevStyleWColor = @n_StyleWColor      --(Wan01)                                                        
...
--                        END, 
                   ISNULL(RTRIM(SKU.Style),'') + @c_PrevColor AS StyleColour,
                   '', 
                   --(Wan01) - END                                                                      
                   ...
                   @c_SkuSize31,
                   @c_SkuSize32,
                 -- (Wan04) - Change SKU.Size to SKUSZ.Sku (START)
                 ...
                 CASE WHEN ISNULL(RTRIM(SKUSZ.Size),'') = @c_SkuSize32 AND ISNULL(RTRIM(SKUSZ.Size),'') <> ''
                     ...
                     ELSE 0
                     END
               -- (Wan04) - Change SKU.Size to SKUSZ.Sku (END)
               --(Wan02) - START
               , CASE WHEN @c_Division LIKE '10%' THEN 'APP/ACC'           --(Wan03)
                      WHEN @c_Division LIKE '20%' THEN 'FW'
                      --WHEN @c_Division LIKE '30%' THEN 'ACC'             --(Wan03)
                      END 
               ,  @c_OrderType   
               , @n_TotalCartons
               --(Wan02) - END  
               , OD.Userdefine02       --(CS01)   
               , OD.Userdefine09       --(CS02)        
            FROM ORDERDETAIL OD  WITH (NOLOCK) 
            ...
            GROUP BY ISNULL(RTRIM(Pickheader.Pickheaderkey),''),                    
                     ...
                     LOADPLAN.lpuserdefdate01,     
                     ORDERS.Storerkey,               
                     ISNULL(RTRIM(SKUSZ.Size),'')              -- (Wan04) - Change SKU.Size to SKUSZ.Sku  
                     ,OD.Userdefine02 
                     , OD.Userdefine09       --(CS02) 
   ...
   --(Wan02) - START Not need, this replace by division in Report
--   SELECT OD.Orderkey, MIN(ISNULL(OD.Userdefine04,'')) AS Userdefine04
--   INTO #TempOD
...
--   GROUP BY OD.Orderkey
   ...
   SELECT T.PickSlipNo, T.Loadkey, T.OrderKey, T.ExternOrderKey, T.Notes, 
          ...
          ISNULL(STORER.Consigneefor,'') Consigneefor--, #TempOD.Userdefine04, T.InvoiceNo   --(Wan02) 
         ,  T.Division, T.InvoiceNo, T.OrderType, T.TotalCartons,T.OrdDetUserdef02,T.OrdDetUserdef09           --(Wan02)  --(CS01)  --(CS02)                                       
   FROM #TempPickSlip T
   LEFT JOIN STORER (NOLOCK) ON T.Consigneekey = STORER.Storerkey AND STORER.Type = '2'   
   --LEFT JOIN #TempOD (NOLOCK) ON T.Orderkey = #TempOD.Orderkey              (Wan02)
   GROUP BY T.PickSlipNo, T.Loadkey, T.OrderKey, T.ExternOrderKey, T.Notes, 
            ...
            T.SkuSize17, T.SkuSize18, T.SkuSize19, T.SkuSize20, T.SkuSize21, T.SkuSize22, T.SkuSize23, T.SkuSize24, 
            T.SkuSize25, T.SkuSize26, T.SkuSize27, T.SkuSize28, T.SkuSize29, T.SkuSize30, T.SkuSize31, T.SkuSize32,
            ISNULL(STORER.Consigneefor,'')--, #TempOD.Userdefine04, T.InvoiceNo              --(Wan02)
         ,  T.Division, T.InvoiceNo, T.OrderType, T.TotalCartons,T.OrdDetUserdef02,T.OrdDetUserdef09           --(Wan02)   --(CS01)  --(CS02)                                                
   ORDER BY  MIN(T.SeqNo)

   DROP TABLE #TempOrder
   DROP TABLE #TempPickSlip
   ---DROP TABLE #TempOD                                                                     --(Wan02)
END
