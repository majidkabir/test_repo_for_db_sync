...
/**************************************************************************/
/* SP: ispPopulateTOASN_Pandora                                           */
/* Creation Date: 04-AUG-2023                                             */
/* Copyright: MAERSK                                                      */
/* Written by:               						                                  */
/*                                                                        */
/* Purpose: WMS-23304 CN Pandora Auto create ASN when MBOL Ship           */
/*                                                                        */
...
CREATE   PROCEDURE [dbo].[ispPopulateTOASN_Pandora] 
...
BEGIN
   ...
   BEGIN     	        
      SELECT TOP 1 
             ...
             @c_Userdefine07       = CAST(ORDERS.Userdefine07 AS NVARCHAR)
      ...
      */   
      ...
      IF NOT EXISTS ( SELECT 1 FROM LOC WITH (NOLOCK) WHERE LOC = @c_ToLoc) 
	       ...
	       SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Invalid To Loc : ' + RTRIM(ISNULL(@c_ToLoc,'')) +
                           ' (ispPopulateTOASN_Pandora)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
		   END
	    ...
	    BEGIN 				
	       ...
	       BEGIN
	       	 ...
	       	 BEGIN
	             ...
	             IF @b_success = 1
	             BEGIN            
	                INSERT INTO RECEIPT (ReceiptKey, ExternReceiptkey, Warehousereference, StorerKey, RecType, Facility, DocType, RoutingTool, CarrierAddress1, CarrierAddress2, CarrierName, CarrierReference, Userdefine01)
	                VALUES (@c_NewReceiptKey, @c_ExternReceiptKey, @c_WarehouseReference, @c_ToStorerKey, @c_Type, @c_ToFacility, @c_DocType, 'N', @c_Address1, @c_Address2, @c_Contact1, @c_Phone1, @c_Userdefine07)
                  ...
                  BEGIN
	                    ...
	                    SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err)
		    	             SET @n_err = 63410   
	    		             SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert Error on Table Receipt (ispPopulateTOASN_Pandora)' + ' ( ' + 
	             ...
	             BEGIN
	                ...
	                SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err) 
		    		       SET @n_err = 63420   
	    			       SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Generate Receipt Key Failed! (ispPopulateTOASN_Pandora)' 
	    			                 + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
	             END
	          END
	       END    
	       ...
	       BEGIN
	          ...
	          SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err)
		    	   SET @n_err = 63430   
	    		   SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Storer Key is BLANK! (ispPopulateTOASN_Pandora)' + ' ( ' + 
                           ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
	       END
	    ...
	    BEGIN         
	       ...
	       BEGIN
	          ...
	          SELECT @c_SKU        = SKU.SKU,   
	                 ...
	                 @c_Channel    = CASE WHEN ORDERDETAIL.Channel = 'B2B' THEN
	                                        'B2C'
	                                      WHEN ORDERDETAIL.Channel = 'B2C' THEN
	                                        'B2B'
	                                  ELSE '' END	                                  	                       
		            ...
		            BEGIN
		                 ...
		                 FETCH NEXT FROM PICK_CUR
		                    INTO @n_ShippedQty,  @c_Lottable01, @c_Lottable02, @c_Lottable03, @d_Lottable04, @d_Lottable05, 
		                         ...
		                         @c_Lottable12, @d_Lottable13, @d_Lottable14, @d_Lottable15, @c_ID
      ...
      BEGIN     	
         ...
         BEGIN
	           SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err)
		    	    SET @n_err = 63440   
	    		    SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Error on Table Receipt (ispPopulateTOASN_Pandora)' + ' ( ' + 
                        ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
         END
         ...
         BEGIN
 	       		SET @n_continue = 3
	      	    SET @c_errmsg='ASN Finalize Error (ispPopulateTOASN_Pandora): ' +  RTRIM(ISNULL(@c_errmsg,'')) 
         END
         */                                    
      END
	    ...
	    BEGIN
	       ...
	       --BEGIN
	          --ROLLBACK TRAN
	       --END
	       EXECUTE nsp_logerror @n_err, @c_errmsg, 'ispPopulateTOASN_Pandora'
