...
/*************************************************************************/
CREATE   PROCEDURE [dbo].[ispRVWAV63]
...
BEGIN
   ...
   BEGIN
      IF NOT EXISTS (SELECT 1 FROM TASKDETAIL TD (NOLOCK) 
                     WHERE TD.Wavekey = @c_Wavekey AND TD.SourceType = 'ispRLWAV63'
                     AND TD.TaskType IN ('FPP','FCP') 
                     )
      BEGIN                                          
         SELECT @n_continue = 3  
         SELECT @n_err = 81010  
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': This Wave has not been released. (ispRVWAV63)'         
      END                 
   END
   ...
   BEGIN
      IF EXISTS (SELECT 1 FROM TASKDETAIL TD (NOLOCK) 
                 ...
                 AND TD.Sourcetype = 'ispRLWAV63'
                 AND TD.TaskType IN ('FPP','FCP')
                 AND TD.Status <> '0')
      BEGIN
         SELECT @n_continue = 3  
         SELECT @n_err = 81020  
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Some Tasks have been started. Not allow to Reverse Wave Released (ispRVWAV63)'       
      END                 
   END
   ...
   BEGIN
      DECLARE CUR_TASKDET CURSOR LOCAL FAST_FORWARD READ_ONLY FOR  
         ...
         AND SourceType = 'ispRLWAV63'
         AND Status = '0'
         AND TaskType IN ('FPP','FCP')
      ...
      BEGIN    
         ...
         BEGIN
            ...
            SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Delete Taskdetail Table Failed. (ispRVWAV63)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
         END 
   ...
   BEGIN
      ...
      BEGIN    
         ...
         BEGIN
           ...
           SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update Pickdetail Table Failed. (ispRVWAV63)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
         END          
   ...
   BEGIN
      ...
      BEGIN
         ...
         SELECT @c_errmsg = 'NSQL' + CONVERT(NVARCHAR(5), @n_err) + ': Update on wave Failed (ispRVWAV63)' + ' ( '
   ...
   RETURN_SP:
   IF CURSOR_STATUS('LOCAL', 'CUR_TASKDET') IN (0 , 1)
   BEGIN
      CLOSE CUR_TASKDET
      DEALLOCATE CUR_TASKDET   
   END

   IF CURSOR_STATUS('LOCAL', 'CUR_PICKDET') IN (0 , 1)
   BEGIN
      CLOSE CUR_PICKDET
      DEALLOCATE CUR_PICKDET   
   END
   ...
   BEGIN
      ...
      BEGIN
         ...
         END
      END
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'ispRVWAV63'
