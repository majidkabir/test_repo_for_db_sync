...
/***************************************************************************/
/* Stored Procedure: ispPRREC24                                            */
/* Creation Date: 04-Mar-2022                                              */
/* Copyright: LFL                                                          */
/* Written by: WLChooi                                                     */
/*                                                                         */
/* Purpose: WMS-18963 - [CN]COSTCO_ASN_Finalization_Update_Lotattribute    */
/*        : Before finalize ASN                                            */
/*                                                                         */
...
BEGIN  
      ...
      BEGIN
         ...
         BEGIN
            IF NOT EXISTS (SELECT 1
                           ...
                           AND ST.[Type] = '2'
                           AND ST.ConsigneeFor = 'COSTCO2')
            BEGIN
               INSERT INTO STORER (StorerKey, Company, [Type], ConsigneeFor)
               SELECT @c_PODETLottable07, @c_SellerCompany, '2', 'COSTCO2'
               ...
               BEGIN  
                  ...
                  SET @c_errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_err)
                                + ': INSERT STORER Table Failed for ' + @c_PODETLottable07 + '. (ispPRREC24)' 
   ...
   BEGIN          
      ...
      BEGIN
         ...
         BEGIN
               ...
               BEGIN
                  ...
                  BEGIN
                     ...
                     BEGIN  
                        ...
                        SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err)  
                        SET @n_err = 65530   
                        SET @c_errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update NCOUNTER Table Failed. (ispPRREC24)' 
                                      + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '  
                        GOTO QUIT_SP
                     END
                  END
            ...
            IF @c_SeqNo <> 'ERR'
            BEGIN
               SET @c_FinalSeqNo = '3' + RIGHT(REPLICATE('0', 7) + @c_SeqNo, 7)
            END
            ELSE
            BEGIN
               ...
               SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err)  
               SET @n_err = 65535  
               SET @c_errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_err)+': The Series Number has reached 3ZZZZZZZ. (ispPRREC24)' 
                             + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '  
               GOTO QUIT_SP
            END
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN  
            ...
            SET @c_errmsg = CONVERT(NVARCHAR(250),@n_err)  
            SET @n_err = 65540  
            SET @c_errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update RECEIPTDETAIL Table Failed. (ispPRREC24)' 
                          + ' ( ' + ' SQLSvr MESSAGE=' + ISNULL(RTRIM(@c_errmsg),'') + ' ) '  
            GOTO QUIT_SP
         END
   ...
   BEGIN
      ...
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'ispPRREC24'
