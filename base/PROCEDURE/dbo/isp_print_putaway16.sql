...
GO
/************************************************************************/
/* Stored Procedure: isp_Print_Putaway16    						            */
/* Creation Date: 19-Jan-2021                                           */
/* Copyright: LFL                                                       */
/* Written by: WLChooi                                                  */
/*                                                                      */
/* Purpose: WMS-16048 - [MY] - Specialized Bicycle - Putaway Advice - CR*/
/*                                                                      */
/* Called By: d_dw_print_putaway16                                      */
/*                                                                      */
/* GitLab Version: 1.0                                                  */
/*                                                                      */
/* Version: 5.4                                                         */
...
/* 12/12/2022   MINGLE  1.0   WMS-21245 add reportcfg for MY(ML01)      */
/************************************************************************/

CREATE PROC [dbo].[isp_Print_Putaway16] (
...
BEGIN
   ...
   SELECT RECEIPT.ReceiptKey,   
          ...
          --(suser_sname()) user_name, 
			 CASE WHEN ISNULL(CL.SHORT,'N') = 'Y' AND ISNULL(@c_userid,'') <> '' THEN @c_userid ELSE (SUSER_SNAME()) END AS Username,	--ML01
   ...
   JOIN FACILITY (NOLOCK) ON ( RECEIPT.Facility = FACILITY.Facility ) 
	LEFT JOIN CODELKUP CL WITH (NOLOCK) ON CL.ListName = 'REPORTCFG' AND CL.Long = 'd_dw_print_putaway16'
