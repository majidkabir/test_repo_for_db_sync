...
/*                                                                      */
/* Updates:                                                             */
/* Date        Ver   Who      Purpose                                   */
/* 21-07-2017  1.1   Ung      WMS-2232 RDT compatible                   */
/************************************************************************/

CREATE PROCEDURE [dbo].[ispGenLot3Lot4Lot6ByJulianDate]
...
--            SET @b_Success = 0
            SET @n_Err = 61329
            SET @c_Errmsg = CONVERT ( NVARCHAR(5), @n_Err) + ' DaysInYear Not Numeric. (ispGenLot3Lot4Lot6ByJulianDate)'            
            GOTO QUIT
         END
         SET @i = @i + 1
      END 
...
--         SET @b_Success = 0
         SET @n_Err = 61330
         SET @c_Errmsg = CONVERT ( NVARCHAR(5), @n_Err) + ' DaysInYear Less Than or Equal to Zero.  (ispGenLot3Lot4Lot6ByJulianDate)'  
         GOTO QUIT
      END
...
--         SET @b_Success = 0
         SET @n_Err = 61331
         SET @c_Errmsg = CONVERT ( NVARCHAR(5), @n_Err) + ' DaysInYear Greater Than 366. (ispGenLot3Lot4Lot6ByJulianDate)'
         GOTO QUIT
      END
...
--         SET @b_Success = 0
         SET @n_Err = 61332         
         SET @c_Errmsg = CONVERT ( NVARCHAR(5), @n_Err) + ' DaysInYear Greater Than 365. (ispGenLot3Lot4Lot6ByJulianDate)'
         GOTO QUIT
      END
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            SET @n_Err = 61333
            SET @c_Errmsg = CONVERT ( NVARCHAR(5), @n_Err) + ' Invalid Lottable04Label Setup.  (ispGenLot3Lot4Lot6ByJulianDate)'
         END
         GOTO QUIT
      END
   END
   ...
   IF ISNULL(@dt_Lottable04,'') <> ''
   BEGIN
   	IF @b_debug = 1
      BEGIN
         SELECT '@dt_Lottable03', @dt_Lottable03
         SELECT '@dt_Lottable04', @dt_Lottable04
         SELECT '@c_susr2',CAST(@c_susr2 as int)
      END    
   	
      SELECT @c_Lottable06 = convert(nvarchar(10),(@dt_Lottable04 - CAST(@c_susr2 as int)) ,111)
   END

      IF @b_debug = 1
      BEGIN
         
         SELECT '@c_Lottable06', @c_Lottable06
      END         
   END
