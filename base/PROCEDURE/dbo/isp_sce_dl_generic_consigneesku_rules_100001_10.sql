...
/************************************************************************/
/* Store Procedure:  isp_SCE_DL_GENERIC_CONSIGNEESKU_RULES_100001_10    */
...
/************************************************************************/
CREATE   PROCEDURE [dbo].[isp_SCE_DL_GENERIC_CONSIGNEESKU_RULES_100001_10] (
   @b_Debug       INT            = 0
 , @n_BatchNo     INT            = 0
 , @n_Flag        INT            = 0
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         SET @c_ttlMsg += N'/Error: Consigneekey is NULL.'
      END
      ELSE
      BEGIN
         IF NOT EXISTS ( SELECT 1
                         ...
                         AND [Type] = '2' )
         BEGIN
            SET @c_ttlMsg += N'/Error: Consignee: ' + @c_ConsigneeKey + ' Not Exists.'
         END

         IF ISNULL(@c_ConsigneeSKU,'') = ''
         BEGIN
            SET @c_ttlMsg += N'/Error: ConsigneeSKU is NULL.'
         END
         ...
         IF ISNULL(@c_SKU,'') = ''
         BEGIN
            SET @c_ttlMsg += N'/Error: SKU is NULL.'
         END
         ELSE
         BEGIN
            IF NOT EXISTS ( SELECT 1
                            ...
                            AND SKU = @c_SKU )
            BEGIN
               SET @c_ttlMsg += N'/Error: SKU: ' + @c_SKU 
                             +  N' Not Exists for Storerkey: ' + @c_Storerkey + '.'
            END
         END
      END
   ...
   IF @b_Debug = 1
   BEGIN
      SELECT '<<SUB-SP-RULES>> - [isp_SCE_DL_GENERIC_CONSIGNEESKU_RULES_100001_10] EXIT... ErrMsg : ' + ISNULL(TRIM(@c_ErrMsg), '')
   END
