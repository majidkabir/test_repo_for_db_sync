...
/******************************************************************************/
/* Store procedure: ispCCDetailUpd_SG02                                       */
/* Copyright      : IDS                                                       */
/*                                                                            */
...
/* 10-03-2021  1.0  YeeKung     WMS-16273  Created                            */
/******************************************************************************/

CREATE PROCEDURE [dbo].[ispCCDetailUpd_SG02]
   @c_SKU              NVARCHAR(20),
...
BEGIN
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         SET Qty      = CASE WHEN @c_CountNo = '1' THEN Qty  + @n_Qty ELSE Qty  END
         ...
         BEGIN  
             SET @n_ErrNo = 164551
             SET @c_ErrMsg = rdt.rdtgetmessage(@n_ErrNo, @c_LangCode, 'DSP')  --UPD CCDtl Fail
             GOTO RollBackTran  
         END  
      ...
      BEGIN  
          ...
          UPDATE dbo.CCDEtail WITH (ROWLOCK)
          SET Qty      = CASE WHEN @c_CountNo = '1' THEN Qty  + @nQTY_PD ELSE Qty  END
            , Qty_Cnt2 = CASE WHEN @c_CountNo = '2' THEN Qty_Cnt2  + @nQTY_PD ELSE Qty_Cnt2 END
            , Qty_Cnt3 = CASE WHEN @c_CountNo = '3' THEN Qty_Cnt3  + @nQTY_PD ELSE Qty_Cnt3 END
            , Status  = '2'
            , Counted_Cnt1 = CASE WHEN @c_CountNo = '1' THEN '1' ELSE Counted_Cnt1 END
            , Counted_Cnt2 = CASE WHEN @c_CountNo = '2' THEN '1' ELSE Counted_Cnt2 END
            , Counted_Cnt3 = CASE WHEN @c_CountNo = '3' THEN '1' ELSE Counted_Cnt3 END
   ...
   VALUES ( 'RDTCC', GetDATE(), @n_Qty) 
   
   IF @n_Qty > 0 AND @c_CCDetailKey <> '' -- Update Remaining Qty to last of the CCDetail line with Same Loc and SKU
   BEGIN
         UPDATE dbo.CCDEtail WITH (ROWLOCK)
         SET Qty      = CASE WHEN @c_CountNo = '1' THEN Qty  + @n_Qty ELSE Qty  END
         , Qty_Cnt2 = CASE WHEN @c_CountNo = '2' THEN Qty_Cnt2 + @n_Qty ELSE Qty_Cnt2 END
   ...
   BEGIN  
      ...
      BEGIN  
         INSERT INTO CCDETAIL (
            ...
            Lottable01, Lottable02, Lottable03, Lottable04, Lottable05, Lottable07, 
            FinalizeFlag, Status, Counted_Cnt1)  
         VALUES (
            @c_CCKey, @c_CCDetailKey, @c_CCSheetNo, '', @c_StorerKey, @c_SKU, '', @c_LOC, '', 0, @c_Qty, 
            @c_Lottable01Value, @c_Lottable02Value, @c_Lottable03Value, NULL, @dt_Lottable05Value, @c_ID, 
            'N', '4', '1')  
         IF @@ERROR <> 0      
         BEGIN      
            SET @n_ErrNo = 164558     
            SET @c_ErrMsg = rdt.rdtgetmessage(@n_ErrNo, @c_LangCode, 'DSP')  --INS CCDtl Fail    
            GOTO RollBackTran      
         END     
      END             
      ...
      BEGIN     
         ...
         VALUES (@c_CCKey, @c_CCDetailKey, @c_CCSheetNo, '', @c_StorerKey, @c_SKU, '', @c_LOC, '', 0, @c_Qty, 
            @c_Lottable01Value, @c_Lottable02Value, @c_Lottable03Value, NULL, @dt_Lottable05Value, @c_ID, 
            'N', '4', '1')  
         IF @@ERROR <> 0      
         BEGIN      
