...
GO
CREATE PROC [dbo].[nsparchiverfdblog]      
...
begin -- main
   ...
   SET CONCAT_NULL_YIELDS_NULL OFF
	...
	declare @n_retain_days int      , -- days to hold data
		...
		@c_datetype NVARCHAR(10),      -- 1=adddate
		@n_archive_rfdblog_records   int, -- # of ID records to be archived
		@n_default_id int,
		@n_strlen int,
		@local_n_err         int,
	...
	begin -- 3
		select  @c_copyfrom_db = livedatabasename,
			...
			@n_retain_days = rfdblognumberofdaystoretain,
			@C_rfdblogactive = rfdblogactive,
			@c_datetype = rfdblogdatetype,
		...
		begin
			...
			select @local_c_errmsg =
				...
				" sqlsvr message = " + dbo.fnc_LTrim(dbo.fnc_RTrim(@local_c_errmsg)) + ")" + ' (nsparchiverfdblog)'
		end
		...
		select @b_success = 1
		select @c_temp = 'archive of rfdblog started with parms; active = '+ dbo.fnc_RTrim(@c_rfdblogactive)+
			' ; copy rows to archive = '+dbo.fnc_RTrim(@copyrowstoarchivedatabase)+
			' ; retain days = '+ convert(char(6),@n_retain_days)
	
		execute nsplogalert
			@c_modulename   = "nsparchiverfdblog",
	...
	begin -- 4
		...
		begin 
			if (@b_debug =1 )
			begin
				print 'starting table existence check for rfdb_log...'
			end
			select @b_success = 1
		...
		begin
			if (@b_debug =1 )
			begin
				print 'building alter table string for rfdb_log...'
			end
		...
		begin -- 5
			...
			begin		
				select @n_archive_rfdblog_records = count(1)
        	                from rfdb_log 
			...
			if ((@n_continue = 1 or @n_continue = 2)  and @copyrowstoarchivedatabase = 'y')
			begin
				select @c_temp = "attempting to archive " + dbo.fnc_RTrim(convert(char(6),@n_archive_rfdblog_records )) +
					" rfdb_log records."
				execute nsplogalert
					@c_modulename   = "nsparchiverfdblog",
					@c_alertmessage = @c_temp ,
			...
			begin   
				if (@b_debug =1 )
				begin
					print "building insert for rfdblog..."
				end
				select @b_success = 1
			...
			begin  
				if (@b_debug =1 )
				begin
					print "delete for rfdb_log..."
				end
				if (@b_debug = 0)
				begin
					select @c_executestatement = 'delete from rfdb_log '
				...
				begin  
					...
					select @local_c_errmsg = convert(char(5),@local_n_err)
					select @local_c_errmsg =
					":  dynamic delete rfdb_log failed. (nsparchiverfdblog) " + " ( " +
	...
	begin
		...
		execute nsplogalert
			@c_modulename   = "nsparchiverfdblog",
			@c_alertmessage = "archive of rfdb_log ended normally.",
			@n_severity     = 0,
			@b_success       = @b_success output,
			@n_err          = @n_err output,
			@c_errmsg       = @c_errmsg output
	...
	begin
		...
		begin
			...
			execute nsplogalert
				@c_modulename   = "nsparchiverfdblog",
				@c_alertmessage = "archive of rfdb_log ended abnormally - check this log for additional messages.",
				@n_severity     = 0,
				@b_success       = @b_success output ,
				@n_err          = @n_err output,
				@c_errmsg       = @c_errmsg output
	...
	begin
		...
		execute nsplogalert
			@c_modulename   = "nsparchiverfdblog",
			@c_alertmessage = "purging rfdb_log tables",
			@n_severity     = 0,
			@b_success       = @b_success output,
			@n_err          = @n_err output,
			@c_errmsg       = @c_errmsg output
	...
	begin
		...
		execute nsp_logerror @n_err, @c_errmsg, "nsparchiverfdblog"
