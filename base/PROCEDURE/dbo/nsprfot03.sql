...
BEGIN
   ...
   BEGIN
      IF (SELECT COUNT(Distinct SKU) FROM PICKDETAIL WHERE CaseId = @c_caseid) > 1
      BEGIN
         SELECT @n_continue = 3 , @n_err = 87601
         SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Commodity is not unique for CaseId# " + dbo.fnc_RTrim(@c_caseid) + ". (ntrLotxIDDetailAdd)"
      END
   END
   IF (@n_continue=1 OR @n_continue=2) AND @c_catchweight = "1"
   BEGIN
      ...
      BEGIN
         ...
         END
         SELECT @c_errmsg = "Pick Ok.  Please press ENTER and fill out the Serial#/Weight Information.."
      END
   ELSE
      BEGIN
         SELECT @c_retrec = "02"
...
ELSE
   BEGIN
      ...
      BEGIN
         SELECT @c_retrec="01"
         SELECT @c_errmsg = "Please, provide catch weight information"
      END
   END
   /* #INCLUDE <SPRFOT03_2.SQL> */
   ...
   ELSE
      BEGIN
         ...
         END
      END
      execute nsp_logerror @n_err, @c_errmsg, "nspRFOT03"
