...
/* 24-Apr-2019  WLCHOOI       WMS-8411-Minus LOTxLOCxID.QTYREPLEN (WL01)*/
/************************************************************************/

CREATE PROCEDURE [dbo].[nspPR_CH01]  
...
SELECT @c_UOMBase = @n_uombase
   ...
   BEGIN       
      ...
      SELECT LOT.STORERKEY,LOT.SKU,LOT.LOT ,  
             ...
             QTYAVAILABLE =  SUM(LOTXLOCXID.QTY) - SUM(LOTXLOCXID.QTYALLOCATED) - SUM(LOTXLOCXID.QTYPICKED) - SUM(LOTXLOCXID.QTYREPLEN) - MIN(LOT.QtyPreallocated) --WL01
      ...
      GROUP BY LOT.STORERKEY, LOT.SKU, LOT.LOT, LOTATTRIBUTE.Lottable04, LOTATTRIBUTE.LOTTABLE05, LOTATTRIBUTE.Lottable02 --WL01
...
--select 'abc' = @c_limitstring1
	    ...
	    ' AND SKUxLOC.LocationType NOT IN ("PICK", "CASE") ' + 
            ...
            ' HAVING (SUM(LOTXLOCXID.QTY) - SUM(LOTXLOCXID.QtyAllocated) - SUM(LOTXLOCXID.QTYPicked) - SUM(LOTXLOCXID.QTYREPLEN) - MIN(LOT.QtyPreAllocated) ) > 0 ' + --WL01
            ' ORDER BY lotATTRIBUTE.LOTTABLE04, LOTATTRIBUTE.LOTTABLE05 ')  
...
END
	...
	BEGIN
        ...
        END
-- 			SELECT 'here'
-- 			SELECT ('DECLARE PREALLOCATE_CURSOR_CANDIDATES CURSOR FAST_FORWARD READ_ONLY FOR  ' +  
--                  'SELECT MIN(LOTXLOCXID.STORERKEY) , MIN(LOTXLOCXID.SKU), LOT.LOT,' )  
--          SELECT ( 'QTYAVAILABLE = (SUM(LOTXLOCXID.QTY) - SUM(LOTXLOCXID.QTYALLOCATED) - SUM(LOTXLOCXID.QTYPICKED) - MIN (LOT.QtyPreAllocated) ) ' )  
--          SELECT ( 'FROM LOT (NOLOCK) , LOTATTRIBUTE (NOLOCK), LOTXLOCXID (NOLOCK), LOC (NOLOCK), ID (NOLOCK)' )  
--          SELECT ( 'WHERE LOTXLOCXID.STORERKEY = "' + @c_storerkey + '"' 
--                   'AND LOTXLOCXID.SKU = "' + @c_sku + "' " +  
--                   " AND LOT.STATUS = 'OK' AND LOC.STATUS = 'OK' AND ID.STATUS = 'OK'  And LOC.LocationFlag = 'NONE' " )
--          SELECT ( " AND lot.lot = lotattribute.lot AND LOTXLOCXID.LOT = LOT.LOT AND LOTXLOCXID.ID = ID.ID AND LOTXLOCXID.LOT = LOTATTRIBUTE.LOT AND LOTXLOCXID.LOC = LOC.LOC " 
-- 			       + " AND LOC.FACILITY = '" + @c_facility + "'"  + @c_LimitString1 + " " )
--          SELECT ( " GROUP BY LOT.LOT , LOTATTRIBUTE.Lottable04, LOTATTRIBUTE.Lottable02, LOTATTRIBUTE.LOTTABLE05  " + 
-- 						" HAVING (SUM(LOTXLOCXID.QTY) - SUM(LOTXLOCXID.QtyAllocated) - SUM(LOTXLOCXID.QTYPicked)  - MIN (LOT.QtyPreAllocated) ) > 0 " )
-- 			SELECT ( " ORDER BY lotATTRIBUTE.LOTTABLE04, LOTATTRIBUTE.LOTTABLE02, LOTATTRIBUTE.LOTTABLE05 ")   
    --        SELECT 'AND LOC.FACILITY = "' + @c_facility + '"' + @c_LimitString1 + '"'
	--END
    		END  
  	END  
END
