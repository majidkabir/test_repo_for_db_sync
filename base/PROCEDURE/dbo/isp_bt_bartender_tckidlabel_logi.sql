...
/******************************************************************************/                     
/* Copyright: IDS                                                             */                     
/* Purpose: isp_BT_Bartender_TCKIDLABEL_LOGI                                  */                     
/*                                                                            */                     
/* Modifications log:                                                         */                     
/*                                                                            */                     
...
/******************************************************************************/   
CREATE   PROC [dbo].[isp_BT_Bartender_TCKIDLABEL_LOGI]                 
...
BEGIN                
   ...
   SELECT TD.ParentTrackingID ,  
   TD.SKU ,  
   TD.TrackingID,  
   (ROW_NUMBER() OVER (PARTITION BY TD.ParentTrackingID, TD.SKU  
                      ORDER BY TD.TrackingID) -1) / @n_MaxLine AS Pageno  
   FROM   TrackingID TD (NOLOCK)  
   WHERE  TD.ParentTrackingID = @c_Sparm01  
          AND TD.sku = @c_Sparm02  
  
   SELECT @n_totalpg=(MAX(Pageno)) FROM #TEMPSERIALNo (NOLOCK)  
