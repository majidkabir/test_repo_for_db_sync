...
GO
/************************************************************************/  
/* Stored Procedure: nsp_GetPickSlipALL                                 */  
/* Creation Date: 05-Aug-2002                                           */  
/* Copyright: IDS                                                       */  
/* Written by:                                                          */  
/*                                                                      */  
/* Purpose: Print BatchPick pickslip RCM ReportType = 'PLISTBATCH'      */  
/*                                                                      */  
/* Called By: Used by dw: d_dw_print_combined_pickslip                  */  
/*                                                                      */  
/* PVCS Version: 1.5                                                    */  
/*                                                                      */  
/* Version: 5.4                                                         */  
...
/* 31-Dec-2002  Ricky      To include record where the Pickmethod = ' ' */  
/* 12-May-2004  June       SOS20748 -CheckIn Changes from ECheng (IDSHK)*/  
/* 15-Oct-2004  Mohit      Change cursor type                           */  
/* 24-Nov-2005  MaryVong   SOS43327 Including PickMethod = ' ' (manual  */  
/*                         picked records) while reprint pickslip with  */  
/*                         system configkey 'RF_BATCH_PICK' turned on   */  
/* 30-Aug-2006  June       SOS57249 - HK Loreal, include SKU.ALTSKU     */  
/* 13-Jul-2007  ONG01      SOS81078 - Add ExternLoadkey                 */  
/* 09-Mar-2010  SPChin     SOS164395 - Bug fixed for fields length      */ 
/* 04-May-2016  CSCHONG    SOS369497 - FrieslandHK- add new field (CS01)*/ 
/* 22-Jun-2017  TLTING     Performance tune - rtrim                     */
/* 28-Jan-2019  TLTING_ext 1.1 enlarge externorderkey field length      */ 
/************************************************************************/  
  
CREATE PROC [dbo].[nsp_GetPickSlipALL] (@c_loadkey NVARCHAR(10))  
AS  
BEGIN  
...
-- 8-APR-2004 (SOS20748) Add Unilever Hong Kong Pick Confirm interface  
   ...
   --@c_Lottable02          NVARCHAR(10),    --SOS164395  
   @c_Lottable02            NVARCHAR(18),    --SOS164395 
   ...
   @c_AltSKU                NVARCHAR(20),   -- SOS57249  
   @c_getStorerkey          NVARCHAR(15),   --CS01
   @c_skususr3              NVARCHAR(30),   --CS01
   ...
   CREATE TABLE #temp_pick  
    ( PickSlipNo       NVARCHAR(10) NULL,  
      ...
      --TrfRoom          NVARCHAR(5)  NULL,  -- wave.TrfRoom  --SOS164395 
      TrfRoom          NVARCHAR(10)  NULL,   -- wave.TrfRoom  --SOS164395 
      ...
      --Lottable02       NVARCHAR(10) NULL,  --SOS164395 
      Lottable02       NVARCHAR(18) NULL,    --SOS164395 
   ...
   SELECT TOP 1 @c_getStorerkey= ISNULL(RTRIM(Storerkey),'')   
   ...
   WHERE CLR.Storerkey = @c_getStorerkey
   AND CLR.Code = N'SHOWFIELD'                                        
   AND CLR.Listname = N'REPORTCFG' 
   AND CLR.Long = 'd_dw_print_combined_pickslip' AND ISNULL(CLR.Short,'') <> 'N'
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         SELECT PickDetail.sku,       PickDetail.loc,  
                ...
                Pickdetail.Lot  
         FROM   PickDetail  WITH (NOLOCK),  LoadPlandetail WITH (NOLOCK),  
                PACK WITH (NOLOCK),  LOC  WITH (NOLOCK),  
                ORDERS WITH (NOLOCK)   
      ...
      BEGIN -- @c_pickslipexists = '0' -- we need to retrieve infromation where pickslipno is NULL  
         ...
         WHERE  PickDetail.OrderKey = LoadPlandetail.OrderKey  
           ...
           AND  ISNULL(RTRIM(PickDetail.PICKSLIPNO),'') = '' --only selects those without task  
         GROUP BY PickDetail.sku,       PickDetail.loc,      PACK.Qty,  
                  PickDetail.storerkey, PICKDETAIL.UOM ,--   PickDetail.OrderKey,  
                  LOC.LogicalLocation,  Pickdetail.Lot  
         ORDER BY PICKDETAIL.SKU, PIckdetail.Lot, Pickdetail.Loc  
      ...
      BEGIN  -- IF NOT EXISTS  
         ...
         BEGIN  
            ...
            BEGIN   -- we have RF batch pick.  
               ...
               SET PICKSLIPNO = @c_pickheaderkey, Trafficcop = null  , EDITDATE =getdate()
               FROM PICKDETAIL P1 , LOADPLANDETAIL L1 WITH (NOLOCK), ORDERS O1 WITH (NOLOCK)  
...
--           @c_VehicleNo = IsNULL(LOADPLAN.TruckSize, ''),  
      ...
      FROM   IDS_LP_VEHICLE WITH (NOLOCK)   
      WHERE  Loadkey = @c_loadkey  
      ...
      IF @c_Lottable06    IS NULL SELECT @c_Lottable06 = ''               --(CS01)
--       IF @c_superorderflag = 'Y' SELECT @c_orderkey = ''  
      ...
      FROM   PACK  WITH (NOLOCK), SKU  WITH (NOLOCK)   
      WHERE  SKU.SKU = @c_SKU  
      ...
      INSERT INTO #Temp_Pick  
           (PickSlipNo,          loadkey,          Route,  
            ...
            LogicalLocation,     InnerPack,   
            AltSKU,showfield,SUSR3,lottable06 )  -- SOS57249   --CS01
      VALUES  
           (@c_pickheaderkey,   @c_loadkey,       @c_Route,  
            ...
            @c_logicalloc,      @n_InnerPack,  
            @c_AltSKU,@c_showfield,@c_skususr3,@c_Lottable06 ) -- SOS57249   --CS01
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         IF EXISTS(SELECT 1 FROM STORERCONFIG WITH (NOLOCK)  
                           WHERE Storerkey = @c_StorerKey AND Configkey = N'ULVITF' AND SVALUE = '1')  
            AND EXISTS(SELECT 1 FROM STORERCONFIG WITH (NOLOCK)   
                           WHERE Storerkey = @c_StorerKey AND Configkey = N'ULVITF_PCF_WHEN_GEN_PICKSLIP' AND SVALUE = '1')    -- New Config Key  
         BEGIN  
                  ...
                  BEGIN  
                     ...
                     BEGIN  
                        ...
                        BEGIN  
                           ...
                           Select @c_errmsg= 'NSQL'+CONVERT(char(5), @n_err)+':Insert failed on TransmitLog2. (nsp_GetPickSlipWave) ( SQLSvr MESSAGE='+LTRIM(RTRIM(@c_errmsg))+')'  
                        END  
                     END  
                  END  
               END  
   ...
   SELECT #TEMP_PICK.*  
     ,LOADPLAN.Facility  
     ,ISNULL(LOADPLAN.Delivery_zone, '') Delivery_zone  
   ,ISNULL(LOADPLAN.ExternLoadkey, '') ExternLoadkey   
   FROM #TEMP_PICK , LOADPLAN WITH (NOLOCK)   
