...
/************************************************************************/  
/* Stored Procedure: nspPR_KR02                                         */  
/* Creation Date: 07-APR-2021                                           */  
/* Copyright: LFL                                                       */  
/* Written by: WMS-16768/16769 Korea HM&COS allocation location priority*/
/*             by putawayzone.zonecategory. (copy from nspPR_CH02)      */                                                               
/*                                                                      */  
/* Purpose:                                                             */  
/*                                                                      */  
/* Called By:                                                           */  
...
CREATE PROCEDURE [dbo].[nspPR_KR02]  
...
BEGIN  
 SET NOCOUNT ON         
   ...
   IF @b_debug = 1  
   BEGIN  
      SELECT "nspPR_KR02 : Before Lot Lookup ....."  
   ...
   IF @b_debug = 1  
   BEGIN  
      SELECT "nspPR_KR02 : After Lot Lookup ....."  
      SELECT '@c_lot'=@c_lot,'@c_lottable01'=@c_lottable01, '@c_lottable02'=@c_lottable02, '@c_lottable03'=@c_lottable03  
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         SELECT @c_SQL = N'DECLARE PREALLOCATE_CURSOR_CANDIDATES CURSOR FAST_FORWARD READ_ONLY FOR  ' +  
               ...
               ' JOIN SKU (NOLOCK) ON SKU.Storerkey = LOTXLOCXID.Storerkey AND SKU.SKU = LOTXLOCxID.SKU ' +
               ...
               ' LEFT JOIN PUTAWAYZONE (NOLOCK) ON LOC.Putawayzone = PUTAWAYZONE.Putawayzone ' +                               
               ...
               ' GROUP BY LOT.LOT, LOTATTRIBUTE.Lottable04, LOTATTRIBUTE.LOTTABLE05 , LOTATTRIBUTE.Lottable02, PUTAWAYZONE.ZoneCategory ' +  
               ...
               -- Changed by June 19.Mar.2004 SOS21238  
               -- Richard request to sort by lot02 first for non-Lot specified  
               ' ORDER BY PUTAWAYZONE.ZoneCategory, LOTATTRIBUTE.Lottable02, lotATTRIBUTE.LOTTABLE04, LOTATTRIBUTE.LOTTABLE05, (SUM(LOTXLOCXID.QTY) - SUM(LOTXLOCXID.QTYALLOCATED) - SUM(LOTXLOCXID.QTYPICKED) - MIN(LOT.QtyPreallocated) ) DESC '  
