...
GO
/************************************************************************/  
/* Store Procedure:  isp_Transfer_Ticket_08                             */  
/* Creation Date:                                                       */  
/* Copyright: IDS                                                       */  
/* Written by: MINGLE                                                   */  
/*                                                                      */  
...
/* Return Status:  None                                                 */  
/*                                                                      */  
/* Usage:  Used for report dw = r_dw_transfer_ticket_08                 */  
/*                                                                      */  
/* Local Variables:                                                     */  
/*                                                                      */  
/* Called By:                                                           */  
...
/* 09-AUG-2022  MINGLE  1.0   DevOps Combine Script(WMS-20256)          */    
/************************************************************************/  
CREATE PROC [dbo].[isp_Transfer_Ticket_08] (@c_TransferKey NVARCHAR(10))  
 AS  
BEGIN  
    SET NOCOUNT ON  
    SET QUOTED_IDENTIFIER OFF  
 ...
 END  
   ...
   ,  FromSkuDesc       NVARCHAR(60)   NULL  
   ,  ToSkuDesc         NVARCHAR(60)   NULL  
      )  
   ...
   --JOIN SKU S     WITH (NOLOCK) ON   
   JOIN  SKU  FromSKU        WITH (NOLOCK) ON (FROMSKU.SKU = TRANSFERDETAIL.FromSku AND FROMSKU.STORERKEY = TRANSFERDETAIL.FROMSTORERKEY)   
   JOIN  SKU  ToSKU          WITH (NOLOCK) ON (ToSKU.SKU = TRANSFERDETAIL.ToSku AND ToSKU.STORERKEY = TRANSFERDETAIL.TOSTORERKEY)   
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         END  
      END  
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'isp_Transfer_Ticket_08'  
