...
/************************************************************************/
/* Stored Proc: ispManningsConsoLQ7                                     */
...
/************************************************************************/
CREATE PROC [dbo].[ispManningsConsoLQ7]
...
BEGIN
   ...
   DECLARE  
         ...
         , @c_LocationType           NVARCHAR(10) = 'DYNPPICK'      
   ...
   BEGIN
      PRINT '---------------------------------------'+ CHAR(13) +
            'Sub PICKCOde: ispManningsConsoLQ7'+ CHAR(13) +
  ...
  ;WITH ORD AS
   ...
   BEGIN
      ...
      BEGIN CATCH  
         ...
         SELECT @n_continue = 3  
         SET @c_ErrMsg = RTRIM(@c_ErrMsg) + '. (ispManningsConsoLQ7)'   
      END CATCH
      ...
      BEGIN
         PRINT 'SKU: ' + @c_SKU + CHAR(13) +
               '@n_QtyLeftToFullFill:     ' + CAST(@n_QtyLeftToFullFill AS VARCHAR) + CHAR(13) +
               '@n_QtyAvailable:          ' + CAST(@n_QtyAvailable AS VARCHAR) + CHAR(13) +
               '@n_Channel_Qty_Available: ' + CAST(@n_Channel_Qty_Available AS VARCHAR)
         ...
         BEGIN
            ...
            BEGIN
               ...
               SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0)) 
                              + ': Get PickDetailKey Failed. (ispManningsConsoLQ7)'
               GOTO QUIT_SP
            END
            ...
            BEGIN
               ...
               SET @n_Err = 81220
               SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0)) 
                              + ': Insert PickDetail Failed. (ispManningsConsoLQ7)'
               GOTO QUIT_SP
            END
...
QUIT_SP:
   ...
   BEGIN
      ...
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'ispManningsConsoLQ7'
