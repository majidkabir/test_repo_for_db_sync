...
/************************************************************************/  
/* Store Procedure:  nsp_ReplenishmentRpt_BatchRefill_07                */  
/* Creation Date: 2007-11-12                                            */  
/* Copyright: IDS                                                       */  
/* Written by: ONG GB                                                   */  
/*                                                                      */  
/* Purpose:  Swire China Wave Replenishment Report (FBR 90109)          */  
/*                                                                      */  
/*                                                                      */  
/*                                                                      */  
...
/* Usage:                                                               */  
/*                                                                      */  
/* Called By: r_replenishment_report05                                  */  
/*                                                                      */  
/* PVCS Version: 1.0                                                    */  
/*                                                                      */  
/* Version: 5.4                                                         */  
...
/* 23-Nov-2007 Aquasora  replenish by less quantity in asc follow by    */  
/*             FIFO on the receiving date                               */  
/*             by  floor qty available (#89979)                         */  
/* 04-Jan-2008 SOS90109 - Change replenishment report sorting  (June01) */  
/* 23-Feb-2012 KHLim01   Reduce blokcing and traceinfo                  */  
/* 05-MAR-2018 Wan02     WM - Add Functype                              */  
/* 05-OCT-2018 CZTENG01  WM - Add StorerKey,ReplGrp                     */  
/************************************************************************/  
  
CREATE PROC  [dbo].[nsp_ReplenishmentRpt_BatchRefill_07]  
...
,              @c_zone08      NVARCHAR(20)-- SOS #89979 storerkey  
...
BEGIN  
   ...
   SELECT @n_continue=1, @b_debug = 0  -- KHLim01  
   ...
   -- End : #89979  
    ...
    BEGIN  
        ...
        SET @d_step3TOP = GETDATE() - DATEADD(second, -1, GETDATE())  
        ...
        SET @c_TraceName = 'nsp_ReplenishmentRpt_BatchRefill_07'          
    END          -- KHLim01 end  
   ...
   BEGIN           
      ...
      CREATE INDEX IX_#REPLENISHMENT_LOTFROMLOCID ON #REPLENISHMENT  
      (  LOT, FROMLOC, ID )  
   ...
   --      WHERE 1=2  
         ...
         BEGIN  
            ...
            AND (SKUxLOC.Storerkey = @c_Storerkey OR @c_Storerkey = 'ALL') --(Wan02)  
         ...
         BEGIN  
            ...
            BEGIN  
               ...
               IF @d_step3 > @d_step3TOP  
               BEGIN  
                  SET @d_step3TOP = @d_step3  
                  SET @c_col5 = LEFT(@c_col5,2) + 'i'  
               END  
            END  
                  ...
                  BEGIN  
                     ...
                     BEGIN  
                        ...
                        END +     
                           ...
                           ORDER BY LOTxLOCxID.Qty, LOTATTRIBUTE.Lottable05,    -- #89979  
   ...
   --                              SET @d_step3 = GETDATE()  
         ...
         IF @b_debug = 3 -- KHLim01  
         BEGIN  
            SET @d_step3 = @d_step3TOP  
   --         SET @d_step3 = @d_step3 - @d_step2  
     ...
     -- KHLim01 start  
      ...
      BEGIN   
         ...
         BEGIN  
            ...
            BEGIN  
               ...
               SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Fail to get REPLENISHKEY. (nsp_ReplenishmentRpt_BatchRefill_07)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + ' ) '  
            END      
            ELSE  
            BEGIN   
               COMMIT TRAN  
         ...
         BEGIN  
           ...
           SELECT  
            ...
            BEGIN  
               ...
               SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Insert into Replenishment table failed. (nsp_ReplenishmentRpt_BatchRefill_07)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + ' ) '  
            END  
      ...
      BEGIN  
         ...
         BEGIN  
            ...
            END  
         END  
         EXECUTE nsp_logerror @n_err, @c_errmsg, 'nsp_ReplenishmentRpt_BatchRefill_07'  
   ...
   BEGIN  
      ...
      AND   LLI.SKU BETWEEN @c_zone09 AND @c_zone10 -- #89979  
      AND   L1.LocAisle BETWEEN @c_zone11 AND @c_zone12 -- #89979  
      AND (R.Storerkey = @c_Storerkey OR @c_Storerkey = 'ALL')       --(Wan02)  
   ...
   BEGIN  
      ...
      VALUES    
         ...
         ,LEFT(CONVERT(VARCHAR(12), @d_endtime - @d_starttime, 114),8)  
         ,LEFT(CONVERT(VARCHAR(12), @d_step1, 114),8) + '-' + CAST(@n_step1Ctn AS NVARCHAR(3))  
         ,LEFT(CONVERT(VARCHAR(12), @d_step2, 114),8) + '-' + CAST(@n_step2Ctn AS NVARCHAR(3))  
         ,LEFT(CONVERT(VARCHAR(12), @d_step3, 114),8) + '-' + CAST(@n_step3Ctn AS NVARCHAR(3))  
         ,LEFT(CONVERT(VARCHAR(12), @d_step4, 114),8) + '-' + CAST(@n_step4Ctn AS NVARCHAR(3))  
         ,LEFT(CONVERT(VARCHAR(12), @d_step5, 114),8) + '-' + CAST(@n_step5Ctn AS NVARCHAR(3))  
