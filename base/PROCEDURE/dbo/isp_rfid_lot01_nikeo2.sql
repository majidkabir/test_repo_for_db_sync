...
GO
/*************************************************************************/  
/* Stored Procedure: isp_RFID_Lot01_NIKEO2                               */  
...
/*************************************************************************/   
CREATE PROCEDURE [dbo].[isp_RFID_Lot01_NIKEO2] 
...
BEGIN  
   ...
   IF @c_Lottable01Value <> '' AND @c_Lottable01Value IS NOT NULL
   BEGIN
      IF NOT EXISTS (   SELECT 1
                        FROM CODELKUP WITH (NOLOCK) 
                        WHERE ListName = 'RDTLOTVal' 
      ...
      BEGIN
         SET @n_Continue = 3
         SET @n_Err = 80610
         SET @c_Errmsg = 'Invalid Lottable01: ' + RTRIM(@c_Lottable01Value) +'. (isp_RFID_Lot01_NIKEO2)'
         GOTO QUIT_SP
      END
   END
   ...
   BEGIN
      ...
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'isp_RFID_Lot01_NIKEO2'
   END
   ELSE
   BEGIN
      SET @b_Success = 1
