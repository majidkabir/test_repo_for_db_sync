...
/* 04-OCT-2021 SYCHUA   1.6   JSM-24356 Fix: channel checking to use    */
/*                            channel_id mapped with correct SKU (SY01) */
/************************************************************************/
...
BEGIN
   ...
   BEGIN
      ...
      SET @c_SQL =
             ...
             + CHAR(13) +  ', LOC.Loc, LOC.LocationHandling, LOC.LogicalLocation, LOTxLOCxID.Lot, LOTxLOCxID.ID,0 '
   ...
   + CHAR(13) +         'AND (UCC.Loc = LOTXLOCXID.Loc) '
             ...
             + CHAR(13) +  ' AND (LOTxLOCxID.QTY - LOTXLOCXID.QtyAllocated - LOTXLOCXID.QtyPicked - LOTXLOCXID.QtyReplen) > 0 '    --CS01a
             ...
             + CASE WHEN ISNULL(RTRIM(@c_Lottable01),'') = '' THEN ' AND LA.Lottable01 = @c_LocationTypeOverride ' ELSE ' AND LA.Lottable01 = @c_Lottable01 ' END   --(Wan01)
         ...
         BEGIN
            ...
            BEGIN
              ...
              BEGIN
                  ...
                  + CHAR(13) + ','''' as channel ,0 as channel_id'     --SY01
     ...
     -- + CHAR(13) + 'AND OD.Channel = CASE WHEN @c_ChannelInvMgmt = ''1'' THEN @c_Channel ELSE OD.Channel END'                   --CS01a
                ...
                SET @c_SQL =N'DECLARE CUR_ORDLINE CURSOR FAST_FORWARD READ_ONLY FOR '
                            ...
                            + CHAR(13) + ',OD.channel,CA.Channel_ID '   --SY01
                            + CHAR(13) + 'FROM ORDERDETAIL OD (NOLOCK) '
                         ...
                         -- + CHAR(13) + ' AND CA.Allocated = ''N'' '
                            + CHAR(13) + ' ORDER BY CASE WHEN (OD.OpenQty - ( OD.QtyAllocated + OD.QtyPreAllocated + OD.QtyPicked )) <  CA.CHANNELAVIQTY THEN (OD.OpenQty - ( OD.QtyAllocated + OD.QtyPreAllocated + OD.QtyPicked )) ELSE CA.CHANNELAVIQTY END
desc '
                  ...
                  OPEN CUR_ORDLINE
                  FETCH NEXT FROM CUR_ORDLINE INTO @c_Orderkey, @c_OrderLineNumber, @n_OrderLineQty,@c_Channel,@n_Channel_id      --CS01a    --SY01
                  ...
                  BEGIN
                        ...
                        BEGIN
                          SELECT @c_SQL
                          SELECT @c_Orderkey '@c_Orderkey', @n_OrderLineQty 'ordqty'
                        END
                       ...
                       BEGIN
                         ...
                         BEGIN
                            ...
                            SELECT @n_CHANNELAvaiQty = CHANNELAVIQTY
                                  --,@n_Channel_id = Channel_ID     --SY01
                            FROM #CHANNELINFO
                            WHERE Orderkey = @c_Orderkey
                            AND Channel = @c_channel
                            AND channel_id = @n_Channel_id      --SY01
                        END
                       ...
                       if @n_InsertQty > @n_CTNQty --OR @n_OrderLineQty > @n_CHANNELAvaiQty   --CS01c   --CS01e
                          BEGIN
                     ...
                     BEGIN
                        ...
                        SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0))
                                       + ': Get PickDetailKey Failed. (ispPRNIK08)'
                        GOTO QUIT_SP
                     END
                     ELSE
                     BEGIN
                        ...
                        BEGIN
                           ...
                           SET @n_Err = 61020
                           SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0))
                                          + ': Insert PickDetail Failed. (ispPRNIK08)'
                           GOTO QUIT_SP
                        END
   ...
   BEGIN
      ...
      JOIN WAVEDETAIL WD WITH (NOLOCK) ON (OD.OrderKey = WD.OrderKey)            WHERE WD.Wavekey = @c_Wavekey
   END
...
QUIT_SP:
   ...
   BEGIN
      ...
      BEGIN
         ...
         END
      END
      EXECUTE nsp_logerror @n_Err, @c_ErrMsg, 'ispPRNIK08'
