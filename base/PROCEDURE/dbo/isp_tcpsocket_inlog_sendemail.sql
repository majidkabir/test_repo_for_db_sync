...
/************************************************************************/
/* Stored Procedure: isp_TCPSocket_Inlog_SendEmail                      */
...
CREATE PROC [dbo].[isp_TCPSocket_Inlog_SendEmail]  
(
  @nMinute       INT
)
AS
...
SET ANSI_NULLS OFF
   ...
   --Reprocess outstanding message in TCPSocket_InLog 
   EXEC isp_TCPSocket_InLog_Reprocess_Message @nMinute
   
   SET @n_Err = @@ERROR
   IF @n_Err <> 0
   BEGIN
      SELECT @n_Err = 70000
      SELECT @c_ErrMsg = 'NSQL'+CONVERT(CHAR(5),@n_Err)+': Error executing isp_TCPSocket_InLog_Reprocess_Message. (isp_TCPSocket_Inlog_SendEmail)'
                       + ' ( SQLSvr MESSAGE=' + ISNULL(RTRIM(@n_Err),'') + ' )'
      GOTO Quit
   END

   IF EXISTS (SELECT 1 FROM #InLog_MessageTemp)
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            SELECT @n_Err = 70001
            SELECT @c_ErrMsg = 'NSQL'+CONVERT(CHAR(5),@n_Err)+': Error Insert into #RecipientTemp Table. (isp_TCPSocket_Inlog_SendEmail)'
                             + ' ( SQLSvr MESSAGE=' + ISNULL(RTRIM(@n_Err),'') + ' )'
            GOTO Quit
         END
      ...
      BEGIN
         ...
         SET @cBody = '<b>The following message over ' + CAST(@nMinute AS NVARCHAR(9)) + ' minutes in TCPSocket_InLog has been re-processed:</b><br />'
         
         IF EXISTS (SELECT 1 FROM #InLog_MessageTemp WITH (NOLOCK) WHERE Status = '0')
         BEGIN
            SET @cBody = @cBody + '<i>Reprocess Aging Message (Status = 0):</i>'
         ...
         IF EXISTS (SELECT 1 FROM #InLog_MessageTemp WITH (NOLOCK) WHERE Status = '5')
         BEGIN
         ...
         BEGIN
            SELECT @n_Err = 70002
            SELECT @c_ErrMsg = 'NSQL'+CONVERT(CHAR(5),@n_Err)+': Error executing sp_send_dbmail. (isp_TCPSocket_Inlog_SendEmail)'
                             + ' ( SQLSvr MESSAGE=' + ISNULL(RTRIM(@n_Err),'') + ' )'
            GOTO Quit
         END
         ...
         BEGIN
            SELECT @n_Err = 70003
            SELECT @c_ErrMsg = 'NSQL'+CONVERT(CHAR(5),@n_Err)+': Error delete from #RecipientTemp Table. (isp_TCPSocket_Inlog_SendEmail)'
                             + ' ( SQLSvr MESSAGE=' + ISNULL(RTRIM(@n_Err),'') + ' )'
            GOTO Quit
         END
      ...
      SET EmailSent = '1'
      FROM TCPSocket_INLog i 
      JOIN #InLog_MessageTemp t ON (i.SerialNo = t.SerialNo)

      SET @n_Err = @@ERROR
      IF @n_Err <> 0
      BEGIN
         SELECT @n_Err = 70004
         SELECT @c_ErrMsg = 'NSQL'+CONVERT(CHAR(5),@n_Err)+': Error updating TCPSocket_INLog Table. (isp_TCPSocket_Inlog_SendEmail)'
                          + ' ( SQLSvr MESSAGE=' + ISNULL(RTRIM(@n_Err),'') + ' )'
         GOTO Quit
      END
   END -- IF EXISTS (SELECT 1 FROM #InLog_MessageTemp)

Quit:  
   IF @n_Err <> 0
      EXECUTE nsp_logerror @n_Err, @c_ErrMsg, 'isp_TCPSocket_Inlog_SendEmail' 
   ...
   IF OBJECT_ID('tempdb..#InLog_MessageTemp','u') IS NOT NULL
      DROP TABLE #InLog_MessageTemp;
