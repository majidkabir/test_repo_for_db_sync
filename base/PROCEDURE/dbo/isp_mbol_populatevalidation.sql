...
/************************************************************************/
/* Store Procedure: isp_MBOL_PopulateValidation                         */
/* Creation Date:                                                       */
/* Copyright: LF                                                        */
/* Written by:                                                          */
/*                                                                      */
...
/*                                                                      */
/* Called By: ue_sendload at w_populate_mbol                            */
/*                           w_populate_orders_type2                    */
/*                                                                      */
/* PVCS Version: 1.0                                                    */
/*                                                                      */
/* Version: 5.4                                                         */
...
/************************************************************************/
CREATE PROC [dbo].[isp_MBOL_PopulateValidation] 
...
SELECT DISTINCT @c_MBOLKey AS MBOLKey
...
AND    CODELKUP.Code = 'MBOLPopulateValidation'
...
BEGIN
   ...
   FETCH NEXT FROM CUR_MBOL_REQUIRED INTO @c_TableName, @c_Description, @c_ColumnName, @c_WhereCondition 
   ...
   BEGIN
      ...
      SET @c_SQL = N'SELECT @c_RecFound = COUNT(1), @c_Orderkey = MIN(ORDERS.Orderkey) 
       ...
       JOIN #TMP_ORDERSLIST ON MBOL.MBOLKey = #TMP_ORDERSLIST.MBOLKey 
       JOIN ORDERS WITH (NOLOCK) ON #TMP_ORDERSLIST.Orderkey = ORDERS.Orderkey 
       LEFT JOIN LOADPLAN WITH (NOLOCK) ON (LOADPLAN.Loadkey = ORDERS.Loadkey)  
       WHERE MBOL.MBOLKey= N''' +  @c_MBOLKey + ''' '
   ...
   FETCH NEXT FROM CUR_MBOL_CONDITION INTO @c_TableName, @c_Description, @c_ColumnName, @c_Condition, @c_Type, @c_WhereCondition   
   ...
   BEGIN
      ...
      BEGIN 
         SET @b_InValid = 1 
         IF CharIndex('MBOL', @c_Condition) > 0 OR CharIndex('ORDERS', @c_Condition) > 0  
   ...
   SET @b_InValid = 0 
   DECLARE CUR_MBOL_SPCONDITION CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
   SELECT Code, Description, Long 
   FROM   CODELKUP WITH (NOLOCK)
   WHERE  ListName = @c_LoadValidationRules
   AND    SHORT    = 'STOREDPROC'

   OPEN CUR_MBOL_SPCONDITION

   FETCH NEXT FROM CUR_MBOL_SPCONDITION INTO @c_TableName, @c_Description, @c_SPName 

   WHILE @@FETCH_STATUS <> -1
   BEGIN
      IF EXISTS (SELECT 1 FROM dbo.sysobjects WHERE name = RTRIM(@c_SPName) AND type = 'P')  
      BEGIN      
         SET @c_SQL = 'EXEC ' + @c_SPName + ' @c_MBOLkey, @c_Orderkeylist NVARCHAR(MAX), @b_Success OUTPUT, @n_Err OUTPUT, @c_ErrMsg OUTPUT '      
      
         EXEC sp_executesql @c_SQL     
            , N'@c_MBOLkey NVARCHAR(10), @c_Orderkeylist, @b_Success Int OUTPUT, @n_Err Int OUTPUT, @c_ErrMsg NVARCHAR(250) OUTPUT'      
            , @c_MBOLkey  
            , @c_Orderkeylist    
            , @b_Success   OUTPUT      
            , @n_Err       OUTPUT      
         ...
         BEGIN 
            SET @b_InValid = 1      
            CLOSE CUR_MBOL_SPCONDITION
            DEALLOCATE CUR_MBOL_SPCONDITION 
            GOTO QUIT
         END 
...
IF CURSOR_STATUS('LOCAL' , 'CUR_MBOL_REQUIRED') in (0 , 1)
BEGIN
   CLOSE CUR_MBOL_REQUIRED
   DEALLOCATE CUR_MBOL_REQUIRED
END

IF CURSOR_STATUS('LOCAL' , 'CUR_MBOL_CONDITION') in (0 , 1)
BEGIN
   CLOSE CUR_MBOL_CONDITION
   DEALLOCATE CUR_MBOL_CONDITION
END

IF CURSOR_STATUS('LOCAL' , 'CUR_MBOL_SPCONDITION') in (0 , 1)
BEGIN
   CLOSE CUR_MBOL_SPCONDITION
   DEALLOCATE CUR_MBOL_SPCONDITION
END
