...
/************************************************************************/            
/* Stored Procedure: nspAL_TRI2                                         */            
/* Creation Date: 20/11/2017                                            */            
/* Copyright: LFL                                                       */            
/* Written by:                                                          */            
/*                                                                      */            
/* Purpose: WMS-3642 SG/TH Triple Allocate piece(6) from shelving       */            
/*          only if can fulfill the order qty                           */
/*                                                                      */            
/* Called By:                                                           */            
/*                                                                      */            
/* PVCS Version: 1.0                                                    */            
...
BEGIN   
   ...
   SELECT @c_SQL = "SELECT TOP 1 @n_QtyAvailable = SUM(LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED) " +
          ...
          RTRIM(@c_Condition)  + 
          CASE WHEN ISNULL(@c_Short,'') <> '' THEN 
             " GROUP By LOTATTRIBUTE.Lottable08 " 
          ELSE    
             " GROUP By LOT.Sku "
          END +
          " HAVING SUM(LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED) >= @n_QtyLeftToFulfill " 
          
      EXEC sp_executesql @c_SQL 
         , N'@c_Storerkey NVARCHAR(15), @c_Sku NVARCHAR(20), @c_Facility NVARCHAR(5), @n_QtyLeftToFulfill INT, @n_QtyAvailable INT OUTPUT'             
         , @c_StorerKey
         , @c_Sku
         , @c_Facility
         , @n_QtyLeftToFulfill
   ...
   SELECT @c_SQL = "DECLARE CURSOR_TRI_AVAILABLE CURSOR FAST_FORWARD READ_ONLY FOR " +
          ...
          " LOTATTRIBUTE.Lottable05, LOTATTRIBUTE.Lottable08 " +
          " FROM LOTATTRIBUTE (NOLOCK) " +
          " JOIN LOT (NOLOCK) ON LOT.LOT = LOTATTRIBUTE.LOT " +
          " JOIN LOTxLOCxID (NOLOCK) ON LOTXLOCXID.Lot = LOT.LOT AND LOTXLOCXID.LOT = LOTATTRIBUTE.LOT " + 
          " JOIN SKUXLOC (NOLOCK) ON SKUXLOC.Storerkey = LOTxLOCxID.Storerkey AND SKUXLOC.Sku = LOTxLOCxID.Sku AND SKUXLOC.Loc = LOTxLOCxID.Loc " +
          ...
          " AND SKUXLOC.LocationType IN('PICK','CASE') " +
          RTRIM(@c_Condition)  + 
          " GROUP By LOT.STORERKEY, LOT.SKU, LOT.LOT, LOTxLOCxID.LOC, LOTxLOCxID.ID, LOTATTRIBUTE.Lottable05, LOTATTRIBUTE.Lottable08, LOC.Loc, LOC.LogicalLocation " +
          ...
          "         ,LOT.Lot DESC, LOC.LogicalLocation, LOC.Loc "  
   ...
   FETCH NEXT FROM CURSOR_TRI_AVAILABLE INTO @c_Storerkey, @c_Sku, @c_LOT, @c_Loc, @c_ID, @n_QtyAvailable, @dt_Lottable05Found, @c_Lottable08Found
   ...
   BEGIN           	
      ...
      /*IF @c_LocationCategory IN ('RACKING','RACK') AND @c_UOM = '2'
      BEGIN
         SELECT TO 1 @n_UCCQty = Qty  --Expect the location have same UCC qty
         FROM UCC (NOLOCK)
