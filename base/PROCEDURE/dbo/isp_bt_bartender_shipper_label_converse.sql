...
/* 2020-08-05 1.9  CSCHONG    WMS-14435 revised col44 mapping (CS08)          */
/* 2020-10-27 1.10 CSCHONG    Performance tunning (CS09)                      */ 
/* 2021-04-02 1.11 CSCHONG    WMS-16024 PB-Standardize TrackingNo(CS10)       */  
/******************************************************************************/                                     
CREATE PROC [dbo].[isp_BT_Bartender_Shipper_Label_CONVERSE]                           
...
BEGIN                          
   ...
   IF @c_Sparm4='1'                
      BEGIN              
         SET @c_SQLJOIN = +' SELECT @n_id,ORD.loadkey,ORD.orderkey,ORD.externorderkey,ORD.type,buyerpo,salesman,ORD.facility,STO.Secondary,'    --8            
                          + CHAR(13) +               
                          +' STO.Company,STO.SUSR1,STO.SUSR2,(STO.Address1+STO.Address2+STO.Address3),Substring(ORD.Notes,1,80),'''',ORD.StorerKey, '   
                          ...
                          + CHAR(13) +              
                          +' ORD.InvoiceAmount,'''','''','               
                          +' ISNULL(C.Description,''''),'''','''','''','''',ORD.DeliveryPlace,'''', '  --50        --CS08   
                          ...
                          +' LEFT JOIN CODELKUP C WITH (NOLOCK) ON C.Listname = ''SHIPLB'' AND C.Storerkey = ORD.Storerkey AND C.code=ORD.ShipperKey '   --CS08 
   ...
   BEGIN            
      SET @c_SQLJOIN = +' SELECT @n_id,ORD.loadkey,ORD.orderkey,ORD.externorderkey,ORD.type,buyerpo,salesman,ORD.facility,STO.Secondary,'    --8            
                       ...
                       + CHAR(13) +              
                       +' ORD.InvoiceAmount,'''','''','               
                       +' ISNULL(C.Description,''''),'''','''','''','''',ORD.DeliveryPlace,'''', '          --CS08 
                       +' '''',ORD.M_Address1,ORD.M_Address2,ORD.M_City,'''','''',ORD.Priority,ORD.Userdefine10,'''','''' '           
                       + CHAR(13) +                
