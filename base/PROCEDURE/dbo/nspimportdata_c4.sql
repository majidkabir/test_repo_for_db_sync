...
GO
CREATE PROC    [dbo].[nspImportData_C4]
 ...
 SELECT @b_debug = 0
    ...
    BEGIN
      ...
      BEGIN
 			...
 			from UPLOADC4PODETAIL left outer join sku
 			  on UPLOADC4PODETAIL.storerkey = sku.storerkey
 ...
 --
         ...
         SELECT ExternPOKey, ExternLinenumber, POkey, POlinenumber
                ...
                BEGIN
                   ...
                   END
 /*
                   SELECT @count1 = COUNT(*) FROM UPLOADC4PODETAIL (nolock) WHERE ExternPOKey = @c_externpokey 
 ...
 */
 ...
 -- ------------------------------------------------------------------
 	...
 	BEGIN
 		...
 		SET status = 'E', remarks = 'Wrong SKU & STORERKEY Combination or SKU Does not exists'
 		FROM UPLOADC4ORDERDETAIL (nolock) LEFT OUTER JOIN sku (nolock)
 		ON UPLOADC4ORDERDETAIL.storerkey = sku.storerkey
 			AND UPLOADC4ORDERDETAIL.sku = sku.sku
 			AND UPLOADC4ORDERDETAIL.status = '0'
 			AND UPLOADC4ORDERDETAIL.storerkey = 'C4LGTH'
 		WHERE sku.sku = NULL
 		UPDATE UPLOADC4ORDERHEADER
 		SET status = 'E', remarks = 'Detail has Wrong sku & storerkey combination or SKU Does not exists'
 		FROM UPLOADC4ORDERHEADER h (nolock) INNER JOIN UPLOADC4ORDERDETAIL d (nolock)
 		ON h.orderkey = d.orderkey
 			AND h.storerkey = 'C4LGTH'
 			AND d.status = 'E'
 			AND h.status = '0'
 			...
 			BEGIN
 				...
 				BEGIN -- does not exists
 					...
 					SELECT @c_Orderkey, @c_externOrderkey, UPLOADC4ORDERHEADER.Storerkey, Orderdate, Deliverydate, 
 						...
 						STORER.Facility, UPLOADC4ORDERHEADER.Type, UPLOADC4ORDERHEADER.Consigneekey
 	...
 	--                        SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Error Inserting into OrderDETAIL(nspImOrderrtData)"
 		...
 		-- for  mode = 3 (DELETE), Delete the details first, and if all details has been deleted, delete the header.
 			...
 			SELECT @count1 = COUNT(*) FROM UPLOADC4ORDERHEADER (NOLOCK) WHERE ExternOrderkey = @c_externOrderkey and OrderGROUP = @c_Ordergroup and mode = @c_mode
 			IF @count1 > 0
 			BEGIN
 				SELECT @n_counter = @n_counter + 1
 			END
