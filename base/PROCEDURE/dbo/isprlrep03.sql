...
GO
/************************************************************************/      
/* Stored Procedure: ispRLREP03                                         */      
/* Creation Date: 04/10/2018                                            */      
/* Copyright: LFL                                                       */      
/* Written by:                                                          */      
/*                                                                      */      
/* Purpose: WMS-6169 TW DTDC Release replenishment task(RPF) from       */
    ...
    BEGIN
       ...
       DECLARE CUR_REPLENISH CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
          SELECT R.StorerKey, R.Sku, R.FromLoc, R.Id, R.ToLoc, R.ToID, R.UOM, R.Qty, R.Lot,
                 R.ReplenishmentKey, ISNULL(R.ReplenishmentGroup,''), R.Priority, FRLOC.LocAisle
          From  REPLENISHMENT R (NOLOCK)   
          JOIN  LOC (NOLOCK) ON (LOC.Loc = R.ToLoc)  
          JOIN  LOC  FRLOC (NOLOCK) ON R.FromLoc = FRLOC.Loc
          WHERE (LOC.putawayZone IN (@c_Zone02, @c_Zone03, @c_Zone04, @c_Zone05, @c_Zone06, @c_Zone07, @c_Zone08, @c_Zone09, @c_Zone10, @c_Zone11, @c_Zone12)
                 OR @c_Zone02 = 'ALL'
                )
          AND (LOC.Facility = @c_Facility OR ISNULL(@c_Facility,'') = '') 
          ...
          AND R.StorerKey = CASE WHEN @c_StorerKey = 'ALL' OR @c_StorerKey = '' THEN  
                                   R.StorerKey ELSE @c_StorerKey END              
          ORDER BY R.ReplenishmentGroup, FRLOC.LogicalLocation, FRLOC.Loc, R.Replenishmentkey

       OPEN CUR_REPLENISH
       
       FETCH FROM CUR_REPLENISH INTO @c_Storerkey, @c_Sku, @c_FromLoc, @c_ID, @c_ToLoc, @c_ToID, @c_UOM, @n_Qty, @c_Lot, @c_Replenishmentkey, @c_ReplenishmentGroup, @c_Priority, @c_LocAisle
       ...
       BEGIN       	      
          ...
          EXEC isp_InsertTaskDetail   
             ...
             ,@c_SourcePriority        = '9'      
             ,@c_SourceType            = 'ispRLREP03'     
             ,@c_SourceKey             = @c_Replenishmentkey      
             ,@c_AreaKey               = '?F'  -- ?F=Get from location areakey 
             ,@c_Groupkey              = @c_LocAisle
             ,@c_Message03             = @c_ReplenishmentGroup
...
RETURN_SP:
    ...
    BEGIN  
       ...
       BEGIN  
          ...
          END  
       END  
       execute nsp_logerror @n_err, @c_errmsg, "ispRLREP03"  
