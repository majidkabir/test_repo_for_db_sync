...
/******************************************************************************/                   
/* Copyright: LFL                                                             */                   
/* Purpose: isp_BT_Bartender_SKULBLSKE_02                                     */                   
/*                                                                            */                   
/* Modifications log:                                                         */                   
/*                                                                            */                   
/* Date       Rev  Author     Purposes                                        */                   
/* 2021-07-31 1.0  Mingle     Created(WMS-17518)                              */                        
/******************************************************************************/                  
                    
CREATE PROC [dbo].[isp_BT_Bartender_SKULBLSKE_02]                        
...
BEGIN                        
  ...
  DECLARE  @d_Trace_StartTime   DATETIME,     
   ...
   WHERE PACKHEADER.Pickslipno = @c_Sparm03  
   ...
   IF NOT EXISTS(
                  ...
                  WHERE CL.Storerkey = 'APEDEMOD' AND CL.LISTNAME = 'APEDEPCK' AND CL.Notes = 'SHOPBOP' AND CL.LONG = @c_consigneekey
                )
   BEGIN
      GOTO EXIT_SP
   END
  ...
  +',Col35,Col36,Col37,Col38,Col39,Col40,Col41,Col42,Col43,Col44'  + CHAR(13) +             
   ...
   SET @c_SQLJOIN = ' SELECT DISTINCT ' + CHAR(13)  
                  + ' S.SKU,S.Descr,S.AltSKU,S.Busr4,S.Style,S.Color,S.Size,S.Busr7,S.Busr8,S.Busr9, ' + CHAR(13)  
                  + ' S.Busr10,'''','''','''','''','''','''','''','''','''', ' + CHAR(13)  
                  + ' '''','''','''','''','''','''','''','''','''','''', ' + CHAR(13)  
                  + ' '''','''','''','''','''','''','''','''','''','''', ' + CHAR(13)  
                  + ' '''','''','''','''','''','''','''','''','''','''', ' + CHAR(13)  
                  ...
                  + ' JOIN SKU S (NOLOCK) ON S.SKU = OD.SKU AND S.Storerkey = PH.Storerkey'
                  + ' WHERE PH.Storerkey = @c_Sparm01 '  + CHAR(13)  
                  + ' AND OD.SKU = @c_Sparm02  ' + CHAR(13)  
                  + ' AND PH.Pickslipno = @c_Sparm03 '  
                  + ' GROUP BY S.SKU,S.Descr,S.AltSKU,S.Busr4,S.Style,S.Color,S.Size,S.Busr7,S.Busr8,S.Busr9,S.Busr10 '  
