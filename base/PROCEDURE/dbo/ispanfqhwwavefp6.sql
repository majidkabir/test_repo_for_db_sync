...
/************************************************************************/
CREATE PROC [dbo].[ispANFQHWWaveFP6]
...
BEGIN
   ...
   BEGIN
      PRINT '---------------------------------------'+ CHAR(13) +
            'Sub PICKCOde: ispANFQHWWaveFP6'+ CHAR(13) +
   ...
   BEGIN
      ...
      BEGIN        
         ...
         BEGIN CATCH  
            ...
            SELECT @n_continue = 3  
            SET @c_ErrMsg = RTRIM(@c_ErrMsg) + '. (ispANFQHWWaveFP6)'   
         END CATCH
      ...
      SELECT @n_NoOfSKU = COUNT(DISTINCT LLI.SKU)
      FROM LOTXLOCXID LLI (NOLOCK)
      WHERE LLI.Loc = @c_LOC
      AND LLI.ID = @c_ID
      AND LLI.Storerkey = @c_Storerkey
            ...
            BEGIN
               ...
               BEGIN
                  ...
                  SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0)) 
                                 + ': Get PickDetailKey Failed. (ispANFQHWWaveFP6)'
                  GOTO QUIT_SP
               END
               ...
               BEGIN
                  PRINT '----------------------------------------------------------------------'   + CHAR(13) +
                        ...
                        'Channel_ID: ' +  CAST(@n_Channel_ID AS NVARCHAR) + CHAR(13) + 
                        '----------------------------------------------------------------------'
               END
               ...
               BEGIN
                  ...
                  SET @n_Err = 81220
                  SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0)) 
                                 + ': Insert PickDetail Failed. (ispANFQHWWaveFP6)'
                  GOTO QUIT_SP
               END
               ...
               BEGIN
                  ...
                  BEGIN
                     ...
                     SET @n_Err = 81230
                     SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0)) 
                                    + ': Update UCC Failed. (ispANFQHWWaveFP6)'
                     GOTO QUIT_SP
                  END
               END
...
QUIT_SP:
   ...
   BEGIN
      ...
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'ispANFQHWWaveFP6'
