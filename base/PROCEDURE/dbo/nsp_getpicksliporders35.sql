...
GO
/************************************************************************/  
/* Store Procedure: nsp_GetPickSlipOrders35                             */  
/* Creation Date:                                                       */  
/* Copyright: IDS                                                       */  
/* Written by:                                                          */  
/*                                                                      */  
...
/* 04-FEB-2015  CSCHONG   1.5   332703 and rename from 01 to 35 (CS01)  */  
/* 18-SEP-2015  SPChin   1.6   SOS352293 - Extend Lottable02 Field Size */  
/*                             to 18 Char                               */  
/* 27-DEC-2018  WLCHOOI   1.7   WMS-6804 - Use BuyerPO as barcode       */  
 ...
 BEGIN  
   ...
   CREATE TABLE #TEMP_PICK  
       ( PickSlipNo       NVARCHAR(10) NULL,  
         ...
         Lottable02       NVARCHAR(18) NULL, --SOS352293  
   ...
   SELECT DISTINCT  
         ...
         CASE WHEN ISNULL(CLR2.Short,'N') = 'Y' THEN '' ELSE LotAttribute.Lottable01 END,   --mingle01
   ...
   LEFT OUTER JOIN Codelkup CLR (NOLOCK) ON (Orders.Storerkey = CLR.Storerkey AND CLR.Code = 'SHOWFIELD'  
                                       AND CLR.Listname = 'REPORTCFG' AND CLR.Long = 'r_dw_print_pickorder35' AND ISNULL(CLR.Short,'') <> 'N')  
   LEFT OUTER JOIN Codelkup CLR1 (NOLOCK) ON (Orders.Storerkey = CLR1.Storerkey AND CLR1.Code = 'ShowBuyerPo'    
                                       AND CLR1.Listname = 'REPORTCFG' AND CLR1.Long = 'r_dw_print_pickorder35' AND ISNULL(CLR1.Short,'') <> 'N') --WL01
   LEFT OUTER JOIN Codelkup CLR2(NOLOCK) ON (Orders.Storerkey = CLR2.Storerkey AND CLR2.Code = 'HideLottable01'    
                                       AND CLR2.Listname = 'REPORTCFG' AND CLR2.Long = 'r_dw_print_pickorder35' AND ISNULL(CLR2.Short,'') <> 'N') --mingle01                            
