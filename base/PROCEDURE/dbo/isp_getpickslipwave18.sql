...
/* Written by: WLChooi                                                  */  
/*                                                                      */  
/* Purpose: WMS-11944 - SG - PMI - Picking Slip                         */  
/*                                                                      */  
/* Called By: RCM - Generate Pickslip                                   */  
/*          : Datawindow - r_dw_print_wave_pickslip_18                  */  
/* copy from Datawindow - r_dw_print_wave_pickslip_18                   */
/*                                                                      */  
/* PVCS Version: 1.0                                                    */  
/*                                                                      */  
/* Version: 5.4                                                         */  
...
BEGIN  
   ...
   CREATE TABLE #Temp_Box (Pickslipno   NVARCHAR(10), PM001   INT, PM002   INT, PM003   INT)
  ...
  SELECT P.Orderkey, P.Pickdetailkey, P.Lot, P.Loc, P.ID INTO #SUB_EARLYPICK
   ...
   AND Long      = 'r_dw_print_wave_pickslip_18'
   ...
   GROUP BY Storerkey
    ...
    INSERT INTO #TMP_PICK  
          ...
          1                              AS UOMQTY,  
          0                              AS TempQty2,  
          ISNULL((SELECT Distinct 'Y' FROM pickdetail WITH (NOLOCK) WHERE pickdetail.PickSlipNo = Pickheader.PickHeaderKey)
          , 'N') AS PrintedFlag,  
          ...
          --CaseQty     = CASE WHEN LOC.Pickzone = 'PMICASEPZ' THEN 
          --              CASE Pack.Casecnt    WHEN 0 THEN 0 ELSE FLOOR(SUM(PickDetail.qty) / Pack.Casecnt) END
          --              ELSE 0 END,  
          --CartonQty   = CASE WHEN LOC.Pickzone = 'PMICARPZ' THEN 
          --              CASE Pack.OtherUnit1 WHEN 0 THEN 0 ELSE FLOOR(SUM(PickDetail.qty) / Pack.OtherUnit1) END
          --              ELSE 0 END,       
          --PackQty     = CASE WHEN LOC.Pickzone IN ('PMIPACKPZ','PMIAGING') THEN 
          --              CASE Pack.Innerpack  WHEN 0 THEN 0 ELSE FLOOR(SUM(PickDetail.qty) / Pack.Innerpack) END
          --              ELSE 0 END,  
          Each_cal    = 0,  
         ...
         -- ISNULL(ORDERS.DeliveryDate, '19000101') DeliveryDate,  
          ...
          ,LOC.Pickzone
          ,CASE WHEN LOC.Pickzone IN ('PMICARPZ','PMIPACKPZ','PMIAGING') THEN 2
                WHEN LOC.Pickzone IN ('PMICASEPZ') THEN 1           
                ELSE 2 END
          ,ORDERS.[Type]
   FROM WAVEDETAIL      WD  WITH (NOLOCK) 
   ...
   LEFT JOIN (  
          ...
          LEFT JOIN CODELKUP C (NOLOCK) ON (C.Storerkey = Storer.Storerkey) AND C.Long = 'r_dw_print_wave_pickslip_18' 
                                 AND C.Listname = 'REPORTCFG' AND C.Code = 'SHOWUPC'
      WHERE  PickDetail.Status < '5'  
      AND WD.WaveKey = @c_waveKey  
      GROUP BY Pickheader.PickHeaderKey,orders.loadkey ,
          PickDetail.OrderKey,  
         ...
         -- CONVERT(NVARCHAR(60), ISNULL(ORDERS.Notes2, '')),  
          ...
          ,CASE WHEN LOC.Pickzone IN ('PMICARPZ','PMIPACKPZ','PMIAGING') THEN 2
                ...
                ELSE 2 END
          ,ORDERS.[Type]
		ORDER BY Pickheader.PickHeaderKey,orders.loadkey, PickDetail.sku
     ...
     IF @c_PrevPAZone <> @c_PZone 
      ...
      BEGIN 
         ...
         SET @c_RPickSlipNo = 'P' + @c_RPickSlipNo          
               ...
               VALUES      
                     ...
                     BEGIN      
                        ...
                        SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert PICKHEADER Failed (isp_GetPickSlipWave18)'   
          ...
          ELSE   
            BEGIN  
               SELECT @n_continue = 3  
               SELECT @n_err = 63502
               SELECT @c_errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Get PSNO Failed. (isp_GetPickSlipWave18)'  
               BREAK   
            END 
            ...
            BEGIN  
               ...
               BEGIN   
                  ...
                  BEGIN  
                     SELECT @n_continue = 3
                     SELECT @n_err = 63503
                      SELECT @c_errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert RefKeyLookup Failed. (isp_GetPickSlipWave18)'    
                     GOTO QUIT
                  END                          
               END   
         ...
         BEGIN  
            SELECT @n_continue = 3  
            SELECT @n_err = 63504
            SELECT @c_errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Update #TMP_PICK Failed. (isp_GetPickSlipWave18)'    
            GOTO QUIT
         END
               ...
               BEGIN      
                  ...
                  SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': UPDATE Pickdetail Failed (isp_GetPickSlipWave18)'   
                               + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '   
                  GOTO QUIT     
               END  
   ...
   DECLARE CUR_WaveOrder CURSOR LOCAL FAST_FORWARD READ_ONLY FOR  
     ...
     WHERE WD.WaveKey = @c_Wavekey                                        
      ...
      FETCH NEXT FROM CUR_WaveOrder INTO @c_GetWavekey,@c_GetLoadkey,@c_GetWDOrdKey
...
QUIT:  
   ...
   BEGIN   
      ...
      BEGIN    
         ROLLBACK TRAN    
      END   
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'isp_GetPickSlipWave18'    
   END  
   ...
   --UPDATE #TMP_PICK
   --SET   #TMP_PICK.Pickzone = 'PACK',
   --      #TMP_PICK.PageGroup = '1'
   --WHERE #TMP_PICK.OrderKey = '0006547279'

   --INSERT INTO #TempTotalQtyPerPickslip

   DECLARE @c_GetPSNo         NVARCHAR(10)
         , @c_Pickzones       NVARCHAR(30)
         , @c_Remarks         NVARCHAR(250) = ''
         , @n_Qty3 INT = 0, @n_Qty2 INT = 0, @n_Qty1 INT = 0
         , @n_TempQty INT
         , @c_Remark1   NVARCHAR(10)
   ...
   FROM #TMP_PICK
   WHERE Pickzone <> 'PMICASEPZ'
   GROUP BY PickSlipNo

   OPEN cur_Loop

   FETCH NEXT FROM cur_Loop INTO   @c_GetPSNo
                                 , @c_Pickzones       
                                 , @n_Qty 
                                 
   WHILE @@FETCH_STATUS <> - 1
   BEGIN
      IF NOT EXISTS (SELECT 1 FROM #Temp_Box WHERE Pickslipno = @c_Pickslipno)
      BEGIN
         INSERT INTO #Temp_Box SELECT @c_GetPSNo,0,0,0
      END

      --Maximum size
      -- PM001: 100, PM002: 250, PM003: 500
      SET @n_TempQty = @n_Qty
      ...
      BEGIN
         IF @n_Qty <= 100
         BEGIN 
            SELECT @n_Qty1 = CASE WHEN FLOOR(@n_Qty / 100) = 0 THEN 1 ELSE FLOOR(@n_Qty / 100) END

            UPDATE #Temp_Box
            SET PM001 = PM001 + @n_Qty1
            WHERE Pickslipno = @c_GetPSNo

            SET @n_Qty = @n_Qty - (@n_Qty1 * 100) --(@n_Qty1 * CASE WHEN FLOOR(@n_Qty / 100) = 0 THEN @n_Qty ELSE FLOOR(@n_Qty / 100) END)
         END
         ELSE IF @n_Qty > 100 AND @n_Qty <= 250
         BEGIN
            SELECT @n_Qty2 = CASE WHEN FLOOR(@n_Qty / 250) = 0 THEN 1 ELSE FLOOR(@n_Qty / 250) END

            UPDATE #Temp_Box
            SET PM002 = PM002 + @n_Qty2
            WHERE Pickslipno = @c_GetPSNo 

            SET @n_Qty = @n_Qty - (@n_Qty2 * 250) --(@n_Qty2 * CASE WHEN FLOOR(@n_Qty / 250) = 0 THEN @n_Qty ELSE FLOOR(@n_Qty / 250) END)

         END
         ELSE IF @n_Qty > 250 AND @n_Qty <= 500
         BEGIN
            SELECT @n_Qty3 = CASE WHEN FLOOR(@n_Qty / 500) = 0 THEN 1 ELSE FLOOR(@n_Qty / 500) END

            UPDATE #Temp_Box
            SET PM003 = PM003 + @n_Qty3
            WHERE Pickslipno = @c_GetPSNo

            SET @n_Qty = @n_Qty - (@n_Qty3 * 500) --(@n_Qty3 * CASE WHEN FLOOR(@n_Qty / 500) = 0 THEN @n_Qty ELSE FLOOR(@n_Qty / 500) END)
         END
         ELSE
         BEGIN
            SET @n_Qty3 = FLOOR(@n_Qty / 500)

            UPDATE #Temp_Box
            SET PM003 = PM003 + @n_Qty3
            WHERE Pickslipno = @c_GetPSNo 

            SET @n_Qty = @n_Qty - @n_Qty3 * 500
      ...
      BEGIN
         ...
         IF @n_Qty3 > 0
         BEGIN
            SET @c_Remarks = CAST(@n_Qty3 AS NVARCHAR(10)) + ' PM003'
            SET @n_Qty = @n_Qty - @n_Qty3 * 500
         END

         SET @n_Qty2 = FLOOR(@n_Qty / 250)

         IF @n_Qty2 > 0
         BEGIN
            SET @c_Remarks = @c_Remarks + ', ' + CAST(@n_Qty2 AS NVARCHAR(10)) + ' PM002'
            SET @n_Qty = @n_Qty - @n_Qty2 * 250
         END

         SET @n_Qty1 = CASE WHEN FLOOR(@n_Qty / 100) = 0 THEN 1 ELSE FLOOR(@n_Qty / 100) END

         IF @n_Qty1 > 0
         BEGIN
            SET @c_Remarks = @c_Remarks + ' and ' + CAST(@n_Qty1 AS NVARCHAR(10)) + ' PM001'
         END
   ...
   --SELECT * FROM #TempTotalQtyPerPickslip
   ...
   SELECT              
      ...
      ,  #TMP_PICK.Notes1             
      ,  #TMP_PICK.Notes2             
      ,  CASE WHEN #TMP_PICK.Pickzone IN ('PMICARPZ','PMIPACKPZ') THEN SUBSTRING(LTRIM(RTRIM(UPPER(#TMP_PICK.LOC))),5,2) + '-' + SUBSTRING(LTRIM(RTRIM(UPPER(#TMP_PICK.LOC))),7,2)
              WHEN #TMP_PICK.Pickzone IN ('PMICASEPZ') AND LEFT(LTRIM(RTRIM(UPPER(#TMP_PICK.LOC))),4) = 'EX2S' 
                   THEN SUBSTRING(LTRIM(RTRIM(UPPER(#TMP_PICK.LOC))),4,1) + '-' + SUBSTRING(LTRIM(RTRIM(UPPER(#TMP_PICK.LOC))),5,2)
              WHEN #TMP_PICK.Pickzone IN ('PMIAGING') 
                   THEN SUBSTRING(LTRIM(RTRIM(UPPER(#TMP_PICK.LOC))),4,1) + '-' + SUBSTRING(LTRIM(RTRIM(UPPER(#TMP_PICK.LOC))),5,2) + '-' + 
                        SUBSTRING(LTRIM(RTRIM(UPPER(#TMP_PICK.LOC))),7,2)  
              ELSE UPPER(#TMP_PICK.LOC) END AS LOC    
      ,  #TMP_PICK.ID                 
      ,  #TMP_PICK.SKU                
      ,  #TMP_PICK.SkuDesc            
      ...
      ,  #TMP_ORDSUM.totalqtyoverallocate,  
         'N' AS Highbayloc,  
          ...
          #TMP_PICK.PageGroup,
          #TMP_PICK.OrdType,
          CASE WHEN #TMP_PICK.Pickzone = 'PMICASEPZ' THEN '' ELSE t.Remarks END AS Remarks
   FROM   #TMP_PICK  
   JOIN   #TMP_ORDSUM ON #TMP_PICK.Orderkey = #TMP_ORDSUM.Orderkey   
   LEFT JOIN #TMP_HIGHBAYLOC ON #TMP_PICK.Loc = #TMP_HIGHBAYLOC.Loc 
   JOIN   #TempTotalQtyPerPickslip t ON #TMP_PICK.PickSlipNo = t.Pickslipno
   ...
   Group BY #TMP_PICK.PickSlipNo     
      ...
      ,  #TMP_ORDSUM.totalqtyoverallocate,  
         CASE WHEN ISNULL(#TMP_HIGHBAYLOC.Loc,'') <> '' THEN  
          ...
          #TMP_PICK.Pickzone,
          #TMP_PICK.PageGroup,
          #TMP_PICK.OrdType, UPPER(#TMP_PICK.LOC), CASE WHEN #TMP_PICK.Pickzone = 'PMICASEPZ' THEN '' ELSE t.Remarks END
   --ORDER BY #TMP_PICK.PageGroup,#TMP_PICK.Pickzone,#TMP_PICK.PickSlipNo,#TMP_PICK.LogicalLoc,UPPER(#TMP_PICK.LOC),#TMP_PICK.SKU
   ORDER BY #TMP_PICK.PageGroup,#TMP_PICK.PickSlipNo,#TMP_PICK.Pickzone,#TMP_PICK.LogicalLoc,UPPER(#TMP_PICK.LOC),#TMP_PICK.SKU
     
  --SELECT '1' AS PickSlipNo
  
  IF OBJECT_ID('tempdb..#Temp_Box') IS NOT NULL
      DROP TABLE #Temp_Box

   IF OBJECT_ID('tempdb..#TempTotalQtyPerPickslip') IS NOT NULL
      DROP TABLE #TempTotalQtyPerPickslip
