...
GO
--
-- Definition for stored procedure isp_ScanOut : 
--

/************************************************************************/
/* Stored Procedure: isp_ScanOut                                        */
/* Creation Date:                                                       */
/* Copyright: IDS                                                       */
/* Written by:                                                          */
/*                                                                      */
/* Purpose: Replace ntrPickingInfoUpdate Trigger, use Stored Proc			*/
         ...
         BEGIN
            ...
            BEGIN
               ...
               BEGIN  
                  SELECT @n_continue = 3  
                  SELECT @n_err = 61781 
                  SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Update Failed On Table PICKDETAIL. (isp_ScanOut)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTRIM(dbo.fnc_RTRIM(@c_errmsg)) + ' ) '  
                  GOTO EXIT_SP
               END              
--                ELSE
--                BEGIN
...
--                END  
               ...
               BEGIN             
                  ...
                  BEGIN
                     ...
                     BEGIN  
                        SELECT @n_continue = 3  
                        SELECT @n_err = 61782 
                        SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Update Failed On Table ORDERS. (isp_ScanOut)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTRIM(dbo.fnc_RTRIM(@c_errmsg)) + ' ) '  
                        GOTO EXIT_SP
                     END              
                  END
               END
               ...
               BEGIN             
                  ...
                  BEGIN
                     ...
                     BEGIN  
                        SELECT @n_continue = 3  
                        SELECT @n_err = 61783 
                        SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Update Failed On Table ORDERDETAIL. (isp_ScanOut)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTRIM(dbo.fnc_RTRIM(@c_errmsg)) + ' ) '  
                        GOTO EXIT_SP
                     END               
                  END 
               END 
                  ...
                  BEGIN 
                     ...
                     BEGIN 
                        ...
                        BEGIN  
                           SELECT @n_continue = 3  
                           SELECT @n_err = 61784 
                           SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Update Failed On Table LOADPLANDETAIL. (isp_ScanOut)' 
            ...
            BEGIN  
              ...
              BEGIN  
                  ...
                  BEGIN  
                     SELECT @n_continue = 3  
                     SELECT @n_err = 61788 
                     SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Update Failed On Table LOADPLAN. (isp_ScanOut)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTRIM(dbo.fnc_RTRIM(@c_errmsg)) + ' ) '  
                     GOTO EXIT_SP
                  END             
               END -- NOT EXISTS
   ...
   BEGIN
      ...
      BEGIN  
         SELECT @n_continue = 3  
         SELECT @n_err = 61790 
         SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Update Failed On Table PICKINGINFO. (isp_ScanOut)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTRIM(dbo.fnc_RTRIM(@c_errmsg)) + ' ) '  
         GOTO EXIT_SP
      END                  
	END -- @n_continue = 1 or @n_continue=2 
   ...
   -- insert into table, TraceInfo for tracing purpose.
     ...
     BEGIN 
        ...
        INSERT INTO TraceInfo VALUES
           ('isp_ScanOut - PS# = ' + @c_PickSlipNo + ' Type = ' + dbo.fnc_RTRIM(@c_TicketType) 
   ...
   /***** End Add by DLIM *****/
	...
	BEGIN
		...
		BEGIN
			...
			END
		END
		EXECUTE dbo.nsp_logerror @n_err, @c_errmsg, 'isp_ScanOut'		
