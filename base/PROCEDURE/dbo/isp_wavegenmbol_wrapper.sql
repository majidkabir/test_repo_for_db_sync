...
GO
/************************************************************************/  
/* Stored Procedure: isp_WaveGenMbol_Wrapper                            */  
...
BEGIN  
   ...
   AND    ConfigKey = 'WAVEGENMBOL_SP'  

   IF ISNULL(RTRIM(@c_SPCode),'') =''
   BEGIN
   	   --SELECT @c_SPCode = 'ispWAVMB01'
       
       SELECT @n_continue = 3  
       SELECT @c_ErrMsg = CONVERT(CHAR(250), @n_Err),
              @n_Err = 31011 -- Should Be Set To The SQL Errmessage but I don't know how to do so.  
       SELECT @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5), @n_Err) + 
              ': Please Setup Stored Procedure Name into Storer Configuration WAVEGENMBOL_SP for '+RTRIM(@c_StorerKey)+' (isp_WaveGenMbol_Wrapper)'  
       GOTO QUIT_SP      
   END
   ...
   BEGIN
       ...
       SELECT @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5), @n_Err) + 
              ': Storerconfig WAVEGENMBOL_SP - Stored Proc name invalid ('+RTRIM(ISNULL(@c_SPCode,''))+') (isp_WaveGenMbol_Wrapper)'  
       GOTO QUIT_SP
   END
   ...
   BEGIN
       SELECT @b_success = 0
       EXECUTE nsp_logerror @n_Err, @c_ErrMsg, 'isp_WaveGenMbol_Wrapper'  
