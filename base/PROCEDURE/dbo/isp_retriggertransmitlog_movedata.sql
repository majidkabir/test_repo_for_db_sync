...
/************************************************************************/  
/* Store Procedure: isp_ReTriggerTransmitLog_MoveData                   */  
...
BEGIN  
   ...
   SELECT @n_Continue = 3    
   SET @n_err = 700003  
   SELECT @c_errmsg = 'ERROR. Not allow to move PickDetail record. (isp_ReTriggerTransmitLog_MoveData)'  
...
BEGIN  
   ...
   SELECT @n_Continue = 3   
   SET @n_err = 700005   
   SELECT @c_errmsg = 'ERROR. Table/Column does not exist. (isp_ReTriggerTransmitLog_MoveData)'  
...
BEGIN  
   ...
   SELECT @n_Continue = 3    
   SET @n_err = 700006  
   SELECT @c_errmsg =  'ERROR. Target / Source Table Column Unmatched. (isp_ReTriggerTransmitLog_MoveData)'  
...
BEGIN  
  ...
  EXEC sp_executesql  @c_SQL  
   ...
   --   END    
      ...
      SET @c_ExecArguments = N'@c_DocKey NVARCHAR(50),@c_RecFound NVARCHAR(1) OUTPUT'  
      ...
      IF @b_Debug = 1  
      BEGIN  
         SELECT @c_SQL '@c_SQL',@c_RecFound '@c_RecFound'  
      END  
 ...
 IF @n_continue=3  -- Error Occured - Process And Return  
   BEGIN  
      ...
      BEGIN  
         ...
         END  
      END  
      execute nsp_logerror @n_err, @c_errmsg, 'isp_ReTriggerTransmitLog_MoveData'  
