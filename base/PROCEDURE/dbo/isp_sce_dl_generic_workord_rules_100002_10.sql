...
GO
/************************************************************************/
/* Store Procedure:  isp_SCE_DL_GENERIC_WORKORD_RULES_100002_10         */
...
CREATE   PROCEDURE [dbo].[isp_SCE_DL_GENERIC_WORKORD_RULES_100002_10] (
   @b_Debug       INT            = 0
 , @n_BatchNo     INT            = 0
 , @n_Flag        INT            = 0
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         SET STG_Status = '3'
           , STG_ErrMsg = LTRIM(TRIM(ISNULL(STG_ErrMsg, ''))) + '/Facility is Null'
         WHERE STG_BatchNo    = @n_BatchNo
         AND   STG_Status       = '1'
         AND   (
                Facility IS NULL
         ...
         BEGIN
            ...
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5), ISNULL(@n_ErrNo, 0))
                            + ': Update record fail. (isp_SCE_DL_GENERIC_WORKORD_RULES_100002_10)';
            ROLLBACK;
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               SET @c_ttlMsg = LTRIM(TRIM(ISNULL(@c_ttlMsg, '')))
            ...
            AND   TRIM(STG.Storerkey) = @c_StorerKey
            AND   TRIM(STG.Facility)  = @c_Facility;
         ...
         BEGIN
            ...
            WHERE Facility = @c_Facility
               ...
               SET @c_ttlMsg = LTRIM(TRIM(ISNULL(@c_ttlMsg, ''))) + N'/Facility not exists';
            GOTO NEXTITEM;
   ...
   IF @b_Debug = 1
   BEGIN
      SELECT '<<SUB-SP-RULES>> - [isp_SCE_DL_GENERIC_WORKORD_RULES_100002_10] EXIT... ErrMsg : ' + ISNULL(TRIM(@c_ErrMsg), '');
   END;
