...
GO
/************************************************************************/
/* Store Procedure:  ispMBRTK10                                         */
/* Creation Date:  15-Aut-2021                                          */
/* Copyright: LFL                                                       */
/* Written by:                                                          */
/*                                                                      */
/* Purpose:  WMS-17721 VN PACSUN - MBOL release container move task     */
/*           storerconfig: ReleaseCMBOL_MV_SP  (Copy SOS#360341)        */
/*                                                                      */
...
BEGIN
   ...
   IF EXISTS (SELECT 1 FROM TASKDETAIL(NOLOCK) WHERE Sourcekey = @c_Mbolkey AND SourceType = 'ispMBRTK10')
   BEGIN
      SELECT @n_continue = 3  
      SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err = 36000   -- Should Be Set To The SQL Errmessage but I don't know how to do so.  
      SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Move Task has been released for the MBOL. Not allow to release again. (ispMBRTK10)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
      GOTO QUIT_SP    	  
   END
   ...
   BEGIN
      ...
      BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': CaseId ''' + RTRIM(@c_CaseId) + ''' of Container '''+ RTRIM(@c_Containerkey) +''' is not found at Packdetail or Pickdetail. (ispMBRTK10)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
         GOTO QUIT_SP 
      END         	               	     	  
      ...
      BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Staging lane not yet assigned to the MBOL (ispMBRTK10)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
         GOTO QUIT_SP 
      END         	
      ...
      BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Container Detail not found for the MBOL (ispMBRTK10)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
         GOTO QUIT_SP 
      END         	            
   END  
   ...
   BEGIN
      ...
      BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Release Move Task Failed. MBOL Is Not Populated Yet. (ispMBRTK10)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
         GOTO QUIT_SP 
      END         
      ...
      BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Release Move Task Failed. Container Order Not Split Yet. (ispMBRTK10)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
         GOTO QUIT_SP 
      END         
   END
         ...
         BEGIN      
           INSERT TASKDETAIL  
            ...
            BEGIN
                ...
                SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert Taskdetail Failed. (ispMBRTK10)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
            END            
         END
...
BEGIN
 ...
 BEGIN
  ...
  END
 END
 execute nsp_logerror @n_err, @c_errmsg, 'ispMBRTK10'
