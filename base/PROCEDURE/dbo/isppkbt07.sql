...
/* Written by: CSCHONG                                                  */
/*                                                                      */
/* Purpose: WMS-17994 - [CN] Honma_Bartender_Carton_Label               */
/*        :                                                             */
/* Called By:                                                           */
/*          :                                                           */
/*        :                                                             */
...
/* 29-SEP-2021 CSCHONG  1.0   Devops scripts combine                    */
/* 11-OCT-2021 CSCHONG  1.1   Created(WMS-17994)                        */     
/************************************************************************/

CREATE PROCEDURE [dbo].[ispPKBT07]
...
BEGIN  
   ...
   DECLARE @n_continue        INT 
         ...
         , @c_OHUDF01         NVARCHAR(20)
         , @c_Getlabeltype    NVARCHAR(30)
   ...
   END
    ...
    SET @c_OHUDF01 = ''
    SET @c_Getlabeltype = @c_labeltype

   SELECT @c_OHUDF01 = LTRIM(RTRIM(OH.Userdefine01))
   FROM ORDERS OH WITH (NOLOCK) 
   WHERE OH.OrderKey = @c_Orderkey 
  
  --IF ISNULL(@c_OHUDF01,'') <> ''
  --BEGIN
    IF ISNULL(@c_OHUDF01,'') <> '' AND  EXISTS (SELECT 1 FROM dbo.BartenderLabelCfg (NOLOCK) WHERE Key05 = @c_OHUDF01 AND Storerkey = @c_Storerkey)
    BEGIN
          SET @c_Getlabeltype = 'CTNMRKLBL'
    END
  --END 
  --ELSE  --IF EXISTS (SELECT 1 FROM dbo.BartenderCmdConfig WITH (NOLOCK) WHERE Type02 = @c_Storerkey AND Type03 = @c_labeltype)
  --BEGIN
  --      SET @c_Getlabeltype = @c_labeltype
  --END

   IF LTRIM(RTRIM(@c_Getlabeltype)) IN ('CTNMARKLBL','CTNMRKLBL')
   BEGIN
      EXEC isp_BT_GenBartenderCommand       
               @cPrinterID = @c_PrinterID
            ,  @c_LabelType = @c_Getlabeltype
            ,  @c_userid = @c_UserId
   ...
   BEGIN
      SET @b_success = 0
      EXECUTE nsp_logerror @n_Err, @c_ErrMsg, 'ispPKBT07'  
