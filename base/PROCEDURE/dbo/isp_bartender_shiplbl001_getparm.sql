...
/******************************************************************************/                 
/* Copyright: LFL                                                             */                 
/* Purpose: isp_Bartender_SHIPLBL001_GetParm                                  */   
/*          Copy from isp_Bartender_Shiplabel_GetParm and modify              */                
/*                                                                            */                 
/* Modifications log:                                                         */                 
/*                                                                            */                 
...
CREATE PROC [dbo].[isp_Bartender_SHIPLBL001_GetParm]                      
(  @parm01            NVARCHAR(250),              
   @parm02            NVARCHAR(250),              
   @parm03            NVARCHAR(250),              
   @parm04            NVARCHAR(250),              
  ...
  DECLARE @d_Trace_StartTime   DATETIME,   
   ...
   SET @c_Trace_ModuleName = ''  
    ...
    ELSE IF EXISTS (SELECT 1 FROM PACKHEADER PH WITH (NOLOCK)  
               WHERE PH.pickslipno = @parm02 AND PH.Storerkey = @parm01)
    BEGIN
       SET @c_printUCClabel = 'Y' 
       SET @c_UCCOption2 = 'Y'
    END
    ...
    BEGIN
          ...
          BEGIN
             SET @c_SQLJOIN = 'SELECT PARM1= @parm01,PARM2=P.OrderKey,PARM3= @parm03 ,PARM4= CASE WHEN @c_getParm07 = ''S'' THEN ''0'' ELSE @parm04 END,' +
    	                        'PARM5='''',PARM6='''',PARM7=@c_getParm07, '+
    		                     'PARM8='''',PARM9='''',PARM10='''',Key1=''LoadKey'',Key2=''OrderKey'', ' +
                              ' Key3 = CASE WHEN ISNULL(Ord.Type,'''') = ''VIP'' THEN ''VIP'' ELSE ''NONVIP'' END, Key4 = ISNULL(Ord.DocType,''''), '+
    		                     ' Key5= @parm03 '  +  
       ...
       BEGIN 
          SET @c_SQLGroup = ' GROUP BY P.OrderKey,Ord.shipperkey,ISNULL(Ord.Type,''''),ISNULL(Ord.DocType,'''') '
    ...
    ELSE
    BEGIN
       IF @c_UCCOption1 = 'Y' AND @c_UCCOption2 = 'N'
       BEGIN
          SET @c_SQL = N'SELECT DISTINCT PARM1=OH.Loadkey, PARM2=PH.Orderkey,PARM3=OH.Shipperkey,PARM4=''0'' '
                       +',PARM5=@Parm02,PARM6=@Parm03,PARM7=OH.ECOM_SINGLE_Flag ,PARM8='''',PARM9='''',PARM10='''' '
                       +',Key1=''LoadKey'',Key2=''OrderKey'',Key3 = CASE WHEN ISNULL(OH.Type,'''') = ''VIP'' THEN ''VIP'' ELSE ''NONVIP'' END ' 
                       +',Key4 = ISNULL(OH.DocType,''''), Key5 = ISNULL(OH.Shipperkey,'''') '
                       + 'FROM PACKHEADER PH WITH (NOLOCK) '
       ...
       END
       ELSE IF @c_UCCOption2 = 'Y' AND @c_UCCOption1 = 'N'
       BEGIN
          SET @c_SQL = N'SELECT DISTINCT PARM1=OH.Loadkey, PARM2=PH.Orderkey,PARM3=OH.Shipperkey,PARM4=''0'' '
                       +',PARM5=@Parm03,PARM6=@Parm03,PARM7=OH.ECOM_SINGLE_Flag ,PARM8='''',PARM9='''',PARM10='''' '
                       +',Key1=''LoadKey'',Key2=''OrderKey'',Key3 = CASE WHEN ISNULL(OH.Type,'''') = ''VIP'' THEN ''VIP'' ELSE ''NONVIP'' END ' 
                       +',Key4 = ISNULL(OH.DocType,''''), Key5 = ISNULL(OH.Shipperkey,'''') '
                       + 'FROM PACKHEADER PH WITH (NOLOCK) '
                       + 'JOIN PACKDETAIL PD WITH (NOLOCK) ON (PH.PickSlipNo = PD.PickslipNo) '
                       ...
                       +' AND PH.Pickslipno = @Parm02 AND PD.CartonNo >= CONVERT(INT,@Parm03) AND PD.CartonNo <= CONVERT(INT,@Parm03) '
       END
    END     
