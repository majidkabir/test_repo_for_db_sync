...
/************************************************************************/
/* Store Procedure:  isp_SCE_DL_GENERIC_INVMOVES_RULES_100007_10        */
/* Creation Date: 05-Sep-2022                                           */
/* Copyright: LFL                                                       */
/* Written by: WLChooi                                                  */
/*                                                                      */
/* Purpose: WMS-20470 - Perform ToPackKey Checking                      */
/*                                                                      */
/*                                                                      */
/* Usage:  @c_InParm1 = '1' Perform NULL ToPackKey Checking             */
/*         @c_InParm2 = '1' Perform Exist ToPackKey Checking            */
/*                                                                      */
...
CREATE PROCEDURE [dbo].[isp_SCE_DL_GENERIC_INVMOVES_RULES_100007_10] (
   @b_Debug       INT            = 0
 , @n_BatchNo     INT            = 0
 , @n_Flag        INT            = 0
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         SET STG_Status = '3'
           , STG_ErrMsg = LTRIM(RTRIM(ISNULL(STG_ErrMsg, ''))) + '/ToPackKey is Null'
         WHERE STG_BatchNo   = @n_BatchNo
         AND   STG_Status      = '1'
         AND   (
                ToPackKey IS NULL
         ...
         BEGIN
            ...
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(5), ISNULL(@n_ErrNo, 0))
                            + ': Update record fail. (isp_SCE_DL_GENERIC_INVMOVES_RULES_100007_10)'
            ROLLBACK
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               SET @c_ttlMsg += N'/ToPackKey(' + @c_PackKey + N') not exists in Pack'
            END
            ...
            BEGIN
               SET @c_ttlMsg += N'/ToPackKey(' + @c_PackKey + N') not exists in SKU '
            END
         END
         ELSE
         BEGIN
   ...
   IF @b_Debug = 1
   BEGIN
      SELECT '<<SUB-SP-RULES>> - [isp_SCE_DL_GENERIC_INVMOVES_RULES_100007_10] EXIT... ErrMsg : ' + ISNULL(RTRIM(@c_ErrMsg), '')
   END
