...
/************************************************************************/
/* Store procedure: isp_WSITF_GeekPlusRobot_Generic_SKU_Outbound        */
...
/* 2020-05-19     Alex           Initial - Jira #WMS-13304              */
/************************************************************************/
CREATE PROC [dbo].[isp_WSITF_GeekPlusRobot_Generic_SKU_Outbound](         
...
BEGIN
   ...
   WHERE InterfaceName = 'SKU_OUTBOUND' AND StorerKey = @c_StorerKey
   ...
   BEGIN
       ...
       SET @c_ErrMsg = 'NSQL' 
                     ...
                     + ': [dbo].[GEEKPBOT_INTEG_CONFIG](InterfaceName=SKU_OUTBOUND, StorerKey='
                     + @c_StorerKey + ') is not setup.(isp_WSITF_GeekPlusRobot_Generic_SKU_Outbound)'
       GOTO QUIT
   END
   ...
   BEGIN
      ...
      BEGIN
         ...
         SET @c_ErrMsg = 'NSQL' 
                       + CONVERT(NVARCHAR(6),@n_Err) 
                       + ': failed to update transmitflag of transmitlog3.(isp_WSITF_GeekPlusRobot_Generic_SKU_Outbound)'
         GOTO QUIT
      END
      FETCH NEXT FROM GEEKPLUS_SKUOUT_REUPD_TL INTO @c_TransmitLogKey
   END
   ...
   BEGIN
      IF @b_Debug = 1
      BEGIN
         PRINT '>>>>>>>> isp_WSITF_GeekPlusRobot_Generic_SKU_Outbound - BEGIN'
      ...
      BEGIN
         ...
         BEGIN
            ...
            SET @c_ErrMsg = 'NSQL' 
                          + CONVERT(NVARCHAR(6),@n_Err) 
                          + ': failed to generate transaction id - ' + @c_ErrMsg + '.(isp_WSITF_GeekPlusRobot_Generic_SKU_Outbound)'
            GOTO QUIT
         END
      ...
      BEGIN
         ...
         SET @c_ErrMsg = 'NSQL' 
                       + CONVERT(NVARCHAR(6),@n_Err) 
                       + ': failed to get storerconfig - RBSTDATA.(isp_WSITF_GeekPlusRobot_Generic_SKU_Outbound)'
         GOTO QUIT
      END
      --construct json .body
      ...
      BEGIN
         ...
         BEGIN TRY
            ...
            SET @c_SQLParams = N'@c_warehouse_code NVARCHAR(16), @c_user_id NVARCHAR(16), '
                             + N'@c_user_key NVARCHAR(16), @c_owner_code NVARCHAR(16), '
                             + N'@c_SC_RBSTDDATA_Value NVARCHAR(5), '
                             + N'@c_StorerKey NVARCHAR(15), @c_FullRequestString NVARCHAR(MAX) OUTPUT'
         ...
         BEGIN
            ...
            SET @c_ErrMsg = 'NSQL' 
                          + CONVERT(NVARCHAR(6),@n_Err) 
                          + ': failed to get dbo.TCPSocket_OUTLog.(isp_WSITF_GeekPlusRobot_Generic_SKU_Outbound)'
            GOTO QUIT
         END
   ...
   BEGIN
      ...
      PRINT '>>>>>>>> isp_WSITF_GeekPlusRobot_Generic_SKU_Outbound - END'
   END 
