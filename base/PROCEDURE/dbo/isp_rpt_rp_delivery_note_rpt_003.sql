...
GO
/***************************************************************************/          
/* Stored Procedure: isp_RPT_RP_DELIVERY_NOTE_RPT_003                      */          
...
/* Purpose: WMS-18912                                                      */          
/*                                                                         */          
/* Called By: rpt_rp_delivery_note_rpt_003                                 */          
/*                                                                         */          
/* GitLab Version: 1.0                                                     */          
/*                                                                         */          
/* Version: 1.0                                                            */          
...
/* 26-OCT-2022  Mingle  1.1   Performance tune(ML02)                       */     
/***************************************************************************/      
      
CREATE   PROCEDURE [dbo].[isp_RPT_RP_DELIVERY_NOTE_RPT_003]      
                                           @c_orderkey_Start        NVARCHAR(10),      
                                           @c_Orderkey_End          NVARCHAR(10),      
                                           @dt_OrderDate_start      DATETIME = NULL,      
                                           @dt_OrderDate_End        DATETIME =NULL,      
                                           @dt_DeliveryDate_start   DATETIME =NULL,      
                                           @dt_DeliveryDate_End     DATETIME =NULL,      
...
BEGIN      
 ...
 CREATE TABLE #TMP_DELNOTE54RPT        
      ( Rowref      INT NOT NULL IDENTITY(1,1) PRIMARY KEY --ML02      
 ...
 --SELECT @dt_OrderDateStart '@dt_OrderDateStart'      
  ...
  IF EXISTS (SELECT 1 FROM ORDERS WITH (NOLOCK)        
   ...
   SELECT DISTINCT OH.OrderKey,OH.StorerKey,ISNULL(S.Company,'') ,ISNULL(ST.Contact1,'')      
   ...
   WHERE OH.Orderdate  >= CASE WHEN ISNULL( @dt_OrderDate_Start,'') <> '' THEN  @dt_OrderDate_Start ELSE OH.Orderdate END      
...
SELECT       
            ...
            AND   Long = 'rpt_rp_delivery_note_rpt_003'      
            AND   ISNULL(Short,'') <> 'N'      
           ...
           AND   CLK.Long='rpt_rp_delivery_note_rpt_003'      
  ...
  GROUP BY TDN54ORD.STCompany,TDN54ORD.Storerkey,OH.ConsigneeKey,OH.C_Company,TDN54ORD.Orderkey,      
   ...
   FETCH NEXT FROM CUR_DN54GETOTHSKU INTO @n_rowref --ML02     
...
END      
   ...
   --AND  SKU = @c_sku      
   --AND Storerkey = @c_GetStorerkey       WHERE Rowref = @n_rowref --ML02      
        
   FETCH NEXT FROM CUR_DN54GETOTHSKU INTO @n_rowref       
   END          
