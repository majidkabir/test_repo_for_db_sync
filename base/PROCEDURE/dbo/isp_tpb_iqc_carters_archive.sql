...
GO
/************************************************************************/
/* Stored Procedure: isp_TPB_IQC_Carters                                */
/* Creation Date:                                                       */
/* Copyright: IDS                                                       */
/* Written by:                                                          */
/*                                                                      */
/* Purpose: TPB billing for CHN Carter InventoryQC Transaction          */
/*                                                                      */
...
/* 07-Jul-18    TLTING    1.2   LOT_LOTTABLE_01, LOT_LOTTABLE_02        */
/************************************************************************/

CREATE PROC [dbo].[isp_TPB_IQC_Carters_Archive]
...
BEGIN
   ...
   SET @c_SQLStatement =
      ...
      'FROM ' + @c_ArchiveDB + '.dbo.V_InventoryQC IQC WITH (NOLOCK) ' +
      'JOIN ' + @c_ArchiveDB + '.dbo.V_InventoryQCDetail IQCD WITH (NOLOCK) ON IQCD.QC_Key = IQC.QC_Key ' +
      'JOIN dbo.Facility WITH (NOLOCK) ON Facility.Facility = IQC.To_facility ' +  -- ??
      'JOIN dbo.LOTATTRIBUTE L WITH (NOLOCK) ON L.lot = IQCD.FromLot ' +
      'WHERE IQC.FinalizeFlag = ''Y'' ' +
      'AND CONVERT(VARCHAR, IQC.EditDate, 112) = CONVERT(VARCHAR, @d_Fromdate, 112) ' +
      @c_SQLCondition
