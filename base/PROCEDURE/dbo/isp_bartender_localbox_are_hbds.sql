...
/******************************************************************************/
/* Copyright: MAERSK                                                          */
/* Purpose: isp_Bartender_LocalBox_ARE_HBDS		                              */
/*                                                                            */
/* Modifications log:                                                         */
/*                                                                            */
/* Date       Rev  Author     Purposes                                        */
/* 2024-11-07 1.0  BCA117     Created(FCR-1111)			                      */
/******************************************************************************/

CREATE   PROC [dbo].[isp_Bartender_LocalBox_ARE_HBDS]
...
AS
BEGIN
print '-----isp_Bartender_LocalBox_ARE_HBDS----'
...
print '---------------'
   ...
   SELECT TOP 1
	  ...
	  ,@OrderNo			= ISNULL(OD.ExternOrderKey,'')
	  ,@PONo			= ISNULL(OD.ExternPOKey,'')
      ,@DeliveryNo		= ISNULL(OD.ConsigneeKey,'')
	  ,@DeliveryName	= ISNULL(OD.notes,'')
	  ,@Address			= ISNULL(OD.C_Address1,'')+ISNULL(OD.C_Address2,'')+ISNULL(OD.C_Address3,'')+ISNULL(OD.C_Address4,'')
      ,@Mail			= ISNULL(OD.notes,'')
	  ,@City			= ISNULL(OD.C_City,'') +' '+ISNULL(OD.C_Country,'')
      ,@BoxNo           = ''
      ,@Remarks         = ''
   ...
   SELECT ISNULL(@Description,'') AS Col01
         ,ISNULL(@OrderNo,'') AS Col02
         ,ISNULL(@PONo,'') AS Col03
         ,ISNULL(@DeliveryNo,'') AS Col04
         ,ISNULL(@DeliveryName,'') AS Col05
         ,ISNULL(@Address,'') AS Col06
         ,ISNULL(@Mail,'') AS Col07
         ,ISNULL(@City,'') AS Col08
         ,ISNULL(@BoxNo,'') AS Col09
		 ,ISNULL(@Remarks,'') AS Col010
		 ,'' AS Col11
		 ,'' AS Col12
