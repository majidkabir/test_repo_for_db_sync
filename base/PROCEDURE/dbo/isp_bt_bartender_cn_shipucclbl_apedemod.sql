...
/********************************************************************************/                   
/* Copyright: LFL                                                               */                   
/* Purpose: isp_BT_Bartender_CN_SHIPUCCLBL_APEDEMOD                             */                   
/*                                                                              */                   
/* Modifications log:                                                           */                   
/*                                                                              */                   
/* Date       Rev  Author     Purposes                                          */                   
/* 2021-07-19 1.0  WLChooi    Created (WMS-17511)                               */  
/********************************************************************************/                  
                    
CREATE PROC [dbo].[isp_BT_Bartender_CN_SHIPUCCLBL_APEDEMOD]                        
...
BEGIN                        
  ...
  DECLARE  @d_Trace_StartTime   DATETIME,     
   ...
   SELECT TOP 1 @c_GetOrderkey = ORDERS.Orderkey,
                @c_Col02       = SUBSTRING(LTRIM(RTRIM(ISNULL(ORDERS.C_State,''))),1,4), 
                @c_Col03       = '(' + SUBSTRING(LTRIM(RTRIM(ISNULL(ORDERS.C_City,''))),1,4) + ')', 
                @c_Col05       = LTRIM(RTRIM(ISNULL(ORDERS.ConsigneeKey,''))),   
                @c_Col06       = LTRIM(RTRIM(ISNULL(ORDERS.C_Company,''))), 
                @c_Col07       = LTRIM(RTRIM(ISNULL(ORDERS.C_Phone2,''))), 
                @c_Col08       = ISNULL(ORDERS.Notes,''),
                @c_Col09       = LTRIM(RTRIM(ISNULL(ORDERS.C_Address1,''))),
                @c_Col10       = LEFT(LTRIM(RTRIM(ISNULL(ORDERS.C_Address2,''))) + ' ' + LTRIM(RTRIM(ISNULL(ORDERS.C_Address3,''))) + ' ' + LTRIM(RTRIM(ISNULL(ORDERS.C_Address4,''))),80),
                @c_Col11       = LTRIM(RTRIM(ISNULL(ORDERS.C_contact1,''))),
                @c_Col12       = LTRIM(RTRIM(ISNULL(ORDERS.C_Phone1,''))),
   ...
   SELECT @c_Col08 = LEFT(LTRIM(RTRIM(@c_Col08)), 80)
   ...
   CREATE TABLE [#Result] (               
      ...
      SET @c_SQLJOIN = + ' SELECT DISTINCT @c_GetOrderkey, @c_Col02, '
                       + ' @c_Col03, ' + CHAR(13) --3
                       + ' @c_AllExtOrderkey, '
                       + ' @c_Col05, ' + CHAR(13) --5
                       + ' @c_Col06, ' + CHAR(13) --6
                       + ' @c_Col07 ,@c_Col08 ,@c_Col09 ,@c_Col10 , ' + CHAR(13) --10   
                       + ' @c_Col11 ,@c_Col12 ,PD.LabelNo ,'''' ,'''' ,'''' ,'''' ,'''' ,'''' ,'''' ,'  + CHAR(13) --20         
                       + ' '''' ,'''' ,'''' ,'''' , ' + CHAR(13) --24
                       + ' '''' ,'''' ,'''' ,'''' ,'''' ,'''', ' + CHAR(13) --30
                       + ' '''' ,PD.CartonNo ,'''' ,'''' ,'''' ,'''' ,'''' ,'''' ,'''' ,'''' ,'  + CHAR(13) --40
                       + ' '''' ,'''' ,'''' ,'''' ,'''' ,'''' ,'''' ,'''' ,'''' ,'''' , ' + CHAR(13) --50         
                       + ' '''' ,'''' ,'''' ,'''' ,'''' ,'''' ,'''' ,PH.Pickslipno ,'''' ,''CN'' '  --60         
                       + CHAR(13) +              
                       ...
                       + ' GROUP BY PD.LabelNo, PH.Pickslipno, PD.CartonNo' + CHAR(13)  
                       + @c_Sorting  
      ...
      SELECT DISTINCT Col13, Col58, CAST(Col32 AS INT)      
      FROM #Result   
      WHERE Col60 = 'CN'  
      ORDER BY Col58, CAST(Col32 AS INT)  
      ...
      BEGIN  
         ...
         SELECT @n_SumQty = SUM(CASE WHEN ISNUMERIC(t.Col16) = 1 THEN CAST(t.Col16 AS INT) ELSE 0 END + 
                                CASE WHEN ISNUMERIC(t.Col19) = 1 THEN CAST(t.Col19 AS INT) ELSE 0 END + 
                                CASE WHEN ISNUMERIC(t.Col22) = 1 THEN CAST(t.Col22 AS INT) ELSE 0 END + 
                                CASE WHEN ISNUMERIC(t.Col25) = 1 THEN CAST(t.Col25 AS INT) ELSE 0 END + 
                                CASE WHEN ISNUMERIC(t.Col28) = 1 THEN CAST(t.Col28 AS INT) ELSE 0 END + 
                                CASE WHEN ISNUMERIC(t.Col31) = 1 THEN CAST(t.Col31 AS INT) ELSE 0 END 
                               )
         FROM #Result t 
         WHERE t.Col58 = @c_Pickslipno  
         AND t.Col13 = @c_LabelNo  
         AND t.Col32 = @c_CartonNo  
         AND t.Col34 = @n_CurrentPage
  
         UPDATE #Result  
         SET Col33 = @n_SumQty  
         WHERE Col58 = @c_Pickslipno  
         AND Col13 = @c_LabelNo  
         AND Col32 = @c_CartonNo  
...
EXIT_SP:      
   ...
   --   @c_TraceName = 'isp_BT_Bartender_CN_SHIPUCCLBL_APEDEMOD',    
