...
/************************************************************************/  
CREATE PROC [dbo].[ispSEPCDConsoLQ7]  
...
BEGIN  
   ...
   BEGIN  
      SELECT TOP 1 @n_TotalExpiryDay = DATEDIFF(DAY, @d_today, l.Lottable04)  
      FROM dbo.LOTATTRIBUTE AS l WITH (NOLOCK)  
      WHERE l.Storerkey = @c_Storerkey  
      AND l.Sku = @c_Sku  
      ORDER BY l.Lottable04 DESC  
   END  
  
   IF @b_debug IN (1,9)  
   BEGIN  
      PRINT '---------------------------------------'+ CHAR(13) +  
            'Sub PICKCOde: ispSEPCDConsoLQ7'+ CHAR(13) +  
  ...
  ;WITH ORD AS  
   ...
   WHERE  OD.Storerkey = @c_Storerkey  
      ...
      AND ISNULL(OD.MinShelfLife,0) + OH.OrderInfo04 <= @n_TotalExpiryDay
   ORDER BY OH.Loadkey  
         ,  OD.Orderkey  
         ,  OD.OrderLineNumber  
   ...
   SELECT TOP 1 @n_MinShelflife = o.ShelfLife              
   FROM #ORDLINE AS o  
   ORDER BY o.ShelfLife   
         ...
         BEGIN  
            ...
            BEGIN  
               ...
               SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0))   
                              + ': Get PickDetailKey Failed. (ispSEPCDConsoLQ7)'  
               GOTO QUIT_SP  
            END  
            ...
            BEGIN  
               ...
               SET @n_Err = 81220  
               SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),ISNULL(@n_Err,0))   
                              + ': Insert PickDetail Failed. (ispSEPCDConsoLQ7)'  
               GOTO QUIT_SP  
            END  
...
QUIT_SP:  
   ...
   BEGIN  
      ...
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'ispSEPCDConsoLQ7'  
