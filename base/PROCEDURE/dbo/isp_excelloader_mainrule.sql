...
GO
/*************************************************************************/    
/* Stored Procedure: isp_ExcelLoader_MainRule                            */    
...
CREATE PROCEDURE [dbo].[isp_ExcelLoader_MainRule](    
    @n_BatchNo          INT               = 0
   ,@n_EIMOpID          INT               = 0
...
BEGIN
   ...
   BEGIN TRY
      ...
      WHERE Flag = 1 AND EIMOpID = @n_EIMOpID 
      ORDER BY Step ASC
   ...
   BEGIN CATCH
      ...
      SET @c_ErrMsg = LTRIM(RTRIM(ERROR_MESSAGE())) + ' (isp_ExcelLoader_MainRule)'    
      IF @b_Debug = 1    
      BEGIN    
         PRINT '[isp_ExcelLoader_MainRule]: Execute ' + @c_Bs_SubRule_SP + ' SP Failed...'     
               + ' @c_ErrMsg=' + @c_ErrMsg    
      END    
      GOTO QUIT 
   END CATCH
