...
/***************************************************************************/    
/* Stored Procedure: ispTransferAllocation03                               */    
/* Creation Date: 13-Apr-2021                                              */    
/* Copyright: LFL                                                          */    
/* Written by: WLChooi                                                     */    
/*                                                                         */    
/* Purpose: WMS-16740 - [CN] Mannings_WMS_TransferAllocation               */    
/*        : Assumption: fromsku = tosku                                    */   
/*        : Duplicate and Modify from ispTransferAllocation02              */  
/*                                                                         */    
...
BEGIN    
   ...
   BEGIN    
      ...
      BEGIN    
         ...
         SET @c_errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Transfer(s) is not allowed to re-allocate. (ispTransferAllocation03)'    
         GOTO QUIT_SP               
      END    
   END    
   ...
   BEGIN    
      ...
      SET @c_errmsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_err)+': PUTZone Does not setup in Codelkup. (ispTransferAllocation03)'    
      GOTO QUIT_SP               
   END    
   ...
   BEGIN    
      ...
      BEGIN   
         ...
         ORDER BY CASE WHEN LOC.LocationType = 'DYNPPICK' AND LOC.LocationCategory = 'DPP' THEN 10    
                       WHEN LOC.LocationType = 'CASE' AND LOC.LocationCategory = 'BULK' THEN 20  
                       WHEN LOC.LocationType NOT IN ('DYNPPICK','CASE')  AND LOC.LocationCategory = 'BULK' THEN 30   
                       END    
               ,  CASE WHEN LOC.LocationCategory IN ('DPP') THEN LLI.Qty - LLI.QtyAllocated - LLI.QtyPicked - LLI.QtyReplen ELSE 0 END  
               ,  LLI.Loc  
               ,  LLI.ID           
               ,  LLI.Lot  
      ...
      -----------------------------------------------------------  
      -- Calculate PickMethod for TaskDetail.CaseiD <> '' - START  
      ----------------------------------------------------------- 
      ...
      AND t.SourceType = 'ispTransferAllocation03'  
      ...
      BEGIN      
         ...
         BEGIN  
            ...
            BEGIN  
               ...
               SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': UPDATE TASKDETAIL Failed. (ispTransferAllocation03)'    
                              + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
               GOTO NEXT_TRF    
            END  
         END  
      ...
      -----------------------------------------------------------  
      -- Calculate PickMethod for TaskDetail.CaseiD <> '' - END  
      ----------------------------------------------------------- 
      ...
      --      SET @b_Success = 0    
      --      SET @n_err  = 81060    
      --      SET @c_errmsg = 'Execute ispPostFinalizeTransferWrapper Failed. (ispTransferAllocation03)'    
      --                     + '(' + @c_errmsg + ')'    
      --      GOTO NEXT_TRF  
      --   END  
      --END  
      ...
      BEGIN    
         ...
         SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': UPDATE TRANSFER Failed. (ispTransferAllocation03)'    
                        + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '    
         GOTO NEXT_TRF    
      END    
      ...
      BEGIN    
         ...
         EXEC nspLogAlert    
               @c_modulename       = 'ispTransferAllocation03'    
   ...
   BEGIN    
      ...
      AND   Code = 'ispTransferAllocation03'    
      AND StorerKey = @c_FromStorerkey    
    
      IF ISNULL(@cRecipients, '') <> ''    
      BEGIN    
         SET @cSubject = 'Mannings Transfer Allocation Alert - ' + REPLACE(CONVERT(NVARCHAR(19), GETDATE(), 126),'T',' ')    
        ...
        EXEC msdb.dbo.sp_send_dbmail    
         ...
         BEGIN    
            SET @n_err = 81220    
            SET @c_Errmsg = 'Error executing sp_send_dbmail. (ispTransferAllocation03)'    
