...
/***************************************************************************************************/        
/* Store Procedure:  isp_BackendOrdersPendHang                                                       */        
...
/* 2022-Apr-20 TLTING         1.0   Initial version   (Copy from isp_BackendCancelOrders)          */        
/*                                                                                                 */        
/***************************************************************************************************/        
        
CREATE PROC [dbo].[isp_BackendOrdersPendHang] (        
...
BEGIN        
   ...
   DECLARE @c_PickSlipNo               NVARCHAR(10)        
         ...
         , @n_BackendOrdersPendHang_Exist INT
   ...
   SET @c_ConsoOrderKey            = ''     
   SET @n_BackendOrdersPendHang_Exist = 0     
   ...
   --kocy02 (s)    
   SELECT @n_BackendOrdersPendHang_Exist = (1)         
   FROM  StorerConfig WITH (NOLOCK)        
   WHERE StorerKey = @c_Storerkey        
   AND (ConfigKey = 'BackendOrdersPendHang')     
   AND SVALUE = '1'    
   --kocy02 (e)    
   ...
   BEGIN        
      ...
      BEGIN        
         ...
         --OR (@c_ConsoOrd_OrdSts = '5' AND @c_ConsoOrd_SOSts = 'PENDHANG')        
      ...
      --   OR (@c_SC_WSFORCECANCORD_SValue = '1') --(CY01)        
         ...
         BEGIN        
            ...
            SET EditWho = SUSER_NAME()        
               , EditDate = GETDATE()        
               , Trafficcop = NULL        
               , SOStatus = @c_StatusPENDHANG         
            WHERE Orderkey = @c_ConsoOrderKey        
            AND Storerkey = @c_Storerkey        
            AND SOStatus <> @c_StatusPENDHANG        
           -- AND Status <> @c_StatusPENDHANG        
            SET @n_Err = @@ERROR        
            IF @n_Err <> 0        
            BEGIN        
               ROLLBACK TRAN        
   ...
   BEGIN        
      ...
      --OR (@c_OH_Status = '5' AND @c_OH_SOStatus = 'PENDHANG')        
 ...
 --  END        
   ...
   IF @n_WSOrdCancMoveToLoc_Exist = 1 OR @n_WSOrdCancMoveToLoc_Task_Exist = 1    --(MC01)        
      ...
      OR @n_BackendOrdersPendHang_Exist = 1 -- kocy02    
   BEGIN           	  
      SET @c_SC_SValue1 = ''        
...
*/        
   ...
   SET EditWho = SUSER_NAME()        
      , EditDate = GETDATE()        
      , Status = '0'  -- @c_StatusCANC  -- @c_OrderStatus        
     -- , SOStatus = @c_StatusCANC  -- @c_SOStatus         
   ...
   BEGIN TRAN
      ...
      VALUES ( @c_TransmitLogKey, 'WSDSTRLOG', @c_Orderkey, 'REJEC', @c_StorerKey , '0' )     
      SET @n_Err = @@ERROR        
      IF @n_Err <> 0        
      BEGIN        
         ROLLBACK TRAN        
      ...
      BEGIN        
         ...
         BEGIN        
            ...
            IF   exists ( SELECT top 1 1        
                  ...
                  AND (SOStatus = 'PENDHANG' ) ) --  OR Status = @c_StatusCANC ) )         
            BEGIN        
               GOTO NEXT_CHILD_ORD        
            END        
            ...
            SET EditWho = SUSER_NAME()         
               ...
               , OrderGroup = ''     
               , Status = '0'
               , SOStatus = 'PENDHANG'   -- '0'  --release        
               , TrafficCop = NULL        
               , Issued = 'Y'         
               , UserDefine04 = ''         
            WHERE Orderkey = @c_DB_ChildOrderKey        
            AND StorerKey = @c_StorerKey        
            AND SOStatus <> 'PENDHANG'         
            --AND Status <> @c_StatusCANC         
            SET @n_Err = @@ERROR        
            IF @n_Err <> 0        
            BEGIN        
