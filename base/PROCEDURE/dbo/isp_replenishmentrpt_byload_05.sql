...
GO
/************************************************************************/
/* Store Procedure:  isp_ReplenishmentRpt_ByLoad_05                     */
/* Creation Date: 24-APR-2019                                           */
/* Copyright: LF                                                        */
/* Written by: CSCHONG                                                  */
/*                                                                      */
/* Purpose: WMS-9132 - [TW] Replenishment Report CR                     */
/*                                                                      */
/* Usage:                                                               */
/*                                                                      */
/* Called By: r_replenishment_by_load05                                 */
/*                                                                      */
/* PVCS Version: 1.0                                                    */
/*                                                                      */
/* Version: 5.4                                                         */
...
/************************************************************************/
CREATE PROC [dbo].[isp_ReplenishmentRpt_ByLoad_05]
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Insert into replenishment table failed. (isp_ReplenishmentRpt_ByLoad_05)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '
         END
      END -- IF @b_success = 1
      FETCH NEXT FROM CUR_Replen INTO @c_FromLoc, @c_FromID, @c_ToLoc, @c_Sku, @n_FromQty, @c_Storerkey, @c_FromLot, @c_PackKey, @c_Priority, @c_UOM
   END -- While
...
EXIT_SP:
   ...
   BEGIN
      ...
      BEGIN
         ...
         END
      END
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'isp_ReplenishmentRpt_ByLoad_05'
   ...
   SELECT DISTINCT 
          ...
          ORD.Sectionkey, 
          L1.loc, L1.facility
          ,(Row_Number() OVER (PARTITION BY R.loadkey ORDER BY R.Sku Asc))  as Seqno  
