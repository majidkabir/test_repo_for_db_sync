...
/* 2023-May-19  kelvinongcy   extend @c_exist & @c_exist1 length  (kocy02) */
/***************************************************************************/               
CREATE    PROCEDURE [dbo].[nspBuildAlterTableString_GENERIC]              
...
BEGIN               
 ...
 DECLARE @c_exist             NVARCHAR(300)  --kocy02                
 DECLARE @c_exist1            NVARCHAR(300)  --kocy02             
 DECLARE @c_whereclause       NVARCHAR(255)                   
 DECLARE @c_inclen            NVARCHAR(300)              
 DECLARE @n_status            tinyint                                    
 DECLARE @n_prec              int                                      
 DECLARE @n_scale             int                                      
 DECLARE @user_type           smallint                     
 DECLARE @n_inclen_flag       int              
 DECLARE @c_Stringlength      NVARCHAR(10),          
 ...
 SELECT @c_Stringlength = ''             
 SELECT @c_exist1 = RTRIM(@c_copyto_db) + '.' + RTRIM(@c_schema) + '.' + RTRIM(@c_tablename)        
 ...
 BEGIN               
  ...
  SELECT @c_errmsg = 'NSQL ' + CONVERT(char(5),@n_err)+':Table does not exist in Target Database ' +              
   @c_tablename + '(dbo.nspBuildAlterTableString_GENERIC)'              
 END                 
 ...
 BEGIN                
  ...
  BEGIN                
   ...
   SELECT @c_errmsg = CONVERT(char(250),@n_err)              
    + ':  Open of cursor failed. (dbo.nspBuildAlterTableString_GENERIC) ' + ' ( ' +              
  ...
  BEGIN                  
   FETCH Next from CUR_INSERT_BUILD into @c_field, @n_length, @n_status, @user_type, @c_typename     
   ...
   BEGIN                
    ...
    SELECT @n_err = 73403              
    SELECT @c_errmsg = CONVERT(char(250),@n_err)              
     + ':  fetch failed. (dbo.nspBuildAlterTableString_GENERIC) ' + ' ( ' +              
     ' SQLSvr MESSAGE = ' + LTRIM(RTRIM(@c_errmsg)) + ')'              
   END                 
    ...
    BEGIN               
     ...
     BEGIN               
      ...
      END    
       ...
       -- TLTING02    
       IF (@c_typename = 'uniqueidentifier')        
       BEGIN           
         SELECT @n_nextrow = @n_nextrow + 1        
         CONTINUE        
       END      
    
      IF ((@c_typename = 'char') OR (@c_typename = 'varchar') OR (@c_typename = 'nchar') OR (@c_typename = 'nvarchar')) --KH01              
      BEGIN              
         IF @n_length = -1            
         BEGIN            
            SET @c_Stringlength = 'MAX'            
         END            
         IF @n_length >= 0            
         BEGIN            
            SET @c_Stringlength = CONVERT(VarCHAR(10),@n_length)            
         END             
         SELECT @c_inclen =  '(' + @c_Stringlength  + ') DEFAULT '''''''' '           
      END              
      ...
      BEGIN              
       SELECT @n_prec = (SELECT prec from sys.syscolumns  , sys.sysobjects where              
              ...
              sys.sysobjects.name =  @c_tablename AND sys.sysobjects .id =  OBJECT_ID(@c_FulltableName) )              
...
IF (@b_debug = 1)              
      ...
      ' EXEC dbo.nspaltertable_GENERIC ''' + RTRIM(@c_schema) + ''','''+  RTRIM(@c_field) + ''',' +              
      RTRIM(convert(char(10),@n_length)) + ',''' + RTRIM(@c_typename) +              
      ''',''' + RTRIM(@c_tablename) +''',''' + RTRIM(@c_copyto_db)   + ''',''' + @c_inclen +''',0,0,'''' '    -- TLTING01 LTRIM(RTRIM(@c_inclen))          
      ...
      BEGIN              
       select @c_exist              
       select @c_exist1              
                     select  @c_field '@c_field', convert(char(10),@n_length) '@n_length',               
      ...
      BEGIN                
         ...
         SELECT @n_err = 73404              
         SELECT @c_errmsg = CONVERT(char(250),@n_err)              
                     + ':  Alter Table Failed. (dbo.nspBuildAlterTableString_GENERIC) ' + ' ( ' +              
                       ' SQLSvr MESSAGE = ' + LTRIM(RTRIM(@c_errmsg)) + ')'              
       BREAK              
      END                 
     END               
 ...
 BEGIN         
  ...
  BEGIN              
   ...
   END              
  END              
  EXECUTE nsp_logerror @n_err, @c_errmsg, 'dbo.nspBuildAlterTableString_GENERIC'              
