...
GO
/************************************************************************/
/* Store procedure: ispIDXLblNoDecode20d                                */
/* Copyright      : IDS                                                 */
/*                                                                      */
/* Purpose: Decode Label No Scanned                                     */
/*          Copied from ispIDTXLblNoDecode02 but this version decode    */  
/*          label with 20 digits                                        */
/* Called from:                                                         */
/*                                                                      */
/* Exceed version: 5.4                                                  */
/*                                                                      */
...
CREATE PROCEDURE [dbo].[ispIDXLblNoDecode20d]
...
BEGIN
   ...
   SET @c_DistCtr = SUBSTRING( RTRIM( @c_LabelNo), 2, 6) 
   SET @c_ConsigneeKey = SUBSTRING( RTRIM( @c_LabelNo), 8, 6)
   SET @c_Section = SUBSTRING( RTRIM( @c_LabelNo), 14, 1) 
   SET @c_Separate = SUBSTRING( RTRIM( @c_LabelNo), 15, 1) 
