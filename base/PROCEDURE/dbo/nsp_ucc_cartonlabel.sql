...
/************************************************************************/  
/* Store Procedure: nsp_UCC_CartonLabel                                 */  
...
/* 20-Apr-2011    Audrey 1.2  SOS#213209 - Extend length cartonNo       */  
/*                            from 3 to 5  (ang01)                      */  
/* 07-MAR-2014    YTWan  1.3  SOS#304694 - Vitec Carton UCC change      */  
/*                            request (Wan01)                           */  
/* 28-Jan-2019  TLTING_ext 1.4 enlarge externorderkey field length      */  
/************************************************************************/  
CREATE PROC [dbo].[nsp_UCC_CartonLabel] (  
...
BEGIN  
   ...
   @c_externorderkey_end   NVARCHAR(50),  --tlting_ext  
   ...
   --(Wan01) - START  
   SET @c_ExecStatements1 = @c_ExecStatements1 + N', FromComp = CASE WHEN ISNULL(RTRIM(STORER.SUSR3),'''') = '''' '  
                          + 'THEN ''IDS'' ELSE ISNULL(RTRIM(STORER.SUSR3),'''') END '   
   --(Wan01) - END  
   ...
   'ORDERS.Type,'+  
     'ORDERS.OrderKey'  
   + ',CASE WHEN ISNULL(RTRIM(STORER.SUSR3),'''') = '''' THEN ''IDS'' ELSE ISNULL(RTRIM(STORER.SUSR3),'''') END '    --(Wan01)  
