...
/* Return Status: NONE                                                  */
/*                                                                      */
/* Usage: Logic must follow original script: ispPopulateTOASN_SUNFLW    */
/*                                                                      */
...
/* 30-Apr-2013  Leong   1.0  SOS# 276517 - Display error message in     */
/*                                         backend job.                 */
/* 27-May-2014  TKLIM   1.1  Added Lottables 06-15                      */
/************************************************************************/

CREATE PROCEDURE [dbo].[ispPopulateTOASN_SUNFLW_2]
...
BEGIN
   ...
   BEGIN
      ...
      BEGIN
         -- SET @n_Continue = 3
         -- SET @n_Err = 63510
         SET @c_ErrMsg = 'NSQL63510: Invalid BillToKey: ' + RTRIM(ISNULL(@c_BillToKey,'')) +
                         '. OrderKey = ' + RTRIM(ISNULL(@c_OrderKey,'')) + '. (ispPopulateTOASN_SUNFLW_2)'
         GOTO QUIT_SP
      END
      ...
      BEGIN
         -- SET @n_Continue = 3
         -- SET @n_Err = 63515
         SET @c_ErrMsg = 'NSQL63515: Storer Config OWITF is turn off: ' + RTRIM(ISNULL(@c_OWITF_Config,'')) +
                         '. OrderKey = ' + RTRIM(ISNULL(@c_OrderKey,'')) + '. (ispPopulateTOASN_SUNFLW_2)'
         GOTO QUIT_SP
      END
      ...
      BEGIN
         ...
         SET @c_ErrMsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_Err)+': ASN ExternReceiptkey Already Exist In System: ' + RTRIM(ISNULL(@c_ExternReceiptkey,'')) +
                         '. ToStorerkey = ' + RTRIM(ISNULL(@c_ToStorerkey,'')) + ', OrderKey = ' + RTRIM(ISNULL(@c_OrderKey,'')) + ' (ispPopulateTOASN_SUNFLW_2)'
      END
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               ...
               BEGIN
                  SET @n_Continue = 3
                  SET @n_Err = 63481
                  SET @c_ErrMsg = 'NSQL' + CONVERT(NVARCHAR(5),@n_Err) + ': Insert Error on Table Receipt (ispPopulateTOASN_SUNFLW_2)'
               END
            END
            ELSE
            BEGIN
               SET @n_Continue = 3
               SET @n_Err = 63502
               SET @c_ErrMsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_Err)+': Generate Receipt Key Failed! (ispPopulateTOASN_SUNFLW_2)'
         ...
         BEGIN
            SET @n_Continue = 3
            SET @n_Err = 63503
            SET @c_ErrMsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_Err)+': To StorerKey is BLANK! (ispPopulateTOASN_SUNFLW_2)'
         END
         ...
         BEGIN
            ...
            BEGIN
               ...
               BEGIN
                  SET @n_Continue = 3
                  SET @n_Err = 63491
                  SET @c_ErrMsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_Err)+': Insert Error on Table Sku (ispPopulateTOASN_SUNFLW_2)'
               END
            END
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               SET @n_Continue = 3
               SET @n_Err = 63492
               SET @c_ErrMsg = 'NSQL'+CONVERT(NVARCHAR(5),@n_Err)+': Update Error on Table Receipt (ispPopulateTOASN_SUNFLW_2)'
            END
         END
      END
   ...
   BEGIN
      ...
      BEGIN
         ...
         END
      END
      EXECUTE nsp_LogError @n_Err, @c_ErrMsg, 'ispPopulateTOASN_SUNFLW_2'
   ...
   BEGIN
      SELECT @b_success = 1
      SELECT @c_ErrMsg = ISNULL(RTRIM(@c_ErrMsg),'')
      WHILE @@TRANCOUNT > @n_StartTCnt
      BEGIN
         COMMIT TRAN
      END
