...
GO
/***************************************************************************/
/* Stored Procedure: isp_GetDespatchManifestUS_DMark                       */
...
CREATE PROC [dbo].[isp_GetDespatchManifestUS_DMark] (
...
BEGIN
   ...
   DECLARE @c_PrintedFlag NVARCHAR(10),@c_PackType1 NVARCHAR(10),@i_TotalCartonsGOH int,
    ...
    IF EXISTS(SELECT 1 FROM DATAMART.ODS.MBOL WITH (NOLOCK) WHERE MBOLKey = @c_mbolkey AND COD_Status = 'P')
    BEGIN
...
--   
--      UPDATE DATAMART.ODS.MBOL WITH (ROWLOCK)
--      SET COD_Status = 'P',
--          TrafficCop = NULL 
--      WHERE MBOLKey = @c_mbolkey 
--  END
   
   IF EXISTS(SELECT 1 FROM DATAMART.ODS.MBOLDETAIL MD (NOLOCK) 
   ...
   BEGIN    
       DECLARE cur_CTNINFO CURSOR LOCAL FAST_FORWARD READ_ONLY FOR  
            ...
            FROM DATAMART.ODS.ORDERS (NOLOCK)
            WHERE Mbolkey = @c_mbolkey
      OPEN cur_CTNINFO  
      ...
      BEGIN
         ...
         FROM [dbo].[fnc_GetVicsBOL_CartonInfo_DMark](@c_mbolkey, @c_consigneekey)
      ...
      SELECT @c_PrintedFlag, 
                MBOL.MBOLKey , 
             CONVERT(NVARCHAR(10), MBOL.Departuredate,101) , -- Vicky01
             ...
             sUser_sName()
      FROM DATAMART.ODS.MBOL MBOL WITH (NOLOCK) 
      INNER JOIN DATAMART.ODS.MBOLDETAIL MBOLDETAIL WITH (NOLOCK) ON ( MBOL.MBOLKey = MBOLDETAIL.MBOLKey )
      INNER JOIN DATAMART.ODS.ORDERS ORDERS WITH (NOLOCK) ON ( MBOLDETAIL.OrderKey = ORDERS.OrderKey )
      INNER JOIN DATAMART.ODS.ORDERDETAIL ORDERDETAIL WITH (NOLOCK) ON ( ORDERS.OrderKey = ORDERDETAIL.OrderKey )
   ...
   BEGIN
      ...
      LEFT JOIN (SELECT  Packheader.Orderkey, COUNT(DISTINCT PackDetail.LabelNo) totalctn
                 ...
                 JOIN DATAMART.ODS.PackHeader PACKHEADER (NOLOCK) ON (MD.Orderkey = Packheader.Orderkey)
                 JOIN   DATAMART.ODS.PackDetail PACKDETAIL (NOLOCK) ON (PackHeader.Pickslipno = PackDetail.Pickslipno)
                  WHERE MD.Mbolkey = @c_mbolkey
