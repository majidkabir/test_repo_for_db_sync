...
/* Written by:                                                          */
/*                                                                      */
/* Purpose: WMS-4808-HK-GBG bebe -PTS Assign from wave                  */
...
BEGIN
   ...
   DECLARE @n_continue       INT
         ...
         , @c_GenLabelNo_SP  NVARCHAR(30) --NJOW02  
   ...
   BEGIN      
       SELECT TOP 1 @c_Userdefine05 = WAVE.Userdefine05, 
                  ...
                  , @c_Storerkey = ORDERS.Storerkey --NJOW02
       FROM WAVE (NOLOCK)
   ...
   ------Validation--------
    ...
    BEGIN          
       ...
       BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Release Failed. Wave Has Been Assigned to PTS Zone. (ispWAVRL04)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
         GOTO RETURN_SP
       END                
       ...
       BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Release Failed. PTS Zone Is In Use. (ispWAVRL04)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
         GOTO RETURN_SP
       END                
       ...
       BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Release Failed. PTS Zone(Userdefine05) Is Not Found. (ispWAVRL04)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
         GOTO RETURN_SP
       END                
       ...
       BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+':  Release Failed. Invalid PLT Method(Userdefine09). (ispWAVRL04)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
         GOTO RETURN_SP
       END                
       ...
       BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Release Failed. Found Open or Shipped Orders In Wave. (ispWAVRL04)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
         GOTO RETURN_SP
       END                
       ...
       BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Release Failed. Missing Pickslip In Wave. (ispWAVRL04)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
         GOTO RETURN_SP
       END                      
       ...
       BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Release Failed. Insufficient PTS Device Location for the Wave. (ispWAVRL04)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '           
         GOTO RETURN_SP
       END                       
    END
   ...
   BEGIN
          ...
          @b_success		OUTPUT,
          @c_GenLabelNo_SP OUTPUT, 
          @n_err		   OUTPUT,
          @c_errmsg		OUTPUT
          
      IF NOT EXISTS (SELECT 1 FROM dbo.sysobjects WHERE name = RTRIM(@c_GenLabelNo_SP) AND type = 'P')
      BEGIN
      	 SET @c_GenLabelNo_SP = ''
      END            
      --NJOW02 End    	             
      ...
      BEGIN           	                    
         ...
         BEGIN
             ...
             SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': PTS Location Not Setup / Not enough PTS Location. (ispWAVRL04)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
         END                   
         
         SET @c_CartonID = ''
         IF ISNULL(@c_GenLabelNo_SP,'') <> ''  --NJOW02
         BEGIN                       	   	         
           SET @c_SQL = 'EXEC ' + @c_GenLabelNo_SP + ' @c_PickSlipNo, @n_CartonNo, @c_LabelNo OUTPUT'
         ...
         INSERT INTO RDT.rdtPTLStationLog (Station, IPAddress, Position, Loc, loadkey, Wavekey, Storerkey, Orderkey, 
                                   ...
                                   VALUES (@c_DeviceId, @c_IPAddress, @c_DevicePosition, @c_PTSLoc, @c_Loadkey, @c_Wavekey, @c_Storerkey, @c_Orderkey, 
                                           @c_Consigneekey, @c_Wavekey, 'ispWAVRL04', @c_Userdefine09, 0, @c_Pickslipno, @c_BatchKey, @c_CartonID)
         ...
         BEGIN
             ...
             SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Insert RTD.rdtPTLStationLog Failed. (ispWAVRL04)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
         END   
...
BEGIN
   ...
   BEGIN
      ...
      END
   END
   execute nsp_logerror @n_err, @c_errmsg, 'ispWAVRL04'
