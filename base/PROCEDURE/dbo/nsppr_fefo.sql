...
/* 30-Sep-2003 Vicky    1.0  Add filter by Facility                     */
/* 30-Sep-2003 Shong    1.1  Bug Fixing                                 */
/* 23-Apr-2004 Shong    1.2  Performance Tuning                         */
/* 07-Jan-2005 Shong    1.3  Change cursor type                         */
/* 14-Dec-2005 MaryVong 1.4  SOS44137 Check @n_StorerMinShelfLife <> 0  */
/* 07-Apr-2008 June		  1.5  SOS99448 : Bug fixing					      */
/* 09-Jul-2012 Shong    1.6  SOS#315773 Take PreAllocatedQty filter by  */
/*                      1.7  Orders.Facility                            */
/* 22-Jul-2015 NJOW01   1.8  347486 - filter by id                      */
/* 28-Aug-2015 CSCHONG  1.9  SOS351309 (CS01)                           */
/* 12-Nov-2015 NJOW02   2.0  Fix preallocated qty by id not correct     */
/* 12-Sep-2017 TLTING   2.1  Dynamic SQL recompile                      */
/* 23-Apr-2018 NJOW03   2.2  WMS-4777 configure to remove sku.facility  */
/* 24-Apr-2019 WLCHOOI  2.3  WMS-8411-Minus LOTxLOCxID.QTYREPLEN (WL01) */
/* 02-Apr-2020 WLChooi  2.4  WMS-12776-New Config: SortLotByMinLoc(WL02)*/
/************************************************************************/

CREATE PROC [dbo].[nspPR_FEFO]
...
BEGIN
   ...
   DECLARE @c_Orderkey         NVARCHAR(10),
           ...
           @c_SortLotByMinLoc  NVARCHAR(30),    --WL02  
           @c_SortCondition    NVARCHAR(4000),  --WL02    
           @c_GroupCondition   NVARCHAR(4000)   --WL02                            
   ...
   --WL02 START
   SELECT @c_SortLotByMinLoc = Code   
   FROM CODELKUP (NOLOCK)  
   WHERE Listname = 'PKCODECFG'  
   AND Storerkey = @c_Storerkey  
   AND Long = 'nspPR_FEFO'  
   AND Code = 'SortLotByMinLoc'
   AND ISNULL(Short,'') <> 'N'

   IF @c_SortLotByMinLoc = 'SortLotByMinLoc'  
   BEGIN
      SET @c_SortCondition = ' ORDER BY Lotattribute.Lottable04, LOC.LogicalLocation, LOC.Loc '
      SET @c_Condition = ' AND ((LOTxLOCxID.QTY) - (LOTxLOCxID.QTYALLOCATED) - (LOTxLOCxID.QTYPICKED) - (LOTxLOCxID.QTYREPLEN) - ISNULL(P.QTYPREALLOCATED,0) ) > 0 '
      SET @c_GroupCondition = ' , LOC.LogicalLocation, LOC.Loc  '
   END
   ELSE
   BEGIN
      SET @c_SortCondition = ' ORDER BY Lotattribute.Lottable04, LOT.Lot '
      SET @c_GroupCondition = ''
   END
   --WL02 END
   ...
   BEGIN
      ...
      SELECT @c_SQLStatement =  " DECLARE  PREALLOCATE_CURSOR_CANDIDATES CURSOR FAST_FORWARD READ_ONLY FOR " +
            " SELECT LOT.STORERKEY, LOT.SKU, LOT.LOT, " +
            " QTYAVAILABLE = (SUM(LOTxLOCxID.QTY) - SUM(LOTxLOCxID.QTYALLOCATED) - SUM(LOTxLOCxID.QTYPICKED) - SUM(LOTxLOCxID.QTYREPLEN) - MAX(ISNULL(P.QTYPREALLOCATED,0)) )  " + --WL01
            ...
            " GROUP By LOT.STORERKEY, LOT.SKU, LOT.LOT, Lotattribute.Lottable04 " + @c_GroupCondition +
            " HAVING SUM(LOTxLOCxID.QTY) - SUM(LOTxLOCxID.QTYALLOCATED) - SUM(LOTxLOCxID.QTYPICKED) - SUM(LOTxLOCxID.QTYREPLEN) - MAX(ISNULL(P.QTYPREALLOCATED,0)) >= " + CAST(@n_UOMBase AS VARCHAR(10)) + --WL01
            --" ORDER BY Lotattribute.Lottable04, LOT.Lot " --WL02
            ISNULL(RTRIM(@c_SortCondition),'')  --WL02
