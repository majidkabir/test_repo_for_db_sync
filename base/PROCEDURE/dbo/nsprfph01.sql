...
BEGIN
   ...
   IF @b_debug = 1
   BEGIN
      SELECT @c_storerkey, @c_sku, @c_id, @c_lot, @c_loc, @n_qty, @c_uom, @c_packkey
   END
   ...
   BEGIN
      ...
      BEGIN
         SELECT @n_continue = 3
         SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err=70201   -- Should Be Set To The SQL Errmessage but I don't know how to do so.
         SELECT @c_errmsg="NSQL"+CONVERT(char(5), @n_err)+": Select Failed On LOTxLOCxID. (nspRFPH01)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
      END
   ELSE IF @n_cnt = 0
      BEGIN
   ...
   IF @n_continue=1 or @n_continue=2
   BEGIN
      IF NOT @n_Qty = @n_SystemQty
      BEGIN
         SELECT @c_retrec = "02"
         SELECT @c_errmsg = "Cycle Count Difference"
      END
   END
   IF @n_continue=3
   BEGIN
   ...
   ELSE
      BEGIN
         ...
         END
      END
      execute nsp_logerror @n_err, @c_errmsg, "nspRFPH01"
