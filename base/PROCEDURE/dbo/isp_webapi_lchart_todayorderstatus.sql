...
GO
/************************************************************************/                
/* Store procedure: isp_WebAPI_LCHART_TodayOrderStatus                  */                
...
/************************************************************************/      
CREATE PROC [dbo].[isp_WebAPI_LCHART_TodayOrderStatus](  
     @b_Debug           INT            = 0  
   , @c_Format          VARCHAR(10)    = ''  
...
BEGIN  
   ...
   IF OBJECT_ID('tempdb..#LCHART_SAMPLEDATA') IS NOT NULL  
   DROP TABLE #LCHART_SAMPLEDATA  
  
   CREATE TABLE #LCHART_SAMPLEDATA(  
      [RowRefNo] INT IDENTITY(1,1),  
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         SET @c_ExecStatements = N'SET @c_ResponseString = ISNULL(RTRIM(( SELECT StorerKey '  
                               + '       ,[Status]=CASE [Status] WHEN ''0'' THEN ''0 - NORMAL'' '  
                               + '               WHEN ''1'' THEN ''1 - PARTIALLY ALLOCATED'' '   
                               + '               WHEN ''2'' THEN ''2 - FULLY ALLOCATED'' '  
                               + '               WHEN ''3'' THEN ''3 - IN PROCESS'' '  
