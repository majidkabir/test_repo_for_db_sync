...
GO
/************************************************************************/
/* Stored Proc: isp_RPT_RP_DespLBL03                                    */
...
/* Called By: r_dw_rpt_rp_despllbl03                                    */
/*          :                                                           */
/* PVCS Version: 1.0                                                    */
/*                                                                      */
/* Version: 7.0                                                         */
...
/************************************************************************/
CREATE   PROC [dbo].[isp_RPT_RP_DespLBL03]
...
BEGIN
   ...
   CREATE TABLE #TMP_DESPLBL
      ...
      ,  Userdefine09         NVARCHAR(20)   NOT NULL DEFAULT('')
      ,  Notes2               NVARCHAR(255)  NOT NULL DEFAULT('')
      ,  S_Company            NVARCHAR(45)   NOT NULL DEFAULT('')
      ,  Division             NVARCHAR(255)  NOT NULL DEFAULT('')
   ...
   SELECT o.Orderkey
       ...
       ,  Userdefine09   = ISNULL(o.Userdefine09  ,'') 
       ,  Notes2         = ISNULL(o.Notes2        ,'') 
       ,  S_Company      = ISNULL(s.Company       ,'') 
       ,  Division       = ISNULL(c.[Description] ,'')
       ,  DL.CartonNo          
   FROM DL
   JOIN dbo.ORDERS AS o WITH (NOLOCK) ON o.OrderKey = DL.Orderkey
   JOIN dbo.STORER AS s WITH (NOLOCK) ON (s.Storerkey = o.Storerkey)
   LEFT OUTER JOIN dbo.CODELKUP AS c WITH (NOLOCK) ON  c.Listname = 'LORORDDIV' 
                                                   AND o.ExternOrderkey LIKE '__' + RTRIM(c.Code) + '%' 
