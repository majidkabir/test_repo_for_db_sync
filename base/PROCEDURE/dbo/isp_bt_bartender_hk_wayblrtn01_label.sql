...
/******************************************************************************/                 
/* Copyright: LFL                                                             */                 
/* Purpose: WMS-13809 - HK - SFHK Return Waybill Label (Bartender)            */                 
/*        : Copy from isp_BT_Bartender_HK_WAYBLBLN01_label                    */ 
/*                                                                            */         
/* Modifications log:                                                         */                 
/*                                                                            */                 
/* Date           Rev  Author     Purposes                                    */    
/* 2020-06-17     1.0  WLChooi    Created (WMS-13809)                         */           
/******************************************************************************/                
                  
CREATE PROC [dbo].[isp_BT_Bartender_HK_WAYBLRTN01_label]                       
...
BEGIN                      
   ...
   SET @c_SQLJOIN = +' SELECT DISTINCT orders.trackingno,orders.c_contact1,'  
                    ...
                    +' '''','''','        --15  
                    +' '''','  
                    +' ISNULL(oi.orderinfo09,''''),'           --CS01  
                    +' ISNULL(oi.orderinfo10,''''),'           --CS01  
                    ...
                    +' '''','''',orders.Externorderkey,orders.orderkey,ISNULL(Orders.RTNTrackingNo,''''),'''','''','''','''','''', '  --35    
                    +' '''','''','''','''','''','''','''','''','''','''' ,'''','''','''','''','''','   --50  
                    +' '''','''','''','''','''','''','''','''','''','''' '                              --60'  
   ...
   BEGIN     
      ...
      SELECT DISTINCT ph.pickslipno,ph.orderkey,pd.sku,s.descr,sum(pd.qty),'N'   
      FROM packheader ph WITH (nolock)    
      JOIN packdetail pd WITH (nolock) on pd.pickslipno = ph.pickslipno  
      JOIN sku s WITH (NOLOCK) ON S.StorerKey = pd.StorerKey and s.Sku = pd.SKU  
      WHERE ph.storerkey = @c_Storerkey  
      AND ph.OrderKey = @c_orderkey  
      GROUP BY ph.pickslipno,ph.orderkey,pd.sku,s.descr  
...
EXIT_SP:    
   ...
   EXEC isp_InsertTraceInfo   
      ...
      @c_TraceName = 'isp_BT_Bartender_HK_WAYBLRTN01_label',  
