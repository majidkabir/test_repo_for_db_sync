...
/******************************************************************************/
/* Stored Procedure: ispWaveDynamicPickUCCAlloc_BySKU                         */
...
/* 19-Mar-2010  Leong      1.0   Bug Fix: change GetKey from REPLENISHMENT to */
/*                                        REPLENISHKEY (leong01)              */
/* 21-Mar-2014  TLTING     1.1   Bug fix SQL2012                              */
/* 20-May-2014  TKLIM      1.2   Change dbo.fnc_RTRIM to RTRIM                */
/* 20-May-2014  TKLIM      1.2   Added Lottables 06-15                        */
/* 15-Dec-2018  TLTING01   1.3   Missing nolock                               */
/******************************************************************************/

CREATE PROCEDURE [dbo].[ispWaveDynamicPickUCCAlloc_BySKU]
   ...
   IF @n_continue=1 OR @n_continue=2
   BEGIN
      SELECT TOP 1 @c_B2D_ReplenGrp = ReplenishmentGroup
      FROM Replenishment WITH (NOLOCK)
      WHERE Replenno = @c_WaveKey
         AND Remark = 'BULK to DP'
      IF ISNULL(RTRIM(@c_B2D_ReplenGrp),'') = ''
      BEGIN
         EXECUTE nspg_GetKey
             @keyname       = 'REPLENISHGROUP',
             @fieldlength   = 10,
   ...
   IF @n_continue=1 OR @n_continue=2
   BEGIN
      SELECT TOP 1 @c_B2P_ReplenGrp = ReplenishmentGroup
      FROM Replenishment WITH (NOLOCK)
      WHERE Replenno = @c_WaveKey
         AND Remark = 'BULK to PP'
      IF ISNULL(RTRIM(@c_B2P_ReplenGrp),'') = ''
      BEGIN
        EXECUTE nspg_GetKey
             @keyname       = 'REPLENISHGROUP',
             @fieldlength   = 10,
   ...
   IF @n_continue=1 OR @n_continue=2
   BEGIN
      SELECT TOP 1 @c_P2D_ReplenGrp = ReplenishmentGroup
      FROM Replenishment WITH (NOLOCK)
      WHERE Replenno = @c_WaveKey
         AND Remark = 'PP to DP'
      IF ISNULL(RTRIM(@c_P2D_ReplenGrp),'') = ''
      BEGIN
        EXECUTE nspg_GetKey
             @keyname       = 'REPLENISHGROUP',
             @fieldlength   = 10,
   ...
   SET @c_PrevPickSlipNo = ''
      ...
      BEGIN
         SELECT @n_continue=3
         SELECT @n_err = 68001
         SELECT @c_errMsg = 'DELETE WaveOrderLn Failed (ispWaveDynamicPickUCCAlloc_BySKU)'
         GOTO RETURN_SP
      END
      ...
      BEGIN
         SELECT @n_continue=3
         SELECT @n_err = 68002
         SELECT @c_errMsg = 'LoadKey not Found, Please Generate Load 1st (ispWaveDynamicPickUCCAlloc_BySKU)'
         GOTO RETURN_SP
      END
     ...
     -- At least one DynamicPick Location exists
      ...
      BEGIN
         SELECT @n_continue=3
         SELECT @n_err = 68003
         SELECT @c_errMsg = @c_DPLoc_Start + ', DynamicPick Location Not Found (ispWaveDynamicPickUCCAlloc_BySKU)'
         GOTO RETURN_SP
      END
      ...
      BEGIN
         SELECT @n_continue=3
         SELECT @n_err = 68004
         SELECT @c_errMsg = 'Update UCC Failed (ispWaveDynamicPickUCCAlloc_BySKU)'
         GOTO RETURN_SP
      END
      ...
      BEGIN
         SELECT @n_continue=3
         SELECT @n_err = 68005
         SELECT @c_errMsg = 'Delete Replenishment Failed (ispWaveDynamicPickUCCAlloc_BySKU)'
         GOTO RETURN_SP
      END
   ...
   --         SET @n_CartonNo = 0
         ...
         BEGIN
            ...
            SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Creation/Opening of Candidate Cursor Failed! (ispWaveDynamicPickUCCAlloc_BySKU)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTRIM(RTRIM(@c_errmsg)) + ' ) '
            GOTO RETURN_SP
         END
         ELSE
         BEGIN
   ...
   --------------------------------
                  ...
                  BEGIN
                     ...
                     BEGIN
                        ...
                        BEGIN
                           ...
                           SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Insert PickDetail Failed! (ispWaveDynamicPickUCCAlloc_BySKU)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTRIM(RTRIM(@c_errmsg)) + ' ) '
                           CLOSE Cur_LLI
                           DEALLOCATE Cur_LLI
                           GOTO RETURN_SP
                        END
                     ...
                     BEGIN
                        ...
                        BEGIN
                           ...
                           SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Update WaveOrderLn Failed (ispWaveDynamicPickUCCAlloc_BySKU)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTRIM(RTRIM(@c_errmsg)) + ' ) '
                           CLOSE Cur_LII
                           DEALLOCATE Cur_LII
                           GOTO RETURN_SP
                        END
                  ...
                  BEGIN
                     SELECT @n_continue = 3
                     SELECT @n_err = 63501
                     SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Insert Into PACKDETAIL Failed. (ispWaveDynamicPickUCCAlloc_BySKU)'
                     CLOSE Cur_LII
                     DEALLOCATE Cur_LII
                     GOTO RETURN_SP
                  END
                  ...
                  BEGIN
                     ...
                     BEGIN
                        ...
                        SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Update UCC Failed (ispWaveDynamicPickUCCAlloc_BySKU)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTRIM(RTRIM(@c_errmsg)) + ' ) '
                        CLOSE Cur_LII
                        DEALLOCATE Cur_LII
                        GOTO RETURN_SP
                     END
               ...
               EXECUTE nspg_GetKey
                     ...
                     BEGIN
                        ...
                        BEGIN
                           ...
                           SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Insert Replenishment Failed! (ispWaveDynamicPickUCCAlloc_BySKU)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTRIM(RTRIM(@c_errmsg)) + ' ) '
                           CLOSE Cur_LII
                           DEALLOCATE Cur_LII
                           GOTO RETURN_SP
                        END
   ...
   --   END
      ...
      BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Delete WaveOrderLn Failed (ispWaveDynamicPickUCCAlloc_BySKU)' + ' ( ' + ' SQLSvr MESSAGE=' + dbo.fnc_LTRIM(RTRIM(@c_errmsg)) + ' ) '
         GOTO RETURN_SP
      END
    ...
    --           select @c_sku '@c_sku', @n_LOT_Qty '@n_LOT_Qty', @n_QtyLeftToFulfill '@n_QtyLeftToFulfill', @n_AllocateQty '@n_AllocateQty'
               ...
               BEGIN
                  ...
                  BEGIN
                     ...
                     SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Dynamic Pick Location Not Found or FULL ! (ispWaveDynamicPickUCCAlloc_BySKU)'
                     CLOSE PickCursor
                     DEALLOCATE PickCursor
                    GOTO RETURN_SP
                  END
         ...
         WHERE TR.PickSeq = @n_PickSeq
              ...
              JOIN LOTxLOCxID (NOLOCK)
               ...
               BEGIN
                  ...
                  SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Permanent Pick Location Not Setup For SKU: ' + RTRIM(@c_SKu) + '! (ispWaveDynamicPickUCCAlloc_BySKU)'
                  GOTO RETURN_SP
               END
   ...
   */
                  ...
                  BEGIN
                     ...
                     BEGIN
                        ...
                        SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)
                              +': Dynamic Pick Location Not Found or FULL ! (ispWaveDynamicPickUCCAlloc_BySKU)'
                        CLOSE PickCursor
                        DEALLOCATE PickCursor
                        GOTO RETURN_SP
                     END
   ...
   BEGIN
      ...
      BEGIN
         ...
         END
      END
      execute nsp_logerror @n_err, @c_errmsg, 'ispWaveDynamicPickUCCAlloc_BySKU'
