...
'main' ,    --end itemclass
'0',        --start skugroup
'ZZZZZZZZZZZ',   --end skugroup
'23/08/2000', --start date
'24/08/2000' --end date

note
code_descr : taken from codelkup table
...
BEGIN
   ...
   BEGIN
      ...
      a.StorerKey
      , b.itemclass
      -- requested to change by suilan
      , code_descr = (select description from codelkup where code = b.skugroup and listname = 'skugroup')
      , a.sku
      , a.qty
      ...
      */
      ...
      BEGIN
         ...
         SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Insert Failed On #ITRN_CUT_BY_SKU (BSM) (nsp_smr_1)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
   ...
   BEGIN
      ...
      BEGIN
         ...
         a.StorerKey
         , c.itemclass
         ,  (select description from codelkup where code = c.skugroup and listname = 'skugroup')
         , a.Sku
         , a.ArchiveQty
         , a.archiveqty * c.stdcube
         , TranType = "DP"
         , convert(datetime,convert(char(10),ArchiveDate,101))
         FROM lot a(nolock), sku c(nolock)
         WHERE a.sku = c.sku
         AND a.archiveqty > 0
         AND EXISTS
   ...
   GROUP BY storerkey, itemclass, code_descr  /**/
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Summary Insert Failed On #BF_TEMP (nsp_smr_1)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
            /* End Trap SQL Server Error */
         END
      END /* if @n_num_recs > 0 */
   END /* continue and stuff */
   ...
   */
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            /* Trap SQL Server Error */
            SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err=74802   -- Should Be Set To The SQL Errmessage but I don't know how to do so.
            SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Summary Insert Failed On #BF_TEMP3a (nsp_smr_1)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
            /* End Trap SQL Server Error */
         END
   ...
   BEGIN
      ...
      , RunningTotal
      FROM #BF
      ORDER BY StorerKey, itemclass, code_descr
   END
   ...
   DECLARE @code_descr NVARCHAR(60)
   DECLARE @Qty int
   ...
   DECLARE @prev_code_descr NVARCHAR(60)
   ...
   , TranType
   FROM #BF
   ORDER BY StorerKey, itemclass, code_descr, EffectiveDate')
   ...
   BEGIN
      ...
      SELECT @prev_code_descr = @code_descr
      ...
      IF (@storerkey <> @prev_storerkey AND @itemclass <> @prev_itemclass AND @code_descr <> @prev_code_descr)
      BEGIN
         IF (@b_debug = 1)
         BEGIN
            select 'prev_storerkey', @prev_storerkey,
            'prev_itemclass', @prev_itemclass,
            'prev_code_descr', @prev_code_descr
         END
   ...
   WHERE a.storerkey = b.storerkey
   group by b.company, a.storerkey, a.itemclass, a.code_descr
   ...
   declare @date_range NVARCHAR(32)
   select @date_range = "From " +dbo.fnc_RTrim(convert(char(2), day(@datemin))) + "k" +
   dbo.fnc_RTrim(convert(char(2), month(@datemin))) + "/" +
   convert(char(4), year(@datemin)) +
   "To " + dbo.fnc_RTrim(convert(char(2), day(@datemax))) + "/" +
   dbo.fnc_RTrim(convert(char(2), month(@datemax))) + "/" +
   convert(char(4), year(@datemax))
