...
/******************************************************************************/
/* Stored Procedure: nspTTMEvaluateNMFTasks                                   */
/* Purpose:                                                                   */
/*                                                                            */
/* Modification log:                                                          */
...
CREATE PROC [dbo].[nspTTMEvaluateNMFTasks]
...
DeclareCursor_NMFTaskCandidates:
   ...
   IF @n_err = 16915
   BEGIN
       CLOSE Cursor_NMFTaskCandidates
       DEALLOCATE Cursor_NMFTaskCandidates
       GOTO DeclareCursor_NMFTaskCandidates
   END
   ...
   BEGIN
      SET @n_Continue = 3
      SET @n_Err = 88201
      SET @c_errmsg = 'NMFCodeError'
      GOTO Quit
   END

   OPEN Cursor_NMFTaskCandidates
   SELECT @n_err = @@ERROR
   ...
   -- Get a task
   SET @c_TaskDetailKey = ''
   FETCH NEXT FROM Cursor_NMFTaskCandidates INTO @c_TaskDetailKey
   ...
   BEGIN
      ...
      -- Get storer config
      SET @cSwapTask = rdt.rdtGetConfig( 1746, 'SwapTask', @c_StorerKey)
      ...
      BEGIN
         ...
         -- Get storer config
         SET @cGetSuggestedLOCSP = rdt.rdtGetConfig( @nFunc, 'GetSuggestedLOCSP', @c_StorerKey)
         ...
         BEGIN
            IF EXISTS( SELECT 1 FROM dbo.sysobjects WHERE name = @cGetSuggestedLOCSP AND type = 'P')
            BEGIN
               DECLARE @cSQL      NVARCHAR(1000)
               DECLARE @cSQLParam NVARCHAR(1000)
               SET @cSQL = 'EXEC rdt.' + RTRIM( @cGetSuggestedLOCSP) + ' @nMobile, @nFunc, @cLangCode, @cUserName, @cStorerKey, @cFacility, ' + 
                  ' @cTaskdetailKey, @cFromLOC, @cID, @cSuggestedLOC OUTPUT, @cPickAndDropLOC OUTPUT, @nErrNo OUTPUT, @cErrMsg OUTPUT'
               SET @cSQLParam =
                  '@nMobile          INT,                  ' + 
                  '@nFunc            INT,                  ' + 
                  '@cLangCode        NVARCHAR( 3),         ' + 
               ...
               EXEC sp_ExecuteSQL @cSQL, @cSQLParam, @nMobile, @nFunc, @cLangCode, @c_userid, @c_StorerKey, @c_Facility,
                  @c_TaskDetailKey, 
                  @c_FromLOC, 
                  @c_FromID, 
                  @c_SuggestedLOC   OUTPUT, 
                  @c_PickAndDropLOC OUTPUT, 
                  @n_Err            OUTPUT, 
                  @c_ErrMsg         OUTPUT
...
Quit:
   -- Close cursor
   IF CURSOR_STATUS( 'global', 'Cursor_NMFTaskCandidates') IN (0, 1) -- 0=empty, 1=record
      CLOSE Cursor_NMFTaskCandidates
   IF CURSOR_STATUS( 'global', 'Cursor_NMFTaskCandidates') IN (-1)   -- -1=cursor is closed
      DEALLOCATE Cursor_NMFTaskCandidates
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
             ...
             END
         END
         EXECUTE nsp_logerror @n_err, @c_errmsg, 'nspTTMEvaluateNMFTasks'
