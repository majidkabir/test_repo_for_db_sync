...
--    Contact         NVARCHAR(60)  null,    
 ...
 AND   CODE = 'IncFontSize'    
   ...
   WHERE ORDERS.Mbolkey = @c_Mbolkey    
    ...
    JOIN ORDERDETAIL k WITH (NOLOCK) ON c.orderkey = k.orderkey
    JOIN SKU l WITH (NOLOCK) ON k.storerkey = l.storerkey AND k.sku = l.sku
    JOIN PICKDETAIL m  WITH (NOLOCK) ON k.orderkey = m.orderkey AND k.sku = m.sku AND k.orderlinenumber = m.OrderLineNumber 
    JOIN LOTATTRIBUTE n WITH (NOLOCK) ON m.lot = n.lot
  ...
  SET @c_Brand = ''    
      ...
      --BEGIN    
         ...
         SELECT @n_Weight = ISNULL(CASE WHEN @n_Weight > 0 THEN @n_Weight ELSE SUM(PD.Qty * S.StdGrossWgt) END,0)    
               --,@n_Cube   = ISNULL(SUM(PD.Qty * S.StdCube),0)                                                      
