...
GO
/************************************************************************/
/* Store Procedure:  isp_ReplenishmentRpt_ByLoad_03                     */
/* Creation Date: 06-Apr-2012                                           */
/* Copyright: LF                                                        */
/* Written by: NJOW                                                     */
/*                                                                      */
/* Purpose: 239826 - PUMA Load Replenishment Report                     */
/*                                                                      */
/* Usage:                                                               */
/*                                                                      */
...
/* 11-Sep-2013  YTWan    1.2  SOS#288777:Show enter Putawayzone & toloc */
/*                            record (Wan01)                            */
/* 09-Mar-2015  NJOW01   1.3  331291-Replen qty determine by order      */
/*                            percentage per style & color              */
/************************************************************************/
CREATE PROC [dbo].[isp_ReplenishmentRpt_ByLoad_03]
...
BEGIN
   ...
   DECLARE CUR_ColorStyle CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
      SELECT SKU.Storerkey, SKU.Style, SKU.Color, Orders.Facility, 
             SUM(ORDERDETAIL.OpenQty) AS OrderQty --NJOW01
      FROM ORDERS (NOLOCK)
      ...
      GROUP BY SKU.Storerkey, SKU.Style, SKU.Color, Orders.Facility --NJOW01
      ORDER BY Orders.Facility, SKU.Storerkey, SKU.Style, SKU.Color
    
   OPEN CUR_ColorStyle
   FETCH NEXT FROM CUR_ColorStyle INTO @c_Storerkey, @c_Style, @c_color, @c_Facility, @n_OrderQty
   ...
   BEGIN
      ...
      BEGIN         	  
         DECLARE CUR_Replen_Inv CURSOR LOCAL FAST_FORWARD READ_ONLY FOR              
              SELECT LLI.Sku, LLI.Lot, LLI.Loc, LLI.ID,                                 
         ...
         OPEN CUR_Replen_Inv
         FETCH NEXT FROM CUR_Replen_Inv INTO @c_Sku, @c_fromlot, @c_fromloc, @c_fromid, @n_fromqty, @c_packkey, @c_UOM
      ...
      BEGIN
         ...
         FETCH NEXT FROM CUR_SKU INTO @c_Sku, @n_OrderQty
   ...
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            ...
            SELECT @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Insert into replenishment table failed. (isp_ReplenishmentRpt_ByLoad_03)' + ' ( ' + ' SQLSvr MESSAGE=' + LTRIM(RTRIM(@c_errmsg)) + ' ) '
         END
      END -- IF @b_success = 1
      FETCH NEXT FROM CUR_Replen INTO @c_FromLoc, @c_FromID, @c_ToLoc, @c_Sku, @n_FromQty, @c_Storerkey, @c_FromLot, @c_PackKey, @c_Priority, @c_UOM
   END -- While
...
EXIT_SP:
   ...
   BEGIN
      ...
      BEGIN
         ...
         END
      END
      EXECUTE nsp_logerror @n_err, @c_errmsg, 'isp_ReplenishmentRpt_ByLoad_03'
