...
/********************************************************************************/                           
/* Copyright: IDS                                                               */                           
/* Purpose: isp_Bartender_Shipper_Label_JP_HM                                   */                           
/*                                                                              */                           
/* Modifications log:                                                           */                           
/*                                                                              */                           
/* Date       Rev  Author     Purposes                                          */                           
/* 2020-04-09 1.0  CSCHONG    Created (WMS-12831)                               */       
/* 2020-05-12 1.1  WLChooi    WMS-13302 - Modify to InsuredAmount / 100 (WL01)  */  
/* 2021-03-29 1.2  CSCHONG    WMS-16286 revised col21 logic (CS02)              */      
/********************************************************************************/                          
                            
CREATE PROC [dbo].[isp_Bartender_Shipper_Label_JP_HM]                                
...
BEGIN                                
   ...
   DECLARE                            
      ...
      @n_CntRec          INT,              
      @c_SQL             NVARCHAR(4000),            
      @c_SQLParm         NVARCHAR(4000),           --tlting01          
      ...
      @c_col21           NVARCHAR(80),   --CS02     
      @c_ORDRDD          NVARCHAR(20)    --CS02
...
DECLARE @d_Trace_StartTime  DATETIME,             
   ...
   SET @c_SQLJOIN = + N' SELECT DISTINCT (N''〒'' + Substring(ORD.C_Zip,1,3) + ''-'' + Substring(ORD.C_Zip,4,4)),ORD.c_phone1,'           
                   ...
                   -- + 'PARSENAME(CONVERT(Money, CASE WHEN IsNumeric(ORD.UserDefine05)=1 THEN CAST(ORD.UserDefine05  AS INT) ELSE 0 END, 1),2),'          
                    + ' CASE WHEN ORD.pmtterm = ''GO'' THEN CAST((OI.InsuredAmount / 100) AS NVARCHAR(80)) ELSE ORD.userdefine05 END ,'   --CS02   --WL01
                    ...
                    + ' STO.Notes2,'''',C1.Description,C2.long,CONVERT(VARCHAR(10), getdate(),112),'          
                  ...
                  --  + ' (ORD.C_State + Space(1) + ORD.C_City + Space(1) + ORD.C_Address2 ) AS Add2, '''','''','''','''','''','''','''','''','''', '  --50  --WL02          
                    + ' ORD.C_City,ORD.c_contact1,ORD.c_contact2,ISNULL(ORD.notes,''''),ISNULL(ORD.notes2,''''),'  
                    +'  ISNULL(ORD.C_address1,''''),ISNULL(ORD.C_address3,''''),ISNULL(ORD.C_address4,''''),ISNULL(ORD.C_Phone2,''''),ISNULL(ORD.C_address2,''''),' --40  --CS01 --CS02 
                    + ' '''', '''','''','''','''','''','''','''','''','''', '  --50  --WL02  
                    + ' '''','''','''','''','''','''','''','''','''', '''' '   --60                    
                    + CHAR(13) +                      
                    ...
                    + ' LEFT JOIN CODELKUP C2 WITH (NOLOCK) ON C2.Listname = ''HMCARTON'' AND C2.UDF01 = CT.CarrierName AND C2.short=PI.Cartontype'          
 ...
 END          
      ...
      BEGIN          
         ...
         SELECT @c_VATAmt = CASE WHEN ORD.Pmtterm='GO' THEN                                                       
                                 CASE WHEN ISNUMERIC(STO.PercentA) = 1 AND ISNUMERIC(ORD.UserDefine05) = 1                  
                                 THEN CONVERT(INT,( (OI.insuredamount / 100) / ((STO.PercentA / 100) + 1) ) )            --WL01          
                                 ELSE 0 END
                            ELSE CASE WHEN ISNUMERIC(STO.PercentA) = 1 AND ISNUMERIC(ORD.UserDefine05) = 1  
     ...
     --CS02 START
      ...
      SELECT @c_ORDRDD = O.Rdd
      FROM ORDERS O WITH (NOLOCK)
      WHERE O.OrderKey=@c_orderkey

      IF @c_ORDRDD = 'O' 
      BEGIN
          SELECT @c_col21 = CAST(STUFF((SELECT ',' + RTRIM(S.notes1) 
                             FROM SKU S WITH (NOLOCK)          
                             JOIN PACKDETAIL PADET WITH (NOLOCK) ON PADET.SKU = S.SKU          
                             JOIN PACKHEADER PAH WITH (NOLOCK) ON PAH.Pickslipno = PADET.Pickslipno          
                             WHERE PAH.Orderkey = @c_orderkey 
                             GROUP BY RTRIM(S.notes1) 
                             ORDER BY RTRIM(S.notes1) FOR XML PATH('')),1,1,'' ) AS NVARCHAR(250))
      END 

         IF @b_debug=1                  
         BEGIN  
             SELECT @c_ORDRDD '@c_ORDRDD',@c_col21 '@c_col21'
         END
     --CS02 END 
      ...
      SET  col22 =  @c_GetField22,          
           col27 =  @c_VATAmt,
           col21 = CASE WHEN ISNULL(@c_col21,'') <> '' THEN @c_col21 ELSE Col21 END       --CS02    
...
EXIT_SP:              
   ...
   EXEC isp_InsertTraceInfo             
      ...
      @c_TraceName = 'isp_Bartender_Shipper_Label_JP_HM',            
