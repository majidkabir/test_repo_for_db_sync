...
BEGIN
   ...
   IF dbo.fnc_LTrim(dbo.fnc_RTrim(@c_lot)) IS NOT NULL
      ...
      BEGIN
         ...
         BEGIN
            SELECT @c_Condition = dbo.fnc_RTrim(@c_Condition) + " AND ( Lotattribute.Lottable04 >= N'" + dbo.fnc_RTrim(CONVERT(char(10), @d_Lottable04, 112)) + "' ) " 
         END
         ELSE
         BEGIN
         ...
         " AND LOC.Facility = N'" + @c_facility + "' " +
		  ...
		  -- " AND SKUxLOC.LocationType NOT IN ('PICK', 'CASE') " +  -- 12-Oct-2004 YTWAN Fixed if overallocated for lot in pick face
