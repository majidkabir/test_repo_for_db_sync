...
/************************************************************************/
/* Stored Procedure: nspTTMEvaluateXDTasks                              */
/* Creation Date:                                                       */
/* Copyright: IDS                                                       */
/* Written by:                                                          */
/*                                                                      */
...
CREATE PROC    [dbo].[nspTTMEvaluateXDTasks]
...
BEGIN
   ...
   /* #INCLUDE <SPEVXD_1.SQL> */
   IF @n_continue = 1 or @n_continue = 2
   BEGIN
      DECLAREcursor_XDTASKCANDIDATES:
      ...
      BEGIN
         SELECT @n_continue = 3
         SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err=79501   -- Should Be Set To The SQL Errmessage but I don't know how to do so.
         SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Execute Of Move Tasks Pick Code Failed. (nspTTMEvaluateXDTasks)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
      END
      IF @n_err = 16915
      BEGIN
         CLOSE cursor_XDTASKCANDIDATES
         DEALLOCATE cursor_XDTASKCANDIDATES
         GOTO DECLAREcursor_XDTASKCANDIDATES
      END
      OPEN CURSOR_XDTASKCANDIDATES
   ...
   BEGIN
      ...
      BEGIN
         FETCH NEXT FROM cursor_XDTASKCANDIDATES INTO @c_taskdetailkey
         IF @@FETCH_STATUS = -1
         BEGIN
            BREAK
         END
         ...
         BEGIN
            ...
            BEGIN
               ...
               BEGIN
                  BEGIN TRANSACTION
                     ...
                     BEGIN
                        SELECT @n_continue = 3
                        SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err=79502   -- Should Be Set To The SQL Errmessage but I don't know how to do so.
                        SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Error Occurred While Attempting To Update TaskDetail. (nspTTMEvaluateXDTasks)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
                     END
                     IF @n_continue = 1 or @n_continue = 2
                     BEGIN
                        UPDATE TASKDETAIL
                        ...
                        BEGIN
                           SELECT @n_continue = 3
                           SELECT @c_errmsg = CONVERT(char(250),@n_err), @n_err=79503   -- Should Be Set To The SQL Errmessage but I don't know how to do so.
                           SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Error Occurred While Attempting To Update TaskDetail. (nspTTMEvaluateXDTasks)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
                        END
                        IF @n_cnt = 1
                        BEGIN
            ...
            ELSE
               BEGIN
                  ...
                  SELECT @b_success = 1
                  EXECUTE nspLogAlert
                  @c_ModuleName   = "nspTTMEvaluateXDTasks",
   ...
   ELSE
      ...
      ELSE
         BEGIN
            ...
            END
         END
         execute nsp_logerror @n_err, @c_errmsg, "nspTTMEvaluateXDTasks"
