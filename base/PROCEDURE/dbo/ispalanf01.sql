...
GO
/************************************************************************/
/* Stored Procedure: ispALANF01                                         */
/* Creation Date: 07-Feb-2014                                           */
/* Copyright: LFL                                                       */
/* Written by: Chee Jun Yan                                             */
/*                                                                      */
/* Purpose: Allocate Best Matched Fullcase from BULK (N,DCToDC) UOM:6   */
/*                                                                      */
/* Called By:                                                           */
/*                                                                      */
/* PVCS Version: 1.5                                                    */
...
BEGIN
   ...
   IF ISNULL(@c_LoadType,'') IN ('N', 'DCToDC')
   BEGIN
      /*****************************/
      /***   CREATE TEMP TABLE   ***/
      /*****************************/
      ...
      BEGIN
         SET @c_PrintResult = ''
         SELECT @c_PrintResult = @c_PrintResult + RTRIM(@c_SKU) + ', ' + CAST(UCCQty AS NVARCHAR) + ', ' + CAST(CntCount AS NVARCHAR) + CHAR(13)
         FROM #NumPool WITH (NOLOCK)
         PRINT '#NumPool: ' + CHAR(13) +
      ...
      BEGIN
         ...
         BEGIN
            ...
            BEGIN
               SET @c_PrintResult = ''
               SELECT @c_PrintResult = @c_PrintResult + RTRIM(@c_SKU) + ', ' + CAST([Sum] AS NVARCHAR) + ', ' + Subset + CHAR(13)
               FROM #CombinationPool WITH (NOLOCK)
               PRINT 'Combination Pool: ' + CHAR(13) +
                     SUBSTRING(@c_PrintResult, 0, LEN(@c_PrintResult)) + CHAR(13)
            END
