...
GO
/************************************************************************/
/* Store Procedure: isp_GetPickSlipOrders108                            */
/* Creation Date:                                                       */
/* Copyright: LFL                                                       */
/* Written by:                                                          */
/*                                                                      */
/* Purpose: WMS-12153 - [TW] SKX - INDBPSLIP - RCMReport                */
/*                                                                      */
/* Called By:                                                           */
/*                                                                      */
...
CREATE PROC [dbo].[isp_GetPickSlipOrders108] (@c_LoadKey NVARCHAR(10), @b_debug INT = 0)
AS
BEGIN
   SET NOCOUNT ON
   SET ANSI_NULLS OFF
   ...
   CREATE TABLE #TempPickSlip (
               ...
               Sizesort               INT NULL,
               TotalQtyPerStyleColor  INT NULL
               ) 

   CREATE TABLE #TEMPUSERDEFINE02 (Orderkey NVARCHAR(10) NULL, SKU NVARCHAR(30) NULL, Storerkey NVARCHAR(15) NULL, UserDefine02 INT NULL )
   CREATE TABLE #TempTotalQtyPerStyleColor (Orderkey NVARCHAR(10) NULL, StyleColor NVARCHAR(100) NULL, LOC NVARCHAR(30), Qty INT NULL )
   ...
   BEGIN
      ...
      SELECT ORDERDETAIL.Orderkey, 
             ...
             CASE WHEN ISNUMERIC(ORDERDETAIL.UserDefine02) = 1 THEN CAST(ORDERDETAIL.UserDefine02 AS INT) ELSE 0 END
      FROM ORDERDETAIL WITH (NOLOCK)
      WHERE ORDERDETAIL.Orderkey = @c_TempOrderKey

      INSERT INTO #TempTotalQtyPerStyleColor
      SELECT PD.Orderkey, ISNULL(RTRIM(S.Style),'') + ISNULL(RTRIM(S.Color),'') StyleColor, PD.LOC, SUM(PD.Qty)
      FROM PICKDETAIL PD (NOLOCK)
      JOIN SKU S (NOLOCK) ON S.SKU = PD.SKU AND S.STORERKEY = PD.STORERKEY
      WHERE PD.Orderkey = @c_TempOrderKey
      GROUP BY PD.Orderkey, ISNULL(RTRIM(S.Style),'') + ISNULL(RTRIM(S.Color),''), PD.LOC
      ...
      SELECT DISTINCT 
            ...
            ,Size = CASE WHEN RCFG.ListName IS NOT NULL AND ISNULL(RTRIM(SKU.Measurement),'') <> '' 
                         ...
                         END   
            ,0--CASE WHEN OH.[Type] = 'X' THEN SUM(OD.Qty) ELSE SUM(PD.Qty) END
      FROM ORDERS         OH   WITH (NOLOCK)
      JOIN PICKDETAIL     PD WITH (NOLOCK) ON (PD.Orderkey = OH.Orderkey)
      --CROSS APPLY (SELECT ORDERDETAIL.SKU, ORDERDETAIL.UserDefine03, COUNT(DISTINCT ORDERDETAIL.UserDefine02) AS Qty, ISNULL(RTRIM(S.Size),'') AS Size
      --             FROM ORDERDETAIL WITH (NOLOCK)
      --             JOIN SKU S (NOLOCK) ON S.Storerkey = ORDERDETAIL.Storerkey AND S.SKU = ORDERDETAIL.SKU
      --             WHERE ORDERDETAIL.Orderkey = @c_TempOrderKey AND ORDERDETAIL.SKU = PD.SKU
      --             GROUP BY ORDERDETAIL.SKU, ORDERDETAIL.UserDefine03, ISNULL(RTRIM(S.Size),'') ) AS OD
      JOIN SKU            SKU  WITH (NOLOCK) ON (SKU.Storerkey = OH.Storerkey AND SKU.SKU = PD.SKU)
      LEFT JOIN CODELKUP  RCFG WITH (NOLOCK) ON (RCFG.ListName = 'REPORTCFG')
                                             ...
                                             AND(RCFG.Long = 'r_dw_print_pickorder108')
      ...
      GROUP BY OH.Orderkey
              ,SKU.Storerkey
              ,SKU.Sku
              ,CASE WHEN RCFG.ListName IS NOT NULL AND ISNULL(RTRIM(SKU.Measurement),'') <> '' 
                         THEN ISNULL(RTRIM(SKU.Measurement),'') 
                         ELSE ISNULL(RTRIM(SKU.Size),'') 
                         END
              ,OH.[Type]  
      ...
      DECLARE pick_cur CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
         ...
         ORDER BY ORDERS.OrderKey, LOC.Pickzone, SizeSort

      OPEN pick_cur
      FETCH NEXT FROM pick_cur INTO @c_SkuSize, @c_OrderKey, @c_Pickzone, @c_Sort                                                     
      ...
      BEGIN
         ...
         SELECT @c_PrevPickzone = @c_Pickzone
         ...
         BEGIN
            ...
            SELECT 'PrevPickzone = ' + @c_PrevPickzone + ', Pickzone= ' + @c_Pickzone
         END

         IF (@c_PrevOrderKey <> @c_OrderKey) OR
            (@c_PrevOrderKey = @c_OrderKey AND @c_PrevPickzone <> @c_Pickzone) OR
         ...
         BEGIN
            ...
            SELECT PickHeader.PickHeaderKey,
                  ...
                  CASE WHEN SKUSZ.SIZE = @c_SkuSize1 --AND ISNULL(SKU.Size,'') <> ''
                                 ...
                                 THEN CASE WHEN ORDERS.[Type] = 'X' THEN SUM(tmp.UserDefine02) ELSE SUM(PICKDETAIL.Qty) END 
                                 ELSE 0
                                 END,
                  ...
                  LOC.Pickzone,
                  ORDERS.[Type],
                  RTRIM(LTRIM(ISNULL(ORDERS.C_Address1,''))) + RTRIM(LTRIM(ISNULL(ORDERS.C_Address2,''))) + 
                  RTRIM(LTRIM(ISNULL(ORDERS.C_Address3,''))) + RTRIM(LTRIM(ISNULL(ORDERS.C_Address4,''))) AS C_Addresses,
                  ISNULL(OD.Userdefine03,'') AS Userdefine03,
                  CASE WHEN ISNUMERIC(CL.Short) = 1 AND ISNULL(CL.Short,0) > 0 THEN CL.Short ELSE 0 END AS Casecnt,
                  CASE WHEN ISNULL(CL1.Code,'') = ORDERS.[Type] THEN ISNULL(CL1.Long,'') ELSE ISNULL(CL1.Notes,'') END AS Logo,
                  SizeSort = CASE WHEN ISNUMERIC(ISNULL(CL2.UDF05,'')) = 1 THEN CAST(ISNULL(CL2.UDF05,'') AS INT) ELSE 0 END,
                  TotalQtyPerStyleColor = 0            
            FROM PICKDETAIL WITH (NOLOCK)
            JOIN ORDERS WITH (NOLOCK) ON PICKDETAIL.OrderKey = ORDERS.OrderKey
            JOIN ORDERDETAIL OD WITH (NOLOCK) ON OD.Orderkey = ORDERS.Orderkey AND OD.OrderLineNumber = PICKDETAIL.OrderLineNumber
                                             AND OD.SKU = PICKDETAIL.SKU
            --CROSS APPLY (SELECT MAX(ORDERDETAIL.Userdefine03) AS Userdefine03 FROM ORDERDETAIL WITH (NOLOCK) WHERE ORDERDETAIL.Orderkey = ORDERS.Orderkey) AS OD
            ...
            CROSS APPLY (SELECT SUM(UserDefine02) AS UserDefine02 FROM #TEMPUSERDEFINE02 WHERE ORDERS.Orderkey = Orderkey                          
                                             AND SKU.Storerkey = Storerkey AND SKU.SKU = SKU ) AS tmp 
            LEFT JOIN CODELKUP CL WITH (NOLOCK) ON CL.Listname = 'SKXMIXPACK' AND CL.Code = OD.Userdefine03
                                               AND CL.Storerkey = ORDERS.Storerkey
            LEFT JOIN CODELKUP CL1 WITH (NOLOCK) ON CL1.Listname = 'RPTLOGO' AND CL1.Code = 'X'
                                               AND CL1.Storerkey = ORDERS.Storerkey
            LEFT JOIN CODELKUP  CL2 WITH (NOLOCK) ON (CL2.ListName = 'SKXSIZE')
                                               AND(CL2.Code = SKUSZ.Size)                                       
                                               AND(CL2.Storerkey= SKU.Storerkey)
            WHERE LOADPLANDETAIL.LoadKey = @c_LoadKey
            AND ORDERS.OrderKey = @c_PrevOrderKey
            AND PICKDETAIL.OrderKey = @c_PrevOrderKey
            AND LOC.Pickzone = @c_PrevPickzone
            ...
            GROUP BY LOADPLANDETAIL.Loadkey,
                     ...
                     ORDERS.[Type],
                     RTRIM(LTRIM(ISNULL(ORDERS.C_Address1,''))) + RTRIM(LTRIM(ISNULL(ORDERS.C_Address2,''))) + 
                     RTRIM(LTRIM(ISNULL(ORDERS.C_Address3,''))) + RTRIM(LTRIM(ISNULL(ORDERS.C_Address4,''))),
                     ISNULL(OD.Userdefine03,''),
                     CASE WHEN ISNUMERIC(CL.Short) = 1 AND ISNULL(CL.Short,0) > 0 THEN CL.Short ELSE 0 END,
                     CASE WHEN ISNULL(CL1.Code,'') = ORDERS.[Type] THEN ISNULL(CL1.Long,'') ELSE ISNULL(CL1.Notes,'') END,
                     CASE WHEN ISNUMERIC(ISNULL(CL2.UDF05,'')) = 1 THEN CAST(ISNULL(CL2.UDF05,'') AS INT) ELSE 0 END,
                     SKU.SKU,
                     ORDERS.LoadKey, ORDERS.[Type]
 ...
 SUCCESS:
   ...
   SELECT PickSlipNo, Loadkey, OrderKey, BuyerPO, OrderGroup, ExternOrderKey, Route, Notes, OrderDate,
         ...
         , Casecnt
         , Logo
         , CASE WHEN OrdType = 'X' THEN 0 ELSE (SELECT SUM(Qty) FROM #TempTotalQtyPerStyleColor t 
                                                WHERE t.StyleColor = #TempPickSlip.StyleColor 
                                                AND t.OrderKey = #TempPickSlip.OrderKey and t.LOC = #TempPickSlip.Loc) END AS TotalQtyPerStyleColor
   FROM #TempPickSlip WITH (NOLOCK)
   GROUP BY PickSlipNo, Loadkey, OrderKey, BuyerPO, OrderGroup, ExternOrderKey, Route, Notes, OrderDate,
            ...
            , Casecnt
            , Logo
   ORDER BY Loadkey, DeliveryDate, Orderkey, LogicalLocation, Loc, StyleColor
