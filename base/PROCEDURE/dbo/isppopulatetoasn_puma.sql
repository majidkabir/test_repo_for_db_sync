...
/************************************************************************/
/* SP: ispPopulateTOASN_PUMA                                            */
/* Creation Date: 31-May-2011                                           */
/* Copyright: IDS                                                       */
/* Written by: NJOW                                                     */
/*                                                                      */
/* Purpose: Auto Create ASN for PUMA ECOM Orders  SOS#216422            */
/*                                                                      */
...
/* 14-Aug-2012  NJOW01  1.0  SOS#253051-Copy exterorderkey to           */
/*                           externreceiptkey if storer.susr5 <> '1'    */
/* 27-May-2014  TKLIM   1.1  Added Lottables 06-15                      */
/* 20-Apr-2018   LZG		1.2  INC0199463 - Fixed rollback transaction error */
/************************************************************************/

CREATE PROCEDURE [dbo].[ispPopulateTOASN_PUMA] 
...
BEGIN
   ...
   BEGIN  
      ...
      SELECT @c_ExternReceiptKey   = CASE WHEN ISNULL(STORER.Susr5,'') = '1' THEN 
      ...
      */
      ...
      BEGIN
         ...
         SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Invalid To Facility: ' + RTRIM(ISNULL(@c_ToFacility,'')) +
                        ' (ispPopulateTOASN_PUMA)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
      END
      ...
      BEGIN             
         ...
         BEGIN
            ...
            BEGIN
               ...
               BEGIN
                  ...
                  SET @c_errmsg = CONVERT(char(250),@n_err)
                  SET @n_err = 63481   
                  SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Insert Error on Table Receipt (ispPopulateTOASN_PUMA)' + ' ( ' + 
            ...
            BEGIN
               ...
               SET @c_errmsg = CONVERT(char(250),@n_err) 
               SET @n_err = 63502   
               SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Generate Receipt Key Failed! (ispPopulateTOASN_PUMA)' 
                             + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
            END
         END    
         ELSE
         BEGIN
            ...
            SET @c_errmsg = CONVERT(char(250),@n_err)
            SET @n_err = 63503   
            SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Storer Key is BLANK! (ispPopulateTOASN_PUMA)' + ' ( ' + 
                          ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
         END
         ...
         BEGIN
            ...
            BEGIN 
               ...
               BEGIN
                  ...
                  SET @c_errmsg = CONVERT(char(250),@n_err)
                  SET @n_err = 63491   
                  SET @c_errmsg='NSQL'+CONVERT(char(5),@n_err)+': Insert Error on Table Sku (ispPopulateTOASN_PUMA)' + ' ( ' + 
                             ' SQLSvr MESSAGE=' + RTRIM(ISNULL(@c_errmsg,'')) + ' ) '
               END
            END
            ...
            BEGIN                
               ...
               BEGIN
                  ...
                  FETCH NEXT FROM PICK_CUR
                        INTO @n_QtyReceived--, @c_Lottable01, @c_Lottable02, @c_Lottable03, @d_Lottable04, @d_Lottable05, @c_Lot, @c_ToLoc, @c_id
                  ...
                  BEGIN
                     ...
                     INSERT INTO RECEIPTDETAIL (ReceiptKey,          ReceiptLineNumber,   ExternReceiptkey,    
                                                ...
                                                --ToID,              ToLot,                   
                                                BeforeReceivedQty,   FinalizeFlag)                                                   
      ...
      BEGIN
         ...
         --IF @@TRANCOUNT = 1 OR @@TRANCOUNT >= @n_starttcnt										-- INC0199463
         --BEGIN
            --ROLLBACK TRAN
         --END
         EXECUTE nsp_logerror @n_err, @c_errmsg, 'ispPopulateTOASN_PUMA'
