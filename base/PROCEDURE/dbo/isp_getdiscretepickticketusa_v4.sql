...
/************************************************************************/
/* Stored Procedure: isp_GetDiscretePickTicketUSA_4                     */
/* Creation Date: 28-JUL-2009                                           */
/* Copyright: IDS                                                       */
/* Written by: NJOW (Modified from isp_GetDiscretePickTicketUSA)        */
/*                                                                      */
/* Purpose:  SOS#141342 - Discrete Pick Ticket By UOM for IDSUS.    		*/
/*                                                                      */
/* Called By:  PB - r_dw_consolidated_pick16_discrete_v4                */
/*                                                                      */
/* PVCS Version: 1.2                                                    */
/*                                                                      */
/* Version: 5.4                                                         */
...
CREATE PROC [dbo].[isp_GetDiscretePickTicketUSA_v4] ( 
...
--										AND SKU.SKU = #TempDiscPickSlip2.Altsku        
	  JOIN  #TMP_UPC ON (#TMP_UPC.Storerkey = #TempDiscPickSlip2.Storerkey  --NJOW
	   ...
	   SELECT   PickSlipNo, Loadkey, Orderkey, StorerKey, PrintedFlag, 
		...
		FROM   #TempDiscPickSlip2
		WHERE  LoadKey = @c_LoadKey
		ORDER BY Loadkey, Storerkey, Orderkey, PickSlipno, Putawayzone, LogicalLoc, Loc, Style, Lottable03, cartongroup, Id
								...
								BEGIN
									...
									INSERT INTO @t_DiscretePS 
										  ...
										  (PickSlipNo, LoadKey, Orderkey, PrintedFlag, StorerKey,
											Putawayzone, TotalAllocateQty, TotalOrderQty, LineText, LineFlag, UOM)
        ...
        SET @b_Repeat_ctn = '1' --NJOW
			...
			BEGIN
				...
				IF @b_Debug = 1
				BEGIN
					SELECT 'Before PackUOM*', @c_PrevPPKInd '@c_PrevPPKInd', @n_Qty '@n_Qty', @n_RemainQty '@n_RemainQty',  @c_PrevCartonGroup '@c_PrevCartonGroup', CONVERT(CHAR(10), @f_Pallet) '@f_Pallet', 
               CONVERT(CHAR(10), @f_OtherUnit1) '@f_OtherUnit1', CONVERT(CHAR(10), @f_Casecnt) '@f_Casecnt', CONVERT(CHAR(10), @f_InnerPack) '@f_InnerPack'
				END
			   ...
			   Insert_t_Discrete:
					...
					BEGIN
                     ...
                     --RIGHT(SPACE(8)+dbo.fnc_RTrim(CONVERT(CHAR(8), @n_UOMQty)),8),'', @c_tempuom)
								...
								INSERT INTO @t_DiscretePS 
										(PickSlipNo, LoadKey, Orderkey, PrintedFlag, StorerKey, 
										Putawayzone, TotalAllocateQty, TotalOrderQty, LineText, LineFlag, UOM, Id)
								VALUES (@c_Pickslipno, @c_Loadkey, @c_Orderkey, @c_Printedflag, @c_Storerkey, 
										@c_Putawayzone, @n_AllocatedQty, @n_OriginalQty,
              ...
              ELSE
			      ...
			      SET @n_RemainQty   = @n_RemainQty - @n_EAQty
					...
					BEGIN 
						...
						SET @n_TotalEAQty = @n_TotalEAQty + @n_EAQty
						SET @n_RemainQty  = @n_RemainQty  - @n_EAQty						
            SET @c_nonecartonflag_all = 'Y'  --NJOW
            SET @c_UOM = 'PC' --NJOW06
   ...
   END   
	   ...
	   --NJOW
   UPDATE @t_DiscretePS  SET UOM ='OTHER' WHERE UOM <> 'CARTON' OR UOM IS NULL
    ...
    SELECT @n_cnt = @@ROWCOUNT
		...
		BEGIN
			 DELETE @t_DiscretePS WHERE rowid = @n_prevrowid
		END
