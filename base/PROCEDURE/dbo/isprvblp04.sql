...
GO
/*************************************************************************/  
/* Stored Procedure: ispRVBLP04                                          */  
/* Creation Date: 21-FEB-2019                                            */  
/* Copyright: LFL                                                        */  
/* Written by:                                                           */  
/*                                                                       */  
/* Purpose: WMS-10285 - TW Build Load Release Picking Task (Reverse)     */            
/*                                                                       */
...
CREATE PROCEDURE [dbo].[ispRVBLP04]      
 ...
 BEGIN  
    ...
    BEGIN
        IF NOT EXISTS (SELECT 1 FROM TASKDETAIL TD (NOLOCK) 
                   WHERE TD.Loadkey = @c_Loadkey AND TD.SourceType = 'ispRLBLP04')
        BEGIN
          SELECT @n_continue = 3  
          SELECT @n_err = 81010  
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Load# ' + RTRIM(@c_loadkey) + ' has not been released. (ispRVBLP04)'         
        END                 
    END
    ...
    BEGIN
        IF EXISTS (SELECT 1 FROM TASKDETAIL TD (NOLOCK) 
                   ...
                   AND TD.Sourcetype = 'ispRLBLP04' 
                   AND TD.Status <> '0')
        BEGIN
          SELECT @n_continue = 3  
          SELECT @n_err = 81020  
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Load# ' + RTRIM(@c_loadkey) + ' Some Tasks have been started. Not allow to Reverse Load Released (ispRVBLP04)'       
        END                 
    END
    ...
    BEGIN
       ...
       AND TASKDETAIL.Sourcetype = 'ispRLBLP04'
       ...
       BEGIN
         ...
         SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Load# ' + RTRIM(@c_loadkey) + ' Delete Taskdetail Table Failed. (ispRVBLP04)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
       END
    END
    ...
    BEGIN
         ...
         BEGIN
           ...
           SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Load# ' + RTRIM(@c_loadkey) + ' Update Pickdetail Table Failed. (ispRVBLP04)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
         END          
    END        
    ...
    BEGIN  
       ...
       BEGIN  
          ...
          SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(5),@n_err)+': Load# ' + RTRIM(@c_loadkey) + ' Update on LOADPLAN Failed (ispRVBLP04)' + ' ( ' + ' SQLSvr MESSAGE=' + RTRIM(@c_errmsg) + ' ) '  
       END  
    END  
...
RETURN_SP:
    ...
    BEGIN  
       ...
       BEGIN  
          ...
          END  
       END  
       execute nsp_logerror @n_err, @c_errmsg, "ispRVBLP04"  
