...
BEGIN
   DECLARE @n_Cnt                INT,
           ...
           @c_orduserdef04       NVARCHAR(30),
           @c_ExternOrdKey       NVARCHAR(50),   --(CS01)
           @c_GetExternOrdKey    NVARCHAR(50),   --(CS01)
           @n_CTNExtOrdkey       INT,             --(CS01)
           @c_OHNotes2           NVARCHAR(150),   --(CS03)
           @c_ODNotes2           NVARCHAR(150),   --(CS03)
           @c_ODUdef03           NVARCHAR(50),    --(CS03)
           @n_CntDelimiters      INT,             --(CS03)
           --@c_Altsku             NVARCHAR(20),     --(CS03)
           @n_cntBOMSku          INT,               --(CS03)
           @n_cntRow             INT               --(CS03)
   ...
   CREATE TABLE #TempWalmartmexicoCartonLBL
   (
   ...
   INSERT INTO #TempWalmartmexicoCartonLBL
     (
      FromAdd            ,
      ToAdd              ,
      ShipBarCode        ,            
...
SELECT DISTINCT (FAC.descr + CASE WHEN ISNULL(FAC.descr,'') <> '' THEN ' ' END +FAC.address1 + ' ' +     --(CS03)
           ...
           --   ORD.BuyerPO ,
            --  ('91' + RIGHT('00000'+ISNULL(ORD.C_Contact2,''),5) ),
             ...
             (ORD.stop),
             ISNULL(ORD.userdefine04,''),ORD.Facility,0,'', '' , ORD.userdefine02, ORD.buyerpo
 ...
 -- ORDER BY PH.Pickslipno  desc
  ...
  SELECT Labelno
  FROM #TempWalmartmexicoCartonLBL
...
/*CS05 Start*/ 
  ...
  SELECT DISTINCT @c_ODNotes2=   OD.Notes2
    ...
    JOIN PackDetail AS PADET WITH (NOLOCK) ON padet.LabelNo=pd.CaseID AND PD.SKU = PADET.SKU  --CS04
 ...
 /*CS03 End*/  
  ...
  /*CS01 End*/
   ...
   UPDATE #TempWalmartmexicoCartonLBL
...
END   
   DROP TABLE #TempWalmartmexicoCartonLBL
END  
