...
GO
/***************************************************************************/
/* Store Procedure: ispPOToASNDetailWrapper                                */
/* Creation Date: 16-DEC-2016                                              */
/* Copyright: LF                                                           */
/* Written by: YTWan                                                       */
/*                                                                         */
/* Purpose: Populate to Receipt Mapping                                    */
/*        : WMS-585 - [TW] ASN RCM  Populate From PO Logic                 */
/*                                                                         */
/* Called By: w_populate_podetail.of_colmapping                            */
/*                                                                         */
/* PVCS Version: 1.0                                                       */
/*                                                                         */
/* Version: 5.4                                                            */
...
/***************************************************************************/
CREATE PROC [dbo].[ispPOToASNDetailWrapper]
...
BEGIN 
   ...
   BEGIN
      ...
      IF @c_SourceTable     = 'PO'
      BEGIN
         SET @c_Where = ' WHERE PO.POKey = N''' + @c_SourceKey + ''' '
      END
   END
   ELSE
   BEGIN
      SET @c_TableAttribute = 'D'
      SET @c_ToTable        = 'RECEIPTDETAIL'

      IF @c_SourceTable = 'PODETAIL'
      BEGIN
         SET @c_Where = ' WHERE PODETAIL.POKey = N''' + @c_SourceKey 
                      + ''' AND PODETAIL.POLineNumber = N''' + @c_SourceLineNumber + ''' '
      END
   END
   ...
   BEGIN
      ...
      SET @c_SQLParm =  N'@c_POKey          NVARCHAR(10)'
                     +  ',@c_POLineNumber   NVARCHAR(30)'
   ...
   BEGIN
      ...
      BEGIN 
         SET @c_SQL = 'EXEC ' + @c_SPName + ' @c_SourceTable, @c_SourceKey, @c_SourceLineNumber '
                              + ', @c_ListName, @c_Code, @c_Storerkey, @c_Code2, @c_ToCol, @c_FromValue OUTPUT, @b_Success OUTPUT'          
