...
/*********************************************************************************/  
/* Stored Procedure: isp_AutoSendMail_UA_EPackingList                               */  
...
/* 09-Sep-2021  GHCHAN   1.0.0      Initial Development                          */  
/*********************************************************************************/  
  
CREATE   PROC [dbo].[isp_AutoSendMail_UA_EPackingList]  
(  
    @b_Debug            INT               = 0
...
--                        + N'<font face="Microsoft YaHei">如果需要提供签收单扫描件，请发送订单号至LFLPorscheDC@lflogistics.com, 进行查询。</font><br>'  
  ...
  --SET @c_Consigneeval = SUBSTRING(@c_Consigneeval, PATINDEX('%[^0]%', @c_Consigneeval+'.'), LEN(@c_Consigneeval)) 
   ...
   SET @SQL = N' SELECT @c_EmailToList=STUFF((SELECT ''; '' + am1.emailaddr '
            + N' FROM KRLOCAL.dbo.AutoMail am1 WITH (NOLOCK) '
            + N' WHERE  am1.custcode =am2.custcode '
            + N' FOR XML PATH('''')), 1, 1, '''') FROM KRLOCAL.dbo.Automail am2 WITH (NOLOCK)'
            + N' WHERE am2.custcode = @c_Consigneeval '
            + N' GROUP BY am2.storerkey, am2.custcode'
   
   SET @c_ExecArgs = N' @c_EmailToList NVARCHAR(500) OUTPUT, @c_ConsigneeVal NVARCHAR(100)'

   EXEC sp_ExecuteSql @SQL
                     ,@c_ExecArgs
                     ,@c_EmailToList = @c_EmailToList OUTPUT
                     ,@c_ConsigneeVal = @c_ConsigneeVal
