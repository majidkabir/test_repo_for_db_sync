...
/************************************************************************/
/* Stored Procedure: isp_GetTotalCube_ByPick2                           */
/* Creation Date:                                                       */
/* Copyright: IDS                                                       */
/* Written by:                                                          */
/*                                                                      */
...
CREATE PROCEDURE [dbo].[isp_GetTotalCube_ByPick2] 
...
AS
   ...
   BEGIN
      SELECT @nTotalCube = ISNULL( SUM( SKU.Cube * (PD.QTY / Pack.CaseCnt)), 0)
      FROM PickDetail PD WITH (NOLOCK)
         ...
         INNER JOIN LOC WITH (NOLOCK) ON (PD.LOC = LOC.LOC)
      WHERE PD.OrderKey = @cOrderKey
         AND LOC.LocationType <> 'PICK' 

      SELECT @nTotalCube = @nTotalCube + ISNULL( SUM( SKU.StdCube * PD.QTY), 0)
      FROM PickDetail PD WITH (NOLOCK)
         INNER JOIN SKU WITH (NOLOCK) ON (PD.StorerKey = SKU.StorerKey AND PD.SKU = SKU.SKU)
         INNER JOIN LOC WITH (NOLOCK) ON (PD.LOC = LOC.LOC)
      WHERE PD.OrderKey = @cOrderKey
