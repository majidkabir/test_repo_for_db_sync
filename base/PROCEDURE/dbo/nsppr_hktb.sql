...
BEGIN  
   ...
   SELECT @c_Lottable02Label = ISNULL(LOTTABLE02LABEL, '') 
   FROM SKU (NOLOCK)
   WHERE SKU = @c_sku
   ...
   BEGIN
      SELECT @c_SortOrder = " ORDER BY LOTATTRIBUTE.LOTTABLE02, LOC.LogicalLocation ASC "
   END
   ELSE
   BEGIN
      SELECT @c_SortOrder = " ORDER BY LOC.LogicalLocation ASC "
   END
   ...
   BEGIN
      ...
      EXEC (" DECLARE PREALLOCATE_CURSOR_CANDIDATES CURSOR FAST_FORWARD READ_ONLY FOR " +  
           ...
           " AND LOC.STATUS = 'OK' AND LOC.LocationFlag <> 'HOLD' " +  
