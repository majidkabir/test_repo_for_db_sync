...
/******************************************************************************/                       
/* Copyright: IDS                                                             */                       
/* Purpose:   isp_BT_UCCLBL_SG_TRIPLE                                         */                       
/*                                                                            */                       
/* Modifications log:                                                         */                       
/*                                                                            */                       
/* Date       Rev  Author     Purposes                                        */    
/*22-Nov-2018 1.0  WLCHOOI   WMS-7030&WMS-7031 TRIPLE ECOM Shipping Label     */       
/*21-Dec-2018 1.1  WLCHOOI   Combined with WMS-7366                           */     
/*14-Jan-2019 1.2  WLCHOOI   Change mapping of Col13 and add new Col25 (WL01) */    
/*18-Mar-2019 1.3  WLCHOOI   Add new mapping col26 ~ col31 (WL02)             */  
/*31-MAY-2019 1.4  CSCHONG   WMS-9261 -add new field col32 to col34 (CS01)    */  
/*18-MAR-2020 1.45 CSCHONG   WMS-12562 -add new field col35 to col36 (CS02)    */
/******************************************************************************/                      
                        
CREATE PROC [dbo].[isp_BT_UCCLBL_SG_TRIPLE]                            
...
BEGIN                            
  ...
  SET @c_SQLJOIN = +' SELECT DISTINCT PD.LabelNo'+ CHAR(13)     
                   ...
                   + ' ORD.B_Address1, ORD.B_Address2,'+ CHAR(13)      --5    
                   + ' ORD.B_Address3,ORD.B_Address4,ORD.C_contact1,ORD.C_Address1,ORD.C_Address2, '+ CHAR(13)     --10 --(WL01)    
                   + ' ORD.C_Address3,ORD.C_State + ''  '' + ORD.C_Zip,ORD.C_City, '+ CHAR(13)      
                   ...
                   + ' CT.CARTONDESCRIPTION,''8111020'',Ord.C_City,Ord.C_State,' + CHAR(13)    --30      --(WL01) --(WL02)   
                   + ' Ord.C_Zip,Ord.userdefine04,ord.dischargeplace,Ord.deliveryplace,TRIM(ORD.M_Company),TRIM(ORD.M_address1),'''','''','''','''','  + CHAR(13) --40  --(WL02) --(CS01) --(CS02)
                   + ' '''','''','''','''','''','''','''','''','''','''', '+ CHAR(13)     --50    
                   + ' '''','''','''','''','''','''','''','''',PH.Pickslipno,''SG'' '     --60    
                   + CHAR(13)    
                   ...
                   +' JOIN PACKINFO PI WITH (NOLOCK) ON PD.Pickslipno = PI.Pickslipno AND PI.CartonNo = PD.CartonNo' + CHAR(13)   --(WL02)  
                   +' JOIN CARTONIZATION CT WITH (NOLOCK) ON CT.CARTONTYPE = PI.CARTONTYPE' + CHAR(13)   --(WL02)  
                   ...
                   +' ORD.ExternOrderkey,ORD.B_Contact1, '+ CHAR(13)       
                   +' ORD.B_Address1, ORD.B_Address2,'     + CHAR(13)    
                   +' ORD.B_Address3,ORD.B_Address4,ORD.C_contact1,ORD.C_Address1,ORD.C_Address2, ' + CHAR(13)    
                   +' ORD.C_Address3,ORD.C_State,ORD.C_Zip,ORD.C_City, '+ CHAR(13)       
                   ...
                   +' Ord.userdefine04,ord.dischargeplace,Ord.deliveryplace,PH.Pickslipno,TRIM(ORD.M_Company),TRIM(ORD.M_address1) '       --(CS01)  --(CS02)
 ...
 WHERE COL01 = @c_sparm02    
   ...
   EXIT_SP:          
      ...
      EXEC isp_InsertTraceInfo         
         ...
         @c_TraceName = 'isp_BT_UCCLBL_SG_TRIPLE',        
