...
GO
/************************************************************************/        
/* Stored Procedure: isp_TPB_Orders_Converse                            */        
/* Creation Date:                                                       */        
/* Copyright: IDS                                                       */        
/* Written by: WeiLi                                                    */        
/*                                                                      */        
/* Purpose: TPB billing for Converse Orders Transaction                 */        
/*                                                                      */        
...
CREATE PROC [dbo].[isp_TPB_Orders_Converse]  
...
BEGIN  
   ...
   ',(Select TOP 1 CD.Notes from CODELKUP CD (NOLOCK) 
      JOIN ORDERS ORDERS1 (NOLOCK) ON ORDERS1.Facility=CD.Code 
   ...
   ',RTRIM(OD.ExternLineNo) ' +  
    ',(Select TOP 1 CD.Code from CODELKUP CD (NOLOCK) 
      JOIN ORDERS ORDERS1 (NOLOCK) ON ORDERS1.Storerkey=CD.Storerkey 
   ...
   ',ORDERS.DeliveryDate ' +        --(WL)
   ',UPPER(RTRIM(OD.SKU)) ' +                     
   ',RTRIM(LOC.LocationType) ' +    
   --','''' ' +                             --BillableNoOfLoc    
   ',(ISNULL((SELECT (PD.Qty * SUM(ISNULL(BOM.Qty,1))) FROM   dbo.Billofmaterial BOM (NOLOCK)    
      WHERE PD.Storerkey = BOM.StorerKey AND  PD.SKU = BOM.SKU ) ,PD.Qty)) ' +                
   ',RTRIM(PD.UOM) ' +            
...
SET @c_SQLStatement = @c_SQLStatement + 
  ',RTRIM(ORDERS.C_Address1) ' +   --(WL)
   ',RTRIM(ORDERS.C_Address2) ' +  
   ',RTRIM(ORDERS.C_Address3) ' +  
   ',RTRIM(ORDERS.C_Address4) ' +  
