...
GO
/************************************************************************/
/* Stored Procedure: nspPR_PH11                                         */
/* Creation Date: 25-JUN-2021                                           */
/* Copyright: LFL                                                       */
/* Written by:                                                          */
/*                                                                      */
/* Purpose: WMS-17195 PH GSK Pre-Allocation                             */
...
/* 05-Nov-2021  NJOW    1.0   DEVOPS combine script                     */
/************************************************************************/

CREATE PROC [dbo].[nspPR_PH11]
...
BEGIN 
    ...
    */
    ...
    BEGIN        
      ...
      SELECT @c_HostWHCode = ISNULL(Userdefine01,'')
      FROM ORDERDETAIL (NOLOCK)
      WHERE Orderkey = @c_Orderkey
      AND OrderLineNumber = @c_OrderLineNumber
    END 
    
    SELECT @c_PH_AllocWHCode = ISNULL(SValue,'')
    FROM STORERCONFIG(NOLOCK)
    WHERE Storerkey = @c_Storerkey
    AND Configkey = 'PH_AllocWHCode'
    
    SELECT @c_LottableList = @c_Lottablelist + RTRIM(Code) + ','
    FROM CODELKUP (NOLOCK)
    WHERE Storerkey = @c_Storerkey
    AND Listname = 'ALLOCLOT' 
   ...
   BEGIN
       ...
       -- Rule#1: Allocate specific LOT
        -- Get lottables
       IF @c_lottable01 <> ' ' OR CHARINDEX('LOTTABLE01', @c_LottableList, 1) > 0
         SELECT @c_LimitString =  dbo.fnc_RTrim(@c_LimitString) + ' AND Lottable01= @c_lottable01 '
          
       IF @c_lottable02 <> ' ' OR CHARINDEX('LOTTABLE02', @c_LottableList, 1) > 0
         SELECT @c_LimitString =  dbo.fnc_RTrim(@c_LimitString) + ' AND lottable02= @c_lottable02 ' 
        
       IF @c_lottable03 <> ' ' OR CHARINDEX('LOTTABLE03', @c_LottableList, 1) > 0
         SELECT @c_LimitString =  dbo.fnc_RTrim(@c_LimitString) + ' AND lottable03= @c_lottable03 '  
        
       IF (@d_lottable04 IS NOT NULL AND @d_lottable04 <> '1900-01-01') OR CHARINDEX('LOTTABLE04', @c_LottableList, 1) > 0
         SELECT @c_LimitString =  dbo.fnc_RTrim(@c_LimitString) + ' AND lottable04 = @d_lottable04 '  
          
       IF (@d_lottable05 IS NOT NULL  AND @d_lottable05 <> '1900-01-01') OR CHARINDEX('LOTTABLE05', @c_LottableList, 1) > 0
         SELECT @c_LimitString =  dbo.fnc_RTrim(@c_LimitString) + ' AND lottable05= @d_lottable05 '  

       IF @c_lottable06 <> ' ' OR CHARINDEX('LOTTABLE06', @c_LottableList, 1) > 0
            SELECT @c_LimitString =  dbo.fnc_RTrim(@c_LimitString) + ' AND lottable06= @c_lottable06 '  

       IF @c_lottable07 <> ' ' OR CHARINDEX('LOTTABLE07', @c_LottableList, 1) > 0 
            SELECT @c_LimitString =  dbo.fnc_RTrim(@c_LimitString) + ' AND lottable07= @c_lottable07 '  

       IF @c_lottable08 <> ' ' OR CHARINDEX('LOTTABLE08', @c_LottableList, 1) > 0 
            SELECT @c_LimitString =  dbo.fnc_RTrim(@c_LimitString) + ' AND lottable08= @c_lottable08 '   

       IF @c_lottable09 <> ' ' OR CHARINDEX('LOTTABLE09', @c_LottableList, 1) > 0 
            SELECT @c_LimitString =  dbo.fnc_RTrim(@c_LimitString) + ' AND lottable09= @c_lottable09 '  

       IF @c_lottable10 <> ' ' OR CHARINDEX('LOTTABLE10', @c_LottableList, 1) > 0 
            SELECT @c_LimitString =  dbo.fnc_RTrim(@c_LimitString) + ' AND lottable10= @c_lottable10 '  

       IF @c_lottable11 <> ' ' OR CHARINDEX('LOTTABLE11', @c_LottableList, 1) > 0
            SELECT @c_LimitString =  dbo.fnc_RTrim(@c_LimitString) + ' AND lottable11= @c_lottable11 '  

       IF @c_lottable12 <> ' ' OR CHARINDEX('LOTTABLE12', @c_LottableList, 1) > 0 
            SELECT @c_LimitString =  dbo.fnc_RTrim(@c_LimitString) + ' AND lottable12= @c_lottable12 '  

       IF (@d_lottable13 IS NOT NULL AND @d_lottable13 <> '1900-01-01') OR CHARINDEX('LOTTABLE13', @c_LottableList, 1) > 0
         SELECT @c_LimitString =  dbo.fnc_RTrim(@c_LimitString) + ' AND lottable13 = @d_lottable13 '  

       IF (@d_lottable14 IS NOT NULL AND @d_lottable14 <> '1900-01-01') OR CHARINDEX('LOTTABLE14', @c_LottableList, 1) > 0
         SELECT @c_LimitString =  dbo.fnc_RTrim(@c_LimitString) + ' AND lottable14 = @d_lottable14 '  

       IF (@d_lottable15 IS NOT NULL AND @d_lottable15 <> '1900-01-01') OR CHARINDEX('LOTTABLE15', @c_LottableList, 1) > 0
      ...
      IF ISNULL(@c_PH_AllocWHCode,'') NOT IN('','0')
      BEGIN
         SELECT @c_LimitString = dbo.fnc_RTrim(@c_LimitString)  + ' AND ISNULL(LOC.HostWHCode,'''') = ISNULL(@c_HostWHCode,'''') '           	
      END
      ...
      EXEC sp_ExecuteSQL @c_SQLStmt, @c_SQLParms, @c_facility, @c_storerkey, @c_SKU
                        ...
                        ,@n_SkuOutgoingShelfLife, @n_SkuShelfLife, @c_HostWHCode        
      --(Wan01) - END
