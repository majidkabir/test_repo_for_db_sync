...
/*******************************************************************************/
/* Stored Procedure: ispGenEOrderReplen01                                      */
/* Creation Date: 06-Aug-2019                                                  */
/* Copyright: LFL                                                              */
/* Written by:                                                                 */
/*                                                                             */
/* Purpose: WMS-9276 CN ANF ECOM Replenishment                                 */   
/*                                                                             */
/* Called By:                                                                  */
/*                                                                             */
/* PVCS Version: 1.0                                                           */
...
BEGIN  
   ...
   DECLARE @c_SKU                      NVARCHAR(20),
           ...
           @c_DoReplenish                   CHAR(1), --NJOW01 
           @n_SplitQty                      INT, --NJOW01
           @c_EOrderReplenByUCC             NVARCHAR(30), --NJOW03
           @c_UCCNo                         NVARCHAR(20), --NJOW03 
           @n_UCCQty                        INT,           --NJOW03         
   ...
   BEGIN	           
      ...
      BEGIN
         ...
         BEGIN
            SELECT @n_continue = 3
            SELECT @n_err = 78301
            SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Insert into Temp Table @tloadkey Failed! (ispGenEOrderReplen01)'        
            GOTO EXIT_SP
         END             
      END  
      ELSE
      ...
      BEGIN    
         ...
         BEGIN
            SELECT @n_continue = 3
            SELECT @n_err = 78303
            SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Insert into Temp Table @tTaskBatchNo Failed! (ispGenEOrderReplen01)'
            GOTO EXIT_SP         
         END
                  
         --Remove task batch already have replenishmentgroup (generated replenismnent)         
         DELETE TBN
      ...
      ELSE 
      BEGIN          
         --Get available task batch of the load plan not yet generate replenishment
         INSERT INTO @tTaskBatchNo (TaskBatchNo)
   ...
   BEGIN
      ...
      BEGIN
         SELECT @n_continue = 3
         SELECT @n_err = 78307
         SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': nspGetRight Failed! (ispGenEOrderReplen01)'
         GOTO EXIT_SP      
      END
      ...
      BEGIN
         SELECT @n_continue = 3
         SELECT @n_err = 78308
         SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': nspGetRight ForceAllocLottable Failed! (ispGenEOrderReplen01)'
         GOTO EXIT_SP      
      END
      ...
      BEGIN
         SET @n_continue = 3
         SET @n_err = 78330
         SET @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': nspGetRight Failed! (ispGenEOrderReplen01)'
         GOTO EXIT_SP      
      END                                                       
   END 
   ...
   BEGIN
      ...
      DECLARE CUR_Bulk_UCCReplen CURSOR LOCAL FAST_FORWARD READ_ONLY FOR
         ...
         JOIN PackTask AS PT WITH (NOLOCK) ON PT.OrderKey = P.OrderKey 
         ...
         JOIN LOC AS l WITH (NOLOCK) ON l.Loc = P.Loc AND l.LocationType NOT IN ('DYNPICKP', 'DYNPICKR','DYNPPICK', 'PICK') AND l.Facility = o.Facility 
         ...
         BEGIN         	
            ...
            BEGIN            	  
            	 ...
            	 BEGIN
                  ...
                  BEGIN
                     SELECT @n_continue = 3
                     SELECT @n_err = 78315
                     SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Update PickDetail Failed! (ispGenEOrderReplen01)'
                  END 
            	 ...
            	 BEGIN
                  ...
                  BEGIN
                     SELECT @n_continue = 3
                     SELECT @n_err = 78313
                     SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': INSERT PickDetail Failed! (ispGenEOrderReplen01)'
                  END                   
            ...
            BEGIN
               ...
               BEGIN
                  SELECT @n_continue = 3
                  SELECT @n_err = 78315
                  SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Update UCC Failed! (ispGenEOrderReplen01)'
               END                               
            END
   ...
   -- Get Loose Pick From Bulk for ECOM Single Order and re-allocate to DPP
   IF @n_continue IN(1,2)
   BEGIN
      ...
      SELECT P.Storerkey, o.Facility, P.Sku, P.LOC, P.LOT, P.ID, SUM(P.Qty) OrderQty, PK.CaseCnt, 
             PK.PackKey, PK.PackUOM3, SKU.Putawayzone, LA.Lottable02 
      FROM PICKDETAIL AS p WITH (NOLOCK)                                                
      JOIN PackTask AS PT WITH (NOLOCK) ON PT.OrderKey = P.OrderKey 
      JOIN ORDERS AS o WITH(NOLOCK) ON o.Orderkey = PT.Orderkey 
      JOIN LOTATTRIBUTE AS LA WITH (NOLOCK) ON p.Lot = LA.Lot
      JOIN @tTaskBatchNo AS tbn  ON tbn.TaskBatchNo = PT.TaskBatchNo 
      JOIN LoadPlanDetail AS lpd WITH (NOLOCK) ON lpd.Orderkey = PT.Orderkey 
      JOIN @tloadkey AS lk  ON lk.LoadKey = lpd.LoadKey     
      ...
      JOIN LOC AS l WITH (NOLOCK) 
            ON l.Loc = P.Loc
            AND l.LocationType NOT IN ('DYNPICKP', 'DYNPICKR','DYNPPICK', 'PICK') 
            AND l.Facility = o.Facility 
      WHERE (PK.CaseCnt > 0 OR @c_EOrderReplenByUCC = '1') 
      AND   P.[Status] = '0'   
      AND   P.UOM IN ('6','7') 
      GROUP BY lpd.LoadKey, P.Storerkey, o.Facility, P.Sku, P.LOC, P.LOT, P.ID, PK.CaseCnt, PK.PackKey, PK.PackUOM3, SKU.Putawayzone, LA.Lottable02
      
      OPEN CUR_Bulk_Replenishment
     
      FETCH NEXT FROM CUR_Bulk_Replenishment INTO 
         @c_StorerKey, @c_Facility, @c_SKU, @c_LOC, @c_LOT, @c_ID, @n_QtyOrdered, @n_CaseCnt, @c_PackKey, @c_UOM, @c_Putawayzone, @c_Lottable02
      ...
      BEGIN    
         ...
         --Find loc with same sku from DPP with same PAZone, Lottable02 and qty available                  
         /*
         ...
         AND LLI.Sku = @c_Sku
         AND (SL.LocationType IN ('PICK', 'CASE') OR LOC.LocationType IN('DYNPICKP', 'DYNPICKR','DYNPPICK', 'PICK'))
         ...
         ORDER BY CASE WHEN MIN(LLI.Lot) = @c_Lot THEN 1 ELSE 2 END, SUM(LLI.Qty - LLI.QtyAllocated - LLI.QtyPicked) DESC, LOC.LogicalLocation, LOC.Loc
         */         
         
         --Find loc with same sku from DPP with same PAZone
         IF ISNULL(@c_ToLoc,'') = ''
         BEGIN         
            SELECT TOP 1 @c_ToLoc = LOC.Loc
         ...
         --Find loc with same sku from DPP any zone
         /*
         IF ISNULL(@c_ToLoc,'') = ''
         BEGIN
            SELECT TOP 1 @c_ToLoc = LOC.Loc
            ...
            AND (SL.LocationType IN ('PICK', 'CASE') OR LOC.LocationType IN('DYNPICKP', 'DYNPICKR','DYNPPICK', 'PICK'))
            AND LOC.LocationFlag = 'NONE'
            AND SL.Qty + SL2.PendingMoveIn > 0
            --AND LOC.Putawayzone = @c_Putawayzone 
            AND LOC.Facility = @c_Facility
            ORDER BY SL.Qty, LOC.LogicalLocation, LOC.Loc         
         END
         */
         ...
         --find empty loc from DPP with same zone
         IF ISNULL(@c_ToLoc,'') = ''
         BEGIN
            SELECT TOP 1 @c_Toloc = LOC.Loc
            FROM LOC (NOLOCK)
         ...
         BEGIN        
            ...
            --retrieve loose pickdetail from bulk
            ...
            BEGIN   
               ...
               IF @n_FullCasePickQty > 0 --if full case update uom to 2
               BEGIN                            
                  UPDATE PICKDETAIL WITH (ROWLOCK)
                     SET UOM = '2', 
                         TrafficCop = NULL,
                  ...
                  BEGIN
                     SELECT @n_continue = 3
                     SELECT @n_err = 78309
                     SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Updating PickDetail Failed! (ispGenEOrderReplen01)'
                     GOTO EXIT_SP         
                  END
               ...
               BEGIN             
                  ...
                  BEGIN
                  	 --Move the pickdetail to DPP if overallocate uom = 7 else 6                    
                        ...
                        BEGIN
                           ...
                           BEGIN
                              ...
                              BEGIN
                                 SELECT @n_continue = 3
                                 SELECT @n_err = 78310
                                 SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Inserting LOTxLOCxID Failed! (ispGenEOrderReplen01)'
                                 GOTO EXIT_SP         
                              END                              
                           END   
                           
                           IF NOT EXISTS(SELECT 1 FROM SKUXLOC (NOLOCK) WHERE Storerkey = @c_Storerkey 
                                         AND Sku = @c_Sku AND Loc = @c_ToLoc)
                           BEGIN
                              ...
                              BEGIN
                                 SELECT @n_continue = 3
                                 SELECT @n_err = 78311
                                 SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Inserting SKUXLOC Failed! (ispGenEOrderReplen01)'
                                 GOTO EXIT_SP         
                              END
                           END                                                                                                   
                        END
                     ...
                     UPDATE PICKDETAIL WITH (ROWLOCK)
                        SET LOC = @c_ToLOC, 
                            ...
                            --DoReplenish = CASE WHEN @c_LotAvailableQty < @n_PickQty THEN 'N' ELSE 'Y' END    --N = Need replen (7)
                     WHERE PickDetailKey = @c_PickDetailKey 
                     IF @@ERROR <> 0 
                     BEGIN
                        SELECT @n_continue = 3
   ...
   --Find Available Qty or Swap with other PickDetail at the DPP location
   IF @n_continue IN(1,2)
   BEGIN                     
      SET @c_PickDetailKey = ''
      ...
      WHILE 1=1 
      BEGIN
      	 --Retrieve pickdetail need replenish for the batch from DPP
         SELECT  TOP 1 
                 ...
                 @c_SKU           = p.Sku,
                 @c_DoReplenish   = P.DoReplenish --NJOW01
         FROM PICKDETAIL AS p WITH (NOLOCK)  
         JOIN Orderdetail as OD (NOLOCK) on OD.orderkey = p.orderkey and od.orderlinenumber = p.orderlinenumber  --tlting              
         JOIN SKUxLOC AS  SL WITH (NOLOCK) 
                  ON SL.Storerkey = P.Storerkey 
                 AND SL.Sku = P.Sku   
                 AND SL.Loc = P.Loc 
         ...
         AND   (SL.LocationType IN ('PICK', 'CASE')
               OR l.LocationType IN('DYNPICKP', 'DYNPICKR','DYNPPICK', 'PICK') )
         AND   P.UOM IN ('7','6')   
         AND   P.DoReplenish = 'N'
         ...
         BEGIN
            ...
            IF @c_ForceAllocLottable <> '1'  --skip if forcealloclottable due to re-sorting loc
            BEGIN
               SELECT TOP 1 
            ...
            IF @n_QtyAvaliable = 0 
            BEGIN
            	  --find DPP loc diffrent Lot,lot,id with lottable matching
               IF @c_ForceAllocLottable = '1'
               BEGIN
                  SELECT
                  ...
                  SELECT @c_SQLSelect =
                    ...
                    '  @c_FromID  = lli.ID   ' + 
                    ' FROM LOTxLOCxID AS lli WITH(NOLOCK) ' +
                    ' JOIN  SKUXLOC AS SL WITH (NOLOCK) ON lli.Storerkey = SL.Storerkey AND lli.Sku = SL.Sku AND lli.Loc = SL.Loc ' +
                    ...
                    ' AND   (SL.LocationType IN (''PICK'', ''CASE'') OR L.LocationType IN(''DYNPICKP'', ''DYNPICKR'',''DYNPPICK'', ''PICK'')) ' + 
                    ' AND L.LocationFlag = ''NONE'' ' + --NJOW02
                   -- ' AND lli.Loc = @c_LOC ' + 
                    ' AND LOT.Status = ''OK'' ' +  
                    ' AND lli.Qty - (lli.QtyAllocated + lli.QtyPicked) > 0  ' +
                    ' AND LOT.Qty - (LOT.QtyAllocated + LOT.QtyPicked + LOT.QtyPreAllocated) >= lli.Qty - (lli.QtyAllocated + lli.QtyPicked) ' +  --(Wan01)                    
                  ...
                  ' ORDER BY L.LogicalLocation, L.Loc, LOT.Lot '
                  --' ORDER BY CASE WHEN L.Loc = @c_Loc THEN 1 ELSE 2 END, (lli.Qty - lli.QtyAllocated - lli.QtyPicked) DESC, L.LogicalLocation, L.Loc '
               ...
               /*ELSE 
               BEGIN
               	 --find same loc any available lot,id  --Remark to avoid take the qty of B2B stock
                  SELECT TOP 1 
                      @n_QtyAvaliable =  lli.Qty - lli.QtyAllocated - lli.QtyPicked, 
                      @c_FromLOT = lli.Lot,
                      @c_FromLOC = lli.Loc, 
            ...
            IF @n_QtyAvaliable > 0 
            BEGIN
            	 --re-allocate the pickdetail to available qty
               IF @b_Debug = 1
               BEGIN
            ...
            BEGIN -- IF @n_QtyAvaliable = 0 
               ...
               -- swap with other Ecom batch of pick not release batch yet 
               SELECT TOP 1 
   ...
   BEGIN   
      ...
      --Retrieve pickdetail qty need replenish (7) (N)
      ...
      SELECT L.Facility, P.Storerkey, P.Sku, P.Loc, 
             ...
             SL.QtyLocationLimit, P.Lot  
      FROM PICKDETAIL AS p WITH (NOLOCK)                  
      LEFT JOIN SKUxLOC AS  SL WITH (NOLOCK) 
               ON SL.Storerkey = P.Storerkey 
              AND SL.Sku = P.Sku   
              AND SL.Loc = P.Loc 
      ...
      AND   P.UOM = '7'  
      AND   P.DoReplenish = 'N'
      GROUP BY L.Facility, P.Storerkey, P.Sku, P.Loc, SL.QtyPicked, SL.Qty, SL.QtyLocationLimit, P.Lot
                            
      OPEN CUR_OVERALLOCATE_LOC
      
      FETCH NEXT FROM CUR_OVERALLOCATE_LOC INTO @c_Facility, @c_StorerKey ,@c_SKU, @c_LOC, @n_ReplenQty, @n_LocCapacity, @c_Lot
      WHILE @@FETCH_STATUS = 0 
      BEGIN             
         ...
         BEGIN          
            ...
            WHERE P.[Status] = '0'
            AND   (SL.LocationType IN ('PICK', 'CASE') 
                  OR OR l.LocationType IN('DYNPICKP', 'DYNPICKR','DYNPPICK', 'PICK'))
            AND   P.UOM = '7'  
            AND   P.DoReplenish = 'N'
            AND   P.StorerKey = @c_StorerKey 
            AND   P.SKU = @c_SKU
            ...
            */  
            ...
            IF @c_EOrderReplenByUCC = '1'  --NJOW03 Start       
            BEGIN
               SET @c_FromLOC = ''
               SET @c_FromID  = ''
               SET @c_FromLOT = ''
               ...
               BEGIN
                  ...
                  BEGIN
                     ...
                     BEGIN
                        SELECT @n_continue = 3
                        SELECT @n_err = 78325
                        SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': nspg_GetKey Failed! (ispGenEOrderReplen01)'
                        GOTO EXIT_SP         
                     END
                     ...
                     BEGIN
                        SELECT @n_continue = 3
                        SELECT @n_err = 78326
                        SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Insert into Replenishment Failed! (ispGenEOrderReplen01)'
                        GOTO EXIT_SP         
                     END                         
            ...
            BEGIN
               ...
               */
               ...
               BEGIN
                  ...
                  SET @n_ReplenQty = @n_ReplenQty -  @n_QtyToTake
                  SET @n_QtyInPickLoc = @n_QtyInPickLoc - @n_QtyToTake -- WWANG01 
               ...
               --FETCH FROM CUR_REPLEN_LOT INTO @c_LOT, @n_QtyAllocPick
         ...
         --update the pickdetail Y indicate replen generated
   ...
   IF EXISTS(SELECT 1 FROM REPLENISHMENT WITH (NOLOCK)
      ...
      dbo.fnc_GetRight(@c_facility, @c_Storerkey, '', 'NoReplenStillGenReplenGroup') = '1'  --NJOW02            
   BEGIN
      ...
      BEGIN
         ...
         BEGIN
            SELECT @n_continue = 3
            SELECT @n_err = 78328
            SELECT @c_errmsg='NSQL'+CONVERT(CHAR(5),@n_err)+': Updating PackTask Failed! (ispGenEOrderReplen01)'
            GOTO EXIT_SP         
         END
