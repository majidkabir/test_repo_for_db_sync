...
GO
/************************************************************************/
/* Stored Procedure: nspDailyInventory2                                 */
/* Creation Date:                                                       */
/* Copyright: IDS                                                       */
/* Written by:                                                          */
/*                                                                      */
...
CREATE PROCEDURE [dbo].[nspDailyInventory2]  AS
BEGIN
   SET NOCOUNT ON 
   SET QUOTED_IDENTIFIER OFF 
   ...
   -- Change by Francis Chan 31 Dec 2002 - Start
   -- Not include the ID to avoid over count the charging pallet
   /*
   ...
   */
   INSERT INTO DailyInventory (Storerkey, Sku, Loc, Id, Qty, InventoryDate)
   SELECT storerkey, sku, loc, ' ', sum(qty - qtypicked), @d_inventorydate
   FROM lotxlocxid (nolock)
   GROUP BY storerkey, sku, loc
   HAVING sum(qty - qtypicked) > 0
   -- Change by Francis Chan 31 Dec 2002 - End
   ...
   SELECT  di.inventorydate, di.loc,
   CommingleSku = case
   when Count(distinct di.sku) <=1 then '0'
else '1' end
