...
GO
/************************************************************************/  
/* Stored Procedure: isp_TCPSocket_Outlog_PTL_SendEmail                 */  
...
CREATE PROC [dbo].[isp_TCPSocket_Outlog_PTL_SendEmail]    
(  
...
SET ANSI_NULLS OFF  
   ...
   BEGIN  
      ...
      BEGIN  
            SELECT @n_Err = 70001  
            SELECT @c_ErrMsg = 'NSQL'+CONVERT(CHAR(5),@n_Err)+': Error Updating TCPSocket_OutLog. (isp_TCPSocket_Outlog_PTL_SendEmail)'  
   ...
   BEGIN  
         SELECT @n_Err = 70000  
         SELECT @c_ErrMsg = 'NSQL'+CONVERT(CHAR(5),@n_Err)+': Error executing sp_send_dbmail. (isp_TCPSocket_Outlog_PTL_SendEmail)'  
                          + ' ( SQLSvr MESSAGE=' + ISNULL(RTRIM(@n_Err),'') + ' )'  
         GOTO QUIT  
   END  
...
Quit:           
   IF @n_Err <> 0  
      EXECUTE nsp_logerror @n_Err, @c_ErrMsg, 'isp_TCPSocket_Outlog_PTL_SendEmail'     
