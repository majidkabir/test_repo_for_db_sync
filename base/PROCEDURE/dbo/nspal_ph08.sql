...
GO
/************************************************************************/    
/* Stored Procedure: nspAL_PH08                                         */    
/* Creation Date: 18-AUG-2021                                           */    
/* Copyright: LFL                                                       */    
/* Written by:                                                          */    
/*                                                                      */    
/* Purpose: WMS-17754 PH PNG Transfer allocation                        */
/*          FEFO.                                                       */
/*                                                                      */
/* Called By:                                                           */    
/*                                                                      */    
...
/* 08-Nov-2021  NJOW01  1.0   DEVOPS combine script                     */
/************************************************************************/    
CREATE PROC [dbo].[nspAL_PH08]        
...
BEGIN    
   ...
   SET @c_SQL = N'   
      ...
      CASE WHEN ISNULL(RTRIM(@c_Lottable01),'') = '' THEN '' ELSE ' AND LOC.HostWHCode = @c_Lottable01 ' END +
      ...
      CASE WHEN @c_UOM = '1' THEN ' ORDER BY LA.Lottable04, CASE WHEN ((LOTxLOCxID.QTY - LOTxLOCxID.QTYALLOCATED - LOTxLOCxID.QTYPICKED - LOTxLOCxID.QtyReplen - ISNULL(TRFLLI.FromQty,0)) % @n_QtyLeftToFulfill) = 0 THEN 1 ELSE 2 END, 4 DESC, LOC.LogicalLocation, LOC.LOC ' ELSE ' ' END +
      CASE WHEN @c_UOM <> '1' THEN ' ORDER BY LA.Lottable04, 4, LOC.LogicalLocation, LOC.LOC ' ELSE ' ' END*/ 
