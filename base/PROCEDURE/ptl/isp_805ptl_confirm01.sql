...
/************************************************************************/
/* Store procedure: isp_805PTL_Confirm01                                */
/* Copyright      : LF Logistics                                        */
/*                                                                      */
/* Purpose: Accept QTY in CS-PCS, format 9-999                          */
/*                                                                      */
/* Date       Rev  Author   Purposes                                    */
/* 16-03-2016 1.0  Ung      SOS363160 Created                           */
/* 10-01-2017 1.1  Ung      WMS-855 Stamp PickDetail.DropID             */
/************************************************************************/

CREATE PROC [PTL].[isp_805PTL_Confirm01] (
...
if @cDebug = '1' 
   ...
   SAVE TRAN isp_805PTL_Confirm01 -- For rollback or commit only our own transaction
...
*/
      ...
      BEGIN
         ...
         BEGIN -- Have balance, need to split
            ...
            SELECT 
               ...
               IPAddress, DeviceID, DevicePosition, Status, LightUp, LightMode, LightSequence, PTLType, SourceKey, DropID, CaseID, RefPTLKey, 
               Storerkey, OrderKey, ConsigneeKey, SKU, LOC, LOT, UOM, Remarks, Func, GroupKey, 'isp_805PTL_Confirm01', ArchiveCop
            FROM PTL.PTLTran WITH (NOLOCK)
   			WHERE PTLKey = @nPTLKey
            IF @@ERROR <> 0
            BEGIN
   ...
   BEGIN
      ...
      BEGIN
         ...
         VALUES
            (@cPickSlipNo, @nCartonNo, @cCartonID, @cLabelLine, @cStorerKey, @cSKU, @nQTY, 'rdt.' + SUSER_SNAME(), GETDATE(), 'rdt.' + SUSER_SNAME(), GETDATE())
         IF @@ERROR <> 0
         BEGIN
            SET @nErrNo = 97466
...
--      SELECT @cDPLKey '@cDPLKey', @cPosition '@cPosition', @cSKU '@cSKU', @cLOC '@cLOC', @nExpectedQTY '@nExpectedQTY'
   ...
   COMMIT TRAN isp_805PTL_Confirm01
   GOTO Quit

   RollBackTran:
   ROLLBACK TRAN isp_805PTL_Confirm01 -- Only rollback change made here
