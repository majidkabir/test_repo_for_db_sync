...
/************************************************************************/
/* Store procedure: isp_801PFL_Confirm01                                */
/* Copyright      : LF Logistics                                        */
/*                                                                      */
/* Purpose: Accept QTY in CS-PCS, format 9-999                          */
/*                                                                      */
/* Date       Rev  Author   Purposes                                    */
/* 21-06-2019 1.0  Ung      WMS-9372 Created                            */
/* 27-08-2019 1.1  SPChin   INC0831212 - Remove TrafficCop              */  
/************************************************************************/

CREATE PROC [PTL].[isp_801PFL_Confirm01] (
...
BEGIN TRY
   ...
   SELECT 
      @cStation = DeviceID, 
      @cDisplayValue = LEFT( DisplayValue, 5), 
   ...
   SAVE TRAN isp_801PFL_Confirm01 -- For rollback or commit only our own transaction
      ...
      BEGIN
         ...
         BEGIN -- Have balance, need to split
            ...
            SELECT 
               ...
               IPAddress, DeviceID, DevicePosition, Status, LightUp, LightMode, LightSequence, PTLType, SourceKey, DropID, CaseID, RefPTLKey, 
               Storerkey, OrderKey, ConsigneeKey, SKU, LOC, LOT, UOM, Remarks, Func, GroupKey, 'isp_801PFL_Confirm01', ArchiveCop
            FROM PTL.PTLTran WITH (NOLOCK)
   			WHERE PTLKey = @nPTLKey
            IF @@ERROR <> 0
            BEGIN
   ...
   BEGIN
      -- Convert to inner
      IF @nInnerPack > 0
         SET @nExpectedQTY = @nExpectedQTY * @nInnerPack
   ...
   */
   ...
   COMMIT TRAN isp_801PFL_Confirm01
   GOTO Quit

   RollBackTran:
   ROLLBACK TRAN isp_801PFL_Confirm01 -- Only rollback change made here
