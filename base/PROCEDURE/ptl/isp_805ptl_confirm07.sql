...
/************************************************************************/
/* Store procedure: isp_805PTL_Confirm07                                */
...
CREATE PROC [PTL].[isp_805PTL_Confirm07] (
...
BEGIN TRY
   ...
   SAVE TRAN isp_805PTL_Confirm07 -- For rollback or commit only our own transaction
      ...
      BEGIN
         ...
         BEGIN -- Have balance, need to split
            ...
            SELECT
               ...
               IPAddress, DeviceID, DevicePosition, Status, LightUp, LightMode, LightSequence, PTLType, SourceKey, DropID, CaseID, RefPTLKey,
               Storerkey, OrderKey, ConsigneeKey, SKU, LOC, LOT, UOM, Remarks, Func, GroupKey, 'isp_805PTL_Confirm07', ArchiveCop
            FROM PTL.PTLTran WITH (NOLOCK)
            WHERE PTLKey = @nPTLKey
            IF @@ERROR <> 0
            BEGIN
  ...
  -- IF Pick and Pack matched light up with EnD
   ...
   COMMIT TRAN isp_805PTL_Confirm07
   GOTO Quit

   RollBackTran:
   ROLLBACK TRAN isp_805PTL_Confirm07 -- Only rollback change made here
