...
GO
/************************************************************************/                                                                                  
/* Store Procedure: lsp_WaveLoad_Delete                                 */                                                                                  
...
BEGIN                                                                                                                                                        
   ...
   DECLARE  @n_StartTCnt               INT = @@TRANCOUNT  
         ...
         ,  @c_SourceType              NVARCHAR(50)   = 'lsp_WaveLoad_Delete'
   ...
   BEGIN
      ...
      SET @c_ErrMsg = 'Delete Selected Load Plan?' 
   ...
   BEGIN TRY
      ...
      SELECT LD.LoadLineNumber
      FROM LOADPLANDETAIL LD WITH (NOLOCK)
      WHERE LD.LoadKey = @c_LoadKey
      ORDER BY LD.LoadLineNumber

      OPEN @CUR_DETAIL
   
      FETCH NEXT FROM @CUR_DETAIL INTO @c_LoadLineNumber                                                                                
                                    
      WHILE @@FETCH_STATUS <> -1
      BEGIN
         SET @b_Deleted = 1
         SET @c_ErrMsg  = ''
         EXEC [WM].[lsp_WaveLoadDetail_Delete]  
              @c_LoadKey            = @c_LoadKey                                                                                                                         
         ...
         BEGIN
            ...
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(6), @n_Err) + ': Error Executing lsp_WaveLoadDetail_Delete. (lsp_WaveLoad_Delete)'   
      ...
      BEGIN
         ...
         BEGIN CATCH
            ...
            SET @n_Err = 557052  
            SET @c_ErrMsg = ERROR_MESSAGE()  
            SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(6), @n_Err) + ': Delete Loadplan Fail. (lsp_WaveLoad_Delete)'     
                           + '(' + @c_ErrMsg + ')'   
...
EXIT_SP:
   ...
   BEGIN     
      ...
      BEGIN
         SET @c_ErrMsg = 'Delete Selected Loadplan is/are done.'

         IF @n_ErrGroupKey > 0 
         BEGIN
            SET @n_Continue = 3
            SET @c_ErrMsg = 'Process Delete Selected Loadplan is/are done with error(s).'
         END
      END
   ...
   BEGIN
      ...
      END
      SET @n_WarningNo = 0
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'lsp_WaveLoad_Delete'
   END
   ELSE
   BEGIN
      SET @b_Success = 1
