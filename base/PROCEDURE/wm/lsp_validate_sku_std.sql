...
GO
/*************************************************************************/  
/* Stored Procedure: lsp_Validate_Sku_Std                                */  
...
BEGIN
   ...
   BEGIN TRY
      ...
      */
      ...
      /*
      CREATE TABLE #SKU( Rowid  INT NOT NULL IDENTITY(1,1) )   
      ...
      IF LEN(@c_SQLSchema) > 0 
      BEGIN
         SET @c_SQL = N'ALTER TABLE #SKU  ADD  ' + SUBSTRING(@c_SQLSchema, 1, LEN(@c_SQLSchema) - 1) + ' '
            
         EXEC (@c_SQL)

         INSERT_REC:
         SET @c_SQL = N' INSERT INTO #SKU' --+  @c_UpdateTable 
                     + ' ( ' + SUBSTRING(@c_TableColumns, 1, LEN(@c_TableColumns) - 1) + ' )'
      ...
      */
      ...
      BEGIN
         SET @n_Continue = 3
         SET @n_err = 559352
         SET @c_errmsg = 'Invalid Packkey: ' + RTRIM(@c_Packkey)
                       + '. (lsp_Validate_Sku_Std)'
                       + ' |' +  RTRIM(@c_Packkey)
         GOTO EXIT_SP
      END
