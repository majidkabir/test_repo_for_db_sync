...
/*************************************************************************/  
/* Stored Procedure: lsp_PrePrint_PLIST_WAVE_LOR                         */  
/* Creation Date: 29-AUG-2022                                            */  
/* Copyright: LFL                                                        */  
/* Written by: WZPang                                                    */  
/*                                                                       */  
/* Purpose: WMS-20573 - [TW] LOR Pick Slip CR - PrePrint Process         */
/*                                                                       */  
/* Called By:                                                            */  
/*                                                                       */  
/*                                                                       */  
...
/* 29-AUG-2022 WZPang   1.0   DevOps Script Combine                      */
/*************************************************************************/   
CREATE PROCEDURE [WM].[lsp_PrePrint_PLIST_WAVE_LOR]  
...
BEGIN  
   ...
   BEGIN TRY 
      ...
      FROM WAVE (NOLOCK)
      WHERE WAVE.WaveKey = @c_Parm1
      AND @n_Where = (SELECT CASE WHEN MAX(WAVE.WaveType) = 'PTL' THEN 1 ELSE 0 END
                      FROM WAVE(NOLOCK)
                      WHERE WAVE.WaveKey = @c_Parm1)
   ...
   BEGIN CATCH
      SET @n_Continue = 3
      SET @c_ErrMsg = ERROR_MESSAGE() + '. (lsp_PrePrint_PLIST_WAVE_LOR)'
      GOTO EXIT_SP
   END CATCH
   ...
   BEGIN
      ...
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'lsp_PrePrint_PLIST_WAVE_LOR'
   END
   ELSE
   BEGIN
      SET @b_Success = 1
