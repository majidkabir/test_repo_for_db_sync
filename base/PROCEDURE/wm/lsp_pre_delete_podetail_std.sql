...
GO
/************************************************************************/  
/* Stored Procedure: lsp_Pre_Delete_PODetail_STD                        */  
/* Creation Date: 03-Apr-2018                                           */  
/* Copyright: LFLogistics                                               */  
/* Written by:                                                          */  
/*                                                                      */  
/* Purpose: PODetail Pre-delete process / validation                    */  
/*                                                                      */  
/* Called By: PODetail delete                                           */  
/*                                                                      */  
/* PVCS Version: 1.0                                                    */  
/*                                                                      */  
/* Version: 8.0                                                         */  
...
/************************************************************************/   
CREATE PROCEDURE [WM].[lsp_Pre_Delete_PODetail_STD]
   ...
   */   
   ...
   BEGIN TRY       
      ...
      BEGIN          
         ...
         BEGIN
            SELECT @n_continue = 3
            SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err = 550901
            SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(6),@n_err)+': Cannot delete PODETAIL where items have been received. (lsp_Pre_Delete_PODetail_STD)'                
         END                         
      END
   END TRY
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         END  
      END  
      execute nsp_logerror @n_err, @c_errmsg, 'lsp_Pre_Delete_PODetail_STD'  
      RETURN  
   END  
   ELSE  
   BEGIN  
