...
GO
/************************************************************************/  
/* Stored Procedure: lsp_Pre_Delete_Transfer_STD                        */  
/* Creation Date: 25-SEP-2018                                           */  
/* Copyright: LFLogistics                                               */  
/* Written by:                                                          */  
/*                                                                      */  
/* Purpose: Transfer Pre-delete process / validation                    */  
/*                                                                      */  
/* Called By: Transfer delete                                           */  
/*                                                                      */  
/* PVCS Version: 1.0                                                    */  
/*                                                                      */  
/* Version: 8.0                                                         */  
...
/************************************************************************/   
CREATE PROCEDURE [WM].[lsp_Pre_Delete_Transfer_STD]
...
BEGIN
   ...
   BEGIN TRY
      ...
      BEGIN          
         ...
         BEGIN
            SELECT @n_continue = 3
            SELECT @c_errmsg = CONVERT(NVARCHAR(250),@n_err), @n_err = 551201
            SELECT @c_errmsg='NSQL'+CONVERT(NVARCHAR(6),@n_err)+': Cannot delete Finalized Transfer. (lsp_Pre_Delete_Transfer_STD)'                
         END                         
      END
   END TRY
   ...
   BEGIN  
      ...
      BEGIN  
         ...
         END  
      END  
      execute nsp_logerror @n_err, @c_errmsg, 'lsp_Pre_Delete_Transfer_STD'  
      RETURN  
   END  
   ELSE  
   BEGIN  
