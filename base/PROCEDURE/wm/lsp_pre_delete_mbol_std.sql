...
GO
/************************************************************************/  
/* Stored Procedure: lsp_Pre_Delete_MBOL_STD                            */  
...
/************************************************************************/   
CREATE PROCEDURE [WM].[lsp_Pre_Delete_MBOL_STD]
...
BEGIN
   ...
   BEGIN TRY
      ...
      BEGIN
         SET @n_continue = 3
         SET @n_err = 557401
         SET @c_errmsg='NSQL'+CONVERT(NVARCHAR(6),@n_err)+': Cannot delete shipped ' + @c_MBOLkey + ' (lsp_Pre_Delete_MBOL_STD) |' + @c_MBOLkey    
         GOTO EXIT_SP             
      END
   END TRY
...
EXIT_SP:
   ...
   ELSE  
      BEGIN  
         ...
         END  
      END  
      execute nsp_logerror @n_err, @c_errmsg, 'lsp_Pre_Delete_MBOL_STD'
      RETURN  
   END  
   ELSE  
   BEGIN  
