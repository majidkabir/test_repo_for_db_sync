...
GO
/*************************************************************************/  
/* Stored Procedure: lsp_CCReleaseCSBySku_Wrapper                        */  
...
/*************************************************************************/   
CREATE PROCEDURE [WM].[lsp_CCReleaseCSBySku_Wrapper]  
...
BEGIN  
   ...
   BEGIN TRY  
      ...
      SELECT @n_Count = 1
      FROM CC WITH (NOLOCK)
      WHERE CC.Storerkey = @c_Storerkey
      AND CC.Sku = @c_Sku
      AND CC.Loc = @c_Loc
      AND CC.Status = '0'
      ...
      BEGIN
         ...
         BEGIN CATCH
            ...
            SET @c_errmsg = 'NSQL' +CONVERT(CHAR(6),@n_err) 
                          + ': Error Executing nspg_getkey - CCKey. (lsp_CCReleaseCSBySku_Wrapper)'
                          + '( ' + @c_errmsg + ' )'
         END CATCH    
         ...
         BEGIN
            ...
            BEGIN CATCH
               ...
               SET @c_ErrMsg = ERROR_MESSAGE()
               SET @c_errmsg = 'NSQL' +CONVERT(CHAR(6),@n_err) 
                             + ': Error Executing nspg_getkey - CCSheetNo. (lsp_CCReleaseCSBySku_Wrapper)'
                             + '( ' + @c_errmsg + ' )'
            END CATCH    
         ...
         BEGIN CATCH
            SET @n_err = 550103
            SET @c_ErrMsg = ERROR_MESSAGE()
            SET @c_errmsg = 'NSQL' +CONVERT(CHAR(6),@n_err) + ': Insert Into CC Fail. (lsp_CCReleaseCSBySku_Wrapper)'
                           + '( ' + @c_errmsg + ' )'
         END CATCH    
         ...
         BEGIN
            ...
            BEGIN CATCH
               SET @n_err = 550104
               SET @c_ErrMsg = ERROR_MESSAGE()
               SET @c_errmsg = 'NSQL' +CONVERT(CHAR(6),@n_err) + ': Error Executing nspg_getkey - CCDetailKey. (lsp_CCReleaseCSBySku_Wrapper)'
                              + '( ' + @c_errmsg + ' )'
            END CATCH    
            ...
            BEGIN CATCH
               SET @n_err = 550105
               SET @c_ErrMsg = ERROR_MESSAGE()
               SET @c_errmsg = 'NSQL' +CONVERT(CHAR(6),@n_err) + ': Insert Into CCDETAIL Fail. (lsp_CCReleaseCSBySku_Wrapper)'
                             + '( ' + @c_errmsg + ' )'
            END CATCH    
...
EXIT_SP:
   ...
   BEGIN
      ...
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'lsp_CCReleaseCSBySku_Wrapper'
   END
   ELSE
   BEGIN
      SET @b_Success = 1
