...
GO
/*************************************************************************/  
/* Stored Procedure: lsp_Pre_Delete_Wrapper                              */  
...
BEGIN  
   ...
   DECLARE @c_PrimaryKey         NVARCHAR(128) = ''
         ...
         , @c_StorerKeyColName   NVARCHAR(60)

   -- Not doing anything if table not exists
   ...
   SELECT TOP 1 @c_StorerKeyColName = RTRIM(Column_Name) 
   FROM INFORMATION_SCHEMA.COLUMNS c
   WHERE c.TABLE_NAME = @c_TableName
   AND   c.TABLE_SCHEMA = @c_Schema
   AND   c.Column_Name LIKE '%Storerkey' 
   ORDER BY ORDINAL_POSITION      
    
   IF @c_StorerKeyColName <> ''  
   BEGIN                  
      ...
      BEGIN
         ...
         IF @c_SQL = ''
         BEGIN
            SET @c_SQL = N'SELECT @c_StorerKey = ' + @c_StorerKeyColName + ' FROM ' + @c_Schema + '.' + @c_TableName + ' WITH (NOLOCK)'
            + CHAR(13) + N' WHERE ' + @c_PrimaryKey 
