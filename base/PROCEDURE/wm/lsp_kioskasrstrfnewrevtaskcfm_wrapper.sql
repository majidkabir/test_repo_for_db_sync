...
GO
/*************************************************************************/  
/* Stored Procedure: WM.lsp_KioskASRSTrfNewRevTaskCfm_Wrapper            */  
...
/*************************************************************************/   
CREATE PROCEDURE [WM].[lsp_KioskASRSTrfNewRevTaskCfm_Wrapper]  
...
BEGIN  
   ...
   BEGIN TRY
      ...
      BEGIN TRY  
         IF @c_ConfirmAt = 'c'
         BEGIN    
            EXEC isp_KioskASRSTRFNewRevTaskCfm
               @c_Jobkey            = @c_Jobkey
         ...
         ELSE
         BEGIN
            EXEC isp_KioskASRSTRFRevTaskCfm
               @c_Jobkey            = @c_Jobkey
            ,  @c_TaskDetailkey     = @c_TaskDetailkey 
            ,  @c_TransferKey       = @c_TransferKey
            ,  @c_TransferLineNumber= @c_TransferLineNumber
      ...
      BEGIN CATCH
         ...
         SET @c_ErrMsg = ERROR_MESSAGE()
         SET @c_ErrMsg = 'NSQL' + CONVERT(CHAR(6), @n_err) 
                       + ': Transfer Sku Confirm Pick Fail. ' +  @c_ErrMsg 
      END CATCH  
      ...
      SET @c_ErrMsg = 'Transfer Sku Confirmed Pick.'
...
EXIT_SP:
   ...
   BEGIN
      ...
      IF @n_AlertNo = 0
      BEGIN
         EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'lsp_KioskASRSTrfNewRevTaskCfm_Wrapper'
      END
   END
   ELSE
   BEGIN
