...
/************************************************************************/
/* Stored Procedure: isp_WM_BackendReleaseShortAllocOrder               */
...
/* 09-OCT-2023  NJOW01  1.0   WMS-23852 Allow skip lottable filtering   */
/* 09-OCT-2023  NJOW01  1.0   DEVOPS Combine Script                     */
/************************************************************************/
CREATE   PROC [WM].[isp_WM_BackendReleaseShortAllocOrder] (
...
BEGIN
   DECLARE @n_AllocBatchNo    BIGINT = 0, 
           ...
           @c_SkipLotChk      NVARCHAR(30)   = ''   --NJOW01
   ...
   AND NOT EXISTS (SELECT 1 FROM BUILDPARMGROUPCFG  AS bpc WITH(NOLOCK)
                   JOIN BUILDPARM AS bp WITH(NOLOCK) ON  bp.ParmGroup = bpc.ParmGroup
                   WHERE bpc.[Type] = 'BackEndAlloc'
                     AND bpc.Facility = OH.Facility
                     AND bpc.Storerkey = OH.StorerKey
                     AND bp.[Active] = '1'
                     AND OH.UpdateSource = '1')
