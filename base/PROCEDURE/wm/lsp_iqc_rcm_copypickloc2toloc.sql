...
GO
/*************************************************************************/  
/* Stored Procedure: WM.lsp_IQC_RCM_CopyPickLoc2ToLoc                    */  
...
BEGIN  
   ...
   DECLARE @t_IQCDetail          TABLE
         ...
         , PickLoc               NVARCHAR(10) NOT NULL DEFAULT('')
         )
   ...
   BEGIN TRY
      ...
      JOIN dbo.SKUxLOC AS sul WITH (NOLOCK) ON sul.StorerKey = iq.StorerKey AND sul.Sku = iq.SKU
      JOIN dbo.LOC AS l WITH (NOLOCK) ON l.Loc = sul.Loc
      WHERE iq.QC_Key = @c_QC_Key
      AND sul.LocationType IN ('PICK', 'CASE')
      AND l.Facility = @c_ToFacility
      ORDER BY ROW_NUMBER() OVER (PARTITION BY iq.QC_Key, iq.QCLineNo ORDER BY sul.Loc)
      ...
      SET   iq.ToLoc = ti.PickLoc
         ,  iq.TrafficCop = NULL
      FROM InventoryQCDetail iq 
      JOIN @t_IQCDetail ti ON ti.QC_Key = iq.QC_Key AND ti.QCLineNo = iq.QCLineNo
      
      SET @c_RowCount = @@ROWCOUNT
      
      IF @@ERROR <> 0 
      BEGIN
         SET @n_Continue = 3
         SET @n_Err = 559951
         SET @c_errmsg = 'NSQL' +CONVERT(CHAR(6),@n_err) + ': Error on Update InventoryQCDetail table. (lsp_IQC_RCM_CopyPickLoc2ToLoc)'
         GOTO EXIT_SP
      END
      
      SET @c_errmsg = CONVERT(NVARCHAR(5),@c_RowCount) + ' Pick Locations Copied.'
   END TRY
   ...
   BEGIN
      ...
      EXECUTE nsp_logerror @n_err, @c_ErrMsg, 'lsp_IQC_RCM_CopyPickLoc2ToLoc'
   END
   ELSE
   BEGIN
      SET @b_Success = 1
