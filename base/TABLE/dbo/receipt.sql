...
(
    ...
    [HoldChannel] nvarchar(1) NOT NULL DEFAULT ('0'),
    [TrackingNo] nvarchar(40) NULL DEFAULT (''),
    [TotalExpectedQty] int NULL,
    [TotalReceivedQty] int NULL,
    CONSTRAINT [PKRECEIPT] PRIMARY KEY ([ReceiptKey]),
    CONSTRAINT [CK_RECEIPT_Status] CHECK (rtrim([Status]) like '[0-9]')
);
GO

CREATE INDEX [IDX_RECEIPT_WarehouseReference] ON [dbo].[receipt] ([WarehouseReference], [StorerKey]);
GO
CREATE INDEX [IX_RECEIPT_ExternReceiptKey] ON [dbo].[receipt] ([StorerKey], [ExternReceiptKey]);
GO
CREATE INDEX [IX_Receipt_TrackingNo] ON [dbo].[receipt] ([TrackingNo]);
GO
CREATE INDEX [IX_VEHICLETRANS_FacilityVehicleNumber] ON [dbo].[receipt] ([Facility], [VehicleNumber]);
GO