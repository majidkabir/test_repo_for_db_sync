...
/******************************************************************************************/
CREATE   VIEW [BI].[V_TH_CITYFR_2_Holding_inout]
AS
select Principal,Type,Date,WMSDoc,CTXDoc,ShipToFrom,Name,Sku,Descr,StockStatus,CD,Brand,ReceiveDate,case when Qty>=0 then qty end as QtyIn,case when Qty<0 then qty end as QtyOut,UOM,Lottable06,lot06
from
(
select case when rd.Lottable06='CTX' then 'CTX Holding' when rd.Lottable06='CFF' then 'CITYFR' when rd.Lottable06='T&G' then 'T&G' else 'Other' end as Principal,
	...
	p.PackUOM3 as UOM,
	case when rd.Lottable06='CTX' then 'CTX Holding' when rd.Lottable06='CFF' then 'CITYFR' when rd.Lottable06='T&G' then 'T&G' else 'Other' end as Lottable06,
...
where a.lottable06 = 'CTX' and a.CD not like ('AB%') and lot06 ='CTX'
