...
GO
CREATE FUNCTION [dbo].[fnc_GetVC_prTaskLUTCoreConfiguration_AckData]
(
   @nSerialNo INT
)
RETURNS @tCoreConfig TABLE 
        (
            CustName NVARCHAR(100) NULL
           ,OperatorID NVARCHAR(50) NULL
           ,ConfirmPwd NVARCHAR(1) NULL
...
BEGIN
   DECLARE @c_AckData NVARCHAR(4000)
          ,@c_CustName NVARCHAR(100)
          ,@c_OperatorID NVARCHAR(50)
          ,@c_ConfirmPwd NVARCHAR(1)
   ...
   BEGIN
       ...
       IF @n_SeqNo = 1
           SET @c_CustName = @c_ColValue
       
       IF @n_SeqNo = 2
           SET @c_OperatorID = @c_ColValue

      IF @n_SeqNo = 3
           SET @c_ConfirmPwd = @c_ColValue
