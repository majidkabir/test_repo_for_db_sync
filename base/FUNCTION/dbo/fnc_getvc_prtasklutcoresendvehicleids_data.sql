...
GO
CREATE FUNCTION [dbo].[fnc_GetVC_prTaskLUTCoreSendVehicleIDs_Data]
(
   @nSerialNo INT
)
RETURNS @tCoreSendVehicleIDData TABLE 
        (
...
SET @c_Parms = REPLACE(SUBSTRING(@c_Data, @n_StartPos + 1, (@n_EndPos - @n_StartPos) -1) ,'''','') 
   ...
   BEGIN
       ...
       IF @n_SeqNo = 4  SET @c_VehicleType     = @c_ColValue 
       IF @n_SeqNo = 5  SET @c_VehicleID       = @c_ColValue           
             
             
  FETCH NEXT FROM CUR1 INTO @n_SeqNo, @c_ColValue
   END
   INSERT INTO @tCoreSendVehicleIDData
     (
       TranDate
      ,DeviceSerialNo
      ,OperatorID
