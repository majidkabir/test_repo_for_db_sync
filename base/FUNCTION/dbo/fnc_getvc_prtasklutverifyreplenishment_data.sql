...
GO
CREATE FUNCTION [dbo].[fnc_GetVC_prTaskLUTVerifyReplenishment_Data]
(
   @nSerialNo INT
)
...
BEGIN
   DECLARE @c_Data            NVARCHAR(4000)
          ...
          ,@c_LocName         NVARCHAR(20)
          ,@c_ItemNo          NVARCHAR(20) 
...
SET @c_Parms = REPLACE(SUBSTRING(@c_Data, @n_StartPos + 1, (@n_EndPos - @n_StartPos) -1) ,'''','') 
   ...
   BEGIN
       ...
       IF @n_SeqNo = 4  SET @c_LocName         = @c_ColValue 
       IF @n_SeqNo = 5  SET @c_ItemNo          = @c_ColValue           
