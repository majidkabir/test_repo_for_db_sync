...
GO
CREATE FUNCTION [dbo].[fnc_GetVC_prTaskLUTCoreSafetyCheck_Data]
(
   @nSerialNo INT
)
RETURNS @tCoreSafeCheckData TABLE 
        (
            TranDate          NVARCHAR(20) NULL
           ,DeviceSerialNo    NVARCHAR(20) NULL
           ,OperatorID        NVARCHAR(20) NULL
           ,OperatorSaid      NVARCHAR(1) NULL
           ,Failure           NVARCHAR(100) NULL 
           ,RepairAction      NVARCHAR(100) NULL
        )
AS
...
SET @c_Parms = REPLACE(SUBSTRING(@c_Data, @n_StartPos + 1, (@n_EndPos - @n_StartPos) -1) ,'''','') 
   ...
   BEGIN
       ...
       IF @n_SeqNo = 4  SET @c_OperatorSaid    = @c_ColValue 
       IF @n_SeqNo = 5  SET @c_Failure         = @c_ColValue           
       IF @n_SeqNo = 6  SET @c_RepairAction    = @c_ColValue
       
             
       FETCH NEXT FROM CUR1 INTO @n_SeqNo, @c_ColValue
   END
   INSERT INTO @tCoreSafeCheckData
     (
       TranDate
      ,DeviceSerialNo
      ,OperatorID
