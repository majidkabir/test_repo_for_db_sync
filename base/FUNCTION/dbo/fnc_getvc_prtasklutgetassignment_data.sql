...
GO
CREATE FUNCTION [dbo].[fnc_GetVC_prTaskLUTGetAssignment_Data]
(
   @nSerialNo INT
)
RETURNS @tGetAssignmentData TABLE 
        (
            TranDate          NVARCHAR(20) NULL
           ,DeviceSerialNo    NVARCHAR(20) NULL
           ,OperatorID        NVARCHAR(20) NULL
           ,MaxNoAssgnmnt     NVARCHAR(5) NULL
           ,AssignMntType     NVARCHAR(2) NULL 
         )
AS
...
BEGIN
   DECLARE @c_Data            NVARCHAR(4000)
          ...
          ,@c_MaxNoAssgnmnt   NVARCHAR(5)  -- Operator╬ô├ç├┐s response to picking region prompt.  
...
SET @c_Parms = REPLACE(SUBSTRING(@c_Data, @n_StartPos + 1, (@n_EndPos - @n_StartPos) -1) ,'''','') 
   ...
   BEGIN
       ...
       IF @n_SeqNo = 4  SET @c_MaxNoAssgnmnt   = @c_ColValue 
       IF @n_SeqNo = 5  SET @c_AssignMntType   = @c_ColValue           
            
             
 FETCH NEXT FROM CUR1 INTO @n_SeqNo, @c_ColValue
   END
   INSERT INTO @tGetAssignmentData
     (
       TranDate
      ,DeviceSerialNo
      ,OperatorID
