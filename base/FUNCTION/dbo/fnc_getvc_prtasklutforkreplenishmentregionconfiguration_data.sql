...
GO
CREATE FUNCTION [dbo].[fnc_GetVC_prTaskLUTForkReplenishmentRegionConfiguration_Data]
(
   @nSerialNo INT
)
RETURNS @tReplenishmentRegionConfigData TABLE 
        (
...
SET @c_Parms = REPLACE(SUBSTRING(@c_Data, @n_StartPos + 1, (@n_EndPos - @n_StartPos) -1) ,'''','') 
  ...
  FETCH NEXT FROM CUR1 INTO @n_SeqNo, @c_ColValue
   END
   INSERT INTO @tReplenishmentRegionConfigData
     (
       TranDate
      ,DeviceSerialNo
      ,OperatorID
