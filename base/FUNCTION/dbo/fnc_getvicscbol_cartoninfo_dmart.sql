...
/************************************************************************/    
/* Store procedure: fnc_GetVicsCBOL_CartonInfo_DMART                    */    
/* Copyright      : IDS                                                 */    
/* FBR:                                                                 */    
/* Purpose: Report                                                      */    
/*                                                                      */    
...
CREATE FUNCTION [dbo].[fnc_GetVicsCBOL_CartonInfo_DMART] (@nCBOLKey BIGINT)      
...
BEGIN      
   ...
   FROM DATAMART.ODS.MBOL MB WITH (NOLOCK)     
   JOIN DATAMART.ODS.MBOLDETAIL MD WITH (NOLOCK) ON MB.Mbolkey = MD.Mbolkey    
   JOIN DATAMART.ODS.ORDERS O WITH (NOLOCK) ON MD.Orderkey = O.Orderkey    
   ...
   IF EXISTS(SELECT 1 FROM DATAMART.ODS.MBOL MB (NOLOCK)     
             JOIN DATAMART.ODS.MBOLDETAIL MD (NOLOCK) ON MB.Mbolkey = MD.Mbolkey    
             JOIN DATAMART.ODS.ORDERS O (NOLOCK) ON MD.Orderkey = O.Orderkey    
   ...
   BEGIN                       
    ...
    FROM   DATAMART.ODS.PICKDETAIL p WITH (NOLOCK)     
    JOIN   DATAMART.ODS.PACKDETAIL PD WITH (NOLOCK) ON PD.PickSlipNo = P.PickSlipNo     
                            AND PD.DropID = P.DropID    
    JOIN  DATAMART.ODS.PACKHEADER PH WITH (NOLOCK) ON PD.Pickslipno = PH.Pickslipno    
    JOIN  DATAMART.ODS.MBOLDETAIL MD WITH (NOLOCK) ON MD.OrderKey = P.OrderKey     
    JOIN  DATAMART.ODS.MBOL MB WITH (NOLOCK) ON MD.Mbolkey = MB.Mbolkey    
--     JOIN  ORDERDETAIL OD WITH (NOLOCK) ON PH.ConsoOrderkey = OD.ConsoOrderkey     
--                                        AND PD.Storerkey = OD.Storerkey    
--                                        AND PD.Sku = OD.Sku     
    JOIN  DATAMART.ODS.ORDERDETAIL OD WITH (NOLOCK) ON P.OrderKey = OD.OrderKey     
    ...
    FROM @tPack TP    
    JOIN DATAMART.ODS.PackInfo pi1 WITH (NOLOCK) ON pi1.PickSlipNo = TP.PickSlipNo AND pi1.CartonNo = TP.CartonNo    
    ...
    BEGIN    
       ...
       JOIN (SELECT PD.PickSlipNo, PD.CartonNo, SUM(ISNULL(S.STDGROSSWGT,0) * PD.Qty) AS [WEIGHT]      
             FROM DATAMART.ODS.PACKDETAIL PD WITH (NOLOCK)     
   ...
   BEGIN    
       ...
       SELECT MB.CBOLKEY, SUM(MBOLDET.TotalCartons) AS TTLCTN ,    
               ...
               ''    
        FROM DATAMART.ODS.MBOL MB WITH (NOLOCK)    
        JOIN DATAMART.ODS.MBOLDETAIL MBOLDET WITH (NOLOCK) ON (MB.Mbolkey = MBOLDET.Mbolkey)    
        WHERE MB.CBOLKEY = @nCBOLKey     
        AND ISNULL(MB.CBOLKEY,0) <> 0    
        GROUP BY MB.CBOLKEY    
   END    
