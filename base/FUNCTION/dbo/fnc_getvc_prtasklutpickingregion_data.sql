...
GO
CREATE FUNCTION [dbo].[fnc_GetVC_prTaskLUTPickingRegion_Data]
(
   @nSerialNo INT
)
RETURNS @tPickRegData TABLE 
        (
            TranDate          NVARCHAR(20) NULL
           ,DeviceSerialNo    NVARCHAR(20) NULL
           ,OperatorID        NVARCHAR(20) NULL
           ,RegionNo          NVARCHAR(5) NULL
           ,SelectedFunc      NVARCHAR(2) NULL 
         )
AS
...
SET @c_Parms = REPLACE(SUBSTRING(@c_Data, @n_StartPos + 1, (@n_EndPos - @n_StartPos) -1) ,'''','') 
   ...
   BEGIN
       ...
       IF @n_SeqNo = 5  SET @c_SelectedFunc    = @c_ColValue 
                 
             
 FETCH NEXT FROM CUR1 INTO @n_SeqNo, @c_ColValue
   END
   INSERT INTO @tPickRegData
     (
       TranDate
      ,DeviceSerialNo
      ,OperatorID
