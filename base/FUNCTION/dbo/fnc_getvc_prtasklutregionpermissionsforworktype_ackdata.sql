...
GO
CREATE FUNCTION [dbo].[fnc_GetVC_prTaskLUTRegionPermissionsForWorkType_AckData]
(
   @nSerialNo INT
)
RETURNS @tRegionWorkTypeTask TABLE 
        (
            RegionNo        NVARCHAR(10) NULL
...
BEGIN
   ...
   BEGIN
       ...
       IF @n_SeqNo = 1 SET @c_RegionNo = @c_ColValue
       
       IF @n_SeqNo = 2 SET @c_RegionName = @c_ColValue
            
       IF @n_SeqNo = 3 SET @c_ErrorCode = @c_ColValue
       
       IF @n_SeqNo = 4 SET @c_ErrorMessage = @c_ColValue
       
       FETCH NEXT FROM CUR1 INTO @n_SeqNo, @c_ColValue
   END
   INSERT INTO @tRegionWorkTypeTask
     (
       RegionNo
      ,RegionName
      ,ErrorCode
