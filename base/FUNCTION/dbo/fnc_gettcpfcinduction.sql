...
/* 05-04-2012   Shong         Copy from US Production                	  */
/************************************************************************/
CREATE FUNCTION [dbo].[fnc_GetTCPFCInduction] (@nSerialNo INT)  
RETURNS @tFCInduction TABLE   
(  
...
AS  
BEGIN  
WITH FCInduction(SerialNo, MessageNum, MessageType, StorerKey, Facility, LPNNo, OrderKey, OrderLineNumber,   
                 ConsoOrderKey, SKU, Qty_Actual, FROMLOC, TOLOC, TransCode, [STATUS]) -- Table name and columns  
    AS (  
     SELECT ti.SerialNo,   
            ti.MessageNum,   
            ...
            ISNULL(RTRIM(SubString(ti.[Data],  109,  20)),'') AS SKU,  
             CASE WHEN ISNUMERIC(RTRIM(SubString(ti.[Data],  129,  10))) = 1   
                 ...
                 ELSE 0   
            END  AS Qty_Actual,  
            ISNULL(RTRIM(SubString(ti.[Data], 139,  10)),'') AS FROMLOC,  
            ISNULL(RTRIM(SubString(ti.[Data], 149,  10)),'') AS TOLOC,                          
            ISNULL(RTRIM(SubString(ti.[Data], 159,   5)),'') AS TransCode,   
            ti.[Status]  
     FROM TCPSocket_INLog ti WITH (NOLOCK)  
--     WHERE ti.SerialNo = @nSerialNo
     WHERE ti.Data LIKE 'FULLCASE%'
...
-- copy the required columns to the result of the function   
   ...
   SELECT SerialNo, MessageNum, MessageType, StorerKey, Facility, LPNNo, OrderKey, OrderLineNumber,   
          ConsoOrderKey, SKU, Qty_Actual, FROMLOC, TOLOC, TransCode, [STATUS]  
   FROM FCInduction   
   RETURN  
END;
