...
GO
CREATE FUNCTION [dbo].[fnc_GetVC_prTaskLUTCoreConfiguration_Data]
(
   @nSerialNo INT
)
RETURNS @tCoreConfigData TABLE 
        (
...
SET @c_Parms = REPLACE(SUBSTRING(@c_Data, @n_StartPos + 1, (@n_EndPos - @n_StartPos) -1) ,'''','') 
   ...
   BEGIN
       ...
       IF @n_SeqNo = 4  SET @c_LanguageCode    = @c_ColValue 
       IF @n_SeqNo = 5  SET @c_Site            = @c_ColValue           
       IF @n_SeqNo = 6  SET @c_Version         = @c_ColValue
       
             
       FETCH NEXT FROM CUR1 INTO @n_SeqNo, @c_ColValue
   END
   INSERT INTO @tCoreConfigData
     (
       TranDate
      ,DeviceSerialNo
      ,OperatorID
