...
GO
CREATE FUNCTION [dbo].[fnc_GetVC_prTaskLUTForkValidReplenishmentRegions_AckData]
(
   @nSerialNo INT
)
RETURNS @tValidReplenishmentRegions TABLE 
        (
...
BEGIN
   ...
   BEGIN
       ...
       IF @n_SeqNo = 1
           SET @c_RegionNo = @c_ColValue
       
       IF @n_SeqNo = 2
           SET @c_RegionName = @c_ColValue
       ...
       FETCH NEXT FROM CUR1 INTO @n_SeqNo, @c_ColValue
   END
   INSERT INTO @tValidReplenishmentRegions
     (
       RegionNo
      ,RegionName
      ,ErrorCode
