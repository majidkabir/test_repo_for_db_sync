...
GO
CREATE FUNCTION [dbo].[fnc_GetVC_prTaskLUTCoreValidVehicleTypes_AckData]
(
   @nSerialNo INT
)
RETURNS @tCoreValidVechileTypes TABLE 
        (   [VechileType] NVARCHAR(2)
           ,[VechileTypeDescr] NVARCHAR(100)
           ,[CaptureID] NVARCHAR(1)
...
BEGIN
   DECLARE @c_AckData NVARCHAR(4000)
          ,@c_VechileType NVARCHAR(2)
          ,@c_VechileTypeDescr NVARCHAR(100)
          ,@c_CaptureID NVARCHAR(1)
   ...
   BEGIN
       ...
       IF @n_SeqNo = 1
           SET @c_VechileType = @c_ColValue
       
       IF @n_SeqNo = 2
           SET @c_VechileTypeDescr = @c_ColValue
       
       IF @n_SeqNo = 3
           SET @c_CaptureID = @c_ColValue
       ...
       FETCH NEXT FROM CUR1 INTO @n_SeqNo, @c_ColValue
   END
   INSERT INTO @tCoreValidVechileTypes
     ( VechileType
      ,VechileTypeDescr
      ,CaptureID
      ,ErrorCode
