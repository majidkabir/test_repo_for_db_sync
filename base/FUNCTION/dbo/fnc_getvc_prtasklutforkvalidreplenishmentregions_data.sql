...
GO
CREATE FUNCTION [dbo].[fnc_GetVC_prTaskLUTForkValidReplenishmentRegions_Data]
(
   @nSerialNo INT
)
RETURNS @tValidReplenishmentRegData TABLE 
        (
...
SET @c_Parms = REPLACE(SUBSTRING(@c_Data, @n_StartPos + 1, (@n_EndPos - @n_StartPos) -1) ,'''','') 
  ...
  FETCH NEXT FROM CUR1 INTO @n_SeqNo, @c_ColValue
   END
   INSERT INTO @tValidReplenishmentRegData
     (
       TranDate
      ,DeviceSerialNo
      ,OperatorID
