...
GO
CREATE FUNCTION [dbo].[fnc_GetVC_prTaskLUTValidLots_Data]
(
   @nSerialNo INT
)
RETURNS @tValidLotsData TABLE 
        (
...
SET @c_Parms = REPLACE(SUBSTRING(@c_Data, @n_StartPos + 1, (@n_EndPos - @n_StartPos) -1) ,'''','') 
   ...
   BEGIN
       ...
       IF @n_SeqNo = 7  SET @c_SKU             = @c_ColValue
       IF @n_SeqNo = 8  SET @c_SeqNo           = @c_ColValue
                    
             
  FETCH NEXT FROM CUR1 INTO @n_SeqNo, @c_ColValue
   END
   INSERT INTO @tValidLotsData
     (
       TranDate
      ,DeviceSerialNo
      ,OperatorID
