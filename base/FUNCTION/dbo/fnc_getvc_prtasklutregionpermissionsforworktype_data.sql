...
GO
CREATE FUNCTION [dbo].[fnc_GetVC_prTaskLUTRegionPermissionsForWorkType_Data]
(
   @nSerialNo INT
)
RETURNS @tWorkTypePermissionData TABLE 
        (
            TranDate          NVARCHAR(20) NULL
           ,DeviceSerialNo    NVARCHAR(20) NULL
           ,OperatorID        NVARCHAR(20) NULL
           ,SelWorkType       NVARCHAR(20) NULL
        )
AS
...
SET @c_Parms = REPLACE(SUBSTRING(@c_Data, @n_StartPos + 1, (@n_EndPos - @n_StartPos) -1) ,'''','') 
   ...
   BEGIN
       ...
       IF @n_SeqNo = 4  SET @c_SelWorkType     = @c_ColValue 
                     
             
 FETCH NEXT FROM CUR1 INTO @n_SeqNo, @c_ColValue
   END
   INSERT INTO @tWorkTypePermissionData
     (
       TranDate
      ,DeviceSerialNo
      ,OperatorID
