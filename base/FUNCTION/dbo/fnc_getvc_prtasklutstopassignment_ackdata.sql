...
GO
CREATE FUNCTION [dbo].[fnc_GetVC_prTaskLUTStopAssignment_AckData]
(
   @nSerialNo INT
)
RETURNS @tStopAssigmentTask TABLE 
        (
...
BEGIN
   ...
   BEGIN
       ...
       IF @n_SeqNo = 1 SET @c_ErrorCode = @c_ColValue       
       IF @n_SeqNo = 2 SET @c_ErrorMessage = @c_ColValue
       
       FETCH NEXT FROM CUR1 INTO @n_SeqNo, @c_ColValue
   END
   INSERT INTO @tStopAssigmentTask
     (
       ErrorCode
      ,ErrorMessage
     )
