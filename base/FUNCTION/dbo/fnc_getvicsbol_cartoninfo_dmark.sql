...
/************************************************************************/  
/* Store procedure: fnc_GetVicsBOL_CartonInfo_DMark                     */  
/* Copyright      : IDS                                                 */  
/* FBR:                                                                 */  
/* Purpose: Report                                                      */  
/*                                                                      */  
...
CREATE FUNCTION [dbo].[fnc_GetVicsBOL_CartonInfo_DMark] (@cMBOLKey NVARCHAR(10), @cConsigneeKey NVARCHAR(15))  
...
BEGIN  
   ...
   FROM DATAMART.ODS.MBOLDETAIL MD (NOLOCK)  
   JOIN DATAMART.ODS.ORDERS O ON MD.Orderkey = O.Orderkey  
   ...
   IF EXISTS(SELECT 1 FROM DATAMART.ODS.ORDERDETAIL o WITH (NOLOCK) WHERE o.MBOLKey = @cMBOLKey 
   ...
   BEGIN      	     	     	
     INSERT INTO @tPack (PickSlipNo, LabelNo, CartonNo, [WEIGHT], [CUBE], ExternConsoOrderkey)
	   ...
	   JOIN   DATAMART.ODS.ORDERS O WITH (NOLOCK) ON P.OrderKey = O.OrderKey 
...
--                                        AND PD.Sku = OD.Sku   
    JOIN  DATAMART.ODS.ORDERDETAIL OD WITH (NOLOCK) ON P.OrderKey = OD.OrderKey   
	   ...
	   BEGIN
	      ...
	      JOIN (SELECT PD.PickSlipNo, PD.CartonNo, SUM(ISNULL(S.STDGROSSWGT,0) * PD.Qty) AS [WEIGHT]  
	            ...
	            JOIN DATAMART.ODS.SKU S WITH (NOLOCK) ON S.StorerKey = PD.StorerKey AND S.SKU = PD.SKU 
   ...
   BEGIN
       ...
       SELECT MBOLDET.MBOLKEY AS MBLKey, SUM(MBOLDET.TotalCartons) AS TTLCTN ,
               ...
               ''
        FROM DATAMART.ODS.MBOLDETAIL MBOLDET WITH (NOLOCK)
        JOIN DATAMART.ODS.ORDERS ORD WITH (NOLOCK) ON (ORD.OrderKey = MBOLDET.OrderKey)
