...
GO
CREATE FUNCTION [dbo].[fnc_GetVC_prTaskLUTCoreValidVehicleTypes_Data]
(
   @nSerialNo INT
)
RETURNS @tCoreValidVehicleData TABLE 
        (
            TranDate          NVARCHAR(20) NULL
           ,DeviceSerialNo    NVARCHAR(20) NULL
           ,OperatorID        NVARCHAR(20) NULL
           ,VoiceAppId        NVARCHAR(10) NULL
...
SET @c_Parms = REPLACE(SUBSTRING(@c_Data, @n_StartPos + 1, (@n_EndPos - @n_StartPos) -1) ,'''','') 
   ...
   BEGIN
       ...
       IF @n_SeqNo = 4  SET @c_VoiceAppId      = @c_ColValue     
             
             
  FETCH NEXT FROM CUR1 INTO @n_SeqNo, @c_ColValue
   END
   INSERT INTO @tCoreValidVehicleData
     (
       TranDate
      ,DeviceSerialNo
      ,OperatorID
