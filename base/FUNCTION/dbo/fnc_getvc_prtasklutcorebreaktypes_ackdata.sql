...
GO
CREATE FUNCTION [dbo].[fnc_GetVC_prTaskLUTCoreBreakTypes_AckData]
(
   @nSerialNo INT
)
RETURNS @tBreakType TABLE 
        (
            [BreakType] NVARCHAR(1) NULL
           ,[BreakTypeDesc] NVARCHAR(255) NULL
...
BEGIN
   DECLARE @c_AckData NVARCHAR(4000)
          ,@c_BreakType NVARCHAR(1)
          ,@c_BreakTypeDesc NVARCHAR(1)
   ...
   BEGIN
       ...
       IF @n_SeqNo = 1
           SET @c_BreakType = @c_ColValue
       
       IF @n_SeqNo = 2
           SET @c_BreakTypeDesc = @c_ColValue
