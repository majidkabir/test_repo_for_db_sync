...
GO
/***************************************************************************************/  
/* Trigger:  ntrRdtPrinterDelete                                                       */  
/* Creation Date:                                                                      */  
/* Copyright: IDS                                                                      */  
/* Written by:                                                                         */  
/*                                                                                     */  
/* Purpose:  Trigger point upon any Delete on the RdtPrinter                           */  
/*                                                                                     */  
/* Return Status:  None                                                                */  
/*                                                                                     */  
/* Usage:                                                                              */  
...
CREATE   TRIGGER [RDT].[ntrRdtPrinterDelete]  
 ON [RDT].[RDTPrinter]  
 FOR DELETE  
 AS  
 BEGIN  
   ...
   BEGIN        
         ...
         SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Delete Failed On Table RdtPrinter. Batch Delete not allow! (ntrRdtPrinterDelete)" + " ( " + " SQLSvr MESSAGE=" + LTRIM(RTRIM(@c_errmsg)) + " ) "  
   END  
