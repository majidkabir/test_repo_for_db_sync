...
/************************************************************************/
/* Trigger: ntrTaskManagerUserDetailUpdate                              */
/* Creation Date:  25-May-2012                                          */
/* Copyright: IDS                                                       */
/* Written by:  TLTING                                                  */
/*                                                                      */
/* Purpose: TaskManagerUserDetail Update Transaction                    */
/*                                                                      */
/* Input Parameters:                                                    */
/*                                                                      */
/* Output Parameters:                                                   */
...
BEGIN
   ...
   -- TLTING01
	...
	BEGIN
		...
		AND  TaskManagerUserDetail.UserLineNumber = INSERTED.UserLineNumber
		...
		BEGIN
			...
			SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": UPDATE Failed on TaskManagerUserDetail table. (ntrTaskManagerUserDetailUpdate)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
		END
	END
   ...
   END   
	...
	BEGIN
		...
		BEGIN
			ROLLBACK TRAN
		END
		execute nsp_logerror @n_err, @c_errmsg, "ntrTaskManagerUserDetailUpdate"
