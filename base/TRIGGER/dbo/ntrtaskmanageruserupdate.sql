...
/************************************************************************/
/* Trigger: ntrTaskManagerUserUpdate                                    */
/* Creation Date:  25-May-2012                                          */
/* Copyright: IDS                                                       */
/* Written by:  TLTING                                                  */
/*                                                                      */
/* Purpose: TaskManagerUser Update Transaction                          */
/*                                                                      */
/* Input Parameters:                                                    */
/*                                                                      */
/* Output Parameters:                                                   */
   ...
   -- TLTING01
	...
	BEGIN
		...
		BEGIN
			...
			SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": UPDATE Failed on TaskManagerUser table. (ntrTaskManagerUserUpdate)" + " ( " + " SQLSvr MESSAGE=" + dbo.fnc_LTrim(dbo.fnc_RTrim(@c_errmsg)) + " ) "
		END
	END
   ...
   END   
	...
	BEGIN
		...
		BEGIN
			ROLLBACK TRAN
		END
		execute nsp_logerror @n_err, @c_errmsg, "ntrTaskManagerUserUpdate"
