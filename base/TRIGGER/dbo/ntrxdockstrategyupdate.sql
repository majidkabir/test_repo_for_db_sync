...
/************************************************************************/
/* Trigger: ntrXDOCKStrategyUpdate                                      */
/* Creation Date:  18-March-2020                                        */
/* Copyright: IDS                                                       */
/* Written by:  TLTING                                                  */
/*                                                                      */
/* Purpose: XDOCKStrategy Update                                        */
/*                                                                      */
/* Input Parameters:                                                    */
/*                                                                      */
/* Output Parameters:                                                   */
   ...
   -- TLTING01
	...
	BEGIN
		...
		BEGIN
			...
			SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": UPDATE Failed on XDOCKStrategy table. (ntrXDOCKStrategyUpdate)" + " ( " + " SQLSvr MESSAGE=" + LTrim(RTrim(@c_errmsg)) + " ) "
		END
	END
   ...
   END   
	...
	BEGIN
		...
		BEGIN
			ROLLBACK TRAN
		END
		EXECUTE nsp_logerror @n_err, @c_errmsg, "ntrXDOCKStrategyUpdate"
