...
GO
/***************************************************************************************/
/* Trigger:  ntrNSCLogDelete                                                           */
/* Creation Date:                                                                      */
/* Copyright: IDS                                                                      */
/* Written by:                                                                         */
/*                                                                                     */
/* Purpose:  Trigger point upon any Delete on the NSCLog                               */
/*                                                                                     */
/* Return Status:  None                                                                */
/*                                                                                     */
/* Usage:                                                                              */
...
CREATE   TRIGGER [dbo].[ntrNSCLogDelete]
 ON [dbo].[NSCLOG]
 FOR DELETE
 AS
 BEGIN
   ...
   BEGIN      
         ...
         SELECT @c_errmsg="NSQL"+CONVERT(char(5),@n_err)+": Delete Failed On Table NSCLog. Batch Delete not allow! (ntrNSCLogDelete)" + " ( " + " SQLSvr MESSAGE=" + LTRIM(RTRIM(@c_errmsg)) + " ) "
   END
   ...
   BEGIN
      ...
      BEGIN
         ...
         END
      END
      EXECUTE nsp_logerror @n_err, @c_errmsg, "ntrNSCLogDelete"
