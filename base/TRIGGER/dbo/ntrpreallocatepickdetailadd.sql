...
/* Written by:                                                         */  
/*                                                                     */  
/* Purpose:  Trigger point upon any Add on the PreAllocatePickDetail   */  
/* Data Modifications:                                                 */  
/*                                                                     */  
/* Updates:                                                            */  
...
CREATE    TRIGGER [dbo].[ntrPreAllocatePickDetailAdd]    
 ...
 BEGIN    
   ...
   BEGIN    
      UPDATE OrderDetail   
      SET OrderDetail.QtyPreAllocated = OrderDetail.QtyPreAllocated +   
            ( SELECT SUM(inserted.Qty )   
              FROM inserted  WITH (NOLOCK)   
              WHERE inserted.orderkey =OrderDetail.OrderKey and inserted.OrderLineNumber = OrderDetail.OrderLineNumber   
            ),    
   ...
   BEGIN    
      UPDATE LOT   
      SET LOT.QtyPreAllocated = LOT.QtyPreAllocated +   
             ( SELECT SUM(inserted.Qty )   
               FROM inserted WITH (NOLOCK)   
